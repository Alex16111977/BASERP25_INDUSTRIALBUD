...
Процедура СоответствиеСвойствСпособОпределенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.СпособыПодбораЗначенияСвойстваНоменклатуры.ПоСвойству"), НСтр("ru='по свойству';uk='за властивістю'"));
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.СпособыПодбораЗначенияСвойстваНоменклатуры.ПоЗначению"), НСтр("ru='по значению';uk='за значенням'"));
	
	Если ТекущиеДанные.ЕстьВозможностьРасчетаПоФормуле Тогда
		СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.СпособыПодбораЗначенияСвойстваНоменклатуры.ПоФормуле"), НСтр("ru='по формуле';uk='за формулою'"));
	КонецЕсли;
	
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.СпособыПодбораЗначенияСвойстваНоменклатуры.ПоАлгоритму"), НСтр("ru='по алгоритму';uk='за алгоритмом'"));
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.СпособыПодбораЗначенияСвойстваНоменклатуры.ЛюбоеЗначение"), НСтр("ru='любое значение';uk='будь-яке значення'"));
...
Процедура СформироватьДанныеДляПодбораПоФормуле(СвойстваПродукции)
	...
	ГруппаПродукция = РаботаСФормулами.ДобавитьОперанд(ДеревоОперандов, "", НСтр("ru='Свойства продукции';uk='Властивості продукції'"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<нет алгоритмов доступных для выбора>';uk='<немає алгоритмів доступних для вибору>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийТекст);
...
Процедура ЗавершитьРедактирование()
	...
	ШаблонСообщения = НСтр("ru='Необходимо заполнить значение настройки соответствия в строке %1';uk='Необхідно заповнити значення настройки відповідності в рядку %1'");
