...
Процедура ПродолжитьВыполнениеСинхронизации(Результат, ДополнительныеПараметры) Экспорт
	...
	// А_: если для узла установлена галочка "Синхронизировать без помощника" —
	// выполняем обмен штатным тихим путём, помощник интерактивного обмена не открывается.
	Если А_СинхронизироватьБезПомощника(ДополнительныеПараметры.УзелОбмена) Тогда
...
// А_: возвращает Истина, если для узла установлена галочка "Синхронизировать без помощника".
// Обобщённо: реквизит ищется в МЕТАДАННЫХ узла, поэтому фича работает для ЛЮБОГО плана обмена,
// в который он добавлен (сейчас — ОбменУправлениеПредприятиемБухгалтерия20 и КазначествоBASERP);
// новый план подключается одним лишь добавлением реквизита, без правки этого кода.
// Для планов без реквизита возвращается Ложь — штатное поведение сохраняется.
&НаСервереБезКонтекста
Функция А_СинхронизироватьБезПомощника(Узел)

	Если НЕ ЗначениеЗаполнено(Узел) Тогда
		Возврат Ложь;
	КонецЕсли;

	Если Узел.Метаданные().Реквизиты.Найти("А_СинхронизироватьБезПомощника") = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Узел, "А_СинхронизироватьБезПомощника") = Истина;
...
#Область А_ПолнаяСинхронизация

// Полный обмен по текущей строке списка: <корреспондент> -> ERP -> <корреспондент>.
// Открывает встроенную обработку А_АвтоматическийОбменДанными и передаёт узел текущей
// строки в реквизит ПланОбмена — обмен стартует автоматически. Все параметры
// (сервер, база, учётка, путь к обработке-исполнителю) берутся из регистра
// А_НастройкиПодключенияБаз.
&НаКлиенте
Процедура А_ПолныйОбменПоТекущей(Команда)
	...
	ОткрытьФорму("Обработка.А_АвтоматическийОбменДанными.Форма",
		Новый Структура("ПланОбмена", ТекущиеДанные.УзелИнформационнойБазы));

КонецПроцедуры

// Полный обмен по всем базам с заполненным планом обмена в регистре
// А_НастройкиПодключенияБаз (порядок — по идентификатору базы: bas_industrialbud, kazna).
&НаКлиенте
Процедура А_ПолныйОбменПоВсем(Команда)

	ОткрытьФорму("Обработка.А_АвтоматическийОбменДанными.Форма",
		Новый Структура("ЗапуститьВсе", Истина));
