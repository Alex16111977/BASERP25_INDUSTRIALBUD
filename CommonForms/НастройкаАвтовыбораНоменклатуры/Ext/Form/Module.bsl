...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Элементы.АлгоритмАвтовыбораХарактеристики.ПодсказкаВвода = НСтр("ru='<нет алгоритмов доступных для выбора>';uk='<немає алгоритмів доступних для вибору>'");
	КонецЕсли;
...
Процедура КомандаСохранитьНастройку(Команда)
	...
	Если СпособАвтовыбораХарактеристики = ПредопределенноеЗначение("Перечисление.СпособыАвтовыбораХарактеристики.ПодбираетсяПоСвойствамПродукции") Тогда
		...
		ПараметрыФормы.Вставить("ХарактеристикаСтрокой", НСтр("ru='<определяется по свойствам основного изделия>';uk='<визначається за властивостями основного виробу>'"));
	...
	ИначеЕсли СпособАвтовыбораХарактеристики = ПредопределенноеЗначение("Перечисление.СпособыАвтовыбораХарактеристики.ПодбираетсяПоАлгоритму") Тогда
		ПараметрыФормы.Вставить("Характеристика", Неопределено);
		ПараметрыФормы.Вставить("ХарактеристикаСтрокой", НСтр("ru='<определяется по алгоритму>';uk='<визначається за алгоритмом>'"));
...
Процедура УправлениеДоступностью(Форма)
	...
	Если Форма.СпособАвтовыбораНоменклатуры = ПредопределенноеЗначение("Перечисление.СпособыАвтовыбораНоменклатуры.УказываетсяВНСИ")
		...
		ЗначениеВыбора.Представление = НСтр("ru='Определяется по свойствам основного изделия (необходимо указать материал)';uk='Визначається за властивостями основного виробу (необхідно вказати матеріал)'");
		
		ЗначениеВыбора = Форма.Элементы.ПодбираетсяПоАлгоритмуНеДоступна.СписокВыбора[0];
		ЗначениеВыбора.Представление = НСтр("ru='Определяется по алгоритму (необходимо указать материал)';uk='Визначається за алгоритмом (необхідно вказати матеріал)'");
	...
	ИначеЕсли Форма.СпособАвтовыбораНоменклатуры = ПредопределенноеЗначение("Перечисление.СпособыАвтовыбораНоменклатуры.УточняетсяПриПроизводстве") Тогда
		...
		ЗначениеВыбора = Форма.Элементы.ПодбираетсяПоСвойствамПродукцииНеДоступна.СписокВыбора[0];
		ЗначениеВыбора.Представление = НСтр("ru='Определяется по свойствам основного изделия (недоступно, т.к. материал уточняется в производстве)';uk='Визначається за властивостями основного виробу (недоступно, тому що матеріал уточнюється у виробництві)'");
		
		ЗначениеВыбора = Форма.Элементы.ПодбираетсяПоАлгоритмуНеДоступна.СписокВыбора[0];
		ЗначениеВыбора.Представление = НСтр("ru='Определяется по алгоритму (недоступно, т.к. материал уточняется в производстве)';uk='Визначається за алгоритмом (недоступно, тому що матеріал уточнюється у виробництві)'");
...
Функция ПроверитьЗаполнениеРеквизитов()
	...
	Если СпособАвтовыбораНоменклатуры = ПредопределенноеЗначение("Перечисление.СпособыАвтовыбораНоменклатуры.ЗадаетсяВСвойствеПродукции") 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбрано свойство, содержащее номенклатуру.';uk='Не вибрано властивість, що містить номенклатуру.'"),,
	...
	Если СпособАвтовыбораХарактеристики = ПредопределенноеЗначение("Перечисление.СпособыАвтовыбораХарактеристики.ПодбираетсяПоАлгоритму")
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран алгоритм, по которому определяется характеристика.';uk='Не вибрано алгоритм, за яким визначається характеристика.'"),,
