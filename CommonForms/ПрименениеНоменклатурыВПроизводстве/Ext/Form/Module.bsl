...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("РежимСпецификацииИзделия") Тогда
		Заголовок = СтрШаблон(НСтр("ru='Спецификации изделия %1';uk='Специфікації виробу %1'"), РеквизитыНоменклатуры.Наименование);
	...
	Иначе
		Заголовок = СтрШаблон(НСтр("ru='Производство (Номенклатура: ""%1"")';uk='Виробництво (Номенклатура: ""%1"")'"), РеквизитыНоменклатуры.Наименование);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоУправлениеПроизводством22") Тогда
		Элементы.СтраницаПлановыеКалькуляции22.Заголовок = НСтр("ru='Плановые калькуляции';uk='Планові калькуляції'");
	//++ Устарело_Производство21
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоУправлениеПроизводством21") Тогда
		Элементы.СтраницаПлановыеКалькуляции.Заголовок = НСтр("ru='Плановые калькуляции';uk='Планові калькуляції'");
	//-- Устарело_Производство21
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ОснованиеНоменклатура) Тогда
		...
		ТекстПредупреждения = НСтр("ru='Форма ""Производство"" используется только в контексте номенклатуры.';uk='Форма ""Виробництво"" використовується тільки в контексті номенклатури.'");
...
Процедура ЗаполнитьУправлениеСпецификациями(ТекущаяСпецификацияХарактеристика = Неопределено)
	...
	РеквизитыСпецификации = Новый Массив;
	РеквизитыСпецификации.Добавить("ТипПроизводственногоПроцесса");
	РеквизитыСпецификации.Добавить("ОсновноеИзделиеНоменклатура");
	РеквизитыСпецификации.Добавить("ОсновноеИзделиеХарактеристика");
	СписокПараметрыВыбораСпецификаций.РеквизитыСпецификации = СтрСоединить(РеквизитыСпецификации, ",");
	
	ИгнорируемыеПараметрыНазначения = Новый Массив;
	Если НЕ ЗначениеЗаполнено(ПараметрНазначенияНаправлениеДеятельности) Тогда
		ИгнорируемыеПараметрыНазначения.Добавить(Перечисления.ВидыПараметровНазначенияСпецификаций.НаправлениеДеятельности);
	КонецЕсли;
	...
	КонецЕсли;
	СписокПараметрыВыбораСпецификаций.Вставить("ИгнорируемыеПараметрыНазначения", ИгнорируемыеПараметрыНазначения);

	ИгнорируемыеУточненияПрименения = Новый Массив;
	Если НЕ ЗначениеЗаполнено(ОснованиеХарактеристика) Тогда
		ИгнорируемыеУточненияПрименения.Добавить(УправлениеДаннымиОбИзделияхКлиентСервер.ОтборПоСвойствамКлючСвязиПредопределенный("Характеристика"));
	КонецЕсли;
	СписокПараметрыВыбораСпецификаций.Вставить("ИгнорируемыеУточненияПрименения", ИгнорируемыеУточненияПрименения);
	
	ПараметрыЗапросаСпецификаций = УправлениеДаннымиОбИзделиях.ПараметрыТекстаЗапросаСпецификацийНоменклатуры();
	ПараметрыЗапросаСпецификаций.ИмяТаблицыИсточникДоступныхСпецификаций = "СпецификацииИХарактеристикиБезОтбора";
	...
	|	ТабличнаяЧасть.ТипПроизводственногоПроцесса,
	|	ТабличнаяЧасть.Статус,
	|	ЛОЖЬ,
	|
	|	ТабличнаяЧасть.ОсновноеИзделиеНоменклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка),
	|	ТабличнаяЧасть.ОсновноеИзделиеХарактеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка),
	|
	|	ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораНоменклатуры.УказываетсяВНСИ),
	|	ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораХарактеристики.УказываетсяВНСИ),
	|	ТабличнаяЧасть.ОсновноеИзделиеНоменклатура,
	|	ТабличнаяЧасть.ОсновноеИзделиеХарактеристика
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|
	|" + УправлениеДаннымиОбИзделиях.ТекстЗапросаСпецификацийНоменклатуры(ПараметрыЗапросаСпецификаций, СписокПараметрыВыбораСпецификаций);
	
	УправлениеДаннымиОбИзделиях.УстановитьПараметрыЗапросаСпецификацийНоменклатуры(Запрос, СписокПараметрыВыбораСпецификаций);
	
	СоответствуютПараметрамНазначения = Запрос.Выполнить().Выгрузить();
	
	Запрос.УстановитьПараметр("СоответствуютПараметрамНазначения", СоответствуютПараметрамНазначения);
	
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	СоответствуютПараметрамНазначения.Спецификация КАК Спецификация
	|ПОМЕСТИТЬ СоответствуютПараметрамНазначения
	|ИЗ
	|	&СоответствуютПараметрамНазначения КАК СоответствуютПараметрамНазначения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	...
	|			(ВЫБРАТЬ
	|				СоответствуютПараметрамНазначения.Спецификация
	|			ИЗ
	|				СоответствуютПараметрамНазначения)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	...
	|	СпецификацииИХарактеристикиБезОтбора.ХарактеристикаСпецификации     КАК Характеристика,
	|	СпецификацииИХарактеристикиБезОтбора.СпособАвтовыбораХарактеристики КАК СпособАвтовыбораХарактеристики
	|ПОМЕСТИТЬ СпецификацииИХарактеристики
	|ИЗ
	|	СпецификацииИХарактеристикиБезОтбора КАК СпецификацииИХарактеристикиБезОтбора
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СоответствуютПараметрамНазначения КАК СоответствуютПараметрамНазначения
	|		ПО (СпецификацииИХарактеристикиБезОтбора.Спецификация = СоответствуютПараметрамНазначения.Спецификация)
	...
	|	СпецификацииИХарактеристики.СпособАвтовыбораХарактеристики КАК СпособАвтовыбораХарактеристики,
	...
	ТаблицаСпецификаций               = Результат[Результат.Количество()-2].Выгрузить();
	ТаблицаХарактеристикИСпецификаций = Результат[Результат.Количество()-1].Выгрузить();
	
	ТаблицаСпецификаций.Индексы.Добавить("Спецификация");
	...
	Для Индекс = 0 По СоответствуютПараметрамНазначения.Количество()-1 Цикл
		Строка = СоответствуютПараметрамНазначения[Индекс];
...
Процедура УстановитьВидимостьГруппыПараметровНазначенияСпецификаций()
	...
	Для каждого ИмяПараметра Из СтрРазделить("НаправлениеДеятельности,ПодразделениеДиспетчер", ",") Цикл
		...
		Если УправлениеДаннымиОбИзделияхПовтИсп.ПараметрНазначенияИспользуется(
			...
			Элементы["ПараметрНазначения"+ИмяПараметра].Видимость       = Истина;
			Элементы["СписокПараметрНазначения"+ИмяПараметра].Видимость = Истина;
