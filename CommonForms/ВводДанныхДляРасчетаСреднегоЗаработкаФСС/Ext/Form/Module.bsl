...
Процедура ПрименитьОграниченияДляНачисленийПриПересчете(ДанныеОНачислениях)
	...
	Если Не ДанныеОНачислениях = Неопределено Тогда 
		Для Каждого СтрокаКоллекции Из ДанныеОНачислениях Цикл
			...
			Если Не ПараметрыРасчетаСреднегоДневногоЗаработкаФСС.ПредельныеВеличиныПоМесяцам.Получить(СтрокаКоллекции.Период) = Неопределено И СтрокаКоллекции.Предел = 0 Тогда
				СуммаПредела = ПараметрыРасчетаСреднегоДневногоЗаработкаФСС.ПредельныеВеличиныПоМесяцам[СтрокаКоллекции.Период];
			ИначеЕсли Не СтрокаКоллекции.Предел = 0 Тогда 
				СуммаПредела = СтрокаКоллекции.Предел
			КонецЕсли;	
...
Процедура ОбновитьПредельныеВеличиныБазыСреднегоЗаработка()
	...
	ПредельнаяВеличина = УчетПособийСоциальногоСтрахования.ПредельнаяВеличинаБазыСтраховыхВзносовПоФизическомуЛицу(Параметры.Сотрудник, МесяцыПредельныхВеличин, Параметры.Сотрудник.ФизическоеЛицо, Параметры.Организация, Параметры.ДатаНачалаСобытия);       
...
Процедура ДополнитьПредположительно()
	...
	Запрос.Текст = 
		...
		|	СУММА(ВЫБОР
		|			КОГДА ПлановыеНачисления.Размер ЕСТЬ NULL 
		|				ТОГДА ПлановыйФОТ.ВкладВФОТ
		...
		|	И ПлановыеНачисления.Используется = ИСТИНА    
		|СГРУППИРОВАТЬ ПО
