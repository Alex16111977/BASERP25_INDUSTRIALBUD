...
Функция ПолучитьПараметрыПисьмаПоШаблону(ВидОповещения, Параметры, КодЯзыкаИсполнителя) Экспорт
	...
	Если ВидОповещения = Перечисления.ВидыОповещенийБюджетныхЗадач.ЗадачаПоступила Тогда
		Результат.Тема = НСтр("ru='Поступила новая задача';uk='Надійшла нова задача'",КодЯзыкаИсполнителя);
	...
	ИначеЕсли ВидОповещения = Перечисления.ВидыОповещенийБюджетныхЗадач.ЗадачаПодходитСрок Тогда
		Результат.Тема = НСтр("ru='Подходит к концу срок исполнения задачи';uk='Добігає кінця термін виконання задачі'",КодЯзыкаИсполнителя);
	...
	ИначеЕсли ВидОповещения = Перечисления.ВидыОповещенийБюджетныхЗадач.ЗадачаПросрочена Тогда
		Результат.Тема = НСтр("ru='Просрочено исполнение задачи';uk='Прострочено виконання задачі'",КодЯзыкаИсполнителя);
...
Процедура ДобавитьТелоПисьма(ВидОповещения, Параметры, Результат, КодЯзыкаИсполнителя)
	...
	Параметры.Вставить("Обращение", НСтр("ru='Здравствуйте';uk='Доброго дня'",КодЯзыкаИсполнителя));
	Если Реквизиты.Пол = Перечисления.ПолФизическогоЛица.Мужской Тогда
		Параметры.Обращение = НСтр("ru='Уважаемый';uk='Шановний'",КодЯзыкаИсполнителя);
		
	ИначеЕсли Реквизиты.Пол = Перечисления.ПолФизическогоЛица.Женский Тогда
		Параметры.Обращение = НСтр("ru='Уважаемая';uk='Шановна'",КодЯзыкаИсполнителя);
	...
	Если НЕ ПустаяСтрока(АдресПубликацииИнформационнойБазыВЛокальнойСети) Тогда
		...
		СсылкаНаЗадачу = НСтр("ru='Для просмотра задачи перейдите по ссылке: <a href=""%1"">%2</a>';uk='Для перегляду задачі перейдіть за посиланням: <a href=""%1""> %2</a>'",КодЯзыкаИсполнителя);
		СсылкаНаЗадачу = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СсылкаНаЗадачу, АдресСсылки, АдресСсылки);
		
	Иначе
		ИмяЯрлыкаИБ = НСтр("ru='Запустить информационную базу';uk='Запустити інформаційну базу'",КодЯзыкаИсполнителя);
		Результат.Вставить("ИмяЯрлыкаИБ", ИмяЯрлыкаИБ);
		СсылкаНаЗадачу = 
			НСтр("ru='Для того чтобы открыть информационную базу Вы можете воспользоваться вложенным файлом ""%1"".';uk='Для того, щоб відкрити інформаційну базу Ви можете скористатися вкладеним файлом ""%1"".'",КодЯзыкаИсполнителя);
...
Функция ЗаполнитьШаблонЗначениямиПодстановки(ШаблонТекста, ЗначенияПодстановки, КодЯзыкаИсполнителя)
	...
	КодФорматнойСтроки = НСтр("ru='Л=ru_RU; ДЛФ=DD';uk='Л=uk_UA; ДЛФ=DD'",КодЯзыкаИсполнителя);
