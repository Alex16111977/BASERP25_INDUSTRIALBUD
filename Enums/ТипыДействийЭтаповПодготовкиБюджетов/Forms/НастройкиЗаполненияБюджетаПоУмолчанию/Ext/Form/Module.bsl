...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ВидБюджета", ВидБюджета)
	 ...
	 ИЛИ НЕ Параметры.Свойство("КлючСтроки", КлючСтроки) Тогда
		ТекстСообщения = НСтр("ru='Непосредственное открытие этой формы не предусмотрено. Для открытия формы можно воспользоваться кнопкой открытия значения в колонке ""Настройки измерений по умолчанию"" в форме настроек ввода бюджетов.';uk='Безпосереднє відкриття цієї форми не передбачено. Для відкриття форми можна скористатися кнопкою відкриття значення в колонці ""Настройки вимірів по умовчанню"" у формі настройок введення бюджетів.'");
		...
		Если Стр.КлючСтроки = КлючСтроки Тогда 		
			Если Не ЗначениеЗаполнено(Стр.Сценарий) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В строке %1 не заполнен сценарий';uk='У рядку %1 не заповнений сценарій'"),
					Сч+1);
			...
			Если ФормироватьБюджетыПоОрганизациям И Не ЗначениеЗаполнено(Стр.Организация) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В строке %1 не заполнена организация';uk='У рядку %1 не заповнена організація'"),
					Сч+1);
				ПутьКДанным = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Если ФормироватьБюджетыПоПодразделениям И Не ЗначениеЗаполнено(Стр.Подразделение) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В строке %1 не заполнено подразделение';uk='У рядку %1 не заповнено підрозділ'"),
					Сч+1);
				ПутьКДанным = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
