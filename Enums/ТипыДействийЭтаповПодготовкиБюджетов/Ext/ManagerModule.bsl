...
//
// Параметры:
//  ТипДействия - ПеречислениеСсылка.ТипыДействийШаговБюджетныхПроцессов - Действие, для которого определяется форма настройки.
//
...
Функция ПолучитьПредставлениеДействия(Знач НастройкиДействия) Экспорт
	...
	Если ЗначениеЗаполнено(ВидыБюджетов) Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
				НСтр("ru='Вид бюджета';uk='Вид бюджету'"),
		...
		Если Строки.Количество() > 0
		   ...
		   И Строки[0].Значение.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(ТекстАналитикиДляРедактирования) Тогда
				...
				СоединяемыеСтроки.Добавить(НСтр("ru='Разрешено для редактирования:';uk='Дозволено для редагування:'"));
		...
		Если КоличествоНастроек > 1 Тогда
			...
			НСтр("ru='и еще видов бюджетов (%1)';uk='і ще видів бюджетів (%1)'"),КоличествоНастроек - 1);
		КонецЕсли;
