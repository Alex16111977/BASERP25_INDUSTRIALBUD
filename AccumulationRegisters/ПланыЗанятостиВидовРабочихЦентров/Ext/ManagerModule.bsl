...
|;uk='Змінює реєстратор рухів - замість документа ""План виробництва"" встановлюється ""Реєстратор плану виробництва"". 
|Заповнює вимір ""Специфікація продукції"" і ""Призначення продукції"" 
|Скасовує рухи в статусі ""Скасовано"".'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ВЫБОР 
		|		КОГДА ПланыЗанятостиВидовРабочихЦентров.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.ПустаяСсылка) 
		|			ТОГДА ДокументПланПроизводства.Статус 
		|		ИНАЧЕ ПланыЗанятостиВидовРабочихЦентров.Статус 
		|	КОНЕЦ КАК Статус,
		...
		|		ПО ПланыЗанятостиВидовРабочихЦентров.Регистратор = ДокументПланПроизводства.Ссылка
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				...
				СтатусРегистратора = Неопределено;
				Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.ПланПроизводства") Тогда
					СтатусРегистратора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Регистратор, "Статус");
				КонецЕсли;					
			
				Если ЗначениеЗаполнено(СтатусРегистратора) Тогда
					...
					Для Каждого Запись Из НаборЗаписейСтарый Цикл
						
						Если НЕ ЗначениеЗаполнено(Запись.Статус) Тогда
							Запись.Статус = СтатусРегистратора;
						КонецЕсли;	
						
					КонецЦикла; 
					
					НаборЗаписейСтарый.УстановитьАктивность(Истина);
