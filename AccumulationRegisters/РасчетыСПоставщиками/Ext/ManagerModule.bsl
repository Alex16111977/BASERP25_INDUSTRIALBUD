...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры = Неопределено) Экспорт
	...
	|		И РасчетыСПоставщиками.Сумма <> 0
	//++ НЕ УТКА
	//Отчеты переработчиков с пустой датой платежа
	|	ИЛИ РасчетыСПоставщиками.Регистратор ССЫЛКА Документ.ОтчетПереработчика
	|		И РасчетыСПоставщиками.ДатаПлатежа = ДАТАВРЕМЯ(1,1,1)
	//-- НЕ УТКА
	|)
