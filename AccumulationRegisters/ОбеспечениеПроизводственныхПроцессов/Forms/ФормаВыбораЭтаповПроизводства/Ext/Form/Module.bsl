...
Процедура НастроитьОтборПоПолучателю()
	...
	Если ОтборПоПолучателю Тогда
		Если ТипЗнч(Параметры.Получатель) = Тип("СправочникСсылка.Склады") Тогда
			Шаблон = НСтр("ru='Потребность на складе: ""%1""';uk='Потреба на складі: ""%1""'");
		Иначе
			Шаблон = НСтр("ru='Потребность в подразделении: ""%1""';uk='Потреба в підрозділі: ""%1""'");
		КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	ТекстНоменклатура = СтрШаблон(НСтр("ru='Номенклатура: %1';uk='Номенклатура: %1'"), Параметры.Номенклатура);
	
	Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
		
		ТекстНоменклатура = ТекстНоменклатура + ", " + СтрШаблон(НСтр("ru='Характеристика: %1';uk='Характеристика: %1'"), Параметры.Характеристика);
		
	КонецЕсли;
	
	Заголовок = СтрШаблон(НСтр("ru='Выбор этапа потребителя полуфабриката (%1)';uk='Вибір етапу споживача напівфабрикату (%1)'"), ТекстНоменклатура);
