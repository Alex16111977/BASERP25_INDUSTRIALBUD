...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Не РедактированиеБюджетныхЗадачВнеПроцессов И Не ЗначениеЗаполнено(Объект.ОсновнаяЗадача) Тогда
			ВызватьИсключение НСтр("ru='В рамках бюджетного процесса разрешено создавать только связанные задачи.';uk='В рамках бюджетного процесу дозволено створювати тільки пов''язані задачі.'");
		КонецЕсли;
...
Процедура ДеревоДокументовВыбор(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ДеревоДокументовДокументПредставление" Тогда
		...
		Иначе
			Если ТекущиеДанные.ДокументПредставление = НСтр("ru='Создать';uk='Створити'") Тогда
				...
				Если МассивДокументы <> Неопределено И МассивДокументы.Количество() > 0 Тогда
					...
					СписокКнопок.Добавить(КодВозвратаДиалога.Да, ?(МассивДокументы.Количество() = 1,
						НСтр("ru='Выбрать найденный';uk='Вибрати знайдений'"), НСтр("ru='Выбрать из найденных';uk='Вибрати зі знайдених'")));
					...
					Если МассивДокументы.Количество() = 1 Тогда
						ТекстВопроса = НСтр("ru='Найден подходящий экземпляр бюджета.';uk='Знайдено відповідний примірник бюджету.'");
					Иначе
						ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Найдено подходящих экземпляров бюджета (%1).';uk='Знайдено відповідних примірників бюджету (%1).'"), МассивДокументы.Количество());
					КонецЕсли;
...
|;uk='Не вдалося записати задачу. 
|Документи по задачі можуть бути не доступні для зміни'");
			КонецПопытки;
...
Процедура ВидКонтрольногоОтчетаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоказатьВыборИзСписка(ОписаниеОповещения,
		СписокВыбора,
...
Процедура СформироватьАвтоматически(Команда)
	...
	Если МассивДокументы <> Неопределено И МассивДокументы.Количество() > 0 Тогда
		СписокКнопок = Новый СписокЗначений;
		ЗаголовокКнопки = НСтр("ru='Использовать найденные';uk='Використовувати знайдені'");
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, ЗаголовокКнопки);
		ЗаголовокКнопки = НСтр("ru='Создавать новые';uk='Створювати нові'");
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, ЗаголовокКнопки);
		
		ШаблонСтроки = НСтр("ru='Найдено подходящих экземпляров бюджета (%1).';uk='Знайдено відповідних примірників бюджету (%1).'");
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Иначе
		...
		Иначе
			ТекстОшибки = НСтр("ru='Создание новых документов не требуется, все экземпляры бюджета уже созданы.';uk='Створення нових документів не потрібно, всі примірники бюджету вже створені.'");
...
Процедура ОчиститьБюджет(Команда)
	...
	Для Каждого Стр Из ВыбранныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Документ) Тогда
			...
			ТекущиеДанные.ДокументПредставление = НСтр("ru='Создать';uk='Створити'");
		КонецЕсли;
	КонецЦикла;
	
	Если НЕ ВидимостьСписка Тогда
		Элементы.ВыполнитьДействиеВводБюджетов.Заголовок = НСтр("ru='<Ввести бюджет>';uk='<Ввести бюджет>'");
...
|Документи по задачі можуть бути не доступні для зміни'");
		...
		ИначеЕсли Объект.СписокДокументов.Количество() = 1 Тогда
			ТекстВвестиБюджет = НСтр("ru='<Ввести бюджет>';uk='<Ввести бюджет>'");
...
Процедура ПроверитьВыполнениеСозданияДокументовКлиент()
	...
	Если НЕ РезультатПроверки.Выполнено Тогда
		...
		ТекстСообщения = НСтр("ru='Идет формирование документов';uk='Йде формування документів'");
...
Функция ЧтениеИБлокировкаЗадачи()
	...
	Исключение
		...
		ТекстОшибкиПодробный = НСтр("ru='Не удалось записать задачу по причине: %1';uk='Не вдалося записати задачу з причини: %1'",КодОсновногоЯзыка);
		...
		ИмяСобытия = НСтр("ru='БюджетныйПроцесс.СозданиеДокументовДляЗадачи';uk='БюджетнийПроцесс.СтворенняДокументівДляЗадачі'",КодОсновногоЯзыка);
		ЗаписьЖурналаРегистрации(ИмяСобытия,
			УровеньЖурналаРегистрации.Ошибка,,,
			ТекстОшибкиПодробный);
		
		ТекстОшибкиКраткий = НСтр("ru='Не удалось записать задачу по причине: %1';uk='Не вдалося записати задачу з причини: %1'");
...
Процедура УправлениеФормой()
	...
	Если ВидимостьСписка Тогда
		Элементы.ИнформацияОЗадаче.Заголовок = НСтр("ru='Документы по задаче';uk='Документи за задачею'");
	Иначе
		Элементы.ИнформацияОЗадаче.Заголовок = НСтр("ru='Действие задачи';uk='Дія задачі'");
	КонецЕсли;
...
Процедура УстановитьПредставлениеДействия()
	...
	Если Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводБюджетов Тогда
		Документ = ТекущийДокумент(Объект.СписокДокументов);
		ТекстДействия = НСтр("ru='<Ввести бюджет>';uk='<Ввести бюджет>'");
	...
	ИначеЕсли Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.УстановкаЛимитовРасходаДС Тогда
		Документ = ТекущийДокумент(Объект.СписокДокументов);
		ТекстДействия = НСтр("ru='<Установить лимиты расхода денежных средств>';uk='<Встановити ліміти витрачання грошових коштів>'");
		ПредставлениеДействия =
			?(ЗначениеЗаполнено(Документ), Строка(Документ), ТекстДействия);
			
	ИначеЕсли Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводПлана Тогда
		Документ = ТекущийДокумент(Объект.СписокДокументов);
		ТекстДействия = НСтр("ru='<Ввести план>';uk='<Ввести план>'");
		ПредставлениеДействия = 
			?(ЗначениеЗаполнено(Документ), Строка(Документ), ТекстДействия);
			
	ИначеЕсли Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.УстановкаЗначенийНефинансовыхПоказателей Тогда
		Документ = ТекущийДокумент(Объект.СписокДокументов);
		ТекстДействия = НСтр("ru='<Ввести значения нефинансовых показателей>';uk='<Ввести значення нефінансових показників>'");
		ПредставлениеДействия =
			?(ЗначениеЗаполнено(Документ), Строка(Документ), ТекстДействия);
	...
	ИначеЕсли Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.УтверждениеБюджетов Тогда
		...
		ИначеЕсли ВсегоДокументов = Утверждено + Отменено Тогда
			ПредставлениеДействия = НСтр("ru='Утверждены: %1 %3, отменены: %2 %4';uk='Затверджено: %1 %3, відмінені %2 %4'");
			ПредставлениеДействия = 
...
Функция СклонениеДокументов(ЧислоДокументов)
	...
	ЕдЧислоИменительный = НСтр("ru='документ';uk='документ'");
	ЕдЧислоРодительный  = НСтр("ru='документа';uk='документа'");
	МнЧислоРодительный  = НСтр("ru='документов';uk='документів'");
...
|Документи по задачі можуть бути не доступні для зміни'");
		...
		Если НЕ ВидимостьСписка Тогда
			Элементы.ВыполнитьДействиеВводБюджетов.Заголовок = 
				?(ЗначениеЗаполнено(Результат), Строка(Результат), НСтр("ru='<Ввести бюджет>';uk='<Ввести бюджет>'"));
		КонецЕсли;
	КонецЕсли;
...
Функция НайтиПодходящиеБюджетыДляДереваДокументов(МодельБюджетирования, Знач ДеревоДокументов)
	...
	КомандаСоздать = НСтр("ru='Создать';uk='Створити'");
