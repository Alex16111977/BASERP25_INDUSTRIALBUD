...
| Зміна | Файли документації | Файли коду |
|---|---|---|
| Drill-down колонки Аналитика / Аналитика1 у звіті А_ОтчетPL | `pl_report_architecture_analyst.md` (нова секція) | `Reports/А_ОтчетPL/Ext/ObjectModule.bsl` + `Templates/.../Template.xml` |
| Поле `А_ФизическоеЛицо` заповнюється у `РегистрНакопления.ПрочиеРасходы` для документів ОЗП | `pl_report_architecture_analyst.md` | `Documents/ОтражениеЗарплатыВФинансовомУчете/Ext/ManagerModule.bsl` |
| Масовий перепровод ОЗП 2024-Q1-2026 (5 документів, 4067/4067 рухів отримали ФЛ) | — | `_Rarzrabotki/Python/scripts/_reprovesti_ozp.py` |
| Регресія `СУММА(Приход−Расход)` ламала «Себестоимость» для підрозділів з перерозподілом — повернуто до `Σ Приход` | `pl_report_architecture_analyst.md` (Грабелька 2026-05-21) | `Reports/А_ОтчетPL/Ext/ObjectModule.bsl` |
...
| FAQ | 1 | pl_faq.md (5 актуальних місяців: Грудень 2025 – Квітень 2026, 223 КБ) |
| Помісячні dumps 2024 | 72 | 12 місяців × 6 buckets (summary/income/cost/opex/marketing/cash) |
| Помісячні dumps 2025 | 72 | 12 місяців × 6 buckets |
| Помісячні dumps 2026 Q1 | 24 | 4 місяці × 6 buckets (Січень 2026 – Квітень 2026) |
| Delta-файли | 27 | Між послідовними місяцями (січ 2024 vs ... → кві 2026 vs бер 2026) |
...
| **Всього файлів на диску** | **~204** | |

**Зміна vs v2:** додано 24 нових місяці dump (2024-01..2025-11), 25 нових delta-файлів. FAQ розширено з 3 на 5 актуальних місяців. У NotebookLM не всі завантажені — більшість на диску як довідник для drill-down (детальні дампи місяців за 2024 не критичні для щоденної RAG-роботи фінансиста, але корисні для аудиту/ретроспективи).

**Дані в 1С на момент upgrade:** 28 А_РасшифровкаЛистов + 843 проведених А_ОтчетPL (Січень 2024 – Квітень 2026).
...
### A. Статичні довідники (5 файлів, ПОСТІЙНІ)
...
| A3 | `pl_report_architecture_analyst.md` | "PL звіт А_ОтчетPL архітектура аналітик — План Факт Каса ЦО СВОД data gap Аналитика drill-down" | Архітектура звіту для фінансиста (без BSL): 3 колонки, коли легально розходяться, ЦО vs СВОД, drill-down Аналитика/Аналитика1 (з 2026-05-21). |
...
| A5 | `pl_pipeline_safeguards.md` | "PnL pipeline захисти — алиаси deny-list А_РучнаяКорректировка дедуп" | 3 шари захисту PnL-pipeline від дублів і відновлення видалених статей: `ARTICLE_NAME_ALIASES` (canonical names), `BLOCKED_ARTICLE_NAMES` (deny-list для create), `А_РучнаяКорректировка` (lock для update). Кейс «Штраф» 2026-05-21. |
| A6 | `pl_finrez_sign_by_type.md` | "PL знак суми ТипСтатьи Доход Расход mirror А_ФинРез_PL Fact_PnL P&L одна формула" | Mirror-семантика знака суми у А_ФинРез_PL за реквізитом `ТипСтатьи` (Доход +, Расход −). Каскад змін: BSL → ETL → OLAP → PBIX. P&L одною формулою без CASE/SIGN у DAX. Acceptance 28/28 OK, 1С == OLAP. |
...
## Грабельки і поточні проблеми коду (2026-05-20)

### А_ФинРез_PL: відсутній регістр `А_ДокументРасшифровка`

При перепроведенні документа `А_ФинРез_PL` через UI 1С виникала помилка `Поле объекта не обнаружено (А_ДокументРасшифровка)` через посилання на регістр, який було заплановано у Stage v2 але **фактично не створено**.

**Виправлено 2026-05-20:** виклик `РегистрыСведений.А_ДокументРасшифровка.ОбновитьПоДокументам(МассивДок)` закоментовано (`Documents/А_ФинРез_PL/Ext/ObjectModule.bsl:64-93`). Документ знов проводиться.

**Деталі та як повернути drill-down:** див. [`pl_report_architecture_analyst.md` § Грабелька 2026-05-20](pl_report_architecture_analyst.md).

### А_ФинРез_PL: дублі ключа в 31.03.2026 (TODO)

Документ `А_ФинРез_PL` № 00000000003 від 31.03.2026 не проводиться через помилку `Запись с такими ключевыми полями существует` у `А_ОтчетPL_Свод`. Причина: у фіналі `СформироватьЗапросСверткиPL()` поле `Комментарий` включено у `СГРУППИРОВАТЬ ПО` (це реквізит, не вимір), що дає дублі ключа для регістра з 12 вимірами. Інші місяці (грудень 2025, січень-лютий-квітень 2026) проводяться нормально. Деталі та фікс — у `pl_report_architecture_analyst.md`.

### Knowledge drift: knowledge_Olap §9 hibly помічений як DONE

`knowledge_Olap/olap_1c_objects.md §9` стверджує що регістр `А_ДокументРасшифровка` створено Stage v2 (2026-05-05). Реально файл відсутній у git історії. **Урок:** перевіряти knowledge через `mcp__1c-workerp__list_metadata_objects` + `Glob` перед посиланням на нього в коді.
