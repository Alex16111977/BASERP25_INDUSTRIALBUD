...
Останнє оновлення: **2026-06-09** (Этап 2 ФормаPL: свёртка додала вимір `ФормаPL` + Ф1/Ф2-розріз факту; mirror-знак збережено)  
Тип: ПОСТІЙНИЙ (бізнес-правило стабільне)  
Пов'язаний файл: pl_articles_catalog.md, pl_report_architecture_analyst.md, `../knowledge_ПрочиеРасходы/formapl_finrez_done.md`, knowledge_Olap/olap_powerbi_pl_pbix.md
...
## Доповнення 2026-06-09: вимір `ФормаPL` у свёртці (Этап 2)

З 2026-06-09 `СформироватьЗапросСверткиPL()` додатково розносить **факт** за виміром `ФормаPL` (Форма1/Форма2)
і пише його в `А_ОтчетPL_Свод.ФормаPL` + ресурси `СуммаФ1`/`СуммаФ2` — за **реальною** `ПрочиеРасходы.ФормаPL`
(замість попередньої евристики ПКО→Ф2 / решта→Ф1). Дохід і CoGS — цілком Форма1; порожня форма → Форма1.
**Mirror-знак за `ТипСтатьи` (ця сторінка) збережено без змін** — він застосовується поверх Ф1/Ф2-розрізу до
всіх 6 ресурсів. Перевірено: Σ Сумма не змінилась (`BASE==MOD` Δ=0,00 на дек.2025), змінилась лише розбивка
Ф1/Ф2 факту; факт Ф2 = оборот `ПрочиеРасходы` по Форма2 до копійки. Деталі:
`../knowledge_ПрочиеРасходы/formapl_finrez_done.md`. OLAP-каскад `Fact_PnL` для розрізу Ф1/Ф2 **факту** — окремий
Этап 4, ще не зроблено (наразі Ф1/Ф2 у Fact_PnL лише зі сторони плану).
