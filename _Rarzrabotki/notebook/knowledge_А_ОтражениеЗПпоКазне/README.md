...
🟢 **2026-06-02…03 (v4)** — перенос Ф2 управл.ЗП на **корень дерева** (`_ПНМП_КореньСтрП`) + 3 фикса баланса А_ВзСС: удержания из дочернего ОЗФУ (источник без подразделения), зеркало движений А_ВзСС ОЗФУ для Начислений Ф1 (точный реверс, gross-НДФЛ/алименты у 8 ФЛ), ведомость/РКО → корень (выплата на то же подр., что и передача). Всё в [16_peredacha_nachisleniy.md](16_peredacha_nachisleniy.md), уроки LESSONS §22-25, промт `PROMPT_ПередачаНачисленийФорма2_Корень.md`.

🟢 **2026-06-10** — файл 21: фикс фантома `А_ВзаиморасчетыССотрудниками` **Форма1** по больничному «за счёт ФСС». У ФЛ без регулярной ЗП на 661 (GROSSбух<НДФЛ) `«Зарплата»Ф1 = Макс(0, GROSSбух−НДФЛ)` клампится к 0 → осиротевший gross-up «Удержание» съедает `НачисленныйНДФЛ` → больничный остаётся **GROSS** (фантом = НДФЛ). Фикс Op4 step-4 `_ОтрЗП_ЗаполнитьБольничныеИзФондов` (гард `ГБ>0 И ГБ<НД`→`ГБ<НД` + `Р.Сумма=Макс(0,ГБ)`). RED→GREEN фев/март/апр; только 1 из 3 больничных-ФСС задет. [21_bolnichny_fss_vzss_phantom.md](21_bolnichny_fss_vzss_phantom.md), LESSONS §25, промт `PROMPT_БольничныйФСС_ВзСС_Удержание.md`.
...
├── 19_forma2_nachalnaya_zadolzhennost.md  🟢 2026-06-02 — Форма2 начальная задолженность из zup_2 (Общий = сальдо ВзСР + начисление-нетто; Ф2=Общий−Ф1)
├── 20_dvizheniya_dds_kazna.md          🟢 2026-06-03 — ДДС Казны = А_ДвиженияДенегДоходыРасходы (зеркало БДДС через перепроведение Ф1/Ф2)
├── 21_bolnichny_fss_vzss_phantom.md    🟢 2026-06-10 — фикс фантома А_ВзСС Ф1 по больничному «за счёт ФСС» (clamp GROSSбух<НДФЛ → осиротевший «Удержание» съедает НДФЛ)
├── LESSONS.md                      25 уроков/антипаттернов (§25 — больничный ФСС clamp; §24 подотчётне; §22-23 Op2b/per-org НДФЛ)
