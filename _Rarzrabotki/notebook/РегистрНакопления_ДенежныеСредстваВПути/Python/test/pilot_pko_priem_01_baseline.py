# -*- coding: utf-8 -*-
"""
PILOT ПКО "Поступление денежных средств из другой кассы" Шаг 01 — Baseline ДО доработки.

Документ: ПриходныйКассовыйОрдер N0000023550 от 08.12.2025 13:53:51
  ХозОп = ПоступлениеДенежныхСредствИзДругойКассы, СуммаДокумента 800 000 UAH
  Касса = "Касса Глобино-2", Подразделение шапки = "Глобино-2"
  А_ПодразделениеОтправитель = "Строительство"
  А_ДокРасходныйКассовыйОрдерПередачиДенег = РКО N0000052985 (Касса "2 Касса Строительство")
  КассаОтправитель = "2 Касса Строительство" (если заполнен)

Ожидаем ДО фикса:
  - РНДС.ДенежныеСредстваВПути — 1 строка Расход 800000, Подразделение = Глобино-2 (шапка).
  - ПрочиеАктивыПассивы — строка Статья "Денежные средства в пути",
      Подразделение = Глобино-2, Источник = "Денежные средства в пути".
  - ДенежныеСредстваНаличные — Приход 800000, Подразделение = Глобино-2.

Артефакт: _artifacts/pilot_pko_priem_01_baseline.json
...
for fld in ("Дата", "Номер", "Проведен", "ПометкаУдаления", "Организация", "Подразделение",
            "Касса", "КассаОтправитель", "ХозяйственнаяОперация",
            "А_ПодразделениеОтправитель", "А_ДокРасходныйКассовыйОрдерПередачиДенег",
    ...
    try:
        ...
        sv = conv(v)
        hdr[fld] = sv
        print(f"  {fld:<40}: {'(пусто)' if sv is None else sv}")
    except Exception:
        pass

# --- Регистры: имя -> имя поля статьи (если есть), флаг наличия Источника ---
REGISTRY = ["ДенежныеСредстваВПути", "ПрочиеАктивыПассивы", "ДенежныеСредстваНаличные", "ДвиженияДенежныхСредств"]
...
# 3) Наличные.Приход
nal_prihod = find_agg("ДенежныеСредстваНаличные", lambda a: a["ВидДвижения"] == "Приход")
for a in nal_prihod:
    print(f"  [Наличные.Приход] Σ={a['СуммаΣ']}  Подразделение = {a['Подразделение']}")

# --- Сохранить JSON ---
out = os.path.join(ART, "pilot_pko_priem_01_baseline.json")
