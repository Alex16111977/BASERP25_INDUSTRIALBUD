# -*- coding: utf-8 -*-
"""PILOT ПостБезнал Шаг 01 — Baseline 00DL-007179."""
...
print("\n=== Реквизиты шапки ===")
hdr = {}
for fld in ("Дата","Номер","Проведен","Организация","Подразделение","БанковскийСчет","БанковскийСчетОтправитель",
            "КассаОтправитель","ХозяйственнаяОперация","Валюта","ВалютаКонвертации","СуммаДокумента",
...
for reg in REGISTRY:
    ...
    for i in range(rr.Количество()):
        ...
        target = f"{otpr}→{getr}"[:60] if otpr or getr else (row.get("БанковскийСчет") or "")[:60]
        print(f"  {str(vd):<8} Σ={sum_:>14}  Подр={str(podr)[:23]:<25} {target}")
    snapshot["movements"][reg] = {"count": rr.Количество(), "rows": rows}
out = os.path.join(ART, "pilot_post_01_baseline.json")
