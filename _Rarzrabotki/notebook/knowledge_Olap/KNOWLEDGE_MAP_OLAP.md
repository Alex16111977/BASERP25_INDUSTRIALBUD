...
| **Останнє оновлення** | 2026-05-19 (Dim_Contracts/ObjektyRaschetov розширені + Dim_TipyDogovorov/Dim_FinAgents нові; dim_contracts/dim_objekty_raschetov → raw_sql денорм; WHITELIST +5; FROZEN_ENUMS +3; PL.pbix ФінАгенти + 8 нових колонок ДоговорыКонтрагентов; user-gated: ОбъектыРасчетов refresh + ТипиДоговорів + Ctrl+S) |
...
| **Balance ПрочиеРасходыДоходы — ПОВНИЙ БАЛАНС** | `Свод_ПрочиеРасходыДоходы` LIVE (BSL+приймання 2026-05-18): Источник∈{ПрочиеДоходы/ПрочиеРасходы/ПартииПрочихРасходов}, зеркало канон-Прямого, `Source=Т.Источник`. **Discovery: ніс ВЕСЬ незакритий гэп → повний баланс Актив=Пассив досягнуто; `СверкаСПАП` зайва.** Регістр `А_ОтчетБаланс_Свод` Σ КО (OD-3, ВСІ Source) = 0,00 == штатний Отчет.УправленческийБаланс (дек2025 278 093 267,32 / янв2026 288 787 750,11) + до підрозділів == ПАП.ОстаткиИОбороты. **OLAP: НОВИХ Source/Dim/колонок PBIX НЕ потрібно** (Источник у FROZEN 31, без субконто). verify_olap_balance_papdirect.py доповнено блоком «ПОЛНЫЙ БАЛАНС». ✅ ETL fact_balance 3 періоди виконано (дек 7757 / янв 7721 / лют 8319 рядків, +ПрочиеРасходы); `verify_olap` **PASS**: ПОЛНЫЙ БАЛАНС 2026-01 Σ Sum_Close (OD-3, всі Source) = **0,00** ⇒ Актив=Пассив у Fact_Balance; Source=ПустаяСсылка ИТОГ −108 631 177,36 не змінився; TaxType==Карточка; конфіги/Dim/PL.pbix НЕ мінялись. PL.pbix — потрібен лише Refresh Fact_Balance + Ctrl+S | ✅ DONE (BSL+verify+ETL+verify_olap); PL.pbix Refresh+Ctrl+S | — | 2026-05-18 |
| **Balance ТипПоказателя — Dim_TipPokazatelya** | Користувач додав вимір `ТипПоказателя` (Перечисление.ВидыСтатейУправленческогоБаланса) у А_ОтчетБаланс_Свод; заповнюється централізовано в `ПровестиБалансСвод` формулою штатного Отчет.УправленческийБаланс (АктивПассив→Пассив; «Налоги»→Пассив). OLAP: `_Fld56131RRef`→`refresh_mapping` (ВидыСтатейУправленческогоБаланса вже WHITELIST/FROZEN; FROZEN_ENUMS латиницю→кирилицю Актив/Пассив/АктивПассив як Source/ТипыНалогов); `fact_balance.json` +TipPokazatelya (raw_sql/enum_resolver/column_map); SQL DDL: ALTER Fact_Balance ADD TipPokazatelya + **Dim_TipPokazatelya** (4: Актив/Пассив/АктивПассив/ПустаяСсылка, seed з 1С COM). ETL 3 періоди + `verify_olap_balance_tippokazatelya.py` **PASS**: TipPokazatelya 0 NULL, FK→Dim 100%, «Налоги»→Пассив, ПОЛНЫЙ БАЛАНС по TipPokazatelya дек 278 093 267,32 / янв 288 787 750,11 == штатний звіт. PL.pbix — користувач САМ додасть таблицю Dim_TipPokazatelya + зв'язок Fact_Balance[TipPokazatelya]→Dim_TipPokazatelya (Many→One) + Refresh+Ctrl+S | ✅ DONE (BSL+verify+ETL+Dim+verify_olap); PL.pbix — за користувачем | — | 2026-05-18 |
| **Balance Расчёты → Контрагент/Договор/Партнёр** | `Свод_РасчетыСПартнерами` тепер пише субконто `Контрагент`/`Партнер`/`Договор` з `АналитикаУчетаПоПартнерам` (деталі; плуги ПустаяСсылка). **OLAP конфіг/Dim/mapping НЕ змінювались** — `fact_balance.json` вже мав `_Fld56098→Counterparty_ID`/`_Fld56099→Partner_ID`/`_Fld56102→Contract_ID`; `refresh_mapping` НЕ потрібен (структура РС не змінилась — лише дані). ETL `--run-once fact_balance --period 2026-01` (run_id=290, 7721; користувач перепровів лише янв2026). `verify_olap_balance_raschety_kontragent.py` **PASS**: розрахункові деталі 2287 Counterparty/Contract/Partner **100% NOT NULL**, плуги порожні, FK→`Dim_Counterparties/Dim_Contracts/Dim_Partners` **0 orphans**, повний баланс Σ Sum_Close=0 / Актив 288 787 750,11 незмінний. PL.pbix модель НЕ міняється (зв'язки `[Counterparty_ID]→Контрагенты`/`[Contract_ID]→ДоговорыКонтрагентов`/`[SettlementObj_ID]→ОбъектыРасчетов` вже є) — лише Refresh+Ctrl+S | ✅ DONE (BSL+pretest+verify+ETL+verify_olap янв2026); PL.pbix Refresh+Ctrl+S; дек2025/лют2026 — за потреби перепровести+ETL | — | 2026-05-18 |
| **Contracts/FinAgents dims** | Dim_Contracts/ObjektyRaschetov розширені (денорм. назви + enum); нові Dim_TipyDogovorov (12) + Dim_FinAgents (13); dim_contracts/dim_objekty_raschetov → raw_sql; WHITELIST +5; FROZEN_ENUMS +3 (ТипыДоговоров/РасчетовСПартнерами/ОбъектовРасчетов). PL.pbix: ФінАгенти + snowflake-зв'язок + 8 нових колонок ДоговорыКонтрагентов; user-gated: ОбъектыРасчетов refresh + ТипиДоговорів + Ctrl+S. verify PASS (8248/14109/12/13 рядків; 0 FK orphans; ПОЛНИЙ БАЛАНС не регресував) | ✅ DONE (ETL+Dim+verify); PL.pbix user-gated Desktop-дії | — | 2026-05-19 |
| **VidyKontragentov dim** | Класифікатор контрагентів для балансу: Справочник.А_ВидыКонтрагентовДляБаланса (5 предопр.: Внутригрупповые/Внутренние/Собственники/Внешние/Кредиторы) → **Dim_VidyKontragentov** (char(32) PK, `_Reference56330`) + 2 FK-колонки Dim_Contracts: `VidKontragenta_ID` (`_Fld56332RRef`) і `NapravlenieUslug_ID` (`_Fld56331RRef` А_НаправлениеОказаниеУслуг → Dim_Directions). Snowflake як TipyDogovorov/FinAgents. Договори розмічені (Внутригруп. 331 / Внутр.підрозділи 1106 / Собственники 10 / Внешние 7162 / Кредиторы 17; НапрУслуг 1104). WHITELIST+refresh_mapping, DDL `ddl_dim_vidy_kontragentov.sql`, step `dim_vidy_kontragentov` у dim_catalogs.json, повний main.py Success. `verify_olap_dim_vid_kontragenta.py` **PASS**: розкладка SQL==1С, FK 0 orphans, крос-звірка «Внутригрупповые» 2026-01 РС==SQL (0,12) до копійки, Σ Sum_Close 2026-01=0,00 (Актив 338 857 601,40 — жива база дрейфує від канона 288M), pytest Глобино-2 PASS. Попутно: користувач видалив биту строку 267 А_СобытияОбъектов (ламала ВСІ записи договорів). **PL.pbix DONE** (MCP): таблиця «ВидыКонтрагентов» + зв'язок від `ДоговорыКонтрагентов[ВидКонтрагента_ID]`; **НаправлениеУслуг — НЕ вимірювання** (рішення фінансиста v2): денорм-колонка `NapravlenieUslug_Name` → видима «Направление (услуги между подр.)» (як Dept_OkazUslug_Name), роль-таблиця «НаправленияУслуг» видалена (активний зв'язок Договоры→НаправленияДеятельности неможливий — неоднозначні шляхи через Fact_Cashflow); партиція Dim_Contracts = native query з явним списком колонок. Граблі/деталі: `olap_powerbi_pl_pbix.md` запис 2026-06-12. User-gated: Desktop Refresh + Ctrl+S. Промт `prompt_dim_vid_kontragenta_napravlenie_uslug.md` | ✅ DONE (ETL+Dim+verify+PL.pbix); Refresh+Ctrl+S за користувачем | — | 2026-06-12 |
