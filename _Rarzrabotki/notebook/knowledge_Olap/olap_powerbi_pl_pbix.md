...
## 2026-06-12 — «ВидыКонтрагентов» (snowflake) + колонка «Направление (услуги между подр.)»

**Зроблено через MCP** (Desktop port 59166):

1. Партиція `Dim_Contracts` таблиці «ДоговорыКонтрагентов» переписана на **native query**
   з ЯВНИМ списком колонок (обхід кешу схеми §13.1; `SELECT *` НЕ використовувати — після
   видалення model-колонки зірочка повертає її при наступному schema-sync, попередження MCP).
2. Нова таблиця **«ВидыКонтрагентов»** (партиція `Dim_VidyKontragentov`, native query;
   видима колонка «ВидКонтрагента», приховані VidKontragenta_ID/Код/Marked_For_Deletion).
3. Нова прихована колонка «ДоговорыКонтрагентов»[`ВидКонтрагента_ID`] + зв'язок
   `ДоговорыКонтрагентов[ВидКонтрагента_ID]` → `ВидыКонтрагентов[VidKontragenta_ID]`
   (Many→One, OneDirection, active). Зріз Fact_Balance/Fact_Cashflow по виду контрагента —
   через ланцюг Contract_ID → ДоговорыКонтрагентов → ВидыКонтрагентов.
4. Нова ВИДИМА колонка «ДоговорыКонтрагентов»[**«Направление (услуги между подр.)»**]
   (SourceColumn `NapravlenieUslug_Name`) — як «Подразделение (услуги между подр.)».

**⚠️ Граблі (боевые, 2026-06-12):**
- **Активний зв'язок Договоры→НаправленияДеятельности НЕМОЖЛИВИЙ**: сервер відхиляє
  commit — «неоднозначные пути» `Fact_Cashflow→СтруктураПредприятия→НаправленияДеятельности`
  (BothDirections!) vs `Fact_Cashflow→ДоговорыКонтрагентов→НаправленияДеятельности`.
  Перша реалізація (роль-таблиця «НаправленияУслуг») працювала, але **рішення фінансиста** —
  не окреме вимірювання, а денорм-колонка (п.4); роль-таблицю видалено.
- **Транзакція batch-create зв'язків відкочується ЦІЛКОМ** при відмові commit (обидва
  зв'язки зникли, хоч List ще показував їх) — створювати зв'язки по одному.
- **MCP table Refresh у Desktop ВИСНЕ** (timeout 30s, конект падає) — дані наповнює
  користувач кнопкою «Обновить» в Desktop (+можливе вікно дозволу native query) + Ctrl+S.

Гейт: після Refresh — DAX-зріз Fact_Balance по «ВидКонтрагента» == SQL-еталону
(verify_olap_dim_vid_kontragenta.py: «Внутригрупповые» 2026-01 = 0,12).
