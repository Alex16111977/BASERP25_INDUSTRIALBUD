...
> ## ⚡ СТАТУС 2026-06-12 (v2): ВСЕ PHASES ВЫПОЛНЕНЫ. За пользователем: Desktop Refresh + Ctrl+S (+финальный DAX-гейт).
> - Phase 5 DONE с ИЗМЕНЕНИЕМ дизайна (решение финансиста): «ВидыКонтрагентов» — таблица+связь (как в промте); **НаправлениеУслуг — НЕ измерение**, а денорм-колонка `NapravlenieUslug_Name` в Dim_Contracts → видимая колонка «Направление (услуги между подр.)» (как Dept_OkazUslug_Name). Роль-таблица «НаправленияУслуг» создана и удалена. Прямая активная связь Договоры→НаправленияДеятельности невозможна — неоднозначные пути через Fact_Cashflow (СтруктураПредприятия→Направления BothDirections). Партиция Dim_Contracts = native query с ЯВНЫМ списком колонок. MCP table Refresh виснет — Refresh жмёт пользователь. Детали/грабли: `olap_powerbi_pl_pbix.md` запись 2026-06-12.
