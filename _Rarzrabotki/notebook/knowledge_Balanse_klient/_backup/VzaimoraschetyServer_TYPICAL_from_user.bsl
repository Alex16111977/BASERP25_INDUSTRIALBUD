Your questions have been answered: "Найден виновник: в коммите da8526dc8 (27.03.2026) в ОбщеМодуле `ВзаиморасчетыСервер` добавлен ранний `Возврат` при флаге `НоваяАрхитектураВзаиморасчетов=Истина` в процедурах ОтразитьЗаданияКРаспределениюРасчетовСКлиентами/Поставщиками. Это блокирует создание заданий → РСППС=0 для ПриобретениеТовУсл. Как исправить?"="вот как было раньше: 
...
#КонецОбласти". You can now continue with these answers in mind.