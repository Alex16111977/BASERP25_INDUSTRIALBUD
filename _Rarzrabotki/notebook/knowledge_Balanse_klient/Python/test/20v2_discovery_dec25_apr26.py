# -*- coding: utf-8 -*-
"""
СКРИПТ 20v2 (Phase 0 для остаточных) — Discovery ПАП vs РСКПС+РСППС
                                          за 01.12.2025—31.12.2026 (после фикса ПереносАванса)

КОПИЯ скрипта 20 (см. 01_formulas.md формула) с расширенным периодом.
Формула: Δ = ПАП.signed - РСКПС.signed(ДолгУпр-ПредоплатаУпр) - РСППС.signed(ДолгУпр-ПредоплатаУпр)

Цель: понять что осталось ПОСЛЕ перепроведения 204 ПереносАванса-документов + А_ФинРез_Баланс.
"""
import sys, io
...
q.Текст = """
// === ПАП: статьи расчётов с партнёрами за 01.12.2025—31.12.2026 ===
ВЫБРАТЬ
...
ГДЕ Р.Период МЕЖДУ ДАТАВРЕМЯ(2025,12,1) И ДАТАВРЕМЯ(2026,12,31,23,59,59)
...
ГДЕ П.Период МЕЖДУ ДАТАВРЕМЯ(2025,12,1) И ДАТАВРЕМЯ(2026,12,31,23,59,59)
    И АП.Организация = &Орг
...
print("=" * 100)
print("СКРИПТ 20v2 — Discovery ПАП vs РСКПС+РСППС за 01.12.2025—31.12.2026 (ПОСЛЕ фикса)")
print("=" * 100)
print("(запрос может занять 1-2 мин...)")
...
print(f"\nΣ ПАП={money(sum_pap)}")
print(f"Σ РСК={money(sum_rsk)}")
print(f"Σ РСП={money(sum_rsp)}")
...
path = save_csv("20v2_full_discovery", rows,
