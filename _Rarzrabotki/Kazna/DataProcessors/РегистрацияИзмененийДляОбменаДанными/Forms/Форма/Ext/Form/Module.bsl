...
Процедура ЗарегистрироватьЗаПериод(Команда)
	...
	Если Не ЗначениеЗаполнено(ПериодНачало) Тогда
		ПоказатьПредупреждение(, "Укажите начало периода");
		Возврат;
	КонецЕсли;
	...
	Если ПериодНачало > ПериодОкончание Тогда
		ПоказатьПредупреждение(, "Начало периода не может быть больше окончания");
		Возврат;
	КонецЕсли;

	ТекстВопроса = "Зарегистрировать изменения документов за период с "
		+ Формат(ПериодНачало, "ДЛФ=D") + " по " + Формат(ПериодОкончание, "ДЛФ=D") + "?";

	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ПериодНачало", ПериодНачало);
	ДополнительныеПараметры.Вставить("ПериодОкончание", ПериодОкончание);

	ПоказатьВопрос(
		Новый ОписаниеОповещения("ЗарегистрироватьЗаПериодЗавершение", ЭтотОбъект, ДополнительныеПараметры),
		ТекстВопроса,
...
Процедура ЗарегистрироватьЗаПериодЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	// Собираем имена метаданных из дерева (как в ДобавитьРегистрациюВыделенныхОбъектов)
	...
	Если МетаИмена.Количество() = 0 Тогда
		ПоказатьПредупреждение(, "Не выбраны объекты для регистрации");
		Возврат;
	КонецЕсли;

	Количество = ЗарегистрироватьЗаПериодНаСервере(
		ДополнительныеПараметры.ПериодНачало,
		ДополнительныеПараметры.ПериодОкончание,
		МетаИмена);

	ПоказатьОповещениеПользователя(
		"Регистрация завершена",
		,
		"Зарегистрировано объектов: " + Количество,
...
&НаСервере
Функция ЗарегистрироватьЗаПериодНаСервере(ДатаНачала, ДатаОкончания, МетаИмена)

	Количество = 0;

	Для Каждого ИмяМетаданных Из МетаИмена Цикл
		...
		Пока Выборка.Следующий() Цикл
			ПланыОбмена.ЗарегистрироватьИзменения(УзелОбменаСсылка, Выборка.Ссылка);
			// Регистрируем также справочники из документа
			ЭтотОбъект().ЗарегистрироватьСправочникиИзДокумента(УзелОбменаСсылка, Выборка.Ссылка);
