...
Функция НайтиБлокЗатрат(ДополнительныеПараметры)
	...
	Для ИндексСтроки=1 По 200 Цикл
		Если ДополнительныеПараметры.ADODBRecordset.EOF()<>0 Тогда
			Прервать;
		КонецЕсли;
		Значение = ПолучитьЗначениеЯчейки(ДополнительныеПараметры, ИндексСтроки, 1);
		Если Значение="Витрати" Тогда
...
Процедура ЗагрузитьТаблицуСчетовADO(МассивЛистов) Экспорт
	...
	////СоответствиеКолонок.Вставить("НаименованиеБанка", 9);
	////СоответствиеКолонок.Вставить("МФОБанка", 10);
...
&НаСервере
Процедура ЗагрузитьСписокЛистов() Экспорт
    ...
    Для Каждого ЛистМассива ИЗ МассивЛистов Цикл
        ...
        Если ИмяЛиста = "Excel_BuiltIn_Database" Тогда
        	Продолжить;
		КонецЕсли;
		Если СтрДлина(ИмяЛиста)=8 И КодСимвола(ИмяЛиста, 4)=КодСимвола("#", 1) Тогда
			ПредставлениеЛиста = Сред(ИмяЛиста, 2, СтрДлина(ИмяЛиста)-3);
			ПредставлениеЛиста = СтрЗаменить(ПредставлениеЛиста, "#", "_");
	        СписокЛистов.Добавить(ИмяЛиста, ПредставлениеЛиста);
		ИначеЕсли СтрДлина(ИмяЛиста)=9 Тогда
			ПредставлениеЛиста = СтрЗаменить(ИмяЛиста, "$", "");
...
Процедура ЗагрузитьДокументПродолжение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Элементы.Главная.ТекущаяСтраница = Элементы.УточнитеЛист;
...
Процедура ВыбратьЛист(Команда)
	...
	Если СписокЛистов.Количество()=0 Тогда
		Возврат;
	КонецЕсли;
	Если Элементы.СписокЛистов.ТекущиеДанные=Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	МассивЛистов = Новый Массив;
	Для Каждого Строка Из Элементы.СписокЛистов.ВыделенныеСтроки Цикл
		
		СтрокаСписка = СписокЛистов.Получить(Строка);
		ПредставлениеЛиста = СокрЛП(СтрокаСписка.Представление);
		ДатаСчетов = Дата(2016, Число(Прав(ПредставлениеЛиста, 2)), Число(Лев(ПредставлениеЛиста, 2)));		
		
		СтруктураЛиста = Новый Структура;
		СтруктураЛиста.Вставить("ИмяЛиста", СтрокаСписка.Значение);
		СтруктураЛиста.Вставить("Дата", ДатаСчетов);
