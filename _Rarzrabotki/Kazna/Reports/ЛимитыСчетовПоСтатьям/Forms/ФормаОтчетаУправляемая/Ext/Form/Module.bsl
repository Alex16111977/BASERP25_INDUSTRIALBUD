...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если НЕ ЗначениеЗаполнено(Отчет.ДатаНачала) ИЛИ НЕ ЗначениеЗаполнено(Отчет.ДатаКонец) Тогда
		
		Отчет.ДатаКонец  = КонецМесяца(ТекущаяДатаСеанса());
...
Процедура ИзменитьПериод(Шаг)
		...
		Отчет.ДатаКонец  = КонецМесяца(ДобавитьМесяц(Отчет.ДатаКонец , Шаг));
		Отчет.ДатаНачала = НачалоМесяца(Отчет.ДатаКонец);
...
Процедура ПоказатьПериод(Форма)
	...
	СтрПериодОтчета = ПредставлениеПериода(НачалоДня(Форма.Отчет.ДатаНачала), КонецДня(Форма.Отчет.ДатаКонец), "ФП = Истина" );
