...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВладелецФайла) Тогда
		...
		Если ТипЗнч(ВладелецФайла) = Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
			Если ЗначениеЗаполнено(ВладелецФайла.ДокументОснование) Тогда
				Список.Параметры.УстановитьЗначениеПараметра("ВладелецСчетОснование", ВладелецФайла.ДокументОснование);
			Иначе		
				
				Список.Параметры.УстановитьЗначениеПараметра("ВладелецСчетОснование", ВладелецФайла);
			КонецЕсли;
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ЗначениеПараметраКД = Неопределено Тогда
		ВладелецФайла = Неопределено;    
		ВладелецСчетОснование = Неопределено;    
	Иначе
		ВладелецФайла = ЗначениеПараметраКД.Значение;
		Если ТипЗнч(ВладелецФайла)=Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
			Если ЗначениеЗаполнено(ВладелецФайла.ДокументОснование) Тогда
				ВладелецСчетОснование = ВладелецФайла.ДокументОснование;
			Иначе	
				ВладелецСчетОснование = ВладелецФайла ;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
//++ Бахтишаев А.Э. , 01/08/16 06:23,
&НаКлиенте
Процедура СоздатьФайлы(Команда)
	...
	Если Не ФайловыеФункцииСлужебныйКлиент.РасширениеРаботыСФайламиПодключено() Тогда
		//Возможно расширение не было установлено, попытаемся установить и подключить еще раз
		ОписаниеОповещения	= Новый ОписаниеОповещения("СоздатьФайлыУстановкаРасширенияРаботыСФайламиЗавершение",ЭтаФорма, ПараметрыДобавления);
		НачатьУстановкуРасширенияРаботыСФайлами(ОписаниеОповещения);
		Возврат;
	КонецЕсли;
	#КонецЕсли
	
	АХ_ОбщегоНазначенияКлиент.ДобавитьИзФайловойСистемыБезРасширения(ПараметрыДобавления);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьФайлыУстановкаРасширенияРаботыСФайламиЗавершение(ПараметрыДобавления) Экспорт
