...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого ТекСтрокаОтработанноеВремя Из ОтработанноеВремя Цикл
		Для СчКолонок=1 По 31 Цикл
			...
			// Разбор ячейки: число -> "Р"; буква -> своё состояние; "8М" -> "М" + ЧасыОфициальные (модуль менеджера)
			КвоЧасов = ТекСтрокаОтработанноеВремя["ПервыйЧасов"+СчКолонок];
			Разбор = Документы.ТабельУчетаРабочегоВремени.РазобратьЗначениеЯчейки(КвоЧасов);
			локСостояние = Разбор.Состояние;
			КвоЧасовФакт = Разбор.ЧасыФакт;
			Если Разбор.Ошибка <> "" Тогда
				Сообщить("Строка " + ТекСтрокаОтработанноеВремя.НомерСтроки + " (" + ТекСтрокаОтработанноеВремя.Сотрудник + "), день " + СчКолонок + ": " + Разбор.Ошибка);
			КонецЕсли;
			 ...
			 Если ЗначениеЗаполнено(локСостояние) ИЛИ  КвоЧасовПлан>0 Тогда
				...
				Иначе	
				 ...
				 Движение.ЧасыОфициальные = Разбор.ЧасыОфициальные;
