...
Процедура ОтработанноеВремяПриИзменении(Элемент)
	...
	// Итог часов по ставке: разбор ячеек на сервере, часы "М" (официальные) не входят
	ЗначенияЯчеек = Новый Массив;
	Для Сч=1 По 31 Цикл
		ЗначенияЯчеек.Добавить(ТекДанные["ПервыйЧасов"+Сч]);
	КонецЦикла;
	ИтогоЧасов = СуммаЧасовФактНаСервере(ЗначенияЯчеек);
...
Процедура ОтработанноеВремяПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Найти(ИмяКолонки,"ПервыйЧасов") Тогда
		//Проверка на допустиые значения
		Разбор = РазобратьЗначениеНаСервере(ТекДанные[ИмяКолонки]);
		Если Разбор.Ошибка <> "" Тогда
			Предупреждение(Разбор.Ошибка, 15);
...
Функция РазобратьЗначениеНаСервере(Значение)
	Возврат Документы.ТабельУчетаРабочегоВремени.РазобратьЗначениеЯчейки(Значение);
КонецФункции

&НаСервереБезКонтекста
Функция СуммаЧасовФактНаСервере(ЗначенияЯчеек)
	Итого = 0;
	Для Каждого Значение Из ЗначенияЯчеек Цикл
		Итого = Итого + Документы.ТабельУчетаРабочегоВремени.РазобратьЗначениеЯчейки(Значение).ЧасыФакт;
	КонецЦикла;
	Возврат Итого;
КонецФункции
