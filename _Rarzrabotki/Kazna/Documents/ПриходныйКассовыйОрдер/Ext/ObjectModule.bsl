...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	А_ОбновитьРасшифровкуФинАгентов();
	//***ERP
	
КонецПроцедуры


Процедура А_ОбновитьРасшифровкуФинАгентов() Экспорт
	РасшифровкаФинАгент.Очистить();
	Если ЭтоНовый()
		ИЛИ НЕ Проведен 
		Тогда
	 ...
	 Запрос.УстановитьПараметр("Регистратор",Ссылка);
	   Запрос.Текст =   "ВЫБРАТЬ
	                   |	ДенежныеСредстваФинАгенты.Сумма,
	                   |	ДенежныеСредстваФинАгенты.ФинАгент
	                   |ИЗ
	                   |	РегистрНакопления.ДенежныеСредстваФинАгенты КАК ДенежныеСредстваФинАгенты
	                   |ГДЕ
	                   |	ДенежныеСредстваФинАгенты.Регистратор = &Регистратор";
	 ...
	 Запрос.УстановитьПараметр("Регистратор",Ссылка);
	   Запрос.Текст =  "ВЫБРАТЬ
	                   |	ДСКасса.Сумма,
	                   |	ДСКасса.СуммаКомиссииФА,
	                   |	ДСКасса.ДокументПередачи,
	                   |	ДСКасса.Регистратор._ФинАгент КАК ФинАгент,
	                   |	ДСКасса.Регистратор.Направление КАК Направление
	                   |ИЗ
	                   |	РегистрНакопления.ДСКасса КАК ДСКасса
	                   |ГДЕ
	                   |	ДСКасса.Регистратор = &Регистратор";
	  РасшифровкаФинАгент.Загрузить(Запрос.Выполнить().Выгрузить());
КонецПроцедуры
