Процедура ВыполнитьДвижениеОбщая(Объект, ВидОплаты, ЭтоРасход, Сторнировать, ПланФакт, БратьИзЗаявки)
	...
	Если Не ЗначениеЗаполнено(НоваяСтрока.Направление)
		...
		НоваяСтрока.Направление = НоваяСтрока.ДокументОснование.СчетНаОплатуПоставщика.Направление;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НоваяСтрока.Направление) ТОГДА
		Если МетаданныеОбъекта.Реквизиты.Найти("Направление")<>Неопределено Тогда
			НоваяСтрока.Направление = Объект.Направление;
		КонецЕсли;
	КонецЕсли;
	...
	Если МетаданныеОбъекта.Реквизиты.Найти("ВалютаДокумента")<>Неопределено Тогда
		Если ЗначениеЗаполнено(Объект.ВалютаДокумента) Тогда
			НоваяСтрока.Сумма = РаботаСКурсамиВалют.ПересчитатьВВалюту(НоваяСтрока.Сумма,Объект.ВалютаДокумента,Константы.ВалютаРегламентированногоУчета.Получить(),Объект.Дата);
		КонецЕсли;	                                                             
	ИначеЕсли МетаданныеОбъекта.Реквизиты.Найти("Валюта")<>Неопределено Тогда
		Если ЗначениеЗаполнено(Объект.Валюта) Тогда
			НоваяСтрока.Сумма = РаботаСКурсамиВалют.ПересчитатьВВалюту(НоваяСтрока.Сумма,Объект.Валюта,Константы.ВалютаРегламентированногоУчета.Получить(),Объект.Дата);
		КонецЕсли;	
	КонецЕсли;
...
Процедура ВыполнитьДвижение(Объект) Экспорт
	...
	Если ТипОбъекта=Тип("ДокументОбъект.ПриходныйКассовыйОрдер")
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратДенежныхСредствПодотчетником 
			...
			ИЛИ Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПоставщиком  тогда
			ЭтоРасход = Истина;
			Сторнировать = Истина;
		КонецЕсли;
