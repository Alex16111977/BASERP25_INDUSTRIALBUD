...
// ЕРП: РегНакопления.А_ВзаиморасчетыССотрудниками БЕЗ фильтра ФормаPL.
// Остатки (НачОст/КонОст) — полные, из ОстаткиИОбороты.
// Обороты — из движений с БЕЛЫМ СПИСКОМ регистраторов:
//   Начисления = только ОтражениеЗарплатыВФинансовомУчете (Приход),
//   Выплаты    = только РасходныйКассовыйОрдер + СписаниеБезналичныхДенежныхСредств (Расход).
// А_ПередачаНачисленийМеждуПодразделениями и прочие регистраторы в обороты НЕ попадают,
// поэтому НачОст + Начисления - Выплаты <> КонОст — это by-design следствие фильтра.
// Конвенция: + = долг работодателя.
