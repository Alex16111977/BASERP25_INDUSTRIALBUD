	...
	Для Каждого Строка Из Строки Цикл
		Если ВыбранноеЗначение = Строка.Подразделение Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'По выбранному подразделению есть планы.';
											|en = 'There are plans by the selected department.'"));
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Если Строка <> Неопределено И (Поле.Имя = "КорзинаЗаказовПартнер"
		...
		Если Поле.Имя = "КорзинаЗаказовПартнер" Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Назначить для выделенного товара поставщика: %1?';
																						|en = 'Assign a supplier for the selected goods: %1?'"), СтрокаКорзины.Партнер);
		ИначеЕсли Поле.Имя = "КорзинаЗаказовПодразделение" Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Назначить для выделенного товара подразделение: %1?';
					|en = 'Assign a department: %1 to the selected goods?'"), СтрокаКорзины.Подразделение);
		ИначеЕсли Поле.Имя = "КорзинаЗаказовСоглашение" Тогда
			ИменаПолей = "Соглашение, Партнер, Валюта";
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Назначить для выделенного товара поставщика: %1
				|и соглашение: %2?';
				|en = 'Assign supplier %1
				|and agreement %2 to the selected goods?'"),СтрокаКорзины.Партнер, СтрокаКорзины.Соглашение);
...
Процедура ЗаполнитьПодразделение(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выделите строки в которых разрешено редактирование подразделения.';
										|en = 'Select lines in which department editing is allowed.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоставщика(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выделите строки в которых разрешено редактирование поставщика.';
										|en = 'Select lines in which you can edit the supplier.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьЦеныПоВидуЦеныПоставщика(Команда)
	...
	Если ЗакупкиКлиент.НеобходимоЗаполнениеЦенПоВидуЦенПоставщика(ЗаказОбъект, "ВыделенныеСтроки", НСтр("ru = 'выделенных строк';
		...
		Если СформированныеЗаказы.Количество() > 0 Тогда
			...
			ПоказатьВопрос
				...
				НСтр("ru = 'Список созданных заказов не пуст. Созданные заказы будут удалены. Удалить заказы и продолжить?';
					|en = 'List of the created orders is not empty. Created orders will be removed. Remove the orders and continue?'"),
				Кнопки);
			Возврат;
		КонецЕсли;
...
Процедура УстановитьЗаголовокФормы(Страница)
	...
	ИначеЕсли Страница = Элементы.СтраницаШаг2 Тогда
		...
		Элементы.ПодЗаголовок.Заголовок = НСтр("ru = 'Шаг 2 из 3. Определение состава и объема заказа';
												|en = 'Step 2 of 3. Determine order content and volume'");
...
Процедура ЗаписатьЗаказНаСервере(ДокументОбъект, СкладРодитель, СписокОшибок)
	...
	ДокументОбъект.Комментарий = НСтр("ru = 'Сформирован автоматически обработкой ""Формирование заказов поставщику по плану"".';
										|en = 'Generated automatically using the ""Generation of purchase orders according to plan"" data processor.'");
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Формирование заказов поставщику по плану';
										|en = 'Generation of purchase orders according to plan'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УдалитьЗаказыНаСервере(СсылкиНаУдаление)
	...
	ТекстСообщения = НСтр("ru = 'Имеются ссылки на объект %1  %2. Документ не удален';
							|en = 'There are references to the %1 object %2. Document is not deleted'");
		...
		Если ПроверитьЕстьДокументыДляОбновления() Тогда
			ПоказатьВопрос(
				...
				НСтр("ru = 'Формирование заказов поставщику по плану';
					|en = 'Generation of purchase orders according to plan'"));
		КонецЕсли;
	Иначе
		ОбновитьДанныеКогдаФормаСтанетАктивной();
	КонецЕсли;
