...
Функция ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, 
	...
	Текст = НСтр("ru='Конфигурация: ';uk='Конфігурація: '") + Метаданные.КраткаяИнформация + НСтр("ru='."
" Версия: ';uk='."
" Версія:'") + Метаданные.Версия + ".
...
Функция ПолучитьФайлССервера(Знач Адрес, Знач ЛокальныйПуть, Знач ВыдаватьСообщения = Истина, 
	...
	Исключение
		...
		ЗаписатьПредупреждение(НСтр("ru='Ошибка при обращении к http://';uk='Помилка при зверненні до http://'") + Соединение.Сервер + "/" + Адрес + "
		...
		Если ЭтоДистрибутивОбновления(Обновление.ПутьКЛокальномуФайлу) Тогда
			Попытка 
				...
				Если НЕ ФайлСуществует(ИмяФайлаОбновления) Тогда
					ЗаписатьОшибку(НСтр("ru='Дистрибутив обновления не содержит 1cv8.cfu: ';uk='Дистрибутив оновлення не містить 1cv8.cfu: '") + Обновление.ПутьКЛокальномуФайлу);
					Возврат Ложь;
				КонецЕсли;
...
Функция УстановленWMI(Знач ВыдаватьСообщения = Истина) Экспорт
	...
	Исключение
		ЗаписатьОшибку(НСтр("ru='Невозможно продолжить, так как на Вашем компьютере не установлен компонент Windows Management Instrumentation. ';uk=""Неможливо продовжити, тому що на Вашому комп'ютері не встановлений компонент Wіndows Management Іnstrumentatіon. """) + ОписаниеОшибки());
		Если ВыдаватьСообщения Тогда
	...
	Попытка
		...
		Иначе	
			ЗаписатьИнформацию(НСтр("ru='Задача планировщика успешно запланирована (';uk='Задача планувальника успішно запланована ('") + 
			    НСтр("ru='команда: ';uk='команда:'") + ИмяФайлаПрограммы + "; " +
			    НСтр("ru='дата: ';uk='дата:'") + ДатаВремя + "; " +
