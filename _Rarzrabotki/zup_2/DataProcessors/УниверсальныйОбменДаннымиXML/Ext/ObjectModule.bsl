	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			...
			Если ФлагОшибки Тогда
				...
				ТекстПредупреждения = НСтр("ru='При выгрузке обработчиков были обнаружены ошибки.';uk='При вивантаженні обробників були виявлені помилки.'");
				Предупреждение(ТекстПредупреждения);
				
			Иначе
				
				ТекстПредупреждения = НСтр("ru='Обработчики успешно выгружены.';uk='Обробники успішно вивантажені.'");
				Предупреждение(ТекстПредупреждения);
	...
	#Если Клиент Тогда
		Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0 
			...
			Попытка
				...
				СтрокаСостояния = НСтр("ru='Выгрузка объектов "" %ТипОбъекта%"". Выгружено: %Количество% Объект: %ИмяОбъекта%';uk='Вивантаження об''єктів "" %ТипОбъекта%"". Вивантажено: %Количество% Об''єкт: %ИмяОбъекта%'");
			...
			Исключение
				...
				СтрокаСостояния = НСтр("ru='Выгрузка объектов """". Выгружено: %Количество% Объект: %ИмяОбъекта%';uk='Вивантаження об''єктів """". Вивантажено: %Количество% Об''єкт: %ИмяОбъекта%'");
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%Количество%", Строка(мСчетчикВыгруженныхОбъектов));
				СтрокаСостояния = СтрЗаменить(СтрокаСостояния, "%ИмяОбъекта%", ИмяМетаданных);
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл") Экспорт
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			...
			Текст = НСтр("ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %Ошибка%';uk='Обробник: ""ПослеЗагрузкиПравилОбмена"": %Ошибка%'");
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	#Если Клиент Тогда		
		Если ЭтоИнтерактивныйРежим Тогда
			...
			ТекстПредупреждения = НСтр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'");
			Предупреждение(ТекстПредупреждения);
		КонецЕсли;
	#КонецЕсли
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			ОткрытьДанныеПротоколовОбменаПриНеобходимости();
			ТекстПредупреждения = НСтр("ru='Выгрузка данных завершена.';uk='Вивантаження даних завершено.'");
			Предупреждение(ТекстПредупреждения);
		КонецЕсли;	
	#КонецЕсли
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = НСтр("ru='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка:';uk='При спробі створення обробки УниверсальныйОбменДаннымиXML відбулася помилка:'") + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
	КонецПопытки;
...
Процедура ИнициализацияВнешнейОбработкиОбработчиковСобытий(РаботаВозможна, ОбъектВладелец) Экспорт
	...
	Если ЭтоРазделеннаяКонфигурация Тогда
		...
		Если Не ФлагРежимОтладкиОбработчиков Или ЭтоВнешняяОбработка Тогда
			ВызватьИсключение(Нстр("ru='Загрузка данных в модели сервиса может выполняться только в безопасном режиме.';uk='Завантаження даних в моделі сервісу може виконуватися тільки в безпечному режимі.'"));
		КонецЕсли;
	КонецЕсли;
