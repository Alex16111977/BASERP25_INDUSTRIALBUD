...
Процедура Автозаполнение() Экспорт
	...
	Если ЗаполнятьДокументыБезУчетаИерархииПодразделений Тогда
		УсловиеНаПодразделения = " РаботникиСрезПоследних.Подразделение В (&парамПодразделение)"; 
	Иначе	
		УсловиеНаПодразделения = " РаботникиСрезПоследних.Подразделение В ИЕРАРХИИ (&парамПодразделение)"; 
	КонецЕсли;	
	Если ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Аванс Тогда
		...
		?(Подразделение.Пустая(),"",Символы.ПС + " И " + УсловиеНаПодразделения ) + "
		|
		|УПОРЯДОЧИТЬ ПО
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Зарплата к выплате ';uk='Зарплата до виплати '")));
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Печать",НСтр("ru='Взаиморасчеты с работниками';uk='Взаєморозрахунки з працівниками'"));
