...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НетФизлица Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не выбран сотрудник!';uk='Не обраний співробітник!'"), Отказ, Заголовок);
	КонецЕсли;

	// Результат
	НетРезультата = НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Результат);
	Если НетРезультата Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан результат!';uk='Не вказаний результат!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// ДатаИзменения
	НетДатыИзменения = НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаИзменения);
	Если НетРезультата Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана дата изменения!';uk='Не вказана дата зміни!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если Не ВыборкаПоШапкеДокумента.КандидатПринятНаИспытательныйСрок Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Кандидат не принят на испытательный срок!';uk='Кандидат не прийнятий на випробувальний строк!'"), Отказ, Заголовок);
		Возврат;
	КонецЕсли;

	Если ВыборкаПоШапкеДокумента.ЕстьРешение Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='По кандидату на дату регистрации уже присутствует результат!';uk='По кандидату на дату реєстрації вже є результат!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.ПланируемоеСобытие <> NULL и ВыборкаПоШапкеДокумента.Результат <> Перечисления.РезультатыИспытательногоСрока.Положительный Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='На дату документа по кандидату уже запланировано событие: ';uk='На дату документа по кандидату вже запланована подія: '") + ВыборкаПоШапкеДокумента.ПланируемоеСобытие, Отказ, Заголовок);
	КонецЕсли;
