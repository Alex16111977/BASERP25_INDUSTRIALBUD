	...
	Если  ИмяМакета = "Диаграмма" Тогда
		...
		Если Выборка.Следующий() Тогда
			Если Выборка.РаботниковВОтпуске > 0 Тогда // будем строить отчет 
				...
				Отчет.ВидОтчета = НСтр("ru='Фактические отпуска';uk='Фактичні відпустки'");
			...
			Иначе
				Предупреждение(НСтр("ru='В документе не зарегистрировано ни одного отпуска!';uk='В документі не зареєстровано жодної відпустки!'"));
			КонецЕсли;
		Иначе
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Диаграмма",НСтр("ru='Диаграмма отпусков';uk='Діаграма відпусток'"));
...
Процедура ПроверитьЗаполнениеСтрокиРаботника(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НетДатыНачала Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата начала отсутствия на работе!';uk='не вказана дата початку відсутності на роботі!'"), Отказ, Заголовок);
	КонецЕсли;

	// ПричинаОтсутствия
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ПричинаОтсутствия) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана причина отсутствия!';uk='не вказана причина відсутності!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончания) и ВыборкаПоСтрокамДокумента.ДатаНачала > ВыборкаПоСтрокамДокумента.ДатаОкончания Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата начала отсутствия не может превышать дату окончания!';uk='дата початку відсутності не може перевищувати дату закінчення!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер <> NULL Тогда
		СтрокаСообщениеОбОшибке = НСтр("ru='период отсутствия пересекается с периодом строки ';uk='період відсутності перетинається з періодом рядка '") + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер + "!"; 
