...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если ВыборкаПоСтрокамДокумента.ОшибкаДублирование Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='по сотруднику уже зарегистрировано изменение параметров индексации другим документом!';uk='по співробітнику вже зареєстрована зміна параметрів індексації іншим документом!'"), Отказ);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиНачисления(ВыборкаПоСтрокамДокумента, ТЧ, Отказ, Заголовок = "")
	...
	Если ТЧ = "НачисленияДо" Тогда
		ИмяТЧ = НСтр("ru='Начисления ""до""';uk='Нарахування ""до""'");
	Иначе	
	    ИмяТЧ = НСтр("ru='Начисления ""после""';uk='Нарахування ""після""'");
	КонецЕсли;
