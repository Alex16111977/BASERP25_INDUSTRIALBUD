...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "ИсторияПогашения" Тогда
		...
		СекцияШапка.Параметры.Работник = Строка(Выборка.Физлицо)+ ?(Выборка.КодПоДРФО="",""," ("+Выборка.КодПоДРФО+")"); 

		Если  ПорядокПогашенияЗайма = Перечисления.ПорядокПогашенияЗаймаПроцентов.Ежемесячно Тогда
			Порядок = НСтр("ru='ежемесячно';uk='щомісяця'",КодЯзыкаПечать);
		Иначе
			Порядок = НСтр("ru='в конце срока';uk='в кінці строку'",КодЯзыкаПечать);
		КонецЕсли;	
		...
		СекцияШапка.Параметры.ИмяВалютыУчета = ?(ВалютаУчета.Код = "980","гривня",ВалютаУчета.Наименование);
		...
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Отчет о погашении займа';uk='Звіт про погашення позики'")));
	...
	ИначеЕсли ИмяМакета = "Договор"  Тогда
		...
		Макет.Параметры.ОкончаниеПогашения = Формат(ДобавитьМесяц(НачалоПогашения, СрокПогашения),"Л="+КодЯзыкаПечать+";ДЛФ=DD");
		Макет.Параметры.ДатаДоговора =  Формат(Дата,"Л="+КодЯзыкаПечать+";ДЛФ=DD");
		Если  ПорядокПогашенияЗайма = Перечисления.ПорядокПогашенияЗаймаПроцентов.Ежемесячно Тогда
			Макет.Параметры.ТекстПорядка = НСтр("ru='ежемесячно';uk='щомісяця'",КодЯзыкаПечать);
		Иначе
			Макет.Параметры.ТекстПорядка = НСтр("ru='в конце срока';uk='в кінці строку'",КодЯзыкаПечать);
		КонецЕсли;	
		ТабДокумент.Вывести(Макет);
		
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Договор займа';uk='Договір позики'")));
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ИсторияПогашения,Договор",НСтр("ru='Отчет о погашении займа';uk='Звіт про погашення позики'"), НСтр("ru='Договор займа';uk='Договір позики'"));
...
Процедура ПроверитьЗаполнениеШапки(Отказ, Заголовок)
	...
	Если СрокПогашения = 0 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан срок погашения займа!';uk='Не вказаний строк погашення позики!'"), Отказ, Заголовок);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(НачалоПогашения) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указано начало погашения займа!';uk='Не вказаний початок погашення позики!'"), Отказ, Заголовок);
	ИначеЕсли НачалоПогашения < НачалоМесяца(Дата) Тогда	
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Начало погашения займа не может предшествовать месяцу выдачи займа!';uk='Початок погашення позики не може бути раніше місяця видачі позики!'"), Отказ, Заголовок);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ВалютаДокумента) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана валюта займа!';uk='Не вказана валюта позики!'"), Отказ, Заголовок);
	ИначеЕсли КурсДокумента = 0 Тогда 
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан курс валюты займа!';uk='Не зазначений курс валюти позики!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапкиРегл(Отказ, Заголовок)
	...
	Если ОтражатьВБухгалтерскомУчете и ПроцентЗаПользованиеЗаймом > 0 и НЕ ЗначениеЗаполнено(СчетУчетаПроцентовПоЗайму) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан бухгалтерский счет учета процентов по займу!';uk='Не вказаний бухгалтерський рахунок обліку відсотків по позиці!'"), Отказ, Заголовок);
	КонецЕсли;
