...
Функция Печать(НазваниеМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Изменение штатного расписания';uk='Зміна штатного розкладу'")));
...
Процедура ПроверитьЗаполнениеСтрокиШтатнойЕдиницы(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КоличествоСтавок) и НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.БылоСтавок) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указано количество ставок по создаваемой штатной единице!';uk='не зазначена кількість ставок по створюваній штатній одиниці!'"), Отказ);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КоличествоСтавок) или НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.БылоСтавок) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидТарифнойСтавки) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан вид тарифной ставки!';uk='не зазначений вид тарифної ставки!'"), Отказ);
		КонецЕсли;
		
		// ГрафикРаботы
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ГрафикРаботы) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан график работы!';uk='не зазначений графік роботи!'"), Отказ);
		КонецЕсли;
		
		// Валюта
		Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВалютаТарифнойСтавки) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана валюта!';uk='не вказана валюта!'"), Отказ);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='в строке № ';uk='у рядку № '") + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока + НСтр("ru=' повторно описывается та же штатная единица!';uk=' повторно описується та ж штатна одиниця!'"), Отказ);
	КонецЕсли;
	
	// конфликтующий документ
	Если ВыборкаПоСтрокамДокумента.НайденоСуществующееДвижение <> Ложь Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='документом ';uk='документом '") + ВыборкаПоСтрокамДокумента.НайденоСуществующееДвижение + НСтр("ru=' штатная единица уже описана!';uk=' штатна одиниця вже описана!'"), Отказ);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиНадбавок(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Подразделение) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрано подразделение!';uk='не обраний підрозділ!'"), Отказ);
	ИначеЕсли ВыборкаПоСтрокамДокумента.ОшибкаПодразделениеНеПринадлежитОрганизации Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='указано подразделение, принадлежащее другой организации!';uk='вказано підрозділ, що належить іншій організації!'"), Отказ);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидНадбавки) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана надбавка!';uk='не обрана надбавка!'"), Отказ);
	КонецЕсли;
	...
	// повторяющиеся строки
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='в строке № ';uk='у рядку № '") + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока + НСтр("ru=' повторно описывается та же надбавка!';uk=' повторно описується та ж надбавка!'"), Отказ);
	КонецЕсли;
	
	// конфликтующий документ
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтныйДокумент) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='документом ';uk='документом '") + ВыборкаПоСтрокамДокумента.КонфликтныйДокумент + НСтр("ru='надбавка по штатной единице уже описана!';uk='надбавка по штатній одиниці вже описана!'"), Отказ);
	КонецЕсли;
