...
Процедура НапечататьПисьмо() Экспорт
	...
	ТекстОписанияЭлектронногоПисьма = НСтр("ru='Электронное письмо № ';uk='Електронний лист № '",КодЯзыкаПечать) + СокрЛП(Номер) + ", " + Дата + "<BR>" + ПредставлениеСтрокиДляHTML((СокрЛП(УчетнаяЗапись) + " <" + УправлениеЭлектроннойПочтой.ПолучитьЗначениеРеквизитаУчетнойЗаписи(УчетнаяЗапись, "АдресЭлектроннойПочты") + ">"));
	...
	Если ЗначениеЗаполнено(ЗаявкаКандидата) Тогда
		ТекстВыходногоДокумента = ТекстВыходногоДокумента + СтрокаТаблицы(НСтр("ru='Заявка кандидата:';uk='Заявка кандидата:'"), ПредставлениеСтрокиДляHTML(Строка(ЗаявкаКандидата)));
	КонецЕсли;
...
Функция ЗахватитьТекст() Экспорт
	...
	ИмяСохраненияФайла = РаботаСФайлами.ПолучитьИмяФайла(РаботаСФайлами.ПолучитьИмяКаталога(), (ИмяФайла + ".HTM"));
