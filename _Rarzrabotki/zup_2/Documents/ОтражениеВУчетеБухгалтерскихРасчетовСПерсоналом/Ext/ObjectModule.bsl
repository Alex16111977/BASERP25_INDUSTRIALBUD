...
Процедура СформироватьНачисленияРаботников(Отказ, КурсВалютыУпрУчета, ВалютаУпрУчета, Заголовок = "")
	...
	Пока Выборка.СледующийПоЗначениюПоля("ФизЛицо") Цикл
		...
		Если ОтработаноДней = 0 Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не заполнен рабочий график работника ';uk='Не заповнений робочий графік працівника '")+Выборка.ФизЛицоНаименование+НСтр("ru=' в управленческом учете!';uk=' в управлінському обліку!'"), Отказ, Заголовок);
...
Процедура СформироватьВыплатыРаботникам(Отказ, КурсВалютыУпрУчета)
	...
	|	
	|ГДЕ
	|	НЕ ЕСТЬNULL(УчетЗаработкаРаботников.УчетНачисленийПоОрганизации,ЛОЖЬ) И
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если КурсВалютыУпрУчета = 0 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не задан курс валюты управленческого учета!';uk='Не заданий курс валюти управлінського обліку!'"), Отказ, Заголовок);
		Возврат;
	КонецЕсли;
