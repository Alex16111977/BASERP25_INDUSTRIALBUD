...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Организация) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана организация, для которой ведется учет!';uk='Не зазначена організація, для якої ведеться облік!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаИзменения) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата изменения учета работника!';uk='не вказана дата зміни обліку працівника!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ПустойСпособОтраженияВБухучете  Тогда
		...
		Сообщить(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан способ отражения заработка работника в бухгалтерском учете!"
"Движение по этой строке производиться не будет.';uk='не вказаний спосіб відображення заробітку працівника в бухгалтерському обліку!"
"Рух по цьому рядку проводитися не буде.'"))
	КонецЕсли;
	
	// повторяющиеся строки		
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер)  Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='по работнику ';uk='по працівнику '") + ВыборкаПоСтрокамДокумента.ФизЛицоНаименование + НСтр("ru=' найдена повторяющаяся строка № ';uk=' знайдений рядок, що повторюється № '") + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер + "!", Отказ, Заголовок);
	КонецЕсли;
	
	// повторяющиеся строки		
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.РегистраторПредставление)  Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='способ учета основного заработка работника ';uk='спосіб обліку основного заробітку працівника '") + ВыборкаПоСтрокамДокумента.ФизЛицоНаименование + НСтр("ru=' уже зарегистрирован ранее документом ';uk=' вже зареєстрований раніше документом '") + ВыборкаПоСтрокамДокумента.РегистраторПредставление + "!", Отказ, Заголовок);
	КонецЕсли;
