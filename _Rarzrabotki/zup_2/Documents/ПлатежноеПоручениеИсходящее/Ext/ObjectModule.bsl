...
Функция ПечатьПлатежногоПоручения(Вариант2009) Экспорт
	...
	СведенияОПолучателе = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(?(ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПереводНаДругойСчет, Организация, Контрагент), Дата,, СчетКонтрагента, "uk");
	...
	Обл.Параметры.Получатель              = ?(ПустаяСтрока(ТекстПолучателя), СведенияОПолучателе.ПолноеНаименование, СокрЛП(ТекстПолучателя));
	Обл.Параметры.ПолучательЕДРПОУ        = УправлениеКонтактнойИнформацией.ПолучитьКодОрганизации(СведенияОПолучателе);
