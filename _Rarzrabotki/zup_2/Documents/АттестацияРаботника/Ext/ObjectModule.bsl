...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если      ИмяМакета = "ОценочныйЛист" Тогда
		...
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Оценочный лист';uk='Оцінний лист'")));
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ОценочныйЛист",НСтр("ru='Оценочный лист';uk='Оцінний лист'"));
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ФизЛицо) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан Работник!';uk='Не вказаний Працівник!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиОценки(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
		...
		СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
										НСтр("ru='"" табл. части ""Список оцениваемых компетенций"": ';uk='"" табл. частини ""Список оцінюваних компетенцій"": '");
		// ФизЛицо
		ЕстьКомпетенция = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Компетенция);
		Если Не ЕстьКомпетенция Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана компетенция!';uk='не обрана компетенція!'"), Отказ, Заголовок);
		КонецЕсли;

		// ДатаНачала
        ЕстьОценка = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Оценка);
		Если Не ЕстьОценка Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана оценка!';uk='не обрана оцінка!'"), Отказ, Заголовок);
		КонецЕсли;

		// ДатаОкончания
        ЕстьДатаОценки = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОценки);
		Если Не ЕстьДатаОценки Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата оценки!';uk='не вказана дата оцінки!'"), Отказ, Заголовок);
		КонецЕсли;
		
	    Если ЕстьКомпетенция и ЕстьОценка и ЕстьДатаОценки Тогда
		  	
	    	Если ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока <> Null  Тогда
	    		СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru=' компетенция не может быть указана в документе дважды (см. строку ';uk=' компетенція не може бути вказана в документі двічі (див. рядок '")  + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока + " )!";
