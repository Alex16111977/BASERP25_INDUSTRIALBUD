...
Процедура Автозаполнение(ПостроительЗапроса = Неопределено, Сотрудник = Неопределено, ТекущаяСтрокаГрафикаРаботы = Неопределено) Экспорт
	...
	Если ПостроительЗапроса = Неопределено Тогда
		...
		Если Сотрудник = Неопределено Тогда
			...
			Если ЗаполнятьДокументыБезУчетаИерархииПодразделений Тогда
				УсловиеНаПодразделения = " И (РаботникиОрганизаций.ПодразделениеОрганизации В (&Подразделение) ИЛИ &Подразделение = &ПустоеПодразделение)"; 
			Иначе	
				УсловиеНаПодразделения = " И (РаботникиОрганизаций.ПодразделениеОрганизации В ИЕРАРХИИ (&Подразделение) ИЛИ &Подразделение = &ПустоеПодразделение)"; 
			КонецЕсли;	
			СотрудникиТекст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|	И РаботникиОрганизаций.ПричинаИзмененияСостояния <> ЗНАЧЕНИЕ(Перечисление.ПричиныИзмененияСостояния.Увольнение)" + УсловиеНаПодразделения + "
			|
			|ОБЪЕДИНИТЬ
			|
			|ВЫБРАТЬ
			...
			|	И РаботникиОрганизаций.ОбособленноеПодразделение = &ОбособленноеПодразделение " + УсловиеНаПодразделения;
	...
	Иначе
		...
		ПостроительЗапроса.Параметры.Вставить("ГоловнаяОрганизация", Организация);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов.Вставить("П13",	НСтр("ru='Форма П-13';uk='Форма П-13'"));
...
Функция СформироватьЗапросПоШапке()
	...
	|	ВводИндивидуальныхГрафиковРаботыОрганизации.Организация КАК ГоловнаяОрганизация,
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПериодРегистрации) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не задан период индивидуального графика!';uk='Не заданий період індивідуального графіка!'"), Отказ, Заголовок);
	КонецЕсли;
	
	 //  Организация
	 Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ГоловнаяОрганизация) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана организация, по которой вводится индивидуальный график!';uk='Не вказана організація, по якій вводиться індивідуальний графік!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиГрафикРаботы(ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									НСтр("ru='"" табл. части ""График работы"": ';uk='"" табл. частини ""Графік роботи"": '");
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Назначение) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрано назначение работника!';uk='не вибрано призначення співробітника!'"), Отказ, Заголовок);
	КонецЕсли;
			...
			Иначе
				...
				Пока ВыборкаПоГрафикуРаботы.Следующий() Цикл
					...
					Если НЕ Отказ Тогда
						...
						НаборГрафикиРаботыПоВидамВремени.Отбор.Сотрудник.Установить(ВыборкаПоГрафикуРаботы.Назначение);
						НаборГрафикиРаботыПоВидамВремени.Отбор.План.Установить(Истина);
						НаборГрафикиРаботыПоВидамВремени.Отбор.Месяц.Установить(ВыборкаПоШапкеДокумента.ПериодРегистрации);
