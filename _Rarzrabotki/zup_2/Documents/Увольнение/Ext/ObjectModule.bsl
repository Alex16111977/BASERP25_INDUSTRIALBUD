	...
	Если ТипОснования = Тип("ДокументСсылка.РезультатИспытательногоСрока") Тогда
		...
		Если Выборка.Следующий() Тогда
			Если Выборка.УжеУволен Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Сотрудник ';uk='Співробітник '") + Выборка.Сотрудник + НСтр("ru=' уже уволен документом ';uk=' вже звільнений документом '") + Выборка.ДокументУвольнения);
			Иначе
