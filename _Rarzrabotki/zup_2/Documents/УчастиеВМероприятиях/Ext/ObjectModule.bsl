...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Мероприятие) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указано мероприятие!';uk='Не вказаний захід!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиРаботники(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ЧастьМероприятия) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана составная часть мероприятия, в которой участвует работник!';uk='не вказана складова частина заходу, в якій бере участь працівник!'"), Отказ, Заголовок);
	ИначеЕсли ВыборкаПоСтрокамДокумента.НевернаяЧастьМероприятия Тогда
		СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru=' указанная часть мероприятия не принадлежит мероприятию ""';uk=' зазначена частина заходу не належить заходу ""'") + ВыборкаПоШапкеДокумента.Мероприятие + """!";
	...
	Если ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока <> NULL Тогда
		СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru=' найдена пересекающаяся строка №';uk=' знайдений рядок, що перетинається №'") + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрока;
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + СтрокаПродолжениеСообщенияОбОшибке, Отказ, Заголовок);
	КонецЕсли;
