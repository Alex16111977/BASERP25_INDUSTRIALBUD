...
Процедура ВывестиСтроку(Выборка, СтруктураПараметров, ТипЗаписиВыборки, ИмяГруппировки)
	...
	ИначеЕсли ТипЗаписиВыборки = ТипЗаписиЗапроса.ОбщийИтог Тогда
		ЗначениеГруппировки = НСтр("ru='ИТОГО: ';uk='РАЗОМ: '",мКодЯзыкаПечать);
	КонецЕсли;
...
Функция ПроверитьКорректностьНастройкиОтчета(Отчет)
	...
	Для каждого Группировка Из Отчет.ГруппировкиОтчета Цикл
		Если Отчет.мТаблицаГруппировки.Найти(Группировка.ИмяГруппировки, "ИмяПоля") = Неопределено Тогда
			СтрОшибки = СтрОшибки + Символы.ПС + Символы.Таб + НСтр("ru=' Задана неверная группировка: ';uk=' Задана невірне групування: '")+Группировка.ПредставлениеГруппировки;			
		КонецЕсли; 
	КонецЦикла; 
	...
	Для каждого Показатель Из Отчет.ПоказателиОтчета Цикл
		Если Отчет.мТаблицаПоказатели.Найти(Показатель.ИмяПоказателя, "ИмяПоля") = Неопределено Тогда
			СтрОшибки = СтрОшибки + Символы.ПС + Символы.Таб + НСтр("ru=' Задан неверный показатель: ';uk=' Задано невірний показник: '")+Показатель.ПредставлениеПоказателя;			
		КонецЕсли; 
	КонецЦикла; 

	Для каждого Отбор Из Отчет.ОтборыОтчета Цикл
		Если Отчет.мТаблицаФильтры.Найти(Отбор.ИмяОтбора, "ИмяПоля") = Неопределено Тогда
			СтрОшибки = СтрОшибки + Символы.ПС + Символы.Таб + НСтр("ru=' Задан неверный отбор: ';uk=' Задано невірний відбір: '")+Отбор.ПредставлениеОтбора;			
		КонецЕсли; 
	КонецЦикла; 

	Если СтрОшибки <> "" Тогда
		Сообщить(НСтр("ru='В настройке отчета обнаружены ошибки:';uk='У настройці звіту виявлені помилки:'")+СтрОшибки+Символы.ПС);
		Возврат Ложь;
	Иначе	
		Возврат Истина;
	КонецЕсли; 
...
Процедура СформироватьДокументРезультатОтчета(Отчет, ДокументРезультат, ФормироватьЗапрос = Истина, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0, ЕстьОшибки = Ложь, ТаблицаДиаграммы = "", ИмяСерииПредставление = "") Экспорт
	...
	Если Отчет.ПоказателиОтчета.Количество() = 0 Тогда
        Предупреждение(НСтр("ru='Не указан ни один показатель!';uk='Не вказаний жоден показник!'") + Символы.ПС + НСтр("ru='Отчет не сформирован!';uk='Звіт не сформовано!'"));
		ЕстьОшибки = Истина;
		Возврат
	КонецЕсли;
	...
	Если ФормироватьЗапрос Тогда
		Если НЕ ПроверитьКорректностьНастройкиОтчета(Отчет) Тогда
			Предупреждение(НСтр("ru='Отчет не сформирован. Отредактируйте настройку отчета!';uk='Звіт не сформований. Відредагуйте настройку звіту!'"));
			ЕстьОшибки = Истина;
			Возврат
		КонецЕсли; 
		СформированныйЗапрос = Отчет.СформироватьЗапрос();
		Если СформированныйЗапрос = Неопределено Тогда
			Сообщить(НСтр("ru='Запрошен слишком сложный отчет. Упростите отчет, уменьшив число группировок отчета или число выводимых показателей.';uk='Запитаний занадто складний звіт. Спростіть звіт, зменшивши число групувань звіту або число показників, що виводиться.'"));
			Возврат
		КонецЕсли; 
	...
	ИначеЕсли ВариантВизуализации = "ТабличныйДокумент" Тогда
		...
		Макет.КодЯзыкаМакета = мКодЯзыкаПечать;
...
Процедура ПечатьДиаграммы(ФормаОтчета) Экспорт
	...
	Если ТаблицаЛегенды.Количество()=0 Тогда
		Предупреждение(НСтр("ru='Отчет не сформирован!';uk='Звіт не сформовано!'"), 60);
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьОтчет(Отчет, ФормаОтчета, ФормироватьЗапрос = Истина, ВариантОтображенияОтчета) Экспорт
	...
	ИначеЕсли ВариантОтображенияОтчета = Перечисления.ВариантыОтображенияОтчетов.Диаграмма Тогда
		...
		Если Строки <> Неопределено Тогда
			...
			КонецЦикла;
		КонецЕсли;
		ЭлементыФормы.ТекстДиаграммы.Заголовок=?(ЗначениеЗаполнено(СписокПоказателей),НСтр("ru='Данные: ';uk='Дані: '") + Сред(СписокПоказателей,3),"");
...
Функция СформироватьЗаголовокОтчета(Отчет) Экспорт
	...
	Для Сч = 1 По 3 Цикл
		...
		Если Префикс = "Верх" Тогда
			ОбластьЗначение.Параметры.ЗаголовокОтчета = НСтр("ru='Статистика кадров предприятия';uk='Статистика кадрів підприємства'",мКодЯзыкаПечать) + ?(Отчет.мНастройка <> Неопределено, Символы.ПС + " (" + Отчет.мНастройка.Наименование + ")","");
			
		ИначеЕсли Префикс = "Середина" Тогда
			ОбластьЗначение.Параметры.Фильтры = НСтр("ru='Отбор: ';uk='Відбір: '",мКодЯзыкаПечать) + СтрФильтры;
		ИначеЕсли Префикс = "Низ" Тогда
			ОбластьЗначение.Параметры.Группировки = НСтр("ru='Группировки: ';uk='Групування: '",мКодЯзыкаПечать) + СтрГруппировки;
			ОбластьЗначение.Параметры.Показатели = НСтр("ru='Показатели: ';uk='Показники: '",мКодЯзыкаПечать) + СтрПоказатели;
		КонецЕсли;
...
Функция ПолучитьВидСравненияПоТексту(Знач СтрокаВидСравнения) Экспорт
	...
	Если Врег(СтрокаВидСравнения) = "РАВНО" ИЛИ Врег(СтрокаВидСравнения) = "ДОРІВНЮЄ" Тогда
		Возврат ВидСравнения.Равно;
	ИначеЕсли Врег(СтрокаВидСравнения) = "НЕ РАВНО" ИЛИ Врег(СтрокаВидСравнения) = "НЕ ДОРІВНЮЄ" Тогда
		Возврат ВидСравнения.НеРавно;
	ИначеЕсли Врег(СтрокаВидСравнения) = "МЕНЬШЕ" ИЛИ Врег(СтрокаВидСравнения) = "МЕНШЕ" Тогда
		Возврат ВидСравнения.Меньше;
	ИначеЕсли Врег(СтрокаВидСравнения) = "МЕНЬШЕ ИЛИ РАВНО" ИЛИ Врег(СтрокаВидСравнения) = "МЕНШЕ АБО ДОРІВНЮЄ" Тогда
		Возврат ВидСравнения.МеньшеИлиРавно;
	ИначеЕсли Врег(СтрокаВидСравнения) = "БОЛЬШЕ" ИЛИ Врег(СтрокаВидСравнения) = "БІЛЬШЕ" Тогда
		Возврат ВидСравнения.Больше;
	ИначеЕсли Врег(СтрокаВидСравнения) = "БОЛЬШЕ ИЛИ РАВНО" ИЛИ Врег(СтрокаВидСравнения) = "БІЛЬШЕ АБО ДОРІВНЮЄ" Тогда
		Возврат ВидСравнения.БольшеИлиРавно;
	ИначеЕсли Врег(СтрокаВидСравнения) = "ИНТЕРВАЛ (>, <)" ИЛИ Врег(СтрокаВидСравнения) = "ІНТЕРВАЛ (>, <)" Тогда
   	...
   	ИначеЕсли Врег(СтрокаВидСравнения) = "СОДЕРЖИТ" ИЛИ Врег(СтрокаВидСравнения) = "МІСТИТЬ" Тогда
		Возврат ВидСравнения.Содержит;
   	ИначеЕсли Врег(СтрокаВидСравнения) = "НЕ СОДЕРЖИТ" ИЛИ Врег(СтрокаВидСравнения) = "НЕ МІСТИТЬ" Тогда
		Возврат ВидСравнения.НеСодержит;
   	ИначеЕсли Врег(СтрокаВидСравнения) = "В СПИСКЕ" ИЛИ Врег(СтрокаВидСравнения) = "В СПИСКУ" тогда
		Возврат ВидСравнения.ВСписке;
   	ИначеЕсли Врег(СтрокаВидСравнения) = "В СПИСКЕ ПО ИЕРАРХИИ" ИЛИ Врег(СтрокаВидСравнения) = "В СПИСКУ ПО ІЄРАРХІЇ" Тогда
		Возврат ВидСравнения.ВСпискеПоИерархии;
   	ИначеЕсли Врег(СтрокаВидСравнения) = "НЕ В СПИСКЕ" ИЛИ Врег(СтрокаВидСравнения) = "НЕ У СПИСКУ" Тогда
		Возврат ВидСравнения.НеВСписке;
   	ИначеЕсли Врег(СтрокаВидСравнения) = "НЕ В СПИСКЕ ПО ИЕРАРХИИ" ИЛИ Врег(СтрокаВидСравнения) = "НЕ В СПИСКУ ПО ІЄРАРХІЇ" тогда
		Возврат ВидСравнения.НеВСпискеПоИерархии;
   	ИначеЕсли Врег(СтрокаВидСравнения) = "ВСЕ ИЗ" ИЛИ Врег(СтрокаВидСравнения) = "ВСІ З" тогда
		Возврат НСтр("ru=' Все из ';uk=' Всі з '");
	Иначе
		Сообщить(НСтр("ru='По тексту ';uk='По тексту '")+СтрокаВидСравнения+ НСтр("ru=' процедурой ПолучитьВидСравненияПоТексту не удалось получить вид сравнения!';uk=' процедурою ПолучитьВидСравненияПоТексту не вдалося отримати вид порівняння!'"));
		Возврат " = ";
	КонецЕсли;	 
...
Функция СтатистикаСпискиПолучитьОписаниеВсехПолей(ОсновнойИсточник, СписокРегистровСведений, ТаблицаПараметрыЗапроса) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРегистровСведений Цикл
		...
		Если ЭлементСписка.Значение = "РаботникиОрганизаций" Тогда
			...
			СтрокаТЗ.ПредставлениеПоля = НСтр("ru='Организация';uk='Організація'",мКодЯзыкаПечать);
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Количество работников';uk='Кількість працівників'",мКодЯзыкаПечать);
			...
			НовоеОписаниеПоля.ПредставлениеПоля =  НСтр("ru='Кол-во работающих по основному месту';uk='Кіл-ть працюючих за основним місцем'",мКодЯзыкаПечать);
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во работающих по внешнему совместительству';uk='Кіл-ть працюючих по зовнішньому сумісництву'",мКодЯзыкаПечать);
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во работающих по внутреннему совместительству';uk='Кількість працюючих по внутрішньому сумісництву'",мКодЯзыкаПечать);;
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Количество мужчин';uk='Кількість чоловіків'",мКодЯзыкаПечать);
			...
			НовоеОписаниеПоля.ПредставлениеПоля =  НСтр("ru='Количество женщин';uk='Кількість жінок'",мКодЯзыкаПечать);
		...
		ИначеЕсли ЭлементСписка.Значение = "СостояниеРаботниковОрганизаций" Тогда
			...
			Для Каждого Значение ИЗ МетаданныеПеречисления.ЗначенияПеречисления цикл
				...
				НовоеОписаниеПоля.ПредставлениеПоля =  НСтр("ru='Имеют состояние: ';uk='Мають стан: '",мКодЯзыкаПечать)+Значение.Представление();
	...
	НоваяСтрока = ТаблицаОписаниеВсехПолей.Добавить();
	НоваяСтрока.ИмяИсточника = ИмяРегистра;
	НоваяСтрока.ПредставлениеПоля = НСтр("ru='Наименование';uk='Найменування'",мКодЯзыкаПечать);
	...
	НоваяСтрока = ТаблицаОписаниеВсехПолей.Добавить();
	НоваяСтрока.ИмяИсточника = ИмяРегистра;
	НоваяСтрока.ПредставлениеПоля = НСтр("ru='ФИО (полностью)';uk='ПІБ (повністю)'",мКодЯзыкаПечать);
	...
	НоваяСтрока = ТаблицаОписаниеВсехПолей.Добавить();
	НоваяСтрока.ИмяИсточника = ИмяРегистра;
	НоваяСтрока.ПредставлениеПоля = НСтр("ru='Фамилия И.О.';uk='Прізвище І.Б.'",мКодЯзыкаПечать);
	...
	НоваяСтрока = ТаблицаОписаниеВсехПолей.Добавить();
	НоваяСтрока.ИмяИсточника = ИмяРегистра;
	НоваяСтрока.ПредставлениеПоля = НСтр("ru='Возраст';uk='Вік'",мКодЯзыкаПечать);
	...
	НоваяСтрока = ТаблицаОписаниеВсехПолей.Добавить();
	НоваяСтрока.ИмяИсточника = ИмяРегистра;
	НоваяСтрока.ПредставлениеПоля = НСтр("ru='Семейное положение';uk='Сімейний стан'",мКодЯзыкаПечать);
	...
	НоваяСтрока = ТаблицаОписаниеВсехПолей.Добавить();
	НоваяСтрока.ИмяИсточника = ИмяРегистра;
	НоваяСтрока.ПредставлениеПоля = НСтр("ru='Возраст родственника';uk='Вік родича'",мКодЯзыкаПечать);
	...
	Пока ВыборкаНазначений.Следующий() цикл
		...
		Если НайдСтрока <> Неопределено тогда
			...
			НоваяСтрока.ПредставлениеПоля = НайдСтрока.ПредставлениеПоля+НСтр("ru=' . Категории';uk=' . Категорії'",мКодЯзыкаПечать);
	...
	Пока Выборка.Следующий() цикл
		...
		Если Выборка.ИмяСправочника = "СемейноеПоложениеФизЛиц" тогда
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Имеют семейное положение: ';uk='Мають сімейний стан: '",мКодЯзыкаПечать)+Выборка.Представление;
		...
		ИначеЕсли Выборка.ИмяСправочника = "ВидыОбразованияФизЛиц" тогда
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Имеют образование: ';uk='Мають освіту: '",мКодЯзыкаПечать)+Выборка.Представление;
		...
		ИначеЕсли Выборка.ИмяСправочника = "СтепениЗнанияЯзыка" тогда	
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Владеют иностранными языками: ';uk='Володіють іноземними мовами: '",мКодЯзыкаПечать)+Выборка.Представление;
		...
		ИначеЕсли Выборка.ИмяСправочника = "СтепениРодстваФизЛиц" тогда				
			...
			НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во родственников по степени: ';uk='Кіл-ть родичів по ступені: '",мКодЯзыкаПечать)+Выборка.Представление;
	...
	НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во записей об образовании';uk='Кіл-ть записів про освіту'",мКодЯзыкаПечать);
	НовоеОписаниеПоля.ОписаниеПоля      = "1";
	НовоеОписаниеПоля.ИтоговоеПоле	    = ИСТИНА;
	НовоеОписаниеПоля.ТипПоля   		= "Вычисляемое";
	НовоеОписаниеПоля.ОписаниеТипов     = ОписаниеТиповЧисло;
	...
	НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во записей об иностранных языках';uk='Кіл-ть записів про іноземні мови'",мКодЯзыкаПечать);
	НовоеОписаниеПоля.ОписаниеПоля      = "1";
	НовоеОписаниеПоля.ИтоговоеПоле	    = ИСТИНА;
	НовоеОписаниеПоля.ТипПоля   		= "Вычисляемое";
	НовоеОписаниеПоля.ОписаниеТипов     = ОписаниеТиповЧисло;
	...
	НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во записей в трудовой книжке';uk='Кіл-ть записів у трудовій книжці'",мКодЯзыкаПечать);
	НовоеОписаниеПоля.ОписаниеПоля      = "1";
	НовоеОписаниеПоля.ИтоговоеПоле	    = ИСТИНА;
	НовоеОписаниеПоля.ТипПоля   		= "Вычисляемое";
	НовоеОписаниеПоля.ОписаниеТипов     = ОписаниеТиповЧисло;
	...
	НовоеОписаниеПоля.ПредставлениеПоля = НСтр("ru='Кол-во членов семьи';uk='Кількість членів сім'ї'",мКодЯзыкаПечать);
	НовоеОписаниеПоля.ОписаниеПоля      = "1";
	НовоеОписаниеПоля.ИтоговоеПоле	    = ИСТИНА;
	НовоеОписаниеПоля.ТипПоля   		= "Вычисляемое";
	НовоеОписаниеПоля.ОписаниеТипов     = ОписаниеТиповЧисло;
...
Функция СтатистикаСпискиПолучитьТекстИсточникаПоИмени(Отчет, Источник, КопияИнтервалыГруппировокОтчета, ТаблицаИсточниковСвойствКатегорий) Экспорт
	...
	Иначе// Текст запроса для остальных регистров сведений
		...
		Если МетаданныеРегистра = Неопределено Тогда
			Предупреждение(НСтр("ru='Функция 'отПолучитьТекстИсточникаПоИмени'. Ошибка: неверное имя регистра сведений: '") + ИмяИсточника + "!");
			Возврат "";
		КонецЕсли;	
    ...
    Если ПустаяСтрока(ТекстИсточника) Тогда
		Предупреждение(НСтр("ru='Функция 'отПолучитьТекстИсточникаПоИмени'. Ошибка: не предусмотренный источник данных: '") + ИмяИсточника + "!");
		Возврат "";
	КонецЕсли; 
...
Процедура ЗаполнитьНачальныеНастройки( ВосстановлениеНастройки = Ложь) Экспорт
	...
	Если ВидОтчета  = НСтр("ru='Статистика кадров организации';uk='Статистика кадрів організації'") Тогда
...
мКодЯзыкаПечать = ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов();


мМассивВидыОтчета = Новый Массив;
мМассивВидыОтчета.Добавить(НСтр("ru='Статистика кадров предприятия';uk='Статистика кадрів підприємства'"));
мМассивВидыОтчета.Добавить(НСтр("ru='Статистика кадров организации';uk='Статистика кадрів організації'"));
