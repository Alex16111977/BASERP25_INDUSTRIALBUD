...
Процедура ПечатьФорма2009(ТабДокумент, ДатаОтчета, Сотрудник, ОрганизацияОтчета, РаботникОтчета, ПустойБланк = Ложь) Экспорт
	...
	//Шапка	
	ОбластьШапка.Параметры.Дата = Формат(ДатаОтчета,"ДФ=""дд.ММ.гггг""");
	ОбластьШапка.Параметры.НазваниеОрганизации = ВРег(ОрганизацияОтчета.НаименованиеПолное);
	...
	Если ЗначениеЗаполнено(РезультатыОсновныеСведения.ДокументУвольнения) Тогда
		...
		Если СтрокаТЧ = Неопределено Тогда
			Основание = "";
		Иначе
			Основание = ", "+СтрокаТЧ.СтатьяКЗОТ.СтатьяЗакона;
	...
	//Отпуска
	номер =0;
	Пока РезультатыОтпуска.Следующий() И номер < 10 Цикл
		номер = 1 + номер;
...
Процедура ПечатьФорма1995(ТабДокумент, ДатаОтчета,Сотрудник, ОрганизацияОтчета, РаботникОтчета, ПустойБланк=Ложь) Экспорт
	...
	Если Не ПустойБланк Тогда
		Пока РезультатыОтпуска.Следующий() Цикл
			...
			Если ТипЗнч(РезультатыОтпуска.ВидОтпуска) = Тип("Строка") Тогда
				ОбластьОтпускаСтрока.Параметры.ВидОтпуска = РезультатыОтпуска.ВидОтпуска;
			Иначе	
