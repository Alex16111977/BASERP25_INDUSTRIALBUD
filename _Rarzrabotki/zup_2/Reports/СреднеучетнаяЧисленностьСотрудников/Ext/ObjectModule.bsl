...
Функция ПолучитьСотрудников( УсловиеНаФизлицо = "" )
	...
	|			 ИЛИ (Состояния.ПериодЗавершения <> ДАТАВРЕМЯ(1,1,1) И Состояния.ПериодЗавершения < &парамНачало))
	|			" + УсловиеНаФизлицо + "
	|
	|			ОБЪЕДИНИТЬ ВСЕ
	|			ВЫБРАТЬ
    ...
    |             ВЫБОР КОГДА Состояния.ПериодЗавершения = ДАТАВРЕМЯ(1,1,1) 
	|		 		ТОГДА Состояния.Период
	|				ИНАЧЕ Состояния.ПериодЗавершения 
	|			 КОНЕЦ КАК Период,
    ...
    |               ПО ((СостоянияДо.Период < Состояния.Период) ИЛИ (СостоянияДо.Период = Состояния.Период И Состояния.ПериодЗавершения <> ДАТАВРЕМЯ(1,1,1)))
	|				 И Состояния.Сотрудник = СостоянияДо.Сотрудник
    |            ГДЕ ВЫБОР КОГДА Состояния.ПериодЗавершения = ДАТАВРЕМЯ(1,1,1) 
	|		 		ТОГДА Состояния.Состояние
	|				ИНАЧЕ Состояния.СостояниеЗавершения 
	|			 	КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Работает)
	|				И	ВЫБОР КОГДА Состояния.ПериодЗавершения = ДАТАВРЕМЯ(1,1,1) 
	|				 		ТОГДА Состояния.Период
	|						ИНАЧЕ Состояния.ПериодЗавершения 
	|					 КОНЕЦ  >= &парамНачало
	|				И	ВЫБОР КОГДА Состояния.ПериодЗавершения = ДАТАВРЕМЯ(1,1,1) 
	...
	|						ИНАЧЕ Состояния.ПериодЗавершения 
	|					 КОНЕЦ  <= &парамКонец
    |            СГРУППИРОВАТЬ ПО Состояния.Сотрудник, ВЫБОР КОГДА Состояния.ПериодЗавершения = ДАТАВРЕМЯ(1,1,1) 
	|		 		ТОГДА Состояния.Период
	|				ИНАЧЕ Состояния.ПериодЗавершения 
	|			 КОНЕЦ 
    |            )КАК Состояния
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ
    |       	РегистрСведений.СостояниеРаботниковОрганизаций КАК СостоянияВместо
    |       	ПО СостоянияВместо.Сотрудник = СостоянияВместо.Сотрудник 
    |        	И СостоянияВместо.Период = Состояния.Период
	|			И	(СостоянияВместо.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускПоБеременностиИРодам)
	|			 ИЛИ СостоянияВместо.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускПоУходуЗаРебенком)
	|			 ИЛИ СостоянияВместо.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован))
	|		ГДЕ	Работники.ПричинаИзмененияСостояния <> ЗНАЧЕНИЕ(Перечисление.ПричиныИзмененияСостояния.Увольнение)                                    
	|		И	Работники.Сотрудник.ВидЗанятости В (&парамМестоРаботы)
	...
	|       И СостоянияВместо.Период ЕСТЬ NULL
    |
	|			" + УсловиеНаФизлицо + "
	|
	|   	) КАК РаботникиНачало
