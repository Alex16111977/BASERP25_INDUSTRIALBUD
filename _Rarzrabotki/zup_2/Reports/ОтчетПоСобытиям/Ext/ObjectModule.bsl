...
Функция СформироватьСтрокуПериода()
	...
	Иначе
		...
		Если ДатаНачала = '00010101000000' ИЛИ ДатаОкончания = '00010101000000' Тогда
			...
			ОписаниеПериода = НСтр("ru='Период: ';uk='Період: '",КодЯзыкаПечать) + Формат(ДатаНачала, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+"; ДФ = ""дд.ММ.гггг""; ДП = ""...""") 
							+ " - "      + Формат(ДатаОкончания, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+"; ДФ = ""дд.ММ.гггг""; ДП = ""...""");
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если НЕ ПустаяСтрока(СтрокаГруппировок) Тогда
		СтрокаГруппировок = НСтр("ru='Группировки строк: ';uk='Групування рядків: '" ,КодЯзыкаПечати) + СтрокаГруппировок;
	...
	Если НЕ ПустаяСтрока(СтрокаОтборов) Тогда
		СтрокаОтборов = НСтр("ru='Отбор: ';uk='Відбір: '" ,КодЯзыкаПечати) + СтрокаОтборов;
	...
	Если НЕ ПустаяСтрока(СтрокаПорядка) Тогда
		СтрокаПорядка = НСтр("ru='Сортировка: ';uk='Сортування: '" ,КодЯзыкаПечати) + СтрокаПорядка;
			...
			Иначе
				...
				Если Событие.ВидСобытия = Перечисления.ВидыСобытий.Прочее Тогда
					...
					Иначе
						...
						СтрокаВывода = СтрокаВывода + НСтр("ru='Прочее событие: ';uk='Інша подія: '",КодЯзыкаПечать);
