...
Функция ПолеЗапросаПоПредставлению(Показатель)
	...
	Если ВидОтчета = "ОрганизационнаяДиаграмма" Тогда
		Если (Показатель.Представление = НСтр("ru='Пол';uk='Стать'",КодЯзыкаПечать))
		 или (Показатель.Представление = НСтр("ru='Дата рождения';uk='Дата народження'",КодЯзыкаПечать)) Тогда
		    Возврат "Работники.Сотрудник.Физлицо."+Показатель.Имя+" КАК "+Показатель.Имя;
		КонецЕсли;
		Если (Показатель.Представление =НСтр("ru='Фамилия';uk='Прізвище'",КодЯзыкаПечать))
		 или (Показатель.Представление =НСтр("ru='Имя';uk=""Ім'я""",КодЯзыкаПечать))
		 или (Показатель.Представление =НСтр("ru='Отчество';uk='По батькові'",КодЯзыкаПечать)) Тогда
		    Возврат "ФИОФизлиц."+Показатель.Имя+" КАК "+Показатель.Имя;
		КонецЕсли;
		Если (Показатель.Представление =НСтр("ru='Вид занятости';uk='Вид зайнятості'",КодЯзыкаПечать)) Тогда
		    Возврат "Работники.Сотрудник."+Показатель.Имя+" КАК "+Показатель.Имя;
		КонецЕсли;
		Если (Показатель.Представление =НСтр("ru='Табельный номер';uk='Табельний номер'",КодЯзыкаПечать)) Тогда
		    Возврат "Работники.Сотрудник.Код КАК "+Показатель.Имя;
		КонецЕсли;
		Если (Показатель.Представление =НСтр("ru='Фотография';uk='Фотографія'",КодЯзыкаПечать)) Тогда
...
Процедура СформироватьЗапросПоСотрудникам()
	...
	Для Каждого Строка Из Показатели цикл
		Если Не Строка.Использование Тогда
			Продолжить;
		КонецЕсли;
		Если Строка.Представление = НСтр("ru='Наименование';uk='Найменування'",КодЯзыкаПечать) Тогда
			Продолжить;
		КонецЕсли;	
		СтрокаПоказателей = СтрокаПоказателей +", " + ПолеЗапросаПоПредставлению(Строка);	
	КонецЦикла;
...
Функция СформироватьОбластьСотрудник(ФизЛицо, Подразделение, Должность, ОбластьВерх, ОбластьСередина, ОбластьБольшаяСередина, ОбластьФото, ОбластьНиз);
	...
	Пока Выборка.Следующий() Цикл
		Для Каждого Строка Из Показатели цикл
			...
			Если Строка.Представление = НСтр("ru='Фотография';uk='Фотографія'",КодЯзыкаПечать) Тогда
				...
				Продолжить;
			КонецЕсли;	
			Если (Строка.Представление = НСтр("ru='Наименование';uk='Найменування'",КодЯзыкаПечать)) ИЛИ (Строка.Представление = НСтр("ru='Фамилия';uk='Прізвище'",КодЯзыкаПечать)) ИЛИ (Строка.Представление = НСтр("ru='Имя';uk=""Ім'я""",КодЯзыкаПечать)) ИЛИ 
				(Строка.Представление = НСтр("ru='Отчество';uk='По батькові'",КодЯзыкаПечать)) ИЛИ (Строка.Представление = НСтр("ru='Должность';uk='Посада'",КодЯзыкаПечать)) Тогда
...
Процедура Печать(Дерево, ТабДокумент, ПоказыватьЗаголовок)
	...
	Если ВидОтчета = "СтруктураПодразделений" Тогда
		ОбластьЗаголовок.Параметры.ВидДиаграммы = НСтр("ru='Структура подразделений';uk='Структура підрозділів'",КодЯзыкаПечать);
	ИначеЕсли ВидОтчета = "СтруктураШтатногоРасписания" Тогда
		ОбластьЗаголовок.Параметры.ВидДиаграммы = НСтр("ru='Структура штатного расписания';uk='Структура штатного розкладу'",КодЯзыкаПечать);
	ИначеЕсли ВидОтчета = "ОрганизационнаяДиаграмма" Тогда
		ОбластьЗаголовок.Параметры.ВидДиаграммы = НСтр("ru='Организационная диаграмма';uk='Організаційна діаграма'",КодЯзыкаПечать);	
	КонецЕсли;	
...
Процедура СформироватьСписокПоказателей() Экспорт
	...
	ИначеЕсли ВидОтчета = "СтруктураШтатногоРасписания" Тогда
		...
		СтрПоказатели.Представление     = НСтр("ru='Количество ставок';uk='Кількість ставок'",КодЯзыкаПечать);
		СтрПоказатели.Имя     			= "КоличествоСтавок";
    	
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Представление     = НСтр("ru='Минимальная ставка';uk='Мінімальна ставка'",КодЯзыкаПечать);
		СтрПоказатели.Имя			    = "МинимальнаяТарифнаяСтавка";
		        
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Представление     = НСтр("ru='Максимальная ставка';uk='Максимальна ставка'",КодЯзыкаПечать);
		СтрПоказатели.Имя			    = "МаксимальнаяТарифнаяСтавка";
        
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Представление     = НСтр("ru='Вид тарифной ставки';uk='Вид тарифної ставки'",КодЯзыкаПечать);
		СтрПоказатели.Имя     			= "ВидТарифнойСтавки";
        
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Представление     = НСтр("ru='Валюта ставки';uk='Валюта ставки'",КодЯзыкаПечать);
		СтрПоказатели.Имя     			= "ВалютаТарифнойСтавки";
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Представление     = НСтр("ru='График работы';uk='Графік роботи'",КодЯзыкаПечать);
	...
	ИначеЕсли ВидОтчета = "ОрганизационнаяДиаграмма" Тогда	
		...
		СтрПоказатели.Представление     = НСтр("ru='Наименование';uk='Найменування'",КодЯзыкаПечать);
		СтрПоказатели.Использование     = Истина;
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "Должность";
        СтрПоказатели.Представление		= НСтр("ru='Должность';uk='Посада'",КодЯзыкаПечать);
		СтрПоказатели.Использование    = Истина;
		
		СтрПоказатели = Показатели.Добавить();
	    СтрПоказатели.Имя 				= "Данные";
	    СтрПоказатели.Представление 	= НСтр("ru='Фотография';uk='Фотографія'",КодЯзыкаПечать);
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя	            = "Фамилия";
        СтрПоказатели.Представление		= НСтр("ru='Фамилия';uk='Прізвище'",КодЯзыкаПечать);
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя	            = "Имя";
        СтрПоказатели.Представление		= НСтр("ru='Имя';uk=""Ім'я""",КодЯзыкаПечать);
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "Отчество";
		СтрПоказатели.Представление		= НСтр("ru='Отчество';uk='По батькові'",КодЯзыкаПечать);
				
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "ТабельныйНомер";
		СтрПоказатели.Представление     = НСтр("ru='Табельный номер';uk='Табельний номер'",КодЯзыкаПечать);
		
        СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "ЗанимаемыхСтавок";
        СтрПоказатели.Представление		= НСтр("ru='Занимаемых ставок';uk='Займаних ставок'",КодЯзыкаПечать);
		
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "ВидЗанятости";
        СтрПоказатели.Представление		= НСтр("ru='Вид занятости';uk='Вид зайнятості'",КодЯзыкаПечать);
				
		СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "Пол";
		СтрПоказатели.Представление		= НСтр("ru='Пол';uk='Стать'",КодЯзыкаПечать);
		
        СтрПоказатели = Показатели.Добавить();
		СтрПоказатели.Имя		        = "ДатаРождения";
        СтрПоказатели.Представление		= НСтр("ru='Дата рождения';uk='Дата народження'",КодЯзыкаПечать);
