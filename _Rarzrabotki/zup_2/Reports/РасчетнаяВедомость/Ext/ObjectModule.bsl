...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = истина) Экспорт
	...
	//ИВК Вова
	ЕсЛи РольДоступна("ОграничениеПоПодразделениям") Тогда
		НастройкиКД = КомпоновщикНастроек.ПолучитьНастройки();
		ПолеПодразделение = КомпоновщикНастроек.Настройки.Отбор.ДоступныеПоляОтбора.Элементы.Найти("Подразделение"); //поле отбора присутствует в СКД
		НовыйОтбор = НастройкиКД.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		НовыйОтбор.ЛевоеЗначение = ПолеПодразделение.Поле;
		спПодразделений = Новый Массив;
		Для каждого тСТрока из глЗначениеПеременной("глТекущийПользователь").ДоступныеПодразделения Цикл
			спПодразделений.Добавить(тСТрока.Подразделение);
		КонецЦикла;
		НовыйОтбор.ПравоеЗначение = спПодразделений;
