...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь, ОчищатьТабличныйДокумент = Истина) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДатаНач) Тогда
		...
		Сообщить(НСтр("ru='Не указана дата начала периода!';uk='Не зазначена дата початку періоду!'"), СтатусСообщения.Важное);
		Возврат
	...
	Если НЕ ЗначениеЗаполнено(ДатаКон) Тогда
		...
		Сообщить(НСтр("ru='Не указана дата окончания периода!';uk='Не зазначена дата закінчення періоду!'"), СтатусСообщения.Важное);
		Возврат
