...
Процедура РазбитьТабличныйДокументПоСтраницам(ТабличныйДокумент)
	...
	Состояние(НСТР("ru='Разделение расчетных листков по страницам ...  (Если вы хотите прервать вывод отчета, нажмите Ctrl+Break)';uk='Розподіл розрахункових листків по сторінках ... (Якщо ви хочете перервати виведення звіту, натисніть  Ctrl+Break)'"));
	...
	ОбластьПервойЯчейки = ТабличныйДокумент.НайтиТекст(НСтр("ru='Расчетный листок';uk='Розрахунковий листок'", ЯзыкПечати));
	...
	ОбластьПоследнейЯчейки = ТабличныйДокумент.НайтиТекст(НСтр("ru='Долг за ';uk='Борг за '", ЯзыкПечати));
	...
	ОбластьПервойЯчейки = ТабличныйДокумент.НайтиТекст(НСтр("ru='Расчетный листок';uk='Розрахунковий листок'", ЯзыкПечати), ОбластьСлед);
...
Процедура НастроитьПостроительЗапросов(ПостроительОтчетов, ВидРасчетногоЛистка) Экспорт
	...
	Если ВидРасчетногоЛистка = "Кратко" тогда
		...
		|		ВЫБОР
		|			КОГДА (НЕ Взаиморасчеты.Регистратор ССЫЛКА Документ.ЗарплатаКВыплатеОрганизаций И НЕ Взаиморасчеты.Регистратор ССЫЛКА Документ.ПриходныйКассовыйОрдер)
...
//		|	ФизЛицо,
	...
	Иначе
		...
		|			ПО НачисленияУдержания.ФизЛицо = РаботникиОрганизацийСрезПоследних.Физлицо
		|				И НачисленияУдержания.ПериодРегистрации = РаботникиОрганизацийСрезПоследних.ПериодРегистрации
		|				И РаботникиОрганизацийСрезПоследних.Организация = НачисленияУдержания.Организация) КАК НачисленияУдержания
		|{ГДЕ
		|	НачисленияУдержания.Сотрудник,
		|	НачисленияУдержания.ФизЛицо,
