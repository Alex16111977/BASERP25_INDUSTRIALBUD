...
Функция Печать() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидДокументаОбОбразовании) тогда	
		ОбластьМакета.Параметры.ВидДокументаОбОбразовании = НСтр("ru='документы не выдаются';uk='документи не видаються'",КодЯзыкаПечать)
	КонецЕсли;
	...
	Если ЗанятияКурса.Количество() > 0 Тогда
		...
		|	ЕСТЬNULL(КурсыОбученияЗанятияКурса.Занятие.ВидЗанятия.Наименование, """+НСтр("ru='форма обучения не указана';uk='форма навчання не вказана'",КодЯзыкаПечать)+""") КАК ФормаОбучения
		|ИЗ
