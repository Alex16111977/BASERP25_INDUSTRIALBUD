...
Процедура ПроверитьПравильностьЗаполненияПериодовСменыДляПятиШестидневки(Отказ, МассивОшибок = Неопределено)
	...
	Если ПериодыСмены.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Не указана продолжительность рабочего времени по дням недели!';uk='Не вказана тривалість робочого часу по днях тижня!'");
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(Выборка.НомерСтроки) + НСтр("ru='"" расписания работы: ';uk='"" розкладу роботи: '");
		
		Если Выборка.ДеньНедели.Пустая()  Тогда
			
			ТекстОшибки = СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан день недели!';uk='не зазначений день тижня!'");
			Если МассивОшибок = Неопределено Тогда
				ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, Отказ); 
			Иначе
				МассивОшибок.Добавить(ТекстОшибки);
		...
		Если НетВремениНачала Тогда
			...
			ТекстОшибки = СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указано начало работы!';uk='не вказаний початок роботи!'");
			Если МассивОшибок = Неопределено Тогда
				ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, Отказ); 
			Иначе
				МассивОшибок.Добавить(ТекстОшибки);
		...
		Если НетВремениОкончания Тогда
			...
			ТекстОшибки = СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указано окончания работы!';uk='не вказано закінчення роботи!'");
			Если МассивОшибок = Неопределено Тогда
				ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, Отказ); 
			Иначе
				МассивОшибок.Добавить(ТекстОшибки);
		...
		Если Выборка.ВремяНачала > Выборка.ВремяОкончания Тогда // эта проверка пока под вопросам - может помешать пользователям
			...
			ТекстОшибки = СтрокаНачалаСообщенияОбОшибке + НСтр("ru='неверно указано время работы!';uk='невірно вказаний час роботи!'");
			Если МассивОшибок = Неопределено Тогда
				ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, Отказ); 
			Иначе
				МассивОшибок.Добавить(ТекстОшибки);
		...
		Если Выборка.НомерСтрокиПересечения <> Null Тогда
			...
			ТекстОшибки = СтрокаНачалаСообщенияОбОшибке + НСтр("ru=' в строке ';uk=' у рядку '") + Выборка.НомерСтрокиПересечения + НСтр("ru=' обнаружен пересекающийся период работы!';uk=' виявлений період роботи, що перетинається!'");
			Если МассивОшибок = Неопределено Тогда
				ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, Отказ); 
			Иначе
				МассивОшибок.Добавить(ТекстОшибки);
...
Процедура ПроверитьПравильностьЗаполненияГрафикаРаботы(Отказ, ОткрыватьФормуЕслиНетОшибок = Ложь) Экспорт
	...
	Если НЕ ПроверитьСоответствиеТипаГрафикаИКоличествоРабочихДнейНедели() Тогда
		ТекстОшибки = НСтр("ru='Тип графика не соответствует количеству рабочих дней в расписании работы!';uk='Тип графіка не відповідає кількості робочих днів у розкладі роботи!'");
		МассивОшибок.Добавить(ТекстОшибки);
		Отказ = Истина;
	КонецЕсли;
	
	Если НЕ ПроверитьПравильностьЗаполненияПериодовДляПятиШестидневки() Тогда
		ТекстОшибки = НСтр("ru='Количество часов за неделю не совпадает с расчетным!';uk='Кількість годин за тиждень не співпадає з розрахунковою!'");
		МассивОшибок.Добавить(ТекстОшибки);
		Отказ = Истина;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГрафикПолногоРабочегоВремени) Тогда
		Если ГрафикПолногоРабочегоВремени.СокращенноеРабочееВремя Тогда
			ТекстОшибки = НСтр("ru='Нельзя в качестве графика полного рабочего времени указывать график сокращенного рабочего времени';uk='Не можна в якості графіка повного робочого часу вказувати графік скороченого робочого часу'");
			МассивОшибок.Добавить(ТекстОшибки);
			Отказ = Истина;
		КонецЕсли;
		
		Если Не СокращенноеРабочееВремя Тогда
			ТекстОшибки = НСтр("ru='График полного рабочего времени указывается только для сокращенного рабочего времени!';uk='Графік повного робочого часу вказується тільки для скороченого робочого часу!'");
			МассивОшибок.Добавить(ТекстОшибки);
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОткрыватьФормуЕслиНетОшибок ИЛИ (МассивОшибок.Количество()>0) Тогда
		...
		ОбработкаКомментариев.ДобавитьСообщение(НСтр("ru='Проверка календаря';uk='Перевірка календаря'"), Перечисления.ВидыСообщений.Раздел);
		Если МассивОшибок.Количество() = 0 Тогда
			ОбработкаКомментариев.ДобавитьСообщение(НСтр("ru='Ошибок не обнаружено';uk='Помилок не виявлено'"), Перечисления.ВидыСообщений.Информация);
		Иначе
...
Процедура АвтозаполнениеКалендаря(ДатаНачалаИнтервала, ДатаОкончанияИнтервала) Экспорт
	...
	Для инд = 0 по ДнейВИнтервале-1 Цикл
		...
		Если ВыходнойСУчетомПроизводственногоКалендаря(ВидДня, индДата, НомерПервогоВыходного, НомерВторогоВыходного) Тогда
		...
		ИначеЕсли ЭтоПятиШестидневка Тогда
			...
			Если УчитыватьПраздники И КоличествоЧасовВсего <> 0 И ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Предпраздничный 
				И НЕ СокращенноеРабочееВремя Тогда
