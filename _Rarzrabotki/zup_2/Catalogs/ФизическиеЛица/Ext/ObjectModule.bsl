...
Функция ПечатьДанныхФизЛица(ДатаАктуальности = Неопределено, ТабДокумент = Неопределено) Экспорт
	...
	Если НастройкаПравДоступа.ДоступнаРольКадровикаУпр() Тогда
		...
		|			ТОГДА """+НСтр("ru='Не работает (уволен)';uk='Не працює (звільнений)'",КодЯзыкаПечать)+"""
		|		ИНАЧЕ """+НСтр("ru='Работает';uk='Працює'",КодЯзыкаПечать)+"""
		|	КОНЕЦ КАК Состояние
		|ИЗ
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			|	ВЫБОР  КОГДА ОтсутствиеНаРаботеРаботники.ПричинаОтсутствия = &Командировка ТОГДА """+НСтр("ru='Командировка';uk='Відрядження'",КодЯзыкаПечать)+""" КОГДА ОтсутствиеНаРаботеРаботники.ПричинаОтсутствия = &Болезнь ТОГДА """+НСтр("ru='Болезнь';uk='Хвороба'",КодЯзыкаПечать)+""" ИНАЧЕ """+НСтр("ru='Отпуск';uk='Відпустка'",КодЯзыкаПечать)+""" КОНЕЦ КАК ВидОтсутствия
			|ИЗ
...
КонецФункции // ПечатьДанныхФизлица
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "ПечатьДанныхФизЛица" Тогда
		...
		Заголовок = НСтр("ru='Данные по: ';uk='Дані по: '") + Наименование;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	Возврат Новый Структура("ПечатьДанныхФизЛица",НСтр("ru='Данные физ. лица';uk='Дані фіз. особи'"));
