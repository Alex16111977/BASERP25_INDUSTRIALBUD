...
// 
Функция ПолучитьЗначенияРеквизитов(Ссылка, ИменаРеквизитов, ТолькоРазрешенные = Ложь) Экспорт 
	...
	Для Каждого КлючИЗначение Из СтруктураРеквизитов Цикл
		...
		Если ТолькоРазрешенные Тогда
			ТекстГдеРазрешенные = ТекстГдеРазрешенные + ?(ПустаяСтрока(ТекстГдеРазрешенные), " (НЕ ", " И (НЕ ") + ИмяПоля + ".Ссылка ЕСТЬ NULL)";
		КонецЕсли; 
	КонецЦикла;
	
	ТекстГдеРазрешенные = " И " + ТекстГдеРазрешенные;
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ 
		|" + ?(ТолькоРазрешенные, "РАЗРЕШЕННЫЕ ", "") + "
		|" + ТекстПолей + "
		|ИЗ
		|	" + Ссылка.Метаданные().ПолноеИмя() + " КАК ПсевдонимЗаданнойТаблицы
		|ГДЕ
		|	ПсевдонимЗаданнойТаблицы.Ссылка = &Ссылка
		|" + ?(ТолькоРазрешенные, ТекстГдеРазрешенные, "") + "
		|");
...
// 
Функция ПолучитьЗначениеРеквизита(Ссылка, ИмяРеквизита, ТолькоРазрешенные = Ложь) Экспорт	

	Результат = ПолучитьЗначенияРеквизитов(Ссылка, ИмяРеквизита, ТолькоРазрешенные);
