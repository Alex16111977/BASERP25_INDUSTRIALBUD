	...
	Для каждого ЭлементСписка Из СписокПоказателей Цикл
		...
		Если ИмяПоказателя = "ФИОРук" или ИмяПоказателя = "ИмяФАМИЛИЯРук" Тогда
			Если ИДКонфигурации() = "УТ" Тогда
				...
				ОтвЛица = РегистрыСведений.ОтветственныеЛицаОрганизации.СрезПоследних(КонецДня(ДатаЗначения), Данные);
	...
	Если Организация <> Неопределено тогда
		...
		Если ИДКонфигурации() = "ЗУП" Тогда
			...
			Если Исполнитель <> Неопределено тогда
				...
				|				ТОГДА "" "" + ПОДСТРОКА(ФИОФизЛицСрезПоследних.Отчество, 1, 1) + "".""
				|			ИНАЧЕ """"
				|		КОНЕЦ, РаботникиОрганизацииСрезПоследних.Сотрудник.Физлицо.Наименование) КАК ФИОПолное
				|ИЗ
				...
				|		        ИЛИ Сотрудник.ВидЗанятости ЕСТЬ NULL )
