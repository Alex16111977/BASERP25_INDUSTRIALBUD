...
Функция ОпределитьТипОграниченийПоПолю(ИмяЭлемента, МетаданныеОбъекта, ДляСправочника = Истина) 
	...
	Если ДляСправочника Тогда
		...
		Иначе
			...
			Иначе
				Если МетаданныеОбъекта.ТипКода = Метаданные.СвойстваОбъектов.ТипКодаСправочника.Строка или
					 МетаданныеОбъекта.ТипКода = Метаданные.СвойстваОбъектов.ТипКодаПланаВидовРасчета.Строка Тогда
					ТипЗначенияПоиска = Тип("Строка");
				Иначе
...
Функция ЗапроситьПродолжитьРаботуПриНекорректныхДанныхПоискаПоСтроке(СообщатьПользователюОбОшибкеВводаДанных)
	...
	Если СообщатьПользователюОбОшибкеВводаДанных Тогда
		ОтветНаВопрос = Вопрос((НСтр("ru='В элемент управления введены некорректные данные.';uk='У елемент керування введені некоректні дані.'") + Символы.ПС + "Продолжить?"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
	Иначе
