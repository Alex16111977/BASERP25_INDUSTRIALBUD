...
Процедура ВыполнитьПроверку(СообщатьОбОшибкахПериодов, УчитиыватьРегистратор)
	...
	Если СообщатьОбОшибкахПериодов Тогда
		Для Каждого Сообщение Из СообщенияОбОшибках Цикл
			СтрокаСообщениеОбОшибке = Сообщение.Физлицо + НСтр("ru=': Противоречие в периодах по основному месту работы';uk=': Протиріччя в періодах за основним місцем роботи'") + Символы.ПС+ "(" + 
			Формат(Сообщение.ДатаНачала, "ДФ='дд МММ гг ""г.""'") + " - " + 
			Формат(Сообщение.ДатаОкончания, "ДФ='дд МММ гг ""г.""'") + НСтр("ru=' и ';uk=' і '") + 
			Формат(Сообщение.ДатаНачалаПред, "ДФ='дд МММ гг ""г.""'") + " - " + 
			Формат(Сообщение.ДатаОкончанияПред, "ДФ='дд МММ гг ""г.""'") + ")";
			СтрокаСообщениеОбОшибке = СтрокаСообщениеОбОшибке + Символы.ПС + НСтр("ru='Документы, которые противоречат друг другу:';uk='Документи, які суперечать один одному:'");
