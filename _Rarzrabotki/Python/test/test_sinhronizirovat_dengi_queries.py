...
# --- Q6: ERP остатки с необязательным фильтром по счёту (v2) ---
def q6():
    assert Q1_ROWS is not None and Q1_ROWS.Количество() > 0, "Q1 пуст"
    acc = Q1_ROWS.Получить(0).БанковскийСчет
    q = erp.NewObject("Запрос")
    q.Text = """
    ВЫБРАТЬ
    	Т.БанковскийСчет КАК БанковскийСчет,
    ...
    q.SetParameter("КонецПериода", KON)
    # с фильтром
    q.SetParameter("ФильтрСчет", False)
    q.SetParameter("БанковскийСчет", acc)
    n_f = q.Execute().Выгрузить().Количество()
    # без фильтра
    q.SetParameter("ФильтрСчет", True)
    n_all = q.Execute().Выгрузить().Количество()
    assert n_f == 1, f"с фильтром рядків={n_f}, очікував 1"
    assert n_all == Q1_ROWS.Количество(), f"без фільтра {n_all} != Q1 {Q1_ROWS.Количество()}"
    return f"з фільтром={n_f}, без={n_all}"
