...
def schet_po_iban(iban):
    # среди дублей IBAN берём счёт с реальными оборотами за период
    q = erp.NewObject("Запрос")
    q.Text = """
    ВЫБРАТЬ ПЕРВЫЕ 1
    	Т.БанковскийСчет КАК Ссылка
    ИЗ
    	РегистрНакопления.ДенежныеСредстваБезналичные.Обороты(&Н, &К, , БанковскийСчет.НомерСчета = &НомерСчета) КАК Т
    УПОРЯДОЧИТЬ ПО
    	Т.СуммаРасход УБЫВ
    """
    q.SetParameter("НомерСчета", iban)
    q.SetParameter("Н", datetime(2026, 1, 1, 0, 0, 0))
    q.SetParameter("К", datetime(2026, 6, 10, 23, 59, 59))
    r = q.Execute().Выгрузить()
    assert r.Количество() == 1, f"счёт {iban} без оборотов не найден"
