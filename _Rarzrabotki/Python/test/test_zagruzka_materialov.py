...
def units_snapshot(erp):
    """Наименование карточки общего названия -> наименование единицы ('' если пусто)."""
    q = erp.NewObject("Запрос")
    q.Text = ("ВЫБРАТЬ ОН.Наименование КАК Н, ОН.Единица.Наименование КАК Ед "
              "ИЗ Справочник.ОбщиеНазванияНоменклатуры КАК ОН ГДЕ НЕ ОН.ЭтоГруппа")
    t = q.Execute().Выгрузить()
    return {str(t.Получить(i).Н): str(t.Получить(i).Ед or "").strip()
...
def main():
    ...
    print(f"ПРОТОКОЛ: Ошибка={res.Ошибка} Текст='{res.ТекстОшибки}' "
          ...
          f"ЗаполненоЕдиниц={res.ЗаполненоЕдиниц} ДозаполненоЕдиниц={res.ДозаполненоЕдиниц}")
    ...
    # Единицы карточек общих названий: новым — при создании, существующим — только если пустая.
    единицы_после = units_snapshot(erp)
    ...
    assert not затёртые, f"ЗАТЁРТЫ заполненные единицы: {list(затёртые.items())[:5]}"
    assert len(изменённые) == int(res.ДозаполненоЕдиниц), (
        f"дозаполнено карточек {len(изменённые)} != протокол {res.ДозаполненоЕдиниц}")
    print(f"ЕДИНИЦЫ: создано с единицей={res.ЗаполненоЕдиниц}, дозаполнено пустых={res.ДозаполненоЕдиниц}, "
          f"затёрто заполненных=0")

    # у строк ТЧ, где карточка с единицей, ссылочная Единица должна быть заполнена
    q = erp.NewObject("Запрос")
    q.Text = ("ВЫБРАТЬ КОЛИЧЕСТВО(*) КАК К ИЗ Справочник.СтруктураСебестоимости.Комплектующие КАК К "
              "ГДЕ К.Ссылка = &Р И К.Единица = ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка) "
              "И К.ОбщееНазвание.Единица <> ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)")
    q.УстановитьПараметр("Р", ref)
    sel = q.Execute().Выбрать()
    sel.Следующий()
    assert int(sel.К or 0) == 0, f"строк ТЧ без Единицы при заполненной карточке: {sel.К}"
