...
MAIN_TITLES = ["Сотрудник", "ИНН", "Приём (ЗУП)", "Увольнение (ЗУП)", "Расхождение дней (Казна − ЗУП)", "Контроль",
               "Казна: дни / часы", "Р (дн/ч)", "М (дн/ч)", "О", "Б", "С", "К", "Н", "В",
               "ЗУП: дни / часы (по орг. — норма)", "Отпуск ЗУП (дн)", "Больничный ЗУП (дн)", "Без оплаты ЗУП (дн)", "Начислено (ЗУП)"]
fails = 0
...
def main():
    ...
    linked = [r for r in emp if (num(r[c["Расхождение дней (Казна − ЗУП)"]]) or 0) != 0]
    check(all((num(r[c["Расхождение дней (Казна − ЗУП)"]]) or 0) == (stack(r[c["Казна: дни / часы"]])[0] or 0) - (stack(r[c["ЗУП: дни / часы (по орг. — норма)"]])[0] or 0) for r in linked),
          f"расхождение = дни Казна − дни ЗУП у всех с ненулевым расхождением ({len(linked)} строк)")
    ...
    if i_ch is not None:
        ...
        check(r[c["Приём (ЗУП)"]] == "23.07.2025" and r[c["Увольнение (ЗУП)"]] == "", "Чешко: приём 23.07.2025, не уволен")
        check(stack(r[c["Р (дн/ч)"]]) == (1.0, 8.0) and stack(r[c["М (дн/ч)"]]) == (11.0, 88.0) and r[c["О"]] == "0", "Чешко: буквы Казны Р 1/8, М 11/88, О 0")
        if total is not None:
            print("   итоги букв:", total[c["Р (дн/ч)"]], "|", total[c["М (дн/ч)"]], "|", total[c["О"]], total[c["Б"]])
            check(stack(total[c["М (дн/ч)"]])[0] == sum((stack(x[c["М (дн/ч)"]])[0] or 0) for x in emp), "итог М = сумма по сотрудникам")
        orgs = children_after(data_all, i_ch)
        print("   организации Чешко:", [(first(o), o[c["Приём (ЗУП)"]], o[c["Увольнение (ЗУП)"]], o[c["ЗУП: дни / часы (по орг. — норма)"]], o[c["Начислено (ЗУП)"]]) for o in orgs])
        check(len(orgs) == 1 and "ІНДАСТРІАЛБУД" in first(orgs[0]), "Чешко: одна действующая организация (уволенная до периода скрыта)")
        if orgs:
            o = orgs[0]
            check(o[c["Приём (ЗУП)"]] == "23.07.2025" and stack(o[c["ЗУП: дни / часы (по орг. — норма)"]]) == (21.0, 168.0) and num(o[c["Начислено (ЗУП)"]]) == 29500,
                  "Чешко/ІНДАСТРІАЛБУД: приём, норма 21/168, начислено — в тех же колонках, что у сотрудника")
            check(o[c["ИНН"]] == "" and o[c["Контроль"]] == "" and o[c["Казна: дни / часы"]] == "" and o[c["М (дн/ч)"]] == "" and o[c["О"]] in ("", "0"), "строка организации: колонки сотрудника и буквы Казны пустые")
    ...
    if b:
        r = b[0]
        print("   Беспалько:", r[c["Приём (ЗУП)"]], "|", r[c["Контроль"]])
        check(r[c["Приём (ЗУП)"]] == "24.04.2025" and "риказ" not in r[c["Контроль"]], "Беспалько: 24.04.2025, без приказов")
    ...
    if r:
        ...
        check(num(r[col(hd, "Часы Казна (Р+М)")]) == r_hours + m_hours and num(r[col(hd, "Дни Казна (Р+М+К)")]) == num(r[col(hd, "Дни официальные (Р+М)")]) + num(r[col(hd, "К")]), "Чешко: часы Казна = Р + М, дни Казна = официальные + К")
