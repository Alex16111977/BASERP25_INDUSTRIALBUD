...
ВЫБРАТЬ
	...
	Ост.НачальныйОстаток КАК НачОст,
	ЕСТЬNULL(Об.Начисления, 0) КАК Приход,
	ЕСТЬNULL(Об.Выплаты, 0) КАК Расход,
	Ост.КонечныйОстаток КАК КонОст
ИЗ
	ВТ_Остатки КАК Ост
		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Обороты КАК Об
		ПО Ост.ФизическоеЛицо = Об.ФизическоеЛицо""", ("ДРФО", "НачОст", "Приход", "Расход", "КонОст")):
