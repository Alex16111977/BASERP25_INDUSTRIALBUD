...
def main():
    ...
    # ЧасыОфициальные = ЧасыФакт + часы М: у строк "Р" официальные равны факту, итог = факт + М
    by_state = {S(r["Состояние"]): (float(r["Факт"]), float(r["Офиц"])) for r in run(kz, """ВЫБРАТЬ Т.Состояние КАК Состояние,
            СУММА(Т.ЧасыФакт) КАК Факт, СУММА(Т.ЧасыОфициальные) КАК Офиц
        ИЗ РегистрНакопления.ДанныеТабелирования КАК Т ГДЕ Т.Регистратор = &Док И Т.Активность
        СГРУППИРОВАТЬ ПО Т.Состояние""", {"Док": ref})}
    print("   по состояниям (факт, офиц):", by_state)
    f_r, o_r = by_state.get("Р", (0.0, 0.0))
    f_m, o_m = by_state.get("М", (0.0, 0.0))
    check(f_r > 0 and o_r == f_r, f"строки Р: ЧасыОфициальные ({o_r}) = ЧасыФакт ({f_r})")
    check(f_m == 0 and o_m == exp_hours, f"строки М: ЧасыФакт 0, ЧасыОфициальные {o_m} = часы М из ТЧ {exp_hours}")
    tot_f = sum(v[0] for v in by_state.values()); tot_o = sum(v[1] for v in by_state.values())
    check(tot_o == tot_f + o_m, f"итог документа: официальные {tot_o} = факт {tot_f} + М {o_m}")
