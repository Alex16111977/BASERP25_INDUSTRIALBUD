...
EXPECTED_DISBALANCE = 34999.48  # сума всіх дисбалансів (може бути будь-яка — для інформації)
...
def test_disbalance_before(conn):
    ...
    Показывает дисбаланс (конечный остаток) по каждому подразделению
    без статьи Прибыли и убытки. Использует ОстаткиИОбороты чтобы
    захватить ВСЕ подразделения с движениями, не только с ненулевым остатком.
    """
    print("\n" + "=" * 60)
    ...
    # ВАЖНО: обидва параметри ОстаткиИОбороты = КонецПериода (як у правильному запиті)
    ...
    print(f"Найдено комбинаций (Орг+Подр+Направление) с дисбалансом: {len(rows)}")
    for org, pod, napr, dis in rows:
        napr_str = napr if napr.strip() else "<пусто>"
        print(f"  {pod} | {napr_str}: {dis:,.2f}")

    total_r = round(total, 2)
    print(f"\nСума дисбалансу (контроль): {total_r:,.2f}")

    if len(rows) == 0:
        print("!! Дисбаланс відсутній — можливо документ вже заповнений")
    else:
        print(f"OK Знайдено {len(rows)} рядків для заповнення")
        if len(rows) == EXPECTED_ROWS:
            print(f"OK Кількість рядків збігається з очікуваним ({EXPECTED_ROWS})")
        else:
            print(f"!! Кількість рядків {len(rows)} відрізняється від очікуваного {EXPECTED_ROWS}")
...
def test_balance_after(conn):
    ...
    По каждому подразделению конечный остаток (все статьи вместе) должен = 0.
    Использует ОстаткиИОбороты — то же самое что и отчет УправленческийБаланс.
    """
    print("\n" + "=" * 60)
    print("ТЕСТ 2: Баланс ПОСЛЕ заполнения документа")
    print("=" * 60)

    # Той самий запит що і відчет УправленськийБаланс — обидва параметри = КонецПериода
    q = conn.NewObject("Запрос")
    q.Текст = (
        "ВЫБРАТЬ"
        "   Ост.Организация.Наименование             КАК Организация,"
    ...
    if len(nonzero) == 0:
        print("OK БАЛАНС ВИРИВНЕНО! Всі комбінації (Орг + Підрозділ + Напрямок) = 0")
        return True
    else:
        print(f"!! Залишилось ненульових комбінацій: {len(nonzero)}")
        for org, pod, napr, итого in nonzero:
            napr_str = napr if napr.strip() else "<пусто>"
            print(f"  {pod} | {napr_str}: {итого:,.2f}")
        return False
