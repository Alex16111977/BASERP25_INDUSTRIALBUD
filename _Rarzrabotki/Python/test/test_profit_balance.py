...
EXPECTED_DISBALANCE = 34999.48  # NET дисбаланс по подразделениям (без Прибыли и убытки)
...
def test_disbalance_before(conn):
    ...
    while sel.Следующий():
        ...
        rows.append((org, pod, dis))
        total += dis

    print(f"Найдено подразделений с ненулевым дисбалансом: {len(rows)}")
    for org, pod, dis in rows:
        print(f"  {pod}: {dis:,.2f}")
    ...
    if len(rows) == 0:
        ...
        print(f"OK Контрольна сума збігається (допуск +-1 грн)")
    else:
...
def test_balance_after(conn):
    ...
    while sel.Следующий():
        ...
        nonzero.append((org, pod, итого))

    if len(nonzero) == 0:
        print("OK БАЛАНС ВИРИВНЕНО! Всі підрозділи: Актив = Пасив")
        return True
    else:
        print(f"!! Ненульових підрозділів: {len(nonzero)}")
        for org, pod, итого in nonzero:
            print(f"  {pod}: {итого:,.2f}")
        return False
