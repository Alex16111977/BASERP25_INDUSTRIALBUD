...
def main():
    ...
    try:
        ...
        LIM = 15
        for i in range(n):
            тр.Получить(i).Синхронизировать = (i < LIM)
        итог2 = обр.АнализироватьДокументы(-1)
        тд = обр.ТаблицаДокументов
        # проверка: нет тихих сбоев анализа позиции (статус-маркеров аварий нет в самих строках)
        пом = 0
        for i in range(тд.Количество()):
            if str(тд.Получить(i).Статус).startswith("ПОМИЛКА"):
                пом += 1
        print(f"[2] АнализироватьДокументы(-1) перших {LIM} поз.: OK — {итог2}; "
              f"документів={тд.Количество()}; рядків-ПОМИЛКА={пом}")
    except Exception as e:
        fails += 1
        ei = getattr(e, 'excepinfo', None)
