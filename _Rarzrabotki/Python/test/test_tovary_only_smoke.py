...
def main():
    ...
    try:
        ...
        сумПоз = 0
        for i in range(nСкл):
            сумПоз += int(склады.Получить(i).Позиций)
    ...
    try:
        if обр.ТаблицаСкладов.Количество() > 0:
            # склад з 2..12 позиціями з НАЙБІЛЬШОЮ к-стю позицій (швидко + найбільший шанс на документи)
            idxMin, minПоз = 0, int(обр.ТаблицаСкладов.Получить(0).Позиций)
            best = -1
            for i in range(обр.ТаблицаСкладов.Количество()):
                p = int(обр.ТаблицаСкладов.Получить(i).Позиций)
                if 2 <= p <= 12 and p > best:
                    best, idxMin, minПоз = p, i, p
            ...
            print(f"    документів={тд.Количество()}; склад.Документов={int(ск0.Документов)}, "
                  f"Дій={int(ск0.Действий)}, Потребують рішення={int(ск0.ТребуютРешения)}; рядків-ПОМИЛКА={пом}")
            if int(ск0.Документов) != тд.Количество():
