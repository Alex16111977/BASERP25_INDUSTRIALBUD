...
&НаКлиенте
Процедура ДобавитьРегистрациюСправочниковИзДокумента(Команда)

	Если Не ЗначениеЗаполнено(УзелОбменаСсылка) Тогда
		Возврат;
	КонецЕсли;

	// Проверяем, что мы на странице списка ссылок
	ТекСтр = Элементы.ВариантыСпискаОбъектов.ТекущаяСтраница;
	Если ТекСтр <> Элементы.СтраницаСписокСсылок Тогда
		Сообщить("Команда доступна только на странице 'Список ссылок'");
		Возврат;
	КонецЕсли;

	// Получаем выбранные строки
	ВыбранныеСтроки = Элементы.СписокСсылок.ВыделенныеСтроки;

	// Если ничего не выбрано, используем текущую строку
	Если ВыбранныеСтроки.Количество() = 0 Тогда
		ТекДанные = Элементы.СписокСсылок.ТекущиеДанные;
		Если ТекДанные = Неопределено Тогда
			Сообщить("Выберите документ в списке");
			Возврат;
		КонецЕсли;

		СсылкаДокумента = ТекДанные.Ссылка;

		// Проверяем, что это документ
		Если ТипЗнч(СсылкаДокумента) = Тип("Строка") Или Не ЗначениеЗаполнено(СсылкаДокумента) Тогда
			Сообщить("Выберите документ в списке");
			Возврат;
		КонецЕсли;

		// Вызываем серверную процедуру для одного документа
		Результат = ЗарегистрироватьСправочникиИзДокументаНаСервере(УзелОбменаСсылка, СсылкаДокумента);

		// Сообщаем о результатах
		Результат.Вставить("Команда",Истина);
		СообщитьОРезультатахРегистрации(Результат);
	Иначе
		// Обрабатываем все выбранные документы
		ОбщийРезультат = Новый Структура("Всего, Успешно", 0, 0);

		Для Каждого Строка Из ВыбранныеСтроки Цикл
			ТекДанные = Элементы.СписокСсылок.ДанныеСтроки(Строка);
			СсылкаДокумента = ТекДанные.Ссылка;
			...
			// Вызываем серверную процедуру для каждого документа
			Результат = ЗарегистрироватьСправочникиИзДокументаНаСервере(УзелОбменаСсылка, СсылкаДокумента);

			// Накапливаем результаты
			ОбщийРезультат.Всего = ОбщийРезультат.Всего + Результат.Всего;
			ОбщийРезультат.Успешно = ОбщийРезультат.Успешно + Результат.Успешно;
		КонецЦикла;

		// Сообщаем об общих результатах
		ОбщийРезультат.Вставить("Команда",Истина);
		СообщитьОРезультатахРегистрации(ОбщийРезультат);
	КонецЕсли;
...
&НаСервере
Функция ЗарегистрироватьСправочникиИзДокументаНаСервере(Узел, СсылкаДокумента)
	
	// Вызываем функцию из модуля объекта обработки
	Возврат ЭтотОбъект().ЗарегистрироватьСправочникиИзДокумента(Узел, СсылкаДокумента);

КонецФункции

#Область РегистрацияЗаПериод

// Обработчик команды регистрации документов за период
//
&НаКлиенте
Процедура ЗарегистрироватьЗаПериод(Команда)

	// Валидация узла обмена
	Если НЕ ЗначениеЗаполнено(УзелОбменаСсылка) Тогда
		ПоказатьПредупреждение(, "Виберіть вузол обміну!");
		Возврат;
	КонецЕсли;

	// Валидация периода
	Если НЕ ЗначениеЗаполнено(Объект.ПериодНачало) ИЛИ НЕ ЗначениеЗаполнено(Объект.ПериодОкончание) Тогда
		ПоказатьПредупреждение(, "Вкажіть період!");
		Возврат;
	КонецЕсли;

	Если Объект.ПериодНачало > Объект.ПериодОкончание Тогда
		ПоказатьПредупреждение(, "Початок періоду не може бути більше закінчення!");
		Возврат;
	КонецЕсли;

	// Получаем позначені типи документів
	Данные = ПолучитьВыбранныеИменаМетаданных(Истина);
	МетаИмена = Данные.МетаИмена;

	// Якщо нічого не позначено - беремо поточну строку
	Если МетаИмена.Количество() = 0 Тогда
		Данные = ПолучитьИменаМетаданныхТекущейСтроки(Истина);
		Если Данные = Неопределено ИЛИ Данные.МетаИмена.Количество() = 0 Тогда
			ПоказатьПредупреждение(, "Позначте документи для реєстрації у дереві!");
			Возврат;
		КонецЕсли;
		МетаИмена = Данные.МетаИмена;
	КонецЕсли;
	...
	ТекстВопроса = "Буде виконано реєстрацію " + Данные.Описание + " та пов'язаних довідників за період з "
		+ Формат(Объект.ПериодНачало, "ДЛФ=D") + " по " + Формат(Объект.ПериодОкончание, "ДЛФ=D") + ".
		|Продовжити?";

	ОписаниеОповещения = Новый ОписаниеОповещения("ЗарегистрироватьЗаПериодЗавершение", ЭтотОбъект,
		Новый Структура("МетаИмена", МетаИмена));
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);

КонецПроцедуры

&НаКлиенте
Процедура ЗарегистрироватьЗаПериодЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	РезультатРегистрации = ЗарегистрироватьЗаПериодНаСервере(
		УзелОбменаСсылка,
		Объект.ПериодНачало,
	...
	// Сообщаем о результатах
	ТекстСообщения = "Оброблено документів: " + РезультатРегистрации.КоличествоДокументов
		+ ", зареєстровано об'єктів: " + РезультатРегистрации.КоличествоОбъектов;
...
&НаСервере
Функция ЗарегистрироватьЗаПериодНаСервере(УзелОбмена, ДатаНачала, ДатаОкончания, МетаИмена)

	Результат = Новый Структура("КоличествоДокументов, КоличествоОбъектов", 0, 0);

	// Получаем состав плана обмена
	СоставУзла = УзелОбмена.Метаданные().Состав;

	// Визначаємо режим роботи: всі документи чи тільки позначені
	РеєструватиВсі = Ложь;
	Для Каждого МетаИмя Из МетаИмена Цикл
		Если МетаИмя = Неопределено Тогда
			РеєструватиВсі = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;

	// Збираємо імена документів для реєстрації
	ИменаДокументовДляРегистрации = Новый Массив;

	Если РеєструватиВсі Тогда
		// Всі документи з плану обміну
		Для Каждого МетаданныеДокумента Из Метаданные.Документы Цикл
			ЭлементСостава = СоставУзла.Найти(МетаданныеДокумента);
			Если ЭлементСостава <> Неопределено Тогда
				ИменаДокументовДляРегистрации.Добавить(МетаданныеДокумента.Имя);
			КонецЕсли;
		КонецЦикла;
	Иначе
		// Тільки позначені документи
		Для Каждого МетаИмя Из МетаИмена Цикл
			Если МетаИмя = Неопределено Тогда
				Продолжить;
			КонецЕсли;

			// МетаИмя має формат "Документ.ИмяДокумента"
			Если СтрНачинаетсяС(МетаИмя, "Документ.") Тогда
				ИмяДокумента = Сред(МетаИмя, СтрДлина("Документ.") + 1);
				// Перевіряємо чи документ входить в план обміну
				МетаданныеДокумента = Метаданные.Документы.Найти(ИмяДокумента);
				Если МетаданныеДокумента <> Неопределено Тогда
					ЭлементСостава = СоставУзла.Найти(МетаданныеДокумента);
					Если ЭлементСостава <> Неопределено Тогда
						ИменаДокументовДляРегистрации.Добавить(ИмяДокумента);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	// Перебираем документы для регистрации
	Для Каждого ИмяДокумента Из ИменаДокументовДляРегистрации Цикл

		// Формируем запрос для выборки документов за период
		Запрос = Новый Запрос;
		...
		Исключение
			Продолжить; // Пропускаем документы, если запрос не выполнился
		КонецПопытки;
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				// Продолжаем, даже если не удалось зарегистрировать
			КонецПопытки;

			// Регистрируем связанные справочники через существующую функцию
			РезультатСправочников = ЭтотОбъект().ЗарегистрироватьСправочникиИзДокумента(УзелОбмена, Выборка.Ссылка);
			Результат.КоличествоОбъектов = Результат.КоличествоОбъектов + РезультатСправочников.Успешно;
