...
Процедура ЗарегистрироватьЗаПериод(Команда)
	...
	// Якщо нічого не позначено - показуємо попередження
...
Функция ЗарегистрироватьЗаПериодНаСервере(УзелОбмена, ДатаНачала, ДатаОкончания, МетаИмена)
	...
	Если РеєструватиВсі Тогда
		...
		Для Каждого ЭлементСостава Из СоставУзла Цикл
			Если ОбщегоНазначения.ЭтоДокумент(ЭлементСостава.Метаданные) Тогда
				ИменаДокументовДляРегистрации.Добавить(ЭлементСостава.Метаданные.Имя);
			КонецЕсли;
		КонецЦикла;
	Иначе
