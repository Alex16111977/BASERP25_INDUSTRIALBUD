...
Процедура СоздатьДокументыВводаОстатков() Экспорт
    ...
    Для Каждого СтрокаОперации Из ХозОперацииТаблица Цикл
        ...
        Для Каждого СтрокаРасчетов Из СтрокиОперации Цикл
            ...
            // Використовуємо ПроверитьСоздатьОбъектРасчетов яка створює елемент в довіднику ОбъектыРасчетов
            // якщо він не існує (ПолучитьОбъектРасчетовПоСсылке тільки шукає і повертає ПустаяСсылка якщо не знайдено!)
            ...
            Если ЗначениеЗаполнено(ОбъектДляРасчетов) Тогда
                ...
                НоваяСтрокаДок.ОбъектРасчетов = ОбъектыРасчетовСервер.ПроверитьСоздатьОбъектРасчетов(РеквизитыОбъектаРасчетов);
            КонецЕсли;

            // Заповнення ОбъектРасчетовОтправитель (каталог)
