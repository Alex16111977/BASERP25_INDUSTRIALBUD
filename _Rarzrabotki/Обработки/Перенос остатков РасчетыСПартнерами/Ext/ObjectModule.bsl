...
Процедура ЗагрузитьОстатки() Экспорт
    ...
    |ИЗ
    |	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаОстатков, Счет.Код В (&СписокСчетов), , ) КАК Остатки
    |ГДЕ
    |	(Остатки.СуммаОстатокДт <> 0
    |			ИЛИ Остатки.СуммаОстатокКт <> 0)
    ...
    Для Каждого СтрокаТЗ Из ТЗОстатки Цикл
        ...
        Если ХозОперация = Неопределено Тогда
            ...
            Сообщить("Не вдалося визначити хозоперацію. Договір: " + ДоговорКод + ", Рахунок: " + КодСчета + ", Сума: " + СуммаОстатка);
            Продолжить;
        КонецЕсли;
