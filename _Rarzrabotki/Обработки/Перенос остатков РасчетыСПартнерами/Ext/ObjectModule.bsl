...
Процедура ЗагрузитьОстатки() Экспорт
    ...
    |   ВЫРАЗИТЬ(Остатки.Субконто1 КАК Справочник.Контрагенты).КодПоЕДРПОУ КАК КонтрагентКодПоЕДРПОУ,
    ...
    Для Каждого СтрокаТЗ Из ТаблицаОстатков Цикл
        ...
        КонтрагентКодПоЕДРПОУ = СокрЛП(СтрокаТЗ.КонтрагентКодПоЕДРПОУ);
        КонтрагентНаименование = СокрЛП(СтрокаТЗ.КонтрагентНаименование);

        Если НЕ ПустаяСтрока(КонтрагентКодПоЕДРПОУ) Тогда
            // Пошук по коду ЄДРПОУ (в ERP реквізит називається "КодПоЕДРПОУ")
            КонтрагентЕРП = Справочники.Контрагенты.НайтиПоРеквизиту("КодПоЕДРПОУ", КонтрагентКодПоЕДРПОУ);
        КонецЕсли;
        ...
        Если НЕ ЗначениеЗаполнено(КонтрагентЕРП) Тогда
            Примечание = Примечание + "Контрагент не знайдено: " + КонтрагентНаименование + " (Код по ЕДРПОУ: " + КонтрагентКодПоЕДРПОУ + "); ";
        КонецЕсли;
