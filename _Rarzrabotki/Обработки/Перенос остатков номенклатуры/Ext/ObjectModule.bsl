...
Процедура СоздатьДокументыВводаОстатков() Экспорт
    ...
    Для Каждого СтрокаСклада Из СкладыТаблица Цикл
        ...
        НовыйДокумент.Дата = КонецДня(ОкончаниеПериода);
        ...
        Для Каждого СтрокаТовара Из СтрокиСклада Цикл
            ...
            Если ЗначениеЗаполнено(СтрокаТовара.СуммаБезНДС) Тогда
                НоваяСтрокаДок.СуммаБезНДС = СтрокаТовара.СуммаБезНДС;
            КонецЕсли;

            Если ЗначениеЗаполнено(СтрокаТовара.СуммаНДС) Тогда
                НоваяСтрокаДок.СуммаНДС = СтрокаТовара.СуммаНДС;
            КонецЕсли;

        КонецЦикла;

        // 3.4. Запись документа
        Попытка
