...
Процедура ЗагрузитьОстатки() Экспорт
  ...
  ТекстЗапроса =
    ...
    |	ВложенныйЗапрос.СкладКод КАК СкладКод,
    |	ВложенныйЗапрос.СкладНаименование КАК СкладНаименование,
    |	ВложенныйЗапрос.Количество КАК Количество,
    |	ВложенныйЗапрос.Сумма КАК Сумма,
    |	ВложенныйЗапрос.ОрганизацияКодПоЕДРПОУ КАК ОрганизацияКодПоЕДРПОУ
    |ИЗ
    |	(ВЫБРАТЬ
    |		Остатки.Счет.Код КАК СчетКод,
    |		ВЫБОР
    |			КОГДА ПозицияНоменклатуры.НомерСтроки = 1
    |				ТОГДА ВЫРАЗИТЬ(Остатки.Субконто1 КАК Справочник.Номенклатура).Код
    |			КОГДА ПозицияНоменклатуры.НомерСтроки = 2
    |				ТОГДА ВЫРАЗИТЬ(Остатки.Субконто2 КАК Справочник.Номенклатура).Код
    |			КОГДА ПозицияНоменклатуры.НомерСтроки = 3
    |				ТОГДА ВЫРАЗИТЬ(Остатки.Субконто3 КАК Справочник.Номенклатура).Код
    |			ИНАЧЕ """"
    ...
    |		СУММА(Остатки.КоличествоКонечныйОстатокДт) КАК Количество,
    |		СУММА(Остатки.СуммаКонечныйОстатокДт) КАК Сумма,
    ...
    Для Каждого СтрокаТЗ Из ТаблицаОстатков Цикл
	 ...
	 КонецЕсли; 
	   ...
	   Если СокрЛП(СтрокаТЗ.СчетКод)="2072" ИЛИ СокрЛП(СтрокаТЗ.СчетКод)="2071" Тогда
	 ...
	 КонецЕсли; 
        ...
        НоменклатураЕРП = Справочники.Номенклатура.НайтиПоРеквизиту("А_ИдКод",СокрЛП(СтрокаТЗ.НоменклатураИдКод));
...
//		КонецЕсли;
        ...
        Если НЕ ПустаяСтрока(СкладКод) Тогда
            СкладЕРП = Справочники.Склады.НайтиПоРеквизиту("А_ИдКод",СкладКод);
        КонецЕсли;
		...
		НоваяСтрока.НоменклатураКод = НоменклатураКод;
		...
		Если ЗначениеЗаполнено(НоваяСтрока.СчетУчета)
			...
			СпрНоменклатура.А_СчетУчетаБух = НоваяСтрока.СчетУчета;
