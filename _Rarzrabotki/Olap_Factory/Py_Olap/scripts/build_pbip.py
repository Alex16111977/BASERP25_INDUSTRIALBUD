...
def ид(имя):
    """TMDL-идентификатор: имя с пробелом/дефисом/любым не-словесным символом ОБЯЗАНО быть
    в одинарных кавычках, иначе парсер валится `InvalidLineType: Непредвиденный тип строки: Other!`
    (проверено 02.08.2026 на `column Назва місяця`). Кириллица сама по себе кавычек не требует."""
    return имя if re.fullmatch(r"\w+", имя, re.UNICODE) else f"'{имя}'"
...
def main():
    ...
    таблицы[ФАКТ] = таблица(ФАКТ, [
        ...
        кол("Номенклатура", "string", "ItemName"),
        ...
        "Analytics_Name", "Document_ID", "Unit_ID", "ItemName", "PlanHours", "FactHours", "EarnedHours",
        "PlanQty", "FactQty", "PlanUAH", "FactUAH", "ETC_UAH"], "vw_Fact_PlanFact"),
        меры_tmdl())

    таблицы["Підрозділи"] = таблица("Підрозділи", [
        кол("Department_ID", "string", "Department_ID", скрыт=True),
        кол("Підрозділ", "string", "Department_Name"),
    ...
    ], партиция("Підрозділи", ["Department_ID", "Department_Name", "Department_Code",
    ...
    связи = [
        ...
        ("fact_executor_dept", f"{ФАКТ}.ExecutorDept_ID", "Підрозділи.Department_ID", False),
