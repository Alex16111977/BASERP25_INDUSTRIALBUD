        ...
        for вид in ("table", "column", "partition", "measure"):
            for m in re.finditer(r"(?m)^\t*" + вид + r"\s+([^=\n]+?)\s*(?:=|$)", txt):
                ...
                if имя_ид.startswith("'"):
                    # 🔴 кавычка ВНУТРИ процитированного имени обязана быть удвоена, иначе
                    # цитата закрывается на середине: `column 'Підрозділ-об'єкт'` ломает парсер
                    # так же, как отсутствие кавычек. Проверяем тело имени между внешними кавычками.
                    тело = имя_ид[1:-1] if имя_ид.endswith("'") else имя_ид[1:]
                    if re.search(r"(?<!')'(?!')", тело):
                        ошибки.append(f"{f.name}: {вид} {имя_ид!r} — одинарная кавычка внутри "
                                      f"имени не удвоена (нужно '' или типографский U+02BC)")
                    continue
                # намеренно уже, чем \w: \w пропускает U+02BC и прочую не-ASCII пунктуацию,
                # поведение парсера TMDL на них не проверено
                if not re.fullmatch(r"[A-Za-zЀ-ӿ0-9_]+", имя_ид):
                    ошибки.append(f"{f.name}: {вид} {имя_ид!r} с пробелом/дефисом/пунктуацией "
                                  f"БЕЗ кавычек")
