...
Функция КопироватьПеренестиНоменклатуруКонтрагентов()
	...
	Попытка
		...
		Если КопироватьНоменклатуру Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Скопировано %1 номенклатурных позиций от %2 к %3';uk='Скопійовано %1 номенклатурних позицій від %2 до %3'"), 
						Строка(СкопированоЗаписейНоменклатуры), КонтрагентИсточник.Наименование, КонтрагентПриемник.Наименование));
		Иначе
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Перенесено %1 номенклатурных позиций от %2 к %3';uk='Перенесено %1 номенклатурних позицій від %2 до %3'"), 
						Строка(СкопированоЗаписейНоменклатуры), КонтрагентИсточник.Наименование, КонтрагентПриемник.Наименование));
		...
		Если Не КопироватьНоменклатуру Тогда
			...
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Удалено %1 номенклатурных позиций у контрагента %2';uk='Видалено %1 номенклатурних позицій у контрагента %2'"), 
						Строка(ТаблицаПереноса.Количество()), КонтрагентИсточник.Наименование));
		КонецЕсли;
...
Процедура ДействиеВыполнить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(КонтрагентПриемник) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Выберите контрагента-приемника для копирования/переноса данных.';uk='Виберіть контрагента-приймача для копіювання/перенесення даних.'"));
		Возврат;
	КонецЕсли;
	
	Если КонтрагентИсточник = КонтрагентПриемник Тогда
		ПоказатьПредупреждение(,НСтр("ru='Контрагент-приемник и контрагент-источник совпадают.';uk='Контрагент-приймач і контрагент-джерело збігаються.'"));
		Возврат;	
	КонецЕсли;
	
	Если ДействиеВыполнитьНаСервере() Тогда
		Если КопироватьНоменклатуру Тогда
			ПоказатьПредупреждение(,НСтр("ru='Копирование данных успешно завершено.';uk='Копіювання даних успішно завершено.'"),10);
		Иначе
			ПоказатьПредупреждение(,НСтр("ru='Перенос данных успешно завершен.';uk='Перенесення даних успішно завершено.'"),10);
		КонецЕсли;	
	Иначе
		ПоказатьПредупреждение(,НСтр("ru='Произошла ошибка при копировании/переносе номенклатуры контрагентов.';uk='Сталася помилка при копіюванні/перенесенні номенклатури контрагентів.'"));
	КонецЕсли;
