...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Пактум_ОбновитьДополнительныеСведенияОКонтрагенте();
	...
	Пактум_СерверПереопределяемый.ДобавитьРеквизитыПактум(ЭтаФорма);
...
Процедура УправлениеФормой(Форма)
	...
	Если ПустаяСтрока(Форма.КонтактнаяИнформацияПолеФактАдресКонтрагента)
		ИЛИ Форма.КонтактнаяИнформацияПолеФактАдресКонтрагента = УправлениеКонтактнойИнформациейКлиентСерверБП.ПредставлениеПустойГиперссылки() Тогда
			Форма.ФактАдресКонтрагентаСовпадает = НСтр("ru='<не заполнен>';uk='<не заповнена>'");
	...
	Если ПустаяСтрока(Форма.КонтактнаяИнформацияПолеПочтовыйАдресКонтрагента)
		ИЛИ Форма.КонтактнаяИнформацияПолеПочтовыйАдресКонтрагента = УправлениеКонтактнойИнформациейКлиентСерверБП.ПредставлениеПустойГиперссылки() Тогда
			Форма.ПочтовыйАдресКонтрагентаСовпадает = НСтр("ru='<не заполнен>';uk='<не заповнена>'");
...
Процедура ПроверкаЗавершенияФоновогоЗадания()
	...
	Если Завершить Тогда
		...
		Пактум_ОчиститьКонтактнуюИнформацию();
		РазместитьКонтактнуюИнформацию();
		ОбновитьОписаниеСхемыНалогообложения(ЭтаФорма);
		Пактум_ОбновитьДополнительныеСведенияОКонтрагенте();
		УправлениеФормой(ЭтаФорма);
...
&НаСервере
Процедура Пактум_УдалитьКоманду(ИмяКоманды)
	КомандаКУдалению = Команды.Найти(ИмяКоманды);
	Если НЕ КомандаКУдалению = Неопределено Тогда
		Команды.Удалить(КомандаКУдалению);	
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура Пактум_ОчиститьКонтактнуюИнформацию()
	Для Каждого Эл Из Элементы.ГруппаКомпоновкиКонтактнойИнформации.ПодчиненныеЭлементы Цикл
		Если Эл.Имя = "ГруппаКонтактнаяИнформацияПолеТелефонКонтрагента"
				Или Эл.Имя = "ГруппаКонтактнаяИнформацияПолеФаксКонтрагенты"
				Или Эл.Имя = "ГруппаКонтактнаяИнформацияПолеEmailКонтрагенты"
				Или Эл.Имя = "ГруппаКонтактнаяИнформацияПолеДругаяИнформацияКонтрагенты" Тогда
			Продолжить;		
		КонецЕсли;
		Элементы.Удалить(Эл);
	КонецЦикла;
	...
	Для Каждого Стр Из КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		Если Не (Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеЮрАдресКонтрагента"
				Или Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеФактАдресКонтрагента"
				Или Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеПочтовыйАдресКонтрагента"
				Или Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеТелефонКонтрагента"
				Или Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеФаксКонтрагенты"
				Или Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеEmailКонтрагенты"
				Или Стр.ИмяРеквизита = "КонтактнаяИнформацияПолеДругаяИнформацияКонтрагенты") Тогда
			УдаляемыеРеквизиты.Добавить(Стр.ИмяРеквизита);
		КонецЕсли;
	КонецЦикла;
	ИзменитьРеквизиты( , УдаляемыеРеквизиты);
	Для Каждого Стр Из УдаляемыеРеквизиты Цикл
		Пактум_УдалитьКоманду("Команда" + Стр);
		Пактум_УдалитьКоманду("КонтекстноеМеню" + Стр);
		Пактум_УдалитьКоманду("КонтекстноеМенюВверх" + Стр);
		Пактум_УдалитьКоманду("КонтекстноеМенюВниз" + Стр);
	КонецЦикла;
	Пактум_УдалитьКоманду("КонтактнаяИнформацияДобавитьПолеВводаГруппаКомпоновкиКонтактнойИнформации");
	Пактум_УдалитьКоманду("КонтекстноеМенюВнизКонтактнаяИнформацияПолеТелефонКонтрагента");
	Пактум_УдалитьКоманду("КонтекстноеМенюВнизКонтактнаяИнформацияПолеEmailКонтрагенты");		
...
&НаКлиенте
Процедура Пактум_ОбработкаИзмененияДополнительныхСведенийОКонтрагенте(Результат, ДополнительныеПараметры) Экспорт
	Пактум_ОбновитьДополнительныеСведенияОКонтрагенте();	
КонецПроцедуры

&НаСервере
Процедура Пактум_ОбновитьДополнительныеСведенияОКонтрагенте()
	ДополнительныеСведенияКонтрагента = Пактум_Сервер.ПолучитьДополнительныеСведенияКонтрагента(Объект.Ссылка);
КонецПроцедуры

&НаКлиенте
Процедура Пактум_Отчет(Команда)
	Пактум_КлиентПереопределяемый.СформироватьРасширенныеДанные(Объект.Ссылка);
КонецПроцедуры
#КонецОбласти
