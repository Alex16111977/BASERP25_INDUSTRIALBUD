...
Процедура УстановитьЗаголовкиИДоступностьСубконто(Форма, Счет, Постфикс = "")
	...
	УстановитьТекстСубконтоАвтоматические(Счет, Форма, ПоляФормы, Постфикс);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьТекстСубконтоАвтоматические(Счет, Форма, ПоляФормы, Постфикс)
	...
	Для Индекс = 1 По 3 Цикл
		Если ПоляФормы.Свойство("Субконто" + Индекс) Тогда
			Если ДанныеСчета["ВидСубконто" + Индекс] = ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы") Тогда
				...
				Форма.Элементы["Субконто" + Постфикс + Индекс].ПодсказкаВвода = НСтр("ru='из свойств номенклатуры';uk='із властивостей номенклатури'");
			ИначеЕсли ДанныеСчета["ВидСубконто" + Индекс] = ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура") Тогда
				Форма.Элементы["Субконто" + Постфикс + Индекс].ТолькоПросмотр = Истина;
				Форма.Элементы["Субконто" + Постфикс + Индекс].ПодсказкаВвода = НСтр("ru='из номенклатуры';uk='з номенклатури'");
			Иначе
