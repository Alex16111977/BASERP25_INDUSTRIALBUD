		...
		Если Выборка.Количество() > 1 Тогда
			...
			КраткоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи профиля ""%1"".
                           ...
                           |Профіль, що постачається, уже існує:'"),
				Наименование);
			...
			Иначе
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Управление доступом.Нарушение однозначности поставляемого профиля';uk='Управління доступом.Порушення однозначності профілю, що постачається'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
