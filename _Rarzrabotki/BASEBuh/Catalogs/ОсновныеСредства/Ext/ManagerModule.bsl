...
|Не можна сформувати інвентарну картку об'єкта!"""), Ссылка);
		...
		|	И ОперацииОС.Регистратор.ВидУлучшения = Значение(Перечисление.ВидыУлучшения.Модернизация) 
		|	И ОперацииОС.Регистратор ССЫЛКА Документ.МодернизацияОС )
		|	ИЛИ ОперацииОС.Регистратор ССЫЛКА Документ.ПереоценкаОС )
		|	И ОперацииОС.Период < &ДатаСведений
		|	И ОперацииОС.ОсновноеСредство = &ОсновноеСредство
		|	И ОперацииОС.Организация = &Организация
		|
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
			 	СтрокаИзмененияСтоимости.Параметры.СодержаниеОперации   = НСтр("ru='Переоценка ОС';uk='Переоцінка ОЗ'");     
			КонецЕсли;
