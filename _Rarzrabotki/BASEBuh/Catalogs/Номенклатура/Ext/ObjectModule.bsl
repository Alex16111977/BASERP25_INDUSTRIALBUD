...
Процедура ПриЗаписи(Отказ)
	...
	ПроверитьОсновнуюВОбщихНазваниях();//Тютюн
...
КонецПроцедуры
//Тютюн   
Процедура ПроверитьОсновнуюВОбщихНазваниях() 
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОбщиеНазванияНоменклатуры.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ОбщиеНазванияНоменклатуры КАК ОбщиеНазванияНоменклатуры
	|ГДЕ
	|	ОбщиеНазванияНоменклатуры.ОсновнаяНоменклатура = &НоменклатураСсылка
	|	И ОбщиеНазванияНоменклатуры.Ссылка <> &ОбщееДляТекущей";
	
	Запрос.УстановитьПараметр("НоменклатураСсылка", Ссылка);  
	Запрос.УстановитьПараметр("ОбщееДляТекущей", ОбщееНазваниеНоменклатуры);

	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		ОбщееОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ОбщееОбъект.ОсновнаяНоменклатура = Справочники.Номенклатура.ПустаяСсылка();
		ОбщееОбъект.Записать();
	КонецЦикла;
