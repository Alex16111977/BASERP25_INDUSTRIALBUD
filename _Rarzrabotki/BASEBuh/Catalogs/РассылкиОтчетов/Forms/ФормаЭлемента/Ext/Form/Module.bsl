...
Процедура ПредпросмотрШаблона(Команда)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Предварительный просмотр шаблона доступен в толстом, тонком или ВЕБ-клиенте.';uk='Попередній перегляд шаблону доступний в товстому, тонкому або вебклієнті.'"));
		Возврат;
