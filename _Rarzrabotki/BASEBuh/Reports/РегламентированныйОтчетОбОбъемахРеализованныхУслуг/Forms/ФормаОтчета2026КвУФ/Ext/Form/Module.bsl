...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2026КвУФ";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("мЗаполнитьПоОсновномуКВЕД", Ложь);

	СтруктураРеквизитовФормы.Вставить("ВалютаРегламентированногоУчета", Константы.ВалютаРегламентированногоУчета.Получить());

	СтруктураРеквизитовФормы.Вставить("СлужебныйДокументЗаполненияИсточникОтчета", "ДокументЗаполненияРеализацияУслуг");
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("мЗаполнитьПоОсновномуКВЕД", СтруктураРеквизитовФормы.мЗаполнитьПоОсновномуКВЕД);
	...
	// встfвляем флажок ручной ввод ВРЕМЕННАЯ ОТМЕНА
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("мЗаполнитьПоОсновномуКВЕД", СтруктураРеквизитовФормы.мЗаполнитьПоОсновномуКВЕД);
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
    ...
    //2026
	ЗаполнитьПоказательНаСервере("КодКВЕД", КодКВЕД, Форма, "Уведомление");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если НЕ СтруктураРеквизитовФормы.мЗаполнитьПоОсновномуКВЕД Тогда
		//заполним из помощника
		ЗаполнитьДокументЗаполненияНаСервере();	
	Иначе
		...
		Строка.T1RXXXXG2S = ЭтаФорма.ТабличныйДокумент.Области.КодКВЕД.Значение;			 
		Строка.T1RXXXXG6S = ЭтаФорма.ТабличныйДокумент.Области.ВЭД.Значение;			 
		...
		Для Нмес = 0 По 2 Цикл
			...
			// Обсяг реалізованих послуг (без ПДВ, акцизного збору та інших непрямих податків),
			B_мес = КО(мБухИтогиМес, Счет70_3);
			НДС_мес = ОБ(мБухИтогиМес, Счет70_3, Счет64); 
			//Доход от операционной аренды активов
			B_мес_713 = КО(мБухИтогиМес, Счет71_3);
			НДС_мес_713 = ОБ(мБухИтогиМес, Счет71_3, Счет64);
			
			Строка["T1RXXXXG"+?(Нмес<2,Нмес+4,7)] = B_мес - НДС_мес + B_мес_713 - НДС_мес_713;
			ИмяЯчейки = "T1RXXXXG"+?(Нмес<2,Нмес+4,7)+"_1";
			...
			Если B_мес_713 <> 0 Тогда
				_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "", "+", "!КО", B_мес_713, Счет71_3, , НачМесяца, КонМесяца);
				_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "Показник ""Податок на додану вартість""", "-", "!ОБ", НДС_мес_713, Счет71_3, Счет64, НачМесяца, КонМесяца);
			КонецЕсли;
		
		КонецЦикла;	
		Строка.T1RXXXXG3 = Строка.T1RXXXXG4 + Строка.T1RXXXXG5 + Строка.T1RXXXXG7;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КонецЕсли;
	ТаблицаБлока = Форма[Форма.СтруктураРеквизитовФормы.мИдГруппы1];  
	A100_2 = 0;	A100_3 = 0;	A100_4 = 0;
	Для каждого Строка Из ТаблицаБлока Цикл
		A100_2 = A100_2 + ОкруглитьЧислоПоФормату(Строка.T1RXXXXG4);
		A100_3 = A100_3 + ОкруглитьЧислоПоФормату(Строка.T1RXXXXG5);
		A100_4 = A100_4 + ОкруглитьЧислоПоФормату(Строка.T1RXXXXG7);
	КонецЦикла;	
	ЗаполнитьПоказательНаСервере("A100_2", A100_2, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("A100_3", A100_3, Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("A100_4", A100_4, Форма, "Уведомление");
	
	ЗаполнитьПоказательНаСервере("A100_1", A100_2 + A100_3 + A100_4, Форма, "Уведомление");
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("мЗаполнитьПоОсновномуКВЕД", 	СтруктураРеквизитовФормы.мЗаполнитьПоОсновномуКВЕД);
...
Процедура ОткрытьПомощникЗаполнения(Команда)
	...
	ПараметрыПомощника = ПолучитьПараметрыОткрытияПомощникЗаполнения();
		
	ОбработчикЗакрытияПомощника = Новый ОписаниеОповещения("ПослеЗакрытияПомощникЗаполнения", ЭтаФорма);
	ОткрытьФорму(ИмяФормыПомощника, ПараметрыПомощника, ЭтаФорма,,,, ОбработчикЗакрытияПомощника);	
КонецПроцедуры 

&НаКлиенте
Функция ПолучитьПараметрыОткрытияПомощникЗаполнения() Экспорт
	
	Возврат Новый Структура("Организация, НачалоПериода, ОкончаниеПериода, СохраненныйДокумент, ДанныеПредыдущегоПериода", 
		...
		ПолучитьДокументСохраненияТаблицы(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета,
		СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.СлужебныйДокументЗаполненияИсточникОтчета));
		
КонецФункции  

&НаСервереБезКонтекста
Функция ПолучитьДокументСохраненияТаблицы(НачалоПериода, ОкончаниеПериода, Организация, ИсточникОтчета)
...
&НаКлиенте
Процедура СохранитьДокументЗаполнения(АдресДанных) Экспорт

	НужныеРеквизитыФормы = Новый Структура("Организация, мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета, мПериодичность, СлужебныйДокументЗаполненияИсточникОтчета");
	ЗаполнитьЗначенияСвойств(НужныеРеквизитыФормы, СтруктураРеквизитовФормы);
	СохранитьДокументЗаполненияНаСервере(АдресДанных, НужныеРеквизитыФормы);	
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СохранитьДокументЗаполненияНаСервере(АдресДанных, СтруктураРеквизитовФормы)
	
	ДокСсылка = ПолучитьДокументСохраненияТаблицы(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, 
		СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.СлужебныйДокументЗаполненияИсточникОтчета);
	...
	Док.ИсточникОтчета = СтруктураРеквизитовФормы.СлужебныйДокументЗаполненияИсточникОтчета;
	Док.НаименованиеОтчета = НСтр("ru='Расшифровка';uk= 'Розшифровка'");
...
&НаКлиенте
Функция ЗаполнитьТаблицуКВЕДИзОтчетаПрошлогоПериода() Экспорт 
	
	НужныеРеквизитыФормы = Новый Структура("Организация, мДатаНачалаПериодаОтчета, СлужебныйДокументЗаполненияИсточникОтчета");
	ЗаполнитьЗначенияСвойств(НужныеРеквизитыФормы, СтруктураРеквизитовФормы);
	НужныеРеквизитыФормы.Вставить("ИдентификаторФормы", УникальныйИдентификатор);
	
	Возврат ЗаполнитьТаблицуКВЕДИзОтчетаПрошлогоПериодаНаСервере(НужныеРеквизитыФормы);
	
КонецФункции

&НаСервереБезКонтекста
Функция ЗаполнитьТаблицуКВЕДИзОтчетаПрошлогоПериодаНаСервере(СтруктураРеквизитовФормы) Экспорт 

	ДатПрошлогоПериода1 = НачалоМесяца(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,-3));
	ДатПрошлогоПериода2 = КонецКвартала(ДатПрошлогоПериода1);
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("Аналитика");
	Результат.Колонки.Добавить("КВЕД");
	
	Если ДатПрошлогоПериода1 >= '2026-01-01' Тогда     
		
		Док = ПолучитьДокументСохраненияТаблицы(ДатПрошлогоПериода1, ДатПрошлогоПериода2,
			СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.СлужебныйДокументЗаполненияИсточникОтчета);
		Если Док <> Неопределено Тогда
			
			СписокСохранения = Док.ДанныеОтчета.Получить(); 
			Для каждого ИсходнаяСтрока Из СписокСохранения.ТаблицаКВЕД Цикл
...
&НаКлиенте
Процедура ПослеЗакрытияПомощникЗаполнения(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия = Истина Тогда	
		...
		НайденнаяСтрока = РегламентированнаяОтчетностьКлиентСервер.НайтиЭлементВДанныхФормыДерево(РазделыОтчета.ПолучитьЭлементы(), "КолонкаРазделыОтчетаСокрНаим", "Уведомление");
		...
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			...
			Иначе
				ВывестиМакетВТаблДокНаСервере("Уведомление", 1);;
			КонецЕсли;
		КонецЕсли;                                                    
...
&НаСервере
Процедура ЗаполнитьДокументЗаполненияНаСервере()
	
	Док = ПолучитьДокументСохраненияТаблицы(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета,
		СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.СлужебныйДокументЗаполненияИсточникОтчета);	              
		
	Если Док <> Неопределено Тогда
		
		СписокСохранения = Док.ДанныеОтчета.Получить();
		ЗначениеВРеквизитФормы(СписокСохранения.T1R, СтруктураРеквизитовФормы.мИдГруппы1); 
		
		Если T1R.Количество() = 0 Тогда
			НоваяСтрока = T1R.Добавить();	
		КонецЕсли;
