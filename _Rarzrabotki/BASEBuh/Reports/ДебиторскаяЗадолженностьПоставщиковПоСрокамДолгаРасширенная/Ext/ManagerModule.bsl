...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат НСтр("ru='Дебиторская задолженность поставщиков по срокам долга на ';uk='Дебіторська заборгованість постачальників по строках боргу на '") + Формат(ПараметрыОтчета.Период, "ДФ=dd.MM.yyyy; ДП=...");
...
Функция ПолучитьПросроченнуюЗадолженностьПоставщикамДляМонитораРуководителя(Организация, ДатаЗадолженности) Экспорт
	...
	СубконтоКонтрагентДоговор.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами);
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ХозрасчетныйОстатки.Субконто2 КАК Договор,  
					 |  ХозрасчетныйОстатки.Субконто3 КАК СчетНаОплату,
	                |	ВЫБОР
...
Функция ПолучитьТекстЗапроса(КоличествоИнтервалов)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ОтправкаПолучениеДокументов.Документ КАК Документ
	|ПОМЕСТИТЬ ПолученныеДоки
	|ИЗ
	|	РегистрСведений.ОтправкаПолучениеДокументов КАК ОтправкаПолучениеДокументов
	|ГДЕ
	|	ОтправкаПолучениеДокументов.ДокументыПолучены
	|	И (ТИПЗНАЧЕНИЯ(ОтправкаПолучениеДокументов.Документ) = ТИП(Документ.ПоступлениеТоваровУслуг)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ОтправкаПолучениеДокументов.Документ) = ТИП(Документ.ПоступлениеДопРасходов))
	|	И ОтправкаПолучениеДокументов.Документ.Организация = &Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ВзаиморасчетыОстатки.Субконто3 КАК СчетНаОплату,
	...
	|					СчетаКД КАК СчетаКД),
	|			&ВидыСубконтоКД,
	|			ВЫРАЗИТЬ(Субконто2 КАК Справочник.ДоговорыКонтрагентов).ВидДоговора В (ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентом)) {(Организация).*, (Субконто1).* КАК Контрагент, (Субконто2).* КАК Договор, (Субконто3).* КАК СчетНаОплату}) КАК ВзаиморасчетыОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|				(ВЫБРАТЬ
	|					ПолученныеДоки.Документ КАК Документ
	|				ИЗ
	|					ПолученныеДоки КАК ПолученныеДоки)
	|	И ПоступлениеТоваровУслуг.Дата МЕЖДУ &ДатаНачалаБас И &Период
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ПоступлениеДопРасходов.Организация = &Организация
	...
	|				ИЗ
	|					ПолученныеДоки КАК ПолученныеДоки)
	|	И ПоступлениеДопРасходов.Дата МЕЖДУ &ДатаНачалаБас И &Период
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Контрагент,
	...
	Для Индекс = 1 По КоличествоИнтервалов Цикл
		...
		//Тютюн  
		|	ВзаиморасчетыОбороты.Субконто3 КАК СчетНаОплату,
		|	ВЫБОР
		...
		|					СчетаКД КАК СчетаКД),
		|			&ВидыСубконтоКД,
		|			ВЫРАЗИТЬ(Субконто2 КАК Справочник.ДоговорыКонтрагентов).ВидДоговора В (ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентом)) {(Организация).*, (Субконто1).* КАК Контрагент, (Субконто2).* КАК Договор, (Субконто3).* КАК СчетНаОплату},
		|			,
		...
		|				ПолученныеДоки КАК ПолученныеДоки)  
		| И (ВЫБОР
		|КОГДА &НачалоИнтервала" + Индекс + " > &ДатаНачалаБас
		|	ТОГДА ПоступлениеТоваровУслуг.Дата МЕЖДУ &НачалоИнтервала" + Индекс + " И &КонецИнтервала" + Индекс + "  
		|ИНАЧЕ  ПоступлениеТоваровУслуг.Дата МЕЖДУ &ДатаНачалаБас И &КонецИнтервала" + Индекс + "
		|КОНЕЦ)    
		...
		| И (ВЫБОР
		|КОГДА &НачалоИнтервала" + Индекс + " > &ДатаНачалаБас
		|	ТОГДА ПоступлениеДопРасходов.Дата МЕЖДУ &НачалоИнтервала" + Индекс + " И &КонецИнтервала" + Индекс + "  
		|ИНАЧЕ  ПоступлениеДопРасходов.Дата МЕЖДУ &ДатаНачалаБас И &КонецИнтервала" + Индекс + "
		|КОНЕЦ)
	...
	|	ВзаиморасчетыОстатки.СчетНаОплату,
	|";
	Для Индекс = 1 По КоличествоИнтервалов Цикл
		ПолныйТекстЗапроса = ПолныйТекстЗапроса + 
		"
	...
	Для Индекс = 1 По КоличествоИнтервалов Цикл
		...
		|			И ВзаиморасчетыОстатки.СчетНаОплату = Обороты" + Индекс + ".СчетНаОплату
		|";
	КонецЦикла;
	...
	|	ОстаткиПоПериодам.СчетНаОплату КАК СчетНаОплату,
	|";
	Для Индекс = 1 По КоличествоИнтервалов Цикл
	...
	|	ОстаткиПоПериодам.СчетНаОплату.*}";
...
Процедура УстановитьПараметры(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	КонецЦикла; 
	 ...
	 БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ДатаНачалаБас" , Дата("20230701"));
...
Функция ПолучитьПросроченнуюЗадолженность(ПараметрыОтчета, КонДата)
	...
	Запрос.УстановитьПараметр("ДатаНачалаБАС", Дата("20230701"));  
	Запрос.УстановитьПараметр("ДатаОстатков", КонецДня(КонДата));
	...
	ТекстЗапросаПоОстаткам = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	                         |	СчетаКонтрагентов.Ссылка КАК Счет
	                         |ПОМЕСТИТЬ СчетаКД
	                         |ИЗ
	                         |	ПланСчетов.Хозрасчетный.ВидыСубконто КАК СчетаКонтрагентов
	                         ...
	                         |		ПоступлениеТоваровУслуг.ДоговорКонтрагента,
	                         |		ПоступлениеТоваровУслуг.Ссылка,
	                         |		&СтандартныйСрокОплатыПокупателей,
	                         |		NULL,
	                         |		ПоступлениеТоваровУслуг.СуммаДокумента
	                         |	ИЗ
	                         |		Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	                         ...
	                         |					ИЗ
	                         |						ПолученныеДоки КАК ПолученныеДоки)
	                         |		И ПоступлениеТоваровУслуг.Дата МЕЖДУ &ДатаНачалаБАС И &ДатаОстатков) КАК ВложенныйЗапрос
	                         |
	                         |ИНДЕКСИРОВАТЬ ПО
	                         |	Контрагент,
	                         |	Договор,
	...
	|	И ВзаиморасчетыОбороты.Субконто3 = ВзаиморасчетыОбороты.КорСубконто3
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|					ПолученныеДоки КАК ПолученныеДоки)
	|	И ВЫБОР
	|			КОГДА &НачалоИнтервала1 > &ДатаНачалаБас
	|				ТОГДА ПоступлениеТоваровУслуг.Дата МЕЖДУ &НачалоИнтервала1 И &КонецИнтервала
	|			ИНАЧЕ ПоступлениеТоваровУслуг.Дата МЕЖДУ &ДатаНачалаБас И &КонецИнтервала
	|		КОНЕЦ";
	...
	Для ИндексЧасти = 1 По КоличествоЧастей Цикл
		...
		|	Обороты" + ИндексЧасти + ".СчетНаОплату,
	...
	|		И ОстаткиДолга.СчетНаОплату = Обороты.СчетНаОплату
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиДолга.Организация,
	|	ОстаткиДолга.Контрагент,
	|	ОстаткиДолга.Договор, 
