...
Функция ПреобразоватьТипXMLОбщий(Значение,Тип,Результат,ОписаниеОшибки = "")
	...
	ИначеЕсли   (ВРег(Тип) = ВРег("DGKod")) //!-- тип "Код контрольованої операції  3 знаки число з ведучими нулями -->
		...
		Исключение
			...
			КонецЕсли;
		КонецПопытки;		
	ИначеЕсли   (ВРег(Тип) = ВРег("DGOZN_NPR")) 
		//!--тип "Ознака неприбуткової установи (організації) (значення 0001-0019, та 0031-0048)"
		ИЛИ (ВРег(Тип) = ВРег("OZN_NPRColumn")) Тогда
		
		Если ТипЗнч(Значение)=Тип("Строка") Тогда
			Результат = СокрЛП(Значение);
		ИначеЕсли ТипЗнч(Значение)=Тип("Число") Тогда
			Результат = Формат(Значение,"ЧЦ=4; ЧДЦ=0; ЧВН=; ЧГ=0");
		Иначе
			Результат = СокрЛП(Значение);
		КонецЕсли;			
		
		ОписаниеОшибки = НСтр("ru='Значение не является числом от 0001-0019 или 0031-0048';uk='Значення не є числом 0001-0019 або 0031-0048'");					
