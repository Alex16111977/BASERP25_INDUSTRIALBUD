...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2025Мес7УФ";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/07/2025";
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("D10HBOS",  ИмяФАМИЛИЯРук, Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("D10HBUH",  ИмяФАМИЛИЯБух,  Форма, "ДекларацияАкциз");
...
Процедура ЗаполнитьСведенияОбОрганизацииПриложения6_8(Форма, НомПрил6 = Неопределено, НомПрил8 = Неопределено)
	...
	Для НомПрил = НачНом По КонКом Цикл
		...
		ЗаполнитьПоказательНаСервере("D8HBOS_" + ТекНомПрил,  ИмяДиректора, Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("D8HBUH_" + ТекНомПрил,  ИмяГлавБуха,  Форма, "ДекларацияАкциз");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КонецЦикла; 
	ЗаполнитьПоказательНаСервере("R011G1", ?(Форма.D6.Итог("D6R01G5")>0, "Х", ""), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R011G2", ?(Форма.D6.Итог("D6R01G5")>0, Форма.D6.Количество(), ""), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R011G3", ?(Форма.D6.Итог("D6R01G5")>0, Форма.D6.Количество(), ""), Форма, "ДекларацияАкциз");
	...
	Если ЭтоУточненка ИЛИ  HZMP8 Тогда 
		...
		ЗаполнитьПоказательНаСервере("R014G2", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R014G3", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
	Иначе
	...
	Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("D10T1RXXXXG2S_1", Форма, "ДекларацияАкциз")) Тогда     
		ЗаполнитьПоказательНаСервере("R016G1","Х", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R016G2", 1,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R016G3", 1,  Форма, "ДекларацияАкциз");
	Иначе
		ЗаполнитьПоказательНаСервере("R016G1", "", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R016G2", 0,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R016G3", 0,  Форма, "ДекларацияАкциз");
	КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Найти(ИмяЯчейки, "HBOS6") > 0 Тогда
			
		Значение = ТабличныйДокумент.Области.HBOS6.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HBOS6",  "ДекларацияАкциз",
																				 "", "",
																				 0, Ложь, Неопределено));
	...
	ИначеЕсли Найти(ИмяЯчейки, "HBUH6") > 0 Тогда
			
		Значение = ТабличныйДокумент.Области.HBUH6.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "HBUH6",  "ДекларацияАкциз",
																				 "", "",
																				 0, Ложь, Неопределено));
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	// заполнение ФИОв руководителей по всем приложениям
	Если    Найти(ИмяЯчейки, "HBOS6") > 0 
		ИЛИ Найти(ИмяЯчейки, "HBUH6") > 0 Тогда
		
		ЗаполнитьПоказательНаСервере("D1" + Лев(ИмяЯчейки,4), Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);
		ЗаполнитьПоказательНаСервере("D10" + Лев(ИмяЯчейки,4), Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);
		
		Для Сч = 1 По D6.Количество() Цикл
			НомСтр = Формат(Сч, "ЧГ=0");
			ЗаполнитьПоказательНаСервере("D6" + Лев(ИмяЯчейки,4)+ "_" + НомСтр, Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);
		КонецЦикла;
		Для Сч = 1 По D8.Количество() Цикл
			НомСтр = Формат(Сч, "ЧГ=0");
			ЗаполнитьПоказательНаСервере("D8" + Лев(ИмяЯчейки,4)+ "_" + НомСтр, Значение, ЭтаФорма, ДополнительныеПараметры.ИмяСтраницы);
		КонецЦикла;
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		Если Лев(Стр.Префикс,3) = "D10"  Тогда
			стр.фВыгрузка = 0;
		ИначеЕсли Лев(Стр.НазваниеСтраницыПанели, 11) = "Приложение8" и НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "ДекларацияАкциз")) Тогда
			стр.фВыгрузка = 0;
		КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для каждого Страница Из ТаблицаСтраницыРаздела1 Цикл
		...
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0209513", "F0209513");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D6.Количество() Цикл
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295613", "F0295613");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D8.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295813", "F0295813");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295913", "F0295913");  
