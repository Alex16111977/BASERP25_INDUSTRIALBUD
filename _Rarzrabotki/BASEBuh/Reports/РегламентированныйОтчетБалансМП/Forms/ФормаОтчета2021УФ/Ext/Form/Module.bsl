...
Процедура ЗаполнитьАвто(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ИспКлассыСчетов) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не установлены ""Используемые классы счетов расходов"" за %1 год! Автоматическое заполнение отчета невозможно!';uk='Не встановлені ""Використовувані класи рахунків витрат"" за %1 рік! Автоматичне заповнення звіту неможливе!'"), Год(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета)); 
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С, флЗаполнятьПрошлыйПериод)
	...
	// строка 160 Дебиторская задолженность за товары, резерв сомнительных долгов
	...
	_ДобавитьСтрокуРасшифровки( ЭтаФорма,"П11553", "",	"+", "!СНДР",П11553_66, Счет66);
	_ДобавитьСтрокуРасшифровки( ЭтаФорма,"П11554", "",	"+", "!СКДР",П11554_66, Счет66);
