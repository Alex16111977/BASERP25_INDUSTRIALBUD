...
&НаСервере
Функция ОбработатьЗаголовокПоказателя(ПоказательИмя, ДоступноеПолеЗаголовок)
	
	Перем РезультатЗаголовокПоказателя;
	
	РезультатЗаголовокПоказателя = "";
	Если ПоказательИмя = "Поступление" Тогда
		РезультатЗаголовокПоказателя = НСтр("ru='Поступление';uk='Надходження'");
	ИначеЕсли ПоказательИмя = "Списание" Тогда
		РезультатЗаголовокПоказателя = НСтр("ru='Списание';uk='Списання'");
	ИначеЕсли ПоказательИмя = "Приход" Тогда
		РезультатЗаголовокПоказателя = НСтр("ru='Приход';uk='Надходження'");
	ИначеЕсли ПоказательИмя = "Расход" Тогда
		РезультатЗаголовокПоказателя = НСтр("ru='Расход';uk='Видаток'");
	Иначе				
		РезультатЗаголовокПоказателя = ДоступноеПолеЗаголовок;
	КонецЕсли;	
	
	Возврат РезультатЗаголовокПоказателя;
...
Процедура ЗаполнитьТаблицуПоказателей(ПоказателиИсточник, ПоказателиПриемник, УсловноеОформление, СохраненныеНастройки = Неопределено)
	...
	Для каждого Показатель Из ПоказателиПриемник.Элементы Цикл
		...
		Если ДоступноеПоле = Неопределено Тогда
			...
			КонецЕсли;
		Иначе
			НоваяСтрока.Представление = ОбработатьЗаголовокПоказателя(ПоказательИмя, ДоступноеПоле.Заголовок);
		КонецЕсли;
...
Функция СтандартныеПоказателиСтруктура()
	...
	Для каждого Элемент Из СтандартныеПоказателиРеестра() Цикл
		...
		Если Элемент = "Ссылка" Тогда
			СтруктураПоказателя.Заголовок = НСтр("ru='Документ';uk='Документ'");
		ИначеЕсли Элемент = "ВалютаДокумента" Тогда
			СтруктураПоказателя.Заголовок = НСтр("ru='Валюта';uk='Валюта'");
		ИначеЕсли Элемент = "СуммаДокумента" Тогда
			СтруктураПоказателя.Заголовок = НСтр("ru='Сумма';uk='Сума'");
		ИначеЕсли Элемент = "ДатаВходящегоДокумента" Тогда
			СтруктураПоказателя.Заголовок = НСтр("ru='Дата вх.';uk='Дата вх.'");
		ИначеЕсли Элемент = "НомерВходящегоДокумента" Тогда
			СтруктураПоказателя.Заголовок = НСтр("ru='Номер вх.';uk='Номер вх.'");
		КонецЕсли;
