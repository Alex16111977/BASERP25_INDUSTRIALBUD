...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	Если ПараметрыОтчета.Кредиторская Тогда
		Возврат НСтр("ru='Кредиторская задолженность поставщикам по срокам долга на ';uk='Кредиторська заборгованість постачальникам по строках боргу на '") + Формат(ПараметрыОтчета.Период, "ДФ=dd.MM.yyyy; ДП=...");
...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	Если ПараметрыОтчета.Кредиторская Тогда 
		ПросроченнаяЗадолженность = ПолучитьПросроченнуюЗадолженностьКт(ПараметрыОтчета, ПараметрыОтчета.Период); 
	Иначе 
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ПараметрыОтчета.Кредиторская Тогда 
		Схема.НаборыДанных.ОсновнойНабор.Запрос = ПолучитьТекстЗапросаКредиторки(КоличествоИнтервалов);
	Иначе 
...
Функция ПолучитьТекстЗапросаКредиторки(КоличествоИнтервалов)
...
Функция ПолучитьПросроченнуюЗадолженность(ПараметрыОтчета, КонДата)
	...
	ТекстЗапросаПоОстаткам = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	                         ...
	                         |		ВзаиморасчетыОстатки.Субконто3 КАК СчетНаОплату,
	                         |		ВЫБОР
	                         |			КОГДА ВЫРАЗИТЬ(ВзаиморасчетыОстатки.Субконто3 КАК Документ.СчетНаОплатуПоставщика).УстановленСрокОплаты
	                         |				ТОГДА ВЫРАЗИТЬ(ВзаиморасчетыОстатки.Субконто3 КАК Документ.СчетНаОплатуПоставщика).СрокОплаты
...
Функция ПолучитьПросроченнуюЗадолженностьКт(ПараметрыОтчета, КонДата)
