...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("Кредиторская"                     , Отчет.Кредиторская); //Тютюн
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Отчет = Форма.Отчет;
	Если Отчет.Кредиторская Тогда 
		Название =  НСтр("ru='Кредиторская задолженность поставщикам по срокам долга';uk='Кредиторська заборгованість постачальникам по строках боргу'");
	Иначе
		Название =  НСтр("ru='Дебиторская задолженность поставщиков по срокам долга';uk='Дебіторська заборгованість постачальників по строках боргу'");
	КонецЕсли;
...
&НаСервере
Процедура КредиторскаяПриИзмененииНаСервере()
ОбновитьТекстЗаголовка(ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура КредиторскаяПриИзменении(Элемент)
	КредиторскаяПриИзмененииНаСервере();
КонецПроцедуры
