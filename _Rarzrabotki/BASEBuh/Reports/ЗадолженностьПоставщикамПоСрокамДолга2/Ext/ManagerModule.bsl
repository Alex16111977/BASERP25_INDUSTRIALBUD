...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "Период2", КонецДня(ПараметрыОтчета.Период2) + 1);
...
Процедура ЗаполнитьПоляВСоответствииСоСпискомИнтервалов(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	ПолеИтога.ПутьКДанным = "ОстатокДолга2";
	ПолеИтога.Выражение   = "Сумма(ОстатокДолга2)";  
	
	ПолеИтога = Схема.ПоляИтога.Добавить();
	ПолеИтога.ПутьКДанным = "Дт6442";
	ПолеИтога.Выражение   = "Сумма(Дт6442)";
	...
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(КомпоновщикНастроек, "Дт6442");
	БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(КомпоновщикНастроек, "ОстатокДолга2");
...
Функция ПолучитьТекстЗапроса(КоличествоИнтервалов)
	...
	|	ХозрасчетныйОстатки.СуммаРазвернутыйОстатокКт КАК ОстатокДолга02,
	|	ПДВ.СуммаОстатокДт КАК Дт6442
	...
	|						СчетаКД КАК СчетаКД),
	|				&ВидыСубконтоКД,
	|				ВЫРАЗИТЬ(Субконто2 КАК Справочник.ДоговорыКонтрагентов).ВидДоговора В (ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентом))) КАК ХозрасчетныйОстатки
	|		ПО ВзаиморасчетыОстатки.Субконто1 = ХозрасчетныйОстатки.Субконто1
	|			И ВзаиморасчетыОстатки.Субконто2 = ХозрасчетныйОстатки.Субконто2
	|			И ВзаиморасчетыОстатки.Счет = ХозрасчетныйОстатки.Счет
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет.Код = ""6442"", , ) КАК ПДВ
	|		ПО ВзаиморасчетыОстатки.Субконто2 = ПДВ.Субконто2
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Контрагент,
	...
	|	ВзаиморасчетыОстатки.ОстатокДолга02,
	|	ВзаиморасчетыОстатки.Дт6442
	...
	|	ОстатокИОбороты.ОстатокДолга02, 
	|	ОстатокИОбороты.Дт6442
	...
	|	ОстаткиПоПериодам.ОстатокДолга02 КАК ОстатокДолга2,
	|	ОстаткиПоПериодам.Дт6442 КАК Дт6442
	|{ВЫБРАТЬ
	|	Организация.*,
	|	Контрагент.*,
	|	Договор.*,
