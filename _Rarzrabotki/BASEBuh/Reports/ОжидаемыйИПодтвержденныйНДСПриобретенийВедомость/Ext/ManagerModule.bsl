...
Процедура ПриВыводеЗаголовка(ПараметрыОтчета,Результат) Экспорт
	...
	// Отбор
	ТекстОтбор = Строка(ПараметрыОтчета.НастройкиКомпоновкиДанных.Отбор);
...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат НСтр("ru='Ожидаемый и подтвержденный НДС приобретений (ведомость) ';uk='Очікуваний і підтверджений ПДВ придбань (відомість) '") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода,
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Для каждого ГруппыВыбранныхПолей Из ВыбранныеПоля Цикл
		
		Если ТипЗнч(ГруппыВыбранныхПолей) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
		
			Для каждого ПолеВГруппе Из ГруппыВыбранныхПолей.Элементы Цикл
				
			    ИмяПоля = Строка(ПолеВГруппе.Поле);
				Если СтрНайти(ИмяПоля, "СуммаНДСПропорционально") > 0 Тогда 
					ПолеВГруппе.Заголовок = НСтр("ru='Сумма НДС (пропорционально)';uk= 'Сума ПДВ (пропорційно)'");
				ИначеЕсли СтрНайти(ИмяПоля, "СуммаНДС") > 0 Тогда 
					ПолеВГруппе.Заголовок = НСтр("ru='Сумма НДС';uk= 'Сума ПДВ'");
				ИначеЕсли СтрНайти(ИмяПоля, "БазаНДС") > 0 Тогда 
					ПолеВГруппе.Заголовок = НСтр("ru='База НДС';uk= 'База ПДВ'");
				КонецЕсли;
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	Для Каждого ЗначениеРасшифровки Из Данные_Расшифровки Цикл                      
		Если ЗначениеРасшифровки.Ключ = "Валюта" Или ЗначениеРасшифровки.Ключ = "Контрагент" Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого СтрокаГруппировки Из ДанныеОтчета.Объект.Группировка Цикл
		Если СтрокаГруппировки.Использование Тогда
			...
			Если Данные_Расшифровки.Получить(СтрокаГруппировки.Поле) = Неопределено Тогда 
	...
	СписокПунктовМеню.Добавить("ОжидаемыйИПодтвержденныйНДСПриобретенийВедомость", НСтр("ru='Ожидаемый и подтвержденный НДС приобретений (ведомость)';uk= 'Очікуваний і підтверджений ПДВ придбань (відомість)'"));
	
	НастройкиРасшифровки = Новый Структура();
	НастройкиРасшифровки.Вставить("ОжидаемыйИПодтвержденныйНДСПриобретенийВедомость", ПользовательскиеНастройки);
