...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "П2T1R";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "26/02/2025";
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если    Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Уведомление" 
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = ПредставлениеПериода(НачалоДня(СтруктураРеквизитовФормы.ДатаНач), КонецДня(СтруктураРеквизитовФормы.ДатаКон), "ФП = Истина; Л="+РегламентированнаяОтчетностьКлиентСервер.ПолучитьКодЯзыкаИнтерфейса());
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("H01", "X",  Форма, "Приложение");	
	ЗаполнитьПоказательНаСервере("H02", "",  Форма, "Приложение");	
	...
	ЗаполнитьПоказательНаСервере("HKBOS", ИННФИОДиректора, 	Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("HKBUH", ИННГлавБуха, 		Форма, "Уведомление");
	//номер листа
	ЗаполнитьПоказательНаСервере("NUMList", 1, 		Форма, "Уведомление");
	ЗаполнитьПоказательНаСервере("NUMList", 1, 		Форма, "Приложение");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если Метаданные.Справочники.ФизическиеЛица.Реквизиты.Найти("УНЗР") = Неопределено Тогда
		...
		ТаблицаКадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
			...
			"ДатаПриема, ФизическоеЛицо, КодПоДРФО, ВидЗанятости, ПриказОПриеме, ПриказОПриемеНомер, ПриказОПриемеДата" + ВидДоговора,
	...
	Иначе
		ТаблицаКадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
			Истина, 
			ТаблицаСотрудниковОрганизации.ВыгрузитьКолонку("Сотрудник"),                                                     
			"ДатаПриема, ФизическоеЛицо, КодПоДРФО, УНЗР, ВидЗанятости, ПриказОПриеме, ПриказОПриемеНомер, ПриказОПриемеДата" + ВидДоговора,
			КонецДня(СтруктураРеквизитовФормы.ДатаКон));
	КонецЕсли;		
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ФизическиеЛица.Страна, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СтраныМира.Украина)
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|	КОНЕЦ КАК Гражданин
	...
	|	СотрудникиОрганизации.УНЗР КАК УНЗР,
	|	ВЫБОР
	|		КОГДА СотрудникиОрганизации.ВидДоговора = &ВидДоговораГигКонтракт
	|			ТОГДА 3
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Строка["T1RXXXXG9"] = Результат.Гражданин;
			Если Результат.Категория <= 2 Тогда 
				Строка["T1RXXXXG10S"] = РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Результат.НомерПриказа, Истина, Истина);
			КонецЕсли;	
			Строка["T1RXXXXG11D"] = Результат.ДатаПриказа;
			Строка["T1RXXXXG12D"] = Результат.ДатаНачала;
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	ПоказателиОтчета.Приложение.Вставить("HPAGES", 1);
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		Если стр.НазваниеСтраницыПанели = "Уведомление" Тогда
			стр.фВыгрузка = 1;
		Иначе				
			стр.фВыгрузка = 0;
		КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Иначе	
		НовСтрока= тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3001005"; 
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Уведомление о приеме работника на работу';uk='Повідомлення про прийняття працівника на роботу'");
		НовСтрока.НазваниеСтраницыПанели ="Уведомление";
		НовСтрока.Префикс = "";
		
		НовСтрока= тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3001101"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 2';uk='Додаток 2'");
		НовСтрока.НазваниеСтраницыПанели ="Приложение";
		НовСтрока.Префикс = "П2";
	КонецЕсли;
