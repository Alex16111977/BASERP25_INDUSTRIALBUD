...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	РасчетЧисленностиКакВ1ПВ = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("РегламентированнаяОтчетность", "РегламентированнаяОтчетность_НДФЛЕСВ_РасчетЧисленностиКакВ1ПВ",Истина);	
	СтруктураРеквизитовФормы.Вставить("РасчетЧисленностиКакВ1ПВ", РасчетЧисленностиКакВ1ПВ);
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Таблица 1';uk='Додаток 1'");
	...
	СтрокаУровня1.Представление            = НСтр("ru='Таблица 4';uk='Додаток 4'");
	СтрокаУровня1.ИмяОбластиТабличногоПоля = "";
	СтрокаУровня1.ОриентацияСтраницы       = ОриентацияСтраницы.Портрет;
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	...
	СтрокаУровня1.Представление            = НСтр("ru='Таблица 6';uk='Додаток 6'");
	СтрокаУровня1.ИмяОбластиТабличногоПоля = "";
	СтрокаУровня1.ОриентацияСтраницы       = ОриентацияСтраницы.Портрет;
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("РасчетЧисленностиКакВ1ПВ",  	СтруктураРеквизитовФормы.РасчетЧисленностиКакВ1ПВ);
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
    ...
    СписокСохранения.Вставить("РасчетЧисленностиКакВ1ПВ",  	СтруктураРеквизитовФормы.РасчетЧисленностиКакВ1ПВ);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	R061G3 = ?(ТаблицаМногострочнойЧастиПуста(СтруктураГруппы11, Форма.СтруктураРеквизитовФормы.КолонкиМногострочныхРазделов[Форма.СтруктураРеквизитовФормы.мИдГруппы11]), "", "X");
	
	R064G3 = ?(ТаблицаМногострочнойЧастиПуста(СтруктураГруппы41, Форма.СтруктураРеквизитовФормы.КолонкиМногострочныхРазделов[Форма.СтруктураРеквизитовФормы.мИдГруппы41]), "", "X");
	
	R065G3 = ?(ТаблицаМногострочнойЧастиПуста(СтруктураГруппы5, Форма.СтруктураРеквизитовФормы.КолонкиМногострочныхРазделов[Форма.СтруктураРеквизитовФормы.мИдГруппы5]), "", "X");
	
	R066G3 = ?(ТаблицаМногострочнойЧастиПуста(СтруктураГруппы61, Форма.СтруктураРеквизитовФормы.КолонкиМногострочныхРазделов[Форма.СтруктураРеквизитовФормы.мИдГруппы61]), "", "X");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Для СчетчикМесяцев = 0 по КолМесяцевВПериоде-1 Цикл 
		...
		Если ЗарплатаКадрыРасширенная Тогда
			...
			Попытка
				ТаблЧисленностиССЧ = КадровыйУчет.СреднесписочнаяЧисленностьРаботающих(СтруктураРеквизитовФормы.Организация, НачалоВЦикле, КонецВЦикле, СтруктураРеквизитовФормы.РасчетЧисленностиКакВ1ПВ); 
			Исключение
			...
			СтруктураДанныхДекларация["R1011G"+Строка(3+СчетчикМесяцев)] = ТаблЧисленностиССЧ.ЧисленностьИнвалидов;
			СтруктураДанныхДекларация["R1012G"+Строка(3+СчетчикМесяцев)] = ТаблЧисленностиССЧ.ЧисленностьЛицСГарантиями;
		...
		Иначе
			...
			ТаблСреднеспЧисленности = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
			СтруктураДанныхДекларация["R1011G"+Строка(3+СчетчикМесяцев)] =  ТаблСреднеспЧисленности[0].Средняя;
			
			ПараметрыСредняяЧисленность.Вставить("ОтборИнвалиды", Ложь);
			ПараметрыСредняяЧисленность.Вставить("ОтборДопГарантии", Истина);
			ТаблСреднеспЧисленности = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
			СтруктураДанныхДекларация["R1012G"+Строка(3+СчетчикМесяцев)] =  ТаблСреднеспЧисленности[0].Средняя;
		...
		СтруктураДанныхДекларация["R102G"+Строка(3+СчетчикМесяцев)] = УчетнаяЧисленность;
		СтруктураДанныхДекларация["R108G"+Строка(3+СчетчикМесяцев)] = СозданоНовыхРМ;
		...
		Если ЗарплатаКадрыРасширенная Тогда
			...
			|	ИЛИ ДанныеУчетаВремениПоСостояниям.ВидУчетаВремени В (&ВидыВремениДекретные)
			|	ИЛИ ДанныеУчетаВремениПоСостояниям.ВидУчетаВремени В (&ВидыВремениБолезнь)
			|СГРУППИРОВАТЬ ПО
			...
			|   СУММА(ОтработанноеВремяПоСотрудникам.ОплаченоДней) КАК КалендарныеДниБольничные
			|ПОМЕСТИТЬ ВТДниБолезни
			|ИЗ
			|  	РегистрНакопления.ОтработанноеВремяПоСотрудникам КАК ОтработанноеВремяПоСотрудникам
			|ГДЕ ОтработанноеВремяПоСотрудникам.Начисление.ОбозначениеВТабелеУчетаРабочегоВремени В (&ВидыВремениБолезнь)
			|    И ОтработанноеВремяПоСотрудникам.Период Между &ДатаНач И &ДатаКон
			|    И  НЕ ОтработанноеВремяПоСотрудникам.Регистратор В
			|			(ВЫБРАТЬ
			|			 	Больничные.Ссылка
			|			 ИЗ
			|			   Документ.БольничныйЛист КАК Больничные
			|            ГДЕ 
			|              Больничные.ЗарегистрироватьДляЕСВ)
			|			
			|СГРУППИРОВАТЬ ПО
			...
			|
			|ПОМЕСТИТЬ ВТДанныеУчетаВремениПоСостояниямПромежуточная
			|ИЗ
			|  	ВТДанныеУчетаВремениПоСостояниямСводно КАК ДанныеУчетаВремениПоСостояниям
			|СГРУППИРОВАТЬ ПО
			|   НАЧАЛОПЕРИОДА(ДанныеУчетаВремениПоСостояниям.Дата, МЕСЯЦ),
			...
			|	ДанныеУчетаВремениПоСостояниям.ФизическоеЛицо КАК ФизическоеЛицо,
			|	ЕСТЬNULL(ДниБолезни.КалендарныеДниБольничные, ДанныеУчетаВремениПоСостояниям.КалендарныеДниБольничные) КАК КалендарныеДниБольничные,
			|	ДанныеУчетаВремениПоСостояниям.КалендарныеДниОтпускПоБеременностиИРодам КАК КалендарныеДниОтпускПоБеременностиИРодам,
			|	ДанныеУчетаВремениПоСостояниям.КалендарныеДниОтпускаБезСохраненияЗП КАК КалендарныеДниОтпускаБезСохраненияЗП
			|
			|ИЗ
			|  	ВТДанныеУчетаВремениПоСостояниямПромежуточная КАК ДанныеУчетаВремениПоСостояниям
			|   ЛЕВОЕ СОЕДИНЕНИЕ
			|  	ВТДниБолезни КАК ДниБолезни
			|	ПО ДанныеУчетаВремениПоСостояниям.ФизическоеЛицо = ДниБолезни.ФизическоеЛицо
			|	 И ДанныеУчетаВремениПоСостояниям.Период = ДниБолезни.ПериодДействия
			|";
        ...
        |   ВЫБОР
		|		КОГДА ПараметрыКатегорийЕСВСрезПоследних.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииЗастрахованныхЛицЕСВ.РаботникПоГигКонтракту)
		|			И ПараметрыКатегорийЕСВСрезПоследних.Вид = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигКонтрактам)
		|			И ПараметрыКатегорийЕСВСрезПоследних.Код = 26
		|			ТОГДА 83
		|       ИНАЧЕ ПараметрыКатегорийЕСВСрезПоследних.Код 
		|	КОНЕЦ КАК КодКатегорииЗЛ,
		...
		Если ЗарплатаКадрыРасширенная Тогда
			...
			|					И ВТЕСВФОТ.ВидЕСВ <> ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.Больничные))
		...
		СоответствиеДни = Новый Соответствие();
		...
		Пока Выборка.Следующий() Цикл
			...
			КонецЕсли;
			Если Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.ОсновнаяЗарплата И Выборка.Период = НачалоВЦикле Тогда
				Если СоответствиеДни.Получить(Выборка.ФизическоеЛицо) = Неопределено Тогда 
					Строка[ПрефиксИмениКолонки+"G14"] = Выборка.РаботаДни;
					СоответствиеДни.Вставить(Выборка.ФизическоеЛицо, Выборка.РаботаДни);
				КонецЕсли;	
			...
			ИначеЕсли Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.ПоДоговорамГПХ Тогда	
				...
				КонецЕсли;
			ИначеЕсли Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.Декретные И Выборка.Период > НачалоВЦикле Тогда
                Строка[ПрефиксИмениКолонки+"G14"] = День(КонецМесяца(Выборка.Период));
			Иначе
			...
			Если ДоначислениеФОТ <> 0 ИЛИ ДоначислениеНалогФОТ <> 0 Тогда
				Если Строка[ПрефиксИмениКолонки+"G16"] = 0 И Строка[ПрефиксИмениКолонки+"G17"] = 0 И Строка[ПрефиксИмениКолонки+"G19"] = 0 
					...
					Номер = Номер - 1;
				ИначеЕсли Строка[ПрефиксИмениКолонки+"G16"] = 0 И Строка[ПрефиксИмениКолонки+"G17"] = 0 И Строка[ПрефиксИмениКолонки+"G19"] = 0 
					И Выборка.ВидЕСВ = Перечисления.ВидыЕСВ.ОсновнаяЗарплата И Выборка.ТипНачисления = 0 И Выборка.Период < НачалоМесяца(КонецВЦикле) Тогда
					//Пустаястрока за минувший период, используем ту же строку
					Номер = Номер - 1;
				Иначе
					Строка = СтруктураГруппы.Добавить();
				КонецЕсли;	
		...
		|    ВТЕСВФОТ КАК ЕСВФОТ
        |ГДЕ
		|    ЕСВФОТ.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигКонтрактам)
		|;
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|    ЕСВФОТ.ФизическоеЛицо,
		...
		|	 ЛЕВОЕ СОЕДИНЕНИЕ	
		|    ВТГиг КАК Гиг
		|    ПО ЕСВФОТ.ФизическоеЛицо = Гиг.ФизическоеЛицо
        |ГДЕ
		|    ЕСВФОТ.ВидЕСВ <> ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоДоговорамГПХ)
		|    И ЕСВФОТ.ВидЕСВ <> ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигКонтрактам)
		|    И Гиг.ФизическоеЛицо ЕСТЬ NULL
		|    И ЕСВФОТ.НалогФОТ > 0
		|";
		ТаблицаКоличестваЗастрахованныхЛиц = Запрос.Выполнить().Выгрузить();
		
		СтруктураДанныхДекларация["R103G"+Строка(СчетчикМесяцев+3)] = ТаблицаКоличестваЗастрахованныхЛиц.Количество();
		...
		СтруктураДанныхДекларация["R106G"+Строка(СчетчикМесяцев+3)] = НайденныеСтроки.Количество();	
		...
		СтруктураДанныхДекларация["R107G"+Строка(СчетчикМесяцев+3)] = НайденныеСтроки.Количество();	
	КонецЦикла;	
	...
	СтруктураРеквизитовФормы.мИдГруппы5 = "T5T1R";
		...
		Если ЗарплатаКадрыРасширенная Тогда
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			    ...
			    |       ИЛИ (КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Перемещение) И КадроваяИсторияСотрудников.ИзменениеДолжности И КадроваяИсторияСотрудников.ВидДоговора <> &ВидДоговораГигКонтракт) )
			    |;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		Иначе
			Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") = Истина Тогда
				...
				|ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
				|	ЕСВСотрудников.Сотрудник КАК Сотрудник
				|ПОМЕСТИТЬ ВТГигКонтракты
				|ИЗ
				|	РегистрСведений.ЕСВСотрудников КАК ЕСВСотрудников
				|
				|ГДЕ
				|	ЕСВСотрудников.Сотрудник.ГоловнаяОрганизация = &Организация
				|	И ЕСВСотрудников.КатегорияЕСВ.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигконтрактам)
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
			    ...
			    |	КадроваяИсторияСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
			    |	ВЫБОР
				|		КОГДА ДоговораГПХ.Сотрудник ЕСТЬ NULL И ГигКонтракты.Сотрудник ЕСТЬ NULL
				...
				|		КОГДА НЕ (ГигКонтракты.Сотрудник ЕСТЬ NULL)
				|		 ТОГДА 9
				|		 ИНАЧЕ 3
				|   КОНЕЦ  КАК ВидЗанятости, 
			    |	КадроваяИсторияСотрудников.ВидСобытия КАК ВидСобытия,
				...
				|   ЛЕВОЕ СОЕДИНЕНИЕ
				|	ВТГигКонтракты КАК ГигКонтракты
				|   ПО КадроваяИсторияСотрудников.Сотрудник = ГигКонтракты.Сотрудник
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
			    |ВЫБРАТЬ
				...
				|   ЛЕВОЕ СОЕДИНЕНИЕ
				|	ВТГигКонтракты КАК ГигКонтракты
				|   ПО ТаблицаКадровыеДанныеСотрудников.Сотрудник = ГигКонтракты.Сотрудник
                |
				|ГДЕ
				|   ТаблицаКадровыеДанныеСотрудников.ДатаУвольнения  МЕЖДУ &ДатаНач И &ДатаКон
				|";
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|   ВЫБОР
			|    КОГДА КадровыеСобытия.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы) 
			|     ТОГДА 1
			|    КОГДА КадровыеСобытия.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство) ИЛИ КадровыеСобытия.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство) 
			|     ТОГДА 2
			|    КОГДА КадровыеСобытия.ВидЗанятости = 3 
			|     ТОГДА 3
            |    КОГДА КадровыеСобытия.ВидЗанятости = 9 
			|     ТОГДА 9
			|     ИНАЧЕ 0
			|   КОНЕЦ КАК Категория,
			|   ФизическиеЛица.КодПоДРФО КАК КодПоДРФО,
...
Процедура Заполнить4ДФ(ПараметрыЗаполненияЗвит1С, ТаблицаКадровыеДанныеСотрудников)
	...
	Для СчетчикМесяцев = 0 по КолМесяцевВПериоде-1 Цикл 
		...
		|	ТаблицаНДФЛБезЛьгот.НалоговыйПериод КАК НалоговыйПериод,
		|	ВЫБОР
		|		КОГДА ВидыЛьготПоНДФЛ1.ГруппаЛьгот = ЛОЖЬ
		...
		|		КОГДА ВидыЛьготПоНДФЛ1.ГруппаЛьгот = ЛОЖЬ
		|			ТОГДА 0
		|		КОГДА ВидыЛьготПоНДФЛ.Ссылка = ВидыЛьготПоНДФЛ1.Льгота1
		|			ТОГДА 0
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЛьготПоНДФЛ КАК ВидыЛьготПоНДФЛ1
		|		ПО ВидыЛьготПоНДФЛ1.Ссылка = ТаблицаЛьгот.Льгота
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЛьготПоНДФЛ КАК ВидыЛьготПоНДФЛ
		|		ПО (ВидыЛьготПоНДФЛ1.ГруппаЛьгот = ИСТИНА)
		|			И (ВидыЛьготПоНДФЛ.Ссылка = ВидыЛьготПоНДФЛ1.Льгота1
		|				ИЛИ ВидыЛьготПоНДФЛ.Ссылка = ВидыЛьготПоНДФЛ1.Льгота2)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаНДФЛБезЛьгот.Контрагент,
		|	ВЫБОР КОГДА ТаблицаНДФЛБезЛьгот.Контрагент ССЫЛКА Справочник.ФизическиеЛица 
       ...
       Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		Для Каждого стр ИЗ ТаблицаНДФЛ Цикл
			...
			СтрокаНачалаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке номер ""%1"" приложения 4"": ';uk='У рядку номер ""%1"" додатку 4"": '"), СокрЛП(КолСтрок));
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("РасчетЧисленностиКакВ1ПВ", 		СтруктураРеквизитовФормы.РасчетЧисленностиКакВ1ПВ);
