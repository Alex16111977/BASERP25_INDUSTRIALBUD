...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2025Кв3УФ";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/10/2025";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для каждого СтрокаМЗ1 Из СтруктураМЗ1 Цикл   
		...
		Если СтрокаМЗ1.MZT1RXXXXG3 <> 0 Тогда 
			...
			СтрокаМЗ1.MZT1RXXXXG8 = (ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG4)
								  +	ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG6))
		...
		СтрокаМЗ1.MZT1RXXXXG13 = ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG9)
							   * ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG12)
		...
		Если СтрокаМЗ1.MZT1RXXXXG3 <> 0 Тогда  
			...
			СтрокаМЗ1.MZT1RXXXXG15 = ?(СтрокаМЗ1.MZT1RXXXXG13 = 0,
								   ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG14),
								   ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG13))
							 	   / ОкруглитьЧислоПоФормату(Форма, СтрокаМЗ1.MZT1RXXXXG3);
		...
		Если СтрокаМЗ1.MZT1RXXXXG8 >= 50 и СтрокаМЗ1.MZT1RXXXXG15 < 1400 Тогда   
			...
			СтрокаМЗ1.MZT1RXXXXG16 = 1400 * СтрокаМЗ1.MZT1RXXXXG3;  
		...
		Если СтрокаМЗ1.MZT1RXXXXG8 < 50 и СтрокаМЗ1.MZT1RXXXXG15 < 700 Тогда   
			...
			СтрокаМЗ1.MZT1RXXXXG17 = 700 * СтрокаМЗ1.MZT1RXXXXG3;  
...
Процедура ЗаполнитьПередачуНеприбыльным(ДатаНачала, ДатаКонца)
	...
	ПередачаНеприбыльным = ЗаполнитьПоСтатьямЗатрат("", СтруктураРеквизитовФормы.ПередачаНеприбыльным_СтатьиЗатрат, ДатаНачала, ДатаКонца);
...
Процедура ЗаполнитьПоказателиПрошогоПериода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если (Месяц(ДатаПрошлогоКвартала) = 3 и ГодПериода = 2025) или ГодПериода = 2024 Тогда	
			Если ЮрЛицо Тогда  
				ИмяСхемы = "J0100126";  // Декларация о прибыли 
			...
			Иначе
				ИмяСхемы = "F0100726";   
			КонецЕсли;	
		ИначеЕсли (Месяц(ДатаПрошлогоКвартала) = 6 и ГодПериода = 2025) Тогда	    
			Если ЮрЛицо Тогда  
		...
		Если ЮрЛицо Тогда
			...
			Иначе
				ИмяСхемы = ?(ЗначениеЗаполнено(СтруктураДанныхДекларация.H091), "J0151428","J0111428"); 
			КонецЕсли;
			...
			ПараметрыЗаполненияПоказателиПрошлогоГода.Вставить("ПараметрыЗапроса", Новый Структура("ИмяСхемы, ТипПериода, НомерПериода, ГодПериода", ИмяСхемы, ТипПериода, НомерПериода, ПрошлыйГодПериода));
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если ЮрЛицо Тогда
		...
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0100528", "J0100128");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151828", "J0111828");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150328", "J0110328");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152828", "J0112828");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150628", "J0110628");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151328", "J0111328");  
		...
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151428", "J0111428");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела1.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151528", "J0111528");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151628", "J0111628");  
		...
		Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "Декларация")) Тогда
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151728", "J0111728");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152028", "J0112028");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела2.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154428", "J0114428");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154828", "J0114828");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154928", "J0114928");  
