...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2025Кв2УФ";
   ...
   ИначеЕсли ИмяЯчейки = "H1KVP"
	...
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 3)) = "H09"  
		...
		ТекЗначение = ?(ЗначениеЗаполнено(ТабличныйДокумент.Области[ИмяЯчейки].Значение), "", "X");
 	    ...
 	    ТабличныйДокумент.Области.H104.Значение = "";
 	    ТабличныйДокумент.Области.H105.Значение = "";
 	    ТабличныйДокумент.Области.H106.Значение = "";
 	    ТабличныйДокумент.Области.H107.Значение = "";
 	    ТабличныйДокумент.Области.H108.Значение = "";
 	    ТабличныйДокумент.Области.H0109.Значение = "";
 	    ТабличныйДокумент.Области.HLIQ.Значение = "";
...
Процедура ЗаполнитьПриложениеРИ_ТЦ(ДатаНачала, ДатаКонца, НомерПер)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе			
			Попытка
				Если Выборка.Статья = СтНД.НП15_РИ_Фин_ДИЯ_ЕН Тогда 
					ЗаполнитьКорректировкуРИ("R03115G3", "+",  Выборка.Сумма, Выборка.Статья)
				КонецЕсли;
			Исключение
			КонецПопытки;
		КонецЕсли;
...
Процедура ЗаполнитьПоказателиПрошогоПериода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если (Месяц(ДатаПрошлогоКвартала) = 3 и ГодПериода = 2024) или ГодПериода = 2023 Тогда 
			Если ЮрЛицо Тогда  
				ИмяСхемы = "J0100125";  // Декларация о прибыли 
			...
			Иначе
				ИмяСхемы = "F0100725";   
			КонецЕсли;
		ИначеЕсли (Месяц(ДатаПрошлогоКвартала) = 3 и ГодПериода = 2025) или ГодПериода = 2024 Тогда	
			Если ЮрЛицо Тогда  
		...
		Если ЮрЛицо Тогда
			...
			Если ПрошлыйГодПериода = 2024 Тогда   //61932
				ИмяСхемы = ?(ЗначениеЗаполнено(СтруктураДанныхДекларация.H091), "J0151426","J0111426"); 
			Иначе
				ИмяСхемы = ?(ЗначениеЗаполнено(СтруктураДанныхДекларация.H091), "J0151427","J0111427"); 
			КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если ЮрЛицо Тогда
		...
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0100527", "J0100127");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151827", "J0111827");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150327", "J0110327");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152827", "J0112827");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150627", "J0110627");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151327", "J0111327");  
		...
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151427", "J0111427");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела1.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151527", "J0111527");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151627", "J0111627");  
		...
		Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "Декларация")) Тогда
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151727", "J0111727");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152027", "J0112027");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела2.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154427", "J0114427");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154827", "J0114827");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154927", "J0114927");  
