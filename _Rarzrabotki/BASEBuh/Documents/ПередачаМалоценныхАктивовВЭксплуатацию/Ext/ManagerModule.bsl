...
Функция ПечатьВводаОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		СписокМОЛ.Свернуть("Получил");
		СписокМА  = Результат.Выгрузить();
		СписокМА.Свернуть("Номенклатура"); 
		КоличествоМА = СписокМА.Количество() - 1; 
		ТЧМалоценныеАктивы = Ссылка.МалоценныеАктивы.Выгрузить();
		ТЧМалоценныеАктивы.Свернуть("Номенклатура");
		Если ТЧМалоценныеАктивы.Количество() <> КоличествоМА Тогда
			Сообщить(НСтр("ru='Внимание! На печать не выводятся МБП!';uk='Увага! На друк не виводяться МШП!'"));
		КонецЕсли;
		...
		Пока ВыборкаПоМОЛ.Следующий() Цикл    
			...
			ДанныеФизЛицаПолучил = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(Ссылка.Организация, ВыборкаПоМОЛ.Получил, Ссылка.Дата);
			ВыборкаПоОС = Результат.Выбрать();
			...
			Пока ВыборкаПоОС.Следующий() Цикл
				...
				Параметры.Заполнить(ВыборкаПоОС); 
				Параметры.Заполнить(ВыборкаПоШапке); 
				Параметры.Количество		  	  = ВыборкаПоОС.Количество;  
				Параметры.ЛиквидационнаяСтоимость = Формат(ВыборкаПоОС.ЛиквидационнаяСтоимость, "ЧЦ=15; ЧДЦ=2; ЧН=");   
				Параметры.Сумма				   	  = Параметры.Количество * ВыборкаПоОС.ПервоначальнаяСтоимость;
				КоличествоВсего					  = КоличествоВсего + Параметры.Количество;  
				СуммаВсего 		 				  = СуммаВсего + Параметры.Сумма;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктВводаОСБюдж") Тогда
		// Формируем табличный документ и добавляем его в коллекцию печатных форм.
		ИмяМакета = "";
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктВводаОСБюдж", НСтр("ru='Акт ввода в эксплуатацию основных средств';uk='Акт введення в експлуатацію основних засобів'"),
			ПечатьВводаОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета),, ИмяМакета);
	КонецЕсли;
