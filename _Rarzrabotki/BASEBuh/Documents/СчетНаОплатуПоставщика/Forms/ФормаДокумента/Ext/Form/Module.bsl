...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи) 
	//Тютюн
	Если Объект.СрокОплаты = 0 И ЗначениеЗаполнено(Объект.ДатаВходящегоДокумента) И ЗначениеЗаполнено(Объект.НомерВходящегоДокумента) Тогда 
		ПроверитьСрокОплатыВКазне();
	КонецЕсли; 
	//КонецИзменений
КонецПроцедуры
...
Процедура ВидимостьСрокОплаты() 
	Элементы.СрокОплаты.Видимость = Объект.УстановленСрокОплаты;
КонецПроцедуры

Процедура ПроверитьСрокОплатыВКазне()  
	
	Попытка
		COMConnector = Новый COMОбъект("V83.COMConnector");
		СтрокаПодключения = "Srvr=""127.0.0.1"";Ref=""kazna"";Usr=""1cAdmin"";Pwd=""!c@dmin""";
		Подключение = COMConnector.Connect(СтрокаПодключения);
	Исключение
		Сообщить("Помилка підключення до Казни: " + ОписаниеОшибки());
		Возврат;
	КонецПопытки;
	
	Попытка
		ЗапросКазна = Подключение.NewObject("Запрос");
		ЗапросКазна.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(СчетНаОплатуПоставщика.ДоговорКонтрагента.УстановленСрокОплаты, ЛОЖЬ) КАК Установлен,
		|	ЕСТЬNULL(СчетНаОплатуПоставщика.ДоговорКонтрагента.СрокОплаты, 0) КАК Срок
		|ИЗ
		|	Документ.СчетНаОплатуПоставщика КАК СчетНаОплатуПоставщика
		|ГДЕ
		|	СчетНаОплатуПоставщика.ДатаВходящегоДокумента = &Дата
		|	И СчетНаОплатуПоставщика.НомерВходящегоДокумента = &Номер
		|	И СчетНаОплатуПоставщика.Контрагент.ЕГРПОУ = &ЕДРПОУКонтрагента
		|	И СчетНаОплатуПоставщика.Организация.ЕГРПОУ = &ЕДРПОУОрганизации"; 
		
		ЗапросКазна.УстановитьПараметр("Номер",  Объект.НомерВходящегоДокумента);  
		ЗапросКазна.УстановитьПараметр("Дата", Объект.ДатаВходящегоДокумента);  
		ЗапросКазна.УстановитьПараметр("ЕДРПОУКонтрагента", Объект.Контрагент.КодПоЕДРПОУ);  
		ЗапросКазна.УстановитьПараметр("ЕДРПОУОрганизации", Объект.Организация.КодПоЕДРПОУ); 
		
		Выборка = ЗапросКазна.Выполнить().Выбрать();
		
		Если Выборка.Следующий() Тогда 
			
			Если Выборка.Установлен Тогда 
				Объект.УстановленСрокОплаты = Истина;  
				Попытка
					Срок = Выборка.Срок;
					Объект.СрокОплаты =  Срок;
				Исключение 
					Сообщить(ОписаниеОшибки());
				КонецПопытки;
				Элементы.СрокОплаты.Видимость = Истина;
				Сообщить("Встановлено срок оплати з договора в казні") ;
			КонецЕсли;
		КонецЕсли;
		
	Исключение
		Сообщить("Помилка отримання данних із Казни: " + ОписаниеОшибки());
	КонецПопытки;
