...
Процедура СкладПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		...
		ПодразделениеСкладПриИзмененииНаСервере();//Тютюн
	КонецЕсли;
...
Процедура ВидОперацииПриИзмененииНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.Оборудование") И  
			Элементы.Страницы.ТекущаяСтраница <> Элементы.ГруппаОборудование Тогда
		Элементы.Страницы.ТекущаяСтраница =  Элементы.ГруппаОборудование;			
	КонецЕсли;
...
Процедура ДобавитьРеквизиты()
 // Масив для нових реквізитів
    ...
    ПолеПодразделение.УстановитьДействие("ПриИзменении", "ПодразделениеПриИзменении");
	...
	Если ЗначениеЗаполнено(Объект.СС) Тогда
		Элементы.ТоварыНоменклатура.БыстрыйВыбор = Ложь;
		Элементы.ТоварыНоменклатура.ИсторияВыбораПриВводе = ИсторияВыбораПриВводе.НеИспользовать; 
	КонецЕсли;
...
Процедура ПодразделениеПриИзменении(Элемент)
    ...
    ПодразделениеСкладПриИзмененииНаСервере();
    
КонецПроцедуры


&НаСервере
Процедура ПодразделениеСкладПриИзмененииНаСервере()
    
	Если ЗначениеЗаполнено(Этаформа.Подразделение) И ЗначениеЗаполнено(Объект.Склад) Тогда
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	МассивНоменклатуры = ПолучитьНоменклатуру();
    
		
	Отбор.Вставить("Ссылка", Новый ФиксированныйМассив(МассивНоменклатуры));
	
	// Відкриваємо форму вибору з відбором
...
Функция ПолучитьНоменклатуру()  
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СтруктураСебестоимостиНоменклатура.ОбщееНазвание КАК ОбщееНазвание
	|ПОМЕСТИТЬ ОН
	|ИЗ
	|	Справочник.СтруктураСебестоимости.Комплектующие КАК СтруктураСебестоимостиНоменклатура
	|ГДЕ
	|	СтруктураСебестоимостиНоменклатура.Ссылка = &СС
	|
	|СГРУППИРОВАТЬ ПО
	|	СтруктураСебестоимостиНоменклатура.ОбщееНазвание
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	Номенклатура.ОбщееНазваниеНоменклатуры В
	|			(ВЫБРАТЬ
	|				ОН.ОбщееНазвание КАК ОбщееНазвание
	|			ИЗ
	|				ОН КАК ОН)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("СС", Объект.СС);
