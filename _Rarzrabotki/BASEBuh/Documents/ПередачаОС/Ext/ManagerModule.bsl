...
Функция ПечатьПриемаПередачиОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		Запрос.УстановитьПараметр("СписокОС"              , СписокОС);
		Запрос.УстановитьПараметр("ТекДата"               , Ссылка.МоментВремени());    
		Запрос.УстановитьПараметр("ТекДатаИзноса"         , ?(ЗначениеЗаполнено(Ссылка.ДокПодготовкаКПередачеОС), Ссылка.ДокПодготовкаКПередачеОС.МоментВремени(), Ссылка.МоментВремени()));
		Запрос.УстановитьПараметр("СостояниеПринятоКУчету", Перечисления.СостоянияОС.ВведеноВЭксплуатацию);
		Запрос.УстановитьПараметр("Организация"           , Ссылка.Организация);
		...
		|	ЕстьNULL(ХозрасчетныйОстатки.СуммаОстатокКт,0) КАК Износ
		|ИЗ
		|	Документ.ПередачаОС.ОС КАК ПередачаОС
		...
		Пока ВыборкаПоМОЛ.Следующий() Цикл      
			...
			ДанныеФизЛицаПолучил = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(Ссылка.Организация, Ссылка.Получил, Ссылка.Дата);
			Параметры.ПолучилДолжность = ДанныеФизЛицаПолучил.Должность;
			Параметры.ПолучилФИО = ФизическиеЛицаКлиентСервер.ИнициалыФамилия(ДанныеФизЛицаПолучил);
			...
			Параметры.СдалФИО 		= ФизическиеЛицаКлиентСервер.ИнициалыФамилия(ДанныеФизЛицаСдал);  
			ТабДокумент.Вывести(ОбластьМакета);
