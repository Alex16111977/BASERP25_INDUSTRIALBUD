...
Процедура УстановитьТипПричиныНевыдачиПокупателюПоУмолчанию(Объект) Экспорт
	...
	ИначеЕсли Объект.ТипПричиныНевыдачиПокупателю <> 22 Тогда
		...
		Если Объект.Контрагент.НеЯвляетсяРезидентом Тогда
			...
			Для каждого ТекТЧ Из СтруктураТЧ Цикл
				...
				Для каждого СтрокаТЧ Из Объект[ТекТЧ.Ключ] Цикл
					
					Если СтрокаТЧ.КодУКТВЭД.Вид = Перечисления.ВидыКодовДляНалоговойНакладной.КодУслуги Тогда
						ЕстьУслуги = Истина;
					Иначе
...
Процедура ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ДанныеОбъекта, СтрокаТЧ, ИмяТабЧасти, СчетаУчета) Экспорт
	...
	ИначеЕсли ДанныеОбъекта.ТипПричиныНевыдачиПокупателю = 22 Тогда

		СтрокаТЧ.СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОЭкспортОтдВидовСХПродукции;		
...
Процедура УстановитьКодУКТВЭДВстрокеТоваров(СтрокаТЧ) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) Тогда
		...
		Если ТипЗнч(ЗначениеНоменклатураГТД) = Тип("СправочникСсылка.НоменклатураГТД") Тогда
			...
			Если СтрокаТЧ.КодУКТВЭД = Неопределено Тогда
				СтрокаТЧ.КодУКТВЭД = Справочники.КлассификаторУКТВЭД.ПустаяСсылка();	
			КонецЕсли;                 
...
Процедура ЗаполнитьСуммыРегл(Объект, ИзмениласьСуммаБезНДС = Истина, ИзмениласьСтавкаНДС = Ложь) Экспорт
	...
	Если Объект.ВалютаДокумента = ВалютаРегл Тогда
		...
		Для каждого ТекТЧ Из СтруктураТЧ Цикл
			Для каждого СтрокаТаблицы ИЗ Объект[ТекТЧ.Ключ]	Цикл
				СтрокаТаблицы.СуммаБезНДСРегл = 0;
				СтрокаТаблицы.СуммаНДСРегл = 0;
			КонецЦикла;	
		КонецЦикла;
	...
	Иначе
		...
		Для каждого ТекТЧ Из СтруктураТЧ Цикл
			Для каждого СтрокаТаблицы ИЗ Объект[ТекТЧ.Ключ]	Цикл
				...
				Если ИзмениласьСуммаБезНДС Тогда
					СтрокаТаблицы.СуммаБезНДСРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(?(Объект.СуммаВключаетНДС, СтрокаТаблицы.Сумма - СтрокаТаблицы.СуммаНДС, СтрокаТаблицы.Сумма), 
				...
				Если ИзмениласьСуммаБезНДС ИЛИ ИзмениласьСтавкаНДС Тогда
...
Функция СостояниеСтрокНалоговойНакладнойСУчетомКорректировок(НалоговаяНакладная, ИсключаемоеП2 = Неопределено) Экспорт
	...
	Результат.Колонки.Добавить("СуммаБезНДСРегл");
	Результат.Колонки.Добавить("СуммаНДСРегл");
	...
	|	НалоговаяНакладнаяТовары.НомерСтрокиНН КАК НомерСтрокиНН,
	...
	|	НалоговаяНакладнаяТовары.КодУКТВЭД КАК КодУКТВЭД,
	...
	|	Приложение2КНалоговойНакладнойТовары.КодУКТВЭД,
	|	ВЫБОР
	|		КОГДА Приложение2КНалоговойНакладнойТовары.ИзменениеКоличества = 0
	|			ТОГДА Приложение2КНалоговойНакладнойТовары.Количество
	|		ИНАЧЕ Приложение2КНалоговойНакладнойТовары.ИзменениеКоличества
	|	КОНЕЦ,
	|	Приложение2КНалоговойНакладнойТовары.Вес,
	|	Приложение2КНалоговойНакладнойТовары.ИзменениеСуммыНДС,
	|	Приложение2КНалоговойНакладнойТовары.ИзменениеСуммы,
	|	Приложение2КНалоговойНакладнойТовары.ИзменениеСуммыБезНДСРегл,
	|	Приложение2КНалоговойНакладнойТовары.ИзменениеСуммыНДСРегл,
	...
	|	НалоговаяНакладнаяУслуги.НомерСтрокиНН,
	...
	|	НалоговаяНакладнаяУслуги.СуммаБезНДСРегл,
	|	НалоговаяНакладнаяУслуги.СуммаНДСРегл,
	|	НалоговаяНакладнаяУслуги.Ссылка.Дата,
	|	ИСТИНА
	|ИЗ
	|	Документ.НалоговаяНакладная.Услуги КАК НалоговаяНакладнаяУслуги
	|ГДЕ
	|	НалоговаяНакладнаяУслуги.Ссылка = &НалоговаяНакладная
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Приложение2КНалоговойНакладнойУслуги.Ссылка,
	|	""Услуги"",
	...
	|	Приложение2КНалоговойНакладнойУслуги.КодУКТВЭД,
	|	ВЫБОР
	|		КОГДА Приложение2КНалоговойНакладнойУслуги.ИзменениеКоличества = 0
	|			ТОГДА Приложение2КНалоговойНакладнойУслуги.Количество
	|		ИНАЧЕ Приложение2КНалоговойНакладнойУслуги.ИзменениеКоличества
	|	КОНЕЦ,
	|	0,
	|	Приложение2КНалоговойНакладнойУслуги.ИзменениеСуммыНДС,
	|	Приложение2КНалоговойНакладнойУслуги.ИзменениеСуммы,
	|	Приложение2КНалоговойНакладнойУслуги.ИзменениеСуммыБезНДСРегл,
	|	Приложение2КНалоговойНакладнойУслуги.ИзменениеСуммыНДСРегл,
	...
	|ГДЕ
	|	Приложение2КНалоговойНакладнойУслуги.Ссылка.НалоговаяНакладная = &НалоговаяНакладная
	|	И Приложение2КНалоговойНакладнойУслуги.Ссылка.Проведен
	|	И Приложение2КНалоговойНакладнойУслуги.Ссылка <> &ИсключаемоеП2
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	НалоговаяНакладнаяОС.НомерСтрокиНН,
	|	НалоговаяНакладнаяОС.ОсновноеСредство,
	|	НЕОПРЕДЕЛЕНО,
	|	НалоговаяНакладнаяОС.СтавкаНДС,
	|	НалоговаяНакладнаяОС.КодУКТВЭД,
	|	НалоговаяНакладнаяОС.Количество,
	|	0,
	|	НалоговаяНакладнаяОС.СуммаНДС,
	|	НалоговаяНакладнаяОС.Сумма,
	|	НалоговаяНакладнаяОС.СуммаБезНДСРегл,
	|	НалоговаяНакладнаяОС.СуммаНДСРегл,
	|	НалоговаяНакладнаяОС.Ссылка.Дата,
	|	ИСТИНА
	|ИЗ
	|	Документ.НалоговаяНакладная.ОС КАК НалоговаяНакладнаяОС
	|ГДЕ
	|	НалоговаяНакладнаяОС.Ссылка = &НалоговаяНакладная
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Приложение2КНалоговойНакладнойОС.Ссылка,
	|	""ОС"",
	|	Приложение2КНалоговойНакладнойОС.НомерСтроки,
	|	Приложение2КНалоговойНакладнойОС.НомерСтрокиНН,
	|	Приложение2КНалоговойНакладнойОС.ОсновноеСредство,
	|	НЕОПРЕДЕЛЕНО,
	|	Приложение2КНалоговойНакладнойОС.СтавкаНДС,
	|	Приложение2КНалоговойНакладнойОС.КодУКТВЭД,
	|	Приложение2КНалоговойНакладнойОС.ИзменениеКоличества,
	|	0,
	|	Приложение2КНалоговойНакладнойОС.ИзменениеСуммыНДС,
	|	Приложение2КНалоговойНакладнойОС.ИзменениеСуммы,
	|	Приложение2КНалоговойНакладнойОС.ИзменениеСуммыБезНДСРегл,
	|	Приложение2КНалоговойНакладнойОС.ИзменениеСуммыНДСРегл,
	...
	|ГДЕ
	|	Приложение2КНалоговойНакладнойОС.Ссылка.НалоговаяНакладная = &НалоговаяНакладная
	|	И Приложение2КНалоговойНакладнойОС.Ссылка.Проведен
	|	И Приложение2КНалоговойНакладнойОС.Ссылка <> &ИсключаемоеП2
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	НалоговаяНакладнаяНМА.НомерСтрокиНН,
	|	НалоговаяНакладнаяНМА.НематериальныйАктив,
	|	НЕОПРЕДЕЛЕНО,
	|	НалоговаяНакладнаяНМА.СтавкаНДС,
	|	НалоговаяНакладнаяНМА.КодУКТВЭД,
	|	НалоговаяНакладнаяНМА.Количество,
	|	0,
	|	НалоговаяНакладнаяНМА.СуммаНДС,
	|	НалоговаяНакладнаяНМА.Сумма,
	|	НалоговаяНакладнаяНМА.СуммаБезНДСРегл,
	|	НалоговаяНакладнаяНМА.СуммаНДСРегл,
	|	НалоговаяНакладнаяНМА.Ссылка.Дата,
	|	ИСТИНА
	|ИЗ
	|	Документ.НалоговаяНакладная.НМА КАК НалоговаяНакладнаяНМА
	|ГДЕ
	|	НалоговаяНакладнаяНМА.Ссылка = &НалоговаяНакладная
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Приложение2КНалоговойНакладнойНМА.Ссылка,
	|	""НМА"",
	|	Приложение2КНалоговойНакладнойНМА.НомерСтроки,
	|	Приложение2КНалоговойНакладнойНМА.НомерСтрокиНН,
	|	Приложение2КНалоговойНакладнойНМА.НематериальныйАктив,
	|	НЕОПРЕДЕЛЕНО,
	|	Приложение2КНалоговойНакладнойНМА.СтавкаНДС,
	|	Приложение2КНалоговойНакладнойНМА.КодУКТВЭД,
	|	Приложение2КНалоговойНакладнойНМА.ИзменениеКоличества,
	|	0,
	|	Приложение2КНалоговойНакладнойНМА.ИзменениеСуммыНДС,
	|	Приложение2КНалоговойНакладнойНМА.ИзменениеСуммы,
	|	Приложение2КНалоговойНакладнойНМА.ИзменениеСуммыБезНДСРегл,
	|	Приложение2КНалоговойНакладнойНМА.ИзменениеСуммыНДСРегл,
	...
	|ГДЕ
	|	Приложение2КНалоговойНакладнойНМА.Ссылка.НалоговаяНакладная = &НалоговаяНакладная
	|	И Приложение2КНалоговойНакладнойНМА.Ссылка.Проведен
	|	И Приложение2КНалоговойНакладнойНМА.Ссылка <> &ИсключаемоеП2
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИсходныеСтроки.НомерСтрокиНН КАК НомерСтрокиНН,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсходныеСтроки.Ссылка) КАК КоличествоДокументов,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсходныеСтроки.ИмяТЧ) КАК КоличествоИмяТЧ,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсходныеСтроки.Номенклатура) КАК КоличествоНоменклатура,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсходныеСтроки.ЕдиницаИзмерения) КАК КоличествоЕдиницаИзмерения,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсходныеСтроки.СтавкаНДС) КАК КоличествоСтавкаНДС,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсходныеСтроки.КодУКТВЭД) КАК КоличествоКодУКТВЭД,
	|	СУММА(ИсходныеСтроки.Количество) КАК Количество,
	|	СУММА(ИсходныеСтроки.Вес) КАК Вес,
	|	СУММА(ИсходныеСтроки.СуммаНДС) КАК СуммаНДС,
	|	СУММА(ИсходныеСтроки.Сумма) КАК Сумма,
	|	СУММА(ИсходныеСтроки.СуммаБезНДСРегл) КАК СуммаБезНДСРегл,
	|	СУММА(ИсходныеСтроки.СуммаНДСРегл) КАК СуммаНДСРегл
	|ПОМЕСТИТЬ СверткаПоНомерСтрокиНН
	|ИЗ
	|	ИсходныеСтроки КАК ИсходныеСтроки
	|
	|СГРУППИРОВАТЬ ПО
	|	ИсходныеСтроки.НомерСтрокиНН
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СверткаПоНомерСтрокиНН.НомерСтрокиНН КАК НомерСтрокиНН,
	|	ВЫБОР
	|		КОГДА СверткаПоНомерСтрокиНН.КоличествоДокументов > 2
	|				ИЛИ СверткаПоНомерСтрокиНН.НомерСтрокиНН = 0
	|				ИЛИ СверткаПоНомерСтрокиНН.КоличествоИмяТЧ > 1
	|				ИЛИ СверткаПоНомерСтрокиНН.КоличествоНоменклатура > 1
	|				ИЛИ СверткаПоНомерСтрокиНН.КоличествоЕдиницаИзмерения > 1
	|				ИЛИ СверткаПоНомерСтрокиНН.КоличествоСтавкаНДС > 1
	|				ИЛИ СверткаПоНомерСтрокиНН.КоличествоКодУКТВЭД > 1
	|				ИЛИ СверткаПоНомерСтрокиНН.КоличествоДокументов = 2
	|					И (СверткаПоНомерСтрокиНН.Количество <> 0
	|						ИЛИ ИсходныеСтроки.КодУКТВЭД.РежимЭкспортногоОбеспечения
	|							И СверткаПоНомерСтрокиНН.Вес <> 0
	|						ИЛИ СверткаПоНомерСтрокиНН.СуммаНДС <> 0
	|						ИЛИ СверткаПоНомерСтрокиНН.Сумма <> 0)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЕстьОшибки,
	|	ВЫБОР
	|		КОГДА СверткаПоНомерСтрокиНН.КоличествоДокументов = 1
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЕстьОстаток,
	|	ИсходныеСтроки.Ссылка КАК НалоговыйДокумент,
	|	ИсходныеСтроки.НомерСтроки КАК НомерСтроки,
	|	ИсходныеСтроки.ИмяТЧ КАК ИмяТЧ,
	|	ИсходныеСтроки.СуммаНДС КАК СуммаНДС,
	|	ИсходныеСтроки.Сумма КАК Сумма,
	|	ИсходныеСтроки.СуммаБезНДСРегл КАК СуммаБезНДСРегл,
	|	ИсходныеСтроки.СуммаНДСРегл КАК СуммаНДСРегл
	|ИЗ
	|	СверткаПоНомерСтрокиНН КАК СверткаПоНомерСтрокиНН
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИсходныеСтроки КАК ИсходныеСтроки
	|		ПО СверткаПоНомерСтрокиНН.НомерСтрокиНН = ИсходныеСтроки.НомерСтрокиНН
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтрокиНН,
	|	ИсходныеСтроки.ЭтоНН УБЫВ,
	|	ИсходныеСтроки.ДатаДокумента,
	|	НалоговыйДокумент,
	|	ИмяТЧ,
	|	НомерСтроки
	|ИТОГИ
	...
	ОписаниеСоставаСтроки = Новый ТаблицаЗначений;
	ОписаниеСоставаСтроки.Колонки.Добавить("НалоговыйДокумент");
	ОписаниеСоставаСтроки.Колонки.Добавить("ИмяТЧ");
	ОписаниеСоставаСтроки.Колонки.Добавить("НомерСтроки");
	...
	Пока ВыборкаСтрокаНН.Следующий() Цикл
		...
		СоставСтроки = ОписаниеСоставаСтроки.СкопироватьКолонки();
...
Функция РассчитатьВес(Номенклатура, Количество, Коэффициент, КодУКТВЭД) Экспорт
	
	Результат = 0;
	Если ЗначениеЗаполнено(КодУКТВЭД) И КодУКТВЭД.РежимЭкспортногоОбеспечения Тогда
		
		КоэфКГ = 0;
		
		Для каждого СтрокаЕдИзм Из Номенклатура.ЕдиницыИзмерения Цикл
			
			Если СтрокаЕдИзм.ЕдиницаИзмерения.Код = "0301" Тогда
				КоэфКГ = СтрокаЕдИзм.Коэффициент;
				Прервать;
			КонецЕсли;
				
		КонецЦикла;      
		
		Если КоэфКГ <> 0 И Коэффициент <> 0 Тогда
			Результат = Количество * Коэффициент / КоэфКГ;
		КонецЕсли;
...
Процедура ЗаполнитьВесТоваров(Объект) Экспорт
	
	Для каждого СтрокаТоваров Из Объект.Товары Цикл
		
		СтрокаТоваров.Вес = РассчитатьВес(СтрокаТоваров.Номенклатура, СтрокаТоваров.Количество, СтрокаТоваров.Коэффициент, СтрокаТоваров.КодУКТВЭД);
	...
	Для Каждого Объект Из МассивОбъектов  Цикл
		...
		Если ТекРабочаяДата >= '2016-04-01' Тогда
			...
			Если НЕ ВыгрузкаПечатьНН_ПоФормеНаДатуДокумента = Истина Тогда
				...
				Если ТекРабочаяДата >= '2024-10-01' Тогда					
					
					НовТабДокумент =  ПечатьДокументаНалоговыйКодекс2024(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, "НалоговаяНакладная2024", Объект);
		            ТабДокументСформирован = Истина;
					
				ИначеЕсли ЗначениеЗаполнено(ДатаНачалаИспользованияФормыНН15) И ТекРабочаяДата >= ДатаНачалаИспользованияФормыНН15 Тогда					
		...
		Если НЕ ТабДокументСформирован Тогда
			...
			Если Объект.Дата >= '2024-10-01' Тогда					
				
				НовТабДокумент =  ПечатьДокументаНалоговыйКодекс2024(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, "НалоговаяНакладная2024", Объект);
	            ТабДокументСформирован = Истина;
				
			ИначеЕсли ЗначениеЗаполнено(ДатаНачалаИспользованияФормыНН15) И Объект.Дата >= ДатаНачалаИспользованияФормыНН15 Тогда
...
Функция ПечатьДокументаНалоговыйКодекс2023(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета = "НалоговаяНакладнаяАльбом", Объект)
	...
	Для Страница = 1 По 1 Цикл
		...
		ВремТаблица.Свернуть("СтавкаНДС", "Сумма, СуммаНДС");
		Для каждого СтрокаТаблицы Из ВремТаблица Цикл                     
			КолонкаДляСуммы = ОпределитьКодСтавки(ОпределитьКолонкуТаблицы(СтрокаТаблицы.СтавкаНДС, ВалютаДокумента, КонтрагентНеРезидент, Объект.Дата, Шапка.ТипПричиныНевыдачиПокупателю));
			Если  Шапка.СоглашениеОРазделеПродукции = Истина Тогда
				// данные не заполняются
			ИначеЕсли КолонкаДляСуммы = "20" Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
			...
			// Определим в какую колонку (в зависимости от ставки НДС) попадает сумма без НДС
			ОбластьМакета.Параметры.КодСтавки = ОпределитьКодСтавки(ОпределитьКолонкуТаблицы(СтрокаТаблицы.СтавкаНДС, ВалютаДокумента, КонтрагентНеРезидент, Объект.Дата, Шапка.ТипПричиныНевыдачиПокупателю));
...
Функция ПечатьДокументаНалоговыйКодекс2024(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета = "НалоговаяНакладнаяАльбом", Объект)
	...
	|	НалоговаяНакладная.ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены КАК ТипПричиныНевыдачиПокупателюПриПродажеНижеОбычнойЦены,
	|	НалоговаяНакладная.ТекстДополненияКНаименованиюПриПродажеНижеОбычнойЦены КАК ТекстДополненияКНаименованиюПриПродажеНижеОбычнойЦены,
	|	НалоговаяНакладная.Контрагент КАК Контрагент,
	|	НалоговаяНакладная.Контрагент.ГоловнойКонтрагент КАК ГоловнойКонтрагент,
	|	НалоговаяНакладная.Контрагент.ОбособленноеПодразделение КАК КонтрагентОбособленноеПодразделение,
	|	НалоговаяНакладная.Контрагент.ДополнятьНаименованиеАдресДаннымиГоловногоКонтрагентаВНН КАК ДополнятьНаименованиеАдресДаннымиГоловногоКонтрагентаВНН,
	|	НалоговаяНакладная.Контрагент.КодФилиала КАК ФилиалПокупателя,
	|	НалоговаяНакладная.ДоговорКонтрагента.МногостороннееСоглашениеОРазделеПродукции КАК СоглашениеОРазделеПродукции,
	|	НалоговаяНакладная.Организация КАК Организация,
	|	НалоговаяНакладная.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
	|	НалоговаяНакладная.Организация.ОбособленноеПодразделение КАК ОбособленноеПодразделение,
	...
	|	НалоговаяНакладная.РеквизитыНомерСтрокиННАктуальны КАК РеквизитыНомерСтрокиННАктуальны,
	...
	|	НалоговаяНакладная.ДоговорКонтрагента.Дата КАК ДатаДоговора,
	|	НалоговаяНакладная.ДоговорКонтрагента.Номер КАК НомерДоговора,
	|	ISNULL(НалоговаяНакладная.ДоговорКонтрагента.ФорвардныйКонтракт, ЛОЖЬ) КАК ФорвардныйКонтракт,
	...
	Для каждого СтрокаТоваров Из ТаблицаНоменклатуры Цикл
		Если ЗначениеЗаполнено(СтрокаТоваров.Вес) И СтрокаТоваров.РежимЭкспортногоОбеспечения Тогда
			СтрокаТоваров.Количество = СтрокаТоваров.Вес;
			СтрокаТоваров.ЕдиницаИзмерения = "кг";
			СтрокаТоваров.ЕдиницаИзмеренияКод = "0301";
			СтрокаТоваров.ЦенаБезНДСРегл = Окр(СтрокаТоваров.СуммаБезНДСРегл / СтрокаТоваров.Вес, 2);
		КонецЕсли;
	КонецЦикла;
	...
	Если  Шапка.ВидОперации = Перечисления.ВидыОперацийНалоговаяНакладная.РаботыОтНерезидента Тогда
		...
		Страна = БухгалтерскийУчетПереопределяемый.ПолучитьСтрануИзКонтактнойИнформации(Шапка.Контрагент, "Юридический");			
		Если НЕ ЗначениеЗаполнено(Страна) Тогда
			ПредупрежденияПечати.Вставить("СтранаКонтрагент", "" + Шапка.Контрагент + ": " + НСтр("ru='не указана страна регистрации контрагента-нерезидента (в контактной информации)! Адрес в налоговой накладной заполнен не будет!';uk='не вказана країна реєстрації контрагента-нерезидента (у контактній інформації)! Адреса в податковій накладній заповнена не буде!'"));
		Иначе
			НазваниеОрганизации = НазваниеОрганизации + ", " + Страна		
		КонецЕсли;
	...
	Для Страница = 1 По 1 Цикл
		...
		Если Шапка.ТипПричиныНевыдачиПокупателю = 7 ИЛИ Шапка.ТипПричиныНевыдачиПокупателю = 22 Тогда
			
			ОбластьМакетаЗаголовок.Параметры.ДатаДоговора = Шапка.ДатаДоговора; 
			ОбластьМакетаЗаголовок.Параметры.НомерДоговора = Шапка.НомерДоговора;
			ОбластьМакетаЗаголовок.Параметры.ФорвардныйКонтракт = ?(Шапка.ФорвардныйКонтракт, "Х", "");
			
		КонецЕсли;
		Если Шапка.ТипПричиныНевыдачиПокупателю = 7 Тогда
			
			ОбластьМакетаЗаголовок.Параметры.ДатаГТД = Шапка.ДатаГТД; 
			ОбластьМакетаЗаголовок.Параметры.НомерГТД = Шапка.НомерГТД;
		...
		Если Шапка.ТипПричиныНевыдачиПокупателю = 22 Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) Тогда
				Если НЕ ПредупрежденияПечати.Свойство("ПустаяНоменклатура") Тогда
					ПредупрежденияПечати.Вставить("ПустаяНоменклатура", НСтр("ru='в одной из строк не заполнено наименование номенклатуры - проверьте заполнение документа!';uk='в одному з рядків не заповнене найменування номенклатури - перевірте заповнення документа!'"));
				КонецЕсли;
			КонецЕсли;
			...
			ИначеЕсли СтрокаТаблицы.КодВид = Перечисления.ВидыКодовДляНалоговойНакладной.КодТовараИмпортированного Тогда
				ОбластьМакета.Параметры.Импортированный = "X";
			ИначеЕсли СтрокаТаблицы.КодВид = Перечисления.ВидыКодовДляНалоговойНакладной.КодСХПродукции Тогда
			...
			ИначеЕсли Шапка.Сводная = Истина И (Шапка.ВидОперации = Перечисления.ВидыОперацийНалоговаяНакладная.УсловнаяПродажа ИЛИ Шапка.ВидОперации = Перечисления.ВидыОперацийНалоговаяНакладная.УсловнаяПродажаСписаниеОС) 
				И Шапка.ТипПричиныНевыдачиПокупателю <> 13 Тогда
	...
	Если ПредупрежденияПечати.Количество() Тогда
		...
		ТекстПредупреждения = НСтр("ru='Печать документа ';uk= 'Друк документу '") + Объект + ":";
		ПервыйПроход = Истина;
...
//
Функция ОпределитьКолонкуТаблицы(СтавкаНДС=Неопределено, ВалютаДокумента, КонтрагентНеРезидент, Дата, ТипПричиныНевыдачи = Неопределено)	
