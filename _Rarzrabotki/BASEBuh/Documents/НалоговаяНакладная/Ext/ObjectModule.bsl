...
Процедура ЗаполнитьСводуюУсловнуюПродажу(ВидНалоговогоНазначения) Экспорт	
	...
	ИначеЕсли ВидНалоговогоНазначения = "НеоблагаемаяНеХозДеятельность" Тогда	
		...
		ЗаполнитьСводуюУсловнуюПродажуНехоз(МассивСтатей);
		Возврат;
...
Процедура ЗаполнитьСводуюУсловнуюПродажуНехоз(СтатьиДекларации)
	...
	АвтоТовар = БухгалтерскийУчетПереопределяемый.ПолучитьЗначениеПоУмолчанию("НоменклатураДляЗаполненияНалоговыхНакладных");	
	Если НЕ ЗначениеЗаполнено(АвтоТовар) Тогда
		Сообщить(НСтр("ru='Для пользователя не указано значение ""Номенклатура для автоматического заполнения налоговых накладных"" в индивидуальных настройках!';uk='Для користувача не вказане значення ""Номенклатура для автоматичного заповнення податкових накладних"" в індивідуальних настройках!'"), СтатусСообщения.Важное);
	КонецЕсли; 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НДСНалоговыйКредит.Регистратор КАК Регистратор,
	|	НДСНалоговыйКредит.БазаНДСОборот КАК БазаНДСОборот,
	|	НДСНалоговыйКредит.НДСОборот КАК СуммаНДСОборот
	...
	|	(НДСНалоговыйКредит.Регистратор ССЫЛКА Документ.РегистрацияВходящегоНалоговогоДокумента
	|			ИЛИ НДСНалоговыйКредит.Регистратор ССЫЛКА Документ.ГТДИмпорт)
	|	И НДСНалоговыйКредит.НДСОборот <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НЕОПРЕДЕЛЕНО КАК Номенклатура,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.НаименованиеТовара КАК НаименованиеТовара,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.КодУКТВЭД КАК КодУКТВЭД,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.Количество КАК Количество,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.СтавкаНДС КАК СтавкаНДС,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.СуммаНДС КАК СуммаНДС,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.Цена КАК Цена,
	|	РегистрацияВходящегоНалоговогоДокументаТовары.Сумма КАК Сумма,
	...
	|		ПО РегистрацияВходящегоНалоговогоДокументаТовары.Ссылка = Регистраторы.Регистратор
	|ГДЕ
	|	РегистрацияВходящегоНалоговогоДокументаТовары.СтатьяДекларацииНДСНалоговыйКредит В(&МассивСтатей)
	...
	|	ГТДИмпортТовары.Номенклатура.НаименованиеПолное,
	|	ВЫБОР
	|		КОГДА ГТДИмпортТовары.Номенклатура.НоменклатураГТД ССЫЛКА Справочник.КлассификаторУКТВЭД
	|			ТОГДА ГТДИмпортТовары.Номенклатура.НоменклатураГТД.Ссылка
	|		ИНАЧЕ ГТДИмпортТовары.Номенклатура.НоменклатураГТД.КодУКТВЭД
	|	КОНЕЦ,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Регистраторы КАК Регистраторы
	|		ПО ГТДИмпортТовары.Ссылка = Регистраторы.Регистратор
	...
	|ГДЕ
	|	ГТДИмпортТовары.СтатьяДекларацииНДСНалоговыйКредит В(&МассивСтатей)
	|	И ГТДИмпортТовары.СуммаНДС <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ГТДИмпортОборудование.Номенклатура,
	|	ГТДИмпортОборудование.Номенклатура.НаименованиеПолное,
	|	ВЫБОР
	|		КОГДА ГТДИмпортОборудование.Номенклатура.НоменклатураГТД ССЫЛКА Справочник.КлассификаторУКТВЭД
	|			ТОГДА ГТДИмпортОборудование.Номенклатура.НоменклатураГТД.Ссылка
	|		ИНАЧЕ ГТДИмпортОборудование.Номенклатура.НоменклатураГТД.КодУКТВЭД
	|	КОНЕЦ,
	|	ГТДИмпортОборудование.Количество,
	|	ГТДИмпортОборудование.ЕдиницаИзмерения,
	|	ГТДИмпортРазделы.СтавкаНДС,
	|	ГТДИмпортОборудование.СуммаНДС,
	|	0,
	|	0,
	|	Регистраторы.Регистратор
	|ИЗ
	|	Документ.ГТДИмпорт.Оборудование КАК ГТДИмпортОборудование
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Регистраторы КАК Регистраторы
	|		ПО ГТДИмпортОборудование.Ссылка = Регистраторы.Регистратор
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ГТДИмпорт.Разделы КАК ГТДИмпортРазделы
	|		ПО ГТДИмпортОборудование.Ссылка = ГТДИмпортРазделы.Ссылка
	|			И ГТДИмпортОборудование.НомерРаздела = ГТДИмпортРазделы.НомерСтроки
	|ГДЕ
	|	ГТДИмпортОборудование.СтатьяДекларацииНДСНалоговыйКредит В(&МассивСтатей)
	|	И ГТДИмпортОборудование.СуммаНДС <> 0
	|ИТОГИ ПО
	|	Регистратор
	|;
	|
	...
	|ВЫБРАТЬ
	|	Регистраторы.Регистратор КАК Регистратор,
	|	ЕСТЬNULL(Регистраторы.Регистратор.ВидДокумента, """") КАК ВидДокумента,
	|	ЕСТЬNULL(Регистраторы.Регистратор.НомерВходящегоДокумента, Регистраторы.Регистратор.НомерГТД) КАК НомерВх,
	|	ЕСТЬNULL(Регистраторы.Регистратор.ДатаВходящегоДокумента, Регистраторы.Регистратор.Дата) КАК ДатаВх,
	|	Регистраторы.Регистратор.Контрагент.ИНН КАК ИННКонтрагента
	|ИЗ
	...
	Запрос.УстановитьПараметр("КонМесяца", КонецМесяца(Дата));
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("МассивСтатей", СтатьиДекларации);
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	ИндексПоследнегоЗапроса = РезультатЗапроса.ВГраница();
	ТаблицаДокументов = РезультатЗапроса[ИндексПоследнегоЗапроса].Выгрузить();
	
	ВыборкаДокументы =  РезультатЗапроса[ИндексПоследнегоЗапроса - 1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаДокументы.Следующий() Цикл
		
		ДанныеДокумента = ТаблицаДокументов.Найти(ВыборкаДокументы.Регистратор, "Регистратор");
		
		ВидДокумента = ?(ТипЗнч(ДанныеДокумента.Регистратор) = Тип("ДокументСсылка.ГТДИмпорт"), 
							"ВМД ",
							"Податкова накладна ")
					 + ?(ДанныеДокумента.ВидДокумента = "-" ИЛИ НЕ ЗначениеЗаполнено(ДанныеДокумента.ВидДокумента),
							"", 
							" (" + ДанныеДокумента.ВидДокумента+ ")");
							
		НомерДокумента = " № " + ?(ЗначениеЗаполнено(ДанныеДокумента.НомерВх), ДанныеДокумента.НомерВх , "б/н");
												  
		ДатаДокумента  = " від " + Формат(ДанныеДокумента.ДатаВх, "ДФ=dd.MM.yyyy");
		
		ИННПоставщика  = ?(ТипЗнч(ДанныеДокумента.Регистратор) = Тип("ДокументСсылка.ГТДИмпорт"),
							  "",
							  ", ІПН постачальника: " + ДанныеДокумента.ИННКонтрагента);
		
		ОписаниеДокумента = ВидДокумента + НомерДокумента + ДатаДокумента + ИННПоставщика;
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, Выборка, "Номенклатура, ЕдиницаИзмерения, Количество, СтавкаНДС, КодУКТВЭД");
			
			Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
				СтрокаТабличнойЧасти.Номенклатура	  = АвтоТовар;
			КонецЕсли;
			
			СтрокаТабличнойЧасти.Содержание = Выборка.НаименованиеТовара + ?(ЗначениеЗаполнено(Выборка.НаименованиеТовара), ", ", "") + ОписаниеДокумента; 
			...
			Если ЗначениеЗаполнено(Выборка.Сумма) Тогда
				...
				СуммаБезНДС = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Выборка.Сумма, 
														мВалютаРегламентированногоУчета, ВалютаДокумента, 
														1, ЗаполнениеДокументов.КурсДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета),
														1, ЗаполнениеДокументов.КратностьДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета));
			Иначе
				
				СуммаБезНДС = СуммаНДС / УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС) * 100;	
...
Процедура ДобавитьКолонкиТоваровРеглЛокально(ТаблицаТоваров, СтруктураШапкиДокумента, ПогрешностиОкругления, ПогрешностиОкругленияПревышение) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		Если СтруктураШапкиДокумента.ВалютаДокумента = СтруктураШапкиДокумента.ВалютаРегламентированногоУчета Тогда
			...
			СтрокаТаблицы.СуммаБезНДСРегл = СтрокаТаблицы.СуммаБезНДСВал;
		Иначе
			
			Если СтрокаТаблицы.СуммаБезНДСРегл = 0 Тогда
...
Процедура ПроводкиПоНДС(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоОС, ТаблицаПоНМА, Отказ)
	...
	Если Не ЗначениеЗаполнено(СтруктураШапкиДокумента.ПериодДвижений) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДвиженияПоРегиструНДСНалоговыеОбязательства(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоУслугам, ТаблицаПоОС, ТаблицаПоНМА, Отказ)
	...
	Если Не ЗначениеЗаполнено(СтруктураШапкиДокумента.ПериодДвижений) Тогда
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В документе %1 не указана дата ГТД. Документ не будет включен в декларацию НДС и не будет иметь проводок.';uk='У документі %1 не вказано дату ВМД. Документ не буде включений до декларації ПДВ та не матиме проводок.'"), Ссылка));
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьСтруктуруШапкиДокумента(Отказ, Заголовок = "") Экспорт
	...
	Если СтруктураШапкиДокумента.ТипПричиныНевыдачиПокупателю = 22 Тогда
		Если НачалоМесяца(СтруктураШапкиДокумента.ДатаГТД) = НачалоМесяца(СтруктураШапкиДокумента.Дата) Тогда
			ПериодДвижений = СтруктураШапкиДокумента.Дата;
		Иначе
			ПериодДвижений = СтруктураШапкиДокумента.ДатаГТД;
		КонецЕсли;
	Иначе
		ПериодДвижений = СтруктураШапкиДокумента.Дата;
	КонецЕсли;                                 
	СтруктураШапкиДокумента.Вставить("ПериодДвижений", ПериодДвижений);
...
Процедура ПодготовитьТаблицы(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоУслугам, ТаблицаПоОС, ТаблицаПоНМА) Экспорт
	...
	СтруктураПолей.Вставить("СуммаБезНДСРегл"    		, "СуммаБезНДСРегл");	
	СтруктураПолей.Вставить("СуммаНДСРегл"    			, "СуммаНДСРегл");	
	СтруктураПолей.Вставить("Вес"            			, "Вес");
	СтруктураСложныхПолей.Вставить("РежимЭкспортногоОбеспечения", "ISNULL(Док.КодУКТВЭД.РежимЭкспортногоОбеспечения, ЛОЖЬ)");
	...
	СтруктураСложныхПолей.Вставить("ЕдиницаИзмерения"          , "ВЫБОР КОГДА Док.ЕдиницаИзмерения = Значение(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка) ТОГДА Номенклатура.БазоваяЕдиницаИзмерения.Наименование ИНАЧЕ Док.ЕдиницаИзмерения.Наименование КОНЕЦ");
	СтруктураСложныхПолей.Вставить("ЕдиницаИзмеренияКод"       , "ВЫБОР КОГДА Док.ЕдиницаИзмерения = Значение(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка) ТОГДА Номенклатура.БазоваяЕдиницаИзмерения.Код ИНАЧЕ Док.ЕдиницаИзмерения.Код КОНЕЦ");
	СтруктураПолей.Вставить("КодЛьготы"          		, "Номенклатура.КодЛьготы");
	СтруктураПолей.Вставить("БланкСтрогогоУчета"		, "Номенклатура.БланкСтрогогоУчета");
	СтруктураПолей.Вставить("Количество"            	, "Количество");
	СтруктураПолей.Вставить("Цена"                      , "Цена");		
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если АвторасчетНДС Тогда
		...
		Документы.НалоговаяНакладная.ЗаполнитьСуммыРегл(ЭтотОбъект, СуммаВключаетНДС);
