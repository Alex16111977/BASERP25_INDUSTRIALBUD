...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, ИмяТаблицы) Экспорт
	...
	Если ДобавленныеИзмененныеСтроки.Товары.Количество() ИЛИ ДобавленныеИзмененныеСтроки.Услуги.Количество() Тогда
...
Процедура ДоговорКонтрагентаПриИзмененииНаСервере()
	...
	//Тютюн
	УстановитьНаправлениеПодразделениеИзДоговора();
	//КонецИзменений
...
Процедура ТоварыЕдиницаИзмеренияПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		"Номенклатура, ЕдиницаИзмерения, Коэффициент, Количество,
		|КодУКТВЭД, РежимЭкспортногоОбеспечения, Вес,
		|Цена, Сумма, СтавкаНДС, СуммаНДС,
		|СуммаБезСкидки, СуммаСкидки,
		|СтатьяДекларацииНДСНалоговыеОбязательства");
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТаблицы, ТекущиеДанные);
...
&НаКлиенте
Процедура УслугиСуммаБезНДСРеглПриИзменении(Элемент)
	РассчитатьСуммыРеглНаСервере(Ложь, Истина);
КонецПроцедуры
...
&НаКлиенте
Процедура ОССуммаБезНДСРеглПриИзменении(Элемент)
	РассчитатьСуммыРеглНаСервере(Ложь, Истина);
КонецПроцедуры
...
&НаКлиенте
Процедура НМАСуммаБезНДСРеглПриИзменении(Элемент)
	РассчитатьСуммыРеглНаСервере(Ложь, Истина);
КонецПроцедуры
...
Процедура ЗаполнитьРассчитатьСуммы(Форма, 
	...
	Документы.НалоговаяНакладная.ЗаполнитьСуммыРегл(Объект);
...
Процедура ОбновитьИтоги(Форма)
	...
	Иначе
		...
		Форма.ИтогиВсегоНДСРегл = Объект.Товары.Итог("СуммаНДСРегл") + Объект.Услуги.Итог("СуммаНДСРегл") + Объект.ОС.Итог("СуммаНДСРегл") + Объект.НМА.Итог("СуммаНДСРегл");	
		Форма.ИтогиВсегоРегл    = Объект.Товары.Итог("СуммаБезНДСРегл")    + Объект.Услуги.Итог("СуммаБезНДСРегл")    + Объект.ОС.Итог("СуммаБезНДСРегл")    + Объект.НМА.Итог("СуммаБезНДСРегл") + Форма.ИтогиВсегоНДСРегл;
	КонецЕсли; 
...
&НаСервере
Процедура РассчитатьСуммыРеглНаСервере(ИзмениласьСуммаБезНДС = Истина, ИзмениласьСтавкаНДС = Ложь)
	
	Документы.НалоговаяНакладная.ЗаполнитьСуммыРегл(Объект, ИзмениласьСуммаБезНДС, ИзмениласьСтавкаНДС);
...
Процедура РассчитатьВес(ТекущаяСтрока)
	...
	ТекущаяСтрока.Вес = ПолучитьВесНаСервере(ТекущаяСтрока.Номенклатура, ТекущаяСтрока.Количество, ТекущаяСтрока.Коэффициент, ТекущаяСтрока.КодУКТВЭД);
...
Процедура ДобавитьРеквизиты()
 // Масив для нових реквізитів
    ДобавляемыеРеквизиты = Новый Массив;

    // Описуємо реквізит "Направление"
    Реквизит_Направление = Новый РеквизитФормы("Направление",Новый ОписаниеТипов("СправочникСсылка.Направления"),,НСтр("ru='Направление';uk='Направлення'"),Истина);
    ДобавляемыеРеквизиты.Добавить(Реквизит_Направление);
    
    // Описуємо реквізит "Подразделение"
    Реквизит_Подразделение = Новый РеквизитФормы("Подразделение",Новый ОписаниеТипов("СправочникСсылка.Подразделения"),,НСтр("ru='Подразделение';uk='Підрозділ'"),Истина);
    ДобавляемыеРеквизиты.Добавить(Реквизит_Подразделение);

    // Додаємо нові реквізити на форму
    ЭтаФорма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);

	ГруппаШапкаЛевая = Элементы.Найти("ГруппаШапкаЛевая"); 
	// Поле для реквізиту "Направление"
    ПолеНаправление = Элементы.Добавить("Направление", Тип("ПолеФормы"), ГруппаШапкаЛевая);
    ПолеНаправление.Вид = ВидПоляФормы.ПолеВвода;
    ПолеНаправление.ПутьКДанным = "Направление";
    ПолеНаправление.Заголовок = НСтр("ru='Направление';uk='Направлення'");
    ПолеНаправление.Ширина = 30;

    // Поле для реквізиту "Подразделение"
    ПолеПодразделение = Элементы.Добавить("Подразделение", Тип("ПолеФормы"), ГруппаШапкаЛевая);
    ПолеПодразделение.Вид = ВидПоляФормы.ПолеВвода;
    ПолеПодразделение.ПутьКДанным = "Подразделение";
    ПолеПодразделение.Заголовок = НСтр("ru='Подразделение';uk='Підрозділ'");
    ПолеПодразделение.Ширина = 30;

    // Заповнення значень реквізитів із об'єкта, якщо вони є
    Если ЗначениеЗаполнено(Объект.Направление) Тогда
        ЭтаФорма.Направление = Объект.Направление;
    КонецЕсли;
    
    Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
        ЭтаФорма.Подразделение = Объект.Подразделение;
    КонецЕсли;		
КонецПроцедуры         

  // Зберігаємо значення реквізитів у об'єкт
&НаСервере
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	ТекущийОбъект.Направление = ЭтаФорма.Направление;
    ТекущийОбъект.Подразделение = ЭтаФорма.Подразделение;
КонецПроцедуры   

&НаСервере
Процедура УстановитьНаправлениеПодразделениеИзДоговора()
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента.Направление) Тогда
	ЭтаФорма.Направление = Объект.ДоговорКонтрагента.Направление;	
КонецЕсли; 

Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента.Подразделение) Тогда
	ЭтаФорма.Подразделение = Объект.ДоговорКонтрагента.Подразделение;	
КонецЕсли;
