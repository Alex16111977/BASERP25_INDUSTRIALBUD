...
// Функция формирует печатную форму Акт списания группы основных средств
//
Функция ПечатьСписанияОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	ТабДокумент   = Новый ТабличныйДокумент();
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СписаниеМалоценныхАктивовИзЭксплуатации_АктСписанияОС";
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		"ВЫБРАТЬ
		|	Док.ФизЛицо		                                    КАК МОЛ,
		|	Док.Номенклатура.Код			                    КАК ИнвентарныйНомер,
		|	Док.Номенклатура.НаименованиеПолное                 КАК НаименованиеОС" + ?(Ссылка.Проведен, ",
		|	Хозрасчетный.СубконтоКт3.Дата                       КАК ДатаВвода,
		...
		Пока ВыборкаПоМОЛ.Следующий() Цикл      
			...
			Параметры.Заполнить(ВыборкаПоШапке);
			Параметры.МОЛ_Должность = ?(ЗначениеЗаполнено(ДанныеФизЛицаСдал.Должность), ДанныеФизЛицаСдал.Должность.Наименование + ",", "");  
			Параметры.МОЛ_ФИО		= ФизическиеЛицаКлиентСервер.ИнициалыФамилия(ДанныеФизЛицаСдал);  
			Параметры.Заполнить(ВыборкаПоКомиссии);
			...
			Параметры.ОтветственныйДолжность = ?(ЗначениеЗаполнено(ДанныеФизЛицаОтветственный.Должность),ДанныеФизЛицаОтветственный.Должность.Наименование + ",","");
			Параметры.ОтветственныйФИО = ФизическиеЛицаКлиентСервер.ИнициалыФамилия(ДанныеФизЛицаОтветственный);  
...
Функция ПечатьСписанияОСГруппа_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)

	УстановитьПривилегированныйРежим(Истина);

	
	ТабДокумент   = Новый ТабличныйДокумент();
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СписаниеМалоценныхАктивовИзЭксплуатации_АктСписанияОСГруппа";
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		|	Док.Номенклатура.НаименованиеПолное                 КАК НаименованиеОС" + ?(Ссылка.Проведен, ",
		|	Хозрасчетный.СубконтоКт3.Дата                       КАК ДатаВвода,
		|	Хозрасчетный.КоличествоОборотКт                     КАК Количество,
		|	ВЫБОР
		|		КОГДА Хозрасчетный.КоличествоОборотКт = 0
		|			ТОГДА Хозрасчетный.СуммаОборот
		|		ИНАЧЕ Хозрасчетный.СуммаОборот / Хозрасчетный.КоличествоОборотКт
		...
		Пока ВыборкаПоМОЛ.Следующий() Цикл      
			...
			Пока ВыборкаПоОС.Следующий() Цикл
				Параметры     = ОбластьМакетаСтрока.Параметры;
				Параметры.Заполнить(ВыборкаПоОС);
				Параметры.ДатаВвода   = ?(Ссылка.Проведен, Формат(ВыборкаПоОС.ДатаВвода, "ДЛФ=Д"), "");  
...
КонецФункции // ПечатьСписанияОСГруппа_2016()
...
//~		ОсновныеСотрудникиФизическихЛиц = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(СписокФизическихЛиц, Истина, Шапка.Организация, Шапка.ДатаДокумента, Шапка.ДатаДокумента);
//-- БУ ЗИК		
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСписанияОСБюжд") Тогда
		ИмяМакета = "";
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктСписанияОСБюжд", НСтр("ru='Акт списания основных средств';uk='Акт списання основних засобів'"),
			ПечатьСписанияОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета), , ИмяМакета);
	КонецЕсли;
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСписанияОСГруппаБюдж") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктСписанияОСГруппаБюдж", НСтр("ru='Акт списания группы основных средств';uk='Акт списання групи основних засобів'"),
			ПечатьСписанияОСГруппа_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета), , ИмяМакета);
	КонецЕсли;
