...
//	ПроверитьОснование(Отказ);	//Тютюн
...
Функция ПодготовитьТаблицуТоваровНаКомиссии(РезультатЗапросаПоКомТоварам, СтруктураШапкиДокумента)
	...
	Если Выборка.Следующий() Тогда 
     	ТаблицаТоваров.ЗаполнитьЗначения(Выборка.Ссылка, "ДоговорКонтрагента");  
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТоварыНаКомиссии(СтруктураШапкиДокумента, Отказ, МассивНепроверяемыхРеквизитов)
	...
	//Тютюн         
	Если ТоварыНаКомиссии.Количество()> 0 И НЕ ЗначениеЗаполнено(Контрагент) Тогда  
		Сообщить("Вкажіть контрагента"); 
		Отказ = Истина;
	КонецЕсли; 
	//КОнецИзменений
...
|Не списано %3 %4 товару %5, рахунок %6'"), Учет, СтрокаТаблицы.НомерСтроки, (СтрокаТаблицы.Количество - СтрокаТаблицы.КоличествоНоминальное), СтрокаТаблицы.Номенклатура.БазоваяЕдиницаИзмерения, СтрокаТаблицы.Номенклатура, СтрокаТаблицы.СчетУчетаЗабалансовыйБУ);
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")  Тогда
		...
		Ответственный = Пользователи.ТекущийПользователь();	
	//Тютюн	
ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.СогласованиеПеремещения") Тогда  

	ЗаполнитьЗначенияСвойств(ЭтотОбъект,Основание,,"Проведен,Дата"); 
	Номер = "";
	Товары.Загрузить(Основание.Товары.Выгрузить());  
	ТоварыНаКомиссии.Загрузить(Основание.ТоварыНаКомиссии.Выгрузить());
	...
	ТипСкладаПолучатель   		= СвойстваСкладаПолучателя.ТипСклада;
	РазделятьПоСтавкамНДС       = БухгалтерскийУчетВызовСервераПовтИсп.ИспользуетсяРазделениеПоСтавкамНДСВНТТ();
	_ДокументОснование =  Основание;
	Документы.ПеремещениеТоваров.ЗаполнитьСтрокиТабличнойЧастиТовары(ЭтотОбъект, ТипСкладаОтправитель, ТипСкладаПолучатель, РазделятьПоСтавкамНДС);
	Если ТоварыНаКомиссии.Количество()>0 и Значениезаполнено(СкладОтправитель) Тогда  
		Контрагент = СкладОтправитель.Контрагент;
	КонецЕсли; 
...
Процедура ПроверитьОснование(Отказ) 
	Если Дата< Дата("20250617")Тогда 
		Возврат
	КонецЕсли;  
	
	Если Не ЗначениеЗаполнено(_ДокументОснование) Тогда  
		Отказ = Истина; 
		Сообщить("Вкажіть документ підставу");
		Возврат;
	КонецЕсли;     
	
	Если СкладОтправитель <> _ДокументОснование.СкладОтправитель Тогда  
		Отказ = Истина; 
		Сообщить("Склад відправник не відповідає погодженню");
		Возврат;
	ИначеЕсли СкладПолучатель <> _ДокументОснование.СкладОтправитель Тогда 
		Отказ = Истина; 
		Сообщить("Склад одержувач не відповідає погодженню");
		Возврат;
	ИначеЕсли Направление <> _ДокументОснование.Направление Тогда	
		Отказ = Истина; 
		Сообщить("Направлення не відповідає погодженню");
		Возврат;
	ИначеЕсли Подразделение <> _ДокументОснование.Подразделение Тогда   
		Отказ = Истина; 
		Сообщить("Підрозділ не відповідає погодженню");
		Возврат;
	КонецЕсли;  
	...
	Запрос.Текст = 
		...
		|	
		|	ВЫБРАТЬ
		|		СогласованиеПеремещенияТовары.Номенклатура,
		|		СУММА(-СогласованиеПеремещенияТовары.Количество)
		|	ИЗ
		|		Документ.СогласованиеПеремещения.Товары КАК СогласованиеПеремещенияТовары
		|	ГДЕ
		|		СогласованиеПеремещенияТовары.Ссылка = &Основание
		|	
		|	СГРУППИРОВАТЬ ПО
		|		СогласованиеПеремещенияТовары.Номенклатура) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТоварРазница.Номенклатура КАК Номенклатура,
		|	ТоварРазница.Количество КАК Количество
		|ИЗ
		|	ТоварРазница КАК ТоварРазница
		|ГДЕ
		|	ТоварРазница.Количество <> 0";
	
	Запрос.УстановитьПараметр("Основание", _ДокументОснование);
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	
	ВыборкаТовары = Запрос.Выполнить().Выбрать();
	
	Пока ВыборкаТовары.Следующий() Цикл
    Сообщить("Тч Товари. Кількість номенклатури/ " + ВыборкаТовары.Номенклатура + " / не відповідає погодженню");
	Отказ = Истина;
...
Запрос = Новый Запрос;
	Запрос.Текст = 
		...
		|		СУММА(ПеремещениеТоваровТовары.Количество) КАК Количество
		|	ИЗ
		|		Документ.ПеремещениеТоваров.ТоварыНаКомиссии КАК ПеремещениеТоваровТовары
		|	ГДЕ
		|		ПеремещениеТоваровТовары.Ссылка = &Ссылка
		|	
		|	СГРУППИРОВАТЬ ПО
		...
		|		СУММА(-СогласованиеПеремещенияТовары.Количество)
		|	ИЗ
		|		Документ.СогласованиеПеремещения.ТоварыНаКомиссии КАК СогласованиеПеремещенияТовары
		|	ГДЕ
		|		СогласованиеПеремещенияТовары.Ссылка = &Основание
		|	
		|	СГРУППИРОВАТЬ ПО
	...
	Пока ВыборкаТовары.Следующий() Цикл
    Сообщить("Тч Товари на комісії. Кількість номенклатури/ " + ВыборкаТовары.Номенклатура + " / не відповідає погодженню");
	Отказ = Истина;
