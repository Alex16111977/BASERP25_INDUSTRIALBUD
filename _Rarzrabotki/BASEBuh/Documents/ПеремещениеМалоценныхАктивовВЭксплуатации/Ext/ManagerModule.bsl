...
Функция ПечатьПеремещениеОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	ТабДокумент   = Новый ТабличныйДокумент();
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ВводВЭксплуатациюОС_АктПеремещениеОС";
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл	
		...
		Пока ВыборкаПоМОЛ.Следующий() Цикл    
			...
			ДанныеФизЛицаСдал 	 = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(Ссылка.Организация, ВыборкаПоМОЛ.Сдал, Ссылка.Дата);  
			...
			Пока ВыборкаПоОС.Следующий() Цикл
				...
				Параметры.Сумма 	 			 = ВыборкаПоОС.ПервоначальнаяСтоимость * ВыборкаПоОС.Количество;
				СуммаВсего 						 = СуммаВсего + ВыборкаПоОС.ПервоначальнаяСтоимость * ВыборкаПоОС.Количество;
			...
			Параметры.ПринялоПодразделение 	 		 = Ссылка.ПодразделениеОрганизации;
			ОбластьМакета.Параметры.СдалДолжность 	 = ДанныеФизЛицаСдал.Должность;  
			Параметры.СдалФИО = ФизическиеЛицаКлиентСервер.ИнициалыФамилия(ДанныеФизЛицаПолучил);  
			ОбластьМакета.Параметры.ПолучилДолжность = ДанныеФизЛицаПолучил.Должность;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктПеремещениеОСБюдж") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктПеремещениеОСБюдж", НСтр("ru='Акт внутреннего перемещения основных средств';uk='Акт внутрішнього переміщення основних засобів'"),
			ПечатьПеремещениеОС_2016(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета),, ИмяМакета);
	КонецЕсли;
