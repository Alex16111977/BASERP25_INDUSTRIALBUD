...
Процедура СкладОтправительПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		...
		//Тютюн
		УстановитьНаправлениеПодразделениеИзСклада();	
		//КонецИзменений
	Иначе
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ЗаполнитьОстатокПоСкладу(ДанныеНоменклатуры,СтруктураПараметров.Номенклатура,Ложь);
...
Функция ПолучитьДанныеТоварыНоменклатураПриИзменении(Знач СтруктураПараметров)
	...
	ДанныеНоменклатура = Новый Структура("СчетУчетаБУ, НовыйСчетУчетаБУ, НалоговоеНазначение, ЕдиницаИзмерения, Коэффициент,
		|СтавкаНДСВРознице, ЦенаВРознице, СуммаВРознице,Остаток");
...
&НаСервере
Процедура ЗаполнитьОстатокПоСкладу(ДанныеНоменклатуры,Номенклатура,ВнеБаланс)
	...
	Если ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		...
		//|					КОГДА ТИПЗНАЧЕНИЯ(Субконто2) = ТИП(Справочник.Номенклатура)
		//|						ТОГДА Субконто2 = &Номенклатура
		//|					КОГДА ТИПЗНАЧЕНИЯ(Субконто3) = ТИП(Справочник.Номенклатура)
		//|						ТОГДА Субконто3 = &Номенклатура
		|				КОНЕЦ
		|				И ВЫБОР
		//|					КОГДА ТИПЗНАЧЕНИЯ(Субконто1) = ТИП(Справочник.Склады)
		//|						ТОГДА Субконто1 = &Cубконто3
		//|					КОГДА ТИПЗНАЧЕНИЯ(Субконто2) = ТИП(Справочник.Склады)
		//|						ТОГДА Субконто2 = &Cубконто3
		|					КОГДА ТИПЗНАЧЕНИЯ(Субконто3) = ТИП(Справочник.Склады)
		|						ТОГДА Субконто3 = &Cубконто3
		|				КОНЕЦ) КАК ХозрасчетныйОстатки"; 
		
		Запрос.УстановитьПараметр("Cубконто3", Объект.СкладОтправитель); 
		Запрос.УстановитьПараметр("Номенклатура", Номенклатура); 
		Если Внебаланс Тогда 
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ДанныеНоменклатуры,ВыборкаДетальныеЗаписи);
		КонецЕсли;	
		
	Иначе Сообщить("Не обрано склад відпрвник, залишок товарів не встановлено!");
	КонецЕсли;
...
Процедура ТоварыДокументОприходованияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("ТипыДокументов", "Метаданные.Документы.СогласованиеПеремещения.ТабличныеЧасти.Товары.Реквизиты.ДокументОприходования.Тип");
...
Процедура ТоварыНаКомиссииНоменклатураПриИзменении(Элемент)
	...
	ЗаполнитьОстатокПоСкладу(ДанныеНоменклатура,Номенклатура,Истина);     
...
Функция ПолучитьДанныеТоварыНаКомиссииНоменклатураПриИзменении(Дата, Организация, Номенклатура, СкладОтправитель, СкладПолучатель)
	...
	ДанныеНоменклатура = Новый Структура("СчетУчетаБУ, НовыйСчетУчетаБУ, ЕдиницаИзмерения, Коэффициент, Остаток");
...
Процедура УстановитьЗаголовокФормы() Экспорт
	...
	ТекстЗаголовка = НСтр("ru='Согласование перемещения';uk='Погодження переміщення'");
...
Процедура ЗаполнитьСтрокиТабличнойЧастиТоварыНаСервере()
	...
	Для Каждого СтрТЧ Из Объект.Товары Цикл
		 ЗаполнитьОстатокСтрТЧ(СтрТЧ);
	КонецЦикла	

КонецПроцедуры // ЗаполнитьСтрокиТабличнойЧастиТоварыНаСервере

Процедура ЗаполнитьОстатокСтрТЧ(Строка)
...
&НаСервере
Процедура УстановитьНаправлениеПодразделениеИзСклада()
	Если ЗначениеЗаполнено(Объект.СкладОтправитель.Направление) Тогда
	ЭтаФорма.Направление = Объект.СкладОтправитель.Направление;	
КонецЕсли; 

Если ЗначениеЗаполнено(Объект.СкладОтправитель.Подразделение) Тогда
	ЭтаФорма.Подразделение = Объект.СкладОтправитель.Подразделение;	
КонецЕсли;
