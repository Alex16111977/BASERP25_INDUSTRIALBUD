...
Процедура ЗакрытиеДоходовИРасходов(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ХозрасчетныйОстатки.Счет.Количественный КАК Количественный,
	...
	Пока ВыборкаОстатков.Следующий() Цикл
		...
		Если ВыборкаОстатков.ВидСчета = ВидСчета.Активный Тогда
			...
			Если ЗакрыватьБУ Тогда
				...
				Если ВыборкаОстатков.Количественный Тогда
					Проводка.КоличествоКт = ВыборкаОстатков.КоличествоОстатокДт;
				КонецЕсли;
			КонецЕсли;			
		...
		Иначе
			...
			Если ЗакрыватьБУ Тогда
				Проводка.Сумма                     = - ВыборкаОстатков.СуммаОстаток;
				Если ВыборкаОстатков.Количественный Тогда
					Проводка.КоличествоДт = ВыборкаОстатков.КоличествоОстатокКт;
				КонецЕсли;
			КонецЕсли;
