...
&НаСервереБезКонтекста
Функция СуммаЗаработнойПлатыПоВедомости(Знач ПлатежнаяВедомость, Знач СчетУчета, Знач ГруппаУчетаНачислений, Знач Ссылка, Знач НомерСтроки, УчетЗарплатыИКадровВоВнешнейПрограмме, Контрагент)
	...
	Если УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
		...
		Если Не Контрагент = Справочники.Контрагенты.ПустаяСсылка() Тогда
			Для Каждого СтрокаВедомости Из ТаблицаВедомостей Цикл
				Если СтрокаВедомости.Банк <> Контрагент Тогда
					СтрокаВедомости.СуммаКВыплате = 0	
				КонецЕсли;	 
			КонецЦикла;
		КонецЕсли;
...
Функция СуммаНалоговПоВедомости(Знач ПлатежнаяВедомость, СчетУчета, Субконто1, Субконто2, Субконто3, Знач Ссылка, Знач НомерСтроки, УчетЗарплатыИКадровВоВнешнейПрограмме)
	...
	//	Субконто1 = УчетНДФЛ.ЗначенияВоенныйСбор().Налог;    Тютюн по просьбам трудящихся
	КонецЕсли;	
...
Функция ПроверитьДублиВедомостей(ТекущиеДанные, ТабличнаяЧасть = "ВыплатаЗаработнойПлаты")
	...
	СтрокаПоиска = Новый Структура("Ведомость, СчетУчета, ГруппаУчетаНачислений", ТекущиеДанные.Ведомость, ТекущиеДанные.СчетУчета, ТекущиеДанные.ГруппаУчетаНачислений);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.Ответственный <> ПараметрыСеанса.ТекущийПользователь Тогда
