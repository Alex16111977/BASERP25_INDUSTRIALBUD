...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(СчетКасса) Тогда
		Если КонецДня(Дата) >= ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьДатуНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ() Тогда
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок, МассивНепроверяемыхРеквизитов)
	...
	Если НТТ ИЛИ СтруктураШапкиДокумента.СуммаОкругления <= 0 Тогда
...
Процедура ПроверитьЗаполнениеТабличнойЧастиОплата(ТаблицаОплата, СтруктураШапкиДокумента, Отказ, Заголовок,МассивНепроверяемыхРеквизитов)
	...
	Если (СтруктураШапкиДокумента.СуммаДокумента < ТаблицаОплата.Итог("СуммаОплаты")) И (ТаблицаОплата.Итог("СуммаОплаты") > 0) Тогда
...
Процедура ДвиженияПоЕдиномуНалогу(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ)
	...
	КонецЕсли;
	СуммаДвижения = СуммаДвижения + ТаблицаПоТоварам.Итог("ПроводкиСуммаАкцизногоНалогаРегл") + СтруктураШапкиДокумента.СуммаОкругленияРегл;	
...
Процедура ПодготовитьСтруктуруШапкиДокумента(Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	СтруктураШапкиДокумента.Вставить("СуммаДокумента", УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Товары") 
		+ ?(НЕ СтруктураШапкиДокумента.ЕстьНДС ИЛИ (СтруктураШапкиДокумента.ЕстьНДС И СтруктураШапкиДокумента.СуммаВключаетНДС), 0, Товары.Итог("СуммаАкцизногоНалога")));
