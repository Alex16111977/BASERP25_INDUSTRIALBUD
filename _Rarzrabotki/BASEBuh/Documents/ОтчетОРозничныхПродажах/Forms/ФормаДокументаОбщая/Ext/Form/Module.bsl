...
&НаКлиенте
Процедура ИтогСуммаНалПриИзменении(Элемент)
	
	Объект.СуммаОкругления = ИтогНал - ИтогВсего 
		- ?(ПлательщикНДС И НЕ Объект.СуммаВключаетНДС, ИтогСуммаНДС + ИтогСуммаАкцизныйНалог, 0) + Объект.Оплата.Итог("СуммаОплаты");
...
&НаКлиенте
Процедура СуммаОкругленияПриИзменении(Элемент)
...
Процедура СубконтоПриИзменении(Элемент)
	...
	НастроитьСубконтоПриИзмененииСубконто(ЭтаФорма, Элемент.Имя);
...
Процедура УправлениеФормой(Форма)
	...
	НастроитьСубконтоПриПодготовкеФормыНаСервере(Форма, "СчетУчетаДоходовОкругление");
	НастроитьСубконтоПриПодготовкеФормыНаСервере(Форма, "СчетУчетаРасходовОкругление");
...
Процедура УстановитьВидимость()
	...
	Элементы.ГруппаИтогСуммаНал.Видимость = НЕ ЭтоОтчетНТТОПродажах;
	Элементы.ГруппаИтогСуммаОкругления.Видимость = НЕ ЭтоОтчетНТТОПродажах;
	Элементы.ГруппаИтогСуммаБезнал.Видимость = НЕ ЭтоОтчетНТТОПродажах;
	
	Элементы.ГруппаДоходыОтОкругления.Видимость = НЕ ЭтоОтчетНТТОПродажах И Объект.СуммаОкругления > 0;
	Элементы.ГруппаРасходыОтОкругления.Видимость = НЕ ЭтоОтчетНТТОПродажах И Объект.СуммаОкругления < 0;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	СчетаУчетаДоходов = БухгалтерскийУчетПереопределяемый.ПолучитьСчетУчетаПрочий(Объект.Организация, Перечисления.ВидыПрочихСчетовУчета.СчетДоходовОкруглениеНаличных);
	Объект.СчетУчетаДоходовОкругление 	= СчетаУчетаДоходов.СчетУчетаБУ;
	Объект.СубконтоДоходовОкругление1 	= СчетаУчетаДоходов.Субконто1;
	Объект.СубконтоДоходовОкругление2 	= СчетаУчетаДоходов.Субконто2;
	Объект.СубконтоДоходовОкругление3 	= СчетаУчетаДоходов.Субконто3;
	
	СчетаУчетаЗатрат = БухгалтерскийУчетПереопределяемый.ПолучитьСчетУчетаПрочий(Объект.Организация, Перечисления.ВидыПрочихСчетовУчета.СчетЗатратОкруглениеНаличных);
	Объект.СчетУчетаРасходовОкругление = СчетаУчетаЗатрат.СчетУчетаБУ;
	Объект.СубконтоРасходовОкругление1 	= СчетаУчетаЗатрат.Субконто1;
	Объект.СубконтоРасходовОкругление2 	= СчетаУчетаЗатрат.Субконто2;
	Объект.СубконтоРасходовОкругление3 	= СчетаУчетаЗатрат.Субконто3;
...
Процедура УстановитьПараметрыВыбораСчетаКассы()
	...
	Если Объект.Дата < ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьДатуНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ() Тогда
		МассивИсключений.Добавить(ПланыСчетов.Хозрасчетный.РозничнаяВыручкаВКассахККМ);
	КонецЕсли;
...
Процедура ОбновитьИтоги(Форма)
	...
	Форма.ИтогНал    = Форма.ИтогВсего 
		+ ?(Форма.ПлательщикНДС И НЕ Форма.Объект.СуммаВключаетНДС, Форма.ИтогСуммаНДС + Форма.ИтогСуммаАкцизныйНалог, 0) 
		- Форма.Объект.Оплата.Итог("СуммаОплаты") + Форма.Объект.СуммаОкругления;
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьСубконтоПриИзмененииСубконто(Форма, ИмяСубконто);
	
	ИмяСчета = ОпределитьИмяСчетаПоИмениСубконто(ИмяСубконто);
	ИмяПоля = ОпределитьИмяСубконтоПоИмениСчета(ИмяСчета);	
...
Процедура УстановитьЗаголовкиИДоступностьСубконто(Форма, ИмяСчета, ИмяПоля)
	...
	Для Индекс = 1 По 3 Цикл  
		ЗаголовокПоля = Форма["Заголовок"+ИмяПоля+Индекс];
		ЗаголовокПоля = Лев(ЗаголовокПоля, СтрДлина(ЗаголовокПоля) -1);
		Если ПустаяСтрока(ЗаголовокПоля) Тогда
			Форма.Элементы[ИмяПоля+Индекс].Видимость = Ложь;
		Иначе
			Форма.Элементы[ИмяПоля+Индекс].Видимость = Истина;
			Форма.Элементы[ИмяПоля+Индекс].Заголовок = ЗаголовокПоля;
		КонецЕсли;
	КонецЦикла;
...
Функция ОпределитьИмяСубконтоПоИмениСчета(ИмяСчета)
	...
	Если ИмяСчета = "СчетУчетаДоходовОкругление" Тогда
		Возврат "СубконтоДоходовОкругление"
	ИначеЕсли ИмяСчета = "СчетУчетаРасходовОкругление" Тогда
		Возврат "СубконтоРасходовОкругление"
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ОпределитьИмяСчетаПоИмениСубконто(ИмяСубконто)
	
	Если СтрНайти(ИмяСубконто, "СубконтоДоходовОкругление") Тогда
		Возврат "СчетУчетаДоходовОкругление"
	ИначеЕсли СтрНайти(ИмяСубконто, "СубконтоРасходовОкругление") Тогда
		Возврат "СчетУчетаРасходовОкругление"
	КонецЕсли;
	
	ВызватьИсключение "Неверное обращение к функции ОпределитьИмяСчетаПоИмениСубконто";
