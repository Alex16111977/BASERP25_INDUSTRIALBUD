...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Обработка.ПодборНоменклатуры.Форма.Форма" Тогда
		...
		УстановитьВидимостьДополнительныхКолонокТоваров(ЭтаФорма);
	КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьДополнительныеКолонкиТоваров(Команда)
	УстановитьВидимостьДополнительныхКолонокТоваров(ЭтаФорма, Истина);
КонецПроцедуры
...
Процедура РаспределитьПропорциональныйНДС(Команда)
	...
	ЕстьПропорционально = Ложь;
	ЕстьНедопустимыеСтавки = Ложь;
	Для каждого ИдентификаторСтроки Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если  НЕ ТекущаяСтрока.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20")
			...
			И НЕ ТекущаяСтрока.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС7") Тогда
				ЕстьНедопустимыеСтавки = Истина;
				ЕстьОшибки = Истина;
		КонецЕсли; 
			
		Если Не ЕстьОшибки Тогда
			МассивСтрокДляОбработки.Добавить(ИдентификаторСтроки);
		КонецЕсли;
		
	КонецЦикла;    
	
	МассивСообщений = Новый Массив;
	Если ЕстьПропорционально Тогда
		МассивСообщений.Добавить(НСтр("ru='В строках не должен стоять флаг ""Пропорционально"".';uk='У рядках не повинен стояти прапорець ""Пропорційно"".'"));
	КонецЕсли;
	Если ЕстьНедопустимыеСтавки Тогда
		МассивСообщений.Добавить(НСтр("ru='В строках должна быть ставка НДС 20%, 14% или 7%.';uk='У рядках має бути ставка ПДВ 20%, 14% або 7%.'"));
	КонецЕсли;
	Если МассивСтрокДляОбработки.Количество() = 0 Тогда
		МассивСообщений.Добавить(НСтр("ru='Нет строк для обработки.';uk='Немає рядків для обробки.'"));
	КонецЕсли; 
	...
	ПоказатьВводЧисла(Новый ОписаниеОповещения("РаспределитьПропорциональныйНДСЗавершение", ЭтотОбъект, Новый Структура("ВыбранныеСтроки, ТекКоэффициент", МассивСтрокДляОбработки, ТекКоэффициент)), ТекКоэффициент, НСтр("ru='Введите % пропорционального НДС';uk='Введіть % пропорційного ПДВ'"), 4,2);
...
Процедура РаспределитьПропорциональныйНДСЗавершение(Число, ДополнительныеПараметры) Экспорт
	...
	Для каждого ИдентификаторСтроки Из ДополнительныеПараметры.ВыбранныеСтроки Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаОбязательства, СтрокаКредит);
		
		СтрокаКредит.Количество 		= СтрокаКредит.Количество * ТекКоэффициент;
		СтрокаОбязательства.Количество 		= СтрокаОбязательства.Количество - СтрокаКредит.Количество;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ТоварыДействиеЗаполнитьТоварыИзXML.Доступность = ЕстьXML;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьДополнительныхКолонокТоваров(Форма, ПоКнопке = Ложь)

	Если ПоКнопке Тогда
		
		Форма.Элементы.ТоварыПоказатьДополнительныеКолонкиТоваров.Пометка = Не Форма.Элементы.ТоварыПоказатьДополнительныеКолонкиТоваров.Пометка;
	...
	Иначе
		...
		Форма.Элементы.ТоварыПоказатьДополнительныеКолонкиТоваров.Видимость = НЕ ЗаполненностьДополнительныхКолонокТоваров;
		Если ЗаполненностьДополнительныхКолонокТоваров Тогда
			Форма.Элементы.ТоварыПоказатьДополнительныеКолонкиТоваров.Пометка = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	ВидимостьДополнительныхКолонокТоваров = Форма.Элементы.ТоварыПоказатьДополнительныеКолонкиТоваров.Пометка;
	
	Форма.Элементы.ТоварыНаименованиеТовара.Видимость = ВидимостьДополнительныхКолонокТоваров;
	Форма.Элементы.ТоварыКоличество.Видимость = ВидимостьДополнительныхКолонокТоваров;
	Форма.Элементы.ТоварыЕдиницаИзмерения.Видимость = ВидимостьДополнительныхКолонокТоваров;
	Форма.Элементы.ТоварыЦена.Видимость = ВидимостьДополнительныхКолонокТоваров;
	Форма.Элементы.ТоварыКодУКТВЭД.Видимость = ВидимостьДополнительныхКолонокТоваров;
