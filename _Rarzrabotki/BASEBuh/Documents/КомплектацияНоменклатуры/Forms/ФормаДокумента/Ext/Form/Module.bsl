...
//Тютюн
&НаКлиенте
Процедура ОтличияКомплектующиеНоменклатураПриИзменении(Элемент)

	
	ТекущиеДанные = Элементы._КомплектующиеДополнительно.ТекущиеДанные;
	
	ДанныеСтрокиТаблицы = ДанныеСтрокиТабличнойЧасти(ТекущиеДанные);
	ДанныеОбъекта = ДанныеОбъекта();
	ДанныеОбъекта.Склад = Объект.СкладДополнительный;
	
	КомплектующиеОтличиеНоменклатураПриИзмененииНаСервере(ДанныеСтрокиТаблицы, ДанныеОбъекта);
...
&НаСервереБезКонтекста
Процедура КомплектующиеОтличиеНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта)
	...
	Документы.КомплектацияНоменклатуры.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(
		ДанныеОбъекта, СтрокаТабличнойЧасти, "_КомплектующиеДополнительно", СведенияОНоменклатуре);
...
Процедура УстановитьНаправлениеПодразделениеИзСклада()
	Если ЗначениеЗаполнено(Объект.Склад.Направление) Тогда
	ЭтаФорма.Направление = Объект.Склад.Направление;	
КонецЕсли; 

Если ЗначениеЗаполнено(Объект.Склад.Подразделение) Тогда
	ЭтаФорма.Подразделение = Объект.Склад.Подразделение;	
КонецЕсли;
