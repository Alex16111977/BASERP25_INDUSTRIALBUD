...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок) Экспорт  
	...
	//Тютюн
	Если _КомплектующиеДополнительно.Количество() Тогда
		
		РезультатЗапросаПоДопКомплектующим = ОбщегоНазначенияРед12.СформироватьЗапросПоТабличнойЧасти(ЭтотОбъект, "_КомплектующиеДополнительно", СтруктураПолей);
	    СтруктураШапкиДокумента.Склад = СкладДополнительный;
		СтруктураШапкиДокумента.ТипСклада = СкладДополнительный.ТипСклада;
		ТаблицаПоДопКомплектующим = ПодготовитьТаблицуКомплектующих(РезультатЗапросаПоДопКомплектующим, СтруктураШапкиДокумента);
		СтруктураШапкиДокумента.Склад = Склад;
		СтруктураШапкиДокумента.ТипСклада = Склад.ТипСклада;

		Для Каждого СтрокаДоп Из ТаблицаПоДопКомплектующим Цикл
			Стр = ТаблицаПоКомплектующим.Добавить();
			ЗаполнитьЗначенияСвойств(Стр,СтрокаДоп);
		КонецЦикла;	
	КонецЕсли;
	//КонецИзменений
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Перем ТаблицаПоКомплектующим, ТаблицаПоКомплектам, ТаблицаПоКомплектующим2;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.Комплектация Тогда
		...
		//Тютюн
		МассивНепроверяемыхРеквизитов.Добавить("_КомплектующиеДополнительно.ДоляСтоимости");
        //КонецИзменений
