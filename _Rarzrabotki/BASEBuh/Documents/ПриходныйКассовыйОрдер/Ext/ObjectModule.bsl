...
Процедура ДвиженияПоРегистрам(Режим, Отказ, Заголовок ,СтруктураШапкиДокумента, ТаблицаОплата)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		Если СтруктураШапкиДокумента.ВыручкаСНТТ Тогда
			...
			Если СтруктураШапкиДокумента.СуммаОкругленияРегл > 0 Тогда
				
				Проводка = ПроводкиБУ.Добавить();
				Проводка.Период       = ДатаДок;
				Проводка.Организация  = Организация;
				Проводка.Содержание   = НСтр("ru='Округление наличных платежей';uk='Заокруглення оплати готівкою'",Локализация.КодЯзыкаИнформационнойБазы());
				Проводка.Сумма        = СтруктураШапкиДокумента.СуммаОкругленияРегл;
				...
				Проводка.СчетКт       = СчетУчетаДоходовОкругление;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СубконтоДоходовОкругление1);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2, СубконтоДоходовОкругление2);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 3, СубконтоДоходовОкругление3); 
				
				
			ИначеЕсли СтруктураШапкиДокумента.СуммаОкругленияРегл < 0 Тогда
				...
				Проводка.СчетДт       = СчетУчетаРасходовОкругление;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СубконтоРасходовОкругление1);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СубконтоРасходовОкругление2);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СубконтоРасходовОкругление3); 
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратДенежныхСредствРаботником Тогда
			...
			Если СчетКасса = ПланыСчетов.Хозрасчетный.КассаВНациональнойВалюте Тогда		
				...
				Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Сотрудники") Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"РаботникиОрганизаций",Контрагент.ФизическоеЛицо);
				Иначе
...
//++ БУ ЗИК
				...
				КонецЕсли;
//~				Если СчетКт = ПланыСчетов.Хозрасчетный.РасчетыСРабочимиИСлужащимиПоДругимОперациям Тогда
//~					ЗаймыСотрудникам.ЗарегистрироватьЧастичноПогашенныйЗаем(Движения, ДоговорЗаймаСотруднику, СуммаДокумента, 0, Дата, Организация, Контрагент, Отказ)
//~	            КонецЕсли;
//-- БУ ЗИК	
			Иначе
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") 
		И Основание.ВидОперации = Перечисления.ВидыОперацийОтчетОРозничныхПродажах.ОтчетККМОПродажах
		И Основание.СчетКасса = ПланыСчетов.Хозрасчетный.РозничнаяВыручкаВКассахККМ Тогда
		...
		СчетУчетаРасчетовСКонтрагентом = Основание.СчетКасса; 
		Если Основание.ПоОбособленномуПодразделению Тогда
			НомерСубконтоОбособленноеПодразделение = 0;
			НомерСубконто = 0;
			Для каждого СтрокаВидСубконто Из СчетУчетаРасчетовСКонтрагентом.ВидыСубконто Цикл
				НомерСубконто = НомерСубконто + 1;
				Если СтрокаВидСубконто.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбособленныеПодразделенияБезОбразованияЮрЛица Тогда
					НомерСубконтоОбособленноеПодразделение = НомерСубконто;
					Прервать;	
				КонецЕсли;
			КонецЦикла;                                                            
			Если НЕ НомерСубконтоОбособленноеПодразделение = 0 Тогда
				ЭтотОбъект["СубконтоКт"+Строка(НомерСубконтоОбособленноеПодразделение)] = Основание.ОбособленноеПодразделениеОрганизации;
			КонецЕсли;
		КонецЕсли;
		
		СуммаДокумента = Основание.СуммаДокумента - Основание.Оплата.Итог("СуммаОплаты") + Основание.СуммаОкругления;
	...
	ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
		...
		СуммаДокумента = СуммаПродаж - СуммаВозврата - СуммаБезналичнойОплаты + Основание.СуммаОкругления;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		Если НЕ ЗначениеЗаполнено(СчетКасса) И КонецДня(Дата) >= ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьДатуНачалаИспользованияСчетаРозничнаяВыручкаВКассахККМ() Тогда
			СчетКасса = ПланыСчетов.Хозрасчетный.РозничнаяВыручкаВКассахККМ;
		КонецЕсли;
	...
	СчетаУчетаДоходов = БухгалтерскийУчетПереопределяемый.ПолучитьСчетУчетаПрочий(Организация, Перечисления.ВидыПрочихСчетовУчета.СчетДоходовОкруглениеНаличных);
	СчетУчетаДоходовОкругление 	= СчетаУчетаДоходов.СчетУчетаБУ;
	СубконтоДоходовОкругление1 	= СчетаУчетаДоходов.Субконто1;
	СубконтоДоходовОкругление2 	= СчетаУчетаДоходов.Субконто2;
	СубконтоДоходовОкругление3 	= СчетаУчетаДоходов.Субконто3;
	
	СчетаУчетаЗатрат = БухгалтерскийУчетПереопределяемый.ПолучитьСчетУчетаПрочий(Организация, Перечисления.ВидыПрочихСчетовУчета.СчетЗатратОкруглениеНаличных);
	СчетУчетаРасходовОкругление = СчетаУчетаЗатрат.СчетУчетаБУ;
	СубконтоРасходовОкругление1 	= СчетаУчетаЗатрат.Субконто1;
	СубконтоРасходовОкругление2 	= СчетаУчетаЗатрат.Субконто2;
	СубконтоРасходовОкругление3 	= СчетаУчетаЗатрат.Субконто3;
...
//++ БУ ЗИК
//~	Если ВидОперации <> Перечисления.ВидыОперацийПКО.ВозвратДенежныхСредствРаботником или СчетУчетаРасчетовСКонтрагентом <> Планысчетов.Хозрасчетный.РасчетыСРабочимиИСлужащимиПоДругимОперациям Тогда
//~		
...
//-- БУ ЗИК	
	...
	Если ВидОперации <> Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		...
		МассивНепроверяемыхРеквизитов.Добавить("СчетУчетаДоходовОкругление");
		МассивНепроверяемыхРеквизитов.Добавить("СчетУчетаРасходовОкругление");
	...
	Если (ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка) Тогда
		...
		Если НЕ ВыручкаСНТТ ИЛИ СуммаОкругления <= 0 Тогда
			МассивНепроверяемыхРеквизитов.Добавить("СчетУчетаДоходовОкругление");
		КонецЕсли;
		Если НЕ ВыручкаСНТТ ИЛИ СуммаОкругления >= 0 Тогда
			МассивНепроверяемыхРеквизитов.Добавить("СчетУчетаРасходовОкругление");
	...
	Если ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		Если СуммаДокумента <> СуммаПродаж - СуммаВозврата - СуммаБезналичнойОплаты + СуммаОкругления Тогда
			ТекстСообщения = НСтр("ru='Не совпадают сумма документа и розничная выручка';uk='Не збігаються сума документа та роздрібний виторг'");
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(
				"Поле", "Корректность", НСтр("ru='Сумма документа';uk='Сума документа'"),,, ТекстСообщения); 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "СуммаДокумента", "Объект", Отказ);
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим) //в разработке
	...
	СуммаОкругленияРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтруктураШапкиДокумента.СуммаОкругления, 
											ВалютаДокумента,
											...
											1);
	СтруктураШапкиДокумента.Вставить("СуммаОкругленияРегл", СуммаОкругленияРегл);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации <> Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
		ПриемРозничнойВыручки.Очистить();
		СуммаПродаж = 0;
		СуммаВозврата = 0;
