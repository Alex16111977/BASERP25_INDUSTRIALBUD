...
Функция ЗаполнитьПоДокументуОснованию(Основание) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.НалоговаяНакладная")  Тогда
		...
		СпецРежимНалогообложения 	   = Основание.СпецРежимНалогообложения;
		ТипПричиныНевыдачиПокупателю = Основание.ТипПричиныНевыдачиПокупателю;
		Сделка = Основание.Сделка;
		НомерГТД = Основание.НомерГТД;
		...
		Иначе
			...
			ИначеЕсли Основание.ВидОперации = Перечисления.ВидыОперацийНалоговаяНакладная.ОсвобожденныеОперации Тогда 
				ВидОперации = Перечисления.ВидыОперацийПриложение2КНалоговойНакладной.ОсвобожденныеОперацииКорректировка;	
			ИначеЕсли Основание.ВидОперации = Перечисления.ВидыОперацийНалоговаяНакладная.НеНДСОперации Тогда 
				ВидОперации = Перечисления.ВидыОперацийПриложение2КНалоговойНакладной.НеНДСОперацииКорректировка;
			КонецЕсли;
			
		КонецЕсли; 
		
		ЗаполнитьДаннымиНалоговойНакладнойСУчетомКорректировок();
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если АвторасчетНДС Тогда
		...
		Документы.Приложение2КНалоговойНакладной.ЗаполнитьСуммыРегл(ЭтотОбъект, СуммаВключаетНДС,, Ложь);
...
Процедура ЗаполнитьДаннымиНалоговойНакладнойСУчетомКорректировок() Экспорт

	НомерГруппыКорректировки = 1;
	НомераСтрокСОшибками = Новый Массив;
	
	СоставНН = Документы.НалоговаяНакладная.СостояниеСтрокНалоговойНакладнойСУчетомКорректировок(НалоговаяНакладная, Ссылка);
	...
	Для каждого ДанныеСтрокиНН Из СоставНН Цикл
		...
		Для каждого СтрокаСостава Из ДанныеСтрокиНН.Состав Цикл
			...
			ИсходнаяСтрока = СтрокаСостава.НалоговыйДокумент[СтрокаСостава.ИмяТЧ][СтрокаСостава.НомерСтроки-1];
			
			НоваяСтрока = ЭтотОбъект[СтрокаСостава.ИмяТЧ].Добавить();
			...
			Если ТипЗнч(СтрокаСостава.НалоговыйДокумент) = Тип("ДокументСсылка.НалоговаяНакладная") Тогда
				
				НоваяСтрока.ИзменениеКоличества = ИсходнаяСтрока.Количество;
				НоваяСтрока.ИзменениеСуммы = ИсходнаяСтрока.Сумма;
				НоваяСтрока.ИзменениеСуммыНДС = ИсходнаяСтрока.СуммаНДС;
				НоваяСтрока.ИзменениеСуммыБезНДСРегл = ИсходнаяСтрока.СуммаБезНДСРегл;
				НоваяСтрока.ИзменениеСуммыНДСРегл = ИсходнаяСтрока.СуммаНДСРегл;
				
			Иначе

				Если ИсходнаяСтрока.ИзменениеКоличества <> 0 Тогда
					НоваяСтрока.Количество = ИсходнаяСтрока.ИзменениеКоличества;
				КонецЕсли;
				Если ИсходнаяСтрока.ИзменениеЦены <> 0 Тогда
					НоваяСтрока.Цена = ИсходнаяСтрока.ИзменениеЦены;
					НоваяСтрока.ИзменениеЦены = 0;
				КонецЕсли;                        
				НоваяСтрока.Сумма = ИсходнаяСтрока.ИзменениеСуммы;
				НоваяСтрока.СуммаНДС = ИсходнаяСтрока.ИзменениеСуммыНДС;
			...
			Документы.Приложение2КНалоговойНакладной.ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ЭтотОбъект, НоваяСтрока, СтрокаСостава.ИмяТЧ);			
			
			НоваяСтрокаСторно = ЭтотОбъект[СтрокаСостава.ИмяТЧ].Вставить(НоваяСтрока.НомерСтроки - 1);
			ЗаполнитьЗначенияСвойств(НоваяСтрокаСторно, НоваяСтрока);
			НоваяСтрокаСторно.НомерСтрокиНН = ДанныеСтрокиНН.НомерСтрокиНН;
			
			НоваяСтрокаСторно.ИзменениеКоличества = - НоваяСтрокаСторно.ИзменениеКоличества;
			НоваяСтрокаСторно.ИзменениеСуммы = - НоваяСтрокаСторно.ИзменениеСуммы;
			НоваяСтрокаСторно.ИзменениеСуммыНДС = - НоваяСтрокаСторно.ИзменениеСуммыНДС;
			НоваяСтрокаСторно.ИзменениеСуммыБезНДСРегл = - НоваяСтрокаСторно.ИзменениеСуммыБезНДСРегл;
			НоваяСтрокаСторно.ИзменениеСуммыНДСРегл = - НоваяСтрокаСторно.ИзменениеСуммыНДСРегл;
			Если СтрокаСостава.ИмяТЧ = "Товары" Тогда
	...
	Документы.Приложение2КНалоговойНакладной.ЗаполнитьСуммыРегл(ЭтотОбъект,,, Ложь);
	ДополнительныеСвойства.Вставить("СуммыРеглЗаполнены", Истина);
	...
	Если НомераСтрокСОшибками.Количество() Тогда
		...
		ДополнительныеСвойства.Вставить("СообщениеПользователю", ТекстСообщения);
...
Процедура ДобавитьКолонкиТоваровРеглЛокально(ТаблицаТоваров, СтруктураШапкиДокумента, ПогрешностиОкругления) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		Если СтруктураШапкиДокумента.ВалютаДокумента = СтруктураШапкиДокумента.ВалютаРегламентированногоУчета Тогда
			...
			СтрокаТаблицы.ИзменениеСуммыБезНДСРегл = СтрокаТаблицы.ИзменениеСуммыБезНДСВал;
		Иначе
			
			Если СтрокаТаблицы.ИзменениеСуммыБезНДСРегл = 0 Тогда
				СтрокаТаблицы.ИзменениеСуммыБезНДСРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаТаблицы.ИзменениеСуммыБезНДСВал, 
																					...
																					ЗаполнениеДокументов.КратностьДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета),
																					1,
																					, ПогрешностиОкругления, "ИзменениеСуммыБезНДСРегл");
			КонецЕсли;
			...
			КонецЕсли;
			СтрокаТаблицы.ИзменениеСуммыСНДСРегл = СтрокаТаблицы.ИзменениеСуммыБезНДСРегл + СтрокаТаблицы.ИзменениеСуммыНДСРегл;
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	ЭтоРЕЗ = Ложь;
	НеФормироватьДвиженияРЕЗ = Ложь;
	Если СтруктураШапкиДокумента.ТипПричиныНевыдачиПокупателю = 22 И ТаблицаПоТоварам.Найти(501, "КодПричиныКорректировки") <> Неопределено Тогда

		ЭтоРЕЗ = Истина;
		Если Не ЗначениеЗаполнено(СтруктураШапкиДокумента.ДатаГТД) Тогда
			...
			НеФормироватьДвиженияРЕЗ = Истина;			
		Иначе
			Если НачалоМесяца(СтруктураШапкиДокумента.ДатаГТД) <> НачалоМесяца(СтруктураШапкиДокумента.Дата) Тогда
				СтруктураШапкиДокумента.ПериодДвижений = СтруктураШапкиДокумента.ДатаГТД;
			КонецЕсли;
		КонецЕсли;
		
	КонецЕсли;  
							  
	Если ЭтоРЕЗ И НеФормироватьДвиженияРЕЗ Тогда
	...
	ИначеЕсли  СтруктураШапкиДокумента.СуммаНДСДокумента < 0
...
Процедура ДвиженияПоРегиструНДСНалоговыеОбязательства(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоУслугам, ТаблицаПоОС, ТаблицаПоНМА, Отказ)
	...
	Если СтруктураШапкиДокумента.ТипПричиныНевыдачиПокупателю = 22 И ТаблицаПоТоварам.Найти(501, "КодПричиныКорректировки") <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(СтруктураШапкиДокумента.ДатаГТД) Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В документе %1 не указана дата ГТД. Документ не будет включен в декларацию НДС.';uk='У документі %1 не вказано дату ВМД. Документ не буде включено до декларації ПДВ.'"), Ссылка));
			Возврат;
		Иначе
			ДатаДвижений = СтруктураШапкиДокумента.ДатаГТД;
		КонецЕсли;
		
	Иначе
		ДатаДвижений = СтруктураШапкиДокумента.ПериодДвижений;
	КонецЕсли;  
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоУслугам, ТаблицаПоОС, ТаблицаПоНМА) Экспорт
	...
	СтруктураПолей.Вставить("ИзменениеСуммыБезНДСРегл" 	, "ИзменениеСуммыБезНДСРегл");
	СтруктураПолей.Вставить("ИзменениеСуммыНДСРегл"  	, "ИзменениеСуммыНДСРегл");
