...
Процедура ЗаполнитьСчетаУчетаВТабЧасти(ДанныеОбъекта, ИмяТабЧасти) Экспорт	
	
	ТабличнаяЧасть = ДанныеОбъекта[ИмяТабЧасти];
...
Процедура ЗаполнитьСтатьюДекларацииНДСНалоговыеОбязательства(ДанныеОбъекта, СтрокаТабличнойЧасти, ИмяТабЧасти) Экспорт
	...
	ИначеЕсли ДанныеОбъекта.ТипПричиныНевыдачиПокупателю = 22 Тогда
		...
		Если СтрокаТабличнойЧасти.КодПричины = 501 Тогда
			СтрокаТабличнойЧасти.СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОЭкспортОтдВидовСХПродукции;
		ИначеЕсли СтрокаТабличнойЧасти.КодПричины = 502 Тогда
			СтрокаТабличнойЧасти.СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОКорректировка_ЭкспОтдВидовСХПрод;
		ИначеЕсли СтрокаТабличнойЧасти.КодПричины = 503 Тогда
			СтрокаТабличнойЧасти.СтатьяДекларацииНДСНалоговыеОбязательства = Справочники.СтатьиНалоговыхДеклараций.НДС_НОКорректировка_ЭксОтдВидСХПрИзмНДС;
		КонецЕсли;
...
Процедура ЗаполнитьСуммыРегл(Объект, ИзмениласьСуммаБезНДС = Истина, ИзмениласьСтавкаНДС = Ложь, ЗаполнятьОтрицательныеСтроки = Неопределено) Экспорт
	
	//при интерактивном редактировании отрицательные строки не изменяем
	Если ЗаполнятьОтрицательныеСтроки = Неопределено Тогда
		ЗаполнятьОтрицательныеСтроки = ТипЗнч(Объект) = Тип("ДокументОбъект.Приложение2КНалоговойНакладной");
	КонецЕсли;
	
	ВалютаРегл = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	ПогрешностиОкругления = Новый Соответствие();
	ПогрешностиОкругленияОтрицательныеСтроки = Новый Соответствие();
	СтруктураТЧ = Новый Структура("Товары, Услуги, ОС, НМА");
	...
	Если Объект.ВалютаДокумента = ВалютаРегл Тогда
		...
		Для каждого ТекТЧ Из СтруктураТЧ Цикл
			Для каждого СтрокаТаблицы ИЗ Объект[ТекТЧ.Ключ]	Цикл
				СтрокаТаблицы.ИзменениеСуммыБезНДСРегл = 0;
				СтрокаТаблицы.ИзменениеСуммыНДСРегл = 0;
			КонецЦикла;	
		КонецЦикла;
		
	Иначе
		
		Если ЗаполнятьОтрицательныеСтроки Тогда
			СоставНН = Документы.НалоговаяНакладная.СостояниеСтрокНалоговойНакладнойСУчетомКорректировок(Объект.Ссылка, Объект.Ссылка); 
		КонецЕсли;
		
		Для каждого ТекТЧ Из СтруктураТЧ Цикл
			Для каждого СтрокаТаблицы ИЗ Объект[ТекТЧ.Ключ]	Цикл
				
				ИсточникКурсов = Объект;
				Погрешности = ПогрешностиОкругления;
				
				Если СтрокаТаблицы.ИзменениеСуммы < 0 Тогда
					
					Если НЕ ЗаполнятьОтрицательныеСтроки Тогда
						Продолжить;
					КонецЕсли;
					
					ИсходнаяСтрока = СоставНН.Найти(СтрокаТаблицы.НомерСтрокиНН, "НомерСтрокиНН");
					Если ИсходнаяСтрока <> Неопределено Тогда
						
						Если ИсходнаяСтрока.Сумма = - СтрокаТаблицы.ИзменениеСуммы Тогда
							
							СтрокаТаблицы.ИзменениеСуммыБезНДСРегл = - ИсходнаяСтрока.СуммаБезНДСРегл;
							СтрокаТаблицы.ИзменениеСуммыНДСРегл = - ИсходнаяСтрока.СуммаНДСРегл;
							Продолжить;
							
						Иначе
							
							ИсточникКурсов = ИсходнаяСтрока.Состав[0].НалоговыйДокумент;
							Погрешности = ПогрешностиОкругленияОтрицательныеСтроки;
				...
				Если ИзмениласьСуммаБезНДС Тогда
					СтрокаТаблицы.ИзменениеСуммыБезНДСРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(?(Объект.СуммаВключаетНДС, СтрокаТаблицы.ИзменениеСуммы - СтрокаТаблицы.ИзменениеСуммыНДС, СтрокаТаблицы.ИзменениеСуммы), 
																			Объект.ВалютаДокумента,
																			ВалютаРегл, 
																			ИсточникКурсов.КурсВзаиморасчетов, 
																			1,
																			ИсточникКурсов.КратностьВзаиморасчетов,
																			1,
																			, Погрешности, "СуммаБезНДСРегл");
				КонецЕсли;												
				
				Если ИзмениласьСуммаБезНДС ИЛИ ИзмениласьСтавкаНДС Тогда
					СтрокаТаблицы.ИзменениеСуммыНДСРегл = УчетНДСКлиентСервер.РассчитатьСуммуНДСсУчетомПогрешности(СтрокаТаблицы.ИзменениеСуммыБезНДСРегл,
															Истина, Ложь,
...
Процедура ЗаполнитьВесТоваров(Объект) Экспорт
	...
	Для каждого СтрокаТоваров Из Объект.Товары Цикл
		...
		СтрокаТоваров.Вес = Документы.НалоговаяНакладная.РассчитатьВес(СтрокаТоваров.Номенклатура, ?(СтрокаТоваров.ИзменениеКоличества = 0, СтрокаТоваров.Количество, СтрокаТоваров.ИзменениеКоличества), СтрокаТоваров.Коэффициент, СтрокаТоваров.КодУКТВЭД);
		...
		Если ТекРабочаяДата >= '2016-04-01' Тогда
			...
			Если НЕ ВыгрузкаПечатьНН_ПоФормеНаДатуДокумента = Истина Тогда
				...
				Если ТекРабочаяДата >= '2024-10-01' Тогда					
					...
					НовТабДокумент =  ПечатьДокументаНалоговыйКодекс2024(Объект, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, "Приложение2КНалоговойНакладной2024");
...
Функция ПечатьДокументаНалоговыйКодекс2023(Объект, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета = "Приложение2КНалоговойНакладной") Экспорт	
	...
	Для Страница = 1 По 1 Цикл
		...
		ВремТаблица.Свернуть("СтавкаНДС", "ИзменениеСуммыБезНДС, ИзменениеСуммыНДС");
		Для каждого СтрокаТаблицы Из ВремТаблица Цикл
			КолонкаДляСуммы = ОпределитьКодСтавки(ОпределитьКолонкуТаблицы(СтрокаТаблицы.СтавкаНДС), Объект.ВалютаДокумента, КонтрагентНеРезидент, Шапка.ТипПричиныНевыдачиПокупателю);
			Если  Шапка.СоглашениеОРазделеПродукции = Истина Тогда
				// данные не заполняются
			ИначеЕсли КолонкаДляСуммы = "20" Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
			...
			// Определим в какую колонку (в зависимости от ставки НДС) попадает сумма без НДС
			ОбластьМакета.Параметры.КодСтавки = ОпределитьКодСтавки(ОпределитьКолонкуТаблицы(СтрокаТаблицы.СтавкаНДС), Объект.ВалютаДокумента, КонтрагентНеРезидент, Шапка.ТипПричиныНевыдачиПокупателю);
...
Функция ПечатьДокументаНалоговыйКодекс2024(Объект, ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета = "Приложение2КНалоговойНакладной") Экспорт	
	Перем СтруктураШапкиДокумента;
	Перем ТаблицаНоменклатуры, ТаблицаПоТаре, ТаблицаПоУслугам, ТаблицаПоОС, ТаблицаПоНМА;
	...
	|	КтоВыписалНалоговуюНакладную 			КАК КтоВыписалНалоговуюНакладную,
	|	КтоВыписалНалоговуюНакладную.КодПоДРФО  КАК ИННКтоВыписалНН,
	|	Приложение2.ДоговорКонтрагента.Дата КАК ДатаДоговора,
	|	Приложение2.ДоговорКонтрагента.Номер КАК НомерДоговора,
	|	ISNULL(Приложение2.ДоговорКонтрагента.ФорвардныйКонтракт, ЛОЖЬ) КАК ФорвардныйКонтракт,
	|	Приложение2.НомерГТД КАК НомерГТД,
	|	Приложение2.ДатаГТД КАК ДатаГТД
	|ИЗ
	|	Документ.Приложение2КНалоговойНакладной КАК Приложение2
	|
	|ГДЕ
	...
	Для каждого СтрокаТоваров Из ТаблицаНоменклатуры Цикл
		Если ЗначениеЗаполнено(СтрокаТоваров.Вес) И СтрокаТоваров.РежимЭкспортногоОбеспечения Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТоваров.ИзменениеКоличества) Тогда
				СтрокаТоваров.ИзменениеКоличества = СтрокаТоваров.Вес;
				СтрокаТоваров.ЦенаМестаБезНДСБезСкидкиРегл = Окр(СтрокаТоваров.ИзменениеСуммыБезНДСРегл / СтрокаТоваров.Вес, 2);
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТоваров.ИзменениеЦеныБезНДССоСкидкойРегл) Тогда
				СтрокаТоваров.ИзменениеЦеныБезНДССоСкидкойРегл = Окр(СтрокаТоваров.ИзменениеСуммыБезНДСРегл / СтрокаТоваров.Вес, 2);
			КонецЕсли;
	...
	Для Страница = 1 По 1 Цикл
		...
		Если Шапка.ТипПричиныНевыдачиПокупателю = 7 ИЛИ Шапка.ТипПричиныНевыдачиПокупателю = 22 Тогда
			...
			Если ТаблицаНоменклатуры.Найти(501, "КодПричиныКорректировки") = Неопределено Тогда
		...
		Если Шапка.ТипПричиныНевыдачиПокупателю = 22 Тогда
			...
			Если ТаблицаНоменклатуры.Количество() > 2 Тогда
				ПредупрежденияПечати.Вставить("КоличествоСтрок", НСтр("ru='в разделе Б должно быть не больше двух строк';uk='у розділі Б має бути не більше двох рядків'"));
			КонецЕсли; 
		...
		Для каждого СтрокаТаблицы Из ТаблицаНоменклатуры Цикл
			...
			ИначеЕсли Шапка.Сводная = Истина И Шапка.ВидОперации = Перечисления.ВидыОперацийПриложение2КНалоговойНакладной.УсловнаяПродажаВозврат 
			...
			Если ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Истина Тогда
				...
				СтрокаСтруктуры.G5  = ТипЧисло.ПривестиЗначение(ОбластьМакета.Параметры.ИзменениеКоличества);
				СтрокаСтруктуры.G6  = ТипЧисло.ПривестиЗначение(ОбластьМакета.Параметры.ЦенаБезНДС);
				Если Не ЗначениеЗаполнено(СтрокаСтруктуры.G5) Тогда
					СтрокаСтруктуры.G6  = 0;	
				КонецЕсли;
				
				СтрокаСтруктуры.G7  = ТипЧисло.ПривестиЗначение(ОбластьМакета.Параметры.ИзменениеЦеныБезНДС);
				СтрокаСтруктуры.G8  = ТипЧисло.ПривестиЗначение(ОбластьМакета.Параметры.Количество);
...
Функция ОпределитьКодСтавки(ИмяКолонки, ВалютаДокумента, КонтрагентНеРезидент, ТипПричиныНевыдачи = Неопределено)
