...
Функция ПечатьДокументаДляЭДО(ДокументСсылка) Экспорт
	...
	ФИОПредставителя = ?(ДанныеПредставителя.Фамилия = Неопределено,"",ДанныеПредставителя.Фамилия + " ") + 
	...
	СтруктураПоказателей.ПервичныйДокумент.Вставить("ПредставительПоставщика", ФИОПредставителя);
	СтруктураПоказателей.ПервичныйДокумент.Вставить("ДолжностьПредставителя",  ДолжностьПредставителя);
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		КоличествоТоварыУслуги = КоличествоТоварыУслуги + 1; 
	КонецЦикла;  
	СтруктураПоказателей.ПервичныйДокумент.Вставить("КоличествоНаименований", КоличествоТоварыУслуги);
...
//
		...
		Если НЕ Шапка.ПолучилПоДругомуДокументу Тогда
			ОбластьМакета.Параметры.ДокументПодтверждающийПолномочия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По доверенности %1 № %2 от %3';uk= 'За довіреністю %1 № %2 від %3'"),
				Шапка.ДоверенностьСерия, Шапка.ДоверенностьНомер, Формат(Шапка.ДоверенностьДата, "ДФ=dd.MM.yyyy"));
		КонецЕсли; 
		ТабДокумент.Вывести(ОбластьМакета); 
