...
Функция ЗаполнитьВзносы(ВременнаяСсылка, МенеджерВременныхТаблиц, Организация, МесяцНачисления, ПоВсемДокументам = Истина) Экспорт
	...
	Запрос.Текст = 
		...
		|       	 КОГДА ЕСТЬNULL(НастройкиУчетаЗарплаты.ОсновнаяКатегорияМобилизованныхРаботников, ЗНАЧЕНИЕ(Справочник.КатегорииЗастрахованныхЛицЕСВ.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.КатегорииЗастрахованныхЛицЕСВ.ПустаяСсылка)
		|       	  ТОГДА НастройкиУчетаЗарплаты.ОсновнаяКатегорияМобилизованныхРаботников
...
Процедура РассчитатьВзносы(ВременнаяСсылка, НаборыЗаписей, ДанныеЗаполнения, ДокументСсылка, Организация, МесяцНачисления, ПоВсемДокументам = Истина) Экспорт
	...
	Запрос.Текст = 
       ...
       |     И НЕ (ТаблицаБаза.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ОсновнаяЗарплата) И ТаблицаБазаВычет.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигКонтрактам))
	   |     И НЕ (ТаблицаБазаВычет.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ОсновнаяЗарплата) И ТаблицаБаза.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигКонтрактам))
	   |СГРУППИРОВАТЬ ПО
	   |	ТаблицаБаза.ФизическоеЛицо,
	   |	ТаблицаБаза.ВидЕСВ,
	   |	ТаблицаБаза.НалоговыйПериод
       ...
       |	И ТаблицаБаза.ВидЕСВ <> ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.ПоГигКонтрактам)
	   |СГРУППИРОВАТЬ ПО
	   |	ТаблицаБаза.ФизическоеЛицо,
	   |	ТаблицаБаза.НалоговыйПериод
	   |;
	...
	Для Каждого СтрокаВзносовФОТ из ВзносыФОТ Цикл
		...
		Если СтрокаВзносовФОТ.ВидЕСВ = Перечисления.ВидыЕСВ.ПоГигКонтрактам Тогда
	    	БазаПолная = СтрокаВзносовФОТ.База;
		КонецЕсли;	
