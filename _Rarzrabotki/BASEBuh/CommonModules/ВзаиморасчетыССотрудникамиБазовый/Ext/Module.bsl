...
Функция ЗарплатаКВыплате(СпособВыплаты, ПараметрыЗаполнения, Регистратор) Экспорт
	...
	Если ПараметрыЗаполнения.ДатаДокумента = НачалоМесяца(ПараметрыЗаполнения.ДатаДокумента) Тогда
		Запрос.УстановитьПараметр("Дата", ПараметрыЗаполнения.ДатаДокумента+1);
	Иначе	
...
Процедура РассчитатьЗарплату(Зарплата, ДанныеДляРасчета) Экспорт
	...
	ВБольшуюСторону = ДанныеДляРасчета.Округление.ВБольшуюСторону; 
	
	Для Каждого СтрокаЗарплаты Из Зарплата Цикл
		КВыплате = СтрокаЗарплаты.КВыплате;
		Множитель = Цел(СтрокаЗарплаты.КВыплате / ТочностьДо);
		СтрокаЗарплаты.КВыплате = Множитель * ТочностьДо; 
		Если ВБольшуюСторону И КВыплате > СтрокаЗарплаты.КВыплате Тогда
			СтрокаЗарплаты.КВыплате = (Множитель + 1) * ТочностьДо;	
		КонецЕсли;	
