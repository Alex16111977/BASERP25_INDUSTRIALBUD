...
Процедура ЗаполнитьКонтрагента(Контрагент, Параметры)
	...
	Попытка
		...
		КонтактнаяИнформацияКопия = Контрагенты.КонтактнаяИнформация.Выгрузить(); 
		...
		Для Каждого Стр Из КонтактнаяИнформацияКопия Цикл
			Если Стр.Вид = Справочники.ВидыКонтактнойИнформации.ПочтовыйАдресКонтрагента 
					Или Стр.Вид = Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента Тогда
				НовСтрока = Контрагенты.КонтактнаяИнформация.Добавить();
...
Функция ПолучитьТекстОшибкиРеестров(КодОшибки)
	Если КодОшибки=1 Тогда
		Возврат НСтр("ru='К сожалению, на данный момент Реестр плательщиков единого налога работает нестабильно. 
				...
				|;uk='На жаль, на даний момент Реєстр платників єдиного податку працює нестабільно.
	...
	ИначеЕсли КодОшибки=2 Тогда
		Возврат	НСтр("ru='К сожалению, на данный момент Реестр плательщиков НДС работает нестабильно. 
				|Карточка контрагента сформирована без заполнения данных по налогообложению. 
				|Запустите повторно заполнение карточки через несколько минут. При повторном заполнении карточки сегодня до 23:59 Ваш баланс не уменьшится.'
				|;uk='На жаль, на даний момент Реєстр платників ПДВ працює нестабільно.
    			|Картка контрагента сформована без заповнення даних з оподаткування.
    			|Запустіть повторно заповнення картки через кілька хвилин. При повторному заповненні картки сьогодні до 23:59 Ваш баланс не зменшиться.'");	
	ИначеЕсли КодОшибки=3 Тогда
		Возврат	НСтр("ru='К сожалению, на данный момент Реестр плательщиков НДС и Реестр плательщиков единого налога работают нестабильно. 
				|Карточка контрагента сформирована без заполнения данных по налогообложению. 
				|Запустите повторно заполнение карточки через несколько минут. При повторном заполнении карточки сегодня до 23:59 Ваш баланс не уменьшится.'
				|;uk='На жаль, на даний момент Реєстр платників ПДВ і Реєстр платників єдиного податку працюють нестабільно.
    			|Картка контрагента сформована без заповнення даних з оподаткування.
    			|Запустіть повторно заповнення картки через кілька хвилин. При повторному заповненні картки сьогодні до 23:59 Ваш баланс не зменшиться.'");	
