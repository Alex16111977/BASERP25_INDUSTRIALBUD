	...
	Иначе
		Запрос.Текст = 
		   "ВЫБРАТЬ РАЗЛИЧНЫЕ 
			...
			|   ВЫБОР
		    |    	КОГДА ОборотыДляНДФЛ.Период >= ДАТАВРЕМЯ(2024,12,1)
		    |    	    И ОборотыДляНДФЛ.НалоговыйПериод < ДАТАВРЕМЯ(2024,12,1)
			|    	    И ЕСТЬNULL(НастройкиУчетаЗарплаты.ВС2024Больничные, Ложь)
		    //Больничные в декабре 2024 или позже с периодом до декабря 2024
		    |			ТОГДА ОборотыДляНДФЛ.НалоговыйПериод
		    |    	КОГДА ОборотыДляНДФЛ.Период < ДАТАВРЕМЯ(2024,12,1)
		    |    	    И ОборотыДляНДФЛ.НалоговыйПериод >= ДАТАВРЕМЯ(2024,12,1)
			|    	    И &ПериодРегистрации >= ДАТАВРЕМЯ(2024,12,1)
			|    	    И ЕСТЬNULL(НастройкиУчетаЗарплаты.ВС2024Отпускные, Ложь)
			//Отпускные до декабря 2024 с периодом декабрь 2024 или позже
		    |			ТОГДА ОборотыДляНДФЛ.НалоговыйПериод
		    |		ИНАЧЕ НАЧАЛОПЕРИОДА(ОборотыДляНДФЛ.Период, МЕСЯЦ) 
...
Процедура РассчитатьНДФЛ(ВременнаяСсылка, НаборыЗаписей, ДанныеЗаполнения, ДокументСсылка, Организация, МесяцНачисления, ПоВсемДокументам = Истина) Экспорт
	...
	Иначе
		Запрос.Текст = 
		   ...
		   |    ВЫБОР
		   |    	КОГДА НачисленияУдержанияПоСотрудникамОбороты.Период >= ДАТАВРЕМЯ(2024,12,1)
		   |    	    И НачисленияУдержанияПоСотрудникамОбороты.НалоговыйПериодНДФЛ < ДАТАВРЕМЯ(2024,12,1)
		   |    	    И ЕСТЬNULL(НастройкиУчетаЗарплаты.ВС2024Больничные, Ложь)
		   //Больничные в декабре 2024 или позже с периодом до декабря 2024
		   |			ТОГДА НачисленияУдержанияПоСотрудникамОбороты.НалоговыйПериодНДФЛ
		   |    	КОГДА НачисленияУдержанияПоСотрудникамОбороты.Период < ДАТАВРЕМЯ(2024,12,1)
		   |    	    И НачисленияУдержанияПоСотрудникамОбороты.НалоговыйПериодНДФЛ >= ДАТАВРЕМЯ(2024,12,1)
		   |    	    И &МесяцНачисления >= ДАТАВРЕМЯ(2024,12,1)
		   |    	    И ЕСТЬNULL(НастройкиУчетаЗарплаты.ВС2024Отпускные, Ложь)
		   //Отпускные до декабря 2024 с периодом декабрь 2024 или позже
		   |			ТОГДА НачисленияУдержанияПоСотрудникамОбороты.НалоговыйПериодНДФЛ
		   |		ИНАЧЕ НАЧАЛОПЕРИОДА(НачисленияУдержанияПоСотрудникамОбороты.Период, МЕСЯЦ) 
		   |	КОНЕЦ КАК НалоговыйПериодНДФЛ,
		   |	НачисленияУдержанияПоСотрудникамОбороты.СуммаОборот КАК СуммаОборот
		   |
	...
	Запрос.Текст = 
	   ...
	   |    ВЫБОР
	   |		КОГДА ЕСТЬNULL(СпециальныеРежимыЗарплатаКадры.РезидентДияСити, ЛОЖЬ) И ВидыДоходовНДФЛ.ВидСтавкиДия <> ЗНАЧЕНИЕ(Перечисление.ВидыСтавокНДФЛ.ПустаяСсылка)
	   |		ТОГДА ЕСТЬNULL(История4.СтавкаНДФЛ, 0)
       |        ИНАЧЕ
	   |        ВЫБОР
	   |     	 КОГДА НЕ ЕСТЬNULL(СписокШахтеров.Шахтер, ЛОЖЬ) 
	   ...
	   |    ЛЕВОЕ СОЕДИНЕНИЕ
	   |	ВТИсторияНормативныхВеличин КАК История4
	   |    ПО ВидыДоходовНДФЛ.ВидСтавкиДия = История4.ВидСтавкиНДФЛ
	   |     И СтрокиРасчета.НалоговыйПериод = История4.НалоговыйПериод
	   |	
	   |;
	   |";
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "УчетНДФЛ.ЗаполнениеВидовПрочихДоходов";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.1.29.2";
...
Процедура ЗаполнениеВидовПрочихДоходов() Экспорт
	
	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("Справочник", "ВидыПрочихДоходовФизическихЛиц",,Истина);	
