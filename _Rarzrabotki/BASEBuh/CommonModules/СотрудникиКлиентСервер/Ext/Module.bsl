...
Процедура УстановитьИнфоНадпись(Форма) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.ДатаУвольнения) Тогда
		...
		Если ПрошлоДнейСМоментаУвольнения > 370 Тогда
			Форма.СотрудникИнфо = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Формат(Форма.ДатаУвольнения, НСтр("ru='Л=ru; ДФ=''ММММ гггг """"г.""""''';uk= 'Л=uk; ДФ=''ММММ гггг """"р.""""'''")), Форма.Элементы.ВАрхиве.Заголовок);
		Иначе
			Форма.СотрудникИнфо = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Формат(Форма.ДатаУвольнения, НСтр("ru='Л=ru; ДФ=''ММММ гггг """"г.""""''';uk= 'Л=uk; ДФ=''ММММ гггг """"р.""""'''")));
		КонецЕсли;			
	...
	Иначе
		Форма.СотрудникИнфо = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Формат(Форма.ДатаПриема, НСтр("ru='Л=ru; ДФ=''ММММ гггг """"г.""""''';uk= 'Л=uk; ДФ=''ММММ гггг """"р.""""'''")));		
	КонецЕсли;		
