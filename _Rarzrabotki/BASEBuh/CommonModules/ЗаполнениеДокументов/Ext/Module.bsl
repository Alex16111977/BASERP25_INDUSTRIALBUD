...
Процедура Заполнить(ДокументОбъект, ДанныеЗаполнения = Неопределено) Экспорт
	...
	//Тютюн   
	Если ЗначениеЗаполнено(ДанныеЗаполнения) И НЕ ТипДанныхЗаполнения = Тип("Структура")  Тогда
		МетаданныеДанныхЗаполнения = ДанныеЗаполнения.Метаданные();  
		Если ОбщегоНазначенияБП.ЕстьРеквизитДокумента("Направление",МетаданныеДанныхЗаполнения) 
			И ОбщегоНазначенияБП.ЕстьРеквизитДокумента("Направление",МетаданныеДокумента) Тогда
			ДокументОбъект.Направление = ДанныеЗаполнения.Направление;	
		КонецЕсли;	
		Если ОбщегоНазначенияБП.ЕстьРеквизитДокумента("Подразделение",МетаданныеДанныхЗаполнения) 
			И ОбщегоНазначенияБП.ЕстьРеквизитДокумента("Подразделение",МетаданныеДокумента) Тогда
			ДокументОбъект.Подразделение = ДанныеЗаполнения.Подразделение;	
		КонецЕсли;
	КонецЕсли;	  
	//КонецИзменений
