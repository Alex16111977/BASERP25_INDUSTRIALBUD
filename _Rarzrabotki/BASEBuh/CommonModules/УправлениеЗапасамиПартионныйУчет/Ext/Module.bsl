...
Процедура ДвижениеПартийТоваров(ТаблицаДокумента = Неопределено, Отказ = Ложь, Партия = Неопределено, Содержание = "") Экспорт
	...
	Если ТаблицаДокумента.Количество() > 0 Тогда
        ...
        //Тютюн         
		Попытка
			Если ЗначениеЗаполнено(ТаблицаДокумента[0].ДокументОприходования) Тогда  
				ДокументМоментВремени = Новый МоментВремени(ТаблицаДокумента[0].ДокументОприходования.Дата+1, ТаблицаДокумента[0].ДокументОприходования.Ссылка);
		...
		Для индекс = 0 по ПартийПоСтроке Цикл
			...
			Если СтрокаДокумента.КорСчетСписанияБУ.Забалансовый = СчетУчета.Забалансовый Тогда
				...
				Если БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(Проводка.СчетДт) Тогда
					...
					//Тютюн Партия док перемещение 
					Попытка
						Если ТипЗнч(СтрокаДокумента.Регистратор) = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда  
							Проводка.СубконтоДт.Партии = СтрокаДокумента.Регистратор.Ссылка;
						КонецЕсли;  
					Исключение
					КонецПопытки;
					//КонецИзменений
...
|Не списано %3 %4 товару %5, рахунок %6%7'"), ТекстУчет, СтрокаДокумента.НомерСтроки, КоличествоОсталосьПогасить, СтрокаДокумента.Номенклатура.БазоваяЕдиницаИзмерения, СтрокаДокумента.Номенклатура, СчетУчета, ТекстДополнительный);
			Иначе
				// сообщим с другим текстом
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 учет. Строка :%2
|Не достаточно %3 %4 товара %5, счет учета %6%7';uk='%1 облік. Рядок :%2
|Не вистачає %3 %4 товару %5, рахунок %6%7'"), ТекстУчет, СтрокаДокумента.НомерСтроки, КоличествоОсталосьПогасить, СтрокаДокумента.Номенклатура.БазоваяЕдиницаИзмерения, СтрокаДокумента.Номенклатура, СчетУчета, ТекстДополнительный);
			КонецЕсли;
...
Процедура ВыполнитьСписание(ТаблицаСписания, МоментКон, Отказ, Партия, Содержание) Экспорт
	...
	КонецЕсли;
	_ДопРасчеты.УстановитьФильтрПартии(ФильтрПартии,МоментКон.Ссылка,ТаблицаСписания.ВыгрузитьКолонку("Номенклатура"),МассивСчетовУчетаБУ,МассивСкладов,МассивНалоговыхНазначений);// Тютюн 04.12.25 задача № от 041225
