...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	ТекстСообщения = НСтр("ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента платформы.';uk='Для вивантаження зовнішньої обробки (звіту) в файл рекомендується встановити розширення для вебклієнта платформи.'");
