		...
		ИначеЕсли ИмяПоказателя = "АдресПоПолям" Или ИмяПоказателя = "АдресПоПолямФакт"  Тогда
			Если ЗначениеЗаполнено(Организация) Тогда
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					АдресXDTO = Обработки.РасширенныйВводКонтактнойИнформации.АдресXMLВXDTO(НайденныеСтроки[0].ЗначенияПолей);
					СтруктураАдреса = Обработки.РасширенныйВводКонтактнойИнформации.КонтактнаяИнформацияВСтаруюСтруктуру(АдресXDTO, Ложь);
					Результат = СтруктураАдреса.ЗначенияПолей;
				КонецЕсли;
			КонецЕсли;	
			Если Результат <> Неопределено Тогда
				Значение = Результат;
