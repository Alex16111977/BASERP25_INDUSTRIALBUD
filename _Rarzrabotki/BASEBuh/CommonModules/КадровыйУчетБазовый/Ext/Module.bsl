...
Функция ПолучитьСреднююЧисленностьЗаМесяцССЧ(Период, Организация, Параметры) Экспорт
	...
	КонецЕсли;
	ОтборИнвалиды2026 = Ложь;
	Если Не Параметры.Свойство("ОтборИнвалиды2026", ОтборИнвалиды2026) Тогда
		ОтборИнвалиды2026 = Ложь	
	КонецЕсли;	
	...
	Если Параметры.ОтборИнвалиды Или ОтборИнвалиды2026 Тогда
	...
	Если ОтборИнвалиды2026 Тогда
		
		ПоляСвязей = Новый Структура;
		ПоляСвязей.Вставить("ФизическоеЛицо","ФизическоеЛицо");
		РесурсыРС = Новый Структура;
		РесурсыРС.Вставить("Инвалидность"," = ИСТИНА"); 
		РесурсыРС.Вставить("ВключатьВКвотуВДвойномРазмере"," = ИСТИНА"); 
		Запрос.Текст = ЗапросВТИнтервалыДействия("ВТИнвалиды2026","РегистрСведений.СведенияОбИнвалидностиФизическихЛиц","ВТСотрудники",ПоляСвязей,РесурсыРС,Истина);
		Запрос.Выполнить();  
		
	
		СхемаЗапроса = Новый СхемаЗапроса;
		ЗапросФильтр = СхемаЗапроса.ПакетЗапросов.Добавить();      
		Фильтр = "ВТИнвалиды2026";
		ЗапросФильтр.ТаблицаДляПомещения = "ВТПериодыИСотрудники2026";
		Оператор = ЗапросФильтр.Операторы[0];
		Оператор.ВыбиратьРазличные = Истина;
	...
	Если ОтборИнвалиды2026 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		|	ПериодыИСотрудники.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ПериодыИСотрудники.ДатаНачала КАК ДатаНачала,  
		|	ПериодыИСотрудники.ДатаОкончания КАК ДатаОкончания 
		|ПОМЕСТИТЬ ВТПериодыИСотрудникиИнвалиды2026
		|ИЗ ВТПериодыИСотрудники2026 КАК ПериодыИСотрудники 
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|  
		|ВЫБРАТЬ
		...
		|	ПериодыИСотрудники.ДатаНачала КАК ДатаНачала,  
		|	ПериодыИСотрудники.ДатаОкончания КАК ДатаОкончания 
		|ИЗ ВТПериодыИСотрудники КАК ПериодыИСотрудники 
		|
		|;
		|ВЫБРАТЬ
		...
		|   ЛЕВОЕ СОЕДИНЕНИЕ 
		|   ВТПериодыИСотрудникиИнвалиды2026 КАК ПериодыИСотрудники
...
Процедура ЗапросСредняяЧисленностьРабота(Параметры, Запрос, ИмяВТ = "ВТПериодыРаботы", ИмяВТСотрудники = "ВТСотрудники")
	...
	ИсключатьТяжелыеУсловияТруда = Ложь;
	ОтборИнвалиды2026 = Ложь;
	...
	Если НЕ Параметры.Свойство("ОтборСотрудники", ОтборСотрудники) Тогда
		ОтборСотрудники = Неопределено;
	КонецЕсли;
	Если НЕ Параметры.Свойство("ИсключатьТяжелыеУсловияТруда", ИсключатьТяжелыеУсловияТруда) Тогда
		ИсключатьТяжелыеУсловияТруда = Ложь;
	КонецЕсли;
	Если НЕ Параметры.Свойство("ОтборИнвалиды2026", ОтборИнвалиды2026) Тогда
		ОтборИнвалиды2026 = Ложь;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		Если ИсключатьТяжелыеУсловияТруда Или ОтборИнвалиды2026 Тогда  
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	КадроваяИсторияСотрудниковНачало.Сотрудник КАК Сотрудник,
			|	КадроваяИсторияСотрудниковНачало.ФизическоеЛицо КАК ФизическоеЛицо,
			|	КадроваяИсторияСотрудниковНачало.ВидЗанятости КАК ВидЗанятости,
			|	КадроваяИсторияСотрудниковНачало.ВидСобытия КАК ВидСобытия,
			|	&НачалоМесяца КАК Период,
			|	КадроваяИсторияСотрудниковНачало.Должность.ТяжелыеУсловияТруда КАК ТяжелыеУсловияТруда
			|ПОМЕСТИТЬ ВТСотрудникиОрганизацииПериодыПолная
			|ИЗ
			...
			|	КОНЕЦ,
			|	КадроваяИсторияСотрудников.Должность.ТяжелыеУсловияТруда
			|ИЗ
			...
			|";
			Запрос.Выполнить();
			Если НЕ ОтборИнвалиды2026 Тогда
				Запрос.Текст = "
				|ВЫБРАТЬ   
				|	СотрудникиОрганизацииПериоды.Сотрудник КАК Сотрудник,
				|	СотрудникиОрганизацииПериоды.ФизическоеЛицо КАК ФизическоеЛицо,
				|	СотрудникиОрганизацииПериоды.ВидЗанятости КАК ВидЗанятости,
				|	СотрудникиОрганизацииПериоды.ВидСобытия КАК ВидСобытия,
				|	СотрудникиОрганизацииПериоды.Период КАК Период
				|ПОМЕСТИТЬ ВТСотрудникиОрганизацииПериоды
				|ИЗ
				|	ВТСотрудникиОрганизацииПериодыПолная КАК СотрудникиОрганизацииПериоды  
				|ГДЕ
				|	СотрудникиОрганизацииПериоды.ТяжелыеУсловияТруда = ЛОЖЬ
				|";
			Иначе   
				Запрос.Текст = "
				|ВЫБРАТЬ   
				...
				|	СотрудникиОрганизацииПериоды.ВидСобытия КАК ВидСобытия,
				|	СотрудникиОрганизацииПериоды.Период КАК Период
				|ПОМЕСТИТЬ ВТСотрудникиОрганизацииПериоды_
				|ИЗ
				|	ВТСотрудникиОрганизацииПериодыПолная КАК СотрудникиОрганизацииПериоды  
				|ГДЕ
				|	СотрудникиОрганизацииПериоды.ТяжелыеУсловияТруда = ЛОЖЬ
				...
				|	МинимальнаяОплатаТруда.Размер КАК РазмерМинимальнойЗарплаты
				...
				|			&КонецМесяца,
				|			Месяц, 
				|			ТИПЗНАЧЕНИЯ(НачислениеУдержание) = ТИП(ПланВидовРасчета.Начисления)
				|			И ФизическоеЛицо В (ВЫБРАТЬ
				|									Сотрудники.ФизическоеЛицо 
				|									ИЗ ВТСотрудникиОрганизацииПериоды_ КАК Сотрудники)
				|									) КАК НачисленияУдержанияПоСотрудникамОбороты      
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МинимальнаяОплатаТруда.СрезПоследних(&КонецМесяца,) КАК МинимальнаяОплатаТруда
				|		ПО ИСТИНА
				|СГРУППИРОВАТЬ ПО
				...
				|	ПОМЕСТИТЬ ВТСотрудникиОрганизацииПериоды
				|ИЗ       
				|   ВТСотрудникиОрганизацииПериоды_ КАК СотрудникиОрганизацииПериоды
				|ВНУТРЕННЕЕ СОЕДИНЕНИЕ
				|	ВТНачисленияПоМесяцам КАК НачисленияПоМесяцам 
				|ПО
				|   СотрудникиОрганизацииПериоды.Сотрудник = НачисленияПоМесяцам.Сотрудник  
				|   И НАЧАЛОПЕРИОДА(СотрудникиОрганизацииПериоды.Период, МЕСЯЦ) = НАЧАЛОПЕРИОДА(НачисленияПоМесяцам.Период, МЕСЯЦ)
				|ГДЕ НачисленияПоМесяцам.РазмерМинимальнойЗарплаты <= НачисленияПоМесяцам.СуммаОборот
				|";
			КонецЕсли;	
	...
	Иначе
		Если ИсключатьТяжелыеУсловияТруда Тогда   
			Запрос.Текст ="ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ВЫБОР
			...
			|	ТекущиеКадровыеДанныеСотрудников.ТекущаяДолжность.ТяжелыеУсловияТруда КАК ТяжелыеУсловияТруда
			|ПОМЕСТИТЬ ВТСотрудникиОрганизацииПериодыПолная
			|ИЗ
			|	РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанныеСотрудников
			|ГДЕ
			|	ТекущиеКадровыеДанныеСотрудников.ГоловнаяОрганизация = &Организация
			|	И ТекущиеКадровыеДанныеСотрудников.ДатаПриема <= &КонецМесяца
			...
			|	ТекущиеКадровыеДанныеСотрудников.ТяжелыеУсловияТруда КАК ТяжелыеУсловияТруда
			|ПОМЕСТИТЬ ВТСотрудникиОрганизацииИнтервалами
			|ИЗ
			|	ВТСотрудникиОрганизацииПериодыПолная КАК ТекущиеКадровыеДанныеСотрудников
			|ГДЕ
			|	ТекущиеКадровыеДанныеСотрудников.ТяжелыеУсловияТруда = ЛОЖЬ";
		Иначе 
			Запрос.Текст ="ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|   ВЫБОР 
			|     КОГДА ТекущиеКадровыеДанныеСотрудников.ДатаПриема < &НачалоМесяца
			...
			|	ТекущиеКадровыеДанныеСотрудников.ТекущийВидЗанятости КАК ВидЗанятости,
			|	ВЫБОР 
			|     КОГДА ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения > &КонецМесяца ИЛИ ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения = ДАТАВРЕМЯ(1, 1, 1)
			|     ТОГДА &КонецМесяца
			|     ИНАЧЕ ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения
			|   КОНЕЦ КАК ДатаОкончания
			|ПОМЕСТИТЬ ВТСотрудникиОрганизацииИнтервалами
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|   "+?(ОтборВнеШтат,"И СотрудникиОрганизации.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство)","")+"
...
Функция ПолучитьСреднююЧисленностьЗаМесяцЭПЗ(Период, Организация, Параметры) Экспорт
	...
	ОтборИнвалиды2026 = Ложь;
	Если Параметры.Свойство("ОтборИнвалиды2026") Тогда
		ОтборИнвалиды2026 = Параметры.ОтборИнвалиды2026	
	КонецЕсли;	
	ИсключатьТяжелыеУсловияТруда = Ложь;
	Если Параметры.Свойство("ИсключатьТяжелыеУсловияТруда") Тогда
		ИсключатьТяжелыеУсловияТруда = Параметры.ИсключатьТяжелыеУсловияТруда	
	КонецЕсли;	

	Если ИсключатьТяжелыеУсловияТруда = Истина Тогда
		...
		|     ИНАЧЕ ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения
		|   КОНЕЦ КАК ДатаОкончания
		|ПОМЕСТИТЬ ВТСотрудникиОрганизацииПолная
		|ИЗ
		|	РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанныеСотрудников
		|ГДЕ
		...
		|   СотрудникиОрганизации.ТяжелыеУсловияТруда КАК ТяжелыеУсловияТруда,  
		...
		|ИЗ ВТСотрудникиОрганизацииПолная КАК СотрудникиОрганизации
		|ГДЕ  СотрудникиОрганизации.ТяжелыеУсловияТруда = ЛОЖЬ  
		|;";
	Иначе
		ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|   ВЫБОР 
...
Функция ПутьКДаннымПоИмениЗапрашиваемыхДанныхГражданства(Знач ИмяПоля)
	...
	СоответствиеЗапрашиваемыхДанных = Новый Соответствие;
	СоответствиеЗапрашиваемыхДанных.Вставить("СТРАНА",					"ЕСТЬNULL(ГражданствоФизическихЛицСрезПоследних.Страна, ЗНАЧЕНИЕ(Справочник.СтраныМира.Украина))");
