...
Процедура СформироватьДвиженияПоОтражениюЗарплатыВРегламентированномУчете(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеДляОтражения) Экспорт
	...
	ФорматОбмена30 = Ложь;
	Если ДанныеДляОтражения.Свойство("ФорматОбмена30") Тогда
		ФорматОбмена30 = ДанныеДляОтражения.ФорматОбмена30;
	КонецЕсли;
	...
	Если ДанныеПоЗарплате.Количество() <> 0 Тогда
		...
		Иначе
			СформироватьДвиженияНачислениеЗарплатыФорматОбмена30(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеПоЗарплате);
		КонецЕсли;	
	КонецЕсли;
	...
	Если ДанныеДляОтражения.Свойство("УдержаннаяЗарплата") Тогда
		...
		Иначе
			СформироватьДвиженияУдержанияФорматОбмена30(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеПоУдержаниям);
		КонецЕсли;	
	КонецЕсли;
	...
	Если ДанныеДляОтражения.Свойство("УдержанныйНДФЛ") Тогда		
		...
		Иначе
			СформироватьДвиженияУдержанныйНДФЛФорматОбмена30(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеПоЗарплате);
		КонецЕсли;	
	КонецЕсли;
	...
	Если ДанныеДляОтражения.Свойство("ОценочныеОбязательства") Тогда		
		Если ДанныеДляОтражения.ОценочныеОбязательства.Количество() > 0 Тогда
...
Процедура СформироватьДвиженияНачислениеЗарплатыФорматОбмена30(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеДляОтражения)
	
	Дополнять = Ложь;
	
	Если ДанныеДляОтражения.Свойство("НачисленнаяЗарплатаИВзносы") Тогда		
		Параметры = УчетЗарплатыПереопределяемый.ПодготовитьПараметрыПоОтражениюЗарплатыВУчетеФорматОбмена30(ДанныеДляОтражения.НачисленнаяЗарплатаИВзносы, Организация, ПериодРегистрации, Дата);
		Реквизиты = Параметры.Реквизиты[0];
		ТаблицаЗаработнойПлаты = Параметры.ТаблицаЗаработнойПлаты;
		Дополнять = Истина;
	КонецЕсли;
	
	ТаблицаЗаработнойПлаты = УчетЗарплатыПереопределяемый.ДополнитьТаблицуЗарплатыАналитикойБухУчетаФорматОбмена30(ТаблицаЗаработнойПлаты, ПериодРегистрации);
...
Процедура СформироватьДвиженияУдержанныйНДФЛФорматОбмена30(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеДляОтражения)
	
	Параметры = УчетЗарплатыПереопределяемый.ПодготовитьПараметрыПоОтражениюНДФЛВУчетеФорматОбмена30(ДанныеДляОтражения.УдержанныйНДФЛ, Организация, ПериодРегистрации, Дата);
	Реквизиты = Параметры.Реквизиты[0];
	ТаблицаНДФЛ = Параметры.ТаблицаНДФЛ;
	
	ТаблицаНДФЛ = УчетЗарплатыПереопределяемый.ДополнитьТаблицуНДФЛАналитикойБухУчетаФорматОбмена30(ТаблицаНДФЛ);
...
Процедура СформироватьДвиженияУдержанияФорматОбмена30(Движения, Отказ, Организация, ПериодРегистрации, Дата, ДанныеДляОтражения)
	
	Параметры = УчетЗарплатыПереопределяемый.ПодготовитьПараметрыПоОтражениюУдержанийВУчетеФорматОбмена30(ДанныеДляОтражения, Организация, ПериодРегистрации, Дата);
	Реквизиты = Параметры.Реквизиты[0];
	
	ТаблицаУдержаний = Параметры.ТаблицаУдержаний;
	
	ТаблицаУдержаний = УчетЗарплатыПереопределяемый.ДополнитьТаблицуУдержанийАналитикойБухУчетаФорматОбмена30(ТаблицаУдержаний);
...
//-- БУ ЗИК	
	...
	Запрос.Текст = 
			...
			|ИЗ
			|	ВТТаблицаВедомостей КАК ДанныеДокумента
			...
			|ИЗ
			|	ВТ_ФизическиеЛица КАК ФизическиеЛица
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
			|		ПО ФизическиеЛица.КодПоДРФО = Контрагенты.КодПоЕДРПОУ
	...
	Запрос.Текст = 
			...
			|	ПО ДанныеВедомостей.ФизическоеЛицо = Контрагенты.ФизическоеЛицо
			|";
...
//~		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "РаботникиОрганизаций", Платеж.ФизическоеЛицо);
//~		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты", Платеж.Контрагент);
//~	КонецЕсли;
//-- БУ ЗИК	
...
//~		ТаблицаСотрудников = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(ФизическиеЛица, Истина, Организация, ДатаСреза, ДатаСреза); 
//-- БУ ЗИК
...
//~		ТаблицаСотрудников = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(СтрокаТаблицы.ФизическоеЛицо, Истина, СтрокаТаблицы.Организация, СтрокаТаблицы.Дата, СтрокаТаблицы.Дата);
//-- БУ ЗИК		
		Если ТаблицаСотрудников.Количество() <> 0 Тогда			
