...
Процедура ДвиженияПоЕдиномуНалогуПрочее(ДокументОбъект,СтруктураШапкиДокумента,Отказ=Ложь) Экспорт
	...
	Если РозничнаяВыручка Тогда
		Сумма = СтруктураШапкиДокумента.СуммаПродаж + СтруктураШапкиДокумента.СуммаОкругления;
...
Процедура ДвиженияПоРегистрамНалоговогоУчетаУпрощенныйНалоговыйУчет(ДокументОбъект, ТаблицаПоВторомуСобытиюНал = Неопределено, ДатаДвижений = Неопределено, НеУчитываемыеДвиженияДокумента = Неопределено, ДвиженияДокументаКакОстатки = Ложь, УчитываемыеДвиженияДокумента = Неопределено
	...
	Для каждого ДвижениеДокумента Из ДвиженияДокумента Цикл
		...
		Если  ДвижениеДокумента.ВедениеВзаиморасчетовНУ = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам 
			...
			ИначеЕсли (СхемаНДС = Перечисления.МоментыОпределенияНалоговойБазы.ПоОплате   И РасчОплВсеДок  = 0) 
			      ИЛИ (СхемаНДС = Перечисления.МоментыОпределенияНалоговойБазы.ПоОтгрузке И РасчОтгрВсеДок = 0) Тогда
					...
					СтрокаВторогоСобытия.СуммаВзаиморасчетов      = СуммаВсеРД;
					СтрокаВторогоСобытия.СуммаНДСПропорционально  = СуммаНДСПрРД;
					СтрокаВторогоСобытия.БазаНДС 				  = СтрокаВторогоСобытия.СуммаВзаиморасчетов - СтрокаВторогоСобытия.СуммаНДС;
					...
					СтрокаВторогоСобытия.СуммаВзаиморасчетов      = СуммаВсеСД;
		...
		Если (НЕ (ДвижениеДокумента.ВедениеВзаиморасчетовНУ = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам) 
			...
			СтрокаВторогоСобытия.СуммаВзаиморасчетов	  = СтрокаВторогоСобытия.СуммаВзаиморасчетов	 - ПСРасчВсе;;
			СтрокаВторогоСобытия.БазаНДС 				  = СтрокаВторогоСобытия.СуммаВзаиморасчетов 	 - СтрокаВторогоСобытия.СуммаНДС;
		...
		Если ( НЕ ДвижениеДокумента.ВедениеВзаиморасчетовНУ = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам 
			...
			СтрокаВторогоСобытия.СуммаВзаиморасчетов	  = СтрокаВторогоСобытия.СуммаВзаиморасчетов - ПСВозвВсе;
			СтрокаВторогоСобытия.БазаНДС 				  = СтрокаВторогоСобытия.СуммаВзаиморасчетов - СтрокаВторогоСобытия.СуммаНДС;
...
Процедура РазбитьПроводкуПоНДСНаПервоеВтороеСобытие(ТаблицаПоВторомуСобытиюНал, ДвиженияДокумента, Проводка,
	...
	Если СуммаВторогоСобытия > 0 Тогда
		...
		Если РасчетыВВалюте Тогда
			СуммаПроводкиВтороеСобытие = УчетНДСКлиентСервер.РассчитатьСуммуНДСсУчетомПогрешности(СуммаВторогоСобытия * КурсЗачетаАванса, Истина, Ложь, СтавкаНДС) * ?(Проводка.Сумма<0, -1, 1);
		КонецЕсли;
		...
		Если СуммаПроводки > СуммаВторогоСобытия Тогда
			...
			// суммы
			Если РасчетыВВалюте Тогда                                                                                  
				ПроводкаВтороеСобытие["Сумма"] = СуммаПроводкиВтороеСобытие;
		...
		Если СуммаВторогоСобытия > 0 Тогда
			...
			Если СуммаПроводки > СуммаВторогоСобытия Тогда
				...
				Если РасчетыВВалюте Тогда
					...
					СуммаВторогоСобытияРегл = УчетНДСКлиентСервер.РассчитатьСуммуНДСсУчетомПогрешности(СуммаВторогоСобытия * СтруктураКурсаДокумента.Курс / СтруктураКурсаДокумента.Кратность, Истина, Ложь, СтавкаНДС);
			...
			Иначе
				...
				Если РасчетыВВалюте Тогда
					СуммаПроводкиВтороеСобытие 			= УчетНДСКлиентСервер.РассчитатьСуммуНДСсУчетомПогрешности(СуммаПроводки * КурсЗачетаАванса, Истина, Ложь, СтавкаНДС) * ?(Проводка.Сумма<0, -1, 1);
					Проводка.Сумма = СуммаПроводкиВтороеСобытие;
...
Функция ПолучитьСуммуПроводкиПоНДС(ТаблицаПоВторомуСобытиюНал, СуммаНДС, 
									  ...
									  ДоговорКонтрагента, 
									  Сделка,  РасчетныйДокумент = Неопределено,
									  ВозвратнаяТара, ДляХозяйственнойДеятельности = Неопределено, ВидДеятельностиНДС = Неопределено, Амортизируется = Неопределено,
									  КурсыВалют = Неопределено, СтавкаНДС = Неопределено) Экспорт
	...
	Если РасчетБазыНДС Тогда

		СуммаБезНДСРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаНДС - СуммаВторогоСобытия, 
																		КурсыВалют.ВалютаРасчетов, КурсыВалют.ВалютаРегламентированногоУчета,
																		КурсыВалют.Курс, 1, 
																		КурсыВалют.Кратность, 1);
		Возврат УчетНДСКлиентСервер.РассчитатьСуммуНДСсУчетомПогрешности(СуммаБезНДСРегл,
											Истина,
											Ложь,
											СтавкаНДС);
...
Функция ДвиженияПоРегистрамНалоговогоУчетаПлатежныхДокументов(ДокументОбъект, СтруктураШапкиДокумента, РеестрПлатежей, Отказ=Ложь) Экспорт
	...
	Для каждого СтрокаПлатежа из РеестрПлатежей Цикл
		...
		Если СтруктураШапкиДокумента.ЕстьНДС Тогда                                                      
			...
			СуммаДляРасчетаПроводки = СтрокаПлатежа.СуммаНДС;
			СтруктураКурсаДокумента = Неопределено;
			
			Если ЭтоПродажа И СтруктураШапкиДокумента.ВалютаДокумента <> СтруктураШапкиДокумента.ВалютаРегламентированногоУчета Тогда
				
				СуммаДляРасчетаПроводки = СтрокаПлатежа.СуммаВзаиморасчетов - СтрокаПлатежа.СуммаНДС;
				СтруктураКурсаДокумента = Новый Структура("Курс, Кратность, ВалютаРасчетов, ВалютаРегламентированногоУчета", 
					СтрокаПлатежа.КурсВзаиморасчетов, СтрокаПлатежа.КратностьВзаиморасчетов, СтруктураШапкиДокумента.ВалютаДокумента, СтруктураШапкиДокумента.ВалютаРегламентированногоУчета);
					
			ИначеЕсли НЕ ЭтоПродажа И СтрокаПлатежа.ВидДеятельностиНДС = Перечисления.ВидыДеятельностиНДС.ПропорциональноОблагаемая Тогда
				СуммаДляРасчетаПроводки = СтрокаПлатежа.СуммаНДСПропорционально;
			КонецЕсли;                                                                               
			
			СуммаПроводки = ПолучитьСуммуПроводкиПоНДС(ТаблицаПоВторомуСобытиюНал, СуммаДляРасчетаПроводки, 
														...
														СтруктураКурсаДокумента, СтрокаПлатежа.СтавкаНДС);
			...
			Если  СуммаПроводки <> 0 
				...
				Если  СтрокаПлатежа.ВедениеВзаиморасчетовНУ = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам
					...
					РазбитьПроводкуПоНДСПоРасчетнымДокументам(ТаблицаПоПервомуСобытиюНДС, ДокументОбъект.Движения.Хозрасчетный, Проводка,
															  СтрокаПлатежа.ДоговорКонтрагента, СтрокаПлатежа.РасчетныйДокумент, 
															  СобытиеНДС,
