...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = НСтр("ru='Добавить общее сведение в набор (рекомендуется)
            ...
            |
            |Вибрана відомість вже входить в кілька наборів, тому рекомендується також включити її в набір ""як є"".
		...
		СделатьПоОбразцу = НСтр("ru='Сделать копию сведения по образцу (с общим списком значений)
            ...
            |
            |Список значень цієї відомості буде однаковий для обох наборів.
            |За допомогою цього варіанту зручно виконувати централізовану настройку списку значень відразу для декількох однотипних відомостей.
