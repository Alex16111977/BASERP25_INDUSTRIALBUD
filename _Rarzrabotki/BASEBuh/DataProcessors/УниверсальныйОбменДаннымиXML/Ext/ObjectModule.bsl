...
Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных(Свойства, ИмяТипа, 
	...
	ИначеЕсли ИмяТипа = "Документ" Тогда
		...
		Если ИмяТипа = "Документ" И Свойства.Имя <>"КорректировкаЗаписейРегистров" И Свойства.Имя <>"УстановкаЦенНоменклатуры" Тогда 
			Если ЗначениеЗаполнено(ИтоговоеОграничениеПоДате) Тогда       
			ИтоговоеОграничениеПоДате = ИтоговоеОграничениеПоДате + " И НЕ ОбъектДляВыгрузки.Организация.ПометкаУдаления"; //Тютюн  
				Иначе 
			ИтоговоеОграничениеПоДате = "ГДЕ НЕ ОбъектДляВыгрузки.Организация.ПометкаУдаления"; //Тютюн  
			КонецЕсли;
		КонецЕсли;
