...
Функция ПочтовыйИндексАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	Если НовоеЗначение = Неопределено Тогда
		// Чтение
		Результат = АдресНациональный.Получить( РаботаСАдресамиКлиентСервер.XPathПочтовогоИндекса() );
	...
	ЗаписьИндекса = АдресНациональный.Получить(РаботаСАдресамиКлиентСервер.XPathПочтовогоИндекса());
...
Функция РайонАдреса(XDTOАдрес, НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если ТипЗнч(АдресНациональный) = Тип("ОбъектXDTO") Тогда
			Возврат ПолучитьXDTOРеквизитОбъекта(АдресНациональный, РаботаСАдресамиКлиентСервер.XPathРайона());
		КонецЕсли;
...
Функция ЗначениеДополнительныхЭлементов(Знач XDTOАдрес) Экспорт
	...
	Результат.ПодчиненныйЭлемент = ДополнительныйЭлементАдреса(АдресНациональный, РаботаСАдресамиКлиентСервер.XPathДополнительногоОбъектаАдресации(91));
...
Функция ОшибкиЗаполненияАдресаXDTO(Знач Адрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	АдресНациональный = Неопределено;
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РегионСокращение", УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(Регион));
	КонецЕсли;
	...
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "ГородСокращение", УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(АдресНациональный.Город));
	КонецЕсли;
