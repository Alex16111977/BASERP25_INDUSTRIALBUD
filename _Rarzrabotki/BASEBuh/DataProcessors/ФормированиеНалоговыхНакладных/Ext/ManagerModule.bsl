...
Процедура ЗаполнитьПодвалНалоговогоДокумента(НалоговыйДокумент)
	...
	МенеджерДокумента = Документы[НалоговыйДокумент.Метаданные().Имя];
	МенеджерДокумента.УстановитьТипПричиныНевыдачиПокупателюПоУмолчанию(НалоговыйДокумент);
	СтруктураТЧ = Новый Структура("Товары, Услуги, ОС, НМА");
	Для каждого ТекТЧ Из СтруктураТЧ Цикл
		МенеджерДокумента.ЗаполнитьСчетаУчетаВТабЧасти(НалоговыйДокумент, ТекТЧ.Ключ);		
	КонецЦикла;		
	МенеджерДокумента.ЗаполнитьСуммыРегл(НалоговыйДокумент);
	МенеджерДокумента.ЗаполнитьВесТоваров(НалоговыйДокумент);
