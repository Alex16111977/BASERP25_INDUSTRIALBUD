...
Функция ПечатьТТН(ПараметрыПечати, ОбъектыПечати) Экспорт
	...
	ДатаНачалаИспользованияФормыТТН_2024 = РегистрыСведений.НастройкиЗаконодательныхИзменений.ЗначениеНастройки("ДатаНачалаИспользованияФормыТТН2024");	
	ДатаНачалаИспользованияФормыТТН_2026 = РегистрыСведений.НастройкиЗаконодательныхИзменений.ЗначениеНастройки("ДатаНачалаИспользованияФормыТТН2026");		
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаНачалаИспользованияФормыТТН_2026) И ДатаНачалаИспользованияФормыТТН_2026 <= Шапка.ДатаДокумента Тогда
		ЗаполнитьТТН2024 (Шапка, ЗапросТовары, ТабДокумент, ПараметрыПечати, "ПФ_MXL_UK_ТТН2026");
	ИначеЕсли ЗначениеЗаполнено(ДатаНачалаИспользованияФормыТТН_2024) И ДатаНачалаИспользованияФормыТТН_2024 <= Шапка.ДатаДокумента Тогда
		ЗаполнитьТТН2024 (Шапка, ЗапросТовары, ТабДокумент,ПараметрыПечати);
	Иначе
...
Процедура ЗаполнитьТТН2024 (Шапка, ЗапросТовары, ТабДокумент, ПараметрыПечати, ИмяМакета = "ПФ_MXL_UK_ТТН2024")
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьТТН." + ИмяМакета);
	...
	ОбластьМакета.Параметры.Грузоотправитель              = Шапка.Грузоотправитель;
	ОбластьМакета.Параметры.Грузополучатель               = Шапка.Грузополучатель;
	ОбластьМакета.Параметры.ГрузоотправительПредставление = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОГрузоотправитель, "ПолноеНаименование,КодПоЕДРПОУ,КодПоДРФО,", Ложь, КодЯзыкаПечать);
	ОбластьМакета.Параметры.ГрузополучательПредставление  = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОГрузополучателе,  "ПолноеНаименование,КодПоЕДРПОУ,КодПоДРФО,", Ложь, КодЯзыкаПечать);
	
	ОбластьМакета.Параметры.ФИОПраваВодителя = Сцепить(Водитель, ПараметрыПечати.ВодительскоеУдостоверение);
	ОбластьМакета.Параметры.ФИОДолжностьВодителя = Сцепить(Водитель, ПараметрыПечати.ДолжностьВодителя);

	ОбластьМакета.Параметры.Длина			         = ПараметрыПечати.Длина; 
	ОбластьМакета.Параметры.Ширина			         = ПараметрыПечати.Ширина; 
	ОбластьМакета.Параметры.Высота			         = ПараметрыПечати.Высота; 
	ОбластьМакета.Параметры.ОбщаяМасса			 	 = ПараметрыПечати.МассаОбщая; 		
	
	МассаГруза = ПараметрыПечати.МассаГруза;
	Масса = ?(Цел(МассаГруза) = МассаГруза, МассаГруза, Цел(МассаГруза) + 1);
	МассаПрописью = НРег(ЧислоПрописью(Масса,"Л=uk_UK", ",,,ж"));
	ОбластьМакета.Параметры.ИтогоМасса = ?(Масса = 0, "", Лев(МассаПрописью, СтрДлина(МассаПрописью)-3));
	
	ОбластьМакета.Параметры.МестоСоставления         = ПараметрыПечати.МестоСоставленияДокумента; 
	
	ОбластьМакета.Параметры.ОрганизацияПеревозчик = Перевозчик;
	
	ДокументыСопровождения = НСтр("ru='Расходная накладная';uk='Видаткова накладна'",КодЯзыкаПечать) + " № "
			+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер, Истина, Истина)
			+ НСтр("ru=' от ';uk=' від '",КодЯзыкаПечать) + Формат(Документ.Дата, "ДЛФ=Д");
	...
	ОбластьМакета.Параметры.ИтогоКоличество = ИтогоКоличество;
	ОбластьМакета.Параметры.ИтогоСуммаСНДС  = ИтогоСуммаСНДС;
	ОбластьМакета.Параметры.ИтогоМасса = МассаГруза;
...
Функция ПечатьТТН_ПеремещениеТоваров(ПараметрыПечати, ОбъектыПечати) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаНачалаИспользованияФормыТТН_2026) И ДатаНачалаИспользованияФормыТТН_2026 <= Шапка.ДатаДокумента Тогда
		ЗаполнитьТТН2024_ПеремещениеТоваров (Шапка, ЗапросТовары, ТабДокумент, ПараметрыПечати, "ПФ_MXL_UK_ТТН2026");
	ИначеЕсли ЗначениеЗаполнено(ДатаНачалаИспользованияФормыТТН_2024) И ДатаНачалаИспользованияФормыТТН_2024 <= Шапка.ДатаДокумента Тогда
		ЗаполнитьТТН2024_ПеремещениеТоваров (Шапка, ЗапросТовары, ТабДокумент, ПараметрыПечати);
	Иначе
...
Процедура ЗаполнитьТТН2024_ПеремещениеТоваров (Шапка, ЗапросТовары, ТабДокумент, ПараметрыПечати, ИмяМакета = "ПФ_MXL_UK_ТТН2024")	
	...
	ДокументыСопровождения = НСтр("ru='Накладная на перемещение';uk='Накладна на переміщення'",КодЯзыкаПечать) + " № "
			+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер, Истина, Истина)
			+ НСтр("ru=' от ';uk=' від '",КодЯзыкаПечать) + Формат(Документ.Дата, "ДЛФ=Д");
...
Функция Сцепить(Рекв1, Рекв2, Рекв3 = Неопределено, Рекв4 = Неопределено, Рекв5 = Неопределено)
	
	МассивСтрок = Новый Массив;
	Если ЗначениеЗаполнено(Рекв1) Тогда
		МассивСтрок.Добавить(СокрЛП(Рекв1));
	КонецЕсли;

	Если ЗначениеЗаполнено(Рекв2) Тогда
		МассивСтрок.Добавить(СокрЛП(Рекв2));
	КонецЕсли;

	Если ЗначениеЗаполнено(Рекв3) Тогда
		МассивСтрок.Добавить(СокрЛП(Рекв3));
	КонецЕсли;

	Если ЗначениеЗаполнено(Рекв4) Тогда
		МассивСтрок.Добавить(СокрЛП(Рекв4));
	КонецЕсли;

	Если ЗначениеЗаполнено(Рекв5) Тогда
		МассивСтрок.Добавить(СокрЛП(Рекв5));
	КонецЕсли;                  
