...
Процедура ЗаполнитьСправочник(ИмяОбъекта, ЗаполнятьПредопределенные = Истина, ЗаполнятьНеПредопределенные = Ложь, ЗаполнениеСинонимовОбъектов = Ложь)
	...
	Если НЕ ВозможноЗаполнениеОбъектаВСервисе(МетаданныеОбъекта.ПолноеИмя()) Тогда
		// нет возможности заполнить в разделенной конфигурации
		Возврат;		
	КонецЕсли;
...
// Проверяет, возможно ли выполнить заполнение, с учетом особеностей работы конфигурации в режиме сервиса
//
Функция ВозможноЗаполнениеОбъектаВСервисе(ПолноеИмя)
	
	ЕстьРаботаВМоделиСервиса = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса");
	Если ЕстьРаботаВМоделиСервиса Тогда
	...
	Если МодульРаботаВМоделиСервиса.РазделениеВключено() Тогда
		...
		Если ЕстьРаботаВМоделиСервиса Тогда
			ЭтоРазделенныеДанные = МодульРаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(
				ПолноеИмя, МодульРаботаВМоделиСервиса.РазделительОсновныхДанных());
		Иначе
			ЭтоРазделенныеДанные = Ложь;
		КонецЕсли;      
		
		Если МодульРаботаВМоделиСервиса.ДоступноИспользованиеРазделенныхДанных() Тогда
			
			Если ЕстьРаботаВМоделиСервиса Тогда
				ЭтоСовместноРазделенныеДанные = МодульРаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(
					ПолноеИмя, МодульРаботаВМоделиСервиса.РазделительВспомогательныхДанных());
			Иначе
				ЭтоСовместноРазделенныеДанные = Ложь;
			КонецЕсли;
			
			Если Не ЭтоРазделенныеДанные И Не ЭтоСовместноРазделенныеДанные Тогда
				// Разделение включено, доступно использование разделенных данных, но это не разделенные данные
				Возврат Ложь;
			КонецЕсли;
		...
		Иначе 
			...
			Если ЭтоРазделенныеДанные Тогда
				// Разделение включено, не доступно использование разделенных данных, но это разделенные данные
				Возврат Ложь;
			КонецЕсли;
...
КонецФункции // ВозможноЗаполнениеОбъектаВСервисе()
	...
	Для каждого ОписаниеРаздела Из СтруктураРазделовКОбработке Цикл
		...
		Для Каждого ОбъектМетаданных Из Метаданные[ОписаниеРаздела.Ключ] Цикл
			...
			Если НЕ МакетНачальноеЗаполнение.ТипМакета = Метаданные.СвойстваОбъектов.ТипМакета.ТекстовыйДокумент Тогда 
				Продолжить;
			КонецЕсли;               
			Если НЕ ВозможноЗаполнениеОбъектаВСервисе(ОбъектМетаданных.ПолноеИмя()) Тогда
				// нет возможности заполнить в разделенной конфигурации
				Продолжить;
			КонецЕсли;
