...
|Відновити рухи, сформовані автоматично?'");
...
Процедура Подключаемый_ПерейтиВСписокРегистра(Команда)
	...
	ДлинаИмениРегистра = СтрДлина(ИмяРегистра);
	...
	Для Каждого ЭлементФормыТабПоля ИЗ Элементы[ИмяРегистра].ПодчиненныеЭлементы Цикл
		Если ТипЗнч(ЭлементФормыТабПоля) = Тип("ПолеФормы") Тогда
			Если Лев(ЭлементФормыТабПоля.Имя, ДлинаИмениРегистра) = ИмяРегистра Тогда
				ИмяСвойства = Сред(ЭлементФормыТабПоля.Имя, ДлинаИмениРегистра + 1);
			Иначе
				ИмяСвойства = ЭлементФормыТабПоля.Имя;
			КонецЕсли;
