...
// Функция анализирует документ и регистрирует все используемые в нём справочники
// которые входят в состав плана обмена
//
// Параметры:
//  Узел           - ПланОбменаСсылка - узел обмена, для которого выполняется регистрация
//  СсылкаДокумента - ДокументСсылка - документ для анализа
//
// Возвращаемое значение:
//  Структура - с полями "Всего" и "Успешно"
//
Функция ЗарегистрироватьСправочникиИзДокумента(Узел, СсылкаДокумента) Экспорт
	
	Результат = Новый Структура("Всего, Успешно", 0, 0);
	
	// Проверяем, что передан документ
	МетаданныеОбъекта = СсылкаДокумента.Метаданные();
	Если Метаданные.Документы.Содержит(МетаданныеОбъекта) = Ложь Тогда
		Возврат Результат;
	КонецЕсли;
	...
	// Коллекция для хранения уже обработанных ссылок (чтобы не регистрировать дважды)
	ОбработанныеСсылки = Новый Соответствие;
	
	// Анализируем реквизиты документа
	Для Каждого Реквизит Из МетаданныеОбъекта.Реквизиты Цикл
		ЗначениеРеквизита = ОбъектДокумента[Реквизит.Имя];
		
		// Проверяем, является ли значение ссылкой на справочник
		Если ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			ОбработатьЗначениеРеквизита(ЗначениеРеквизита, Узел, СоставУзла, ОбработанныеСсылки, Результат);
		КонецЕсли;
	КонецЦикла;
	
	// Анализируем табличные части документа
	Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
		Для Каждого СтрокаТЧ Из ОбъектДокумента[ТабличнаяЧасть.Имя] Цикл
			// Анализируем реквизиты строки табличной части
...
// Вспомогательная процедура для обработки значения реквизита
//
Процедура ОбработатьЗначениеРеквизита(Значение, Узел, СоставУзла, ОбработанныеСсылки, Результат)
	
	// Проверяем, что значение является ссылочным типом
	Если Не ЭтоСсылочныйТип(Значение) Тогда
		Возврат;
	КонецЕсли;
	
	// Проверяем, что это справочник или документ
	...
	Если НЕ ЭтоСправочник И НЕ ЭтоДокумент Тогда
		Возврат;  // Игнорируем другие типы (планы видов характеристик, регистры и т.д.)
	КонецЕсли;

	// Проверяем, что мы ещё не обрабатывали эту ссылку
	Если ОбработанныеСсылки[Значение] <> Неопределено Тогда
		Возврат;
	КонецЕсли;

	// Отмечаем ссылку как обработанную
	ОбработанныеСсылки.Вставить(Значение, Истина);

	// Проверяем, входит ли объект в состав плана обмена
	ЭлементСостава = СоставУзла.Найти(МетаданныеСсылки);
	Если ЭлементСостава = Неопределено Тогда
		// Объект не входит в план обмена
		Возврат;
	КонецЕсли;

	// Регистрируем объект (справочник или документ)
	Результат.Всего = Результат.Всего + 1;
	
	Попытка
		ВыполнитьКомандуРегистрацииОбъекта(Истина, Узел, Значение);
		Результат.Успешно = Результат.Успешно + 1;
	Исключение
		// Игнорируем ошибки регистрации отдельных объектов
	КонецПопытки;
	
КонецПроцедуры

// Проверяет, является ли значение ссылочным типом
//
Функция ЭтоСсылочныйТип(Значение)
	
	ТипЗначения = ТипЗнч(Значение);
	
	// Проверяем основные ссылочные типы
	Попытка
