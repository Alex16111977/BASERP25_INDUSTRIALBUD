...
Процедура ЗарегистрироватьЗаПериод(Команда)
	...
	ТекстВопроса = "Буде виконано реєстрацію всіх документів та пов'язаних довідників за період з "
...
&НаСервере
Функция ЗарегистрироватьЗаПериодНаСервере(УзелОбмена, ДатаНачала, ДатаОкончания)
	...
	// Перебираем все типы документов из метаданных
	Для Каждого МетаданныеДокумента Из Метаданные.Документы Цикл

		// Проверяем, входит ли документ в состав плана обмена
		ЭлементСостава = СоставУзла.Найти(МетаданныеДокумента);
		Если ЭлементСостава = Неопределено Тогда
			Продолжить; // Документ не входит в план обмена
		КонецЕсли;

		// Формируем запрос для выборки документов за период
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ Ссылка ИЗ Документ." + МетаданныеДокумента.Имя + "
