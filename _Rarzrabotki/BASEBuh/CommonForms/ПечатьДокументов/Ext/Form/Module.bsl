...
Процедура СохранитьПечатнуюФормуВФайл()
	...
	Для Каждого ФайлДляЗаписи Из ФайлыВоВременномХранилище Цикл
		...
		ПолучаемыеФайлы.Добавить(Новый ОписаниеПередаваемогоФайла(?(СтрНайти(ИмяФайла, "." + Расширение) > 0, ИмяФайла, ИмяФайла + "." + Расширение), ФайлДляЗаписи.АдресВоВременномХранилище));
	КонецЦикла;
