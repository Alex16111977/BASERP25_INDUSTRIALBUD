...
Процедура ПечатьМонитораРуководителя(Команда)
	...
	ДатаОбновленияМонитораСтрокой = Формат(ДатаПоследнегоОбновленияМонитора, НСтр("ru='Л=ru; ДФ=''dd MMMM yyyy ""г."" H:mm''';uk= 'Л=uk; ДФ=''dd MMMM yyyy ""р."" H:mm'''"));	
...
Процедура ОбновитьЗаголовкиПериодов(Форма)
	...
	ЯзыкДляФормата = "Л=" + Локализация.КодЯзыкаИнтерфейса() + "; ";
	Форма.ПериодДата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='на %1';uk='на %1'"), Формат(Форма.ТекущаяДата, ЯзыкДляФормата + "ДФ='д ММММ'"));
	...
	Иначе
		ФорматНачалаПериода =  ЯзыкДляФормата + "ДФ='д ММММ'";
	КонецЕсли;

	Форма.ПериодМесяц = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='с %1 по %2';uk='з %1 по %2'"), Формат(НачалоПериода, ФорматНачалаПериода), Формат(КонецПериода, ЯзыкДляФормата + "ДФ='д ММММ'"));	
...
Функция ПодготовитьПарараметрыПечатиМонитораНаСервере()
	...
	ОбластьЗаголовок.Параметры.Сегодня = Формат(ДатаПоследнегоОбновленияМонитора, НСтр("ru='Л=ru; ДФ=''dd MMMM yyyy ""г."" H:mm''';uk= 'Л=uk; ДФ=''dd MMMM yyyy ""р."" H:mm'''"));
