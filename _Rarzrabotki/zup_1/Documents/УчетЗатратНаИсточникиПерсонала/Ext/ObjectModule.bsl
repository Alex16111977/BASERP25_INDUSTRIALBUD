...
Процедура ПроверитьЗаполнениеСтрокиЗатрат(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									НСтр("ru='"" табл. части ""Затраты"": ';uk='"" табл. частини ""Витрати"": '");
	
	// ИсточникПерсонала
    ЕстьИсточникПерсонала = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ИсточникПерсонала);
	Если Не ЕстьИсточникПерсонала Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбран источник!';uk='не вибрано джерело!'"), Отказ, Заголовок);
	КонецЕсли;
