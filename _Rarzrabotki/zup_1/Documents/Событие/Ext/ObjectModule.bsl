...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если Не Проведен Тогда
		Предупреждение(НСтр("ru='Перед печатью необходимо провести документ';uk='Перед друком необхідно провести документ'"));
	...
	Если      ИмяМакета = "Диаграмма" Тогда
		...
		Отчет.ВидОтчета = НСтр("ru='Планируемая занятость работников (не утвержденная)';uk='Планована зайнятість працівників (не затверджена)'");
...
Функция СформироватьЗапросПоШапке(Режим)
	...
	|	ВЫБОР
	|		КОГДА ПланируемаяЗанятостьПомещений.Занятость <> &Свободно
	|				ИЛИ ПланируемаяЗанятостьПомещений.Период >= ПланируемыеВстречи.НачалоСобытия
	|			ТОГДА ""Нельзя""
	|		ИНАЧЕ ""Можно""
...
Функция СформироватьЗапросПоСписокРаботников(Режим)
	...
	|	ВЫБОР
	|		КОГДА СобытийныйПланЗанятостиФизлицСрезПоследних.Состояние <> &Свободно
	|					И (НЕ СобытийныйПланЗанятостиФизлицСрезПоследних.Состояние ЕСТЬ NULL )
...
Процедура ПроверкаДублированиеКонтактныхЛиц(Отказ)
	...
	Если СторонниеЛица.Найти(КонтактноеЛицо) <> Неопределено Тогда
		Отказ = Истина;
		# Если Клиент Тогда
		Сообщить(НСтр("ru='В табл. части ""Кандидаты"" продублировано лицо, указанное также в шапке документа.';uk='В табл. частини ""Кандидати"" продубльована особа, зазначена також у шапці документа.'"));
		# КонецЕсли
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		Отказ = Истина;
		# Если Клиент Тогда
		Сообщить(НСтр("ru='Повторяющие записи в строке номер ""';uk='Записи, що повторюються, в рядку номер ""'")+Выборка.НомерСтроки+НСтр("ru='"" и в строке ""';uk='"" і в рядку ""'")+Выборка.КонфликтнаяСтрока+НСтр("ru=' табл. части ""Кандидаты""';uk=' табл. частини ""Кандидати""'"));
		# КонецЕсли
	КонецЦикла;
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	//Если  ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Ответственный) И ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ФизЛицо) И РаботникиОрганизации.Найти(ВыборкаПоШапкеДокумента.ФизЛицо, "Работник") <> Неопределено Тогда
	...
	Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Помещение) и ВыборкаПоШапкеДокумента.ПроверяемоеЗначение <> "Можно" Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Помещение на указанный период времени ранее уже зарезервировано!';uk='Приміщення на зазначений період часу раніше вже зарезервовано!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.НачалоСобытия = ВыборкаПоШапкеДокумента.ОкончаниеСобытия или НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.НачалоСобытия) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не задана продолжительность встречи!';uk='Не задана тривалість зустрічі!'"), Отказ, Заголовок);
	ИначеЕсли ВыборкаПоШапкеДокумента.НачалоСобытия > ВыборкаПоШапкеДокумента.ОкончаниеСобытия Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Начало встречи не может быть позже её окончания!';uk='Початок зустрічі не може бути пізніше її закінчення!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиСписокРаботников(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	НСтр("ru='"" табл. части ""Список работников"": ';uk='"" табл. частини ""Список працівників"": '");
	
	// Сотрудник
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда
		Сообщить(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбран сотрудник!';uk='не обраний співробітник!'"));
	КонецЕсли;
	
	// Занятость работника
	Если ВыборкаПоСтрокамДокумента.ПроверяемоеЗначение <> "Можно" Тогда
		Сообщить(СтрокаНачалаСообщенияОбОшибке + НСтр("ru=' сотрудник в указанный период времени уже занят!';uk=' співробітник в зазначений період часу вже зайнятий!'"));
	КонецЕсли;
...
Процедура ДобавитьСтрокуВДвиженияПоСписокРаботников(ВыборкаПоШапкеДокумента, ВыборкаПоСписокРаботников, 
	...
	Если СтруктураПроведенияПоРегистрамСведений.Свойство(ИмяРегистра) Тогда
		...
		// Реквизиты
		Движение.ДатаОкончания	= ВыборкаПоШапкеДокумента.ОкончаниеСобытия;
		
		
		Движение = Движения[ИмяРегистра].Добавить();
		
		// Свойства
		Движение.Период			= ВыборкаПоШапкеДокумента.ОкончаниеСобытия;
