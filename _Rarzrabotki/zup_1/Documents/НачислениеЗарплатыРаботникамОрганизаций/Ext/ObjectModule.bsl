...
Функция ЗаполнитьТаблицуРасчетногоЛистка(ТекущийСотрудник) Экспорт
	...
	|				ТОГДА """ + НСтр("ru='Замена: ';uk='Заміна: '",КодЯзыкаПечать) + """
	...
	|				ТОГДА """ + НСтр("ru='Совмещение: ';uk='Суміщення: '",КодЯзыкаПечать) + """
	|				ИНАЧЕ """"
	|		КОНЕЦ
	...
	|				ТОГДА """"
	|			ИНАЧЕ """ + НСтр("ru=' (по внутр. совмест-ву)';uk=' (по внутр. сумісн-ву)'",КодЯзыкаПечать) + """
	|		КОНЕЦ
	|		 + ВЫБОР
	...
	|			ТОГДА """ + НСтр("ru='ЕСВ';uk='ЄСВ'",КодЯзыкаПечать) + """ 
	...
	|		""" + НСтр("ru='Погашение займа';uk='Погашення позики'",КодЯзыкаПечать) + """,
	|		""В"",
	|		NULL,
	|		НАЧАЛОПЕРИОДА(ПогашениеЗаймовРаботникамиОрганизаций.Период, МЕСЯЦ),
	...
	|				ТОГДА """ + НСтр("ru='Через кассу ';uk='Через касу '",КодЯзыкаПечать) + """
	|			КОГДА Взаиморасчеты.СпособВыплаты = Значение(Перечисление.СпособыВыплатыЗарплаты.ЧерезБанк)
    |				ТОГДА """ + НСтр("ru='Перечислено в банк ';uk='Перераховано в банк '",КодЯзыкаПечать) + """
	|			ИНАЧЕ """ + НСтр("ru='Способ выплаты не указан ';uk='Спосіб виплати не вказаний '",КодЯзыкаПечать) + """
	|		КОНЕЦ + 
	...
	|				ТОГДА """ + НСтр("ru='(под расчет)';uk='(під розрахунок)'",КодЯзыкаПечать) + """
	...
	|				ТОГДА """ + НСтр("ru='(не указано)';uk='(не вказано)'",КодЯзыкаПечать) + """
	|			КОГДА Взаиморасчеты.ХарактерВыплаты = Значение(Перечисление.ХарактерВыплатыЗарплаты.ПустаяСсылка)
	|					И Взаиморасчеты.СуммаВзаиморасчетов < 0
	|				ТОГДА """ + НСтр("ru='(погашено задолженности)';uk='(погашено заборгованості)'",КодЯзыкаПечать) + """
	|				ИНАЧЕ """ + НСтр("ru='(межрасчет)';uk='(міжрозрахунок)'",КодЯзыкаПечать) + """
	|		КОНЕЦ),
	|		""Г"",
	|		NULL,
	...
	Если ДобавитьДанныеИзДокумента Тогда
		...
		ТекстПериода = Формат(ПериодРегистрации, "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+"; ДФ='ММММ гггг '"+НСтр("ru='г.';uk='р.'",КодЯзыкаПечать)+"''");
		...
		Для Каждого СтрокаВзносы Из СтрокиВзносы Цикл
			...
			Если ИспользуетсяЕСВ Тогда
				СтрокаТаблицы.ВидРасчета			= НСтр("ru='ЕСВ';uk='ЄСВ'",КодЯзыкаПечать);
		...
		Для Каждого СтрокаЗаймов Из СтрокиЗаймов Цикл
			...
			СтрокаТаблицы.ВидРасчета			= НСтр("ru='Погашение займа';uk='Погашення позики'",КодЯзыкаПечать);
...
Процедура АвтозаполнениеНачисления(Регистратор, Начисления, Сотрудники ) Экспорт
	...
	Если НЕ ПредварительныйРасчет Тогда
		...
		|			И	Движения.ДатаНачала <= ПлановыеНачисления.ДатаОкончания
...
Процедура РассчитатьПогашениеЗаймов(Сотрудники, КомментироватьРасчет = Ложь ) Экспорт 
	...
	Пока ВыборкаПоСсудам.Следующий() Цикл
		Если КомментироватьРасчет Тогда
				КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет ';uk='Розрахунок '"), , ВыборкаПоСсудам.ДоговорЗайма, ВыборкаПоСсудам.ДоговорЗайма);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Начислено процентов: ';uk='Нараховано відсотків: '") + ВыборкаПоСсудам.НачисленоПроцентов, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Погашено займа: ';uk='Погашено позики: '") + ВыборкаПоСсудам.ПогашеноЗайма, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Погашено процентов: ';uk='Погашено відсотків: '") + ВыборкаПоСсудам.ПогашеноПроцентов, КомментарийВидаРасчета);
		КонецЕсли;
...
Процедура НачисленияИБазовыеПериодыИндексации() 
	...
	ДанныеДляРасчета.Колонки.Добавить("Должность", Новый ОписаниеТипов("СправочникСсылка.ДолжностиОрганизаций"));
...
Процедура ПроверитьЗаполнениеСтрокиВзносы(ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	ИмяТЧ = ?(ВыборкаПоСтрокамДокумента.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Взносы,"Взносы",НСтр("ru='Взносы ФОТ';uk='Внески ФОП'"));
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									НСтр("ru='"" табл. части ""';uk='"" табл. частини ""'")+ИмяТЧ+""": ";
...
Процедура ПроверитьЗаполнениеСтрокиПогашениеЗайма(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
 									НСтр("ru='"" табл. части ""Погашение займов"": ';uk='"" табл. частини ""Погашення позик"": '");
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДоговорЗайма) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не задан договор займа погашение которого выполняется!';uk='не заданий договір позики погашення якої виконується!'"), Отказ, Заголовок);
	КонецЕсли;
	
	Если ВыборкаПоСтрокамДокумента.ДоговорЗайма.Сотрудник <> ВыборкаПоСтрокамДокумента.Сотрудник Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='сотрудник не совпадает с сотрудником указанным в договоре займа!';uk='співробітник не збігається з співробітником зазначеним у договорі позики!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура СформироватьВзаиморасчетыСРаботниками( Сотрудники = Неопределено )
	...
	Если Истина Тогда
		...
		|		"+?(ИспользуетсяЕСВ,"ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтражениеВзносовВРеглУчете.СрезПоследних(&парамПериодРегистрации, Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) И ПодразделениеОрганизации = ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка) ) КАК СпособыОтраженияВзносов
