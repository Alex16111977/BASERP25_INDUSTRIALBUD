...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если      ИмяМакета = "Диаграмма" Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Отчет.ВидОтчета = НСтр("ru='График отпусков';uk='Графік відпусток'");
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.Решение) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указано решение по документу!';uk='Не вказано рішення по документу!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиРаботника(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.ПроверяемоеЗначение = "Нельзя" Тогда
		СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru=' на указанный период документом ';uk=' на вказаний період документом '") + ВыборкаПоСтрокамДокумента.Регистратор + НСтр("ru=' уже был запланирован другой отпуск или мероприятие!';uk=' вже була запланована відпустка або інший захід!'");
