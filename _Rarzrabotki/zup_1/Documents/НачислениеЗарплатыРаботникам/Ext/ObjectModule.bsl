...
Функция СформироватьТаблицуНачислений(Начисления)
	...
	Пока Начисления.Следующий() Цикл
		...
		Если Начисления.КурсВалюты1 = NULL Тогда
			Сообщить(НСтр("ru='Для расчетного периода не задан курс валюты ';uk='Для розрахункового періоду не заданий курс валюти '") + Начисления.Валюта1 + "!", СтатусСообщения.Важное);
		КонецЕсли;
		Если Начисления.КурсВалюты2 = NULL Тогда
			Сообщить(НСтр("ru='Для расчетного периода не задан курс валюты ';uk='Для розрахункового періоду не заданий курс валюти '") + Начисления.Валюта2 + "!", СтатусСообщения.Важное);
		КонецЕсли;
		Если Начисления.КурсВалюты3 = NULL Тогда
			Сообщить(НСтр("ru='Для расчетного периода не задан курс валюты ';uk='Для розрахункового періоду не заданий курс валюти '") + Начисления.Валюта3 + "!", СтатусСообщения.Важное);
		КонецЕсли;		
		Если Начисления.КурсВалюты4 = NULL Тогда
			Сообщить(НСтр("ru='Для расчетного периода не задан курс валюты ';uk='Для розрахункового періоду не заданий курс валюти '") + Начисления.Валюта4 + "!", СтатусСообщения.Важное);
		КонецЕсли;		
...
Процедура РассчитатьПогашениеЗаймов() 
	...
	Иначе	
		...
		Если Структура.Курс = 0 или Структура.Кратность = 0 Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан курс валюты управленческого учета.';uk='Не зазначений курс валюти управлінського обліку.'"));
			Возврат;
		КонецЕсли;
...
Функция Автозаполнение(ПостроительЗапроса = НеОпределено) Экспорт
	...
	Если КурсВалютыУпрУчета = 0 Тогда
		Сообщить(НСтр("ru='Для расчетного периода не задан курс валюты управленческого учета (';uk='Для розрахункового періоду не заданий курс валюти управлінського обліку ('") + ВалютаУпрУчета + НСтр("ru=') для расчетов с персоналом!';uk=') для розрахунків з персоналом!'"), СтатусСообщения.Важное);
		Возврат Ложь;
	КонецЕсли;
			...
			Если УчетнаяПолитикаПоПерсоналу.РасчетЗарплатыПоОтветственным Тогда
				Если Подразделение.Пустая() Тогда
					...
					Если Запрос.Выполнить().Пустой() Тогда
						Сообщить(НСтр("ru='Ответственный за этот документ не является расчетчиком ни одного из подразделений компании';uk='Відповідальний за цей документ не є звільненим з жодного із підрозділів компанії'"));
						Возврат  Ложь;
					КонецЕсли;
				Иначе
					Запрос.Текст = 
...
Функция ЗаполнитьТаблицуРасчетногоЛистка(ФизЛицо, Сотрудник) Экспорт
	...
	Пока ВыборкаПоФЛ.Следующий() Цикл
		...
		ОбластьШапка.Параметры.Работник 	 = НСтр("ru='Работник: ';uk='Працівник: '",КодЯзыкаПечать) + ВыборкаПоФЛ.ФизЛицо;
		...
		Если СуммаНаНачалоПериода = NULL Тогда
			...
			ОбластьПодвал.Параметры.ТекстДолгНаНачалоПериода = НСтр("ru='Долг за предприятием на начало месяца: ';uk='Борг за підприємством на початок місяця: '",КодЯзыкаПечать);
		...
		Иначе
			...
			Если СуммаНаКонецПериода >= 0  Тогда
				ОбластьПодвал.Параметры.ТекстДолгНаКонецПериода = НСтр("ru='Долг за предприятием на конец месяца: ';uk='Борг за підприємством на кінець місяця: '",КодЯзыкаПечать);
		...
		Если Модифицированность  Тогда
			...
			Для Каждого СтрокаЗайма Из СтрокиЗаймы Цикл
				НомерСтроки = ВысотаТаблицы + КоличествоУдержаний;
				ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаУдержано).Текст = НСтр("ru='Погашение займа';uk='Погашення позики'");
				ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаУдержано).РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
				ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаУдержано).ГоризонтальноеПоложение = ГоризонтальноеПоложение.Лево;
				ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаПериодУдержано - 1).Объединить();
	...
	Если НеНайденыЗаписиВЗапросе и Модифицированность Тогда
		...
		ОбластьШапка.Параметры.Работник 	 = НСтр("ru='Работник: ';uk='Працівник: '",КодЯзыкаПечать) + ФизЛицо;
		...
		Для Каждого СтрокаЗайма Из СтрокиЗаймы Цикл
			НомерСтроки = ВысотаТаблицы + КоличествоУдержаний;
			ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаУдержано).Текст = НСтр("ru='Погашение займа';uk='Погашення позики'",КодЯзыкаПечать);
			ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаУдержано).РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
			ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаУдержано).ГоризонтальноеПоложение = ГоризонтальноеПоложение.Лево;
			ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаУдержано , НомерСтроки, КолонкаПериодУдержано - 1).Объединить();
			ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаПериодУдержано, НомерСтроки, КолонкаПериодУдержано).Текст = ОбщегоНазначения.ОписаниеПериода(НачалоМесяца(ПериодРегистрации),КонецМесяца(ПериодРегистрации));
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПериодРегистрации) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не задан период начисления зарплаты!';uk='Не заданий період нарахування зарплати!'"), Отказ, Заголовок);
	КонецЕсли;
	
	Если ПоВременнойСхемеМотивации и ВидСхемыМотивации.Пустая() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='По временному расчету должен указываться вид схемы мотивации!';uk='За тимчасовим розрахунком має зазначатися вид схеми мотивації!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиНачисления(ВыборкаПоСтрокамДокумента, Отказ, Заголовок = "")
	...
	Если ВыборкаПоСтрокамДокумента.ПериодДействияНачало > ВыборкаПоСтрокамДокумента.ПериодДействияКонец Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='дата начала начисления не может быть больше даты окончания!';uk='дата початку нарахування не може бути більше дати закінчення!'"), Отказ, Заголовок);	
	КонецЕсли;
