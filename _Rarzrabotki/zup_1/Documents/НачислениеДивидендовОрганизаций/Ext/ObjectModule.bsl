...
Процедура РассчитатьНДФЛ() Экспорт
	...
	ВидДохода = ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета.ДоходНДФЛ,Справочники.ВидыДоходовНДФЛ.Код12);
	...
	Запрос.УстановитьПараметр("СтавкаРезидента", ПроведениеРасчетов.ПолучитьСтавкуНДФЛ(КонецМесяца(ПериодРегистрации), ВидДохода.ВидСтавкиРезидента));
	Запрос.УстановитьПараметр("СтавкаНеРезидента", ПроведениеРасчетов.ПолучитьСтавкуНДФЛ(КонецМесяца(ПериодРегистрации), ВидДохода.ВидСтавкиРезидента));
	...
	|	НачислениеДивидендовОрганизацииНачисления.ВоенныйСбор,
	|	ВЫБОР
    ...
    СтавкаВС 	= ПроведениеРасчетов.ПолучитьСтавкуНДФЛ(КонецМесяца(ПериодРегистрации), Перечисления.ВидыСтавокНДФЛ.ВоенныйСбор);
	
	Для каждого СтрокаТЧ Из ВременнаяТаблица Цикл
		СтрокаТЧ.НДФЛ = Макс(СтрокаТЧ.Результат,0) * СтрокаТЧ.СтавкаНДФЛ;
		СтрокаТЧ.ВоенныйСбор = Макс(СтрокаТЧ.Результат,0) * СтавкаВС;
	КонецЦикла;
...
Функция СформироватьЗапросПоНачисления()
	...
	Запрос.УстановитьПараметр("ВидРасчета", ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета,ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДивидендыРаботников));
	Запрос.УстановитьПараметр("СчетУчета", ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета.СчетУчета,ПланыСчетов.Хозрасчетный.РасчетыПоНачисленнымДивидендам));
	Запрос.УстановитьПараметр("ВидДохода", ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета.ДоходНДФЛ,Справочники.ВидыДоходовНДФЛ.Код12));
	Запрос.УстановитьПараметр("ВидСтавки", ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета.ДоходНДФЛ.ВидСтавкиРезидента,Перечисления.ВидыСтавокНДФЛ.Основная));
	...
	Иначе
		Запрос.УстановитьПараметр("ВидДоходаВС", ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета.ДоходНДФЛ.ОблагаетсяВоеннымСбором2021,Справочники.ВидыДоходовНДФЛ.ВоенныйСбор109));
	КонецЕсли;	
	...
	|
	|	&ВидДохода КАК ДоходНДФЛ,
	|	Основной.Результат КАК Доход,
	|	Основной.НДФЛ КАК Налог,
	|	Работники.ПодразделениеОрганизации,
	|	&ВидСтавки КАК ВидСтавки,
	|
	|   &ВидДоходаВС КАК ВидДоходаВС,
	|   &ВидСтавкиВС КАК ВидСтавкиВС,
	|	Основной.ВоенныйСбор КАК ВоенныйСбор
	|
	|
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан акционер!';uk='не вказаний акціонер!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДобавитьСтрокуВзаиморасчетыСРаботникамиОрганизаций(ВыборкаПоСтрокамДокумента)
	...
	ЗаполнитьЗначенияСвойств(Движение, ВыборкаПоСтрокамДокумента);
	Движение.ВидДвижения			= ВидДвиженияНакопления.Расход;
	Движение.СуммаВзаиморасчетов	= ВыборкаПоСтрокамДокумента.ВоенныйСбор;
	Движение.КодОперации			= Перечисления.КодыОперацийВзаиморасчетыСРаботникамиОрганизаций.НДФЛ;
...
Процедура ДобавитьСтрокуВзаиморасчетыПоНДФЛ(ВыборкаПоСтрокамДокумента)
	...
	Движение = Движения.ВзаиморасчетыПоНДФЛ.Добавить();
	ЗаполнитьЗначенияСвойств(Движение, ВыборкаПоСтрокамДокумента);
	Движение.ДоходНДФЛ = ВыборкаПоСтрокамДокумента.ВидДоходаВС;
	Движение.Налог = ВыборкаПоСтрокамДокумента.ВоенныйСбор;
	Движение.ВидСтавки = ВыборкаПоСтрокамДокумента.ВидставкиВС;
	Движение.ВидДвижения			= ВидДвиженияНакопления.Приход;
