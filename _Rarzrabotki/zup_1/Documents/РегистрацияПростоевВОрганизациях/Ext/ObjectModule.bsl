...
Функция ПечатьСредняя() Экспорт
	...
	Если ДатаНачала >= ПроведениеРасчетов.ДатаИзмененияПорядка100_486() Тогда
		...
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	СУММА(РегистрацияПростоевВОрганизациях.Результат) КАК ПревышениеПремии,
		|	РегистрацияПростоевВОрганизациях.Ссылка
		|ИЗ
		|	Документ.РегистрацияПростоевВОрганизациях.РасчетСреднего КАК РегистрацияПростоевВОрганизациях
		|ГДЕ
		|	РегистрацияПростоевВОрганизациях.Ссылка.Ссылка = &парамСсылка
		|	И РегистрацияПростоевВОрганизациях.ПревышениеПремии = ИСТИНА
		|
		|СГРУППИРОВАТЬ ПО
		|	РегистрацияПростоевВОрганизациях.Ссылка";
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Регистрация простоев ';uk='Реєстрація простоїв '")));
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	Возврат Новый Структура("Средняя", НСтр("ru='Расчет средней';uk='Розрахунок середньої'"));
...
Функция ПолучитьРеквизитыНачисления() Экспорт
	...
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПлановыеНачисленияРаботниковОрганизаций.СрезПоследних(&ДатаАктуальности, Сотрудник = &парамСотрудник И ВидРасчетаИзмерение <> ВидРасчета ) КАК ОсновноеНачисление
	|ПО		ИСТИНА
	|
	...
	ИначеЕсли ВидПростоя = Перечисления.ВидыПростояРаботника.ПриостановкаТрудовогоДоговора Тогда
		
		ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПростойСПриостановкойТрудДог		
...
Функция АвтозаполнениеРасчетСреднего() Экспорт
	...
	Если ДатаНачала < ПроведениеРасчетов.ДатаИзмененияПорядка100() Тогда
		...
		Если Истина Тогда
			...
			Иначе
				ТекстЗапросаВТ = " 
				|	ВЫБРАТЬ
				|		&парамНачало	КАК ПериодНачало,
				|		1				КАК Коэффициент,
				|		&парамКонец 	КАК ПериодКонец
				|ПОМЕСТИТЬ ВТКоэффициент	
				|";
	...
	Иначе
		...
		Если  НачалоДня(ДобавитьМесяц(ПериодРасчетаСреднегоНачало,1)-1) > ПериодРасчетаСреднегоКонец Тогда
			...
			Если ((ОтработаноЗа2Мес = 0 И ОтработаноЗа4Мес = 0) ИЛИ НачалоМесяца(ДатаНачала) = НачалоМесяца(ДатаПриема))
				ИЛИ (ОтработаноЗа2Мес = 0 И ОтработаноЗа4Мес = 0 И НачалоМесяца(ДобавитьМесяц(ДатаНачала,-1)) = НачалоМесяца(ДатаПриема)) Тогда
				// проработал меньше целого месяца
				// если есть за что платить, пользователю прийдется заполнить эти суммы вручную
				//
...
Функция РассчитатьРасчетСреднего(КомментироватьРасчет = Ложь) Экспорт
	...
	Иначе
		...
		Если  РасчетСреднего.Количество() > 0 И РасчетСреднего[0].ВидРасчета = ПланыВидовРасчета.СреднийЗаработок.ПоОкладу Тогда
			...
			ПараметрыОклада = ПроведениеРасчетов.ПолучитьПараметыОклада1213(ДатаНачала, Сотрудник);
			РасчетСреднего[0].Результат			= ПараметрыОклада["Оклад"];
			РасчетСреднего[0].ОтработаноДней	= ПараметрыОклада["НормаДней"];
			РасчетСреднего[0].ОтработаноЧасов	= ПараметрыОклада["НормаЧасов"];
			РасчетСреднего[0].КалендарныеДни	= ПараметрыОклада["КалендарныеДни"];
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ВидПростоя) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан вид простоя!';uk='Не зазначений вид простою!'"), Отказ);
	КонецЕсли;
