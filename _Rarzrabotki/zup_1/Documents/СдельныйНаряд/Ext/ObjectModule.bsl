...
Функция СформироватьЗапросПоИсполнителиРегл(ВыборкаПоШапкеДокумента)
	...
	|ПОМЕСТИТЬ ВТСотрудникиСоСдельнымЗаработком
	|   ИЗ
	|	(		ВЫБРАТЬ
	|			   Плановые.Сотрудник,
	|			   Плановые.ВидРасчета,
	...
	|				И ВидРасчета.СпособРасчета В (ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.СдельныйЗаработок),ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоДоговоруФиксированнойСуммой))
	|				И Сотрудник В (ВЫБРАТЬ
	|								Сотрудник
	|								ИЗ
	|								ВТСотрудникиДокумента)) КАК Плановые
	|				ГДЕ Плановые.Актуальность
	|
	|			ОБЪЕДИНИТЬ ВСЕ
	|
	|			ВЫБРАТЬ
	...
	|								ВТСотрудникиДокумента)) КАК Плановые
	|			ГДЕ 
	|			   Плановые.ВидРасчета.СпособРасчета В (ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.СдельныйЗаработок),ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоДоговоруФиксированнойСуммой))
	...
	|			ГДЕ 
	|			   НАЧАЛОПЕРИОДА(Плановые.Период, МЕСЯЦ) = &ДатаАктуальности
	|			   И Сотрудник В (ВЫБРАТЬ
	|								Сотрудник
	|								ИЗ
	|								ВТСотрудникиДокумента)
	|			   И Плановые.ВидРасчета <> Плановые.ВидРасчетаИзмерение
	|			   И Плановые.ВидРасчета.СпособРасчета В (ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.СдельныйЗаработок),ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоДоговоруФиксированнойСуммой))
	|			   И Плановые.Актуальность
	|     ) КАК Плановые
	|					
	|;				
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СдельныйНарядИсполнители.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР КОГДА СдельныйНарядИсполнители.Сотрудник.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство) 
	|         ТОГДА СдельныйНарядИсполнители.Сотрудник.ОсновноеНазначение
	|         ИНАЧЕ СдельныйНарядИсполнители.Сотрудник
	|	КОНЕЦ КАК Сотрудник,
	...
	|				Сотрудник
	|			ИЗ
	|				ВТСотрудникиДокумента)) КАК Работники
	|		ПО СдельныйНарядИсполнители.Сотрудник = Работники.Сотрудник
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиСоСдельнымЗаработком КАК ПлановыеНачисления
	...
	|								Сотрудник
	|							ИЗ
	|								ВТСотрудникиДокумента)) КАК РаботникиОрганизаций
	|			ГДЕ
...
Функция СформироватьЗапросПоВыработкаРегл(Режим)
	...
	ТекстЗапроса = "ВЫБРАТЬ
				   ...
				   |	СдельныйНарядВыработка.НалоговоеНазначение,
				   |	СдельныйНарядВыработка.НалоговоеНазначениеНДС,
	               |	СдельныйНарядВыработка.СтатьяВаловыхРасходов
	               |ИЗ
	               |	Документ.СдельныйНаряд.Выработка КАК СдельныйНарядВыработка
...
Процедура ПроверитьЗаполнениеСтрокиИсполнители(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, ВыборкаПоРаботникиРегл, Отказ, Заголовок, УчетнаяПолитикаПоПерсоналуОрганизации)
	...
	НСтр("ru='"" табл. части ""Исполнители"": ';uk='"" табл. частини ""Виконавці"": '");
	...
	Иначе
		Если ОтражатьВУправленческомУчете И Не ВыборкаПоСтрокамДокумента.СотрудникУпрУчета Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='сотрудник не принят по управленческому учету!';uk='співробітник не прийнятий по управлінському обліку!'"), Отказ, Заголовок);
		КонецЕсли; 
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням 
		...
		Если СуммаВремени = 0 Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указано ни одно значение отработаных часов для заполняемых дней!';uk='не вказано жодне значення відпрацьованих годин для днів, що заповнюються!'"), Отказ, Заголовок);
		КонецЕсли; 
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ЗаДень 
		...
		Если ВыборкаПоСтрокамДокумента["Часов" + Строка(День(ДатаНачалаПериода))] = 0 Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указано значение отработаных часов для вводимого дня!';uk='не вказано значення відпрацьованих годин для дня, що вводиться!'"), Отказ, Заголовок);
		КонецЕсли; 
	...
	Если ВыборкаПоШапкеДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если НЕ ВыборкаПоСтрокамДокумента.СотрудникЧислитсяВОрганизации Тогда
			Если ВыборкаПоСтрокамДокумента.Сотрудник.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.Подряда Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='с указанным сотрудником не заключен договор подряда в данном периоде!';uk='з зазначеним співробітником не укладений договір підряду в даному періоді!'"), Отказ, Заголовок);
			Иначе	
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='указанный сотрудник не принят в организацию в данном периоде!';uk='зазначений співробітник не прийнятий в організацію в даному періоді!'"), Отказ, Заголовок);
		    КонецЕсли;
		КонецЕсли;
		
		// Проверка: если работиник работает по договору ГПХ, то должен быть указан договор
		// Иначе Проверка: вид расчета у работника должен быть сдельный наряд
		Если ВыборкаПоСтрокамДокумента.Сотрудник.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.Подряда Тогда
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДокументОснование) Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='у сотрудника не указан договор ГПХ (в поле ""Документ основание"")!';uk='у співробітника не вказаний договір ЦПХ (у полі ""Документ-підстава"")!'"), Отказ, Заголовок);
			КонецЕсли;
		ИначеЕсли НЕ ВыборкаПоСтрокамДокумента.ПроверкаВидРасчета Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='у сотрудника не назначено начисление со способом расчета ""Сдельный заработок""!';uk='у співробітника не призначено нарахування зі способом розрахунку ""Відрядний заробіток""!'"), Отказ, Заголовок);
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиВыработка(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
	НСтр("ru='"" табл. части ""Выработка"": ';uk='"" табл. частини ""Виробіток"": '");
	
	// ТехнологическаяОперация
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ТехнологическаяОперация) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не выбрана технологическая операция!';uk='не обрана технологічна операція!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// СтатьяЗатрат
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.СтатьяЗатрат) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана статья затрат!';uk='не зазначена стаття витрат!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиВыработкаРегл(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоШапкеДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.СчетЗатрат) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан счет бухгалтерского учета!';uk='не зазначений рахунок бухгалтерського обліку!'"), Отказ, Заголовок);
		КонецЕсли;
			...
			Если ВыборкаПоРаботники.СуммаКНачислению <> 0 Тогда
				...
				Если ВыборкаПоШапкеДокумента.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням 
					...
					Для ИндексДня = 1 По КоличествоДнейВПериодеРегистрации Цикл
						Если МассивРаспределенияНачисленнойСуммыПоДням[ИндексДня-1] <> 0 Тогда
							НоваяСтрока = НаборДвижений.Добавить();
							НоваяСтрока.Сотрудник			= ВыборкаПоРаботники.Назначение;
...
Процедура ДвиженияРегистровБУиНУРегл(СтруктураШапкиДокумента, ВыборкаПоРаботники, ТаблицаРабот)
	...
	Сч = 0;
	ПроводкаСубконтоДт = Новый Соответствие;
	ПроводкаСубконтоДт.Вставить("Подразделения");
	ПроводкаСубконтоДт.Вставить("НоменклатурныеГруппы");
	ПроводкаСубконтоДт.Вставить("СтатьиЗатрат");
	ПроводкаСубконтоДт.Вставить("ОбъектыСтроительства");
		...
		Если ВыборкаПоРаботники.СуммаКНачислению <> 0 Тогда
			...
			Для каждого СтрокаТЗ Из ТаблицаРабот Цикл
				...
				Если Результат <> 0 Тогда
					...
					ПроводкаСчетДт = СтрокаТЗ.СчетЗатрат;
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТЗ.СчетЗатрат, ПроводкаСубконтоДт, "Подразделения",        ПодразделениеОрганизации);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТЗ.СчетЗатрат, ПроводкаСубконтоДт, "НоменклатурныеГруппы", СтрокаТЗ.НоменклатурнаяГруппа);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТЗ.СчетЗатрат, ПроводкаСубконтоДт, "СтатьиЗатрат",         СтрокаТЗ.СтатьяЗатрат);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТЗ.СчетЗатрат, ПроводкаСубконтоДт, "ОбъектыСтроительства", СтрокаТЗ.ОбъектСтроительства);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТЗ.СчетЗатрат, ПроводкаСубконтоДт, "НалоговыеНазначения",  СтрокаТЗ.НалоговоеНазначение);
					
					Если ВыборкаПоРаботники.СчетКт = NULL или НЕ ЗначениеЗаполнено(ВыборкаПоРаботники.СчетКт) Тогда
						
						ПроводкаСчетКт = ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета.СчетУчета,ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.СдельнаяОплата.СчетУчета);
						Если НЕ (ПроводкаСчетКТ.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.РаботникиОрганизации,"ВидСубконто") = Неопределено) Тогда
							ПроводкаСубконтоКт1 = ВыборкаПоРаботники.ФизЛицо;
						Иначе
							ПроводкаСубконтоКт1 = "";
						КонецЕсли;	
						ПроводкаСубконтоКт2 = "";
					...
					Иначе
						...
						ПроводкаСчетКт = ВыборкаПоРаботники.СчетКт;
						ПроводкаСубконтоКт1 = ВыборкаПоРаботники.СубконтоКт1;
						ПроводкаСубконтоКт2 = ВыборкаПоРаботники.СубконтоКт2;
						ПроводкаСубконтоКт3 = ВыборкаПоРаботники.СубконтоКт3;
					...
					Если НЕ ИспользуетсяЕСВ Тогда
						СтрокаДанных.ВидРасчетаБазы	= ?(ЗначениеЗаполнено(ВидРасчета),ВидРасчета,ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.СдельнаяОплата);
					Иначе
					...
					// измерения
					СтрокаДанных.Сотрудник	= ?(ЗначениеЗаполнено(ВыборкаПоРаботники.Сотрудник.ОсновноеНазначение), ВыборкаПоРаботники.Сотрудник.ОсновноеНазначение, ВыборкаПоРаботники.Сотрудник);
					СтрокаДанных.Назначение	= ВыборкаПоРаботники.Сотрудник;
					
					// ресурсы
					СтрокаДанных.Результат			= Результат;
					
					// реквизиты
					СтрокаДанных.НалоговоеНазначение		= СтрокаТЗ.НалоговоеНазначение;
					СтрокаДанных.НалоговоеНазначениеНДС		= СтрокаТЗ.НалоговоеНазначениеНДС;
					СтрокаДанных.СтатьяВаловыхРасходов			= СтрокаТЗ.СтатьяВаловыхРасходов;
					
					СтрокаДанных.СчетДт			= ПроводкаСчетДт;
					Инд = 1;
					Для каждого ВидСубконто Из ПроводкаСчетДт.ВидыСубконто Цикл
						
						СтрокаДанных["СубконтоДт"+ Инд] = ПроводкаСубконтоДт[ВидСубконто.ВидСубконто];
						Инд = Инд + 1;
						
					КонецЦикла; 
					
					СтрокаДанных.СчетКт      = ПроводкаСчетКт;
					СтрокаДанных["СубконтоКт1"] = ПроводкаСубконтоКт1;
					СтрокаДанных["СубконтоКт2"] = ПроводкаСубконтоКт2;
					СтрокаДанных["СубконтоКт3"] = ПроводкаСубконтоКт3;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Отказ Тогда
		Сообщить(НСтр("ru='Документ был проведен в редакции 1.1 и перепроведению не подлежит!';uk='Документ був проведений у редакції 1.1 і перепроведенню не підлягає!'"));
		Возврат
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	КонецЕсли;
	ЗаполнениеДокументов.ПроверитьЗаполнениеШапкиДокумента(ЭтотОбъект, Новый Структура(РеквизитыШапки,НСтр("ru='Не указан способ учета времени и выработки по наряду!';uk='Не вказаний спосіб обліку часу і виробітку за нарядом!'")), Отказ, Заголовок);
	
	// проверка совпадения сумм по т.ч. 
	Если ОтражатьВУправленческомУчете Тогда
		Если Выработка.Итог("Сумма") <> Исполнители.Итог("СуммаКНачислению") Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не совпадают итоговые суммы к начислению по управленческому учету по табличным частям!';uk='Не збігаються підсумкові суми до нарахування з управлінського обліку за табличними частинами!'"), Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	Если ОтражатьВБухгалтерскомУчете Тогда
		Если Выработка.Итог("СуммаРегл") <> Исполнители.Итог("СуммаКНачислениюРегл") Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не совпадают итоговые суммы к начислению по регламентированному учету по табличным частям!';uk='Не збігаються підсумкові суми до нарахування по регламентованому обліку по табличним частинам!'"), Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	...
	Если УчитыватьОтработанноеВремя Тогда
		...
		Иначе
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='На следующих работников данные об отработанном времени уже введены: ';uk='На наступних працівників дані про відпрацьований час вже введені: '") + СтрокаСотрудников, Истина);
			Отказ = Истина;
