...
Процедура Автозаполнение(ПостроительЗапроса,Реквизиты) Экспорт
	...
	ПостроительЗапроса.Параметры.Вставить("РегламентВалюта",		Константы.ВалютаРегламентированногоУчета.Получить());
...
Функция ЗаполнитьРаботавшимиВПраздники() Экспорт
	...
	|		И ПлановыеНачисленияРаботниковОрганизаций.ВидРасчетаИзмерение <> ПлановыеНачисленияРаботниковОрганизаций.ВидРасчета
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|	И ЕСТЬNULL(СостояниеСотрудниковПоПраздникам.Состояние, &Работает)= &Работает";
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ПериодРегистрации) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан период регистрации (месяц)!';uk='Не зазначений період реєстрації (місяць)!'"), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиНачислений(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НетДатаВыхода Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата выхода на работу!';uk='не вказана дата виходу на роботу!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// ВидРасчета
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидРасчета) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан вид оплаты!';uk='не зазначений вид оплати!'"), Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоСтрокамДокумента.ЗанимаемыхСтавок = NULL Тогда
		СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru='на ';uk='на '") + Формат(ВыборкаПоСтрокамДокумента.ДатаВыхода, "ДЛФ=DD") + НСтр("ru=' работник ';uk=' працівник '") + ВыборкаПоСтрокамДокумента.СотрудникНаименование + НСтр("ru=' еще не принят на работу!';uk=' ще не прийнятий на роботу!'");
	...
	ИначеЕсли ВыборкаПоСтрокамДокумента.ЗанимаемыхСтавок = 0 Тогда
		СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru='на ';uk='на '") + Формат(ВыборкаПоСтрокамДокумента.ДатаВыхода, "ДЛФ=DD") + НСтр("ru=' работник ';uk=' працівник '") + ВыборкаПоСтрокамДокумента.СотрудникНаименование + НСтр("ru=' уже уволен (с ';uk=' вже звільнений (з '") + Формат(ВыборкаПоСтрокамДокумента.ДатаПоследнегоДвиженияПоРаботнику, "ДЛФ=DD") + ")!";
		Сообщить(СтрокаНачалаСообщенияОбОшибке + СтрокаПродолжениеСообщенияОбОшибке);
	...
	Если ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер <> NULL Тогда
		СтрокаСообщениеОбОшибке = НСтр("ru='указана повторяющаяся строка (см. строку  № ';uk='вказанийрядок, що повторюється (див. рядок № '") + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер + ")!"; 
