...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ)
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Коэффициент) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан коэффициент!';uk='не вказаний коефіцієнт!'"), Отказ);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемНаРаботуВОрганизацию")
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Размер прошлого оклада: ';uk='Розмір минулого окладу: '") + Выборка.СтарыйОклад + НСтр("ru='; размер нового оклада: ';uk='; розмір нового окладу: '") + Выборка.НовыйОклад + ".",,,СтатусСообщения.Информация);
