...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураПечатныхФорм = Новый Структура("ОтражениеЗарплатыВРегламентированномУчете",НСтр("ru='Отражение зарплаты';uk='Відображення зарплати'"));
...
Функция КорректноеСочетаниеСчетов(Выборка,Сообщение)
	...
	Если (СчетДт = ПланыСчетов.Хозрасчетный.РасчетыПоОплатеТруда 
		...
		Сообщение = НСтр("ru='Проводки в дебет ';uk='Проводки в дебет '")+Выборка.НаименованиеСчетДт +НСтр("ru=' не должны отражаться в налоговом учете!';uk=' не повинні відображатися у податковому обліку!'");
	КонецЕсли;
...
Процедура ДобавитьРезервОтпусков(Выборка,КоэффициентРезерваОтпусков,СтрокаДанных,ИзмененияНК1578)
	...
	Если ИзмененияНК1578 И СтрокаДанных.НеОтражатьВБУ Тогда
		...
		ОтражениеВУчете.Удалить(СтрокаДанных);
...
Процедура АвтозаполнениеПроводок() Экспорт
	...
	Если Истина Тогда
		...
		|        И НЕ Основной.ВидРасчета.ВидОтпуска.НеВключатьВРезервОтпусков)
		|			ИЛИ Основной.ВидРасчета.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуКомпенсацияОтпуска) 	
		...
		|        И НЕ Основной.ВидРасчетаБазы.ВидОтпуска.НеВключатьВРезервОтпусков)
		|			ИЛИ Основной.ВидРасчетаБазы.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуКомпенсацияОтпуска) 	
		|				ТОГДА &РезервОтпусков
		|         ИНАЧЕ ЛОЖЬ
		|       КОНЕЦ КАК РезервОтпусков,
		|		2 КАК Порядок,
...
Процедура РассчитатьНачисленияПоКатегории( Сотрудники, Категория, НаборНачисления )
	...
	Если Истина Тогда
		...
		|   БазаИзНачислений.СпособОтражения			КАК Ссылка,
		...
		|   БазаИзНачислений.СпособОтражения
		| ";
		Запрос.Текст = ТекстЗапросаВТ;
		Запрос.Выполнить();
	...
	Если Истина Тогда
		...
		|   	" + СтрЗаменить(ТекстПараметра,"ИмяПараметра","Ссылка")+"		КАК СпособОтражения,
		|		
...
Процедура РассчитатьВзносыФОТ( Сотрудники ) Экспорт
	...
	Пока ВыборкаПоНачислениям.Следующий() Цикл 
		...
		Иначе
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Отражение взносов ФОТ по сотруднику ';uk='Відображення внесків ФОП по співробітнику '")+ВыборкаПоНачислениям.Сотрудник.Наименование+НСтр("ru=' может быть выполнено не верно.';uk=' може бути виконано не вірно.'"));
			Отказ = Ложь;
		КонецЕсли;
	КонецЦикла;
	НаборНачисления.Записать();
...
Процедура	РассчитатьВзносыФОТЗаписи( Сотрудники ) Экспорт
	...
	Если Истина Тогда
		...
		|   		" + СтрЗаменить(ТекстПараметраСчетКт,"ИмяПараметра","СубконтоКт2")+"					КАК СубконтоКт2,
		|   		" + СтрЗаменить(ТекстПараметраСчетКт,"ИмяПараметра","СубконтоКт3")+"					КАК СубконтоКт3,
		|   		" + СтрЗаменить(ТекстПараметра,"ИмяПараметра","Ссылка")+"						КАК СпособОтраженияВзносы,
		|    
	...
	Если Истина Тогда
		...
		|			Основной.СпособОтраженияВзносы,
		|    
	...
	|	Основной.СтатьяВаловыхРасходов,
	|	Основной.СтатьяЗатрат,
	|	Основной.СпособОтраженияВзносы КАК СпособОтражения,	
	|	
...
Функция СформироватьЗапросПоНачисления( Сотрудники )
	...
	|	СтрокиНачисления.СпособОтражения,
...
Функция СформироватьЗапросПоОтражению() Экспорт
	...
	|		Основной.ВзносыОсновногоНачисления	КАК ВзносыОсновногоНачисления,
	|		Основной.СпособОтраженияЕСВФОТ		КАК СпособОтраженияЕСВФОТ,
	|		Основной.СпособОтражения			КАК СпособОтражения,
	|	
	|		"+ТекстПараметрыНачисления+",
	|		Основной.Результат
	|	ИЗ	Документ.ОтражениеЗарплатыВРеглУчете.Начисления КАК Основной
...
Процедура ПроверитьЗаполнениеСтрокиОтраженияВУчете(ВыборкаПоДоходам, Отказ, Заголовок = "")
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоДоходам.НомерСтроки) + НСтр("ru='"" табл. части ""Проводки"": ';uk='"" табл. частини ""Проводки"": '");
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Дата < ПроведениеРасчетов.ДатаВводаНПНК() Тогда	
		Если НЕ УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "ОтражениеЗарплатыДо01042011") Тогда
			Сообщить(НСтр("ru='Запрещено проведение документов отражения за период до ';uk='Заборонено проведення документів відображення за період до '")+Формат(ПроведениеРасчетов.ДатаВводаНПНК(),"ДФ=dd.MM.yyyy")+НСтр("ru=' г.!';uk=' р.!'"),СтатусСообщения.ОченьВажное);
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЕсли;	
