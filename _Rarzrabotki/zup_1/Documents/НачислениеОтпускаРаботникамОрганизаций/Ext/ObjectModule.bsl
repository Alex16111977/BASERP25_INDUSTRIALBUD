...
Функция ПечатьСредняя() Экспорт
	...
	Если ПолучитьДатуНачалаСобытия() >= ПроведениеРасчетов.ДатаИзмененияПорядка100_486() Тогда
	...
	Если ПолучитьДатуНачалаСобытия() >= ПроведениеРасчетов.ДатаИзмененияПорядка100_486() Тогда
		...
		|ИЗ
		|	Документ.НачислениеОтпускаРаботникамОрганизаций.РасчетСреднего КАК НачислениеОтпускаРаботникамОрганизаций
		|ГДЕ
		|	НачислениеОтпускаРаботникамОрганизаций.Ссылка.Ссылка = &парамСсылка
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Начисление отпуска ';uk='Нарахування відпустки '")));
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	Возврат Новый Структура("П6_от_09_10_1995, Средняя",НСтр("ru='Форма П6 от 09.10.1995 (бланк)';uk='Форма П6 від 09.10.1995 (бланк)'"), НСтр("ru='Расчет средней';uk='Розрахунок середньої'"));
...
Функция РассчитатьРасчетСреднего(КомментироватьРасчет = Ложь) Экспорт
		...
		Пока Выборка.Следующий() Цикл
			...
			Если КомментироватьРасчет Тогда
				...
				Если мКоэффициентПовышенияОкладов <> 1 И (Выборка.БазовыйПериодНачало < ПериодИзмененияКоэффициента ИЛИ КоэффициентПовышенияОкладов <> 1) Тогда 
					ТекстКомментарияФормулаСКоэффициентом = НСтр("ru='Результат: ';uk='Результат: '") + Выборка.Результат + "*" + нКоэффициентПовышенияОкладов + " = " + ОКР(нКоэффициентПовышенияОкладов * Выборка.Результат,2,0);
				КонецЕсли;
	...
	Иначе
		...
		Если  РасчетСреднего.Количество() > 0 И РасчетСреднего[0].ВидРасчета = ПланыВидовРасчета.СреднийЗаработок.ПоОкладу Тогда
			ДатаНачалаСобытия = ПолучитьДатуНачалаСобытия();
			ПараметрыОклада = ПроведениеРасчетов.ПолучитьПараметыОклада1213(ДатаНачалаСобытия, Сотрудник, , , Истина);
			РасчетСреднего[0].Результат			= ПараметрыОклада["Оклад"];
			РасчетСреднего[0].ОтработаноДней	= ПараметрыОклада["НормаДней"];
			РасчетСреднего[0].ОтработаноЧасов	= ПараметрыОклада["НормаЧасов"];
			РасчетСреднего[0].КалендарныеДни	= ПараметрыОклада["КалендарныеДни"];
...
Процедура ПолучитьПериодПоРаботнику() Экспорт
	...
	КонецЕсли;
	Если ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.КомпенсацияОтпуска И ДатаНачалаСобытия >= Дата(2023,9,12) И ДатаНачалаСобытия <= Дата(2023,12,31) Тогда
...
Процедура АвтозаполнениеНачисленияОтпуск() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДатаНачалаСобытия) или НЕ ЗначениеЗаполнено(ДатаОкончанияСобытия)	Тогда
		Возврат;
	КонецЕсли;	
...
Процедура РассчитатьНачисления(Сотрудники, КомментироватьРасчет = Ложь ) Экспорт
	...
	Для каждого СтрокаСторно Из ТаблицаСторноЗаписей Цикл
		Если Час(СтрокаСторно.ПериодДействияНачало)=0 И Минута(СтрокаСторно.ПериодДействияНачало)=0 И Секунда(СтрокаСторно.ПериодДействияНачало)=1 Тогда
			Если СтрокаСторно.ПериодДействияКонец - СтрокаСторно.ПериодДействияНачало < 86400 Тогда
				Продолжить;
			КонецЕсли;	
		КонецЕсли;	
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ) Экспорт
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.Сотрудник.Организация <> ВыборкаПоШапкеДокумента.Организация Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Сотрудник не соответствует организации!';uk='Співробітник не відповідає організації!'"), Отказ);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ОтпускаОрганизаций") Тогда
		...
		Если Основание.ВидОперации = Перечисления.ВидыОперацийОтпускаОрганизаций.Отпуск Тогда
		   ...
		   |";
			...
			Если ВсеОтпуска.Количество() = 0 тогда
				...
				Сообщить(НСтр("ru='По всем физ.лицам уже рассчитаны отпуска';uk='По всім фіз. особам вже розраховані відпустки'"));
				Возврат;
			КонецЕсли;
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(Сотрудник) Тогда	
				ТекущаяСтрока = ВсеРаботники.ВыбратьСтроку(НСтр("ru='Выберите работника:';uk='Виберіть працівника:'"));
			Иначе
		...
		ИначеЕсли Основание.ВидОперации = Перечисления.ВидыОперацийОтпускаОрганизаций.КомпенсацияОтпуска Тогда
			...
			Если ВсеОтпуска.Количество() = 0 тогда
				мНеСоздаватьДокумент = Истина;
				Сообщить(НСтр("ru='По всем физ.лицам уже выплачена компенсация';uk='По всім фіз. особам вже виплачена компенсація'"));
				Возврат;
			КонецЕсли;
		...
		ИначеЕсли Основание.ВидОперации = Перечисления.ВидыОперацийОтпускаОрганизаций.КорректировкаОтпуска Тогда	
			Сообщить(НСтр("ru='Ввод на основании для корректировки не предусмотрен';uk='Введення на підставі для коригування не передбачено'"));
			мНеСоздаватьДокумент = Истина;
			Возврат;
		КонецЕсли;	
	КонецЕсли;
