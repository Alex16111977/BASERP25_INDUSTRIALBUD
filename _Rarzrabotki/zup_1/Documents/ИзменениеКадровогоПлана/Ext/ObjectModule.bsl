...
Процедура ЗаполнитьТабличнуюЧастьРабочиеМестаПоТекущемуСостоянию() Экспорт
	...
	Если МассивДолжностейСОшибками.Количество() > 0 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Для следующих должностей не указана должность для набора персонала! Рабочие места по этим должностям не могут быть заполнены:';uk='Для наступних посад не вказана посада для набору персоналу! Робочі місця за цими посадами не можуть бути заповнені:'"));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаИзменений) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указана дата изменений!';uk='Не вказана дата змін!'"), Отказ, Заголовок);
	КонецЕсли;
