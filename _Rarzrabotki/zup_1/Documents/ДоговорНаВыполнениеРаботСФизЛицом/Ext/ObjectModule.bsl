...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Договор" Тогда
		...
		Макет.Параметры.ПредставлениеОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,/,КодПоЕДРПОУ,КодПоДРФО,Телефоны,/,ИНН,НомерСвидетельства,/,НомерСчета,Банк,МФО,/,ЮридическийАдрес,",,КодЯзыкаПечать);
		Если Выборка.ЧастотаВыплат = Перечисления.ХарактерВыплатыПоДоговору.Ежемесячно Тогда
			Макет.Параметры.ЧастотаВыплат =  НСтр("ru='ежемесячно';uk='щомісяця'",КодЯзыкаПечать);
		ИначеЕсли Выборка.ЧастотаВыплат = Перечисления.ХарактерВыплатыПоДоговору.ОднократноВКонцеСрока Тогда
			Макет.Параметры.ЧастотаВыплат =  НСтр("ru='однократно в конце срока';uk='одноразово в кінці терміну'",КодЯзыкаПечать);
		ИначеЕсли Выборка.ЧастотаВыплат = Перечисления.ХарактерВыплатыПоДоговору.ПоСдельномуНаряду Тогда
			Макет.Параметры.ЧастотаВыплат =  НСтр("ru='по сдельному наряду';uk='по відрядному наряду'",КодЯзыкаПечать);		
		КонецЕсли;	
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Договор ';uk='Договір '")));
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Договор", НСтр("ru='Договор';uk='Договір'"));
	...
	Если НазначитьВзносы Тогда
		...
		Пока ВыборкаПоВзносам.Следующий() Цикл
			...
			ИначеЕсли ВыборкаПоВзносам.Действие = Перечисления.ВидыДействияСНачислением.НеИзменять тогда
			    Сообщить(НСтр("ru='На дату начала договора ';uk='На дату початку договору '") + Ссылка + НСтр("ru=' сотруднику уже назначено удержание взноса ';uk=' співробітнику вже призначено утримання внеску '") + ВыборкаПоВзносам.Налог.Наименование );
   			КонецЕсли;	
		КонецЦикла;	
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			Если НЕ (СуммаЗаРаботу = 0 И ХарактерОплаты <> Перечисления.ХарактерВыплатыПоДоговору.ПоСдельномуНаряду) Тогда
