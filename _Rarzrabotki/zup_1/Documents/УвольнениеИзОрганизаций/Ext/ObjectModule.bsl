...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Увольнение из организации ';uk='Звільнення з організації '")));
...
Функция ОпределитьКВОДнейОтпускаДляКомпенсации(Сотрудник = Неопределено, ДатаУвольнения)Экспорт
	...
	|			ВЫРАЗИТЬ(ОстатокОтпуска.ПоложеноДнейОтпуска*(РАЗНОСТЬДАТ(ОстатокОтпуска.РабочийПериодНачало,&ДатаУвольнения, ДЕНЬ)+1)/
...
Функция СформироватьЗапросДляПечати(Режим)
	...
	ИначеЕсли Режим = "ПоТабличнойЧастиДокумента" Тогда
        ...
        |	УвольнениеИзОрганизацииРаботники.СтатьяКЗоТ.СтатьяЗакона КАК СтатьяЗакона,
...
Функция СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента, Режим)
	...
	|	ТЧРаботникиОрганизации.СтатьяКЗОТ КАК СтатьяКЗОТ,
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.ВидСтрокиЗапроса = "ДанныеДляДвиженийКадров" Тогда
		...
		Если НетДатыУвольнения Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата увольнения!';uk='не зазначена дата звільнення!'"), Отказ, Заголовок);
		КонецЕсли;
		...
		Если ВыборкаПоСтрокамДокумента.ПрежняяСтавка = NULL Тогда
			СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru='на ';uk='на '") + Формат(ВыборкаПоСтрокамДокумента.ДатаУвольнения, "ДЛФ=DD") + НСтр("ru=' работник ';uk=' працівник '") + ВыборкаПоСтрокамДокумента.СотрудникНаименование + НСтр("ru=' еще не принят на работу!';uk=' ще не прийнятий на роботу!'");
		...
		ИначеЕсли ВыборкаПоСтрокамДокумента.ПрежняяСтавка = 0 Тогда
			СтрокаПродолжениеСообщенияОбОшибке = НСтр("ru='на ';uk='на '") + Формат(ВыборкаПоСтрокамДокумента.ДатаУвольнения, "ДЛФ=DD") + НСтр("ru=' работник ';uk=' працівник '") + ВыборкаПоСтрокамДокумента.СотрудникНаименование + НСтр("ru=' уже уволен (с ';uk=' вже звільнений (з '") + Формат(ВыборкаПоСтрокамДокумента.ДатаПоследнегоДвиженияПоРаботнику, "ДЛФ=DD") + ")!";
			Сообщить(СтрокаНачалаСообщенияОбОшибке + СтрокаПродолжениеСообщенияОбОшибке);
	...
	Пока ВыборкаСтрокЗапроса.Следующий() Цикл
		...
		Если ВидЗанятостиПоПриказу = Перечисления.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы  Тогда
			...
			Если ВыборкаСтрокЗапроса.ВидЗанятостиПоДругомуМестуРаботы = Перечисления.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство  Тогда
				СтрокаСообщениеОбОшибке = НСтр("ru='нельзя уволить работника с основного места работы до тех пор,"
"	пока он оформлен внутренним совместителем!"
"	(Сотрудник: ';uk='не можна звільнити працівника з основного місця роботи до тих пір,"
"	поки він оформлений внутрішнім сумісником!"
"	(Співробітник: '") + ВыборкаСтрокЗапроса.Сотрудник + ")"; 
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + СтрокаСообщениеОбОшибке, Отказ, Заголовок);
			КонецЕсли;
