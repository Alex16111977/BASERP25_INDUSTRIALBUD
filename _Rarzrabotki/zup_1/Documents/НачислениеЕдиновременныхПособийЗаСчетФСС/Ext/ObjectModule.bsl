...
Процедура Рассчитать() Экспорт
    ...
    Запрос = Новый Запрос("ВЫБРАТЬ
                          ...
                          |	ДатыПособий.Физлицо,
                          |	ДатыПособий.ФамилияУмершего,
                          |	ДатыПособий.ИмяУмершего,
                          |	ДатыПособий.ОтчествоУмершего,
                    	  |	ДатыПособий.СвидетельствоСерия,
                          |	ДатыПособий.СвидетельствоНомер,
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан работник!';uk='не вказаний працівник!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// ВидПособия
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидРасчета) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан вид пособия!';uk='не зазначений вид допомоги!'"), Отказ, Заголовок);
	КонецЕсли;
	
	// ДатаСобытия
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСобытия) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указана дата события!';uk='не вказана дата події!'"), Отказ, Заголовок);
	КонецЕсли;
