...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,НСтр("ru='Оплата по среднему заработку ';uk='Оплата за середнім заробітком '")));
...
Функция АвтозаполнениеРасчетСреднего() Экспорт
	...
	Если ДатаНачала < ПроведениеРасчетов.ДатаИзмененияПорядка100() Тогда
		...
		КонецЦикла;
	Иначе
		//	1. Проверка если данных для расчета средней еще нет (человек мало проработал, начисляем по окладу)
		Если  НачалоДня(ДобавитьМесяц(ПериодРасчетаСреднегоНачало,1)-1) > ПериодРасчетаСреднегоКонец Тогда
			ПолучитьДанныеОбОтработанномВремени(ОтработаноЗа2Мес, ОтработаноЗа4Мес, ОсновнойВидРасчета);
...
Процедура ПолучитьПериодПоРаботнику() Экспорт
	...
	Если (НЕ ГрафикРаботы = Неопределено И ГрафикРаботы.СуммированныйУчетРабочегоВремени) 
