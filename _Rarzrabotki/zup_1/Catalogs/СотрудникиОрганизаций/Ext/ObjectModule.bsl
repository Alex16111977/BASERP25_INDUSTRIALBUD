	...
	Если ИмяМакета = "Печать" Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ИначеЕсли Выборка.ВидГрафика = Перечисления.ВидыРабочихГрафиков.Шестидневка Тогда
				ВидГрафика = НСтр("ru='Шестидневка';uk='Шестиденка'",КодЯзыкаПечать);
...
ИначеЕсли ИмяМакета = "СправкаОПриемеНаНовоеРабочееМесто" Тогда 
	...
	Если ИмяМакета = "ЛичнаяКарточка" Тогда 
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, НСтр("ru='Личная карточка';uk='Особова картка'"));
	Иначе
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, НСтр("ru='Трудовой договор';uk='Трудовий договір'"));
	КонецЕсли;	
...
Процедура СформироватьСправкуОПриемеНаНовоеРабочееМесто() Экспорт  
	...
	Если Выборка.Следующий() Тогда
		...
		Если ВыборкаГарантии.Следующий() Тогда
			Если ВыборкаГарантии.НаличиеГарантий И ПоНаправлениюОргановЗанятости Тогда
				ВидСправки = НСтр("ru='Для принятых по направлению и имеющих гарантии трудоустройства (порядок 347)';uk='Для прийнятих по направленню і тих, що мають гарантії працевлаштування (порядок 347)'");	
			ИначеЕсли (ВыборкаГарантии.НаличиеГарантий И НЕ ПоНаправлениюОргановЗанятости)
				...
				ВидСправки = НСтр("ru='Для принятых на новое место работы (порядок 153)';uk='Для прийнятих на нове місце роботи (порядок 153)'")				
			КонецЕсли;
		Иначе
			ВидСправки = НСтр("ru='Для принятых на новое место работы (порядок 153)';uk='Для прийнятих на нове місце роботи (порядок 153)'")				
		КонецЕсли;
	...
	Иначе	
		Сообщить(НСтр("ru='Работник ';uk='Працівник '") + Ссылка.Наименование + НСтр("ru=' не работает на новом рабочем месте';uk=' не працює на новому робочому місці'"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ЛичнаяКарточка, Печать, СправкаОПриемеНаНовоеРабочееМесто",НСтр("ru='Личная карточка (форма П-2)';uk='Особова картка (форма П-2)'"),НСтр("ru='Трудовой договор';uk='Трудовий договір'"),НСтр("ru='Справка о приеме на новое рабочее место';uk='Довідка про прийняття на нове робоче місце'"));
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоГруппа И Физлицо.Пустая() Тогда
		ТекстИсключения = НСтр("ru='Для сотрудника не задано физическое лицо!';uk='Для співробітника не задана фізична особа!'");
	КонецЕсли;
