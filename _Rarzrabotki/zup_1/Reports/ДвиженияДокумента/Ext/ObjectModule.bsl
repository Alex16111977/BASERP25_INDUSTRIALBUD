...
Функция ОпределитьВидРегистраДляПечати(ВидРегистра)
	
	Если ВидРегистра = "Накопления" Тогда
		Возврат НСтр("ru='накопления';uk='накопичення'");
		
	ИначеЕсли ВидРегистра = "Сведений" Тогда
		Возврат НСтр("ru='сведений';uk='відомостей'");	
		
	ИначеЕсли ВидРегистра = "Бухгалтерии" Тогда
		Возврат НСтр("ru='бухгалтерии';uk='бухгалтерії'");
		
	ИначеЕсли ВидРегистра = "Расчета" Тогда
		Возврат НСтр("ru='расчета';uk='розрахунку'");
...
Функция ОпределитьВозможностьПолученияОтчета()
	...
	Если Документ.Пустая() Тогда
		Предупреждение(НСтр("ru='Запишите и проведите документ, по которому Вы фомируете отчет';uk='Запишіть і проведіть документ, по якому Ви формуєте звіт'"));
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		Если (ВидРегистра = "Сведений") 
			...
			Если (ВидРегистра = "Накопления")
				...
				СтруктураПредставлениеПолей.Вставить("ВидДвижения", НСтр("ru='Вид движения';uk='Вид руху'"));
			...
			Если ВидРегистра = "Сведений" Тогда
				Если Строка(СвойстваОбъекта.ПериодичностьРегистраСведений) =  "Непериодический" Тогда
					ТекстИзмерения = СформироватьСписокПолей(СвойстваОбъекта.Измерения, СтруктураПредставлениеПолей);
				Иначе
					ТекстИзмерения = СформироватьСписокПолей(СвойстваОбъекта.Измерения, СтруктураПредставлениеПолей, Истина);
				КонецЕсли;
			Иначе
				ТекстИзмерения = СформироватьСписокПолей(СвойстваОбъекта.Измерения, СтруктураПредставлениеПолей, Истина);
			КонецЕсли;	
			...
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстИзмерения, НСтр("ru='Измерения';uk='Виміри'"), 40));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРесурсы, НСтр("ru='Ресурсы';uk='Ресурси'"), 30));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРеквизиты, НСтр("ru='Реквизиты';uk='Реквізити'"), 20));
		...
		ИначеЕсли ВидРегистра = "Расчета" Тогда
			...
			СтруктураПредставлениеПолей.Вставить("ВидРасчета", НСтр("ru='Вид расчета';uk='Вид розрахунку'"));
			СтруктураПредставлениеПолей.Вставить("ПериодРегистрации", НСтр("ru='Период регистрации';uk='Період реєстрації'"));
			...
			Если СвойстваОбъекта.БазовыйПериод Тогда
				...
				СтруктураПредставлениеПолей.Вставить("БазовыйПериодНачало", НСтр("ru='Начало базового периода';uk='Початок базового періоду'"));
				СтруктураПредставлениеПолей.Вставить("БазовыйПериодКонец", НСтр("ru='Конец базового периода';uk='Кінець базового періоду'"));
			КонецЕсли;
			...
			Если СвойстваОбъекта.ПериодДействия Тогда
				...
				СтруктураПредставлениеПолей.Вставить("ПериодДействияНачало", НСтр("ru='Начало периода действия';uk='Початок періоду дії'"));
				СтруктураПредставлениеПолей.Вставить("ПериодДействияКонец", НСтр("ru='Конец периода действия';uk='Кінець періоду дії'"));
			КонецЕсли;
			...
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", СпецифическиеДанныеРегистраРасчета, НСтр("ru='Данные расчета';uk='Дані розрахунку'"), 30));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстИзмерения, НСтр("ru='Измерения';uk='Виміри'"), 40));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРесурсы, НСтр("ru='Ресурсы';uk='Ресурси'"), 30));
			ДобавитьструктуруДанныхВМассивПолей(МассивПолей, Новый Структура("СписокПолей, ИмяКолонки, Ширина", ТекстРеквизиты, НСтр("ru='Реквизиты';uk='Реквізити'"), 20));
		...
		ОбластьЗаголовок.Параметры.ВидРегистра = ОпределитьВидРегистраДляПечати(ВидРегистра);
		ДокументРезультат.Вывести(ОбластьЗаголовок);
...
Функция СформироватьСписокПолей(МетаданныеРесурса, СтруктураПредставлениеПолей, Период = Ложь)
	
	СписокПолей = "";
	Если ПоказыватьПериод И Период Тогда
