...
Процедура ДобавитьОбластьЗаголовок(Макет)
	...
	Для каждого ИзмерениеСтроки из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		ПредставлениеГруппировкиСтрок = ПредставлениеГруппировкиСтрок + " " 
										...
										+ " (" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ИзмерениеСтроки.ТипИзмерения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаИнтерфейса()) + ");";
	...
	Для каждого ИзмерениеКолонки из ПостроительОтчета.ИзмеренияКолонки Цикл
		...
		ПредставлениеГруппировкиКолонок = ПредставлениеГруппировкиКолонок + " " 
										  ...
										  + " (" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ИзмерениеКолонки.ТипИзмерения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаИнтерфейса()) + ");";
	...
	Для каждого Отбор из ПостроительОтчета.Отбор Цикл
		...
		Если Отбор.Использование И Отбор.Имя <> "Периодичность" Тогда
			...
			ИначеЕсли Отбор.ВидСравнения = ВидСравнения.ВСписке ИЛИ Отбор.ВидСравнения = ВидСравнения.ВСпискеПоИерархии ИЛИ Отбор.ВидСравнения = ВидСравнения.НеВСписке ИЛИ Отбор.ВидСравнения = ВидСравнения.НеВСпискеПоИерархии Тогда
				...
				ПредставлениеОтборы = ПредставлениеОтборы + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(Отбор.ВидСравнения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаИнтерфейса()) + " (" + Отбор.Значение + ")";
	...
	Для каждого ВыбранноеПоле из ПостроительОтчета.ВыбранныеПоля Цикл
		...
		Если НЕ ЭтоПолеПоказатель(ВыбранноеПоле.Имя) Тогда
			...
			+ " (" 
			+ ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПолучитьРазмещениеВыбранногоПоля(ВыбранноеПоле.Имя)), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаИнтерфейса()) 
			+ ", " 
			+ ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПолучитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя)),  ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаИнтерфейса()) 
			+ ");";
	...
	Для каждого ПолеПорядка из ПостроительОтчета.Порядок Цикл
		...
		ПредставлениеСортировка = ПредставлениеСортировка + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПолеПорядка.Представление) + " (" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПолеПорядка.Направление), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаИнтерфейса()) + ");";
...
Процедура ДобавитьВТекстЗапросаДанныеОФизлице(ТекстЗапроса, ТаблицаПолей = Неопределено, СтруктураПредставлениеПолей = Неопределено, Параметры = Неопределено, СоответствиеДанныхОФизлицеИИзмерений = Неопределено, СтруктураЗаменять = Неопределено) Экспорт
	...
	Для каждого Поле из ТаблицаПолей Цикл
		...
		|	" + Поле.ПутьКДанным + ".УНЗР КАК " + Поле.ИмяИзмерения + "УНЗР,
		...
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "УНЗР),
		...
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "УНЗР", "УНЗР");
		...
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "УНЗР", Поле.ИмяИзмерения);
	...
	Если мДействиеФормыВыполнитьВнешнююОбработку <> Неопределено Тогда
		...
		Если ОбработкиЗапускаемыеВручную.Количество() <> 0 Тогда
			...
			Для каждого ЭлОбработка Из ОбработкиЗапускаемыеВручную Цикл
				...
				Кнопка.Пояснение = НСтр("ru='Выполнить обработку ""';uk='Виконати обробку ""'") + ЭлОбработка.Наименование + """";
	...
	Если мДействиеКоманднаяПанельФормыСправкаФормы <> Неопределено Тогда
		...
		Если Форма.ЭлементыФормы.Найти("КоманднаяПанельФормы") <> Неопределено Тогда
			...
			Если Форма.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Найти("Справка") <> Неопределено Тогда
				...
				КнопкаСправкаФормы.Текст = НСтр("ru='Показать/скрыть справку';uk='Показати/приховати довідку'");
				КнопкаСправкаФормы.Подсказка = НСтр("ru='Показать/скрыть справку';uk='Показати/приховати довідку'");
				КнопкаСправкаФормы.Пояснение = НСтр("ru='Показать/скрыть справку';uk='Показати/приховати довідку'");
