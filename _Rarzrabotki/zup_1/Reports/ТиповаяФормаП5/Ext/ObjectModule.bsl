...
Процедура СформироватьОтчет(ДокументРезультат, Режим, ВозвращаемоеСообщениеОбОшибке = Неопределено, ИзДокумента = Ложь) Экспорт
	...
	ЗапросТабель.УстановитьПараметр("ПустойГрафик", Справочники.ГрафикиРаботы.ПустаяСсылка()); 
	...
	Если Режим = "Табель" Тогда
		...
		Если РабочаяДата < НачалоМесяца(ДатаНач) И НЕ ИзДокумента Тогда
			...
			ВозвращаемоеСообщениеОбОшибке = НСтр("ru='Отчет можно формировать только за текущий или предыдущий периоды!';uk='Звіт можна формувати тільки за поточний або попередній періоди!'");
			Возврат;
		Иначе
		...
		|ИЗ
		|	РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций.ФактическийПериодДействия((Назначение В (ВЫБРАТЬ Сотрудник ИЗ ВТСотрудники)) И НЕ(ДокументОснование ССЫЛКА Документ.ЗаменыОрганизаций)) КАК ФактическийПериодДействия
		|ГДЕ
		...
		|	ИЛИ (ФактическийПериодДействия.ВидРасчета.ВидВремени В (ЗНАЧЕНИЕ(Перечисление.ВидыВремени.ДополнительноОплачиваемоеВПределахНормы))
		...
		|	ВЫБОР КОГДА (Начисления.ПеречислениеВидВремени = &ОтработаноСверхНормы) 
		|	ИЛИ (Начисления.ВидРасчета.ВидВремени В (ЗНАЧЕНИЕ(Перечисление.ВидыВремени.ДополнительноОплачиваемоеВПределахНормы))
		...
		|ГДЕ
		|	ЗаменыОрганизаций.Сотрудник В(ВЫБРАТЬ Сотрудник ИЗ ВТСотрудники)
		|	И (ЗаменыОрганизаций.ДатаНачала < &НачалоПериода
		|				И ЗаменыОрганизаций.ДатаОкончания >= &НачалоПериода
		|			ИЛИ ЗаменыОрганизаций.ДатаОкончания > &КонецПериода
		|				И ЗаменыОрганизаций.ДатаНачала <= &КонецПериода
		|			ИЛИ ЗаменыОрганизаций.ДатаНачала >= &НачалоПериода
		|				И ЗаменыОрганизаций.ДатаОкончания <= &КонецПериода)
		...
		|ИЗ
		|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&НачалоПериода, Сотрудник В (ВЫБРАТЬ Сотрудник ИЗ ВТСотрудники)) КАК РаботникиОрганизацийСрезПоследних
		|ГДЕ
		...
		|	РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
		|ГДЕ
		|	РаботникиОрганизаций.Сотрудник В(ВЫБРАТЬ Сотрудник ИЗ ВТСотрудники)
		...
		|ПОМЕСТИТЬ ВТРаботникиОрганизацийГрафики
		|ИЗ
		...
		|	РаботникиОрганизаций.Период
		|ИЗ
		|	ВТРаботникиОрганизацийГрафики КАК РаботникиОрганизаций
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
	    ...
	    |		КОГДА Основной.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора)
	    |			ТОГДА ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.Простой)
		...
		|		НеотфильтрованноеВремя.ВключатьПраздникиВВидОтпуска КАК ВключатьПраздникиВВидОтпуска,
		...
		|		НеотфильтрованноеВремя.УчитыватьПраздники КАК УчитыватьПраздники
		|		ИЗ (
		|		ВЫБРАТЬ РАЗЛИЧНЫЕ 
		...
		|		ЕСТЬNULL(ВТОсновныеНачисления.ВидРасчета.ВидОтпуска.ВключатьПраздники,Ложь) КАК ВключатьПраздникиВВидОтпуска, 
		|		ВЫБОР 
		...
		|						   NULL
		|					КОГДА НЕ ВТОсновныеНачисления.ПеречислениеВидВремени В (Значение(Перечисление.ВидыВремени.ЦелодневноеНеотработанное), Значение(Перечисление.ВидыВремени.ОтработанноеСверхНормы),Значение(Перечисление.ВидыВремени.ДополнительноОплачиваемоеВПределахНормы))
		|						И ГрафикПоОсновнымНачислениям.ОсновноеЗначение = 0 ТОГДА 
		|		   				   &Выходной
		...
		|		ВТГрафикиПериодами.ГрафикРаботы.УчитыватьПраздники КАК УчитыватьПраздники
		|	ИЗ ВТСотрудникДата
		...
		|		ПО ИндивидуальныйГрафикПоРаботникам.ГрафикРаботы = ВЫБОР КОГДА ВТСотрудникДата.Сотрудник.ОсновноеНазначение = ЗНАЧЕНИЕ(Справочник.СотрудникиОрганизаций.ПустаяСсылка) 
		|												ТОГДА ВТСотрудникДата.Сотрудник
		|												ИНАЧЕ ВТСотрудникДата.Сотрудник.ОсновноеНазначение
		|												КОНЕЦ
		...
		|УПОРЯДОЧИТЬ ПО
		|	Организация, " + ПорядокГруппировки + " СотрудникНаименование, Должность, ТабельныйНомер, ДатаТабеля, ПолеСортировки";
		...
		Если ГруппироватьПодразделения Тогда
			...
			Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("Организация") Цикл
				...
				Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("ПодразделениеОрганизации") Цикл
								...
								Пока ВыборкаПоРаботнику.Следующий() Цикл
									...
									Если ГрафаНеявок(ВыборкаПоРаботнику.ВидВремени)>2 Тогда
										...
										Если ВыборкаПоРаботнику.ВнутрисменныеОтклоненияЧасов = 0 И ВыборкаПоРаботнику.НеотработаноЧасов = 0
											И НЕ (ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.ОтпускПоБеременностиИРодам)
											И НЕ (ВыборкаПоРаботнику.ДатаТабеля >= ПроведениеРасчетов.ДатаИзмененияПорядка1266() 
												И ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Болезнь)Тогда
											//это невыход в выходной - в отчете не отображаем
											Продолжить;
										КонецЕсли;
									ИначеЕсли ГрафаНеявок(ВыборкаПоРаботнику.ВидВремени)>0 Тогда
										//отпуск
										Если ВыборкаПоРаботнику.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник 
											И НЕ ВыборкаПоРаботнику.ВключатьПраздникиВВидОтпуска Тогда   
									...
									Если (НЕ ВыборкаПоРаботнику.ВидВремени.РабочееВремя)
										И (ВыборкаПоРаботнику.ВидДня <> Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник ИЛИ (ВыборкаПоРаботнику.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник И 
										(НЕ(ВыборкаПоРаботнику.УчитыватьПраздники)ИЛИ(ВыборкаПоРаботнику.ВключатьПраздникиВВидОтпуска))) 
										ИЛИ (ВыборкаПоРаботнику.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник И ВыборкаПоРаботнику.ДатаТабеля >= ПроведениеРасчетов.ДатаИзмененияПорядка1266() И ВыборкаПоРаботнику.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Болезнь)) 
					...
					Если Не ФормированиеПечатныхФорм.ПроверитьВыводТабличногоДокумента(ДокументРезультат,ОбластьДанныхИтоги) Тогда
					...
					Если ВыводитьПримечание Тогда
						Если Не ФормированиеПечатныхФорм.ПроверитьВыводТабличногоДокумента(ДокументРезультат,ОбластьДанныхПримечание) Тогда
							ДокументРезультат.ВывестиГоризонтальныйРазделительСтраниц();
						КонецЕсли;	
	...
	ИначеЕсли Режим = "Бланк" Тогда 
		...
		Если ДокументРезультат.ВысотаТаблицы = 0 Тогда
			...
			Если ОтборОрганизации и ТипЗнч(Организация) = Тип("СправочникСсылка.Организации") Тогда
				ОбластьДанныхШапка.Параметры.ИмяОрганизации = ВРег(Организация.НаименованиеПолное);
