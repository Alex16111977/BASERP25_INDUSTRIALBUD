...
Процедура ОбработкаВнешнегоСобытияЗвит1С(Источник, Событие, Данные) Экспорт
	...
	ИначеЕсли Команда = "REPORT" Тогда
		...
		Если СоответствиеОтчета = Неопределено Тогда 
			...
			Если СоответствиеОтчета = Неопределено Тогда 
				...
				Если ЗначениеЗаполнено(СоответствиеОтчета) Тогда
					...
					|у даній версії облікової системи ще не реалізовано. 
					...
					|Докладніше див.:   https://its.bas-soft.eu/bmk/erc";
					Компонента.CancelReport(ОтчетЗвит1С, ТекстОшибки);
				...
				Иначе	
					...
					ТекстОшибки = "Заповнення звіту """+ИмяСхемы+"""
								  |даними з облікової системи не передбачено!";
					Компонента.CancelReport(ОтчетЗвит1С, ТекстОшибки);
		...
		Если ОрганизацииЗвит1С.Количество() = 0 Тогда
			ТекстОшибки = "В обліковій системі не знайдено фірму з ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+?(РегламентированнаяОтчетность.ИДКонфигурации()<> "УТ", " на "+Формат(ДатаКонцаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy"), "")+"
		...
		ИначеЕсли ОрганизацииЗвит1С.Количество() > 1 Тогда                                                                                                                       
			ТекстОшибки = "В обліковій системі знайдено декілька фірм з однаковим ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+?(РегламентированнаяОтчетность.ИДКонфигурации()<> "УТ", " на "+Формат(ДатаКонцаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy"), "")+Символы.ПС;
	...
	ИначеЕсли Команда = "REQUESTFIRMCARD" Тогда    
		...
		Если ОрганизацииЗвит1С.Количество() <> 1 Тогда
			Компонента.Show1CMessage("Заповнення картки підприємства не можливе. В обліковій системі не знайдена, або існує декілька організацій з кодом ЄДРПОУ: " + КодЕДРПОУОрганизации, 3);
