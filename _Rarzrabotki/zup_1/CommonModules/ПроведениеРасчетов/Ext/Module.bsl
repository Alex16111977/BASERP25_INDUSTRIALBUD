...
Функция ОплачиваемоеВремя(ИсходныеДанные, СтрокаДвижений, КомментироватьРасчет, КомментарийВладелец, СообщениеОбОшибке = Ложь, ГрафикиБезРабочегоВремени)
	...
	Если ИсходныеДанные.ОтработаноВремени = Null Тогда
		Если ИсходныеДанные.ОшибкаВводаОтработанногоВремени Тогда
			ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Данные об отработанном времени введены сводно. Расчет времени по данному виду расчета невозможен';uk=': Дані про відпрацьований час введені зведено. Розрахунок часу по даному виду розрахунку неможливий'"));
		ИначеЕсли ИсходныеДанные.ОшибкаВводаИндивидуальногоГрафикаРаботы Тогда
			ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Индивидуальный график работы введен сводно. Расчет времени по данному виду расчета невозможен';uk=': Індивідуальний графік роботи введений зведено. Розрахунок часу по даному виду розрахунку неможливий'"));
		Иначе
			ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не найдены данные об отработанном времени';uk=': Не знайдені дані про відпрацьований час'"));
		КонецЕсли;
		СообщениеОбОшибке = Истина;
		Возврат 0;
	ИначеЕсли ИсходныеДанные.ПредупреждениеНесоответствияТабеляИНачисления Тогда
		ОбщегоНазначения.ОшибкаРасчета(НСтр("ru='Предупреждение: ';uk='Попередження: '") + ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Данные об отработанном времени введенные табелем,  не соответствуют введенным начислениям';uk=': Дані про відпрацьований час введені табелем, не відповідають введеним нарахуванням'"));
	КонецЕсли;
	...
	Если КомментироватьРасчет Тогда
		Если ИсходныеДанные.СпособВводаВремени = 1 
			...
			Если ИсходныеДанные.СпособВводаВремени = 1 Тогда
				ПредставлениеРасшифровки = НСтр("ru='табелем учета рабочего времени или сдельным нарядом';uk='табелем обліку робочого часу або відрядними нарядом'");
			Иначе
				ПредставлениеРасшифровки = НСтр("ru='индивидуальным графиком работы';uk='індивідуальним графіком роботи'");
			КонецЕсли;
	...
	Если КомментироватьРасчет Тогда
		ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Оплачиваемое время ';uk='Оплачуваний час '") + ?(ИсходныеДанные.ВидУчетаВремени = Перечисления.ВидыУчетаВремени.ПоДням, НСтр("ru='(в днях) ';uk='(у днях) '"), НСтр("ru='(в часах) ';uk='(в годинах) '")) + ОплачиваемоеВремя, КомментарийВладелец);
		ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Вид учета времени: ';uk='Вид обліку часу: '") + ИсходныеДанные.ВидУчетаВремени, КомментарийВладелец);
	...
	Если ОплачиваемоеВремя = 0 И Не ЗначениеЗаполнено(ИсходныеДанные.НормаВремениЗаМесяц) Тогда
		...
		Если КомментироватьРасчет Или КоличествоСтрокГрафика = 0 Тогда
			...
			Если ИсходныеДанные.СпособВводаВремени = 1 Тогда
				...
				РасшифровкаКомментария.Добавить(СтрокаДвижений.ПериодДействия);
				Если КомментироватьРасчет Тогда
					ТекстКомментария = НСтр("ru='В сдельном наряде или табеле учета рабочего времени за ';uk='У відрядному наряді або табелі обліку робочого часу за '") + Формат(СтрокаДвижений.ПериодДействия, "ДФ='MMММ yyyy ""г.""'") + НСтр("ru=' нет ';uk=' немає '") + ТекстВидаВремени + НСтр("ru='! Необходимо заполнить ';uk='! Необхідно заповнити '");
				Иначе
					...
					ТекстКомментария = НСтр("ru='У ';uk='У '") + ФИО + НСтр("ru=' в сдельном наряде или табеле учета рабочего времени за ';uk=' у відрядному наряді або табелі обліку робочого часу за '") + Формат(СтрокаДвижений.ПериодДействия, "ДФ='MMММ yyyy ""г.""'") + НСтр("ru=' нет ';uk=' немає '") + ТекстВидаВремени + НСтр("ru='! Необходимо заполнить ';uk='! Необхідно заповнити '");
				КонецЕсли;
				
				ОбщегоНазначения.КомментарийРасчета(ТекстКомментария, КомментарийВладелец, НСтр("ru='документ';uk='документ'"), РасшифровкаКомментария, Перечисления.ВидыСообщений.Ошибка);
			...
			ИначеЕсли ИсходныеДанные.СпособВводаВремени = 2 Тогда
				...
				Если КомментироватьРасчет Тогда
					ТекстКомментария = НСтр("ru='В индивидуальном графике за ';uk='В індивідуальному графіку за '") + Формат(СтрокаДвижений.ПериодДействия, "ДФ='MMММ yyyy ""г.""'") + НСтр("ru=' нет ';uk=' немає '") + ТекстВидаВремени + НСтр("ru='! Необходимо заполнить ';uk='! Необхідно заповнити '");
				Иначе
					ФИО = ИсходныеДанные.СотрудникНаименование;
					УниверсальныеМеханизмы.Просклонять(глЗначениеПеременной("глКомпонентаСклоненияФИО"), ФИО, 2, ФИО);
					
					ТекстКомментария = НСтр("ru='У ';uk='У '") + ФИО + НСтр("ru=' в индивидуальном графике за ';uk=' в індивідуальному графіку за '") + Формат(СтрокаДвижений.ПериодДействия, "ДФ='MMММ yyyy ""г.""'") + НСтр("ru=' нет ';uk=' немає '") + ТекстВидаВремени + НСтр("ru='! Необходимо заполнить ';uk='! Необхідно заповнити '");
				КонецЕсли;
...
Процедура ПроставитьДанныеСтроки(Сотрудник, Назначение, ДатаСобытия, ПериодРегистрации, ВидРасчета, ТекущаяСтрока, ЭтоНачисление = Истина) Экспорт
	...
	|ГДЕ
	|	ГрафикиРаботыПоВидамВремени.ГрафикРаботы = &парамСотрудник
	|	И ГрафикиРаботыПоВидамВремени.Сотрудник = &парамНазначение
	|	И ГрафикиРаботыПоВидамВремени.Месяц = НАЧАЛОПЕРИОДА(&парамДатаСобытия, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник,
	|	ГрафикРаботы,
	...
	Запрос.Текст = ТекстЗапроса;
	Запрос.Выполнить();
	Запрос.Текст = "ВЫБРАТЬ * ИЗ ВТГрафикиРаботыПоВидамВремени";
	
	//ВТГрафикиРаботыПоВидамВремениНормы
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ГрафикиРаботы.Сотрудник		КАК Сотрудник,
	|	ГрафикиРаботы.ГрафикРаботы	КАК ГрафикРаботы,
	|	ГрафикиРаботы.Документ		КАК Документ,
	|	ГрафикиРаботы.ОсновноеЗначение	КАК ОсновноеЗначение,
	|	ГрафикиРаботы.ДополнительноеЗначение КАК ДополнительноеЗначение
	|ПОМЕСТИТЬ ВТГрафикиРаботыПоВидамВремениНормы
	...
	Запрос.Текст = ТекстЗапроса;
	Запрос.Выполнить();
	Запрос.Текст = "ВЫБРАТЬ * ИЗ ВТГрафикиРаботыПоВидамВремениНормы";
	
	//ВТСводныеИндивидуальныеГрафики
	ТекстЗапроса = "ВЫБРАТЬ
	|	СводныеИндивидуальныеГрафики.Дней КАК Дней,
	...
	|ГДЕ
	|	СводныеИндивидуальныеГрафики.Сотрудник В(&парамСотрудник)
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ ВТГрафикиРаботыПоВидамВремени КАК ГрафикРаботыПоСотруднику
	|ПО ИСТИНА
	|       
	|
	|//-----------------------
	...
	|ИЗ ВТГрафикиРаботыПоВидамВремени КАК ГрафикиРаботы
	|	ГДЕ		
	...
	|		СУММА( ГрафикиРаботы.ОсновноеЗначение  )		КАК НормаДнейЗаМесяц,
	|		СУММА( ГрафикиРаботы.ДополнительноеЗначение )	КАК НормаЧасовЗаМесяц
	|	ИЗ ВТГрафикиРаботыПоВидамВремениНормы КАК ГрафикиРаботы
	|	СГРУППИРОВАТЬ ПО
	|		ГрафикиРаботы.ГрафикРаботы,
	|		ГрафикиРаботы.Сотрудник,
	|		ГрафикиРаботы.Документ
	...
	| УПОРЯДОЧИТЬ ПО Оклады.ДатаНачала УБЫВ
	|";
...
Функция ГрафикиИНормыПоСотрудникам(Организация, СотрудникиИНазначения, ПериодРегистрации) Экспорт
	...
	|	РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	|ПО
	|	ГрафикиРаботыПоВидамВремени.ГрафикРаботы = СотрудникиИНазначения.Сотрудник
	|	И ГрафикиРаботыПоВидамВремени.Сотрудник = СотрудникиИНазначения.Назначение
	|ГДЕ
	|	ГрафикиРаботыПоВидамВремени.Месяц = НАЧАЛОПЕРИОДА(&парамПериодРегистрации, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник,
	|	ГрафикРаботы,
	...
	|ПОМЕСТИТЬ ВТГрафикиРаботыПоВидамВремениНормы
	|	ИЗ	РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботы
	|	ГДЕ		НАЧАЛОПЕРИОДА( ГрафикиРаботы.Дата, МЕСЯЦ ) = НАЧАЛОПЕРИОДА( &парамПериодРегистрации, МЕСЯЦ ) 
	|		И	ГрафикиРаботы.ВидУчетаВремени = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаВремени.ПоДням) 
	|		И НЕ ГрафикиРаботы.Документ ССЫЛКА Документ.ТабельУчетаРабочегоВремениОрганизации
	|		И НЕ ГрафикиРаботы.Документ ССЫЛКА Документ.СдельныйНаряд
	|		И НЕ ГрафикиРаботы.Документ ССЫЛКА Документ.ВводИндивидуальныхГрафиковРаботыОрганизации 
	...
	|	РегистрСведений.СводныеИндивидуальныеГрафики КАК СводныеИндивидуальныеГрафики
	|ПО
	|	СводныеИндивидуальныеГрафики.Сотрудник = СотрудникиИНазначения.Сотрудник
	|	И СводныеИндивидуальныеГрафики.Назначение = СотрудникиИНазначения.Назначение
	|ГДЕ
	|	СводныеИндивидуальныеГрафики.Месяц = НАЧАЛОПЕРИОДА(&парамПериодРегистрации, МЕСЯЦ)
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник,
	|	Назначение";
	Запрос.Текст = ТекстЗапроса;
	...
	|	СотрудникиИНазначения.Назначение КАК Назначение,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ГрафикРаботыПоСотруднику.Сотрудник,ЗНАЧЕНИЕ(Справочник.СотрудникиОрганизаций.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СотрудникиОрганизаций.ПустаяСсылка)
	|			ТОГДА Сотрудники.ГрафикРаботы 
	|			ИНАЧЕ Сотрудники.Сотрудник
	|	КОНЕЦ КАК ГрафикРаботы,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеИндивидуальногоКалендаря.НормаДнейЗаМесяц, 0)<> 0 
	|			ТОГДА ДанныеИндивидуальногоКалендаря.НормаДнейЗаМесяц 
	|		КОГДА ЕСТЬNULL(СводныеИндивидуальныеГрафики.Дней, 0)<> 0 
	|			ТОГДА  СводныеИндивидуальныеГрафики.Дней
	|			ИНАЧЕ  ДанныеКалендаря.НормаДнейЗаМесяц
	|	КОНЕЦ	КАК НормаДнейЗаМесяц,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеИндивидуальногоКалендаря.НормаЧасовЗаМесяц, 0)<> 0 
	|			ТОГДА ДанныеИндивидуальногоКалендаря.НормаЧасовЗаМесяц 
	|		КОГДА ЕСТЬNULL(СводныеИндивидуальныеГрафики.Часов, 0)<> 0 
	|			ТОГДА  СводныеИндивидуальныеГрафики.Часов
	|		ИНАЧЕ  ДанныеКалендаря.НормаЧасовЗаМесяц
	|	КОНЕЦ КАК НормаЧасовЗаМесяц
	|ИЗ  ВТСотрудникиИНазначения КАК СотрудникиИНазначения
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|РегистрСведений.РаботникиОрганизаций.СрезПоследних(КОНЕЦПЕРИОДА(&парамПериодРегистрации, МЕСЯЦ), Сотрудник В (ВЫБРАТЬ СотрудникиИНазначения.Назначение
	|																									ИЗ ВТСотрудникиИНазначения КАК СотрудникиИНазначения)) КАК Сотрудники
	|ПО СотрудникиИНазначения.Назначение = Сотрудники.Сотрудник
	|
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТГрафикиРаботыПоВидамВремени КАК ГрафикРаботыПоСотруднику
	|	ПО ГрафикРаботыПоСотруднику.ГрафикРаботы = СотрудникиИНазначения.Сотрудник
	|  	 И ГрафикРаботыПоСотруднику.Сотрудник = СотрудникиИНазначения.Назначение
	|
	|	// данные нормы календаря по индивидуальным графикам
	|	ЛЕВОЕ СОЕДИНЕНИЕ (
	|	ВЫБРАТЬ
	...
	|		ГрафикиРаботы.Документ
	|) КАК ДанныеИндивидуальногоКалендаря
	|	ПО ДанныеИндивидуальногоКалендаря.Сотрудник = СотрудникиИНазначения.Назначение
	|   И ДанныеИндивидуальногоКалендаря.ГрафикРаботы = СотрудникиИНазначения.Сотрудник
	|	// данные нормы календаря
	|ЛЕВОЕ СОЕДИНЕНИЕ (
	|	ВЫБРАТЬ
	|		ГрафикиРаботы.ГрафикРаботы	КАК ГрафикРаботы,
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТСводныеИндивидуальныеГрафики  КАК СводныеИндивидуальныеГрафики
	|	ПО СводныеИндивидуальныеГрафики.Сотрудник = Сотрудники.Сотрудник
	|	 И СводныеИндивидуальныеГрафики.Назначение = СотрудникиИНазначения.Назначение
	|";          
	Запрос.Текст = ТекстЗапроса;
...
КонецФункции
 ...
 // Функция возвращает дату ввода раздела по налогу на прибыль в Налоговом кодексе
//
Функция ДатаВводаНПНК() Экспорт
	
	Возврат Дата(2011,4,1);
...
Функция АвтозаполнениеРаботникиОрганизации( Регистратор, ПроверятьДубли = Истина, ЭтоДоговорники = Ложь, ЭтоУволенные = Ложь) Экспорт
	...
	ЗаполнятьДокументыБезУчетаИерархииПодразделений = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекущийПользователь, "ЗаполнятьДокументыБезУчетаИерархииПодразделений");
	...
	Иначе
		...
		ПоВсемПодразделениямОрганизации = Ложь;
		Если ЗаполнятьДокументыБезУчетаИерархииПодразделений Тогда
			УсловиеНаПодразделение = "ПодразделениеОрганизации В (&парамПодразделение)";
		Иначе	
...
Процедура АвтозаполнениеВзносы( Регистратор, Взносы, ЭтоВзносы, Сотрудники, ПоРегистратору = Ложь ) Экспорт
	...
	Если НужноРасчитыватьКоэффициент Тогда
		...
		|	  ТОГДА 0
		|	 КОГДА Основной.Сотрудник.ДатаПриемаНаРаботу > НАЧАЛОПЕРИОДА(Основной.БазовыйПериодНачало, МЕСЯЦ)
		|	   ИЛИ (КОНЕЦПЕРИОДА(Основной.Сотрудник.ДатаУвольнения,ДЕНЬ) < КОНЕЦПЕРИОДА(Основной.БазовыйПериодКонец, МЕСЯЦ)
	   	|	    	И Основной.Сотрудник.ДатаУвольнения <> ДАТАВРЕМЯ(1,1,1))
		|	  ТОГДА 0
...
Процедура АвтозаполнениеНДФЛ( Регистратор, НДФЛ, Сотрудники, ПоРегистратору = Ложь, СНачалаГода = Ложь, ГодЗаполнения = 0 ) Экспорт
	...
	Если Истина Тогда
		...
		|		И	Начисления.ВидРасчета.ДоходНДФЛ <> ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.ВоенныйСбор109)
		|           " + ?(ПоРегистратору, "И	Начисления.Регистратор = &парамРегистратор", "") + "
		|
		|	СГРУППИРОВАТЬ ПО
		|		Начисления.Сотрудник,
	...
	Пока ИсходныеДанные.Следующий() Цикл
		...
		Если (НеобходимыеДанные.ВТабличнойЧастиЕстьУчетВремени И ОсновныеНачисленияОрганизаций ИЛИ УправленческиеНачисления) И СтрокаДвижений.Авторасчет Тогда
			...
			Если ИсходныеДанные.ЗачетОтработанногоВремени Тогда
				...
				Иначе
					ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + СтрокаДвижений.ВидРасчета + НСтр("ru=': Не задан вид учета времени';uk=': Не заданий вид обліку часу'"));
			...
			Если ОсновныеНачисленияОрганизаций И ИсходныеДанные.ЗачетНормыВремени Тогда
				...
				Если ИсходныеДанные.НормаОплачиваемыхЧасов = NULL Тогда
					Если Не БылоСообщениеОРабочемВремени Тогда
						ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет рабочих дней в заданном периоде';uk=': Немає робочих днів у заданому періоді'"));
					КонецЕсли;
				...
				ИначеЕсли Не БылоСообщениеОВидеВремени Тогда
					ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не задан вид учета времени';uk=': Не заданий вид обліку часу'"));
				КонецЕсли;
			КонецЕсли;
			...
			Если ОсновныеНачисленияОрганизаций И ИсходныеДанные.ЗачетКалендарных Тогда
				...
				Если СтрокаДвижений.Назначение = СтрокаДвижений.Сотрудник Тогда
					СтрокаДвижений.КалендарныеДни = ?(ИсходныеДанные.ВидРасчета.ВидОтпуска.ВключатьПраздники,ИсходныеДанные.КалендарныеДниСПраздниками,ИсходныеДанные.КалендарныеДни) * ?(СтрокаДвижений.Сторно,-1,1);
				КонецЕсли;
			КонецЕсли;
...
Процедура РассчитатьЗаписиНабораСторноВзносовВФонды(ИсходныеДанныеСторно,
	...
	Пока ИсходныеДанныеСторно.Следующий() Цикл
		...
		Если КомментироватьРасчет Тогда
			...
			КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет ';uk='Розрахунок '")+ИсходныеДанныеСторно.Налог+НСтр("ru=' за ';uk=' за '") + Формат(ИсходныеДанныеСторно.БазовыйПериодНачало,"ДФ='МММ гггг'"), , ИсходныеДанныеСторно.ВидРасчетаНаименование, ИсходныеДанныеСторно.ВидРасчета);
			КомментарийБазы = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База сторно: ';uk='База сторно: '") + ИсходныеДанныеСторно.База, КомментарийВидаРасчета,,,, Ложь);
			КомментарийБазы = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База с ограничением сторно: ';uk='База з обмеженням сторно: '") + ИсходныеДанныеСторно.БазаВзноса, КомментарийВидаРасчета,,,, Ложь);
			ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма взноса сторно : ';uk='Сума внеска сторно : '") + ИсходныеДанныеСторно.Результат, КомментарийВидаРасчета);
...
Процедура РассчитатьЗаписьРегистраРасчета(СтрокаДвижений,
	...
	Если СтрокаДвижений.Авторасчет Тогда
		...
		Если КомментироватьРасчет Тогда
			Если ВзносыВФонды Тогда
				КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет ';uk='Розрахунок '")+СтрокаДвижений.Налог+НСтр("ru=' за ';uk=' за '") + Формат(СтрокаДвижений.БазовыйПериодНачало,"ДФ='МММ гггг'"), , ИсходныеДанные.ВидРасчетаНаименование, СтрокаДвижений.ВидРасчета);
			Иначе
				КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет ';uk='Розрахунок '"), , ИсходныеДанные.ВидРасчетаНаименование, СтрокаДвижений.ВидРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Способ расчета: ';uk='Спосіб розрахунку: '") + ИсходныеДанные.СпособРасчета, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Порядок расчета: ';uk='Порядок розрахунку: '") + ВизуализироватьФормулуРасчета(ИсходныеДанные.ВидРасчета), КомментарийВидаРасчета);
			КонецЕсли;
		КонецЕсли;
		...
		Если СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Процентом Тогда
			...
			Если ИсходныеДанные.РезультатБаза = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определены базовые начисления ';uk=': Не визначені базові нарахування'"));
			Иначе
				Если КомментироватьРасчет Тогда
					КомментарийБазы = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовые начисления составили всего: ';uk='Базові нарахування склали всього: '") + ИсходныеДанные.РезультатБаза, КомментарийВидаРасчета,,,, Ложь);
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Процент: ';uk='Відсоток: '") + СтрокаДвижений[ПоказательРазмер], КомментарийВидаРасчета);
				КонецЕсли;
			КонецЕсли;
			...
			Если ТипЗнч(СтрокаДвижений.ДокументОснование) = Тип("ДокументСсылка.ДоговорНаВыполнениеРаботСФизЛицом") Тогда
				Если СтрокаДвижений.ДокументОснование.ХарактерОплаты = Перечисления.ХарактерВыплатыПоДоговору.ПоСдельномуНаряду  Тогда
					Если ИсходныеДанные.РазмерСдельногоЗаработка = NULL Тогда
						ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': нет данных о фактической выработке ';uk=': немає даних про фактичний виробіток'"));
					Иначе	
				...
				КонецЕсли;
			Иначе
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': оплата не должна производиться без договора подряда ';uk=': оплата не повинна проводитися без договору підряду '"));
			КонецЕсли;
		...
		ИначеЕсли
			...
			Если НормаВремени <> 0 Тогда
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Тарифная ставка: ';uk='Тарифна ставка: '") + СтрокаДвижений[ПоказательРазмер], КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Норма времени: ';uk='Норма часу: '") + НормаВремени, КомментарийВидаРасчета);
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Тарифная ставка: ';uk='Тарифна ставка: '") + СтрокаДвижений[ПоказательРазмер], КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Отработанное время: ';uk='Відпрацьований час: '") + ОтработаноВремениОсновное, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Процент: ';uk='Відсоток: '") + СтрокаДвижений.Показатель2, КомментарийВидаРасчета);
			КонецЕсли;
		...
		ИначеЕсли 	СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаВечерниеЧасы Тогда
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент вечерних: ';uk='Коефіцієнт вечірніх: '") + КоэффициентВечерних, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Часовой тариф: ';uk='Часовий тариф: '") + ЧасовойТариф, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Отработанное время: ';uk='Відпрацьований час: '") + ОтработаноВремениОсновное, КомментарийВидаРасчета);
			КонецЕсли;
		...
		ИначеЕсли 	СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаНочныеЧасы Тогда
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент ночных: ';uk='Коефіцієнт нічних: '") + КоэффициентНочных, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Часовой тариф: ';uk='Часовий тариф: '") + ЧасовойТариф, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Отработанное время: ';uk='Відпрацьований час: '") + ОтработаноВремениОсновное, КомментарийВидаРасчета);
			КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПочтовыйСбор Тогда
			...
			Если ИсходныеДанные.РазмерПочтовогоПеревода = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определен размер почтового переода  ';uk=': Не визначено розмір поштового переказу '"));
			...
			ИначеЕсли ИсходныеДанные.Ставка = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определена ставка почтового сбора  ';uk=': Не визначена ставка поштового збору '"));
			Иначе
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияАлиментов Тогда
			...
			Если ИсходныеДанные.БазаИндексации = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определен размер алиментов для индексации  ';uk=': Не визначений розмір аліментів для індексації '"));
			...
			Иначе
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовый месяц: ';uk='Базовий місяць: '") + БазовыйМесяц, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент индексации: ';uk='Коефіцієнт індексації: '") + КоэффициентИндексации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База: ';uk='База: '") + ИсходныеДанные.БазаИндексации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Прожиточный минимум: ';uk='Прожитковий мінімум: '") + ИсходныеДанные.АлиментыТрудоспособные, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База индексации: ';uk='База індексації: '") + РезультатБаза, КомментарийВидаРасчета);
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ВыслугаЛет Тогда
			...
			Если ИсходныеДанные.РезультатБаза = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определена база ';uk=': Не визначена база '"));
			...
			Иначе
					...
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка выслуги: ';uk='Ставка вислуги: '") + ИсходныеДанные.СтавкаВыслуги, КомментарийВидаРасчета);
				КонецЕсли;
				...
				Если ИсходныеДанные.Ограничение>0 Тогда
					//С ограничением
					Если КомментироватьРасчет Тогда
						ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ограничение: ';uk='Обмеження: '") + ИсходныеДанные.Ограничение, КомментарийВидаРасчета);
					КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.АлиментыПроцентом Тогда
			...
			// Размер - процент удержания
			Если ИсходныеДанные.РезультатБаза = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определены базовые начисленияя ';uk=': Не визначені базові нарахування'"));
			ИначеЕсли ИсходныеДанные.ДетейВДокументеВсего = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': В исполнительном листе на уплату алементов не указанны данные о детях ';uk=': У виконавчому листі на сплату аліментів не вказані дані про дітей '"));
			Иначе
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.АлиментыПроцентомДоПредела Тогда
			...
			Если ИсходныеДанные.РезультатБаза = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не определены базовые начисленияя ';uk=': Не визначені базові нарахування'"));
			ИначеЕсли ИсходныеДанные.ДетейВДокументеВсего = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': В исполнительном листе на уплату элементов не указанны данные о детях ';uk=': У виконавчому листі на сплату елементів не вказані дані про дітей '"));
			Иначе
		...
		ИначеЕсли
			...
			Если НЕ СтрокаДвижений.Сторно Тогда 
				Если ИсходныеДанные.СреднийЗаработок = NULL 
					...
					Тогда
					ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных для расчета среднего заработка ';uk=': Немає даних для розрахунку середнього заробітку '"));
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоСреднегоЗаработка Тогда
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма средней: ';uk='Сума середньої: '") + ЗаработокЗаЕдиницуВремени, КомментарийВидаРасчета);
				Если ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимДням Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Оплачиваемое время (в рабочих днях): ';uk='Оплачуваний час (у робочих днях): '") + ОплачиваемоеВремя, КомментарийВидаРасчета);
				ИначеЕсли ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимЧасам Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Оплачиваемое время (в рабочих часах): ';uk='Оплачуваний час (в робочих годинах): '") + ОплачиваемоеВремя, КомментарийВидаРасчета);
				ИначеЕсли ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоКалендарнымДням Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Оплачиваемое время (в календарных днях): ';uk='Оплачуваний час (у календарних днях): '") + ОплачиваемоеВремя, КомментарийВидаРасчета);
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ВыходноеПособие 
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма средней: ';uk='Сума середньої: '") + ЗаработокЗаМесяц, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Количество месяцев, за которые оплачивается пособие: ';uk='Кількість місяців, за які сплачується допомога: '") + КоличествоМесяцев, КомментарийВидаРасчета);
			КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуКомпенсацияОтпуска 
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма средней: ';uk='Сума середньої: '") + ЗаработокЗаМесяц, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Время, за которые компенсируется отпуск: ';uk='Час, за який компенсується відпустка: '") + КалендарныхДней, КомментарийВидаРасчета);
			КонецЕсли;
		...
		ИначеЕсли  СпособРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПоСреднему  
			...
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма средней: ';uk='Сума середньої: '") + ЗаработокЗаЕдиницуВремени, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Процент оплаты: ';uk='Відсоток оплати: '") + ПроцентОплаты, КомментарийВидаРасчета);
				Если ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимДням Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Оплачиваемое время (в рабочих днях): ';uk='Оплачуваний час (у робочих днях): '") + ОплачиваемоеВремя, КомментарийВидаРасчета);
				ИначеЕсли ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимЧасам Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Оплачиваемое время (в рабочих часах): ';uk='Оплачуваний час (в робочих годинах): '") + ОплачиваемоеВремя, КомментарийВидаРасчета);
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.КомпенсацияНевыплаченнойЗарплаты Тогда
			...
			Если ИсходныеДанные.СуммаКомпенсации = NULL Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': в базовом периоде нет задолженности перед работником';uk=': у базовому періоді немає заборгованості перед працівником'"));
			Иначе	
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База компенсации: ';uk='База компенсації: '")+ИсходныеДанные.СуммаКомпенсации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='КоэффициентКомпенсации: ';uk='КоэффициентКомпенсации: '")+КоэффициентКомпенсации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Период расчета коєффициента: (';uk='Період розрахунку коефіціента: ('")+Формат(Начало,"ДФ=""ММММ гггг 'г.' """)+", "+Формат(Конец,"ДФ=""ММММ гггг 'г.' """)+")", КомментарийВидаРасчета);
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияЗарплаты Тогда
			...
			Если СтрокаДвижений.Назначение.ВидЗанятости = Перечисления.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство Тогда
				Результат = 0;	
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='По внутреннему совместителю индексация не начисляется. Расчет индексации производится только по основному месту работы!';uk='По внутрішньому суміснику індексація не нараховується. Розрахунок індексації проводиться тільки за основним місцем роботи!'"));
				КонецЕсли;
				
			ИначеЕсли РезультатПолнаяБаза = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных о базе индексации ';uk=': Немає даних про базу індексації '"));
			...
			ИначеЕсли СтрокаДвижений.Сотрудник.ВидЗанятости = Перечисления.ВидыЗанятостиВОрганизации.Совместительство
				...
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет сведений о базе индексации с основного места работы ';uk=': Немає відомостей про базу індексації з основного місця роботи '"));
			Иначе	
				...
				Если ИсходныеДанные.СпособОсновногоРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоМесячнойТарифнойСтавкеПоЧасам
					...
					или ИсходныеДанные.ОсновнойГрафикРаботы.НеполныйРабочийДень Тогда
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База индексации:';uk='База індексації:'") + РезультатБаза, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовый период: ';uk='Базовий період: '") + БазовыйМесяц, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент индексации: ';uk='Коефіцієнт індексації: '") + КоэффициентИндексации, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Размер фиксированной индексации: ';uk='Розмір фіксованої індексації: '") + ИсходныеДанные.РезультатФиксированнаяИндексация, КомментарийВидаРасчета);
					...
					Если КоэффициентИндексацииПропорционально > 0 И КоэффициентИндексацииПропорционально < 1 Тогда
						Если ИсходныеДанные.ВидУчетаВремени = Перечисления.ВидыУчетаВремени.ПоЧасам Тогда
							КомментарийКоэффициентИндексацииПропорционально = " (" + Окр(ИсходныеДанные.ОтработаноЧасовБаза, 0) + "/" + ИсходныеДанные.ДополнительноеЗначениеНормаВремениЗаМесяц + ")";
						Иначе
							КомментарийКоэффициентИндексацииПропорционально = " (" + Окр(ИсходныеДанные.ОтработаноДнейБаза, 0) + "/" + ИсходныеДанные.НормаВремениЗаМесяц + ")";
						КонецЕсли	
					КонецЕсли;	
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент индексации пропорционально: ';uk='Коефіцієнт індексації пропорційно: '") + Окр(КоэффициентИндексацииПропорционально, 4)+КомментарийКоэффициентИндексацииПропорционально, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Фиксированная индексация: ';uk='Фіксована індексація: '") + РезультатФиксированнаяИндексация, КомментарийВидаРасчета);
				КонецЕсли;
				
				Если ТипЗнч(ИсходныеДанные.Регистратор) = Тип("ДокументСсылка.ИзменениеПараметровРасчетаИндексации") Тогда
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияСоциальныхНачислений Тогда
			...
			Если РезультатПолнаяБаза = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных о базе ';uk=': Немає даних про базу '"));
			Иначе
				...
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База индексации соц.начислений:';uk='База індексації соц.нарахувань:'") + РезультатБаза, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовый период соц.начислений: ';uk='Базовий період соц.нарахувань: '") + Начало, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент индексации соц.начислений: ';uk='Коефіцієнт індексації соц.нарахувань: '") + КоэффициентИндексации, КомментарийВидаРасчета);
				КонецЕсли;
				
				Результат  = РезультатБаза * КоэффициентИндексации;
			КонецЕсли;
			
			// ДоначислениеОтОбратного
			//
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоначислениеОтОбратного Тогда
			
			Если ИсходныеДанные.РезультатБаза = NULL Тогда
				СуммаОтОбратного = РасчетОтОбратного(СтрокаДвижений[ПоказательРазмер],ИсходныеДанные);
				Результат = СуммаОтОбратного;
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма ""чистая"": ';uk='Сума ""чиста"": '") + СтрокаДвижений[ПоказательРазмер], КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма ""от обратного"": ';uk='Сума ""від зворотного"": '") + СуммаОтОбратного, КомментарийВидаРасчета);
				КонецЕсли;
			Иначе
				Если КомментироватьРасчет Тогда
					КомментарийБазы = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовые начисления составили всего: ';uk='Базові нарахування склали всього: '") + ИсходныеДанные.РезультатБаза, КомментарийВидаРасчета,,,, Ложь);
				...
				СуммаОтОбратного = РасчетОтОбратного(СтрокаДвижений[ПоказательРазмер],ИсходныеДанные);
				Результат = Макс(СуммаОтОбратного-ИсходныеДанные.РезультатБаза,0);
				Если КомментироватьРасчет Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма ""чистая"": ';uk='Сума ""чиста"": '") + СтрокаДвижений[ПоказательРазмер], КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма ""от обратного"": ';uk='Сума ""від зворотного"": '") + СуммаОтОбратного, КомментарийВидаРасчета);
				КонецЕсли;
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоМЗП Тогда
			...
			Если РезультатБаза = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных о базе для доначисления до минимальной зарплаты ';uk=': Немає даних про базу для донарахування до мінімальної зарплати '"));
				
			ИначеЕсли МинимальнаяЗарплата = 0 Тогда
				ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных о размере минимальной зарплаты ';uk=': Немає даних про розмір мінімальної зарплати '"));
			...
			Иначе	
				...
				Если КомментироватьРасчет Тогда
					...
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Минимальная зарплата: ';uk='Мінімальна зарплата: '") + МинимальнаяЗарплата, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Минимальная зарплата пропорционально: ';uk='Мінімальна зарплата пропорційно: '") + МинимальнаяЗарплатаПропорционально, КомментарийВидаРасчета);
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПроизвольнаяФормула Тогда
			...
			Для НомерСтроки = 1 По ИсходныеДанные.КоличествоПоказателей Цикл
				Если ИсходныеДанные["Показатель"+НомерСтроки] = Null Тогда
					ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': не получен Показатель ""';uk=': не отриманий Показник ""'") + ИсходныеДанные["Показатель"+НомерСтроки+"Наименование"] + """");
					РезультатПолучен = Ложь;
				ИначеЕсли КомментироватьРасчет Тогда
					Если ИсходныеДанные["Показатель"+НомерСтроки+"Наименование"] = "Расчетная база" ИЛИ ИсходныеДанные["Показатель"+НомерСтроки+"Наименование"] = "Розрахункова база" Тогда
			...
			Если РезультатПолучен Тогда
				...
				Исключение					
					ОбщегоНазначения.КомментарийРасчета(ИсходныеДанные.СотрудникНаименование + "; " +ИсходныеДанные.ВидРасчетаНаименование +НСтр("ru=' Ошибка в формуле расчета!';uk=' Помилка у формулі розрахунку!'"), КомментарийВидаРасчета, ,,Перечисления.ВидыСообщений.Ошибка);
				КонецПопытки;
			КонецЕсли;			
			...
			Если СтрокаДвижений.ВидРасчета.СтавкаПоПериодуРегистрации Тогда
				...
				Если НЕ СтрокаДвижений.Сторно Тогда
					...
					Если ИсходныеДанные.РезультатБаза = NULL И НЕ (СтрокаДвижений.БазовыйПериодНачало < СтрокаДвижений.ПериодРегистрации И БазаДо <> 0 И БазаПолная <> 0) Тогда
						...
						Иначе	 
							ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Не удалось определить базу взноса ';uk=': Не вдалося визначити базу внеску '"));
					...
					ИначеЕсли ИсходныеДанные.Предел = NULL Тогда
						ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных о пределе ';uk=': Немає даних про межу '"));
					...
					ИначеЕсли (База - БазаДо) <> 0 И (ИсходныеДанные.Ставка = Неопределено ИЛИ ИсходныеДанные.Ставка = NULL) Тогда
						ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': Нет данных о ставке ';uk=': Немає даних про ставку '"));
						СтрокаДвижений.База			= 0;
						СтрокаДвижений.БазаВзноса	= 0;
						СтрокаДвижений.БазаВычет	= 0;
					...
					Иначе
						...
						Если КомментироватьРасчет Тогда
							...
							КомментарийБазы = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовые начисления составили всего: ';uk='Базові нарахування склали всього: '") + БазаВсего, КомментарийВидаРасчета,,,, Ложь);
							РасшифровкаБазы = ИсходныеДанные.Выбрать();
							Пока РасшифровкаБазы.Следующий() Цикл
								...
								КонецЕсли;
							КонецЦикла;
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База расчетная: ';uk='База розрахункова: '") + База, КомментарийВидаРасчета,,,, Ложь);
							Если БазаДо <> 0 Тогда 
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База до: ';uk='База до: '") + БазаДо, КомментарийВидаРасчета,,,, Ложь);
							Конецесли;	
							Если БазаВычет <> 0 Тогда 
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База вычет: ';uk='База відрахування: '") + БазаВычет, КомментарийВидаРасчета,,,, Ложь);
							КонецЕсли;	
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База с учетом ограничений: ';uk='База з врахуванням обмежень: '") + БазаВзноса, КомментарийВидаРасчета,,,, Ложь);
							Если БазаДоначисление <> 0 Тогда
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	в т.ч. доначисление до минимальной базы : ';uk='	в т.ч. донарахування до мінімальної бази : '") + БазаДоначисление, КомментарийВидаРасчета);
							КонецЕсли;
							Если	СтрокаДвижений.ВидРасчета.СпособРасчетаПоШкале = Перечисления.СпособыРасчетаПоШкале.Прогрессивный Тогда 
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет по прогрессивной шкале (Нижний порог, Порог): (';uk='Розрахунок по прогресивній шкалі (Нижній поріг, Поріг): ('")+НижнийПорог+", "+Порог+")", КомментарийВидаРасчета);
							КонецЕсли;
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма предела : ';uk='Сума межі : '") + Предел, КомментарийВидаРасчета);
							Если МинимальнаяБаза <> 0 Тогда
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Минимальная база : ';uk='Мінімальна база : '") + МинимальнаяБаза, КомментарийВидаРасчета);
							КонецЕсли;
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка : ';uk='Ставка : '") + Ставка, КомментарийВидаРасчета);
							Если КоэффициентСтавки <> 0 И КоэффициентСтавки <> 1 Тогда
								 ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент ставки : ';uk='Коефіціент ставки : '") + КоэффициентСтавки, КомментарийВидаРасчета);
								 ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка с учетом коэф. : ';uk='Ставка з урахуванням коеф. : '") + Окр(Ставка * КоэффициентСтавки,5), КомментарийВидаРасчета);
							КонецЕсли;	
							
							Если Коэф <> 1 Тогда
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Коэффициент : ';uk='	Коефіцієнт : '") + Коэф, КомментарийВидаРасчета);
							КонецЕсли;
						КонецЕсли;
				...
				Если НЕ СтрокаДвижений.Сторно Тогда
					...
					Иначе
						...
						Если КомментироватьРасчет Тогда
							...
							КомментарийБазы = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Базовые начисления составили всего: ';uk='Базові нарахування склали всього: '") + База, КомментарийВидаРасчета,,,, Ложь);
							РасшифровкаБазы = ИсходныеДанные.Выбрать();
							Пока РасшифровкаБазы.Следующий() Цикл
								Если РасшифровкаБазы.РезультатБаза <> 0 Тогда
									ОбщегоНазначения.КомментарийРасчета(РасшифровкаБазы.ВидРасчетаРазрезНаименование + ": " + РасшифровкаБазы.РезультатБаза, КомментарийБазы);
							...
							КонецЕсли;
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма предела : ';uk='Сума межі : '") + Предел, КомментарийВидаРасчета);
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма дохода за месяц: ';uk='Сума доходу за місяць: '") + БазаЗаМесяц, КомментарийВидаРасчета);
							ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма дохода за месяц с учетом предела : ';uk='Сума доходу за місяць із урахуванням межі: '") + БазаВзносаЗаМесяц, КомментарийВидаРасчета);
							Если Коэф <> 1 Тогда
								ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Коэффициент : ';uk='	Коефіцієнт : '") + Коэф, КомментарийВидаРасчета);
							КонецЕсли;
						КонецЕсли;
		...
		Иначе // Не найден способ расчета
			ОбщегоНазначения.ОшибкаРасчета(ИсходныеДанные.СотрудникНаименование + "; " + ИсходныеДанные.ВидРасчетаНаименование + НСтр("ru=': вид расчета не предусмотрен';uk=': вид розрахунку не передбачено'"));
		КонецЕсли;
		
		Если СтрокаДвижений.Сторно Тогда
			СтрокаДвижений.Результат = Результат * -1;
			Если КомментироватьРасчет Тогда
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет сторно';uk='Розрахунок сторно'"), КомментарийВидаРасчета, , , Перечисления.ВидыСообщений.ВажнаяИнформация);
			КонецЕсли;
		...
		Если КомментироватьРасчет Тогда
			ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Результат расчета: ';uk='Результат розрахунку: '") + СтрокаДвижений.Результат, КомментарийВидаРасчета);
			Если	СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Взносы 
			ИЛИ	СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ВзносыФОТ Тогда
				Если СтрокаДвижений.РезультатДоначисление <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	в т.ч. на сумму доначисления до минимальной базы: ';uk='	в т.ч. на суму донарахування до мінімальної бази: '") + СтрокаДвижений.РезультатДоначисление, КомментарийВидаРасчета);
				КонецЕсли;
			КонецЕсли;	
		КонецЕсли;
...
Функция ПолучитьСтруктуруНеобходимыхДанных(МассивСпособРасчета, МассивПоказателей, НеобходимыеДанные = "" )
	...
	НеобходимыеДанные.Вставить("ПараметрыНалогов", Ложь);
	...
	Для Каждого СпособРасчета Из МассивСпособРасчета Цикл
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоначислениеОтОбратного Тогда
			НеобходимыеДанные.База       = Истина;
			НеобходимыеДанные.ПараметрыНалогов = Истина;	
	...
	Для Каждого Показатель Из МассивПоказателей Цикл
		...
		НеобходимыеДанные.МинимальнаяЗарплата = НеобходимыеДанные.МинимальнаяЗарплата Или Показатель = Справочники.ПоказателиСхемМотивации.МинимальнаяЗарплата Или Показатель = Справочники.ПоказателиСхемМотивации.МинимальнаяЗарплатаЧасовая;
...
Функция ПолучитьДанныеДляРасчета(ИмяРегистра,
	...
	Если НеобходимыеДанные.НеобходимыИндексыИнфляции Тогда
		...
		Если Выборка.Количество()>0 Тогда
			...
			КомментарийОшибки = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Индексация зарплаты может рассчитаться неверно! Не установлена социальная группа у расчетов: ';uk='Індексація зарплати може бути розрахована не вірно! Не встановлено соціальну групу у розрахунків: '"), , Индексация.Наименование, Индексация, Перечисления.ВидыСообщений.ВажнаяИнформация);
			Пока Выборка.Следующий() Цикл
	...
	Если Истина Тогда
		...
		Если Истина Тогда
			...
			|			ТОГДА ОтработанноеВремя.Дней
			|		ИНАЧЕ 0
			|		КОНЕЦ) КАК ДнейПоТабелю,
			|	СУММА( ВЫБОР 
			|		КОГДА Основной.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени = ОтработанноеВремя.ВидИспользованияРабочегоВремени
			|			ТОГДА ОтработанноеВремя.Часов
			|		ИНАЧЕ 0
			|		КОНЕЦ) КАК ЧасовПоТабелю,
			|	МАКСИМУМ(ВЫБОР
			|			КОГДА ПроверкаКоличестваЗаписей.Сотрудник ЕСТЬ НЕ NULL 
		...
		Если Истина Тогда
			...
			|							И РабочееВремяРаботниковОрганизаций.ВидИспользованияРабочегоВремени В (&КлассификаторВЧ, &КлассификаторНЧ, &КлассификаторНУ, &КлассификаторРВ )
	...
	Если НеобходимыеДанные.ПоказателиСхемМотивации Тогда
		...
		Если НеобходимыеДанные.ОтработаноВремени Тогда
			...
			Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.ИзменениеПараметровРасчетаИндексации") Тогда
				...
				|								КОГДА ЕСТЬNULL(СводныеИндивидуальныеГрафики.ОшибкаВводаИндивидуальногоГрафикаРаботы, ЛОЖЬ)
				|									ТОГДА NULL
				|								ИНАЧЕ ЕСТЬNULL(СводныеИндивидуальныеГрафики.НормаВремени, ЕСТЬNULL(ДанныеГрафика.ОсновноеЗначениеПериодДействия, 0))
				|							КОНЕЦ)
		...
		Если НеобходимыеДанные.ПараметрыНалогов Тогда
			ТекстЗапроса = ТекстЗапроса + ", 
			...
			|	СтавкиНДФЛ3.Ставка КАК СтавкаВС,
			|	СтавкиНДФЛ2.Порог  КАК ПределНДФЛ";
			Если КомментироватьРасчет Тогда
				ПоляЗапросаМаксимум.Добавить("СтавкаЕСВ");
				ПоляЗапросаМаксимум.Добавить("ПределЕСВ");
				ПоляЗапросаМаксимум.Добавить("СтавкаНДФЛ1");
				ПоляЗапросаМаксимум.Добавить("СтавкаНДФЛ2");
				ПоляЗапросаМаксимум.Добавить("СтавкаВС");
				ПоляЗапросаМаксимум.Добавить("ПределНДФЛ");
			КонецЕсли;
		КонецЕсли;
			...
			Если КомментироватьРасчет Тогда
				...
				Если НеобходимыеДанные.ИндексацияЗарплаты ИЛИ НеобходимыеДанные.ДоплатаДоМЗП Тогда
					ТекстЗапроса = ТекстЗапроса + "
						...
						|	Регистратор КАК Регистратор,
						|	НомерСтроки КАК НомерСтроки
						|ИЗ РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций.БазаОсновныеНачисленияРаботниковОрганизаций(&парамИзмеренияОсновногоНазначения, &парамИзмеренияБазовогоНазначения, , " + Условие + ") Как База1
		...
		Если НеобходимГрафик Тогда
			...
			Запрос.УстановитьПараметр("КлассификаторНУ", Справочники.КлассификаторИспользованияРабочегоВремени.Сверхурочные);
			Запрос.УстановитьПараметр("КлассификаторРВ", Справочники.КлассификаторИспользованияРабочегоВремени.РаботаВыходныеИПраздничные);
		...
		Если НеобходимыеДанные.ВыслугаЛет Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|   И ВЫБОР КОГДА Стаж.ДнейСтажа ЕСТЬ NULL ТОГДА Стаж.ДатаПриема ИНАЧЕ ДОБАВИТЬКДАТЕ(&парамПериодРегистрации, ДЕНЬ, - Стаж.ДнейСтажа) КОНЕЦ <= ДОБАВИТЬКДАТЕ( НАЧАЛОПЕРИОДА(Основной.БазовыйПериодНачало), ГОД, - СтавкиВыслуги.НижнийПорог )
				|   И ВЫБОР КОГДА Стаж.ДнейСтажа ЕСТЬ NULL ТОГДА Стаж.ДатаПриема ИНАЧЕ ДОБАВИТЬКДАТЕ(&парамПериодРегистрации, ДЕНЬ, - Стаж.ДнейСтажа) КОНЕЦ > ДОБАВИТЬКДАТЕ( НАЧАЛОПЕРИОДА(Основной.БазовыйПериодНачало), ГОД, - СтавкиВыслуги.Порог )
				|
				|";
		КонецЕсли;
		...
		Если НеобходимыеДанные.ИндексацияЗарплаты Тогда
			...
			ПрименяетсяФиксированнаяИндексация = (ПроведениеРасчетов.ДатаВвводаФиксированнойИндексации() <= Регистратор.ПериодРегистрации);
			...
			ПрименяетсяИндексацияПоДожностям = (ПроведениеРасчетов.ДатаВвводаИндексацииПоДожностям() <= Регистратор.ПериодРегистрации);
			...
			Если ВариантыАлгоритмовРасчетаЗарплаты().ПорогИндексации103 = Перечисления.ВариантыАлгоритмовПорогИндексации103.ПрименятьПоИндексуИнфляции Тогда
				...
				ПорогИндексации = 0;
			Иначе	
				ПорогИндексации = ?(ПроведениеРасчетов.ДатаИзмененияПорогаИндексации() <= Регистратор.ПериодРегистрации, 1.03, 1.01);
			КонецЕсли;	
			...
			Если Истина Тогда
				...
				|ИЗ РегистрСведений.ПараметрыРасчетаИндексации Как ПараметрыРасчетаИндексации
				|ГДЕ 
				...
				|    И ПараметрыРасчетаИндексации.Должность = ЗНАЧЕНИЕ(Справочник.ДолжностиОрганизаций.ПустаяСсылка)
				|    И Основной.Сотрудник = ПараметрыРасчетаИндексации.Сотрудник
				|    И ВЫБОР КОГДА &ДатаИзмененияПорядкаИндексации  > НАЧАЛОПЕРИОДА(Основной.ДатаНачала, МЕСЯЦ)
				|   	ТОГДА КОНЕЦПЕРИОДА(Основной.ДатаНачала, МЕСЯЦ) >= ПараметрыРасчетаИндексации.Период
				|     	ИНАЧЕ Основной.ДатаНачала > ПараметрыРасчетаИндексации.Период КОНЕЦ
				...
				|   Основной.Сотрудник.ДатаПриемаНаРаботу КАК ДатаПриемаСотрудника
			...
			Если Истина Тогда
				...
				|	"+ ?(ПрименяетсяИндексацияПоДожностям, "ПараметрыФиксированнойИндексации.Должность","ЗНАЧЕНИЕ(Справочник.ДолжностиОрганизаций.ПустаяСсылка)")+ " КАК Должность,
				...
				|  И ПараметрыФиксированнойИндексации.Должность "+ ?(ПрименяетсяИндексацияПоДожностям, "<>","=")+ " ЗНАЧЕНИЕ(Справочник.ДолжностиОрганизаций.ПустаяСсылка)
				|ИНДЕКСИРОВАТЬ ПО
				|  Сотрудник
				|;
				|
				...
				|	 И "+ ?(ПрименяетсяИндексацияПоДожностям, "Основной.Должность","ЗНАЧЕНИЕ(Справочник.ДолжностиОрганизаций.ПустаяСсылка)")+ " = ПараметрыФиксированнойИндексации.Должность 
			...
			Если Истина Тогда
				...
				Если ПрименяетсяФиксированнаяИндексация Тогда
					...
					ТекстНачалоИндексации ="ВЫБОР КОГДА &ДатаВвводаФиксированнойИндексации <= БазовыеПериодыИндексации.ДатаПриемаСотрудника
											|	ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(БазовыеПериодыИндексации.ДатаПриемаСотрудника, МЕСЯЦ, -1), МЕСЯЦ)
											|	ИНАЧЕ БазовыеПериодыИндексации.ДатаПриемаСотрудника
											|	КОНЕЦ";
					Запрос.УстановитьПараметр("ДатаВвводаФиксированнойИндексации", ПроведениеРасчетов.ДатаВвводаФиксированнойИндексации());
				Иначе
		...
		Если НеобходимыеДанные.ПараметрыНалогов Тогда 
				...
				|ПО Основной.Сотрудник = ПараметрыНалоговЕСВ.Сотрудник
				...
				|ПО ИСТИНА
				|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНДФЛ.СрезПоследних(&парамПериодРегистрации,ВидСтавки = &парамВС) КАК СтавкиНДФЛ3
				|ПО ИСТИНА";
			КонецЕсли;
	...
	Если ИмяРегистра = "УдержанияРаботниковОрганизаций" Тогда
		...
		Если НеобходимаБаза Тогда
			...
			Если КомментироватьРасчет Тогда
				...
				|БазСКоментарием.ОтработаноДнейБаза КАК ОтработаноДнейБаза,
				|БазСКоментарием.ОтработаноЧасовБаза КАК ОтработаноЧасовБаза,
				|БазСКоментарием.ОплаченоДнейЧасовБаза КАК ОплаченоДнейЧасовБаза,
			...
			Иначе 
				ТекстЗапроса = ТекстЗапроса + ",
				|База.РезультатБаза КАК РезультатБаза,
				|База.ОтработаноДнейБаза КАК ОтработаноДнейБаза,
				|База.ОтработаноЧасовБаза КАК ОтработаноЧасовБаза,
		...
		Если НеобходимаБаза Тогда
			...
			|		СУММА(Основной.ОтработаноДнейБаза)		КАК ОтработаноДнейБаза,
			|		СУММА(Основной.ОтработаноЧасовБаза)		КАК ОтработаноЧасовБаза,
			|		СУММА(Основной.ОплаченоДнейЧасовБаза)	КАК ОплаченоДнейЧасовБаза
			|ПОМЕСТИТЬ ВТБазы	
			|		ИЗ (
			|		ВЫБРАТЬ 
			|			Основной.Регистратор КАК Регистратор, 
			...
			|			БазаИзНачислений.ОтработаноДней		КАК ОтработаноДнейБаза,
			|			БазаИзНачислений.ОтработаноЧасов	КАК ОтработаноЧасовБаза,
			|			БазаИзНачислений.ОплаченоДнейЧасов	КАК ОплаченоДнейЧасовБаза
			...
			Если КомментироватьРасчет Тогда
				...
				|			СУММА(БазаИзНачислений.Результат)			КАК РезультатБаза,
				|			СУММА(БазаИзНачислений.ОтработаноДней)		КАК ОтработаноДнейБаза,
				|			СУММА(БазаИзНачислений.ОтработаноЧасов)		КАК ОтработаноЧасовБаза,
				|			СУММА(БазаИзНачислений.ОплаченоДнейЧасов)	КАК ОплаченоДнейЧасовБаза 
	...
	Если ИмяРегистра = "ВзносыВФонды" Тогда
		...
		Если Истина Тогда 
			...
			|			СУММА(ВЫБОР КОГДА БазаИзНачислений.ВидРасчета.ЭтоОтпуск И БазаИзНачислений.ПериодРегистрации = &парамПериодРегистрации ТОГДА БазаИзНачислений.Результат ИНАЧЕ 0 КОНЕЦ)	КАК РезультатБазаОтпускные, 
			|			СУММА(БазаИзНачислений.Результат)		КАК РезультатБаза 
			|ПОМЕСТИТЬ ВТБазы	
			|		ИЗ РегистрРасчета.ВзносыВФонды КАК Основной
			| 
	...
	Если Истина Тогда 
		...
		Если Истина Тогда
			...
			Иначе //Перечисления.ВариантыАлгоритмовРасчетНДФЛ17.ОтдельныйРасчетБазы
				...
				|		Основной.ДоходНДФЛ    				КАК ДоходНДФЛ,
				|		Основной.НомерСтроки   				КАК НомерСтроки,
				|		БазовыеВидыРасчета.Ссылка			КАК ВидРасчетаЕСВ,
				|		СУММА( Начисления.Результат )		КАК Доход
				|ПОМЕСТИТЬ ВТДоходВРазрезе
				|	ИЗ	" + ВТСотрудникиПоНалоговымПериодамИСтрокам + " КАК Основной
				|	 
				|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций КАК Начисления
				|   ПО		Основной.Сотрудник = Начисления.Сотрудник
				...
				|		"+?(ТолькоПоДвижениямТекущегоДокумента,"И Начисления.Регистратор=&парамРегистратор","")+"
				|	ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовРасчета.ВзносыВФонды.БазовыеВидыРасчета КАК БазовыеВидыРасчета
				|	ПО		Начисления.ВидРасчета = БазовыеВидыРасчета.ВидРасчета
				|	 И		БазовыеВидыРасчета.Ссылка.ЕСВ
				|	 И		БазовыеВидыРасчета.Ссылка.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.Взносы)
				|
				|   СГРУППИРОВАТЬ ПО
				|		Основной.Сотрудник,
				|		Основной.НалоговыйПериод,
				...
				ТекстЗапросаВТ = "
				|  	ВЫБРАТЬ
				|		ЗапросПоВзносам.Сотрудник    				КАК Сотрудник,
				|		ЗапросПоВзносам.НалоговыйПериод    			КАК НалоговыйПериод,
				|		ЗапросПоВзносам.ДоходНДФЛ    				КАК ДоходНДФЛ,
				|		СУММА(ЗапросПоВзносам.СуммаВзносов)			КАК СуммаВзносов,
				|		СУММА(ЗапросПоВзносам.БазаВзносов)			КАК БазаВзносов,
				|		СУММА(ЗапросПоВзносам.СуммаВзносовБЛ)		КАК СуммаВзносовБЛ,
				|		СУММА(ЗапросПоВзносам.БазаВзносовБЛ)		КАК БазаВзносовБЛ
				|		
				|ПОМЕСТИТЬ ВТВзносы
                |   ИЗ(
				|  	ВЫБРАТЬ
				...
				|		Основной.НалоговыйПериод    		КАК НалоговыйПериод,
				|		Основной.ДоходНДФЛ    				КАК ДоходНДФЛ,
				|		ВЫБОР КОГДА Основной.КатегорияРасчета = ЗНАЧЕНИЕ(Перечисление.КатегорииРасчетов.Первичное) ИЛИ Основной.КатегорияРасчета = ЗНАЧЕНИЕ(Перечисление.КатегорииРасчетов.ЗависимоеПервогоУровня)
				|		 ТОГДА ВЫБОР КОГДА Основной.База <> 0 И Основной.Доход < Основной.База
				|					ТОГДА Основной.Взнос*Основной.Доход/Основной.База 
				|				КОГДА Основной.База <> 0 И Основной.Доход >= Основной.База
				|					ТОГДА Основной.Взнос 
				|				ИНАЧЕ 0 
				|			   КОНЕЦ
				|		 ИНАЧЕ 0 
				...
				|		ВЫБОР КОГДА Основной.КатегорияРасчета = ЗНАЧЕНИЕ(Перечисление.КатегорииРасчетов.Первичное) ИЛИ Основной.КатегорияРасчета = ЗНАЧЕНИЕ(Перечисление.КатегорииРасчетов.ЗависимоеПервогоУровня)
				|		 ТОГДА ВЫБОР КОГДА Основной.База <> 0 И Основной.Доход < Основной.База
				|					ТОГДА Основной.БазаВзноса*Основной.Доход/Основной.База 
				|				КОГДА Основной.База <> 0 И Основной.Доход >= Основной.База
				...
				|		ВЫБОР КОГДА Основной.КатегорияРасчета = ЗНАЧЕНИЕ(Перечисление.КатегорииРасчетов.ЗависимоеВторогоУровня)
				|		 ТОГДА ВЫБОР КОГДА Основной.База <> 0 И Основной.Доход < Основной.База
				|					ТОГДА Основной.Взнос*Основной.Доход/Основной.База 
				|				КОГДА Основной.База <> 0 И Основной.Доход >= Основной.База
				|					ТОГДА Основной.Взнос 
				...
				|		Основной.НалоговыйПериод    		КАК НалоговыйПериод,
				|		Основной.ДоходНДФЛ    				КАК ДоходНДФЛ,
				|		Основной.ВидРасчетаЕСВ.КатегорияРасчета	КАК КатегорияРасчета,
				|		Основной.Доход	    				КАК Доход,
				|		СУММА( ВзносыВФонды.База)			КАК База,
				|		СУММА( ВзносыВФонды.БазаВзноса)		КАК БазаВзноса,
				|		СУММА( ВзносыВФонды.Результат)		КАК Взнос
				|	ИЗ	ВТДоходВРазрезе КАК Основной
				|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВзносыВФонды КАК ВзносыВФонды
				|   ПО	Основной.Сотрудник = ВзносыВФонды.Сотрудник
				|	  И	Основной.НалоговыйПериод = НАЧАЛОПЕРИОДА( ВзносыВФонды.БазовыйПериодНачало, МЕСЯЦ )
				|	  И	ВзносыВФонды.ПериодРегистрации <= &парамПериодРегистрации
				|	  И	Основной.ВидРасчетаЕСВ = ВзносыВФонды.ВидРасчета
				|	  "+?(ТолькоПоДвижениямТекущегоДокумента,"И ВзносыВФонды.Регистратор=&парамРегистратор","")+"
				|	СГРУППИРОВАТЬ ПО
				|			Основной.Сотрудник,
				|			Основной.НалоговыйПериод,
				|			Основной.ДоходНДФЛ,
				|			Основной.ВидРасчетаЕСВ.КатегорияРасчета,
				|			Основной.Доход
				|   ) КАК Основной
				|   ) КАК ЗапросПоВзносам
				|	СГРУППИРОВАТЬ ПО
				|		ЗапросПоВзносам.Сотрудник,
				|		ЗапросПоВзносам.НалоговыйПериод,
				|		ЗапросПоВзносам.ДоходНДФЛ
				|";
	...
	Если НЕ СтрокаДвижений.Сторно Тогда
		...
		Иначе //Перечисления.ВариантыАлгоритмовРасчетНДФЛ17.ОтдельныйРасчетБазы
			...
			Иначе
				...
				КонецЕсли;
				Доход15 = Предел;
				БазаВзносов15 = Мин(Предел,Макс(БазаВзносов,0));
				...
				БазаВзносов17 = Мин(Доход17,Макс(БазаВзносов,0)-БазаВзносов15);
		...
		Если КомментироватьРасчет Тогда
			...
			Если НЕ ИсходныеДанные.ИспользуетсяНК Тогда
				КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет НДФЛ за ';uk='Розрахунок ПДФО за '") + Формат(СтрокаДвижений.НалоговыйПериод,"ДФ='МММ гггг'") + НСтр("ru=' (Код дохода ';uk=' (Код доходу '") + СтрокаДвижений.ДоходНДФЛ + ")");
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма дохода : ';uk='Сума доходу : '") + Доход, КомментарийВидаРасчета);
				Если ДоходДо <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	доход до: ';uk='    дохід до:'") + ДоходДо, КомментарийВидаРасчета);
				КонецЕсли;	
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Учтено взносов : ';uk='Враховано внесків: '") + СуммаВзносов, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка НДФЛ : ';uk='Ставка ПДФО :'") + СтавкаНДФЛ, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Всего сумма льготы : ';uk='Усього сума пільги :'") + СуммаЛьготыВсего, КомментарийВидаРасчета);
				Если ИсходныеДанные.ЭтоМесячныйДоход И КоэфЛьготы <> 1 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Коєффициент учета вида дохода в месячном доходе : ';uk='	Коефіцієнт обліку виду доходу в місячному доході : '") + КоэфЛьготы, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Учтенная сумма льготы по текущему виду дохода: ';uk='    Врахована сума пільги по поточному виду доходу:'") + СуммаЛьготы, КомментарийВидаРасчета);
				КонецЕсли;
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Действующая льгота : ';uk='    Діюча пільга :'") + Льгота, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Количество льгот : ';uk='    Кількість пільг :'") + ИсходныеДанные.КвоЛьгот, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Количество порогов для применения льготы : ';uk='	Кількість порогів для застосування пільги : '") + КвоПороговЛьгот, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Порог льготы : ';uk='	Поріг пільги : '") + КвоПороговЛьгот * ПорогЛьготы, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Минимальная оплата труда : ';uk='Мінімальна оплата праці :'") + МинимальнаяОплатаТруда, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База налога : ';uk='База податку :'") + БазаНалога, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Налог : ';uk='Податок :'") + Налог, КомментарийВидаРасчета);
			ИначеЕсли ИсходныеДанные.ВариантНДФЛ = Перечисления.ВариантыАлгоритмовРасчетНДФЛ17.ДоначислениеНаСуммуПревышения Тогда 	
				КомментарийВидаРасчета = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Расчет НДФЛ за ';uk='Розрахунок ПДФО за '") + Формат(СтрокаДвижений.НалоговыйПериод,"ДФ='МММ гггг'") + НСтр("ru=' (Код дохода ';uk=' (Код доходу '") + СтрокаДвижений.ДоходНДФЛ + ")");
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Доход полный за период : ';uk='Дохід повний за період :'") + ДоходПолный, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма дохода : ';uk='Сума доходу : '") + Доход, КомментарийВидаРасчета);
				Если ДоходДо <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	доход до: ';uk='    дохід до:'") + ДоходДо, КомментарийВидаРасчета);
				КонецЕсли;	
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Учтено ЕСВ : ';uk='Враховане ЄСВ :'") + СуммаВзносов, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка НДФЛ : ';uk='Ставка ПДФО :'") + СтавкаНДФЛ, КомментарийВидаРасчета);
				Если СтавкаНДФЛУвеличенная <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка НДФЛ (увеличенная) : ';uk='Ставка ПДФО (збільшена) :'") + СтавкаНДФЛУвеличенная, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	порог : ';uk='    поріг :'") + ПорогСтавкаНДФЛУвеличенная, КомментарийВидаРасчета);
				КонецЕсли;	
				...
				Если Превышение <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	по обычной ставке: ';uk=' за звичайною ставкою: '") + (Налог-НалогПревышение), КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	превышение порога: ';uk='    перевищення порогу:'") + Окр(Превышение,2), КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	доначислено налога: ';uk='    донараховано податку:'") + НалогПревышение, КомментарийВидаРасчета);
				КонецЕсли;
				Если НалогДо <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	налог до: ';uk='    податок до:'") + НалогДо, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	начислено налога: ';uk='    нараховано податку:'") + (Налог - НалогДо), КомментарийВидаРасчета);
				КонецЕсли;
			...
			Иначе	
				...
				КонецЕсли;	
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Учтено ЕСВ : ';uk='Враховане ЄСВ :'") + СуммаВзносов, КомментарийВидаРасчета);
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База ЕСВ : ';uk='База ЄСВ :'") + БазаВзносов, КомментарийВидаРасчета);
				Если СуммаВзносовБЛ <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Учтено ЕСВ с больничных : ';uk='Враховане ЄСВ із лікарняних :'") + СуммаВзносовБЛ, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База ЕСВ с больничных : ';uk='База ЄСВ із лікарняних :'") + БазаВзносовБЛ, КомментарийВидаРасчета);
				КонецЕсли;	
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка НДФЛ : ';uk='Ставка ПДФО :'") + СтавкаНДФЛ, КомментарийВидаРасчета);
				Если СтавкаНДФЛУвеличенная <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Ставка НДФЛ (увеличенная) : ';uk='Ставка ПДФО (збільшена) :'") + СтавкаНДФЛУвеличенная, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	порог : ';uk='    поріг :'") + ПорогСтавкаНДФЛУвеличенная, КомментарийВидаРасчета);
				КонецЕсли;
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Учтено других видов дохода : ';uk='Враховано інших видів доходу :'") + ДоходПолный, КомментарийВидаРасчета);
				Если ИсходныеДанные.ЭтоМесячныйДоход И СуммаЛьготыВсего <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Всего сумма льготы : ';uk='Усього сума пільги :'") + СуммаЛьготыВсего, КомментарийВидаРасчета);
					Если ИсходныеДанные.ЭтоМесячныйДоход И КоэфЛьготы <> 1 Тогда
						ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	Коєффициент учета вида дохода в месячном доходе : ';uk='	Коефіцієнт обліку виду доходу в місячному доході : '") + КоэфЛьготы, КомментарийВидаРасчета);
				...
				Если Доход15 <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Доход (15%) : ';uk='Дохід (15%) :'") + Доход15, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма взносов (15%) : ';uk='Сума внесків (15%) :'") + Строка(СуммаВзносов15 + СуммаВзносовБЛ15), КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База налога (15%) : ';uk='База податку (15%) :'") + БазаНалога15, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Налог (15%) : ';uk='Податок (15%) :'") + Налог15, КомментарийВидаРасчета);
				КонецЕсли;
				Если Доход17 <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Доход (17%) : ';uk='Дохід (17%) :'") + Доход17, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Сумма взносов (17%) : ';uk='Сума внесків (17%) :'") + Строка(СуммаВзносов17 + СуммаВзносовБЛ17), КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='База налога (17%) : ';uk='База податку (17%) :'") + БазаНалога17, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Налог (17%) : ';uk='Податок (17%) :'") + Налог17, КомментарийВидаРасчета);
				КонецЕсли;
				Если Доход17 <> 0 И Доход15 <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Налог (15%+17%) : ';uk='Податок (15%+17%) :'") + Налог, КомментарийВидаРасчета);
				КонецЕсли;	
				Если НалогДо <> 0 Тогда
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	налог до: ';uk='    податок до:'") + НалогДо, КомментарийВидаРасчета);
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='	начислено налога: ';uk='    нараховано податку:'") + (Налог - НалогДо), КомментарийВидаРасчета);
...
Функция РассчитатьСреднююПорядок100(Регистратор,КомментироватьРасчет = Ложь) Экспорт 
	...
	Если Не Регистратор.Метаданные().ТабличныеЧасти.Найти("Отпуска") = Неопределено Тогда
		...
		КонецЕсли	
	ИначеЕсли НЕ Регистратор.Метаданные().Реквизиты.Найти("ДатаУвольнения") = Неопределено Тогда
		ДатаНачалаСобытия = Регистратор.ДатаУвольнения
	Иначе
	...
	Если ДатаНачалаСобытия < ПроведениеРасчетов.ДатаИзмененияПорядка100() Тогда
		...
		Запрос.УстановитьПараметр( "парамФизлицо", Регистратор.Сотрудник.Физлицо ); 
		Запрос.УстановитьПараметр( "парамПериодРегистрации", Регистратор.ПериодРегистрации );
		...
		Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.РасчетПриУвольненииРаботникаОрганизаций") Тогда 
			Запрос.УстановитьПараметр( "ПериодРасчетаСреднегоДляПособияНачало", Регистратор.ПериодРасчетаСреднегоДляПособияНачало);
			Запрос.УстановитьПараметр( "ПериодРасчетаСреднегоДляПособияКонец", Регистратор.ПериодРасчетаСреднегоДляПособияКонец);
		Иначе
			Запрос.УстановитьПараметр( "ПериодРасчетаСреднегоДляПособияНачало", Регистратор.ПериодРасчетаСреднегоНачало);
			Запрос.УстановитьПараметр( "ПериодРасчетаСреднегоДляПособияКонец", Регистратор.ПериодРасчетаСреднегоКонец);		
		КонецЕсли;	
		Запрос.УстановитьПараметр( "ВидУчетаВремениДляСредней", Регистратор.ВидУчетаВремениДляСредней );
		Запрос.УстановитьПараметр( "ЧислоМесяцевСредней", Регистратор.ЧислоМесяцев ); 
		...
		Если Истина Тогда
			...
			|			ВЫБОР КОГДА Начисления.ГрафикРаботы.СокращатьКалендарныеДни И Начисления.ВидРасчета.ЭтоОтпуск ТОГДА
			...
			|	(Назначение = &парамСотрудник И Сотрудник.Физлицо = &парамФизлицо
			|	ИЛИ Сотрудник = &парамСотрудник И Назначение.Физлицо <> &парамФизлицо )
		...
		|			Средняя.БазовыйПериодКонец			КАК БазовыйПериодКонец,
		|			Средняя.КоэффициентПовышенияОкладов	КАК КоэффициентПовышенияОкладов,
		|			Начисления.Результат/Начисления.ВидРасчета.ЧислоМесяцев * &ЧислоМесяцевСредней *"+КоэфПремии+"		КАК Результат,
		|			Начисления.Результат/Начисления.ВидРасчета.ЧислоМесяцев * &ЧислоМесяцевСредней *"+КоэфПремии+"		КАК РезультатПолный
		| " + ?(КомментироватьРасчет,",	
		| 			Начисления.ВидРасчета КАК ВидРасчетаРазрез","") + "
		|		ИЗ РегистрРасчета.РасчетСреднегоЗаработка КАК Средняя
		| 
		...
		|			// фильтр в случае если не надо учитывать внутренние совмещения: или основное назначение или замена			
		|			"+ ?(мНеУчитыватьСовместителейДляСредних,"И Начисления.Назначение.ОсновноеНазначение <> Начисления.Сотрудник","")+"
		|			И	Начисления.ПериодДействия МЕЖДУ &ПериодРасчетаСреднегоДляПособияНачало И &ПериодРасчетаСреднегоДляПособияКонец	
		|			И	Начисления.ПериодРегистрации <= &парамПериодРегистрации
		|			И	Начисления.ВидРасчета.ЧислоМесяцев > 1
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовРасчета.СреднийЗаработок.БазовыеВидыРасчета КАК БазовыеВидыРасчета
		|		ПО		Средняя.ВидРасчета = БазовыеВидыРасчета.Ссылка  
	...
	Иначе
		...
		Запрос.УстановитьПараметр( "ДатаПриема", Регистратор.Сотрудник.ДатаПриемаНаРаботу );
...
Функция ПолучитьПараметыОклада1213(мДатаНачала, Сотрудник, ПериодРасчетаСреднегоНачало = Неопределено, ПериодРасчетаСреднегоКонец = Неопределено, За12Месяцев = Ложь) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если Истина Тогда
			...
			|ВЫБРАТЬ 
			|	ДанныеГрафика.ГрафикРаботы.ГрафикПолногоРабочегоВремени КАК ГрафикПолногоРабочегоВремени
		...
		Иначе
			...
			КурсВылюты = Структура.Курс;
			КратностьВылюты = Структура.Кратность;
			СуммаРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Оклад,Выборка.Валюта,ВалютаРегл,
								КурсВылюты, 1, КратностьВылюты, 1);
			ПараметрыОклада["Оклад"] = Окр(СуммаРегл*КвоМесяцев, 2);					
		КонецЕсли;	
...
Функция ПроверитьНастройкуВидаРасчета(ВидРасчета, Отказ, ПроверятьСоответствиеБазовыхИВедущихРасчетов = Истина) Экспорт
		...
		Для Каждого СтрокаБазовыйРасчет из ВидРасчета.БазовыеВидыРасчета Цикл
			...
			Иначе
				...
				Если ВидРасчета.ВедущиеВидыРасчета.Найти(СтрокаБазовыйРасчет.ВидРасчета,"ВидРасчета") = Неопределено Тогда
					ТекстСообщения = ТекстСообщения + ?(Не ЗначениеЗаполнено(ТекстСообщения),"",Символы.ПС) + НСтр("ru='Список ведущих видов расчета отличается от списка базовых расчетов. В списке нет расчета ""';uk='Список ведучих видів розрахунку відрізняється від списку базових розрахунків. У списку немає розрахунку ""'")+СтрокаБазовыйРасчет.ВидРасчета+"""!";
					Прервать;
				КонецЕсли; 
	...
	Если ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаОбъект.ВзносыВФонды") Тогда
		ПроверкаБазыЕСВ(ВидРасчета);
	КонецЕсли;
...
Функция ПолучитьВидУчетаВремени(СпособРасчета, ВидВремени, СуммированныйУчетРабочегоВремени = Ложь, ВидУчетаВремениДляСредней = Неопределено) Экспорт
	...
	Если СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоМесячнойТарифнойСтавкеПоЧасам
		...
		или СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоЧасовойТарифнойСтавкеУмножитьНаПроцент   
...
Функция ПолучитьСписокНачисленийДляПредварительногоРасчета() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |		 И НЕ НачисленияДляПредварительногоРасчета.ИспользоватьИзРегистров
	               |ГДЕ
...
Функция РасчетОтОбратного(Сумма, ИсходныеДанные) Экспорт
	...
	ПорогЕСВ = ?(ИсходныеДанные.ПределЕСВ = NULL,0,ИсходныеДанные.ПределЕСВ);
	...
	ПорогНДФЛ = ?(ИсходныеДанные.ПределНДФЛ = NULL,0,ИсходныеДанные.ПределНДФЛ);
	...
	Иначе	
		...
		ВариантаАлгоритмаНДФЛ = ПроведениеРасчетов.ВариантыАлгоритмовРасчетаЗарплаты().РасчетНДФЛ17;
		
		Если ВариантаАлгоритмаНДФЛ = Перечисления.ВариантыАлгоритмовРасчетНДФЛ17.ДоначислениеНаСуммуПревышения Тогда
		...
		Иначе
			Результат2 = (Сумма+ПорогНДФЛ*(СтавкаНДФЛ1-СтавкаЕСВ*СтавкаНДФЛ1-СтавкаНДФЛ2+СтавкаЕСВ*СтавкаНДФЛ2))/(1-СтавкаЕСВ-СтавкаНДФЛ2+СтавкаЕСВ*СтавкаНДФЛ2);
			Результат3 = (Сумма+ПорогНДФЛ*(СтавкаНДФЛ1-СтавкаЕСВ*СтавкаНДФЛ1-СтавкаНДФЛ2+СтавкаЕСВ*СтавкаНДФЛ2)+ПорогЕСВ*(СтавкаЕСВ-СтавкаЕСВ*СтавкаНДФЛ2))/(1-СтавкаНДФЛ2);
		КонецЕсли;	
		...
		Иначе
			Если Результат < ПорогНДФЛ Тогда
				НДФЛ = Окр((Результат-Окр(Результат*СтавкаЕСВ,2))*СтавкаНДФЛ1,2);
			Иначе
				ЕСВ1 = Окр(ПорогНДФЛ*СтавкаЕСВ,2);
				НДФЛ1 = Окр((ПорогНДФЛ-ЕСВ1)*СтавкаНДФЛ1,2);
				НДФЛ2 = Окр((Результат-ПорогНДФЛ-ЕСВ+ЕСВ1)*СтавкаНДФЛ2,2);
				НДФЛ = НДФЛ1+НДФЛ2;
			КонецЕсли;	
		КонецЕсли;
...
Функция ПолучитьСписокОсновныхВариантовНачисленийОрганизации() Экспорт
	...
	СписокСпособов.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ДоначислениеОтОбратного);
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ РАСЧЕТА СТРАХОВОГО СТАЖА
...
Функция РасчетСтраховогоСтажа(ДатаНачала, Организация, Сотрудник) Экспорт
	...
	Запрос.УстановитьПараметр("парамОрганизация",	Организация );
	ДатаПриема = Сотрудник.ДатаПриемаНаРаботу;
	...
	|	НачальныеСведенияОСтажеРаботниковОрганизации.НачальныйСтаж,
	...
	//Если Выборка.Следующий() Тогда
	//	ДатаОтсчетаДоНачалаСобытия = Выборка.ДатаОтсчетаДоНачалаСобытия;
	...
	ИначеЕсли ДатаПриема >= МаксимальныйПериодСправки Тогда
...
Функция ВизуализироватьФормулуРасчета(Объект, Режим = "Текст") Экспорт
	...
	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС Тогда		
		Если Режим = "Текст" Тогда  
			СтрокаФормулы = "<Средний дневной(часовой) заработок>  * <Процент оплаты (50% - 100%)> * <Время в днях(часах)>"
		Иначе
			СтрокаФормулы = СтрокаФормулы + "
							|Результат = <FONT color=#993366><Средний дневной заработок></FONT> умножить на <FONT color=#993366><Процент оплаты (50% - 100%)></FONT> умножить на <FONT color=#993366><Время в днях(часах)></FONT>"			
		КонецЕсли;
	...
	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоначислениеОтОбратного Тогда
		Если Режим = "Текст" Тогда
			СтрокаФормулы = "Максимум(<Сумма ""от обратного""> - <Расчетная база>, <0>)";
		Иначе
			СтрокаФормулы = СтрокаФормулы + "
							|Результат = <FONT color=#0000ff>Максимум</FONT><FONT color=#FF0000>(</FONT><FONT color=#0000ff>Сумма ""от обратного""</FONT><FONT color=#FF0000> - </FONT><FONT color=#0000ff>Расчетная база</FONT> <FONT color=#FF0000>,</FONT> 0<FONT color=#FF0000>)</FONT>"		
		КонецЕсли;
   	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоМЗП Тогда
		Если Режим = "Текст" Тогда
...
Функция ПолучитьТаблицуРабочихПериодовОтпусков(ФизЛицо,ПериодС=Неопределено,ПериодПо) Экспорт
	...
	Если фОшибка=1 Тогда
		Сообщить(НСтр("ru='Для сотрудника ';uk='Для співробітника '")+ФизЛицо+НСтр("ru=' нарушена непрерывность рабочих периодов! Подробности в отчете по использованию отпусков!';uk=' порушена безперервність робочих періодів! Подробиці у звіті по використанню відпусток!'"),СтатусСообщения.Важное);
	КонецЕсли;
...
//
Процедура ВыполнитьДвиженияПоЗарплате( ДокументВыплаты, Движения, СпособОплаты, ВыплаченностьЗарплаты, 
	...
	Если ДокументВыплаты.Метаданные().Имя = "РасходныйКассовыйОрдер" И ДокументВыплаты.ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
	    ...
	    |ИЗ	Документ."+ ДокументВыплаты.Метаданные().Имя +" КАК Выплаты
		|
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ	Документ.ЗарплатаКВыплатеОрганизаций.ПараметрыОплаты КАК ПараметрыОплаты
		|ПО Выплаты.ДокументОснование = ПараметрыОплаты.Ссылка
		|	И Выплаты.СчетУчета = ПараметрыОплаты.СчетУчета
		
		|И	ПараметрыОплаты.Сотрудник в (&парамСотрудники)        
		|
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ	Документ.ЗарплатаКВыплатеОрганизаций.РаботникиОрганизации КАК РаботникиОрганизации
		|ПО	Выплаты.ДокументОснование = РаботникиОрганизации.Ссылка
		|И	РаботникиОрганизации.Сотрудник = ПараметрыОплаты.Сотрудник       
		|И	РаботникиОрганизации.ВыплаченностьЗарплаты = &Выплачено       
		|И	РаботникиОрганизации.СпособВыплаты = &СпособОплаты       
		|
	...
	|
	|ИТОГИ СУММА(СуммаВзаиморасчетов), СУММА(СуммаОкругления), МАКСИМУМ(Период), МАКСИМУМ(ПериодВзаиморасчетов)
		...
		Пока ВыборкаСотрудник.Следующий() Цикл
			...
			Пока ВыборкаВедомость.Следующий() Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Выборка.Ведомость.ВидВыплаты = Справочники.ВидыВыплат.Депоненты Тогда
...
Процедура	ДобавитьСтрокуВзаиморасчетыСРаботникамиОрганизаций(Выборка, НаборЗаписей, НаборЗаписейУпр)
	...
	Если Выборка.Ведомость.ВидВыплаты = Справочники.ВидыВыплат.Очередная ИЛИ Выборка.Ведомость.ВидВыплаты = Справочники.ВидыВыплат.ОчереднаяПогашениеДолгов ИЛИ Выборка.Ведомость.ВидВыплаты = Справочники.ВидыВыплат.ПогашениеДолгов Тогда
		мХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Зарплата;
	ИначеЕсли Выборка.Ведомость.ВидВыплаты = Справочники.ВидыВыплат.Аванс ИЛИ Выборка.Ведомость.ВидВыплаты = Справочники.ВидыВыплат.АвансПредварительныйРасчет ТОгда
...
Функция ПолучитьТекстПоказателя(НеобходимыеДанные, ВидПВР, Предопределенный = Ложь, КоличествоПоказателей, КомментироватьРасчет)		
	...
	Если 	НеобходимыеДанные.ОтработаноВремени или 
		...
		ИначеЕсли ВидПВР = "УдержанияРаботниковОрганизаций" Тогда
			Показатель1Текст = Показатель1Текст +		
			?(НеобходимыеДанные.База, "
			|	КОГДА Показатели.Показатель1 = ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.РасчетнаяБаза)
			|		ТОГДА База.РезультатБаза
		...
		Если ЗначениеЗаполнено(ВидРасчета) И (ТипВидРасчета = Тип("ПланВидовРасчетаСсылка.ОсновныеНачисленияОрганизаций")
			...
			Если СтруктураВидаРасчета.СпособРасчета <> Перечисления.СпособыРасчетаОплатыТруда.ПроизвольнаяФормула Тогда
				...
				ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоДневнойТарифнойСтавке
					...
					КонецЕсли;	
				ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоЧасовойТарифнойСтавкеУмножитьНаПроцент Тогда	
					...
					Если Режим = "ПлановыеНачисления" Тогда
						...
						СведенияОВидеРасчета["Показатель2Наименование"] = "Процент";
			...
			ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуДляОтпуска
				...
				ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПочтовыйСбор Тогда
					...
					СведенияОВидеРасчета.Вставить("Показатель1НаименованиеВидимость", Истина);
					СведенияОВидеРасчета.Вставить("Валюта1Видимость", Истина);
				ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияАлиментов Тогда
					СведенияОВидеРасчета.Вставить("Показатель1Наименование", "Коэффициент индексации");
					СведенияОВидеРасчета.Вставить("Показатель1Видимость", Ложь);
					СведенияОВидеРасчета.Вставить("Показатель1НаименованиеВидимость", Ложь);
					СведенияОВидеРасчета.Вставить("Валюта1Видимость", Ложь);	
			...
			Если ВозможностьИзменения = Перечисления.ИзменениеПоказателейСхемМотивации.НеИзменяется И ЗапрашиватьПриКадровыхПеремещениях
				...
				Если ЭтоНачисление Тогда
					Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента["ТарифныйРазряд" + СчПоказателя])
						и ТипПоказателя = Перечисления.ТипыПоказателейСхемМотивации.ТарифныйРазряд Тогда
						ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан размер тарифа ';uk='не вказаний розмір тарифу '") + ВыборкаПоСтрокамДокумента["Показатель" + СчПоказателя+"Наименование"], Отказ, Заголовок);
					КонецЕсли;	
				КонецЕсли;	
	...
	Иначе
		...
		Если ВыборкаПоСтрокамДокумента.КоличествоПоказателей > 0 И ВыборкаПоСтрокамДокумента.Действие <> Перечисления.ВидыДействияСНачислением.Прекратить И СпособРасчетаТребуетРазмер(ВыборкаПоСтрокамДокумента.СпособРасчета) Тогда
			...
			ИначеЕсли ЭтоНачисление И ВыборкаПоСтрокамДокумента.ТарифныйРазряд1.Пустая() И ВыборкаПоСтрокамДокумента.ТребуетВводаТарифногоРазряда Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан разряд!';uk='не вказаний розряд!'"), Отказ, Заголовок);
...
////////////////////////////////////////////////////////////////////////////////
// БЫСТРОЕ СОЗДАНИЕ РАСЧЕТОВ (используется при переносе из ЗиК 7.7)

// Создает полною копию переданного вида расчета
//
Функция СоздатьКопиюРасчета(ВидРасчета, Наименование, ИскатьСозданные, ЗаполнитьБазовые = Истина,ЗаполнитьВедущие = Истина,ЗаполнитьВытесняющие = Истина,ЗаполнитьЯвляетсяБазой = Истина) Экспорт 
	
	Если ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетассылка.ОсновныеНачисленияОрганизаций") Тогда
		Если ИскатьСозданные Тогда
			ВР = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию(Наименование);
			Если ВР.Пустая() Тогда
				ВРНовый = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.СоздатьВидРасчета();
			Иначе
				ВРНовый = ВР.ПолучитьОбъект();
			КонецЕсли;
		Иначе	
			ВРНовый = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.СоздатьВидРасчета();
		КонецЕсли;
	Иначе
		Если ИскатьСозданные Тогда
			ВР = ПланыВидовРасчета.УдержанияОрганизаций.НайтиПоНаименованию(Наименование);
			Если ВР.Пустая() Тогда
				ВРНовый = ПланыВидовРасчета.УдержанияОрганизаций.СоздатьВидРасчета();
			Иначе
				ВРНовый = ВР.ПолучитьОбъект();
			КонецЕсли;
		Иначе	
		...
		КонецЕсли;
	КонецЕсли;	
	ЗаполнитьЗначенияСвойств(ВРНовый,ВидРасчета);
	ВРНовый.Наименование = Наименование;
	Если ЗаполнитьБазовые Тогда
		ТЧ = ВидРасчета.БазовыеВидыРасчета.Выгрузить();
		ВРНовый.БазовыеВидыРасчета.Загрузить(ТЧ);
	КонецЕсли;	
	Если ЗаполнитьВедущие Тогда
		ТЧ = ВидРасчета.ВедущиеВидыРасчета.Выгрузить();
		ВРНовый.ВедущиеВидыРасчета.Загрузить(ТЧ);
	КонецЕсли;
	Если ЗаполнитьВытесняющие Тогда
		ТЧ = ВидРасчета.ВытесняющиеВидыРасчета.Выгрузить();
		ВРНовый.ВытесняющиеВидыРасчета.Загрузить(ТЧ);
	КонецЕсли;	
	ВРНовый.Записать();
	
	Если ЗаполнитьЯвляетсяБазой Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "
		|ВЫБРАТЬ
		...
		ЯвляетсяБазой = Запрос.Выполнить().Выгрузить();
		Для Каждого ВР из ЯвляетсяБазой Цикл
			ВРОбъект = ВР.ВидРасчета.ПолучитьОбъект();
			НС = ВРОбъект.БазовыеВидыРасчета.Добавить();
			НС.ВидРасчета = ВРНовый.Ссылка;
			ВРОбъект.Записать();
		КонецЦикла;	
...
// Создает требуемый вид расчета по эталону
//
Процедура БыстроеСозданиеРасчетов(ВидРасчета, ИскатьСозданные = Истина) Экспорт
	
	Если ВидРасчета = "Оклад по дням (по разрядам)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОкладПоДням;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные);
		ВРНовый.ТребуетВводаТарифногоРазряда = Истина;
		ВРНовый.Записать();
	ИначеЕсли ВидРасчета = "Оклад по часам (по разрядам)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОкладПоЧасам;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные);
		ВРНовый.ТребуетВводаТарифногоРазряда = Истина;
		ВРНовый.Записать();
	ИначеЕсли ВидРасчета = "Оплата по часовому тарифу (по разрядам)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ТарифЧасовой;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные);
		ВРНовый.ТребуетВводаТарифногоРазряда = Истина;
		ВРНовый.Записать();	
	ИначеЕсли ВидРасчета = "Оплата по дневному тарифу по (разрядам)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ТарифДневной;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные);
		ВРНовый.ТребуетВводаТарифногоРазряда = Истина;
		ВРНовый.Записать();
	ИначеЕсли ВидРасчета = "Месячная премия (суммой)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.Месячная;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные,ложь,ложь,ложь);
		ВРНовый.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой;
		ВРНовый.КатегорияРасчета = Перечисления.КатегорииРасчетов.Первичное;
		ВРНовый.Записать();
	ИначеЕсли ВидРасчета = "Квартальная премия (суммой)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.Квартальная;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные,ложь,ложь,ложь);
		ВРНовый.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой;
		ВРНовый.КатегорияРасчета = Перечисления.КатегорииРасчетов.Первичное;
		ВРНовый.Записать();
	ИначеЕсли ВидРасчета = "Премия по итогам работы за год (суммой)" Тогда
		ВРЭталон = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.Годовая;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные,ложь,ложь,ложь);
		ВРНовый.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой;
		ВРНовый.КатегорияРасчета = Перечисления.КатегорииРасчетов.Первичное;
		ВРНовый.Записать();
	ИначеЕсли ВидРасчета = "Профсоюзный взнос" Тогда
		ВРЭталон = ПланыВидовРасчета.УдержанияОрганизаций.АлиментыПроцентом;
		ВРНовый = СоздатьКопиюРасчета(ВРЭталон,ВидРасчета, ИскатьСозданные,истина,ложь,ложь,ложь);
		ВРНовый.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Процентом;
		ВРНовый.КатегорияРасчета = Перечисления.КатегорииРасчетов.Первичное;
		ВРНовый.СпособОтраженияВБухучете = 0;
		ВРНовый.Записать();	
	КонецЕсли;
	
КонецПроцедуры	

// По группе расчетов устанавливает вид расчетов в качестве базового для взносов, алиментов, премий
//
Процедура НастроитьБазуПоГруппе(Группа,ПВР,ВидРасчета)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ОсновныеНачисленияОрганизаций.Ссылка,
	               |	ОсновныеНачисленияОрганизаций.ГруппыРасчетов77,
				   |    Базовые.ВидРасчета
	               |ИЗ
	               |	ПланВидовРасчета.ОсновныеНачисленияОрганизаций КАК ОсновныеНачисленияОрганизаций
				   |    ЛЕВОЕ СОЕДИНЕНИЕ
				   |	ПланВидовРасчета."+ПВР+".БазовыеВидыРасчета КАК Базовые
				   |    ПО Базовые.ВидРасчета = ОсновныеНачисленияОрганизаций.Ссылка
				   |       И Базовые.Ссылка = &ВидРасчета
	               |ГДЕ
	               |	ОсновныеНачисленияОрганизаций.ГруппыРасчетов77 Подобно &Группа
				   |    И Базовые.ВидРасчета ЕСТЬ NULL
				   |";
	...
	Пока Выборка.Следующий() Цикл
		НС = ВРОбъект.БазовыеВидыРасчета.Добавить();
		НС.ВидРасчета = Выборка.Ссылка;
	КонецЦикла;	
	ВРОбъект.Записать();
	
КонецПроцедуры	

// Настройка созданных видов расчетов после переноса из 7.7
//
Процедура НастройкаВидовРасчетов77() Экспорт
	
	//Установить налогообложение
	НастроитьБазуПоГруппе("БазаБезработицаГПХ", "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.БезработицаГПХ20090101);
	НастроитьБазуПоГруппе("БазаБезработицаФЗП", "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.Безработица20090101);
	НастроитьБазуПоГруппе("БазаДляБезработицы", "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.Безработица20090101);
	НастроитьБазуПоГруппе("БазаДляПенсионного", "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.Пенсионный20070101);
	НастроитьБазуПоГруппе("БазаДляСоцстраха",   "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.Соцстрах20050101);
	НастроитьБазуПоГруппе("БазаПенсионныйФЗП",  "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.ПенсионныйФОТ20050101);
	НастроитьБазуПоГруппе("БазаСоцСтрахНесчФЗП","ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.СоцстрахНесчФОТ20050101);
	НастроитьБазуПоГруппе("БазаСоцСтрахФЗП",    "ВзносыВФонды",ПланывидовРасчета.ВзносыВФонды.СоцстрахФОТ20050101);
	
	//Установить алименты
	НастроитьБазуПоГруппе("БазаДляАлиментов", "УдержанияОрганизаций", ПланывидовРасчета.УдержанияОрганизаций.АлиментыПроцентом);
	НастроитьБазуПоГруппе("БазаДляАлиментов", "УдержанияОрганизаций", ПланывидовРасчета.УдержанияОрганизаций.АлиментыПроцентомДоПредела);
	НастроитьБазуПоГруппе("БазаДляАлиментов", "УдержанияОрганизаций", ПланывидовРасчета.УдержанияОрганизаций.ИЛПроцентом);
	НастроитьБазуПоГруппе("БазаДляАлиментов", "УдержанияОрганизаций", ПланывидовРасчета.УдержанияОрганизаций.ИЛПроцентомДоПредела);

	//Установить премии
	НастроитьБазуПоГруппе("БазаДляПремий", "ОсновныеНачисленияОрганизаций", ПланывидовРасчета.ОсновныеНачисленияОрганизаций.Месячная);
	НастроитьБазуПоГруппе("БазаДляПремий", "ОсновныеНачисленияОрганизаций", ПланывидовРасчета.ОсновныеНачисленияОрганизаций.Квартальная);
	НастроитьБазуПоГруппе("БазаДляПремий", "ОсновныеНачисленияОрганизаций", ПланывидовРасчета.ОсновныеНачисленияОрганизаций.Годовая);
	
	//Заполнить созданные типовые виды расчетов
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ОсновныеНачисленияОрганизаций.Ссылка,
				   |	ОсновныеНачисленияОрганизаций.Наименование,
	               ...
	               |ИЗ
	               |	ПланВидовРасчета.ОсновныеНачисленияОрганизаций КАК ОсновныеНачисленияОрганизаций
				   |ГДЕ ОсновныеНачисленияОрганизаций.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПустаяСсылка)
				   |
				   |ОБЪЕДИНИТЬ ВСЕ
				   |
				   |ВЫБРАТЬ
				   ...
				   |ГДЕ УдержанияОрганизаций.СпособРасчета = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаОплатыТруда.ПустаяСсылка)
				   |";

	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПроведениеРасчетов.БыстроеСозданиеРасчетов(Выборка.Наименование);
	КонецЦикла;
