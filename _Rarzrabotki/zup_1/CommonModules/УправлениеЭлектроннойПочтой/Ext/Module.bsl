...
Функция ОткрытьФайлДляРедактированияВнешнимХТМЛРедактором(ИмяФайла) Экспорт
	...
	Если Найти(ИмяПриложения, "[FILENAME]") = 0 ИЛИ СтрЧислоВхождений(ИмяПриложения, "[FILENAME]") > 1 Тогда
		ОбщегоНазначения.СообщитьОбОшибке("В настройках пользователя некорректно задана командная строка для запуска внешнего HTML редактора текстов.
										  |Необходимо один раз указать место для подстановки имени файла в виде шаблона [FileName].",, НСтр("ru='Редактирование невозможно.';uk='Редагування неможливе.'"));
		Возврат Ложь;
	КонецЕсли;
...
// Функция производит регистрацию кандидата по переданному физическому лицу
Функция ЗарегистрироватьФизлицоКакКандидата(ФизЛицо,ФормаВладелец = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ФизЛицо) = Тип("СписокЗначений") Тогда
		СписокФизЛиц = ФизЛицо.Скопировать();
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Объект",СписокФизЛиц);
	...
	|ГДЕ
	|	КандидатыНаРаботу.ФизЛицо в (&Объект)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ЭлементСписка = СписокФизЛиц.НайтиПоЗначению(Выборка.ФизЛицо);
		СписокФизЛиц.Удалить(ЭлементСписка);
	КонецЦикла;
	
	Если СписокФизЛиц.Количество() = 0 Тогда
		# Если Клиент Тогда
		Сообщить(НСтр("ru='Кандидаты уже зарегистрированы!';uk='Кандидати вже зареєстровані!'"));
		# КонецЕсли
		Возврат Ложь;
	КонецЕсли; 
