...
Функция ПолучитьПредставлениеАдресаПоСтруктуре(СтруктураАдреса) Экспорт
	...
	Если Не АдресЗаПределамиУкраины Тогда
		...
		ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Дом), НСтр("ru=', дом № ';uk=', будинок № '",Локализация.КодЯзыкаИнформационнойБазы()));
		ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Корпус), НСтр("ru=', корпус ';uk=', корпус '",Локализация.КодЯзыкаИнформационнойБазы()));
		ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Квартира), НСтр("ru=', кв.';uk=', кв.'",Локализация.КодЯзыкаИнформационнойБазы()));
...
Функция ПолучитьПредставлениеАдреса(Запись) Экспорт
	...
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(Запись.Поле7), НСтр("ru=', дом № ';uk=', будинок № '",Локализация.КодЯзыкаИнформационнойБазы()));
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(Запись.Поле8), НСтр("ru=', корпус ';uk=', корпус '",Локализация.КодЯзыкаИнформационнойБазы()));
	ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(Запись.Поле9), НСтр("ru=', кв.';uk=', кв.'",Локализация.КодЯзыкаИнформационнойБазы()));
...
Функция ВходитВИнтервал(Знач Номер, Интервал) Экспорт
	...
	ИначеЕсли (Найти(Интервал, "-") = 0) И ОбщегоНазначения.ТолькоЦифрыВСтроке(Интервал, Ложь) Тогда 
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено, КодЯзыка = "ru") Экспорт
	Сведения = Новый Структура("Представление, ПолноеНаименование, КодПоЕДРПОУ, КодПоДРФО, ИНН, НомерСвидетельства, Телефоны, ЮридическийАдрес, ФактическийАдрес, Банк, МФО, КоррСчет, НомерСчета, НомерСчетаУстаревший, ПредставлениеБанковскогоСчета, МФО_Платеж, НомерСчетаСПереносом, АдресБанка, ПлательщикНалогаНаПрибыль");
	Запрос   = Новый Запрос;
	Данные   = Неопределено;
	...
	Если НЕ ЗначениеЗаполнено(ТекБанковскийСчет) Тогда
		...
		|	""""                           КАК КоррСчет,
		|	""""                           КАК АдресБанка,
		|   Истина   КАК ПлательщикНалогаНаПрибыль
		|ИЗ
	...
	ИначеЕсли ЮрФизЛицо.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		Сведения.Вставить("КодПоЕДРПОУ", Данные.КодПоЕДРПОУ);
	Иначе
		...
		Если ЗначениеЗаполнено(ПервыйНомерСчета) Тогда
			...
			ПредставлениеСчета = НСтр("ru='т/с ';uk='п/р '",КодЯзыка) + ПервыйНомерСчета 
				+ ?(ЗначениеЗаполнено(ВторойНомерСчета), 
					...
					+ ?(НуженМФОВторогоСчета, ", " + НСтр("ru='МФО ';uk='МФО '",КодЯзыка) + СокрЛП(Данные.МФО), "")+ ")", "") 
				+ " " + НСтр("ru='в банке ';uk='у банку '",КодЯзыка) + Данные.Банк
				+ ?(НуженМФОПервогоСчета, ", " + НСтр("ru='МФО ';uk='МФО '",КодЯзыка) + СокрЛП(Данные.МФО), "");
...
Процедура ПередЗаписьюВФормеКонтактнойИнформации(Отказ, ЭтаФорма) Экспорт
	...
	Если (ТипЗнч(ЭтаФорма.мСтруктураИзмерений) = Тип("Структура") И (ЭтаФорма.РегистрСведенийМенеджерЗаписи.Вид <> ЭтаФорма.мСтруктураИзмерений.Вид ИЛИ ЭтаФорма.РегистрСведенийМенеджерЗаписи.Объект <> ЭтаФорма.мСтруктураИзмерений.Объект)) ИЛИ ЭтаФорма.мСтруктураИзмерений = Неопределено Тогда
		...
		Если НаборРегистра.Количество() > 0 Тогда
			...
			ОтветНаВопрос = Вопрос(НСтр("ru='Запись с данными параметрами уже существует. Удалить старые значения?';uk='Запис з даними параметрами вже існує. Вилучити старі значення?'"), РежимДиалогаВопрос.ОКОтмена);
