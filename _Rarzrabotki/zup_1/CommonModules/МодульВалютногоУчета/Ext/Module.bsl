...
Функция ПересчитатьИзВалютыВВалюту(Сумма, ВалютаНач, ВалютаКон, ПоКурсуНач, ПоКурсуКон, 
	...
	Если ПоКурсуНач     = 0 
	 ...
	 или ПоКратностьКон = 0 Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='При пересчете из валюты ';uk='При перерахунку з валюти '")+ВалютаНач+НСтр("ru='' в валюту ';uk=' у валюту '")+ВалютаКон+НСтр("ru=' обнаружен нулевой курс. Пересчет не произведен!';uk=' виявлено нульовий курс. Перерахунок не зроблено!'"));		
		Возврат 0;
	КонецЕсли;
