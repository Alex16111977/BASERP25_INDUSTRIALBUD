...
Функция РегламентированныйКалендарьЗаполнен(ДатаНач, ДатаКон, ВыводитьСообщения = Ложь) Экспорт 
	...
	Если Не КалендарьЗаполнен и ВыводитьСообщения Тогда
		Сообщение = Сообщение + ?(Найти(Сообщение,",")=0,НСтр("ru=' год!';uk=' рік!'"),НСтр("ru=' годы!';uk=' роки!'"));
		Сообщить(Сообщение + НСтр("ru='"
"Интерфейсы: ""Полный"", ""Расчет зарплаты организаций"", ""Кадровый учет организаций"", пункт меню Предприятие - Производственный календарь.';uk='"
"Інтерфейси: ""Повний"", ""Розрахунок зарплати організацій"", ""Кадровий облік організацій"", пункт меню Підприємство - Виробничий календар.'"));
	КонецЕсли;
...
Функция ПодобратьФИОРаботникаКандидата(СтандартнаяОбработка, Режим, Знач Текст, Организация = "", ВозвращатьФизлицо = Ложь) Экспорт
	...
	Если Режим = "Работники" Тогда
		...
		|ГДЕ
		|	Сотрудники.Сотрудник.Наименование ПОДОБНО &Текст СПЕЦСИМВОЛ ""~""";
...
Процедура ОткрытьРабочийСтол() Экспорт
	...
	Форма = Обработки.РабочийСтол.ПолучитьФорму("Форма");
...
Функция ПолучитьСписокПрочихСостоянийРаботника() Экспорт
	...
	СписокСпособов.Добавить(Перечисления.СостоянияРаботникаОрганизации.Мобилизован);
	СписокСпособов.Добавить(Перечисления.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора);
...
Функция ПроверкаУникальностиНомераКадровогоДокумента(ЕдиныйНумераторКадровыхДокументов, Номер, Дата, Ссылка) Экспорт
	...
	Если ЕдиныйНумераторКадровыхДокументов Тогда
		...
		Если Не ПроверкаУникальностиНомераПоДокументам(СписокДокументов, Номер, Дата, Ссылка) Тогда
			ВызватьИсключение НСтр("ru='Неуникальный номер кадрового документа! (';uk='Неунікальний номер кадрового документа! ('") + Номер + ")";
		КонецЕсли;
	КонецЕсли;
КонецФункции
...
Функция МожноОтключатьВнутреннееСовместительство(Организация) Экспорт
	...
	#Если Клиент Тогда
	Состояние(НСтр("ru='Выполняется поиск внутренних совместителей';uk='Виконується пошук внутрішніх сумісників'"));
	#КонецЕсли
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Отключить поддержку внутреннего совместительства невозможно, т.к. некоторые работники заняты на нескольких местах работы!';uk='Відключити підтримку внутрішнього сумісництва неможливо, тому що деякі працівники зайняті на декількох місцях роботи!'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция НомераКадровыхДокументовУникальны(Организация) Экспорт
	...
	Отказ = Ложь;
	ЗаголовокСообщения = НСтр("ru='Включить единый кадровый нумератор невозможно, т.к. следующие номера используются в нескольких кадровых документах:';uk='Включити єдиний кадровий нумератор неможливо, т. я. наступні номери використовуються в кількох кадрових документах:'");
		...
		ИначеЕсли (ПоследняяДатаОкончания > Выборка.ДатаНачала) ИЛИ (ПоследняяДатаОкончания = '0001-01-01') Тогда
			...
			Если СообщенияОбОшибках <> НеОпределено и
				...
				Если Выборка.Регистратор1 = Регистратор Тогда
					СообщениеОбОшибке.Регистратор1 = Выборка.ПредставлениеРегистратор1 + НСтр("ru=' (текущий документ)';uk=' (поточний документ)'");
				Иначе
					СообщениеОбОшибке.Регистратор1 = "" + Выборка.ПредставлениеРегистратор1;
				КонецЕсли;
				
				Если Выборка.Регистратор2 = Регистратор Тогда
					СообщениеОбОшибке.Регистратор2 = Выборка.ПредставлениеРегистратор2 + НСтр("ru=' (текущий документ)';uk=' (поточний документ)'");
				Иначе
				...
				Если Выборка.Регистратор1 <> ПоследнийРегистратор1 
					...
					Если ПоследнийРегистратор1 = Регистратор Тогда
						СообщениеОбОшибке.ПослРегистратор1 = ПоследнийПредставлениеРегистратор1 + НСтр("ru=' (текущий документ)';uk=' (поточний документ)'");
					Иначе
				...
				Если Выборка.Регистратор1 <> ПоследнийРегистратор2 
					...
					Если ПоследнийРегистратор2 = Регистратор Тогда
						СообщениеОбОшибке.ПослРегистратор2 = ПоследнийПредставлениеРегистратор2 + НСтр("ru=' (текущий документ)';uk=' (поточний документ)'");
					Иначе
...
Функция ДополнитьРабочиеПериоды(МенеджерВременныхТаблиц) Экспорт
	...
	|	КОНЕЦ КАК Период,
	|ВЫБОР 
	|  КОГДА РаботникиОрганизации.ДатаОкончания < РаботникиОрганизации.Сотрудник.ДатаУвольнения ИЛИ РаботникиОрганизации.Сотрудник.ДатаУвольнения = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|  ТОГДА РаботникиОрганизации.ДатаОкончания
	|  ИНАЧЕ РаботникиОрганизации.Сотрудник.ДатаУвольнения
	|КОНЕЦ КАК ДатаОкончания,
	|РаботникиОрганизации.ВидОтпуска	КАК ВидОтпуска,
	|РаботникиОрганизации.Организация	КАК Организация
	|ИЗ
