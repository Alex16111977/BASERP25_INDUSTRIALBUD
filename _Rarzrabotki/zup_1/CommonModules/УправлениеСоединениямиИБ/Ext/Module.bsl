...
Процедура УстановитьБлокировкуСоединенийПриОбновлении() Экспорт
	...
	УстановитьБлокировкуСоединений(НСтр("ru=' в связи с необходимостью обновления конфигурации.';uk="" у зв'язку з необхідністю оновлення конфігурації."""), 
...
#КонецЕсли
	...
	Попытка
		...
		Для Каждого Соединение Из Соединения.Соединения Цикл
			...
			СтрСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Разрывается соединение: Пользователь %1, компьютер %2, установлено %3, режим %4';uk='Розривається з`єднання: Користувач %1, компьютер %2, встановлено %3, режим %4'",Локализация.КодЯзыкаИнформационнойБазы()), Соединение.UserName,  Соединение.HostName, Соединение.ConnectedAt, Соединение.AppID);
			ЗаписьЖурналаРегистрации(НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'",Локализация.КодЯзыкаИнформационнойБазы()), УровеньЖурналаРегистрации.Информация, , , СтрСообщения);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'",Локализация.КодЯзыкаИнформационнойБазы()), УровеньЖурналаРегистрации.Ошибка, , , ОписаниеОшибки);
#Если Клиент Тогда
...
Функция ПроверитьПараметрыАдминистрированияИБ(ПараметрыАдминистрированияИБ, Знач ВыдаватьСообщения = Истина) Экспорт
  ...
  Результат = Ложь;
	Попытка
		...
		Результат = Истина;
	Исключение
		Сообщение = НСтр("ru='Не удалось подключиться к кластеру серверов.';uk='Не вдалося підключитися до кластеру серверів.'",Локализация.КодЯзыкаИнформационнойБазы());
		ЗаписьЖурналаРегистрации(НСтр("ru='Завершение работы пользователей';uk='Завершення роботи користувачів'",Локализация.КодЯзыкаИнформационнойБазы()), УровеньЖурналаРегистрации.Ошибка,,, 
