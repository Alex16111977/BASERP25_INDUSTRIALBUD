...
Процедура ПечатьПоДополнительнойКнопке(УдалитьДеревоМакетов, Объект, Форма = Неопределено, ТекстКнопки) Экспорт
	...
	Если ТекстКнопки = НСтр("ru='Печать';uk='Друк'") Тогда
		ОткрытьФормуВыбораПечатныхФормОбъекта(Объект, Форма);
	Иначе
		НапечататьДокументПоУмолчанию(Объект);
	КонецЕсли;
...
Функция НапечататьДокументПоУмолчанию(Объект, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если СтрокаПоУмолчанию = Неопределено Тогда
		...
		Сообщить(НСтр("ru='Печать по умолчанию не определена!';uk='Друк по умовчанню не визначений!'")); 
		Возврат Ложь;
...
//
Функция НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено, ИсточникМакета = Неопределено) Экспорт
