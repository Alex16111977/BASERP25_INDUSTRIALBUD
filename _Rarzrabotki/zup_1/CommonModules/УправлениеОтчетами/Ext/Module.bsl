...
// Это делается для 
//1) удаления элементов порядка, по которым не происходит упорядочивание
//2) для избежания ошибки времени выполнения при выполнении запроса
//
// Параметры:
...
//	ТаблицаПолей.Колонки.Добавить("ТипЗначения");  // тип значения поля, для которого добавляются свойства и категории.
	...
	Для Инд = 0 По МетаданныеРегистра.Измерения.Количество()-1 Цикл
		...
		Если Измерение.Тип.СодержитТип(Тип("СправочникСсылка.Организации")) Тогда
...
Процедура ДействияПриИзмененииОтображения(Отчет, ФормаОтчета, ВыбраннаяКнопка) Экспорт
		...
		КоличествоВариантовОтображения = 3;
		
		ПредыдущийВариантОтображения = Отчет.ВариантОтображения;
		
		Для Каждого ВариантОтображения Из Перечисления.ВариантыОтображенияОтчетов Цикл
			Если Строка(ВариантОтображения) = ВыбраннаяКнопка.Имя Тогда
				 Отчет.ВариантОтображения = ВариантОтображения;
				 Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Иначе
			...
			Если ПредыдущийВариантОтображения = Перечисления.ВариантыОтображенияОтчетов.Таблица или Отчет.ВариантОтображения = Перечисления.ВариантыОтображенияОтчетов.Диаграмма Тогда
				ВсегоИзмерений = Отчет.ОтчетДиаграмма.ПостроительОтчета.ИзмеренияКолонки.Количество();
				Если ВсегоИзмерений > 1 Тогда
					Для Сч = 1 По ВсегоИзмерений - 1 Цикл
						Отчет.ОтчетДиаграмма.ПостроительОтчета.ИзмеренияКолонки.Удалить(Отчет.ОтчетДиаграмма.ПостроительОтчета.ИзмеренияКолонки.Получить(ВсегоИзмерений - Сч));
					КонецЦикла;
				КонецЕсли;
				ВсегоИзмерений = Отчет.ОтчетДиаграмма.ПостроительОтчета.ИзмеренияСтроки.Количество();
				Если ВсегоИзмерений > 1 Тогда
					Для Сч = 1 По ВсегоИзмерений - 1 Цикл
						Отчет.ОтчетДиаграмма.ПостроительОтчета.ИзмеренияСтроки.Удалить(Отчет.ОтчетДиаграмма.ПостроительОтчета.ИзмеренияСтроки.Получить(ВсегоИзмерений - Сч));
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
...
//  Нет.
//
Процедура ЗаполнитьМенюОтчетаОтображение(Отчет, ФормаОтчета,ДействиеКнопок) Экспорт
    ...
    ПерваяКнопка = КнопкиДействия.Найти("Действия" + Отчет.ВариантОтображения);
	...
	Если (Отчет.Метаданные().Имя = "СтатистикаКадров") и (Отчет.ПоказателиОтчета.Найти(Истина,"ИтоговоеПоле") = Неопределено) Тогда
		Значение = Перечисления.ВариантыОтображенияОтчетов.Таблица;
		Кнопка = КнопкиОтображение.Вставить(0,Значение,ТипКнопкиКоманднойПанели.Действие,Значение,ДействиеКнопок);
		Кнопка.Пометка = Истина;
		Кнопка = КнопкиДействия.Вставить(НомерКнопкиТаблица,"Действия" + Значение,ТипКнопкиКоманднойПанели.Действие,Значение,ДействиеКнопок);
		Кнопка.Пометка = Истина;
	Иначе
		Для каждого Значение Из Перечисления.ВариантыОтображенияОтчетов Цикл
			НадоПометитьКнопку = Значение = Отчет.ВариантОтображения;
			Кнопка = КнопкиОтображение.Вставить(0,Значение,ТипКнопкиКоманднойПанели.Действие,Значение,ДействиеКнопок);
	...
	Для НСтр = 1 По ОбластьПоказателиВысота Цикл
		...
		Если НЕ Булево(СокрЛП(ОбластьПоказатели.Область(НСтр, 4).Текст)) Тогда
			...
			ИначеЕсли Врег(СтдВалюта) = "МСФО" Тогда
				ПредставлениеПоля = ПредставлениеПоля + " в " +Константы.ВалютаМеждународногоУчета.Получить();
			КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ ДЛЯ БУХГАЛТЕРСКИХ ОТЧЕТОВ
