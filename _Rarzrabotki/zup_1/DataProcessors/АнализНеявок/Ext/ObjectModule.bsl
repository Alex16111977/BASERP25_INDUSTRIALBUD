...
Функция СформироватьЗапрос()
	...
	|	И БольничныйЛист.Организация = &ГоловнаяОрганизация
	...
	|		КОГДА НевыходыНачисления.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени = ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.Прогул)
	...
	|		И НевыходыНачисления.Ссылка.Организация = &ГоловнаяОрганизация
	...
	|   И НачислениеОтпуска.ВидРасчета.ВидОтпуска = ВТСостояниеРаботников.ВидОтпуска) 
	|		И (НачислениеОтпуска.ДатаНачала = ВТСостояниеРаботников.ДатаНачала
	|		ИЛИ НачислениеОтпуска.ДатаОкончания = ВТСостояниеРаботников.ДатаОкончания)
	|	ГДЕ
	|	НачислениеОтпуска.Ссылка.ПериодРегистрации МЕЖДУ НАЧАЛОПЕРИОДА(&ДатаНачала, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ДатаОкончания, МЕСЯЦ)
	|	И 
	|	НачислениеОтпуска.Ссылка.Организация = &ГоловнаяОрганизация
	...
	|		КОГДА ОплатаПоСреднему.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПоСреднемуМоб)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован)
	|		КОГДА ОплатаПоСреднему.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПосреднемуМобОрганизации)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован)
	|		ИНАЧЕ NULL
	|	КОНЕЦ													 КАК Состояние,
	...
	|		И ОплатаПоСреднему.Организация = &ГоловнаяОрганизация
	...
	|	ВЫБОР
	|	КОГДА РегистрацияПростоев.ВидПростоя = ЗНАЧЕНИЕ(Перечисление.ВидыПростояРаботника.ПриостановкаТрудовогоДоговора)
	|	ТОГДА ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора)
	|ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Простой)
	|	КОНЕЦ														КАК Состояние,
	...
	|	ВТСостояниеРаботников КАК СостояниеРаботников
	|ПО
	|   РегистрацияПростоев.Сотрудник = СостояниеРаботников.Сотрудник
	|   И (СостояниеРаботников.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Простой)
	|	ИЛИ СостояниеРаботников.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора))
	|ГДЕ
	|	РегистрацияПростоев.ПериодРегистрации МЕЖДУ НАЧАЛОПЕРИОДА(&ДатаНачала, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ДатаОкончания, МЕСЯЦ)
	|		И РегистрацияПростоев.Организация = &ГоловнаяОрганизация
	...
	|			И (ВЫБОР КОГДА (КадровыеДанные.КадровыйДокумент ССЫЛКА Документ.ОтпускаОрганизаций)
	|					  И НЕ (РасчетныеДанные.РасчетныйДокумент ССЫЛКА Документ.НевыходыВОрганизациях)
	|						ТОГДА РасчетныеДанные.ВидОтпуска = КадровыеДанные.ВидОтпуска
	|						ИНАЧЕ
	|						ИСТИНА
	|						КОНЕЦ)
	...
	|		КОГДА Неявка = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора) ТОГДА NULL
	|		КОГДА Неявка = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Мобилизован) ТОГДА ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПосреднемуМобОрганизации)
	...
	Если ЗначениеЗаполнено(ПодразделениеОрганизации) Тогда
	    ...
	    |	РаботникиОрганизации.ПодразделениеОрганизации В ИЕРАРХИИ (&ПодразделениеОрганизации)
		|";
	КонецЕсли;	
...
Функция Автозаполнение() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Необходимо указать организацию!';uk='Необхідно вказати організацію!'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Неявки Цикл
		Если СтрокаТаблицы.Неявка = Перечисления.СостоянияРаботникаОрганизации.Мобилизован Тогда 
			Продолжить;
		КонецЕсли;
...
Процедура СоздатьДокументы() Экспорт
	...
	Для Каждого СтрокаТаблицы Из Неявки Цикл
		...
		Если СтрокаТаблицы.Отметка 
			...
			СтрокаСообщенияОбОшибке = СтрокаТаблицы.Сотрудник.Наименование + " " + СтрокаТаблицы.Неявка + НСтр("ru=' от ';uk=' від '") + Формат(СтрокаТаблицы.ДатаНачала,"ДФ=дд.ММ.гггг")+": ";
			...
			ИначеЕсли ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) = Тип("ДокументСсылка.ОплатаПоСреднемуЗаработку") Тогда
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ВидРасчета) 	
					...
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщенияОбОшибке+НСтр("ru='необходимо указать вид расчета по среднему заработку';uk='необхідно вказати вид розрахунку за середнім заробітком'"));
					Продолжить;
				КонецЕсли;
				...
				РасчетныйДокумент.ДатаНачала				= СтрокаТаблицы.ДатаНачала;
				РасчетныйДокумент.ДатаОкончания				= ?(СтрокаТаблицы.Неявка = Перечисления.СостоянияРаботникаОрганизации.Мобилизован, КонецМесяца(СтрокаТаблицы.ДатаНачала), СтрокаТаблицы.ДатаОкончания);
			...
			ИначеЕсли ТипЗнч(СтрокаТаблицы.РасчетныйДокумент) = Тип("ДокументСсылка.НачислениеОтпускаРаботникамОрганизаций") Тогда
				...
				Если СтрокаТаблицы.НовыйДокумент Тогда
					...
					РасчетныйДокумент.ПриказОтпускОрганизации = СтрокаТаблицы.КадровыйДокумент; 
				КонецЕсли;
			...
			Иначе // прочие невыходы
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ВидРасчета)
					И (ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.Простой 
					ИЛИ ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.Прогулы 
					ИЛИ ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.ОтсутствуетПоНевыясненнойПричине) Тогда
					
					// попробуем найти подходящий вид расчета для вида неявки
					ЗапросПоВР = Новый Запрос;
					ЗапросПоВР.Текст = "
					|ВЫБРАТЬ
					|	МАКСИМУМ(ВЫБОР 
					|		КОГДА ОсновныеНачисленияОрганизаций.ОбозначениеВТабелеУчетаРабочегоВремени = &БукваВТабелеПрогул
					|			ТОГДА ОсновныеНачисленияОрганизаций.Ссылка
					|		КОНЕЦ)  КАК  ВидРасчетаПрогул,
					|	МАКСИМУМ(ВЫБОР 
					|		КОГДА ОсновныеНачисленияОрганизаций.ОбозначениеВТабелеУчетаРабочегоВремени = &БукваВТабелеПростой
					|			ТОГДА ОсновныеНачисленияОрганизаций.Ссылка
					|		КОНЕЦ)  КАК  ВидРасчетаПростой
					|	ИЗ
					|		ПланВидовРасчета.ОсновныеНачисленияОрганизаций КАК ОсновныеНачисленияОрганизаций
					|	ГДЕ
					|		ОсновныеНачисленияОрганизаций.СпособРасчета = &РасчетНулеваяСумма
					|       И (ОсновныеНачисленияОрганизаций.ОбозначениеВТабелеУчетаРабочегоВремени = &БукваВТабелеПрогул ИЛИ
					|       ОсновныеНачисленияОрганизаций.ОбозначениеВТабелеУчетаРабочегоВремени = &БукваВТабелеПростой)
					|		И ОсновныеНачисленияОрганизаций.ВидВремени = &ВидВремени
					|";
					
					ЗапросПоВР.УстановитьПараметр("РасчетНулеваяСумма", Перечисления.СпособыРасчетаОплатыТруда.НулеваяСумма);
					ЗапросПоВР.УстановитьПараметр("БукваВТабелеПрогул",Справочники.КлассификаторИспользованияРабочегоВремени.Прогул);
					ЗапросПоВР.УстановитьПараметр("БукваВТабелеПростой",Справочники.КлассификаторИспользованияРабочегоВремени.Простой);
					ЗапросПоВР.УстановитьПараметр("ВидВремени", Перечисления.ВидыВремени.ЦелодневноеНеотработанное);
					ЗапросПоВР.УстановитьПараметр("ПростойСПриостановкойТрудДог", ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПростойСПриостановкойТрудДог);
					
					ВыборкаПоВР = ЗапросПоВР.Выполнить().Выбрать();
					Если ВыборкаПоВР.Следующий() Тогда
						ВидРасчетаПростой =?(ВыборкаПоВР.ВидРасчетаПростой = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка(),ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка(),ВыборкаПоВР.ВидРасчетаПростой);
						ВидРасчетаПрогул  =?(ВыборкаПоВР.ВидРасчетаПрогул  = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка(),ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка(),ВыборкаПоВР.ВидРасчетаПрогул);
					Иначе
						ВидРасчетаПростой = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка();
						ВидРасчетаПрогул = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка();
					КонецЕсли;	
					
					
					Запрос = Новый Запрос;
					
					Если ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.Простой  Тогда
						ВидУчетавремени = Справочники.КлассификаторИспользованияРабочегоВремени.Простой	
					ИначеЕсли ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.Прогулы  Тогда
						ВидУчетавремени = Справочники.КлассификаторИспользованияРабочегоВремени.Прогул	
					ИначеЕсли ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.ОтсутствуетПоНевыясненнойПричине Тогда
						ВидУчетавремени = Справочники.КлассификаторИспользованияРабочегоВремени.НеявкиПоНевыясненнымПричинам	
					КонецЕсли;
					
					Запрос.УстановитьПараметр("ВидУчетаВремени",ВидУчетаВремени);
					Запрос.УстановитьПараметр("ВидНеявки",ВидНеявки);
					Запрос.УстановитьПараметр("РасчетНулеваяСумма", Перечисления.СпособыРасчетаОплатыТруда.НулеваяСумма);
					
					Запрос.Текст = " 
					|ВЫБРАТЬ
					|	ВЫБОР
					|   	КОГДА &ВидНеявки = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтсутствуетПоНевыясненнойПричине)
					|   			ТОГДА &ВидРасчета
					|		КОГДА &ВидНеявки = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Прогулы)
					|   			ТОГДА &ВидРасчетаПрогул
					|		КОГДА &ВидНеявки = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Простой)
					|   			ТОГДА &ВидРасчетаПростой
					|   	КОНЕЦ КАК ВидРасчета
					|ИЗ
					|	ПланВидовРасчета.ОсновныеНачисленияОрганизаций КАК ОсновныеНачисления
					|ГДЕ
					|	ОсновныеНачисления.СпособРасчета = &РасчетНулеваяСумма 
					|	И	ОсновныеНачисления.ОбозначениеВТабелеУчетаРабочегоВремени = &ВидУчетаВремени";
					
					Запрос.УстановитьПараметр("ВидРасчета", ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НевыходРегл);
					Запрос.УстановитьПараметр("ВидРасчетаПрогул", ВидРасчетаПрогул);
					Запрос.УстановитьПараметр("ВидРасчетаПростой", ВидРасчетаПростой);
					
					Выборка = Запрос.Выполнить().Выбрать();
					Если Выборка.Следующий() Тогда
						СтрокаТаблицы.ВидРасчета = Выборка.ВидРасчета;
					КонецЕсли;
				ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаТаблицы.ВидРасчета) И ВидНеявки = Перечисления.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора Тогда
					
					СтрокаТаблицы.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПростойСПриостановкойТрудДог
					
				КонецЕсли;
				
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ВидРасчета) 
					Или СтрокаТаблицы.ВидРасчета.СпособРасчета <> Перечисления.СпособыРасчетаОплатыТруда.НулеваяСумма Тогда
					ОбщегоНазначения.СообщитьОбОшибке(СтрокаСообщенияОбОшибке+НСтр("ru='необходимо выбрать вид расчета по нулевой сумме с видом учета времени ';uk='необхідно вибрати вид розрахунку за нульовою сумою з видом обліку часу '")+ВидУчетавремени); 
					Продолжить;
				КонецЕсли;
				
				Если ВидНеявки <> Перечисления.СостоянияРаботникаОрганизации.Простой И ВидНеявки <> Перечисления.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора Тогда
					...
					Если Выборка.Следующий() Тогда
						...
						СтрокаНачисления.ГрафикРаботы = Выборка.ГрафикРаботы;
					КонецЕсли;
				...
				Иначе
					...
					РасчетныйДокумент.ДатаНачала = СтрокаТаблицы.ДатаНачала;
					РасчетныйДокумент.ДатаОкончания = СтрокаТаблицы.ДатаОкончания;
					Если СтрокаТаблицы.Неявка = Перечисления.СостоянияРаботникаОрганизации.ПриостановкаТрудовогоДоговора Тогда
						РасчетныйДокумент.ВидПростоя = Перечисления.ВидыПростояРаботника.ПриостановкаТрудовогоДоговора
					Иначе
						РасчетныйДокумент.ВидПростоя = Перечисления.ВидыПростояРаботника.НеПоВинеРаботника;
						РасчетныйДокумент.ПроцентОплаты = 2/3 * 100;// 66.67%
					КонецЕсли;	
					РасчетныйДокумент.ВидРасчета = СтрокаТаблицы.ВидРасчета;
					РасчетныйДокумент.Организация	= Организация;
					РасчетныйДокумент.Ответственный	= глЗначениеПеременной("глТекущийПользователь");
				...
				КонецЕсли;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(РасчетныйДокумент.Ссылка) Тогда	
				РасчетныйДокумент.Организация	= Организация;
				РасчетныйДокумент.Ответственный	= глЗначениеПеременной("глТекущийПользователь");
				РасчетныйДокумент.Дата = ОбщегоНазначения.ПолучитьРабочуюДату();
				РасчетныйДокумент.УстановитьНовыйНомер();
...
Процедура РассчитатьДокументы() Экспорт
	...
	ПредыдущийРасчетныйДокумент	= Неопределено;
	ПредыдущийСотрудник			= Неопределено;
	Для Каждого СтрокаТаблицы Из Неявки Цикл
		...
		Если СтрокаТаблицы.Отметка И ЗначениеЗаполнено(СтрокаТаблицы.РасчетныйДокумент) Тогда
			...
			Попытка
				Если ТипЗнч(РасчетныйДокумент) = Тип("ДокументОбъект.НевыходыВОрганизациях") Тогда
					РасчетныйДокумент.РассчитатьНачисления(СтрокаТаблицы.Сотрудник);
				ИначеЕсли ТипЗнч(РасчетныйДокумент) = Тип("ДокументОбъект.НачислениеПоБольничномуЛисту") Тогда
					
					РасчетныйДокумент.Записать();
	
					РасчетныйДокумент.ЧислоМесяцев = 6;
	
					РасчетныйДокумент.ВидУчетаВремениДляСредней	= Перечисления.ВидыУчетаВремениДляСредней.ПоКалендарнымДням;
		
					РасчетныйДокумент.Записать();
	                РасчетныйДокумент.РасчитатьПараметрыСредней();
				...
				ИначеЕсли ТипЗнч(РасчетныйДокумент) = Тип("ДокументОбъект.НачислениеОтпускаРаботникамОрганизаций") Тогда
					Если ПредыдущийРасчетныйДокумент = СтрокаТаблицы.РасчетныйДокумент И ПредыдущийСотрудник = СтрокаТаблицы.Сотрудник Тогда
						Продолжить;
					КонецЕсли;	
					РасчетныйДокумент.Записать();
					...
					РасчетныйДокумент.Записать();
	                ПредыдущийРасчетныйДокумент = СтрокаТаблицы.РасчетныйДокумент;
					ПредыдущийСотрудник = СтрокаТаблицы.Сотрудник
				ИначеЕсли ТипЗнч(РасчетныйДокумент) = Тип("ДокументОбъект.ОплатаПоСреднемуЗаработку") Тогда
					РасчетныйДокумент.ЧислоМесяцев = 2;
					РасчетныйДокумент.ПолучитьПериодПоРаботнику();
					РасчетныйДокумент.ГодоваяПремияБазовыйПериодНачало = НачалоГода(НачалоГода(РасчетныйДокумент.ПериодРасчетаСреднегоНачало)-1);
					РасчетныйДокумент.ГодоваяПремия = ПроведениеРасчетов.РассчитатьПремияЗаГод(РасчетныйДокумент.ГодоваяПремияБазовыйПериодНачало, РасчетныйДокумент.Сотрудник);
					РасчетныйДокумент.Записать();
	                ...
	                ПредыдущийРасчетныйДокумент = СтрокаТаблицы.РасчетныйДокумент;
					ПредыдущийСотрудник = СтрокаТаблицы.Сотрудник
				ИначеЕсли ТипЗнч(РасчетныйДокумент) = Тип("ДокументОбъект.РегистрацияПростоевВОрганизациях") Тогда
				    РасчетныйДокумент.РассчитатьНачисления(СтрокаТаблицы.Сотрудник,Ложь)
				Иначе
