...
Процедура ПроверитьЗаполнениеСтроки(ТипТаблицы = Неопределено, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = НСтр("ru='В строке номер ""';uk='У рядку номер ""'")+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки) +
									НСтр("ru='"" табл. части ';uk='"" табл. частини '")+?(ТипТаблицы = "Удержание", "Удержания", "Начисления")+": ";
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВидРасчета)  Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указано ';uk='не вказано '") + ?(ТипТаблицы = "Удержание", "удержание!", "начисление!"), Отказ, Заголовок);
				...
				Если ВозможностьИзменения = Перечисления.ИзменениеПоказателейСхемМотивации.НеИзменяется Тогда
					...
					Если ВыборкаПоСтрокамДокумента["Показатель" + СчПоказателей] = 0 Тогда
						...
						Если ТипПоказателя <> Перечисления.ТипыПоказателейСхемМотивации.ОценочнаяШкалаПроцентная
							...
							ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан размер показателя ';uk='не вказаний розмір показника '") + Показатель, Отказ, Заголовок);
						КонецЕсли;
					КонецЕсли;
					...
					Если ТипТаблицы = "Начисление" Тогда
						Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента["ТарифныйРазряд" + СчПоказателей])
							и ТипПоказателя = Перечисления.ТипыПоказателейСхемМотивации.ТарифныйРазряд Тогда
							ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + НСтр("ru='не указан размер тарифа ';uk='не вказаний розмір тарифу '") + Показатель, Отказ, Заголовок);					
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура УтвердитьВидСхемыМотивации() Экспорт
	...
	Иначе
		ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не указан вид схемы мотивации';uk='Не зазначений вид схеми мотивації'"));
	КонецЕсли;
