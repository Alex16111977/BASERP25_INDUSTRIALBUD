...
Процедура ЗагрузитьКурсыСИТС(ИндикаторФормы = Неопределено ,НадписьВалютыФормы = Неопределено, ВыводитьСообщения = Ложь ) Экспорт
	...
	ИначеЕсли НачДата < НачалоГода(ДобавитьМесяц(ТекущаяДата(), -12)) ИЛИ КонДата > КонецМесяца(ДобавитьМесяц(ТекущаяДата(), -1)) Тогда
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На диске ИТС доступны курсы с %1 по %2. Курс для валюты не загружен.';uk='На диску ІТС доступні курси з %1 по %2. Курс для валюти не завантажений.'"), Формат(НачалоГода(ДобавитьМесяц(ТекущаяДата(), -12)),"ДЛФ=Д") , Формат(КонецМесяца(ДобавитьМесяц(ТекущаяДата(), -1)),"ДЛФ=Д"))); 	
	Иначе
