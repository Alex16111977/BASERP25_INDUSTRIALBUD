				...
				ИначеЕсли индФлага = 2 и СтрокаДерева.ЗагружатьОпрос тогда
					...
					Для каждого ВложениеКонверта из СтрокаДерева.КонвертЭП.Вложения Цикл
						...
						Попытка
							...
							Сообщить(НСтр("ru='Загружена информация в объект: ';uk=""Завантажена інформація в об'єкт: """) + ДокОпроса);
						Исключение
							Сообщить(НСтр("ru='При загрузке файла ';uk='При завантаженні файлу '") + ИмяФайла + НСтр("ru=' произошла ошибка';uk=' виникла помилка'"));
...
Процедура ПрочитатьФайл(ИмяФайла, ПотокВывода) Экспорт
	...
	//Попытка
		...
		Если НЕ ХМЛ.Прочитать() Тогда
			Сообщение(НСтр("ru='Невозможно прочитать файл!';uk='Неможливо прочитати файл!'"));
		КонецЕсли;
		...
		Если Версия <> "1.0" Тогда
			Сообщение(НСтр("ru='Неверная версия формата данных (';uk='Невірна версія формату даних ('")+Версия+").");
		КонецЕсли;
		...
		Если (ХМЛ.ТипУзла <> ТипУзлаXML.НачалоЭлемента) ИЛИ (ХМЛ.Имя <> "РезультатОпроса") Тогда
			Сообщение(НСтр("ru='Не найдены результаты опроса!';uk='Не знайдені результати опитування!'"));
		КонецЕсли;
		...
		Если Анкета.Пустая() Тогда
			Сообщение(НСтр("ru='Не обнаружена анкета с идентификатором ';uk='Не виявлена анкета з ідентифікатором '") + ИдАнкеты);
			Сообщение(НСтр("ru='Возможно изменился идентификатор или она удалена';uk='Можливо змінився ідентифікатор або вона вилучена'"));
		КонецЕсли;
...
Процедура Сообщение(ТекстСообщения)
	...
	Сообщить(НСтр("ru='Загрузка файла прервана.';uk='Завантаження файлу перервано.'"), СтатусСообщения.Важное);
	Попытка
		а	=	1/0;
	Исключение
	...
	Иначе
		...
		Если (ТипЗначения.Типы().Получить(0) = Тип("Дата")  ) ИЛИ 
			...
			Попытка
				СтрокаСписка.ТиповойОтвет = XMLЗначение(Тип(ТипЗначения.Типы().Получить(0)), ЗначениеВОтвет);
			Исключение
				Сообщить("Ответ на вопрос: """+Строка(Вопрос)+НСтр("ru='"". Не удалось преобразовать ответ ""';uk='"". Не вдалося перетворити відповідь ""'")+ЗначениеВОтвет+НСтр("ru='"" к типу ""';uk='"" до типу ""'")+ТипЗначения.Типы().Получить(0)+ НСтр("ru='"". Ответ не загружен!';uk='"". Відповідь не завантажено!'"));
			КонецПопытки;
		Иначе
			СтрокаСписка.ТиповойОтвет = Значение;
		КонецЕсли;
...
Процедура ПоместитьВопросыВОбъектЗагрузки(ХМЛ, ПотокВывода, ДатаЗаписи, Анкета, ТабличныйВопрос = Неопределено, НомерСтрокиВтаблице = 0)
	...
	Для каждого СтрокаОтвет из ОтветыТаблицы Цикл
		...
		ИначеЕсли ЕстьФизическиеЛица И Вопрос = ПланыВидовХарактеристик.ВопросыДляАнкетирования.КодДРФО  и ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.ФизическиеЛица") тогда
			ПотокВывода.КодПоДРФО = Ответ;
