...
Процедура ЗаполнитьСправочник(ИмяОбъекта)
	...
	// заполнение производится на языке ИБ
...
Процедура ЗаполнитьРегистрСведений(ИмяОбъекта)
	...
	Если НЕ ИмяОбъекта = "ПереводыТекстов" Тогда 
		// заполнение производится на языке ИБ
		КодЯзыка = Локализация.КодЯзыкаИнформационнойБазы();
		Макет.КодЯзыкаМакета = КодЯзыка;
	КонецЕсли;	
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если НЕ ТекущаяЗапись.ЭтоГруппа Тогда
			//Дополнительные реквизиты
			Для СчК = 4 по ЧислоРеквизитов Цикл
...
Процедура ЗаполнитьПланВидовРасчета(ИмяОбъекта, РежимЗагрузки = Ложь, ОтборИмя = "") Экспорт
	...
	НеОбновлятьПВР = Ложь;
	НеОбновлятьТЧПВР = Ложь;
	НаборЗаписей = РегистрыСведений.НастройкаОбновленияВидовРасчета.СоздатьНаборЗаписей(); 
	НаборЗаписей.Прочитать(); 
	ТаблицаНастроекОбновления = НаборЗаписей.Выгрузить();
	
	Если ВРег(ИмяОбъекта) = ВРег("ОсновныеНачисленияОрганизаций") Тогда
		НастройкиОбновления = ТаблицаНастроекОбновления.Найти(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка(),"ВидРасчета");
	ИначеЕсли ВРег(ИмяОбъекта) = ВРег("УдержанияОрганизаций") Тогда
		НастройкиОбновления = ТаблицаНастроекОбновления.Найти(ПланыВидовРасчета.УдержанияОрганизаций.ПустаяСсылка(),"ВидРасчета");
	ИначеЕсли ВРег(ИмяОбъекта) = ВРег("ВзносыВФонды") Тогда
		НастройкиОбновления = ТаблицаНастроекОбновления.Найти(ПланыВидовРасчета.ВзносыВФонды.ПустаяСсылка(),"ВидРасчета");
	ИначеЕсли ВРег(ИмяОбъекта) = ВРег("СреднийЗаработок") Тогда
		НастройкиОбновления = ТаблицаНастроекОбновления.Найти(ПланыВидовРасчета.СреднийЗаработок.ПустаяСсылка(),"ВидРасчета");
	КонецЕсли;	
	Если НастройкиОбновления <> Неопределено Тогда
		НеОбновлятьПВР = НастройкиОбновления.НеОбновлятьАвтоматически;
		НеОбновлятьТЧПВР = НастройкиОбновления.НеОбновлятьТабличныеЧасти;
	КонецЕсли;

	
	Если НеОбновлятьПВР Тогда
		Сообщить(НСтр("ru='Обновление плана видов расчета ';uk='Оновлення плану видів розрахунку '")+ИмяОбъекта+НСтр("ru=' отключено. Проверьте настройки самостоятельно.';uk=' вимкнено. Перевірте настройки самостійно.'"));
		Возврат;
	КонецЕсли;
	Если НеОбновлятьТЧПВР Тогда
		Сообщить(НСтр("ru='Обновление базовых, ведущих, вытесняющих расчетов для плана видов расчета ';uk='Оновлення базових, ведучих, витісняючих розрахунків для плану видів розрахунку '")+ИмяОбъекта+НСтр("ru=' отключено. Проверьте настройки самостоятельно.';uk=' вимкнено. Перевірте настройки самостійно.'"));
	КонецЕсли;
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если ТаблицаНастроекОбновления <> Неопределено Тогда 
			НастройкиОбновления = ТаблицаНастроекОбновления.Найти(СсылкаНаЭлемент,"ВидРасчета");
			Если НастройкиОбновления <> Неопределено Тогда
				НеОбновлять = НастройкиОбновления.НеОбновлятьАвтоматически;
				НеОбновлятьТЧ = НастройкиОбновления.НеОбновлятьТабличныеЧасти;
			Иначе
				НеОбновлять = Ложь;
				НеОбновлятьТЧ = Ложь;
			КонецЕсли;
		...
		КонецЕсли;	
       	Если НеОбновлять Тогда
			Сообщить(НСтр("ru='Обновление вида расчета ';uk='Оновлення виду розрахунку '")+Имя+НСтр("ru=' отключено. Проверьте настройки самостоятельно.';uk=' вимкнено. Перевірте настройки самостійно.'"));
			Продолжить;
		КонецЕсли;
		Если НеОбновлятьТЧ и НЕ НеОбновлятьТЧПВР Тогда
			Сообщить(НСтр("ru='Обновление базовых, ведущих, вытесняющих расчетов для вида расчета ';uk='Оновлення базових, ведучих, витіснячих розрахунків для виду розрахунку '")+Имя+НСтр("ru=' отключено. Проверьте настройки самостоятельно.';uk=' вимкнено. Перевірте настройки самостійно.'"));
		КонецЕсли;
		...
		Если Найти(ИмяОбъекта,"Управленческие") = 0 Тогда 
			Если НЕ НеОбновлятьТЧПВР И НЕ НеОбновлятьТЧ Тогда 
		...
		Если ФлагИзменений Тогда
			Попытка
				...
				ТекущаяЗапись.Записать();
			Исключение
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось записать объект план видов расчета %1: %2!';uk=""Не вдалося записати об'єкт план видів розрахунку %1: %2!"""), ИмяОбъекта, ТекущаяЗапись),СтатусСообщения.ОченьВажное);
...
Процедура ЗаполнитьВсеОбъекты() Экспорт
	...
	ЗаполнитьПланВидовРасчета("УправленческиеНачисления");
	ЗаполнитьПланВидовРасчета("УправленческиеУдержания");
