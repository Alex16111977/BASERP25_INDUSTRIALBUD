...
Процедура Автозаполнение() Экспорт
	...
	Если ДокументыНачисления.Количество()= 0 Тогда
		#Если Клиент Тогда
		Предупреждение(НСтр("ru='Необходимо выбрать ведомости, по которойым будут формироваться платежки!';uk='Необхідно вибрати відомості, за якими будуть формуватися платіжки!'"));
		#КонецЕсли
		Возврат;
	КонецЕсли;
...
Процедура АвтозаполнениеВедомость() Экспорт
	...
	ОкруглятьВБольшую = РегистрыСведений.ПараметрыРасчетаЗарплатыОрганизаций.Получить(Новый Структура ("Организация", Организация)).ПриУплатеЕСВОкруглятьВБольшуюСторону;
		...
		Пока ВыборкаПоКонтрагент.Следующий() Цикл
			...
			Пока ВыборкаПоСчетКонтрагента.Следующий() Цикл
				...
				Пока ВыборкаПоНалогам.Следующий() Цикл
					...
					Если ОкруглятьВБольшую Тогда
						Сум = Окр(Сум,2) + ?(Сум-Окр(Сум,2)>0, 0.01, 0);
					Иначе
						Сум = Окр(Сум,2);
					КонецЕсли;	
					ИтогСум = ИтогСум + Сум;
