...
Процедура Перерассчитать() Экспорт
	...
	Заголовок = ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Перерасчет документов';uk='Перерахунок документів'"));
	...
	Пока ИндексСтрокиТаблицы > 0 Цикл
		...
		Если СтрокаТаблицы.Пометка Тогда
			...
			Если ПроведениеРасчетов.ДокументНельзяИзменятьЗаднимЧислом(СтрокаТаблицы.Документ, ОписаниеПричиныОтказа) Тогда
				ОбщегоНазначения.КомментарийРасчета(СтрЗаменить(ОписаниеПричиныОтказа,НСтр("ru='Документ ';uk='Документ '"),Строка(СтрокаТаблицы.Документ)), , "Открыть", СтрокаТаблицы.Документ,Перечисления.ВидыСообщений.ВажнаяИнформация);
				Продолжить;	
			КонецЕсли;
			...
			Попытка
				...
				ОбщегоНазначения.КомментарийРасчета(Строка(ДокументОбъект) + НСтр("ru=' обработан успешно. ';uk=' оброблений успішно. '"), Заголовок, "Открыть", СтрокаТаблицы.Документ);
			Исключение
				ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Документ ';uk='Документ '") + Строка(ДокументОбъект) + НСтр("ru=' не может быть перерассчитан!"
