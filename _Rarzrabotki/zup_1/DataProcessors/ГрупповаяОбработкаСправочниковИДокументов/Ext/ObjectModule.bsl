...
Процедура ПечатьРеестра(МассивДокументов,ИндикаторОбработки = Неопределено) Экспорт
	...
	Для каждого Ссылка Из МассивДокументов Цикл
		...
		Если ЗначениеЗаполнено(Область.Параметры.ВидОперации) Тогда
			Область.Параметры.ВидОперации = Локализация.ПолучитьЛокализованноеПредставление(Область.Параметры.ВидОперации, КодЯзыкаПечати);
		КонецЕсли;	
