	...
	Пока ВыборкаМоделейБюджетирования.Следующий() Цикл
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'Не удалось создать бюджетные задачи по модели: %1 по причине: %2';
								|en = 'Cannot create budget tasks using the %1 model due to: %2'", КодОсновногоЯзыка);
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки, ВыборкаМоделейБюджетирования.Наименование, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(НСтр("ru = 'БюджетныйПроцесс.СозданиеБюджетныхЗадач';
											|en = 'БюджетныйПроцесс.СозданиеБюджетныхЗадач'", КодОсновногоЯзыка),
	...
	Для Каждого ЭкземплярБюджета Из МассивОбъектов Цикл
		...
		Если Проводить Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'БюджетныйПроцесс.ОтправкаОповещения';
												|en = 'БюджетныйПроцесс.ОтправкаОповещения'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'В настройках действия не определен нефинансовый показатель для создания документа.';
				|en = 'Non-financial indicator for document creation is not determined in the action settings.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СформироватьБюджетыПоНастройкамДействияВФоне(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось создать документы по бюджетной задаче: %1%, по причине: %2%';
							|en = 'Cannot create documents for the %1% budget task, due to: %2%'", КодОсновногоЯзыка);
...
Процедура ДополнитьПараметрыСледующегоФормируемогоДокументаБюджетнойЗадачи(Знач Задача, Параметры, ПродолжитьОбработку)
			...
			Если СписокДокументов.Количество() = 0 Тогда
				...
				Иначе
					...
					ТекстОшибки = НСтр("ru = 'В задаче %1% в настройках действия не определен нефинансовый показатель для создания документа.';
										|en = 'Non-financial indicator for document creation is not determined in the %1% task of the action settings. '",
						КодОсновногоЯзыка);
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось создать документ для задачи %1 по причине: %2';
							|en = 'Cannot create the document for the %1 task due to: %2'", КодОсновногоЯзыка);
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки, ЗадачаОбъект, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'БюджетныйПроцесс.СозданиеДокументовДляЗадачи';
										|en = 'БюджетныйПроцесс.СозданиеДокументовДляЗадачи'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
...
Процедура ЗаписьНайденногоДокументаВБюджетнуюЗадачуВОтдельнойТранзакции(Параметры, ДокументСсылка, ПродолжитьОбработку)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось записать документ в задачу %1 по причине: %2';
							|en = 'Cannot write the document in the %1 task due to: %2'", КодОсновногоЯзыка);
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
