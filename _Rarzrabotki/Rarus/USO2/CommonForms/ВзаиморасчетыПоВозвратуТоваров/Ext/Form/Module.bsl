...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаРасшифровки.Заказ) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Объект расчетов"" в строке %НомерСтроки% списка ""Взаиморасчеты""';
									|en = 'The ""Settlement object"" column is not populated in line %НомерСтроки% of the ""Mutual settlements"" list'");
		...
		Если Не ЗначениеЗаполнено(СтрокаРасшифровки.Сумма) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Сумма"" в строке %НомерСтроки% списка ""Взаиморасчеты""';
									|en = 'The ""Amount"" column is not populated in line %НомерСтроки% of the ""Mutual settlements"" list'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%", СтрокаРасшифровки.НомерСтроки);
	...
	Если РасшифровкаПлатежа.Количество() > 0 И СуммаТоваров < РасшифровкаПлатежа.Итог("Сумма") Тогда
		ТекстСообщения = НСтр("ru = 'Сумма по строкам в табличной части превышает сумму документа %СуммаТоваров% %Валюта%';
								|en = 'Amount of rows in tabular section exceeds document amount %СуммаТоваров% %Валюта%'");
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Организация в заказе в строке %1 списка ""Взаиморасчеты"" не соответствует организации документа %2';
				|en = 'Company in the order in line %1 of the ""Mutual settlements"" list does not correspond to the company of document %2'"),
			Выборка.НомерСтроки,
			Организация);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстСообщения,
...
Процедура ПодборПоОстаткам(Команда)
	...
	Если Проведен Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Подбор по остаткам возможен только в непроведенном документе.';
													|en = 'Selection by balance is available only in the unposted document.'"));
		Возврат;
	КонецЕсли;
