...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ХозяйственнаяОперация)
		...
		Элементы.НадписьИнфо.Заголовок = НСтр("ru = 'По выбранным заказам уже оформлены накладные. Необходимо скорректировать имеющиеся накладные с учетом поступления командой ""Перезаполнить выбранную"" или оформить новую накладную на оставшиеся неоформленными товары командой ""Оформить новую"".';
												|en = 'Invoices are already registered for the selected orders. Adjust the invoices considering the goods receipt by clicking ""Refill selected one"", or register a new invoice for the remaining unregistered goods by clicking ""Register new one"".'");
...
Функция ПолучитьНастройкиОтборов(ПараметрыОтборов) 
	...
	Если ЗначениеЗаполнено(ОтборПоТоварам) Тогда
		...
		ТаблицаОтборов.Колонки.Добавить("Ссылка",
			Метаданные.Документы.ДвижениеПродукцииИМатериалов.ТабличныеЧасти.Товары.Реквизиты.Распоряжение.Тип);
