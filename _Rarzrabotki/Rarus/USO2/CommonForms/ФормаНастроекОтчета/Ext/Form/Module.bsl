...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("НастройкиОтчета", НастройкиОтчета) Тогда
		ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""НастройкиОтчета"".';
								|en = 'Service parameter ""НастройкиОтчета"" is not passed. '");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""ВариантНаименование"".';
								|en = 'Service parameter ""ВариантНаименование"" is not passed.'");
	КонецЕсли;
	...
	Если ТипЗнч(ИдентификаторЭлементаСтруктурыНастроек) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""Заголовок"".';
									|en = 'Service parameter ""Title"" is not transferred.'");
		КонецЕсли;
		...
		Если Не Параметры.Свойство("ТипЭлементаСтруктурыНастроек", ТипЭлементаСтруктурыНастроек) Тогда
			ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""ТипЭлементаСтруктурыНастроек"".';
									|en = 'Service parameter ""SettingsStructureItemType"" is not transferred.'");
		КонецЕсли;
	Иначе
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройки отчета ""%1""';
				|en = 'Report ""%1"" settings'"), ВариантНаименование);
	КонецЕсли;
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("ru = 'Элемент добавлен в список';
										|en = 'Item is added to the list'");
		Иначе
			ЗаголовокОповещения = НСтр("ru = 'Элементы добавлены в список';
										|en = 'Items are added to the list'");
		КонецЕсли;
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru = 'Настройки.xml';
							|en = 'Settings.xml'"), Истина);
КонецПроцедуры
...
Функция ПараметрыГруппировкиОтборов()
	...
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';
										|en = 'The selected items cannot be grouped as they have different parents.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыСдвигаОтборов()
	...
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';
										|en = 'The selected items cannot be transferred as they have different parents.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Заголовок = НСтр("ru = 'Выводить параметры и отборы';
							|en = 'Show parameters and filters'");
	...
	Если (Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр)
		...
		Если Строки.Действие = "Переместить" Тогда
			Строки.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть перемещены.';
										|en = 'Parameters cannot be transferred.'");
...
Процедура ВыбратьРежимОтображения(ОтборыУзлаНастроек, ИмяКоллекции, ИдентификаторСтроки, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков, ТекущийРежимОтображения = Неопределено)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВШапкеОтчета", НСтр("ru = 'В шапке отчета';
														|en = 'In report header'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр("ru = 'Только флажок в шапке отчета';
																	|en = 'Only check box in report header'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВНастройкахОтчета", НСтр("ru = 'В настройках отчета';
															|en = 'In report settings'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр("ru = 'Только флажок в настройках отчета';
																		|en = 'Only check box in report settings'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить("НеПоказывать", НСтр("ru = 'Не показывать';
