...
Процедура ЗаполнитьЭтапыГрафикаПоПредыдущимЗаказам(Команда)
	...
	Если СуммаОплатыПоДокументу = 0 И СуммаЗалогаПоДокументу = 0 Тогда
		...
		Если ЭтапыОплатыНеЗаполнены Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Сумма заказанных строк нулевая. Заполнение этапов оплаты не требуется.';
														|en = 'Amount of order lines is zero. It is not required to fill in payment steps.'"));
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Для поставщика ""%Поставщик%"", нет проведенных заказов. Заполнение этапов отменено.';
								|en = 'No posted orders for the ""%Поставщик%"" supplier. Stage population is canceled.  '");
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ПорядокОплаты)
			...
			ТекстОшибки = НСтр("ru = 'Поле ""Оплата"" не заполнено';
								|en = 'The ""Payment"" field is required'");
...
Функция СформироватьПредставлениеШаблонаГрафика(МассивЭтаповГрафика)
	...
	ПарПредмета=НСтр("ru = 'день,дня,дней,м,день,дня,дней,м,0';
					|en = 'day, day, days, m, day, day, days, m, 0'");
	
	ШаблонПредставленияЭтапа = НСтр("ru = '%ВариантОплаты% %ПроцентПлатежа%% (%Отсрочка%)';
									|en = '%ВариантОплаты% %ПроцентПлатежа%% (%Отсрочка%)'");
...
Процедура ПроверитьКорректностьЖелаемойДатыПоступления(ПараметрыЗаполнения)
	...
	Если ЗначениеЗаполнено(ЖелаемаяДатаПоступления) И ЖелаемаяДатаПоступления < НачалоДня(Дата) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Желаемая дата поступления меньше даты документа. Необходимо ввести корректную желаемую дату.';
							|en = 'Required receipt date is less than the document date. Enter a correct required date.'");
	...
	Если Результат <> Неопределено И Результат = КодВозвратаДиалога.ОК Тогда
		...
		ОбщегоНазначенияУТКлиент.ВвестиДатуСКонтролемПустогоЗначения(
			...
			НСтр("ru = 'Введите желаемую дату поступления';
				|en = 'Enter required receipt date'"),
			ЧастиДаты.Дата,
			Оповещение);
	...
	Если ВыбраннаяДата <> Неопределено И ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		Если ВыбраннаяДата < НачалоДня(Дата) Тогда
			ТекстПредупреждения = НСтр("ru = 'Желаемая дата поступления меньше даты документа. Этапы оплаты не могут быть заполнены.';
										|en = 'Required receipt date is less than the document date. Payment steps cannot be filled in.'");
