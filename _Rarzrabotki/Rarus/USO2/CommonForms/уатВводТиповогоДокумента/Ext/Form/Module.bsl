...
Процедура СформироватьТД(Команда)
	...
	ИначеЕсли ТипЗнч(ДокУАТ) = Тип("ДокументСсылка.уатСливГСМ") Тогда
		Если НЕ СливГСМ_Оприходование И НЕ СливГСМ_Перемещение И НЕ ЭтоСливНаАЗС(ДокУАТ) Тогда
			ТекстОшибки = "Не указан ни один вид типового документа!";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
			Возврат;
		Иначе
...
Функция ЭтоСливНаАЗС(ДокУАТ)
	Возврат ДокУАТ.ВидОперации = Перечисления.уатВидыОперацийСливГСМ.НаАЗС;
КонецФункции
