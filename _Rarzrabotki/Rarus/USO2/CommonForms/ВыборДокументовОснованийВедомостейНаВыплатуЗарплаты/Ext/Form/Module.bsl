...
Процедура УстановитьЗаголовки()
	...
	Если ВидДокумента = Перечисления.ВидыДокументовМежрасчетныхНачислений.НачислениеЗарплаты
		И ПорядокВыплаты <> Перечисления.ХарактерВыплатыЗарплаты.Зарплата Тогда
		ПредставлениеСписка = НСтр("ru = 'Доначисления зарплаты';
									|en = 'Accrue salary additionally '")
	Иначе
		ОбъектМетаданных = Метаданные.Документы.Найти(ИмяВидаДокумента);
