	...
	Если УсиленнаяЗащитаЗакрытогоКлюча Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'У сертификата установлена усиленная защита закрытого ключа.
			           ...
			           |
			           |Cannot remember and write the password.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ЗакончитьПроверкуПодписания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("ИнформацияОбОшибке") Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось пройти проверку подписания с помощью программы %1 по причине:';
									|en = 'Cannot pass the signing check using application %1 due to:'"), Программа)
...
Функция ОписаниеОшибкиПраваДоступа()
	...
	Возврат
		НСтр("ru = 'Недостаточно прав для записи пароля.
		           ...
		           |en = 'Insufficient rights to write password.
		           |
		           |Password can be written by the user specified in
		           |the ""User"" and ""Added by"" certificate fields, or administrator.'");
