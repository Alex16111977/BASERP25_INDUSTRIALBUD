...
Процедура СохранитьДанные(Отказ, ОповещениеЗавершения = Неопределено)
	...
	ТекстКнопкиДа = НСтр("ru = 'Изменилось место сотрудника в структуре предприятия';
						|en = 'Employee''s place in the company structure is changed'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При редактировании изменено место сотрудника в структуре предприятия. 
					...
					|en = 'When editing, the employee place in the company structure was changed.
					|If a previous department record is corrected (it was incorrect), click ""Error is corrected"".
					|If the employee department is changed from %1, click ""%2""'"), 
