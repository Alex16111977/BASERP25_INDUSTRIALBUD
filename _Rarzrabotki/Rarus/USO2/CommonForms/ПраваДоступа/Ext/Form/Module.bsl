...
Процедура ИсключитьИзГруппы(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяГруппаДоступа) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Группа доступа не выбрана.';
										|en = 'Access group is not selected.'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		ПоказатьПредупреждение(,
			НСтр("ru = 'Недостаточно прав для редактирования группы доступа.
			           ...
			           |en = 'Insufficient rights to edit access groups.
			           |Only a person responsible for access group participants and administrator can edit access group.'"));
		Возврат;
	КонецЕсли;
		...
		Иначе
			ОписаниеОшибки =
				НСтр("ru = 'Невозможно исключить пользователя из группы доступа,
				           ...
				           |не ответственный за участников группы доступа и
				           |не полноправный администратор.';
				           |en = 'Cannot exclude user from the access group
				           |as the current user 
				           |is not responsible for members of the access group and is
