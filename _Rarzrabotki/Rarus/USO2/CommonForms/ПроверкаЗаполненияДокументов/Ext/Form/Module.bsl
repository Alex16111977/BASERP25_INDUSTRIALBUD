...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьАкцизныеМарки Тогда
		...
		ИнтеграцияИСУТ.АкцизныеМаркиЗаполнитьСлужебныеРеквизиты(ЭтотОбъект, "КоличествоФакт");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не заполнено поле ""Номенклатура"" в строке %1 списка ""Товары""';
																							|en = 'The ""Products"" field is not populated in line %1 of the Goods list'"), НомерСтрокиДляПользователя);
		...
		Если ИспользоватьХарактеристикиНоменклатуры И Не ЗначениеЗаполнено(СтрокаТЧ.Характеристика) И СтрокаТЧ.ХарактеристикиИспользуются Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не заполнено поле ""Характеристика"" в строке %1 списка ""Товары""';
																							|en = 'Characteristic is not populated in line %1 of the Goods list'"), НомерСтрокиДляПользователя);
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Тип номенклатуры %1 в строке %2 списка ""Товары"" отличается от допустимых типов: %3';
				|en = 'Type of products %1 in line %2 of the Goods list differs from available types: %3'"),
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ИспользоватьАкцизныеМарки Тогда
		СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПризнакМаркируемаяПродукция");
	...
	СтруктураДействий.МаркируемаяПродукцияВТЧ                = ИспользоватьАкцизныеМарки;
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия,КэшированныеЗначения)
	...
	ЗаполнитьСвойстваМаркируемойПродукции();
	Для Каждого СтрокаТЧ Из Товары Цикл
...
&НаСервере
Процедура ЗаполнитьСвойстваМаркируемойПродукции()
	...
	Для Каждого Строка Из ТаблицаТовары Цикл
		Строка.НомерСтроки = ТаблицаТовары.Индекс(Строка) + 1;
	КонецЦикла;
	НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(ТаблицаТовары, СтруктураДействий);
	
	Для Каждого СтрокаТЧ Из Товары Цикл
		
		СтрокаПоиска = ТаблицаТовары.Найти(СтрокаТЧ.Номенклатура, "Номенклатура");
		Если СтрокаПоиска <> Неопределено Тогда
			СтрокаТЧ.ТабачнаяПродукция    = СтрокаПоиска.ТабачнаяПродукция;
			СтрокаТЧ.АлкогольнаяПродукция = СтрокаПоиска.АлкогольнаяПродукция;
		КонецЕсли;
...
Функция ОбработатьДанныеШтрихкодаМаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования)
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		РезультатПоиска = АкцизныеМаркиЕГАИС.НайтиСтрокиТоваров(
			...
			Неопределено, АлкогольнаяПродукция, ПараметрыЗаполнения);
...
Функция ПослеОбработкиШтрихкодов(РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	Если РезультатОбработкиШтрихкода.ТребуетсяОбработкаШтрихкода Тогда
		...
		ДанныеДляОбработки.МаркируемаяПродукцияВТЧ                = ИспользоватьАкцизныеМарки;
