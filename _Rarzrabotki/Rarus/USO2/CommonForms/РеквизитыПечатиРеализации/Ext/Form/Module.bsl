...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ИначеЕсли Параметры.ТипОбъекта = "ПередачаТоваровХранителю"
		Или (Параметры.ТипОбъекта = "ПомощникПродаж"
			И Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи)Тогда
	...
	Если Модифицированность И Не СохранитьПараметры Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Реквизиты печати реализации товаров и услуг были изменены. Закрыть без сохранения реквизитов?';
				|en = 'Print attributes of sales of goods and services were changed. Close the form without saving the attributes?'"),
			СписокКнопок);
