...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(Параметры.Договор) = Тип("СправочникСсылка.ДоговорыЛизинга") Тогда
		
		СвойстваСписка = ОбщегоНазначения.СтруктураСвойствДинамическогоСписка();
		
		СвойстваСписка.ТекстЗапроса = СтрЗаменить(СписокДокументов.ТекстЗапроса, "ВЫБРАТЬ NULL КАК Ссылка", ТекстЗапросаДокументыДоговоровЛизинга());
		СвойстваСписка.ОсновнаяТаблица = СписокДокументов.ОсновнаяТаблица;
		СвойстваСписка.ДинамическоеСчитываниеДанных = Истина;
	...
	//++ НЕ УТ
	ТипыДокументовПоДоговору.Добавить(Тип("ДокументСсылка.ПоступлениеПредметовЛизинга"));
	ТипыДокументовПоДоговору.Добавить(Тип("ДокументСсылка.ПриобретениеУслугПоЛизингу"));
	//-- НЕ УТ
...
//++ НЕ УТ
&НаСервере
Функция ТекстЗапросаДокументыДоговоровЛизинга()
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаявкаНаРасходованиеДенежныхСредств) Тогда
		...
		|				ГДЕ
		|					ДанныеТабличнойЧасти.Ссылка = ДанныеДокумента.Ссылка
		|					И ДанныеТабличнойЧасти.ДоговорЛизинга = &Договор)
		|";
	КонецЕсли;
