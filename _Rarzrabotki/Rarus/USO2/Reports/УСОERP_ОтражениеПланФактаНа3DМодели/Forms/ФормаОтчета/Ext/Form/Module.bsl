...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтобразитьСостояниеОтчета(НСтр("ru = 'Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';
									|en = 'The report is not generated. Click ""Generate"" to generate the report.'"));
	...
	КонецЕсли;
	НастройкиДанных = 1; //УСО
	
	ПолеВьюверИнициализировано = Истина;
...
Процедура ПриОткрытии(Отказ)
	...
	НастройкиДанныхПриИзменении(Неопределено); //УСО
КонецПроцедуры
...
#Иначе
	ПоказатьПредупреждение(, (НСтр("ru = 'Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.';
									|en = 'To edit the composition schema, run the application in the thick client mode.'")));
#КонецЕсли
КонецПроцедуры
...
&НаКлиенте
Процедура Сформировать(Отбор = Ложь)
	
	// {УСО}
	Отобразить3DМодель();
	// {/УСО}
	
	Если Не Отбор Тогда    //УСО
...
Процедура ПослеФормирования(Результат, ЗагрузитьРезультатФормированияОтчета) Экспорт 
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет не сформирован';
											|en = 'The report is not generated'"),, Заголовок);
	КонецЕсли;
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Если Результат.Статус <> "Выполняется" Тогда
		ЗагрузитьРезультатФормированияОтчета();
	Иначе	
		ОтобразитьСостояниеОтчета(НСтр("ru = 'Отчет формируется...';
										|en = 'Generating the report...'"), БиблиотекаКартинок.ДлительнаяОперация48);
	КонецЕсли;
	Возврат Результат;
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтобразитьСостояниеОтчета(НСтр("ru = 'Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';
									|en = 'Another report option is selected. Click ""Run report"" to receive the report. '"),
	...
	Если ПараметрыОбновления.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыОбновления.ВариантМодифицирован
			...
			ОтобразитьСостояниеОтчета(НСтр("ru = 'Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';
											|en = 'Settings were changed. Click ""Run report"" to get the report.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьКонструктив(ИдВыделенного, МассивКонструктивов)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УСОERP_Конструктив.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.УСОERP_Конструктив КАК УСОERP_Конструктив
		|ГДЕ
		|	УСОERP_Конструктив.Наименование = &Наименование
		|	И УСОERP_Конструктив.Ссылка В(&Ссылка)";
	
	Запрос.УстановитьПараметр("Наименование", ИдВыделенного);
	Запрос.УстановитьПараметр("Ссылка", МассивКонструктивов);
	...
	КонецЦикла;
	Возврат Справочники.УСОERP_Конструктив.ПустаяСсылка();
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьВсеКонструктивы(ОбъектСтроительства)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УСОERP_Конструктив.Ссылка КАК Ссылка,
		|	УСОERP_Конструктив.Наименование КАК Наименование
		|ИЗ
		|	Справочник.УСОERP_Конструктив КАК УСОERP_Конструктив
		|ГДЕ
		|	УСОERP_Конструктив.Владелец = &Владелец";
	...
	МассивОбъектСтроительства = Новый Массив;
	
	Пока Выборка.Следующий() Цикл
		МассивОбъектСтроительства.Добавить(Выборка.Наименование);
	КонецЦикла;
	
	Возврат МассивОбъектСтроительства;
	
КонецФункции

&НаКлиенте
Процедура НастройкиДанныхПриИзменении(Элемент)
	Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ВыполнениеРабот", НастройкиДанных = 1);
	Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ОбеспеченностьСтроительства", НастройкиДанных = 2);
	Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ПрямыеЗатраты", НастройкиДанных = 3);
	Элементы.НастройкаОтклонениеВДнях.Видимость = НастройкиДанных = 1;
	Элементы.НастройкаПерерасходИлиНЗП.Видимость = НастройкиДанных = 2;
	Элементы.НастройкаЭкономияИлиПерерасходПоПрямымЗатратам.Видимость = НастройкиДанных = 3;
КонецПроцедуры

&НаКлиенте
Процедура СформироватьНаименованияГраниц(Интервалы, ГраницаОт)
	
	КоличествоИнтервалов = Интервалы.Количество();
	Если Интервалы = НастройкаОтклонениеВДнях Тогда
		НаименованиеИнтервалаСвыше = НСтр("ru='Свыше %ГраницаОт%'");
	Иначе	
		НаименованиеИнтервалаСвыше = НСтр("ru='От %ГраницаОт% до 100'");
	КонецЕсли;
	
	НаименованиеИнтервалаОтДо  = НСтр("ru='От %ГраницаОт% до %ГраницаДо%'");
	ГраницаДо    = 100;
	
	Для ТекИндекс = 1 По КоличествоИнтервалов-1 Цикл
		ПредыдущийЭтап = Интервалы[ТекИндекс-1];
		ЭтотЭтап = Интервалы[ТекИндекс];
		Если ЭтотЭтап.ГраницаОт = 0 Тогда
			ЭтотЭтап.ГраницаОт = ПредыдущийЭтап.ГраницаОт + 1;
		КонецЕсли;
		Пока (ТекИндекс - 1 >= 0) И ЭтотЭтап.ГраницаОт < Интервалы[ТекИндекс-1].ГраницаОт Цикл
	...
	Если КоличествоИнтервалов = 1 Тогда
		...
		Интервалы[0].ГраницаОт = ГраницаОт;
		Интервалы[0].ГраницаДо = ГраницаДо;
		Интервалы[0].НаименованиеИнтервала = СтрЗаменить(НаименованиеИнтервалаСвыше, "%ГраницаОт%", Интервалы[0].ГраницаОт);
	...
	ИначеЕсли КоличествоИнтервалов > 1 Тогда
		...
		Для ТекИндекс = 1 По КоличествоИнтервалов-1 Цикл
			...
			Если ЭтотЭтап.ГраницаОт = ПредыдущийЭтап.ГраницаОт Тогда
				ЭтотЭтап.ГраницаОт = ПредыдущийЭтап.ГраницаОт + 1;
			КонецЕсли;
			
			Если ТекИндекс < КоличествоИнтервалов Тогда
				
				ПредыдущийЭтап.ГраницаДо = Интервалы[ТекИндекс].ГраницаОт-1;
				НаименованиеИнтервалаОтДоПредставление = СтрЗаменить(НаименованиеИнтервалаОтДо, "%ГраницаОт%", ПредыдущийЭтап.ГраницаОт);
				НаименованиеИнтервалаОтДоПредставление = СтрЗаменить(НаименованиеИнтервалаОтДоПредставление, "%ГраницаДо%", ПредыдущийЭтап.ГраницаДо);
				ПредыдущийЭтап.НаименованиеИнтервала = НаименованиеИнтервалаОтДоПредставление;
				
			КонецЕсли;
			
			Интервалы[КоличествоИнтервалов-1].ГраницаДо = ГраницаДо;
			Интервалы[КоличествоИнтервалов-1].НаименованиеИнтервала = СтрЗаменить(НаименованиеИнтервалаСвыше, "%ГраницаОт%", Интервалы[КоличествоИнтервалов-1].ГраницаОт);
...
&НаКлиенте
Процедура НастройкаОтклонениеВДняхПриИзменении(Элемент)
	
	СформироватьНаименованияГраниц(НастройкаОтклонениеВДнях, 0);
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкаОтклонениеВДняхПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
...
&НаКлиенте
Процедура НастройкаПерерасходИлиНЗППриИзменении(Элемент)
	
	СформироватьНаименованияГраниц(НастройкаПерерасходИлиНЗП, -100);
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкаПерерасходИлиНЗППриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
...
&НаКлиенте
Процедура НастройкаЭкономияИлиПерерасходПоПрямымЗатратамПриИзменении(Элемент)
	
	СформироватьНаименованияГраниц(НастройкаЭкономияИлиПерерасходПоПрямымЗатратам, -100);
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкаЭкономияИлиПерерасходПоПрямымЗатратамПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
...
&НаКлиенте
Процедура ПриИзмененииЦвета(ТекущаяСтрока)
	
	Зеленый = Новый Цвет(77, 222, 77);
	Желтый = Новый Цвет(255, 255, 0);
	Красный = Новый Цвет(255, 0, 0);

	Если ТекущаяСтрока.Цвет = Красный Тогда
		ТекущаяСтрока.ЦветСтрока = "Красный";
	ИначеЕсли ТекущаяСтрока.Цвет = Желтый Тогда
		ТекущаяСтрока.ЦветСтрока = "Жёлтый";
	ИначеЕсли ТекущаяСтрока.Цвет = Зеленый Тогда
		ТекущаяСтрока.ЦветСтрока = "Зелёный";
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура НастройкаОтклонениеВДняхЦветПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.НастройкаОтклонениеВДнях.ТекущиеДанные;	
	ПриИзмененииЦвета(ТекущаяСтрока);	
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкаПерерасходИлиНЗПЦветПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.НастройкаПерерасходИлиНЗП.ТекущиеДанные;	
	ПриИзмененииЦвета(ТекущаяСтрока);	
	
КонецПроцедуры

&НаКлиенте
Процедура НастройкаЭкономияИлиПерерасходПоПрямымЗатратамЦветПриИзменении(Элемент)

	ТекущаяСтрока = Элементы.НастройкаЭкономияИлиПерерасходПоПрямымЗатратам.ТекущиеДанные;	
	ПриИзмененииЦвета(ТекущаяСтрока);	
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьСОтоборомПоКонструктивуМодели(Команда)
	
	ИдВыделенного = СтрЗаменить(Элементы["ПолеВьювер"].Документ.defaultView.getSelectName(), "component_mesh_", "");
	Если Не ЗначениеЗаполнено(ИдВыделенного) Тогда
		Возврат;
	КонецЕсли;	
	МассивКонструктивов = Новый Массив;
	Для Каждого СтрокаКонструктив Из Отчет.ТаблицаКонструктивов Цикл
		МассивКонструктивов.Добавить(СтрокаКонструктив.Конструктив);
	КонецЦикла;	
	СсылкаНаКонструктив = ПолучитьКонструктив(ИдВыделенного, МассивКонструктивов);
	Если Не ЗначениеЗаполнено(СсылкаНаКонструктив) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не найдены данные по конструктиву " + ИдВыделенного);
		Возврат;
	КонецЕсли;
	
	Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Очистить();
	НовыйЭлементОтбора = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
    ПолеОтбора = Новый ПолеКомпоновкиДанных("Конструктив");
    НовыйЭлементОтбора.ЛевоеЗначение = ПолеОтбора;
    НовыйЭлементОтбора.Использование = Истина;
    НовыйЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
    НовыйЭлементОтбора.ПравоеЗначение = СсылкаНаКонструктив; 
...
Процедура ОтобразитьВьюер()
	...
	Попытка
		...
		//Конструктивы = ПолучитьВсеКонструктивы(Отчет.ОбъектСтроительства);
		
		МассивИспользуемыхКонструктивов = Новый Массив;
		Если НастройкиДанных = 1 Тогда
			Для Каждого СтрокаНастройка Из НастройкаОтклонениеВДнях Цикл
				Для Каждого Конструктив Из Отчет.ТаблицаКонструктивов Цикл
					Если Конструктив.ОтклонениеВДнях >= СтрокаНастройка.ГраницаОт 
						И (СтрокаНастройка.ГраницаДо = 0 Или Конструктив.ОтклонениеВДнях < СтрокаНастройка.ГраницаДо) Тогда
						Если СтрокаНастройка.Цвет = Зеленый Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#4DDE4D");
						ИначеЕсли СтрокаНастройка.Цвет = Красный Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#FF0000");
						ИначеЕсли СтрокаНастройка.Цвет = Желтый Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#FFFF00");
						КонецЕсли;
						МассивИспользуемыхКонструктивов.Добавить(Конструктив.ИдентификаторКонструктива);
					КонецЕсли;	 
				КонецЦикла;	 
			КонецЦикла;	
		ИначеЕсли НастройкиДанных = 2 Тогда
			Для Каждого СтрокаНастройка Из НастройкаПерерасходИлиНЗП Цикл
				Для Каждого Конструктив Из Отчет.ТаблицаКонструктивов Цикл
					Если Конструктив.ПерерасходИлиНЗП >= СтрокаНастройка.ГраницаОт 
						И (СтрокаНастройка.ГраницаДо = 0 Или Конструктив.ПерерасходИлиНЗП < СтрокаНастройка.ГраницаДо) Тогда
						Если СтрокаНастройка.Цвет = Зеленый Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#4DDE4D");
						ИначеЕсли СтрокаНастройка.Цвет = Красный Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#FF0000");
				...
				КонецЦикла;	 
			КонецЦикла;	
		ИначеЕсли НастройкиДанных = 3 Тогда
			Для Каждого СтрокаНастройка Из НастройкаЭкономияИлиПерерасходПоПрямымЗатратам Цикл
				Для Каждого Конструктив Из Отчет.ТаблицаКонструктивов Цикл
					Если Конструктив.ЭкономияИлиПерерасходПоПрямымЗатратам >= СтрокаНастройка.ГраницаОт 
						И (СтрокаНастройка.ГраницаДо = 0 Или Конструктив.ЭкономияИлиПерерасходПоПрямымЗатратам < СтрокаНастройка.ГраницаДо) Тогда
						Если СтрокаНастройка.Цвет = Зеленый Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#4DDE4D");
						ИначеЕсли СтрокаНастройка.Цвет = Красный Тогда
							Элементы["ПолеВьювер"].Документ.defaultView.changeObjectColorName("component_mesh_" + Конструктив.ИдентификаторКонструктива, "#FF0000");
...
&НаСервере
Процедура ПолучитьКонструктивыССервера()
	ОтчетОб = РеквизитФормыВЗначение("Отчет");
	Если ОтчетОб.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("ТаблицаКонструктивов") Тогда
		МассивТаблицы = ОтчетОб.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.ТаблицаКонструктивов;
		Для Каждого ЭлМассива Из МассивТаблицы Цикл
			СтрокаК = Отчет.ТаблицаКонструктивов.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаК, ЭлМассива);
		КонецЦикла;
	КонецЕсли;
	Если ОтчетОб.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("ОбъектСтроительства") Тогда
		Отчет.ОбъектСтроительства = ОтчетОб.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.ОбъектСтроительства;
	КонецЕсли;	
	//ЗначениеВРеквизитФормы(ОтчетОб, "Отчет");
КонецПроцедуры

&НаСервере
Процедура ИнициализироватьПолеВьювер()
	
	РеквизитПоля = Новый РеквизитФормы("ПолеВьювер", Новый ОписаниеТипов("Строка"));
	
	НеобходимаОчистка = Ложь;
	
	Для Каждого ТекРеквизит Из ПолучитьРеквизиты() Цикл 
		Если ТекРеквизит.Имя = "ПолеВьювер" Тогда 
			НеобходимаОчистка = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если НеобходимаОчистка Тогда 
		Элементы.Удалить(Элементы.ГруппаПолеВьювер.ПодчиненныеЭлементы["ПолеВьювер"]);
	КонецЕсли;
	
	ДобавляемыеРеквизиты = Новый Массив();
	ДобавляемыеРеквизиты.Добавить(РеквизитПоля);
	
	УдаляемыеРеквизиты = Новый Массив();
	Если НеобходимаОчистка Тогда 
		УдаляемыеРеквизиты.Добавить("ПолеВьювер");
	КонецЕсли;
	
	ИзменитьРеквизиты(ДобавляемыеРеквизиты, УдаляемыеРеквизиты);
	
	ЭлементПоля = Элементы.Добавить("ПолеВьювер", Тип("ПолеФормы"), Элементы.ГруппаПолеВьювер);
	ЭлементПоля.Вид                = ВидПоляФормы.ПолеHTMLДокумента;
	ЭлементПоля.ПутьКДанным        = "ПолеВьювер";
	ЭлементПоля.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	
	КомандаСформироватьПоКонструктиву = Элементы.Добавить("СформироватьСОтоборомПоКонструктивуМодели", Тип("КнопкаФормы"), ЭлементПоля.КонтекстноеМеню);
	КомандаСформироватьПоКонструктиву.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели;
	КомандаСформироватьПоКонструктиву.ИмяКоманды = "СформироватьСОтоборомПоКонструктивуМодели";
	
КонецПроцедуры

&НаКлиенте
Процедура Отобразить3DМодель()
	
	НастройкаОбъектСтроительства = Неопределено;
	
	Для Каждого ТекущаяНастройка Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл 
		Если ТипЗнч(ТекущаяНастройка) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных")
				И ТекущаяНастройка.Параметр = Новый ПараметрКомпоновкиДанных("ОбъектСтроительства") Тогда 
			НастройкаОбъектСтроительства = ТекущаяНастройка;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если НастройкаОбъектСтроительства = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	ОбъектСтроительства = НастройкаОбъектСтроительства.Значение;
	
	Если Отчет.ОбъектСтроительства = ОбъектСтроительства Тогда 
		Возврат;
	КонецЕсли;
	
	Отчет.ОбъектСтроительства = ОбъектСтроительства;
	
	Если ПолеВьюверИнициализировано Тогда 
		ИнициализироватьПолеВьювер();
		ПолеВьюверИнициализировано = Ложь;
	КонецЕсли;
	
	ЭтотОбъект["ПолеВьювер"] = ПолучитьПутьКИндекснойСтраницеWebKit(ОбъектСтроительства);
...
&НаСервереБезКонтекста
Функция ПолучитьПутьКИндекснойСтраницеWebKit(ОбъектСтроительства)
	
	ПутьКИндекснойСтранице = "";
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Ссылка", ОбъектСтроительства);
	
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СМ_ОбъектыСтроительства.УСОERP_ИндекснаяСтраницаWebKit КАК УСОERP_ИндекснаяСтраницаWebKit
	|ИЗ
	...
	Если Выборка.Следующий() Тогда 
		ПутьКИндекснойСтранице = Выборка.УСОERP_ИндекснаяСтраницаWebKit;
	КонецЕсли;
	
	Если ПустаяСтрока(ПутьКИндекснойСтранице) Тогда 
		ПутьКИндекснойСтранице = Константы.УСОERP_ИндекснаяСтраницаWebKit.Получить();
	КонецЕсли;
	
	Возврат ПутьКИндекснойСтранице;
	
КонецФункции // ПолучитьПутьКИндекснойСтраницеWebKit()
