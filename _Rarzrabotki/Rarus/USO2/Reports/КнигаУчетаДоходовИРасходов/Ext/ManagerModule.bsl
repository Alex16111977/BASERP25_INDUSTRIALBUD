...
Процедура ПечатьТитульногоЛиста(ПараметрыОтчета)
	...
	Секция.Параметры.НаПериод = СтрШаблон(НСтр("ru = 'на %1 год';
												|en = 'for %1'"), Формат(Год(ПараметрыОтчета.Период), "ЧГ="));
...
Процедура ПечатьРаздела2(ПараметрыОтчета)
	...
	Секция.Параметры.ЗаПериод = НСтр("ru = 'за';
									|en = 'for'") + " " + ПолучитьПредставлениеПериода(ПараметрыОтчета.Период);
	ТабличныйДокумент.Вывести(Секция);
...
Процедура ВывестиИтогиЗаКвартал(ПараметрыОтчета, ТекКвартал, ПоследнийКварталПериода, Макет, ТабличныйДокумент)
	...
	Секция.Параметры.ПечТекстИтогов = СтрШаблон(НСтр("ru = 'Итого за %1 квартал';
													|en = 'Total for %1 quarter'"), СтроковыеФункцииКлиентСервер.ПреобразоватьЧислоВРимскуюНотацию(РимскоеЧисло, Ложь));
	ТабличныйДокумент.Вывести(Секция);
	...
	Если ТекКвартал <> 1 Тогда
		Секция = Макет.ПолучитьОбласть("Итого|Основная");
		Секция.Параметры.ПечТекстИтогов = НСтр("ru = 'Итого за';
												|en = 'Total for'") + " " + НазваниеПериодаНарастающимИтогом(ТекКвартал);
		ТабличныйДокумент.Вывести(Секция);
...
Процедура ВывестиУменьшенияНалогаЗаКвартал(ПараметрыОтчета, ТекКвартал, ПоследнийКварталПериода, Макет, ТабличныйДокумент)
	...
	Секция.Параметры.ПредставлениеПериодаИтогов =
		СтрШаблон(НСтр("ru = 'Итого за %1 квартал';
						|en = 'Total for %1 quarter'"), СтроковыеФункцииКлиентСервер.ПреобразоватьЧислоВРимскуюНотацию(РимскоеЧисло, Ложь));
	...
	Если ТекКвартал <> 1 Тогда
		Секция = Макет.ПолучитьОбласть("Итог");
		Секция.Параметры.ПредставлениеПериодаИтогов = НСтр("ru = 'Итого за';
															|en = 'Total for'") + " " + НазваниеПериодаНарастающимИтогом(ТекКвартал);
...
Процедура ВывестиУплаченныйТорговыйСборЗаКвартал(ПараметрыОтчета, ТекущийКвартал, Макет, ТабличныйДокумент)
	...
	Если ТекущийКвартал <> 1 Тогда
		Секция = Макет.ПолучитьОбласть("Итог");
		Секция.Параметры.ПредставлениеПериодаИтогов = НСтр("ru = 'Итого за';
															|en = 'Total for'") + " " + НазваниеПериодаНарастающимИтогом(ТекущийКвартал);
