...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ТекстСообщения = НСтр("ru = 'Отчет не поддерживает получение данных до даты начала учета внеоборотных активов версии 2.4 - %1.
                           ...
                           | You need to use ""Regulated Accounting"" report - ""Detailed calculations"" - ""Detailed calculation of depreciation"".'");
...
Функция ПолучитьЗначенияОтбораДанных(НастройкиОсновнойСхемы)
	...
	ЗначенияОтбораДанных.Вставить("СписокНМА", ВнеоборотныеАктивыСлужебный.ЗначениеОтбораОтчета("НематериальныйАктив", ЭлементыОтбора));
...
Процедура УстановитьТекстЗапроса(СхемаКомпоновкиДанных)
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса, 
		"&НормаАмортизации", 
		ВнеоборотныеАктивыЛокализация.ТекстПоляНормаАмортизации("НематериальныеАктивы.АмортизационнаяГруппа"));
