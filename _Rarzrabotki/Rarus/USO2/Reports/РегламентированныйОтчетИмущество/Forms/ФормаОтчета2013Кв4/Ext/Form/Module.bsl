	...
	Если ИмяПоказателя = "П000030009003_Числ" Тогда
		...
		Если ЗначениеЗаполнено(Область.Значение) Тогда
			Если Число(Область.Значение) > 12 Тогда
				Область.Значение = "";
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru = 'Значение в числителе коэффициента К не должно превышать 12. Значение показателя очищено.';
...
Процедура ПроверитьЗаполнение_Раздел2(СочетаниеРазделов, СтатусПроверки)
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодВидаИмущества) ИЛИ СтрНайти(ПодСтрПоискаКодовВидаИмущества, КодВидаИмущества) = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 код вида имущества должен принимать одно из значений: ""1"", ""2"", ""3"", ""4"", ""5"", ""6"", ""7""" + ДопКодВидаИмуществаВСообщении + ".", "Раздел2", "П000020000101", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
