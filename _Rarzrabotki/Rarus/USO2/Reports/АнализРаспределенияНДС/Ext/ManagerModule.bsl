...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	ОбластьЗаголовок.Параметры.Период = СтрЗаменить(НСтр("ru = 'Период: &1';
														|en = 'Period: &1'"), "&1", Период);
...
Процедура ВывестиВОтчетДанныеПоРаспределениюКосвенныхРасходов(ПериодБазы, ПараметрыОтчета, ДокументРезультат, Макет)
	...
	|		ПО 
	|			ПартииПрочихРасходов.ДокументПоступленияРасходов = ИнойДокументПодтвержденияНДС.Ссылка
	...
	|	ЕСТЬNULL(СчетФактураПолученный.Ссылка,ЕСТЬNULL(ИнойДокументПодтвержденияНДС.Ссылка,СчетФактураПолученныйНА.Ссылка)),
