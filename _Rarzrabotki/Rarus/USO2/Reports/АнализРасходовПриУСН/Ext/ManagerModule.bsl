...
Функция ПолучитьНастройкиОтчетаПоОтбору(Параметры, ИмяВариантаНастроек = Неопределено) Экспорт
	...
	Для Каждого Элемент Из ПользовательскиеНастройки.Элементы Цикл
		...
		ИначеЕсли ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			Для каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
				Если ЭлементОтбора.ИдентификаторПользовательскойНастройки = Элемент.ИдентификаторПользовательскойНастройки
						И Параметры.Отбор.Свойство(Строка(ЭлементОтбора.ЛевоеЗначение), Элемент.ПравоеЗначение) Тогда
					Элемент.ЛевоеЗначение = ЭлементОтбора.ЛевоеЗначение;
	...
	Если Параметры.Свойство("ПериодОтчета") Тогда
		КомпоновкаДанныхКлиентСервер.УстановитьПараметр(ПользовательскиеНастройки, "Период", Параметры.ПериодОтчета);
	КонецЕсли;
...
// Функция на основании вида расходов возвращает соответсвующее имя варианта настроек для формирования отчета по данному
