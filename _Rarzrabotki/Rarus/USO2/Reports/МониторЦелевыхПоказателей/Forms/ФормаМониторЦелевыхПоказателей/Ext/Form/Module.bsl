...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Параметры.ИдентификаторВариантаАнализа) Тогда
		...
		Заголовок = НСтр("ru = 'Анализируемый показатель';
						|en = 'Analyzed indicator'") + " """ + НаименованиеВариантаАнализа + """";
...
Процедура Печать(Команда)
	Если СоставМонитораНеопределен Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Сформировать сводный отчет невозможно, т.к. монитор показателей не настроен.';
													|en = 'Cannot generate a summary report as the indicator monitor is not set up.'"),, НСтр("ru = 'Ошибка формирования сводного отчета';
																																				|en = 'An error occurred while generating the summary report'"));
	ИначеЕсли СоставМонитораПустойПоОтбору Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Сформировать сводный отчет невозможно, т.к. в текущий отбор не входит ни один показатель.';
													|en = 'Cannot generate a summary report as the current filter does not include any indicator.'"),, НСтр("ru = 'Ошибка формирования сводного отчета';
																																							|en = 'An error occurred while generating the summary report'"));
	Иначе 
...
Процедура ДобавитьГруппыКолонкиЭлементов(РодительскаяГруппа, КлючУникальностиГруппы, ВариантОтображенияПоказателейСтроки = Неопределено)
	...
	Если ВариантОтображенияПоказателейСтроки = ВариантыОтображенияВариантовАнализа.Кратко Тогда
		Для Сч = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл 
			ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементовКратко" + КлючУникальностиГруппы + "__" + Сч, НСтр("ru = 'Колонка №';
																																	|en = 'Column No.'") + Сч, РодительскаяГруппа);
	...
	Иначе
		Если Элементы.Найти("ГруппаКолонкаЭлементовПодробно" + КлючУникальностиГруппы) = Неопределено Тогда
			Для Сч = 1 По СтруктураНастроек.КоличествоКолонокМонитораПоказателей Цикл 
				ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементовПодробно" + КлючУникальностиГруппы + "__" + Сч, НСтр("ru = 'Колонка №';
																																		|en = 'Column No.'") + Сч, РодительскаяГруппа);
				ЗаполнитьЗначенияСвойств(ГруппаКолонкаЭлементов, СтильГруппыКолонкиПоказателей());
			КонецЦикла;
		КонецЕсли;
...
Процедура НарисоватьДиаграмму(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	МассивРеквизитов.Добавить(
		Новый РеквизитФормы("Диаграмма__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru = 'Диаграмма для';
																								|en = 'Chart for'") + " " + ВариантАнализа));
	...
	// Добавим поля формы
	ГруппаЗаголовокПоказателя = ГоризонтальнаяГруппа("Заголовок__" + ИдентификаторЭлементаФормы, НСтр("ru = 'Статусы';
																										|en = 'Statuses'"), ГруппаЭлементов);
...
Процедура НарисоватьТаблицу(ВариантАнализа, ИсточникДанных, ГруппаЭлементов, ИдентификаторЭлементаФормы)
	...
	Если Не ПустойИсточникДанных И Не РасчетыНеактуальны Тогда
		...
		МассивРеквизитов.Добавить(
			Новый РеквизитФормы("ТаблицаЗначений__" + ИдентификаторЭлементаФормы, ОписаниеТипов, "", НСтр("ru = 'Таблица для';
																											|en = 'Table for'") + " " + ВариантАнализа));
	Иначе
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Тип("Диаграмма"));
		ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
	...
	// Добавим поля формы
	ГруппаЗаголовокПоказателя = ГоризонтальнаяГруппа("Заголовок__" + ИдентификаторЭлементаФормы, НСтр("ru = 'Заголовок показателя';
																										|en = 'Indicator title'"), ГруппаЭлементов);
	ЗаполнитьЗначенияСвойств(ГруппаЗаголовокПоказателя, СтильГруппыЭлементовЗаголовка());
...
Процедура СоздатьБазовуюСтруктуруЭлементовФормы(СоставМонитора, СтруктураВывода)
	...
	ГруппаПоказатели = ВертикальнаяГруппа("ГруппаПоказатели", НСтр("ru = 'Показатели';
																	|en = 'Indicators'"), Элементы.ОсновнаяСтраницаМонитора);
	
	Если РежимАнализаВзаимосвязанных Тогда
		ВариантАнализа = МониторингЦелевыхПоказателейВызовСервера.ПолучитьСвойстваВариантаАнализаПоИдентификатору(Параметры.ИдентификаторВариантаАнализа);
		НаименованиеВариантаАнализа = ВариантАнализа.Ссылка.Наименование;
		
		ГруппаАнализируемыйПоказатель = ГоризонтальнаяГруппа("ГруппаАнализируемыйПоказатель", НСтр("ru = 'Анализируемый показатель';
																									|en = 'Analyzed indicator'") + " """ + НаименованиеВариантаАнализа + """", ГруппаПоказатели);
		...
		Для Сч = 1 По 2 Цикл 
			ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементовАнализируемыйПоказатель" + "__" + Сч, НСтр("ru = 'Колонка №';
																															|en = 'Column No.'") + Сч, ГруппаАнализируемыйПоказатель);
		...
		Если Зависимые.Количество() > 0 Тогда
			...
			ГруппаЗависимые = ГоризонтальнаяГруппа("Группа" + ИмяВидаПоиска, НСтр("ru = 'Зависимые показатели';
																					|en = 'Dependent indicators'"), ГруппаПоказатели);
			ЗаполнитьЗначенияСвойств(ГруппаЗависимые, СтильГруппыПоказателей());
			
			// Колонок в режиме анализа взаимосвязей всегда 2 - диаграмма, справка о показателей.
			Для Сч = 1 По 2 Цикл 
				ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементов" + ИмяВидаПоиска + "__" + Сч, НСтр("ru = 'Колонка №';
																														|en = 'Column No.'") + Сч, ГруппаЗависимые);
			КонецЦикла;
		КонецЕсли;
		...
		Если Влияющие.Количество() > 0 Тогда
			...
			ГруппаВлияющие = ГоризонтальнаяГруппа("Группа" + ИмяВидаПоиска, НСтр("ru = 'Влияющие показатели';
																				|en = 'Influencing indicators'"), ГруппаПоказатели);
			ЗаполнитьЗначенияСвойств(ГруппаВлияющие, СтильГруппыПоказателей());
			
			// Колонок в режиме анализа взаимосвязей всегда 2 - диаграмма, справка о показателей.
			Для Сч = 1 По 2 Цикл 
				ГруппаКолонкаЭлементов = ВертикальнаяГруппа("ГруппаКолонкаЭлементов" + ИмяВидаПоиска + "__" + Сч, НСтр("ru = 'Колонка №';
																														|en = 'Column No.'") + Сч, ГруппаВлияющие);
			КонецЦикла;
		КонецЕсли;
	Иначе
		ГруппаПоказателиКратко = ВертикальнаяГруппа("ГруппаПоказателиКратко", НСтр("ru = 'Показатели (кратко)';
																					|en = 'Indicators (briefly)'"), ГруппаПоказатели);
		ГруппаПоказателиПодробно = ВертикальнаяГруппа("ГруппаПоказателиПодробно", НСтр("ru = 'Показатели (подробно)';
																						|en = 'Indicators (detailed)'"), ГруппаПоказатели);
	...
	Если ПравоДоступа("Изменение", Метаданные.Справочники.ВариантыАнализаЦелевыхПоказателей) Тогда
		Элементы.ПояснениеЕслиМониторНеНастроен.Заголовок = ПостояннаяЧасть
			+ Символы.ПС + НСтр("ru = 'Для настройки используйте рабочее место ""Финансовый результат и контроллинг"" - ""Настройка доступности вариантов анализа"".';
								|en = 'To configure, use the workplace ""Financial result and controlling"" - ""Set up availability of analysis options"".'");
	Иначе
		Элементы.ПояснениеЕслиМониторНеНастроен.Заголовок = ПостояннаяЧасть
...
Функция ЗаполнитьОтчетНаСервере(ПараметрыСоставаМонитора)
	...
	НаименованиеЗадания = НСтр("ru = 'Заполнение отчета ""Монитор целевых показателей""';
								|en = 'Fill in the ""Target indicator dashboard"" report'");
	...
	Исключение  
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение отчета ""Монитор целевых показателей""';
										|en = 'Fill in the ""Target indicator dashboard"" report'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
