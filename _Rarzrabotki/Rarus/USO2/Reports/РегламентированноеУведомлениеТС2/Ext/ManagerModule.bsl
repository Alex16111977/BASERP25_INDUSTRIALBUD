...
Процедура ЗаполнитьДаннымиТС2(ДокОбъект, Выборка)
	...
	Исключение
		...
		СтрОш = НСтр("ru = 'Не удалось получить ФИО подписанта';
					|en = 'Не удалось получить ФИО подписанта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура СоздатьСконвертированныйТС2(Выборка) Экспорт 
	...
	Исключение
		...
		КонецЕсли;
		Ошибка = ИнформацияОбОшибке();
		СтрОш = НСтр("ru = 'Создание уведомления ТС-2';
					|en = 'Создание уведомления ТС-2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
