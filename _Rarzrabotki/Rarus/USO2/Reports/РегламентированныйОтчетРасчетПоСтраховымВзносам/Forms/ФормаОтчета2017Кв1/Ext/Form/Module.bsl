...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П00018М103001") Тогда
		...
		Иначе
			...
			|en = ' Код вида предпринимательской деятельности, установленный законодательством субъекта Российской Федерации, указанный в заявлении на получение патента должен состоять из 6 цифр. Значение очищено.'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
		...
		Если ОблПарногоПоказателя1 <> Неопределено
			...
			Если ЗначениеЗаполнено(ЗначПоказателя)
				...
				Если ЗначениеЗаполнено(ЗначПарногоПоказателя1)
					...
					НСтр("ru = 'Значения в строках %1 и %2 очищены';
						|en = 'Значения в строках %1 и %2 очищены'"), Стр1, Стр2);
				Иначе
					ДополнительныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Значение в строке %1 очищено';
						|en = 'Значение в строке %1 очищено'"),
				...
				|en = 'Не допускается одновременное указание сведений, подтверждающих членство в студенческом отряде, и сведений, подтверждающих очную форму обучения. %1.'"),
				ДополнительныйТекст);
				Сообщение.Сообщить();
				Мофицированность = Истина;
			КонецЕсли;
	...
	Если ИмяПоказателя = "П000030004001" Тогда
		...
		Если ЗначениеЗаполнено(ПредставлениеСтраницыРаздела3НайденнойПоЗначениюВСтроке040) Тогда
			...
			|en = 'Номер %1 уже используется на странице ""%2"". Значение в строке 040 очищено.'"),
...
Процедура ДобавитьСтроку(Команда)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		...
		Если СтрНачинаетсяС(ИмяОбласти, СтруктураРеквизитовФормы.мИдГруппы1М1) Тогда
			...
			Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_4") <> Неопределено Тогда
				...
				|en = 'Нельзя добавлять более четырех групп строк 060-073 в разделе 1.'");
			КонецЕсли;
		...
		ИначеЕсли СтрНачинаетсяС(ИмяОбласти, СтруктураРеквизитовФормы.мИдГруппы1М2) Тогда
			...
			Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_2") <> Неопределено Тогда
				...
				|en = 'Нельзя добавлять более двух групп строк 080-093 в разделе 1.'");
			КонецЕсли;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил1Подр1_3_1" Тогда
		...
		Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_2") <> Неопределено Тогда
			...
			|en = 'Нельзя добавлять более двух групп строк 001-050 в подразделе 1.3.1 приложения 1 к разделу 1.'");
		КонецЕсли;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил1Подр1_3_2" Тогда
		...
		Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_30") <> Неопределено Тогда
			...
			|en = 'Нельзя добавлять более тридцати групп строк 001-050 в подразделе 1.3.2 приложения 1 к разделу 1.'");
		КонецЕсли;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил1Подр1_4" Тогда
		...
		Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_2") <> Неопределено Тогда
			...
			|en = 'Нельзя добавлять более двух групп строк 001-050 в подразделе 1.4 приложения 1 к разделу 1.'");
		КонецЕсли;
...
Процедура ВыгрузитьСведенияРаботниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ПроверитьВозможностьВыгрузкиСведенийРаботников() Тогда
		...
		НСтр("ru = 'Не удалось выгрузить сведения работающих лиц из Раздела 3 (%1).';
...
Процедура ПомещениеФайлаСДаннымиДляРаздела3(Результат, АдресФайла, ВыбИмяФайла, ДопПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Если Прав(ВыбИмяФайла, 5) <> ".pncb" Тогда
			...
			|en = 'Необходимо выбрать файл, содержащий персонифицированные сведения о застрахованных лицах, с расширением ""pncb""!'"));
		...
		Если ЗаполнениеРаздела3ВыполненоУспешно Тогда
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Данные файла %1, содержащего персонифицированные сведения о застрахованных лицах, успешно импортированы.';
				|en = 'Данные файла %1, содержащего персонифицированные сведения о застрахованных лицах, успешно импортированы.'"), ВыбИмяФайла);
			Сообщение.Сообщить();
...
Процедура ЗаполнитьРаздел3ДаннымиИзФайла(АдресФайла, ПолноеИмяФайла, Каталог, ЗаполнениеРаздела3ВыполненоУспешно)
	...
	Если НЕ (Файл_Версия = Отчет_Версия
		...
		|en = 'Данные файла, содержащего персонифицированные сведения о застрахованных лицах, не соответствует экземпляру отчета!'");
		Сообщение.Сообщить();
		УдалитьВременныйФайл(ФайлЗагрузки);
		Возврат;
	КонецЕсли;
...
Процедура СообщитьОбОшибкеФорматаИмпорта()
	...
	Сообщение.Текст = НСтр("ru = 'Нарушен формат файла, содержащего персонифицированные сведения о застрахованных лицах!';
							|en = 'Нарушен формат файла, содержащего персонифицированные сведения о застрахованных лицах!'");
	Сообщение.Сообщить();
	...
	Если Область.Имя = "ПоМесту" Тогда
		...
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 12 Тогда
			...
			Если НовыйКодПоМесту = "124" И ПрежнийКодПоМесту <> "124" Тогда
				...
				|en = 'Внимание! Код ""124"" места, по которому представляется расчет, указывается за 1 квартал, полугодие, 9 месяцев,
				|в случае снятия с учета, в качестве индивидуального предпринимателя (главы крестьянского (фермерского) хозяйства).'");
				ПоказатьПредупреждение(, ТекстПредупреждения);
			...
			ИначеЕсли НовыйКодПоМесту = "240" И ПрежнийКодПоМесту <> "240"
				...
				|en = 'Внимание! Код ""240"" места, по которому представляется расчет, указывается за 1 квартал, полугодие, 9 месяцев,
				|в случае реорганизации (ликвидации) юридического лица - (главы) крестьянского (фермерского) хозяйства.'");
				ПоказатьПредупреждение(, ТекстПредупреждения);
