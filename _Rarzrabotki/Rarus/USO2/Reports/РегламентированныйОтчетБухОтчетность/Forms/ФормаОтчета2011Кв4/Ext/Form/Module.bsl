...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.КоличествоСтрокИнформационногоМакета = СтрЧислоСтрок(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2011Кв4_Изменения").ПолучитьТекст());
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ИмяРаздела = "ДополнительныеФайлы" Тогда
		Если ТабличныйДокумент.Области.Количество() = 0 Тогда
			ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ?(ИмяРаздела = "ФормаОтчета", СтруктураОтчета.ОПУ.ИмяМакета, СтруктураОтчета[ИмяРаздела].ИмяМакета)));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьДеревоНастроекСоставаДопСтрок(ИмяРаздела)
	...
	ОбъектОтчета.ПолучитьДеревоНастроекСоставаДопСтрок(СтруктураРеквизитовФормы.мВыбраннаяФорма,
									   			  	   ИмяРаздела,
		...
		Если СохраненныеДанные.Свойство("СтруктураОтчета", СохраненнаяСтруктураОтчета) Тогда
			...
			Для Каждого Элемент Из СтруктураОтчета Цикл
				...
				Если СтрЧислоВхождений(Элемент.Значение.ИмяМакета, Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета"))) > 0 Тогда
					...
					Элемент.Значение.ИмяМакета = Сред(Элемент.Значение.ИмяМакета, СтрДлина(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_") + 1);
...
Функция ПолучитьСтруктуруОтчета2011Кв4()
	...
	ОбъектОтчета = ОбъектОтчета(ЭтаФорма.ИмяФормы);
	Возврат ОбъектОтчета.ПолучитьСтруктуруОтчета2011Кв4(СтруктураРеквизитовФормы.мВыбраннаяФорма,
									   					СтруктураРеквизитовФормы);
...
Процедура ДополнитьСтруктуруОтчетаОбщимиОтчетами(СоставНастройкаОтчета)
	...
	ОбъектОтчета = ОбъектОтчета(ЭтаФорма.ИмяФормы);
	ОбъектОтчета.ДополнитьСтруктуруОтчетаОбщимиОтчетами(СтруктураРеквизитовФормы.мВыбраннаяФорма,
	...
	Если ЛОЖЬ И ОписаниеМногострочнойЧасти.Масштаб.МаксимальноеКоличествоСтрок <> 0	
	   И КоличествоСтрокВМногострочнойЧасти >= ОписаниеМногострочнойЧасти.Масштаб.МаксимальноеКоличествоСтрок Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Невозможно добавить строку многострочного раздела: достигнуто максимальное количество.';
								|en = 'Невозможно добавить строку многострочного раздела: достигнуто максимальное количество.'");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", Знач ОбластьИмя = "")
	...
	// ПОЯСНЕНИЯ 3
	//
	ИначеЕсли ИмяСтраницыПанели = "Пояснения3" Тогда
	...
	// ПОЯСНЕНИЯ 5
	//
	ИначеЕсли ИмяСтраницыПанели = "Пояснения5" Тогда
	...
	// ПОЯСНЕНИЯ 6
	//
	ИначеЕсли ИмяСтраницыПанели = "Пояснения6" Тогда	
	...
	// ПОЯСНЕНИЯ 7
	//
	ИначеЕсли ИмяСтраницыПанели = "Пояснения7" Тогда
	...
	// ПОЯСНЕНИЯ 9
	//
	ИначеЕсли ИмяСтраницыПанели = "Пояснения9" Тогда
	...
	Иначе
		...
		Если НЕ (ВРег(Прав(ВыбИмяФайла, 4)) = ".TIF"
			...
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = '%1 должен иметь одно из допустимых расширений: TIF, PDF, JPG!';
				|en = '%1 должен иметь одно из допустимых расширений: TIF, PDF, JPG!'"), ВидДопФайла);
...
Процедура ЗаполнитьПустыеСтруктурыДанных(ЗаполнятьСведенияОбОрганизации = Истина)
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если НЕ Раздел.МногостраничностьВРазделе
			...
			ТабличныйДокумент.Очистить();
			ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ?(Раздел.ИмяСтраницы = "ФормаОтчета", СтруктураОтчета.ОПУ.ИмяМакета, СтруктураОтчета[Раздел.ИмяСтраницы].ИмяМакета)));
...
Процедура ОчиститьТекущуюСтраницу(Команда)
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ДополнительныеФайлы" Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Внимание! Будут очищены сведения о дополнительных файлах.%1Продолжить операцию?';
																					|en = 'Внимание! Будут очищены сведения о дополнительных файлах.%1Продолжить операцию?'"), Символы.ПС);
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Внимание! Будут очищены все показатели текущего отчета.%1Продолжить операцию?';
																					|en = 'Внимание! Будут очищены все показатели текущего отчета.%1Продолжить операцию?'"), Символы.ПС);
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	ФормаНастройкиОтчета = ПолучитьФорму("Отчет.РегламентированныйОтчетБухОтчетность.Форма.НастройкиОтчета", ПараметрыФормы, ЭтаФорма);
...
Процедура ВывестиНаПечатьКНД0710099(ПараметрыПечати)
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2015-01-01' Тогда
		...
		Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2016-01-01' Тогда
			ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2016Кв1_КНД0710099_1");
		Иначе
			ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2015Кв1_КНД0710099_1");
		КонецЕсли;
	...
	ИначеЕсли СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		...
		ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2013Кв1_КНД0710099_1");
	...
	Иначе
		...
		ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2011Кв4_КНД0710099_1");
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2015-01-01' Тогда
		Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2016-01-01' Тогда
			ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2016Кв1_КНД0710099_2");
		Иначе
			ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2015Кв1_КНД0710099_2");
		КонецЕсли;
		ПечатнаяФорма.ПолеСверху = 5;
		ПечатнаяФорма.ПолеСправа = 5;
		ПечатнаяФорма.ПолеСнизу  = 5;
		ПечатнаяФорма.ПолеСлева  = 5;
	ИначеЕсли СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2013Кв1_КНД0710099_2");
	Иначе
		ПечатнаяФорма = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаПечати2011Кв4_КНД0710099_2");
	КонецЕсли;
...
Процедура Печатать(Команда)
	...
	ФормаВыбора = ПолучитьФорму("Отчет.РегламентированныйОтчетБухОтчетность.Форма.ВыборПечатнойФормы", ПараметрыФормы);
