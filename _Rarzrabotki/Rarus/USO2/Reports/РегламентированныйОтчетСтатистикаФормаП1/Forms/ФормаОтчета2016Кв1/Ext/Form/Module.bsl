...
Процедура ПроверкаКСШаблоном(Команда)
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Проверка контрольных соотношений в неактуальной версии формы не поддерживается';
...
Процедура ПроверкаКСШаблономПослеСохраненияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ФормаОтчетаП1Пр3) = Тип("УправляемаяФорма") Тогда
