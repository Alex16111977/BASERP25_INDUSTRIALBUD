...
Функция ПолучитьПредставлениеОпросаДляЗаголовка(РеквизитыОпрос)
	...
	Если РеквизитыОпрос.ДатаНачала <> Дата(1,1,1) ИЛИ РеквизитыОпрос.ДатаОкончания <> Дата(1,1,1) Тогда
		ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru = 'проводился в период';
																				|en = 'it was held in the period'") + " ";
	КонецЕсли;
	
	Если РеквизитыОпрос.ДатаНачала <> Дата(1,1,1) Тогда
		ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru = 'с';
																				|en = 'from'") + " " + Формат(РеквизитыОпрос.ДатаНачала,"ДФ=dd.MM.yy") + " ";
	КонецЕсли;
	
	Если РеквизитыОпрос.ДатаОкончания <> Дата(1,1,1) Тогда
		ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru = 'по';
																				|en = 'to'") + " " + Формат(РеквизитыОпрос.ДатаОкончания,"ДФ=dd.MM.yy")+ " ";
	КонецЕсли; 
	
	ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru = 'на основании документа';
																			|en = 'on the document basis'") + " " + РеквизитыОпрос.Представление + ".";
