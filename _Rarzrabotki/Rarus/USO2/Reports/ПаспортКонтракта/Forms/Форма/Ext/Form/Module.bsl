...
Процедура ОбновитьНаСервере()
	...
	#Область ГрафикПлатежей
		...
		Для Каждого СтрокаГрафика Из ГрафикПлатежей Цикл 
			...
			// Уточнение или форматирование информации
			ОбластьГрафикПлатежейСтрока.Параметры["ГрафикНазначениеПлатежа"] = НазначениеПлатежа(ОсновнаяИнформация, СтрокаГрафика, ЭтоКонтрактСЗаказчиком);
...
Функция ИННКПП(ОсновнаяИнформация, ЗаказчикИсполнитель)
	...
	Если ЗначениеЗаполнено(ОсновнаяИнформация[ЗаказчикИсполнитель + "КПП"]) Тогда
		МассивСтрок.Добавить(НСтр("ru = 'КПП';
									|en = 'CRTR'") + ": " + ОсновнаяИнформация[ЗаказчикИсполнитель + "КПП"]);
	КонецЕсли;
...
Функция БанковскиеСчета(ОсновнаяИнформация, БанковскиеСчета, ЗаказчикИсполнитель)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
		...
		РеквизитыСчета.Добавить(НСтр("ru = 'БИК';
									|en = 'Branch ID'")+ ": " + Строка(НайденнаяСтрока[Префикс + "БИКБанка"]));
...
&НаСервереБезКонтекста 
Функция НазначениеПлатежа(ОсновнаяИнформация, СтрокаГрафика, ЭтоКонтрактСЗаказчиком)
	НазначениеПлатежа = "";
	...
	Иначе
		...
		ПараметрыНазначения.Вставить("ПлатежиПо275ФЗ", ОсновнаяИнформация["КонтрактПлатежиПо275ФЗ"]);
		...
		ПараметрыНазначения.Вставить("ПредметОплаты", СтрокаГрафика.ГрафикКомментарий);
		ПараметрыНазначения.Вставить("ВариантОплаты", 
			?(СтрНайти(СтрокаГрафика.ГрафикКомментарий, "Аванс") > 0, 
				Перечисления.ВариантыОплатыПоставщику.АвансДоПодтверждения, 
				Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления));
		ПараметрыНазначения.Вставить("ПунктКонтрактаПредмета", "");
		ПараметрыНазначения.Вставить("ПунктКонтрактаОплаты", "");
		ПараметрыНазначения.Вставить("СуммаДокумента", СтрокаГрафика.ГрафикСумма);
		ПараметрыНазначения.Вставить("Валюта", ОсновнаяИнформация["КонтрактВалюта"]);
		ПараметрыНазначения.Вставить("БанковскийСчетКонтрагента", Неопределено);
		ПараметрыНазначения.Вставить("НазначениеПлатежа", "");
		ПараметрыНазначения.Вставить("Ссылка", Документы.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка());
		...
		ДенежныеСредстваСервер.ЗаполнитьТекстНазначенияПлатежа(ПараметрыНазначения);
		НазначениеПлатежа = ПараметрыНазначения.НазначениеПлатежа;
	КонецЕсли;
...
Функция ТекстЗапросаПаспортКонтракта()
	...
	|	ДоговорыИнтеркампани.ГосударственныйКонтракт.Код,
	...
	|	ЭтапыГрафикаИсполненияДоговора.ДатаПоГрафику КАК ГрафикДата,
	|	ЭтапыГрафикаИсполненияДоговора.СуммаОплаты КАК ГрафикСумма,
	|	ЭтапыГрафикаИсполненияДоговора.Комментарий КАК ГрафикКомментарий
	|ИЗ
	|	ОсновнаяИнформацияОКонтракте КАК ОсновнаяИнформацияОКонтракте
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ГрафикИсполненияДоговора.ЭтапыГрафикаИсполненияДоговора КАК ЭтапыГрафикаИсполненияДоговора
	|		ПО ОсновнаяИнформацияОКонтракте.Контракт = ЭтапыГрафикаИсполненияДоговора.Ссылка.Договор
	|ГДЕ
	|	// Только этапы оплаты
	|	ЭтапыГрафикаИсполненияДоговора.СуммаОплаты <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
