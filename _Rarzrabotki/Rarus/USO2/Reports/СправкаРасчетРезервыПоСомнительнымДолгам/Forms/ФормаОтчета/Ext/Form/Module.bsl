...
Процедура ВыбратьПериодЗавершение(РезультатВыбора, ДопПараметры) Экспорт
	...
	// До 2017 года не выводятся колонки по встречной задолженности
	// Обработаем ситуацию, когда отчет строится сначала за период в 2016 году, а потом в 2017 году или наоборот
	// Если требуется добавить/удалить колонки встречной задолженности, то инициализируем компоновщик вновь.
	Если КонецПериода <> Отчет.КонецПериода 
		И ((КонецПериода < '2017-01-01' И Отчет.КонецПериода >= '2017-01-01') 
		ИЛИ (КонецПериода >= '2017-01-01' И Отчет.КонецПериода < '2017-01-01')) Тогда
...
Процедура ИнициализацияКомпоновщикаНастроек()
	...
	// Для этого используем отдельный вариант настроек компоновщика, в имя которого добавлен суффикс "До2017" 
	// В самих настройках в имена группировок также добавлен суффикс "До2017" для вывода отдельных макетов.
	Если ЗначениеЗаполнено(Отчет.КонецПериода) И Отчет.КонецПериода < '2017-01-01' Тогда
		Суффикс = "До2017";
	Иначе
		Суффикс = "";
	КонецЕсли;	
	БухгалтерскиеОтчетыВызовСервера.ИнициализацияКомпоновщикаНастроек(ЭтаФорма, ОрганизацияИзменилась, "СомнительныеДолги"+Суффикс);
