...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ORG_SCALE" Тогда
		...
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Неопределено);
	ИначеЕсли Область.Имя = "FISH_CAT" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "FISH_CAT");
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Неопределено);
	ИначеЕсли Область.Имя = "OBJ_EXPLOITATION" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "OBJ_EXPLOITATION");
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, Неопределено);
	ИначеЕсли Область.Имя = "SEP_DIV" 
		Или Область.Имя = "FINDIVID"
		Или Область.Имя = "AREA_INFO1"
		Или Область.Имя = "AREA_INFO2"
		Или Область.Имя = "AREA_INFO4"
		Или Область.Имя = "AREA_INFO5"
		Или Область.Имя = "POPUL_LOC"
		Или Область.Имя = "DRINK_WTS"
		Или Область.Имя = "NOT_REG_OBJ"
		Или Область.Имя = "URBAN_AIR" Тогда
	...
	ИначеЕсли Область.Имя = "СгенерироватьИдентификатор" Тогда
		...
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
	ИначеЕсли Область.Имя = "СгенерироватьИдентификатор1" Тогда
		ПолеТабличногоДокументаФормаОтчета.Области.ID_EO.Значение = Строка(Новый УникальныйИдентификатор);
		СтандартнаяОбработка = Ложь;
		Модифицированность = Истина;
	ИначеЕсли Область.Имя = "ОргРПН" Или Область.Имя = "КодОргРПН" Тогда
		...
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, ОбластьДляВвода, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, "ОргРПН");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "FISH_CAT", 512, , "Категория рыбного значения", КоллекцияСписковВыбора["FISH_CAT"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "OBJ_EXPLOITATION", 512, , "Эксплуатация", КоллекцияСписковВыбора["OBJ_EXPLOITATION"]);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	СпПарам = "ОргРПН,КодОргРПН,ID_ORG,FNAME_ORG,SNAME_ORG,INN_ORG,ADDJ_OKATO,ADDJ_INDEX,ADDR_JUR,";
	СпПарам = СпПарам + "ADDJ_STREET,OKVED_CODE,КодОКТМООНВ,RPT_OKATO,КодОКВЭД2ОНВ,";
	СпПарам = СпПарам + "OBJ_CODE,ID_EO,NAME_EO,OKATO_EO,ADDR_EO";
	...
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СпПарам, ",", Истина) Цикл 
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки[Элт]) Тогда 
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен обязательный показатель!", "ПолеТабличногоДокументаФормаОтчета", Элт);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьИмяФайла(ПараметрыВыгрузки)
	...
	ИмяФайлаXML = ПараметрыВыгрузки.ОргКодСубъект + "_2тпа_"
...
Функция СформироватьСтруктуруПараметров()
	...
	ПоказателиОтчета = Новый Структура();
	ЗаполнитьПоказателиОтчетаНаСервере(ПоказателиОтчета);
	ТабДокумент = ПоказателиОтчета.ПолеТабличногоДокументаФормаОтчета;
...
Процедура ДополнитьПараметрыВыгрузки(ПараметрыВыгрузки)
	AREA_INFO = 0;
	AREA_INFO = AREA_INFO + ?(ПараметрыВыгрузки.AREA_INFO1 = Истина, 1, 0);
	AREA_INFO = AREA_INFO + ?(ПараметрыВыгрузки.AREA_INFO2 = Истина, 2, 0);
	AREA_INFO = AREA_INFO + ?(ПараметрыВыгрузки.AREA_INFO4 = Истина, 8, 0);
	AREA_INFO = AREA_INFO + ?(ПараметрыВыгрузки.AREA_INFO5 = Истина, 16, 0);
	ПараметрыВыгрузки.Вставить("AREA_INFO", AREA_INFO);
	ПараметрыВыгрузки.Вставить("NOT_REG_OBJ_BOOL", ?(ПараметрыВыгрузки.NOT_REG_OBJ = Истина, "true", "false"));
	ПараметрыВыгрузки.Вставить("SEP_DIV_BOOL", ?(ПараметрыВыгрузки.SEP_DIV = Истина, "true", "false"));
	ПараметрыВыгрузки.Вставить("URBAN_AIR_BOOL", ?(ПараметрыВыгрузки.URBAN_AIR = Истина, "true", "false"));
	ПараметрыВыгрузки.Вставить("FINDIVID_BOOL", ?(ПараметрыВыгрузки.FINDIVID = Истина, "true", "false"));
	ПараметрыВыгрузки.Вставить("POPUL_LOC_BOOL", ?(ПараметрыВыгрузки.POPUL_LOC = Истина, "true", "false"));
	ПараметрыВыгрузки.Вставить("DRINK_WTS_BOOL", ?(ПараметрыВыгрузки.DRINK_WTS = Истина, "true", "false"));
	...
	Для Инд = 1 По 9 Цикл 
		Заполнено = Ложь;
		Для Гр = 3 По 8 Цикл
			Заполнено = Заполнено Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000100"+Инд+"0" + Гр]);
		КонецЦикла;
		
		ПараметрыВыгрузки.Вставить("П000001000101_" + Инд, ?(Заполнено, "10" + Инд, ""));
		Для Гр = 3 По 8 Цикл
			ПараметрыВыгрузки.Вставить("П00000100010" + Гр + "_" + Инд, ПараметрыВыгрузки["П000100"+Инд+"0" + Гр]);
		КонецЦикла;
	КонецЦикла;
	...
	Для Инд = 1 По 3 Цикл 
		Заполнено = Ложь;
		Для Гр = 3 По 6 Цикл
			Заполнено = Заполнено Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000300"+Инд+"0" + Гр]);
		КонецЦикла;
		
		ПараметрыВыгрузки.Вставить("П000003000101_" + Инд, ?(Заполнено, "30" + Инд, ""));
		Для Гр = 3 По 6 Цикл
			ПараметрыВыгрузки.Вставить("П00000300010" + Гр + "_" + Инд, ПараметрыВыгрузки["П000300"+Инд+"0" + Гр]);
		КонецЦикла;
	КонецЦикла;
	...
	Для Инд = 1 По 5 Цикл
		Заполнено = Ложь;
		Для Гр = 4 По 5 Цикл
			Заполнено = Заполнено Или ЗначениеЗаполнено(ПараметрыВыгрузки["П000500"+Инд+"0" + Гр]);
		КонецЦикла;
		
		ПараметрыВыгрузки.Вставить("П000005000101_" + Инд, ?(Заполнено, "50" + Инд, ""));
		Для Гр = 4 По 5 Цикл
			ПараметрыВыгрузки.Вставить("П00000500010" + Гр + "_" + Инд, ПараметрыВыгрузки["П000500"+Инд+"0" + Гр]);
		КонецЦикла;
	КонецЦикла;
	...
	Пока Истина Цикл 
		...
		Если Не ПараметрыВыгрузки.Свойство("П000002000101_" + ИндСтр) Тогда 
			Прервать;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000002000102_" + ИндСтр])
			И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000002000103_" + ИндСтр])
			И Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000002000104_" + ИндСтр]) Тогда 
			
			ПараметрыВыгрузки["П000002000101_" + ИндСтр] = "";
		КонецЕсли;
	КонецЦикла;
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		ИндСтр = Формат(Инд, "ЧГ=");
		Если Не ПараметрыВыгрузки.Свойство("П000004000100_" + ИндСтр) Тогда 
			Прервать;
		КонецЕсли;
		
		Заполнено = Ложь;
		Для Ит = 1 По 8 Цикл 
			Заполнено = Заполнено Или ЗначениеЗаполнено(ПараметрыВыгрузки["П00000400010" + Ит + "_" + ИндСтр]);
		КонецЦикла;
		Если Не Заполнено Тогда
			ПараметрыВыгрузки["П000004000100_" + ИндСтр] = "";
		КонецЕсли; 
	КонецЦикла;
