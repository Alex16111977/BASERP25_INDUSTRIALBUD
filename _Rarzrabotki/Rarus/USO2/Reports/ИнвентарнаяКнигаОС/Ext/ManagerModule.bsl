...
Функция СформироватьОписаниеТекстаЗапроса(Подразделение, МОЛ, НачалоПериода, КонецПериода)
	...
	Если ИспользоватьВерсию22 Тогда
		...
		|	МАКСИМУМ(ПараметрыЦелевогоФинансированияОС.Период) КАК ДатаПоследнегоСчетаБухгалтерскогоУчетаОСЦФ
		|ПОМЕСТИТЬ ДатыПоследнихДвиженийПоРегистрамСведений
		|ИЗ
		|	ДвиженияОС КАК ДвиженияОС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПервоначальныеСведенияОСБухгалтерскийУчет КАК ПервоначальныеСведенияОС
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет КАК МестонахождениеОСПоследнее
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет КАК МестонахождениеОСПредпоследнее
		...
		|		ПО ДвиженияОС.ОсновноеСредство = СчетаБухгалтерскогоУчетаОС.ОсновноеСредство
		|			И ДвиженияОС.ДатаДвиженийБухгалтерии >= СчетаБухгалтерскогоУчетаОС.Период
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СчетаБухгалтерскогоУчетаОС КАК СчетаБухгалтерскогоУчетаОС
		|		ПО ДатыПоследнихДвиженийПоРегистрамСведений.ОсновноеСредство = СчетаБухгалтерскогоУчетаОС.ОсновноеСредство
		|			И ДатыПоследнихДвиженийПоРегистрамСведений.ДатаПоследнегоСчетаБухгалтерскогоУчетаОС = СчетаБухгалтерскогоУчетаОС.Период
		|			И (СчетаБухгалтерскогоУчетаОС.Организация = &Организация)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыЦелевогоФинансированияОС КАК ПараметрыЦелевогоФинансированияОС
		|		ПО ДатыПоследнихДвиженийПоРегистрамСведений.ОсновноеСредство = ПараметрыЦелевогоФинансированияОС.ОсновноеСредство
		|			И ДатыПоследнихДвиженийПоРегистрамСведений.ДатаПоследнегоСчетаБухгалтерскогоУчетаОСЦФ = ПараметрыЦелевогоФинансированияОС.Период
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет КАК МестонахождениеОСПоследнее
		...
		|			И СостоянияОСОрганизаций.Период <= &КонецПериода22
	...
	Если ИспользоватьВерсию24 Тогда
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ИнвентарнаяКнига.ПоследнееМОЛ КАК Справочник.ФизическиеЛица).Представление, """") КАК ПоследнееМОЛПредставление,
		|	ИнвентарнаяКнига.ПредпоследнееМОЛ КАК ПредпоследнееМОЛ,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ИнвентарнаяКнига.ПредпоследнееМОЛ КАК Справочник.ФизическиеЛица).Представление, """") КАК ПредпоследнеМОЛПредставление,
		|	ИнвентарнаяКнига.ПоследнееМестонахождение КАК ПоследнееМестонахождение,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ИнвентарнаяКнига.ПоследнееМестонахождение КАК Справочник.СтруктураПредприятия).Представление, """") КАК ПоследнееМестонахождениеПредставление,
		|	ИнвентарнаяКнига.ПредпоследнееМестонахождение КАК ПредпоследнееМестонахождение,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ИнвентарнаяКнига.ПредпоследнееМестонахождение КАК Справочник.СтруктураПредприятия).Представление, """") КАК ПредпоследнееМестонахождениеПредставление,
