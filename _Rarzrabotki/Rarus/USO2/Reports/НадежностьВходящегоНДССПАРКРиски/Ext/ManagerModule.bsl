...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	Если ПараметрыОтчета.Свойство("РежимРасшифровки") И ПараметрыОтчета.РежимРасшифровки Тогда
		ЗаголовокОтчета = НСтр("ru = 'Расшифровка предъявленного НДС%1';
								|en = 'Explanation of charged VAT%1'");
	Иначе
		ЗаголовокОтчета = НСтр("ru = 'Надежность поставщиков по предъявленному НДС%1';
								|en = 'Supplier reliability according to the charged VAT%1'");
	КонецЕсли;
...
Процедура НастроитьВариантыОтчета(Настройки, ОписаниеОтчета) Экспорт
	...
	ОписаниеВарианта.Описание = НСтр("ru = 'Контроль надежности входящего НДС с помощью сервиса СПАРК Риски.';
									|en = 'Control of incoming VAT reliability using the service 1C-SPARK Risks.'");
	...
	ОписаниеВарианта.Размещение.Вставить(Метаданные.Подсистемы.ФинансовыйРезультатИКонтроллинг.Подсистемы.УчетНДС, "");
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	СписокПунктовМеню.Добавить("НадежностьВходящегоНДССПАРКРиски", НСтр("ru = 'Надежность поставщиков по предъявленному НДС';
																		|en = 'Supplier reliability according to the charged VAT'"));
	
	НастройкиРасшифровки = Новый Структура();
	НастройкиРасшифровки.Вставить("НадежностьВходящегоНДССПАРКРиски", ПользовательскиеНастройки);
...
//
Функция ДобавитьКомандуВходяшийНДССПАРКРиски(КомандыОтчетов) Экспорт
	
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.НадежностьВходящегоНДССПАРКРиски) Тогда
		
		КомандаОтчет = КомандыОтчетов.Добавить();
		
		КомандаОтчет.Менеджер = Метаданные.Отчеты.НадежностьВходящегоНДССПАРКРиски.ПолноеИмя();
		КомандаОтчет.Представление = НСтр("ru = 'Надежность поставщиков по предъявленному НДС';
											|en = 'Supplier reliability according to the charged VAT'");
		
		КомандаОтчет.МножественныйВыбор = Истина;
		КомандаОтчет.Важность = "Обычное";
		КомандаОтчет.ДополнительныеПараметры.Вставить("ИмяКоманды", "ВходяшийНДССПАРКРиски");
