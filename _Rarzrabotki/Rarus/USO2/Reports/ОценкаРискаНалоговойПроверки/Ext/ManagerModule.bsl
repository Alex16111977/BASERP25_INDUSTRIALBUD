...
Процедура СреднемесячныйУровеньЗарплаты(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	ЧастиОКВЭД = СтрРазделить(СведенияПредельныхЗначений.КодОКВЭД, ".", Ложь);
	Пока ЧастиОКВЭД.Количество() > 0 Цикл
		Отбор.КодОКВЭД = СтрСоединить(ЧастиОКВЭД, ".");
		...
		Если СтрокиДанных.Количество() > 0 Тогда
			...
			Прервать;
		КонецЕсли;
		ЧастиОКВЭД.Удалить(ЧастиОКВЭД.ВГраница());
	КонецЦикла;
