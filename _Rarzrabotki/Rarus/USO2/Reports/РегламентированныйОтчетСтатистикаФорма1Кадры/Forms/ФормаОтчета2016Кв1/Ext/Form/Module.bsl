...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение
		= "РАБОТНИКОВ ОРГАНИЗАЦИЙ в " + НРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'")) + " году";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 2 по 12 Цикл 
		...
		Области["П0114"+ГрСтр].Значение = Области["П0115"+ГрСтр].Значение+Области["П0116"+ГрСтр].Значение+Области["П0117"+ГрСтр].Значение;
	КонецЦикла;
