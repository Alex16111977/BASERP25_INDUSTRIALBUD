...
Процедура ПересчитататьДанныеЦепочек()
	...
	Если ЛистыА.Количество() = 0 Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не найден соответствующий лист А';
																|en = 'Не найден соответствующий лист А'"));
		Возврат;
	ИначеЕсли ЛистыА.Количество() > 1 Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Найдено более одного листа А с указанным номером';
																|en = 'Найдено более одного листа А с указанным номером'"));
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПересчитататьДанныеЦепочекЗавершение", ЭтотОбъект, Новый Структура("НомерУчастника, ЛистыА", НомерУчастника, ЛистыА));
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Произвести пересчет цепочек (операция не может быть отменена)?';
											|en = 'Произвести пересчет цепочек (операция не может быть отменена)?'"), РежимДиалогаВопрос.ДаНет, 0);
КонецПроцедуры
...
Процедура Перенумеровать(Команда)
	...
	ТекстВопроса = НСтр("ru = 'При перенумерации страниц данные листов Г будут очищены. Продолжить?';
						|en = 'При перенумерации страниц данные листов Г будут очищены. Продолжить?'");
