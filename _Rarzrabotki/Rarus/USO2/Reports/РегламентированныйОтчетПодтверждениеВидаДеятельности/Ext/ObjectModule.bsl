...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	ИначеЕсли СтрДлина(СокрЛП(ПараметрыВыгрузки.РегНомер)) < 10 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Задан регистрационный номер страхователя, не состоящий из 10 цифр';
		|en = 'Задан регистрационный номер страхователя, не состоящий из 10 цифр'"), "Приложение1", "РегистрационныйНомерФСС");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодПодчиненности) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задан код подчиненности';
		|en = 'Не задан код подчиненности'"), "Приложение1", "КодПодчиненности");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(СокрЛП(ПараметрыВыгрузки.КодПодчиненности)) < 5 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Задан код подчиненности, не состоящий из 5 цифр';
		|en = 'Задан код подчиненности, не состоящий из 5 цифр'"), "Приложение1", "КодПодчиненности");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ОГРН) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки, НСтр(
		"ru = 'Не задан ОГРН';
		|en = 'Не задан ОГРН'"), "Приложение2", "ОГРН");
		СтатусПроверки = Ложь;
	Иначе
