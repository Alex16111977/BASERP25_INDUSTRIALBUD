...
Процедура ЗаполнитьЧисленностьРаботающихГраждан(ОбластьКарточка, ДатаОтчета, Организация)
	...
	Если ЗарплатаКадрыПовтИсп.СтандартныеПодсистемыВерсии302() Тогда
		ВызываемыйМодуль = ОбщегоНазначения.ОбщийМодуль("ОтчетыСервер");
	Иначе
		ВызываемыйМодуль = ОбщегоНазначения.ОбщийМодуль("ОтчетыКлиентСервер");
	КонецЕсли;
	
	ВызываемыйМодуль.ДобавитьВыбранноеПоле(НастройкиОтчета, "Организация");
	ВызываемыйМодуль.ДобавитьВыбранноеПоле(НастройкиОтчета, "КатегорияВоинскогоУчета");
	ВызываемыйМодуль.ДобавитьВыбранноеПоле(НастройкиОтчета, "ВсегоРаботающих");
	ВызываемыйМодуль.ДобавитьВыбранноеПоле(НастройкиОтчета, "ВсегоВЗапасе");
	ВызываемыйМодуль.ДобавитьВыбранноеПоле(НастройкиОтчета, "ОфицеровВЗапасе");
	ВызываемыйМодуль.ДобавитьВыбранноеПоле(НастройкиОтчета, "ПрапорщиковИСолдатВЗапасе");
...
Функция ТелефоныПодписантов(ПараметрыЗаголовка)
	...
	Для Каждого ДанныеПодписанта Из РабочиеТелефоныПодписантов Цикл 
		ТелефоныПодписантов.Вставить(ДанныеПодписанта.Объект, НСтр("ru = 'тел';
																	|en = 'tel'") + ". " + ДанныеПодписанта.Представление);
	КонецЦикла;
