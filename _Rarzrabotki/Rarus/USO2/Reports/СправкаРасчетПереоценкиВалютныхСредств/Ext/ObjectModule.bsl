...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ИспользоватьЛизинг = Константы.ИспользоватьЛизинг.Получить();
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ИспользоватьЛизинг", ИспользоватьЛизинг);
	//-- НЕ УТ
...
Функция ТекстЗапросаРасчетыСПоставщиками()
	...
	|			ВЫБОР 
	|				КОГДА ЕСТЬNULL(СчетаУчета.СчетУчетаАвансовВыданных, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчета.СчетУчетаАвансовВыданных
	|				КОГДА ЕСТЬNULL(СчетаУчетаПоУмолчанию.СчетУчетаАвансовВыданных, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчетаПоУмолчанию.СчетУчетаАвансовВыданных
	|				ИНАЧЕ
	|					ОстаткиИОбороты.ГруппаФинансовогоУчета.СчетУчетаАвансовВыданных
	|			КОНЕЦ
	|		ИНАЧЕ
	|			ВЫБОР 
	|				КОГДА ЕСТЬNULL(СчетаУчета.СчетУчетаРасчетовСПоставщиками, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчета.СчетУчетаРасчетовСПоставщиками
	|				КОГДА ЕСТЬNULL(СчетаУчетаПоУмолчанию.СчетУчетаРасчетовСПоставщиками, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчетаПоУмолчанию.СчетУчетаРасчетовСПоставщиками
	|				ИНАЧЕ
	|					ОстаткиИОбороты.ГруппаФинансовогоУчета.СчетУчетаРасчетовСПоставщиками
	|			КОНЕЦ
	|	КОНЕЦ КАК СчетУчета,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияРасчетовСПартнерами КАК СчетаУчета
	|	ПО ОстаткиИОбороты.Организация = СчетаУчета.Организация
	|		И ОстаткиИОбороты.ГруппаФинансовогоУчета = СчетаУчета.ГруппаФинансовогоУчета
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияРасчетовСПартнерами КАК СчетаУчетаПоУмолчанию
	|	ПО СчетаУчетаПоУмолчанию.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|		И СчетаУчетаПоУмолчанию.ГруппаФинансовогоУчета = ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаРасчетов.ПустаяСсылка)
	//-- НЕ УТ
	|{ГДЕ
	|	""РасчетыСПоставщиками"" КАК РазделПереоценки,
...
Функция ТекстЗапросаРасчетыСКлиентами()
	...
	|		КОГДА ОстаткиИОбороты.ПредоплатаУпр <> 0 ИЛИ ОстаткиИОбороты.ПредоплатаРегл <> 0 ТОГДА
	|			ВЫБОР 
	|				КОГДА ЕСТЬNULL(СчетаУчета.СчетУчетаАвансовПолученных, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчета.СчетУчетаАвансовПолученных
	|				КОГДА ЕСТЬNULL(СчетаУчетаПоУмолчанию.СчетУчетаАвансовПолученных, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчетаПоУмолчанию.СчетУчетаАвансовПолученных
	|				ИНАЧЕ
	|					ОстаткиИОбороты.ГруппаФинансовогоУчета.СчетУчетаАвансовПолученных
	|			КОНЕЦ
	|		ИНАЧЕ
	|			ВЫБОР 
	|				КОГДА ЕСТЬNULL(СчетаУчета.СчетУчетаРасчетовСКлиентами, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчета.СчетУчетаРасчетовСКлиентами
	|				КОГДА ЕСТЬNULL(СчетаУчетаПоУмолчанию.СчетУчетаРасчетовСКлиентами, &ПустойСчет) <> &ПустойСчет
	|					ТОГДА СчетаУчетаПоУмолчанию.СчетУчетаРасчетовСКлиентами
	|				ИНАЧЕ
	|					ОстаткиИОбороты.ГруппаФинансовогоУчета.СчетУчетаРасчетовСКлиентами
	|			КОНЕЦ
	|	КОНЕЦ КАК СчетУчета,
	//-- НЕ УТ
	|	ОстаткиИОбороты.Организация КАК Организация,
