...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Исключение
		...
		Если СтрНайти(ОписаниеОшибки, НСтр("ru = 'Поле не найдено (Субконто';
											|en = 'The field is not found (Extra dimension'")) Тогда
			ОписаниеПолей = "";
			...
			Шаблон = НСтр("ru = 'При заданных видах субконто формирование отчета с текущими группировками невозможно.';
							|en = 'Cannot generate a report with the current groupings and the specified extra dimension types.'");
			Если НЕ ПустаяСтрока(ОписаниеПолей) Тогда
				Шаблон = Шаблон + Символы.ПС + НСтр("ru = 'По причине: Недоступны для вывода поля группировки %1';
													|en = 'Due to: Grouping fields %1 are not available for display'");
			КонецЕсли;
		...
		Если ЭлементСтруктуры.Использование Тогда
			Для Каждого Поле Из ЭлементСтруктуры.ПоляГруппировки.Элементы Цикл
				Если Поле.Использование 
					...
					И СтрНайти(ОписаниеОшибки, НСтр("ru = 'Поле не найдено';
													|en = 'The field is not found'")) Тогда
