		...
		Если КодОперации = "1011705" ИЛИ КодОперации = "1011707" Тогда
			...
			Если ТекущаяОбластьИмя = "П000200007003"
				...
				Сообщение.Текст = НСтр("ru = 'Значения по строкам 60, 80, 90, 100 раздела 2 очищены вследствие изменения кода операции по строке 070.';
										|en = 'Значения по строкам 60, 80, 90, 100 раздела 2 очищены вследствие изменения кода операции по строке 070.'");
...
Процедура ПроверкаКС_1_29(СтруктураРезультата)
	...
	Заголовки.Вставить("ОписаниеКС"			, 
		...
		НСтр("ru = 'в стр.150 Раздела 3, должна быть равна сумме стр.180 Раздела 8, в которых стр.010 Раздела 8 = 20 ';
			|en = 'в стр.150 Раздела 3, должна быть равна сумме стр.180 Раздела 8, в которых стр.010 Раздела 8 = 20 '") +
		НСтр("ru = 'и сумме стр.180 Приложения 1 к Разделу 8, в которых стр.010 = 20, с учетом погрешности округления ';
			|en = 'и сумме стр.180 Приложения 1 к Разделу 8, в которых стр.010 = 20, с учетом погрешности округления '") +
		НСтр("ru = '(при наличии)';
			|en = '(при наличии)'"));
	...
	Настройки.ЭтоОшибка = НЕ Показатели.Р03С150.Итог = 
		РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, РасчетнаяСумма);
	Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки,
