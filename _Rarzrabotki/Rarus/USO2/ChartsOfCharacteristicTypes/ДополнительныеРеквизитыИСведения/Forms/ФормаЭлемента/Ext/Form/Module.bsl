	   ...
	   И НЕ Объект.ДополнительныеЗначенияСВесом Тогда
		...
		ТекстВопроса =
			НСтр("ru = 'Очистить введенные весовые коэффициенты?
			           ...
			           |en = 'Clear the entered weight factors?
			           |
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеСведения", НСтр("ru = 'Общие дополнительные сведения';
																|en = 'Common additional information'"), Истина);
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеРеквизиты", НСтр("ru = 'Общие дополнительные реквизиты';
																|en = 'Common additional attributes'"), Истина);
	...
	Если ВариантТекстаВопроса = "ИзменениеПометкиУдаления" Тогда
		Если Модифицированность Тогда
			ТекстВопроса = НСтр("ru = 'Для %1 пометки удаления необходимо записать внесенные изменения. Записать данные?';
								|en = 'You should write made changes for the %1 deletion mark. Write data?'");
	...
	Если ВариантТекстаВопроса = "ПереходКСпискуЗначений" Тогда
		ТекстВопроса =
			НСтр("ru = 'Переход к работе со списком значений
			           ...
			           |
			           |Данные будут записаны.';
			           |en = 'You can start working with a list of values
			           |only after data is written.
			           |
			           |Data will be written.'");
	Иначе
		ТекстВопроса =
			НСтр("ru = 'Данные будут записаны.';
				|en = 'Data will be written.'")
	КонецЕсли;
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Контекст) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Следующие настройки формата автоматически не применяются в большинстве мест:';
									|en = 'The following format settings are not applied in most places:'");
	...
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		...
		Иначе
			Заголовок = НСтр("ru = 'Добавление дополнительного реквизита';
							|en = 'Adding additional attribute'");
		КонецЕсли;
		...
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru = 'дополнительное сведение';
																		|en = 'additional information'"), НСтр("ru = 'дополнительный реквизит';
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru = 'сведения';
																		|en = 'information'"), НСтр("ru = 'реквизита';
																								|en = 'attribute'")),
				   ...
				   И НЕ Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Недопустимо удалять тип ""%1"",
						           ...
						           |en = 'Cannot delete type ""%1""
						           |as additional values have already been entered.
						           |Delete additional values you entered.
						           |
			...
			Если НЕ СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимо одновременно использовать типы значения
					           ...
					           |
					           |Второй тип удален.';
					           |en = 'Cannot use value types
					           |""%1"" and
					           |""%2"" simultaneously.
					           |
					           |The second type was deleted.'"),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) );
	...
	Иначе
		...
		Если СвойстваВладельца.ЭтоДополнительноеСведение <> Истина Тогда
			ШаблонУточнения = НСтр("ru = 'Список значений общий с реквизитом ""%1""';
									|en = 'Value list common with the ""%1"" attribute'");
		Иначе
			ШаблонУточнения = НСтр("ru = 'Список значений общий со сведением ""%1""';
									|en = 'Value list common with the ""%1"" information'");
		КонецЕсли;
			   ...
			   И СписокНаборов.Количество() < 2 Тогда
				...
				Если Объект.ЭтоДополнительноеСведение Тогда
					ШаблонУточнения = НСтр("ru = 'Сведение входит в набор: %1';
											|en = 'Information is included in set: %1'");
				Иначе
					ШаблонУточнения = НСтр("ru = 'Реквизит входит в набор: %1';
											|en = 'The attribute belongs to the set: %1'");
				КонецЕсли;
				...
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ru = 'Общее сведение входит в %1 %2';
												|en = 'Common information is included in %1 %2'");
					Иначе
						ШаблонУточнения = НСтр("ru = 'Общий реквизит входит в %1 %2';
												|en = 'Common attribute is included in %1 %2'");
					КонецЕсли;
				...
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ru = 'Общее сведение входит в набор: %1';
												|en = 'Common information is included in set: %1 '");
					Иначе
						ШаблонУточнения = НСтр("ru = 'Общий реквизит входит в набор: %1';
												|en = 'Common attribute is included in set: %1 '");
					КонецЕсли;
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ru = 'Сведение не входит в набор';
											|en = 'Information is not included in the set'");
				Иначе
					ТекстКомментария = НСтр("ru = 'Реквизит не входит в набор';
											|en = 'The attribute does not belong to the set'");
				КонецЕсли;
			Иначе
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ru = 'Общее сведение не входит в наборы';
											|en = 'Common information is not included in sets'");
				Иначе
					ТекстКомментария = НСтр("ru = 'Общий реквизит не входит в наборы';
											|en = 'Common attribute is not included in sets'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = НСтр("ru = 'Добавить общее сведение в набор (рекомендуется)
			...
			|
			|The selected information already belongs to several sets. It is recommended that you include it in this set ""as is"".
			|In such case, it will be possible to filter data of different types by it in lists and reports.'");
		СделатьОбщим = НСтр("ru = 'Сделать дополнительное сведение общим и добавить в набор
			...
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Make additional information public and add it to the set
			|
			|This option is right for cases when information should be the same for both sets.
			|You can filter data of different types in lists and reports.'");
		СделатьПоОбразцу = НСтр("ru = 'Сделать копию сведения по образцу (с общим списком значений)
			...
			|en = 'Back up data by example (with common list of values)
			|
			|List of values for this data will be the same for both sets.
			|With this option, you can configure list of values for similar data at one time.
			|You can edit name and some other data properties.'");
		СоздатьКопированием = НСтр("ru = 'Сделать копию сведения
			|
			|Будет создана копия сведения%1';
			|en = 'Back up data 
			|
			|Data%1 backup will be created'");
	Иначе
		ДобавитьОбщий = НСтр("ru = 'Добавить общий реквизит в набор (рекомендуется)
			...
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Add a common attribute to the set (recommended)
			|
			|Selected attribute is already included in several sets, that is why it is recommended that you include it in the set ""as is"".
			|In this case, it will be possible to filter data of different types in lists and reports by the attribute.'");
		СделатьОбщим = НСтр("ru = 'Сделать реквизит общим и добавить в набор
			...
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Make the attribute common and add to a set
			|
			|This option is suitable when the attribute should be the same for both sets.
			|In such case, it will be possible to filter data of different types in lists and reports by the attribute.'");
		СделатьПоОбразцу = НСтр("ru = 'Сделать копию реквизита по образцу (с общим списком значений)
			...
			|en = 'Back up attribute by example (with common list of values)
			|
			|List of values for this attribute will be the same for both sets.
			|With this option, you can configure list of values for multiple similar attributes at one time.
			|You can edit name and some other attribute properties.'");
		СоздатьКопированием = НСтр("ru = 'Сделать копию реквизита
			|
			|Будет создана копия реквизита%1';
			|en = 'Back up attribute
			|
			|Attribute%1 backup will be created'");
	КонецЕсли;
...
Функция НаименованиеУжеИспользуется(Знач Заголовок, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование, Знач Представления)
	...
	Если ЗначениеЗаполнено(Выборка.НаборСвойств) Тогда
		Если Выборка.ЭтоДополнительноеСведение Тогда
			ТекстВопроса = НСтр("ru = 'Существует дополнительное сведение с наименованием
			                          |""%1"".';
			                          |en = 'Additional information exists with name
			                          |""%1"".'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Существует дополнительный реквизит с наименованием
			                          |""%1"".';
			                          |en = 'An additional attribute with name
			                          |""%1"" already exists.'");
		КонецЕсли;
	Иначе
		Если Выборка.ЭтоДополнительноеСведение Тогда
			ТекстВопроса = НСтр("ru = 'Существует общее дополнительное сведение с наименованием
			                          |""%1"".';
			                          |en = 'Public additional information exists with name
			                          |""%1"".'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Существует общий дополнительный реквизит с наименованием
			                          |""%1"".';
			                          |en = 'A common additional attribute exists with name
			                          |""%1"".'");
		КонецЕсли;
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС
		                         ...
		                         |otherwise, the application may not work properly.'"),
		НовоеНаименование);
...
Функция ИмяУжеИспользуется(Знач Имя, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС
		                         ...
		                         |en = 'It is recommended that you use another name;
		                         |otherwise, the application may function incorrectly.
		                         |
