	...
	Для Каждого Стр Из ВидыБюджетов Цикл
		Если Стр.КлючСтроки <> ТекущиеДанные.КлючСтроки И Стр.КлючСтроки = Стр.КлючСтрокиНастройкиАналитики Тогда
			СписокНастроек.Добавить(Стр.КлючСтрокиНастройкиАналитики,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Аналогично с.%1.%2';
																			|en = 'Similar to .%1.%2'"),Стр.НомерСтроки,Стр.ВидБюджета));
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработкаПроверкиЗаполненияНаКлиенте(Отказ)
	...
	Для Каждого Стр Из ВидыБюджетов Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.ВидБюджета) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке %1 не заполнен вид бюджета';
																															|en = 'Budget kind is not populated in line %1 '"),Сч+1),,,
		...
		Если Не ЗначениеЗаполнено(Стр.ПредставлениеИзмерений) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке %1 не заполнены настройки измерений';
																															|en = 'Measure settings are not filled in line %1'"),Сч+1),,,
