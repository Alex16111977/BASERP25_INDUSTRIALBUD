	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Запись пооперационного расписания';
				|en = 'Write operation timetable'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ОтменитьТранзакцию();
 		...
 		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Выполнение операций';
				|en = 'Operation performance'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Попытка
		...
		Если ДанныеЗаблокированы Тогда
			...
			ТекстИсключения = НСтр("ru = 'При попытке изменения способа управления операциями возникала ошибка:
				...
				|department timetable operations are under recalculation'");
