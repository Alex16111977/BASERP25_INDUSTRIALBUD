	...
	Если Не ЭтоИзмененияПараметраРаботыПрограммы(ПоследниеИзменения) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для параметра работы программы ""%1"" не найдены изменения.';
				|en = 'Changes are not found for the ""%1"" application parameter.'"), ИмяПараметра)
...
Функция ЗагрузитьОбновитьПараметрыРаботыПрограммыВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ru = 'Фоновое обновление параметров работы программы';
														|en = 'Background update of the application parameters'");
	...
	Если Не ДоступноВыполнениеФоновыхЗаданий() Тогда
		ВызватьИсключение
			НСтр("ru = 'Обновление параметров работы программы не может быть выполнено,
			           ...
			           |en = 'Cannot update the application parameters as configuration extensions 
			           |that modify rights in configuration roles are connected. 
			           |Disable such extensions for update.'");
	КонецЕсли;
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		КраткоеПредставлениеОшибки =
			НСтр("ru = 'Не удалось обновить параметры работы программы по причине:
			           ...
			           |en = 'Cannot update application parameters due to: 
			           |Background job executing the update is canceled.'");
		...
		Иначе
			КраткоеПредставлениеОшибки =
				НСтр("ru = 'Не удалось обновить параметры работы программы по причине:
				           ...
				           |en = 'Cannot update application parameters due to: 
				           |Background job executing the update did not return the result.'");
		КонецЕсли;
	   ...
	   И Не ВыполнятьОбновлениеБезФоновогоЗадания() Тогда
		ВызватьИсключение
			НСтр("ru = 'Не удалось обновить параметры работы программы по причине:
			           ...
			           |en = 'Cannot update application parameters due to: 
			           |Connected configuration extensions are found.'");
	КонецЕсли;
	   ...
	   И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение
			НСтр("ru = 'Не удалось обновить параметры работы программы по причине:
			           ...
			           |en = 'Cannot update application parameters due to: 
			           |Cannot update in the data area.'");
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
			...
			ТекстОшибки =
				НСтр("ru = 'Информационная база не может быть обновлена из-за проблемы в главном узле:
				           ...
				           |%1';
				           |en = 'Infobase cannot be updated due to the problem in the main node:
				           |- the main node was updated incorrectly (configuration version number might not be increased
				           |  that is why catalog ""Metadata object IDs"" was not populated);
				           |- or priority data export was canceled 
				           |(items of catalog ""Metadata object IDs"").
				           |
				           |Update the main node again, register priority data for export 
				           |and repeat data synchronization:
				           |- in the main node, run application with parameter /C ЗапуститьОбновлениеИнформационнойБазы;
				           |%1'");
			...
			Если НастройкаПодчиненногоУзлаРИБ Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					НСтр("ru = '- затем повторите создание подчиненного узла.';
						|en = '- then repeat subnode creation.'"));
			Иначе
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					НСтр("ru = '- затем повторите синхронизацию данных с этой информационной базой
					           ...
					           | (first in the main node, then in the infobase after restart).'"));
			КонецЕсли;
...
Процедура ПроверитьВозможностьОбновленияВМоделиСервиса(ИмяПараметра, НовоеЗначение, Операция)
	...
	ИмяСобытия = НСтр("ru = 'Параметры работы программы.Не выполнено обновление в неразделенном режиме';
						|en = 'Application operation parameters. Update in the undivided mode denied.'",
		ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Комментарий =
		НСтр("ru = '1. Перешлите сообщение в техническую поддержку.
		           ...
		           |en = '1. Forward the message to technical support.
		           |2. Try to solve the issue by yourself. To do so,
		           | run the application with 1C:Enterprise 8 command line option
		           | ""/С ЗапуститьОбновлениеИнформационнойБазы"" on behalf of the user
		           | with service administrator rights, i.e. in shared mode.
		           |
		           |Details on problem parameter:'");
	...
	ТекстОшибки =
		НСтр("ru = 'Параметры работы программы не обновлены в неразделенном режиме.
		           ...
		           |en = 'Application parameters are not updated in the undivided mode.
		           |Contact service administrator. See the event log for details.'");
