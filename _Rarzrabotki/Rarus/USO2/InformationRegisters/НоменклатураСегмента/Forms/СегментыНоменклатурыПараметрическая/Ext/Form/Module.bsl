	...
	Если Поле.Имя = "СегментыКоличествоХарактеристик" Тогда
		...
		Если ТекущиеДанные.Характеристики.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Характеристики не назначены.';
																	|en = 'Characteristics are not assigned.'"));
			Возврат;
		КонецЕсли;
	...
	Если ИспользуютсяХарактеристики Тогда
		...
		Если Элементы.Сегменты.ДанныеСтроки(ВыделенныеСтроки[0]).Характеристики.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для данной номенклатуры предусмотрен учет по характеристикам, но не указано ни одной характеристики. Включение в сегмент невозможно.';
																	|en = 'Accounting by characteristics is available for these products, but no characteristics are specified. Cannot include in the segment. '"));
			Возврат;
		КонецЕсли;
