	...
	Если ИспользоватьДатуИВремяВСрокахЗадач Тогда
		...
		Иначе
			РазностьДат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'менее чем через час.';
					|en = 'less than in an hour.'"), Строка(ЧислоЧасов));
		КонецЕсли;
	Иначе
		...
		Иначе
			РазностьДат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'менее чем через день.';
					|en = 'less than in a day.'"), Строка(ЧислоДней));
		КонецЕсли;
	КонецЕсли;
