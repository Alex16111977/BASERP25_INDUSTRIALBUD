...
Процедура РассчитатьГрафикВФоне(Распоряжение) Экспорт
	...
	НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Расчет нормативного графика этапов распоряжения ""%1""';
			|en = 'Calculation of target plan of stages of reference ""%1""'"),
	...
	Для каждого Задание Из Задания Цикл
		...
		Исключение
			...
			СобытиеЖурналаРегистрации = НСтр(
				...
				|en = 'Target plan of production stages.Cancel the background job'",
...
Процедура ОжидатьЗавершениеТранзакцийЗаписиЭтапов(Распоряжение)
	...
	Исключение
		...
		СобытиеЖурналаРегистрации = НСтр(
			...
			|en = 'Target plan of production stages.Awaiting stage writing completion'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			Иначе
				...
				ВызватьИсключение СтрШаблон(
					НСтр("ru = 'В процессе расчета нормативного графика обнаружено зацикливание на этапе ""%1"".';
						|en = 'Loop was detected on the %1 step upon the target plan calculation.'"),
...
Процедура ЗаписатьРезультатыРасчетаГрафика(Распоряжение, Этапы) Экспорт
	...
	Исключение
		...
		СобытиеЖурналаРегистрации = НСтр(
			...
			|en = 'Target plan of production stages.Writing calculation results'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
