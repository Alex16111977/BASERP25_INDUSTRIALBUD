...
Процедура ЗагрузитьРекомендуемыеЭлектронныеПредставления(Параметры = Неопределено) Экспорт
	...
	Исключение
		СтрОш = НСтр("ru = 'Не удалось получить шаблоны для обновления';
					|en = 'Не удалось получить шаблоны для обновления'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Исключение
		СтрОш = НСтр("ru = 'Не удалось распаковать шаблоны для обновления';
					|en = 'Не удалось распаковать шаблоны для обновления'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ДобавитьШаблоныИзКонфигурации(АдресВременногоХранилища, КоличествоНезагруженныхШаблонов)
	...
	Исключение
		СтрОш = НСтр("ru = 'Не удалось удалить временные файлы';
					|en = 'Не удалось удалить временные файлы'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Для Каждого ШаблонЭВФ Из ШаблоныЭВФ Цикл
		...
		Если НЕ ДобавитьРеквизитыИзФайлаШаблона(ШаблонЭВФ) Тогда
			Если Не ТихийРежим Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не распознан формат XML-шаблона ""%1""';
		...
		Если СтрНайти(ШаблонЭВФ.ВерсияФормата, "1.3") = 0 Тогда
			Если Не ТихийРежим Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Версия формата XML-шаблона ""%1"" не соответствует версии ""1.3""';
		...
		Исключение
			Если Не ТихийРежим Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось записать XML-шаблон ""%1"":%2';
	...
	Если WSОперация <> Неопределено Тогда
		...
		Для НомерШаблона = 1 По КоличествоОбновляемыхШаблонов Цикл
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetTemplateBody).';
												|en = 'Обновление шаблонов Росстата. Ошибка вызова операции веб-сервиса (GetTemplateBody).'", КодОсновногоЯзыка),
					УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				Если Не ТихийРежим Тогда 
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка доступа к веб-сервису при получении данных шаблона Росстата:|%1 ""%2"".';
																																	|en = 'Ошибка доступа к веб-сервису при получении данных шаблона Росстата:|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы));
				КонецЕсли;
			...
			Если НЕ (ТипЗнч(Ответ) = Тип("ОбъектXDTO")
				...
				Если Не ТихийРежим Тогда 
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Формат ответа веб-сервиса не позволяет получить данные шаблона Росстата:|%1 ""%2"".';
			...
			Исключение
				Если Не ТихийРежим Тогда 
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при распаковке данных шаблона Росстата:|%1 ""%2"".';
			...
			Если ДанныеШаблона = Неопределено Тогда
				Если Не ТихийРежим Тогда 
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Архив не содержит необходимый файл шаблона:|%1 ""%2"".';
