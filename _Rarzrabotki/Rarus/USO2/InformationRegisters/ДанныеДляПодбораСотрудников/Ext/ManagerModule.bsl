...
Процедура ОбновитьДанныеДляПодбораСотрудников(Сотрудники, ОтключитьБизнесЛогику = Ложь, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Если Не РезультатЗапросаПоСотрудникам.Пустой() Тогда
		...
		Запрос.Текст =
			...
			|ПОМЕСТИТЬ ВТКадроваяИсторияСотрудников_ДляОбновленияДанныхДляПодбораСотрудников
			|ИЗ
		...
		Запрос.Текст =
			...
			|ИЗ
			|	ВТКадроваяИсторияСотрудников_ДляОбновленияДанныхДляПодбораСотрудников КАК РегистрСведений";
