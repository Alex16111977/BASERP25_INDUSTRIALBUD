...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Контролировать Тогда
		Заколовок = НСтр("ru = 'Номенклатура без контроля обеспечения (Склад: %1)';
						|en = 'Products without supply control (Warehouse: %1)'");
	Иначе
		Заколовок = НСтр("ru = 'Номенклатура с контролем обеспечения (Склад: %1)';
						|en = 'Products with supply control (Warehouse: %1)'");
	КонецЕсли;
	...
	ИначеЕсли Строка.ТипЗаписи = 2 И Строка.КоличествоИсключений = 1 Тогда
		Строка.Характеристика =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для %1 характеристики';
																		|en = 'for the %1 characteristic'"), Строка.КоличествоИсключений);
	Иначе
		Строка.Характеристика =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для %1 характеристик';
																		|en = 'for %1 characteristics'"), Строка.КоличествоИсключений);
	КонецЕсли;
