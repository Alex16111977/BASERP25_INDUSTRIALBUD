	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать()
			           ...
			           |en = 'An error occurred in procedure РегистрыСведений.НастройкиПравОбъектов.Прочитать()
			           |
			           |Incorrect parameter value СсылкаНаОбъект ""%1"".
			           |Rights cannot be set for the objects of table ""%2"".'"),
		...
		Если Настройки.Колонки.Найти(Строка.Право) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать()
				           ...
				           |
				           |Right ""%2""
				           |cannot be configured for  objects of table ""%1"", however it is written
				           |to information register НастройкиПравОбъектов
				           |for object ""%3"".
				           |
				           |Maybe, the infobase
				           |is not updated or there are update errors.
		...
		Если ПустыеСсылкиВладелецПрав.Получить(ТипВладельцаПрав) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в процедуре ОбновитьВспомогательныеДанныеРегистра
				           ...
				           |en = 'An error occurred in procedure ОбновитьВспомогательныеДанныеРегистра
				           |of the manager module of information register НастройкиПравОбъектов.
				           |
				           |Right owner type ""%1"" is not specified in dimension Object.'"),
				ТипВладельцаПрав);
		КонецЕсли;
...
Функция ВозможныеПрава()
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов
		           ...
		           |en = 'An error occurred in the ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов
		           |procedure of the УправлениеДоступомПереопределяемый common module.'")
		+ Символы.ПС
		+ Символы.ПС;
			...
			Если (ТипыПодпискиЗаписатьЗависимыеНаборыЗначенийДоступа.Получить(ТипОбъекта) <> Неопределено
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
					           ...
					           |but used to fill in access value sets
					           |as it is specified in one of subscriptions to the event:
					           |- ЗаписатьЗависимыеНаборыЗначенийДоступа*,
					           |- ЗаписатьНаборыЗначенийДоступа*.
					           |It is required to specify type in defined type ""Access value""
					           |for correct population of register НаборыЗначенийДоступа.'"),
					Строка(ТипСсылки));
			КонецЕсли;
			...
			Если СвойстваВидовДоступа.ПоТипамЗначений.Получить(ТипСсылки) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
					           ...
					           |but it is detected in the description of access kind ""%2"". '"),
					Строка(ТипСсылки),
			...
			Если СвойстваВидовДоступа.ПоТипамГруппИЗначений.Получить(ТипСсылки) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
					           ...
					           |но обнаружен в описании вида доступа ""%2"".';
					           |en = 'Rights owner type ""%1""
					           |cannot be used as a type of access value groups
					           |but it is detected in the description of access kind ""%2"".'"),
					Строка(ТипСсылки),
					СвойстваВидовДоступа.ПоТипамЗначений.Получить(ТипСсылки).Имя);
			КонецЕсли;
			...
			Если ТипыПодпискиОбновитьГруппыВладельцевНастроекПрав.Получить(ТипОбъекта) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Тип владельца прав ""%1""
					           ...
					           |en = 'Rights owner type ""%1""
					           |is not specified in defined type ""Object rights settings owner"".'"),
					Строка(ТипОбъекта));
			КонецЕсли;
		...
		Если ПраваВладельца.Получить(ВозможноеПраво.Имя) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для владельца прав ""%1""
				           ...
				           |en = 'The ""%2""
				           |right is defined again for the ""%1"" right owner.'"),
				ВозможноеПраво.ВладелецПрав,
				ВозможноеПраво.Имя);
		КонецЕсли;
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("ru = 'Для владельца прав ""%1""
						           ...
						           |en = 'The ""*"" character is specified for the ""%1""
				...
				Иначе
					ОписаниеОшибки =
						НСтр("ru = 'Для владельца прав ""%1""
						           ...
						           |В этом случае отдельных таблиц указывать не нужно.';
						           |en = 'The ""*"" character is specified for the ""%1""
						           |right owner for the ""%2"" right in tables for change.
						           |In this case, do not specify separate tables.'")
				КонецЕсли;
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("ru = 'Для владельца прав ""%1""
						           ...
						           |en = 'The ""*"" character is specified for the ""%1""
						           |right owner for the ""%2"" right in tables for reading.
						           |Chracter ""*"" is already specified in tables for reading for the ""%3"" right.'")
				Иначе
					ОписаниеОшибки =
						НСтр("ru = 'Для владельца прав ""%1""
						           |для права ""%2"" в таблицах для изменения указан символ ""*"".
						           ...
						           |en = 'The ""*"" character is specified for the ""%1""
						           |right owner for the ""%2"" right in tables for change.
						           |Character ""*"" is already specified in the tables for changes for the ""%3"" right.'")
				КонецЕсли;
		...
		ИначеЕсли Свойство = "ЧтениеВТаблицах" Тогда
			ОписаниеОшибки =
				НСтр("ru = 'Для владельца прав ""%1""
				           ...
				           |en = 'Specific table for reading ""%3""
				           |is specified for the ""%1"" right owner for the ""%2"" right.
				           |But it does not make sense, as right Read depends only on right Read.
				           |Only using the ""*"" character makes sense.'");
