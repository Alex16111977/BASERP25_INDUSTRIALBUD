...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.МодельТС) = Тип("СправочникСсылка.ТранспортныеСредства") Тогда
		МодельТС = Параметры.МодельТС.уатМодель;
	Иначе
	...
	флДоступностьФормулы = ПравоДоступа("Редактирование", Метаданные.РегистрыСведений.уатНормыРасходаГСМ);
...
Процедура ЗаполнитьФормулуВидаНормыПоУмолчанию(ИмяВидаНормы)
	...
	Если НаличиеСпидометра Тогда
		...
		ФормулыВидовНорм.Вставить("ЛинейнаяНормаКоэффициент", "[Kсн]+[Kтм]+[Kтс]+[Kур]+[Кнк]");
	Иначе
		ФормулыВидовНорм.Вставить("НормаСпециальнаяНаМоточас", "[Ht]*[T]");
		ФормулыВидовНорм.Вставить("НормаСпециальнаяНаМоточасКоэффициент", "[Kсн]+[Kтм]+[Kтс]+[Kур]+[Кнк]");
	КонецЕсли;
	...
	ФормулыВидовНорм.Вставить("НормаНаЗапуск", "[Hst]");
	...
	Если МодельТС.ТипТС.уатВидТС = Перечисления.уатВидыТС.Самосвал Тогда
