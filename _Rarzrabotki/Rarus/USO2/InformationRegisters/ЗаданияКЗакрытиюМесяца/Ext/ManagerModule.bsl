	...
	Попытка
		...
		Для Каждого ТекущаяОрганизация Из МассивОрганизаций Цикл
			...
			Для Каждого ТекущаяОперация Из МассивОпераций Цикл
				...
				//++ НЕ УТ
				ВнеоборотныеАктивы.ПередЗаписьюЗаданийКЗакрытиюМесяца(Месяц, ТекущаяОперация, ТекущаяОрганизация);
				//-- НЕ УТ
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось сформировать задание к закрытию месяца за %1 в организации %2 по причине: %3';
				|en = 'Cannot generate the job for month-end closing for %1 in the %2 company due to: %3'"),
			Выборка.Месяц,
			Выборка.Организация,
			ТекстОшибки);
...
Процедура СформироватьЗаданияНаСледующийПериод(ВременныеТаблицы, СледующийМесяц) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Операция = Перечисления.ОперацииЗакрытияМесяца.ФормированиеФинансовогоРезультата Тогда
			
			СледующийПериод = НачалоМесяца(КонецГода(СледующийМесяц));
			
		ИначеЕсли Выборка.Операция = Перечисления.ОперацииЗакрытияМесяца.ФормированиеРезервовПоСомнительнымДолгам Тогда
			
			СледующийПериод = Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПериодРасчетаРезервовПоСомнительнымДолгам(
				Выборка.Организация, СледующийМесяц);
			
		Иначе
			
			ВнеоборотныеАктивы.ОпределитьСледующийПериодФормированияЗаданий(
				Выборка.Операция, Выборка.Организация, СледующийПериод);
