...
Процедура СформироватьЗаданияКРаспределениюНДС(Регистраторы)
	...
	//++ НЕ УТ
	ИмяТаблицыДействующиеУчетныеПолитики = "ДействующиеУчетныеПолитики_втИзменениеВыручки";
	ТекстЗапроса = ТекстЗапроса 
		+ ОбщегоНазначения.РазделительПакетаЗапросов() 
		+ НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ТекстЗапросаДействующихУчетныхПолитик("ВтИзменениеВыручки", ИмяТаблицыДействующиеУчетныеПолитики)
		+ ОбщегоНазначения.РазделительПакетаЗапросов() 
		+ НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ТекстЗапросаЗаданийКЗакрытиюМесяцаПриИзмененииРасчетов(ИмяТаблицыДействующиеУчетныеПолитики);
	//-- НЕ УТ
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось сформировать задание к распределению НДС за %1 в организации %2 по причине: %3';
					|en = 'Cannot generate VAT allocation job for %1 in company %2 due to: %3'"),
				Выборка.Месяц,
				Выборка.Организация,
...
Функция ТекстЗапросаСуммВзаиморасчетовПоМассиву(ИмяВТ = "") Экспорт
	...
	|ГДЕ
	|	РасчетыСКлиентами.Регистратор ССЫЛКА Документ.КорректировкаРеализации
	|	И РасчетыСКлиентами.Регистратор В (&МассивДокументов)
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСКлиентами.Регистратор,
	|	РасчетыСКлиентами.АналитикаУчетаПоПартнерам,
	|	РасчетыСКлиентами.Валюта,
	...
	|ГДЕ
	|	РасчетыСПоставщиками.Регистратор ССЫЛКА Документ.КорректировкаПриобретения
	|	И РасчетыСПоставщиками.Регистратор В (&МассивДокументов)
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСПоставщиками.Регистратор,
	|	РасчетыСПоставщиками.АналитикаУчетаПоПартнерам,
	|	РасчетыСПоставщиками.Валюта,
	...
	|				ПО Ключи.Ссылка = ДанныеРегистра.АналитикаУчетаПоПартнерам
	|	ГДЕ
	|		ДанныеРегистра.ДокументРегистратор В (&МассивДокументов)
	|		И НЕ (ДанныеРегистра.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|				И Ключи.Организация = ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация))
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
