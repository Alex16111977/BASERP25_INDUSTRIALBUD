	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось сформировать задание к формированию записей книги покупок/продаж за %1 в организации %2 по причине: %3';
				|en = 'Cannot generate the job for generating purchase/sales ledger entries for %1 in the %2 company due to: %3'"),
			Выборка.Месяц,
			Выборка.Организация,
			ТекстОшибки);
			
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Учет НДС';
										|en = 'VAT accounting'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура СформироватьЗаданияПоДокументам(МассивДокументов) Экспорт
	...
	Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
		...
		ИначеЕсли СтрокаТипа.Тип = Тип("ДокументСсылка.ИнойДокументПодтвержденияНДС") Тогда
			СформироватьЗаданияПоИнымДокументамПодтвержденияНДС(МассивСсылок);
		КонецЕсли;
...
Процедура СформироватьЗаданияПоИнымДокументамПодтвержденияНДС(МассивСсылок)
