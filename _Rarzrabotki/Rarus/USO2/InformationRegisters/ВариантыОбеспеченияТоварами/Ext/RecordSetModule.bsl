...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Источник обеспечения, указанный в способе обеспечения потребностей, для типа обеспечения ""Перемещение"" должен отличаться от склада, для которого опеределяется вариант обеспечения товарами.';
								|en = 'Supply source specified in the demand fulfillment method for the ""Transfer"" supply type should differ from the warehouse for which goods supply method is determined.'");
