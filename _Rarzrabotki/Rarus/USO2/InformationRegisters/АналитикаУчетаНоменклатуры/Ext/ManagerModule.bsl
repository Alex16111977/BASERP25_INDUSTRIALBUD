...
Функция ПолучитьНаборЗаписей(ПараметрыАналитики)
	...
	Если Не ЗначениеЗаполнено(СтруктураАналитики.Номенклатура) Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: не указана номенклатура.';
								|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: products are not specified.'");
		...
		Если РеквизитыНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда 
			...
			Если ЗначениеЗаполнено(СтруктураАналитики.Назначение)
				...
				ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана номенклатура с типом ""Многооборотная тара"", и указано назначение.';
										|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: products with the ""Reusable package"" type are specified, and the intended use is indicated.'")
			КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(РеквизитыНоменклатуры.ИспользованиеХарактеристик)
			...
			Если ЗначениеЗаполнено(СтруктураАналитики.Характеристика) Тогда
				ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана для номенклатуры не настроено ведение характеристики, но характеристика указана.';
										|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: characteristic is not configured for the products, but the characteristic is indicated.'")
			КонецЕсли;
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(СтруктураАналитики.Характеристика) Тогда
				ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: указана для номенклатуры настроено ведение характеристики, но характеристика не указана.';
										|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: characteristic is configured for the products, but the characteristic is not indicated.'")
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтруктураАналитики.Серия) Тогда
		...
		Если ТипЗнч(СтруктураАналитики.МестоХранения) <> Тип("СправочникСсылка.Склады")
			...
			ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: серия указана, но место хранения не того типа.';
									|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: series is specified, but the storage location is of the wrong type.'");
		КонецЕсли;
		...
		Если РеквизитыНоменклатуры.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Товар Тогда
			...
			ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: серия указана, но номенклатура не с типом ""Товар"".';
									|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: series is specified, but the products are not of the ""Goods"" type.'");
	...
	Если ЗначениеЗаполнено(СтруктураАналитики.СтатьяКалькуляции) Тогда
		Если ТипЗнч(СтруктураАналитики.МестоХранения) <> Тип("СправочникСсылка.СтруктураПредприятия") Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка при получении набора регистра сведений АналитикаУчетаНоменклатуры: СтатьяКалькуляции указана, но место хранения не того типа.';
									|en = 'An error occurred while retrieving a set of the data register ProductAccountingDimension: CalculationArticle is specified, but the storage location is of the wrong type.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстЗначенияКлючейАналитикиВКоллекции(ИменаПолей)
	...
	|	КОНЕЦ КАК Серия,
	|	ВЫБОР
	|		КОГДА &ПолеСкладТовар В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|			ТОГДА &МестаУчетаТовар
	|		ИНАЧЕ &ПолеСкладТовар
	|	КОНЕЦ КАК СкладТовар,
	|	ВЫБОР
	|		КОГДА &ПолеСкладМногооборотнаяТара В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|			ТОГДА &МестаУчетаМногооборотнаяТара
	|		ИНАЧЕ &ПолеСкладМногооборотнаяТара
	|	КОНЕЦ КАК СкладМногооборотнаяТара,
	|	ВЫБОР
	|		КОГДА &ПолеСкладУслуга В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|			ТОГДА &МестаУчетаУслуга
	|		ИНАЧЕ &ПолеСкладУслуга
	|	КОНЕЦ КАК СкладУслуга,
	|	ВЫБОР
	|		КОГДА &ПолеСкладРабота В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка), ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
