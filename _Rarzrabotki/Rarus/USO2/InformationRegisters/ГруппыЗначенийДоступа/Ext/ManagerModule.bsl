	...
	Иначе
		ВызватьИсключение
			НСтр("ru = 'Ошибка в процедуре ОбновитьГруппировкиПользователей
			           ...
			           |en = 'An error occurred in ОбновитьГруппировкиПользователей 
			           |procedure of manager module of data registry of Access value group.
			           |
...
Процедура ОбновитьГруппыЗначенияДоступа(ЗначениеДоступа, ЕстьИзменения, ЗначенияСИзменениямиПоТипам)
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка при обновлении групп значений доступа.';
			|en = 'An error occurred while updating access value groups.'")
		+ Символы.ПС
		+ Символы.ПС;
		...
		Если СвойстваВидаДоступа.ТипГруппЗначений = Тип("Неопределено") Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У значения доступа ""%1""
				           ...
				           |%3';
				           |en = 'Cannot read the Link attribute of access value ""%1""
				           |of type ""%2""
				           | due to:
				           |%3'"),
			 ...
			 ИЛИ ТабличнаяЧастьМетаданные.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У типа значений доступа ""%1""
					           ...
					           |en = 'Special tabular section ГруппыДоступа
					           |with special attribute ГруппаДоступа is not created
			...
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У значения доступа ""%1""
					           ...
					           |%3';
					           |en = 'Cannot read the ГруппаДоступа tabular section
					           |with the ГруппаДоступа attribute of access value ""%1""
					           |of type ""%2""
					           | due:
					           |%3'"),
					Строка(ЗначениеДоступа),
			...
			Если МетаданныеТипа.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У типа значений доступа ""%1""
					           ...
					           |en = 'Special attribute ГруппаДоступа
			...
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У значения доступа ""%1""
					           ...
					           |%3';
					           |en = 'Cannot read the ГруппаДоступа attribute of access value ""%1""
					           |of type ""%2""
					           | due:
					           |%3'"),
					Строка(ЗначениеДоступа),
	...
	Иначе
		ВызватьИсключение
			НСтр("ru = 'Ошибка в процедуре ОбновитьГруппыИсполнителей
			           ...
			           |
			           |Указаны неверные параметры.';
			           |en = 'An error occurred in ОбновитьГруппыИсполнителей procedure
			           | of manager module of data registry ГруппыЗначенийДоступа.
			           |
			           |Parameters are incorrect.'");
	КонецЕсли;
