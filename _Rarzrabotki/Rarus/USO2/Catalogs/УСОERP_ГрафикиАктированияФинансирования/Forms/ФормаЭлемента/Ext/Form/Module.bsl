...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийВидГрафика = Объект.ВидГрафика;
...
Процедура ПриОткрытии(Отказ)
	 ...
	 НастроитьВидимостьСтраницГрафика();
...
Процедура ВидГрафикаПриИзменении(Элемент)
	...
	Если ТекущийВидГрафика<> Объект.ВидГрафика Тогда
		Если ТекущийВидГрафика = ПредопределенноеЗначение("Перечисление.УСОERP_ВидыГрафиковАктированияФинансирования.ГрафикАктирования")
			И ЗначениеЗаполнено(Объект.ВариантАктирования) Тогда
			ТекстВопроса = НСтр("ru='Вариант актирования будет очищен. Продолжить?'");
			
			СписокКнопок = Новый СписокЗначений;
			СписокКнопок.Добавить(КодВозвратаДиалога.Да);
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет);
			
			ОписаниеОповещения = Новый ОписаниеОповещения("ПодтверждениеУдаленияАктирования", ЭтотОбъект,);
			
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок);
			
		ИначеЕсли ТекущийВидГрафика = ПредопределенноеЗначение("Перечисление.УСОERP_ВидыГрафиковАктированияФинансирования.ГрафикФинансирования")
			И Объект.Этапы.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ru='Этапы финансирования будут очищены. Продолжить?'");
			
			СписокКнопок = Новый СписокЗначений;
			СписокКнопок.Добавить(КодВозвратаДиалога.Да);
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет);
			
			ОписаниеОповещения = Новый ОписаниеОповещения("ПодтверждениеУдаленияФинансирования", ЭтотОбъект,);
...
Процедура Подключаемый_РазрешитьРедактированиеРеквизитовОбъекта(Команда)
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыФормы.Вставить("ВидГрафика", Объект.ВидГрафика);
		ОткрытьФорму("Справочник.УСОERP_ГрафикиАктированияФинансирования.Форма.РазблокированиеРеквизитов",ПараметрыФормы,,,,, 
...
Процедура РассчитатьИтоговыеПоказателиГрафикаОплаты(Форма)
	...
	Если Форма.Объект.ВидГрафика = ПредопределенноеЗначение("Перечисление.УСОERP_ВидыГрафиковАктированияФинансирования.ГрафикФинансирования") Тогда
...
&НаКлиенте
Процедура НастроитьВидимостьСтраницГрафика()
	
	Элементы.ГрафикФинансирования.Видимость = (Объект.ВидГрафика = ПредопределенноеЗначение("Перечисление.УСОERP_ВидыГрафиковАктированияФинансирования.ГрафикФинансирования"));
	Элементы.ГрафикАктирования.Видимость 	= (Объект.ВидГрафика = ПредопределенноеЗначение("Перечисление.УСОERP_ВидыГрафиковАктированияФинансирования.ГрафикАктирования"));
	
КонецПроцедуры

&НаКлиенте
Процедура ПодтверждениеУдаленияАктирования(РезультатВопроса, ДополнительныеПараметры) Экспорт
    
    Ответ = РезультатВопроса;
	Если Ответ <> КодВозвратаДиалога.Да Тогда
		Объект.ВидГрафика = ТекущийВидГрафика;
        Возврат;
    КонецЕсли;
	
	ТекущийВидГрафика = Объект.ВидГрафика;
    Объект.ВариантАктирования = ПредопределенноеЗначение("Перечисление.УСОERP_ВидыГрафиковАктированияФинансирования.ПустаяСсылка");
	Объект.Сдвиг = 0;
	НастроитьВидимостьСтраницГрафика();
...
&НаКлиенте
Процедура ПодтверждениеУдаленияФинансирования(РезультатВопроса, ДополнительныеПараметры) Экспорт
