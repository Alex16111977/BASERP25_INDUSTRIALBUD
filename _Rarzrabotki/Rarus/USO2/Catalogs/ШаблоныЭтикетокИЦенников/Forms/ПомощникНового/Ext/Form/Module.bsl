...
Процедура ПриИзмененииНазначенияСервер()
	...
	Если Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляТоваров
		...
		ИЛИ Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляШтрихкодовУпаковок
...
Процедура НапечататьОбразец(Команда)
	...
	Если Не ЗначениеЗаполнено(ДляЧего) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Для чего""';
																|en = 'The ""For what"" field is not filled in'"),,"ДляЧего");
		Возврат;
	КонецЕсли;
