...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Владелец", МодельБюджетирования) Тогда
		ТекстСообщения = НСтр("ru = 'Непосредственное открытие этой формы не предусмотрено. Для открытия формы можно воспользоваться командой ""Добавить по виду бюджета"" в форме списка правил лимитов по данным бюджетирования.';
								|en = 'Cannot open this form directly. To open the form, use the ""Add by budget profile"" command in the form of list of limit rules by budgeting data.'");
...
Процедура ЗаполнитьДеревоДоступныхКСозданиюЭлементов()
	...
	Для каждого СтрокаСтатьиБюджета Из СтрокиСтатей Цикл
		...
		КорневойЭлемент = ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(СтрокаСтатьиБюджета,
	...
	Если НайденныеСтроки.Количество() Тогда
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'Не удалось создать правила лимитов для вида бюджета: %1 модели бюджетирования: %2, по причине: %3';
								|en = 'Cannot create limit rules for the %1 budget profile of the %2 budgeting model due to: %3 '",
				КодОсновногоЯзыка);
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
			                                                                      ...
			                                                                      ПредставлениеОшибки);
			ЗаписьЖурналаРегистрации(НСтр("ru = 'ПравилаЛимитовПоДаннымБюджетирования.СозданиеПравил';
											|en = 'ПравилаЛимитовПоДаннымБюджетирования.СозданиеПравил'", КодОсновногоЯзыка),
		...
		Если ПравилаСозданы Тогда
			ЗаголовокДекорации = НСтр("ru = 'Правила созданы успешно';
										|en = 'Rules are created successfully'", КодОсновногоЯзыка)
			                   + Символы.ПС + НСтр("ru = 'Создано правил лимитов: %1';
													|en = 'Created limit rules: %1'", КодОсновногоЯзыка);
		...
		Иначе
			ЗаголовокДекорации = НСтр("ru = 'Не удалось создать правила по причине:';
										|en = 'Cannot create rules due to:'", КодОсновногоЯзыка)
			                   + Символы.ПС + "%1";
