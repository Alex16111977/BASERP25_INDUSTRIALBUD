	...
	Если СтандартныеПодсистемыПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не используется.';
				|en = 'Catalog ""%1"" is not used.'"), НазваниеСправочника(ОбъектыРасширений));
	КонецЕсли;
	   ...
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не может использоваться
			           ...
			           |en = 'Catalog ""%1"" cannot be used in the infobase 
			           |with unconfirmed main node cancellation. 
			           |
			           |To restore connection with the main node, launch 1C:Enterprise 
			           |and click Restore or set from 1C:Enterprise script 
			           |the main node saved in constant ""Main node"".
			           |
			           |To confirm cancellation of connection with the main node, launch 1C:Enterprise 
			           |and click Disable or clear constant ""Main node"" from 1C:Enterprise script.'"),
				...
				Если Не ЗначениеЗаполнено(СвойстваЗагружаемого.Ссылка) Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
						           ...
						           |""%1"".';
						           |en = 'An error occurred when importing the metadata object ID.
						           |Cannot import a new item for which a reference of new 
						           |""%1"" is not specified.'"),
						Объект.ПолноеИмя);
				КонецЕсли;
			КонецЕсли;
			...
			Если ПолныеИменаЗагружаемых.Получить(НРег(Объект.ПолноеИмя)) <> Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
					           ...
					           |""%1"".';
					           |en = 'An error occurred when importing the metadata object IDs.
				...
				Если КлючиЗагружаемых.Получить(КлючОбъектаМетаданных) <> Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
						           ...
						           |""%1"".';
						           |en = 'An error occurred when importing the metadata object ID.
						           |Cannot import two items with the same key of the metadata object:
						           |""%1"".'"),
				...
				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу <> СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
						           ...
						           |en = 'An error occurred when importing the metadata object IDs.
						           |Cannot import an item which key of metadata object 
						           |""%1"" does not correspond to full name ""%2"".'"),
			...
			Если ОбъектМетаданных = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректное значение параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
					           ...
					           |en = 'Invalid value of the ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных parameter: 
					           |A non-existent metadata object is specified: ""%1"".'"),
		...
		Иначе	
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректный тип параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:
				           |""%1""';
				           |en = 'Invalid type of the ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных parameter: 
				           |""%1"" '"),
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректное значение параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных.
				           |Указан несуществующий объект метаданных: ""%1"".';
				           |en = 'Invalid value of the ОписаниеОбъектовМетаданных в функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных parameter: 
				           |A non-existent metadata object is specified: ""%1"".'"),
				ОписаниеОбъектаМетаданных);
		Иначе
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных общего модуля ОбщегоНазначенияПереопределяемый.';
			|en = 'An error occurred in the ПриДобавленииПереименованийОбъектовМетаданных procedure of the ОбщегоНазначенияПереопределяемый common module.'");
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не совпадают имена типов переименованного объекта метаданных.
			           |Прежний тип: ""%1"",
			           |новый тип: ""%2"".';
			           |en = 'Names of renamed metadata object types are not the same.
			           |Previous type: ""%1"",
			           |new type: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
	...
	Если Итог.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для типа объекта метаданных ""%1"" не требуется описывать переименование,
			           ...
			           |%2.';
			           |en = 'It is not required to describe renaming for metadata object type ""%1""
			           |as the information on metadata objects of this type is updated automatically.
			           |
			           |It is required to describe renaming only for the following types:
			           |%2.'"),
			СтароеПолноеИмя,
			СписокДопустимыхТипов);
	КонецЕсли;
	...
	Если Не Объект.ПометкаУдаления Тогда
		ВызватьИсключениеПоОшибке(ОбъектыРасширений,
			НСтр("ru = 'Удаление идентификаторов объектов, у которых значение
			           ...
			           |en = 'Cannot delete object IDs whose value
			           |of the ""Deletion mark"" attribute is set to False.'"));
	КонецЕсли;
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = НСтр("ru = 'Переименование идентификаторов объектов метаданных СтароеПолноеИмя -> НовоеПолноеИмя:';
											|en = 'Rename IDs of metadata objects СтароеПолноеИмя -> НовоеПолноеИмя:'")
		...
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений
				+ НСтр("ru = 'Добавление новых идентификаторов объектов метаданных:';
						|en = 'Add new metadata object IDs:'")
		...
		Если Не ТолькоПроверка
		   ...
		   И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ИмяСобытия = НСтр("ru = 'Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';
								|en = 'Metadata object IDs.Import critical changes'",
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru = 'Критичные изменения могут быть выполнены только
				           ...
				           |en = 'Critical changes can be executed only
				           |in the main node of the distributed infobase.
				           |See the event log for the content of the required changes.'"));
		КонецЕсли;
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("ru = 'Идентификаторы объектов расширений.Выполнены критичные изменения';
							|en = 'Extension object IDs.Critical changes made'",
							ОбщегоНазначения.КодОсновногоЯзыка()),
						НСтр("ru = 'Идентификаторы объектов метаданных.Выполнены критичные изменения';
							|en = 'Metadata object IDs.Critical changes made'",
							ОбщегоНазначения.КодОсновногоЯзыка())),
		...
		Если СтрЧислоВхождений(ВРег(ПолноеИмя), ВРег("Подсистема.")) > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при поиске дочерней подсистемы по полному имени (при поиске ""%1"" была найдена ""%2"").
				           ...
				           |en = 'An error occurred when searching for a subsidiary subsystem by full name (searched for ""%1"", ""%2"" was found).
				           |Do not give subsystems the same names, nor use a newer version of the platform.'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при поиске объекта метаданных по полному имени (при поиске ""%1"" был найден ""%2"").';
					|en = 'An error occurred when searching for metadata object by full name (searched for ""%1"", ""%2"" was found).'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		КонецЕсли;
	КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при работе со справочником ""%1"".';
			|en = 'An error occurred when operating with catalog ""%1"".'"),
		НазваниеСправочника(ОбъектыРасширений));
		...
		Если Объект.ЭтоНовый() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru = 'Создание нового идентификатора объекта
				           ...
				           |en = 'New object ID can be created 
				           |only automatically on catalog data update.'"));
		...
		ИначеЕсли ЗапрещеноИзменятьПолноеИмя(Объект) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При изменении идентификатора объекта указано
				           ...
				           |en = 'Full name ""%1"" was specified 
				           |while changing the object ID; this name can be set 
				           |only automatically when updating data of the catalog.'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При изменении идентификатора объекта указано
					           ...
					           |en = 'Full name ""%1"" 
					           |specified while changing
					           |object ID is already in use in the ""%2"" catalog.'"),
			...
			Если ВРег(Объект.ПолноеИмя) <> ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений,
					НСтр("ru = 'Изменение реквизита ""Полное имя"" может быть выполнено
					           |только в главном узле распределенной информационной базы.';
					           |en = 'Attribute ""Full name"" can be changed
					           |only in the main node of the distributed infobase.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеОшибкиИдентификаторыОбъектовРасширенийНедоступныВНеразделенномРежиме()
	...
	Возврат
		НСтр("ru = 'Справочник ""Идентификаторы объектов расширений""
		           ...
		           |en = 'The ""Extension object IDs"" catalog 
		           |cannot be used in an undivided mode.'");
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Объект метаданных не найден по полному имени:
					           |""%1"".';
					           |en = 'Metadata object was not found by full name: 
					           |""%1"".'"),
			   ...
			   И Не ЭтоПодсистема(ОбъектМетаданных) Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Объект метаданных не поддерживается:
					           ...
					           |en = 'Metadata object is not supported:
					           |""%1"".
					           |
					           |Supported metadata objects are listed in the function comment.'"),
					ПолноеИмяОбъектаМетаданных);
				Ошибки.Добавить(ОписаниеОшибки);
				Продолжить;
			КонецЕсли;
			   ...
			   И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификаторы объектов метаданных расширений не поддерживаются в неразделенном режиме.
					           ...
					           |en = 'Extension metadata object IDs are not supported in the undivided mode.
					           |Cannot return the ID of the ""%1"" metadata object
					           |of extension ""%2"" and version ""%3"".'"),
			...
			ШаблонОшибки = ?(Расширение <> Неопределено,
				НСтр("ru = 'Для объекта метаданных ""%1""
				           ...
				           |en = 'For the ""%1"" metadata object,
				           |ID is not found in the ""Object IDs of extension versions"" information register.'"),
				НСтр("ru = 'Для объекта метаданных ""%1""
				           |не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".';
				           |en = 'For the ""%1"" 
				           |metadata object ID is not found in catalog ""Metadata object IDs"".'"));
		...
		ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
			...
			ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
				НСтр("ru = 'Для объекта метаданных ""%1""
				           ...
				           |en = 'For the ""%1"" 
				           |metadata object some IDs are found in catalog ""Metadata object IDs"" and 
				           |information register ""Object IDs of extension versions"".'"),
				НСтр("ru = 'Для объекта метаданных ""%1""
				           |найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".';
				           |en = 'For the ""%1"" 
				           |metadata object some IDs are found in catalog ""Metadata object IDs"".'"));
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПолноеИмяОбъектаМетаданных);
			ДобавитьУточнениеОшибкиПараметровРаботыПрограммыДляРазработчика = Истина;
			Ошибки.Добавить(ОписаниеОшибки);
			Продолжить;
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для объекта метаданных ""%1""
					           ...
					           |en = 'ID for the ""%1"" 
					           |metadata object is found in the ""%2"" catalog,
					           |to which the deleted metadata object corresponds.'"),
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если ОдинЭлемент Тогда
			ЗаголовокОшибки = НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().';
									|en = 'An error occurred when executing function ОбщегоНазначения.ИдентификаторОбъектаМетаданных().'");
		...
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().';
									|en = 'An error occurred while executing the ОбщегоНазначения.ИдентификаторОбъектаМетаданных() function.'");
		Иначе
			ЗаголовокОшибки = НСтр("ru = 'Ошибки при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().';
									|en = 'Errors occurred when executing the ОбщегоНазначения.ИдентификаторыОбъектаМетаданных() function.'");
		КонецЕсли;
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				ПредставлениеКоличества = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru = ';%1 ошибка;; %1 ошибки; %1 ошибок; %1 ошибки';
						|en = ';%1 error;; %1 errors; %1 errors; %1 errors'"), (КоличествоОшибок - НомерОшибки));
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '... И еще %1';
						|en = '... And also %1'"),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().
				           ...
				           |""%1"".';
				           |en = 'An error occurred when performing the ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору() function.
				           |
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().';
							|en = 'An error occurred when executing function ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().'");
						...
						Если РасширенияКонфигурации.Получить(Отбор, ИсточникРасширенийКонфигурации.СеансОтключенные).Количество() > 0 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Расширение конфигурации %1 установлено, но отключено при запуске сеанса.
								           ...
								           |en = 'The %1 configuration extension is installed but disabled when starting the session.
								           |Usually it means that an error occurred when connecting a configuration extension.'"),
								Свойства.ИмяРасширения);
						Иначе
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Расширение конфигурации %1 установлено, но не подключено.
								           |Требуется перезапустить сеанс.';
								           |en = 'The %1 configuration extension is installed but not connected. 
								           |Restart the session.'"),
								Свойства.ИмяРасширения);
						КонецЕсли;
					Иначе
				...
				ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатору ""%1""
						           ...
						           |en = 'ID ""%1""
						           |found in catalog ""%2"" 
						           |matches a non-existing metadata object 
						           |""%3"".'"),
			...
			Иначе
				ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификатору ""%1""
					           ...
					           |en = 'ID ""%1""
					           |found in catalog ""%2""
					           |matches a metadata object ""%3""
					           |whose full name is different from the specified in the ID.'"),
					Строка(Свойства.Ссылка),
					НазваниеСправочника(Свойства.ОбъектРасширений),
		...
		Если Не Свойства.ОбъектРасширений И Свойства.ПометкаУдаления И Не ВерсияПрограммыОбновленаДинамически Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1""
				           ...
				           |en = 'ID ""%1"" 
				           |was found in catalog ""%2"", 
				           |but the ""Deletion mark"" attribute was set to True.'"),
				Строка(Свойства.Ссылка),
				НазваниеСправочника(Свойства.ОбъектРасширений));
		...
		Если ТекущаяПопытка > 10 Тогда
			ВызватьИсключение
				НСтр("ru = 'Не удалось выполнить замену дублей идентификаторов объектов метаданных.
				           ...
				           |Perform the action in the exclusive mode.'");
		КонецЕсли;
		...
		Если Не БезОшибок Тогда
			ВызватьИсключение
				НСтр("ru = 'Не удалось выполнить замену дублей идентификаторов объектов метаданных.
				           ...
				           |en = 'Cannot replace duplicates of metadata object IDs.
				           |For more information, see ID replacement errors in the event log.'");
		КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'При записи объекта ""%1"" возникла ошибка:
							           |%2';
							           |en = 'An error occurred when writing object ""%1"": 
							           |%2'"),
						...
						Исключение
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'При записи движений объекта ""%1"" в ""%2"" возникла ошибка:
								           |%3';
								           |en = 'An error occurred when writing ""%1"" object movements in ""%2"": 
								           |%3'"),
								ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
								...
								Исключение
									...
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'При записи по регистратору ""%1"" в ""%2"" возникла ошибка:
											           |%3';
											           |en = 'An error occurred in ""%2"" when writing by recorder ""%1"": 
											           |%3'"),
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'При удалении записи ""%1"" возникла ошибка:
							           |%2';
							           |en = 'An error occurred when deleting record ""%1"":
							           |%2'"),
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'При добавлении записи ""%1"" возникла ошибка:
							           |%2';
							           |en = 'An error occurred when adding entry ""%1"":
							           |%2'"),
							ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(СтруктураИзмерений)),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
			...
			Иначе
				СообщитьОбОшибке(НСтр("ru = 'Значения не заменяются в данных типа';
										|en = 'Values are not changed in the type data'")
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ru = 'Идентификаторы объектов расширений.Замена идентификатора';
				|en = 'Extension object IDs.Change ID'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Идентификаторы объектов метаданных.Замена идентификатора';
				|en = 'Metadata object IDs.Change ID'",
				ОбщегоНазначения.КодОсновногоЯзыка())),
