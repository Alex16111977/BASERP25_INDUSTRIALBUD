	...
	Если Отказ Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Курс валюты можно связать только с курсом независимой валюты.';
				|en = 'Exchange rates can be linked to the rate of independent currency only.'"));
	КонецЕсли;
	...
	Если Не ЭтоНовый()
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Валюта не может быть подчиненной, так как она является основной для других валют.';
				|en = 'The currency cannot be subordinate as it is the main one for other currencies.'"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ВыполняетсяФоновыйПересчетКурсов() Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось записать валюту, так как еще не завершился фоновый пересчет курсов.
			...
			|en = 'The currency could not be recorded, because the background exchange rate calculation is not completed yet.
