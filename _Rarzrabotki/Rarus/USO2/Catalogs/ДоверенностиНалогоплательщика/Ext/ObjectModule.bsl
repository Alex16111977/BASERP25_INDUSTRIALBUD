...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Владелец) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбрана организация налогоплательщика';
																|en = 'Не выбрана организация налогоплательщика'"), ЭтотОбъект, "Владелец", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДатаВыдачи) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указана дата выдачи доверенности';
																|en = 'Не указана дата выдачи доверенности'"), ЭтотОбъект, "ДатаВыдачи", , Отказ);
	КонецЕсли;
	
	Если ПолномочияПредставителя.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнены сведения о полномочиях представителя';
																|en = 'Не заполнены сведения о полномочиях представителя'"), ЭтотОбъект, "ПолномочияПредставителя", , Отказ);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПредставительЯвляетсяЮЛ Тогда
		...
		Если ЗначениеЗаполнено(ПредставительЮЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПредставительЮЛ_ИНН, Ложь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН организации-представителя';
																	|en = 'Некорректно задан ИНН организации-представителя'"), ЭтотОбъект, "ПредставительЮЛ_ИНН", , Отказ);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПредставительЮЛ_КПП) И НЕ РегламентированнаяОтчетностьКлиентСервер.КППСоответствуетТребованиям(ПредставительЮЛ_КПП) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан КПП организации-представителя';
																	|en = 'Некорректно задан КПП организации-представителя'"), ЭтотОбъект, "ПредставительЮЛ_КПП", , Отказ);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПредставительЮЛ_ОГРН) И СтрДлина(СокрЛП(ПредставительЮЛ_ОГРН)) <> 13 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ОГРН организации-представителя';
																	|en = 'Некорректно задан ОГРН организации-представителя'"), ЭтотОбъект, "ПредставительЮЛ_ОГРН", , Отказ);
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ПредставительЯвляетсяСотрудником Тогда
			...
			Если НЕ ЗначениеЗаполнено(ПредставительФЛ_ИНН) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У представителя, являющегося физлицом и не являющегося сотрудником организации должен быть задан ИНН';
																		|en = 'У представителя, являющегося физлицом и не являющегося сотрудником организации должен быть задан ИНН'"), ЭтотОбъект, "ПредставительФЛ_ИНН", , Отказ);
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставительФЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ПредставительФЛ_ИНН, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН физлица-представителя';
																|en = 'Некорректно задан ИНН физлица-представителя'"), ЭтотОбъект, "ПредставительФЛ_ИНН", , Отказ);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставительФЛ_ОГРН) И СтрДлина(СокрЛП(ПредставительФЛ_ОГРН)) <> 15 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ОГРН физлица-представителя';
																|en = 'Некорректно задан ОГРН физлица-представителя'"), ЭтотОбъект, "ПредставительФЛ_ОГРН", , Отказ);
	КонецЕсли;
	...
	Если ПризнакДоверителя = 2 ИЛИ ПризнакДоверителя = 3 Тогда
		...
		Если ПризнакДоверителя = 3 И ДоверительЯвляетсяЮЛ Тогда // доверитель - уполномоченный представитель
			...
			Если ЗначениеЗаполнено(ДоверительЮЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ДоверительЮЛ_ИНН, Ложь) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН доверителя-юрлица';
																		|en = 'Некорректно задан ИНН доверителя-юрлица'"), ЭтотОбъект, "ДоверительЮЛ_ИНН", , Отказ);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДоверительЮЛ_КПП) И НЕ РегламентированнаяОтчетностьКлиентСервер.КППСоответствуетТребованиям(ДоверительЮЛ_КПП) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан КПП доверителя-юрлица';
																		|en = 'Некорректно задан КПП доверителя-юрлица'"), ЭтотОбъект, "ДоверительЮЛ_КПП", , Отказ);
			КонецЕсли;
			
			// проверка ОГРН юрлица-доверителя
			Если ЗначениеЗаполнено(ДоверительЮЛ_ОГРН) И СтрДлина(СокрЛП(ДоверительЮЛ_ОГРН)) <> 13 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ОГРН доверителя-юрлица';
																		|en = 'Некорректно задан ОГРН доверителя-юрлица'"), ЭтотОбъект, "ДоверительЮЛ_ОГРН", , Отказ);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДоверительРук_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ДоверительРук_ИНН, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН руководителя организации-доверителя';
																		|en = 'Некорректно задан ИНН руководителя организации-доверителя'"), ЭтотОбъект, "ДоверительРук_ИНН", , Отказ);
			КонецЕсли;
		...
		Если ПризнакДоверителя = 2 ИЛИ (ПризнакДоверителя = 3 И ДоверительИмеетУЛ) Тогда
			...
			Если ЗначениеЗаполнено(ДоверительФЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(ДоверительФЛ_ИНН, Истина) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН физлица-доверителя';
																		|en = 'Некорректно задан ИНН физлица-доверителя'"), ЭтотОбъект, "ДоверительФЛ_ИНН", , Отказ);
			КонецЕсли;
			
			// проверка ОГРН физлица-доверителя
			Если ЗначениеЗаполнено(ДоверительФЛ_ОГРН) И СтрДлина(СокрЛП(ДоверительФЛ_ОГРН)) <> 15 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ОГРНИП физлица-доверителя';
																		|en = 'Некорректно задан ОГРНИП физлица-доверителя'"), ЭтотОбъект, "ДоверительФЛ_ОГРН", , Отказ);
			КонецЕсли;
	...
	Если ЗаверенаНотариально Тогда
		...
		Если НотариусЯвляетсяЮЛ Тогда
			...
			Если ЗначениеЗаполнено(НотариусЮЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(НотариусЮЛ_ИНН, Ложь) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН юрлица нотариуса';
																		|en = 'Некорректно задан ИНН юрлица нотариуса'"), ЭтотОбъект, "НотариусЮЛ_ИНН", , Отказ);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(НотариусЮЛ_КПП) И НЕ РегламентированнаяОтчетностьКлиентСервер.КППСоответствуетТребованиям(НотариусЮЛ_КПП) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан КПП юрлица нотариуса';
																		|en = 'Некорректно задан КПП юрлица нотариуса'"), ЭтотОбъект, "НотариусЮЛ_КПП", , Отказ);
			КонецЕсли;
			
			// проверка ОГРН юрлица-нотариуса
			Если ЗначениеЗаполнено(НотариусЮЛ_ОГРН) И СтрДлина(СокрЛП(НотариусЮЛ_ОГРН)) <> 13 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ОГРН юрлица нотариуса';
																		|en = 'Некорректно задан ОГРН юрлица нотариуса'"), ЭтотОбъект, "НотариусЮЛ_ОГРН", , Отказ);
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(НотариусФЛ_ИНН) И НЕ РегламентированнаяОтчетностьВызовСервера.ИННСоответствуетТребованиямНаСервере(НотариусФЛ_ИНН, Истина) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно задан ИНН нотариуса-физлица';
																	|en = 'Некорректно задан ИНН нотариуса-физлица'"), ЭтотОбъект, "НотариусФЛ_ИНН", , Отказ);
		КонецЕсли;
