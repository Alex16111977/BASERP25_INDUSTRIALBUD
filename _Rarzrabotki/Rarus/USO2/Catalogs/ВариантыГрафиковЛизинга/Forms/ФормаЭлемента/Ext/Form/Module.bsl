	...
	Иначе
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьГрафикПослеВопроса", ЭтотОбъект, Новый Структура("Команда", Команда)),
			НСтр("ru = 'Перед загрузкой необходимо записать объект.
				...
				|en = 'Write the object before importing.
				|Write and continue?'"),
...
Процедура НастроитьФормуПоДоговору()
	...
	Элементы.ГрафикОплатУслугаПоЛизингу.Заголовок                   = СтрШаблон(НСтр("ru = 'Сумма лизингового платежа (%1)';
																					|en = 'Lease payment amount (%1)'"), СтрокаВалюта);
	Элементы.ГрафикОплатЗачетОбеспечительногоПлатежа.Заголовок      = СтрШаблон(НСтр("ru = 'Сумма обеспечительного платежа (%1)';
																					|en = 'Security payment amount (%1)'"), СтрокаВалюта);
	Элементы.ГрафикОплатВыкупПредметаЛизинга.Заголовок              = СтрШаблон(НСтр("ru = 'Сумма выкупа предмета лизинга (%1)';
																					|en = 'Leasing item redemption amount (%1)'"), СтрокаВалюта);
	Элементы.ГрафикНачисленийУслугаПоЛизингу.Заголовок              = СтрШаблон(НСтр("ru = 'Сумма услуг по лизингу (%1)';
																					|en = 'Leasing services amount (%1)'"), СтрокаВалюта);
	Элементы.ГрафикНачисленийЗачетОбеспечительногоПлатежа.Заголовок = СтрШаблон(НСтр("ru = 'Сумма зачета обеспечительного платежа (%1)';
																					|en = 'Security payment offset amount (%1)'"), СтрокаВалюта);
	Элементы.ГрафикНачисленийВыкупПредметаЛизинга.Заголовок         = СтрШаблон(НСтр("ru = 'Сумма выкупа предмета лизинга (%1)';
																					|en = 'Leasing item redemption amount (%1)'"), СтрокаВалюта);
...
Функция СуммыГрафиковСходятся()
	...
	Если Не СуммыУслугПоЛизингуРавны Тогда
		Текст = НСтр("ru = 'Различаются итоговые суммы услуг по лизингу на закладках ""Оплаты"" и ""Начисления"".';
					|en = 'Total leasing services amounts are different on the Payments and Accruals tabs.'");
	...
	Если Не СуммыОбеспечительногоПлатежаРавны Тогда
		Текст = НСтр("ru = 'Различаются итоговые суммы зачета обеспечительного платежа на закладках ""Оплаты"" и ""Начисления"".';
					|en = 'Total security payment offset amounts are different on the Payments and Accruals tabs.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
	...
	Если Не СуммыВыкупаПредметаЛизингаРавны Тогда
		Текст = НСтр("ru = 'Различаются итоговые суммы выкупа предмета лизинга на закладках ""Оплаты"" и ""Начисления"".';
					|en = 'Total leasing item redemption amounts are different on the Payments and Accruals tabs.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);
	КонецЕсли;
...
Процедура ЗаписатьГрафик(ЗакрыватьПослеЗаписи = Ложь)
	...
	Если Не СуммыГрафиковСходятся() Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработкаОтветаНаВопросИгнорировать", ЭтотОбъект, ЗакрыватьПослеЗаписи),
			...
			|Игнорировать?';
			|en = 'Total amounts are different in the current schedule option.
			|Ignore?'"),
			РежимДиалогаВопрос.ДаНет);
	Иначе
