	...
	Если ДанныеОсновнойСпецификации = Неопределено Тогда
		...
		Если ДанныеПоНоменклатуре.Свойство("Спецификация") И ЗначениеЗаполнено(ДанныеПоНоменклатуре.Спецификация) Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка при чтении данных ресурсной спецификации: %Спецификация%.';
									|en = 'An error occurred when reading data of the %Спецификация% bill of resources.'");
		...
		Иначе
			ТекстИсключения = НСтр("ru = 'Ошибка при чтении данных ресурсной спецификации: не передана спецификация.';
									|en = 'An error occurred when reading BOR data: the BOM is not transferred.'");
		КонецЕсли;
...
Функция ОписаниеИсточниковДанныхОперандов(Объект = Неопределено) Экспорт
	...
	// [Основное изделие]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'ОсновноеИзделие';
																						|en = 'ОсновноеИзделие'"), НСтр("ru = 'Основное изделие';
																														|en = 'Main product'"));
	...
	Если Объект = Неопределено
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'ВыходныеИзделия';
																							|en = 'ВыходныеИзделия'"), НСтр("ru = 'Выходные изделия';
	...
	// [МатериалыИУслуги]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'МатериалыИУслуги';
																						|en = 'МатериалыИУслуги'"), НСтр("ru = 'Материалы и работы';
	...
	// [Материал]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'Материал';
																						|en = 'Material'"), НСтр("ru = 'Материал (текущая строка)';
	...
	// [Подразделение]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'Подразделение';
																						|en = 'Department'"), НСтр("ru = 'Подразделение';
																														|en = 'Department'"));
	...
	Если НастройкиПодсистемыПроизводство.ИспользуетсяПроизводство22 И Не НастройкиПодсистемыПроизводство.ИспользуетсяПроизводство21 Тогда
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'Распоряжение';
																							|en = 'Reference'"), НСтр("ru = 'Заказ на производство';
																														|en = 'Production order'"));
			...
			Если Строка.Спецификация.Пустая() Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не назначена основная спецификация для материала в строке %1 (спецификация ""%2"")';
						|en = 'Main BOM is not specified for material in line %1 (the ""%2"" BOM)'"),
					Строка.НомерСтроки,
	...
	Для ИндексТекущихДанных = 0 По МассивДанных.ВГраница() Цикл
		...
		Если ТекущиеДанные = Неопределено Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не удалось получить данные спецификации по номенклатуре: %Ссылка%';
									|en = 'Cannot receive BOM data for products: %Ссылка%'");
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Получение данных спецификации';
				|en = 'Receiving BOM data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
