...
Процедура ОткрытьЧерезБраузер(Команда)
	...
	Иначе 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Идентификатор документа не заполнен!';
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.Продукция.СписокВыбора.Добавить(Справочники.ПродукцияВЕТИС.ПустаяСсылка(), НСтр("ru = '<Произвольная>';
																							|en = '<Произвольная>'"));
	Элементы.БлагополучиеМестности.СписокВыбора.Добавить("ru='Местность благополучна по заразным болезням животных'");
