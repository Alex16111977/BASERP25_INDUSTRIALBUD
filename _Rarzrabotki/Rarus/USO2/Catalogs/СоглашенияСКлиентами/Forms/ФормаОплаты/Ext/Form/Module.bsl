...
Функция ОплатаКорректна()
	...
	Если Объект.ДоступноВнешнимПользователям И Объект.ЭтапыГрафикаОплаты.Количество() = 0 Тогда
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Соглашение доступно внешним пользователям. Необходимо заполнить список ""Этапы графика оплаты""';
								|en = 'Agreement is available for external users. Populate list ""Payment schedule steps""'");
		...
		Если Объект.ТребуетсяЗалогЗаТару Тогда
			Если Не ЗначениеЗаполнено(ТекСтрока.ПроцентПлатежа)
				...
				ТекстОшибки = НСтр("ru = 'Для этапа должен быть указан процент платежа или процент залога за тару';
									|en = 'Percentage of payment and package deposit should be specified for the step'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки + АдресОшибки,
					,
	...
	Если КоличествоЭтапов > 0 Тогда
		...
		Если Объект.ТребуетсяЗалогЗаТару
			...
			ТекстОшибки = НСтр("ru = 'Процент залога за тару по всем этапам должен быть равен 100%';
								|en = 'Percentage of the package deposit at all stages should be equal to 100%'");
			Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
			...
			Для Каждого ЭтапГрафикаОплаты Из Объект.ЭтапыГрафикаОплаты Цикл
				...
				Если ЭтапГрафикаОплаты.ВариантОплаты <> Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки Тогда
					...
					ТекстОшибки = НСтр("ru = 'Вариант оплаты ""%ВариантОплаты%"" нельзя использовать';
										|en = 'Payment option ""%ВариантОплаты%"" cannot be used'") + Символы.ПС;
	...
	Если РежимУчетаОтсрочки = 1 И Не ЗначениеЗаполнено(Объект.Календарь) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не указан календарь для учета отсрочки по рабочим дням.';
							|en = 'Calendar for delay accounting by work days is not specified.'");
...
Процедура ПроверитьЭтапыГрафикаОплаты(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПредыдущееЗначениеСдвигаОплаты > Выборка.ЗначениеСдвигаОплаты Тогда
			...
			|en = 'Delay in the %ИндексТекущегоЭтапа% line 
			|should not be less than in the %ИндексПредыдущегоЭтапа% line'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ИндексТекущегоЭтапа%",    Выборка.НомерСтроки);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ИндексПредыдущегоЭтапа%", Выборка.НомерСтроки-1);
