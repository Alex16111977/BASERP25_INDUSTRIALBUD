...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НеЗаполненИсточникСуммыОперации Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Поле Источник суммы не заполнено';
					|en = 'The ""Amount source"" field is not filled in'"), ,"ИсточникСуммыОперации", "Запись", Отказ);
	КонецЕсли;
	Если НеЗаполненТипИтога Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Поле Тип итога не заполнено';
					|en = 'The ""Total type"" field is not filled in'"), ,"ТипИтога", "Запись", Отказ);
	КонецЕсли;
