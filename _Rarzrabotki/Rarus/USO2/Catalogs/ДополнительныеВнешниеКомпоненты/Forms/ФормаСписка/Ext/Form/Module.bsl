...
Процедура СоздатьЭлементСправочникаВК(Адрес)
	...
	Если ЗначениеЗаполнено(НазваниеВИБ) Тогда
		...
		ТекстВопроса = НСтр("ru = 'В информационной базе уже есть внешняя компонента %1 (Версия %2).
						...
						|en = 'There is the %1 add-in in the infobase (Version %2).
						|There is the %3 add-in in the selected file (Version %4).
						|Import an external module from the file and replace the existing module?'");
