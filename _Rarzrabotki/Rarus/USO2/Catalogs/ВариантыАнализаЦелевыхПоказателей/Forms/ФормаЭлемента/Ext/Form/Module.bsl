	...
	Если Элементы.ПериодСравнения.АвтоОтметкаНезаполненного
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Период сравнения"" не заполнено';
				|en = 'The ""Comparison period"" field is not filled in'"), , 
			"ПериодСравнения", ,
			Отказ);
	КонецЕсли;
	...
	Если Элементы.ОбъектАнализа.АвтоОтметкаНезаполненного
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Объект анализа"" не заполнено';
				|en = 'The ""Analysis object"" field is not filled in'"), , 
			"ОбъектАнализа", ,
			Отказ);
	КонецЕсли;
	...
	Если Элементы.ЗначениеАнализа.АвтоОтметкаНезаполненного
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Значение"" не заполнено';
				|en = 'The Value field is not filled in'"), , 
			"ЗначениеАнализа", ,
			Отказ);
		КонецЕсли;
	...
	Если Элементы.ЗначениеАнализаДополнительное.АвтоОтметкаНезаполненного
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Доп. значение"" не заполнено';
				|en = 'The ""Additional value"" field is not filled in'"), , 
...
Процедура ОтветственныйПриИзменении(Элемент)
	...
	Если Не Объект.Ответственный.Пустая() Тогда
		Если Объект.НастройкиДоступности.НайтиСтроки(ОтборПоПользователю).Количество() = 0 Тогда
			СтрокаВопроса = Формат(НСтр("ru = 'Сделать доступным вариант анализа для';
										|en = 'Make analysis option available for'")) + " " + Объект.Ответственный + "?"; 
...
Процедура ЗаполнитьНаименованиеПоШаблону(Команда)
	...
	Объект.Наименование = Строка(Элементы.ЗначениеАнализа.ТекстРедактирования) + ?(ПараметрВалютаРасчета = Неопределено, "", " " + "(" + ПараметрВалютаРасчета.Значение + ")")
						  + ?(ОбъектАнализаДоступен, " " + НСтр("ru = 'по';
																|en = 'to'")+ " " + Строка(Элементы.ОбъектАнализа.ТекстРедактирования), "")
...
Процедура НастроитьОтборы(Команда)
	...
	ЗаголовокФормыНастройкиСхемыКомпоновкиДанных = НСтр("ru = 'Настройка отборов и упорядочивания для варианта анализа ""%1""';
														|en = 'Set up filters and arrangement for the ""%1"" analysis option '");
	...
	Если ПроверитьЗаполнение() Тогда
		...
		Если СтруктураНастроекДемоДанных.НастройкиТребующиеОбновления.ТребуетсяОбновление Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ПоказатьВопросЗавершение", ЭтаФорма, СтруктураНастроекДемоДанных), 
				...
				|en = 'Settings are changed since sample data has been saved. 
				|Continue with data loss?'"), РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура УстановитьНадписьЗнаковПослеЗапятой(ЗнаковПослеЗапятой, Элементы)
	...
	ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
		ЗнаковПослеЗапятой, 
		НСтр("ru = 'знак';
			|en = 'sign'"), НСтр("ru = 'знака';
								|en = 'sign'"), НСтр("ru = 'знаков';
													|en = 'signs'"), "м") + " " + НСтр("ru = 'после запятой';
																						|en = 'after comma'");
КонецПроцедуры
...
Процедура ИнициализироватьКомпоновщикНастроек()
	...
	Если НЕ ПараметрВалютаРасчета = Неопределено И ПараметрВалютаРасчета.Значение = Справочники.Валюты.ПустаяСсылка() Тогда
		КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек.ПользовательскиеНастройки, НСтр("ru = 'ВалютаРасчета';
																											|en = 'ВалютаРасчета'"), МониторингЦелевыхПоказателей.ПолучитьВалютуПоУмолчанию(), Истина);
	КонецЕсли;
КонецПроцедуры
