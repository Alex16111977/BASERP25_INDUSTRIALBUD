		...
		ИначеЕсли Выборка.Количество > 1 Тогда
			Если СтрДлина(Выборка.НоменклатураНаименование) > 45 Тогда 
				СтрокаКоличество = СтрШаблон(НСтр("ru = '( + еще %1)';
													|en = '( + %1 more)'"), Выборка.Количество - 1);
			...
			Иначе
				СтрокаСписка.Данные["Сопоставлено"] = СтрШаблон(НСтр("ru = '%1 ( + еще %2...)';
																	|en = '%1 ( + more %2...)'"), Выборка.НоменклатураНаименование, Выборка.Количество - 1);
			КонецЕсли;
			СтрокаСписка.Оформление["Сопоставлено"].УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветГиперссылкиГосИС);
		Иначе
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'По коду акцизной марки %1 не удалось определить код алкогольной продукции';
						|en = 'Cannot define the alcoholic product code by excise stamp code %1'"),
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Марке по штрихкоду %1 не сопоставлена алкогольная продукция';
						|en = 'Brand by barcode %1 is not mapped with alcoholic products'"),
					ДанныеШтрихкода.Штрихкод));
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'По штрихкоду %1 не удалось найти справку 2';
						|en = 'Cannot find certificate 2 by barcode %1'"),
					ДанныеШтрихкода.Штрихкод));
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'По штрихкоду %1 не удалось найти алкогольную продукцию';
						|en = 'Cannot find alcoholic products by the %1 barcode'"),
					ДанныеШтрихкода.Штрихкод));
		КонецЕсли;
