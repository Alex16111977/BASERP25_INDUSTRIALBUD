...
Процедура ПодготовитьФорму()
	...
	Если Объект.СтатусПодключения = Перечисления.СтатусыУчастниковОбменаЭД.Присоединен
		И Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		Элементы.ГруппаКонтрагент.ЦветФона = ЦветаСтиля.ЦветФонаНекорректногоКонтрагентаВДокументе;
	КонецЕсли;
...
Процедура КонтрагентПриИзмененииНаСервере(МенятьИдентификаторКонтрагента = Истина)
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		Элементы.ГруппаКонтрагент.ЦветФона = ЦветаСтиля.ЦветФонаФормы;
	КонецЕсли;
