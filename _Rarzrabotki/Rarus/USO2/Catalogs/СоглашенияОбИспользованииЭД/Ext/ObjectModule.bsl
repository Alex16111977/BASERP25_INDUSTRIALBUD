...
Процедура ПередЗаписью(Отказ)
	...
	Если ЕстьНастройкаДляОтправки() И ИспользуетсяДляОтправки Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для данного сочетания организации, контрагента, договора уже есть настройка для отправки. Операция отменена.';
								|en = 'For this combination of organization, counterparty, or contract, there is already a setting to send. The operation is canceled.'");
	...
	Если НЕ ПометкаУдаления Тогда
		...
		Если Результат.Следующий() Тогда
			ШаблонСообщения = НСтр("ru = 'Операция отменена.
				...
				|EDI configuration profile marked for deletion is used in the current EDI configuration:
				|%1'");
...
Функция НастройкаЭДОУникальна() Экспорт
	...
	|	И НастройкиЭДО.ИдентификаторКонтрагента = &ИдентификаторКонтрагента
	|	И НастройкиЭДО.ИдентификаторОрганизации = &ИдентификаторОрганизации";
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ШаблонСообщения = НСтр("ru = 'В информационной базе уже существует настройка ЭДО со статусом подключения ""Присоединен""
										...
										|en = 'EDI setting with connection status ""Connected"" 
										|between counterparty %1 and company %2 already exists in the infobase'");
...
Функция ЕстьНастройкаДляОтправки() Экспорт
	...
	|	И Соглашения.СтатусПодключения <> Значение(Перечисление.СтатусыУчастниковОбменаЭД.Отсоединен)";
