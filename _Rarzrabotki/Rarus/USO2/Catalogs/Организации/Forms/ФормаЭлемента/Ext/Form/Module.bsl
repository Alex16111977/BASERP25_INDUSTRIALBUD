...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе ВызватьИсключение НСтр("ru = 'Невозможно определить вид организации. Обратитесь к администратору.';
								|en = 'Cannot determine company kind. Contact your administrator.'");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(УчетнаяПолитикаОрганизаций.УчетнаяПолитика) И Не Объект.ОбособленноеПодразделение Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Учетная политика"" не заполнено.';
				|en = 'The ""Accounting policy"" field is not populated.'"), ,
...
Процедура ОбменФССНажатие(Элемент, СтандартнаяОбработка)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Документооборот с ФСС';
										|en = 'Data interchange with SSF'"), Новый ОписаниеОповещения("ОбменФССНажатиеОбъектЗаписан", ЭтотОбъект));
...
Процедура ОбменРАРНажатие(Элемент, СтандартнаяОбработка)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Документооборот с Росалкогольрегулированием';
										|en = 'Data interchange with Federal Service for Alcohol Market Regulation'"), Новый ОписаниеОповещения("ОбменРАРНажатиеОбъектЗаписан", ЭтотОбъект));
...
Процедура ПрефиксПриИзменении(Элемент)
	...
	Если Найти(Объект.Префикс, "-") > 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Нельзя в префиксе организации использовать символ ""-"".';
													|en = 'Company prefix contains invalid character ""-"".'"));
...
Процедура ОткрытьПараметрыПФР(Команда)
	...
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Пенсионный фонд';
										|en = 'Pension fund'"), Новый ОписаниеОповещения("ОткрытьПараметрыПФРОбъектЗаписан", ЭтотОбъект));
	//-- НЕ УТ
...
Процедура ОткрытьПараметрыФСС(Команда)
	...
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Фонд социального страхования';
										|en = 'Social Security Fund'"), Новый ОписаниеОповещения("ОткрытьПараметрыФССОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьПараметрыТФОМС(Команда)
	...
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Фонд обязательного медицинского состояния';
										|en = 'Compulsory medical condition fund'"), Новый ОписаниеОповещения("ОткрытьПараметрыТФОМСОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьПараметрыКлассификаторов(Команда)
	...
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Классификаторы';
										|en = 'Classifiers'"), Новый ОписаниеОповещения("ОткрытьПараметрыКлассификаторовОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьПараметрыРасчетаЗарплаты(Команда)
	...
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Территориальные условия и график работы';
										|en = 'Site conditions and work schedule'"), Новый ОписаниеОповещения("ОткрытьПараметрыРасчетаЗарплатыОбъектЗаписан" ,ЭтотОбъект));
...
Процедура ОткрытьПараметрыМестаРегистрации(Команда)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Иностранная организация';
										|en = 'Foreign company'"), Новый ОписаниеОповещения("ОткрытьПараметрыИностраннойОрганизацииОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьКадровыйУчетИРасчетЗарплаты(Команда)
	...
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Параметры кадрового учета и расчета зарплаты';
										|en = 'Parameters of HR recordkeeping and payroll'"), Новый ОписаниеОповещения("ОткрытьКадровыйУчетИРасчетЗарплатыОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьБухучетИВыплатуЗарплаты(Команда)
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Бухучет и выплата зарплаты';
										|en = 'Accounting and salary payment'"), Новый ОписаниеОповещения("ОткрытьБухучетИВыплатуЗарплатыОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьВоинскийУчет(Команда)
	//++ НЕ УТ
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Воинский учет';
										|en = 'Military registration'"), Новый ОписаниеОповещения("ОткрытьВоинскийУчетОбъектЗаписан", ЭтотОбъект));
...
Процедура ОткрытьРегистрациюВНалоговыхОрганах(Команда)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Налоговые органы';
										|en = 'Tax authorities'"), Новый ОписаниеОповещения("ОткрытьРегистрациюВНалоговыхОрганахОбъектЗаписан", ЭтотОбъект));
...
Процедура ДобавитьИзображениеЛоготипа(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Для выбора изображения необходимо записать объект. Записать?';
							|en = 'To select an image, write the object. Write?'");
		Ответ = Неопределено;
...
Процедура ПредварительныйПросмотрСчетаНаОплату(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) ИЛИ Модифицированность Тогда
		...
		|You can preview the proforma invoice only after the data is recorded.
...
Процедура ПредварительныйПросмотрСчетаНаОплатуФрагмент()
    ...
    СтруктураДополнительныхПараметров = Новый Структура("ЗаголовокФормы", НСтр("ru = 'Предварительный просмотр печатной формы ""Счет на оплату"".';
																				|en = 'Preview of ""Proforma invoice"" print form.'"));
...
Процедура УстановитьПорядокУплатыНалоговНаМестах(Команда)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Порядку уплаты авансов по налогам';
										|en = 'Tax advance payments'"), Новый ОписаниеОповещения("ОткрытьПорядокУплатыАвансовПоНалогам", ЭтотОбъект));
...
Процедура УправлениеФормойНаСервере()
	...
	ИначеЕсли ВидОрганизации = "ОбособленноеПодразделение" Тогда
		...
		Элементы.Наименование.ПодсказкаВвода = НСтр("ru = 'например, Филиал ТД (Тамбов)';
													|en = 'for example, Branch trading house (Tambov)'");
		Элементы.НаименованиеСокращенное.ПодсказкаВвода = НСтр("ru = 'например, ТД Филиал (Тамбов)';
																|en = 'for example, trading house Branch (Tambov)'");
...
Процедура ЗаполнитьОписаниеИностраннойОрганизации()
	...
	РегистрацияВОЭЗ = ?(Объект.ЗарегистрированВОЭЗ, НСтр("ru = 'Зарегистрирована';
...
Процедура ЗаполнитьОписаниеКадровогоУчетаИРасчетаЗарплаты()
	...
	СтрокаОписания = СтрЗаменить(
				...
				НСтр("ru = '%ИспользуетсяТрудФармацевтов% %ИспользуетсяТрудЧленовЛетныхЭкипажей% %ИспользуетсяТрудЧленовЭкипажейМорскихСудов% %ИспользуетсяТрудШахтеров% %ИспользуютсяРаботыСДосрочнойПенсией% %ПродолжениеСтроки%';
					|en = '%ИспользуетсяТрудФармацевтов% %ИспользуетсяТрудЧленовЛетныхЭкипажей% %ИспользуетсяТрудЧленовЭкипажейМорскихСудов% %ИспользуетсяТрудШахтеров% %ИспользуютсяРаботыСДосрочнойПенсией% %ПродолжениеСтроки%'"));
	...
	СтрокаОписания = СтрЗаменить(
				...
				НСтр("ru = 'Контролировать права сотрудника на отпуск в кадровых документах: %КонтрольОтпускаСотрудникаВКадровыхДокументах%.';
					|en = 'Control employee leave entitlement in human resources documents: %КонтрольОтпускаСотрудникаВКадровыхДокументах%.'"));
...
Процедура ПроверитьЧтоЭтоНеНовыйЭлемент(ИмяФормы, ОповещениеПослеЗаписи, ТекстВопроса = Неопределено)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ТекстВопроса = Неопределено Тогда
			...
			|You can go to ""%ИмяФормы%"" only after writing the data.
...
Процедура ВыбратьДатуПараметровИмущественныхНалогов()
	...
	ТекстСообщения = НСтр("ru = 'Дата, с которой должны вступить в силу изменения по установленной ставке и льготам по налогу на имущество.';
							|en = 'Date from which the changes of the set rate and the property tax privileges should be applied.'");
	ПараметрыФормы.Вставить("ПоясняющийТекст", ТекстСообщения);
...
Процедура ЗапроситьРежимИзмененияУчетнойПолитики(ДатаИзменения, Отказ, ДополнительныеПараметры = Неопределено)
	...
	ТекстКнопкиДа = НСтр("ru = 'Изменилась учетная политика';
						|en = 'Accounting policy is changed'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'При редактировании изменилась учетная политика для организации.
					...
					|en = 'Accounting policy of the company was changed during editing.
					|If you changed previous data (it was incorrect), click ""Error corrected"". 
					|If accounting policy was changed from %1, click ""Accounting policy changed""'"), 
...
Процедура СтавкаНалогаНаИмуществоДатаНачалоВыбораЗавершение(ВыбранныйПериод, ДополнительныеПараметры) Экспорт 
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Ссылка)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				...
				|en = 'Date is not selected.
				|Initial values are set for property tax rate settings.'"));
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			...
			|en = 'Date is not selected.
			|The current month start date is set for property tax rate settings.'"));
...
Процедура ОбменРПННажатие(Элемент, СтандартнаяОбработка)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Документооборот с РПН';
										|en = 'Data interchange with RPN'"), Новый ОписаниеОповещения("ОбменРПННажатиеОбъектЗаписан", ЭтотОбъект));
...
Процедура ОбменФТСНажатие(Элемент, СтандартнаяОбработка)
	...
	ПроверитьЧтоЭтоНеНовыйЭлемент(НСтр("ru = 'Документооборот с ФТС';
										|en = 'Data interchange with FCS'"), Новый ОписаниеОповещения("ОбменФТСНажатиеОбъектЗаписан", ЭтотОбъект));
