...
Процедура ПроверитьДублиСтрокСтатейБюджетовПоАналитике(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого КлючиИЗначение Из СоответствиеСтатей Цикл
		ТекстОшибки = НСтр("ru = 'Статья ""%1"" встречается внутри вида бюджета несколько раз с одинаковой аналитикой';
							|en = 'The ""%1"" item is found in the budget profile several times with the same dimension'");
...
Процедура ПроверитьОтсутствиеИзмеренийАналитикВРежимеВвода(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		ТекстОшибки = НСтр("ru = 'Обнаружен недоступный для ввода элемент ""%1""';
							|en = 'Item ""%1"" unavailable for entering is detected'");
	...
	Для Каждого СтрокаЭлементаРодителя Из ТаблицаРодителей Цикл
		...
		Пока ТаблицаПроверки.Количество() И ТаблицаПроверки[0].Флаг > 1 Цикл
			Сообщение = НСтр("ru = 'Фиксированное значение ""%1"" не должно повторятся более одного раза';
							|en = 'Fixed value ""%1"" should not be repeated more than once'");
		...
		Если НеверныйПорядокПериодов(МассивВышестоящихПериодовКолонки)
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "",
								НСтр("ru = 'Больший период не может входить в меньший!';
									|en = 'Larger period cannot include a smaller one.'"), "");
			Прервать;
		...
		Если ЕстьНеделяВДекадах(МассивВышестоящихПериодовСтроки)
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "",
									НСтр("ru = 'Неделя не может входить в декаду!';
										|en = 'Week cannot be included in the ten-day period.'"), "");
			Возврат;
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Если Не ВходитВПроизводныйПоказатель Тогда
			...
			ТекстОшибки = НСтр("ru = 'Обнаружен недоступный для ввода элемент ""Все показатели""';
								|en = 'The ""All indicators"" item unavailable for entering is detected'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,, ТекстОшибки, "");
			Прервать;
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Если Не ВходитВПроизводныйПоказатель Тогда
			...
			ТекстОшибки = НСтр("ru = 'Обнаружен недоступный для ввода элемент ""Все статьи""';
								|en = 'The ""All items"" item unavailable for entering is detected'");
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", ТекстОшибки, "");
		...
		Если СталоСтрок = ВсегоСтрок Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "",
				НСтр("ru = 'Обнаружены циклические ссылки в формулах!';
					|en = 'Circular references are detected in formulas.'"), "");
	...
	Если Не ИсточникиДанных.Количество() Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", 
			НСтр("ru = 'Не выбраны статьи бюджетов или показатели бюджетов в структуре вида бюджета';
				|en = 'Budget items or budget indicators are not selected in the budget profile structure'"), "");
	КонецЕсли;
		...
		Если Не ЕстьИсточникДокумент Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", 
				НСтр("ru = 'Для ввода данных должны быть выбраны статьи бюджета';
					|en = 'To enter data, select budget items'"), "");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого КлючиИЗначение Из Структура Цикл
		Для Каждого СтрокаОшибки Из КлючиИЗначение.Значение Цикл
			ТекстОшибки = НСтр("ru = 'Для %1
								...
								|en = 'The ""%4"" dimension not available on %3 
								|is specified for %1 ""%2""'");
	...
	Для Каждого КлючиИЗначение Из Структура Цикл
		Для Каждого СтрокаОшибки Из КлючиИЗначение.Значение Цикл
			ТекстОшибки = НСтр("ru = 'Для %1 ""%2"" недоступен учет по валюте';
								|en = 'Accounting by currency is not available for ""%2"" %1'");
		...
		Для Каждого СтатьяБюджета Из НайденныеСтроки Цикл
			Если СтатьяБюджета.Строки.Количество() Тогда 
				...
				ТекстОшибки = НСтр("ru = 'Ошибка в ячейке [%1; %2]: запрещен ввод на уровне группировок';
									|en = 'Error in the cell [%1; %2]: entering on the grouping level is prohibited'");
	...
	Для Каждого Периодичность Из СтруктураКолонок.Периодичности Цикл
		...
		Если ИндексОтчета > ИндексТекущей Тогда
			ТекстОшибки = НСтр("ru = 'Периодичность, выбираемая в группировках, не может быть больше, чем периодичность самого вида бюджета (%1)';
								|en = 'Frequency selected in groupings cannot be greater than the budget profile frequency (%1)'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, 
...
Процедура ОчиститьКэшВспомогательныхДанных(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось очистить кэш вспомогательных данных по причине: %Причина%';
								|en = 'Cannot clear auxiliary data cache due to: %Причина%'");
