...
Процедура Далее(Команда)
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборОрганизацииИБанка Тогда
		...
		Если Не ЗначениеЗаполнено(Банк) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите банк';
			...
			Если КомпонентаОбработка = 0 Тогда // Компонента
				Если НЕ ЗначениеЗаполнено(ИмяВнешнегоМодуля) Тогда
					ТекстСообщения = НСтр("ru = 'Загрузите внешнюю компоненту из файла.';
											|en = 'Import an add-in from a file.'");
...
Процедура ПослеПолученияНастроекОбмена(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		ДвоичныеДанныеФайлаНастроек = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
		АдресФайлаНастроекОбмена = ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайлаНастроек, УникальныйИдентификатор);
	...
	#Иначе
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось подключить указанный внешний модуль';
									|en = 'Cannot connect the specified external module'");
			ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция ЗапускЗаданияПоПолучениюИнформацииОВнешнейКомпоненте(Знач URLИнфоФайла, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Скачивание информационного файла внешней компоненты через интернет.';
															|en = 'Download an add-in information file from the Internet.'");
...
Функция УчастникиОбменаИзФайла(Знач АдресФайла, Знач Организация, Знач ЛокальныйФайл = Ложь)
	...
	Исключение
		Операция = НСтр("ru = 'Загрузка настроек обмена из файла';
						|en = 'Import exchange settings from the file'");
		...
		Иначе
			ШаблонТекстСообщения = НСтр("ru = 'Произошла ошибка при чтении файла, полученного из банка.
									|%1';
									|en = 'An error occurred when reading the file received from the bank.
									|%1'");
