...
//
Функция РезультатСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(ДеревоМаркированнойПродукции, ТаблицаШтрихкодовВерхнегоУровня, ШтрихКодыУпаковок = Неопределено) Экспорт
	...
	Если ШтрихКодыУпаковок = Неопределено Тогда
		...
		ШтрихКодыУпаковок = ШтрихкодированиеИС.ШтрихкодыУпаковок(
				ШтрихкодированиеИС.ЗначенияШтрихкодовВНаличииИзДереваМаркируемойПродукции(ДеревоМаркированнойПродукции, Неопределено));
	...
	Для Каждого СтрокаДерева Из ДеревоМаркированнойПродукции.Строки Цикл
		...
		Иначе
			Результат = РезультатСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(СтрокаДерева, ТаблицаШтрихкодовВерхнегоУровня, ШтрихкодыУпаковок);
		КонецЕсли;
		
		Если СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаТабачнойПродукцииМОТП.ПачкиБезБлока
		...
		КонецЕсли;
			...
			Иначе
				...
				ПараметрыЗаписи.Вставить("НеРассчитыватьХешСумму", Ложь);
		...
		Если ТипЗнч(ДеревоМаркированнойПродукции) = Тип("ДеревоЗначений")
			...
			НоваяСтрокаШтрихкодыВерхнегоУровня.ЗначениеШтрихкода = СтрокаДерева.ЗначениеШтрихкода;	
...
Функция СгенерироватьАкцизнуюМарку(КодАкцизнойМарки, Номенклатура, Характеристика, ЗаписьПриОбновленииИБ = ЛОЖЬ) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание новой акцизной марки';
										|en = 'Создание новой акцизной марки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция СформироватьПечатнуюФормуШтрихкодыУпаковок(МассивОбъектов, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Выборка.ЗначениеШтрихкода) Тогда
			Продолжить;
		КонецЕсли;
		...
		ТипШтрихкода = ШтрихкодированиеИС.ОпределитьТипШтрихкода(Выборка.ЗначениеШтрихкода, ТипУпаковки);
