...
Функция СформироватьПечатнуюФормуРеестрСертификатовНоменклатуры(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|		ТаблицаТоваров.Ссылка.ПередачаПоЗаказам   КАК ЕстьРаспоряжения,
	|		ТаблицаТоваров.ЗаказКлиента               КАК Распоряжение,
	|		МАКСИМУМ(ТаблицаТоваров.НомерСтроки)      КАК НомерСтроки,
	...
	|			ПО ДвиженияСерийТоваров.Документ = ДокументыУчетаСертификатов.Ссылка
	|	ГДЕ
	|		ДвиженияСерийТоваров.Документ ССЫЛКА Документ.ПередачаТоваровХранителю
	|		И ДокументыУчетаСертификатов.Ссылка ССЫЛКА Документ.ПередачаТоваровХранителю
	//-- НЕ УТ
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		ПО СтрокиТоваров.Ссылка = ДокументыПоКоторымВедетсяУчетСертификатов.Ссылка
	|ГДЕ
	|	СтрокиТоваров.Ссылка ССЫЛКА Документ.ПередачаТоваровХранителю
	|	И ЕСТЬNULL(ДвиженияСерийТоваров.Ссылка, ЗНАЧЕНИЕ(Документ.ПередачаТоваровХранителю.ПустаяСсылка)) ССЫЛКА Документ.ПередачаТоваровХранителю
	|	И НЕ СтрокиТоваров.ЕстьРаспоряжения
	//-- НЕ УТ
	|
	|ИНДЕКСИРОВАТЬ ПО
								...
								Пока Выборка.Следующий() Цикл
									...
									Если Не Выборка.ВестиУчетСертификатовНоменклатуры Тогда
										...
										СтруктураПараметров.Вставить("Сертификат",НСтр("ru = '<Учет сертификатов не ведется>';
																						|en = '<Certificates are not recorded>'"));
									...
									ИначеЕсли Выборка.НетДействующихСертификатов Тогда
										...
										Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'Для товара %1 отсутствуют зарегистрированные сертификаты. Требуется указать сертификаты номенклатуры.';
												|en = 'There are no registered certificates for goods %1. It is required to specify the product certificates.'"),
									...
									ИначеЕсли Не Выборка.СертификатДействует Тогда
										...
										Если ЗначениеЗаполнено(Выборка.Серия) Тогда
											ТекстОшибки = НСтр("ru = '<%ТипСертификата%: нет действующих сертификатов для серии %Серия%>';
																|en = '<%ТипСертификата%: no valid certificates for series %Серия%>'");
	...
	Пока ВыборкаПоДокументамБезУчетаСертификатов.Следующий() Цикл
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В документе %1 отсутствуют товары, для которых ведется учет сертификатов. Печать реестра сертификатов номенклатуры не требуется.';
				|en = 'Goods for which certificate accounting is kept are missing in document %1. Printing of product certificate registry is not required.'"),
						...
						Пока Выборка.Следующий() Цикл
							...
							Если Выборка.НетФайлов Тогда
								...
								НСтр("ru = 'В сертификате номенклатуры %1 отсутствуют присоединенные изображения. Требуется прикрепить файл изображения.';
									|en = 'There is no attached image in product certificate %1. Attach the image file.'"),
	...
	Пока ВыборкаПоДокументамБезУчетаСертификатов.Следующий() Цикл
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В документе %1 отсутствуют товары, для которых ведется учет сертификатов. Печать изображений сертификатов номенклатуры не требуется.';
				|en = 'Goods for which certificate accounting is kept are missing in document %1. Printing of product certificate images is not required.'"),
			Строка(ВыборкаПоДокументамБезУчетаСертификатов.Ссылка));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
			ВыборкаПоДокументамБезУчетаСертификатов.Ссылка);
...
Функция СформироватьПечатнуюФормуИзображенияСертификатовИзДокументовБезДублей(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ВыборкаПоДокументамБезУчетаСертификатов.Следующий() Цикл
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Goods for which certificate accounting is kept are missing in document %1.
			|Printing of product certificate images is not required.'"),
			Строка(ВыборкаПоДокументамБезУчетаСертификатов.Ссылка));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
			ВыборкаПоДокументамБезУчетаСертификатов.Ссылка);
