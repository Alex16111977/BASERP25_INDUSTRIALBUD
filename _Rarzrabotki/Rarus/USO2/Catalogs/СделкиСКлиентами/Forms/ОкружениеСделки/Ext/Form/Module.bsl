		...
		Если КоличествоСделок.ВСделках > 0 Тогда
			Результат = " " + ?(
				...
				НСтр("ru = '(сделок : %КолСделок%, взаимодействий - %КолВзаимодействий%)';
					|en = '(transactions : %КолСделок%, interactions - %КолВзаимодействий%)'"),
				НСтр("ru = '(сделок : %КолСделок%)';
					|en = '(transactions: %КолСделок%)'"));
...
Процедура СоздатьУзелПартнера(Родитель, Партнер, ПартнерыРодителя, мКонтакты, УровеньИерархии, Роль = Неопределено)
	...
	УзелПартнера.Представление = ?(
		Не ЗначениеЗаполнено(Роль) , ?(Партнер.Пустая(),НСтр("ru = '<Не указан>';
															|en = '<Not specified>'"),Партнер.Наименование),"(" + Роль + ") " 
		+ ?(Партнер.Пустая(),НСтр("ru = '<Не указан>';
									|en = '<Not specified>'"),Партнер.Наименование)
	...
	ЗаполнитьСвязиПоЗапросу(
		...
		НСтр("ru = 'первичный интерес';
			|en = 'primary interest'"));//сформировать связи по первичному интересу
...
Процедура СоздатьУзелГруппыПартнеров(Родитель, ГруппаПартнера, ПартнерыРодителя, мКонтакты, УровеньИерархии)
	...
	УзелГруппыПартнеров.Представление = "(" + НСтр("ru = 'в группе';
													|en = 'in group'") + ") " + ГруппаПартнера.Наименование;
...
Процедура СоздатьУзелКонтактногоЛица(Родитель, КонтактноеЛицо, Партнеры, КонтактыРодителя, УровеньИерархии, Роль = Неопределено)
	...
	ЗаполнитьСвязиПоЗапросу(
		...
		УровеньИерархии,
		НСтр("ru = 'первичный интерес';
			|en = 'primary interest'"));//сформировать связи через первичный интерес
