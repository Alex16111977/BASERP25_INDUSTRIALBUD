...
Функция ПолучитьСтрокуТипОтношений()
	...
	СтрокаТипОтношений = ?(Клиент,НСтр("ru = 'Клиент';
										|en = 'Client'"),"") + ?(Поставщик, НСтр("ru = ', Поставщик';
																				|en = ', Supplier'"),"") + ?(Перевозчик, НСтр("ru = ', Перевозчик';
	...
	Если ЭтоКомпания = 0 Тогда
		...
		ИначеЕсли ВидКомпании = 1 Тогда
			Возврат НСтр("ru = 'Юридическое лицо(за пределами РФ)';
						|en = 'Business entity (outside RF)'");
		ИначеЕсли ВидКомпании = 2 Тогда
			Возврат НСтр("ru = 'Индивидуальный предприниматель(ПБОЮЛ)';
						|en = 'Individual entrepreneur (Non-incorporated entrepreneur)'");
		Иначе
...
Процедура ПерейтиНаФинальнуюСтраницу()
	...
	Иначе
		ЗаголовокСтраницы = НСтр("ru = 'Проверьте введенные сведения, и нажмите кнопку ""Создать""  для регистрации нового партнера.';
								|en = 'Verify the information that you entered, and then click ""Create"" to register a new partner.'");
...
Функция ПервичнаяИнформацияЗаполненаНеправильно()
	...
	Если ЭтоКомпания = 0 Тогда
		Если ИспользуютсяТолькоПартнеры Тогда
			Если ПустаяСтрока(ПолноеЮридическоеНаименование) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Не указано сокращенное юр. наименование партнера';
													|en = 'Abbreviated partner name is not specified'"),"ПолноеЮридическоеНаименование");
				Отказ = Истина;
			КонецЕсли;
		Иначе
			Если ПустаяСтрока(ПолноеНаименование) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Не указано публичное наименование партнера';
													|en = 'Partner public name is not specified'"),"ПолноеНаименование");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ЭтоКомпания = 1 Тогда
		Если ПустаяСтрока(ФамилияПартнера) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Не указано фамилия партнера';
												|en = 'Partner last name is not specified'"),"ФамилияПартнера");
			Отказ = Истина;
		КонецЕсли;
		Если ПустаяСтрока(ИмяПартнера) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Не указано имя партнера';
												|en = 'Partner name is not specified'"),"ИмяПартнера");
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли; 
	...
	Если ЭтоКомпания = 0 Тогда
		...
		Если АдресЭППартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты партнера';
												|en = 'Specify partner''s email address'"),"АдресЭППартнера");
			Отказ = Истина;
		...
		Если ТелефонПартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать телефон партнера';
												|en = 'Specify partner phone number'"),"ТелефонПартнера");
			Отказ = Истина;
			ОбязательностьКонтактнойИнформацииОтработана = Истина;
		...
		Если УказатьДанныеКонтактногоЛица Тогда
			...
			Если АдресЭПКонтактногоЛицаОбязателен
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты контактного лица';
													|en = 'Specify email address of contact person'"), "АдресЭПКЛ");
				Отказ = Истина;
				ОбязательностьКонтактнойИнформацииОтработана = Истина;
			...
			Если ТелефонКонтактногоЛицаОбязателен 
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать телефон контактного лица';
													|en = 'Specify contact person phone number'"), "ТелефонКЛ");
				Отказ = Истина;
				ОбязательностьКонтактнойИнформацииОтработана = Истина;
			...
			Если МобильныйТелефонКонтактногоЛицаОбязателен 
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать мобильный телефон контактного лица';
													|en = 'Specify cell phone number of contact person'"), "МобильныйТелефонКЛ");
				Отказ = Истина;
				ОбязательностьКонтактнойИнформацииОтработана = Истина;
		...
		Если Не ОбязательностьКонтактнойИнформацииОтработана 
			...
			Если (УказатьДанныеКонтактногоЛица) Тогда
				...
				Если ПустаяСтрока(ТелефонКЛ) И ПустаяСтрока(МобильныйТелефонКЛ) И ПустаяСтрока(АдресЭПКЛ) Тогда
					...
					ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать хотя бы одно значение контактной информации';
														|en = 'You must specify at least one contact information value'"),"АдресЭППартнера");
					Отказ = Истина;
	...
	Иначе
		...
		Если АдресЭППартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты партнера';
												|en = 'Specify partner''s email address'"),"АдресЭППартнераФизЛицо");
			Отказ = Истина;
			ОбязательностьКонтактнойИнформацииОтработана = Истина;
		...
		Если ТелефонПартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать телефон партнера';
												|en = 'Specify partner phone number'"),"ТелефонПартнераФизЛицо");
			Отказ = Истина;
			ОбязательностьКонтактнойИнформацииОтработана = Истина;
		...
		Если Не ОбязательностьКонтактнойИнформацииОтработана
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать хотя бы одно значение контактной информации';
												|en = 'You must specify at least one contact information value'"),"АдресЭППартнераФизЛицо");
			Отказ = Истина;
	...
	Если УказыватьЮридическиеРеквизиты Тогда
		...
		Если Не ИспользуютсяТолькоПартнеры И ЭтоКомпания = 0 Тогда
			Если ПустаяСтрока(ПолноеЮридическоеНаименование) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Не указано сокращенное юр. наименование';
													|en = 'Abbreviated company name is not specified'"),"НаименованиеКонтрагента");
				Отказ = Истина; 
			КонецЕсли;
		КонецЕсли;
		...
		Если ЭтоКомпания = 0 И (ВидКомпании = 0 Или ВидКомпании = 3) Тогда
			Если ПустаяСтрока(ИНН) И Не ПустаяСтрока(КПП) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Указан КПП, но не указан ИНН';
													|en = 'CRTR is specified, specify TIN'"),"ИНН");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЭтоКомпания = 0 И НЕ ВидКомпании = 1 И НЕ УпрощенныйВводДоступен Тогда
			Если ПустаяСтрока(ИНН) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Не указано значение ИНН';
													|en = 'TIN value is not specified'"),"ИНН");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЭтоКомпания = 1 И УказыватьЮридическиеРеквизиты И УказатьДокументУдостоверяющийЛичность Тогда
			Если Не ЗначениеЗаполнено(ВидДокумента) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Не указано вид документа удостоверяющего личность';
													|en = 'Identity document kind is not specified'"),"ВидДокумента");
				Отказ = Истина
			КонецЕсли;
		КонецЕсли;
	...
	Если УказатьДанныеКонтактногоЛица И ЭтоКомпания = 0 Тогда
		...
		Если ПустаяСтрока(ФамилияКЛ) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Не указана фамилия контактного лица';
												|en = 'Last name of contact person is not specified.'"),"ФамилияКЛ");
			Отказ = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(ИмяКл) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Не указано имя контактного лица';
												|en = 'Contact person name is not specified'"),"ИмяКЛ");
			Отказ = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(ТелефонКЛ) И ПустаяСтрока(АдресЭПКЛ) И ПустаяСтрока(МобильныйТелефонКЛ) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать хотя бы одно значение контактной информации контактного лица';
												|en = 'Specify at least one contact information value of the contact person'"),"АдресЭПКЛ");
			Отказ = Истина;
		КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Если  НЕ ПустаяСтрока(АдресЭППартнера) И ЭтоКомпания = 0 Тогда
			Если ОбщегоНазначенияУТКлиентСервер.ПроверитьЗаполнениеПочтовогоАдреса(АдресЭППартнера) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Неверный формат адреса электронной почты партнера';
													|en = 'Incorrect format of partner''s email address'"), "АдресЭППартнера");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если  НЕ ПустаяСтрока(АдресЭППартнераФизЛицо) И ЭтоКомпания = 1 Тогда
			Если ОбщегоНазначенияУТКлиентСервер.ПроверитьЗаполнениеПочтовогоАдреса(АдресЭППартнераФизЛицо) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Неверный формат адреса электронной почты партнера';
													|en = 'Incorrect format of partner''s email address'"), "АдресЭППартнераФизЛицо");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если  УказатьДанныеКонтактногоЛица И НЕ ПустаяСтрока(АдресЭПКЛ) И ЭтоКомпания = 0 Тогда
			Если ОбщегоНазначенияУТКлиентСервер.ПроверитьЗаполнениеПочтовогоАдреса(АдресЭПКЛ) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Неверный формат адреса электронной почты контактного лица';
													|en = 'Incorrect format of contact person''s email address'"), "АдресЭПКЛ");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
...
Функция СведенияГоловногоКонтрагентаЗаполненыНеправильно()
	...
	Если ВыбратьГоловногоКонтрагента = 1 Тогда
		...
		Если ПустаяСтрока(ПолноеНаименованиеГоловногоКонтрагента) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Не указано сокращенное юр. наименование';
												|en = 'Abbreviated company name is not specified'"), "ПолноеНаименованиеГоловногоКонтрагента");
			Отказ = Истина;
		КонецЕсли;
			...
			Если КПП = КППГоловногоКонтрагента Тогда
				ВывестиСообщениеПользователю(
					НСтр("ru = 'КПП головного контрагента должен отличаться от КПП обособленного подразделения';
						|en = 'Main counterparty CRTR shall differ from branch office CRTR'"), КППГоловногоКонтрагента);
...
Функция РасчетныйСчетЗаполненНеправильно()
	...
	Если УказатьБанковскийСчетКонтрагента Тогда
		...
		Если ПустаяСтрока(НомерСчета) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать номер банковского счета';
												|en = 'You must specify bank account number'"), "НомерСчета");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать валюту денежных средств';
												|en = 'You must specify currency'"), "ВалютаДенежныхСредств");
			Отказ = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(БИКБанка) И Не ИностранныйБанк Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка';
												|en = 'You must specify bank branch ID'"), "БИКБанка");
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользуетсяБанкДляРасчетов Тогда
			...
			Если ПустаяСтрока(БИКБанкаДляРасчетов)
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка для расчетов';
													|en = 'Specify branch ID of the bank for settlements'"), "БИКБанкаДляРасчетов");
				Отказ = Истина;
			КонецЕсли;
...
Функция РасчетныйСчетГоловногоКонтрагентаЗаполненНеправильно()
	...
	Если УказатьБанковскийСчетГоловногоКонтрагента Тогда
		...
		Если ПустаяСтрока(НомерСчетаГоловногоКонтрагента) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать номер банковского счета';
												|en = 'You must specify bank account number'"), "НомерСчетаГоловногоКонтрагента");
			Отказ = Истина;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредствГоловнойКонтрагент) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать валюту денежных средств';
												|en = 'You must specify currency'"), "ВалютаДенежныхСредствГоловнойКонтрагент");
			Отказ = Истина;
		КонецЕсли;
		
		Если ПустаяСтрока(БИКБанкаГоловнойКонтрагент) И Не ИностранныйБанкГоловнойКонтрагент Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка';
												|en = 'You must specify bank branch ID'"), "БИКБанкаГоловнойКонтрагент");
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользуетсяБанкДляРасчетовГоловнойКонтрагент Тогда
			...
			Если ПустаяСтрока(БИКБанкаДляРасчетовГоловнойКонтрагент)
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка для расчетов';
													|en = 'Specify branch ID of the bank for settlements'"), "БИКБанкаДляРасчетовГоловнойКонтрагент");
				Отказ = Истина;
			КонецЕсли;
			...
			Если ТекущиеДанные.Различие Тогда
				...
				Если ИспользуютсяТолькоПартнеры Тогда
					ТекстВопроса = НСтр("ru = 'Выбранный контрагент имеет существенное различие с введенными данными. Продолжить?';
										|en = 'Selected counterparty has a significant difference with data entered. Continue?'");
				Иначе
	...
	Если Не ЕстьОшибка Тогда
		...
		Если ЗначениеЗаполнено(БИКБанка) Тогда
			...
			Если Не ЗначениеЗаполнено(Банк) Тогда
				...
				ТекстОшибки = НСтр("ru = 'В справочнике Банки не найден банк по БИК';
									|en = 'The bank was not found by branch ID in the Banks catalog'") +" = " +БИКБанка;
		...
		Если ЗначениеЗаполнено(БИКБанкаДляРасчетов) Тогда
			...
			Если Не ЗначениеЗаполнено(БанкДляРасчетов) Тогда
				...
				ТекстОшибки = НСтр("ru = 'В справочнике Банки не найден банк по БИК';
									|en = 'The bank was not found by branch ID in the Banks catalog'") +" = " +БИКБанкаДляРасчетов;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьЗаголовки()
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                     ...
	                     НСтр("ru = 'Фактический адрес контрагента';
								|en = 'Physical address of the counterparty'"), ИспользуютсяТолькоПартнеры);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
...
Процедура СоздатьПартнера(СписокСозданных)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание нового партнера.';
										|en = 'Create a new partner.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
