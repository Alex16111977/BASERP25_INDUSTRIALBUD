...
// Функция определяет, используются или нет группы доступа видов бюджетов.
//
...
// Функция определяет, есть ли у текущего пользователя право изменять вид бюджета.
//
//	Параметры:
//		Объект - любая коллекция с полями
//			ГруппаДоступа - СправочникСсылка.ГруппыДоступаВидовБюджетов
//			Ссылка - ссылка на справочник, к которому возможно ограничение доступа по группе доступа видов бюджетов.
//
//	Возвращаемое значение:
...
// Функция возвращает группу доступа видов бюджетов по умолчанию для текущего пользователя.
//
//	Параметры:
//		Объект - любая коллекция с полями
//			ГруппаДоступа - СправочникСсылка.ГруппыДоступаВидовБюджетов
...
//
//	Возвращаемое значение:
//		СправочникСсылка.ГруппыДоступаВидовБюджетов - группа доступа по умолчанию.
//
Функция ПолучитьГруппуДоступаПоУмолчанию(Объект) Экспорт
	
	ГруппаДоступаПоУмолчанию = Справочники.ГруппыДоступаВидовБюджетов.ПустаяСсылка();
