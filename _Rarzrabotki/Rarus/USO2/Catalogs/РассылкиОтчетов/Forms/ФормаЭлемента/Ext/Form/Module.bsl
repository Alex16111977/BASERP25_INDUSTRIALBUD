	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru = 'Рассылка отчетов. Открытие формы элемента';
														|en = 'Report bulk mail. Open item form'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если Объект.Подготовлена Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не выбрана периодичность запуска';
									|en = 'Launch frequency is not selected'");
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для отчета ''%1'' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок ''Подготовлена''.';
						|en = 'Not all required settings are filled in for report ''%1''. Fill in all required settings or clear check box ''Prepared''.'"),
		...
		Если РассылкаНеПерсонализирована И ОтчетПерсонализирован Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В настройках отчета ''%1'' задан отбор по получателю рассылки. Необходимо отключить этот отбор или изменить вид рассылки на ''Свой отчет для каждого получателя''.';
					|en = 'Filter by bulk mail recipient is set in the ""%1"" report settings."" ""Remove this filter or change the bulk mail kind to ""Unique report for each recipient"".'"),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
		...
		Если Объект.Персонализирована И	НЕ ОтчетПерсонализирован Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В настройках отчета ""%1"" не указан отбор по получателю рассылки.';
					|en = 'Filter by bulk mail recipient is not specified in the ""%1"" report settings.'"),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("ru = 'Все выбранные отчеты уже включены в рассылку';
									|en = 'All the selected reports are already included in bulk mail.'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("ru = 'Отчет включен в рассылку';
											|en = 'Report is included in the bulk mail'");
			Иначе
				ЗаголовокОповещения = НСтр("ru = 'Отчеты включены в рассылку';
											|en = 'Reports are included in the bulk mail'");
			КонецЕсли;
...
Процедура УказатьПолучателяРассылки(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("ru = 'Использовать получателя в параметрах возможно только для вида рассылки ""%1""';
								|en = 'Recipient can be used in parameters only for bulk mail kind ""%1""'");
	...
	Если НайденныеИдентификаторыОбъектовМетаданных.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Тип получателей не найден';
										|en = 'Recipient type is not found'"));
		Возврат;
	КонецЕсли;
			...
			Иначе
				...
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("ru = 'Для настройки отчета не найдено описание доступного поля';
													|en = 'To configure the report, the available field description is not found'"));
					Возврат Неопределено;
				КонецЕсли;
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""';
				|en = 'You can use recipient in the template text only for the bulk mail kind ""%1""'"),
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		...
		Если Не Объект.Подготовлена Тогда
			...
			|en = 'Prepare bulk email before check.
		...
		Иначе
			...
			|en = 'Write bulk email before check.
			|Click ""Continue"" to write bulk email. '");
		КонецЕсли;
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				НСтр("ru = 'Группа установлена в соответствии с видом рассылки';
					|en = 'Group is set according to bulk mail kind'"));
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов. Проверка способа доставки';
													|en = 'Report bulk mail. Check delivery method'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = НСтр("ru = 'Тест способа доставки успешно пройден.';
										|en = 'Delivery method test is successfully passed.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("ru = 'Тест способа доставки не пройден.';
										|en = 'Delivery method test failed.'");
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов. Инициализация отчета';
													|en = 'Report bulk mail. Report initialization'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура СобытияРассылки()
	...
	Если ПараметрыФормыЖурналаРегистрации = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Рассылка еще не выполнялась.';
										|en = 'Bulk email was not performed yet.'"));
		Возврат;
	КонецЕсли;
