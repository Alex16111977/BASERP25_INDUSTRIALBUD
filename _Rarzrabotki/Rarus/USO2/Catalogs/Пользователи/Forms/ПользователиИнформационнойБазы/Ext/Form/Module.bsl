...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для открытия списка пользователей информационной базы.';
								|en = 'Insufficient rights to open infobase user list.'");
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Отмена сопоставления пользователя информационной базы с пользователем в справочнике.
		           ...
		           |en = 'Cancel mapping infobase user with user in catalog.
		           |
		           |Mapping should be canceled only if the operation was incorrect, for example, 
		           |when updating infobase. That is why it is not recommended that you cancel mapping by any other reason.'"),
		Кнопки,
		,
