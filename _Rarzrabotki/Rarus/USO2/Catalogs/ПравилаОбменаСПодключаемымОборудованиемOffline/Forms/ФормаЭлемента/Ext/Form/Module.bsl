	...
	Если КоличествоТоваров > 0 Тогда
		Если Товары[КоличествоТоваров-1].Код - 99999 > ТекущийОбъект.МаксимальныйКод И ТекущийОбъект.МаксимальныйКод <> 0 Тогда
			...
			НСтр("ru = 'В табличной части ""Товары"" слишком много товаров. Измените максимальное количество товаров или уменьшите количество товаров к выгрузке при помощи отбора.';
				|en = 'Tabular section Goods contains too many goods. Change a maximum quantity of goods or reduce quantity of goods for exporting using the filter.'"),,"Товары",,Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура МаксимальныйКодПриИзменении(Элемент)
	...
	Состояние(НСтр("ru = 'Выполняется обновление списка товаров...';
					|en = 'Goods list is being updated...'"));
	ОбновитьСписокТоваровНаСервере();
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Такой код уже назначен для номенклатуры %Номенклатура%';
									|en = 'Such code is already assigned for products %Номенклатура%'");
			...
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Характеристика), " " + НСтр("ru = 'с характеристикой';
																				|en = 'with characteristic'") + " """ + НайденнаяСтрока.Характеристика + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Упаковка), " " + НСтр("ru = 'в упаковке';
																		|en = 'in packaging'") + " """ + НайденнаяСтрока.Упаковка + """", ""))+НСтр("ru = '. Обменять их местами?';
