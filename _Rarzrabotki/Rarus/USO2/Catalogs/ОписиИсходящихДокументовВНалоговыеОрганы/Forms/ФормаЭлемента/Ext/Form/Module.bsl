...
Процедура РазбитьНаНесколько(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Текущий ответ на требование будет разбит на несколько. Продолжить?';
						|en = 'Текущий ответ на требование будет разбит на несколько. Продолжить?'");
...
Процедура ДобавитьДокументы(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Требование""';
																|en = 'Не заполнено поле ""Требование""'"), , "Объект.Требование");
		Возврат;
	КонецЕсли;
...
Процедура УстановитьТолькоПросмотр(ПризнакТолькоПросмотр = Истина)
	...
	Элементы.Требование.ТолькоПросмотр = ПризнакТолькоПросмотр;
...
Процедура ВыбратьИДобавитьВложения()
	...
	КонтекстЭДОКлиент.ЗагрузитьСканСДискаВОтчетНаТребование(ОписаниеОповещения, УникальныйИдентификатор);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьИДобавитьВложения_Завершение(Результат, ВходящийКонтекст) Экспорт
