		...
		Иначе
			...
			Иначе
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					|en = 'When editing, you changed the department bookkeeping.
					|If you just corrected previous data (it was incorrect), click ""Error is corrected"".
					|If the department bookkeeping changed from %1, click ""Bookkeeping changed""'"), 
...
Процедура ТерриториальныеУсловияПФРИстория(Команда)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
			...
			|You can go to site conditions change history only after data is written.
			|Write the data?'");
...
Процедура ИзменитьРегистрациюВНалоговомОргане(Команда)
	...
	Если Объект.ОбособленноеПодразделение
		...
		ТекстВопроса = НСтр("ru = 'Был установлен признак обособленного подразделения.
			...
			|Записать?';
			|en = 'Flag of branch office was set.
			|To proceed, save the changes.
			|Write?'");
...
Функция ЗаголовкиГруппСтраницыАдаптацияУвольнение() Экспорт
	...
	ЗаголовкиГрупп.Вставить("МероприятияАдаптацииГруппа", НСтр("ru = 'При приеме или переводе в подразделение назначается задание';
																|en = 'Job is assigned during hiring or transfer to department'"));
	ЗаголовкиГрупп.Вставить("МероприятияУвольненияГруппа", НСтр("ru = 'При увольнении или переводе из подразделения назначается задание';
																|en = 'Job is assigned after termination of employment or transfer from department'"));
			...
			Для каждого ОписаниеПоля Из ПроверяемыеПоля Цикл
				...
				Если Не ЗначениеЗаполнено(Запись[ОписаниеПоля.Имя]) Тогда   
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: не заполнено поле ""%2"".';
																									|en = '%1: the ""%2"" field is not filled in.'"), Синоним, ?(ЗначениеЗаполнено(ОписаниеПоля.Синоним), ОписаниеПоля.Синоним, ОписаниеПоля.Имя));
					ПутьКРеквизитуФормы = "БухучетЗарплатыПодразделенийРаспределение";
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,	ПутьКРеквизитуФормы, , Отказ);
				КонецЕсли;	
