...
Процедура ПередЗаписью(Отказ)
	...
	Иначе
		...
		ПрежниеЗначение = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "ОбособленноеПодразделение,Владелец,Родитель,РегистрацияВНалоговомОргане");
		...
		Если ПрежниеЗначение.Владелец <> Владелец Тогда
			ВызватьИсключение НСтр("ru = 'Нельзя менять организацию - владельца элемента справочника ""Подразделения""';
									|en = 'Cannot change a company that is the owner of the ""Departments"" catalog item'");;
		КонецЕсли;
		...
		ИначеЕсли Родитель <> ПрежниеЗначение.Родитель Тогда
			...
			Если ПрежниеЗначение.РегистрацияВНалоговомОргане <> РегистрацияВНалоговомОрганеВышестоящего Тогда
				ДополнительныеСвойства.Вставить("ОбновитьИсториюРегистрацийВНалоговомОргане", Истина);
			КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				|en = 'Cannot change arrangement setting for department ""%1"". 
				|Maybe another user is editing the department'"),
