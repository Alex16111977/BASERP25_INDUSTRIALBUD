...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Работа помощника подключения к сервису Яндекс.Касса не была завершена.';
									|en = 'Wizard of connection to Yandex.Checkout is not completed.'");
...
Процедура ЗаполнитьПараметрыФормы(ОбъектДанных)
	...
	Элементы.Варианты.Заголовок = ?(ОтображатьОрганизацию,
		НСтр("ru = 'Выберите вариант использования сервиса и организацию';
			|en = 'Select service usage option and company'"),
		НСтр("ru = 'Выберите вариант использования сервиса';
			|en = 'Select service usage option'"));
...
Процедура ИзменитьЗаголовок()
	...
	Если ЗначениеЗаполнено(Организация) Тогда 
		Шаблон = НСтр("ru = 'Настройка подключения %1 к сервису';
						|en = 'The %1 service connection setting'");
	...
	Иначе
		Элементы.Заголовок.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Настройка подключения к сервису';
																		|en = 'Configure connection to service'"));
	КонецЕсли;	
...
Процедура ОтобразитьПризнакНедействительна()
	...
	Если Недействительна Тогда
		...
		МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = '(Недействующая)';
																|en = '(Inactive)'") + " ", Новый Шрифт(,,,Истина)));
...
Процедура ПриОткрытииСтраницыКарточкаНастройки(СтраницаСохранения)
	...
	МассивСтрок.Добавить(НСтр("ru = 'Для сохранения настройки нажмите Сохранить, для изменения нажмите Назад, а для закрытия нажмите Отмена';
								|en = 'To save the setting, click Save; to change the setting, click Back; to close the setting, click Cancel'"));
	МассивСтрок.Добавить(Символы.ПС);
	МассивСтрок.Добавить(НСтр("ru = 'Нажатие кнопки Сохранить означает согласие с';
								|en = 'By clicking Save, you accept'") + Символы.НПП);
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Условиями использования сервиса';
															|en = 'Service Use Conditions'"),,,,"https://kassa-yandex.1c.ru/agreement"));
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
		...
		?(СтраницаСохранения, 
			...
			НСтр("ru = 'Для изменения настройки нажмите Далее, а для закрытия нажмите Отмена';
				|en = 'To change the setting, click Next, to close the setting, click Cancel'")));
...
Процедура ПроверкаЗаполненияСтраницаВариантыИВыборОрганизации(Отказ)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Иначе 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Настройте организацию и повторите попытку настройки';
																	|en = 'Set up organization and try to set up again'"),,,,Отказ);
		КонецЕсли;
...
Процедура СоздатьЗаписатьНастройкуЯндексКассы(Отказ)
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Запись настройки по Яндекс Кассе';
																			|en = 'Write Yandex.Checkout setting'"),
			ОписаниеОшибки(), НСтр("ru = 'Возникла ошибка при записи настройки по Яндекс.Кассе';
									|en = 'An error occurred while saving Yandex.Checkout setting'"), 6);
	КонецПопытки;
