...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтоАналитикаПрочее Тогда
		...
		Если РазрешитьРедактирование И ИсточникВсеЭлементы Тогда
			Объект.НаименованиеДляПечати = НСтр("ru = '<все %1>';
												|en = '<all %1>'", КодОсновногоЯзыка);
			Объект.НаименованиеДляПечати = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Объект.НаименованиеДляПечати, НРег(ВидАналитики));
		Иначе
			Если РазрешитьРедактирование Тогда
				Объект.НаименованиеДляПечати = НСтр("ru = '<добавляемые и прочие %1>';
													|en = '<added and other %1>'", КодОсновногоЯзыка);
			Иначе
				Объект.НаименованиеДляПечати = НСтр("ru = '<прочие %1>';
													|en = '<other %1>'", КодОсновногоЯзыка);
			КонецЕсли;
			Объект.НаименованиеДляПечати = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Объект.НаименованиеДляПечати, НРег(ВидАналитики));
		КонецЕсли;
	Иначе
	...
	Если Не ЭтоАналитикаПрочее Тогда
		Если Не ЗначениеЗаполнено(Объект.ЗначениеАналитики) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"ЗначениеАналитики",
				НСтр("ru = 'Не заполнено значение аналитики';
					|en = 'Dimension value is not filled in'"), "");
		КонецЕсли;
	КонецЕсли;
