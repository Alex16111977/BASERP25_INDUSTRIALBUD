	...
	Если (НЕ ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей())
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Хранилище настроек новостей';
				|en = 'ISL:News.News setting storage'"), // ИмяСобытия
			НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаЗагрузки. Нарушение прав доступа';
				|en = 'News. News setting storage. ОбработкаЗагрузки. Access right violation'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			НСтр("ru = 'Попытка вызова ХранилищаНастроек.НастройкиНовостей.ОбработкаЗагрузки, когда недоступна роль ЧтениеНовостей или ПолныеПрава и не включен привилегированный режим.';
				|en = 'Attempt to call ХранилищаНастроек.НастройкиНовостей.ОбработкаЗагрузки, when the role ЧтениеНовостей or ПолныеПрава is unavailable, and privileged mode is not enabled.'"), // Комментарий
...
Процедура ОбработкаПолученияОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если (НЕ ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей())
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Хранилище настроек новостей';
				|en = 'ISL:News.News setting storage'"), // ИмяСобытия
			НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаПолученияОписания. Нарушение прав доступа';
				|en = 'News. News setting storage. ОбработкаПолученияОписания. Access right violation'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			НСтр("ru = 'Попытка вызова ХранилищаНастроек.НастройкиНовостей.ОбработкаПолученияОписания, когда недоступна роль ЧтениеНовостей или ПолныеПрава и не включен привилегированный режим.';
				|en = 'Attempt to call ХранилищаНастроек.НастройкиНовостей.ОбработкаПолученияОписания, when the role ЧтениеНовостей or ПолныеПрава is unavailable, and privileged mode is not enabled.'"), // Комментарий
			Истина); // ВестиПодробныйЖурналРегистрации
		Возврат;
	КонецЕсли;
...
Процедура ОбработкаСохранения(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если (НЕ ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей())
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Хранилище настроек новостей';
				|en = 'ISL:News.News setting storage'"), // ИмяСобытия
			НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаСохранения. Нарушение прав доступа';
				|en = 'News. News setting storage. ОбработкаСохранения. Access right violation'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			НСтр("ru = 'Попытка вызова ХранилищаНастроек.НастройкиНовостей.ОбработкаСохранения, когда недоступна роль ЧтениеНовостей или ПолныеПрава и не включен привилегированный режим.';
				|en = 'Attempt to call ХранилищаНастроек.НастройкиНовостей.ОбработкаСохранения, when the role ЧтениеНовостей or ПолныеПрава is unavailable, and privileged mode is not enabled.'"), // Комментарий
			Истина); // ВестиПодробныйЖурналРегистрации
		Возврат;
	КонецЕсли;
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось записать константу НастройкиНовостей.
						...
						|Подробности:
						|%5';
						|en = 'Cannot write the НастройкиНовостей constant.
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка';
						|en = 'News. News setting storage. ОбработкаСохранения. Error'"), // ИдентификаторШага
	...
	ИначеЕсли КлючОбъекта = "ДоступныеЛентыНовостей" Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Ошибка при записи настроек новостей. Настройки для ключа ДоступныеЛентыНовостей не должны записываться. Записывать можно только ОтключенныеЛентыНовостей.';
				|en = 'An error occurred while writing news settings. Settings for key ДоступныеЛентыНовостей should not be written. You can write only ОтключенныеЛентыНовостей.'"), // Комментарий
			ОбработкаНовостейВызовСервера.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ОбработкаУстановкиОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если (НЕ ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей())
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Хранилище настроек новостей';
				|en = 'ISL:News.News setting storage'"), // ИмяСобытия
			НСтр("ru = 'Новости. Хранилище настроек новостей. ОбработкаУстановкиОписания. Нарушение прав доступа';
				|en = 'News. News setting storage. ОбработкаУстановкиОписания. Access right violation'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			НСтр("ru = 'Попытка вызова ХранилищаНастроек.НастройкиНовостей.ОбработкаУстановкиОписания, когда недоступна роль ЧтениеНовостей или ПолныеПрава и не включен привилегированный режим.';
				|en = 'Attempt to call ХранилищаНастроек.НастройкиНовостей.ОбработкаУстановкиОписания, when the role ЧтениеНовостей or ПолныеПрава is unavailable, and and privileged mode is not enabled.'"), // Комментарий
			Истина); // ВестиПодробныйЖурналРегистрации
		Возврат;
	КонецЕсли;
