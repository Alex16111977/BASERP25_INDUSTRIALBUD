...
Процедура ПапкиПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru = 'Удалить папку ""';
						|en = 'Delete folder ""'") + Строка(ТекущиеДанные.Значение) 
	+ НСтр("ru = '"" и переместить все ее содержимое в папку ""Удаленные""';
			|en = '"" and transfer all its contents to the ""Deleted"" folder'");
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Команда ""Ответить"" может быть выполнена только для входящего письма.';
											|en = 'You can click ""Reply"" only for incoming messages. '"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Команда ""Ответить всем"" может быть выполнена только для входящего письма.';
											|en = 'You can click ""Reply to all"" only for incoming messages.'"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
	...
	Если ВыборКорректен(Элементы.Список.Имя, Истина) Тогда
		...
		Если ТипЗнч(ТекущееВзаимодействие) <> Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") 
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Команда ""Переслать"" может быть выполнена только для писем.';
											|en = 'You can click ""Forward"" only for messages. '"));
			Возврат;
		КонецЕсли;
	Иначе
		Возврат;
...
Процедура ДобавитьВЗакладки(Команда)
	...
	Если СтрНачинаетсяС(ИмяТекущегоЭлемента, "Список") И Не ВыборКорректен(ИмяТекущегоЭлемента) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбран элемент для добавления в закладки.';
										|en = 'Item for adding to tabs is not selected.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтложитьРассмотрениеВыполнить(Команда)
	...
	ПоказатьВводДаты(ОбработчикОповещенияОЗакрытии, ДатаОтработки, НСтр("ru = 'Отработать после';
																		|en = 'Process after'"), ЧастиДаты.ДатаВремя);
	...
	Если ИмяТекущегоЭлемента = "Папки" Тогда
		Если ТипЗнч(ПапкиТекущиеДанные.Значение) = Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты") 
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Команда не может быть выполнена для данного объекта';
											|en = 'Command cannot be executed for this object'"));
			Возврат;
		ИначеЕсли ПапкиТекущиеДанные.ЕстьПравоНаИзменение = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Недостаточно прав для изменения папок.';
											|en = 'Insufficient rights to change folders.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПереслатьКакВложение(Команда)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Пересылать как вложения можно только отправленные и полученные письма.';
								|en = 'Only sent and received emails can be forwarded as attachments.'");
...
Процедура ИзменитьОтборСписок(ИмяТаблицы, ДанныеДляОбработки);
	...
	Если ДанныеДляОбработки.Значение = НСтр("ru = 'Все';
											|en = 'All'") Тогда
	...
	ИначеЕсли ИмяТаблицы = "Свойства" Тогда
		...
		Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") 
			И ДанныеДляОбработки.Значение = НСтр("ru = 'Не указан';
	...
	ИначеЕсли ИмяТаблицы = "Закладки" Тогда
		...
		ЗаголовокПанелиНавигации   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("ru = 'Закладка';
																													|en = 'Tab'"), ДанныеДляОбработки.Значение);
...
Процедура ОтобразитьПредпросмотрВзаимодействия(СсылкаДокументВзаимодействий, ИмяТекущейСтраницы)
	...
	Если ТипЗнч(СсылкаДокументВзаимодействий) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") Тогда
		...
		ПредпросмотрHTML = Взаимодействия.СформироватьТекстHTMLДляВходящегоПисьма(СсылкаДокументВзаимодействий, Ложь, Ложь, Не ВключитьНебезопасноеСодержимое, ЕстьНебезопасноеСодержимое);
		Предпросмотр = "";
	...
	ИначеЕсли ТипЗнч(СсылкаДокументВзаимодействий) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее") Тогда
		...
		ИмяТекущейСтраницы = Элементы.СтраницаПредпросмотрHTML.Имя;
		ПредпросмотрHTML = Взаимодействия.СформироватьТекстHTMLДляИсходящегоПисьма(СсылкаДокументВзаимодействий, Ложь, Ложь, Не ВключитьНебезопасноеСодержимое, ЕстьНебезопасноеСодержимое);
		Предпросмотр = "";
	...
	Иначе
		...
		Если Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаСвойства Тогда
			Элементы.ВыборВариантаНавигации.Заголовок = НСтр("ru = 'По';
															|en = 'To'") + " " + ПредставлениеТекущегоСвойства;
	...
	Для Каждого Строка Из РодительскаяСтрока.Строки Цикл
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Строка.Значение) Тогда
			...
			НоваяСтрока.Представление = ?(ПустаяСтрока(СтрокаДетали.Тема), 
				НСтр("ru = 'Тема не указана';
					|en = 'Subject is not specified'"), СтрокаДетали.Тема) + " " + НСтр("ru = 'от';
																				|en = 'dated'") + " " 
	...
	Иначе
		...
		Если ДанныеДляОбработки.Значение = НСтр("ru = 'Все';
			...
			Результат.ТекстСообщенияОшибка = НСтр("ru = 'Закладку без отбора создать нельзя.';
													|en = 'Cannot create a tab without filter.'");
			Возврат Результат;
		КонецЕсли;
		...
		Если ИмяЭлементаФормы = "ПредметыПанельНавигации" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				НаименованиеЗакладки       = НСтр("ru = 'Предмет';
													|en = 'Subject'") + " = " + Строка(ДанныеДляОбработки.Значение); 
				Текст = ?(ТолькоПочта, НСтр("ru = 'Письма по предмету %1';
											|en = 'Emails by subject %1'"), НСтр("ru = 'Взаимодействия по предмету %1';
																					|en = 'Interactions on the subject %1'"));
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru = 'Не указан';
														|en = 'Not specified'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru = 'Предмет не указан';
													|en = 'Subject is not specified'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма без предмета';
																		|en = 'Emails without subject'"), НСтр("ru = 'Взаимодействия без предмета';
																											|en = 'Interaction without subject'"));
				
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru = 'Прочие вопросы';
														|en = 'Other matters'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru = 'Прочие вопросы';
													|en = 'Other matters'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Прочие письма';
																		|en = 'Other emails'"), НСтр("ru = 'Прочие взаимодействия';
																									|en = 'Other interactions'"));
				
			Иначе
				
				НаименованиеЗакладки       = НСтр("ru = 'Тип предмета';
													|en = 'Subject type'") + " " + Строка(ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма по: %1';
																		|en = 'Emails by: %1'"), НСтр("ru = 'Взаимодействия по: %1';
		...
		ИначеЕсли ИмяЭлементаФормы = "Свойства" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("Строка") 
				...
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма';
																		|en = 'Emails'"), НСтр("ru = 'Взаимодействия';
																							|en = 'Interactions'"));
			Иначе
				...
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, 
				                                   НСтр("ru = 'Письма с заданным свойством: %1';
														|en = 'Emails with the specified property: %1'"), 
				                                   НСтр("ru = 'Взаимодействия с заданным свойством: %1';
														|en = 'Interactions with specified property: %1'"));
		...
		ИначеЕсли ИмяЭлементаФормы = "Категории" Тогда
			...
			НаименованиеЗакладки       = НСтр("ru = 'Входит в категорию';
												|en = 'Belongs to the category'") + " " + Строка(ДанныеДляОбработки.Значение);
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма из категории: %1';
																	|en = 'Emails from category: %1'"), НСтр("ru = 'Взаимодействия из категории: %1';
																											|en = 'Interactions from the category: %1'"));
			Результат.ПредставлениеЭлемента = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.Значение);
		...
		ИначеЕсли ИмяЭлементаФормы = "Контакты" Тогда
			...
			Если ОбщегоНазначения.ЗначениеСсылочногоТипа(ДанныеДляОбработки.Значение) Тогда
				...
				Текст = ?(ТолькоПочта, НСтр("ru = 'Письма по контакту %1';
											|en = 'Emails by contact %1'"), НСтр("ru = 'Взаимодействия по контакту %1';
																					|en = 'Interactions by contact %1'"));
				Результат.ПредставлениеЭлемента = 
			...
			ИначеЕсли ДанныеДляОбработки.Значение = НСтр("ru = 'Контакт не подобран';
														|en = 'Contact is not selected'") Тогда
				
				НаименованиеЗакладки       = НСтр("ru = 'Контакт не указан';
													|en = 'Contact is not specified'");
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма без подобранных контактов';
																		|en = 'Emails without selected contacts'"), НСтр("ru = 'Взаимодействия без подобранных контактов';
																														|en = 'Interactions without selected contacts'"));
				
			Иначе
				
				НаименованиеЗакладки       = НСтр("ru = 'Тип контакта';
													|en = 'Contact type'") + " " + Строка(ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
				Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма по контактам: %1';
																		|en = 'Emails by contacts: %1'"), НСтр("ru = 'Взаимодействия по контактам: %1';
																												|en = 'Interactions by contacts: %1'"));
				Результат.ПредставлениеЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Результат.ПредставлениеЭлемента, ДанныеДляОбработки.ОписаниеТипов.Типы()[0]);
		...
		ИначеЕсли ИмяЭлементаФормы = "Папки" Тогда
			...
			Результат.ПредставлениеЭлемента = ?(ТолькоПочта, НСтр("ru = 'Письма в папке: %1';
																	|en = 'Emails are in folder: %1'"), НСтр("ru = 'Взаимодействия в папке: %1';
																									|en = 'Interactions in the folder: %1'"));
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеВXML(КомпоновщикНастроек.ПолучитьНастройки()) =  ЗначениеВXML(Выборка.КомпоновщикНастроек.Получить()) Тогда
			Результат.ТекстСообщенияОшибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Закладка с такими настройками уже существует : %1';
																											|en = 'Tab with these settings already exists : %1'"),
...
Процедура ВыполнитьПолнотекстовыйПоиск()
	...
	Если ТекстОшибки = Неопределено Тогда
		...
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Найдено %1 %2.';
																						|en = 'Found %1 %2.'"),
			?(ТолькоПочта,НСтр("ru = 'писем';
								|en = 'emails'"), НСтр("ru = 'взаимодействий';
													|en = 'interactions'")) + ": ",
...
Функция НайтиВзаимодействияПолнотекстовыйПоиск(КоличествоЭлементов)
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда
		...
		Возврат НСтр("ru = 'Слишком много результатов, уточните запрос.';
					|en = 'Too many results. Refine your search criteria.'");
	КонецЕсли;
