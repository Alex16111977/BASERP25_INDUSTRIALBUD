	...
	Пока ВыборкаОшибок.Следующий() Цикл
		...
		Если ВыборкаОшибок.ТипОшибки = "ОшибкаБаланса" Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: нарушен баланс по измерениям %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';
									|en = 'An error occurred in the algorithms while writing  %Регистратор% % to the РезервыТоваровОрганизаций register: violated balance of dimensions %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Contact application developers. '");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаНулевоеКоличество" Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: в месяце %Месяц% записывается нулевое количество по измерениям: %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';
									|en = 'An error occurred in the algorithms while writing  %Регистратор% % to the РезервыТоваровОрганизаций register: zero quantity is written in month %Месяц% for dimensions: %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Contact application developers. '");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаПринадлежностиВидовЗапасов" Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: по измерениям записывается вид запасов, не принадлежащий организации %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';
									|en = 'An error occurred in the algorithms while writing  %Регистратор% % to the РезервыТоваровОрганизаций register: inventory kind which does not belong to the company is written for dimensions %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Contact application developers.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаПодобранногоВидаЗапасов" Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: организация и кор. организация совпадают, но при этом тип запасов вида запасов не ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';
									|en = 'An error occurred in the algorithms while writing  %Регистратор% % to the РезервыТоваровОрганизаций register: company and cor. company match but stock type of stock kind is not ТоварНаХраненииСПравомПродажand %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Contact application developers.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаПриходРезерваПоТоварНаХраненииСПравомПродажи" Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: приход резерва по типу запасов ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';
									|en = 'An error occurred in algorithms when writing %Регистратор% to the РезервыТоваровОрганизаций register: reserve receipt by stock type ТоварНаХраненииСПравомПродажи %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Contact software developers.'");
		ИначеЕсли ВыборкаОшибок.ТипОшибки = "ОшибкаТипаНоменклатуры" Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: пишется резерв не по товару или многооборотной таре по измерениям %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Обратитесь к разработчикам программы.';
									|en = 'An error occurred in the algorithms while writing  %Регистратор% % to the РезервыТоваровОрганизаций register: reserve is written not for goods or reusable packaging %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%. Contact application developers.    '");
		Иначе
			ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах записи %Регистратор% в регистр РезервыТоваровОрганизаций: не классифицированная ошибка по измерениям %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%';
									|en = 'An error occurred in the algorithms while writing  %Регистратор% to the РезервыТоваровОрганизаций register: unclassified error in dimensions %АналитикаНоменклатуры%-%Организация%-%ВидЗапасов%-%НомерГТД%'");
		КонецЕсли;
		...
		Если Не ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(
			...
			Если НераспределенноеСторно <> 0 Тогда
				ТекстИсключения = НСтр("ru = 'Ошибка в алгоритмах при записи %Регистратор% в регистр РезервыТоваровОрганизаций: образуется сторно, которое не нужно для покрытия резервов по изменениям: %Организация% - %АналитикаНоменклатуры% - %ВидЗапасов% - %НомерГТД%, лишнее сторно %НераспределенноеСторно%. Обратитесь к разработчикам программы.';
										|en = 'An error occurred in the algorithms while writing  %Регистратор% % to the РезервыТоваровОрганизаций register: storno is formed which is not required to cover change reserves: %Организация% - %АналитикаНоменклатуры% - %ВидЗапасов% - %НомерГТД%, excessive  storno %НераспределенноеСторно%. Contact the application developers.   '");
