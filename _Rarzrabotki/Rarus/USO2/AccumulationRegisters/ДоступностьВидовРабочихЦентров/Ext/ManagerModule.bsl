...
// Заполняет движения документа по регистру "Доступность видов рабочих центров".
//
// Параметры:
...
Процедура ПроверитьЗагрузкаНеПревышаетДоступностьГрафика(Интервалы) Экспорт
	...
	|		И ВЫРАЗИТЬ(Доступность.Регистратор КАК Документ.ЗаказНаПроизводство).Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство.Закрыт)) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
		...
		Пока ВыборкаДатаИнтервала.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.Загрузка > Доступность Тогда
					...
					Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
