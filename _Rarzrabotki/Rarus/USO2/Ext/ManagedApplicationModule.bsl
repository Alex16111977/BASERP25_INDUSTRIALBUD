...
Перем СметнаяКомпонента Экспорт; // Объект для работы с Внешней нормативной базой (COM объект клиентской библиотеки).
Перем СМ_ВерсияКомпоненты Экспорт; // Версия библиотеки для работы с Внешней нормативной базой.
Перем глТекущийПользователь Экспорт; // Текущий пользователь (для блокировок сметной подсистемы).
Перем ГлобальнаяСтрокаДереваСмет Экспорт; // Текущая рабочая смета (для сметной подсистемы).
// Конец Эрикос.СметнаяПодсистема
