	...
	Если ЗначениеЗаполнено(ПутевойЛист_Заправки_ТС) Тогда
		Если ПутевойЛист_Заправки_ВидЗаправки <> ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаТалоны") Тогда
			...
			Если ПутевойЛист_Заправки_ВидЗаправки =	ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаПластиковаяКарта")
				...
				ПутевойЛист_Заправки_ПластиковаяКарта = уатОбщегоНазначения.уатПластиковаяКартаТСВодителя(ПутевойЛист_Заправки_ТС, Водитель,ПутевойЛист_ВремяВыезда);
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьЗаполнениеПростоев()
	...
	Для НомерПростоя = 1 По 6 Цикл
		...
		Если ПутевойЛистДатаВремяВыезда() <> '00010101' И ПутевойЛистДатаВремяВозвращения() <> '00010101'
			...
			ТекстНСТР = НСтр("en='The downtime period in line %1 overrun of waybill';ru='Период простоя в строке %1 выходит за границы путевого листа'");
			НадписьНекорректноеЗаполнениеПростоев = СтрШаблон(ТекстНСТР, НомерПростоя);
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура КнопкаКалендарь(Команда)
	ОбработкаНажатияНаКнопку("К");
КонецПроцедуры
...
Процедура ВыбратьПластиковуюКарту(Команда)
	...
	ОткрытьФорму("Обработка.уатТерминалПриемаИВыдачиПутевыхЛистов.Форма.ФормаВыбораСправочников",
		Новый Структура("ИмяОбъектаВыбора, СписокОбъектов, Заголовок", "Список", уатОбщегоНазначения.уатСписокТопливныхКарт(СписокВладельцев, ПутевойЛист_ВремяВыезда), НСтр("en = 'Choose fuel card'; ru = 'Выберите топливную карту'")),
		ЭтаФорма,,,, ОповещениеВыбора, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура Завершить(Команда)
	...
	Строка2 = ?(ЗначениеЗаполнено(ПутевойЛист_СпидометрВозвращенияВодитель), Новый ФорматированнаяСтрока(" (" + ПутевойЛист_СпидометрВозвращенияВодитель + ")", ,Новый Цвет(0,150,70)), Новый ФорматированнаяСтрока(""));
	Элементы.ПутевойЛист_СпидометрВозвращенияСтрока.Заголовок  = Новый ФорматированнаяСтрока(Новый ФорматированнаяСтрока(Строка(ПутевойЛист_СпидометрВозвращения)), Строка2);
	
	Строка2 = ?(ЗначениеЗаполнено(ПутевойЛист_ОстатокТопливаВозвратВодитель), Новый ФорматированнаяСтрока(" (" + ПутевойЛист_ОстатокТопливаВозвратВодитель + ")", ,Новый Цвет(0,150,70)), Новый ФорматированнаяСтрока(""));
	Элементы.ПутевойЛист_ОстатокТопливаВозвратСтрока.Заголовок = Новый ФорматированнаяСтрока(Новый ФорматированнаяСтрока(Строка(ПутевойЛист_ОстатокТопливаВозврат)), Строка2);
	Элементы.ПанельСЦифрами.Видимость = Ложь;
	Элементы.ГруппаПанельИнформации.Видимость = Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗавершено;
	УстановитьТаймерРаботыСоСтраницей();
...
&НаСервереБезКонтекста
Функция ДокументыТСВодителя(Водитель, ТС, ТекДата) Экспорт
	Рез = Новый СписокЗначений;
	
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СписокТСДляТО.ВидРемонта КАК ВидОбслуживания,
	|	СписокТСДляТО.ДатаРемонта КАК ДатаТО
	...
	|	РегистрСведенийуатДокументыВодителей.Категория КАК Категория,
	|	ВЫБОР
...
Процедура ЗаполнитьСписокТипов()
	...
	НовТип = ТипыЭлементовФормы.Добавить(); НовТип.ИмяЭлемента = "ПутевойЛист_Прицепы_Прицеп1"; НовТип.ТипЭлемента = "СправочникСсылка.ТранспортныеСредства";
	НовТип = ТипыЭлементовФормы.Добавить(); НовТип.ИмяЭлемента = "ПутевойЛист_Прицепы_Прицеп2"; НовТип.ТипЭлемента = "СправочникСсылка.ТранспортныеСредства";
	НовТип = ТипыЭлементовФормы.Добавить(); НовТип.ИмяЭлемента = "ПутевойЛист_Прицепы_Прицеп3"; НовТип.ТипЭлемента = "СправочникСсылка.ТранспортныеСредства";
	НовТип = ТипыЭлементовФормы.Добавить(); НовТип.ИмяЭлемента = "ПутевойЛист_Прицепы_Прицеп4"; НовТип.ТипЭлемента = "СправочникСсылка.ТранспортныеСредства";
	...
	НовТип = ТипыЭлементовФормы.Добавить(); НовТип.ИмяЭлемента = "ПутевойЛист_Заправки_ТС"; НовТип.ТипЭлемента = "СправочникСсылка.ТранспортныеСредства";
	...
	НовТип = ТипыЭлементовФормы.Добавить(); НовТип.ИмяЭлемента = "ПутевойЛист_Заправки_ПластикКарта"; НовТип.ТипЭлемента = "СправочникСсылка.уатТопливныеКарты";
...
Процедура УстановитьОграниченияДляРолиВодитель()
	Если НЕ РольДоступна("ПолныеПрава")
		И НЕ РольДоступна("уатДиспетчер") Тогда
...
Функция ПроверитьЗаполнениеЗаправкиПЛ()
	...
	Для Каждого ТекОбязРекв Из мсвОбязРеквизиты Цикл
		Если НЕ ЗначениеЗаполнено(ЭтаФорма[ТекОбязРекв]) И НЕ Элементы["Декорация" + ТекОбязРекв].Видимость Тогда
			Элементы["Декорация" + ТекОбязРекв].Видимость = Истина;
		ИначеЕсли ЗначениеЗаполнено(ЭтаФорма[ТекОбязРекв]) И Элементы["Декорация" + ТекОбязРекв].Видимость Тогда
...
Процедура ПриИзмененииВидаЗаправки()
	Если ПутевойЛист_Заправки_ВидЗаправки = ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаПластиковаяКарта")
		...
		ПутевойЛист_Заправки[ПутевойЛист_Заправки_НомерСтроки-1].ГСМ = ПутевойЛист_Заправки_ГСМ;;
		ПутевойЛист_Заправки_ПластиковаяКарта = уатОбщегоНазначения.уатПластиковаяКартаТСВодителя(ПутевойЛист_ТС, Водитель, ,ПутевойЛист_ВремяВыезда);
		ПутевойЛист_Заправки[ПутевойЛист_Заправки_НомерСтроки-1].ПластиковаяКарта = ПутевойЛист_Заправки_ПластиковаяКарта;
	Иначе
...
Процедура ОбновитьИнформациюГотово()
	...
	Для Сч = 1 По 4 Цикл
		Если Элементы["Прицеп" + Сч].Видимость Тогда
			стрПрицепы = стрПрицепы + ", " + Строка(ЭтаФорма["ПутевойЛист_Прицепы_Прицеп" + Сч]);
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПолучитьПлановуюДатуТО(ТС, ДатаТО, ДатаРаботы, ПараметрВыработки, ПериодичностьОбслуживания, КоличествоПериодов, Норма, ВыработкаПослеТО) Экспорт
	
	Если ЗначениеЗаполнено(ДатаТО) Тогда
		ДатаПоследнегоТО = ДатаТО;
	Иначе 
		ДатаПоследнегоТО = ТС.ДатаВводаВЭксплуатацию;
	КонецЕсли;
	...
	Иначе
		тКолвоДней = (НачалоДня(ДатаРаботы) - НачалоДня(ДатаПоследнегоТО)) / 86400;
		тСрПробег  = ?(тКолвоДней = 0, 0, ВыработкаПослеТО / тКолвоДней);
		тПланДата  = ?(тСрПробег = 0, '00010101', НачалоДня(ДатаПоследнегоТО) + (Норма / тСрПробег * 86400));
