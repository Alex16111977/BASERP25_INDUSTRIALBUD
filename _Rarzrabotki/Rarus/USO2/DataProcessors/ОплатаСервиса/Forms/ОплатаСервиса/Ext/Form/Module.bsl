...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    СвойстваВерсии = ПрограммныйИнтерфейсСервисаПовтИсп.СвойстваВерсииИнтерфейса();
    Если СвойстваВерсии.Версия < 5 Тогда
        ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Автоматическая оплата сервиса не поддерживается.';
																|en = 'Automatic payment for the service is not supported.'"),,,, Отказ);
        Возврат;
    КонецЕсли; 
    ...
    Если Абонент.РольПользователя <> Перечисления.РолиПользователейАбонентов.ВладелецАбонента Тогда
        ПоказатьОшибку(НСтр("ru = 'Оплата сервиса доступна только пользователю с ролью ''Владелец абонента''.';
							|en = 'Payment for the service is available only for the user with the role ''Subscriber owner''.'"));
    КонецЕсли;
    
    ОбслуживающиеОрганизации.Загрузить(ПрограммныйИнтерфейсСервисаПовтИсп.ОбслуживающиеОрганизацииАбонента());
...
Процедура ВсеТарифы(Команда)
    ...
    ОткрытьФорму("Обработка.ОплатаСервиса.Форма.ВыбораТарифа", ПараметрыФормы, ЭтаФорма,,,,Оповещение);
...
Процедура ПоказатьПодписки(СоздаватьВариантыПродления)
    ...
    Для Каждого Подписка Из ОсновныеПодписки Цикл
        ...
        Если Не ЗначениеЗаполнено(ТарифПодписки) Тогда
            ТарифПодписки = ПрограммныйИнтерфейсСервисаПовтИсп.ТарифСервиса(Подписка.КодТарифа); 
...
Процедура СоздатьЭлементыПодписки(Группа, СоздаватьВариантыПродления, ЦветФонаПодписки, РодительскаяГруппа)
    ...
    ИмяГруппаРасшишение = Префикс + "ГруппаРасширение";
    ...
    ДекорацияТариф = Элементы.Добавить(ИмяТариф, Тип("ДекорацияФормы"), ГруппаТариф);
    ...
    ДекорацияСрокДействия.Заголовок = ?(Группа.ДатаПодключения > ТекущаяДатаСеанса(), 
        СтрШаблон(НСтр("ru = 'С %1
                       |до %2';
                       |en = 'Since %1
                       | till %2'"), Формат(Группа.ДатаПодключения, "ДФ='d MMMM yyyy ''г.'''"),
    ...
    Для Каждого СтрокаРасширения Из Расширения Цикл
        ...
        ГруппаРасширение = Элементы.Добавить(ИмяГруппаРасшишение + РасширенияГруппы.Индекс(СтрокаРасширения), Тип("ГруппаФормы"), ГруппаРасширения);
	...
	Если Группа.ПериодыПродления.Количество() = 0 Тогда
        ТекстПродления = НСтр("ru = 'Эту подписку нельзя продлить.';
								|en = 'This subscription cannot be extended.'"); 
    ...
    ТекстПродления = ?(Группа.ПериодыПродления.Количество() > 0, НСтр("ru = 'Продлить:';
																		|en = 'Extend:'"), НСтр("ru = 'Эту подписку нельзя продлить.';
																								|en = 'This subscription cannot be extended.'")); 
    ДекорацияПродлитьНа = Элементы.Добавить(ИмяПродлитьНа, Тип("ДекорацияФормы"), ГруппаОсновная); 
    ДекорацияПродлитьНа.Вид = ВидДекорацииФормы.Надпись;
    ДекорацияПродлитьНа.Заголовок = ТекстПродления;
...
Процедура Подключаемый_ОткрытьОписание(Команда)
    ...
    ОткрытьФорму("Обработка.ОплатаСервиса.Форма.ОписаниеТарифа", ПараметрыФормы, ЭтаФорма);
...
Процедура ОткрытьФормуОбработкиДанных()
    ...
    ОткрытьФорму(ИмяФормыОбработкиОтвета, ПараметрыФормы, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция ВыставитьСчетПоВыбранномуТарифу(КодТарифа, КодПериода)
    ...
    Если ИспользоватьТарифыОО Тогда
        ТарифОО = ПрограммныйИнтерфейсСервисаПовтИсп.ТарифОбслуживающейОрганизации(КодОО, КодТарифа);
        Если Не ЗначениеЗаполнено(ТарифОО) Тогда
            ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                НСтр("ru = 'Не найден тариф обслуживающей организации ''%1'' по коду тарифа ''%2''.';
					|en = 'The service company tariff ''%1'' is not found upon the tariff code ''%2''.'"), КодОО, КодТарифа);
        КонецЕсли;
        Тариф = ПрограммныйИнтерфейсСервисаПовтИсп.ТарифСервиса(ТарифОО.КодБазовогоТарифа);
    Иначе
        Тариф = ПрограммныйИнтерфейсСервисаПовтИсп.ТарифСервиса(КодТарифа);
        Если Не ЗначениеЗаполнено(Тариф) Тогда
            ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                НСтр("ru = 'Не найден тариф сервиса по коду тарифа ''%1''.';
					|en = 'The service tariff is not found upon the tariff code ''%1''.'"), КодТарифа);
        КонецЕсли; 
    КонецЕсли; 
...
Процедура ПриИзмененииОбслуживающейОрганизации()
    ...
    Если Не ТекущаяОО.РазрешеноАвтоматическоеВыставлениеСчетов Тогда
        ПоказатьОшибку(НСтр("ru = 'Для продления доступа к сервису обратитесь к обслуживающей организации.';
							|en = 'Call the servicing company to extend the access to the service.'"));
    КонецЕсли; 
