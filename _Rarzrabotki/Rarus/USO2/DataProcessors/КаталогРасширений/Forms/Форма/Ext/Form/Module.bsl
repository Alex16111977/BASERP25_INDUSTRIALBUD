...
&НаКлиенте
Перем СоответсвиеТиповРазделам;
...
Процедура ПриОткрытии(Отказ)
	...
	СоответсвиеТиповРазделам = Новый Соответствие;
...
Процедура НачатьПодключениеКМенеджеруСервиса()
	...
	ПоказатьОповещениеОЗагрузкеДанных(НСтр("ru = 'Подключение к Менеджеру сервиса...';
											|en = 'Connect to the Service Manager...'"));
	
	Если Результат Тогда
		ПодключитьОбработчик();
	Иначе
		КодОшибки = 999;
		ТекстОшибки = НСтр("ru = 'Не удалось подключиться к Менедджеру сервиса.
							|Повторите попытку позднее или обратитесь к админисратору сервиса.';
							|en = 'Cannot connect to the Service Manager.
							|Try later or call the service admin.'");
...
Процедура ОбработатьРезультатВыполненияЗапросаКМС()
	...
	Если НЕ ДанныеХранилища.success Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось выполнить запрос';
										|en = 'Cannot complete the query'") + Символы.ПС +
			НСтр("ru = 'Код ошибки: %1';
				|en = 'Error code: %1'") + Символы.ПС +
			НСтр("ru = 'Описание: %2';
				|en = 'Description: %2'"), ДанныеХранилища.error.code, ДанныеХранилища.error.msg);	
	...
	Если ДанныеХранилища.ИмяМетода = "ПолучитьСписокДоступныхРасширений" 
		...
		Если НЕ ЕстьЕщеРасширения Тогда
			ПоказатьОповещениеОЗагрузкеДанных(НСтр("ru = 'Это все достуные расширения.';
													|en = 'They are all available extensions.'"));
		КонецЕсли;
...
Процедура ОбновитьСписокРасширений(ЗагрузитьЕще = Ложь, ЭтоПоиск = Ложь, Знач СтрокаАвтоПодбора = Неопределено)
	...
	Если НЕ ОбновитьНаСервере(
						ЗагрузитьЕще,
						ЭтоПоиск,
						СоответсвиеТиповРазделам.Получить(ВариантСписка),
...
Функция ОбновитьНаСервере(ЗагрузитьЕще, ЭтоПоиск, МассивТиповДоступа, АдресХранилищаРезультат, ИдентификаторПоследнегоРасширения, ТолькоУстановленные, СтрокаПоиска)
	...
	Если ЭтоПоиск И ПустаяСтрока(СтрокаПоиска) Тогда
		Возврат КаталогРасширений.ПолучитьСписокДоступныхРасширенийВФоне(АдресХранилищаРезультат, ИдентификаторПоследнегоРасширения, МассивТиповДоступа,,, ТолькоУстановленные);
	ИначеЕсли ЭтоПоиск ИЛИ (ЗагрузитьЕще И НЕ ПустаяСтрока(СтрокаПоиска)) Тогда
		Возврат КаталогРасширений.ПолучитьСписокНайденныхРасширенийВФоне(АдресХранилищаРезультат, ИдентификаторПоследнегоРасширения, МассивТиповДоступа, СтрокаПоиска,,, ТолькоУстановленные);
	Иначе
		Возврат КаталогРасширений.ПолучитьСписокДоступныхРасширенийВФоне(АдресХранилищаРезультат, ИдентификаторПоследнегоРасширения, МассивТиповДоступа,,, ТолькоУстановленные);	
	КонецЕсли;
...
Функция ЗагрузитьДанныеРасширений(Знач МассивРасширений)
	...
	СтрокиЕще = Расширения.НайтиСтроки(Новый Структура("ПоказатьСледующиеРасширения, Раздел", НСтр("ru = 'Показать еще';
																									|en = 'Show more'"), ВариантСписка));
	...
	Для Каждого ДанныеРасширения Из МассивРасширений Цикл
		...
		НоваяСтрока.НаименованиеВерсии = СтрШаблон(НСтр("ru = 'Версия: %1';
														|en = 'Version: %1'"), НоваяСтрока.НаименованиеВерсии);
		НоваяСтрока.Разработчик = СтрШаблон(НСтр("ru = 'Разработчик: %1';
												|en = 'Developer: %1'"), НоваяСтрока.Разработчик);
...
Функция ПодключитьсяКМенеджеруСервиса()
	...
	АдресХранилищаРезультат = ПоместитьВоВременноеХранилище(Неопределено, АдресХранилищаРезультат);
	Возврат КаталогРасширений.ПолучитьСписокРазделовВФоне(АдресХранилищаРезультат);
...
Процедура ПоказатьРазделы(ДанныеХранилища)
	...
	Для Каждого ДанныеРаздела Из ДанныеХранилища.result Цикл
		...
		СоответсвиеТиповРазделам.Вставить(ДанныеРаздела.name, ДанныеРаздела.types);	
...
Функция СостояниеРасширения(Знач Состояние)
	Возврат СтрШаблон(НСтр("ru = 'Состояние: %1';
							|en = 'State: %1'"), Состояние);
КонецФункции
