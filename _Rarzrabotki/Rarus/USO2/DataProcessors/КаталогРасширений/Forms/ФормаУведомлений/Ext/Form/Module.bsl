	...
	Для Каждого Оповещение Из ДанныеДляОповещения Цикл
		...
		Если Оповещение.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияРасширений.Установлено") Тогда
			Текст = НСтр("ru = 'Установка расширения завершена';
						|en = 'The extension is installed'");
			СостояниеДоп = НСтр("ru = 'установлено';
								|en = 'installed'");
		Иначе 
			Текст = НСтр("ru = 'Удаление расширения завершено';
						|en = 'The extension is deleted'");	
			СостояниеДоп = НСтр("ru = 'удалено';
								|en = 'deleted'");
		КонецЕсли;
		
		Пояснение = СтрШаблон(НСтр("ru = 'Расширение ""%1"" %2, необходимо перезапустить приложение';
									|en = 'The extension ""%1"" %2, the application must be restarted'"), Оповещение.Расширение, СостояниеДоп);
...
Процедура ОбработатьПерезапуск(Оповещение) Экспорт 
	...
	Вопрос = СтрШаблон(НСтр("ru = 'Расширение ""%1"" %2, перезапустить приложение?';
							|en = 'The extension ""%1"" %2, restart the application?'"), Оповещение.Расширение, СостояниеДоп);
	...
	ПоказатьВопрос(Описание, Вопрос, РежимДиалогаВопрос.ДаНет,,, НСтр("ru = 'Перезапуск приложения';
																		|en = 'Restarting of the application'"));
