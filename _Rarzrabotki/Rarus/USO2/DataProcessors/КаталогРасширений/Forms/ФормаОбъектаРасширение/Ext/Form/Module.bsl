...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ КаталогРасширений.ПолучитьДанныеВерсииРасширенияВФоне(АдресХранилищаРезультат, Параметры.ИдентификаторВерсии) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не удалось отправить запрос в МС.';
																|en = 'Cannot send the request to MS.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура АдресИнформацииОПоставщикеНажатие(Элемент)
	ПерейтиПоНавигационнойСсылке(АдресИнформацииОПоставщике);
КонецПроцедуры

&НаКлиенте
Процедура АдресИнформацииОКонфигурацииНажатие(Элемент)
	ПерейтиПоНавигационнойСсылке(АдресИнформацииОКонфигурации);
КонецПроцедуры
...
Процедура ВыполнитьДействие(Команда)
	...
	ТекстВопроса = СтрШаблон(НСтр("ru = '%1 расширение ""%2""?';
									|en = '%1 extension ""%2""?'"), ПервоеСлово, НаименованиеРасширения);
	
	Если ЗначениеЗаполнено(ДополнительныйТекстПриУстановке) Тогда
...
Процедура Оценить(Команда)
	...
	Если Тек_Оценка = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите оценку';
																|en = 'Choose an assessment'"),, "ПиктограммаОценка_3");	
		Возврат;	
	КонецЕсли;
...
Процедура УдалитьОтзыв(Команда)
	...
	ПоказатьВопрос(Описание, НСтр("ru = 'Удалить вашу оценку и отзыв?';
									|en = 'Delete your assessment and review?'"), РежимДиалогаВопрос.ДаНет,,, НСтр("ru = 'Удаление оценки';
																											|en = 'Removal of an assessment'"));
...
Процедура ОбработатьРезультатВыполненияЗапросаКМС()
	...
	ИначеЕсли ДанныеХранилища.ИмяМетода = "ИзменитьОценкуПользователя" Тогда
		Тек_ДатаРедактирования = ТекущаяДата();
...
Процедура ОбработатьОтветДействие(Ответ, ЭтоУстановка) Экспорт 
	...
	Если ЭтоУстановка Тогда
		Результат = УстановитьНаСервере(АдресХранилищаРезультат, ИдентификаторРасширения);
	Иначе
		Результат = УдалитьНаСервере(АдресХранилищаРезультат, ИдентификаторРасширения);	
	КонецЕсли;
...
Функция УдалитьНаСервере(АдресХранилищаРезультат, ИдентификаторРасширения)
	
	АдресХранилищаРезультат = ПоместитьВоВременноеХранилище(Неопределено, АдресХранилищаРезультат);
	
	КаталогРасширений.УстановитьОповещение(ИдентификаторРасширения, ПользователиКлиентСервер.ТекущийПользователь(), Перечисления.СостоянияРасширений.Удаляется);
	
	Возврат КаталогРасширений.НачатьУдалениеРасширенияВФоне(АдресХранилищаРезультат, ИдентификаторРасширения);
...
&НаСервереБезКонтекста
Функция УстановитьНаСервере(АдресХранилищаРезультат, ИдентификаторРасширения)
	
	АдресХранилищаРезультат = ПоместитьВоВременноеХранилище(Неопределено, АдресХранилищаРезультат);
	
	КаталогРасширений.УстановитьОповещение(ИдентификаторРасширения, ПользователиКлиентСервер.ТекущийПользователь(), Перечисления.СостоянияРасширений.Устанавливается);
	
	Возврат КаталогРасширений.НачатьУстановкуРасширенияВФоне(АдресХранилищаРезультат, ИдентификаторРасширения);
...
Процедура УстановитьЧислоОценок(Точка, Серия, НомерОценки, Количество)
	...
	Подсказка = СтрШаблон(НСтр("ru = 'Оценок %1: %2';
								|en = 'Assessments %1: %2'"), НомерОценки, Количество);	
...
Процедура ОтобразитьДанныеОценокИОтзывов(ИнформацияОРасширении)
	...
	Статистика_КоличествоУстановок = ИнформацияОРасширении.ПредствалениеУстановок;
	...
	Для Каждого ДанныеОценки Из ИнформацияОРасширении.Оценки Цикл
		...
		Если НЕ ПустаяСтрока(НоваяСтрока.Версия) Тогда
			НоваяСтрока.ПредставлениеВерсии = СтрШаблон(НСтр("ru = 'Оценка для версии: %1';
															|en = 'Assessment for the version: %1'"), НоваяСтрока.Версия);
		Иначе
			НоваяСтрока.ПредставлениеВерсии = НСтр("ru = 'Оценка без указания версии';
													|en = 'Assessment without specification of a version'");
		КонецЕсли;
