...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьАвтоГРАФ.Получить() Тогда
		ТекстСообщения = НСтр("en='Opening is possible only when enabled using GPS system for AutoGRAF.';ru='Открытие возможно только тогда, когда включено использование системы GPS - АвтоГРАФ.'");
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	// Выполним загрузку
	Если Не Отказ Тогда 
		ТекстНСТР = НСтр("en='Load data of the GPS AutoGRAPH system';ru='Выполняется загрузка данных системы GPS АвтоГРАФ'");
...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	КодВозврата = уатЗащищенныеФункцииСервер_проф.ПолучитьУчетнуюЗаписьСистемыМониторингаДляТС(МассивТС, Перечисления.уатСистемаGPS.АвтоГРАФ);
	...
	Для Каждого ТекСтрока Из МассивТС Цикл
		...
		Если КодВозврата = 0 И ЗагрузитьПробегИРасходГСМ Тогда // Загружаем пробег и расход ГСМ.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТСАвтоГРАФ(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // Загружаем дополнительные сведения.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияПоТСАвтоГРАФ(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата=0 И ЗагрузитьКоординаты Тогда // Загружаем дополнительные сведения.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьКоординатыПоТСАвтоГРАФ(ДатаНачала, ДатаОкончания, МассивТС, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
