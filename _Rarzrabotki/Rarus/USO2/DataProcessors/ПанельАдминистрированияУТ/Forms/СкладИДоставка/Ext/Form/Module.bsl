...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокОтключения = НСтр("ru = 'Перед отключением обособленного обеспечения, необходимо отключить учет затрат по всем направлениям деятельности.';
								|en = 'Disable cost accounting for all lines of business before disabling pegged supply.'");
	//++ НЕ УТКА
	ЗаголовокОтключения = НСтр("ru = 'Перед отключением обособленного обеспечения, необходимо отключить:
		...
		|en = 'Before disabling pegged supply, disable: 
		|– Usage of own production (in Master data and settings – Manufacturing)
		|– Usage of toll manufacturing (in Master data and settings – Sales)'");
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетЗатратПоНаправлениямДеятельности") Тогда
		ЗаголовокОтключения = НСтр("ru = 'Перед отключением обособленного обеспечения, необходимо отключить:
			...
			|– Usage of own production (in Master data and settings – Manufacturing)
			|– Usage of toll manufacturing (in Master data and settings – Sales)
			|Also disable cost accounting in all lines of business.'");
	КонецЕсли;
	//-- НЕ УТКА
	...
	Элементы.КомментарийОбеспечениеРЦ.Заголовок =
		НСтр("ru = 'Этот функционал доступен, если разрешено использование хотя бы одного из следующих видов документов:
			...
			|en = 'This functionality is available if using at least one of the following document kinds is allowed:
			|- ""Transfer order"" (the ""Reference information and administration"" section — ""Warehouse and delivery"")
			|- ""Material order for production"" (the ""Reference information and administration"" section —""Production"")'");
	//-- НЕ УТ
...
Процедура УстановитьЗаголовкиИПодсказкиДляУТКА()
	...
	ПодстановкаСпособы = НСтр("ru = 'покупка, перемещение, сборка';
								|en = 'purchase, transfer, assembly'");
//++ НЕ УТКА
	ПодстановкаСпособы = НСтр("ru = 'покупка, перемещение, сборка, производство';
								|en = 'purchase, transfer, assembly, production'");
//-- НЕ УТКА
	ЗаголовокЭлемента = НСтр("ru = 'Формирование заказов в соответствии со способом обеспечения (%1) для поддержания остатка (min-max) и обеспечения заказов на отгрузку. Расчет потребностей с учетом плановых сроков поставки и интервала между поставками.';
							|en = 'Order generation in accordance with fulfillment method (%1) for replenishing inventory (min-max) and fulfilling shipment orders. Calculation of demands on the basis of scheduled delivery due dates and interval between deliveries.'");
	...
	Элементы.КомментарийРасширенноеФормированиеЗаказовПоПотребностям.Заголовок = 
		...
		|en = 'This functionality is available if production management is used (in Master data and settings — Manufacturing)
		|or if using at least one of the following document kinds is allowed:
		|– Purchase orders (in Master data and settings — Purchases)
		|– Assembly orders (in Master data and settings — Warehouse and delivery)
		|– Transfer orders (in Master data and settings — Warehouse and delivery)'");
	//-- НЕ УТКА
