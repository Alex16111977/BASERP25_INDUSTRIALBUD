...
Процедура ПриОткрытии(Отказ)
	...
	ДополнитьПараметрыПроверкиКодовМаркировки(Отказ);
	
	Если Отказ Тогда
		Возврат;
	Иначе
		УточнитьНеобходимостьОбновленияКлючаСессии();
	КонецЕсли;
	...
	ИначеЕсли ТребуетсяОбновлениеКлючаСессии Тогда
		ПодключитьОбработчикОжидания("ЗапроситьКлючСессииНачало", 0.6, Истина);
	Иначе
...
Процедура ДекорацияНедопустимыеКодыМаркировкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ИзменитьОтборНедопустимыеКодыМаркировки" Тогда
		...
		Если УстановленОтборНедопустимыеКодыМаркировки Тогда
			УстановитьОтборПоНедопустимымКодамМаркировки();
		Иначе
			ПроверитьПризнакУстановкиОтбораВДереве(ЭтотОбъект);
		КонецЕсли;
			...
			Если ТекущиеДанные.ТипУпаковки = ПредопределенноеЗначение("Перечисление.ПрочиеЗоныПересчетаТабачнойПродукцииМОТП.ПачкиБезБлока")
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Удалять можно только строки в статусе ""Не числилось"", упаковки верхнего уровня, пачки без блоков или блоки без коробки.';
						|en = 'Удалять можно только строки в статусе ""Не числилось"", упаковки верхнего уровня, пачки без блоков или блоки без коробки.'"),,,
					"ДеревоМаркированнойПродукции",
					Отказ);
...
//		Элемент.ПодчиненныеЭлементы.ПодобраннаяТабачнаяПродукцияХарактеристика.ТолькоПросмотр = ТекущиеДанные.НоменклатураСопоставлена;
//	КонецЕсли;
...
Процедура ОткрытьПодборСерий(Текст = "", ТекущиеДанные = Неопределено)
	...
	ИнтеграцияИСКлиент.ОткрытьПодборСерий(
		ЭтотОбъект, ПараметрыУказанияСерий, Текст, Ложь, ТекущиеДанные);
...
#Область УстановкаОтбораНедопустимыеКодыМаркировки

&НаКлиенте
Процедура УстановитьОтборПоНедопустимымКодамМаркировки()
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		СоответствуетОтбору = Ложь;
		УстановитьОтборПоНедопустимымКодамМаркировкиВСтрокеДерева(СтрокаДерева, СоответствуетОтбору);
...
&НаКлиенте
Процедура УстановитьОтборПоНедопустимымКодамМаркировкиВСтрокеДерева(Знач СтрокаДерева, СоответствуетОтбору)
	...
	Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
		...
		УстановитьОтборПоНедопустимымКодамМаркировкиВСтрокеДерева(ПодчиненнаяСтрока, СоответствуетОтбору);
...
Процедура ОтметитьСтрокуКакНайденную(СтрокаДерева)
	...
	ИначеЕсли СтрокаДерева.НедопустимыйКодМаркировки Тогда
		...
		ДанныеШтрихкода.Вставить("ТекстОшибки",               СтрокаДерева.ПредставлениеПроверкиКодаМаркировки);
		ДанныеШтрихкода.Вставить("ЗначениеШтрихкода",         СтрокаДерева.ЗначениеШтрихкода);
		ДанныеШтрихкода.Вставить("Номенклатура",              СтрокаДерева.Номенклатура);
		ДанныеШтрихкода.Вставить("ПредставлениеНоменклатуры", СтрокаДерева.ПредставлениеСодержимоеУпаковки);

		ПараметрыОткрытияФормы = Новый Структура();
		ПараметрыОткрытияФормы.Вставить("ДанныеШтрихКода", ДанныеШтрихкода);

		ОткрытьФорму("Обработка.ПроверкаИПодборТабачнойПродукцииМОТП.Форма.ИнформацияОНевозможностиДобавленияОтсканированного",
			ПараметрыОткрытияФормы, ЭтотОбъект, УникальныйИдентификатор,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПересчитатьИтогиПоПроверкеСодержимогоУпаковкиПриИзмененииСтроки(СтрокаДерева)
	...
	РодительскаяСтрока = СтрокаДерева.ПолучитьРодителя();
	Пока РодительскаяСтрока <> Неопределено Цикл
		ПроверкаИПодборПродукцииМОТПКлиентСервер.ПересчитатьИтогиПоПроверкеСодержимогоУпаковки(РодительскаяСтрока, Ложь);
		РодительскаяСтрока = РодительскаяСтрока.ПолучитьРодителя();
	КонецЦикла;
...
Процедура ОбновитьВыводимоеПредставлениеПроверкиСодержимого(Форма, ТекущиеДанные)
	...
	Если ТекущиеДанные = Неопределено
		Или Не ИнтеграцияИСКлиентСервер.ЭтоУпаковка(ТекущиеДанные.ТипУпаковки) Тогда
...
Процедура ПересчитатьВсеИтогиФормы()
	...
	ПроверкаИПодборПродукцииМОТПКлиентСервер.ПересчитатьИтогиПоПроверкеСодержимогоУпаковок(ДеревоМаркированнойПродукции);
	ПересчитатьХешСуммыВсехУпаковок();
	ПроверкаИПодборПродукцииМОТПКлиентСервер.ОпределитьТипыВсехУпаковок(ДеревоМаркированнойПродукции);
...
Процедура ПересчитатьХешСуммыВсехУпаковок(ИдентификаторСтрокиУпаковки = Неопределено)
	...
	Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Пачки Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокаДерева <> Неопределено Тогда
			...
			ТребуетсяПеремаркировка = Выборка.ТребуетсяПеремаркировка
			                          И Не СтрокаДерева.СтатусПроверки = Перечисления.СтатусыПроверкиНаличияПродукцииИС.Отсутствует;
...
Процедура СформироватьПредставлениеДляСтрокиДереваМаркированнойПродукции(СтрокаДерева)
	...
	ПроверкаИПодборПродукцииМОТПКлиентСервер.СформироватьПредставлениеДляСтрокиДереваМаркированнойПродукции(СтрокаДерева);
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	Если ИспользоватьСерииНоменклатуры Тогда
		Если Не ЗначениеЗаполнено(Склад) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В форму не передан склад.';
																	|en = 'В форму не передан склад.'"),,,,Отказ);
...
&НаКлиенте
Процедура ДополнитьПараметрыПроверкиКодовМаркировки(Отказ)
	
	ПараметрыСканирования = ПараметрыСканированияКодовМаркировки();
	ДопустимыеСтатусыМОТП = Неопределено;
	
	ТребуетсяОбращениеКСервисуМОТП = ПараметрыСканирования.ЗапрашиватьСтатусыМОТП Или ПараметрыСканирования.ЗапрашиватьДанныеНеизвестныхШтрихкодовМОТП;
	...
	Если НЕ Отказ Тогда
		ПараметрыПроверкиКодовМаркировки.Вставить("КонтролироватьСтатусыКодовМаркировки", ПараметрыСканирования.ЗапрашиватьСтатусыМОТП);
		ПараметрыПроверкиКодовМаркировки.Вставить("КонтролироватьВладельца",              ПараметрыСканирования.ЗапрашиватьСтатусыМОТП);
		ПараметрыПроверкиКодовМаркировки.Вставить("ДопустимыеСтатусыКодовМаркировки",     ДопустимыеСтатусыМОТП);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура УточнитьНеобходимостьОбновленияКлючаСессии()
	
	Если Не ПроверкаНеПоДокументу И Не РежимПодбораСуществующихУпаковок И Не ТребуетсяОбращениеКСервисуМОТП Тогда
		ТребуетсяОбращениеКСервисуМОТП = ЕстьВложенностьУпаковокВДокументе(ПроверяемыйДокумент);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЕстьВложенностьУпаковокВДокументе(ПроверяемыйДокумент)
	
	ЕстьВложенностьУпаковок = Ложь;

	ШтрихкодыУпаковок = ШтрихкодированиеИС.ШтрихкодыУпаковокДокумента(ПроверяемыйДокумент);
...
Процедура ЗагрузитьДанныеДокумента()
	...
	Если ДлительнаяОперация <> Неопределено Тогда
		...
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Получение структуры упаковок из ИС МОТП.';
												|en = 'Получение структуры упаковок из ИС МОТП.'");
...
Функция НачатьЗагрузкуДанныхДокумента(ПараметрыСканирования)
	...
	Если ПроверкаНеПоДокументу Тогда
		...
		Если ПроверяемыеДанные.ТаблицаТабачнойПродукции.Количество() = 0
	...
	Иначе
		...
		Если ТаблицаТабачнойПродукции.Количество() = 0 Тогда
			Если РежимПодбораСуществующихУпаковок Тогда
				ВыполнитьДлительнуюОперацию = Ложь;
			Иначе
				ШтрихкодыУпаковокДокумента = ШтрихкодированиеИС.ШтрихкодыУпаковокДокумента(ПроверяемыйДокумент);
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	Элементы.ФормаВключитьОтключитьПроверкуПоВладельцу.Видимость = ПараметрыПроверкиКодовМаркировки.КонтролироватьСтатусыКодовМаркировки;
...
Функция НачатьЗавершениеПроверки(ПараметрыОкончанияПроверки)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Перенос в фоне результатов проверки и подбора в документ.';
															|en = 'Перенос в фоне результатов проверки и подбора в документ.'");
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	...
	Если РежимПодбораСуществующихУпаковок И ЭтоШтрихкодТабачнойПродукции Тогда
		...
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.Номенклатура) Тогда
			...
			ДанныеШтрихкода.Вставить("ТекстОшибки", НСтр("ru = 'Не удалось идентифицировать табачную продукцию';
														|en = 'Не удалось идентифицировать табачную продукцию'"));
			
			ПроверкаИПодборПродукцииМОТПКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, ДанныеШтрихкода);
		...
		Если НЕ ПродукцияСоответствуетДокументуОснованию(ЭтотОбъект, ДанныеШтрихкода) Тогда
			...
			ОткрытьФорму("Обработка.ПроверкаИПодборТабачнойПродукцииМОТП.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытияФормы,
...
Процедура ПроверитьШтрихкодВДеревеМаркированнойПродукции(ДанныеШтрихкода, ОбработкаДанныхТСД = Ложь)
	...
	ИначеЕсли РежимПодбораСуществующихУпаковок
		...
		Если Результат.ЕстьОшибки Тогда
			...
			ПроверкаИПодборПродукцииМОТПКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, Результат.АдресХранилищаДереваУпаковки);
...
Процедура УправлениеВидимостьюКомандУпаковок()
	...
	КомандыВидимы = ДетализацияСтруктурыХранения <> ПредопределенноеЗначение("Перечисление.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Пачки");
...
Процедура Подключаемый_ПолученыДанныеИзТСД(ТаблицаТоваров, ДополнительныеПараметры) Экспорт
	...
	Если ЗагрузкаДанныхТСД.ЕстьОшибкиВДеревеУпаковок Тогда
		...
		ПроверкаИПодборПродукцииМОТПКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, ЗагрузкаДанныхТСД.АдресДереваУпаковок);
...
Функция ПредставлениеРежимаПроверки()
	...
	Если Не ЗначениеЗаполнено(РежимПроверки) Тогда
		...
		РежимПроверки = ПредопределенноеЗначение("Перечисление.ВариантыПроверкиПоступившейПродукцииИС.ОставлятьТамГдеНайдены");
	...
	ИначеЕсли РежимПроверки = ПредопределенноеЗначение("Перечисление.ВариантыПроверкиПоступившейПродукцииИС.ПеремещатьТудаГдеДолжныБыть") Тогда
	...
	Если Не РежимПросмотра Тогда
		СтрокаДействие = Новый ФорматированнаяСтрока(СтрокаДействие,, ЦветаСтиля.ЦветГиперссылкиГосИС,, "ИзменитьРежимПроверки");
	КонецЕсли;
...
Функция ПредставлениеУровняДетализации()
	...
	Если Не ЗначениеЗаполнено(ДетализацияСтруктурыХранения) Тогда
		...
		ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная;
	...
	Если Не РежимПросмотра Тогда
		СтрокаДействие = Новый ФорматированнаяСтрока(СтрокаДействие,, ЦветаСтиля.ЦветГиперссылкиГосИС,, "ИзменитьДетализацию");
	КонецЕсли;
...
Процедура ИзменитьРежимДетализации(НовыйРежимДетализации, СоответствиеШтрихкодовСтрокДерева)
	...
	Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная
		И НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.КоробкиСБлоками Тогда
		
		ТребуетсяПересчет = Истина;
		ПовыситьУровеньДетализацииДоКоробокСБлоками();
		
	ИначеЕсли (ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная
		ИЛИ ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.КоробкиСБлоками)
		И НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.БлокиСПачками Тогда
	...
	ИначеЕсли (ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная
		Или ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.КоробкиСБлоками
		Или ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.БлокиСПачками)
		И НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Пачки Тогда
		
		ТребуетсяПересчет = Истина;
		ПонизитьУровеньДетализацииДоПачки();
		ИдентификаторТекущейПроверяемойУпаковки = - 1;
		
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Пачки
		И НовыйРежимДетализации <> Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Пачки Тогда
	...
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.БлокиСПачками
		И (НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная
		ИЛИ НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.КоробкиСБлоками) Тогда
...
Процедура ПовыситьУровеньДетализацииДоКоробокСБлоками()
	...
	Для Каждого СтрокаДерева Из ЭлементыДерева Цикл
		Если СтрокаДерева.ТипУпаковки <> Перечисления.ПрочиеЗоныПересчетаТабачнойПродукцииМОТП.ПачкиБезБлока Тогда
...
Процедура ПонизитьУровеньДетализацииДоБлоковСПачками()
	...
	Для Каждого СтрокаДерева Из ЭлементыДерева Цикл
		Если СтрокаДерева.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаТабачнойПродукцииМОТП.БлокиБезКоробки
...
Процедура УстановитьУровеньДетализацииВышеЧемПачки(НовыйРежимДетализации)
	...
	Если НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.Полная
	 ИЛИ НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.КоробкиСБлоками Тогда
		ДобавленнаяСтрокаБлокиБезКоробки();
	КонецЕсли;
...
Процедура УстановитьУровеньДетализацииВышеЧемБлокиСПачками(НовыйРежимДетализации)
	...
	Для Каждого БлокКПереносу Из БлокиКПереносу Цикл
		...
		Для Каждого ПачкаКПереносу Из БлокКПереносу.ПолучитьЭлементы() Цикл
			Если НовыйРежимДетализации <> Перечисления.ДетализацияСтруктурыХраненияТабачнойПродукцииМОТП.КоробкиСБлоками Тогда
				СтрокаСПачкой = НоваяСтрока.ПолучитьЭлементы().Добавить();
...
Функция ДобавленнаяСтрокаБлокиБезКоробки()
	...
	ИначеЕсли ЭлементыДерева[0].ТипУпаковки <> Перечисления.ПрочиеЗоныПересчетаТабачнойПродукцииМОТП.ПачкиБезБлока Тогда
		НоваяСтрока = ЭлементыДерева.Вставить(0);
	Иначе
