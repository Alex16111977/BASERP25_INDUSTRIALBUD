...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьDynafleet.Получить() Тогда
		ТекстСообщения = НСтр("en='Opening is possible only when enabled using of GPS system Dynafleet.';ru='Открытие возможно только тогда, когда включено использование системы GPS - Dynafleet.'");
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	Если Не ЗагрузитьПробегИРасходГСМ И Не ЗагрузитьКоординаты Тогда 
		ТекстОшибки = НСтр("ru='Необходимо выбрать хотя бы один параметр для загрузки (пробег и расход/координаты)'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,"ЗагрузитьПробегИРасходГСМ",,Отказ);
	КонецЕсли;
	
	// Выполним загрузку.
	Если Не Отказ Тогда 
		ТекстНСТР = НСтр("en='Load data GPS system Dynafleet';ru='Выполняется загрузка данных системы GPS Dynafleet'");
...
Процедура ВосстановитьНастройки()
	...
	Если ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда 
		СтруктураНастроек = ХранилищеНастроекДанныхФорм.Загрузить(
			"Обработка.уатЗагрузкаДанныхDynafleet", 
			"НастройкиЗагрузки"
		);
	КонецЕсли;
...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	КодВозврата = уатЗащищенныеФункцииСервер_проф.ПолучитьУчетнуюЗаписьСистемыМониторингаДляТС(МассивТС, Перечисления.уатСистемаGPS.Dynafleet);
	...
	Для Каждого ТекСтрока Из МассивТС Цикл
		...
		Если КодВозврата = 0 И ЗагрузитьПробегИРасходГСМ Тогда // Загружаем пробег и расход ГСМ.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТСDynafleet(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата = 0 И ЗагрузитьКоординаты Тогда // Загружаем координаты.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьКоординатыПоТСDynafleet(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
...
Процедура РасширенноеЗаполнениеУстановитьОтбор()
	...
	ОткрытьФорму(
		...
		Новый Структура("ИсточникПодбора", "Обработка_уатЗагрузкаДанныхDynafleet"), 
		ЭтотОбъект,
		,
		,
		,
