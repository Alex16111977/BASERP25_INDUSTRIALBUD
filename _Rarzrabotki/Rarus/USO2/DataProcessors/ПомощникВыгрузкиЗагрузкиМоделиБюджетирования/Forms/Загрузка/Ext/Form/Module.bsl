	...
	Если ПустаяСтрока(ИмяФайлаАрхива) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не указан путь к файлу с данными';
				|en = 'Path to the file with data is not specified'"), , "ИмяФайлаАрхива", "ИмяФайлаАрхива", Отказ);
		Возврат;
	КонецЕсли;
...
Процедура Подключаемый_СтраницаФильтры_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Заголовок = НСтр("ru = 'Выбор загружаемых видов бюджетов и этапов подготовки бюджетов.';
					|en = 'Select imported budget kinds and budget preparation steps.'");
...
Процедура СтраницаПриветствие_ПриПереходеДалее_ПроверкаСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Существует Тогда
		ИзменитьПорядковыйНомерПерехода(-1);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'По указанному пути файл с данными не найден';
				|en = 'File with data was not found by the specified path'"), , "ИмяФайлаАрхива", "ИмяФайлаАрхива");
		Возврат;
	КонецЕсли;
	...
	Элементы.НадписьСтатусЗагрузки.Заголовок = 
		?(СтатусВыполненнойЗагрузки,
			...
			НСтр("ru = 'Загрузка выполненна с ошибками';
				|en = 'Import is completed with errors'"));
...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	Если ЕстьОшибки Тогда
		ИтоговаяИнформация =  НСтр("ru = 'Протокол:';
									|en = 'Protocol:'") + Символы.ПС+ ПротоколОбмена.ПолучитьТекст();
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("Текст",      НСтр("ru = '<Найденное значение недоступно для текущего пользователя из-за ограничений по записям. Обратитесь к администратору базы.>';
																				|en = '<Found value is unavailable for the current user due to record restrictions. Contact infobase administrator.>'"));
	ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаПодсказкиВвода);
		...
		Если НЕ ЗначениеЗаполнено(СтрокаЗаменыАналитики.АналитикаБД)
		   И НЕ СтрокаЗаменыАналитики.ЗагружатьАналитикуИзИсточника Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не заполнено значение аналитики: %1. Загрузка модели не может быть выполнена, выбирите значение аналитики.';
									|en = 'Dimension value is not specified: %1. Cannot import the model; select dimension value.'");
	...
	Для каждого ТипДанных Из ТипыЗагружаемыхДанных Цикл
		...
		Если НЕ ПравоДоступа("Чтение", ОбъектМетаданных) Тогда
			...
			ТекстСообщения = НСтр("ru = 'У текущего пользователя нет прав на чтение таблицы %1, необходимой для загрузки данных по модели. Загрузка будет прервана. Обратитесь к администратору базы.';
									|en = 'The current user has no rights to read the %1 table which is necessary for data import by model. Import will be canceled. Contact infobase administrator.'");
...
Функция РазархивироватьФайлНаСервере(АдресХранилищаФайлаНаСервере, ТекстОшибки = "")
	...
	Попытка
		...
		Если ОбъектЧтение.Элементы.Количество() = 0 Тогда
			...
			ТекстОшибки = НСтр("ru = 'Нет файлов в архиве.';
								|en = 'No files in the archive.'");
			Попытка	
				УдалитьФайлы(ИмяФайлаАрхива);
			Исключение
				ТекстОшибки = ТекстОшибки + Символы.ПС + 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка удаления временных файлов: %1';
																				|en = 'An error occurred when deleting temporary files: %1'"), ИнформацияОбОшибке().Описание);
			КонецПопытки;
			Возврат Неопределено;
		КонецЕсли;
		...
		Иначе
			ИмяФайлаМоделиСтрока = НСтр("ru = 'ВыгрузкаМоделиКА.xml';
										|en = 'ВыгрузкаМоделиКА.xml'");
			//++ НЕ УТКА
			ИмяФайлаМоделиСтрока = НСтр("ru = 'ВыгрузкаМодели.xml';
										|en = 'ВыгрузкаМодели.xml'");
			//-- НЕ УТКА
			
			ЭлементПоиска = ОбъектЧтение.Элементы.Найти(ИмяФайлаМоделиСтрока);
			
			Если ЭлементПоиска = Неопределено Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найден файл - %1, в архиве.';
																							|en = 'The %1 file is not found, in the archive.'"), ИмяФайлаМоделиСтрока);
				Попытка
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка распаковки архива: %1';
				|en = 'An error occurred while unpacking archive: %1.'"), Символы.ПС + ИнформацияОбОшибке().Описание);
		Попытка
			УдалитьФайлы(ИмяФайлаАрхива);
