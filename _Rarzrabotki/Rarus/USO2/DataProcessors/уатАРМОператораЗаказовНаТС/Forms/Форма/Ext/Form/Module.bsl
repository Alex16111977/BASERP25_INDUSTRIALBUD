...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбъектМетаданных = Метаданные.Документы.Найти("ЗаказКлиента");
...
Процедура СформироватьЗаказыНаТСПоЗаявкам() 
	...
	Для каждого ТекЗаявка ИЗ Заявки цикл
		...
		РеквизитыШапки.Вставить("Соглашение", ПолучитьЗначениеРеквизита(ТекЗаявка.ДокументЗаявка, "Соглашение"));
		РеквизитыШапки.Вставить("Партнер", ТекЗаявка.Партнер);
		РеквизитыШапки.Вставить("КонтактноеЛицоГрузоотправителя", ЗаявкаКонтактноеЛицоОтправителя);
		РеквизитыШапки.Вставить("КонтактноеЛицоГрузополучателя", ТекЗаявка.КонтактноеЛицоПолучателя);
		...
		РеквизитыШапки.Вставить("ОтправлениеС", НачалоДня(ТекЗаявка.ДатаВыполнения) + (ЗаявкаВремяОтправленияС - Дата('00010101')));
		РеквизитыШапки.Вставить("ОтправлениеПо", НачалоДня(ТекЗаявка.ДатаВыполнения) + (ЗаявкаВремяОтправленияПо - Дата('00010101')));
		...
		РеквизитыШапки.Вставить("ДоставкаС", НачалоДня(ТекЗаявка.ДатаВыполнения) + (ТекЗаявка.ВремяДоставкиС - Дата('00010101')));
		РеквизитыШапки.Вставить("ДоставкаПо", НачалоДня(ТекЗаявка.ДатаВыполнения) + (ТекЗаявка.ВремяДоставкиПо - Дата('00010101')));
...
Процедура ОбновитьСписокНеобработанныхЗаявок()
	...
	Для Каждого ТекСтрока Из мсвЗаявки Цикл
		...
		НовЗаявка.Партнер = ТекСтрока.ДокументПартнер;
		...
		НовЗаявка.АдресДоставкиСтрока = Строка(НовЗаявка.АдресДоставки);
...
Функция ПолучитьТблНеобработанныхЗаявокСервер()
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	уатРегистрацияОснованийДляЗаказовНаТССрезПоследних.Документ КАК Документ,
	|	уатРегистрацияОснованийДляЗаказовНаТССрезПоследних.Документ.Контрагент КАК ДокументКонтрагент,
	|	уатРегистрацияОснованийДляЗаказовНаТССрезПоследних.Документ.Партнер КАК ДокументПартнер,
	...
	|	уатРегистрацияОснованийДляЗаказовНаТССрезПоследних.Статус КАК Статус,
	|	"""" КАК АдресДоставки,
