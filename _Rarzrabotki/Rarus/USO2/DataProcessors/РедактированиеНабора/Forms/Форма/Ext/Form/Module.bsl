		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПричиныОтменыЗаказовКлиентов")
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Необходимо указать причину отмены в строке %1 списка ""Комплектующие""';
								|en = 'Specify a cancellation reason in the %1 line of the Components list'"), НомерСтроки),
				,
...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "Комплектующие", НСтр("ru = 'Комплектующие';
																									|en = 'Components'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьРучныеСкидки(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "Комплектующие", НСтр("ru = 'Комплектующие';
																								|en = 'Components'")) Тогда
		Возврат;
	КонецЕсли;
