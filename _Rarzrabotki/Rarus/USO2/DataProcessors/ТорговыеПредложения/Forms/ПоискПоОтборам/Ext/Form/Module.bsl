	...
	Если Не ЗавершениеРаботы И Товары.Количество() И Не ВыполняетсяЗакрытие Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект),
			НСтр("ru = 'Список подобранных товаров будет очищен. Продолжить?';
				|en = 'List of selected goods will be cleared. Continue?'"),
...
Процедура Подключаемый_Нажатие(Элемент)
	...
	ИначеЕсли Найти(Элемент.Имя, "ЗаголовокОтбора_") Тогда
		...
		ИмяРеквизита = Сред(Элемент.Имя, СтрНайти(Элемент.Имя, "_") + 1);
		Если ИмяРеквизита = "Цена" Тогда
			Если Найти(Элемент.Заголовок, НСтр("ru = 'от';
												|en = 'dated'")) Тогда
				ИмяРеквизита = "ЦенаОт";
			Иначе
				ИмяРеквизита = "ЦенаДо";
...
Процедура ЗапроситьЦеныНажатие(Элемент)
	...
	Если Товары.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны товары для запроса цен.';
										|en = 'Goods for price request are not selected.'"));
		Возврат;
	КонецЕсли;
...
Процедура НайтиПредложения(Команда)
	...
	// Для начала поиска паузс 0.1 сек., для отмены поиска 0.8 сек.
...
Процедура КомандаОчиститьТовары(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Список товаров для заказа будет очищен. Продолжить?';
									|en = 'List of goods to order will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьПризнакУстановкиОтборов(СтрокаОтборов = "")
	...
	Элементы.ПоказатьСкрытьОтборы.Заголовок = НСтр("ru = 'Отборы';
													|en = 'Filters'") + ?(ОтборыУстановлены, " *", "");
...
Процедура ОбновитьНадписьИтогов()
	...
	ИначеЕсли Товары.Итог("Сумма") = 0 Тогда
		НадписьПодобраноТоваров = СтрШаблон(НСтр("ru = 'Товаров %1 поз., цены по запросу';
												|en = 'Goods %1 items, prices upon request'"), КоличествоТоваров);
	Иначе
		НадписьПодобраноТоваров = НСтр("ru = 'Товаров %1 поз. на сумму %2 %3';
										|en = 'Goods %1 pos. to the amount of %2 %3'");
...
Процедура НайтиТорговыеПредложенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		СостояниеПоиска = СтрШаблон(НСтр("ru = 'Отображается %1 торговых предложений';
										|en = '%1 selling propositions are displayed'"), 
...
Процедура ЗаполнитьНавигациюКатегорий(Идентификатор, ОчиститьСтруктуру = Ложь, Отказ = Ложь)
	...
	Если ЭтоКорневаяКатегория Тогда
		Элементы.СтруктураКатегорий.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Все товары';
																				|en = 'All goods'"),,,, "id");
	...
	Иначе
		...
		Заголовок = СтрШаблон(НСтр("ru = 'Торговые предложения - %1';
									|en = 'Selling propositions - %1'"), СтрокаДобавления.Представление);
...
Процедура ОбновитьЭлементыОтборовФормы(ПараметрыЗапроса)
	...
	Если Не ПустаяСтрока(НаименованиеКонтрагента) И РазрешитьИзменениеКонтрагента Тогда
		БыстрыеОтборы.Добавить("НаименованиеКонтрагента", СтрШаблон(НСтр("ru = 'Поставщик: %1';
																		|en = 'Supplier: %1'"), НаименованиеКонтрагента));
	КонецЕсли;
	Если ЦенаОт <> 0 ИЛИ ЦенаДо <> 0 Тогда
		БыстрыеОтборы.Добавить("Цена", СтрШаблон(НСтр("ru = 'Цена, %1: %2';
														|en = 'Price, %1: %2'"), Валюта,
			?(ЦенаОт = 0, "", СтрШаблон(НСтр("ru = 'от %1';
											|en = 'from %1'"), ЦенаОт))
			+ ?(ЦенаОт <> 0 И ЦенаДо <> 0, " ", "")
			+ ?(ЦенаДо = 0, "", СтрШаблон(НСтр("ru = 'до %1';
												|en = 'until %1'"), ЦенаДо))));
	КонецЕсли;
	...
	Для каждого СтрокаТовары Из Товары Цикл
		...
		Если СтрокиСписка.Количество() Тогда
			СтрокиСписка[0].ГиперссылкаДобавить = ?(СтрокаТовары.Количество = 0,
				НСтр("ru = 'Добавить';
					|en = 'Add'"), СтрокаТовары.Количество);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьРежимСортировки(Элементы, НовыйРежим, Валюта)
	...
	Элементы.СписокЦена.Заголовок = СтрШаблон(НСтр("ru = 'Цена, %1';
													|en = 'Price, %1'"), Валюта);
...
Процедура ОбработатьВыборСписка()
	...
	Если ИмяКолонки = Элементы.СписокГиперссылкаДобавить.Имя Тогда
		...
		Если ЗапрашиватьКоличество Тогда
			...
			ПоказатьВводЧисла(ВводКоличестваЗавершение, КоличествоВЗаказе, НСтр("ru = 'Укажите количество';
																				|en = 'Specify a quantity'"), 15, 3);
		Иначе
...
&НаСервере
Процедура УстановитьКартинкуКомандыНайти(ДлительнаяОперация)
