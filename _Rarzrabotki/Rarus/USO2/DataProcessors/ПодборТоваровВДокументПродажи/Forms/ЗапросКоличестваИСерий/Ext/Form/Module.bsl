...
Процедура ОК(Команда)
	...
	Если РазборкаНаКомплектующие Тогда
		...
		Если ВариантыОбеспеченияСоответствие.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выбрано нескольно вариантов обеспечения, должно быть выбрано одно';
																	|en = 'You have selected several supply options; you must select only one'"),,,,Отказ);
		КонецЕсли;
		Если КоличествоСерий > 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выбрано нескольно серий, должна быть выбрано одна';
																	|en = 'You have selected several series; you must select only one'"),,,,Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура КУказаниюСерий(Команда)
	...
	Если ТекущиеДанные.СтатусУказанияСерий = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Для этого действия серии указывать не нужно.';
								|en = 'Do not specify series for this action.'");
...
Процедура РаспределитьОстаток(Команда)
	Если Не РаспределитьОстатокНаСервере() Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Все количество товара распределено по сериям.';
									|en = 'All goods quantity is allocated by series.'"));
	КонецЕсли;
КонецПроцедуры
		...
		Если СообщитьОбОшибке Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Введено количество упаковок меньше, чем уже подобрано. Подобрано %1 %2.';
					|en = 'You have entered less packaging than it is already selected. Selected %1 %2.'"),
				КоличествоУпаковок,
...
Процедура ДополнитьТаблицуДоступныхДействий()
	...
	Для Каждого Строка Из ТаблицаДоступныхДействий Цикл
		...
		Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
			...
			Строка.Действие = СтрШаблон(НСтр("ru = 'Резервировать к %1';
											|en = 'Reserve by %1'"), ДатаСтрокой);
			
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется И ЗаполненаДатаОтгрузки Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru = 'К обеспечению (срок %1)';
											|en = 'For supply (due %1)'"), ДатаСтрокой);
			
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно И ЗаполненаДатаОтгрузки Тогда
			
			Строка.Действие = СтрШаблон(НСтр("ru = 'Обеспечивать обособленно (срок %1)';
											|en = 'Provide on pegging basis (due %1)'"), ДатаСтрокой);
		...
		Если ИзмененноеКоличество > ДоступноеИзмененненноеКоличество Тогда
			...
			ТекстСообщенияОформлено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Оформлено и изменить вариант обеспечения нельзя для %1 %2.';
					|en = 'Registered and cannot change supply option for %1 %2.'"),
					КоличествоОформлено,
					ЕдиницаИзмеренияНоменклатуры);
			
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Введено количества больше, чем требуется перераспределить.%1 Доступно для изменения %2 %3.';
					|en = 'You have entered more quantity than it is required to reallocate. %1 Available for change %2 %3.'"),
	...
	Если ТекущиеДанные.СтрокаЗаказа
		...
		Если ТекущиеДанные.Количество < КоличествоОформлено Тогда
			...
			НСтр("ru = 'Введено количество меньше, чем уже оформлено. Оформлено %1 %2.';
				|en = 'The entered quantity is less than the registered one. Registered %1 %2.'"),
			КоличествоОформлено,
			ЕдиницаИзмеренияНоменклатуры);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,);
		КонецЕсли;
