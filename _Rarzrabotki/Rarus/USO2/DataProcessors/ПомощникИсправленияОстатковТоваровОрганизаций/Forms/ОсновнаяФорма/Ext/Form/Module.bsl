	...
	Если Константы.КонтролироватьОстаткиТоваровОрганизаций.Получить() 
		...
		ТекстСообщения = 
			НСтр("ru = 'Для работы с помощником требуется роль ""%ПропускКонтроляТоваровОрганизацийПриОтменеПриходов%"" или отключение функциональной опции ""%КонтролироватьОстаткиТоваровОрганизаций%"". Обратитесь к администратору.';
				|en = 'To use the wizard, you need to have the ""%ПропускКонтроляТоваровОрганизацийПриОтменеПриходов%"" role or disable the ""%КонтролироватьОстаткиТоваровОрганизаций%"" functional option. Contact the administrator.'");
...
Процедура НастроитьЭлементыФормы()
	...
	Если НомерСтраницы = 7 Тогда
		ЗаголовокОтчета = НСтр("ru = 'Отчет о проделанной работе за %МесяцИсправления%';
								|en = 'Progress report for the %MonthOfCorrection%'");
		ЗаголовокОтчета = 
...
Функция СформироватьЗаголовокШаг3Пояснение2()
	...
	ТекстЗаголовка = НСтр("ru = 'При запуске переформирования резервов будут очищены движения регистра ""Резервы товаров организаций"" по проблемным позициям, начиная с %МесяцРодительныйПадеж% %Год% года, и сформированы новые резервы по отрицательным остаткам. Новые резервы будут записаны в документы корректировок регистров. Для каждой организации будет создан отдельный документ.';
							|en = 'When regenerating reserves is started, the ""Company goods reserves"" register records for problematic positions will be cleared starting from %МесяцРодительныйПадеж% %Год% and new negative balance reserves will be generated. New reserves will be recorded in register adjustment documents. A separate document will be generated for each company.'");
...
Процедура ПроверитьРольНастройкаНСИДоступна(СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны(Метаданные.Роли.НастройкаНСИ.Имя) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для настройки передачи требуется роль ""%НастройкаНСИ%"". Обратитесь к администратору.';
								|en = 'You need the ""%НастройкаНСИ%"" role to set up transfer. Contact the administrator.'");
