...
Процедура СформироватьНовыеРезервы(ПоНовымНастройкам = Ложь) Экспорт
	...
	Если БылиОшибки Тогда
		ТекстСообщения = НСтр("ru = 'При переформировании резервов товаров организации возникли ошибки. Подбробности в журнале регистрации.';
								|en = 'Errors occurred when regenerating company goods reserves. For more information, see the event log.'");
	...
	Пока ВыборкаОрганизацииСОтрицательнымОстатком.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр(
				...
				|en = 'Cannot generate corrections of detailed balance. For more information, see the event log.'");
			ВызватьИсключение ТекстСообщения;
	...
	Пока Не Завершение Цикл
 		Если Итерация > 50 Тогда
			ТекстИсключения = НСтр("ru = 'Превышен лимит итераций перезаполнения видов запасов';
									|en = 'Limit of inventory kind refilling iteration is exceeded'", Метаданные.ОсновнойЯзык.КодЯзыка);
			ЗаписьЖурналаРегистрации(
				ИмяСобытия(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Обработки.ПомощникИсправленияОстатковТоваровОрганизаций,
...
Процедура СформироватьИсправительныеСчетаФактуры() Экспорт
	...
	Если БылиОшибки Тогда
		...
		|en = 'Cannot generate replacement tax invoices under all changed documents. For more information, see the event log.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Для Каждого Стр Из ДокументыКПерезаполнению Цикл 
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'Возникла ошибка при перезаполнении видов запасов за период %ТекущийМесяц%: %ПричинаОшибки%';
								|en = 'An error occurred while refilling inventory kinds for the %ТекущийМесяц% period: %ПричинаОшибки%'");
	...
	Если РезультатЗапроса[1].Пустой() <> РезультатЗапроса[2].Пустой() Тогда
		ТекстИсключения = 
			НСтр("ru = 'По аналитике учета номенклатуры %АналитикаУчетаНоменклатуры% не найдены документы для перезаполнения видов запасов';
				|en = 'No documents are found for refilling inventory kinds under the %АналитикаУчетаНоменклатуры% product accounting dimension'",
