	...
	Иначе
		...
		Если Уровень = 7 Тогда
			Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Данные о улицах для введенного адреса отсутствуют';
														|en = 'Data on streets is missing for the entered address'");
		Иначе
...
Функция ДанныеКлассификатораПоИдентификатору(Знач ПараметрыПоиска)
	...
	Если ДанныеКлассификатора.Отказ Тогда
		...
		Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Автоподбор и проверка адреса недоступны:';
													|en = 'Auto pick and address check are not available:'") + Символы.ПС + ДанныеКлассификатора.КраткоеПредставлениеОшибки;
		Результат.Отказ = Истина;
		Возврат Результат;
