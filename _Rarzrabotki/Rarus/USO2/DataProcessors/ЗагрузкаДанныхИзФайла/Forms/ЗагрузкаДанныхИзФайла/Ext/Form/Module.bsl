...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для открытия загрузки данных из файла';
								|en = 'Insufficient rights to open data import from file'"));
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = НСтр("ru = 'Введенные данные не будут записаны.';
									|en = 'Entered data will not be written.'");
		Возврат;
	КонецЕсли;
	...
	Если Отказ Тогда
		...
		ТекстВопроса = НСтр("ru = 'Введенные данные не будут записаны. Закрыть форму?';
							|en = 'Entered data will not be written. Close the form?'");
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Отменить сопоставление?';
									|en = 'Cancel mapping?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных
		...
		Элементы.Далее.Заголовок = ?(ТипЗагрузки = "ВставкаИзБуфераОбмена",
				НСтр("ru = 'Вставить в список';
					|en = 'Insert into list'"), НСтр("ru = 'Далее >';
														|en = 'Next >'"));
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ru = 'Строки, в которых не заполнены обязательные колонки, будут пропущены.';
												|en = 'Lines with blank required columns will be skipped.'")
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Выполнено сопоставление';
										|en = 'Mapped'"),, НСтр("ru = 'Сопоставлено элементов:';
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ru = 'Несопоставленные (';
															|en = 'Unmapped ('") + Статистика.НеСопоставленных + ")";
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вставка из буфера обмена (%1)';
																				|en = 'Pasting (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
...
Процедура УстановитьВидимостьЭлементовФормы()
	ЭтотОбъект.Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ru = 'Загрузка данных в справочник';
																	|en = 'Data import to catalog'"), Параметры.Заголовок);
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	Если ТипЗнч(ОписаниеСтрокиВыбора) = Тип("Структура") И ОписаниеСтрокиВыбора.Свойство("Представление") Тогда
		ЭтотОбъект.Заголовок = НСтр("ru = 'Загрузка данных в справочник';
									|en = 'Data import to catalog'") + " """ +ОписаниеСтрокиВыбора.Представление + """";
	Иначе
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка данных в справочник ""%1""';
																							|en = 'Import data to catalog ""%1""'"), ПредставлениеСправочника(ИмяОбъектаСопоставления));
	КонецЕсли;
	...
	Если ТаблицаСДаннымиПустая() Тогда
		Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			ПоказатьПредупреждение(, (НСтр("ru = 'Для вставки сопоставленных данных в список, необходимо заполнить текстовое поле.';
											|en = 'To insert mapped data in the list, fill in the text field.'")));
			Возврат;
		КонецЕсли;
		
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ru = 'Для сопоставления и загрузки данных, необходимо заполнить таблицу.';
											|en = 'Fill in the table to map and import data.'")));
		Иначе
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ru = 'Обязательная колонка""';
									|en = 'Mandatory column ""'") + " " + СписокНезаполненныхКолонок[0]
				+ НСтр("ru = '"" содержит незаполненные строки, эти строки будут пропущены при загрузке.';
						|en = '"" contains empty strings, these strings will be ignored during import.'");
		Иначе
			ТекстПроКолонки = НСтр("ru = 'Обязательные колонки""';
									|en = 'Mandatory columns ""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
				+ НСтр("ru = '"" содержат незаполненные строки, эти строки будут пропущены при загрузке.';
						|en = '"" contain empty strings, these strings will be ignored during import.'");
		КонецЕсли;
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				ТекстНеНайдено = НСтр("ru = '%3 строк будут пропущены:';
										|en = '%3 strings will be skipped:'") + Символы.ПС + "  - " + НСтр("ru = 'Нет данных в программе: %1';
																										|en = 'No data in application: %1'") 
					+ Символы.ПС + "  - " +НСтр("ru = 'Несколько вариантов для вставки: %2';
												|en = 'Several options to insert: %2'");
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ru = 'Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1';
										|en = 'Strings for which there are multiple options in the application will be skipped: %1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ru = 'Строки, для которых в программе нет соответствующих данных, будут пропущены: %1';
										|en = 'Strings for which there are no matching data in the application will be skipped: %1'");
			...
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр("ru = 'Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".';
																|en = 'To view skipped lines and data selection for insertion, click Next.'");
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		...
		|en = 'To import data, save fill-in-the-blank template to a file to fill out in another application.
		|After that, import the table saved in one of the following formats:
		|• Excel 97 Workbook (.xls) and Excel 2007 (.xlsx) Workbook
		|• LibreOffice Calc spreadsheets (.ods)
		|• Delimited text (.csv)
		|• Table document (.mxl)'") + Символы.ПС;
	Иначе
		ТекстПодсказки = НСтр("ru = 'Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.';
								|en = 'To fill in the table, copy data to the table from external file via clipboard.'") + Символы.ПС;
	КонецЕсли;
			...
			Иначе
				...
				Иначе
					ПоказатьПредупреждение(, НСтр("ru = 'Шаблон файла не был сохранен.';
													|en = 'File template was not saved.'"));
					Возврат;
				КонецЕсли;
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru = 'Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла';
																|en = 'Subsystem ЗагрузкаДанныхИзФайла: Perform server method of importing data from file'");
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru = 'Заполнение таблицы сопоставления загруженными данными из файла.';
																|en = 'Populate the mapping table with imported data from file.'");
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Запись загруженных данных из файла';
															|en = 'Write data imported from file'");
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru = 'Не удалось произвести запись данных.';
										|en = 'Cannot write data.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
