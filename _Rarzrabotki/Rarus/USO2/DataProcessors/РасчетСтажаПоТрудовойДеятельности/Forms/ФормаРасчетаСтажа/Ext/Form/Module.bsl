...
Процедура Подключаемый_ИспользуетсяПриИзменении(Элемент)
	...
	ОбновитьОбсобенностиРасчетаСтажа(ЭтотОбъект);
...
Процедура ТрудоваяДеятельностьФизическихЛицПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Элементы.ТрудоваяДеятельностьФизическихЛиц.ТекущиеДанные.ДатаНачала < ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведений() Тогда
...
Процедура Подключаемый_ТрудоваяДеятельностьФизическихЛицИспользуетсяПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ОбновитьСтажПоПериодам Тогда
			...
			УстановитьОбсобенностиРасчетаСтажа(ЭтотОбъект, ИндексВидаСтажа, Особенность);
...
&НаСервере
Процедура РасчитатьДлительностиПериодовТрудовойДеятельности()
...
Процедура ПроинициализироватьФорму()
	...
	РасчитатьДлительностиПериодовТрудовойДеятельности();
	...
	Для Каждого СтажПоТрудовойДеятельности Из СтажиПоТрудовойДеятельности Цикл
		...
		Если ДатаКалькуляцииСтажей < СтажПоТрудовойДеятельности.ДатаОтсчета Тогда
			ДатаКалькуляцииСтажей = СтажПоТрудовойДеятельности.ДатаОтсчета;
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДатаКалькуляцииСтажей) Тогда
		...
		ПараметрыПолучения.КадровыеДанные = "ДатаНачалаУчета,ДатаУвольнения";
	...
	Для Каждого ЭлементСписка Из ВидыСтажей Цикл
		...
		Если ЗначениеЗаполнено(ДанныеРасчетаСтажей[ИндексВидаСтажа].ДатаОтсчета) Тогда
			...
			СтажИспользуется.Подсказка = НСтр("ru = 'на';
												|en = 'to'") + " " + Формат(ДанныеРасчетаСтажей[ИндексВидаСтажа].ДатаОтсчета, "ДЛФ=D") + " " + НСтр("ru = 'составил';
																																				|en = 'created'") + " "
		...
		Если Запись["Используется" + ИндексВидаСтажа] = 1 Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(Запись.ДатаОкончания)
					И ПредыдущаяЗапись <> Неопределено
					И (Запись.ДатаОкончания <= ПредыдущаяЗапись.ДатаОкончания
				...
				ИначеЕсли ПредыдущаяЗапись <> Неопределено
					И Запись.ДатаНачала <= ПредыдущаяЗапись.ДатаОкончания Тогда
					
					Продолжительность = ПродолжительностьСтажа(КонецДня(ПредыдущаяЗапись.ДатаОкончания) + 1, Запись.ДатаОкончания, Форма.ДатаКалькуляцииСтажей);
				...
				ИначеЕсли Запись.ДатаОкончания > Форма.ДатаКалькуляцииСтажей Тогда
	...
	Иначе
		...
		Если Форма.ДанныеРасчетаСтажей[ИндексВидаСтажа].Пользовательский Тогда
			ДополнениеПредставления = НСтр("ru = 'на';
											|en = 'to'") + " " + Формат(Форма.ДанныеРасчетаСтажей[ИндексВидаСтажа].ДатаОтсчета, "ДЛФ=D");
...
Функция ВозвращаемыеКоллекции()
	...
	Для Каждого ЭлементСписка Из ВидыСтажей Цикл
		...
		Если ЭлементСписка.Пометка Тогда
			...
			СтрокиСтажа = СтажиФизическихЛиц.НайтиСтроки(Новый Структура("ВидСтажа,ДатаОтсчета", ВидСтажа, ДатаКалькуляцииСтажей));
			...
			Если РазмерМесяцев <> 0 Или РазмерДней <> 0 Тогда
				...
				СтрокаСтажа.Период = ДатаКалькуляцииСтажей;
				СтрокаСтажа.ДатаОтсчета = ДатаКалькуляцииСтажей;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОбсобенностиРасчетаСтажа(Форма, ИндексВидаСтажа, Особенность)
	
	ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(
		Форма, "ОсобенностиРасчета" + ИндексВидаСтажа, Особенность);
	
	ОбновитьОбсобенностиРасчетаСтажа(Форма);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьОбсобенностиРасчетаСтажа(Форма)
