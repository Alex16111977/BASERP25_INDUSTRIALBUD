...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ТекстОшибки = НСтр("ru = 'Для удаления помеченных необходимо войти в область данных.';
							|en = 'To delete the marked ones, enter the data area.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	...
	Если Не ВидимостьРасписания Тогда 
		Элементы.УдалениеПомеченныхИспользование.Заголовок = НСтр("ru = 'Автоматически удалять помеченные объекты';
																	|en = 'Automatically delete selected objects'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru = 'Удаление помеченных еще выполняется.
		|Прервать?';
		|en = 'Deletion of the marked ones is in progress.
		|Stop?'");
...
Процедура ПоказатьСвязиНеУдаленныхНаСервере()
	...
	Иначе
		...
		Шаблон = НСтр("ru = 'Места использования объекта ""%1"" (%2):';
						|en = 'Usage locations of object ""%1"" (%2):'");
		...
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("ru = 'Удаление записей регистров или очистка значений констант выполняется из их карточек.';
									|en = 'Register record deletion or constant value cleanup is carried out from their cards. '");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Иначе
		...
		Иначе
			ТекстВопроса = НСтр("ru = 'Снять с выделенных объектов (%1) пометку на удаление?';
								|en = 'Clear marks for deletion for the selected objects (%1)?'");
		КонецЕсли;
		...
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("ru = 'Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';
						|en = 'To resolve complex dependencies, it is recommended that you try the deletion with the enabled check box ""Block all operations in the application and speed up the deletion"" (click ""< Home"").  '");
		КонецЕсли;
	Иначе
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Иначе
		Текст = НСтр("ru = 'Удаляются объекты, помеченные на удаление...';
					|en = 'Deleting the objects marked for deletion...'");
	КонецЕсли;
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Удаление помеченных объектов (интерактивное)';
														|en = 'Deletion of marked objects (interactive)'");
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ru = 'Не удалось прочитать результат удаления помеченных';
								|en = 'Cannot read the result of deleting marked items'"), "");
			Возврат;
		КонецЕсли;
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не получилось удалить объекты, помеченные на удаление (%1):';
						|en = 'Cannot delete objects marked for deletion (%1):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';
						|en = 'Successfully deleted: %1 from %2, other objects are not deleted (%3):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ru = 'Удаление не выполнено по причине:';
					|en = 'Deletion is not performed due to a reason:'") + Символы.ПС + КраткоеПредставлениеОшибки;
