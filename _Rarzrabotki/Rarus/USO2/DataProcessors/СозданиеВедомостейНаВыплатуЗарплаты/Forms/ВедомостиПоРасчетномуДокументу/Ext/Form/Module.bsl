...
Функция НастроитьПоРасчетномуДокументу()
	...
	Если Запрос.Выполнить().Пустой()Тогда
		ТекстСообщения = НСтр("ru = 'По документу не осталось сумм к выплате';
								|en = 'No amounts for payment against the document'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат Ложь
	КонецЕсли;
	
	// Должен существовать способ выплаты по документу, подходящий по порядку выплаты
	ВидДокументаОснования = Перечисления.ВидыДокументовМежрасчетныхНачислений.ПоДокументу(РасчетныйДокумент);
	ПорядокВыплаты = 
...
Процедура ЗаполнитьСписокВедомостей(ВедомостиПоРасчетномуДокументу)
	...
	ПоказыватьСтатьиРасходов		= ВРазрезеИсточников И ПолучитьФункциональнуюОпцию("РаботаВБюджетномУчреждении");
