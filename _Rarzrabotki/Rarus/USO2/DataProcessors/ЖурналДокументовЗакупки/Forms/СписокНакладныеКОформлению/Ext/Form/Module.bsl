...
#КонецОбласти
	...
	Если ИспользоватьОтветственноеХранениеВПроцессеЗакупки Тогда
		Элементы.СтраницаРаспоряженияНаОформление.Заголовок = НСтр("ru = 'К оформлению приобретения (приемки)';
																	|en = 'To register purchase (receiving)'");
	КонецЕсли;
...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Если МассивТиповДокументПоступления.Найти(ТипЗнч(ТекущиеДанные.Ссылка)) = Неопределено Тогда
		...
		Если ИспользоватьОтветственноеХранениеВПроцессеЗакупки Тогда
			ТекстПредупреждения = НСтр("ru = 'Команда не может быть выполнена для указанного объекта.
				...
				|Select a line with the ""Purchase of goods and services"" or ""Goods receiving for storage"" reference type.'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Команда не может быть выполнена для указанного объекта.
				|Выберите строку с типом распоряжения ""Приобретение товаров и услуг"".';
				|en = 'Command cannot be executed for the specified object.
				|Select a line with the ""Purchase of goods and services"" reference type.'");
		КонецЕсли;
	...
	ИначеЕсли ХозОперацииПоступленияТоваров.Найти(ТекущиеДанные.ХозяйственнаяОперацияПриемки) <> Неопределено Тогда
		...
		ШаблонСтроки = НСтр("ru = 'Невозможно оформить документ ""Акт о расхождениях после %1"" на основании распоряжения %2 с операцией ""%3"".';
							|en = 'Cannot register the ""Discrepancy certificate after %1"" based on the %2 reference with the ""%3"" operation.'");
...
Процедура ОформитьПоПриемке(Команда)
	...
	Если ТекущиеДанные.СостояниеОрдера = 5 Тогда
		ТекстПредупреждения = НСтр("ru = 'Перезаполнение по приемке не требуется, так как нет ни одного ордера.';
									|en = 'Not required to refill receipt, as there are no notes.'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат
	КонецЕсли;
...
Процедура ОформитьПоступлениеТоваров(Команда)
	...
	Если ТекущиеДанные.СостояниеПоступленияТоваров = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Оформление документа ""Поступление товаров"" не требуется.';
									|en = 'The ""Uninvoiced receipt"" document registration is not required.'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат
	КонецЕсли;
...
Процедура ОформитьПриобретениеТоваровПоРаспоряжению(Команда)
	...
	Если ТекущиеДанные.СостояниеПоступленияПриобретения = 0 
		...
		ТекстПредупреждения = НСтр("ru = 'Оформление приобретения товаров по поступлениям не требуется.';
									|en = 'Goods purchase registration by receipts is not required.'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат
	КонецЕсли;
...
Процедура ЗаполнитьСпискиВыбораПоСостояниям()
	...
	СписокВыбора.Добавить("1", НСтр("ru = 'Оформлены ордера, не оформлены накладные';
									|en = 'Goods receipt notes are registered, purchase invoices are not'"), Ложь, БиблиотекаКартинок.СоздатьНакладную);
	СписокВыбора.Добавить("5", НСтр("ru = 'Не оформлены ордера, оформлены накладные';
									|en = 'Goods receipt notes are not registered, purchase invoices are registered'"), Ложь, БиблиотекаКартинок.НеОформленыОрдера);
	СписокВыбора.Добавить("3", НСтр("ru = 'Ордера не соответствуют накладным';
									|en = 'Goods receipt notes do not match purchase invoices'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
	...
	СписокВыбора.Добавить("0", НСтр("ru = 'Приобретения соответствуют поступлениям';
									|en = 'Acquisitions correspond to receipts'"), Ложь, БиблиотекаКартинок.ОформленаНакладная);
	СписокВыбора.Добавить("1", НСтр("ru = 'Не оформлены приобретения';
									|en = 'Purchases not registered'"), Ложь, БиблиотекаКартинок.СоздатьНакладную);
	СписокВыбора.Добавить("3", НСтр("ru = 'Приобретения не соответствуют поступлениям';
									|en = 'Acquisitions do not correspond to receipts'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
