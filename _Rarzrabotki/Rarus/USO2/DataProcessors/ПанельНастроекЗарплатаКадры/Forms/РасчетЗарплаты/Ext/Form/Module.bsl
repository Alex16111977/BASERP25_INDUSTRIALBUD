...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РаботаВБюджетномУчреждении Тогда
		...
		Элементы.Декорация10.Заголовок = НСтр("ru = 'Использование профессиональных квалификационных групп и уровней при описании позиции штатного расписания и при расчете заработной платы.';
												|en = 'Use of professional qualification groups and levels when describing a staff list position and calculating salary.'");
...
&НаКлиенте
Процедура НастройкиУчетаВремениЗаполнятьВТабелеЧасыОтклоненийПриИзменении(Элемент)
	НастройкиУчетаВремениПрежняя.ЗаполнятьВТабелеЧасыОтклонений = НастройкиУчетаВремени.ЗаполнятьВТабелеЧасыОтклонений;
	ЗаписатьНастройкиНаКлиенте("НастройкиУчетаВремени");
	ПодключитьОбработчикОжиданияОбновленияИнтерфейса();
КонецПроцедуры
