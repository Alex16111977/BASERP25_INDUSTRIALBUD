				...
				ЛогШаговВыполненияВложенный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибки внедрения подсистемы Новости:
							|%1';
							|en = 'Errors occurred while implementing the News subsystem:
							|%1'"),
						Результат));
			КонецЕсли;
						...
						Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
									...
									// Непосредственно загрузка файла, начало.
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'Начало попытки %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл %6.
												|(URL %7).';
												|en = 'Start of attempt %1 / %2 to receive file %3 from server %4 for news feed %5 to temporary file %6.
												|(URL %7).'"), 
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'При попытке %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл получен ответ сервера с кодом %6, заголовки:
												...
												|en = 'When attempting %1 / %2 to receive file %3 from server %4 for news feed %5 into a temporary file, a server response is received with code %6, titles:
												|%7
												|
												|(URL %8)
												|Import time (sec): %9'"),
										...
										Если ВременныйФайл.Существует() Тогда
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru = 'Успешное завершение попытки %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл %6.
													...
													|Первые строки файла:
													|%9';
													|en = 'Successful end of attempt %1 / %2 of file receiving %3 from server %4 for news feed %5 into temporary file %6.
													|File of %7 bytes in size received.
													|(URL %8)
													|
													|First file strings:
													|%9'"),
												...
												Исключение
													...
													ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
														...
														НСтр("ru = 'Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения текста ошибки сервера';
															|en = 'News. Import news. Import news files. An error occurred when receiving the server error text'"), // ИдентификаторШага
											...
											Исключение
												...
												ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("ru = 'ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось удалить временный файл %1 для ленты новостей %2 по причине:
														...
														|
														|Рекомендуется вручную удалить этот временный файл.';
														|en = 'ЗагрузитьФайлыНовостейССервера (received from the Internet): Cannot remove the %1 temporary file for the %2 news feed due to:
														|%3
														|
														|It is recommended that you remove this temporary file manually.'"),
													СокрЛП(ИмяВременногоФайла),
										...
										Иначе
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru = 'Неудачное завершение попытки %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл %6 - после попытки загрузки, файла все еще не существует.
													|(URL %7).';
													|en = 'Unsuccessful end of attempt %1 / %2 to receive file %3 from server %4 for news feed %5 to temporary file %6 - the file still does not exist after an attempt to import it.
													|(URL %7).'"),
												ПопыткаПолученияФайла,
												НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла,
												ТекущаяЛентаНовостей.ИмяФайла,
											...
											Исключение
												...
												ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
													...
													НСтр("ru = 'Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка расшифровки текста ошибки сервера';
														|en = 'News. Import news. Import news files. An error occurred when decrypting server error text'"), // ИдентификаторШага
													УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
													, // ОбъектМетаданных
													, // Данные
													ТекстСообщения, // Комментарий
								...
								Исключение
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Попытка %1 / %2. Ошибка получения файла %3 с сервера %4 для ленты новостей %5 во временный файл по причине:
											|%6';
											|en = 'Attempt %1 / %2. An error occurred when receiving file %3 from server %4 for news feed %5 to temporary file due to:
											|%6'"),
										ПопыткаПолученияФайла,
										НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла,
										ТекущаяЛентаНовостей.ИмяФайла,
							...
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Загрузка: загрузка файла новостей из ленты новостей %1 (протокол %2, сайт %3, имя файла %4, имя файла на сервере %5).
									...
									|en = 'Import: import news file from news feed %1 (protocol %2, website %3, file name %4, file name on server %5).
									|Successfully? %6
									|Execution time (ms): %7'"),
								ТекущаяЛентаНовостей.ЛентаНовостей,
								ТекущаяЛентаНовостей.Протокол,
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей';
				|en = 'ISL:News.News import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка новостей. Загрузка файлов новостей';
				|en = 'News. Import news. Import news files'"), // ИдентификаторШага
			?(КодРезультата > 0,
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
				+ НСтр("ru = 'Сохранение даты последних загрузок';
						|en = 'Save the date of the last imports'")
				+ Символы.ПС;
							...
							Исключение
								...
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'В загруженном файле для ленты новостей %1 находится текст не в формате xml. Лента новостей будет пропущена.
										...
										|Первые строки файла:
										|%3';
										|en = 'The imported file for the news feed %1 contains text not in XML format. The news feed will be skipped. 
										|
									...
									Если (ОбъектХДТО.Свойства().Получить("version") = Неопределено) Тогда
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'Невозможно определить версию rss в ленте новостей %1 - отсутствует свойство version. Необходимы данные в формате версии 2.0.';
												|en = 'Cannot determine RSS version in news feed %1 - the Version property is missing. Data in format of version 2.0 is required.'"),
											ТекущаяСтрока.ЛентаНовостей);
										...
										Если (Число(лкВерсия) <> 2.0) Тогда
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru = 'Неподдерживаемая версия новостей RSS %1 в ленте новостей %2';
													|en = 'Unsupported RSS news version %1 in news feed %2'"),
												ОбъектХДТО.version,
												ТекущаяСтрока.ЛентаНовостей);
								...
								Иначе
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Неподдерживаемый тип новостей %1 в файле %2 для ленты новостей %3
											...
											|Первые строки файла:
											|%4';
											|en = 'Unsupported news type %1 in file %2 for the news feed %3
											|
											|First lines of file:
											|%4'"),
							...
							Исключение
								...
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'ПарсингХМЛФайловНовостей: Не удалось удалить временный файл %1 для ленты новостей %2 по причине:
										...
										|
										|Рекомендуется вручную удалить этот временный файл.';
										|en = 'ПарсингХМЛФайловНовостей: Cannot delete temporary file %1 for the %2 news feed due to:
										|%3
										|
						...
						Иначе
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Отсутствует загруженный файл %1 для ленты новостей %2. Возможно, недостаточно места на диске, нет прав для записи в этот каталог или произошла какая-то другая ошибка.';
									|en = 'Imported file %1 for the news feed %2 is missing. There may be not enough space on the disk, you might not have sufficient rights to save files to this directory, or another error occurred.'"),
								СокрЛП(ИмяВременногоФайла),
								ТекущаяСтрока.ЛентаНовостей);
					...
					Иначе
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не загружен файл новостей для ленты новостей %1 (пустой адрес в хранилище после загрузки файла [%2] с сайта [%3] по протоколу [%4]). Лента новостей не обработана.';
								|en = 'News file for the news feed %1 was not imported (empty address in the storage after importing file [%2] from website [%3] using protocol [%4]). News feed was not processed.'"),
							ТекущаяСтрока.ЛентаНовостей,
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей';
				|en = 'ISL:News.News import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка новостей. Парсинг файлов новостей';
				|en = 'News. Import news. News file parsing'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
							...
							Для Каждого ТекущийЭлементСписка Из СписокВсехЗагруженныхСерверныхЛентНовостей Цикл
								Если ТекущийЭлементСписка.Пометка = Ложь Тогда
									...
									ТекстСообщения = НСтр("ru = 'Лента новостей %ЛентаНовостейНаименование% помечена на удаление, т.к. она была отключена на сервере 1С';
															|en = 'News feed %ЛентаНовостейНаименование% is marked for deletion as it was disabled on 1C server'");
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка классификаторов';
				|en = 'ISL:News.Classifier import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка классификаторов';
				|en = 'News. Import classifiers'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Иначе
			...
			ОписаниеРезультата = НСтр("ru = 'Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера: Отключена работа с новостями через интернет. Обновление классификаторов не выполнялось.';
										|en = 'Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера: Work with news via the Internet is disabled. Classifiers were not updated.'");
		КонецЕсли;
...
Процедура ОбновитьСписокСправочниковИВерсииССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ИнтернетПоддержкаПользователейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Получение списка последних обновлений, вызов getLastUpdatesList
						...
						|Результат:
						|%5';
						|en = 'Receive a last update list, call getLastUpdatesList
						|configName = %1
						|configVersion = %2
						|errCode = %3
						|errDescription = %4
		...
		Иначе
			ОписаниеРезультата = НСтр("ru = 'Не удалось подключиться к веб-сервису обновления классификаторов.';
										|en = 'Cannot connect to the web service of classifier update.'");
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка классификаторов';
				|en = 'ISL:News.Classifier import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка классификаторов. Список обновлений классификаторов';
				|en = 'News. Import classifiers. Classifiers update list'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей, Прокси, КонтекстВыполнения = Неопределено)
				...
				Иначе
					ОписаниеРезультата = ОписаниеРезультата
						+ НСтр("ru = 'Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(rss): Отключена работа с новостями через интернет. Обновление классификаторов не выполнялось.';
								|en = 'Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(rss): Work with news via the Internet is disabled. Classifiers were not updated.'")
						+ Символы.ПС;
				КонецЕсли;
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'В файле полученном для ленты новостей %1 отсутствует поле channel.lastBuildDate. Все новости будут прочитаны и загружены, но это займет больше времени.';
									|en = 'No channel.lastBuildDate field in the file received for the %1 news feed. All news will be read and imported but it will take longer.'"),
								ЛентаНовостей)
							+ Символы.ПС;
					КонецЕсли;
				Иначе
			...
			Иначе
				ОписаниеРезультата = ОписаниеРезультата
					+ НСтр("ru = 'Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(atom): Отключена работа с новостями через интернет. Обновление классификаторов не выполнялось.';
							|en = 'Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(atom): Work with news via the Internet is disabled. Classifiers were not updated.'")
					+ Символы.ПС;
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей';
				|en = 'ISL:News.News import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей';
				|en = 'News. News import. News file parsing. News file parsing '"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьНовость(
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если НРег(ФорматНовостей) = "rss" Тогда
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					Исключение
						...
						ТекстСообщения = НСтр("ru = 'Неудачная попытка записи новости по причине:
							|%ИнформацияОбОшибке%';
							|en = 'Unsuccessful attempt to write news due to:
							|%ИнформацияОбОшибке%'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
						ОписаниеРезультата = ОписаниеРезультата
							+ ТекстСообщения
			...
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗаписыватьНовость%", ?(ЗаписыватьНовость = Истина, НСтр("ru = 'Будет записана';
																																		|en = 'It will be written'"), НСтр("ru = 'Будет пропущена';
																																									|en = 'Will be skipped'")));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = 'В базе еще не создана';
																																			|en = 'It has not been created in the base yet'"), НСтр("ru = 'Уже существует в базе';
																																												|en = 'Already exists in the infobase'")));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ДатаПубликацииИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru = '(в базе не было)';
																																					|en = '(it was not in the base)'"), " (" + Формат(ДатаПубликацииИБ, "ДЛФ=DT") + ")"));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗначениеПометкаУдаленияИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("ru = '(в базе не было)';
																																							|en = '(it was not in the base)'"), " (" + ЗначениеПометкаУдаленияИБ + ")"));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%УИН_ИБ%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = '(еще не создана)';
																																	|en = '(not created yet)'"), СтруктураНовости.УИН_ИБ));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%НавигационнаяСсылка%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("ru = '(еще не создана)';
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей';
				|en = 'ISL:News.News import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка новостей. Загрузка новости';
				|en = 'News. Import news. Import news'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Загрузка новостей';
						|en = 'ISL:News.News import'"), // ИмяСобытия
					НСтр("ru = 'Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных';
						|en = 'News. News import. News file parsing. News file parsing. News parsing. Read news. Binary data unit'"), // ИдентификаторШага
...
Процедура ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка(Обновление, КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Загрузка даты последнего обновления стандартного списка';
			|en = 'Import date of the last standard list update'"));
...
Функция ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Подключение к веб-сервису обновления классификаторов';
			|en = 'Connect to the web service of classifiers update'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка классификаторов';
				|en = 'ISL:News.Classifier import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков';
				|en = 'News. Import classifiers. Connection with web service of standard list update'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
