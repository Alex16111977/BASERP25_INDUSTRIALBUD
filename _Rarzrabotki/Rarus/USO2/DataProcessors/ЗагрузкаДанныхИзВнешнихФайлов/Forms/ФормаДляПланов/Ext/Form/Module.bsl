...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ОтборПоТипуНоменклатуры) Тогда
		...
		ТекстОтбора = НСтр("ru = 'Для выбора доступна номенклатура с типом: %Тип%.';
							|en = 'Products with type: %Тип% are available for selection.  '");
		Тип = "";
	...
	Иначе
		...
		Элементы.Шаг1Надпись1.Заголовок = НСтр("ru = 'Необходимо скопировать колонки в таблицу из внешнего файла (Excel. Word и пр.) через буфер обмена. Обязательно должна быть заполнена одна из колонок: ""Штрихкод"", ""Код"", ""Артикул"" или ""Номенклатура"".';
												|en = 'Copy columns to the table from an external file (Excel, Word, etc.) using the clipboard. One of the columns is required: ""Barcode"", ""Code"", ""product ID"" or ""Products"".'");
