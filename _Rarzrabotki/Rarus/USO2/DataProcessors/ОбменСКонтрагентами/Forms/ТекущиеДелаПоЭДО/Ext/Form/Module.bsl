...
Процедура ИнициализироватьРазделы()
	...
	ДобавитьОписаниеРаздела("Входящие", НСтр("ru = 'Входящие';
											|en = 'Incoming'"), , Ложь);
	ДобавитьОписаниеРаздела("Обработать", НСтр("ru = 'Отразить в учете';
												|en = 'Record in accounting'"));
	...
	ДобавитьОписаниеРаздела("Исходящие", НСтр("ru = 'Исходящие';
												|en = 'Outgoing'"), , Ложь);
	...
	ДобавитьОписаниеРаздела("Приглашения", НСтр("ru = 'Приглашения';
												|en = 'Invitations'"), , Ложь);
...
Функция ТекстЗапросаКоличестваЭлементовПоРазделуБезОтбора(Знач Раздел) 
	...
	ИначеЕсли Раздел = "ТребуетсяСогласие" Тогда
		...
		|		Справочник.СоглашенияОбИспользованииЭД КАК ОсновнаяТаблица
		|	ГДЕ
		|		ОсновнаяТаблица.СтатусПодключения = ЗНАЧЕНИЕ(Перечисление.СтатусыУчастниковОбменаЭД.ТребуетсяСогласие)
		|		ИЛИ (ОсновнаяТаблица.СтатусПодключения = ЗНАЧЕНИЕ(Перечисление.СтатусыУчастниковОбменаЭД.Присоединен)
		|			И ОсновнаяТаблица.Контрагент = &ПустойУчастникЭДО)) КАК ВложенныйЗапрос";
		
		МассивТипов = Метаданные.ОпределяемыеТипы.УчастникЭДО.Тип.Типы();
		Если МассивТипов.Количество() > 1 Тогда
			ПустойУчастникЭДО = "НЕОПРЕДЕЛЕНО";
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(МассивТипов[0]) Тогда
			ПустойУчастникЭДО = СтрШаблон("ЗНАЧЕНИЕ(%1.ПустаяСсылка)",
				ОбщегоНазначения.ИмяТаблицыПоСсылке(Новый (МассивТипов[0])));
		Иначе
			ПустойУчастникЭДО = """""";
		КонецЕсли;
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПустойУчастникЭДО", ПустойУчастникЭДО);
...
Процедура УстановитьБыстрыеОтборыПоРазделу(Знач Раздел, Знач БыстрыеОтборы, Список)
	...
	Для каждого СтрокаОтбора Из БыстрыеОтборы Цикл
		...
		ИначеЕсли Имя = "ВидЭД" Тогда
			...
			Иначе
				ОбластьДобавления = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
					ГруппаБыстрыйОтбор, НСтр("ru = 'Вид документа';
		...
		ИначеЕсли Имя = "ДополнительнаяИнформация" Тогда
			...
			ГруппаИЛИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
				ГруппаБыстрыйОтбор, НСтр("ru = 'Дополнительная информация';
										|en = 'Additional information'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИЛИ);
...
Процедура ОтклонитьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'При отклонении приглашений возникли ошибки.
			...
			|en = 'Errors occurred when rejecting the invitations.
			|Test EDI settings with the specified counterparties.'");
...
Процедура ПринятьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'При принятии приглашений возникли ошибки.
			|Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';
			|en = 'Errors occurred while accepting invitations.
			|It is required to test EDI settings with the specified counterparties.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'При отправке приглашений возникли ошибки.
			|Необходимо выполнить тест настроек ЭДО с указанными контрагентами.';
			|en = 'Errors occurred when sending the invitations.
			|Test EDI settings with the specified counterparties. '");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПакетыОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаголовокОповещения = НСтр("ru = 'Отправка электронных документов';
								|en = 'Electronic documents sending'");
	...
	Для Каждого СтрокаТаблицы Из МассивЭД Цикл
		...
		Исключение
			...
			ТекстОперации  = НСтр("ru = 'изменение статуса пакетов ЭД';
									|en = 'modification of ED package status'");
...
Процедура ОбновитьЗаголовокФормы()
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Текущие дела ЭДО (%1)';
								|en = 'Current EDI to-dos (%1)'"), СуффиксЗаголовка);
...
Процедура УстановитьСтатусОтменен(МассивЭД)
	...
	ТекстОповещения = НСтр("ru = 'Изменен статус пакетов на ""Отменен"": (%1)';
							|en = 'Status of packages is changed to ""Canceled"": (%1)'");
...
Процедура ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейНастроекЭДОМаркерам)
	...
	Для каждого НастройкаЭДО Из Элементы.ПриглашенияЭД.ВыделенныеСтроки Цикл
		...
		Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
			...
			ШаблонСообщения = НСтр("ru = 'Для отправки приглашения к обмену ЭД для получателя %1
										...
										|en = 'To send an invitation for ED exchange to recipient %1, 
										|fill in an email.'");
	...
	Для каждого КлючИЗначение Из СоответствиеПрофилейНастроекЭДОМаркерам Цикл
		...
		Если РезультатОтправки <> 0 Тогда
			Для каждого СтрокаТаблицы Из ТаблицаПриглашенийДляОтправки Цикл
				ИскомаяНастройкаЭДО = СтрокаТаблицы.НастройкаЭДО.ПолучитьОбъект();
			...
			КонецЦикла;
			ОтправленоПриглашений = ОтправленоПриглашений + ТаблицаПриглашенийДляОтправки.Количество();
		КонецЕсли;
	КонецЦикла;
...
Процедура ПринятьОтклонитьКонтактЧерезОператораЭДОНаСервере(КоличествоПринятыхПриглашений, СоответствиеПрофилейНастроекЭДОМаркерам, ПриглашениеПринято)
	...
	Для каждого НастройкаЭДО Из Элементы.ПриглашенияЭД.ВыделенныеСтроки Цикл
		...
		Если ПриглашениеПринято Тогда
			Если Не ОбменСКонтрагентамиВнутренний.ПроверитьЗаполнениеНастройкиЭДО(ИскомаяНастройкаЭДО) Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
