...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Закрыть.Заголовок = НСтр("ru = 'Закрыть';
											|en = 'Close'") 
		ИЛИ Элементы.Закрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО';
												|en = 'Complete connection to 1C:EDI'") Тогда
		ПрограммноеЗакрытие = Истина;
	КонецЕсли;
	
	Если ПодключитьЭДО 
		И НовыйСтатусДокумента = ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Одобрено")
		И Элементы.Закрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО';
											|en = 'Complete connection to 1C:EDI'") Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Закрыть мастер без подключения к сервису 1С-ЭДО?';
									|en = 'Close the wizard without connecting to 1C:EDI service?'");
...
Процедура НапечататьБланкСертификата(Команда)
	...
	ПечатныеФормы = УправлениеПечатьюКлиент.НоваяКоллекцияПечатныхФорм("ПечатьСертификат");
		...
		ИначеЕсли Элементы.ГруппаРасписка.ТекущаяСтраница = Элементы.ГруппаРаспискаПодписаниеРукой Тогда
			...
			Если Не ЗначениеЗаполнено(ФайлСканРасписка) Тогда
				ТекстСообщения = НСтр("ru = 'Укажите скан-копию подписанного бланка сертификата';
										|en = 'Specify a scanned copy of the signed certificate form'");
	...
	Иначе
		ТекстВопроса  = СтрШаблон(
			НСтр("ru = 'Не удалось выполнить подписание сертификата электронной подписью по причине:
                  ...
                  |en = 'Cannot digitally sign the certificate due to:
                  |%1
                  |
                  |Sign the certificate form manually and send the scanned copy?'"),
...
Процедура Шаг1ПослеАвтонастройки(СтруктураРезультат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Результат ИЛИ ИдентификаторАбонента = "" Тогда
		...
		ОбработатьОшибкиАвтонастройки(СтруктураРезультат);
...
Процедура Шаг1ВторичноеЗаявлениеПослеАвтонастройки()
	...
	Если Результат Тогда
		...
		СтатусКомментарий 	= НСтр("ru = 'Выполнено ';
									|en = 'Completed'") + ИзмененныеРеквизиты;
		Статус 				= ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Одобрено");
		НастройкаЗавершена 	= Истина;
		
		НастроитьДляПереходаИзДругойМодели();
		
	Иначе
		СтатусКомментарий 	= НСтр("ru = 'Заявление одобрено но не удалось создать учетную запись.';
									|en = 'Application is approved, but cannot create an account.'");
...
&НаСервере
Процедура НастроитьДляПереходаИзДругойМодели()
	
	УчетнаяЗапись = ДокументЗаявление.УчетнаяЗапись;
	
	Если УчетнаяЗапись.ЕстьОшибкиСменыМоделиХраненияКлючей Тогда
		
		// Дописываем пользователя для случая перехода в облако
		КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
		ТекущаяМодель 	   = КонтекстЭДОСервер.ТекущаяМодельРаботыСКлючами();
		ЭтоПереходВОблако  = ТекущаяМодель = Перечисления.МодельРаботыСКлючами.ВМоделиСервиса;
		
		Если ЭтоПереходВОблако 
			И УчетнаяЗапись.ЭлектроннаяПодписьВМоделиСервиса
			И УчетнаяЗапись.ЕстьОшибкиСменыМоделиХраненияКлючей Тогда
			
			СписокПользователей.Очистить();
			СписокПользователей.Добавить(Пользователи.ТекущийПользователь(),, Истина);
			ЗаписатьПользователейУчетныхЗаписейДокументооборота(УчетнаяЗапись);
			
		КонецЕсли;
		
		// Сбрасываем флаг ошибки
		УчетнаяЗаписьОбъект = УчетнаяЗапись.ПолучитьОбъект();
		УчетнаяЗаписьОбъект.ЕстьОшибкиСменыМоделиХраненияКлючей = Ложь;
...
&НаКлиенте
Процедура ОбработатьОшибкиАвтонастройки(СтруктураРезультат)
	
	Если ТипЗнч(СтруктураРезультат) = Тип("Структура") Тогда
		
		Если СтруктураРезультат.Свойство("ПараметрыФункции") 
			И ТипЗнч(СтруктураРезультат.ПараметрыФункции) = Тип("Структура") 
			И СтруктураРезультат.ПараметрыФункции.Свойство("ТекстОшибокДляМастераПодключенияК1СОтчетности")
			И ЗначениеЗаполнено(СтруктураРезультат.ПараметрыФункции.ТекстОшибокДляМастераПодключенияК1СОтчетности) Тогда
			ТекстОшибокДляМастераПодключенияК1СОтчетности = СтруктураРезультат.ПараметрыФункции.ТекстОшибокДляМастераПодключенияК1СОтчетности;
		КонецЕсли;
		
		Если СтруктураРезультат.Свойство("ОшибкаУстановкиЛичногоСертификата") 
			И ЗначениеЗаполнено(СтруктураРезультат.ОшибкаУстановкиЛичногоСертификата) Тогда
			ТекстОшибокДляМастераПодключенияК1СОтчетности = СтруктураРезультат.ОшибкаУстановкиЛичногоСертификата;
		КонецЕсли;
	...
	НеУдалосьПодключитьсяКСерверу = СтрНайти(ВРЕГ(ТекстОшибокДляМастераПодключенияК1СОтчетности), НСтр("ru = 'СЕРВЕР';
																										|en = 'SERVER'"));
	Элементы.ОсновнаяПанель.ТекущаяСтраница = Элементы.НастрокаПрограммыШаг1;
...
Процедура УправлениеКнопкамиНавигации()
	...
	Если ИндексТекущейСтраницы = 0 Тогда
		...
		ИначеЕсли Элементы.ГруппаСостоянийЗаявление.ТекущаяСтраница = Элементы.ГруппаСостояниеОшибкаПодключенияКСерверу Тогда
			КнопкаДалее.Заголовок 			= НСтр("ru = 'Обновить статус заявления';
													|en = 'Update the application status'");
	...
	ИначеЕсли ИндексТекущейСтраницы = 1 Тогда
		КнопкаЗакрыть.Заголовок 			= НСтр("ru = 'Отмена';
													|en = 'Cancel'");
		КнопкаДалее.Заголовок 				= НСтр("ru = 'Подписать и отправить сертификат';
													|en = 'Sign and send sertificate'");
		КнопкаДалее.Видимость 				= Истина;
    	КнопкаНазад.Видимость 				= Истина;
		КнопкаДалее.КнопкаПоУмолчанию 		= Истина;		    
    ...
    ИначеЕсли ИндексТекущейСтраницы = ВсегоСтраниц - 1 Тогда
		...
		Если ПодключитьЭДО Тогда
			КнопкаЗакрыть.Заголовок 		= НСтр("ru = 'Завершить подключение к 1С-ЭДО';
													|en = 'Complete connection to 1C:EDI'");
		Иначе
			КнопкаЗакрыть.Заголовок 		= НСтр("ru = 'Закрыть';
													|en = 'Close'");
		КонецЕсли;
	...
	Если КнопкаЗакрыть.Заголовок = НСтр("ru = 'Закрыть';
										|en = 'Close'") 
		ИЛИ КнопкаЗакрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО';
											|en = 'Complete connection to 1C:EDI'") Тогда
...
Процедура ЗакрытьМастер()
	...
	Если Элементы.Закрыть.Заголовок = НСтр("ru = 'Завершить подключение к 1С-ЭДО';
											|en = 'Complete connection to 1C:EDI'")
		И ПодключитьЭДО 
...
Процедура НастроитьВнешнийВидЭлементовУправления()
	...
	Если ЭтоВторичноеЗаявление Тогда
		...
		ИзмененныеРеквизиты = "";
		ДобавитьРеквизитКСтроке(НСтр("ru = 'изменение состава контролирующих органов';
									|en = 'change regulatory authorities '"), ИзменитьСоставКонтролирующихОрганов, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru = 'изменение сотрудника-владельца сертификата';
									|en = 'change the employee owning the certificate'"), ИзменитьВладельцаСертификата, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru = 'переиздание сертификата';
									|en = 'reissue certificate'"), ПереиздатьСертификат, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru = 'продление лицензии на 1С-Отчетность';
									|en = '1C Reporting license extension:'"), ПродлитьЛицензиюНа1СОтчетность, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru = 'изменение номера мобильного телефона для SMS-уведомления';
									|en = 'change cell phone number for SMS notification '"), ИзменитьМобильныйТелефон, ИзмененныеРеквизиты);
		ДобавитьРеквизитКСтроке(НСтр("ru = 'изменение реквизитов подключения к 1С-Отчетности';
									|en = 'change 1C Reporting connection parameters'"), ИзменитьРеквизитыПодключенияК1СОтчетности, ИзмененныеРеквизиты);
		...
		ТекстЗаголовкаПриОдобрении = НСтр("ru = 'Ваше заявление%1одобрено';
											|en = 'Your application is%1 approved'");
		ТекстЗаголовкаПриОдобрении = СтрЗаменить(ТекстЗаголовкаПриОдобрении, "%1", 
			?(ПустаяСтрока(ИзмененныеРеквизиты), " ", НСтр("ru = ' на ';
															|en = ' to'") + ИзмененныеРеквизиты + " "));
		...
		ТекстЗаголовкаПриОтклонении = НСтр("ru = 'Ваше заявление%1отклонено';
											|en = 'Your application%1 is rejected  '");
		...
		Если ПереиздатьСертификат Тогда
			ТекстСообщения = НСтр("ru = 'Далее произойдет установка сертификатов и автоматическая настройка электронного документооборота.';
									|en = 'Certificates will be installed and electronic data interchange will be configured automatically.  '");
		Иначе
			ТекстСообщения = НСтр("ru = 'Далее произойдет автоматическая настройка электронного документооборота.';
									|en = 'Electronic data interchange will be configured automatically. '");
		КонецЕсли;
		...
		Элементы.ПодсказкаКонечныйРезультат.Заголовок = НСтр("ru = 'Поздравляем!
			...
			|en = 'Congratulations!
			|New settings of connection to 1C Reporting have been applied successfully'");
		...
		Элементы.ОткрытьСсылкуНаСтатьюПоПодключению.Заголовок = НСтр("ru = 'Инструкция по изменению настроек подключения к 1С-Отчетности';
																	|en = 'How to change 1C Reporting connection settings'");
			
	Иначе
			
		Заголовок =  НСтр("ru = 'Помощник подключения к 1С-Отчетности';
							|en = '1C Reporting connection wizard'");
...
Процедура ПриДобавленииКлючаВРеестр()
	...
	ВидимостьПредупреждения = ДокументЗаявление.ВключитьЛицензиюКриптоПро И НЕ УдалосьПогаситьВыводОшибок;
	Элементы.ПредупреждениеКриптоПро.Видимость  = ВидимостьПредупреждения;
	Элементы.ПредупреждениеКриптоПро1.Видимость = ВидимостьПредупреждения;
