...
Процедура ВыгрузитьПакеты(ВидимыеЦиклы)
	...
	Если ВидимыеЦиклы.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны циклы обмена!';
										|en = 'Exchange cycles are not selected.'"));
	Иначе
		Если ВидимыеЦиклы.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузить цикл обмена ""%1""?';
																						|en = 'Export the ""%1"" exchange cycle?'"), КонтекстЭДОКлиент.ПредставлениеЦиклаОбмена(ВидимыеЦиклы[0]));
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузить выбранные %1 циклов обмена?';
																						|en = 'Export the selected %1 of exchange cycles?'"), Строка(ВидимыеЦиклы.Количество()));
		КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны отправки ФСС!';
										|en = 'Sendings to FTS are not selected.'"));
		Возврат;
	КонецЕсли;
	...
	Если РежимВыгрузки = 0 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Зашифрованный пакет отсутствует.';
											|en = 'The encrypted package is missing.'"));
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru = 'Выберите каталог для выгрузки отправленного пакета';
								|en = 'Select directory to export the sent package'");
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОбУчастииВМеждународнойГруппеКомпаний", НСтр("ru = 'Уведомления об участии в международной группе компаний';
																								|en = 'Notifications of participation in international company group'"));
	КонецЕсли;
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		ИначеЕсли РезультатВыбора.Значение = "РеестрыФНС" Тогда
			...
			ТекстВопроса = НСтр("ru = 'Создание реестров НДС выполняется из формы ""1С-Отчетность"".
                                 ...
                                 |en = 'VAT registries are created from the ""1C Reporting"" form.
                                 |Go to the ""1C Reporting"" form?'");
		...
		ИначеЕсли РезультатВыбора.Значение = "РеестрыАкцизы" Тогда
			...
			ТекстВопроса = НСтр("ru = 'Создание реестров по акцизам выполняется из формы ""1С-Отчетность"".
                                 |Перейти к форме ""1С-Отчетность""?';
                                 |en = 'Excise registries are created from the 1C Reporting form.
                                 |Go to the 1C Reporting form?'");
...
Функция СодержимоеОтправкиФСРАРВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСРАР, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Протокол';
									|en = 'Protocol'") + " " + СсылкаНаОбъектОтправкиФСРАР["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
...
Процедура ВыгрузитьСодержимоеОтправкиФСРАР(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны отправки в Росалкогольрегулирование!';
										|en = 'Sendings to Federal Service for Alcohol Market Regulation are not selected.'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиРПНВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиРПН, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Протокол';
									|en = 'Protocol'") + " " + СсылкаНаОбъектОтправкиРПН["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиРПН(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не выбраны отправки в Росприродназдор!';
									|en = 'Sendings to Federal Supervisory Natural Resources Management Service are not selected.'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	Если РежимВыгрузки = 0 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Подпись';
									|en = 'Signature'") + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 1 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Квитанция"];
		ИмяФайлаДляКлиента = НСтр("ru = 'Квитанция';
									|en = 'Receipt'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"];
	ИначеЕсли РежимВыгрузки = 2 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФТС["Протокол"];
		ИмяФайлаДляКлиента = НСтр("ru = 'Протокол';
									|en = 'Protocol'") + " " + СсылкаНаОбъектОтправкиФТС["ИдентификаторОтправкиНаСервере"] + ".html";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФТС(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не выбраны отправки в ФТС!';
									|en = 'Sendings to FCS are not selected.'"));
		Возврат;
	КонецЕсли;
...
Функция ПолучитьНастройкиДляДинамическогоСписка(ИмяДинамическогоСписка)
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаЗаявление" Тогда
		...
		Если ВидДокументаЗаявленияОВвозе = "Заявление" Тогда
			...
			|{ГДЕ
			|	(ТранспортноеСообщение.ЦиклОбмена.Организация В (&Организация)) КАК Поле2}
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	ЦиклОбмена
			|;
			...
			|{ГДЕ
			|	(КрайнееСообщение.ЦиклОбмена.Организация В (&Организация)) КАК Поле2}
			|
			|СГРУППИРОВАТЬ ПО
			|	КрайнееСообщение.ЦиклОбмена
			|
			...
			|	И ЦиклыОбмена.ПометкаУдаления = ЛОЖЬ
			|{ГДЕ
			|	(ЦиклыОбмена.Организация В (&Организация)) КАК Поле2}";
			#КонецОбласти
...
Процедура СообщитьОСменеРежимов(ПредыдущийРежим, НовыйРежим)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Режим отображения списка был переключен с ""%1"" на ""%2""';
																		|en = 'List display mode was switched from ""%1"" to ""%2"" '"), ПредыдущийРежим, НовыйРежим);
