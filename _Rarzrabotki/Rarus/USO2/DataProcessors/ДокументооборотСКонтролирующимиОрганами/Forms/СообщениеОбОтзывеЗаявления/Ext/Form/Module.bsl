	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка выгрузки сообщения об отзыве во временный файл:
                  |%1';
                  |en = 'An error occurred while exporting a revocation message to the temporary file:
	...
	Если НЕ ЗначениеЗаполнено(УзелСвОтзыв) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Некорректная структура XML файла: не обнаружен узел ""СвОтзыв"".';
				|en = 'Incorrect structure of XML file: the ""СвОтзыв"" node is not found.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Некорректная структура XML файла: некорректное значение атрибута ""ПричЗам"".';
					|en = 'Incorrect structure of XML file: incorrect value of the ""ПричЗам"" attribute.'"),,,, Отказ);
			Возврат;	
		КонецЕсли;
	Иначе
