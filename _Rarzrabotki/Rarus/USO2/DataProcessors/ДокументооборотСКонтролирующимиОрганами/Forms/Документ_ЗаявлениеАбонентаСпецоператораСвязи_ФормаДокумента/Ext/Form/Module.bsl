...
Процедура ОбновитьСтатус(Команда)
	...
	ИначеЕсли Объект.Статус=ПредопределенноеЗначение("Перечисление.СтатусыЗаявленияАбонентаСпецоператораСвязи.Отправлено") Тогда
		...
		ТекстВопроса = НСтр("ru = 'Вы уверены, что хотите проверить наличие ответа на заявление?';
							|en = 'Are you sure you want to check if a response to your application is available?'");
...
Процедура ПроверитьЗаполнениеДокумента(ВыполняемоеОповещение)
	...
	Если ПустаяСтрока(Объект.ПолноеНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите полное наименование организации.';
																|en = 'Specify full name of the company.'"), ,"Объект.ПолноеНаименование");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.КраткоеНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите краткое наименование организации.';
																|en = 'Specify the company''s short name.'"), ,"Объект.КраткоеНаименование");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	СтрЦифры = "0123456789";
	Если ПустаяСтрока(Объект.НомерОсновнойПоставки1с) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан регистрационный номер.';
																|en = 'Registration number is not specified.'"), ,"Объект.НомерОсновнойПоставки1с");
		ИтогПроверки = Ложь;
	КонецЕсли;

	Если ПустаяСтрока(Объект.ИНН)Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите ИНН организации.';
																|en = 'Specify company TIN.'"), ,"Объект.ИНН");
		ИтогПроверки = Ложь;
	ИначеЕсли КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ИНН,10) Тогда
		...
		Если НЕ (КонтекстЭДОКлиент.ПроверитьКПП(Объект.КПП)) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите КПП организации.';
																	|en = 'Specify company CRTR.'"), ,"Объект.КПП");
			ИтогПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ОГРН, 13)) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите ОГРН организации.';
																	|en = 'Specify the company registration number.'"), ,"ОГРН");
			ИтогПроверки = Ложь;
		КонецЕсли;
	...
	ИначеЕсли КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ИНН,12) Тогда
		...
		Если (КонтекстЭДОКлиент.ПроверитьКПП(Объект.КПП)) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Указан ИНН физического лица и КПП юридического лица.';
																	|en = 'Individual''s TIN and CRTR of the business entity are specified.'"), ,"Объект.КПП");
			ИтогПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ (КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ОГРН, 15, Истина)) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно заполнен ОГРН организации.';
																		|en = 'Company registration number is filled in incorrectly.'"), ,"ОГРН");
				ИтогПроверки = Ложь;
		КонецЕсли;
		
		Объект.ТипОрганизации = Ложь;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно заполнен ИНН организации.';
																|en = 'Company TIN is filled in incorrectly.'"), ,"Объект.ИНН");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьРегистрационныйНомерПФР(Объект.РегНомерПФР, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно заполнен регистрационный номер в ПФР.';
																|en = 'Registration number in PF is filled in incorrectly.'"), ,"Объект.РегНомерПФР");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.РегНомерФСС, 10, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно заполнен регистрационный номер в ФСС.';
																|en = 'Registration number in SSF is filled in incorrectly.'"), ,"Объект.РегНомерФСС");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(Объект.ДополнительныйКодФСС, 10, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно заполнен дополнительный регистрационный номер в ФСС.';
																|en = 'Additional registration number in SSF is filled in incorrectly.'"), ,"Объект.ДополнительныйКодФСС");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(Объект.ЭлектроннаяПочта) Тогда
		...
		|реквизит необязательный для заполнения.';
		|en = 'Email address is filled in incorrectly, 
		|optional attribute.'"), ,"Объект.ЭлектроннаяПочта");
		ИтогПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ТелефонОсновной) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите основной телефон организации.';
																|en = 'Specify main phone of the company.'"), ,"Объект.ТелефонОсновной");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.АдресЮридический) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан юридический адрес.';
																|en = 'Legal address is not specified.'"), ,"ЮрАдрес");
		ИтогПроверки = Ложь;
	Иначе
		...
		Если ИтогПроверкиАдресЮр тогда
			...
			Если Не ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Некорректно заполнен юридический адрес: %1';
																																|en = 'Legal address is filled in incorrectly: %1'"),СтрокаОшибкаАдреса), ,"ЮрАдрес");
				ИтогПроверки = Ложь;
			КонецЕсли;
		Иначе
			ИтогПроверки = Ложь;
	...
	Если ПустаяСтрока(Объект.АдресФактический) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан фактический адрес.';
																|en = 'Physical address is not specified.'"), ,"ФактАдрес");
		ИтогПроверки = Ложь;
	Иначе
		...
		Если ИтогПроверкиАдресФакт тогда
			...
			Если Не ПустаяСтрока(СтрокаОшибкаАдреса) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Некорректно заполнен фактический адрес: %1';
																																|en = 'Physical address is filled in incorrectly: %1'"),СтрокаОшибкаАдреса), ,"ФактАдрес");
				ИтогПроверки = Ложь;
			КонецЕсли;
		Иначе
			ИтогПроверки = Ложь;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПФамилия) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите фамилию сотрудника.';
																|en = 'Specify employee last name.'"), ,"Объект.ВладелецЭЦПФамилия");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПИмя) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите имя сотрудника.';
																|en = 'Specify employee name.'"), ,"Объект.ВладелецЭЦПИмя");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ДокументооборотСКОКлиентСервер.ПроверитьСНИЛС(Объект.ВладелецЭЦПСНИЛС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно указан СНИЛС сотрудника. Не соответствует маске 999-999-999 99';
																|en = 'Employee IIAN is specified incorrectly. It does not correspond to mask 999-999-999 99'"), ,"Объект.ВладелецЭЦПСНИЛС");
		ИтогПроверки = Ложь;
	ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьСНИЛС(Объект.ВладелецЭЦПСНИЛС, Ложь, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно указан СНИЛС сотрудника. Не сошлось контрольное число (СНИЛС не существует)';
																|en = 'Employee''s IIAN is specified incorrectly. Checksum is not identical (IIAN does not exist)'"), ,"Объект.ВладелецЭЦПСНИЛС");
		ИтогПроверки = Ложь;
	ИначеЕсли Объект.ВладелецЭЦПСНИЛС = "000-000-000 00" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Предупреждение. Указан ""нулевой"" СНИЛС 000-000-000 00. Обычно это допустимо только для иностранных граждан.';
																|en = 'Warning. ""Zero"" IIAN is specified 000-000-000 00. It is usually allowed only to foreign citizens.'"));
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецЭЦППол) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните поле ""Пол""';
																|en = 'Fill in the ""Sex"" field'"), ,"Объект.ВладелецЭЦППол");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если Объект.ТипОрганизации Тогда
		...
		Если ПустаяСтрока(Объект.ВладелецЭЦПДолжность) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите должность сотрудника.';
																	|en = 'Specify employee position.'"), ,"Объект.ВладелецЭЦПДолжность");
			ИтогПроверки = Ложь;
		ИначеЕсли СтрДлина(Объект.ВладелецЭЦПДолжность) > 64 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Длина значения в поле Должность не должна превышать 64 символа.';
																	|en = 'Value length in the Position field cannot be longer than 64 characters.'"), ,"Объект.ВладелецЭЦПВидДокумента");
			ИтогПроверки = Ложь;
		КонецЕсли;
	...
	Если СтрДлина(Объект.ВладелецЭЦППодразделение) > 64 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Длина значения в поле Подразделение не должна превышать 64 символа.';
																|en = 'Value length in the Department field cannot be longer than 64 characters.'"), ,"Объект.ВладелецЭЦППодразделение");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецЭЦПГражданство) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните поле ""Гражданство""';
																|en = 'Fill in the ""Citizenship"" field'"), ,"Объект.ВладелецЭЦПГражданство");
		ИтогПроверки = Ложь;
	ИначеЕсли НЕ ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.КодАльфа2Заполнен(Объект.ВладелецЭЦПГражданство)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните код альфа-2 у страны, указанной в качестве гражданства сотрудника-владельца ЭП';
																|en = 'Specify alpha code-2 of the country specified as citizenship of the employee owning DS '"), ,"Объект.ВладелецЭЦПГражданство");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПВидДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите вид документа, удостоверяющего личность сотрудника.';
																|en = 'Specify identity document kind of the employee.'"), ,"Объект.ВладелецЭЦПВидДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПСерияДокумента) И НЕ КонтекстЭДОКлиент.ПроверитьСериюДокумента(Объект.ВладелецЭЦПВидДокумента,Объект.ВладелецЭЦПСерияДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите серию документа удостоверяющего личность сотрудника.';
																|en = 'Specify series of the employee''s identity document.'"), ,"Объект.ВладелецЭЦПСерияДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПНомерДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите номер документа удостоверяющего личность сотрудника.';
																|en = 'Specify the employee''s identity document number.'"), ,"Объект.ВладелецЭЦПНомерДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.ВладелецЭЦПКемВыданДокумент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите кем выдан документ удостоверяющий личность сотрудника.';
																|en = 'Specify the person who issued the employee''s identity document.'"), ,"Объект.ВладелецЭЦПКемВыданДокумент");
		ИтогПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецЭЦПДатаВыдачиДокумента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите дату выдачи документа удостоверяющего личность сотрудника.';
																|en = 'Specify date of issuance of the employee''s identity document.'"), ,"Объект.ВладелецЭЦПДатаВыдачиДокумента");
		ИтогПроверки = Ложь;
	ИначеЕсли Объект.ВладелецЭЦПДатаВыдачиДокумента > КонтекстЭДОКлиент.ТекущаяДатаНаСервере() ИЛИ Год(Объект.ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Указана недопустимая дата выдачи документа удостоверяющего личность.';
																|en = 'Invalid issue date of identity document is specified.'"), ,"Объект.ВладелецЭЦПДатаВыдачиДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоПаспортРФ(Объект.ВладелецЭЦПВидДокумента) Тогда
		...
		Если ПустаяСтрока(КодПодразделенияБезТире) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните поле ""Код подразделения""';
																	|en = 'Fill in the ""Department code"" field'"), ,"Объект.ВладелецЭЦПКодПодразделения");
			ИтогПроверки = Ложь;
		ИначеЕсли СтрДлина(Объект.ВладелецЭЦПКодПодразделения) <> 7
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно указан код подразделения. Не соответствует маске ХХХ-ХХХ, где X – любая цифра';
																	|en = 'Department code is specified incorrectly. It does not correspond to the XXX-XXX mask, where X is any digit.'"), ,"Объект.ВладелецЭЦПКодПодразделения");
			ИтогПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.ВладелецЭЦПДатаРождения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните поле ""Дата рождения""';
																|en = 'Fill in the ""Date of birth"" field'"), ,"Объект.ВладелецЭЦПДатаРождения");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПМестоРождения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните поле ""Место рождения""';
																|en = 'Fill in the ""Birthplace"" field'"), ,"Объект.ВладелецЭЦПМестоРождения");
		ИтогПроверки = Ложь;
	КонецЕсли;
	
	Если Объект.Получатели.Количество() < 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите хотя бы одно направление сдачи, ФНС или ПФР.';
																|en = 'Specify at least one submission direction: to FTS or PF.'"), ,"Объект.Получатели");
		ИтогПроверки = Ложь;
	ИначеЕсли Объект.Получатели.Количество()=1 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Среди направлений сдачи должен быть ФНС или ПФР.';
																	|en = 'FTS or PF should be among submission directions.'"), ,"Объект.Получатели");
			ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если Объект.Получатели.НайтиСтроки(ОтборФСГС).Количество() > 0
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для подключения направления Росстат должно быть подключено направление ФНС, необходимо добавить направление ФНС.';
																|en = 'To connect Federal State Statistics Service direction, connect FTS direction. You need to add FTS direction.'"), ,"Объект.Получатели");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если ПустаяСтрока(Объект.РегНомерПФР)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Указано направление ПФР, но в сведениях об организации не заполнен регистрационный номер в ПФР.';
																|en = 'PF direction is specified but PF registration number is not specified in the company information.'"), ,"Объект.РегНомерПФР");
		ИтогПроверки = Ложь;
	КонецЕсли;
	...
	Если Объект.ЭлектроннаяПодписьВМоделиСервиса Тогда
		Если Не ЗначениеЗаполнено(Объект.ИдентификаторПроверкиТелефонаДляПаролей) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Проверка телефона для получения паролей не выполнена. Воспользуйтесь мастером подключения для отправки заявления.';
																	|en = 'Phone number for obtaining passwords is not checked. Use the connection wizard to send the application.'"));
			ИтогПроверки = Ложь;
		КонецЕсли;
	Иначе
		Если ВыборКриптопровайдера = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран криптопровайдер.';
																	|en = 'Select cryptographic service provider.'"), ,"ВыборКриптопровайдера");
			ИтогПроверки = Ложь;
		Иначе
	...
	Если Объект.ПодатьЗаявкуНаСертификатДляФСРАР Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.КодРегионаФСРАР) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'На закладке ""Прочие контролирующие органы"" не указан код региона Росалкогольрегулирования';
																	|en = 'Region code of Federal Service for Alcohol Market Regulation is not specified on the ""Other regulatory authorities"" tab'"), ,"Объект.КодРегионаФСРАР");
			ИтогПроверки = Ложь;
		КонецЕсли;
...
Функция ПроверкаЗаполненияУЦ()
	Если НЕ ЗначениеЗаполнено(Объект.СпецоператорСвязи) ИЛИ СпецоператорИУЦ = Неопределено тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран Удостоверяющий центр.';
																|en = 'Certification authority is not selected.'"), ,"СпецоператорИУЦ");
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Процедура ОтправитьЗаявлениеНаСервер()
	...
	ТекстВопроса = НСтр("ru = 'Нажимая <Продолжить>, вы подтверждаете, что принимаете ""Соглашение о присоединении к регламенту удостоверяющего центра"". Продолжить ?';
						|en = 'By clicking <Continue> you agree to accept ""Agreement on joining the Regulation of Certification authority"". Continue?'");
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось записать заявление по причине: %1';
					|en = 'Cannot write the application due to: %1'"),
				ОписаниеОшибки()));
		Возврат;
	КонецПопытки;
...
Процедура ПреобразоватьАдресКодРегиона(Адрес,ТипАдреса,ИтогПроверки)
	...
	Иначе
		...
		Если ПустаяСтрока(КодРегиона) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, неверно указан регион.';
																															|en = '%1, region is specified incorrectly.'"),?(ТипАдреса = 0," Юридический адрес"," Фактический адрес")), ,?(ТипАдреса = 0,"ЮрАдрес","ФактАдрес"));
			ИтогПроверки = Ложь;
		Иначе
...
Функция ПроверитьОрганизацию()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Сначала укажите организацию!';
																|en = 'Specify company first.'"), ,"Объект.Организация");
		Булево = Истина;
	КонецЕсли;
	Возврат Булево;
КонецФункции
...
Процедура ОбновитьСтатусПослеПроверкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ТекстВопроса = НСтр("ru = 'Вы уверены, что хотите отправить заявление на регистрацию?';
							|en = 'Are you sure you want to send your registration application?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	КонецЕсли;
...
Функция ПолучитьКодыРосстатПрописью(Знач Получатели)
	...
	ИначеЕсли КоличествоКодовФСГС > 1 Тогда
		Результат = НСтр("ru = 'Коды Росстата (%1)';
						|en = 'Russian Federal State Statistics Service codes (%1)'");
