			...
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сертификат с отпечатком ""%1"" не найден в хранилище сертификатов.';
						|en = 'Certificate with thumbprint ""%1"" is not found in the certificate storage.'"),
	...
	Если ПризнакВыбораНесколькихСтрок Тогда
		...
		Если ОдинИзСертификатовПросрочен Тогда
			...
			ТекстВопроса = НСтр("ru = 'Среди выбранных сертификатов есть такие, срок действия которых истек.
								...
								|en = 'There are expired certificates among the selected ones.
								|Are you sure you want to continue selection?'");
	...
	Иначе
		...
		Если НЕ ЭтоСертификатГОСТ2012(ТекДанные) Тогда
			ТекстПредупреждения = НСтр("ru = 'Выбран сертификат с алгоритмом подписи, который не удовлетворяет требованиям Минобороны РФ.
										...
										|en = 'You selected a certificate with a signature algorithm that does not comply with the requirements of the Ministry of Defence of the Russian Federation.
										|Specify a qualified certificate with GOST R 34.10-2012 signature algorithm.'");
