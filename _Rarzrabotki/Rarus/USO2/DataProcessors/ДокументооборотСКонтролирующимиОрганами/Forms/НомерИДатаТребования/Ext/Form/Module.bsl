...
Процедура ПродолжитьОтправку(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Номер) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните номер требования';
																|en = 'Fill the number of the request'"), ,"Номер");
		ЕстьОшибка = Истина;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Дата) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните дату требования';
																|en = 'Fill the date of the request'"), ,"Дата");
		ЕстьОшибка = Истина;
	КонецЕсли;
...
Функция ПолучитьВложениеНаСервере(ИмяФайла)
	...
	Если СтрВложения.Количество() = 0 Тогда
		Результат.ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вложение с именем  %1 не обнаружено.';
																									|en = 'Insertion with the name %1 is not found.'"), Символ(34) + ИмяФайла + Символ(34));
		Возврат Результат;
	КонецЕсли;
