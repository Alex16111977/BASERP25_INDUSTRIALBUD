...
Функция ИнформацияОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ru = 'Правила регистрации объектов этой информационной базы (%1) от %2';
							|en = 'Object registration rules of this infobase (%1) from %2'");
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';
								|en = 'Exchange plan %1 is specified in the registration rules, and import in progress is for exchange plan %2'");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(1, НСтр("ru = 'Ошибка открытия файла правил';
								|en = 'An error occurred when opening the rule file'", КодОсновногоЯзыка));
	Сообщения.Вставить(2, НСтр("ru = 'Ошибка при загрузке правил';
								|en = 'An error occurred when importing rules'", КодОсновногоЯзыка));
	Сообщения.Вставить(3, НСтр("ru = 'Ошибка формата правил';
								|en = 'Rule format error'", КодОсновногоЯзыка));
	Сообщения.Вставить(4, НСтр("ru = 'Ошибка при получении файла правил для чтения';
								|en = 'An error occurred when receiving a reading rule file'", КодОсновногоЯзыка));
	Сообщения.Вставить(5, НСтр("ru = 'Загружаемые правила регистрации не предназначены для текущего плана обмена.';
								|en = 'Imported registration rules are not for the current exchange plan.'", КодОсновногоЯзыка));
