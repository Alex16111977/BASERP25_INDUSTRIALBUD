...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьOmnicomm.Получить() Тогда
		ТекстСообщения = НСтр("en='Opening is possible only when use of GPS - Omnicomm is enabled.';ru='Открытие возможно только тогда, когда включено использование системы GPS - Omnicomm.'");
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	// Выполним загрузку.
	Если Не Отказ Тогда 
		ТекстНСТР = НСтр("en='Omnicomm GPS system data downloading is in progress';ru='Выполняется загрузка данных системы GPS Omnicomm'");
...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	мТС = ТС.Выгрузить().ВыгрузитьКолонку("Ссылка");
	КодВозврата = уатЗащищенныеФункцииСервер_проф.ПолучитьУчетнуюЗаписьСистемыМониторингаДляТС(мТС, Перечисления.уатСистемаGPS.Omnicomm);
	
	Для Каждого ТекСтрока Из мТС Цикл
		...
		Если КодВозврата=0 И ЗагрузитьПробегИРасходГСМ Тогда // Загружаем пробег и расход ГСМ.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеТСOmnicommВРегистр(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки,,УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // Загружаем дополнительные сведения.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияТСOmnicommВРегистр(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки,,УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата=0 И ЗагрузитьКоординаты Тогда // Загружаем координаты.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьКоординатыТСOmnicommВРегистр(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки,,УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
