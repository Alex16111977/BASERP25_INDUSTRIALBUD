...
Процедура ТаблицаЗапасы(Параметры, АдресРезультата) Экспорт
	...
	ТекстыЗапросов.Добавить(ОбщегоНазначенияУТ.РазделительЗапросовВПакете());
	ТекстыЗапросов.Добавить(РегистрыСведений.СхемыОбеспечения.ВременнаяТаблицаРазличныеТоварыИСклады("ВтТовары"));
	ТекстыЗапросов.Добавить(ОбщегоНазначенияУТ.РазделительЗапросовВПакете());
...
Процедура ТаблицаПотребностейПоЗаказамИОстатков(Параметры, АдресРезультата) Экспорт
	...
	?(Параметры.ТипОтбора = "ОтборПоЗначениям", "",
		...
		|	НЕ РазличныеТоварыИСклады.Номенклатура ЕСТЬ NULL КАК РаспределительныйЦентрЕстьВТаблице,") + "
	...
	?(Параметры.ТипОтбора = "ОтборПоЗначениям", "",
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РазличныеТоварыИСклады КАК РазличныеТоварыИСклады
		|		ПО РазличныеТоварыИСклады.Номенклатура   = Т.Номенклатура
		|		 И РазличныеТоварыИСклады.Характеристика = Т.Характеристика
		|		 И РазличныеТоварыИСклады.Назначение     = Т.Назначение
		|		 И РазличныеТоварыИСклады.Склад          = ТопологияСкладов.Центр") + "
		|
	...
	Если Параметры.ТипОтбора <> "ОтборПоЗначениям" Тогда
		...
		Текст =
			...
			|				ИЗ
			|					РазличныеТоварыИСклады КАК Фильтр)) КАК Работы
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|			
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РазличныеТоварыИСклады КАК Фильтр
...
Процедура УдалитьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		ИмяСобытияЖурнала = НСтр("ru = 'Формирование заказов: удаление документов';
								|en = 'Order generation: document deletion'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗаписатьДокумент(ДокументОбъект, СообщенияОбОшибках)
	...
	Иначе //нет ошибок, проводим документ
		...
		Исключение
			...
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
			ИмяСобытияЖурнала = НСтр("ru = 'Обеспечение потребностей.Создание заказов';
									|en = 'Demand fulfillment.Order creation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ВыполнитьОбеспечениеЗаказа(Заказ, ИндексыСтрок, ПереченьВариантов)
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ru = 'Невозможно разместить заказ %1. Возможно документ редактируется другим пользователем.';
								|en = 'Cannot place order %1. Document may be edited by other user.'");
		Сообщение.Текст = СтрШаблон(Сообщение.Текст, Документ.Ссылка);
		Сообщение.Сообщить();
		ИмяСобытияЖурнала = НСтр("ru = 'Обеспечение потребностей.Размещение в заказах';
								|en = 'Demand fulfillment.Placement in orders'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЕстьОшибки = Истина;
		Возврат ЕстьОшибки;
	КонецПопытки;
	...
	Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Невозможно разместить заказ %1. Неверные данные об отгрузках / оформлении накладных по заказу.';
									|en = 'Cannot place order %1. Incorrect data on shipments / registration of invoices against the order.'");
			Сообщение.Текст = СтрШаблон(Сообщение.Текст, Документ.Ссылка);
			Сообщение.Сообщить();
			ИмяСобытияЖурнала = НСтр("ru = 'Обеспечение потребностей.Размещение в заказах';
									|en = 'Demand fulfillment.Placement in orders'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗаполнитьОбеспечениеДляЗаказаНаПроизводство(МенеджерДокумента, Ссылка, ТаблицаОбеспечения, ТаблицаТовары)
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		Исключение
			...
			ИмяСобытияЖурнала = НСтр("ru = 'Состояние обеспечения заказов.Выполнение рекомендаций';
									|en = 'Order fulfillment state.Recommendation fulfillment'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ИмяСобытияЖурнала, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ТаблицаПолуфабрикатов(МенеджерВременныхТаблиц)
	...
	Возврат РегистрыНакопления.ОбеспечениеПроизводственныхПроцессов.ПолуфабрикатыВПроизводстве(
		МенеджерВременныхТаблиц, "РазличныеТоварыИСклады");
