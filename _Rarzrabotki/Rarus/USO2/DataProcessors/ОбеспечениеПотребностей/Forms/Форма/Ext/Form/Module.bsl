...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВесЗаказа.Заголовок = СтрШаблон(НСтр("ru = 'Вес (%1)';
													|en = 'Weight (%1)'"), ЕдиницаИзмеренияВеса);
	
	ВалютаУправленческогоУчета = Константы.ВалютаУправленческогоУчета.Получить();
	Элементы.СуммаЗаказа.Заголовок = СтрШаблон(НСтр("ru = 'Сумма (%1)';
													|en = 'Amount (%1)'"), ВалютаУправленческогоУчета);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		Элементы.ПартнерСклад.Заголовок = НСтр("ru = 'Партнер/Склад/Подразделение';
												|en = 'Partner/Warehouse/Department'");
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ИначеЕсли ЗавершениеРаботы 
		...
		ТекстПредупреждения = НСтр("ru = 'При продолжении закрытия не будет выполнено размещение заказов на отгрузку
								...
								|en = 'If you continue closing, shipment orders will not be placed
								|in the generated delivery orders.'");
		Возврат;
...
Процедура ЗаказПереработчикуКИсполнению(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Невозможно установить статус. Не выбраны заказы переработчику';
																|en = 'Cannot set the status. Tolling orders are not selected'"));
	КонецЕсли;
	//-- НЕ УТ
	Возврат;
...
Процедура ЗаказНаПроизводствоКПроизводству(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Невозможно установить статус. Не выбраны заказы на производство';
																|en = 'Cannot set the status. Production orders are not selected'"));
	КонецЕсли;
	//-- НЕ УТКА
	Возврат;
...
Функция ПолучениеТаблицыСпособовОбеспеченияВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Формирование заказов: получение таблицы способов обеспечения';
															|en = 'Generate orders: receive the supply method table'");
...
Процедура ЗаполнитьСлужебныеРеквизитыТаблицыСпособов(СтрокаТаблицы)
	...
	ИначеЕсли СтрокаТаблицы.НетГрафикаПоставок Тогда
		СтрокаТаблицы.ДнейДоЗаказаПоГрафикуПредставление = НСтр("ru = '<не заполнен график поставок>';
																|en = '<delivery schedule is not populated>'");
	ИначеЕсли СтрокаТаблицы.НетПлановойДатыЗаказаПоКалендарю Тогда
		СтрокаТаблицы.ДнейДоЗаказаПоГрафикуПредставление = НСтр("ru = '<не заполнен график работы предприятия>';
																|en = '<enterprise schedule is not populated>'");
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(Строка.ДатаЗаказа) Тогда
			ДатаЗаказаНаступила = НСтр("ru = 'Не заполнена дата поставки';
										|en = 'Delivery date is not filled in'");
		Иначе
		...
		ИначеЕсли Не Строка.ФормироватьПлановыеЗаказы И Строка.ДоТочкиЗаказа > 0 Тогда
			...
			ТекстРекомендации = НСтр("ru = 'Не заказывать товар. Ожидать наступления точки заказа.';
									|en = 'Do not order the goods. Wait for order point.'");
			ЭтоТочкаЗаказа = Ложь;
			Заказать       = Ложь;
...
Функция ДалееКШагуЗаказы(Результат)
	...
	Элементы.Получатель.Заголовок = ?(ЕстьРаботы И ЕстьТовары, НСтр("ru = 'Получатель';
		...
		?(ЕстьРаботы, НСтр("ru = 'Подразделение-получатель';
							|en = 'Receiving department'"), НСтр("ru = 'Склад';
																	|en = 'Warehouse'")));
...
Процедура УстановитьСтраницыПомощника(ИмяШага)
	...
	ИначеЕсли ИмяШага = "ШагНастройкаСпособовОбеспечения" Тогда
		...
		Элементы.НадписьШаг.Заголовок = НСтр("ru = 'Шаг 2 из 5. Настройка способов обеспечения';
											|en = 'Step 2 of 5. Customize supply methods'");
	...
	ИначеЕсли ИмяШага = "ШагЗапасы" Тогда
		...
		Элементы.НадписьШаг.Заголовок = НСтр("ru = 'Шаг 3 из 5. Поддержание запасов';
											|en = 'Step 3 of 5. Inventory level control'");
	...
	ИначеЕсли ИмяШага = "ШагЗаказы" Тогда
		...
		Элементы.НадписьШаг.Заголовок = НСтр("ru = 'Шаг 4 из 5. Обеспечение заказов к отгрузке';
											|en = 'Step 4 of 5. Orders for shipment'");
	...
	ИначеЕсли ИмяШага = "ШагСформированныеЗаказы" Тогда
		...
		Элементы.НадписьШаг.Заголовок = НСтр("ru = 'Шаг 5 из 5. Уточнение сформированных заказов';
											|en = 'Step 5 of 5. Confirmation of created orders'");
...
Функция ТаблицаСпособовЗаполненаВерно()
	...
	Если Не ЕстьОтмеченные Тогда
		...
		ТекстОшибки = НСтр("ru = 'Для перехода к следующему шагу необходимо отметить хотя бы один способ обеспечения из списка';
							|en = 'To go to the next step, select at least one supply method from the list'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<не указан обеспечивающий склад>';
																|en = '<supplying warehouse is not specified>'"));
			...
			Если Строка.УпаковкаЗаказа.Пустая() Тогда
				Если Строка.ИспользоватьУпаковки 
					...
					Упаковка = НСтр("ru = '<задать упаковку для округления>';
									|en = '<set a packaging for rounding>'");
