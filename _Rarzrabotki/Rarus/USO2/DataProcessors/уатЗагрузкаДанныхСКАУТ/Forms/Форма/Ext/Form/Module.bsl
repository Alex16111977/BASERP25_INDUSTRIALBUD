...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьСКАУТ.Получить() Тогда
		ТекстСообщения = НСтр("en='Opening is possible only when enabled using of GPS system, SKAUT.';ru='Открытие возможно только тогда, когда включено использование системы GPS - СКАУТ.'");
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	// Выполним загрузку
	Если Не Отказ Тогда 
		ТекстНСТР = НСтр("en='Load data systems GPS SKAUT';ru='Выполняется загрузка данных системы GPS СКАУТ'");
...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	КодВозврата = уатЗащищенныеФункцииСервер_проф.ПолучитьУчетнуюЗаписьСистемыМониторингаДляТС(МассивТС, Перечисления.уатСистемаGPS.СКАУТ);
	...
	Для Каждого ТекСтрока Из МассивТС Цикл
		...
		Если КодВозврата = 0 И ЗагрузитьПробегИРасходГСМ Тогда // Загружаем пробег и расход ГСМ.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТССКАУТ(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // Загружаем дополнительные сведения.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияПоТССКАУТ(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
		
		Если КодВозврата=0 И ЗагрузитьКоординаты Тогда // Загружаем координаты.
			КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьКоординатыПоТССКАУТ(ДатаНачала, ДатаОкончания, ТекСтрока.ТС, ТекстОшибки, УчетнаяЗаписьСистемыМониторинга);
		КонецЕсли;
