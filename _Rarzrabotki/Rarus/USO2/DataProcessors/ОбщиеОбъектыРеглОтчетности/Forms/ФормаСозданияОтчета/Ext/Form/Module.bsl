...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Ограничиваем исходное дерево отчетов для ИП на УСН 6%.
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.УпроститьФормуРегОтчетностиФильтрОтчетов(ДеревоВсехИспользуемыхОтчетовПоКатегориям, КоличествоЭлементовВДеревеОтчетов);
	...
	// Скрываем некоторые элементы формы для ИП на УСН 6%
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.УпроститьФормуСозданияОтчета(ЭтотОбъект);
...
Функция ДеревоВсехИспользуемыхОтчетов()
	...
	Для каждого СтрокаПервогоУровня Из ДеревоВсехИспользуемыхОтчетов.Строки Цикл
		...
		Если ЗначениеЗаполнено(ОтобранныеОтчеты) Тогда // перемещаем УОС в конец коллекции
			...
			ИндексОтчетаУОС = СтрокаПервогоУровня.Строки.Индекс(ОтчетУОС);
			Смещение = СтрокаПервогоУровня.Строки.Количество() - ИндексОтчетаУОС - 1;
			СтрокаПервогоУровня.Строки.Сдвинуть(ИндексОтчетаУОС, Смещение);
		КонецЕсли;
	...
	Для каждого СтрокаТаблицыГруппа Из ТаблицаГруппДляСортировки Цикл
		Если СтрокаТаблицыГруппа.СтрокаВДереве <> Неопределено Тогда
			...
			Если СтрокаТаблицыГруппа.СтрокаВДереве.Наименование = "Росстат" Тогда
				ОтобранныеОтчеты = СтрокаТаблицыГруппа.СтрокаВДереве.Строки.НайтиСтроки(ПараметрыОтбораУОС);
				Если ЗначениеЗаполнено(ОтобранныеОтчеты) Тогда // перемещаем УОС в конец коллекции
					ОтчетУОС = ОтобранныеОтчеты[0];
					ИндексОтчетаУОС = СтрокаТаблицыГруппа.СтрокаВДереве.Строки.Индекс(ОтчетУОС);
					Смещение = СтрокаТаблицыГруппа.СтрокаВДереве.Строки.Количество() - ИндексОтчетаУОС - 1;
					СтрокаТаблицыГруппа.СтрокаВДереве.Строки.Сдвинуть(ИндексОтчетаУОС, Смещение);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
