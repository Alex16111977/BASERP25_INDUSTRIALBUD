...
Процедура ВыгрузитьНаДиск(ВФормате)
				...
				Если СписокПечатаемыхЛистов.Количество() > 0
					...
					Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Сохранение %1';
							|en = 'Saving %1'"), ИмяФайла), , , БиблиотекаКартинок.Записать);
		...
		Для Каждого Стр1 Из СписокЛистов.ПолучитьЭлементы() Цикл
			...
			Если Стр1.Пометка Тогда
				...
				Попытка
					...
					Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохранение %1';
																							|en = 'Saving %1'"), ИмяФайла), , , БиблиотекаКартинок.Записать);
	...
	Если ПризнакОшибки Тогда
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось сохранить печатный бланк в указанный каталог.%1Возможно, нет прав для записи в выбранный каталог, нет доступа к файлу, т. к. файл открыт в другой программе, недостаточно места на диске.';
																								|en = 'Cannot save the letterhead to the specified directory. %1 You might not have sufficient rights to save files to the selected directory, the file  cannot be accessed as it is opened in another application, or there is not enough disk space.'"), Символы.ПС), , НСтр("ru = 'Ошибка записи на диск.';
																																																																																|en = 'An error occurred when writing to the disk.'"));
