...
&НаКлиенте
Процедура ОткрытьВходящиеСообщенияУТМ(Команда)
	...
	Если ОрганизацииЕГАИС.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не заполнено поле ""Организация ЕГАИС""';
				|en = 'The ""USAIS company"" field is required'"),,
			"ОрганизацияЕГАИС");
...
Процедура ОбновитьСпискиДокументов()
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Акты постановки на баланс ЕГАИС';
			|en = 'USAIS recognition certificates'"),
		Истина, Истина, Истина);
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Акты списания ЕГАИС';
			|en = 'USAIS write-off certificates'"),
		Истина, Истина, Истина);
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Передачи в регистр №2 ЕГАИС';
			|en = 'Transfers to register No.2 USAIS'"),
		Истина, Истина, Истина);
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Возвраты из регистра №2 ЕГАИС';
			|en = 'Returns from register No.2 USAIS'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Запросы акцизных марок ЕГАИС';
			|en = 'USAIS excise stamp requests'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	Если Выборка.Следующий() И Выборка.КоличествоСообщений > 0 Тогда
		ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Есть сообщения (%1), ожидающие отправки в ЕГАИС. Выполните обмен.';
										|en = 'There are messages (%1) waiting to be sent to USAIS. Exchange.'"), Выборка.КоличествоСообщений);
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Не заданы параметры подключения к УТМ для организации ЕГАИС %1';
					|en = 'Connection to UTM parameters are not specified for USAIS company %1'"), ВыбраннаяОрганизацияЕГАИС),,
			"ОрганизацияЕГАИС");
	КонецЕсли;
