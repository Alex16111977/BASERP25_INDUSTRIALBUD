...
Процедура ВыполнитьЗапросСопоставлений(Команда)
	...
	Запросы.Вставить("ЗапросСчетовПоУмолчанию", "СчетаПоУмолчанию");
	Запросы.Вставить("ЗапросПрочихСчетов", "ПрочиеСчета");
	Запросы.Вставить("ЗапросСопоставлений", "Хозрасчетный");
...
Процедура ВыполнитьЗапросы(ОписаниеЗапросов)
	...
	ОсновныеПараметры = РеглУчетВыборкиСерверПовтИсп.ПараметрыОтраженияРеглУчетаПоУмолчанию();
	...
	Для Каждого Описание Из ОписаниеЗапросов Цикл
		Если Не ЗначениеЗаполнено(ЭтаФорма[Описание.Ключ + "Текст"]) Тогда
			ОбщаяПроцедураИнициализации(Описание.Ключ);
		КонецЕсли;
...
&НаСервере
Процедура ОбщаяПроцедураИнициализации(Ключ)
	
	Если Ключ = "ЗапросДанных" Тогда
		ИнициализироватьЗапросДанных();
	ИначеЕсли Ключ = "ЗапросСчетов" Тогда
		ИнициализироватьЗапросСчетов();
	ИначеЕсли Ключ = "ЗапросСчетовПоУмолчанию" Тогда
		ИнициализироватьЗапросСчетовПоУмолчанию();
	ИначеЕсли Ключ = "ЗапросПрочихСчетов" Тогда
		ИнициализироватьЗапросПрочихСчетов();
	ИначеЕсли Ключ = "ЗапросСопоставлений" Тогда
		ИнициализироватьЗапросСопоставлений();
	ИначеЕсли Ключ = "ЗапросПроверки" Тогда
		ИнициализироватьЗапросПроверки();
	ИначеЕсли Ключ = "ЗапросХозрасчетный" Тогда
		ИнициализироватьЗапросХозрасчетный();
	КонецЕсли;
