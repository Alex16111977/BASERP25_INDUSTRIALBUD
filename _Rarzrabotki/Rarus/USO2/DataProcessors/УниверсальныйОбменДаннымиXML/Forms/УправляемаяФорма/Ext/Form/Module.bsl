...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
		ВызватьИсключение НСтр("ru = 'Использование обработки в интерактивном режиме доступно только администратору.';
								|en = 'Data processor use in interactive mode is available to administrator only.'");
	КонецЕсли;
	...
	ЗаголовокФормы = НСтр("ru = 'Универсальный обмен данными в формате XML (%ВерсияОбработки%)';
							|en = 'Universal data exchange in XML format (%ВерсияОбработки%)'");
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Выполнить загрузку данных из файла обмена?';
													|en = 'Import data from the exchange file? '"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ИмяФайлаПравилПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		СообщитьПользователю(НСтр("ru = 'Не найден файл правил обмена';
									|en = 'File of exchange rules is not found'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Загрузить правила обмена данными?';
											|en = 'Import data exchange rules?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("ru = 'Файл обмена';
																|en = 'Exchange file '"),, УникальныйИдентификатор);
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Удалить выбранные данные в информационной базе?';
											|en = 'Delete the selected data in the infobase?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("ru = 'Файл обмена';
																				|en = 'Exchange file '"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,НСтр("ru = 'Файл правил обмена';
																				|en = 'Exchange rule file '"),, УникальныйИдентификатор);
...
Процедура ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("ru = 'Подключение успешно установлено.';
									|en = 'Connection has been successfully established.'"));
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Не найден файл обмена на сервере.';
										|en = 'Exchange file is not found on server.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("ru = 'Не указан файл правил для загрузки данных.';
										|en = 'File of data import rules is not specified.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Универсальный обмен данными в формате XML';
										|en = 'Universal data exchange in XML format'", ОбъектДляСервера.КодОсновногоЯзыка()),
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Файл внешней обработки отладчиков событий не существует на сервере';
										|en = 'External data processor file of event debuggers does not exist on the server'"));
			Возврат Неопределено;
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("ru = 'Режим работы (на клиенте)';
						|en = 'Operation mode (on client)'"), НСтр("ru = 'Режим работы (на сервере)';
																	|en = 'Operation mode (on server)'"));
...
Процедура ПоместитьФайлПравилЗагрузкиВХранилище()
	...
	НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,
		?(ЧерезХранилище, НСтр("ru = 'Файл обмена';
								|en = 'Exchange file '"), ИмяФайлаПравилЗагрузки), ЧерезХранилище, УникальныйИдентификатор);
