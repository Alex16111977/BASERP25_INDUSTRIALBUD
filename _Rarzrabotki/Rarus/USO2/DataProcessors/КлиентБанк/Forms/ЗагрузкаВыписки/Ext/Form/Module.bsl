	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Для выбора файла должна быть выделена строка с банковским счетом.
				...
				|If the bank account list is empty, make sure that bank exchange is enabled in the bank account card.     '"),
			,
			...
			Если Не ЗначениеЗаполнено(Счет.НастройкаОбмена) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для банковского счета %1 не настроен ЭДО. Выполните настройку ЭДО, или воспользуйтесь выгрузкой в файл.';
						|en = 'EDI is not configured for bank account %1. Configure EDI or use export to file.'"), Счет.Ссылка);
	...
	Иначе
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = '<Не удалось прочитать файл выписки. Проверьте корректность файла.>';
									|en = '<Failed to read the statement file. Validate the file.>'");
		КонецЕсли;
		#КонецЕсли
	КонецЕсли;
	...
	Если ТипЗнч(Счет.Выписка.ДанныеВыписки) = Тип("Структура") Тогда
			...
			Если СписокСчетовВыписки.Найти(Счет.НомерСчета) = Неопределено Тогда
				ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Файл не содержит данных по банковскому счету %1 с номером %2>';
						|en = '<The file does not contain information of bank account %1 with number %2>'"),
					Строка(Счет.Ссылка),
					Счет.НомерСчета);
		...
		Иначе
			Возврат НСтр("ru = '<Файл не содержит сведений о расчетных счетах>';
						|en = '<The file does not contain current account information>'");
		КонецЕсли;
		
		Если ДанныеВыписки.Свойство("Заголовок", ДанныеЗаголовкаВыписки) Тогда
			ТекстЗаголовка = "
			|" + НСтр("ru = 'Отправитель';
						|en = 'From'") + "              " + ДанныеЗаголовкаВыписки.Отправитель + "
			|" + НСтр("ru = 'Дата создания';
						|en = 'Creation date'") + "            " + ДанныеЗаголовкаВыписки.ДатаСоздания + " " + ДанныеЗаголовкаВыписки.ВремяСоздания + "
			...
			Для каждого РасчетныйСчет Из РасчетныеСчетаВыписки Цикл
				Если РасчетныйСчет.РасчСчет = Счет.НомерСчета Тогда
					...
					|" + НСтр("ru = 'Начальный остаток';
								|en = 'Start balance'") + "        " + РасчетныйСчет.НачальныйОстаток + "
					|" + НСтр("ru = 'Поступило';
					...
					|" + НСтр("ru = 'За день';
						...
						Иначе
							ПериодОстатков = "
					|" + НСтр("ru = 'За период';
								|en = 'For the period'") + "                " + РасчетныйСчет.ДатаНачала + " - " + РасчетныйСчет.ДатаКонца;
						КонецЕсли;
					КонецЕсли;
...
Процедура Подключаемый_ПодключитьРасширениеРаботыСФайламиИВыбратьФайл()
	...
	ТекстСообщения = НСтр("ru = 'Для выбора файла необходимо установить расширение работы с файлами.';
							|en = 'To select a file, install the file operation extension.'");
...
Процедура ПодключитьРасширениеРаботыСФайламиИЗагрузитьФайл(Счет)
	...
	ТекстСообщения = НСтр("ru = 'Для загрузки файлов выписки необходимо установить расширение работы с файлами.';
							|en = 'To import statement files, install the file operation extension.'");
		...
		Если Лев(Врег(СокрЛП(ТекущаяСтрока)), 20) <> "1CCLIENTBANKEXCHANGE" Тогда
			ТекстСообщения =
				НСтр("ru = '<Файл выписки не является файлом обмена, или неверно указана кодировка в настройках обмена банковского счета.>';
					|en = '<The statement file is not an exchange file, or encoding is incorrect in the exchange settings of the bank account.>'");
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПрочитатьДанныеВыпискиИЗагрузитьНаСервере(ИдСчета, АдресФайла)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Загрузка банковской выписки.Удаление временного файла';
				|en = 'Import bank statement.Remove temporary file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
