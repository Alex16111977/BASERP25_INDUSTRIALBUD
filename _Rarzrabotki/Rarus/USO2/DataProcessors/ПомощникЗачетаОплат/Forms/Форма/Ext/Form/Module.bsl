	 ...
	 ИЛИ НЕ РасчетыПоДоговорам И (ТипЗнч(Документ) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Зачет оплаты не требуется использовать для документа %1';
				|en = 'No need to use payment offset for document %1'"),
			Документ);
		...
		Если МассивСтрок.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Данные зачета оплаты были изменены. Сохранить изменения?';
					|en = 'Payment offset data was changed. Save the changes?'"),
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<Будет создан документ ""Взаимозачет задолженности"">';
																|en = '<The ""Debt offset"" document will be created>'"));
...
Процедура УправлениеЭлементамиФормы()
	...
	Если ИспользоватьДоговоры Тогда
		Элементы.ПояснениеКТаблицеОплаты.Заголовок = НСтр("ru = 'Список нераспределенных платежей и документов оплаты, отнесенных на выбранный договор. Сумма будет распределена на зачитываемый объект расчетов.';
															|en = 'List of non-allocated payments and payment documents allocated to the selected document. Amount will be allocated to the settlements object being offset.'");
	Иначе
		Элементы.ПояснениеКТаблицеОплаты.Заголовок = НСтр("ru = 'Список нераспределенных платежей и документов оплаты. Сумма будет распределена на зачитываемый объект расчетов.';
															|en = 'List of non-allocated payments and payment documents. Amount will be allocated to the settlements object being offset.'");
	КонецЕсли;
	...
	Если ИспользоватьДоговоры Тогда
		Элементы.ПояснениеКТаблицеАвансы.Заголовок = НСтр("ru = 'Остатки авансов по договорам контрагента. Для зачета/переноса аванса будет создан документ ""Взаимозачет задолженности"".';
															|en = 'Remaining advances under the counterparty contracts. The ""Debt offset"" document will be created for advance offset/ transfer.'");
	Иначе
		Элементы.ПояснениеКТаблицеАвансы.Заголовок = НСтр("ru = 'Остатки авансов. Для зачета/переноса аванса будет создан документ ""Взаимозачет задолженности"".';
															|en = 'Remaining advances. The ""Debt offset"" document will be created to offset/Transfer the advanced payment.'");
	КонецЕсли;
	...
	Если РасчетыПоДоговорам Тогда
		Заголовок = НСтр("ru = 'Зачет оплат по договору:';
						|en = 'Payment offset under contract:'") + " " + Строка(Договор);
	Иначе
		Заголовок = НСтр("ru = 'Зачет оплат по документу:';
						|en = 'Payment offset against the document:'") + " " + Строка(Документ);
	КонецЕсли;
...
Функция ТекстЗапросаПоРасчетамСПоставщиками()
	...
	|		И НЕ ТИПЗНАЧЕНИЯ(РасчетыСПоставщиками.Регистратор) = ТИП(Документ.ВводОстатков) ТОГДА
	|		ИСТИНА
	|	ИНАЧЕ
	|		ЛОЖЬ
	|	КОНЕЦ                                                            КАК ДоступноРедактирование,
...
Функция ТекстЗапросаПоРасчетамСКлиентами()
	...
	|		И НЕ ТИПЗНАЧЕНИЯ(РасчетыСКлиентами.Регистратор) = ТИП(Документ.ВводОстатков) ТОГДА
	|		ИСТИНА
	|	ИНАЧЕ
	|		ЛОЖЬ
	|	КОНЕЦ                                                            КАК ДоступноРедактирование,
...
Процедура ЗачестьПлатежДляВыбранныхСтрок(МассивСтрок)
	...
	Если Недостает <= 0 Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Сумма к оплате полностью зачтена.';
												|en = 'Amount due is fully set off.'"), Режим);
		Возврат;
...
Процедура СохранитьРезультатЗачетаОплатыИЗакрыть()
	...
	Если ЕстьРасчетыМеждуОрганизациями И МассивОбработанныхДокументов.Количество() > 0
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациями") Тогда
			СторонаОплатыТекст = ?(ЗеркальныйЗачетОплатыМеждуОрганизациями, НСтр("ru = 'комиссионера';
																				|en = 'consignee'"),НСтр("ru = 'комитента';
																											|en = 'consignor'") );
			СторонаОплатыВыполненоТекст = ?(ЗеркальныйЗачетОплатыМеждуОрганизациями, НСтр("ru = 'комитента';
																							|en = 'consignor'"),НСтр("ru = 'комиссионера';
																													|en = 'consignee'") );
		Иначе
			СторонаОплатыТекст = ?(ЗеркальныйЗачетОплатыМеждуОрганизациями, НСтр("ru = 'получателя';
																				|en = 'of recipient'"),НСтр("ru = 'отправителя';
																											|en = 'of sender'") );
			СторонаОплатыВыполненоТекст = ?(ЗеркальныйЗачетОплатыМеждуОрганизациями, НСтр("ru = 'отправителя';
																							|en = 'of sender'"),НСтр("ru = 'получателя';
																													|en = 'of recipient'") );
		КонецЕсли;
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Зачтена оплата на стороне %1. 
			...
			|en = 'Payment on the %1 side is offset.
			|Offset similar payments on the %2 side?'"), СторонаОплатыВыполненоТекст ,СторонаОплатыТекст);
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось создать документ взаимозачета. %1';
				|en = 'Cannot create offset document. %1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , ПолеОшибки, , Отказ);
	КонецПопытки;
...
Процедура ОбновитьИнформационнуюНадписьИтогов()
	...
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
		...
		НСтр("ru = 'Зачтено на сумму %1 %2.';
			|en = 'Set off in the amount of %1 %2.'"),
			...
			Если ЗначениеЗаполнено(ЗеркальныйПлатежСсылка) Тогда
				...
				Если Успешно Тогда
					...
					Исключение
						...
						НСтр("ru = 'Помощник зачета оплаты, зеркальное изменение документов оплат.';
							|en = 'Payment offset wizard, inverted change of payment documents.'", КодОсновногоЯзыка),
