...
Процедура ПроверитьВерсиюНаСервере()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не удалось получить версию сервера ГИСМ, возможно, требуется обновление конфигурации.';
																|en = 'Cannot receive the SPMS server version. Perhaps it is required to update the configuration.'"));
	КонецЕсли;
...
Процедура ОбновитьВерсии()
	...
	Если Не ВерсияАктуальна Тогда
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ru = 'Текущая версия конфигурации не поддерживает актуальный формат обмена с ГИСМ (%1)';
												|en = 'Current configuration version does not support relevant format of exchange with SPMS (%1)'"), ВерсииСхемОбмена.Сервер);
	Иначе
		ТекстСообщенияОВерсии = СтрШаблон(НСтр("ru = 'Обмен с ГИСМ выполняется с использованием актуального формата обмена (%1)';
												|en = 'Exchange with SPMS using current exchange format (%1)'"), ВерсииСхемОбмена.Сервер);
