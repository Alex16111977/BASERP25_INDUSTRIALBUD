...
Процедура ВыбратьФайлы(ЗаменитьСтроку = Ложь)
	...
	Если ПоддерживаетсяРасширениеРаботыСФайлами = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для множественного выбора файлов выгрузки рекомендуется установить расширение работы с файлами.';
								|en = 'It is recommended that you install the file operation extension to select multiple files for export.'");
...
Процедура ВыбратьФайлыПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Файлы выгрузки (*.%1)|*.%1|Все файлы(*.*)|*.*';
			|en = 'Export files (*.%1)|*.%1|All files(*.*)|*.*'"), "xml");
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Заполнение консолидированной декларации по НДС';
				|en = 'Fill in consolidated VAT declaration'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ИначеЕсли ИмяУзла = "РегНомТД"
					И (СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1") Тогда
...
Функция ВыгрузкаРаздел8(СтруктураПараметров)
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1" ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1" Тогда
...
Процедура ПостобработкаДереваВыгрузки(ДеревоВыгрузки)
	...
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1" ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2019Кв1" Тогда
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого ДокументРазделов Из ДокументыРазделов Цикл
		...
		Для Каждого ДокУзелСумУплНА Из ДокУзлыСумУплНА Цикл
			...
			Если ПараметрыВыгрузки.ПравилаЗаполнения = "2019Кв1" Тогда
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	Если СтрВыбраннаяФорма <> "ФормаОтчета2015Кв1"
	   ...
	   И СтрВыбраннаяФорма <> "ФормаОтчета2019Кв1" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Консолидация отчетов по НДС реализована только для форм:
			...
			|en = 'VAT reporting consolidation is used only for the following forms:
			|""Annex No. 1 to the order of the FTS of Russia dated October 29, 2014 No. MMV-7-3/558@"", 
			|""Annex No. 1 to the order of the FTS of Russia dated October 29, 2014 No. MMV-7-3/558@
			| (in edition of the decree of the FTS of Russia dated December 20, 2016 No.MMV-7-3/696@),
			| (in edition of the decree of the FTS of Russia dated December 28, 2018 No. CA-7-3/853@)""!'"));
		Возврат Неопределено;
	...
	Исключение
		...
		Если ИнформацияОбОшибке.Причина = Неопределено Тогда
			Сообщение.Текст = НСтр("ru = 'При открытии формы регламентированного отчета произошла ошибка!';
									|en = 'An error occurred when opening a compliance report form.'");
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела9(СтрокаРаздела9, СтруктураПараметров)
	...
	ПараметрыСтрокиПром.Вставить("НомерДата", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомСчФПрод), "" + СтрокаРаздела9.НомСчФПрод + ";", "")
		+ СтрокаРаздела9.ДатаСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДата", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомСчФПрод), "" + СтрокаРаздела9.НомСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаСчФПрод);
		
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправления", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрСчФ), "" + СтрокаРаздела9.НомИспрСчФ + ";", "")
		+ СтрокаРаздела9.ДатаИспрСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленный", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрСчФ), "" + СтрокаРаздела9.НомИспрСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаИспрСчФ);
		
	ПараметрыСтрокиПром.Вставить("НомерДатаКорректировки", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомКСчФПрод), "" + СтрокаРаздела9.НомКСчФПрод + ";", "")
		+ СтрокаРаздела9.ДатаКСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаКорректировочный", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомКСчФПрод), "" + СтрокаРаздела9.НомКСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаКСчФПрод);
		
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправленияКорректировки", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрКСчФ), "" + СтрокаРаздела9.НомИспрКСчФ + ";", "")
		+ СтрокаРаздела9.ДатаИспрКСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленныйКорректировочный", ?(ЗначениеЗаполнено(СтрокаРаздела9.НомИспрКСчФ), "" + СтрокаРаздела9.НомИспрКСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела9.ДатаИспрКСчФ);
	...
	Для Каждого НомерДатаДок Из СтрокаРаздела9.ДокПдтвОпл Цикл
		...
		НомерДатаДокумента = ?(ЗначениеЗаполнено(НомерДокумента), "" + НомерДокумента + ";", "") + ДатаДокумента;
		НомераДатыДокументов = НомераДатыДокументов + ?(ЗначениеЗаполнено(НомераДатыДокументов), "," + Символы.ПС, "") + НомерДатаДокумента;
		НомерОтДатаДокумента = ?(ЗначениеЗаполнено(НомерДокумента), "" + НомерДокумента + " " + СтрОт + " ", "") + ДатаДокумента;
		НомераОтДатыДокументов = НомераОтДатыДокументов + ?(ЗначениеЗаполнено(НомераОтДатыДокументов), "," + Символы.ПС, "") + НомерОтДатаДокумента;
	КонецЦикла;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела91(СтрокаРаздела91, СтруктураПараметров)
	...
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2019Кв1" Тогда
		ПараметрыСтроки.РегНомерТД = ?(ЗначениеЗаполнено(СтрокаРаздела91.РегНомТД), МассивВСтрокуСРазделителями(СтрокаРаздела91.РегНомТД, ";"), Неопределено);
		ПараметрыСтроки.КодВидаТовара = ?(ЗначениеЗаполнено(СтрокаРаздела91.КодВидТовар), МассивВСтрокуСРазделителями(СтрокаРаздела91.КодВидТовар, ";"), Неопределено);
	КонецЕсли;
	...
	ПараметрыСтрокиПром.Вставить("НомерДата", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомСчФПрод), "" + СтрокаРаздела91.НомСчФПрод + ";", "")
		+ СтрокаРаздела91.ДатаСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДата", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомСчФПрод), "" + СтрокаРаздела91.НомСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаСчФПрод);
	
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправления", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрСчФ), "" + СтрокаРаздела91.НомИспрСчФ + ";", "")
		+ СтрокаРаздела91.ДатаИспрСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленный", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрСчФ), "" + СтрокаРаздела91.НомИспрСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаИспрСчФ);
	
	ПараметрыСтрокиПром.Вставить("НомерДатаКорректировки", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомКСчФПрод), "" + СтрокаРаздела91.НомКСчФПрод + ";", "")
		+ СтрокаРаздела91.ДатаКСчФПрод);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаКорректировочный", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомКСчФПрод), "" + СтрокаРаздела91.НомКСчФПрод + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаКСчФПрод);
	
	ПараметрыСтрокиПром.Вставить("НомерДатаИсправленияКорректировки", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрКСчФ), "" + СтрокаРаздела91.НомИспрКСчФ + ";", "")
		+ СтрокаРаздела91.ДатаИспрКСчФ);
	ПараметрыСтрокиПром.Вставить("НомерОтДатаИсправленныйКорректировочный", ?(ЗначениеЗаполнено(СтрокаРаздела91.НомИспрКСчФ), "" + СтрокаРаздела91.НомИспрКСчФ + " " + СтрОт + " ", "")
		+ СтрокаРаздела91.ДатаИспрКСчФ);
