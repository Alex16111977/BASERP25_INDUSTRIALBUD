	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.ПолноеНаименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для продолжения укажите имя версии файла.';
				|en = 'Specify file version name to continue.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьВерсиюФайла(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка записи версии присоединенного файла';
										|en = 'Files.An error occurred when writing attached file version'", ОбщегоНазначения.КодОсновногоЯзыка()),
