...
Процедура ДеревоМаркированнойПродукцииПередУдалением(Элемент, Отказ)
	...
	Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ТекущиеДанные.ТипУпаковки) Тогда
	...
	Иначе
		...
		Если КоличествоУпаковокКоторыеНеобходимоПеремаркировать > 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Есть упаковки, которые требуют перемаркировки.';
										|en = 'Есть упаковки, которые требуют перемаркировки.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьВыводимоеПредставлениеПроверкиСодержимого(Форма, ТекущиеДанные)
	...
	Если ТекущиеДанные = Неопределено
		Или Не ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ТекущиеДанные.ТипУпаковки) Тогда
...
Процедура ОпределитьТипыВсехУпаковок()
	...
	Для Каждого СтрокаДерева Из ДеревоМаркированнойПродукции.ПолучитьЭлементы() Цикл
		...
		Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки) Тогда
...
Процедура ОпределитьТипУпаковки(СтрокаДерева, ВключаяПодчиненные)
	...
	Для Каждого ПодчиненнаяСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		...
		Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ПодчиненнаяСтрока.ТипУпаковки) Тогда
...
Процедура ОпределитьТипУпаковкиПриИзмененииСтроки(ИдентификаторИзмененойСтроки)
	...
	Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ИзмененнаяСтрока.ТипУпаковки) Тогда
	...
	Пока РодительИзмененнойСтроки <> Неопределено 
		И ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(РодительИзмененнойСтроки.ТипУпаковки) Цикл
...
Процедура ОтобразитьИнформациюОНеобходимостиПеремаркировки()
	...
	Если КоличествоУпаковокКоторыеНеобходимоПеремаркировать > 0 Тогда
		...
		ТекстТребуется = СтрШаблон(НСтр("ru = 'Требуется перемаркировка упаковок - %1.';
										|en = 'Требуется перемаркировка упаковок - %1.'"), КоличествоУпаковокКоторыеНеобходимоПеремаркировать);
		СтрокаТребуется = Новый ФорматированнаяСтрока(ТекстТребуется,, ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС);
		...
		СтрокаОтбор = Новый ФорматированнаяСтрока(ТескстОтбор,, ЦветаСтиля.ЦветГиперссылкиГосИС,, "ИзменитьОтборТребуетсяПеремаркировка");
...
Функция СтрокаСУпаковкойДляПеремаркировки()
	...
	Иначе
		...
		Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(РодительскаяСтрока.ТипУпаковки) Тогда
...
Процедура ДобавитьУпаковкуСтрокиВМассивИзмененныхУпаковок(СтрокаДерева, МассивУпаковокСИзменившимсяСоставом)
	...
	Если СтрокаРодитель <> Неопределено
		И ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(СтрокаРодитель.ТипУпаковки) Тогда
...
Процедура СформироватьПредставлениеСодержимогоУпаковки(СтрокаДерева)
	...
	ИначеЕсли СтрокаДерева.КоличествоПодчиненныхУпаковок = 0 Тогда
		...
		СтрокаДерева.ПредставлениеСодержимоеУпаковки = СтрШаблон(НСтр("ru = 'бутылок -  %1';
																		|en = 'бутылок -  %1'"), СтрокаДерева.КоличествоПодчиненныхБутылок);
...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	Если Не ПроверкаНеПоДокументу
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан документ для проверки.';
	...
	Если Не ПроверкаНеПоДокументу
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В обработку передан документ некорректного типа.';
																|en = 'В обработку передан документ некорректного типа.'"),,,,Отказ);
...
Процедура УправлениеВидимостьюСтраницФормы()
	...
	ИначеЕсли КоличествоСтрокВерхнегоУровня = 1 Тогда
		...
		Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ЕдинственнаяСтрока) Тогда
			ЕстьДанныеВДереве = Истина;
...
Процедура ЗагрузитьДанныеДокумента(СоответствиеШтрихкодовСтрокДерева, ПомещатьВоВременноеХранилище)
	...
	ИначеЕсли ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.АктПостановкиНаБалансЕГАИС")
		...
		ДанныеПроверяемогоДокумента = ШтрихкодированиеЕГАИС.ВложенныеШтрихкодыУпаковокПоДокументу(ПроверяемыйДокумент, Не РедактированиеФормыНедоступно);
		ЗагрузитьДеревоМаркированнойПродукции(ДанныеПроверяемогоДокумента.ДеревоУпаковок,
		                                      СоответствиеШтрихкодовСтрокДерева,
		                                      СохраненнаяНастройкаДетализацииСтруктурыХранения);
...
Процедура ЗагрузитьДеревоМаркированнойПродукции(ДеревоПоДаннымДокумента, СоответствиеШтрихкодовСтрокДерева, СохраненнаяНастройкаДетализацииСтруктурыХранения)
	...
	Для каждого СтрокаПоДаннымДокумента Из ДеревоПоДаннымДокумента.Строки Цикл
		...
		//Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПоДаннымДокумента.Номенклатура, "АлкогольнаяПродукция") Тогда
			ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(
				СтрокаПоДаннымДокумента,
				ДетализацияСтруктурыХраненияДанныхДокумента,
...
Процедура ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(СтрокаИсточника, ДетализацияДанныхИсточника, КоллекцияСтрокПриемника, ДетализацияДанныхПриемника, СоответствиеШтрихкодовСтрокДерева)
	...
	Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(НоваяСтрока.ТипУпаковки) Тогда
...
Процедура ИзменитьСтатусПроверкиВСтрокеДерева(СтрокаДерева, СтатусПроверки, УстанавливатьБезусловно)
	...
	ИначеЕсли ЗначениеЗаполнено(СтатусПроверки)
		...
		И (СтрокаДерева.СтатусПроверки = Перечисления.СтатусыПроверкиНаличияПродукцииИС.НеПроверялась
		  Или СтрокаДерева.СтатусПроверки = Перечисления.СтатусыПроверкиНаличияПродукцииИС.Отложена) Тогда
	...
	Для Каждого ПодчиненнаяСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		...
		УстанавливатьБезусловно =  ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки)
		                           И СтрокаДерева.СтатусПроверки = Перечисления.СтатусыПроверкиНаличияПродукцииИС.Отсутствует;
...
Процедура ПослеУспешногоОбнаруженияОтсканированногоШтрихкодаВДереве(НайденнаяСтрокаДерева, 
	...
	Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(НайденнаяСтрокаДерева.ТипУпаковки) Тогда
...
Процедура ОбработатьНеНайденныйВДеревеШтрихКод(ДанныеШтрихкода, ТекущаяСтрокаДерева, ОбработкаДанныхТСД)
	...
	ИначеЕсли ДанныеШтрихкода.ВидыПродукции.Найти(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Алкогольная")) <> Неопределено Тогда
		...
		Если ЭтоШтрихкодАлкогольнойПродукции Тогда
			...
			ОткрытьФорму("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытияФормы,
				ЭтаФорма, УникальныйИдентификатор,,,ОповещениеЗакрытияФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПроверитьШтрихкодВДеревеМаркированнойПродукции(ДанныеШтрихкода, ОбработкаДанныхТСД = Ложь)
	...
	ИначеЕсли РежимПодбораСуществующихУпаковок
		...
		Если Результат.ЕстьОшибки Тогда
			...
			ШтрихкодированиеИСКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, Результат.АдресХранилищаДереваУпаковки);
...
Функция РезультатДобавленияСуществующейУпаковкиВДеревоМаркированнойПродукции(ДанныеШтрихкода, СоответствиеШтрихкодовСтрокДерева, ОбработкаДанныхТСД)
	...
	ЭтоУпаковка = ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ДанныеШтрихкода.ТипУпаковки);
...
Процедура ИзменитьСостояниеПроверкиУпаковки(СтрокаДерева)
	...
	Если Не ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(СтрокаДерева.ТипУпаковки) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДоступностьУпаковкиДляПроверки(СтрокаДерева, ДоступныеДляПроверкиУпаковки)
	...
	Если СтрокаДерева.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии")
		Или СтрокаДерева.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.Отложена")
		Или СтрокаДерева.СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.НеЧислилась") Тогда
...
Функция УпаковкаНеСодержитсяВДанныхДокумента(ТекущаяСтрокаДерева, Детализация)
	...
	ИначеЕсли ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ТекущаяСтрокаДерева.ТипУпаковки) Тогда
