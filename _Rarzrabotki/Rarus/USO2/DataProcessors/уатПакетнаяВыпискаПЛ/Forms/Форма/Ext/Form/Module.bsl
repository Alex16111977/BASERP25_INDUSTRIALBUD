...
Процедура ПодготовкаТСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	уатИнтерфейсВводаТС.РеквизитТСНачалоВыбора(Элемент, Элементы.Подготовка.ТекущиеДанные.ТС, ДанныеВыбора, СтандартнаяОбработка, СтруктураОтбора);
...
Процедура ПодготовкаПрицеп1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	уатИнтерфейсВводаТС.РеквизитТСНачалоВыбора(Элемент, Элементы.Подготовка.ТекущиеДанные.Прицеп1, ДанныеВыбора, СтандартнаяОбработка, СтруктураОтбор);
КонецПроцедуры
...
Процедура ПодготовкаПрицеп2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтруктураОтбор = Новый Структура("уатОрганизация", Объект.Организация);
	уатИнтерфейсВводаТС.РеквизитТСНачалоВыбора(Элемент, Элементы.Подготовка.ТекущиеДанные.Прицеп2, ДанныеВыбора, СтандартнаяОбработка, СтруктураОтбор);
КонецПроцедуры
...
Процедура СформироватьПодготовкуСервер()
	...
	тблЗапросДанные.Колонки.Добавить("ВесБрутто1");
	тблЗапросДанные.Колонки.Добавить("ВесБрутто2");
	тблЗапросДанные.Колонки.Добавить("Объем1");
	тблЗапросДанные.Колонки.Добавить("Объем2");
	тблЗапросДанные.ЗаполнитьЗначения(0, "ВесБрутто1");
	тблЗапросДанные.ЗаполнитьЗначения(0, "ВесБрутто2");
	тблЗапросДанные.ЗаполнитьЗначения(0, "Объем1");
	тблЗапросДанные.ЗаполнитьЗначения(0, "Объем2");
	...
	Подготовка.Загрузить(тблЗапрос);
...
Функция СформироватьПЛСервер(ТекСтрока, СообщениеПредыдущийПЛ = "")
	...
	Если уатПраваИНастройки.уатПолучитьПраваИНастройкиПользователя(докПЛ.Организация, ПланыВидовХарактеристик.уатПраваИНастройки.ВестиУчетЗатрат) Тогда
		Если Не уатОбщегоНазначения.уатНайтиПланЗатрат(докПЛ.ПланЗатрат, докПЛ.Организация, докПЛ.ТранспортноеСредство, докПЛ.ТранспортноеСредство.уатКолонна) Тогда 
			Сообщить("Не найден план затрат (ТС """ + Строка(докПЛ.ТранспортноеСредство)
...
Процедура ЗаполнитьЗначенияОтветственных()
	...
	КонецЕсли;
	ВыдалДиспетчер_ = уатПраваИНастройки.уатПолучитьПраваИНастройкиПользователя(СтруктураОбъектовНастроек, ПредопределенноеЗначение("ПланВидовХарактеристик.уатПраваИНастройки.ВыдалДиспетчер"));
	...
	КонецЕсли;
	ВыпустилМеханик_ = уатПраваИНастройки.уатПолучитьПраваИНастройкиПользователя(СтруктураОбъектовНастроек, ПредопределенноеЗначение("ПланВидовХарактеристик.уатПраваИНастройки.ВыпустилМеханик"));
	...
	КонецЕсли;
	ПринялДиспетчер_ = уатПраваИНастройки.уатПолучитьПраваИНастройкиПользователя(СтруктураОбъектовНастроек, ПредопределенноеЗначение("ПланВидовХарактеристик.уатПраваИНастройки.ПринялДиспетчер"));
	...
	КонецЕсли;
	ПринялМеханик_ = уатПраваИНастройки.уатПолучитьПраваИНастройкиПользователя(СтруктураОбъектовНастроек, ПредопределенноеЗначение("ПланВидовХарактеристик.уатПраваИНастройки.ПринялМеханик"));
