		...
		Если Не ДанныеМонитора.authResult.authenticated Тогда
			...
			ОтобразитьСостояниеОшибка(
				НСтр("ru = 'Некорректные данные аутентификации пользователя Интернет-поддержки.
					...
					|Enable online user support in the application settings.'"),
				СообщениеЖурнала,
				Ложь);
	...
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '<body>● Обновление конфигурации<br />
				...
				|&nbsp;&nbsp;&nbsp;%2</body>';
				|en = '<body>● Configuration update<br />
				|&nbsp;&nbsp;&nbsp;Update is not required. Current configuration version <strong>%1</strong><br />
				|&nbsp;&nbsp;&nbsp;%2</body>'"),
	...
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '<body>● Обновление платформы 1С:Предприятие<br />
				...
				|en = '<body>● 1C:Enterprise platform update<br />
				|&nbsp;&nbsp;&nbsp;Update is not required. Current 1C:Enterprise platform version <strong>%1</strong></body>'"),
		...
		Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<body>● Исправления (патчи)<br />
					...
					|en = '<body>● Corrections (patches)<br />
					|&nbsp;&nbsp;&nbsp;Correction of errors for the version <strong>%1</strong> is not required</body>'"),
				?(ДляНовойВерсии,
					ОбновлениеКонфигурации.Версия,
					ИнтернетПоддержкаПользователей.ВерсияКонфигурации())));
	Иначе
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<body>● Исправления (патчи)<br />
						...
						|en = '<body>● Corrections (patches)<br />
						|&nbsp;&nbsp;&nbsp;<strong>For a new version of configuration %1</strong><br />
						|&nbsp;&nbsp;&nbsp;Corrected errors: <strong>%2</strong></body>'"),
					ОбновлениеКонфигурации.Версия,
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<body>● Исправления (патчи)<br />
						...
						|&nbsp;&nbsp;&nbsp;Исправляемые ошибки: <strong>%2</strong></body>';
						|en = '<body>● Corrections (patches)<br />
						|&nbsp;&nbsp;&nbsp;<strong>For the current version of configuration %1</strong><br />
						|&nbsp;&nbsp;&nbsp;Corrected errors: <strong>%2</strong></body>'"),
					ИнтернетПоддержкаПользователей.ВерсияКонфигурации(),
					КоличествоИсправлений));
		КонецЕсли;
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				...
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '● Обновление платформы 1С:Предприятие<br />
						...
						|en = '● 1C:Enterprise platform update<br />
						|&nbsp;&nbsp;&nbsp;<strong>Version %1</strong>   <span style=""color: #333333""><a href=""%2"">Migrate to a new platform version</a></span><br />
						|&nbsp;&nbsp;&nbsp;Current 1C:Enterprise platform version <strong>%3</strong>'"),
					ОбновлениеПлатформы.Версия,
			...
			Возврат ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				...
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '● Обновление платформы 1С:Предприятие<br />
						...
						|en = '● 1C:Enterprise platform update<br />
						|&nbsp;&nbsp;&nbsp;<strong>Version %1</strong><br />
						|&nbsp;&nbsp;&nbsp;Current 1C:Enterprise platform version <strong>%2</strong>'"),
					ОбновлениеПлатформы.Версия,
					ИнтернетПоддержкаПользователей.ТекущаяВерсияПлатформы1СПредприятие())
				+ ?(ПустаяСтрока(ТекстПояснениеПлатформа), "", "<br />&nbsp;&nbsp;&nbsp;" + ТекстПояснениеПлатформа)
				+ "</body>");
