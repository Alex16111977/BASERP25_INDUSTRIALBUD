	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаданиеНаОтборРазмещениеТовара") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			...
			НСтр("ru = 'Задание на отбор (размещение) товара';
				|en = 'Goods picking (placement) job'"),
			...
			Если СтруктураОбъектов.Ключ = "Документ.РасходныйОрдерНаТовары" 
				...
				Для Каждого РасходныйОрдер Из СтруктураОбъектов.Значение Цикл
					ТекстСообщения = НСтр("ru = 'Печать задания на размещение для документа ""%Документ%"" не предусмотрена.';
											|en = 'Cannot print a placement job for the ""%Документ%"" document.'");
		...
		Пока ВыборкаПоШапке.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаПоШапке.СкладПредставление) Тогда
				ТекстСообщения = НСтр("ru = 'Невозможно напечатать %НазваниеФормы% для ""%Документ%"",
					...
					|as the Warehouse field is empty.'");
			...
			ИначеЕсли ВыборкаПоШапке.НарушенаОрдернаяСхема Тогда
				...
				|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%"",
				|as advanced warehousing is used in warehouse ""%Склад%"", that is why the job should be printed from the note.'");
			...
			ИначеЕсли (ТипЗнч(ВыборкаПоШапке.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары")
				...
				ТекстСообщения = НСтр("ru = 'Невозможно напечатать %НазваниеФормы% для ""%Документ%"",
					...
					|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%"",
					|because this print form is not intended for warehouse (wareroom) with bins used for storing remaining products.
					|You can print a picking (placement) job from document ""Goods picking (placement)"".'");
				...
				И НЕ ВыборкаПоСсылкам.НайтиСледующий(Новый Структура("Ссылка", ВыборкаПоШапке.Ссылка)) Тогда
					Если ТипЗнч(ВыборкаПоШапке.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
						ТекстСообщения = НСтр("ru = 'Невозможно напечатать ""%НазваниеФормы%"" для ""%Документ%"",
							...
							|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%""
							|as there are no lines of products with the Goods type and ""Ship"" or ""Ship on pegging basis"" action in the tabular section.'");
					ИначеЕсли ТипЗнч(ВыборкаПоШапке.Ссылка) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
						ТекстСообщения = НСтр("ru = 'Невозможно напечатать ""%НазваниеФормы%"" для ""%Документ%"",
							...
							|en = 'Cannot print the ""%НазваниеФормы%"" for ""%Документ%"" 
							|as there are no product lines with the Goods type and the Ship action in the tabular section.'");
					Иначе
						ТекстСообщения = НСтр("ru = 'Невозможно напечатать ""%НазваниеФормы%"" для ""%Документ%"",
							|т.к. в табличной части отсутствует отгружаемая номенклатура с типом ""Товар"".';
							|en = 'Cannot print ""%НазваниеФормы%"" for ""%Документ%"",
							|as there are no shipped goods and services with type Goods in the tabular section.'");
					КонецЕсли;
		...
		Если ВыборкаПоШапке.Количество() = 0 И СтруктураОбъектов.Ключ = "Документ.ВводОстатков" Тогда
			Если ПолучитьФункциональнуюОпцию("ИспользоватьМногооборотнуюТару") Тогда	
				ТекстСообщения = НСтр("ru = 'Для данного типа операции документа ""Ввод остатков""
					...
					|en = 'Printing of ""%НазваниеФормы%"" is not required for
					|this operation type of the ""Balance input"" document.
					|Printing is available only for operations of the ""Own goods"", ""Consigned goods received""
					|and ""Reusable package received from suppliers"" type'");
			Иначе 
				ТекстСообщения = НСтр("ru = 'Для данного типа операции документа ""Ввод остатков""
					|печать ""%НазваниеФормы%"" не требуется. 
					|Печать возможна только для операций типа ""Собственные товары"" и ""Полученные на комиссию товары""';
					|en = 'Printing of ""%НазваниеФормы%"" is not required for this
					|operation type of the ""Balance input"" document.
					|Printing is available only for operations of the ""Own goods"" and ""Goods received for commission"" type    '");
			КонецЕсли;
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%НазваниеФормы%",НазваниеФормы);
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Документ%", ВыборкаПоШапке.СсылкаПредставление);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
