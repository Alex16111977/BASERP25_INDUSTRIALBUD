		...
		ИначеЕсли Элемент.ИмяЭлемента = "СтрокаПоТерминалу" Тогда
			...
			Если СтрокаПоТерминалу.Свойство("ТипОперации", Знч) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Знч) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не указан тип операции. Элемент ""СтрокаПоТерминалу"" №%2 не загружен!';
				...
				Сообщение.Текст =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не найден элемент ""ТипОперации"". Элемент ""СтрокаПоТерминалу"" №%2 не загружен!';
							|en = 'Item ""ТипОперации"" is not found in the bank report on acquiring transactions (item ""Отчет"" No.%1, item ""СтрокаПоТерминалу"" No.%2). Item ""СтрокаПоТерминалу"" No.%2 is not imported. '"),
						НомерОтчета,
			...
			Если СтрокаПоТерминалу.Свойство("НомерТерминала", Знч) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Знч) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не указан номер терминала!';
							...
							Если НЕ АтрибутыЭлементаДанныеПоЗапросу.ИНН = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Организация, "ИНН") Тогда
								...
								ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
									НСтр("ru = 'В отчете банка по операциям эквайринга идентификационный номер налогоплательщика (ИНН) не соответствует идентификационному номеру налогоплательщика договора эквайринга в информационной базе!';
										|en = 'In the bank report on acquiring transactions, taxpayer identification number (TIN) does not match the taxpayer identification number of acquiring contract in the infobase. '"));
							...
							Если НЕ АтрибутыЭлементаДанныеПоЗапросу.БИК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.БанковскийСчет.Банк, "Код") Тогда
								...
								ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
									НСтр("ru = 'В отчете банка по операциям эквайринга банковский идентификационный код (БИК) не соответствует банковскому идентификационному коду договора эквайринга в информационной базе!';
										|en = 'In the bank report on acquiring transactions, bank identification code (branch ID) does not match the bank identification code of acquiring contract in the infobase. '"));
							...
							Если НЕ АтрибутыЭлементаДанныеПоЗапросу.РасчетныйСчетОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.БанковскийСчет, "НомерСчета") Тогда
								...
								ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
									НСтр("ru = 'В отчете банка по операциям эквайринга не указан расчетный счет организации или не соответствует расчетному счету организации договора эквайринга в информационной базе!';
										|en = 'In the bank report on acquiring transactions, company account is not specified or it does not match the company current account of acquiring contract in the infobase. '"));
						...
						Сообщение.Текст =
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'В информационной базе не найден эквайринговый терминал с кодом %1!';
									|en = 'POS terminal with code %1 is not found in the infobase.'"),
			...
			Иначе
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не найден элемент ""НомерТерминала""!';
			...
			Если СтрокаПоТерминалу.Свойство("ВремяОперации", Знч) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Знч) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не указано время операции!';
			...
			Иначе
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не найден элемент ""ВремяОперации""!';
			...
			Если СтрокаПоТерминалу.Свойство("СуммаОперации", Знч) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Знч) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не указана сумма операции!';
			...
			Иначе
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не найден элемент ""СуммаОперации""!';
							|en = 'Item ""СуммаОперации"" is not found in the bank report on acquiring transactions (item ""Отчет"" No.%1, item ""СтрокаПоТерминалу"" No.%2).'"),
						НомерОтчета,
						НомерСтрокиПоТерминалу);
				Сообщение.Сообщить();
				...
				Если НЕ ЗначениеЗаполнено(Знч) Тогда
					...
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не указан номер карты!';
							|en = 'Card number is not specified in the bank report on acquiring transactions (item ""Отчет"" No.%1, item ""СтрокаПоТерминалу"" No.%2). '"),
						НомерОтчета,
						НомерСтрокиПоТерминалу);
					Сообщение.Сообщить();
			...
			Иначе
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) не найден элемент ""НомерКарты""!';
						|en = 'Item ""НомерКарты"" is not found in the bank report on acquiring transactions (item ""Отчет"" No.%1, item ""СтрокаПоТерминалу"" No.%2).'"),
					НомерОтчета,
					НомерСтрокиПоТерминалу);
				Сообщение.Сообщить();
			...
			Если СтрокаПоТерминалу.Свойство("Валюта", Знч) Тогда
				...
				Если ЗначениеЗаполнено(Знч)
				   ...
				   И НЕ СокрЛП(СтрокаПоТерминалуВалюта) = СокрЛП(ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Валюта.Наименование) Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1, элемент ""СтрокаПоТерминалу"" №%2) код валюты не соответствует коду валюты банковского счета!';
																									|en = 'Currency code in the bank report on acquiring transactions (item ""Report"" No.%1, item ""СтрокаПоТерминалу"" No.%2) does not match a currency code of the bank account.'"), НомерОтчета, НомерСтрокиПоТерминалу);
					Сообщение.Сообщить();
					...
					Если НЕ КонецДня(СтрЗаменить(ЭлементКонтрольнойСуммыПоДате.ЗначениеЭлемента, "-", "")) = КонецДня(ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Дата) Тогда
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1) дата в элементе ""КонтрольнаяСуммаПоДате"" не соответствует дате операции!';
								|en = 'Date in item ""КонтрольнаяСуммаПоДате"" does not match a transaction date in the bank report on acquiring transactions (item ""Отчет"" No.%1).  '"),
							НомерОтчета);
						Сообщение.Сообщить();
			...
			Если НЕ КоличествоЗаписейОплата = ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Покупки.Количество() Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1) количество записей по оплате в элементе ""КонтрольнаяСуммаПоДате"" не соответствует количеству операций по оплате!';
						|en = 'In the bank report on acquiring transactions (item ""Отчет"" No.%1), number of payment entries in item ""КонтрольнаяСуммаПоДате"" does not match the number of payment transactions.'"),
					НомерОтчета);
				Сообщение.Сообщить();
			...
			Если НЕ СуммаИтогоОплата = ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Покупки.Итог("Сумма") Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1) итоговая сумма по оплате в элементе ""КонтрольнаяСуммаПоДате"" не соответствует итоговой сумме операций по оплате!';
						|en = 'In the bank report on acquiring transactions (item ""Отчет"" No.%1),  total payment amount in item ""КонтрольнаяСуммаПоДате"" does not match the total payment transaction amount.'"),
					НомерОтчета);
				Сообщение.Сообщить();
			...
			Если НЕ КоличествоЗаписейВозврат = ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Возвраты.Количество() Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1) количество записей по возврату и по отмене в элементе ""КонтрольнаяСуммаПоДате"" не соответствует количеству операций по возврату!';
						|en = 'In the bank report on acquiring transactions (item ""Отчет"" No.%1), number of entries of refund and cancellation in item ""КонтрольнаяСуммаПоДате"" does not match the number of refund transactions.'"),
					НомерОтчета);
				Сообщение.Сообщить();
			...
			Если НЕ СуммаИтогоВозврат = ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Возвраты.Итог("Сумма") Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В отчете банка по операциям эквайринга (элемент ""Отчет"" №%1) итоговая сумма по возврату и по отмене в элементе ""КонтрольнаяСуммаПоДате"" не соответствует итоговой сумме операций по возврату!';
						|en = 'In the bank report on acquiring transactions (item ""Отчет"" No.%1),  total amount of refund and cancellation in item ""КонтрольнаяСуммаПоДате"" does not match the total refund transaction amount.'"),
					НомерОтчета);
				Сообщение.Сообщить();
		...
		Если НЕ ЗначениеЗаполнено(ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Дата) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При загрузке элемента ""Отчет"" №%1 в документе ""Отчет банка по операциям эквайринга"" не заполнена дата операции по платежной карте. Документ не создан!';
					|en = 'Payment card transaction date is not populated when importing item ""Report"" No.%1 in document ""Bank report on acquiring transactions"". Document was not created. '"),
				НомерОтчета);
			Сообщение.Сообщить();
		...
		ИначеЕсли ОбъектДокументаОтчетБанкаПоОперациямЭквайринга.Покупки.Количество()
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При загрузке элемента ""Отчет"" №%1 в документе ""Отчет банка по операциям эквайринга"" не заполнены платежи. Документ не создан!';
					|en = 'Payments are not populated when importing item ""Report"" No.%1 in document ""Bank report on acquiring transactions"". Document was not created. '"),
				НомерОтчета);
			Сообщение.Сообщить();
