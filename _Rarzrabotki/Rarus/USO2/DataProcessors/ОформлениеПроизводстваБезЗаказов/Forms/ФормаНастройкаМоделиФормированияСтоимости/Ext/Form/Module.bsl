...
Процедура СпецификацияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокИзделий = Новый Массив;
	
	Для Каждого ТекИзделие Из ПараметрыОтбора Цикл
		
		ДанныеСтроки = Новый Структура("Подразделение, Номенклатура, Характеристика");
		ЗаполнитьЗначенияСвойств(ДанныеСтроки, ТекИзделие);
		
		СписокИзделий.Добавить(ДанныеСтроки);
		
	КонецЦикла;
	
	УправлениеДаннымиОбИзделияхКлиент.ОткрытьФормуВыбораСпецификацийПоСпискуНоменклатуры(СписокИзделий, ПараметрыВыбораСпецификаций, ЭтаФорма);
