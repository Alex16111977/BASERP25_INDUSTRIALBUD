...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
		ВызватьИсключение НСтр("ru = 'Работа со всеми внешними ресурсами (синхронизация данных, отправка почты и т.п.) заблокирована.';
								|en = 'Operation of all external resources (data synchronization, email dispatching etc) is blocked.'");
	КонецЕсли;
	...
	Если Состояние <> Перечисления.СостоянияМиграцииПриложения.Выполняется Тогда
		...
		Наименование = СтрШаблон(НСтр("ru = '%1 (Миграция приложения)';
										|en = '%1 (Application migration)'"), Метаданные.Синоним);
...
Процедура СтраницаАдресСервисаОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru = 'Миграция приложения в сервис';
					|en = 'Application migration to service'");
...
Функция СтраницаАдресСервисаДалее()
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен адрес сервиса';
																|en = 'Service address is not filled in'"), , "АдресСервиса");
	...
	Если ЧастиАдреса.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректный адрес сервиса';
																|en = 'Incorrect service address'"), , "АдресСервиса");
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СтраницаИдентификационныеДанныеОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru = 'Аутентификация в сервисе';
					|en = 'Authentication in service'");
	
	Элементы.ДекорацияИдентификационныеДанные.Заголовок = СтрШаблон(НСтр("ru = 'Вход в сервис: %1';
																		|en = 'Logon to service: %1'"), АдресСервиса);
...
Функция СтраницаИдентификационныеДанныеДалее()
	...
	Если Не ЗначениеЗаполнено(РегистрацияНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Полное имя (Ф.И.О.)""';
																|en = 'The ""Full name"" field is not filled in'"), , "РегистрацияНаименование", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Логин) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Имя(логин)""';
																|en = 'The ""Name (login)"" field is not filled in'"), , "Логин", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РегистрацияЭлектроннаяПочта) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Электронная почта""';
																|en = 'The Email field is not filled in'"), , "РегистрацияЭлектроннаяПочта", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Пароль) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Пароль""';
																|en = 'The Password field is not filled in'"), , "Пароль", , Отказ);
	ИначеЕсли Пароль <> ПодтверждениеПароля Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Подтверждение пароля не совпадает с паролем';
																|en = 'Password confirmation does not match password'"), , "ПодтверждениеПароля", , Отказ);
	КонецЕсли;
...
Функция СтраницаВыборАбонентаДалее()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран абонент';
																|en = 'Subscriber not selected'"), , "КодАбонента");
	КонецЕсли;
...
Функция СтраницаСозданиеПриложениеДалее()
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Наименование""';
...
Процедура СтраницаСозданиеПриложениеОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru = 'Параметры создания приложения';
					|en = 'Application creation parameters'");
...
Процедура СтраницаОжиданиеОткрытие(ЭтоПереходНазад)
	...
	Строки.Добавить(СтрШаблон(НСтр("ru = 'Код абонента: %1';
									|en = 'Subscriber code: %1'"), КодАбонента) + Символы.ПС);
	Строки.Добавить(СтрШаблон(НСтр("ru = 'Код приложения: %1';
									|en = 'Application code: %1'"), КодПриложения) + Символы.ПС);
	Строки.Добавить(НСтр("ru = 'Контакты обслуживающей организации';
						|en = 'Service company contacts'") + ":" + Символы.ПС);
	Строки.Добавить(Символы.Таб);
	Строки.Добавить(Символы.Таб + СтрШаблон(НСтр("ru = 'e-mail: %1';
												|en = 'e-mail: %1'"), ОбслуживающаяОрганизацияЭлектроннаяПочта) + Символы.ПС);
	Строки.Добавить(Символы.Таб);
	Строки.Добавить(Символы.Таб + СтрШаблон(НСтр("ru = 'тел.: %1';
												|en = 'phone: %1'"), ОбслуживающаяОрганизацияТелефон) + Символы.ПС);
...
Функция СостояниеПерехода()
	...
	СостояниеПерехода.Вставить("ТекстСообщенийОтправленоОбработано", СтрШаблон(НСтр("ru = '%1 / %2';
																					|en = '%1 / %2'"), СостояниеПерехода.НомерОтправленногоСообщения, СостояниеПерехода.НомерПринятогоСообщения));
	СостояниеПерехода.Вставить("ТекстОбъектовВыгруженоЗагружено", СтрШаблон(НСтр("ru = '%1 / %2';
																				|en = '%1 / %2'"), СостояниеПерехода.ВыгруженоОбъектов, СостояниеПерехода.ЗагруженоОбъектов));
	СостояниеПерехода.Вставить("ТекстОсталосьВыгрузитьЗагрузить", СтрШаблон(НСтр("ru = '%1 / %2';
																				|en = '%1 / %2'"), СостояниеПерехода.ИзмененоОбъектов, СостояниеПерехода.ЗагрузитьОбъектов));
...
Процедура ОбновитьСтатусСопоставления(Форма)
	...
	Если НесопоставленоПользователей > 0 Тогда
		Заголовок = СтрШаблон(НСтр("ru = 'Пользователей из базы не сопоставлено: %1';
									|en = 'Unmapped infobase users: %1'"), НесопоставленоПользователей);
	Иначе
		Заголовок = НСтр("ru = 'Все пользователи из базы сопоставлены';
						|en = 'All users from the base are mapped'");
	КонецЕсли;
...
Функция ВыбратьАбонента()
	...
	Если АбонентыПользователя.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Нет прав для добавления приложений абоненту.';
								|en = 'No rights to add applications to the subscriber.'");
