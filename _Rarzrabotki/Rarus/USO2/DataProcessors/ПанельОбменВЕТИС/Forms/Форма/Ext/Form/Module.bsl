...
Процедура ОбновитьСпискиДокументов()
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Входящие транспортные операции ВетИС';
			|en = 'Incoming VetIS transport operations'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Исходящие транспортные операции ВетИС';
			|en = 'Outgoing VetIS transport operations'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Инвентаризации продукции ВетИС';
			|en = 'Stocktaking of VetIS manufactured products'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Запросы складского журнала ВетИС';
			|en = 'VetIS warehouse book requests'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Объединения записей складского журнала ВетИС';
			|en = 'Merge of VetIS warehouse book records'"),
		Ложь,    // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	ДобавитьДокумент(
		...
		НСтр("ru = 'Производственные операции ВетИС';
			|en = 'VetIS production operations'"),
		Истина,  // Оформите
		Истина,  // Отработайте
		Истина); // Ожидайте
	...
	Запрос.УстановитьПараметр("ПустойТорговыйОбъект",        ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("ТорговыйОбъектВЕТИС"));
	Запрос.УстановитьПараметр("ПустойПроизводственныйОбъект",ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("ПроизводственныйОбъектВЕТИС"));
	...
	Если Выборка.Следующий() И Выборка.КоличествоСообщений > 0 Тогда
		ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Есть сообщения (%1), ожидающие отправки в ВетИС. Выполните обмен.';
										|en = 'There are messages (%1) waiting to be sent to VetIS. Exchange.'"), Выборка.КоличествоСообщений);
