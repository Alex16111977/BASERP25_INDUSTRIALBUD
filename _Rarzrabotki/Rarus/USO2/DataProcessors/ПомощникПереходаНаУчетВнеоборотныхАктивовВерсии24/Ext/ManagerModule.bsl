...
Процедура ЗаполнитьГруппыПроверокДляРегистрации(ТаблицаГруппПроверок) Экспорт
	...
	ОписаниеГруппыПроверок.Наименование = НСтр("ru = 'Переход на учет внеоборотных активов версии 2.4';
												|en = 'Transfer to capital asset accounting 2.4'");
...
Процедура ТекстЗапросаДатыПринятияКУчету(ПараметрыОбработчика, ТекстыЗапроса)
	...
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(Состояния.ДатаСостояния, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
...
Процедура ТекстЗапросаВложенияВоВнеоборотныеАктивы_БУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ОстаткиВложенийВоВнеоборотныеАктивы_БУ";
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма БУ';
										|en = 'BKG amount'"), Выборка.СуммаРегл, Выборка.СуммаРегл24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма ПР';
										|en = 'PD amount'"), Выборка.СуммаПР, Выборка.СуммаПР24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма ВР';
										|en = 'TD amount'"), Выборка.СуммаВР, Выборка.СуммаВР24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма с НДС (упр)';
										|en = 'Amount VAT included (man)'"), Выборка.Сумма, Выборка.Сумма24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма без НДС (упр)';
										|en = 'Amount VAT exempt (man)'"), Выборка.СуммаБезНДС, Выборка.СуммаБезНДС24, СписокСтрок);
	...
	Для каждого ДанныеСтроки Из ТаблицаВсехЗначений Цикл
		...
		Если ДанныеСтроки.Счетчик > 0 Тогда
			...
			ТекстПроблемы = НСтр("ru = 'В документе %1 в табличной части ""%2"" указан лишний способ отражения расходов:
                                  ...
                                  |
                                  |Необходимо воспользоваться помощником в режиме ""Исправить ошибки"".';
                                  |en = 'Extra method of expense record is specified in the ""%2"" tabular section of the %1 document:
                                  |Department: %3
                                  |Line of business: %4
                                  |Expense item: %5
                                  |Expense dimension: %6
		...
		ИначеЕсли ДанныеСтроки.Счетчик < 0 Тогда
			...
			ТекстПроблемы = НСтр("ru = 'В документе %1 в табличной части ""%2"" отсутствует способ отражения расходов:
                                  ...
                                  |
                                  |Необходимо воспользоваться помощником в режиме ""Исправить ошибки"".';
                                  |en = 'Expense record method is missing in the ""%2"" tabular section of the %1 document:
                                  |Department: %3
                                  |Line of business: %4
                                  |Expense item: %5
                                  |Expense dimension: %6
	...
	Если ИсходныеДанные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковОсновныхСредств
		...
		Если ИсходныеДанные.Местонахождение <> ИсходныеДанные.МестонахождениеУУ
			...
			ТекстПроблемы = НСтр("ru = 'Отличается местонахождение по бухгалтерскому учету (%1) от местонахождения по международному учету (%2).
                                  ...
                                  |Location cannot differ at the time of migration.
                                  |It is recommended that you register transfer to one department either in bookkeeping or in international accounting and transfer the data again.'");
		...
		Если ИсходныеДанные.СуммаКапитальныхВложенийВключаемыхВРасходыНУОстаток <> 0
			...
			ТекстПроблемы = НСтр("ru = 'Обнаружен остаток амортизационной премии %1 %2 на счете КВ, который должен быть включен в расходы до перехода.
                                  ...
                                  |en = '%1 %2 bonus depreciation balance that must be included in the expenses before the transfer is detected on the CI account.
                                  |It is recommended that you correct the accounting data and transfer the data again.'");
