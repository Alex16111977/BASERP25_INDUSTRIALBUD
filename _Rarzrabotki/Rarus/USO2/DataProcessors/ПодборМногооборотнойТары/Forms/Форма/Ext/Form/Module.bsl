	...
	Если Не ПеренестиВДокумент И Модифицированность И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Многооборотная тара не перенесена в документ. Перенести?';
				|en = 'Reusable package is not transferred to the document. Transfer?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ЗаполнитьДеревоМногооборотнойТары()
	...
	Если РезультатЗапроса[2].Пустой() Тогда
		ИнформационнаяНадпись = НСтр("ru = 'В документе отсутствуют товары. Многооборотная тара не требуется.';
									|en = 'There are no goods in the document. Reusable package is not required.'");
...
Процедура ВывестиПодчиненныеУпаковки(УпаковкиСтроки, УпаковкаУзел, ТаблицаУпаковок, ТараДокумента, ВсяТараПодобрана)
	...
	Для Каждого ТекСтрока Из ПодчиненныеУпаковки Цикл
		...
		Если ЗначениеЗаполнено(НоваяСтрокаУпаковка.НоменклатураМногооборотнаяТара) Тогда
			...
			Если ОстатокТары > 0 И ОстатокТары >= ТекСтрока.МинимальноеКоличествоУпаковокМногооборотнойТары Тогда
			...
			Если ТекСтрока = ПодчиненныеУпаковки[ПодчиненныеУпаковки.Количество()-1] И КоличествоНоменклатурыКРаспределению > 0 Тогда
				...
				КоличествоНоменклатурыКРаспределению = 0;
				НоваяСтрокаУпаковка.РекомендуемоеКоличествоТары = НоваяСтрокаУпаковка.РекомендуемоеКоличествоТары + 1;
				НоваяСтрокаУпаковка.ПодобранноеКоличествоТары = НоваяСтрокаУпаковка.РекомендуемоеКоличествоТары;
