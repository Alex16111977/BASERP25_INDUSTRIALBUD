...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Иначе
		...
		Элементы.Отбор.Заголовок = ?(ВидимостьОтбора, НСтр("ru = 'Скрыть отбор';
															|en = 'Hide filter'"), НСтр("ru = 'Показать отбор';
																						|en = 'Show filter'"));
		...
		Элементы.РежимЗаполнения.Заголовок = ?(ВидимостьРежимаЗаполнения, НСтр("ru = 'Скрыть режим заполнения';
																				|en = 'Hide population mode'"), НСтр("ru = 'Показать режим заполнения';
																														|en = 'Show fill mode'"));
...
Процедура ЗаполнитьТаблицуТоваров(Команда)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru = 'Сохранить данные о количестве, шаблонах в табличной части при перезаполнении?';
							|en = 'Save information about quantity, templates in the tabular section when refilling?'");
...
Процедура УстановитьКоличествоЦенников(Команда)
	...
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьКоличествоЦенниковЗавершение", ЭтотОбъект, Новый Структура("МассивСтрок, ВыбранноеЗначение", МассивСтрок, ВыбранноеЗначение)), ВыбранноеЗначение, НСтр("ru = 'Введите количество ценников';
																																																				|en = 'Enter price tag quantity'"), 10, 2);
...
Процедура УстановитьКоличествоЭтикеток(Команда)
	...
	ВыбранноеЗначение = Неопределено;
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьКоличествоЭтикетокЗавершение", ЭтотОбъект, Новый Структура("МассивСтрок, ВыбранноеЗначение", МассивСтрок, ВыбранноеЗначение)), ВыбранноеЗначение, НСтр("ru = 'Введите количество';
																																																				|en = 'Enter quantity'"), 10, 2);
...
Процедура ВыбратьВариантЗаполненияУпаковок(Команда)
	...
	Если Элементы.Товары.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не выделено ни одной строки. Заполнение упаковок невозможно.';
								|en = 'No lines selected. Cannot fill in packaging.'");
		...
		Если Строка <> Неопределено Тогда
			Если Строка.Выбран Тогда
				Если Строка.КоличествоЭтикеток = 0 Тогда
					...
					ТекстСообщения = НСтр("ru = 'В строке %Номер% не указано количество этикеток. Упаковки в ней не будут заполнены.';
											|en = 'Number of labels is not specified in line %Номер%. Packagings will not be populated in it.'");
			...
			Иначе
				...
				ТекстСообщения = НСтр("ru = 'Строка %Номер% не выбрана. Упаковки в ней не будут заполнены.';
										|en = 'Line %Номер% is not selected. Packings in this line will not be populated.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Номер%", Строка.НомерСтроки);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
		КонецЕсли;
			...
			Если СтрокаТЧ.КоличествоЭтикеток = 0 Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не заполнено количество этикеток в строке: %1';
						|en = 'Number of labels is not populated in line: %1'"),
					НомерСтроки + 1);
			...
			Если СтрокаТЧ.КоличествоЦенников = 0 Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не заполнено количество ценников в строке: %1';
						|en = 'Number of price tags is not populated in line: %1'"),
					НомерСтроки + 1);
...
Процедура ПроверитьЗаполнениеШаблоновЭтикеток(ТоварыДляПечати, ЕстьОшибки = Ложь)
	...
	Для Каждого СтрокаТоваров Из СтрокиТоваров Цикл
		ТекстСообщения = НСтр("ru = 'У номенклатуры ""%Номенклатура%"" не выбран шаблон этикетки.';
								|en = 'Label template is not selected for the ""%Номенклатура%"" products.'");
