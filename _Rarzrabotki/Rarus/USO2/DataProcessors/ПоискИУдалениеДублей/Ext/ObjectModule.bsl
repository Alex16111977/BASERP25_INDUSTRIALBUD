...
Функция МенеджерОбластиПоискаДублей(Знач ИмяОбластиПоискаДанных) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неизвестный тип объекта метаданных ""%1""';
																					|en = 'Unknown metadata object type ""%1""'"), ИмяОбластиПоискаДанных);
КонецФункции
		...
		Если ДоступноеПолеКД = Неопределено Тогда
			ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""%1"" не существует.';
																			|en = 'The ""%1"" field does not exist.'"), Строка(ПолеКД)));
			Продолжить;
		КонецЕсли;
			...
			Если FuzzySearch = Неопределено Тогда
				Результат.ОписаниеОшибки = 
					НСтр("ru = 'Не удалось подключить внешнюю компоненту FuzzyStringMatchExtension из макета ""ОбщийМакет.КомпонентаПоискаСтрок""
					           ...
					           |en = 'Cannot connect the FuzzyStringMatchExtension external component from the ""CommonTemplate.LineSearchComponent"" template
		...
		Если РазмерВозвращаемойПорции > 0 И (ТаблицаДублей.Количество() > РазмерВозвращаемойПорции) Тогда
			...
			Если Найдено > 0 И ТаблицаДублей.Количество() = 0 Тогда
				Результат.ОписаниеОшибки = НСтр("ru = 'Найдено слишком много дублей одного элемента.';
												|en = 'Too many duplicates of one item are found.'");
			Иначе
				Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Найдено слишком много дублей. Показаны только первые %1.';
						|en = 'Too many duplicates are found. Only the first %1 are shown.'"), РазмерВозвращаемойПорции);
			КонецЕсли;
			Прервать;
		КонецЕсли;
	КонецЦикла;
