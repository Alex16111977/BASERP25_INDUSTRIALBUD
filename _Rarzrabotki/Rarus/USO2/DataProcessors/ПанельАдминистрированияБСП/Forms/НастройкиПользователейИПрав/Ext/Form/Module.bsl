	 ...
	 Или Не ПользователиСлужебный.ВнешниеПользователиВнедрены() Тогда
		...
		Элементы.ОписаниеРаздела.Заголовок =
			НСтр("ru = 'Администрирование пользователей, настройка групп доступа, управление пользовательскими настройками.';
				|en = 'User administration, access group setup, custom settings management.'");
	КонецЕсли;
	...
	Если НаборКонстант.ИспользоватьВнешнихПользователей Тогда
		...
		ТекстВопроса =
			НСтр("ru = 'Разрешить доступ внешним пользователям?
			           ...
			           |en = 'Allow external user access?
			           |
			           |After logon, user selection list will be empty (attribute ""Show in the selection list"" in cards of all users will be cleared and hidden).
			           |
			           |'");
	...
	Иначе
		ТекстВопроса =
			НСтр("ru = 'Запретить доступ внешним пользователям?
			           ...
			           |en = 'Deny access to external users?
			           |
			           |The ""Logon to the application is permitted"" attribute will be
			           |cleared in all external users'' cards.'");
