...
Функция ТекстЗапросаСпискаРаспоряженийНаОформление()
	...
	|	Таблица.Назначение КАК Назначение,
	|	Таблица.Склад КАК Склад,
	|	МАКСИМУМ(Таблица.СостояниеПоступления) КАК СостояниеПоступления,
	|	МАКСИМУМ(Таблица.СостояниеПриходногоОрдера) КАК СостояниеПриходногоОрдера,
	...
	|		Таблица.СостояниеПриходногоОрдера КАК СостояниеПриходногоОрдера,
	...
	|			ИЗ
	|				ВтДокументыОформленияПоОстаткам КАК ТаблицаДокументов
	|			)
	|	) КАК Таблица
	...
	|				ТОГДА 0
	|			КОГДА Таблица.КОформлениюНакладных > 0
	|					И ЕСТЬNULL(ТаблицаНакладныеОрдера.КОформлениюНакладныхПоРаспоряжению, 0) = 0
	|				ТОГДА 1
	|			КОГДА Таблица.КОформлениюНакладных > 0
	|					И ЕСТЬNULL(ТаблицаНакладныеОрдера.КОформлениюНакладныхПоРаспоряжению, 0) > 0
	|				ТОГДА 2
	...
	|					И Таблица.КОформлениюОрдеровВсего = 0
	|				ТОГДА 0
	|			КОГДА Таблица.КОформлениюОрдеров > 0
	|					И ЕСТЬNULL(ТаблицаНакладныеОрдера.КОформлениюПоступленийПоОрдерам, 0) = 0
	|				ТОГДА 1
	|			КОГДА ЕСТЬNULL(ТаблицаНакладныеОрдера.КОформлениюПоступленийПоОрдерам, 0) > 0
	|				ТОГДА 3
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СостояниеПриходногоОрдера
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКПоступлению КАК ТаблицаНакладныеОрдера
	|		ПО Таблица.ДокументПоступления = ТаблицаНакладныеОрдера.ДокументПоступления
	|			И Таблица.Номенклатура = ТаблицаНакладныеОрдера.Номенклатура
	|			И Таблица.Характеристика = ТаблицаНакладныеОрдера.Характеристика
	|			И Таблица.Назначение = ТаблицаНакладныеОрдера.Назначение
	|			И Таблица.Склад = ТаблицаНакладныеОрдера.Склад
	|			И ТаблицаНакладныеОрдера.Активность
	|			И ТаблицаНакладныеОрдера.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			И ТаблицаНакладныеОрдера.Регистратор ССЫЛКА Документ.ПриходныйОрдерНаТовары
	|
	|СГРУППИРОВАТЬ ПО
	|	Таблица.ДокументПоступления,
	|	Таблица.Склад
	...
	|	Таблица.Склад КАК Склад,
	|	Таблица.СостояниеПоступления КАК СостояниеНакладной,
	|	Таблица.СостояниеПриходногоОрдера КАК СостояниеОрдера,
	|	Реестр.ДатаДокументаИБ КАК Дата,
	|	Реестр.НомерДокументаИБ КАК Номер,
	|	ТИПЗНАЧЕНИЯ(Таблица.Ссылка) КАК ТипРаспоряжения,
	|	Реестр.Организация КАК Организация,
	|	Реестр.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	Реестр.Партнер КАК Партнер,
	|	Реестр.Контрагент.Ключ КАК Контрагент,
	|	Реестр.Подразделение КАК Подразделение,
	|	Реестр.Ответственный КАК Ответственный,
	|	Реестр.Валюта КАК Валюта,
	|	Реестр.Сумма КАК Сумма,
	|	Реестр.Комментарий КАК Комментарий,
	|	Реестр.ПометкаУдаления КАК ПометкаУдаления,
	|	ВЫБОР
	|		КОГДА Реестр.Ссылка ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
	|			ТОГДА ВЫРАЗИТЬ(Реестр.Ссылка КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Договор
	|		КОГДА Реестр.Ссылка ССЫЛКА Документ.ВозвратТоваровОтКлиента
	|			ТОГДА ВЫРАЗИТЬ(Реестр.Ссылка КАК Документ.ВозвратТоваровОтКлиента).Договор
	|		КОГДА Реестр.Ссылка ССЫЛКА Документ.ПоступлениеТоваровОтХранителя
	|			ТОГДА ВЫРАЗИТЬ(Реестр.Ссылка КАК Документ.ПоступлениеТоваровОтХранителя).Договор
	|	КОНЕЦ КАК Договор,
	|	ВЫБОР
	|		КОГДА Реестр.Ссылка ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
	|			ТОГДА ВЫРАЗИТЬ(Реестр.Ссылка КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).Соглашение
	|		КОГДА Реестр.Ссылка ССЫЛКА Документ.ВозвратТоваровОтКлиента
	|			ТОГДА ВЫРАЗИТЬ(Реестр.Ссылка КАК Документ.ВозвратТоваровОтКлиента).Соглашение
	|		КОГДА Реестр.Ссылка ССЫЛКА Документ.ПоступлениеТоваровОтХранителя
	|			ТОГДА ВЫРАЗИТЬ(Реестр.Ссылка КАК Документ.ПоступлениеТоваровОтХранителя).Соглашение
...
Функция ЗаполнитьСпискиВыбораПоСостояниямОтгрузки()
	...
	СписокВыбора.Добавить("0", НСтр("ru = 'Все оформлено';
									|en = 'Everything is registered'"), Ложь, БиблиотекаКартинок.ОформленаНакладная);
	...
	СписокВыбора.Добавить("0", НСтр("ru = 'Соответствуют накладным';
									|en = 'Correspond to invoices'"), Ложь, БиблиотекаКартинок.ОформленаНакладная);
	СписокВыбора.Добавить("1", НСтр("ru = 'Не оформлены';
									|en = 'Not registered'"), Ложь, БиблиотекаКартинок.СоздатьНакладную);
	СписокВыбора.Добавить("3", НСтр("ru = 'Не соответствуют накладным';
									|en = 'Do not correspond to invoices'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
	СписокВыбора.Добавить("4", НСтр("ru = 'Не используется ордерная схема';
									|en = 'Advanced warehousing is not used'"), Ложь, БиблиотекаКартинок.ПустаяКартинка);
				...
				Иначе
					...
					Если ЕстьНакладная И Не ЕстьОрдер И Не РежимБезВопросов Тогда
						ТекстОшибки = НСтр("ru = 'Недоступно оформление по приемке, так как не создано ни одного ордера.';
											|en = 'Cannot register acceptance as no note is created.'");
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, "Объект.СписокРаспоряженияНаОформление",ТекстОшибки, Неопределено);
					Иначе
...
Функция РазделитьЗаказыПоПолнымИменамНакладных(ПоОрдерам, СписокОшибок)
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		Если ПоОрдерам Тогда
			...
			Если Строка.СостояниеОрдера = 0 Тогда
				...
				ТекстОшибки = НСтр("ru = 'Для распоряжения %1 невозможно перезаполнить накладную по приемке, т.к. ордера соответствуют накладным.';
									|en = 'Cannot refill an acceptance invoice for the %1 reference as the notes match the invoices.'");
			...
			ИначеЕсли Строка.СостояниеОрдера = 4 Тогда
				...
				ТекстОшибки = НСтр("ru = 'Для распоряжения %1 невозможно оформить накладную по приемке, т.к. склад не использует ордерную схему.';
									|en = 'Cannot register an acceptance invoice for the %1 reference as the warehouse does not use advanced warehousing.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
		...
		Иначе // оформление по заказам
			...
			//++ НЕ УТ
					Или ТипЗнч(Строка.Ссылка)= Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя")
			//-- НЕ УТ
				Тогда
...
Функция ОформитьНакладнуюПоТаблице(ДокументыПоТипамНакладных, ПоОрдерам, СписокОшибок)
	...
	Для Каждого Распоряжение Из Заказы Цикл
		...
		Если ТипЗнч(Распоряжение)= Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
			//++ НЕ УТ
			Или ТипЗнч(Распоряжение)= Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя")
			//-- НЕ УТ
			Тогда
