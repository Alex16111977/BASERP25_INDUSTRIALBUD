...
Процедура СоздатьДокументПродажи(ПоляДокумента, ПараметрыОснования, МассивДокументов, ПараметрыФормирования) Экспорт
	...
	//-- НЕ УТ
	ИначеЕсли ПоляДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровОтКлиента Тогда
...
Функция ТекстЗапросаЗаказыКОформлению(Организация = Неопределено, Склад = Неопределено)
	...
	|ПОМЕСТИТЬ РаспоряженияНаОформление
	|ИЗ
	|	Документ.ЗаказКлиента КАК РаспоряженияНаОформление
	|ГДЕ
	|	РаспоряженияНаОформление.Проведен
	...
	|						РаспоряженияНаОформление.Ссылка КАК Ссылка
	|					ИЗ
	|						РаспоряженияНаОформление КАК РаспоряженияНаОформление)) КАК ЗаказыКлиентов
	|	ГДЕ
	...
	|						РаспоряженияНаОформление.Ссылка КАК Ссылка
	|					ИЗ
	|						РаспоряженияНаОформление КАК РаспоряженияНаОформление)
