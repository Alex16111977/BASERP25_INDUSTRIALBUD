...
Процедура ПроверитьЗаполнениеАкцизныхМарокРегистр2(Объект, Отказ)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если СтрокаТЧ.Поштучная = Истина Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'В строке %1 для %2 указана поштучная Справка 2 которая не может быть учтена в регистре 2 ЕГАИС.';
					|en = 'В строке %1 для %2 указана поштучная Справка 2 которая не может быть учтена в регистре 2 ЕГАИС.'"),
				СтрокаТЧ.НомерСтроки,
				СтрокаТЧ.АлкогольнаяПродукция);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибки,
		...
		Если НайденныеСтроки.Количество() Тогда
			ТекстОшибки = НСтр("ru = 'В строке %1 для %2 считана поштучная акцизная марка которая не может быть учтена в регистре 2 ЕГАИС.';
								|en = 'В строке %1 для %2 считана поштучная акцизная марка которая не может быть учтена в регистре 2 ЕГАИС.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки,
				СтрокаТЧ.НомерСтроки,
				СтрокаТЧ.АлкогольнаяПродукция);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
// Описание
// Определяет вид продукци по переданному списку штрихкодов упаковок.
// Параметры:
// 	ШтрихкодыУпаковок - Массив - Список типа СправочникСсылка.ШтрихкодыУпаковокТоваров
// Возвращаемое значение:
// 	Соответствие - Соответсвие СправочникСсылка.ШтрихкодыУпаковокТоваров:ПеречислениеСсылка.ВидыПродукцииИС.
Функция СоответствиеШтрихкодовУпаковокИВидовПродукции(ШтрихкодыУпаковок)
	...
	"выбрать РАЗЛИЧНЫЕ
	|	Внутренний.Штрихкод,
	|	Внутренний.ВидПродукции
	|ИЗ
	|	(ВЫБРАТЬ
	|		ШтрихкодыУпаковокТоваров.Ссылка КАК Штрихкод,
	|		ВЫБОР
	|			КОГДА ШтрихкодыУпаковокТоваров.Номенклатура.АлкогольнаяПродукция
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.Алкогольная)
	|			КОГДА ШтрихкодыУпаковокТоваров.Номенклатура.ТабачнаяПродукция
	...
	|		И ШтрихкодыУпаковокТоваров.Номенклатура <> &ПустаяНоменклатура
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		ВЫБОР
	|			КОГДА ВложенныеШтрихкоды.Штрихкод.Номенклатура.АлкогольнаяПродукция
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.Алкогольная)
	|			КОГДА ВложенныеШтрихкоды.Штрихкод.Номенклатура.ТабачнаяПродукция
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.Табачная)
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК ВидПродукции
	|	ИЗ
	...
	|	ГДЕ
	|		ВложенныеШтрихкоды.Ссылка В (&ШтрихкодыУпаковок)
	|		И ВложенныеШтрихкоды.Штрихкод.Номенклатура <> &ПустаяНоменклатура) как Внутренний";
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Результат.Вставить(Выборка.Штрихкод, Выборка.Видпродукции);
	КонецЦикла;
