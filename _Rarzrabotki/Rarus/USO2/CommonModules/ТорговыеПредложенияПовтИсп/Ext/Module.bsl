...
Функция КодЕдиницыИзмерения(ЕдиницаИзмерения) Экспорт
	...
	Если ЗначениеЗаполнено(ЕдиницаИзмерения) Тогда
		Возврат СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЕдиницаИзмерения, "Код"));
	КонецЕсли;
...
Функция ПолучитьВалюту(Код) Экспорт
	...
	Отказ = Ложь;
	ПараметрыКоманды = ТорговыеПредложения.ПараметрыКомандыПолучитьВалютуПоКоду(Код);
...
Функция ТипыДанных() Экспорт
	...
	Результат.Вставить("ОбъектСопоставления", Метаданные.ОпределяемыеТипы.ОбъектыСопоставленияБизнесСеть.Тип);
