...
Функция ЦелиАттестованныхСпециалистов(ГруппыПродукции)
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		Если МасГруппаПриказа.Количество() = 1 Тогда
			СтрТипПродукции  = Неопределено;
			СтрГруппаПриказа = СокрЛП(МасГруппаПриказа[0]);
		Иначе
		...
		Если ГруппаПриказа <> СтрГруппаПриказа Тогда
			...
			ГруппаПродукции = ГруппыПродукции.Получить(ГруппаПриказа);
			Если ГруппаПродукции = Неопределено И СтрТипПродукции <> Неопределено Тогда
