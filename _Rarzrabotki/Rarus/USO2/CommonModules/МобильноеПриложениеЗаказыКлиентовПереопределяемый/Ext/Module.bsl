...
Процедура ВыгрузитьКлиентовДляОбмена(ЗаписьXML, УзелОбмена, ДопСвойства, ВыгружатьПолноеДосье = Ложь) Экспорт
	...
	ЗаголовокЗаписи = НСтр("ru = 'Обмен партнерами.Начало пакета выгрузки партнеров';
							|en = 'Partner exchange.Start of partner exchange package'", КодЯзыка);
	...
	ЗаголовокЗаписи = НСтр("ru = 'Обмен партнерами.Конец пакета выгрузки партнеров';
							|en = 'Partner exchange.End of partner export package'", КодЯзыка);
...
Процедура ВыгрузитьПрайсЛистДляОбмена(ЗаписьXML, СообщениеОбмена, УзелОбмена, НомерСообщения, ВсеЦены, ДопСвойства) Экспорт
	...
	Если ТаблицаПрайсЛиста.Количество() = 0 Тогда
		...
		СообщениеОбмена = НСтр("ru = 'Нет доступных цен.';
								|en = 'No available prices.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Возврат;
	КонецЕсли;
	ВремяНачалаОбмена = ТекущаяДатаСеанса();
	ТребуетсяПродолжение = Ложь;
...
Процедура ЗаписатьКонтактнуюИнформациюСправочника(ИнформацияПоОбъекту, Знач ТаблицаКИ, Знач ИмяСправочника)
	...
	ИмяСправочника = СтрЗаменить(ИмяСправочника, НСтр("ru = 'Партнер';
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обмен мобильное приложение 1С:Заказы.Запись справочника';
				|en = 'Exchange 1C:Orders mobile application.Write catalog'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ИнформацияНДС = 0 Тогда
		Если СтруктураВидаЦены.ЦенаВключаетНДС Тогда
			ТекстПроНДС = НСтр("ru = '(с НДС)';
								|en = '(VAT included)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Иначе
			ТекстПроНДС = НСтр("ru = '(без НДС)';
								|en = '(excl. VAT)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
...
Процедура ЗаполнитьИнформациюПоДоставке(ДокументОбъект, МобильныеДанные, ТипДокумента)
	...
	Иначе
		ДокументОбъект.АдресДоставки = НСтр("ru = '<адрес не указан/>';
											|en = '<address is not specified/>'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ЗаменитьПартнераНаКлиента(Знач Строка)
	...
	СтрокаЗамены = Строка;
	СтрокаЗамены = СтрЗаменить(СтрокаЗамены, НСтр("ru = 'Партнер';
													|en = 'Partner'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru = 'Клиент';
			|en = 'Client'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	СтрокаЗамены = СтрЗаменить(СтрокаЗамены, НСтр("ru = 'партнер';
													|en = 'partner'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		НСтр("ru = 'клиент';
			|en = 'client'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура СоздатьОбновитьДоверенность(СтруктураОплаты, Сообщение)
	...
	ДоверенностьКомментарий = СтрШаблон(НСтр("ru = 'Оплата по доверенности №%1 от %2';
											|en = 'Payment under the letter of authority No. %1 of %2'"),
