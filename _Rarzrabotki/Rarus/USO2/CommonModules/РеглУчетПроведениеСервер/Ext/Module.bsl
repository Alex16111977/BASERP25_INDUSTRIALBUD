	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Установка даты начала УУ на плане счетов Хозрасчетный.';
					|en = 'Set the start date of MA on the Self-financing chart of accounts.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		Для каждого СтрокаОшибки Из ТаблицаОшибокОтражения Цикл
			...
			Если ТипЗнч(СтрокаОшибки.ВидСчета) <> Тип("ПеречислениеСсылка.ВидыСчетовРеглУчета") Тогда
				ТекстСообщения = НСтр("ru = 'При записи счетов требующих настроек, ошибка: некорректно задан вид счета ""%1"".';
										|en = 'An error occurred when writing the accounts requiring settings: the ""%1"" account kind is specified incorrectly.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если ЕстьБлокировкаДанных Тогда
		...
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Отражение документов ""%1""
				...
				|in compliance accounting is stopped as the incoming data is locked – infobase update is not completed.'"),
			ТипДокумента);
