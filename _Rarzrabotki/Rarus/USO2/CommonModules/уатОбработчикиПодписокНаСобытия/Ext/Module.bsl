...
Процедура ПроверитьПрефиксОбъектаПоОрганизации(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", Источник.Метаданные())
		И Источник.Ссылка.Организация.Префикс <> Источник.Организация.Префикс Тогда
	...
	Если ВыборкаГруппировка.Следующий() Тогда 
		...
		Иначе 
			...
			Иначе 
				...
				ИначеЕсли флУстановленоНаТекТС И ПредТипТС <> ТекТипАгрегата Тогда 
					МенеджерЗаписи = РегистрыСведений.уатУстановленныеАгрегаты.СоздатьМенеджерЗаписи();
					МенеджерЗаписи.Период                  = ПериодЗаписи;
					МенеджерЗаписи.УзелОбъектаЭксплуатации = Источник.Ссылка;
					МенеджерЗаписи.ТипАгрегата             = ПредТипТС;
					МенеджерЗаписи.ТранспортноеСредство    = ТекТС;
					
					МенеджерЗаписи.Прочитать();
					
					Если МенеджерЗаписи.Выбран() Тогда 
						МенеджерЗаписи.ТипАгрегата = ТекТипАгрегата;
...
Процедура ЗарегистрироватьОснованиеДляЗаказаНаТС(Источник, Отказ, РежимПроведения)
	...
	ИначеЕсли Метаданные.Перечисления.Найти(
		...
		Если ПараметрРегистрации = 
				...
				уатОбщегоНазначенияТиповые.уатЕстьДокумент("ЗаказКлиента") И 
				ТипЗнч(Источник) <> Тип("ДокументОбъект.ЗаказКлиента") Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
