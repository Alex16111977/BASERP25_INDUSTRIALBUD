...
// Возвращает параметры необходимые для выполнения действия "ПроверитьЗаполнитьНазначениеПобочногоВыхода".
//
// Параметры:
//  Объект								 - Структура, ДанныеФормыСтруктура, ДокументОбъект.ЭтапПроизводства2_2	 - данные документа "этап производства"
//  СхемаОбособленияКэшированноеЗначение - Структура															 - схема обособления по документу
// 
// Возвращаемое значение:
//  Структура - список параметров.
//
Функция ПараметрыДействияПроверитьЗаполнитьНазначениеПобочногоВыхода(Объект, СхемаОбособленияКэшированноеЗначение = Неопределено) Экспорт
	
	Если (СхемаОбособленияКэшированноеЗначение = Неопределено
		ИЛИ Объект.Распоряжение <> СхемаОбособленияКэшированноеЗначение.Распоряжение
		ИЛИ Объект.НазначениеПродукция <> СхемаОбособленияКэшированноеЗначение.НазначениеПродукция) Тогда
	...
	Иначе
		СхемаОбособления = СхемаОбособленияКэшированноеЗначение;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СхемаОбособления.НазначениеПобочногоВыхода) Тогда
		
		НазначениеПобочногоВыхода = СхемаОбособления.НазначениеПобочногоВыхода;
		
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья Тогда
		
		НазначениеПобочногоВыхода = Объект.НазначениеМатериалы;
		
	Иначе
		НазначениеПобочногоВыхода = Справочники.Назначения.ПустаяСсылка();
	КонецЕсли;
	
	ПараметрыДействия = Новый Структура("НазначениеПобочногоВыхода", НазначениеПобочногоВыхода);
...
//  ЖелаемаяДатаОтгрузки			 - Дата																			 - желаемая дата отгрузки.
// 
// Возвращаемое значение:
//  Структура - Содержит свойства: Склад, ВариантОбеспечения.
//
Функция ПараметрыОбеспеченияМатериалаЭтапа(Номенклатура, Характеристика, Производится, Подразделение, ИсточникПолученияПолуфабриката, ЗаказНаПроизводство, ЖелаемаяДатаОтгрузки) Экспорт
	...
	ТаблицаМатериалов.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаМатериалов.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаМатериалов.Колонки.Добавить("ЖелаемаяДатаОтгрузки", Новый ОписаниеТипов("Дата"));
	...
	СтрокаМатериал.ЖелаемаяДатаОтгрузки = ЖелаемаяДатаОтгрузки;
	
	ПараметрыОбеспечения = ПараметрыОбеспеченияМатериаловЭтапа(ТаблицаМатериалов, ЗаказНаПроизводство);
	
	ПараметрыОбеспеченияМатериала = Новый Структура("Склад, ВариантОбеспечения, ДатаОтгрузки");
	ЗаполнитьЗначенияСвойств(ПараметрыОбеспеченияМатериала, ПараметрыОбеспечения[0]);
	
	Возврат ПараметрыОбеспеченияМатериала;
...
Функция ПараметрыОбеспеченияМатериаловЭтапа(ТаблицаМатериалов, ЗаказНаПроизводство) Экспорт
	...
	|	МатериалыИУслуги.ЖелаемаяДатаОтгрузки                                               КАК ЖелаемаяДатаОтгрузки,
	|	ВЫБОР 
	...
	|	ЕСТЬNULL(ДатыПлановойПоставки.Дата, ВЫБОР
	|			КОГДА Товары.ЖелаемаяДатаОтгрузки > &НачалоТекущегоДня
	|				ТОГДА Товары.ЖелаемаяДатаОтгрузки
	...
	ТекстЗапросаПараметрыОбеспечения = ПроизводствоСервер.ТекстЗапросаПараметрыОбеспечения(
		"МатериалыИУслуги",,,Перечисления.ВариантыЗаполненияОбеспеченияПроизводства.ПоСхемамОбеспечения);
...
Процедура ЗаполнитьОбеспечениеВСтрокеЭтапаПроизводства(ТекущаяСтрока, ПараметрыДействия) Экспорт
	...
	ПараметрыОбеспечения = ОбеспечениеПроизводства.ПараметрыОбеспеченияМатериалаЭтапа(
								...
								ПараметрыДействия.ЖелаемаяДатаОтгрузки);
	
	ТекущаяСтрока[ПараметрыДействия.ИмяПоляСклад] = ПараметрыОбеспечения.Склад;
	ТекущаяСтрока.ДатаОтгрузки                    = ПараметрыОбеспечения.ДатаОтгрузки;
	
	// переопределение поведения для режима "Отражение выполненного этапа производства"
	Если ПараметрыДействия.Свойство("ВариантОбеспечения") И ЗначениеЗаполнено(ПараметрыДействия.ВариантОбеспечения) Тогда
		Если ПараметрыДействия.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить
			И ПараметрыОбеспечения.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется Тогда
			ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить;
		ИначеЕсли ПараметрыДействия.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить
			И ПараметрыОбеспечения.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно Тогда
			ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно;
		Иначе
			ВариантОбеспечения = ПараметрыДействия.ВариантОбеспечения;
		КонецЕсли;
	...
	Если ПараметрыДействия.Свойство("ПроверитьСериюРассчитатьСтатус") Тогда
		ПараметрыДействия.СтруктураПараметровДействияСерий.Склад = ПараметрыОбеспечения.Склад;
	КонецЕсли;
...
Функция ПараметрыВыбораОбеспеченияЭтапа(ТекущаяСтрока, Объект, ТабличнаяЧасть, ПутиКДанным, ПараметрыЗаполнения) Экспорт
	
	ДанныеСтроки = ТабличнаяЧасть.НайтиПоИдентификатору(ТекущаяСтрока);
...
Процедура ЗаполнитьНазначениеОбеспеченияЭтапа(Объект, ПараметрыРедактированияЭтапа) Экспорт
	
	ТабличнаяЧасть = Объект.ОбеспечениеМатериаламиИРаботами;
	Если ПараметрыРедактированияЭтапа.Режим = "Обработка" Тогда
		...
		Для каждого Строка Из ТабличнаяЧасть Цикл
			ДанныеЭтапа = ДанныеЭтапов.Получить(Строка[ИмяРеквизитаСсылка]);
			Если ДанныеЭтапа = Неопределено Тогда
				ДанныеЭтапа = Новый Структура("НазначениеПолуфабрикаты,НазначениеМатериалы");
				Отбор = Новый Структура(ИмяРеквизитаСсылка, Строка[ИмяРеквизитаСсылка]);
				НайденныеСтроки = Объект.Этапы.НайтиСтроки(Отбор);
				Если НайденныеСтроки.Количество() > 0 Тогда
					ЗаполнитьЗначенияСвойств(ДанныеЭтапа, НайденныеСтроки[0]);
				КонецЕсли;
				ДанныеЭтапов.Вставить(Строка[ИмяРеквизитаСсылка], ДанныеЭтапа);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Строка.Назначение) Тогда
				Если Строка.Производится Тогда
					Строка.НазначениеОбеспечения = ДанныеЭтапа.НазначениеПолуфабрикаты;
				Иначе
					Строка.НазначениеОбеспечения = ДанныеЭтапа.НазначениеМатериалы;
				КонецЕсли;
			Иначе
				Строка.НазначениеОбеспечения = Строка.Назначение;
			КонецЕсли;
		КонецЦикла;
	Иначе
		Для каждого Строка Из ТабличнаяЧасть Цикл
			Если Не ЗначениеЗаполнено(Строка.Назначение) Тогда
				Если Строка.Производится Тогда
					Строка.НазначениеОбеспечения = Объект.НазначениеПолуфабрикаты;
				Иначе
					Строка.НазначениеОбеспечения = Объект.НазначениеМатериалы;
				КонецЕсли;
			Иначе
				Строка.НазначениеОбеспечения = Строка.Назначение;
			КонецЕсли;
