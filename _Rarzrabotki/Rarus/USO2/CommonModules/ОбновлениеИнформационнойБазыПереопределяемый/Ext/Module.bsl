...
Процедура ПриФормированииОчередейОтложенныхОбработчиков(ОбработчикИОчередь) Экспорт
	...
	ОбработчикИОчередь.Вставить("Документы.СообщениеОбменСБанками.ОбработатьДанныеДляПереходаНаНовуюВерсию", 5);
	ОбработчикИОчередь.Вставить("РегистрыСведений.УдалитьПротоколОбменаЕГАИС.ОбработатьДанныеДляПереходаНаНовуюВерсиюВходящиеСообщения", 1);
	...
	ОбработчикИОчередь.Вставить("Справочники.ЕГАИСПрисоединенныеФайлы.ОбработатьДанныеДляПереходаНаНовуюВерсию", 3);
	ОбработчикИОчередь.Вставить("РегистрыСведений.СоответствиеНоменклатурыЕГАИС.ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеСправок2", 1);
	ОбработчикИОчередь.Вставить("Справочники.ЭДПрисоединенныеФайлы.ОбработатьДанныеДляПереходаНаНовуюВерсию", 5);
	...
	ОбработчикИОчередь.Вставить("РегистрыСведений.СведенияОФайлах.ОбработатьДанныеДляПереходаНаНовуюВерсию", 1);
	ОбработчикИОчередь.Вставить("РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы", 1);
	ОбработчикИОчередь.Вставить("РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений", 4);
	...
	ОбработчикИОчередь.Вставить("Документы.ЧекЕГАИСВозврат.ОбработатьДанныеДляПереходаНаНовуюВерсию", 1);
