...
Процедура ДобавитьВТекстЗапросаРеквизитыДокумента(ТекстЗапроса, ИмяТаблицы = Неопределено) Экспорт
	...
	Если ИмяТаблицы = Неопределено Тогда
		...
		|	ЛОЖЬ КАК АмортизационнаяПремияУказанаВПроцентах,
...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты, ПараметрыРеквизитовОбъекта) Экспорт
	...
	ПроверитьРеквизитыУчета(Объект, ПараметрыРеквизитовОбъекта, МассивНепроверяемыхРеквизитов, Отказ);
...
Процедура ПриВыполненииКоманды(ИмяКоманды, Форма) Экспорт
	...
	Если ИмяКоманды = Элементы.ЗаполнитьИсточникиФинансирования.Имя Тогда
...
Процедура ЗаполнитьСлужебныеРеквизитыПоОрганизацииИДате(Форма) Экспорт

	//++ Локализация
	Форма.ОСНЮрЛицо = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Форма.Объект.Организация, Форма.Объект.Дата);
	//-- Локализация
...
Процедура УстановитьПолеНаправлениеДеятельности(ТекстЗапроса) Экспорт

	ПолеНаправлениеДеятельности = "&НаправлениеДеятельности";
	
	//++ Локализация
	
	ПолеНаправлениеДеятельности =
	"ВЫБОР
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НаправлениеДеятельности_Локализация", ПолеНаправлениеДеятельности);
...
Процедура ТекстЗапросаТаблицаПараметрыАмортизацииОСБУ(ТекстыЗапроса)
	...
	Документы.МодернизацияОС2_4.ТекстЗапросаВтСуммаМодернизации(ТекстыЗапроса);
	ВнеоборотныеАктивыСлужебный.ТекстЗапросаТаблицаВтСписокОС(ТекстыЗапроса, "Документ.МодернизацияОС2_4");
	ВнеоборотныеАктивыСлужебный.ТекстЗапросаВтТаблицаОС(ТекстыЗапроса, "Документ.МодернизацияОС2_4");
...
Процедура СтатьяРасходовАмортизационнойПремииПриИзменении(Форма, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.НомерЭтапа = 1 Тогда
		...
		ПараметрыДействия.Вставить("ИмяЭлемента", "СтатьяРасходовАмортизационнойПремии");
		
		ДополнительныеПараметрыДействия = Новый Структура("НомерЭтапа", 1);
		ПараметрыДействия.Вставить("ДополнительныеПараметры", ДополнительныеПараметрыДействия);
	...
	Иначе
		...
		Форма.АналитикаРасходовАмортизационнойПремииОбязательна = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
									Объект.СтатьяРасходовАмортизационнойПремии, 
...
Процедура СпособВводаАмортизационнойПремии_ВПроцентахПриИзменении(Форма, ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.НомерЭтапа = 1 Тогда
		...
		ПараметрыДействия = Новый Структура;
		ПараметрыДействия.Вставить("ИмяЭлемента", "СпособВводаАмортизационнойПремии_ВПроцентах");
...
Процедура ПроверитьРеквизитыУчета(Объект, ПараметрыРеквизитовОбъекта, МассивНепроверяемыхРеквизитов, Отказ)
	...
	Если ВнеоборотныеАктивыКлиентСервер.ЗначениеСвойстваРеквизитаОбъекта("СтатьяРасходовАмортизационнойПремии", "Видимость", ПараметрыРеквизитовОбъекта) Тогда
		ПланыВидовХарактеристик.СтатьиРасходов.ПроверитьЗаполнениеАналитик(
			Объект, "СтатьяРасходовАмортизационнойПремии, АналитикаРасходовАмортизационнойПремии", МассивНепроверяемыхРеквизитов, Отказ);
	КонецЕсли; 
...
Процедура ПроверитьВозможностьВводаАмортизационнойПремииВПроцентах(Объект, Отказ)
	...
	Если Объект.АмортизационнаяПремияУказанаВПроцентах
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект, "СпособВводаАмортизационнойПремии",, Отказ); 
	КонецЕсли;
