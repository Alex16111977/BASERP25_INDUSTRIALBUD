...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение значения нового реквизита ""Режим анкетирования"" у документов ""Анкета"" прошлых периодов.
		...
		|en = 'Filling in values of new attribute ""Questionnaire mode"" of the ""Questionnaire"" documents of previous periods.
		|Until the processing is completed, ""Questionnaire mode"" of these documents will be shown incorrectly.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение значения нового реквизита ""Способ отображения подсказки"" в справочнике ""Вопросы шаблона анкеты"".
		...
		|en = 'Filling out the ""Tooltip display method"" attribute of the ""Questionnaire template questions"" catalog.
		|The tooltip display method in this catalog will be shown incorrectly until processing is complete.'");
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ОчередьОтложеннойОбработки = 1;
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение значений нового реквизита ""Вид"" в вопросах для анкетирования.
		...
		|en = 'Filling in values of the new attribute ""Kind"" in the questionnaire questions.
		|Until the processing is complete, types of the questionnaire questions will be shown incorrectly.'");
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ОчередьОтложеннойОбработки = 1;
...
Процедура ПостроениеФормыЗаполненияПоРазделу(Форма,ТекущиеДанныеДеревоРазделов) Экспорт
	...
	Если ТекущиеДанныеДеревоРазделов.ТипСтроки = "Раздел" Тогда
		...
		Форма.Элементы.НадписьВступление.Заголовок  = НСтр("ru = 'Раздел';
	...
	Иначе
		...
		Вступление = ?(ПустаяСтрока(Форма.Вступление), НСтр("ru = 'Нажмите далее для заполнения анкеты.';
															|en = 'Click Next to fill in the questionnaire.'"), Форма.Вступление);
		Заключение = ?(ПустаяСтрока(Форма.Заключение), НСтр("ru = 'Спасибо за то что заполнили анкету.';
															|en = 'Thank you for filling out the questionnaire.'"), Форма.Заключение);
		...
		ИначеЕсли СтрокаТаблицы.ТипОтвета = Перечисления.ТипыОтветовНаВопрос.Число Тогда
			...
			Если СтрокаТаблицы.МинимальноеЗначение <> 0 ИЛИ СтрокаТаблицы.МаксимальноеЗначение <> 0 Тогда
				...
				ИначеЕсли СтрокаТаблицы.МинимальноеЗначение = 0 И СтрокаТаблицы.МаксимальноеЗначение <> 0 Тогда
					ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Допустим ввод значения до %1';
																									|en = 'You can enter value to %1'"), СтрокаТаблицы.МаксимальноеЗначение);
				Иначе
					ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Допустим ввод значения от %1';
