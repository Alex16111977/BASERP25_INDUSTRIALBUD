		...
		Если НастройкиЗагрузки.Свойство("ИмяФайлаНастроек", ИмяФайлаНастроек) И ЗначениеЗаполнено(ИмяФайлаНастроек) Тогда
			Если ОбщегоНазначенияПовтИсп.РежимРаботыПрограммы().ЭтоАдминистраторПрограммы = Истина Тогда
				...
				Попытка
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Используются обработчики из файла настроек загрузки ""%1"".';
																					|en = 'Используются обработчики из файла настроек загрузки ""%1"".'"), ИмяФайлаНастроек));
				Исключение
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не найден файл настроек загрузки ""%1"": %2.';
