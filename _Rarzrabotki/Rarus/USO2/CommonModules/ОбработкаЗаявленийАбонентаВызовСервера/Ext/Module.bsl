...
Функция ОтправленныеЗаявленияАбонентов(
		...
		ИзмененныеРеквизиты  = Неопределено) Экспорт
...
Функция ОбновитьРеквизитыЗаявления(Документ, РеквизитыДокумента) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обработка заявлений.Обновление статуса';
										|en = 'Обработка заявлений.Обновление статуса'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбменССерверомПолучитьОтвет(ДокументЗаявление, ЭтоВызовИзМастера)
	...
	Иначе
		...
		Если УзелDOM.Количество() > 0 Тогда
			...
			СтатусКомментарий = НСтр("ru = 'При получении ответа возникла ошибка: ';
									|en = 'При получении ответа возникла ошибка: '") + сообщениеОшибки;
		Иначе
			СтатусКомментарий = НСтр("ru = 'При получении ответа возникла ошибка: ';
									|en = 'При получении ответа возникла ошибка: '") + кодРезультата;
		КонецЕсли;
