			...
			Если ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
					           ...
					           |en = 'An error occurred in function ЕстьРоль of module УправлениеДоступом.
					           |Access kind ПравоИзменения
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
				           ...
				           |""ПравоЧтения"", ""ПравоИзменения"" if they are used.'"),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено описание возможных прав для таблицы ""%1""';
				|en = 'Description of possible rights for table ""%1"" is not found'"),
	...
	Если ОписаниеПрава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдено описание права ""%1"" для таблицы ""%2""';
				|en = 'Right description ""%1"" for table ""%2"" is not found'"),
...
Процедура ПриСозданииФормыЗначенияДоступа(Форма,
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ПриСозданииФормыЗначенияДоступа
		           ...
		           |en = 'An error occurred in the ПриСозданииФормыЗначенияДоступа procedure
		           |of the УправлениеДоступом common module.'");
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр СозданиеНового не указан, а автоматическое заполнение
				           ...
				           |%1';
				           |en = 'СозданиеНового parameter is not specified and automatic filling
				           |from the ""Объект.Ссылка"" form attribute is not available due to:
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр ТипЗначения не указан, а автоматическое заполнение
				           |из реквизита формы ""Объект.Ссылка"" недоступно по причине:
				           |%1';
				           |en = 'ТипЗначения parameter is not specified and automatic filling
				           |from the ""Объект.Ссылка"" form attribute is not available due to:
				           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Параметр Реквизит не указан, а автоматическое заполнение
				           ...
				           |from the ""Объект.ГруппаДоступа"" form attribute is not available due to:
				           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
			...
			Исключение
				...
				ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось получить значение реквизита формы ""%1"", указанного параметре Реквизит по причине:
					           |%2';
					           |en = 'Cannot receive an attribute value of form ""%1"" specified in parameter Attribute due to:
	   ...
	   И СозданиеНового Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для добавления требуются разрешенные ""%1"".';
				|en = 'Permitted ""%1"" are required for adding.'"),
...
Функция ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа(ТипЗначенийДоступа, ВозвращатьВсе = Ложь) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа
		           |общего модуля УправлениеДоступом.';
		           |en = 'An error occurred in the ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа procedure
		           |of the УправлениеДоступом common module.'");
	...
	Если ТипЗнч(Список) <> Тип("ДинамическийСписок") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
			           ...
			           |en = 'An error occurred when calling procedure НастроитьОтборыДинамическогоСписка
			           |of common module УправлениеДоступом. Value ""%1"" of the List parameter is not a dynamic list.'"), Строка(Список));
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ВызватьИсключение
			НСтр("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
			           ...
			           |en = 'An error occurred when calling НастроитьОтборыДинамическогоСписка procedure of the common УправлениеДоступом module.
			           |Main table of transferred dynamic list is not specified.'");
	КонецЕсли;
	...
	Если Не Список.ПроизвольныйЗапрос Тогда
		ВызватьИсключение
			НСтр("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
			           ...
			           |en = 'An error occurred when calling procedure НастроитьОтборыДинамическогоСписка of common module УправлениеДоступом.
			           |The ""Arbitrary query"" check box is not selected in the transferred dynamic list.'");
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Псевдоним) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
				           ...
				           |""%1"".';
				           |en = 'An error occurred when calling procedure НастроитьОтборыДинамическогоСписка of common module УправлениеДоступом.
				           |Cannot find an alias of the ""%1"" main table
				           |in the transferred dynamic list.'"), Список.ОсновнаяТаблица);
		КонецЕсли;
	...
	Если НЕ НаборыЗаполняются Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неверные параметры.
			           ...
			           |Object type ""%1""
			           |is not found in both event subscriptions
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 ""%2""
			           ...
			           |en = '%1 ""%2""
			           |generated an empty access value set.'"),
			ТипЗнч(СсылкаНаОбъект),
			Объект);
	КонецЕсли;
		...
		Если ПовторяющиесяНовыеИмена.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в параметрах процедуры ЗаменитьПраваВНастройкахПравОбъектов
				           ...
				           |%1.';
				           |en = 'An error occurred in parameters of procedure ЗаменитьПраваВНастройкахПравОбъектов
				           |of common module УправлениеДоступом.
				           |
	...
	Если Отключить И Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки =
			НСтр("ru = 'Некорректный вызов процедуры ОтключитьОбновлениеКлючейДоступа общего модуля УправлениеДоступом.
			           ...
			           |en = 'Invalid call of procedure ОтключитьОбновлениеКлючейДоступа of the УправлениеДоступом common module.
			           |Only rightful user or
			           |user in the privileged mode can disable the update of access keys.'");
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект ""%1"" сформировал набор значений доступа,
				           ...
				           |Указывать требуется только специальные виды доступа
				           |""ПравоЧтения"", ""ПравоИзменения"", если они используются.';
				           |en = 'Object ""%1"" has generated a set of access values
				           |that contains unknown access kind ""%2"" that is not required.
				           |
				           |Specify only special access kinds
				           |""ПравоЧтения"" and ""ПравоИзменения"" if they are used.'"),
				ТипЗнч(СсылкаНаОбъект),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
						ВызватьИсключение
							НСтр("ru = 'Профиль Администратор должен быть хотя бы у одного пользователя,
							           ...
							           |en = 'At least one user
							           |with logon rights must have the Administrator profile.'");
					КонецЕсли;
				КонецЕсли;
