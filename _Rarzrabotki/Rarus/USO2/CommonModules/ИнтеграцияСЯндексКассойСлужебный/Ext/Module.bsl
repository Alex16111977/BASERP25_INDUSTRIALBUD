...
Процедура ПроверитьОрганизациюНаСоответствиеТребованиямПриПодключенииКСервису(Знач Организация,
	...
	Если Не ЗначенияРеквизитов.Резидент Тогда // Нерезидентов не подключаем.
		ТекстСообщения = НСтр(
			...
			|en = 'Only resident companies of the Russian Federation can be connected to the Yandex.Checkout service'");
	...
	Иначе
		...
		Иначе
			...
			Если НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ЗначенияРеквизитов.ИНН, 
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ru = 'Ошибка проверки ИНН организации. %1';
									|en = 'An error occurred while checking company TIN. %1'"), ТекстСообщения),,
					"Организация",,Отказ);
			КонецЕсли;
		...
		Если СтрДлина(ЗначенияРеквизитов.ИНН) = 10 И Не ЗначениеЗаполнено(ЗначенияРеквизитов.КПП) Тогда 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите КПП организации';
																	|en = 'Specify company CRTR'"), , "Организация",,
				Отказ);
		...
		ИначеЕсли ЗначениеЗаполнено(ЗначенияРеквизитов.КПП) Тогда
			...
			Если НЕ РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(ЗначенияРеквизитов.КПП,
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ru = 'Ошибка проверки КПП организации. %1';
									|en = 'An error occurred while checking company CRTR. %1'"), ТекстСообщения),,
					"Организация",,Отказ);
...
Функция ВыполнитьМетодСервисаВФоне(Знач ИмяМетода, Знач ВходящиеПараметры, Знач ИдентификаторФормы) Экспорт
	...
	Если ИмяМетода = "ДанныеОснованияПлатежаВСервисе" Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение данных основания платежа в сервисе Яндекс.Касса.';
																|en = 'Receive data on reason for payment in Yandex.Checkout service.'");
	...
	ИначеЕсли ИмяМетода = "ПлатежнаяСсылка" Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение ссылки для оплаты через Яндекс.Кассу.';
																|en = 'Receive link to the page of payment via Yandex.Checkout.'");
	...
	ИначеЕсли ИмяМетода = "ПолучитьНастройки" Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение настроек по Яндекс.Кассе.';
																|en = 'Receive settings via Yandex.Checkout.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ru = 'Интеграция с Яндекс.Кассой: Заказы.';
					|en = 'Integration with Yandex.Checkout: Orders.'");
	...
	Описание = НСтр("ru = 'Интеграция с Яндекс.Кассой: Оплаты.';
					|en = 'Integration with Yandex.Checkout: Payment.'");
...
Функция СтрокаАутентификации(ДанныеАутентификации)
	...
	Возврат "Basic " + СтрокаBase64БезBOM(ДанныеАутентификации.Логин + ":" + ДанныеАутентификации.Пароль);
	...
	Если Результат.КодСостояния >= 400 Тогда
		...
		Если ТекстСообщения = Неопределено Тогда						
			ТекстСообщения = НСтр("ru = 'Ошибка выполнения команды сервиса (%1).';
									|en = 'Error running service command (%1).'");
		КонецЕсли;
...
Функция ТекстыСообщенийОбОшибках(ИдентификаторОшибки, ДополнительныеПараметры = Неопределено)
	...
	Если ИдентификаторОшибки = "ФормированиеСсылкиПроверкаТипаСсылки" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			НСтр("ru = 'Формирование платежной ссылки для Яндекс Кассы';
				|en = 'Generating a payment link for Yandex.Checkout'"),
			СтрШаблон(НСтр("ru = 'На основании %1 ссылка на страницу оплаты через Яндекс.Кассу формироваться не может';
							|en = 'Link to the page of payment via Yandex.Checkout cannot be generated on the basis of %1'"),
				...
				НСтр("ru = 'Ошибка при проверке типов оснований формирования платежа через Яндекс.Кассу';
					|en = 'An error occurred while checking basis types for payment generation via Yandex.Checkout.'")); 
		...
		Сообщение = СообщениеОбОшибке(
			...
			СтрШаблон(НСтр("ru = 'Не удалось определить реквизиты документа %1';
				...
				НСтр("ru = 'Не удалось сформировать соответствие общепринятых реквизитов к реквизитам документа, см. процедуру ИнтеграцияСЯндексКассойПереопределяемый.СоответствиеРеквизитовДокументовОснованийПлатежа';
					|en = 'Cannot generate the correspondence of common attributes to the document attributes, see the ИнтеграцияСЯндексКассойПереопределяемый.СоответствиеРеквизитовДокументовОснованийПлатежа procedure'")); 
	...
	ИначеЕсли ИдентификаторОшибки = "ФормированиеСсылкиПроверкаЗаполненияРеквизитов" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			СтрШаблон(НСтр("ru = 'Не заполнен реквизит %1, формирование ссылки на страницу оплаты через Яндекс.Кассу не возможно';
							|en = 'The %1 attribute is not filled in. Cannot generate a link to page of payment via Yandex.Checkout'"),
	...
	ИначеЕсли ИдентификаторОшибки = "ФормированиеСсылкиНеНайденаДействительнаяНастройка" Тогда
		Сообщение = СообщениеОбОшибке(
			...
			НСтр("ru = 'Не найдена действующая настройка Яндекс.Кассы для этой организации, формирование ссылки на страницу оплаты через Яндекс.Кассу не возможно';
				|en = 'Valid setting of Yandex.Checkout is not found for this company. Cannot generate the link to the page of payment via Yandex.Checkout'"),
	...
	ИначеЕсли ИдентификаторОшибки = "ФормированиеСсылкиВалютаДокументаНеРазрешена" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			СтрШаблон(НСтр("ru = 'Счет в валюте %1 не может быть выставлен для оплаты через Яндекс.Кассу.';
							|en = 'Tax invoice in the %1 currency cannot be issued for payment via Yandex.Checkout.'"),
	...
	ИначеЕсли ИдентификаторОшибки = "ФормированиеСсылкиСуммаСчетаПревышаетМаксимальноДопустимую" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			СтрШаблон(НСтр("ru = 'Максимально допустимая сумма счета для оплаты через Яндекс.Кассу без заключения договора с сервисом составляет %1 руб. 
							...
							|en = 'Maximum allowable amount of proforma invoice via Yandex.Checkout without concluding a contract with the service is %1 rub.
							|To remove the restriction, conclude a contract with the Yandex.Checkout service or divide the proforma invoice into several ones with smaller amounts.'"),
	...
	ИначеЕсли ИдентификаторОшибки = "НеНайденыОжидаемыеДанныеВТелеОтвета" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			НСтр("ru = 'При чтении данных полученных от сервиса произошла ошибка.';
				|en = 'An error occurred while reading data received from the service.'"),
			СтрШаблон(НСтр("ru = 'Не обнаружены данные типа %1';
							|en = 'Cannot find data of the %1 type'"), ДополнительныеПараметры.ИмяОжидаемогоТипа));
	...
	ИначеЕсли ИдентификаторОшибки = "НеУдалосьПреобразоватьТелоОтветВОбъектXDTO" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			СтрШаблон(НСтр("ru = 'Не удалось построить объектную модель данных ответа. 
							...
							|More information: %1'"), ДополнительныеПараметры.ИнформацияОбОшибке));
	...
	ИначеЕсли ИдентификаторОшибки = "ОшибкаВыполненияКомандыСервиса" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			ВидОперацииПоКоманде(ДополнительныеПараметры.ИмяКоманды),
			СтрШаблон(НСтр("ru = '%1';
							|en = '%1'"), ДополнительныеПараметры.ОписаниеОшибки),
			ДополнительныеПараметры.ПодробноеПредставлениеОшибки);
	...
	ИначеЕсли ИдентификаторОшибки = "ОшибкаИнициализацииСервиса" Тогда
		Сообщение = СообщениеОбОшибке(
			НСтр("ru = 'Взаимодействие с сервисом уведомлений по операциям Яндекс.Кассы';
				|en = 'Interaction with notification services of Yandex.Checkout transactions'"),
	...
	ИначеЕсли ИдентификаторОшибки = "НеВключенаФункциональнаяОпция" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			НСтр("ru = 'Не включена функциональная опция ""Интеграция с Яндекс.Кассой"", использование функций взаимодействия с сервисом Яндекс.Касса недоступно.';
				|en = 'The ""Integration with Yandex.Checkout"" functional option is disabled. Functions of interaction with Yandex.Checkout are not available.'"));
	ИначеЕсли ИдентификаторОшибки = "НеВключенаИПП" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			НСтр("ru = 'Интернет-поддержка пользователей не подключена, использование функций взаимодействия с сервисом Яндекс.Касса недоступно.';
				|en = 'Online user support is not connected. Functions of interaction with Yandex.Checkout are not available.'"));
	ИначеЕсли ИдентификаторОшибки = "НеНайденаДействительнаяНастройка" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			НСтр("ru = 'Не найдена действующая настройка Яндекс.Кассы, формирование ссылки на страницу оплаты через Яндекс.Кассу не возможно';
				|en = 'Active setting of Yandex.Checkout was not found. Cannot generate a link to the page of payment via Yandex.Checkout'"),
	...
	ИначеЕсли ИдентификаторОшибки = "НеНайденыДействующиеНастройкиСДоговором" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			ВидОперацииПоКоманде(ДополнительныеПараметры.ИмяКоманды),
			НСтр("ru = 'Не найдены действующие настройки Яндекс.Кассы для варианта использования сервиса ""С договором"".';
				|en = 'Valid Yandex.Checkout settings are not found for the service use ""With contract"".'"),
	...
	ИначеЕсли ИдентификаторОшибки = "НеНайденыДействующиеНастройкиБезДоговора" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			ВидОперацииПоКоманде(ДополнительныеПараметры.ИмяКоманды),
			НСтр("ru = 'Не найдены действующие настройки Яндекс.Кассы для варианта использования сервиса ""Без договора"".';
				|en = 'Valid Yandex.Checkout settings are not found for the service use ""Without contract"".'"),
	...
	ИначеЕсли ИдентификаторОшибки = "НеНайденыДействующиеНастройки" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			...
			НСтр("ru = 'Не найдены действующие настройки Яндекс.Кассы.';
				|en = 'Valid Yandex.Checkout settings were not found.'"),
	...
	Если Не БлокироватьСообщенияОбОшибках Тогда
		...
		Если ИмяКоманды = "ЗагрузитьОперацииПоЯндексКассе" Тогда 
			ТекстСообщения = НСтр("ru = 'Ошибка выполнения загрузки операций по Яндекс.Кассе';
									|en = 'An error occurred when importing Yandex.Checkout transactions'");
		ИначеЕсли ИмяКоманды = "ПолучитьНастройки" Тогда 
		...
		ВидОперации = СтрШаблон(НСтр("ru = 'Команда ""%1"".';
