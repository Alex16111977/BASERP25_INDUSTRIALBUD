...
Процедура ЗаполнитьУполномоченногоПоПрямымВыплатамФСС(ДокументОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.УдалитьФИОУполномоченного)
		...
		Если Найденные.Количество() > 0 Тогда
			...
			Текст = СтрШаблон(
				НСтр("ru = 'Для уполномоченного представителя ""%1"" нашли физлицо ""%2"" (всего нашли физлиц %3).';
					|en = '""%2"" individual was found for the ""%1"" authorized representative (total individuals found: %3).'"),
		...
		Иначе
			...
			Текст = СтрШаблон(НСтр("ru = 'Для уполномоченного представителя ""%1"" НЕ нашли физлицо.';
									|en = 'Individual was NOT found for the ""%1"" authorized representative.'"), ДокументОбъект.УдалитьФИОУполномоченного);
		КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение доли неполного рабочего времени в больничных листах.';
										|en = 'Filling in the percentage of part-time work in sick leave records.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Фиксация информации о способе выплаты пособия в документах ""Заявление сотрудника на выплату пособия"".';
										|en = 'Record information on allowance payment method in the Employee application for allowance payment document.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Конвертация кодов причин перерасчета в документах ""Заявление сотрудника на выплату пособия"" в формат версии 1.7.5.';
										|en = 'Conversion of recalculation reason codes in the Employee application for allowance payment documents to format 1.7.5.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Переключение способа выплаты документах ""Заявление сотрудника на выплату пособия"" на вариант ""На карту МИР"", появившийся в версии формата 1.7.5.';
										|en = 'Switch the payment method in the ""Employee application for allowance payment"" documents to the ""To MIR card"" option appeared in format 1.7.5.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение уполномоченного представителя в документах ""Заявление сотрудника на выплату пособия"".';
										|en = 'Fill in authorized representative in the ""Employee application for allowance payment"" documents.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение адресных кодов (ФИАС, КЛАДР) в документах ""Заявление сотрудника на выплату пособия"".';
										|en = 'Fill in the address codes (FIAS, ARCA) in the ""Employee application for allowance payment"" documents.'");
	...
	Если Не СтруктураАдреса.Свойство("Индекс") Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'В адресе ""%1"" отсутствует поле Индекс, возможно указан адрес за пределами РФ.';
				|en = 'The Index fields is missing in the ""%1"" address, the address might have been specified outside the RF.'"),
			Адрес);
	Иначе
		Попытка
...
Функция НаименованиеПодсистемы(КодЯзыка = Неопределено)
	Возврат НСтр("ru = 'Пособия социального страхования';
				|en = 'Social insurance allowances'", ?(КодЯзыка = Неопределено, ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(), КодЯзыка));
КонецФункции
