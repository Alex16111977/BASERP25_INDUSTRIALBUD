		...
		Для Каждого ПолеЕстественногоКлюча Из ПоляЕстественногоКлюча Цикл
			...
			Если ТипыПоля = Неопределено Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
                          ...
                          |object field was not found.'", Метаданные.ОсновнойЯзык.КодЯзыка),
					...
					Если ТипПоля = Тип("ХранилищеЗначения") Тогда
						...
						ВызватьИсключение СтрШаблон(
							НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2: использование
                                  ...
                                  |en = 'Field %1 cannot be used as a natural key field of object %2: value type ХранилищеЗначения cannot be used as a natural key field.
					...
					Иначе
						...
						ВызватьИсключение СтрШаблон(
							НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
                                  ...
                                  |en = 'Field %1 cannot be used as a natural key field of object %2:
                                  |object %3 can be used as a field type if it is not included in
                                  |the common data set through overridable procedure
...
Процедура ВызватьИсключениеПриНаличииВРазделенныхДанныхСсылокНаНеразделенныеБезПоддержкиСопоставленияСсылок(Знач ОбъектМетаданных, Знач ИмяПоля, Знач МетаданныеСсылки, Знач ПриВыгрузке)
	...
	ИначеЕсли ОбщегоНазначенияБТС.ЭтоНаборЗаписей(ОбъектМетаданных) Тогда
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'В качестве значения измерения, ресурса или реквизита %1 разделенного набора записей %2 используются ссылки на
                  |неразделенный объект %3';
                  |en = 'References toundivided object %3
                  |are used as a value of dimension, resource or attribute %1 of divided record set %2'", Метаданные.ОсновнойЯзык.КодЯзыка),
			ИмяПоля,
			ОбъектМетаданных.ПолноеИмя(),
			МетаданныеСсылки.ПолноеИмя());
	...
	ДополнениеОшибки = СтрШаблон(
		НСтр("ru = 'При этом неразделенный объект %1 не включен в состав типов общих данных,
              ...
              |ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке().';
              |en = 'With that, undivided object %1 does not belong to common data types for which
              |links can be mapped on export and import.
              |This situation is prohibited for if the data is imported to another infobase,
              |links to object %1 will be dead.
              |
              |To fix this situation, implement for object %1 an engine for determining
              |the fields that uniquely identify its natural key, and include object %1 in
              |common data types for which links can be mapped on export and import,
              |by specifying metadata object %1 in procedure
              |ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке().'", Метаданные.ОсновнойЯзык.КодЯзыка),
		МетаданныеСсылки.ПолноеИмя());
	...
	Если Не ПриВыгрузке Тогда
		...
		ДополнениеОшибки = ДополнениеОшибки + + Символы.ПС + СтрШаблон(
			НСтр("ru = 'Если корректное сопоставление ссылок на неразделенные данные в ИБ, из которой выгружены
                  ...
                  |en = 'If correct mapping of references to shared data in the infobase
                  |from which data and infobase are exported,
                  |and into which they are imported, is guaranteed by other tools, metadata object %1 in procedure
