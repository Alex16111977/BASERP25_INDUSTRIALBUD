...
#КонецЕсли
	...
	ЗаписатьПримечание(Ссылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Подключение, БезопасныйРежим = ""%1"".';
																	|en = 'Connection, БезопасныйРежим = ""%1"".'"), БезопасныйРежим));
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("ru = 'Настройки быстрого доступа к дополнительным отчетам и обработкам';
							|en = 'Settings of quick access to additional reports and data processors'");
	...
	Если ТипыЗаполнениеОбъекта.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗаполнениеОбъектов") Тогда
			...
			Команда.Представление = НСтр("ru = 'Дополнительные обработки заполнения...';
										|en = 'Additional data processors of filling in...'");
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент ""%1"" не подключен, потому что ""Дополнительные отчеты и обработки"" отключены в настройках программы.';
				|en = 'The ""%1"" item is connected, as ""Additional reports and data processors"" are disabled in program settings.'"),
			"'" + Строка(Ссылка) + "'");
		Возврат;
	КонецЕсли;
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'При подключении дополнительного отчета ""%1"" возникла ошибка:';
																			|en = 'An error occurred while connecting additional report ""%1"":'"), Строка(Ссылка))
	...
	Иначе
		...
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент %1 не является дополнительным отчетом';
				|en = 'Item %1 is not an additional report'"),
			"'"+ Строка(Ссылка) +"'");
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуру ""%1"" не передан отчет';
				|en = 'Report is not passed to the ""%1"" procedure'"),
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение НСтр("ru = '""Дополнительные отчеты и обработки"" отключена в настройках программы.';
								|en = '""Additional reports and data processors"" is disabled in program settings.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Объект ""%1"" не является дополнительным отчетом';
				|en = 'Object ""%1"" is not an additional report'"), Строка(Ссылка));
	КонецЕсли;
			...
			ИначеЕсли Не ЕстьПравоЧтения Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';
						|en = 'The ""%1"" <report is not available because you do not have read rights for additional reports and processors.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЭтоПоставляемаяОбработка(Отчет) Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" недоступен в модели сервиса.';
						|en = '<The ""%1"" report is not available in SaaS'"),
					СтрокаТаблицы.Представление);
			Иначе
				...
				Исключение
					...
					ТекстОшибки = """" + Объект.Наименование + """:"
						+ Символы.ПС + НСтр("ru = 'Не удалось определить режим совместимости разрешений по причине:';
											|en = 'Cannot define a compatibility mode of extensions due to:'")
						+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
						+ Символы.ПС
						+ Символы.ПС + НСтр("ru = 'Объект заблокирован в режиме совместимости с версией 2.1.3.';
											|en = 'Object was locked in the compatibility mode with version 2.1.3.'");
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Команда %1: Завершение.';
																	|en = 'Command %1: End.'"), ИдентификаторКоманды));
			...
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Неподдерживаемый вид объектов метаданных: ""1""';
						|en = 'Unsupported metadata object kind: ""1""'"), СтрокаТаблицы.Вид));
			ОтчетыИОбработки.Удалить(ОбратныйИндекс);
			Продолжить;
		КонецЕсли;
			...
			Если Результат.ИмяОбъектаЗанято Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя ""%1"" занято объектами ""%2""';
						|en = 'The ""%1"" name is used by ""%2"" objects '"),
					ИмяОбъекта,
				...
				ЗаписьЖурналаРегистрации(
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при удалении задания ""%1"":%2';
							|en = 'An error occurred while deleting the ""%1"" job:%2'"),
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если Не РегистрационныеДанные.БезопасныйРежим И Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Результат.ТекстОшибки = НСтр("ru = 'Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.';
									|en = 'To connect data processor run in the unsafe mode, administrative rights are required.'");
		Возврат Результат;
	КонецЕсли;
	...
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> ЭтоВнешнийОтчет Тогда
		Результат.ТекстОшибки = НСтр("ru = 'Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';
									|en = 'Data processor kind from external data processor data does not correspond to its extension.'");
		Возврат Результат;
	КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для команды ""%1"" не определен способ запуска.';
					|en = 'The launch method is not defined for the ""%1"" command.'"), КомандаОписание.Представление));
		КонецЕсли;
...
#КонецЕсли
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			...
			|en = 'Cannot enable additional report from file.
			|It might be not suitable for this application version.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Невозможно подключить дополнительную обработку из файла.
			|Возможно, она не подходит для этой версии программы.';
			|en = 'Cannot enable additional data processor from file.
			|It might be not suitable for this application version.'");
		КонецЕсли;
