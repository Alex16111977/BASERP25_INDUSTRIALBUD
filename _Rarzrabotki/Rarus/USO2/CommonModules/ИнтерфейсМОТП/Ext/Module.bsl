...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено) Экспорт
	...
	ДеревоУпаковок.Колонки.Добавить("ЗначениеШтрихкода", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200)));
	...
	Для Каждого ПакетКодовМаркировки Из ПакетыКодовМаркировки Цикл 
		...
		Для Каждого КодМаркировки Из ПакетКодовМаркировки Цикл
			...
			Если ШтрихкодированиеМОТП.ЭтоКодМаркировкиТабачнойПачки(КодМаркировки, GTIN) Тогда
				...
				СтрокаДерева.ЗначениеШтрихкода = КодМаркировки;
			...
			Иначе
				...
				Если Результат.ДанныеОбАгрегации <> Неопределено
					...
					GTIN = Неопределено;
					МРЦ  = Неопределено;
					ЭтоКодМаркировкиБлока = ШтрихкодированиеМОТП.ЭтоКодМаркировкиБлока(КодМаркировки, GTIN, МРЦ);
					
					ДанныеСтроки = Новый Структура;
					ДанныеСтроки.Вставить("GTIN",         GTIN);
					ДанныеСтроки.Вставить("МРЦ",          МРЦ);
					ДанныеСтроки.Вставить("ИННВладельца", ДанныеСтатуса.ИННВладельца);
					ДанныеСтроки.Вставить("Статус",       ДанныеСтатуса.Статус);
					ДанныеСтроки.Вставить("КоличествоБлоков", ?(ЭтоКодМаркировкиБлока, 1, 0));
				...
				Иначе
					...
					Иначе
						...
						GTIN = Неопределено;
						МРЦ  = Неопределено;
						Если ШтрихкодированиеМОТП.ЭтоКодМаркировкиБлока(КодМаркировки, GTIN, МРЦ) Тогда
							СтрокаДерева.GTIN             = GTIN;
...
Функция ЗапроситьСтатусыКодовМаркировки(КодыМаркировки, СтатусыКодовМаркировкиКеш = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения статусов кодов маркировки из сервиса ИС МОТП';
				|en = 'Ошибка получения статусов кодов маркировки из сервиса ИС МОТП'") + Символы.ПС +
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Данные статусах кодов маркировки не получены из ИС МОТП.';
						|en = 'Данные статусах кодов маркировки не получены из ИС МОТП.'"),
	...
	Иначе
		...
		ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Данные о статусах кодов маркировки не получены из ИС МОТП.
				     |Текст ошибки: %1';
				     |en = 'Данные о статусах кодов маркировки не получены из ИС МОТП.
				     |Текст ошибки: %1'"),
		...
		ЗаписьЖурналаРегистрации(
			...
			СтрШаблон(
				НСтр("ru = 'Ошибка получения вложенных кодов маркировки по коду упаковки %1 из сервиса ИС МОТП';
					|en = 'Ошибка получения вложенных кодов маркировки по коду упаковки %1 из сервиса ИС МОТП'"),
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Данные об агрегации кодов маркировки не получены из ИС МОТП.';
						|en = 'Данные об агрегации кодов маркировки не получены из ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения МРЦ для кода маркировки из сервиса ИС МОТП';
				|en = 'Ошибка получения МРЦ для кода маркировки из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если МассивДанныхОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'МРЦ для кода маркировки не получена из ИС МОТП.';
						|en = 'МРЦ для кода маркировки не получена из ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка запроса списка кодов маркировки, принадлежащих участнику из сервиса ИС МОТП';
				|en = 'Ошибка запроса списка кодов маркировки, принадлежащих участнику из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Иначе
			...
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Идентификатор запроса списка кодов маркировки, принадлежащих участнику не получен из ИС МОТП.
					     ...
					     |Текст ошибки: %2';
					     |en = 'Идентификатор запроса списка кодов маркировки, принадлежащих участнику не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2'"),
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения статуса участника по ИНН из сервиса ИС МОТП';
				|en = 'Ошибка получения статуса участника по ИНН из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					СтрШаблон(НСтр("ru = 'Статус участника с ИНН %1 не получен из ИС МОТП.';
									|en = 'Статус участника с ИНН %1 не получен из ИС МОТП.'"), ИНН),
					РезультатОтправкиЗапроса);
	...
	Иначе
		...
		ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Статус участника с ИНН %1 не получен из ИС МОТП.
				     |Текст ошибки: %2';
				     |en = 'Статус участника с ИНН %1 не получен из ИС МОТП.
				     |Текст ошибки: %2'"),
			ИНН,
			РезультатОтправкиЗапроса.ТекстОшибки);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения списка ИНН контрагентов организации из сервиса ИС МОТП';
				|en = 'Ошибка получения списка ИНН контрагентов организации из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					СтрШаблон(НСтр("ru = 'Список ИНН контрагентов организации %1 не получен из ИС МОТП.';
									|en = 'Список ИНН контрагентов организации %1 не получен из ИС МОТП.'"), Организация),
					РезультатОтправкиЗапроса);
	...
	Иначе
		...
		ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Список ИНН контрагентов организации %1 не получен из ИС МОТП.
				     |Текст ошибки: %2';
				     |en = 'Список ИНН контрагентов организации %1 не получен из ИС МОТП.
				     |Текст ошибки: %2'"),
			Организация,
			РезультатОтправкиЗапроса.ТекстОшибки);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения реестра продукции из сервиса ИС МОТП';
				|en = 'Ошибка получения реестра продукции из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Реестр продукции не получен из ИС МОТП.';
						|en = 'Реестр продукции не получен из ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения карточки продукции из сервиса ИС МОТП';
				|en = 'Ошибка получения карточки продукции из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					СтрШаблон(НСтр("ru = 'Карточка продукции с идентификатором %1 не получена из ИС МОТП.';
									|en = 'Карточка продукции с идентификатором %1 не получена из ИС МОТП.'"), Идентификатор),
					РезультатОтправкиЗапроса);
	...
	Иначе
		...
		ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Карточка продукции с идентификатором %1 не получена из ИС МОТП.
				     |Текст ошибки: %2';
				     |en = 'Карточка продукции с идентификатором %1 не получена из ИС МОТП.
				     |Текст ошибки: %2'"),
			Идентификатор,
			РезультатОтправкиЗапроса.ТекстОшибки);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения документа по идентификатору из ИС МОТП';
				|en = 'Ошибка получения документа по идентификатору из ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Иначе
			...
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Документ по идентификатору не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2';
					     |en = 'Документ по идентификатору не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2'"),
				РезультатОтправкиЗапроса.КодСостояния,
				РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка запроса списка документов участника из сервиса ИС МОТП';
				|en = 'Ошибка запроса списка документов участника из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Иначе
			...
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Идентификатор запроса списка документов участника не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2';
					     |en = 'Идентификатор запроса списка документов участника не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2'"),
				РезультатОтправкиЗапроса.КодСостояния,
				РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения статуса заказа на формирование списка кодов маркировки участника ИС МОТП';
				|en = 'Ошибка получения статуса заказа на формирование списка кодов маркировки участника ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Иначе
			...
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Статус заказа на формирование списка кодов маркировки участника не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2';
					     |en = 'Статус заказа на формирование списка кодов маркировки участника не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2'"),
				РезультатОтправкиЗапроса.КодСостояния,
				РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения результата заказа на формирование списка кодов маркировки участника ИС МОТП';
				|en = 'Ошибка получения результата заказа на формирование списка кодов маркировки участника ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Иначе
			...
			ВозвращаемоеЗначение.ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Результат заказа на формирование списка кодов маркировки участника не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2';
					     |en = 'Результат заказа на формирование списка кодов маркировки участника не получен из ИС МОТП.
					     |Код состояния HTTP: %1.
					     |Текст ошибки: %2'"),
				РезультатОтправкиЗапроса.КодСостояния,
				РезультатОтправкиЗапроса.ТекстВходящегоСообщенияJSON);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения цепочки движения кода маркировки из сервиса ИС МОТП';
				|en = 'Ошибка получения цепочки движения кода маркировки из сервиса ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Цепочка движений кода маркировки не получена из ИС МОТП.';
						|en = 'Цепочка движений кода маркировки не получена из ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка создания нового бизнес-заказ на эмиссию кодов маркировки СУЗ ИС МОТП';
				|en = 'Ошибка создания нового бизнес-заказ на эмиссию кодов маркировки СУЗ ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Новый бизнес-заказ на эмиссию кодов маркировки не создан.';
						|en = 'Новый бизнес-заказ на эмиссию кодов маркировки не создан.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения статуса пула кодов маркировки из СУЗ ИС МОТП';
				|en = 'Ошибка получения статуса пула кодов маркировки из СУЗ ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Статусы пула кодов маркировки не получены из ИС МОТП.';
						|en = 'Статусы пула кодов маркировки не получены из ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения кодов маркировки из бизнес-заказа СУЗ ИС МОТП';
				|en = 'Ошибка получения кодов маркировки из бизнес-заказа СУЗ ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Коды маркировки из бизнес-заказа СУЗ не получены из ИС МОТП.';
						|en = 'Коды маркировки из бизнес-заказа СУЗ не получены из ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка передачи отчета об использовании кодов маркировки СУЗ ИС МОТП';
				|en = 'Ошибка передачи отчета об использовании кодов маркировки СУЗ ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Отчет об использовании кодов маркировки не передан в СУЗ.';
						|en = 'Отчет об использовании кодов маркировки не передан в СУЗ.'"),
					РезультатОтправкиЗапроса);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'МОТП';
				|en = 'МОТП'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("ru = 'Ошибка получения отчета об использовании кодов маркировки из СУЗ ИС МОТП';
				|en = 'Ошибка получения отчета об использовании кодов маркировки из СУЗ ИС МОТП'") + Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если ДанныеОбработки = Неопределено Тогда
				...
				ВозвращаемоеЗначение.ТекстОшибки = ИнтерфейсМОТПСлужебный.ТекстОшибкиПоРезультатуОтправкиЗапроса(
					НСтр("ru = 'Отчет об использовании не получен из СУЗ ИС МОТП.';
						|en = 'Отчет об использовании не получен из СУЗ ИС МОТП.'"),
					РезультатОтправкиЗапроса);
	...
	Для Каждого КлючИЗначение Из КодыМаркировкиУпаковок Цикл
		...
		Если ТипЗнч(КлючИЗначение) <> Тип("Строка") Тогда // Логистическая упаковка или блок
			...
			Если Детализация = Неопределено
				...
				СтрокаДерева = ДеревоУпаковок.Строки.Добавить();
				СтрокаДерева.ЗначениеШтрихкода = КодМаркировкиУпаковки;
		...
		Иначе // Табачная пачка
			...
			Если Детализация = Неопределено
				...
				СтрокаДерева = ДеревоУпаковок.Строки.Добавить();
				СтрокаДерева.ЗначениеШтрихкода = КлючИЗначение;
				СтрокаДерева.Статус            = ДанныеВерхнегоУровня.Статус;
