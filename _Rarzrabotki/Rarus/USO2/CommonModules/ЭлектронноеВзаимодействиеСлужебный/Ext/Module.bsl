...
Процедура ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(ОписаниеСобытия, КодСобытия, УровеньВажности = Неопределено,
	...
	Если КодСобытия = "ЭлектронноеВзаимодействие" Тогда
		Подсистема = НСтр("ru = 'Общая подсистема';
							|en = 'Common subsystem'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "ОбменСБанками" Тогда
		Подсистема = НСтр("ru = 'Обмен с банками';
							|en = 'Bank exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "ОбменСКонтрагентами" Тогда
		Подсистема = НСтр("ru = 'Обмен с контрагентами';
							|en = 'Exchange with counterparties'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "ОбменССайтами" Тогда
		Подсистема = НСтр("ru = 'Обмен с сайтами';
							|en = 'Exchange with sites'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "РегламентныеЗадания" Тогда
		Подсистема = НСтр("ru = 'Регламентные задания';
							|en = 'Scheduled jobs'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "БизнесСеть" Тогда
		Подсистема = НСтр("ru = 'Бизнес-сеть';
							|en = 'Business network'",	ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "ТорговыеПредложения" Тогда
		Подсистема = НСтр("ru = 'Торговые предложения';
							|en = 'Selling propositions'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = "ИнтеграцияСЯндексКассой" Тогда
		Подсистема = НСтр("ru = 'Интеграция с Яндекс Кассой';
							|en = 'Integration with Yandex.Checkout'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Подсистема = НСтр("ru = 'Общая подсистема';
							|en = 'Common subsystem'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	...
	ИмяСобытия = СтрЗаменить(НСтр("ru = 'Электронное взаимодействие.%1';
									|en = 'Electronic interaction.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		"%1", Подсистема);
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип второго параметра Реквизиты: %1';
																							|en = 'Incorrect type of the second parameter Attributes: %1'"), Строка(ТипЗнч(Реквизиты)));
		КонецЕсли;
...
Функция РасшифровкаКодаСостоянияHTTP(КодСостояния)
	...
	СоответствиеКодов.Вставить(501, НСтр("ru = 'Сервер не поддерживает возможностей, необходимых для обработки запроса.';
										|en = 'The server is not capable of processing the query.'"));
	СоответствиеКодов.Вставить(502, НСтр("ru = 'Сервер, выступая в роли шлюза или прокси-сервера, получил недействительное ответное сообщение от вышестоящего сервера.';
										|en = 'The server received an invalid response from the upstream server while acting as a gateway or proxy server.'"));
	СоответствиеКодов.Вставить(503, НСтр("ru = 'Сервер временно не имеет возможности обрабатывать запросы по техническим причинам.';
										|en = 'The server cannot process requests at this moment due to technical reasons.'"));
...
Процедура ПроверитьВозможностьФиксацииЗамераВремени(ОписаниеЗамера, КоличествоДанных)
	...
	Если КоличествоДанных < 0 Тогда
		ТекстОшибки = НСтр("ru = 'Параметр ""КоличествоДанных"" должен быть положительным (текущее значение ""%1"").';
							|en = 'The ""КоличествоДанных"" parameter must be positive (current value is ""%1"").'",
	...
	ИначеЕсли ОписаниеЗамера.ЗамерЗавершен Тогда
		ТекстОшибки = НСтр("ru = 'Замер ключевой операции ""%1"" уже завершен. Повторное использование описания этого замера запрещено.';
							|en = 'The ""%1"" key operation is already measured. Cannot use this measurement description again.'",
		                   ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		Иначе
			...
			Если НовыйURL = Неопределено Тогда
				...
				СтруктураВозврата.СообщениеОбОшибке = НСтр("ru = 'Некорректное перенаправление, отсутствует HTTP-заголовок ответа ""Location"".';
															|en = 'Incorrect redirection, HTTP title of the ""Location"" response is missing.'");
				Возврат СтруктураВозврата;
			КонецЕсли;
			...
			Если ПустаяСтрока(НовыйURL) Тогда
				...
				СтруктураВозврата.СообщениеОбОшибке = НСтр("ru = 'Некорректное перенаправление, пустой HTTP-заголовок ответа ""Location"".';
															|en = 'Incorrect redirection, empty HTTP title of the ""Location"" response.'");
				Возврат СтруктураВозврата;
			КонецЕсли;
			...
			Если Перенаправления.Найти(НовыйURL) <> Неопределено Тогда
				...
				СтруктураВозврата.СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Циклическое перенаправление.
																	...
																	|An attempt to go to %1 was made before.'"), НовыйURL);
				Возврат СтруктураВозврата;
			КонецЕсли;
