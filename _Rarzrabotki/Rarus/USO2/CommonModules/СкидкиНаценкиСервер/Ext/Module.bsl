	...
	Если ИспользованиеСкидкиНаценки = Неопределено Тогда
		...
		Если ДополнительнаяКоманда = "НастроитьСкидки" Тогда
			ИнформацияОДействииСкидок = Новый ФорматированнаяСтрока(НСтр("ru = 'Настроить скидки (наценки)';
																		|en = 'Set up discounts (markups)'"),, ЦветаСтиля.ЦветГиперссылки,,"НастроитьСкидки");
		КонецЕсли;
		Возврат;
	КонецЕсли;
		...
		Если НЕ ИспользованиеСкидкиНаценки.ДействуетНаВсехСкладах Тогда
			...
			Если ИспользованиеСкидкиНаценки.ДействуетВТиповыхСоглашениях > 0 Тогда
				...
				ТекстНадписи = ТекстНадписи + НСтр("ru = 'участникам типовых соглашений';
													|en = 'participants of standard agreements'") + " (" + ИспользованиеСкидкиНаценки.ДействуетВТиповыхСоглашениях + ")";
				ТребуетсяЗапятая = Истина;
			...
			Если ИспользованиеСкидкиНаценки.ДействуетВИндивидуальныхСоглашениях > 0 Тогда
				...
				ТекстНадписи = ТекстНадписи + НСтр("ru = 'участникам индивидуальных соглашений';
													|en = 'participants of individual agreements'") + " (" + ИспользованиеСкидкиНаценки.ДействуетВИндивидуальныхСоглашениях + ")";
				ТребуетсяЗапятая = Истина;
	...
	Если ДополнительнаяКоманда <> Неопределено
		...
		Если ДополнительнаяКоманда = "Изменить" Тогда
			СтрокаИзменить = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить';
																|en = 'Change'"),, ЦветаСтиля.ЦветГиперссылки,,"Изменить");
		...
		ИначеЕсли ДополнительнаяКоманда = "ИзменитьОбщийСтатус" Тогда
			СтрокаИзменить = Новый ФорматированнаяСтрока(НСтр("ru = 'Изменить общий статус';
																|en = 'Change overall status'"),, ЦветаСтиля.ЦветГиперссылки,,"ИзменитьОбщийСтатус");
		Иначе
			СтрокаИзменить = "";
		КонецЕсли;
		...
		Если НастройкиКомпоновкиДанных.Свойство("Запрос", ЗапросНастройкиКомпоновкиДанных) Тогда
					 ...
					 Для каждого ПараметрЗапроса из ЗапросДляПроверкиПараметров.Параметры Цикл
						 Если Не ПараметрыЗапросаНастройкиКомпоновкиДанных.Свойство(ПараметрЗапроса.Ключ) Тогда
							 ...
							 ТекстИсключения = НСтр("ru = 'В реквизите ХранилищеНастроекКомпоновкиДанных коллекция параметров не соответствует параметрам текста запроса';
													|en = 'The parameter collection does not match parameters of the request text in the StorageOfDataLayoutSettings attribute'");
							 Прервать;
						 КонецЕсли;	 
					 КонецЦикла;	 
			...
			Иначе	
				...
				ТекстИсключения = НСтр("ru = 'В реквизите ХранилищеНастроекКомпоновкиДанных нет данных о запросе';
										|en = 'No data on the request in the StorageOfDataLayoutSettings attribute'");
			КонецЕсли;
		КонецЕсли;
