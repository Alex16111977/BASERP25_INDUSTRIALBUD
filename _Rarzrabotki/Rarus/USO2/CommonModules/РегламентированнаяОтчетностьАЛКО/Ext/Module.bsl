...
//
Функция ИмяДействующегоМакета(ИмяФормы, ОсновноеИмяМакета, 
	...
	МетаданныеОтчета = ОбъектОтчетаАЛКО(ИмяФормы, ОбъектЭтогоОтч).Метаданные();
	...
	Если ЗначениеЗаполнено(СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции) Тогда
		...
		Если ВставитьКодПродукции Тогда
			...
			КонтрагентВидДеятельности = "
				...
				|		ТОГДА ""Закупка, хранение и поставки алкогольной продукции""
				|	ИНАЧЕ ""Производство, хранение и поставки произведенной алкогольной продукции""
				|КОНЕЦ";
...
//
//
Процедура ПравильныеПараметрыВСтруктуре(СтруктураПараметров,
	...
	Для каждого КлючЗначение Из СтруктураПараметров Цикл
		...
		// Правильный тип у параметра. Нужно лишь проверить не пуст ли.
			Если НЕ ЗначениеЗаполнено(ЗначениеПараметра) Тогда
				...
				СтруктураПараметров[ИмяПараметра] = СтруктураНастроек.ДопустимыйТип[ИмяТипаЗначения];

				Если СтруктураПараметров[ИмяПараметра] = Неопределено Тогда
					СтруктураПараметров[ИмяПараметра] = ПустоеЗначениеПоТипу(ИмяТипаЗначения);
				ИначеЕсли ТипЗнч(СтруктураПараметров[ИмяПараметра]) = Тип("Строка")
						И СтруктураПараметров[ИмяПараметра] = "Неопределено" Тогда
					СтруктураПараметров[ИмяПараметра] = Неопределено;
				КонецЕсли;
...
Функция ПолучитьПустуюСтруктуруАдреса() Экспорт
	...
	ПустойАдресXML = 	"<КонтактнаяИнформация xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xs=""http://www.w3.org/2001/XMLSchema"" xmlns=""http://www.v8.1c.ru/ssl/contactinfo"" Представление="""">
						|  <Комментарий/>
...
Функция ПолучитьСтруктуруАдресаИзСтандартногоПредставленияИлиXML(знач Адрес) Экспорт
	...
	Если СтруктураАдресаСМассивами.Свойство("Здание") Тогда
		...
		Если СтруктураАдресаСМассивами.Здание.Количество() > 0 Тогда
			СтрокаДома = СтрокаДома
				+ ?(ЗначениеЗаполнено(СтруктураАдресаСМассивами.Здание.ТипЗдания), Нрег(Строка(СтруктураАдресаСМассивами.Здание.ТипЗдания))
	...
	Если СтруктураАдресаСМассивами.Свойство("Корпуса") Тогда
		...
		Для Каждого ЭлМассива Из МассивКорпусов Цикл
			...
			Если СокрЛП(ЭлМассива.ТипКорпуса) <> "Литера" Тогда
				...
				ТекКорпус = ?(ЗначениеЗаполнено(ЭлМассива.ТипКорпуса), Нрег(Строка(ЭлМассива.ТипКорпуса))
					+ " ", "") + Строка(ЭлМассива.Номер);
				СтрокаКорпуса = СтрокаКорпуса + ?(СтрокаКорпуса = "", "", ", ")
					+ ТекКорпус;
	...
	Если СтруктураАдресаСМассивами.Свойство("Помещения") Тогда
		...
		Для Каждого ЭлМассива Из МассивПомещений Цикл
			...
			ТекПомещение = ?(ЗначениеЗаполнено(ЭлМассива.ТипПомещения), Нрег(Строка(ЭлМассива.ТипПомещения))
				+ " ", "") + Строка(ЭлМассива.Номер);
			СтрокаКвартира = СтрокаКвартира + ?(СтрокаКвартира = "", "", ", ")
				+ ТекПомещение;
	...
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.Район) Тогда
		...
		Если СтруктураАдресаСМассивами.Свойство("РайонСокращение") Тогда
			...
			АдресВФорматеОтчета.Район = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.РайонСокращение), Строка(АдресВФорматеОтчета.Район), Строка(АдресВФорматеОтчета.Район)
				+ " " + Строка(СтруктураАдресаСМассивами.РайонСокращение));
	...
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.Город) Тогда
		...
		Если СтруктураАдресаСМассивами.Свойство("ГородСокращение") Тогда
			...
			АдресВФорматеОтчета.Город = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.ГородСокращение), Строка(АдресВФорматеОтчета.Город), Строка(АдресВФорматеОтчета.Город)
				+ " " + Строка(СтруктураАдресаСМассивами.ГородСокращение));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.НаселенныйПункт) Тогда
		...
		Если СтруктураАдресаСМассивами.Свойство("НаселенныйПунктСокращение") Тогда
			...
			АдресВФорматеОтчета.НаселенныйПункт = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.НаселенныйПунктСокращение), Строка(АдресВФорматеОтчета.НаселенныйПункт), Строка(АдресВФорматеОтчета.НаселенныйПункт)
				+ " " + Строка(СтруктураАдресаСМассивами.НаселенныйПунктСокращение));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.Улица) Тогда
		...
		Если СтруктураАдресаСМассивами.Свойство("УлицаСокращение") Тогда
			...
			АдресВФорматеОтчета.Улица = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.УлицаСокращение), Строка(АдресВФорматеОтчета.Улица), Строка(АдресВФорматеОтчета.Улица)
				+ " " + Строка(СтруктураАдресаСМассивами.УлицаСокращение));
		КонецЕсли;
...
Функция ПолучитьАдресОбъекта(знач Ссылка, ПредставлениеАдреса = Неопределено,
		...
		знач ДатаАдреса = Неопределено, ВидСтрокиАдреса = "JSON", НеЗаполненыКодыФИАС = Истина) Экспорт

	Если НЕ (ВидСтрокиАдреса = "JSON" ИЛИ ВидСтрокиАдреса = "XML") Тогда
		ВидСтрокиАдреса = "JSON";	
	КонецЕсли;
	
	Если ВидСтрокиАдреса = "JSON" Тогда
...
Функция ПолучитьАдресJSONОбъекта(знач Ссылка, ПредставлениеАдреса = Неопределено,
	...
	Если СтрокаJSON = Неопределено Тогда
		...
		Если НЕ СтрокаXML = Неопределено Тогда
			СтрокаJSON = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВJSON(СтрокаXML, ВидКонтактнойИнформации);			
		КонецЕсли;
...
Функция ВернаяНакладная(ДокументСсылка, ТаблицаСообщений,
	...
	Перем ИмяРаздела, НомерЛиста, СтрокаГде, СтрВКакомРазделе,
		...
		ДатаНачалаОтчета, ДатаКонцаОтчета, НижняяГраницаДатыПоXSD,
		ВерхняяГраницаДатыПоXSD, Обязательный, НомерГТДОбязателен, НомерТТНОбязателен;
	...
	Если НЕ (НомерТТНОбязателен ИЛИ НомерГТДОбязателен) Тогда
	...
	Если ЗначениеЗаполнено(НомерТТН) Тогда
		...
		Если СтрДлина(НомерТТН) > 50 Тогда

			ТекстОписанияОшибки = СтрокаГде + " в номере " + СтрТипНакладнойЧего
				+ " содержится более 50 символов!";
	...
	Если ЗначениеЗаполнено(НомерГТД) Тогда
		...
		Если СтрДлина(НомерГТД) > 50 Тогда
...
Функция ВерноеЗаполнениеОП(ДокументСсылкаАлкоОтчет, ТаблицаСообщений,
		...
		СведенияОбОП = Неопределено, РежимОтладки = Неопределено) Экспорт
	...
	ИначеЕсли НЕ (ПустаяСтрока(ДанныеОП)
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные[0].Значение) = Тип("Структура")
			...
			ПроверкаАдресаПройдена = ВерныйАдрес(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, 
												СтатусПроверки, СведенияОбОП, ВспомогательнаяСтруктураДанных, РежимОтладки);
...
Функция ВерноеЗаполнениеТитульный(ДокументСсылкаАлкоОтчет, ТаблицаСообщений,
		...
		СтруктураПараметров, РежимОтладки = Неопределено) Экспорт
	...
	ПроверкаАдресаПройдена = ВерныйАдрес(ДокументСсылкаАлкоОтчет, ТаблицаСообщений,  
										СтатусПроверки, ПараметрыВыгрузки.АдресОрганизации, ВспомогательнаяСтруктураДанных, РежимОтладки);
	...
	Если ПараметрыВыгрузки.Свойство("АдресМестаДеятельности") Тогда
		...
		ПроверкаАдресаПройдена = ВерныйАдрес(ДокументСсылкаАлкоОтчет, ТаблицаСообщений,  
											СтатусПроверки, ПараметрыВыгрузки.АдресМестаДеятельности, ВспомогательнаяСтруктураДанных, РежимОтладки);
...
Функция ВерныйКодВидаПродукции(ДокументСсылка, ТаблицаСообщений,
	...
	Перем ИмяПоказателя, ИмяРаздела, НомерЛиста, СписокКодов, СтрокаГде,
		СтрВКакомРазделе, НомерСтрокиСтроковый, ИмяТаблицы;
	...
	Если РежимОтладки Тогда
		...
		Если НЕ ТипЗнч(СтруктураИнформацииВидаПродукции) = Тип("Структура") Тогда
			ВызватьИсключение "В функцию РегламентированнаяОтчетностьАЛКО.ВерныйКодВидаПродукции() 
	...
	Если ТипЗнч(Код) = Тип("Строка") И ЗначениеЗаполнено(Код) Тогда
		...
		// Код вне допустимого списка. 
			// Ручной ввод - если не было недавнего изменения списка, это ошибка.
			// Предупреждаем, но даем выгрузить.
			ТекстОписанияОшибки = СтрокаГде + " указан код вида продукции " + """" + Код
				+ """" + ", который не предусмотрен текущей схемой выгрузки отчета!";
			...
			СтруктураОшибки.Вставить("ЛистРаздела", НомерЛиста);
			СтруктураОшибки.Вставить("ТипОшибки", "КодПродукцииНеИзСписка");
			...
			Если СтрДлина(Код) <> 3 Тогда
				...
				ТекстОписанияОшибки = СтрокаГде + " указан код вида продукции " + """"
					+ Код + """" + " неверной длины!
					|Код должен состоять из 3-х символов, обычно цифр.";
			...
			Если НЕ ВСтрокеВсеЦифры(Код) Тогда
				...
				ТекстОписанияОшибки = СтрокаГде + " указан код вида продукции " + """"
					...
					|Формально такая ситуация допустима, но на практике коды продукции всегда состоят из цифр.";
				...
				СтруктураОшибки.Вставить("Показатель", ИмяПоказателя);
				СтруктураОшибки.Вставить("ЛистРаздела", НомерЛиста);
				СтруктураОшибки.Вставить("ТипОшибки", "КодПродукцииНеЦифры");
	...
	Иначе
		...
		ТекстОписанияОшибки = СтрокаГде + " не задан код вида продукции!";
	...
	Если НЕ КонтрольОП Тогда
					...
					// Нужно убедиться что длина ИНН соответствует длине страны.
						...
						// Неверная длина ИНН.
							ТекстОписанияОшибки = СтрокаГде
								...
								|Длина Номера должна быть " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(ДолжнаБытьДлинаИНН)
								+ ", но длина Номера состовляет " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(ДлинаИНН) + ".";
		...
		// Неверная длина ИНН.
			ТекстОписанияОшибки = СтрокаГде
				...
				+ РодПадежЧейИННКПП + " " + ИНН + "!
				|Длина Номера согласно схемы XSD должна быть от " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(МинДлинаПоXSD)
				+ " до " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(МаксДлинаПоXSD) + ".
				|Но длина Номера состовляет " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(ДлинаИНН) + ".";
...
Функция СтрокаГдеСообщенийОбОшибках(СтрВКакомРазделе, НомерЛиста = Неопределено,
	...
	СтрНомерЛиста = ?(ТипЗнч(НомерЛиста) = Тип("Число"), РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(НомерЛиста), "");
...
//		Булево						Истина, если ошибки не найдены.
//
Функция ВерныйАдрес(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки,
		АдресСтруктура, ВспомогательнаяСтруктураДанных,
		РежимОтладки = Неопределено) Экспорт
	...
	Если АдресСтруктура.Свойство("АдресXML") Тогда
	// Перезаполняем адрес из XML строки.
		АдресСтруктураИзXML = ПолучитьСтруктуруАдресаИзСтандартногоПредставленияИлиXML(АдресСтруктура.АдресXML);
		// АдресСтруктура может содержать не только адрес. Поэтому не присваиваем, а заполняем свойства.
		ЗаполнитьЗначенияСвойств(АдресСтруктура, АдресСтруктураИзXML);
	КонецЕсли;
	...
	Если АдресСтруктура.Свойство("КодСтраны") Тогда
		...
		Если ЗначениеЗаполнено(АдресСтруктура.КодСтраны) Тогда
			...
			// Ошибка - код страны должен состоять из 3 символов.
				...
				СообщитьОбОшибкеЗаполнения(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, СтруктураОшибки, РежимОтладки,
					ВспомогательнаяСтруктураДанных.МаксимальноеКоличествоОшибок, ВспомогательнаяСтруктураДанных.ТекущееКоличествоОшибок);
	...
	Для каждого Поле Из АдресСтруктура Цикл

		Если (Поле.Ключ <> "КодСтраны") И (Поле.Ключ <> "Страна")
				И (Поле.Ключ <> "АдресXML") И (Поле.Ключ <> "УникальныйНомерФИАС")
				И (Поле.Ключ <> "ДополнительныеКоды")
				// Для случая структуры ОП.
				И (Поле.Ключ <> "Наименование") И (Поле.Ключ <> "КПП")
				И ЗначениеЗаполнено(Поле.Значение) Тогда

				// Для иностранного адреса будет заполнено поле ПредставлениеАдреса.
			// Для российского - любые поля помимо ПредставлениеАдреса.
			// КодРегиона для незаполненного адреса может быть "00".
			Если Поле.Ключ = "КодРегиона" И Поле.Значение = "00" Тогда			
				Продолжить;			
			КонецЕсли; 
	...
	Если АдресРоссийский Тогда
		...
		ОписаниеСтруктурыАдреса = ?(ТипЗначенияСтруктурыАдресаИзОтчета = Тип("Структура")
			ИЛИ ТипЗначенияСтруктурыАдресаИзОтчета = Тип("ФиксированнаяСтруктура"), ОписаниеСтруктурыАдресаИзОтчета, ОписаниеСтруктурыАдреса);
		...
		Для каждого КлючЗначение Из ОписаниеСтруктурыАдреса Цикл
			...
			Если АдресСтруктура.Свойство(ИмяПоляАдреса) Тогда
				...
				СтруктураОписанияПоля = КлючЗначение.Значение;
				...
				ИначеЕсли ДлинаПоляАдреса > СтруктураОписанияПоля.МаксДлина Тогда
					...
					АдресСтруктура[ИмяПоляАдреса] = СократитьПолеАдреса(АдресСтруктура[ИмяПоляАдреса], 
				...
				Если СтруктураОписанияПоля.ТолькоЦифры Тогда
					...
					Если НЕ ВСтрокеВсеЦифры(ЗначениеПоляАдреса) Тогда
...
Процедура ПоказатьСтраницуМногострочногоРазделаАЛКО(Форма, Шаг, Раздел,
	...
	Если Раздел <> "Лицензии" Тогда
		...
		СтрОбособленноеПодразделение = РегламентированнаяОтчетностьАЛКОКлиентСервер.СформироватьОбособленноеПодразделениеАЛКО(АктивнаяСтраницаРаздела.ДополнительныеДанные[0].Значение.СведенияОбОП, Форма.ЭтоПБОЮЛ, Форма.ОрганизацияИНН);
	...
	Если ЭтоВиноградная Тогда
		УстановитьВидимостьРазделовАЛКО(Форма, Раздел, Форма.НомерАктивнойСтраницыМногострочногоРаздела, "Сведения по месту 
...
Процедура УстановитьВидимостьРазделовАЛКО(Форма, НаименованиеРаздела,
		ИндексСтраницы = Неопределено, ЗаголовокОП_ИП = Неопределено,
		ЗаголовокОП_ЮЛ = Неопределено) Экспорт
...
Процедура ПриСозданииНаСервереАЛКО(Форма,
		НомерАлкоОтчета = Неопределено) Экспорт
			...
			Если ЗначениеЗаполнено(ЗаявлениеОКорректировке) Тогда
				...
				Если НЕ СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка = ЗаявлениеОКорректировке Тогда
					...
					Сообщение.Текст = НСтр("ru = 'Очищена ссылка на ""Отчет, корректировка сведений которого необходима""
						...
						|en = 'Очищена ссылка на ""Отчет, корректировка сведений которого необходима""
						|поскольку указанная в ссылке декларация уже используется в другом 
						|Заявлении о предоставлении корректирующих деклараций после установленного срока.'");
					Сообщение.Сообщить();
	...
	Если ТранзакцияАктивна() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений."
				+ ИмяРегистраСведений + "
				|- ошибка блокировки при добавлении записей из таблицы значений.'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПереходНаСтрокуТаблицыФормыАЛКО(Форма, ИдГруппы, НомерСтроки,
	...
	// Ситуация возможна только при ручном вводе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Номер строки оказался больше максимально возможного.
			...
			|en = 'Номер строки оказался больше максимально возможного.
			|будет произведена попытка перехода на последнюю строку'"));
	...
	// Никуда не перешли - текущая строка вне ограничений вывода.
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Строка № "
			+ НомерСтроки
...
Функция ЗаполнитьПоСтраницеРазделаАЛКО(ИдГруппы, ИндексСтраницы,
	...
	Если ИндексСтраницы >= 0 Тогда
		...
		СтрокаПервые = ?(НЕ КоличествоПервых = Неопределено, " ПЕРВЫЕ "
			+ РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(КоличествоПервых), "");
...
Процедура ВывестиНаПечатьТитульный(СтруктураПараметровСтраницы,
	...
	Если НЕ НомерПриложения = Неопределено Тогда
		...
		Если МассивВиноградных.Найти(НомерПриложения) <> Неопределено Тогда
...
Функция СформироватьСтруктуруПоказателейИСтруктуруПараметровАЛКО(Форма)
	...
	Если ТипЗнч(АдресОрганизации) = Тип("Структура") Тогда
		...
		ПараметрыВыгрузки.АдресОрганизации = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(АдресОрганизации);
	...
	Если Форма.ДополнительныеДанные.Титульный.Свойство("АдресМестаДеятельности") Тогда
		...
		Если ТипЗнч(АдресМестаДеятельности) = Тип("Структура") Тогда
			...
			ПараметрыВыгрузки.АдресМестаДеятельности = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(АдресМестаДеятельности);
...
Функция СформироватьИдФайлАЛКО(ПараметрыВыгрузки)
	...
	ИначеЕсли ПараметрыВыгрузки.НомерАлкоОтчета = 13 ИЛИ ПараметрыВыгрузки.НомерАлкоОтчета = 27 Тогда
		ПрефиксИмениФайлаВыгрузки = "V1";
	ИначеЕсли ПараметрыВыгрузки.НомерАлкоОтчета = 14 ИЛИ ПараметрыВыгрузки.НомерАлкоОтчета = 28 Тогда
		ПрефиксИмениФайлаВыгрузки = "V2";
	ИначеЕсли ПараметрыВыгрузки.НомерАлкоОтчета = 15 ИЛИ ПараметрыВыгрузки.НомерАлкоОтчета = 29 Тогда
	...
	ИначеЕсли ПараметрыВыгрузки.НомерАлкоОтчета > 18 Тогда
		// Декларации с 2019 года.
		НомерПриложения2019 = ПараметрыВыгрузки.НомерАлкоОтчета - 18;
		ПрефиксИмениФайлаВыгрузки = Формат(НомерПриложения2019, "ЧЦ=2; ЧН=; ЧГ=0; ЧВН="); // 01, 02,.. 10, 11.
	КонецЕсли;
...
Функция СтрокаТаблицыКонтактнойИнформацииОбъекта(знач Ссылка, 
	...
	Если ЕстьДата Тогда
		...
		ТаблицаКонтактнойИнформации = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивОбъектов, , , ДатаАдреса);
...
Функция СократитьПолеАдреса(Поле, ИмяПоля, МаксимальнаяДлинаПоля)

	СтруктураСокращаемыхСлов = Новый Структура;
	
	СтруктураСокращаемыхСлов.Вставить("Дом", 				Новый СписокЗначений);
	СтруктураСокращаемыхСлов.Вставить("Корпус", 			Новый СписокЗначений);
	СтруктураСокращаемыхСлов.Вставить("Квартира", 			Новый СписокЗначений);
	
	СтруктураСокращаемыхСлов.Дом.Добавить("домовладение", 	"вл");
	СтруктураСокращаемыхСлов.Дом.Добавить("владение", 		"вл");
	
	СтруктураСокращаемыхСлов.Корпус.Добавить("корпус",		"корп");
	СтруктураСокращаемыхСлов.Корпус.Добавить("строение",	"стр");
	СтруктураСокращаемыхСлов.Корпус.Добавить("сооружение",	"сооруж");
	СтруктураСокращаемыхСлов.Корпус.Добавить("участок",		"уч");
	
	СтруктураСокращаемыхСлов.Квартира.Добавить("квартира",	"кв");
	СтруктураСокращаемыхСлов.Квартира.Добавить("офис",		"оф");
	СтруктураСокращаемыхСлов.Квартира.Добавить("помещение",	"пом");
	СтруктураСокращаемыхСлов.Квартира.Добавить("комната",	"к");
	СтруктураСокращаемыхСлов.Квартира.Добавить("этаж",		"эт");
	
	Если СтрДлина(Поле) > МаксимальнаяДлинаПоля Тогда
	
		Если СтруктураСокращаемыхСлов.Свойство(ИмяПоля) Тогда
		
			Для каждого ЗначениеПредставление Из СтруктураСокращаемыхСлов[ИмяПоля] Цикл
			
				Поле = СтрЗаменить(Поле, ЗначениеПредставление.Значение, ЗначениеПредставление.Представление);
...
Функция ПолучитьПараметрыДлительнойОперацииАЛКО(Форма, ВидДлОперации,
	...
	ИначеЕсли ВидДлОперации = ВидыДлитОперацийРеглОтчетов.Выгрузка
		...
		// достаточно долгой - поэтому для данных операций порог срабатывания выполнения в фоне снижен до 100.
		// 
		...
		Иначе	
			...
			РежимДлительнойОперации = (ВсегоСтрокБезУдаленных > 100);		
		КонецЕсли; 
		Результат.Вставить("РежимДлительнойОперации", РежимДлительнойОперации);
...
Процедура СкопироватьРегистрыСведенийАЛКО(ДокументСсылкаАлкоОтчетИсточник,
	...
	Для Каждого ИмяРегистраСведений Из СписокИменРегистровСведений Цикл
		...
		СкопироватьРегистрСведенийАЛКО(ДокументСсылкаАлкоОтчетИсточник, ДокументСсылкаАлкоОтчетПриемник, СтруктураСведенийРегистраОтчета);
...
Процедура СкопироватьРегистрСведенийАЛКО(ДокументСсылкаАлкоОтчетИсточник,
	...
	Перем ИмяРегистра, СтруктураСервисРегистров, МаксИндексСтраницы,
		ВсегоСтрокКопируем, КолОтработанных, ВспомогательныйАдрес;
...
Процедура СкопироватьВыборкуРегистраСведенийАЛКО(ВыборкаИсточник,
		НаборЗаписейПриемника, СтруктураСведенийОтчета, РежимОтладки = Неопределено)

	Перем ДокументСсылкаАлкоОтчетИсточник, ДокументСсылкаАлкоОтчетПриемник,
		ВсегоСтрок, КолОтработанных, КолВБлокеЗаписей,
...
Процедура ИмпортАдресныхДанныхТаблицИзДополнительныхДанных(ДополнительныеДанныеСтраницы,
	...
	Для Каждого ЭлементДопДанных Из ДополнительныеДанныеСтраницы Цикл
		...
		// Это адресная информация.												
			...
			АдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(АдресМестаДеятельности, Представление, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации);
...
Процедура ВосстановитьРегистрыСведенийАЛКО(ДокументСсылкаАлкоОтчет,
...
Функция СформироватьПредставлениеОшибокАЛКО(ТаблСообщений, ДатаКонцаОтчета,
	...
	// Если есть ошибки разработки - их в первую очередь.
		...
		// Такая ошибка может быть только одна.
			Секция_ЗаголовокОшибки = МакетОформления.ПолучитьОбласть("Заголовок"
				+ ТипОшибки);
			ПредставлениеОшибок.Вывести(Секция_ЗаголовокОшибки);
			// Ничего не сворачиваем, выдаем сразу.
			Секция_ИсправлениеОшибки = МакетОформления.ПолучитьОбласть("Исправление"
				+ ТипОшибки);
		...
		Для каждого РазделОшибок Из СписокРазделов Цикл
			...
			Если ОшибкиРаздела.Количество() > 0 Тогда
				...
				Секция_ЗаголовокОшибки = МакетОформления.ПолучитьОбласть("ЗаголовокОшибкиРаздела");
...
Процедура ВыгрузитьОтчетАЛКО(ПараметрыДокумента, АдресХранилища,
	...
	Если (ТипЗнч(ИмяФайлаXML) = Тип("Строка"))
		...
		// ИмяФайлаXML имя файла.
			...
			// Запись события в журнал регистрации для системного администратора.
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Запись документа Выгрузка регламентированного отчета.';
												|en = 'Запись документа Выгрузка регламентированного отчета.'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		РегламентированнаяОтчетность.ОбработатьСобытие1СОтчетности(НСтр("ru = 'Регламентированный отчет. Выгрузка';
																		|en = 'Регламентированный отчет. Выгрузка'"), ДокументСсылкаАлкоОтчет);
...
Процедура ЗаполнитьСведенияОбОрганизацииАЛКО(Форма, ТолькоАдрес = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаXML) Тогда
		...
		СтрокаXML = ПолучитьАдресXMLОбъекта(РабочаяОрганизация, ПредставлениеАдреса, СписокВидовКонтактнойИнформации, Ложь, Форма.ДатаПодписи);
		...
		Если (НЕ ЗначениеЗаполнено(СтрокаXML)) и Форма.ЭтоПБОЮЛ Тогда
			...
			СтрокаXML = ПолучитьАдресXMLОбъекта(ОбъектОрганизации, ПредставлениеАдреса, СписокВидовКонтактнойИнформации, Истина, Форма.ДатаПодписи);
		...
		// Повторяем поиск контактной информации в режиме "если нет нужных видов контактной информации, любой".
			...
			СтрокаXML = ПолучитьАдресXMLОбъекта(РабочаяОрганизация, ПредставлениеАдреса, СписокВидовКонтактнойИнформации, Истина, Форма.ДатаПодписи);
		КонецЕсли;
...
Процедура ИнициализацияНаСервереАЛКО(Форма, БезОткрытияФормы = Ложь) Экспорт
	...
	// В противном случае уже сохранено и будет восстановлено из сохраненных данных.
		...
		Если ЕстьПереключательОП Тогда
			...
			СписокВыбораРадиоБаттона.Добавить(0, ?(Форма.ЭтоПБОЮЛ, "По месту нахождения ИП", "По месту нахождения головной организации"));

			ЗаголовокОП_ПБОЮЛ = ?(ЭтоВиноградная, "По месту нахождения виноградника", "По месту нахождения объекта торговли");
			СписокВыбораРадиоБаттона.Добавить(1, ?(Форма.ЭтоПБОЮЛ, ЗаголовокОП_ПБОЮЛ, "По месту нахождения ОП"));
		КонецЕсли;
								...
								Если ЭлементСтруктуры.Значение[ИндСтрокаТаблЗнач].ДополнительныеДанные.Свойство("СведенияОбОП", СведенияОбОП) Тогда
									...
									// ИНН более не используем.
										...
										Если ПустаяСтрока(СведенияОбОП.АдресXML) Тогда
											...
											Попытка
												АдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(СведенияОбОП, Представление, Перечисления.ТипыКонтактнойИнформации.Адрес);
											Исключение
												АдресXML = "";
											КонецПопытки;
...
Процедура СоздатьТаблицуФормыРазделаБезПодвалаАЛКО(Форма, ИдГруппы,
	...
	Поле.Подсказка = НСтр("ru = 'Помеченные крестиком не выгружаются, не печатаются и не учавствуют в итогах,
		...
		|en = 'Помеченные крестиком не выгружаются, не печатаются и не учавствуют в итогах,
		|изменяется двойным щелчком, клавишей DEL или действием ""Удалить"".'");
	...
	Если ТранзакцияАктивна() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений.СведенияРеглОтчетАлкоЖурнал - ошибка блокировки при очистке Журнала.';
											|en = 'РегистрСведений.СведенияРеглОтчетАлкоЖурнал - ошибка блокировки при очистке Журнала.'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ТранзакцияАктивна() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений.СведенияРеглОтчетАлкоЖурнал - ошибка блокировки при записи в Журнал.';
											|en = 'РегистрСведений.СведенияРеглОтчетАлкоЖурнал - ошибка блокировки при записи в Журнал.'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
