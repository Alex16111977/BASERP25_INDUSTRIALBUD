...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Комментарий = НСтр("ru = 'Выполняется распределение расчетов с клиентами по %ОкончаниеПериода%';
						|en = 'Allocating settlements with customers till %ОкончаниеПериода%'", КодЯзыка);
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Взаиморасчеты.Начало распределение расчетов';
			|en = 'Settlements. Payment allocation start'", КодЯзыка), УровеньИнформация, , ОкончаниеПериодаРасчета, Комментарий);
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Взаиморасчеты.Распределены расчеты с клиентами за период';
				|en = 'Mutual settlements.Settlements with customers are allocated for the period'", КодЯзыка), УровеньИнформация, , Формат(НачалоПериода, "ДФ='yyyy.MM'"));
	...
	Комментарий = НСтр("ru = 'Распределены расчеты с клиентами с %НачалоПериода% по %ОкончаниеПериода%';
						|en = 'Settlements with customers are allocated from %НачалоПериода% to %ОкончаниеПериода%'", КодЯзыка);
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Взаиморасчеты.Окончание распределения расчетов';
			|en = 'Settlements. Payment allocation end '", КодЯзыка), УровеньИнформация, , ОкончаниеПериодаРасчета, Комментарий);
КонецПроцедуры
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Комментарий = НСтр("ru = 'Выполняется распределение расчетов с поставщиками по %ОкончаниеПериода%';
						|en = 'Allocating settlements with suppliers till %ОкончаниеПериода%'", КодЯзыка);
	Комментарий = СтрЗаменить(Комментарий, "%ОкончаниеПериода%", Формат(ОкончаниеПериодаРасчета, ФорматПериода));
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Взаиморасчеты.Начало распределение расчетов';
			|en = 'Settlements. Payment allocation start'", КодЯзыка), УровеньИнформация, , ОкончаниеПериодаРасчета, Комментарий);
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Взаиморасчеты.Распределены расчеты с поставщиками за период';
				|en = 'Mutual settlements.Settlements with suppliers are allocated for the period'", КодЯзыка), УровеньИнформация, , Формат(НачалоПериода, "ДФ='yyyy.MM'"));
	...
	Если ЗначениеЗаполнено(ПараметрыРасчета) И ПараметрыРасчета.Свойство("ГраницаВзаиморасчетов") Тогда
		Если ЗначениеЗаполнено(ПараметрыРасчета.ГраницаВзаиморасчетов) Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Распределение расчетов выполнено до %ДатаАктуальности%. 
										...
										|After that, you will be prompted to regenerate the report. '");
...
Процедура РаспределитьРасчетыСПоставщикамиФоновымЗаданием(Период, АналитикиКРасчету, РасчетКВосстановлению, Ключ = "")
	...
	НаименованиеЗадания = НСтр("ru = 'Выполняется распределение расчетов с поставщиками';
								|en = 'Allocating settlements with suppliers'");
...
Процедура РаспределитьРасчетыСКлиентамиФоновымЗаданием(Период, АналитикиКРасчету, РасчетКВосстановлению, Ключ = "")
	...
	НаименованиеЗадания = НСтр("ru = 'Выполняется распределение расчетов с клиентами';
								|en = 'Allocating settlements with customers'");
	...
	Если ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ШаблонЗапретаДанных) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось запустить распределение взаиморасчетов за %1 из-за установленной даты запрета.';
					|en = 'Cannot start settlements allocation for %1 due to the set closing date. '"),
				Формат(Период,"ДФ='MMMM yyyy'"));
				
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Взаиморасчеты';
										|en = 'Mutual settlements'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось очистить регистр: %1 у документа: %2 по причине: %3';
					|en = 'Cannot clear register: %1 in document: %2 due to: %3'"),
				ИмяРегистра,
				Элемент.Ключ,
