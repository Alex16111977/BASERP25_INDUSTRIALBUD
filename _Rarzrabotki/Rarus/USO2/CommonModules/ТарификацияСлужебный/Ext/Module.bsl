...
Функция МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2() Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru = 'МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2';
													|en = 'МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2'", КодЯзыка());
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_ОбработчикОбновления() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru = 'Регистрация тарифицируемых услуг. Обработчик обновления';
												|en = 'Tariffed service registration. Update handler'", КодЯзыка());
	...
	Попытка
		...
		ТекстИнформации = НСтр("ru = 'Услуги успешно отправлены в Менеджер сервиса';
								|en = 'Services were successfully sent to the service manager'");
		ЗаписьЖурналаРегистрации(ИмяСобытия, 
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru = 'Регистрация тарифицируемых услуг. Регламентное задание';
												|en = 'Tariffed service registration. Scheduled job'", КодЯзыка());
...
Процедура ЗапроситьЛицензииУникальныхУслуг_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru = 'Запрос лицензий уникальных услуг. Регламентное задание';
												|en = 'Unique service license request. Scheduled job'", КодЯзыка());
...
Функция ИмяСобытияТарификации() Экспорт
	...
	Возврат НСтр("ru = 'Управление тарифами в модели сервиса.';
				|en = 'Manage tariffs in SaaS. '", КодЯзыка());
...
Процедура ПроверитьСтруктуруПоставщиковУслуг(ПоставщикиУслуг)
	...
	Для Каждого Поставщик Из ПоставщикиУслуг Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Имеется несколько поставщиков услуг с идентифкатором:';
									|en = 'There are several service suppliers with ID:'") + " " + Поставщик.Идентификатор;
		КонецЕсли;
	КонецЦикла;
