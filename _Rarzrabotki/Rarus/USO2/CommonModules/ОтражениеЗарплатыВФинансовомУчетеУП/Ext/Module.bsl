...
// Процедура формирования дополнительных движений для отражения списания резервов отпусков.
//
// Параметры:
//	ДополнительныеСвойства - Структура - структура с дополнительными свойствами документа, используемыми при проведении.
//	Движения - КоллекцияДвижений - коллекция движений документа.
//	Отказ - Булево - Признак отказа от проведения документа.
//
Процедура ОтразитьСписаниеРезерваОтпусков(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	Реквизиты = ДополнительныеСвойства.Реквизиты;
	Таблица = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаНачисленныеОтпускаЗаСчетРезерва;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОценочныеОбязательстваЗарплатаКадры") Тогда
		...
		Если НастройкиРезервовОтпусков.ФормироватьРезервОтпусковБУ Тогда
			МодульРезервОтпусков.СформироватьДвиженияВыплатаОтпусковЗаСчетРезерва(Движения, Отказ, Реквизиты.Организация, Реквизиты.ПериодРегистрации, Таблица);
			ДвиженияОценочныеОбязательстваПоСотрудникам = Движения.ОценочныеОбязательстваПоСотрудникам.Выгрузить();
			МодульРезервОтпусков.СформироватьДвиженияСписаниеРезерваОтпусков(Движения, Отказ, Реквизиты.Организация, Реквизиты.ПериодРегистрации, ДвиженияОценочныеОбязательстваПоСотрудникам);
		КонецЕсли;
...
Процедура СформироватьДвиженияПоДокументу(Движения, Отказ, Организация, ПериодРегистрации, ДанныеДляПроведения, СтрокаСписокТаблиц = "") Экспорт
	...
	Если ИспользоватьДокументОтражениеЗарплатыДляСписанияРезервовОтпусков(ПериодРегистрации) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Строка Из СтрокиКРаспределению Цикл
		...
		Для Каждого ВидВзноса Из ВидыВзносов Цикл
			...
			Если Строка.РаспределятьНУ Тогда
				МассивРаспределенияНУ = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Строка[ВидВзноса.Представление + "НУ"], МассивКоэффициентов);
			Иначе
				МассивРаспределенияНУ = МассивРаспределения;
			КонецЕсли;
			
			Если МассивРаспределения = Неопределено И МассивРаспределенияНУ = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Индекс = 0 По СтрокиБазы.Количество()-1 Цикл
				...
				Если МассивРаспределенияНУ = Неопределено Тогда
					РазницаНУ = МассивРаспределения[Индекс];
				ИначеЕсли Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.ЕжегодныйОтпускРезервы Тогда
					РазницаНУ = МассивРаспределенияНУ[Индекс];
				Иначе
					РазницаНУ = НоваяСтрока.СуммаРегл - МассивРаспределенияНУ[Индекс];
				КонецЕсли;
