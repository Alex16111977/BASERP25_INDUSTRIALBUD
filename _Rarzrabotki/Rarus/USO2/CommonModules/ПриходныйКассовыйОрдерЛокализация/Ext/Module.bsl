...
#Область ВозвратАвансаПогашениеДолга
	...
	|	Расчеты.КорКонтрагент КАК СубконтоКт1,
	|	Расчеты.КорДоговор КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
...
#Область ВозвратДС
	...
	|		КОГДА Расчеты.ОбъектРасчетов = Расчеты.Ссылка И Расчеты.ОбъектРасчетов <> РасчетыПоДокументам.ОбъектРасчетов ТОГДА РасчетыПоДокументам.ПредоплатаРегл ИНАЧЕ 0 КОНЕЦ, Расчеты.СуммаРегл) КАК Сумма,
	|	ЕСТЬNULL(ВЫБОР
	|		КОГДА Расчеты.ОбъектРасчетов = РасчетыПоДокументам.РасчетныйДокумент ТОГДА РасчетыПоДокументам.ДолгУпр
	|		КОГДА Расчеты.ОбъектРасчетов = Расчеты.Ссылка И Расчеты.ОбъектРасчетов <> РасчетыПоДокументам.ОбъектРасчетов ТОГДА РасчетыПоДокументам.ПредоплатаУпр ИНАЧЕ 0 КОНЕЦ, Расчеты.СуммаУпр) КАК СуммаУУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ДенежныеСредства) КАК ВидСчетаДт,
	|	Операция.Касса КАК АналитикаУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	...
	|		КОГДА Расчеты.ОбъектРасчетов = Расчеты.Ссылка И Расчеты.ОбъектРасчетов <> РасчетыПоДокументам.ОбъектРасчетов ТОГДА РасчетыПоДокументам.Предоплата ИНАЧЕ 0 КОНЕЦ, Расчеты.Сумма)) КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	...
	|		ЕСТЬNULL(Расчеты.Подразделение, Операция.Подразделение) ИНАЧЕ Операция.Подразделение КОНЕЦ КАК ПодразделениеКт,
	|	ВЫБОР
	|		КОГДА Расчеты.ОбъектРасчетов = РасчетыПоДокументам.РасчетныйДокумент ТОГДА
	|		Расчеты.НаправлениеДеятельности ИНАЧЕ Операция.НаправлениеДеятельности КОНЕЦ КАК НаправлениеДеятельностиКт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	...
	|	ЕСТЬNULL(ВЫБОР
	|		КОГДА Расчеты.ОбъектРасчетов = РасчетыПоДокументам.РасчетныйДокумент ТОГДА РасчетыПоДокументам.Долг
	|		КОГДА Расчеты.ОбъектРасчетов = Расчеты.Ссылка И Расчеты.ОбъектРасчетов <> РасчетыПоДокументам.ОбъектРасчетов ТОГДА РасчетыПоДокументам.Предоплата ИНАЧЕ 0 КОНЕЦ, Расчеты.Сумма) КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	...
	|	ПО
	|		Расчеты.Ссылка = Операция.Ссылка
	|		И Расчеты.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтДругойОрганизации))
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РасчетыСПоставщикамиПоДокументам КАК РасчетыПоДокументам
	|	ПО
	...
	|	И ЕСТЬNULL(ВЫБОР
	|		КОГДА Расчеты.ОбъектРасчетов = РасчетыПоДокументам.РасчетныйДокумент ТОГДА РасчетыПоДокументам.ДолгРегл
	|		КОГДА Расчеты.ОбъектРасчетов = Расчеты.Ссылка И Расчеты.ОбъектРасчетов <> РасчетыПоДокументам.ОбъектРасчетов ТОГДА РасчетыПоДокументам.ПредоплатаРегл ИНАЧЕ 0 КОНЕЦ, Расчеты.СуммаРегл)
	|		+ ЕСТЬNULL(ВЫБОР
	|		КОГДА Расчеты.ОбъектРасчетов = РасчетыПоДокументам.РасчетныйДокумент ТОГДА РасчетыПоДокументам.ДолгУпр
