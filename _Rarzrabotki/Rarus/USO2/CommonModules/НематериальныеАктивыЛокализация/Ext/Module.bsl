...
Функция ТекстЗапросаФормыСписка() Экспорт
	...
	Иначе
		...
		|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаПринятияКУчетуУпр,
		|	ВЫБОР
		|		КОГДА НаличиеФайлов.ЕстьФайлы ЕСТЬ NULL ТОГДА 0
		|		КОГДА НаличиеФайлов.ЕстьФайлы ТОГДА 1
		|		ИНАЧЕ 0
...
Функция ОписаниеЗапросаДляВыбора(Параметры, УстановитьВсеОтборы = Ложь) Экспорт
	...
	Если (ДоступныеКонтексты.Свойство("БУ") ИЛИ ДоступныеКонтексты.Свойство("УУ"))
		...
		ПолеДатаПринятияКУчетуРегл = "ЕСТЬNULL(ПервоначальныеСведенияНМА.ДатаПринятияКУчетуБУ, ДАТАВРЕМЯ(1, 1, 1))";
...
Функция ПечатьНМА1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаОбъектов.СрокПолезногоИспользования) Тогда
			ПараметрыШапки.СрокПолезногоИспользования = ВнеоборотныеАктивыКлиентСервер.РасшифровкаСрокаПолезногоИспользования(
