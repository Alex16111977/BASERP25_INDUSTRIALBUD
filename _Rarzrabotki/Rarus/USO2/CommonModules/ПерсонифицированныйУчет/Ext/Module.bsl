		...
		Пока Выборка.Следующий() Цикл
			...
			Если ТипЗнч(ЗапрашиваемыеДанные.Ссылка) = Тип("СправочникСсылка.КомплектыОтчетностиПерсучета") Тогда
				...
				ЗапрашиваемыеДанные.Наименование = НСтр("ru = 'Комплект форм квартальной отчетности в ПФР';
														|en = 'Set of quarterly report forms for the PF'");
			КонецЕсли;	
		КонецЦикла;	
	КонецЕсли;	
...
Процедура ОпределитьТаблицуОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания) Экспорт
	...
	ОписаниеДокументаОтчетности.Наименование = НСтр("ru = 'Комплект квартальной отчетности в ПФР (до 2014 г.)';
													|en = 'Set of quarterly reports for PF (before 2014)'");
...
Процедура СоздатьВТДанныеСтажаПФР2014(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ПараметрыПолученияДанных, ПараметрыОтбора)
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'Не указан источник данных о стаже.';
								|en = 'Source of length of service data is not specified.'");
...
Процедура ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаЗаписейСтажа, Отказ, РасширеннаяПроверка = Истина, Поле = "") Экспорт
	...
	Иначе 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1, записи о стаже: %2';
																						|en = 'Employee %1, entries of length of service: %2 '"), ВыборкаЗаписейСтажа.СотрудникНаименование, ТекстОшибки);
	КонецЕсли;	
...
Процедура ДокументыСЗВПроверитьДополнительнойЗаписиСтажа(Ошибки, Ссылка, Запись, ОсновнаяЗапись, НомерСтрокиСотрудник, НомерСтроки, Отказ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ОсобыеУсловияТруда) 
		...
		ТекстОшибки = НСтр("ru = 'дополнительная запись может вводиться только при заполнении хотя бы одного из реквизитов основной записи: Особые условия труда , Выслуга лет, Территориальные условия, Основание исчисляемого стажа.';
							|en = 'additional entry can be entered only when populating at least one attribute of the main entry: Special working conditions, Seniority, Site conditions, Estimated length of service basis.    '");  
	...
	Если (НЕ УказаныТерриториальныеУсловия) И (НЕ УказаныОсобыеУсловияТруда) И (НЕ УказаноОснованиеВыслугиЛет) Тогда
		ТекстОшибки =НСтр("ru = 'в дополнительной записи должна быть задана либо территориальная льгота, либо особые условия труда, либо выслуга лет.';
							|en = 'territorial benefit, or special working conditions, or seniority should be specified in the additional entry.'");     
		ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, Запись, Отказ, Истина);
	КонецЕсли;	 
	...
	Если УказаныТерриториальныеУсловия Тогда
		Если  НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ТерриториальныеУсловия) Тогда
			ТекстОшибки = НСтр("ru = 'в дополнительной записи территориальная льгота может быть указана только при указании территориальной льготы в основной записи.';
								|en = 'territorial benefit can be specified in an additional entry only if the territorial benefit is specified in the main entry.'");     
	...
	Если УказаныОсобыеУсловияТруда Тогда
		...
		Если (УсловияТрудаЗаписи <> Справочники.ОсобыеУсловияТрудаПФР.Ст27_1)
			...
			ТекстОшибки = НСтр("ru = 'в дополнительной записи код условий труда должен принимать значение 27-1, 27-2 или 27-7.';
								|en = 'code of working conditions should be 27-1, 27-2 or 27-7 in an additional entry.'");     
		...
		Если (УсловияТрудаЗаписи = Справочники.ОсобыеУсловияТрудаПФР.Ст27_7) 
			...
			ТекстОшибки = НСтр("ru = 'в дополнительной записи код условий труда 27-7 может быть указан только в случае указания в основной записи кода исчисляемого стажа УИК104.';
								|en = 'working condition code 27-7 can be specified in an additional entry only if estimated length of service code UIC104 is specified in the main entry.'");     
			ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, Запись, Отказ, Истина, "ОсобыеУсловияТруда");
		КонецЕсли;	 
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ОснованиеВыслугиЛет) Тогда
		Если СтрНайти(",,27-СМ,27-ГД,27-СМХР,27-ГДХР,",","+КодВыслугиЛетЗаписи+",") = 0 Тогда
			ТекстОшибки = НСтр("ru = 'если в основной записи не указан код выслуги лет, то в дополнительной записи основание выслуги лет должно быть пустым или указаны коды 28-СМ,28-ГД,28-СМХР или 28-ГДХР.';
								|en = 'if the seniority code is not specified in the main entry, leave the seniority basis blank or enter codes 28-SM,28-GD,28-SMHR or 28-GDHR in the additional entry.'");     
	...
	Иначе//В основной записи код выслуги указан
		...
		Если ПроверочнаяСтрокаКодов = "" Тогда
			ШаблонТекстаОшибки = НСтр("ru = 'при указании в основной записи кода выслуги лет %3 в дополнительной записи код выслуги указываться не может.';
										|en = 'when specifying seniority code %3 in the main entry, you cannot specify it in the additional entry.'");  
 		...
 		Иначе
			...
			Если НЕ СтрНайти(ПроверочнаяСтрокаКодов,","+КодВыслугиЛетЗаписи+",") Тогда
				ШаблонТекстаОшибки = НСтр("ru = 'при указании в основной записи кода выслуги лет %1 в дополнительной записи могут быть указаны только следующие коды выслуги: %2.';
											|en = 'when specifying seniority code %1 in the main entry, the additional entry can have only the following codes: %2.'");
			...
			Пока Выборка.СледующийПоЗначениюПоля("НомерДополнительнойЗаписи") Цикл
				...
				Если Не ЗначениеЗаполнено(Выборка.НомерПравила)
					...
					ШаблонСообщения = НСтр("ru = 'комбинация значений реквизитов: Основание выслуги лет = %1; Основание исчисляемого стажа = %2; Особые условия труда = %3; Код позиции списка = %4 - не является допустимой.';
											|en = 'combination of the following attribute values: Seniority basis = %1, Estimated length of service basis = %2, Special working conditions = %3, List position code = %4 - is invalid. '");
				...
				Пока Выборка.Следующий() Цикл
					...
					Для Сч = 1 По 2 Цикл
						...
						Если (ФорматПараметра = "_") Тогда
							Если (ЗначениеПараметра <> 0) Тогда//ПУСТОЕ
								СтрокаОшибкиФормата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %1 должен быть пустым (=0).';
																													|en = 'parameter of the estimated length of service No. %1 must be left blank (=0).'"), ТекстПараметра); 
						...
						ИначеЕсли (ФорматПараметра = "Г") Тогда
							Если (ЗначениеПараметра > 100) Тогда//ГОДЫ
								СтрокаОшибкиФормата =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %1 должен содержать значение не > 100 (лет).';
																													|en = 'parameter of the estimated length of service No. %1 must contain a value of less than 100 (years).'"), ТекстПараметра); 
								ОшибкаФорматаСтажа = Новый Структура("Текст, Поле", СтрокаОшибкиФормата, ИмяПоляПараметр);
								МассивСообщений.Добавить(ОшибкаФорматаСтажа);	
							Иначе	
								ПараметрПрошелПроверку = ИСТИНА;
						...
						ИначеЕсли (ФорматПараметра = "М") Тогда 
							Если (ЗначениеПараметра > 11) Тогда//МЕСЯЦЫ
								СтрокаОшибкиФормата =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %1  должен содержать значение не > 11 (месяцев).';
																													|en = 'parameter of the estimated length of service No. %1 must contain a value of less than 11 (months).'"), ТекстПараметра);
								ОшибкаФорматаСтажа = Новый Структура("Текст, Поле", СтрокаОшибкиФормата, ИмяПоляПараметр);
								МассивСообщений.Добавить(ОшибкаФорматаСтажа);	
							Иначе	
								ПараметрПрошелПроверку = ИСТИНА;
						...
						ИначеЕсли (ФорматПараметра = "Д") Тогда 
							Если (ЗначениеПараметра > 30) Тогда//ДНИ
								СтрокаОшибкиФормата =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %1 должен содержать значение не > 30 (дней).';
																													|en = 'parameter of the estimated length of service No. %1 must contain a value of less than 30 (days).'"), ТекстПараметра);
								ОшибкаФорматаСтажа = Новый Структура("Текст, Поле", СтрокаОшибкиФормата, ИмяПоляПараметр);
								МассивСообщений.Добавить(ОшибкаФорматаСтажа);	
							Иначе	
								ПараметрПрошелПроверку = ИСТИНА;
						...
						ИначеЕсли (ФорматПараметра = "м") Тогда 
							Если (ЗначениеПараметра > 59) Тогда//МИНУТЫ
								СтрокаОшибкиФормата =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'параметр исчисляемого стажа № %1 должен содержать значение не > 59 (минут).';
																													|en = 'parameter of the estimated length of service No. %1 must contain a value of less than 59 (minutes).'"), ТекстПараметра);
								ОшибкаФорматаСтажа = Новый Структура("Текст, Поле", СтрокаОшибкиФормата, ИмяПоляПараметр);
								МассивСообщений.Добавить(ОшибкаФорматаСтажа);	
							Иначе	
								ПараметрПрошелПроверку = ИСТИНА;
					...
					Если (СокрЛП(ФорматФОВ) <> "___")и(СтрокаЗначенияПараметров="000") Тогда//
						...
						СтрокаОшибкиФормата =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в параметрах (Годы,Месяцы,Дни) должно быть указано отработанное время.';
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение реквизита «Идентификатор cтроки cотрудника» документа «Сведения о заработке и стаже застрахованных лиц, СЗВ-ИСХ».';
									|en = 'Populate the ""Employee line ID"" attribute of the ""Information on length of service and earnings of insured persons, SZV-ISH"" document.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистра «Записи о стаже сведения о страховом стаже застрахованных лиц СЗВ-СТАЖ».';
									|en = 'Populate the Entries on length of service information on pensionable service of insured persons, SZV STAZH register.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Уточнение формата страхового номера ПФР.';
									|en = 'PF insurance number format clarification.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Уточнение формата регистрационного номера ПФР.';
									|en = 'PF registration number format clarification.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение реквизитов документов СЗВ-СТАЖ, СЗВ-ИСХ и ОДВ-1.';
									|en = 'Fill in attributes of the SZV-STAZH, SZC-ISH and ODV-1 documents.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.9.157";
	...
	Если ЗаПределамиРФ Тогда
		Если АдресСписком.Страна = "РОССИЯ" Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'сохранен как неструктурированный адрес.';
										|en = 'saved as unstructured address.'");
		Иначе
			// Иностранный адрес
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		ИначеЕсли Не ПроверитьРегистрационныйНомерПФР(ДанныеОрганизации.РегистрационныйНомерПФР) Тогда
			ТекстСообщения = НСтр("ru = 'У организации неверно заполнен регистрационный номер ПФР.';
									|en = 'PF registration number is filled in incorrectly for the company.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Организация, "РегистрационныйНомерПФР", , Отказ);
		КонецЕсли;
		...
		Если ДанныеОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
			Если Не ЗначениеЗаполнено(ДанныеОрганизации.РегистрацияВНалоговомОргане) Тогда
				ШаблонСообщения = НСтр("ru = 'Для организации не указана регистрация в ИФНС, действующая на %1';
										|en = 'Registration with IFTS valid on %1 is not specified for the company'");
			...
			ИначеЕсли Не ЗначениеЗаполнено(ДанныеОрганизации.КПП) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У организации не заполнен код по КПП';
																		|en = 'CRTR code is not populated for the company'"), Организация, "КПП", ,Отказ);
			ИначеЕсли Не РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(ДанныеОрганизации.КПП, "") Или СтрЗаменить(ДанныеОрганизации.КПП, "0", "") = "" Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У организации неверно указан код по КПП';
																		|en = 'CRTR code is incorrectly specified for the company'"), Организация, "КПП", ,Отказ);
			КонецЕсли;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеОрганизации.ИНН) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У организации не заполнен ИНН';
																	|en = 'Company TIN is required'"), Организация, "ИНН", ,Отказ);
		ИначеЕсли Не РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ДанныеОрганизации.ИНН, ЗарплатаКадры.ЭтоЮридическоеЛицо(Организация), "") Или СтрЗаменить(ДанныеОрганизации.ИНН, "0", "") = "" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У организации неверно указан ИНН';
																	|en = 'TIN is incorrectly specified for the company'"), Организация, "ИНН", ,Отказ);
		КонецЕсли;
	КонецЕсли;	
...
Функция ПолучитьПредставленияПроверяемыхРеквизитов() Экспорт
	...
	ПредставленияПроверяемыхРеквизитов.Вставить("МестоРожденияВСвидетельстве",   НСтр("ru = 'не указано место рождения в свидетельстве';
																						|en = 'birthplace is not specified in the certificate'"));	
	...
	ПредставленияПроверяемыхРеквизитов.Вставить("СерияДокумента", НСтр("ru = 'не заполнена серия документа';
																		|en = 'document series is required'"));
	...
	ПредставленияПроверяемыхРеквизитов.Вставить("КемВыдан", НСтр("ru = 'не указано кем выдан документ';
																|en = 'issuing authority of the document is not specified'"));
...
Процедура ПроверитьСоответствиеИзменившихсяДанныхДаннымСвидетельства(Ссылка, ДанныеСотрудника, Отказ) Экспорт
	Если ЗначениеЗаполнено(ДанныеСотрудника.ФамилияВСвидетельстве) И ВРег(СокрЛП(ДанныеСотрудника.Фамилия)) = ВРег(СокрЛП(ДанныеСотрудника.ФамилияВСвидетельстве)) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: Фамилия в свидетельстве совпадает с фамилией в изменившихся данных.';
																					|en = 'Employee %1: Last name in the certificate matches the one in the changed data.'"), ДанныеСотрудника.СотрудникНаименование);
	...
	Если ЗначениеЗаполнено(ДанныеСотрудника.ИмяВСвидетельстве) И ВРег(СокрЛП(ДанныеСотрудника.Имя)) = ВРег(СокрЛП(ДанныеСотрудника.ИмяВСвидетельстве)) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: Имя в свидетельстве совпадает с именем в изменившихся данных.';
																					|en = 'Employee %1: Name in the certificate matches the one in the changed data.'"), ДанныеСотрудника.СотрудникНаименование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ДанныеСотрудника.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник",,Отказ);
	КонецЕсли;
    Если ЗначениеЗаполнено(ДанныеСотрудника.ОтчествоВСвидетельстве) И ВРег(СокрЛП(ДанныеСотрудника.Отчество)) = ВРег(СокрЛП(ДанныеСотрудника.ОтчествоВСвидетельстве)) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: Отчество в свидетельстве совпадает с отчеством в изменившихся данных.';
																					|en = 'Employee %1: Patronymic in the certificate matches the one in the changed data.'"), ДанныеСотрудника.СотрудникНаименование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ДанныеСотрудника.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник",,Отказ);
	КонецЕсли;
КонецПроцедуры	
...
Процедура ПроверитьСНИЛСЗастрахованногоЛица(ДанныеЗЛ, НомерВПачке, ПравилоПроверки, Ошибки, ИсточникаДанныхЗЛ, Отказ = Ложь)
	...
	Если ЗначениеЗаполнено(ДанныеЗЛ[ПравилоПроверки.ПутьКДанным]) И 
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: %2';
																					|en = 'Employee %1: %2'"), ДанныеЗЛ.Наименование, ТекстСообщения);		
...
Процедура ПроверитьУдостоверениеЛичностиЗастрахованногоЛица(ДанныеЗЛ, НомерВПачке, ПравилаПроверки, Ошибки, ИсточникаДанныхЗЛ, Отказ = Ложь)
	...
	Если ПравилоПроверкиВидДокумента <> Неопределено 
		...
		Если ПравилоПроверкиСерияДокумента <> Неопределено
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: %2.';
																						|en = 'Employee %1: %2.'"), ДанныеЗЛ.Наименование, ТекстОшибкиСерия);
	...
	Если ПравилоПроверкиВидДокумента <> Неопределено 
		...
		Если ПравилоПроверкиНомерДокумента <> Неопределено 
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: %2.';
																						|en = 'Employee %1: %2.'"), ДанныеЗЛ.Наименование, ТекстОшибкиНомер);
					...
					Если КатегорииЗЛИностранцев.Найти(ДанныеДокумента.КатегорияЗастрахованныхЛиц) <> Неопределено Тогда  
						...
						Если ВыборкаПоСтрокамДокумента.НачисленоНакопительная <> 0 Тогда		
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: для категории %2, данные о начисленных суммах на накопительную часть пенсии должны отсутствовать.';
																										|en = 'Employee %1: information about amounts accrued to funded component of pension should be missing for category %2.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование, ДанныеДокумента.КатегорияЗастрахованныхЛиц);
						...
						Если ВыборкаПоСтрокамДокумента.УплаченоНакопительная <> 0 Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сотрудник %1: для категории %2, данные о уплаченных суммах на накопительную часть пенсии должны отсутствовать.';
																										|en = 'Employee %1: information about amounts paid to funded component of pension should be missing for category %2.'"), ВыборкаПоСтрокамДокумента.СотрудникНаименование, ДанныеДокумента.КатегорияЗастрахованныхЛиц);
						...
						ИначеЕсли ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаРождения) 
							...
							Если Разность >= 0.10 Тогда
								ШаблонТекстаОшибки = НСтр("ru = 'Сотрудник %1: - страховые взносы на страховую часть трудовой пенсии умноженные на тариф страховых взносов на накопительную часть пенсии  и деленные на  тариф страховых взносов  на страховую часть  должно равняться сумме страховых взносов на накопительную часть трудовой пенсии (допустимое расхождение до 10 копеек)';
															|en = 'Employee %1:  - insurance contributions to the insurance part of labor pension multiplied by funded component of labor pension and divided by insurance contribution rate must be equal to the funded component of labor pension (allowable discrepancy to 10 copecks)'");
		...
		Пока ВыборкаПоСтрокамДокумента.СледующийПоЗначениюПоля("НомерСтрокиСтаж") Цикл
			...
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаНачалаПериода) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончанияПериода) Тогда
				...
				Иначе
					ТекстОшибки = НСтр("ru = 'не указана дата начала периода.';
										|en = 'period start date not specified.'");     
					...
					ТекстОшибки = НСтр("ru = 'не указана дата окончания периода.';
										|en = 'period end date not specified.'");     
			...
			ИначеЕсли Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаНачалаПериода) Тогда
				ТекстОшибки = НСтр("ru = 'при указании даты окончания периода необходимо указывать дату начала периода.';
									|en = 'if you specify period end date, you should specify period start date.'");     
				ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ДатаНачалаПериода");
			ИначеЕсли Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончанияПериода) Тогда
				ТекстОшибки = НСтр("ru = 'при указании даты начала периода необходимо указывать дату окончания периода.';
									|en = 'if you specify period start date, you should specify period end date.'");     
				ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ДатаОкончанияПериода");
			...
			Иначе//Обе даты заполнены
				...
				Если ВыборкаПоСтрокамДокумента.ДатаНачалаПериода < ОтчетныйПериод Тогда
					ТекстОшибки = НСтр("ru = 'дата начала периода должна быть не ранее начала периода.';
										|en = 'period start date should not be less than the period start.'");     
					ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ДатаНачалаПериода");
				...
				Если ВыборкаПоСтрокамДокумента.ДатаОкончанияПериода > ОкончаниеПериода Тогда
					ТекстОшибки = НСтр("ru = 'дата окончания периода должна быть не позднее окончания периода.';
										|en = 'period end date should not be greater than the period end.'");     
					ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ДатаОкончанияПериода");
				КонецЕсли;	 
				...
				Если ВыборкаПоСтрокамДокумента.ДатаНачалаПериода > ВыборкаПоСтрокамДокумента.ДатаОкончанияПериода Тогда
					ТекстОшибки = НСтр("ru = 'дата начала периода не должна превышать дату окончания периода.';
										|en = 'period start date cannot be greater than end date.'");     
					ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ДатаНачалаПериода");
				КонецЕсли;
			...
			Если РасширеннаяПроверкаСтажа Тогда
					...
					Если ТекущаяОсновнаяЗапись.ДатаОкончанияПериода <> Неопределено Тогда
						Если ВыборкаПоСтрокамДокумента.ДатаНачалаПериода <= ТекущаяОсновнаяЗапись.ДатаОкончанияПериода Тогда
							ТекстОшибки = НСтр("ru = 'дата начала периода должна быть больше даты окончания предыдущего периода';
												|en = 'Period start date should be greater than previous period end date'");     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ДатаНачалаПериода");
						КонецЕсли;	 
					КонецЕсли;
				...
				Если (ЭтоОсновнаяЗапись) Тогда
					Если Не ВыделятьСтажДоговорников Тогда  
						...
						Если (ВыборкаПоСтрокамДокумента.НомерОсновнойЗаписи <> ОжидаемыйНомерОсновнойЗаписи)  Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'неверный номер основной записи. Ожидается номер: %1';
																										|en = 'incorrect main entry number. Awaiting number: %1'"), ОжидаемыйНомерОсновнойЗаписи);     
				...
				Иначе
					...
					Если (ВыборкаПоСтрокамДокумента.НомерДополнительнойЗаписи <> ОжидаемыйНомерДополнительнойЗаписи)  Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'неверный номер дополнительной записи. Ожидается номер: %1';
																									|en = 'incorrect additional entry number. Awaiting number: %1'"), ОжидаемыйНомерДополнительнойЗаписи);     
				...
				Если УказаныТерриториальныеУсловия Тогда
					...
					Если СписокТерриториальныеУсловияЗаписи.НайтиПоЗначению(ПредставлениеЗначения) <> Неопределено Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанные в дополнительной записи территориальные условия %1 не должны повторяться в рамках одной основной записи.';
																									|en = 'site conditions %1 specified in the additional entry cannot repeat within one main entry.'"), ПредставлениеЗначения);     
				...
				Если УказаныОсобыеУсловияТруда Тогда
					...
					Если СписокУсловияТрудаЗаписи.НайтиПоЗначению(ПредставлениеЗначения) <> Неопределено Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанные в дополнительной записи условия труда %1 не должны повторяться в рамках одной основной записи.';
																									|en = 'work conditions %1 specified in the additional entry cannot repeat within one main entry.'"), ПредставлениеЗначения);     
				...
				Если УказаноОснованиеВыслугиЛет Тогда
					...
					Если СписокВыслугаЛетЗаписи.НайтиПоЗначению(ПредставлениеЗначения) <> Неопределено Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'указанный в дополнительной записи код выслуги лет %1 не должны повторяться в рамках одной основной записи.';
																									|en = 'seniority code %1 specified in the additional entry cannot repeat within one main entry.'"), ПредставлениеЗначения);     
				...
				Если УказанКодПозицииСписка Тогда 
					...
					Если НЕ УказаныОсобыеУсловияТруда Тогда
						ТекстОшибки = НСтр("ru = 'код позиции списка не может указываться без указания кода особых условий труда.';
											|en = 'list position code cannot be specified without specifying the code of special working conditions.'");     
					...
					Если (ДлинаКПС=8)или(ДлинаКПС=14) Тогда
						...
						Иначе	
							ТекстОшибки = НСтр("ru = 'неверный код позиции списка вредных профессий.';
												|en = 'incorrect code of unhealthy occupation list position.'");     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "КодПозицииСписка");
						КонецЕсли;	
					...
					Иначе	
						ТекстОшибки = НСтр("ru = 'длина кода позиции списка должна быть равна либо 5, либо 8, либо 14 символам.';
											|en = 'the length of list position code should be equal to 5, or 8, or 14 characters.'");     
						ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "КодПозицииСписка");
					КонецЕсли;	 
				КонецЕсли;
				...
				Если УказаныОсобыеУсловияТруда Тогда
					...
					Если ВыборкаПоСтрокамДокумента.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаПФР.Ст27_1 Тогда
						Если (НазваниеСписка <> "Список№1") Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %1 должен быть указан код позиции Списка №1.';
																										|en = 'specify position code of List No. 1 for special working conditions with code %1.'"), КодОсобыхУсловийТруда);     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "КодПозицииСписка");
						КонецЕсли;	 
					...
					ИначеЕсли ВыборкаПоСтрокамДокумента.ОсобыеУсловияТруда = Справочники.ОсобыеУсловияТрудаПФР.Ст27_2 Тогда
						...
						Если (НЕ УказанКодПозицииСписка) Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %1 должен быть указан код позиции Списка №1 или Списка №2.';
																										|en = 'specify position code of List No. 1 or List No. 2 for special working conditions with code %1.'"), КодОсобыхУсловийТруда);     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "КодПозицииСписка");
						КонецЕсли;	 
						
						Если (НазваниеСписка = "Список№1") Тогда
							Если (Первые8СимволовКПС <> "1010300Б")и(Первые8СимволовКПС <> "1010100Г")и(Первые8СимволовКПС <> "1010100Д")  Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %1  указан неверный код позиции списка.';
																											|en = 'invalid code of the list position is specified for special working conditions with code %1.'"), КодОсобыхУсловийТруда);     
								ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "КодПозицииСписка");
							КонецЕсли;	
					...
					Иначе
						...
						Если УказанКодПозицииСписка Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для особых условий труда с кодом %1 код позиции списка указываться не должен.';
																										|en = 'do not specify a list position code for special working conditions with code %1.'"), КодОсобыхУсловийТруда);     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "КодПозицииСписка");
						КонецЕсли;	 
					КонецЕсли;	 
				КонецЕсли;	
				...
				Если УказаноОснованиеВыслугиЛет Тогда
					...
					Если (ВыборкаПоСтрокамДокумента.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсии.Ст27_СМ) 
						...
						Если ДоляСтавкиУказанаНеверно(ВыборкаПоСтрокамДокумента.ТретийПараметрВыслугиЛет) Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 третий параметр должен содержать долю ставки (число от 0.1 до 2)';
																										|en = 'the third parameter should contain rate share (number from 0.1 to 2) for seniority basis %1'"), КодОснования);     
						...
						Иначе
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 должен указываться только третий параметр';
																										|en = 'only the third parameter should be specified for seniority basis %1'"), КодОснования);  
					...
					ИначеЕсли ВыборкаПоСтрокамДокумента.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсии.Ст27_ПД Тогда
						...
						Если УказанПараметр2 Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 второй параметр указываться не должен.';
																										|en = 'the second parameter should not be specified for seniority basis %1.'"), КодОснования);     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка,  ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ВторойПараметрВыслугиЛет");
					...
					ИначеЕсли ВыборкаПоСтрокамДокумента.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсии.Ст27_ПДРК Тогда
						...
						ИначеЕсли УказанПараметр3 
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 третий параметр может содержать только ставку равную 0 или 1.';
																										|en = 'the third parameter can contain only rate equal to 0 or 1 for seniority basis %1.'"), КодОснования);
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ТретийПараметрВыслугиЛет");
						КонецЕсли;	
					...
					ИначеЕсли (ВыборкаПоСтрокамДокумента.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсии.САМОЛЕТ) 
						...
						Если  УказанПараметр3 Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 параметр 3 указываться не должен.';
																										|en = 'parameter 3 should not be specified for seniority basis %1.'"), КодОснования);     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ТретийПараметрВыслугиЛет");
						КонецЕсли;	 
						
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 параметры 1 и 2 должны содержать отработанное время (часы и минуты).';
																									|en = 'parameters 1 and 2 should contain hours worked (hours and minutes) for seniority basis %1.'"), КодОснования);  
					...
					ИначеЕсли (ВыборкаПоСтрокамДокумента.ОснованиеВыслугиЛет = Справочники.ОснованияДосрочногоНазначенияПенсии.РЕАКТИВН)
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 параметры 2 и 3 указываться не должны.';
																									|en = 'parameters 2 and 3 should not be specified for seniority basis %1.'"), КодОснования);
						Если УказанПараметр2 Тогда     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ВторойПараметрВыслугиЛет");
						КонецЕсли;	
						Если УказанПараметр3 Тогда     
					...
					Иначе
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для основания выслуги лет %1 параметры выслуги лет 1,2,3 указываться не должны.';
																									|en = 'parameters 1,2,3 should not be specified for seniority basis %1.'"), КодОснования); 
						Если УказанПараметр1 Тогда    
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ПервыйПараметрВыслугиЛет");
						КонецЕсли;	
						Если УказанПараметр2 Тогда    
				...
				Иначе
					Если ПараметрыУказаны Тогда
						ТекстОшибки = НСтр("ru = 'параметры выслуги лет 1,2,3 без указания основания выслуги лет указываться не должны.';
											|en = 'parameters of seniority 1,2,3 cannot be specified without seniority basis.'");     
						Если УказанПараметр1 Тогда    
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ПервыйПараметрВыслугиЛет");
						КонецЕсли;	
						Если УказанПараметр2 Тогда    
				...
				Если УказанТретийПараметрИсчисляемогоСтажа Тогда
					...
					Если Не ЭтоПараметрСПравомНаДосрочнуюПенсию И (УказаноОснованиеИсчисляемогоСтажа Или ЗаданаГруппаФОВ Или УказаноОснованиеВыслугиЛет Или ЗаданаГруппаУсловияТруда) Тогда
						ШаблонТекстаОшибки = НСтр("ru = 'при указании третьего параметра исчисляемого стажа ""%1"" группы реквизитов должны быть пустыми: Исчисляемый стаж: основание, Фактически отработанное время, Выслуга лет';
													|en = 'if the third parameter of the ""%1"" estimated length of service is specified, attribute groups should be empty: Estimated length of service: basis, actual hours worked, Seniority '");  
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												ШаблонТекстаОшибки,
					...
					Если ЭтоПараметрСПравомНаДосрочнуюПенсию И ЗаданаГруппаФОВ Тогда
						ШаблонТекстаОшибки = НСтр("ru = 'при указании третьего параметра исчисляемого стажа ""%1"" группа реквизитов фактически отработанного времени указываться не должна.';
													|en = 'if the third parameter of the ""%1"" estimated length of service is specified, attribute group of actual hours worked should not be specified.'");  
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												ШаблонТекстаОшибки,
												ВыборкаПоСтрокамДокумента.ТретийПараметрИсчисляемогоСтажаКод);
					...
					Если (ВыборкаПоСтрокамДокумента.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.МЕСЯЦ Или ВыборкаПоСтрокамДокумента.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.МЕДНЕТРУД)
						...
						ШаблонТекстаОшибки = НСтр("ru = 'при указании третьего параметра исчисляемого стажа ""%1"" обязательно заполнение хотя бы одной из групп реквизитов ""Условия труда"", ""Исчисление стажа: код"", ""Досрочное назначение пенсии"".';
													|en = 'if the third parameter of the ""%1"" estimated length of service is specified, at least one attribute group ""Working conditions"", ""Service estimation: code"", ""Early old-age pension granting"" must be specified.'");  
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												ШаблонТекстаОшибки,
												ВыборкаПоСтрокамДокумента.ТретийПараметрИсчисляемогоСтажаКод);
				...
				Если ЗаданаГруппаФОВ Тогда
					Если (НЕ УказаныОсобыеУсловияТруда) И (НЕ УказаноОснованиеИсчисляемогоСтажа)
						...
						ТекстОшибки = НСтр("ru = 'параметры 1,2 исчисляемого стажа должны указываться только при заполнении других групп реквизитов (Территориальные условия, Условия труда, Исчисляемый стаж: основание).';
											|en = 'parameters 1 and 2 of the estimated length of service must be specified only when filling in other groups of attributes (Site conditions, Work conditions, Estimated length of service: basis).'");
					...
					Если ВыборкаПоСтрокамДокумента.ОснованиеИсчисляемогоСтажа	<>	Справочники.ОснованияИсчисляемогоСтраховогоСтажа.ВОДОЛАЗ Тогда
						...
						Если ВыборкаПоСтрокамДокумента.ПервыйПараметрИсчисляемогоСтажа > 11 Тогда
							ТекстОшибки = НСтр("ru = 'первый параметр исчисляемого стажа (Месяцы) не должен быть больше 11.';
												|en = 'first parameter of estimated length of service (Months) should not be more than 11.'");     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ПервыйПараметрИсчисляемогоСтажа");							
						КонецЕсли;	 	
						Если ВыборкаПоСтрокамДокумента.ВторойПараметрИсчисляемогоСтажа > 30 Тогда
							ТекстОшибки = НСтр("ru = 'второй параметр исчисляемого стажа (Дни) не должен быть больше 30.';
												|en = 'the second parameter of the estimated length of service (Days) cannot be more than 30.'");     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ВторойПараметрИсчисляемогоСтажа");
						КонецЕсли;	 	
						...
						Если (ФОВДляСравнения > КалендарноеВремя) Тогда
							ТекстОшибки = НСтр("ru = 'указанное фактически отработанное время превышает календарное время.';
												|en = 'the specified actual hours worked exceed the calendar time.'");     
							ДокументыСЗВДобавитьОшибкуСтажа(Ошибки, ТекстОшибки, Ссылка, ВыборкаПоСтрокамДокумента, Отказ, РасширеннаяПроверкаСтажа, "ПервыйПараметрИсчисляемогоСтажа");
						КонецЕсли;	
					КонецЕсли;	 
				...
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Не удалось проверить файл %1 программой CheckXML';
											|en = 'Cannot check file %1 by CheckXML application.'"),
										ОписаниеФайла.ИмяФайла);
					...
					Иначе
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Не удалось проверить файл %1 программой CheckUFA';
											|en = 'Cannot check file %1 by CheckUFA application.'"),
										ОписаниеФайла.ИмяФайла);
				...
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'Не удалось проверить файл %1 программой ПО ПД';
											|en = 'Cannot check file %1 by Document Verification Software application.'"),
										ОписаниеФайла.ИмяФайла);
...
Функция ПроверитьФайлыПрограммойCheckUFA(КаталогПрограммы, КаталогФайлов, ПроверяемыйФайл)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ТекстИсключения = НСтр("ru = 'Проверка файлов отчетности на сервере не возможная.';
								|en = 'Cannot check reporting files on the server.'");
...
Процедура УстановитьПараметрыВыбораТерриториальныхУсловий(Элементы, ИмяЭлементаТерриториальныеУсловия, ИсключаемыеУсловия = Неопределено) Экспорт 
	...
	ИсключаемыеЗначения.Добавить(ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ТерриториальныеУсловияПФР.ЗАГР"));
		...
		Если ПериодЗаполненКорректно = Неопределено Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Строка %1: для сотрудника %2 не указаны сведения о стаже.';
					|en = 'The string%1: the information about length of service is not specified for the employee %2.'"),
		...
		ИначеЕсли ПериодЗаполненКорректно = Ложь Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Строка %1: для сотрудника %2 период стажа выходит за границы отчетного (корректируемого) периода.';
					|en = 'The string%1: the length of service for the employee %2 is above the limits of the reporting (corrected) period.'"),
				Выборка.НомерСтроки, Выборка.Сотрудник);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ,
				"Объект.Сотрудники[" + Формат(Выборка.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].НомерСтроки" , , Отказ);
		КонецЕсли;
		...
		Если УникальныеСтраховыеНомера[ДанныеСотрудника.СтраховойНомерПФР] <> Неопределено Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Строка %1: СНИЛС %2 уже был введен ранее в строке %3.';
					|en = 'The string%1: IIAN%2 has already been filled in the string %3.'"),
		...
		Если УникальныеИНН[ДанныеСотрудника.ИНН] <> Неопределено Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Строка %1: сотрудник с ИНН %2 был введен ранее в строке %3.';
					|en = 'The string%1: the employee with the TIN%2 has been filled in before in the string %3.'"),
