		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			ОписаниеИсключения1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В состав плана обмена %1 должны входить следующие объекты метаданных: %2';
					|en = 'The following metadata objects should be included in exchange plan %1: %2'"),
				ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			ОписаниеИсключения2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2';
					|en = 'The following metadata objects should NOT be included in exchange plan %1: %2'"),
				ИмяПланаОбмена,
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			ОписаниеИсключения3 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации.
					...
					|en = 'Objects with the set flag of automatic registration should not be included in exchange plan %1.
					|It is required to prohibit autoregistration for the following metadata objects: %2'"),
				ИмяПланаОбмена,
	...
	Если ПараметрыКлиента.Количество() = 0
	   И ТекущийРежимЗапуска() <> Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Не заполнены параметры клиента на сервере.';
								|en = 'Client parameters on server are not entered.'");
	КонецЕсли;
...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияВерсииПрограммы() Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Версия программы обновлена, требуется перезапустить сеанс.';
						|en = 'Application version is updated, restart the session.'");
...
Процедура ВосстановитьПредопределенныеЭлементы() Экспорт
	...
	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Восстановление предопределенных элементов следует выполнять только в главном узле РИБ.
			...
			|en = 'Restore predefined items only in DIB master node.
			|Then synchronize it with subnodes.'");
	КонецЕсли;
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ВызватьИсключение НСтр("ru = 'Регистрация изменений неразделенных данных в разделенном режиме.';
									|en = 'Changes registration of shared data in the undivided mode.'");
		КонецЕсли;
		...
		Если Не ЭтоРазделенныйПланОбмена Тогда
			ВызватьИсключение НСтр("ru = 'Регистрация изменений для неразделенных планов обмена не поддерживается.';
									|en = 'Changes registration for undivided exchange plans is not supported. '");
		КонецЕсли;
		...
		Если ЭтоРазделенныйОбъектМетаданных Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация изменений для разделенных объектов не поддерживается.';
										|en = 'Changes registration for divided objects is not supported.'");
		КонецЕсли;
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	Возврат Символы.ПС + Символы.ПС 
		+ НСтр("ru = 'Для разработчика: возможно требуется обновить вспомогательные данные,
					...
					|en = 'For developer: perhaps it is required to update auxiliary data
					|which influences the application operations. To update it, you can:
					|- Use external data processor
					|""Developer tools: Update auxiliary data"".
					|- Or run the application with 1C:Enterprise 8 command line parameter
					|  ""/C ЗапуститьОбновлениеИнформационнойБазы"".
					| - Or increase the configuration version number to
					| execute the infobase data update procedures during the next launch.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("ru = 'Для возможности работы программы.';
			|en = 'For the application operation.'")));
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("ru = 'Недопустимо создавать предопределенный элемент помеченный на удаление.';
				|en = 'Cannot create a predefined item marked for deletion.'");
	Иначе
		ИмяРеквизита = "";
		   ...
		   И СтарыеСвойства.ПометкаУдаления <> Истина Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недопустимо помечать на удаление предопределенный элемент:
				           |""%1"".';
				           |en = 'Cannot mark for deletion the predefined item:
				           |""%1"".'"),
		...
		ИначеЕсли (ИмяРеквизита <> "" И Параметры[ИмяРеквизита] <> "" И СтарыеСвойства[ИмяРеквизита] = ""
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:
				           |""%1"".';
				           |en = 'An item cannot be bound to the name of the item marked for deletion:
				           |""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
			...
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Обмен данными.Отключение связи с главным узлом';
						|en = 'Data exchange.Disconnected from the main node'",
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'У счета ""%1"" субконто №%2 ""%3"" не совпадает с предопределенным субконто ""%4"".';
							|en = 'The extra dimension No.%2 ""%3"" of the ""%1"" account does not match the predefined extra dimension ""%4"".'"),
						Строка(Счет),
						Индекс + 1,
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вариант встроенного языка конфигурации ""%1"" не поддерживается.
			           ...
			           |en = 'Option of script of configuration ""%1"" is not supported.
			           |It is required to use script option ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для запуска необходима версия платформы 1С:Предприятие %1 или выше.';
				|en = 'For launch 1C:Enterprise platform version %1 or higher is required.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НеПоддерживаемаяВерсияПлатформы) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается.
			           ...
			           |en = 'Mode of configuration compatibility with 1C:Enterprise, version %1 is not supported.
			           |To launch it, install the ""Do not use"" compatibility mode in the configuration when developing in version %2
			           |(or ""Version %2"" when developing in older versions).'"),
	...
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено свойство конфигурации Версия.';
								|en = 'Property of configuration Version is not entered.'");
	Иначе
		Попытка
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".
				           ...
				           |en = 'Property of configuration version ""%1"" is entered incorrectly 
				           |Correct format is, for example, ""1.2.3.45"".'"),
				Метаданные.Версия);
		КонецПопытки;
	...
	Если Не ТекущийПользовательИБ.АутентификацияСтандартная
	   ...
	   И Не ТекущийПользовательИБ.АутентификацияOpenID Тогда
		ВызватьИсключение НСтр("ru = 'Вход в программу запрещен. Обратитесь к администратору.';
								|en = 'Logon into the application is prohibited. Contact administrator.'");
	КонецЕсли;
	 ...
	 Или Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		ВызватьИсключение
			НСтр("ru = 'В конфигурации в свойстве ОсновныеРоли не указаны стандартные роли
			           ...
			           |en = 'In configuration in property ОсновныеРоли, standard roles
			           |АдминистраторСистемы and ПолныеПрава are not specified or excess roles are specified.'");
	КонецЕсли;
			...
			Если Не ДоступенПривилегированныйРежим Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно установить параметры сеанса по причине: профиль безопасности %1 отсутствует в кластере серверов 1С:Предприятия, или для него запрещено использование в качестве профиля безопасности безопасного режима.
						...
						|en = 'Cannot set session parameters due to: security profile %1 does not exist in the 1C:Enterprise server cluster or it cannot be used as a security profile of the safe mode.
						|
						|To restore the application functionality, disable usage of the security profile via the cluster console and configure security profiles again using the configuration interface (the corresponding commands are located in the application settings section).'"),
					ПрофильИнформационнойБазы);
			КонецЕсли;
		...
		Если Не ДоступенПривилегированныйРежим Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно установить параметры сеанса по причине: профиль безопасности %1 не содержит разрешения на установку привилегированного режима. Возможно, он был отредактирован через консоль кластера.
					|
					|Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';
					|en = 'Cannot set the session parameters due to: security profile %1 does not contain permissions to set the privileged mode. It might have been edited via the cluster console.
					|
					|To restore the application functionality, disable usage of security profiles using the cluster console and configure security profiles again using the configuration interface (the corresponding commands are located in the application settings section).'"),
				ПрофильИнформационнойБазы);
		...
		Исключение
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно установить параметры сеанса по причине: %1.
					...
					|en = 'Cannot set the session parameters due to: %1 
					|
					|A security profile might have been set for the infobase via the cluster console, that does not allow external modules to run without setting the safe mode. In this case, to restore the application, disable usage of the security profile via the cluster console and configure security profiles again using the configuration interface (the corresponding commands are located in the application settings section). The application will be automatically configured so that it can be used together with enabled security profiles.'"),
