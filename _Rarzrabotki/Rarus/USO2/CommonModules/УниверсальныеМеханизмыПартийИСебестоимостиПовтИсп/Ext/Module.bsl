...
Функция ПравилаЗаполненияПоляТипЗаписи(ИмяДокументаДляОтбора = "") Экспорт
	...
	НовоеПравило = ТаблицаПравил.Добавить();
	НовоеПравило.ИмяДокумента               = "УСОERP_РеализацияСтроительныхРабот";
	НовоеПравило.ПустоеЗначениеРегистратора = Документы.УСОERP_РеализацияСтроительныхРабот.ПустаяСсылка();
	НовоеПравило.ХозяйственнаяОперация      = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
	НовоеПравило.ПоложительноеКоличество    = Неопределено;
	НовоеПравило.ТипЗаписиПриход            = Неопределено;
	НовоеПравило.ТипЗаписиРасход            = Перечисления.ТипыЗаписейПартий.Потребление;
	НовоеПравило.ДокументИсточникВПриходе   = Ложь;
	НовоеПравило.ДокументИсточникВРасходе   = Ложь;
	НовоеПравило.КорПартияВРасходе          = Ложь;
	// {УСО 2.4}:{Управление строительной организацией}{Григорьев А.Ю.} }
...
Функция ПартионныйУчетВерсии22(Дата = Неопределено) Экспорт
	...
	ИначеЕсли НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПартионныйУчет") Тогда
		ВызватьИсключение НСтр("ru = 'Некорректно установлены функциональные опции партионного учета';
								|en = 'Functional options of batch accounting are set incorrectly'"); // такого быть не должно - обе опции включаются синхронно
	КонецЕсли;
