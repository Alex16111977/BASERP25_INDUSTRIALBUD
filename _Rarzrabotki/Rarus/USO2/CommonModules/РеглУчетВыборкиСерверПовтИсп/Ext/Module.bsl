...
Функция ЗапросДанных(ДокументИмя, ЭтоОбъектРасчетов = Истина) Экспорт
	...
	Иначе
		...
		//<{УСО 2.4}:{Управление строительной организацией}
		//ТекстыСозданияВременныхТаблиц.Добавить(ТекстПартии()); 			
		ТекстыСозданияВременныхТаблиц.Добавить(УСОERP_ЗащищенныеФункцииСервер.УСОERP_ТекстПартии(ДокументИмя));
		//>{УСО 2.4}:{Управление строительной организацией}
	КонецЕсли;
...
#Область ВтСтроки
	...
	|	ИНАЧЕ
	|		ВЫБОР КОГДА КорАналитика.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга)
	|		КОГДА Движения.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|		КОНЕЦ
...
Функция ТекстВТРасчетов()
	...
	//++ НЕ УТКА
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца КАК ЗаказД
	|		ПО ЗаказД.Ссылка = Расчеты.ЗаказКлиента
	//-- НЕ УТКА
	|ГДЕ
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отражение в регламентированном учете';
				|en = 'Record in compliance accounting'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В модуле менеджера документа %1 отсутствует функция ТекстОтраженияВРеглУчете()';
					|en = 'Function ТекстОтраженияВРеглУчете() is missing in the manager module of document %1'"), ДокументИмя);
		КонецПопытки;
	ИначеЕсли ЗначениеЗаполнено(ИмяОбщегоМодуляДляОтражения) Тогда
		Попытка
...
Функция ШаблонКомментарияОшибок(КодОшибки) Экспорт
	Если КодОшибки = "СЧЕТ_ЗАПРЕЩЕН" Тогда
		Возврат НСтр(
			...
			|en = 'The ""%Счет%"" banned account is specified for the ""%ВидСчета%"" %Измерение%%МестоАналитика% accounting.'");
	ИначеЕсли КодОшибки = "СЧЕТ_ЗАБАЛАНСОВЫЙ" Тогда
		Возврат НСтр(
			"ru = 'Для учета ""%ВидСчета%"" %Измерение%%МестоАналитика% указан забалансовый счет ""%Счет%"".';
			|en = 'The ""%Счет%"" off-balance account is specified for the ""%ВидСчета%"" %Измерение%%МестоАналитика% accounting.'");
	ИначеЕсли КодОшибки = "СЧЕТ_ПУСТОЙ" Тогда
		Возврат НСтр(
			"ru = 'Для учета ""%ВидСчета%"" %Измерение%%МестоАналитика% не указан счет.';
			|en = 'Account is not specified for the ""%ВидСчета%"" %Измерение%%МестоАналитика% accounting.'");
	Иначе
	...
	Если ЗначениеЗаполнено(АналитикаУчета) Тогда
		...
		ИначеЕсли ТипАналитики = Тип("ПеречислениеСсылка.ТипыНалогообложенияНДС") Тогда
			Аналитика = НСтр("ru = 'для налогообложения ""%АналитикаУчета%""';
							|en = 'for the ""%АналитикаУчета%"" taxation'");
		Иначе
			Аналитика = НСтр("ru = 'для аналитики учета ""%АналитикаУчета%""';
							|en = 'for the ""%АналитикаУчета%"" accounting dimension'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДополнительнаяИнформацияРасчетовСПартнерами) Тогда
		Если ДополнительнаяИнформацияРасчетовСПартнерами = "РасчетыСКомиссионерами"
			...
			Результат = НСтр("ru = 'Расчеты с комиссионерами по товарам';
							|en = 'Goods settlements with consignees '");
		...
		ИначеЕсли ДополнительнаяИнформацияРасчетовСПартнерами = "РасчетыСКомиссионерами"
			...
			Результат = НСтр("ru = 'Авансы с комиссионерами по вознаграждению';
							|en = 'Advances with consignees for remuneration'");
...
Функция ТипыДокументовКПоследовательномуОтражению() Экспорт
	...
	//<{УСО 2.4}:{Управление строительной организацией}
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_АктПриемкиРабот"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_ЗакрытиеОбъектаСтроительства"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_ИнвестиционныйНДС"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_ПокупкаДоли"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_РаспределениеИнвестиционногоНДС"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_РеализацияСтроительныхРабот"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_РеализацияСтроительныхРаботВнутренняя"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_РасторжениеПокупкиДоли"));
	Результат.Добавить(Тип("ДокументСсылка.УСОERP_СдачаОбъектаНедвижимости"));
	//>{УСО 2.4}:{Управление строительной организацией}
...
Функция ИспользуемыеТаблицыДляОтраженияТипа(ИмяТипаДокумента) Экспорт
	...
	//-- НЕ УТ
	 ...
	 // {УСО 2.4}:{Управление строительной организацией}{{ 	 
	 // УСОERP_АктПриемкиРабот
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.УСОERP_АктПриемкиРабот");
	ТаблицыТиповДокументов.Вставить("УСОERP_АктПриемкиРабот", ТаблицыДокумента);
	// УСОERP_РеализацияСтроительныхРабот
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.УСОERP_РеализацияСтроительныхРабот");
	ТаблицыТиповДокументов.Вставить("УСОERP_РеализацияСтроительныхРабот", ТаблицыДокумента);
	// УСОERP_РеализацияСтроительныхРаботВнутренняя
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.УСОERP_РеализацияСтроительныхРаботВнутренняя");
	ТаблицыТиповДокументов.Вставить("УСОERP_РеализацияСтроительныхРаботВнутренняя", ТаблицыДокумента);
	// УСОERP_ПокупкаДоли
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.УСОERP_ПокупкаДоли");
	ТаблицыТиповДокументов.Вставить("УСОERP_ПокупкаДоли", ТаблицыДокумента);
	// УСОERP_РасторжениеПокупкиДоли
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.УСОERP_РасторжениеПокупкиДоли");
	ТаблицыТиповДокументов.Вставить("УСОERP_РасторжениеПокупкиДоли", ТаблицыДокумента);

	// УСОERP_ВводОстатковДСПоОбъектамСтроительства
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.УСОERP_ВводОстатковДСПоОбъектамСтроительства");
	ТаблицыТиповДокументов.Вставить("УСОERP_ВводОстатковДСПоОбъектамСтроительства", ТаблицыДокумента);
	// {УСО 2.4}:{Управление строительной организацией} }}
