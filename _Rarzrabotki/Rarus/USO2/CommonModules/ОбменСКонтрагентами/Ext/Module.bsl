...
#Область Справочники_ПрофилиНастроекЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Процедура = "Справочники.ПрофилиНастроекЭДО.ОбработатьДанныеДляПереходаНаНовуюВерсиюВнешнийВызов";
	...
	Обработчик.БлокируемыеОбъекты = "Документ.ЭлектронныйДокументВходящий,"
		...
		+ "Справочник.ПрофилиНастроекЭДО";
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: изменение учетных параметров справочника профили настроек ЭДО.
	|Настройка и обмен электронными документами с контрагентами временно невозможен.';
	|en = '1С:Exchange with counterparties: changing of accounting parameters of the EDI settings profile catalog.
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Версия = Версия;
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "Справочники.СоглашенияОбИспользованииЭД.ОбработатьДанныеДляПереходаНаНовуюВерсиюВнешнийВызов";
	...
	Обработчик.БлокируемыеОбъекты = "Справочник.СоглашенияОбИспользованииЭД,"
		...
		+ "Документ.ЭлектронныйДокументИсходящий";
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: изменение учетных параметров справочника ""Настройки ЭДО"".
	|Настройка и обмен электронными документами с контрагентами временно невозможен.';
	|en = '1С:Exchange with counterparties: changing of accounting parameters of the ""EDI settings"" catalog.
	|Setup and exchange of electronic documents with counterparties are temporarily unavailable.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
Функция СтатусДокументооборота(Документ) Экспорт
	...
	ТекстСообщенияОбОшибкеВоВходящихПараметрах = НСтр("ru = 'Неверный первый параметр ""Документ"": 
		...
		|en = 'The incorrect first ""Document"" parameter: 
		|- The value must be not empty link to the electronic document or the document included in the ""ElectronicDocumentBases"" defined type.'");
	...
	ИначеЕсли СостояниеВерсииЭлектронногоДокумента = Перечисления.СостоянияВерсийЭД.ОжидаетсяОтправка
		ИЛИ СостояниеВерсииЭлектронногоДокумента = Перечисления.СостоянияВерсийЭД.ОжидаетсяОтправкаПолучателю
		ИЛИ СостояниеВерсииЭлектронногоДокумента = Перечисления.СостоянияВерсийЭД.ОжидаетсяПодтверждение Тогда
...
Процедура КлючевыеРеквизитыТекстСостоянияЭДОПриИзменении(ПараметрыПриИзменении) Экспорт
	...
	Если ПараметрыПриИзменении.Организация <> Реквизит.Организация
		...
		|		ИНАЧЕ 2
		|	КОНЕЦ + ВЫБОР
		|		КОГДА СоглашенияОбИспользованииЭД.ИспользуетсяДляОтправки
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК Порядок
		|ИЗ
			...
			Если ЭДОНастроен Тогда
				Если ОбменСКонтрагентамиВнутренний.ДокументЯвляетсяСчетомФактурой(ПараметрыПриИзменении.ДокументСсылка) Тогда
					ТекстСостоянияЭДО = НСтр("ru = 'Требуются действия с нашей стороны';
											|en = 'Actions from our side are required'");
				Иначе
...
#Область РегистрыСведений_СостоянияЭД_УдалитьСостояниеУдалитьОжидаетсяИзвещение
	...
	Обработчик.Комментарий = НСтр("ru = 'Изменяет состояние документов с ""Ожидается извещение"" на ""Ожидается извещение о получении""';
									|en = 'Changes the document state from ""Notification is awaited"" to ""Receipt notification is awaited""'");
...
#Область ОбменСКонтрагентами_ПроверитьКонтрагентовБЭД
	...
	Обработчик.Комментарий = НСтр("ru = 'Проверяет контрагентов на подключение к сервису 1С-ЭДО.';
									|en = 'Checks if counterparties are connected to 1C:EDI service.'");
...
#Область ОбменСКонтрагентами_ПеревестиНаНовуюАрхитектуруЭДО
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Создает входящие и исходящие ЭД';
									|en = 'Exchange with counterparties: Creates incoming and outgoing EDs'");
...
#Область ОбменСКонтрагентами_ПеревестиНаНовуюАрхитектуруПакетыЭД
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Удаляет полученные и отправленные пакеты ЭД';
									|en = 'Exchange with counterparties: Deletes received and sent ED packages'");
...
#Область ОбменСКонтрагентами_ПеревестиНаНовуюАрхитектуруПроизвольныхЭД
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Создает входящие и исходящие ЭД для произвольных ЭД';
									|en = 'Exchange with counterparties: Creates incoming and outgoing EDs for custom EDs'");
...
#Область ОбменСКонтрагентами_ПеревестиНаНовуюАрхитектуруЭДОСтарыеCML
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Создает входящие и исходящие ЭД для старых CML документов';
									|en = 'Exchange with counterparties: Creates incoming and outgoing EDs for old CML documents'");
...
#Область ОбменСКонтрагентами_ЗаполнитьДатуПоследнегоИзмененияСостоянияЭДО
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Заполняет дату последнего изменения состояния ЭДО';
									|en = 'Exchange with counterparties: Fills in the date of the latest EDI state change'");
...
#Область РегистрыСведений_ЖурналСобытийЭД_ЗаполнитьРесурсВладелецЭД
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Заполнение регистра сведений Журнал событий ЭДО';
									|en = 'Exchange with counterparties: Filling in the ""EDI event log"" information register'");
...
#Область ОбменСКонтрагентами_ЗаполнитьСостоянияСтарыхУПДУКД
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Обновление регистра сведений Состояния электронных документов';
									|en = 'Exchange with counterparties: Update the ""Electronic document states"" information register'");
...
#Область ОбменСКонтрагентами_ЗаменитьУстаревшиеВидыЭД
	...
	Обработчик.Комментарий = НСтр("ru = 'Обмен с контрагентами: Обновление устаревших видов ЭД';
									|en = 'Exchange with counterparties: Update obsolete ED kinds'");
...
#Область Справочники_ЭДПрисоединенныеФайлы_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Настройка и обмен электронными документами с контрагентами временно невозможен.';
	|en = '1С:Exchange with counterparties: change parameters of the ""Attached files (Electronic documents)"" catalog.
...
#Область РегистрыСведений_СостоянияЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = '1С:Exchange with counterparties: update description of electronic document bases.
	|The Create section can have empty data in the current EDI to-dos.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_ЭлектронныйДокументИсходящий_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление исходящих электронных документов.';
									|en = '1С:Exchange with counterparties: update outgoing electronic documents.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область РегистрыСведений_КонтрольОтраженияВУчетеЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|The ""Record in accounting"" section in the current EDI to-dos may be filled in incompletely.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Версия_1_5_1

#Область Справочники_ПрофилиНастроекЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.5.1.14";
	Обработчик.РежимВыполнения = "Отложенно";
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Настройка и обмен электронными документами с контрагентами временно невозможен.';
	|en = '1С:Exchange with counterparties: changing of parameters of the ""EDI settings"" catalog.
	|Configuration and exchange of electronic documents with counterparties are temporarily unavailable.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_ЭлектронныйДокументВходящий_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление входящих электронных документов.';
									|en = '1С:Exchange with counterparties: update incoming electronic documents.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			НастройкаЭДО.Комментарий = НСтр("ru = '##Настройка ЭДО помечена на удаление автоматически при обновлении.';
											|en = '##EDI setting is automatically marked for deletion during update.'");
		...
		Пока ВыборкаУчастников.Следующий() Цикл
			...
			Если Не ВыборкаУчастников.СоглашениеОбИспользованииЭД.ПометкаУдаления Тогда
				...
				НастройкаЭДО.УстановитьПометкуУдаления(Истина);
				НастройкаЭДО.Наименование = НСтр("ru = 'Не использовать';
												|en = 'Do not use'") + " - " + НастройкаЭДО.Наименование;
				НастройкаЭДО.Комментарий = НСтр("ru = '##Настройка ЭДО помечена на удаление автоматически при обновлении.';
												|en = '##EDI setting is automatically marked for deletion during update.'");
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НастройкаЭДО);
			КонецЕсли;
	...
	Пока НЕ Результат.Пустой() Цикл
		...
		Исключение
			...
			Операция = НСтр("ru = 'Перенос данных электронных документов на новую архитектуру';
							|en = 'Transfer electronic document data to new architecture'");
	...
	Пока НЕ Результат.Пустой() Цикл
		...
		Исключение
			...
			Операция = НСтр("ru = 'Перенос данных произвольных электронных документов на новую архитектуру';
							|en = 'Transfer data of arbitrary electronic documents to new architecture'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки);
	...
	Пока Не Результат.Пустой() Цикл
		...
		Исключение
			...
			Операция = НСтр("ru = 'Пометка на удаление пакетов электронных документов';
							|en = 'Mark electronic document packages for deletion'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Операция = НСтр("ru = 'Перенос данных пакетов электронных документов на новую архитектуру';
							|en = 'Transfer data of electronic document packages to new architecture'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ТекстОшибки);
	...
	Попытка
		...
		Пока Результат.Следующий() Цикл
			...
			Для каждого Строка Из ИскомыйПрофильНастроек.ИсходящиеДокументы Цикл
				...
				Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура Тогда
					Строка.ВерсияФормата = ?(ИскомыйПрофильНастроек.ИспользоватьУПД, НСтр("ru = 'ФНС 5.01 (УПД:Счет-фактура и первичный документ)';
																							|en = 'FTS 5.01 (UTD:Tax invoice and source document)'"), НСтр("ru = 'ФНС 5.02';
																																							|en = 'FTS 5.02'"));
					Строка.Приоритет = 4;
				КонецЕсли;
				...
				Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
					Строка.ВерсияФормата = ?(ИскомыйПрофильНастроек.ИспользоватьУКД, НСтр("ru = 'ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';
																							|en = 'FTS 5.01 (UAD:Corrective tax invoice and source document)'"), НСтр("ru = 'ФНС 5.02';
																																											|en = 'FTS 5.02'"));
					Строка.Приоритет = 6;
				КонецЕсли;
		...
		Пока Результат.Следующий() Цикл
			...
			Для каждого Строка Из ИскомаяНастройка.ИсходящиеДокументы Цикл
				...
				КонецЕсли;
				Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура Тогда
					Строка.ВерсияФормата = ?(ИскомаяНастройка.ИспользоватьУПД, НСтр("ru = 'ФНС 5.01 (УПД:Счет-фактура и первичный документ)';
																					|en = 'FTS 5.01 (UTD:Tax invoice and source document)'"), НСтр("ru = 'ФНС 5.02';
																																					|en = 'FTS 5.02'"));
					Строка.Приоритет = 4;
				КонецЕсли;
				...
				КонецЕсли;
				Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
					Строка.ВерсияФормата = ?(ИскомаяНастройка.ИспользоватьУКД, НСтр("ru = 'ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)';
																					|en = 'FTS 5.01 (UAD:Corrective tax invoice and source document)'"), НСтр("ru = 'ФНС 5.02';
																																										|en = 'FTS 5.02'"));
					Строка.Приоритет = 6;
				КонецЕсли;
			...
			Исключение
				...
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Не удалось загрузить правила преобразования формата %1.';
						|en = 'Cannot import the %1 format conversion rules.'"),
