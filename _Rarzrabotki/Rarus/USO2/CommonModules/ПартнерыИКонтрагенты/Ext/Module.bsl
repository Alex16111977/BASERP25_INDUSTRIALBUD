			...
			ИначеЕсли СвойстваГоловногоКонтрагента.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Контрагент %1 не может быть выбран головным, т.к. не является юридическим лицом';
						|en = 'Counterparty %1 cannot be selected as the main one as it is not a business entity'"),
					СвойстваГоловногоКонтрагента.Наименование);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, КлючДанных, "ГоловнойКонтрагент", , Отказ);
			...
			ИначеЕсли ЗначениеЗаполнено(СвойстваГоловногоКонтрагента.ИНН) И СвойстваГоловногоКонтрагента.ИНН <> Объект.ИНН Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ИНН контрагента не равен указанному в головном подразделении';
						|en = 'Counterparty TIN is not equal to the one specified in the main department'"),
					СвойстваГоловногоКонтрагента.Наименование);
			...
			Иначе
				ТекстОшибки =
					НСтр("ru = 'Контрагент должен быть юридическим лицом, т.к. является головным подразделением для других контрагентов';
						|en = 'Counterparty should be a business entity as it is a main department for other counterparties '");
			КонецЕсли;
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			                                СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			                                  НСтр("ru = 'Данные ИНН и КПП уже указаны для другого контрагента , ответственный - %1.';
													|en = 'These TIN and CRTR are already specified for another counterparty, responsible person - %1.'"),
	...
	Если ЗначениеЗаполнено(Объект.РегистрационныйНомер) И Объект.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
		...
		Если ВыборкаКонтрагент <> Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Данные регистрационного номера в этой же стране регистрации уже указаны для контрагента с кодом %1, ответственный - %2.';
									|en = 'Data of the registration number in the same registration country is already specified for the counterparty with code %1, responsible person - %2.'");
...
Функция СформироватьСтрокуТекущихТиповОтношений(ДанныеПартнера) Экспорт
	...
	СтрокаТипОтношений = ?(ДанныеПартнера.Клиент, НСтр("ru = 'Клиент';
														|en = 'Client'"),"") + ?(ДанныеПартнера.Поставщик, НСтр("ru = ', Поставщик';
																												|en = ', Supplier'"),"")
