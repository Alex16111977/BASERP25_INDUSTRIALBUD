...
Функция ПерерасчетыДоступны(Форма, ИмяРеквизитаПериодаРегистрации = "ПериодРегистрации", ИмяРеквизитаДатыНачала = "ДатаНачала", ВозможноВытеснение = Истина) Экспорт
	...
	Объект = Форма.Объект;
	ЭтоДокументИсправление = ЗначениеЗаполнено(Объект.ИсправленныйДокумент);
	
	Возврат (ЭтоДокументИсправление И Не ИсправлениеВТекущемПериоде(Форма, ИмяРеквизитаПериодаРегистрации))
		Или (Не ЭтоДокументИсправление И ВозможноВытеснение
			И (ЗначениеЗаполнено(Объект[ИмяРеквизитаДатыНачала]) И Объект[ИмяРеквизитаПериодаРегистрации] > Объект[ИмяРеквизитаДатыНачала]))
