...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	СтрокаПараметра.Описание = НСтр("ru = 'Имя пользователя конференции информационного центра';
									|en = 'Name of the info center conference user'");
	...
	СтрокаПараметра.Описание = НСтр("ru = 'Пароль пользователя конференции информационного центра';
									|en = 'Password of the info center conference user'");
	СтрокаПараметра.Тип = Новый ОписаниеТипов("Строка");
	...
	СтрокаПараметра.Описание = НСтр("ru = 'Имя пользователя программного интерфейса службы поддержки';
									|en = 'Name of the user of technical support application interface'");
	СтрокаПараметра.Тип = Новый ОписаниеТипов("Строка");
	...
	СтрокаПараметра.Описание = НСтр("ru = 'Пароль пользователя программного интерфейса службы поддержки';
									|en = 'Password of the user of technical support application interface'");
	СтрокаПараметра.Тип = Новый ОписаниеТипов("Строка");
	...
	СтрокаПараметра.Описание = НСтр("ru = 'Имя пользователя сервиса центра идей';
									|en = 'Idea center service username'");
	СтрокаПараметра.Тип = Новый ОписаниеТипов("Строка");
	...
	СтрокаПараметра.Описание = НСтр("ru = 'Пароль пользователя сервиса центра идей';
									|en = 'Password of the idea center service user'");
	СтрокаПараметра.Тип = Новый ОписаниеТипов("Строка");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ТехнологияСервисаИнтеграцияСБСП.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий          = НСтр("ru = 'Сформировать словарь полных путей к формам в справочнике ""ПолныеПутиКФормам"".';
												|en = 'Generate a dictionary of full paths to forms in the ""ПолныеПутиКФормам"" catalog.'");
	Иначе
		Обработчик                     = Обработчики.Добавить();
		Обработчик.Версия              = "*";
	...
	Иначе
		...
		Обработчик.Комментарий      = НСтр("ru = 'Обновление информационных ссылок для форм.';
											|en = 'Update of information links for forms.'");
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСсылкиДляФормВМоделиСервиса() Экспорт
	...
	Попытка
		...
		ТекстОшибки = НСтр("ru = 'Не удалось обновить Информационные ссылки';
							|en = 'Cannot update Information links'");
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(ИмяСобытия, 
				...
				НСтр("ru = 'Не удалось определить тип информационной ссылки:';
					|en = 'Cannot determine information link type:'") + Символы.ПС + Символы.ПС + ПредставлениеОбъекта);
	...
	Если ЗаписыватьОшибкуВЖР Тогда
		ИмяСобытияЖР = ИнформационныйЦентрСервер.ПолучитьИмяСобытияДляЖурналаРегистрации()
			...
			+ НСтр("ru = 'Информационные ссылки';
					|en = 'Information links'", ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка());
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖР,
			...
			НСтр("ru = 'При обновлении списка форм, в конфигурации не найдена в форма с полным путем:';
				|en = 'Form with the full path was not found in the configuration when updating the form list:'", ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка())
				+ ПолныйПутьКФорме);
	КонецЕсли;
