...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("ru = 'Адресный классификатор еще не загружался.';
												|en = 'Address classifier has not been imported yet.'");
		Возврат Результат;
	КонецЕсли;
	...
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адресный классификатор был загружен %1 назад.';
																					|en = 'Address classifier was imported %1 ago.'"),
...
Функция КодыАдреса(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Иначе
		ОписаниеПричиныОшибки = НСтр("ru = 'Передано недопустимое значение адреса в функцию получения кодов адреса';
									|en = 'Invalid address value was passed to function of receiving address codes'");
	...
	Иначе
		...
		Если РезультатПроверки.КодСостояния = 400 Тогда
			...
			ПредставлениеОшибки = НСтр("ru = 'Проверка адреса не выполнена. Некоторые поля адреса содержат некорректные сведения.';
										|en = 'Address checking failed. Some address fields contain incorrect information.'");
			...
			Если ФайлыНовогоФИАС.Количество() = 0 Тогда
				...
				Если ФайлыСтарогоФИАС.Количество() > 0 Тогда
					...
					ТекстПричины = НСтр("ru = 'Не допускается загрузка адресных сведений без поддержки муниципальных адресов.';
										|en = 'You cannot import address data without support of municipal addresses.'");
					
				Иначе
					
					ТекстПричины = НСтр("ru = 'Файл не содержит адресных сведений ФИАС';
										|en = 'File does not contain FIAS address data '");
				...
				|по причине: %2
				|Необходимо использовать адресные сведения расположенные по адресу https://its.1c.ru/download/fias2';
				|en = 'Cannot import address data from file:%1
				|due to: %2
				|Use address data from https://its.1c.ru/download/fias2'"), ПолноеИмяФайла, ТекстПричины));
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес)
	...
	Если СтрСравнить("ВСвободнойФорме", Адрес.AddressType) = 0 Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			НСтр("ru = 'Адрес не может быть проверен, так как он введен в свободной форме';
				|en = 'The address cannot be checked because, as it was entered as a free-form address'") );
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(Адрес.Area) Тогда
		...
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru = 'Не указан регион адреса';
																						|en = 'Address region is not specified'"));
		Возврат Результат;
	КонецЕсли;
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Указан несуществующий регион ""%1""';
						|en = 'Non-existing ""%1"" region is specified'"), Адрес.Area + " " + Адрес.AreaType));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли Не РегионЗагруженВАдресныеСведения.Загружен Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ",
			НСтр("ru = 'Отсутствуют адресные сведения по региону';
				|en = 'No address data by region'") + ": " + Адрес.Area + " " + Адрес.AreaType);
	...
	Для каждого АдресСтрока Из Адреса.Выгрузить() Цикл
		...
		Если КакиеУровниПроверять["Distict"] <> Неопределено И АдресСтрока["Наименование" + КакиеУровниПроверять["Distict"]] = NULL Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Район ""%1"" отсутствует в ФИАС';
						|en = 'District ""%1"" is missing in FIAS'"), Адрес.District + " " + Адрес.DistrictType));
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Area", НСтр("ru = 'Субъект РФ';
															|en = 'RF territorial entity'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
			
		Если НЕ ДанныеУровняКорректные(Адрес, "MunDistrict", НСтр("ru = 'Муниципальный район';
																	|en = 'Municipal area'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Distict", НСтр("ru = 'Район';
																|en = 'District'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
			
		Если НЕ ДанныеУровняКорректные(Адрес, "Settlement", НСтр("ru = 'Поселение';
																|en = 'Settlement'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
			
		Если НЕ ДанныеУровняКорректные(Адрес, "City", НСтр("ru = 'Город';
															|en = 'City'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "CityDistrict", НСтр("ru = 'Внутригородской район';
																	|en = 'Urban area'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Territory", НСтр("ru = 'Территория';
																|en = 'Area'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Locality", НСтр("ru = 'Населенный пункт';
																|en = 'Settlement'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Street", НСтр("ru = 'Улица';
															|en = 'Street'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
	...
	Если СтрСравнить(ПочтовыйИндекс, Адрес.ZipCode) <> 0 Тогда
		ДополнительныйТекст = ?(ЗначениеЗаполнено(ПочтовыйИндекс), " " + НСтр("ru = 'Возможно, правильный индекс';
																				|en = 'Index may be correct'") + " "  + ПочтовыйИндекс, "");
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Индекс ""%1"" не соответствует адресу.%2';
																			|en = 'Zip code ""%1"" does not match the address.%2'"), Адрес.ZipCode, ДополнительныйТекст));
	КонецЕсли;
...
Функция ДанныеУровняКорректные(Знач Адрес, Знач ИмяУровня, Знач ПредставлениеУровня, Знач АдресСтрока, Знач КакиеУровниПроверять, Знач ОшибкиПроверки)
	...
	Если КакиеУровниПроверять[ИмяУровня] <> Неопределено И ПустаяСтрока(АдресСтрока["НаименованиеУровень" + КакиеУровниПроверять[ИмяУровня]]) Тогда
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 ""%2"" отсутствует в ФИАС';
																	|en = '%1 ""%2"" is absent in FIAS'"), ПредставлениеУровня,
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("ru = 'Респ';
							|en = 'Rep'") Тогда
		Возврат НСтр("ru = 'Республика';
					|en = 'Republic'") + " " + Наименование;
	ИначеЕсли Сокращение = НСтр("ru = 'обл';
								|en = 'reg'") Тогда
		Возврат Наименование + " " + НСтр("ru = 'область';
											|en = 'region'");
	ИначеЕсли Сокращение = НСтр("ru = 'г';
								|en = 'year'") Тогда
		Возврат НСтр("ru = 'г.';
					|en = 'year'") + " " + Наименование;
	КонецЕсли;
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Очистка региона ""%1 - %2"" (осталось %3) ...';
																											|en = 'Clearing region ""%1 - %2"" (%3 is left) ...'"),
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("10400000", НСтр("ru = 'Элемент улично-дорожной сети, планировочной структуры дополнительного адресного элемента';
																|en = 'Item of street-road network, planning structure of additional address element'")));
...
Функция КонтактнаяИнформацияВФорматеКлючЗначениеВJSON(Адрес)
	...
	ВызватьИсключение НСтр("ru = 'Переданный адрес не может быть преобразован в функции АдресныйКлассификаторСлужебный.КодыАдреса.';
							|en = 'The passed address cannot be converted to functions AddressClassifierInternal.AddressCodes.'");
