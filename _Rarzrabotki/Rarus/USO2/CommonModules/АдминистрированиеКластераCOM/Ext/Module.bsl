			...
			Если ОбщегоНазначения.ЭтоWindowsСервер() Тогда 
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '%1
					           ...
					           |en = '%1
					           |
					           |If component ""comcntr"" is not registered or its version does not match the required one,
					           |register it on the computer %2executing command cmd with increase of UAC privileges
					           |for the OS Windows account on which behalf 1C:Enterprise is running.
					           |
					           |For example,
					           |regsvr32.exe ""%3""'"),
