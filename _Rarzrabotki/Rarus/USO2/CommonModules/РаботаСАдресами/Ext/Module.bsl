...
Функция РегионАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Невозможно определить субъекта РФ, ожидается адрес.';
								|en = 'Cannot determine subject of the RF; address pending.'");
...
Функция ГородАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Невозможно определить город, ожидается адрес.';
								|en = 'Cannot determine city; address pending.'");
...
Процедура ОбработкаЧастыхСокращенийВАдресах(Знач ДанныеАнализа) Экспорт
	...
	СубъектРФ.Вставить(НСтр("ru = 'СПБ';
							|en = 'ST. PETERSBURG'"), "Санкт-Петербург");
...
Функция КодМуниципальногоРайона(Знач ТипМуниципальногоРайона)
	...
	Если СтрСравнить(ТипМуниципальногоРайона, НСтр("ru = 'м.р-н';
													|en = 'm. area'")) = 0 Тогда
			КодМуниципальногоРайона = "1";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, НСтр("ru = 'г.о.';
														|en = 'u. d.'")) = 0 Тогда
			КодМуниципальногоРайона = "2";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, НСтр("ru = 'вн.тер.';
														|en = 'intracity territory'")) = 0
		Или СтрСравнить(ТипМуниципальногоРайона, НСтр("ru = 'вн.тер. г.';
														|en = 'intracity area'")) = 0 Тогда
...
Функция КодПоселения(Знач ТипПоселения)
	...
	Если СтрСравнить(ТипПоселения, НСтр("ru = 'г. п.';
										|en = 'u. s.'")) = 0 Тогда
		КодПоселения = "1";
	ИначеЕсли СтрСравнить(ТипПоселения, НСтр("ru = 'с.п.';
											|en = 'r. s.'")) = 0 Тогда
		КодПоселения = "2";
	ИначеЕсли СтрСравнить(ТипПоселения, НСтр("ru = 'меж.тер.';
											|en = 'interterritorial'")) = 0 Тогда
		КодПоселения = "3";
	ИначеЕсли СтрСравнить(ТипПоселения, НСтр("ru = 'вн.р-н';
											|en = 'intracity district'")) = 0 Тогда
		КодПоселения = "4";
	КонецЕсли;
