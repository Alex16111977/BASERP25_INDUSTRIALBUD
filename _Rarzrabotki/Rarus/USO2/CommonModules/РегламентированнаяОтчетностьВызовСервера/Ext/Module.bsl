...
Функция СформироватьСтруктуруПараметровФайлаВыгрузкиНаСервере(Адрес, ВерФормВыгрузки) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление временного файла регламентированной отчетности';
										|en = 'Delete a temporary file of compliance reporting'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
			...
			Если ОбластьНастройки <> Неопределено Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Регламентированная отчетность';
													|en = 'Compliance reporting'"), УровеньЖурналаРегистрации.Ошибка, , , ПутьКФорме + Символы.ПС + ОписаниеОшибки());
				КонецПопытки;
				ЧтениеJSON.Закрыть();
			КонецЕсли;
		КонецЕсли;
		...
		Если СвойстваВидаОтчета.Свойство("ВерсияФормата") Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка регламентированной отчетности';
												|en = 'Import compliance reporting'"), УровеньЖурналаРегистрации.Ошибка, , , ИсточникОтчета + Символы.ПС + ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Журнал отчетов (статусы): ошибка записи';
											|en = 'Report log (statuses) : writing error'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция НеотправляемыйРеглОтчетИмеетВозможностьИзмененияСтатусаОтправкиИзФормыОтчетаВручную(ИмяОбъекта)
	...
	МассивИмен.Добавить("РегламентированныйОтчетДоходыОтТуристскоРекреационнойДеятельности");
...
Процедура ОпределитьНаличиеКодаПодразделенияВСправочникеОрганыФСГС(Знач Подразделение) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка получения кода ФСГС подразделения';
										|en = 'The error is occurred when the FSO department code  is received'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
#КонецОбласти
