...
#Область Версия_11_4_8

#Область ТМЦВЭксплуатации_ИспользоватьТМЦВЭксплуатации_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ЧитаемыеОбъекты = "Константа.ИспользоватьТМЦВЭксплуатации";
	Обработчик.ИзменяемыеОбъекты = "Константа.ИспользоватьТМЦВЭксплуатации";
...
#Область РегистрыНакопления_ПрочиеРасходы_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = 'Renews the register ""Other expenses""
	|-Fills in the attribute ""Economic transaction"" for records of the document ""Regulated transaction"" (the transaction ""Recognition of leasing payments in NA"").'");
...
#Область РегистрыНакопления_ПартииПрочихРасходов_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|- Заполняет реквизит ""Хозяйственная операция"" для записей документа ""Регламентая операция"" (операция ""Признание в НУ лизинговых платежей"").';
	|en = 'Renews the register ""Batches of other expenses"":
	|- Fills in the attribute ""Economic transaction"" for records of the document ""Regulated transaction"" (the transaction ""Recognition of leasing payments in NA"").'");
...
#Область РегистрыНакопления_РасчетыПоФинансовымИнструментам_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет движения документов по регистру накопления ""Расчеты по финансовым инструментам"".';
									|en = 'Renews document flows upon the register of accumulation ""Payments upon financial instruments"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы.СверкаВзаиморасчетов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "Любой";

#КонецОбласти

#Область Документы_СчетФактураКомитента_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.РеестрДокументов,"
	//++ Локализация
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура полученный от комитента (принципала)"".';
									|en = 'Fills in service attributes of the document ""Tax invoice received from the consignor (principal)"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_СчетФактураКомиссионеру_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура выданный комиссионеру (агенту)"".';
									|en = 'Fills in service attributes of the document ""Tax invoice issued to commissioner (agent)"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_СчетФактураПолученный_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура полученный"".';
									|en = 'Fills in service attributes of the document ""Tax invoice received"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_СчетФактураНалоговыйАгент_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура налогового агента"".';
									|en = 'Fills in service attributes of the document ""Tax invoice of the tax agent"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_СчетФактураПолученныйНалоговыйАгент_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура полученный (налоговый агент)"".';
									|en = 'Fills in service attributes of the document ""Tax invoice received (tax agent)"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_СчетФактураВыданный_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура выданный"".';
									|en = 'Fills in service attributes of the document ""Tax invoice issued"".'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область Документы_ВозвратТоваровМеждуОрганизациями_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = '1) It fills in the method of prime cost specification and the transfer document in product returns between companies in the table part Products. The method of prime cost specification ""From the transfer document"" is installed for strings with the filled transfer documents and the ""From the current document"" for the rest. 
	|2) It fills the CDF number in product returns between companies in the table part Products. If several CDF numbers from the table part StorageTypes match the same string from the table part Products, it splits out strings in the table part Products.
	|You will not be able to edit returns between companies until the processor is completed.'");
...
#Область РегистрыСведений_РеестрДокументов_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет регистр ""Реестр документов"" по данным относящихся к нему документов.';
									|en = 'Fills in the ""Document registry"" register according to the related document data.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	//++ Локализация
...
#Область РегистрыНакопления_ДенежныеСредстваВПути_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрНакопления.ДенежныеСредстваВПути,"
		+ "Документ.ОтчетОРозничныхПродажах";
	Обработчик.ИзменяемыеОбъекты = "РегистрНакопления.ДенежныеСредстваВПути";
	Обработчик.БлокируемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru = 'Обновляет записи регистра накопления ""Денежные средства в пути"" по документам ""Отчет о розничных продажах"".';
									|en = 'Updates the accumulation register entries “Cash in transit” according to the “Retail Sales Report” documents.'");

#КонецОбласти

#Область РегистрыНакопления_РасчетыПоЭквайрингу_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ЧитаемыеОбъекты = "Документ.ОтчетОРозничныхПродажах,"
		+ "РегистрНакопления.РасчетыПоЭквайрингу";
	Обработчик.ИзменяемыеОбъекты = "РегистрНакопления.РасчетыПоЭквайрингу";
	Обработчик.БлокируемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru = 'Обновляет записи регистра накопления ""Расчеты по эквайрингу"" по документам ""Отчет о розничных продажах"".';
									|en = 'Updates the accumulation register entries “Calculations on acquiring” according to the “Retail Sales Report” documents.'");
...
#Область ОбновлениеИнформационнойБазыУТ_ЗадатьНаименованияНаДругихЯзыкахВидовКонтактнойИнформации
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка наименований видов КИ на других языках.';
									|en = 'Setting names of CI kinds in other languages.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	//++ Локализация

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.ВидыКонтактнойИнформации.ЗаполнитьВидыКонтактнойИнформации";
	НоваяСтрока.Порядок = "После";
	//-- Локализация
...
#Область Документы_ВозвратТоваровОтКлиента_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'В возвратах товаров от клиента в табличной части Товары заполняет номер ГТД. В случае если одной строке из табличной части Товары соответствует несколько номеров ГТД из табличной части ВидыЗапасов, то разбивает строки в табличной части Товары. До окончания выполнения обработчика вы не сможете редактировать возвраты товаров от клиента.';
									|en = 'Fills in the CFD number in product returns from the customer in the table part Products. If several CFD numbers from the table part StorageTypes match the same string from the table part Product, it splits out strings in the table part Products. You cannot edit product returns from the customer until the processor is completed.'");

#КонецОбласти

#Область Документы_СверкаВзаиморасчетов_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ИзменяемыеОбъекты = "Документ.СверкаВзаиморасчетов";
	Обработчик.БлокируемыеОбъекты = "Документ.СверкаВзаиморасчетов";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение документов ""Сверка взаиморасчетов"" данными по взаиморасчетам.';
									|en = 'Filling of documents ""Reconciliation of accounts"" with the data upon mutual payments.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область РегистрыНакопления_РасчетыСКлиентамиПоСрокам_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ОчередьОтложеннойОбработки = 2;
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрНакопления.РасчетыСКлиентамиПоСрокам,"
		+ "Справочник.КлючиАналитикиУчетаПоПартнерам,"
		+ "РегистрНакопления.РасчетыСКлиентамиПоДокументам";
	Обработчик.ИзменяемыеОбъекты = "РегистрНакопления.РасчетыСКлиентамиПоСрокам,"
		+ "Справочник.КлючиАналитикиУчетаПоПартнерам";
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправляет движения в регистре взаиморасчетов.';
									|en = 'Исправляет движения в регистре взаиморасчетов.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
...
#Область РегистрыНакопления_РасчетыСПоставщикамиПоСрокам_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрНакопления.РасчетыСПоставщикамиПоСрокам,"
		+ "Справочник.КлючиАналитикиУчетаПоПартнерам,"
		+ "РегистрНакопления.РасчетыСПоставщикамиПоДокументам";
	Обработчик.ИзменяемыеОбъекты = "РегистрНакопления.РасчетыСПоставщикамиПоСрокам,"
		+ "Справочник.КлючиАналитикиУчетаПоПартнерам";
	Обработчик.БлокируемыеОбъекты = "Справочник.КлючиАналитикиУчетаПоПартнерам";
	Обработчик.Комментарий = НСтр("ru = 'Исправляет движения в регистре взаиморасчетов.';
									|en = 'Исправляет движения в регистре взаиморасчетов.'");
...
#Область ОбновлениеИнформационнойБазыУТ_ДополнитьПользовательскиеПрофилиРолямиСчетовФактур
...
#Область Справочники_НастройкиХозяйственныхОпераций_ЗаполнитьПредопределенныеНастройкиХозяйственныхОпераций
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет реквизиты предопределенных элементов справочника  ""Настройки хозяйственных операций"" согласно заданным настройкам в текстовых макетах.';
									|en = 'Updates attributes of predefined items of the ""Business transaction settings"" catalog according to the settings specified in text templates.'");
			...
			Если ДополнительныеПараметры.ЗаписыватьВОднойТранзакции Тогда
				...
				Исключение
					...
					ТекстСообщения = НСтр("ru = 'Не удалось зафиксировать транзакцию записи движения в регистр %ИмяРегистра% по причине: %Причина%';
											|en = 'Cannot register a record write transaction in the %ИмяРегистра% register due to:%Причина%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяРегистра%", ПолноеИмяРегистра);
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось заполнить вид контактной информации для списка у элемента: %Ссылка% по причине: %Причина%';
										|en = 'Cannot fill in contact information kind for the list in item %Ссылка% due to: %Причина%'");
	...
	Пока ОбъектыДляОбработки.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обработать %ИмяОбъекта%: %Ссылка% по причине: %Причина%';
									|en = 'Cannot process %ИмяОбъекта%: %Ссылка%  due to: %Причина%'");
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать вид контактной информации: %1 по причине: %2';
					|en = 'Cannot process contact information kind: %1 due to: %2'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре ЗадатьНаименованияНаДругихЯзыкахВидовКонтактнойИнформации не удалось обработать некоторые виды контактной информации (пропущены): %1';
				|en = 'The ЗадатьНаименованияНаДругихЯзыкахВидовКонтактнойИнформации procedure cannot process some contact information kinds (skipped): %1'"),
				ПроблемныхОбъектов);
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.ВидыКонтактнойИнформации,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Процедура ЗадатьНаименованияНаДругихЯзыкахВидовКонтактнойИнформации обработала очередную порцию видов КИ: %1';
																			|en = 'The ЗадатьНаименованияНаДругихЯзыкахВидовКонтактнойИнформации procedure has processed contact information kinds: %1'"),
					ОбъектовОбработано));
	КонецЕсли;
...
Процедура СообщитьОНеудачнойБлокировке(Ссылка) Экспорт
	ТекстСообщения = НСтр("ru = 'Не удалось заблокировать объект: %Объект% по причине: %Причина%';
							|en = 'Cannot lock the object: %Объект% due to: %Причина%'");
...
Процедура СообщитьОНеудачнойОбработке(ИнформацияОбОшибке, Ссылка) Экспорт
	ТекстСообщения = НСтр("ru = 'Не удалось обработать объект: %Объект% по причине: %Причина%';
							|en = 'Cannot process object %Объект% due to: %Причина%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Объект%", Ссылка);
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
							УровеньЖурналаРегистрации.Предупреждение,
