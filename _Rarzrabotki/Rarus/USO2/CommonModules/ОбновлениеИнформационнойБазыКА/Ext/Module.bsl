...
#Область РегистрыСведений_СостоянияОСОрганизаций_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет движения регистра ""Состояния ОС организаций (версия 2.2)""';
									|en = 'Updates movements of the register ""Company OS conditions (version 2.2)""'");

#КонецОбласти

#Область РегистрыСведений_СчетаБухгалтерскогоУчетаОС_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет движения регистра ""Счета бухгалтерского учета ОС (версия 2.2)""';
									|en = 'Updates movements of the register ""Bookkeeping OS accounts (version 2.2)""'");
