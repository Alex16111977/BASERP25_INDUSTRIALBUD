...
Процедура ЗафиксироватьДлительностьКлючевойОперации(Параметры)
	...
	Если БезопасныйРежим() <> Ложь Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа подсистемы оценка производительности в безопасном режиме не поддерживается';
										|en = 'Performance assessment subsystem is not supported in the safe mode'", КодОсновногоЯзыка),
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаКлючевойОперации) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Невозможно зафиксировать замер с пустой датой начала';
										|en = 'Cannot commit measurement with empty start date'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Информация,,,Строка(КлючеваяОперация));
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось сохранить замеры производительности';
											|en = 'Cannot save performance measurements'", КодОсновногоЯзыка),
...
Процедура ЗагрузитьФайлОценкиПроизводительности(ИмяФайла, АдресХранилища) Экспорт
	...
	Исключение
		...
		ШаблонСтроки = НСтр("ru = 'Не удалось распаковать архив %1 %2';
							|en = 'Cannot unpack the archive %1 %2'");
