...
Функция ПоддерживаетсяУстановкаОбновлений() Экспорт
	...
	Если ПараметрыРаботыКлиента.РазделениеВключено Тогда 
		Результат.ОписаниеОшибки = 
			НСтр("ru = 'Установка обновлений для приложения в Интернете выполняется централизованно через Менеджер сервиса.';
				|en = 'Updates for an application on the Internet are installed centrally through the service manager.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Не ПараметрыРаботыКлиента.ЭтоПолноправныйПользователь Тогда 
		Результат.ОписаниеОшибки = НСтр("ru = 'Для установки обновления требуются права администрирования.';
										|en = 'You need permissions to administer data to install the update.'");
		Возврат Результат;
	КонецЕсли;
...
#Если ВебКлиент Тогда
	Результат.ОписаниеОшибки =
		НСтр("ru = 'Установка обновления не доступна в веб-клиенте.
		           ...
		           |en = 'This update is not available in the web client.
		           |You need to install a full distribution of the 1C: Enterprise technology platform for Windows.'");
	
#ИначеЕсли МобильныйКлиент Тогда
	Результат.ОписаниеОшибки = НСтр("ru = 'Установка обновления доступна только в ОС Windows.';
									|en = 'Installing the update is available only in Windows.'");
#Иначе
	...
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда 
		...
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ НСтр("ru = 'Установка обновления не доступна при подключении через веб-сервер.
			             ...
			             |en = 'The update is not available when connecting via a web server.
			             |You need to configure a direct connection to the information base.'");
	КонецЕсли;
	...
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 1 Тогда
		Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда 
			Возврат НСтр("ru = 'Создавать временную резервную копию ИБ и выполнять откат при нештатной ситуации';
						|en = 'Create temporary IB backup and roll back in case of emergency'");
		Иначе 
			Возврат НСтр("ru = 'Создавать временную резервную копию ИБ и не выполнять откат при нештатной ситуации';
						|en = 'Create temporary IB backup and don''t roll back in case of emergency'");
		КонецЕсли;
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда 
			Возврат НСтр("ru = 'Создавать резервную копию ИБ и выполнять откат при нештатной ситуации';
						|en = 'Create IB backup and roll back in case of emergency'");
		Иначе 
			Возврат НСтр("ru = 'Создавать резервную копию ИБ и не выполнять откат при нештатной ситуации';
						|en = 'Create IB backup and don''t roll back in case of emergency'");
		КонецЕсли;
	Иначе
		Возврат "";
	КонецЕсли;
	...
	Если ПустаяСтрока(КаталогСкрипта) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			...
			НСтр("ru = 'Обновление выполнено с очень старой версии программы. 
			           ...
			           |en = 'Application is updated from very old version. 
			           |Update log data was not imported. See the log
			           |in the %temp% temporary folder having the following form:
			           |1Cv8Update.<xxxxxxxx>(digits).'"),
			, 
			Истина);
	...
	Иначе 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Папка с журналом обновления расположена %1';
					|en = 'Folder with update log is located %1'"), КаталогСкрипта),
			,
			Истина);
	...
	Если ЭтоФайловаяБаза И Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите существующий каталог для сохранения резервной копии ИБ.';
											|en = 'Specify an existing directory to save the IB backup.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.РежимОбновления = 0 Тогда // Обновить сейчас
		...
		Если ЭтоФайловаяБаза И ОбновлениеКонфигурацииВызовСервера.НаличиеАктивныхСоединений(ПараметрыПриложения[ИмяПараметра]) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Невозможно продолжить обновление конфигурации, так как не завершены все соединения с информационной базой.';
											|en = 'The configuration update cannot proceed as connections with infobase were not terminated.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если Параметры.ВыслатьОтчетНаПочту
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите допустимый адрес электронной почты.';
											|en = 'Specify an allowable email address.'"));
			Возврат Ложь;
		КонецЕсли;
		Если Не ПоддерживаетсяПланировщикЗаданий() Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Планировщик заданий поддерживается только начиная с операционной системы версии 6.0 Vista.';
											|en = 'Job scheduler is supported only as of Windows Vista operating system 6.0.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ДатаОбновленияУказанаВерно(Параметры)
	...
	Если Параметры.ДатаВремяОбновления < ТекущаяДата Тогда
		ТекстСообщения = НСтр("ru = 'Обновление конфигурации может быть запланировано только на будущую дату и время.';
								|en = 'Configuration update can be scheduled only for a future date and time.'");
	ИначеЕсли Параметры.ДатаВремяОбновления > ДобавитьМесяц(ТекущаяДата, 1) Тогда
		ТекстСообщения = НСтр("ru = 'Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';
								|en = 'Configuration update can be scheduled not later than in a month from the current date.'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("ru = 'Обновление конфигурации';
				|en = 'Update configuration'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
КонецФункции
	...
	Если НастройкиОбновления.КонфигурацияИзменена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Обновление конфигурации';
			...
			НСтр("ru = 'Конфигурация отличается от основной конфигурации информационной базы.';
				|en = 'The configuration is different from the basic configuration of the infobase.'"), 
...
Процедура ЗапуститьСкриптОбновления(Параметры, ПараметрыАдминистрирования)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ru = 'Выполняется процедура обновления конфигурации:';
			|en = 'Updating configuration:'") + " " + ИмяГлавногоФайлаСкрипта);
