	...
	Для Каждого СтрокаСтруктурныеЕдиницы Из ПодчиненныеСтруктурныеЕдиницы Цикл
		...
		Пока ВыборкаПодразделений.Следующий() Цикл
			...
			Исключение
				...
				|en = 'Cannot change subordinate department %1 when writing %2.
				|Maybe department is being edited by another user.'"),
				ПодразделениеОбъект.Наименование,
				?(ИсточникДанных = Неопределено, НСтр("ru = 'всех подразделений';
														|en = 'all departments'"), ИсточникДанных.Наименование));
 	...
 	Для Каждого Результат Из РезультатыПоиска Цикл
		...
		Если ЭтоСсылочныйОбъектМетаданных(Результат.Метаданные) Тогда
			...
			Для Каждого ТабЧасть Из Результат.Метаданные.ТабличныеЧасти Цикл
				...
				КонецЦикла;
			КонецЦикла;
			// Обработка стандартных табличных частей объекта.
...
Функция КлючевыеРеквизитыПредставлениеПредупреждения(ПредставлениеРеквизита, ДействиеПриИзмененииОчистка)
	...
	ПредставлениеПредупреждения = НСтр("ru = 'Редактирование %1 приведет к %2 табличных частей документа';
										|en = 'Editing of %1 will lead to %2 of tabular sections of the document'");
			...
			Если Не ПолучатьДанныеИзСервиса Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Нормативно-правовая информация БЗК';
						|en = 'PCL regulatory information'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ИзменитьНаименованиеФизическихЛиц(ДанныеФИО)
	...
	Пока ДанныеФИО.Следующий() Цикл
		...
		Исключение
			...
			|en = 'Cannot change individual''s name %1.
...
Функция ЗапросРассчитанныеДаты(МенеджерВременныхТаблиц, ОписаниеРегистра)
	...
	ТекстЗапроса = 
		...
		|						И СостоянияДействующиеНаДаты.Приоритет = 0
		|					ТОГДА ЛОЖЬ
		|				КОГДА СостоянияДействующиеНаДаты.Начало >= СостоянияДействующиеНаДатыВытесняющие.Начало
		|						И СостоянияДействующиеНаДаты.ДатаНачалаСобытия < СостоянияДействующиеНаДатыВытесняющие.ДатаНачалаСобытия
		|						И СостоянияДействующиеНаДатыВытесняющие.Приоритет = 0
		|					ТОГДА ИСТИНА
...
Процедура УстановитьПараметрыНабораСвойств(ПараметрыОбновления = Неопределено) Экспорт
