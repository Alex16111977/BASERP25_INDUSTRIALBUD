...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняется проверка и исправление ошибок регистрации страховые взносы по физическим лицам.';
									|en = 'Checking and correcting errors of registration insurance contributions for individuals. '");
	...
	Обработчик.Комментарий = НСтр("ru = 'Перенос данных о корректировках выплаты';
									|en = 'Transfer data on payment adjustments'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение интервальных данных регистра ""Значения периодических показателей сотрудников""';
									|en = 'Fill in the interval data of the ""Values of periodic employee indicators"" register'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение интервальных данных регистра ""График работы сотрудников""';
									|en = 'Fill in interval data of the ""Employee work schedule"" register'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение даты окончания в регистре ""Периоды стажа ПФР"" по договорам ГПХ';
									|en = 'Fill in end date in the ""PF service periods"" register under civil law contracts'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление регистрации доходов по договорам в учете страховых взносов.';
									|en = 'Update income registration under contracts in insurance contribution accounting.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение поля ""Доначислить зарплату при необходимости"" в документах.';
									|en = 'Fill in the ""Accrue the salary additionally if necessary"" field in documents.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.9.251";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Процедура = "РегистрыСведений.ПодчиненностьПодразделенийОрганизаций.ПереЗаполнитьПодчиненностьПодразделений";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("17a7da11-1c2a-43a4-b473-0a56b5168f10");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.8.33";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыЗарплатаКадрыРасширенный.УстановитьПараметрыНабораСвойствСтатьиРасходовЗарплата";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЛьготыСотрудников") Тогда
		...
		МодульЛьготыСотрудников.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		...
		МодульУправленческаяЗарплата.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПроизвольныеКадровыеПриказы") Тогда
		...
		МодульПроизвольныеКадровыеПриказы.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.БазовыеСправочныеДанные.Контрагенты") Тогда
		...
		МодульКонтрагентыСлужебный.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.Чатботы") Тогда
		...
		МодульЧатботы.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.РабочийКалендарь") Тогда
		МодульРаботаСРабочимКалендаремБЗК = ОбщегоНазначения.ОбщийМодуль("РаботаСРабочимКалендаремБЗК");
		МодульРаботаСРабочимКалендаремБЗК.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ИнтеграцияРекрутинговыхСайтов") Тогда
		МодульИнтеграцияРекрутинговыхСайтовРедакция3 = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияРекрутинговыхСайтовРедакция3");
		МодульИнтеграцияРекрутинговыхСайтовРедакция3.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	КонецЕсли;
...
Процедура УстановитьОсновнойОтпуск() Экспорт
	...
	Если ОсновнойОтпускСсылка = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не обнаружен предопределенный элемент Справочник.ВидыОтпусков.Основной';
								|en = 'Predefined item Справочник.ВидыОтпусков.Основной is not found'");
	КонецЕсли;
...
Процедура ОбновитьВидНачисленияДляПособияНаПогребениеСотруднику() Экспорт
	...
	|	БухучетНачисленияУдержанияПоСотрудникам.ДанныеМежрасчетногоПериода,
