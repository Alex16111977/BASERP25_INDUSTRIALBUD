...
Процедура СформироватьДвиженияДенежныеСредстваКонтрагентПоКлиентам(МассивДокументов) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Денежные средства - Контрагент""
						...
						|when performing settlements with customers due to'") + ":" + Символы.ПС + "%Причина%";
...
Процедура СформироватьДвиженияДенежныеСредстваКонтрагентПоПоставщикам(МассивДокументов) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Денежные средства - Контрагент""
						...
						|en = 'Cannot execute deferred posting for the ""Records - Cash - Counterparty"" register
						|when performing settlements with suppliers due to'") + ":" + Символы.ПС + "%Причина%";
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка, , ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
...
Процедура СформироватьДвиженияКонтрагентКонтрагент(МассивДокументовРасчетов) Экспорт
	...
	Попытка
		...
		//++ НЕ УТ
		Иначе
			ДокументыДебета  = РасчетыСПартнерами[3].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
			ДокументыКредита = РасчетыСПартнерами[4].Выгрузить();
		//-- НЕ УТ
		КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Контрагент - Контрагент""
									|по причине: %Причина%';
									|en = 'Cannot execute deferred posting for the ""Records - Counterparty - Counterparty"" register
									|due to: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отложенное проведение взаимозачета';
				|en = 'Deferred offset'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура СформироватьДвиженияКонтрагентДоходыРасходы(МассивДокументовРасчетов) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить отложенное проведение по регистру ""Движения Контрагент - Доходы/Расходы""
									|по причине: %Причина%';
									|en = 'Cannot execute deferred posting for the ""Records - Counterparty - Income/Expenses"" register
									|due to: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отложенное проведение взаимозачета';
...
#Область ТекстЗапросаРезервыПоСомнительнымДолгам
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ  втРегистраторы КАК Т
	|			ПО Резервы.Регистратор = Т.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК КлючиАналитики
	|			ПО Резервы.АналитикаУчетаПоПартнерам = КлючиАналитики.КлючАналитики
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Резервы.Регистратор) = ТИП(Документ.СписаниеЗадолженности)
	|
	|СГРУППИРОВАТЬ ПО
	|	Резервы.Период,
	|	Резервы.ВидДвижения,
...
//Документ СписаниеЗадолженности
		...
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ОбъектРасчетов,
		|	РасчетыСПартнерами.Валюта,
		|	ЕСТЬNULL(Резервы.Сумма, РасчетыСПартнерами.Сумма) КАК СуммаВВалюте,
		|	ЕСТЬNULL(Резервы.СуммаУпр, РасчетыСПартнерами.СуммаУпр) КАК Сумма,
		|	ЕСТЬNULL(Резервы.СуммаРегл, РасчетыСПартнерами.СуммаРегл) КАК СуммаРегл,
		|	РасчетыСПартнерами.ТипСуммы КАК ТипСуммы,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеБезнадежнойЗадолженностиЗаСчетРезервовПоСомнительнымДолгам),
		|	РасчетыСПартнерами.КонтрагентОрганизация КАК КонтрагентОрганизация,
		|	НЕОПРЕДЕЛЕНО КАК Подразделение,
		|	РасчетыСПартнерами.Валюта КАК ВалютаВзаиморасчетов,
		|	ЕСТЬNULL(Резервы.СуммаРегл, РасчетыСПартнерами.СуммаРегл) / 
		|		ВЫБОР КОГДА ЕСТЬNULL(Резервы.Сумма, РасчетыСПартнерами.Сумма) = 0 
		|			ТОГДА ВЫБОР КОГДА ЕСТЬNULL(Резервы.СуммаРегл, РасчетыСПартнерами.СуммаРегл) = 0 
		|							ТОГДА 1 
		|							ИНАЧЕ ЕСТЬNULL(Резервы.СуммаРегл, РасчетыСПартнерами.СуммаРегл)
		|					КОНЕЦ
		|			ИНАЧЕ ЕСТЬNULL(Резервы.Сумма, РасчетыСПартнерами.Сумма)
		|		КОНЕЦ КАК КурсВалютыВзаиморасчетов,
		|	ЕСТЬNULL(Резервы.Сумма, РасчетыСПартнерами.Сумма) КАК СуммаВВалютеВзаиморасчетов,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ИсточникГФУРасчетов
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		...
		|ГДЕ
		|	РасчетыСПартнерами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И ТИПЗНАЧЕНИЯ(РасчетыСПартнерами.Регистратор) = ТИП(Документ.СписаниеЗадолженности)
		|
		|УПОРЯДОЧИТЬ ПО
...
//Документ СписаниеЗадолженности
		...
		|	ЕСТЬNULL(Резервы.СуммаУпр, РасчетыСПартнерами.СуммаУпр) КАК Сумма,
		|	ЕСТЬNULL(Резервы.СуммаРегл, РасчетыСПартнерами.СуммаРегл) КАК СуммаРегл,
		|	ЕСТЬNULL(Резервы.ТипСуммы, РасчетыСПартнерами.ТипСуммы) КАК ТипСуммы,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеБезнадежнойЗадолженностиЗаСчетРезервовПоСомнительнымДолгам),
		|	НЕОПРЕДЕЛЕНО КАК Подразделение,
		|	РасчетыСПартнерами.Валюта КАК ВалютаВзаиморасчетов,
		|	ЕСТЬNULL(Резервы.СуммаРегл, РасчетыСПартнерами.СуммаРегл) / 
...
Функция ТекстЗапросаДвиженияКонтрагентДоходыРасходы()
	...
	#Область ТекстЗапросаДенежныеСредстваКонтрагент
	...
	|
	|ВЫБРАТЬ
	|	РезервыПоДолгам.Регистратор,
	|	АналитикаПартнеров.Организация,
	|	АналитикаПартнеров.Партнер,
	|	АналитикаПартнеров.Контрагент,
	|	АналитикаПартнеров.Договор,
	|	АналитикаПартнеров.НаправлениеДеятельности,
	|	РезервыПоДолгам.ОбъектРасчетов,
	|	СУММА(РезервыПоДолгам.Сумма) КАК Сумма,
	|	СУММА(РезервыПоДолгам.СуммаУпр) КАК СуммаУпр,
	|	СУММА(РезервыПоДолгам.СуммаРегл) КАК СуммаРегл
	|ИЗ
	|	РегистрНакопления.РезервыПоСомнительнымДолгам КАК РезервыПоДолгам
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаПартнеров
	|	ПО РезервыПоДолгам.АналитикаУчетаПоПартнерам = АналитикаПартнеров.КлючАналитики
	|
	|СГРУППИРОВАТЬ ПО
	|	РезервыПоДолгам.Регистратор,
	|	АналитикаПартнеров.Организация,
