...
//	* ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение - Строка - значения: Всегда, Никогда, ПоНастройкамДоговора - если по остаткам подобрался вид запасов ТоварыПринятыеНаХранение
//															то в зависимости от флага он или подставится в ТЧ, или подставится собственный, а по ТоварыПринятыеНаХранение
//															расходный сформируется резерв.
			...
			Иначе
				Если ОбновлениеИнформационнойБазы.ЭтоВызовИзОбработчикаОбновления("Отложенно") Тогда
					ВызватьИсключение НСтр("ru = 'Создание новых видов запасов в режиме отложенного обновления запрещено.';
											|en = 'Cannot create new inventory kinds in deferred update mode.'");
				КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	...
	Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков <> "ТоварыОрганизаций"
		...
		ТекстИсключения = НСтр("ru = 'Процедуру ЗаполнитьВидыЗапасовПоТоварамОрганизаций можно вызывать только с таблицей остатков ТоварыОрганизаций или ПереданныеМеждуОрганизациямиТовары';
								|en = 'The ЗаполнитьВидыЗапасовПоТоварамОрганизаций procedure can be called only with the ТоварыОрганизаций balance table or with ПереданныеМеждуОрганизациямиТовары'");
...
Процедура ЗаполнитьВидыЗапасовПоОстаткамКОформлению(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	...
	Если ПараметрыЗаполненияВидовЗапасов.ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию = Неопределено Тогда
		ТекстИсключения = НСтр("ru = 'В вызывающем коде не задано значение параметра ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию.';
								|en = 'Value of the ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию parameter is not specified in the called code.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков = "ПустаяТаблицаОстатков"
		И Не ПараметрыЗаполненияВидовЗапасов.ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию Тогда
		ТекстИсключения = НСтр("ru = 'ПустаяТаблицаОстатков нужна для заполнения видов запасов по умолчанию, поэтому ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию должен быть выстален в ИСТИНА.';
								|en = 'ПустаяТаблицаОстатков is needed to fill in the types of stocks by default, therefore, ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию should be set to TRUE.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
...
Процедура ПроведениеДокументовПоРегиструСебестоимостьТоваров(Параметры="", АдресХранилища="") Экспорт
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Запасы сервер.Проведение документов по регистру себестоимость товаров';
			|en = 'Inventory server.Document posting for the goods cost register'", КодЯзыка), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Начато перепроведение документов по регистру ""Себестоимость товаров""';
			|en = 'Reposting of documents for the ""Goods cost"" register has started'", КодЯзыка), 
	...
	Пока Выборка.Следующий() Цикл
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заполнена колонка ""Номер ГТД"" в строке %1 списка ""%2""';
				|en = 'The ""CCD number"" column is not populated in line %1 of the ""%2"" list'"),
			Выборка.НомерСтроки,
			ПредставлениеТЧ);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
...
Процедура ПроверитьНаличиеИзделийВШапкеСУчетомПоГТД(ДокументОбъект, Отказ) Экспорт
	...
	Если РеквизитыИзделия.ВестиУчетПоГТД = Истина
		...
		ШаблонСообщения = НСтр("ru = 'В документе выбрана номенклатура, для которой ведется учет по ГТД.
							|При включенной опции ""Запретить оформление операций с импортными товарами без номеров ГТД"" выбор такой номенклатуры запрещен.';
							|en = 'Products with accounting by CCD are selected in the document.
							|If the ""Prohibit registration of operations with imported goods without CCD numbers"" option is selected, you cannot select these products.'");
			...
			Иначе
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'ИсправлениеОтрицательныхОстатков.Ошибка исправления остатков';
						|en = 'ИсправлениеОтрицательныхОстатков.An error occurred while correcting inventory'",
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'ИсправлениеОтрицательныхОстатков.Ошибка перепроведения документа';
					|en = 'ИсправлениеОтрицательныхОстатков.An error occurred while reposting the document'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
			...
			Пока ВыборкаВидЗапасов.Следующий() Цикл
				...
				Пока ВыборкаНомерГТД.Следующий() Цикл
					...
					Если ОтрицательныйОстаток + ПокрывающийРезерв > 0 Тогда
						...
						|en = 'While writing %Регистратор% to the ТоварыОрганизаций and РезервыТоваровОрганизаций registers,
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение = ПараметрыЗаполненияВидовЗапасов.ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение;
	...
	Если ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) <> ВРег("Всегда")
		...
		ТекстИсключения = НСтр("ru = 'В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение.';
								|en = 'In the types of stocks parameters it is not defined whether to insert the goods accepted for responsible storage into the tabular part or not.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
			...
			Если ЗначениеЗаполнено(Выборка.ВидЗапасов) Тогда
				...
				ИначеЕсли Выборка.НастройкаПередачи = Перечисления.СпособыПередачиТоваров.ПередачаНаКомиссию
					...
					Иначе
						...
						НалогообложениеКомиссионногоВидаЗапасов = Справочники.Организации.НалогообложениеНДС(Выборка.НовыйВидЗапасовВладелецТовара,
																	Неопределено, // Склад
																	ДокументОбъект.Дата);
					КонецЕсли;
...
Функция ЗапросРаспределенияРезервовПоТаблицеОстатков(МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Ключи
	|			ПО (Ключи.Ссылка = СторноПриходаРезерва.АналитикаУчетаНоменклатуры)
	|	ГДЕ
...
Процедура ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ИсходнаяТаблицаСторноДвижений, РезультирующаяТаблицаСторноДвижений, ГлубинаРекурсии)
	...
	Если ГлубинаРекурсии > 100 Тогда
		...
		ТекстИсключения = НСтр("ru = 'Превышена максимальная глубина рекурсии при формировании сторно резервов по расходу. Сохраните текущее состояние информационной базы и передайте ее разработчикам программы.';
								|en = 'The maximum recursion depth is exceeded while generating expense reserve storno. Save the current infobase status and pass it to the application developers.'");
		ВызватьИсключение ТекстИсключения;
	...
	Если ДокументОбъект.Движения.Найти("ТоварыОрганизаций") <> Неопределено Тогда
		...
		Если ДокументОбъект.Движения.Найти("РезервыТоваровОрганизаций") = Неопределено Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка в описании движений документа: документ является регистратором ТоварыОрганизаций и не является регистратором РезервыТоваровОрганизаций.';
									|en = 'An error occurred in the document record description: the document is the ТоварыОрганизаций recorder and is not the РезервыТоваровОрганизаций recorder.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Функция ЗапросРаспределенияБезУчетаПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц, ДополнительныеСвойства, ПараметрыЗаполнения)
	...
	|	ВЫБОР 
	|		КОГДА &ИспользоватьГруппыФинУчета
	|		ТОГДА ЕСТЬNULL(ТаблицаДокумента.АналитикаУчетаНоменклатуры.Номенклатура.ГруппаФинансовогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК ГруппаФинансовогоУчета
	|ИЗ
...
Процедура ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(МенеджерВременныхТаблиц, ТекстЗапроса, ИмяТаблицы, ИмяПоля, ТекстВыражениеВыборки, ТекстЗначенияПоУмолчанию)
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД% %НалогообложениеНДС%';
									|en = 'By company %Организация% there is not enough %Количество% %ЕдиницаИзмерения% goods %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД% %НалогообложениеНДС%'"); 
			...
			Если КонтролироватьНалогообложениеНДС(ПараметрыЗаполненияВидовЗапасов) Тогда
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НалогообложениеНДС%", НСтр("ru = 'по налогообложению';
																							|en = 'by taxation'") + " " + ПараметрыЗаполненияВидовЗапасов.ОтборыВидовЗапасов.НалогообложениеНДС);
			Иначе
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.Склад)
				...
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% на сумму %Сумма%%Валюта% товара: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%';
										|en = '%ТипДвижения% %Количество% %ЕдиницаИзмерения% of goods to the amount of %Сумма%%Валюта% are missing in the %Организация% company: %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% %КонтрольГТД%'");
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицы.Сумма) Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %ТипДвижения% %Количество% %ЕдиницаИзмерения% на сумму %Сумма%%Валюта% товара: %ПредставлениеНоменклатуры% %КонтрольГТД%';
										|en = '%ТипДвижения% %Количество% %ЕдиницаИзмерения% of goods to the amount of %Сумма%%Валюта% are missing in the %Организация% company: %ПредставлениеНоменклатуры% %КонтрольГТД%'");
			Иначе
...
Процедура СообщитьОКонтролеНалогообложенияПриПодбореВидовЗапасов(Знач ДокументОбъект, Знач ПараметрыЗаполненияВидовЗапасов)
	...
	Если КонтролироватьНалогообложениеНДС(ПараметрыЗаполненияВидовЗапасов) Тогда
		...
		Если НалогообложениеНДСДокумента <> Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД 
			...
			|en = 'Note that the ""%НалогообложениеНДС%"" taxation type is not supported by company policy. In this case, only consigned goods can be written off.'");
...
Функция НаименованиеМестаХраненияДляСообщенияОбОшибкахКонтроляОстатковПоОрганизациям(СтрокаТаблицы, ХозяйственнаяОперация = Неопределено)
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'В процедуре НаименованиеМестаХраненияДляСообщенияОбОшибкахКонтроляОстатковПоОрганизациям не учтен тип места хранения ""%1""';
								|en = 'The ""%1"" storage location type is not recorded in the StorageLocationNameForReportAboutBalanceControlByCompaniesError procedure'");
...
Процедура СформироватьВТТаблицаОстатков(ИмяТаблицыОстатков,
	...
	Если ИмяТаблицыОстатков = "ТоварыОрганизаций" Тогда
		Если ВычитатьДвиженияПоТоварамОрганизаций = Неопределено Тогда
			ТекстИсключения = НСтр("ru = 'Ошибка формирования таблицы остатков товаров организаций: не передан параметр ВычитатьДвиженияПоТоварамОрганизаций.';
									|en = 'An error occurred while generating company stock balance: the ВычитатьДвиженияПоТоварамОрганизаций parameter is not transferred.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
	...
	ИначеЕсли ИмяТаблицыОстатков = "ПустаяТаблицаОстатков" Тогда
		ТаблицаОстатковПустая(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов);
...
Процедура ТаблицаОстатковРеализованныхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	РеализованныеТовары.АналитикаУчетаНоменклатурыОтгрузки.СкладскаяТерритория КАК СкладОтгрузки,
	...
	|	РеализованныеТовары.АналитикаУчетаНоменклатурыОтгрузки.СкладскаяТерритория,
	...
	|	ТаблицаВидыЗапасов.АналитикаУчетаНоменклатурыОтгрузки.СкладскаяТерритория,
...
// Процедура формирует пустую таблицу остатков - для того, чтобы отрабатывало только заполнение видов запасов по умолчанию
//
Процедура ТаблицаОстатковПустая(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	Для Каждого СтрПереданных Из ТаблицаОстатковПереданныхТоваров Цикл
		...
		Для Каждого СтрОстатков Из СтрокиОстатковТоваровОрганизаций Цикл
			...
			Иначе
				...
				КоличествоКРаспределению = КоличествоКВычету - СтрОстатков.КоличествоОстаток;
			КонецЕсли;
		КонецЦикла;
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Период > ДобавитьМесяц(ТекущаяДатаСеанса(), 12) Тогда
			...
			ТекстИсключения = НСтр("ru = 'Документ %Регистратор% проведен по регистру ТоварыОрганизаций с датой, которая больше текущей на 12 месяцев. Отмените проведение этого документа и исправьте дату.';
									|en = '%Регистратор% Document is posted on the register CompanyGoods with a date that is more than the current by 12 months. Cancel the posting of this document and correct the date.'");
...
Процедура ПередУдалениемРегистратораПроверитьНаличиеЗаписейВРезервахТоваровОрганизаций(Источник, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ТекстИсключения = НСтр("ru = 'По документу %Документ% есть движения в регистре РезервыТоваровОрганизаций. Удаление документа невозможно.';
								|en = 'There are records in the РезервыТоваровОрганизаций register by the %Документ% document. Cannot delete the document.'");
