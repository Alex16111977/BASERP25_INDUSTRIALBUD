...
Процедура СформироватьКомплектПечатныхФорм(МассивОбъектов,
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.ПечатьТранспортнойНакладной.СформироватьПечатнуюФормуТранспортнойНакладной(КомплектПечати.Объекты,
																								ОбъектыПечати));
	КонецЦикла;
...
//	Структура - структура с данными для печати формы ТОРГ - 12.
//
...
//
// Возвращаемое значение:
//	Структура - структура с данными для печати формы МХ - 1.
//
Функция ПолучитьДанныеДляПечатнойФормыМХ1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДокументПередачи.Номер       КАК Номер,
	|	ДокументПередачи.Дата        КАК Дата,
	|	ДокументПередачи.Дата        КАК ДатаДокумента,
	...
	|	ВЫБОР
	|		КОГДА ВтШапка.Валюта = &ВалютаРегл
	|			ТОГДА Товары.Цена
...
#Область ПередачаХранителюСобственныхТоваров // (Дт 45.01 :: Кт 41.01)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Передача хранителю собственных товаров (Дт 45.01 :: Кт 41.01)
	...
	|	""Передача товаров хранителю"" КАК Содержание
	|
	|ИЗ
...
#Область ПередачаХранителюСкладПроизводства // (Дт 45.01 :: Кт 20)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Передача хранителю с производственного склада (Дт 45.01 :: Кт 20)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ПередачаТоваровПринятыхНаОтветХранение // (Дт 002.03 :: Кт 002.01)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Передача товаров, принятых на ответственное хранение (Дт 002.03 :: Кт 002.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТМЦУХранителей) КАК СчетДт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Передача товаров, принятых на ответственное хранение"" КАК Содержание
	|
	|ИЗ 
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
