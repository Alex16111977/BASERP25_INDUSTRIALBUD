	...
	Если НеобходимостьОбработки.ОбработатьСН Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вы отменили применение северных надбавок для организации ""%1"". 
						...
						|Delete northern allowances?'"), ДанныеОрганизации.Наименование);
...
Функция ОрганизацияЗаполнена(ПроверяемыйОбъект) Экспорт
	...
	Если Не ОрганизацияЗаполнена Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для заполнения документа необходимо выбрать организацию.';
										|en = 'To populate the document, select a company.'"), , НСтр("ru = 'Ошибка заполнения';
																												|en = 'Population error'"));
	КонецЕсли;
	...
	Если ДополнительныеПараметры.НеобходимостьОбработки.ОбработатьРК Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вы отменили применение районного коэффициента для организации ""%1"". 
						...
						|The regional factor rate will be deleted for all departments of this company.
						|The previous calculation results will be saved but regional factor supplement will not be made later.
						|Delete regional factors?'"), ДополнительныеПараметры.ДанныеОрганизации.Наименование);
