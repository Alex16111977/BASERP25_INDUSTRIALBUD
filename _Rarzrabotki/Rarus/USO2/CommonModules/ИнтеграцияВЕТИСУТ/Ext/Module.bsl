...
Функция ЗаполнитьСгенерироватьСерии(ТабличнаяЧастьТовары,
	...
	Для Каждого СтрТабл из ТаблицаУстановкиСерий Цикл
		...
		Если СтрТабл.ЕстьУточнения Тогда
			СтрокаТЧ = ТабличнаяЧастьУточнения.Найти(СтрТабл.НомерСтрокиУточнения, "НомерСтроки");
		Иначе
			СтрокаТЧ = ТабличнаяЧастьТовары.Найти(СтрТабл.НомерСтрокиТовары, "НомерСтроки");
		КонецЕсли;
