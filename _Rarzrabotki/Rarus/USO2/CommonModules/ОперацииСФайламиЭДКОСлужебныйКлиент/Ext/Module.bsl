		...
		Если НЕ ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
			Если СтрДлина(Имя) > 260 Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Длина пути одного или нескольких файлов превышает 260 символов.
                                               ...
                                               |en = 'Длина пути одного или нескольких файлов превышает 260 символов.
                                               |Файлы не могут быть сохранены.
                                               |Выберите другой каталог сохранения файлов.'"));
				Возврат;
			КонецЕсли;
...
Процедура ДобавитьФайлыПослеПомещенияФайлов(ПомещенныеФайлы, ВходящийКонтекст) Экспорт
	...
	Если ЗначениеЗаполнено(ПомещенныеФайлы) Тогда
		Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
			ОписанияФайлов.Добавить(ПолучитьОписаниеФайла(ПомещенныйФайл.Имя, ПомещенныйФайл.Хранение));
		КонецЦикла;	
	КонецЕсли;
...
Процедура ДобавитьФайлыПослеВыполнения(ОписанияФайлов, ВходящийКонтекст)
	...
	// Внтури процедуры к описанию файлов добавляются дополнительные свойства. Не переносить.
...
Функция ТекстОшибкиДобавленияОдногоФайла(ФайлыДокумента, Знач МаксимальныйРазмерФайла, Знач ДопустимыеТипыФайлов) Экспорт
	...
	Если ЗначениеЗаполнено(ДопустимыеТипыФайлов) Тогда
		ТипыФайлов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(ДопустимыеТипыФайлов);
		
		Для Каждого ОписаниеФайла Из ФайлыДокумента Цикл
			Если ТипыФайлов.Найти(ОписаниеФайла.Расширение) = Неопределено Тогда
...
Процедура ДобавитьФайлыСПредпросмотром(ОповещениеОЗавершении, ИдентификаторФормы, Заголовок, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.Файлы) Тогда
		Оповещение = Новый ОписаниеОповещения("ДобавитьФайлыСПредпросмотромПослеПоказаФайлов", ЭтотОбъект, Контекст);
		ДобавитьФайлыСПредпросмотромОткрытьФормуПредпросмотра(Оповещение, Контекст);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ДобавитьФайлыСПредпросмотромПослеВыбораФайлов", ЭтотОбъект, Контекст);
