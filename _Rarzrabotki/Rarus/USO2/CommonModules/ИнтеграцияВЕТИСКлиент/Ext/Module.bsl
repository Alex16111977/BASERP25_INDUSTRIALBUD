...
Функция ИменаКолонокДальнейшиеДействия()
	...
	Результат.Добавить("ДальнейшееДействиеВЕТИС1");
	Результат.Добавить("ДальнейшееДействиеВЕТИС2");
	Результат.Добавить("ДальнейшееДействиеВЕТИС3");
	Возврат Результат;
...
Процедура ПодготовитьСообщенияКПередаче(ДинамическийСписок, ПараметрыПередачи, ПравоИзменения) Экспорт
	...
	Контекст = ИнтеграцияИСКлиент.СтруктураПодготовкиСообщенийКПередаче(ДинамическийСписок, ПараметрыПередачи.ДальнейшееДействие, ЭтотОбъект);
...
Функция ВыборСтрокиДинамическогоСпискаКорректен(Форма, ИмяСписка, ПоТекущейСтроке = Ложь, СообщатьОшибкуПользователю = Истина) Экспорт
	...
	Если СообщатьОшибкуПользователю Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Операция недоступна для выбранного объекта';
																|en = 'Операция недоступна для выбранного объекта'"),,ИмяСписка);
	КонецЕсли;
	...
	Если НЕ СобытиеОбработано Тогда
		...
		УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"": некорректное имя события оповещения ВетИС ""%2""';
				|en = '""%1"": некорректное имя события оповещения ВетИС ""%2""'"),
			Форма.ИмяФормы,
			ИмяСобытия);
