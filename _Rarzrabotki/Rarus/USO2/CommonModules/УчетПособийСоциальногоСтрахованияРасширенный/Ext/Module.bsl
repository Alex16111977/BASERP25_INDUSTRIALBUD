...
Процедура ПередСкрытиемПерсональныхДанныхСубъектов(Субъекты, ТаблицаИсключений, Отказ) Экспорт
	...
	ПричинаОтменыСкрытия = НСтр("ru = 'субъект получал пособия социального страхования';
								|en = 'the subject received social insurance allowances'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ЗаполнитьПоказателиРасчетаПособий(ТаблицаПоказателей) Экспорт
	...
	НоваяСтрока = ТаблицаПоказателей.Добавить();
	НоваяСтрока.СпособРасчета = Перечисления.СпособыРасчетаНачислений.ОплатаДнейУходаЗаДетьмиИнвалидами;
	НоваяСтрока.Показатель = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.УчитыватьМРОТ");
КонецПроцедуры
...
Процедура РассчитатьПособиеПоСтрокамНабора(СпособРасчета, СтрокиРасчета, СтрокиНабора, УсловияОплатыПособий, ДополнительныеПараметры) Экспорт
	...
	РассчитатьОплатуДнейУходаЗаДетьмиИнвалидвами(СпособРасчета, СтрокиРасчета);
КонецПроцедуры	
...
Процедура РассчитатьПособиеДоПолутораЛетПоСтрокамНабора(СпособРасчета, СтрокиРасчета, СтрокиНабора, УсловияОплатыПособий, ДополнительныеПараметры)
	...
	Для Каждого СтрокаТаблицы Из СтрокиРасчета Цикл	
		...
		Если ТекущиеУсловияОплаты.СреднийДневнойЗаработок > ТекущиеУсловияОплаты.МинимальныйСреднедневнойЗаработок Тогда
...
Процедура РассчитатьПособиеДоПолутораЛетНезастрахованнымПоСтрокамНабора(СпособРасчета, СтрокиРасчета, СтрокиНабора, УсловияОплатыПособий, ДополнительныеПараметры)
	...
	Для Каждого СтрокаТаблиц Из СтрокиРасчета Цикл
		СтрокаНабора = СтрокиНабора[СтрокаТаблиц.ИдентификаторСтроки]; 
		...
		Если ТекущиеУсловияОплаты = Неопределено 
			...
			ТекстСообщения = Нстр("ru = '%1; %2: Не указано количество детей, за которыми производится уход. Возможно не зарегистрирован отпуск по уходу за ребенком.';
									|en = '%1; %2: Number of children who are cared for is not specified. Maybe, child care leave is not registered.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СтрокаТаблиц.Сотрудник, СтрокаТаблиц.ВидРасчета);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Продолжить;
		КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Восстановление данных дней болезни, ухода за детьми.';
									|en = 'Restore data on sick days, child care days.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Восстановление счетчика страховых случаев для больничных листов продолжительностью до 3 дней и причинами нетрудоспособности 03-10 и 12-15.';
									|en = 'Restore insured event counter for sick leave records for up to 3 days and with 03-10 and 12-15 reasons for incapacity for work.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка параметров для набора свойств справочников.';
									|en = 'Set parameters for catalog property set.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Фиксация информации о стаже в реестрах сведений, необходимых для назначения и выплаты пособий.';
									|en = 'Record information about length of service in the information registers required to assign and pay allowances.'");
...
Процедура ЗаполнитьЗначенияПоказателейРасчетаЗарплатыПоТаблицеЗначений(ДополнительныеПоказатели, ТаблицаПоказателей) Экспорт
	...
	Для Каждого СтрокаПоказателя Из ДополнительныеПоказатели Цикл
		...
		ДополнительныеПараметры.ОкончаниеПериода = ПериодРасчета.ДатаОкончания;
		ДополнительныеПараметры.ПорядокРасчета = ПорядокРасчета;
		ДанныеДляРасчета = ДанныеДляРасчетаСреднегоЗаработкаФСССотрудника(СтрокаПоказателя.Сотрудник, ДополнительныеПараметры);
...
// Для участников проекта по прямым выплатам ФСС очищает результат расчета начислений за счет ФСС.
Процедура ПослеРасчетаНачисленийПоДаннымНабора(ТаблицаНабора, Организация, ПериодРегистрации, СтрокиПоИдентификатору, СвойстваНачислений) Экспорт
	Если Не ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(Организация, ПериодРегистрации) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаНабора Из ТаблицаНабора Цикл
		СвойстваТекущегоНачисления = СвойстваНачислений[СтрокаНабора.ВидРасчета];
...
Функция ПредупреждениеОРасчетеИзМинимальногоСреднегоЗаработка(МинимальныйСреднедневнойЗаработок)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пособие рассчитывается с использованием среднедневного заработка из МРОТ: %1 р.';
																		|en = 'The allowance is calculated using average daily earnings from minimum wage rate: %1'"), МинимальныйСреднедневнойЗаработок);
...
Процедура РассчитатьОплатуДнейУходаЗаДетьмиИнвалидвами(СпособРасчета, СтрокиРасчета, МенеджерВременныхТаблиц = Неопределено)
	...
	Для Каждого РасчитываемаяСтрока Из СтрокиРасчета Цикл
		...
		Если ВремяВДнях = Неопределено Тогда
			ВремяВДнях = 0;
		КонецЕсли;
		НормаДней = РасчетЗарплаты.ЗначениеПоказателяПоИдентификатору(РасчитываемаяСтрока.Показатели, "НормаДней");
		Если НормаДней = Неопределено Тогда
			НормаДней = 0;
		КонецЕсли;
		МРОТ = РасчетЗарплаты.ЗначениеПоказателяПоИдентификатору(РасчитываемаяСтрока.Показатели, "МРОТ");
		Если МРОТ = Неопределено Тогда
			МРОТ = 0;
		КонецЕсли;
		УчитыватьМРОТ = РасчетЗарплаты.ЗначениеПоказателяПоИдентификатору(РасчитываемаяСтрока.Показатели, "УчитыватьМРОТ");
		...
		Если РасчитываемаяСтрока.ВремяВЧасах = Истина Тогда
			...
			РасчитываемаяСтрока.Результат = Макс(СреднийЗаработокОбщий * НормаДней * ПродолжительностьДня, УчитыватьМРОТ * МРОТ) / (НормаДней * ПродолжительностьДня) * КоэффициентИндексацииСреднегоЗаработка * ПродолжительностьДня * ВремяВДнях;
			РасчитываемаяСтрока.ОплаченоЧасов = ПродолжительностьДня * ВремяВДнях;
		Иначе
			РасчитываемаяСтрока.Результат = Макс(СреднийЗаработокОбщий * НормаДней, УчитыватьМРОТ * МРОТ) / НормаДней * КоэффициентИндексацииСреднегоЗаработка * ВремяВДнях;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
