...
Процедура ВыполнитьКонтрольРезультатовПроведения(ЗаявкаОбъект, Отказ) Экспорт
	...
	//++НЕ УТ
	ИспользоватьЛимитыБюджетирования  = ПолучитьФункциональнуюОпцию("ИспользоватьЛимитыРасходаДенежныхСредствБюджетирования");
	//--НЕ УТ
	...
	//++НЕ УТ
	Если ИспользоватьЛимитыБюджетирования Тогда
...
Функция ПланФактЛимитов(ДатаПроверки, Знач Документ = Неопределено, Знач ВалютаЗаявки = Неопределено, ОтборРасшифровки = Неопределено) Экспорт
	...
	Если ПродолжитьРасчет Тогда
		...
		Если ВозможенКонтрольПоДокументу Тогда
			КолонкиПоискаПоЗаявке = "СтатьяБюджетов, НачалоПериода, КонецПериода, Аналитика1, Аналитика2, Аналитика3, Аналитика4, Аналитика5, Аналитика6";
...
Процедура ЗаявкаПроходитПоЛимитамБюджетирования(ЗаявкаОбъект, Ошибки, НарушеныТолькоИнформационныеЛимиты)
	...
	Если ПродолжитьРасчет Тогда
		...
		КолонкиПоискаПоЗаявке = "СтатьяБюджетов, Валюта, НачалоПериода, КонецПериода, Аналитика1, Аналитика2, Аналитика3, Аналитика4, Аналитика5, Аналитика6";
		Если ФактПоЗаявкам.Колонки.Найти("Организация") <> Неопределено Тогда
			КолонкиПоискаПоЗаявке = КолонкиПоискаПоЗаявке + ", Организация";
		КонецЕсли;
		Если ФактПоЗаявкам.Колонки.Найти("Подразделение") <> Неопределено Тогда
	...
	Для каждого ИспользуемаяАналитика Из ИерархияАналитик Цикл
		...
		Иначе
			ПредставлениеЗначения = БюджетнаяОтчетностьКлиентСервер.ПредставлениеПрочейАналитикиБюджетирования(
				ИмяИзмеренияВидАналитики,, КэшПредставлений);
		КонецЕсли;
...
Функция ПлановыеДанныеДляПланФакта(ПравилаЛимитовСДопДанными, ФактическиеДанные, ДополнительныеПараметры)
	...
	ОтборПоЗаявке = ДополнительныеПараметры.ОтборПоЗаявке;
...
//	ДополнительныйОтборПоПравиламЛимитов - Массив, СправочникСсылка.ПравилаЛимитовПоДаннымБюджетирования. Дополнительный не инвертируемый отбор по правилам лимитов.
//	                                       Применяется при расшифровке отчета по лимитам.
//	                                       Неопределено. Значение по умолчанию.
//
// Возвращаемое значение:
...
// Параметры:
//	Дата - Дата - дата, на которую проверяется состояние лимитов.
//	ОтборПоСтатьямБюджетов - Массив, СправочникСсылка.СтатьиБюджетов - ограничение использования правил лимитов по бюджетам.
