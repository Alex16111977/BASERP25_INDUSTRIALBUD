	...
	Для каждого ЭлементКоллекции Из СтрокиЗаймов Цикл
		...
		Иначе
			Для каждого СтрокаТЗ Из СтрокиРаспределения Цикл
				...
				НоваяСтрока.Подразделение	= СтрокаТЗ.Подразделение;
				НоваяСтрока.Сумма = СтрокаТЗ.Результат;
				Если СтрокаТЗ.ВидУдержания = Перечисления.ВидыОсобыхНачисленийИУдержаний.НачисленоПроцентовПоЗайму И ЗначениеЗаполнено(СтатьяПрочиеРасчетыСПерсоналом) Тогда
					НоваяСтрока.СтатьяРасходов = СтатьяПрочиеРасчетыСПерсоналом;
				Иначе
					НоваяСтрока.СтатьяРасходов 	= СтрокаТЗ.СтатьяРасходов;
				КонецЕсли;
			КонецЦикла;
