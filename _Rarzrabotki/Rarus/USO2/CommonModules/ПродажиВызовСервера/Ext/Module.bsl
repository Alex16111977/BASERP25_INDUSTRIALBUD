...
Функция СформироватьДанныеЗаполненияРеализации(МассивСсылок, ИмяДокумента, РеквизитыШапки) Экспорт
	...
	ИмяДокументаРТУ    = ?(ИмяДокумента = Метаданные.Документы.ПередачаТоваровХранителю.Имя,
				НСтр("ru = 'передачи товаров хранителю';
					|en = 'transfers of goods to the custodian'"),
	...
	Пока ЗаказыСНедопустимойОперацией.Следующий() Цикл
		...
		Если МассивСсылок.Количество() > 1 Тогда
			ТекстСообщения = НСтр("ru = 'Распоряжение %Распоряжение% пропущено, т.к. невозможно оформить документ ""%ИмяДокумента%"" на основании распоряжения с операцией ""%ХозяйственнаяОперация%"".';
									|en = 'The %Reference% reference is skipped as you cannot register the ""%DocumentName%"" document based on the reference with the ""%BusinessTransaction%"" operation.'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Невозможно оформить документ ""%ИмяДокумента%"" на основании распоряжения с операцией ""%ХозяйственнаяОперация%"".';
									|en = 'Cannot register the ""%DocumentName%"" document based on the reference with the ""%BusinessTransaction%"" operation.'");
		КонецЕсли;
	...
	Если ПродажиСервер.СообщитьОбОшибкахФормированияДанныхЗаполненияРеализации(ВыборкаРеквизитыШапки) Тогда
		...
		ТекстОшибки = НСтр("ru = 'Ввод одной %Документ% на основании выделенных распоряжений невозможен';
							|en = 'Cannot enter one %Документ% based on the selected references'");
...
Функция СформироватьДанныеЗаполненияАктовВыполненныхРабот(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер Или
		...
		ТекстОшибки = НСтр("ru = 'Ввод одного акта выполненных работ на основании выделенных распоряжений невозможен';
							|en = 'Cannot enter one acceptance note based on the selected references'");
...
Функция СформироватьДанныеЗаполненияОтчетаДавальцу(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияВалютаВзаиморасчетов Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru = 'Валюта взаиморасчетов';
													|en = 'Mutual settlements currency'"), Отказ);
	КонецЕсли;
			...
			Пока ВыборкаЗаказы.Следующий() Цикл
				...
				ИначеЕсли ВыборкаЗаказы.ЕстьОшибкиСтатус Тогда
					...
					ТекстОшибки = НСтр("ru = 'Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусах ""К отгрузке"", ""Закрыт"".';
										|en = 'The %Документ% document is in the ""%Статус%"" status. The ""Input on basis"" method is allowed only in the ""For shipment"", ""Closed"" statuses. '");
