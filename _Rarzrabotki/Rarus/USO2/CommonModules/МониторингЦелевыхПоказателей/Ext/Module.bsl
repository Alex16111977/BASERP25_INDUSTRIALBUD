...
Процедура ЗаполнитьСтруктуруЦелейИВариантыАнализа() Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Загрузка демонстрационной модели целевых показателей';
				|en = 'Import target indicator demonstration model'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьТаблицуДляМонитора(ТаблицаЗначенийМонитора, ИдентификаторЭлементаФормы, ВариантАнализа, ИсточникСПараметрами) Экспорт
	...
	Для Каждого КолонкаИсточника Из КолонкиИсточника Цикл
		Если КолонкаИсточника.Имя = ДинамическиеСвойстваВариантаАнализа.ОбъектАнализа Тогда
			КолонкаИсточника.Имя = НСтр("ru = 'КолонкаОбъектАнализа';
										|en = 'КолонкаОбъектАнализа'") + ?(Не ПустаяСтрока(ИдентификаторЭлементаФормы), "__", "") + ИдентификаторЭлементаФормы;
		ИначеЕсли КолонкаИсточника.Имя = ДинамическиеСвойстваВариантаАнализа.ЗначениеАнализа Тогда
			КолонкаИсточника.Имя = НСтр("ru = 'КолонкаЗначениеАнализа';
										|en = 'КолонкаЗначениеАнализа'") + ?(Не ПустаяСтрока(ИдентификаторЭлементаФормы), "__", "") + ИдентификаторЭлементаФормы;
		КонецЕсли;
	КонецЦикла;
		...
		Если ИсточникДанных.СвойстваДанныхПоПериодам.ВидЦелевогоЗначения = ВидыЦелевыхЗначенийВариантовАнализа.Значение Тогда
			...
			ИначеЕсли ИсточникДанных.СвойстваДанныхПоПериодам.ВидОтклоненияОтЦелевогоЗначения = ВидыОтклоненийОтЦелевыхЗначенийПоказателей.Относительное Тогда
				СвойстваВариантаАнализа.ЦелевыеЗначения = ПолучитьСокращенноеПредставлениеЧисла(ИсточникДанных.СвойстваДанныхПоПериодам.ЦелевоеЗначение, СвойстваВариантаАнализа.ТочностьРасчетаДробнойЧасти, СвойстваВариантаАнализа.КратностьЗначений) +
					ПредставлениеРазмерности + Символы.ПС +
					НСтр("ru = 'с позитивным отклонением';
						|en = 'with positive variance '") + " " + ИсточникДанных.СвойстваДанныхПоПериодам.ГраничноеПозитивноеОтклонение +
					РазмерностьОтклонения + " " +
					НСтр("ru = 'и негативным отклонением';
						|en = 'and negative variance'") + " " + ИсточникДанных.СвойстваДанныхПоПериодам.ГраничноеНегативноеОтклонение +
					РазмерностьОтклонения;
...
Функция ПроверитьСхемуКомпоновкиДанных(МакетСКДПоказателя) Экспорт
	...
	Если МакетСКДПоказателя = Неопределено Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя не заполнен.';
									|en = 'Target indicator calculation template is not filled in.'"));
	...
	Если МакетСКДПоказателя.НаборыДанных.Найти("ТаблицаПериодов") = Неопределено Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя не содержит обязательного набора данных ""ТаблицаПериодов"". 
										...
										|en = 'Target indicator calculation template does not contain required data set ""ТаблицаПериодов"".
										|It is necessary to set up the target indicator calculation template in accordance with the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Если МакетСКДПоказателя.НаборыДанных.Найти("ЦелевыеЗначения") = Неопределено Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя не содержит обязательного набора данных ""ЦелевыеЗначения"". 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template does not contain required data set ""ЦелевыеЗначения"".
										|It is necessary to set up the target indicator calculation template in accordance with the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Если МакетСКДПоказателя.НаборыДанных.Найти("ДанныеДляАнализа") = Неопределено Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя не содержит обязательного набора данных ""ДанныеДляАнализа"". 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template does not contain required data set ""ДанныеДляАнализа"".
										|It is necessary to set up the target indicator calculation template in accordance with the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Если ТаблицаПериодовЕсть И Не ЦелевыеЗначенияЕсть ИЛИ МакетСКДПоказателя.СвязиНаборовДанных.Количество() < 2 Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя не содержит связи между таблицей периодов и целевых значений. 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template does not contain any link between the period table and the target value table.
	...
	Если ТаблицаПериодовЕсть И Не ДанныеДляАнализаЕсть ИЛИ МакетСКДПоказателя.СвязиНаборовДанных.Количество() < 2 Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя не содержит связи между таблицей периодов и данными для анализа. 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template does not contain any link between the period table and the data for analysis.
										|Configure the target indicator calculation template in accordance with the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Если МакетСКДПоказателя.ВариантыНастроек.Количество() <> 1 Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя должен содержать один вариант настроек. 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template must contain one settings option. 
										|Configure the target indicator calculation template in accordance with the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Для Каждого СлужебныйРесурс Из СлужебныеРесурсы Цикл 
		...
		Если ИскомоеСлужебноеПоле = Неопределено Тогда
			...
			СтрокаОшибки = НСтр("ru = 'Шаблон расчета целевого показателя должен содержать служебное поле %СлужебныйРесурс. 
											|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
											|en = 'Target indicator calculation template must contain the %ServiceResource service field. 
											|Configure the target indicator calculation template in accordance with the requirements.'");
	...
	Если ИскомоеСлужебноеПоле = Неопределено Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя должен содержать служебное поле ""Период"". 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template must contain the ""Period"" service field.
										|Configure the target indicator calculation template in accordance with the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Если КоличествоНеслужебныхИзмерений < 1 Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя должен содержать неслужебные измерения. 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template should contain non-service dimensions. 
										|Set up target indicator calculation template according to the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
	...
	Если КоличествоНеслужебныхРесурсов < 1 Тогда
		...
		МассивОшибок.Добавить(НСтр("ru = 'Шаблон расчета целевого показателя должен содержать неслужебные ресурсы. 
										|Необходимо настроить шаблон расчета целевого показателя в соответствии с требованиями.';
										|en = 'Target indicator calculation template should contain non-service resources. 
										|Set up target indicator calculation template according to the requirements.'"));
		РезультатПроверки.МакетКорректный = Ложь;
...
Процедура ЗаменитьИмяПериодичности(Выражение, НовоеИмяПериодичности)
	...
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""ДЕНЬ""';
											|en = '""DAY""'"), """" + НовоеИмяПериодичности + """");
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""НЕДЕЛЯ""';
											|en = '""WEEK""'"), """" + НовоеИмяПериодичности + """");
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""ДЕКАДА""';
											|en = '""TEN-DAY PERIOD""'"), """" + НовоеИмяПериодичности + """");
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""МЕСЯЦ""';
											|en = '""MONTH""'"), """" + НовоеИмяПериодичности + """");
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""КВАРТАЛ""';
											|en = '""QUARTER""'"), """" + НовоеИмяПериодичности + """");
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""ПОЛУГОДИЕ""';
											|en = '""HALF-YEAR""'"), """" + НовоеИмяПериодичности + """");
	Выражение = СтрЗаменить(Выражение, НСтр("ru = '""ГОД""';
											|en = '""YEAR""'"), """" + НовоеИмяПериодичности + """");
		...
		Для Каждого СтрокаИсточникаДанных Из ДанныеИсточникаДанных Цикл
			...
			Иначе
				...
				ИначеЕсли ТипАнализа = ТипыАнализа.ИзмерениеТекущегоСостояния Тогда
					ТекстСерии = ИсточникДанных.СвойстваЗоныАнализаИТренда.ПояснениеКСтатусу + " " + НСтр("ru = 'на';
																											|en = 'to'") + " " + Формат(ИсточникДанных.СвойстваДанныхПоПериодам.ДатаПоследнегоЗначения, "ДФ=dd.MM.yy; ДЛФ=D") + " " + "=" + " " + ЗначениеСтроки;
				Иначе
...
Процедура ПодготовитьИсточник(ИсточникДанных, ВариантАнализа)
	...
	ИначеЕсли ТипАнализа = ТипыАнализа.ИзмерениеТекущегоСостояния Тогда
		...
		ОтборПоДате = Новый Структура(НСтр("ru = 'Период';
											|en = 'Period'"), НачалоДня(ДатаПоследнегоЗначения));
	...
	ИначеЕсли ТипАнализа = ТипыАнализа.СравнениеСПрошлымПериодом Тогда 
		...
		СтрокаПрошлогоПериода.ПериодСравнения = НСтр("ru = 'Прошлый';
													|en = 'Previous'") + " " + ПредставлениеПрошлогоПериода; 
		...
		СтрокаТекущегоПериода.ПериодСравнения = НСтр("ru = 'Текущий';
													|en = 'Current'") + " " + ПредставлениеТекущегоПериода; 
		...
		Если ВариантАнализа.РассчитыватьПрогноз И Не ИсточникДанных.СвойстваПрогнозаДанныхПоПериодам.СтатистическиеПараметрыТренда.Пустой Тогда 
			...
			СтрокаПрогнозногоПериода.ПериодСравнения = НСтр("ru = 'Прогноз на';
															|en = 'Forecast for'") + " " + Формат(ДатаПрогноза, "Л=ru; ДФ=dd.MM.yy"); 
...
Функция КонтрольКорректностиНастроекИДанных(ТипПроверяемогоОбъекта, ПроверяемыйОбъект, ИсточникДанных, ПараметрыРасчета)
	...
	Если ТипПроверяемогоОбъекта = "РезультатПолученияДанных"
		...
		ИсточникДанных.ОписаниеОшибки = НСтр("ru = 'Неверный шаблон расчета';
											|en = 'Incorrect calculation template'") + ": " + ПроверяемыйОбъект.ИнформацияОбОшибках;
	...
	Если Не СхемаКомпоновкиДанных = Неопределено Тогда
		...
		Если СхемаКомпоновкиДанных.Параметры.Найти("НеИзменятьСвязьПоДатеРасчета") = Неопределено
			...
			Для Каждого СвязьНаборовДанных Из СвязиНаборовДанных Цикл 
				Если СвязьНаборовДанных.НаборДанныхПриемник = НСтр("ru = 'ДанныеДляАнализа';
																	|en = 'ДанныеДляАнализа'") Тогда 
					Если СтрНайти(СвязьНаборовДанных.ВыражениеИсточник, НСтр("ru = 'Период';
																			|en = 'Period'")) Тогда
						СвязьНаборовДанных.ВыражениеПриемник = НСтр("ru = 'Период';
																	|en = 'Period'") + ПолучитьИмяЗначенияПеречисления(ПериодичностьРасчетаПоказателя);
					КонецЕсли;
				ИначеЕсли СвязьНаборовДанных.НаборДанныхПриемник = НСтр("ru = 'ЦелевыеЗначения';
																		|en = 'ЦелевыеЗначения'") Тогда 
					СвязьНаборовДанных.ВыражениеИсточник = "КОНЕЦПЕРИОДА(" + НСтр("ru = 'Период';
																					|en = 'Period'") + ", """ + ВРег(ПолучитьИмяЗначенияПеречисления(ПериодичностьРасчетаПоказателя)) + """)";
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли Не СхемаКомпоновкиДанных.Параметры.Найти("НеИзменятьСвязьПоДатеРасчета") = Неопределено
			...
			Для Каждого СвязьНаборовДанных Из СвязиНаборовДанных Цикл 
				Если СтрНайти(СвязьНаборовДанных.ВыражениеИсточник, НСтр("ru = 'Период';
					...
					СвязьНаборовДанных.ВыражениеИсточник = "КОНЕЦПЕРИОДА(" + НСтр("ru = 'Период';
																					|en = 'Period'") + ", """ + ВРег(ПолучитьИмяЗначенияПеречисления(ПериодичностьРасчетаПоказателя)) + """)";
					СвязьНаборовДанных.ВыражениеПриемник = "КОНЕЦПЕРИОДА(" + НСтр("ru = 'ДатаРасчета';
																					|en = 'ДатаРасчета'") + ", """ + ВРег(ПолучитьИмяЗначенияПеречисления(ПериодичностьРасчетаПоказателя)) + """)";
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
