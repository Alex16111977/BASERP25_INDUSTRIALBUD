...
Процедура СформироватьОценочныеОбязательстваПоСотрудникам() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%2';
				|en = 'Cannot update leave reserve data by the ""%1"" document due to:
				|%2'"), 
				Выборка.Ссылка,
	...
	Если ОбъектовОбработано < ТребуетОбработки Тогда
		...
		|en = 'The РезервОтпусков.СформироватьОценочныеОбязательстваПоСотрудникам
		|procedure cannot update leave reserve data in documents %1.'"), 
		ПроблемныхОбъектов);
	...
	ИначеЕсли ОбъектовОбработано <> 0 Тогда
		...
		|en = 'The РезервОтпусков.СформироватьОценочныеОбязательстваПоСотрудникам
		|procedure has processed leave reserve accounting documents: %1 items'"), 
		ОбъектовОбработано));
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%2';
				|en = 'The РезервОтпусков.ПодготовитьДокументыДляФормированияОценочныхОбязательствПоСотрудникам procedure cannot prepare data by the ""%1"" document due to:
				|%2'"), 
				Выборка.Ссылка,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,, 
