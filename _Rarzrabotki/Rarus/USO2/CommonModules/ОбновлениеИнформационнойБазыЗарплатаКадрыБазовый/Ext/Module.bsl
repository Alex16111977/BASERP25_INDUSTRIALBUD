...
Функция ОписаниеБлокируемыхДанных(Знач МетаданныеОбъекта, Знач ПоляБлокировки = Неопределено) Экспорт
			...
			Иначе
				...
				Если Описание.ПоляБлокировки.Количество() = 0 Тогда
					Шаблон = НСтр("ru = 'Неверное имя полей блокировки %1';
									|en = 'Incorrect name of lock fields %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			Для Каждого ПолеБлокировки Из Описание.ПоляБлокировки Цикл
				Если МассивИзмерений.Найти(ПолеБлокировки.Ключ) = Неопределено Тогда
					Шаблон = НСтр("ru = 'Неверное имя поля блокировки %1';
									|en = 'Incorrect name of lock field %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Недопустимый тип объекта';
								|en = 'Invalid object type'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Иначе
		...
		Если ЗапросВыбораНеНайден Тогда
			ВызватьИсключение НСтр("ru = 'Не определен запрос выбора данных. Ошибка в имени временной таблицы получения данных';
									|en = 'Data selection query is not determined. An error occurred in the name of temporary table of data receipt'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") Тогда
		...
		Обработчик.Комментарий     = НСтр("ru = 'Отключение рассылок с некорректными настройками персонифицированных отчетов.';
											|en = 'Disable mailings with incorrect settings of personal reports.'");
	КонецЕсли;
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка параметров для набора дополнительных свойств статей финансирования зарплаты';
									|en = 'Setting of parameters for collection of additional articles in salary funding lines'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.9.20";
	Обработчик.Процедура = "ЗарплатаКадры.УстановитьПараметрыНабораСвойств";
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения = "Отложенно";
