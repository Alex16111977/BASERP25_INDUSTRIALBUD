...
// Возвращает таблицу среза допоследних регистра.
//
// Параметры:
//		ИмяРегистра 				- Строка - имя периодического регистра сведений, как  оно
//										задано в конфигураторе.
...
Функция ОписаниеПериодаДляСоздатьВТИмяРегистра()
...
Функция ОписаниеФильтраДляПолученияЗаписейНаНачалоПериода(ОписаниеПакетаЗапросовПолученияДвижений, ОписаниеФильтраПолученияДвижений, ПолучатьВсеЗаписи = Ложь)
	...
	ОписаниеФильтраСреза.СоответствиеИзмеренийРегистраИзмерениямФильтра = ОбщегоНазначенияКлиентСервер.СкопироватьСоответствие(ОписаниеФильтраПолученияДвижений.СоответствиеИзмеренийРегистраИзмерениямФильтра);
...
Функция СкопироватьОписаниеФильтра(КопируемоеОписаниеФильтра)		
	...
	КопированноеОписание.СоответствиеИзмеренийРегистраИзмерениямФильтра = ОбщегоНазначенияКлиентСервер.СкопироватьСоответствие(КопируемоеОписаниеФильтра.СоответствиеИзмеренийРегистраИзмерениямФильтра);	
...
Функция ДобавитьЗапросПолученияЗаписейНаНачалоПериода(ОписаниеПакетаЗапросов, ТолькоРазрешенные, ОписаниеРегистра, ОписаниеФильтра,  ПараметрыПостроения, ИмяДополняемойВТ)		
	...
	ДобавляемыйОператор = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(КопируемыйОператор);	
...
Процедура ДобавитьЗапросСрезаДляПолученияДвиженийПервичногоРегистра(ОписаниеПакетаЗапросов, ТолькоРазрешенные, ОписаниеРегистра, ОписаниеФильтра, ПараметрыПостроения, ИмяДополняемойВТ)		
	...
	ОписаниеФильтраСреза.СоответствиеИзмеренийРегистраИзмерениямФильтра = ОбщегоНазначенияКлиентСервер.СкопироватьСоответствие(ОписаниеФильтра.СоответствиеИзмеренийРегистраИзмерениямФильтра);
...
Процедура ДобавитьЗапросВТСрезПервичныйРегистр(ОписаниеПакетаЗапросов, ИмяРегистра, Знач ТолькоРазрешенные, Знач ОписаниеФильтра, Знач ПараметрыПостроения = Неопределено, Знач ИмяСоздаваемойТаблицы = Неопределено)	
	...
	ВыраженияПолейТаблицыСреза = Новый Соответствие;
	Для Каждого Измерение Из ОписаниеРегистра.Измерения Цикл
		...
		ВыраженияПолейТаблицыСреза.Вставить(Измерение, ВыражениеИзмерениеВТСрез);
	...
	Для Каждого Ресурс Из ОписаниеРегистра.Ресурсы Цикл
		...
		ВыраженияПолейТаблицыСреза.Вставить(Ресурс, "РегистрСведенийРесурсы." + Ресурс);
	КонецЦикла;	
	...
	Для Каждого Реквизит Из ОписаниеРегистра.Реквизиты Цикл
		...
		ВыраженияПолейТаблицыСреза.Вставить(Реквизит, "РегистрСведенийРесурсы." + Реквизит);
	КонецЦикла;	
