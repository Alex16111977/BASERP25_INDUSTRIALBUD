...
Процедура ЗаполнитьСопоставленнуюПродукцию(ТаблицаСопоставления) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.СопоставленоКоличество > 1 Тогда
			СтрокаТаблицы.Сопоставлено = Истина;
			СтрокаТаблицы.СопоставлениеНоменклатура   = СтрШаблон(НСтр("ru = '%1 ( + еще %2...)';
																		|en = '%1 ( + еще %2...)'"), Выборка.НоменклатураНаименование, Выборка.СопоставленоКоличество - 1);
...
Функция ПредставлениеСтатусаЕГАИС(СтатусЕГАИС, ВходящиеДальнейшееДействие, ДопустимыеДействия) Экспорт
	...
	Если ТипЗнч(СтатусЕГАИС) <> Тип("ФорматированнаяСтрока") Тогда
		...
		Если СтатусыЦветТекстаТребуетВниманияЕГАИС.Найти(СтатусЕГАИС) <> Неопределено Тогда
			СтатусЕГАИСПредставление = Новый ФорматированнаяСтрока(Строка(СтатусЕГАИС),,ЦветаСтиля.ЦветТекстаТребуетВниманияГосИС);
		Иначе
...
Процедура СверткаРегистраСоответствиеНоменклатурыЕГАИС() Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Свертка регистра Соответствие номенклатуры ЕГАИС';
										|en = 'Свертка регистра Соответствие номенклатуры ЕГАИС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция РезультатПроверкиДанныхТСД(МассивСчитанныхДанных, ПараметрыПроверки) Экспорт
	...
	Если ПараметрыПроверки.ЭтоФормаДокумента Тогда
		...
		ПараметрыСканирования = ШтрихкодированиеИСКлиентСервер.ИнициализироватьПараметрыСканирования(ПараметрыПроверки.Форма);
	...
	Для Каждого ЭлементМассива Из МассивСчитанныхДанных Цикл
		...
		Если ЭлементМассива.Свойство("ШтрихкодМаркиАлкогольнойПродукции")
			...
			ЗагрузкаДанныхТСД.МаркируемаяПродукция.Добавить(ДанныеМаркируемойПродукции);
	...
	Если ДетализацияСтруктурыХранения = Неопределено Тогда
		...
		Если ЗагрузкаДанныхТСД.МаркируемаяПродукция.Количество() = 0 
	...
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
		...
		Если КоличествоИмеющихсяУпаковок <> 0 Тогда
			...
			ДанныеИмеющихсяУпаковок = ШтрихкодированиеИС.ВложенныеШтрихкодыУпаковок(
				СсылкиНаСуществующиеУпаковки, ПараметрыСканирования);
		...
		Если ЗагрузкаДанныхТСД.МаркируемаяПродукция.Количество() = 0 
			...
			ЗагрузкаДанныхТСД.ТекстОшибки = НСтр("ru = 'В считанных штриховых кодах нет данных по бутылкам и партионной алкогольной продукции.';
												|en = 'В считанных штриховых кодах нет данных по бутылкам и партионной алкогольной продукции.'");
			Возврат ЗагрузкаДанныхТСД;
	...
	ПолучитьДанныеПоШтрихКодамТСД(ЗагрузкаДанныхТСД, ПараметрыСканирования, ПараметрыПроверки);
	...
	Если ПараметрыПроверки.ЭтоФормаДокумента
		...
		ПроверитьНеобходимостьУказанияДополнительныхДанныхДляАкцизныхМарок(ЗагрузкаДанныхТСД, ПараметрыСканирования);
	КонецЕсли;
	
	АкцизныеМаркиЕГАИС.ДанныеДокументаОснования(ПараметрыСканирования);
	
	ЗагрузкаДанныхТСД.АдресДанныхДокументаОснования = ПараметрыСканирования.АдресДанныхДокументаОснования;
	ЗагрузкаДанныхТСД.ДокументОснование             = ПараметрыСканирования.ДокументОснование;
...
Процедура ПроверитьКорректностьСчитанныхТСДУпаковок(РезультатПроверки, ДеревоМаркированнойПродукции, НовыеУпаковки, СуществующиеУпаковки, СоответствиеШтрихкодовСтрокДерева)
	...
	Если МассивПредполагаемыхРодителей.Количество() > 1 Тогда
		...
		РезультатПроверки.ТекстОшибки = НСтр("ru = 'Для считанных упаковок не удалось упаковку, в которой содержатся другие упаковки.';
											|en = 'Для считанных упаковок не удалось упаковку, в которой содержатся другие упаковки.'");
