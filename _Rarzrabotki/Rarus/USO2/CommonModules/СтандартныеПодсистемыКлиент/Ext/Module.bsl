	...
	Иначе
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'Для того чтобы открыть список активных пользователей, перейдите в меню
				       ...
				       |en = 'To open the active user list, click
...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Текст = НСтр("ru = 'Название программы: [НазваниеПрограммы] 
	                   ...
	                   |Standard subsystem library version:[ВерсияБСП];
	                   |Operating system: [ОперационнаяСистема];
	                   |RAM: [ОперативнаяПамять];
	                   |COM connector name: [ИмяCOMСоединителя];
	                   |Basic: [ЭтоБазоваяВерсияКонфигурации]
	                   |Rightful user: [ЭтоПолноправныйПользователь]
	                   |Sample:[ЭтоУчебнаяПлатформа]
		...
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения =
				НСтр("ru = 'Вход в программу невозможен.
				           ...
				           |Update the 1C:Enterprise platform version first. '");
		Иначе
			ТекстСообщения =
				НСтр("ru = 'Вход в программу невозможен.
				           |Необходимо обратиться к администратору для обновления версии платформы 1С:Предприятие.';
				           |en = 'Logon to the application is not available.
				           |Contact administrator for update of the 1C:Enterprise platform version. '");
		КонецЕсли;
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения =
				НСтр("ru = 'Рекомендуется завершить работу программы и обновить версию платформы 1С:Предприятия.
				         ...
				         |en = 'We recommend that you exit the application and update the 1C:Enterprise platform version.
				         |Otherwise, some application features will not be available or may not work properly.'");
		Иначе
			ТекстСообщения = 
				НСтр("ru = 'Рекомендуется завершить работу программы и обратиться к администратору для обновления версии платформы 1С:Предприятия.
				         |В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';
				         |en = 'We recommend that you exit the application and contact your administrator to update the 1C:Enterprise platform version.
				         |Otherwise, some application features will not be available or may not work properly.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	ТекстВопроса =
		НСтр("ru = 'Версия программы обновлена (внесены изменения в конфигурацию информационной базы).
		           ...
		           |en = 'The application version was updated (changes were made to the infobase configuration).
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru = 'Невозможно выбрать группу объектов метаданных.
				           ...
				           |en = 'Cannot select a metadata object group.
				           |Select the metadata object.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыЗапуска.Количество() < 2 Тогда
		ВызватьИсключение
			НСтр("ru = 'При указании параметра запуска ВойтиВОбластьДанных,
			           ...
			           |en = 'It is required to specify a separator value as
			           |an additional parameter when specifying the ВойтиВОбластьДанных launch parameter.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение
			НСтр("ru = 'Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.';
				|en = 'The separator value in ВойтиВОбластьДанных parameter must be number.'");
	КонецПопытки;
...
Процедура ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	ТекстПредупреждения = НачалоОписанияОшибки + Символы.ПС
		+ НСтр("ru = 'Техническая информация об ошибке записана в журнал регистрации.';
				|en = 'Technical information on the error is recorded in the event log.'")
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Рекомендуется увеличить объем памяти до %1 Гб.';
			|en = 'It is recommended to increase the memory size up to %1 GB.'"), РекомендуемыйОбъем);
