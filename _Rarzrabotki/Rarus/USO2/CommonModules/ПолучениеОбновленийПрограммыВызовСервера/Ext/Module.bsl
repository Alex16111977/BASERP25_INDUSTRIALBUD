...
Процедура ВключитьОтключитьАвтоматическуюУстановкуИсправлений(Знач ЗначениеНастройки) Экспорт
	...
	Если Не ПараметрыОбновления.ПолучатьИсправления Тогда
		ВызватьИсключение НСтр("ru = 'Получение исправлений недоступно в текущем режиме работы.';
								|en = 'Cannot receive corrections in the current operation mode.'");
	КонецЕсли;
...
Функция ПараметрыПроверкиВерсииПлатформыПриЗапуске() Экспорт
	...
	ТекстСообщения = "<body>" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для работы с программой %1 использовать версию платформы &quot;1С:Предприятие 8&quot; не ниже: <b>%2</b>
			...
			|en = 'To work with %1 application, use platform version &quot;1С:Enterprise 8&quot; not earlier than: <b>%2</b>
			|<br /> Current version: %3'"),
		?(ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена,
			НСтр("ru = 'необходимо';
				|en = 'required'"),
	...
	Если Не Результат.ЭтоАдминистраторСистемы Тогда
		...
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("ru = 'Вход в программу невозможен.<br />
				...
				|en = 'Logon to the application is not available.<br /> 
	...
	Иначе
		...
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("ru = 'Рекомендуется обновить версию платформы 1С:Предприятия.<br />
				...
				|en = 'It is recommended that you update 1C:Enterprise platform version. <br />
				|Otherwise, some application features will not be available or may not function properly.'");
		...
		Если Результат.Статус = "Отменено" Тогда
			...
			ПолучениеОбновленийПрограммы.ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru = 'Не удалось выполнить фоновую проверку наличия обновлений. Задание отменено администратором.';
					|en = 'Cannot make a background check of updates. The task is cancelled by an administrator.'"));
			Возврат Неопределено;
