...
Процедура ЗаполнитьКатегориюДоходаВОперацияНалоговогоУчетаПоНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Для Каждого СтрокаТабличнойЧасти Из ДокументОбъект.НДФЛУдержанный Цикл
				Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.КатегорияДохода) Тогда
					...
					Если СтрокаТабличнойЧасти.МесяцНалоговогоПериода < ДатаНачалаУчетаПоКатегориям Тогда
						// оставляем категорию дохода незаполненной
					...
					СтрокаУдержанный = НДФЛУдержанный.Добавить();
					СтрокаУдержанный.ФизическоеЛицо = ДокументОбъект.Сотрудник;
					СтрокаУдержанный.СтавкаНалогообложенияРезидента = СтрокаТабличнойЧасти.СтавкаНалогообложения;
					СтрокаУдержанный.Ставка = СтрокаТабличнойЧасти.Ставка;
					СтрокаУдержанный.МесяцНалоговогоПериода = СтрокаТабличнойЧасти.МесяцНалоговогоПериода;
					СтрокаУдержанный.Подразделение = СтрокаТабличнойЧасти.ОбособленноеПодразделение;
					СтрокаУдержанный.КодДохода = СтрокаТабличнойЧасти.КодДохода;
					СтрокаУдержанный.КатегорияДохода = СтрокаТабличнойЧасти.КатегорияДохода;
					СтрокаУдержанный.ВключатьВДекларациюПоНалогуНаПрибыль = СтрокаТабличнойЧасти.ВключатьВДекларациюПоНалогуНаПрибыль;
					СтрокаУдержанный.СрокПеречисления = СтрокаТабличнойЧасти.СрокПеречисления;
					СтрокаУдержанный.СуммаВыплаченногоДохода = СтрокаТабличнойЧасти.СуммаВыплаченногоДохода;
					СтрокаУдержанный.Сумма = СтрокаТабличнойЧасти.Сумма;
					СтрокаУдержанный.ДокументОснование = ?(ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументОснование), СтрокаТабличнойЧасти.ДокументОснование, ДокументОбъект.Ссылка);
					...
					Если СтрокиИсчисленногоПоСтавке13.Количество() Тогда
						...
						Если ПромежуточныйУдержанный.Количество() = 1 Тогда
							...
							СтрокаТабличнойЧасти.КатегорияДохода = КатегорияДохода;
							СтрокаУдержанный.КатегорияДохода = КатегорияДохода;
						...
						ИначеЕсли ПромежуточныйУдержанный.Количество() > 1 Тогда
							...
							Для Каждого НоваяПромежуточная Из ПромежуточныйУдержанный Цикл
								Если ПерваяСтрока Тогда 
									...
									СтрокаУдержанный.КатегорияДохода = НоваяПромежуточная.КатегорияДохода;
									СтрокаУдержанный.Сумма = НоваяПромежуточная.Сумма;
									СтрокаУдержанный.СуммаВыплаченногоДохода = НоваяПромежуточная.СуммаВыплаченногоДохода;
									ПерваяСтрока = Ложь;
								Иначе
									...
									НоваяСтрокаУдержанный = НДФЛУдержанный.Добавить();
									ЗаполнитьЗначенияСвойств(НоваяСтрокаУдержанный, НоваяПромежуточная);
									НоваяСтрокаУдержанный.ФизическоеЛицо = ДокументОбъект.Сотрудник;
									НоваяСтрокаУдержанный.СтавкаНалогообложенияРезидента = СтрокаТабличнойЧасти.СтавкаНалогообложения;
									НоваяСтрокаУдержанный.Подразделение = СтрокаТабличнойЧасти.ОбособленноеПодразделение;
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось заполнить категорию дохода НДФЛ в Операция учета НДФЛ: ';
											|en = 'Cannot fill in PIT income category in the PIT accounting transaction:'"), УровеньЖурналаРегистрации.Ошибка,, Выборка.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗаполнитьКатегориюДоходаВУдержанномНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ ВТРегистраторы
	|ИЗ
	|	РегистрНакопления.РасчетыНалогоплательщиковСБюджетомПоНДФЛ КАК УдержанныйНДФЛ
	|ГДЕ
	|	УдержанныйНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И УдержанныйНДФЛ.МесяцНалоговогоПериода >= ДАТАВРЕМЯ(2017,1,1)
	|	И УдержанныйНДФЛ.КатегорияДохода = ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ПустаяСсылка)
	|	И НЕ ТИПЗНАЧЕНИЯ(УдержанныйНДФЛ.Регистратор) В (&ИсключаемыеРегистраторы)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|		ПО УдержанныйНДФЛ.ДокументОснование = ИсчисленныйНДФЛ.ДокументОснование
	|			И (ИсчисленныйНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
	|			И УдержанныйНДФЛ.ФизическоеЛицо = ИсчисленныйНДФЛ.ФизическоеЛицо
	|			И УдержанныйНДФЛ.СтавкаНалогообложенияРезидента = ИсчисленныйНДФЛ.СтавкаНалогообложенияРезидента
	|			И УдержанныйНДФЛ.Подразделение = ИсчисленныйНДФЛ.Подразделение
	|			И УдержанныйНДФЛ.Организация = ИсчисленныйНДФЛ.Организация
	|ГДЕ
	|	УдержанныйНДФЛ.Регистратор В
	|			(ВЫБРАТЬ
	|				Регистраторы.Регистратор
	|			ИЗ
	|				ВТРегистраторы КАК Регистраторы)
	|	И УдержанныйНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|
	|СГРУППИРОВАТЬ ПО
	|	УдержанныйНДФЛ.Регистратор,
		...
		Если ПараметрыОбновления <> НеОпределено Тогда
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось исправить движения %1 по причине %2';
												|en = 'Cannot correct records %1 due to %2'"),
					Выборка.Ссылка,
...
Процедура УточнитьКатегориюДоходаВУдержанномНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	|
	|ИМЕЮЩИЕ
	|	(СУММА(Коэффициенты.Коэффициент) < -0.1
	|		ИЛИ СУММА(Коэффициенты.Коэффициент) > 0.1)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 1000
	...
	|ИЗ
	|	ВТУдержанный КАК ВТУдержанный
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКоэффициенты КАК ВложенныйЗапрос
								...
								Пока Выборка.СледующийПоЗначениюПоля("КодДохода") Цикл
									Пока Выборка.СледующийПоЗначениюПоля("ДокументОснование") Цикл
										Пока Выборка.СледующийПоЗначениюПоля("СуммаВыплаченногоДохода") Цикл
											...
											Пока Выборка.Следующий() Цикл
												НоваяСтрока = ВременныйНабор.Добавить();
												ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
												НоваяСтрока.СуммаВыплаченногоДохода = Выборка.Сумма;
