...
Процедура ПроверитьНаличиеИсправленияСторнирования(ИсправленныйДокумент, ИсключаемыйДокумент = Неопределено, Отказ = Ложь) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Документ %1 уже %2 документом %3';
							|en = '%1 document is already %2 by document %3'");
...
Процедура ПриПроведенииИсправления(ДокументСсылка, Движения, РежимПроведения, Отказ, РеквизитыДляПроведения = Неопределено, СтруктураВидовУчета = Неопределено, Объект = Неопределено, ИмяПериодРегистрации = "ПериодРегистрации", Результат = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ПроведениеПоВсемУчетам Или СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Если Сведения.Исправлен Или Сведения.Сторнирован Тогда
			...
			ВызватьИсключение НСтр("ru = 'Проведение запрещено.';
									|en = 'Posting is prohibited.'") + Символы.ПС + Сведения.Представление;
		КонецЕсли;
	КонецЕсли;
	...
	Если Сведения.Исправлен И Сведения.ДокументИсправление <> ДокументСсылка Тогда
		// Несколько одновременных исправлений одного и того же документа запрещено.
		ВызватьИсключение НСтр("ru = 'Проведение еще одного исправления запрещено.';
								|en = 'Unable to modify.'") + Символы.ПС + Сведения.Представление;
	КонецЕсли;
...
Функция СведенияОбИсправленииДокумента(ДокументСсылка) Экспорт
	...
	ПоддерживаетИсправление = МетаданныеДокумента.Реквизиты.Найти("ИсправленныйДокумент") <> Неопределено;
	ПоддерживаетСторнирование = Документы.СторнированиеНачислений.ТипыДокументовСторнируемыхИнтерактивно().СодержитТип(
	...
	Текст = СтрЗаменить(Текст, "#Таблица", МетаданныеДокумента.ПолноеИмя());
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(Выборка.СторнирующийДокумент) Тогда
			...
			Результат.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
