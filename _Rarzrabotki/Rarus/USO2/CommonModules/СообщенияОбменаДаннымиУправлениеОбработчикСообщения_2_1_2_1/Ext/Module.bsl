	...
	Попытка
		...
		Если Не ПустаяСтрока(КодЭтогоУзла)
			...
			Если ВерсияБСП243 Тогда
				...
				Если ПсевдонимЭтогоУзла <> Тело.Code Тогда
					СтрокаСообщения = НСтр("ru = 'Ожидаемый код предопределенного узла в этом приложении ""%1"" не соответствует фактическому ""%2"" или псевдониму ""%3"". План обмена: %4';
											|en = 'Expected code of the predefined node in the ""%1"" application does not match the actual one ""%2"" or the ""%3"" alias. Exchange plan: %4'");
			...
			Иначе
				СтрокаСообщения = НСтр("ru = 'Код предопределенного узла в этом приложении ""%1"" не соответствует ожидаемому ""%2"". План обмена: %3';
										|en = 'Predefined node code in the ""%1"" application does not match the expected one ""%2"". Exchange plan: %3'");
		...
		Если Отказ Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Возникли ошибки в процессе выгрузки справочников для корреспондента %1.';
											|en = 'Errors occurred while exporting catalogs for the correspondent %1.'"),
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Возникли ошибки в процессе загрузки справочников от корреспондента %1.';
											|en = 'Errors occurred while importing catalogs from the correspondent %1.'"),
				Строка(Корреспондент));
		КонецЕсли;
