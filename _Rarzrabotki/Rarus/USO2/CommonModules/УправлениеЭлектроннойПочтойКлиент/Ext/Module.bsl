		...
		Если РезультатВыполнения.ЕстьОшибки Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'При получении почты были ошибки. Подробности см. в журнале регистрации';
					|en = 'Errors occurred while receiving the mail. For more information, see the event log.'"));
...
Функция ПредставлениеРезультатаОтправкиПолученияПисемПользователя(РезультатВыполнения)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загружено писем: %1';
																					|en = 'Emails imported: %1'"), РезультатВыполнения.ПолученоПисем);
