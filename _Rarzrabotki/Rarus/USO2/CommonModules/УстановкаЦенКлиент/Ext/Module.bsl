						...
						Исключение
							...
							Если ЗначениеЗаполнено(СтрокаТаблицыЦен.Характеристика) Тогда
								ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" с характеристикой ""%Характеристика%""
								                          ...
								                          |en = 'Price is not calculated for the ""%Номенклатура%"" products with
								                          |the ""%Характеристика%"" characteristic by the ""%ВидЦены%"" price type
								                          |due to: %ОписаниеОшибки%'");
							Иначе
								ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" не рассчитана цена 
								                          |по виду цен ""%ВидЦены%""
								                          |по причине: %ОписаниеОшибки%';
								                          |en = 'Price is not calculated for the ""%Номенклатура%"" products
								                          |by the ""%ВидЦены%"" price type
								                          |due to: %ОписаниеОшибки%'");
							КонецЕсли;
...
Процедура ВыгрузитьВExcelПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ru = 'Не удалось записать данные в файл. Подробности:';
								|en = 'Cannot write data to file. More information:'") + " "
		                    + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
