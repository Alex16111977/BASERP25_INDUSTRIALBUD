...
Процедура ПриОпределенииЗапросаРеестраПрямыхВыплатПоБольничнымЛистам(Запрос, ТаблицаОснований)
	...
	Запрос.Текст = ТекстЗапросаСписокЗаявлений(ТаблицаОснований, Ложь);
	...
	|		ПО Заявления.НомерЛисткаНетрудоспособности = ХэшиЭЛН.НомерЛисткаНетрудоспособности";
...
Функция ТекстЗапросаСписокЗаявлений(ТаблицаОснований, ДобавлятьУведомлениеОПрекращенииОтпускаПоУходуЗаРебенком) Экспорт
	Если ТипЗнч(ТаблицаОснований) = Тип("ТаблицаЗначений") Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ТаблицаОснований.Заявление КАК Ссылка,
		|	ТаблицаОснований.ПервичныйДокумент КАК ПервичныйДокумент,
	...
	Иначе
		...
		|				ИЗ
		|					ВТУчтенныеЗаявления КАК УчтенныеЗаявления)";
		Если ДобавлятьУведомлениеОПрекращенииОтпускаПоУходуЗаРебенком Тогда
			Текст = СтрЗаменить(Текст, "ВТЗаявления", "ВТЗаявленияБезУведомлений");
			Текст = Текст + ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов()
			+
			...
			|			И (РеестрыУведомлений.Ссылка <> &Ссылка)
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявлениеСотрудникаНаВыплатуПособия КАК Заявления
			...
			|		ПО ПрекращаемыеЗаявления.Заявление = ЗаявленияБезУведомлений.Ссылка
			|ГДЕ
			...
			|	И ЗаявленияБезУведомлений.Ссылка ЕСТЬ NULL";
		КонецЕсли;
