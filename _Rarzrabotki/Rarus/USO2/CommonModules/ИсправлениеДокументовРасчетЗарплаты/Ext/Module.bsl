...
Процедура ЗаполнитьНачисленияПерерасчетПоНачисленияЗаднимЧислом(Организация, ПериодРегистрации,
	НачисленияЗаднимЧислом, НачисленияПерерасчет, ИсключаемыйРегистратор,
	...
	Иначе
		Для Каждого Строка Из ДанныеПерерасчетов Цикл
			...
			НоваяСтрока.Регистратор = ИсключаемыйРегистратор;
			НоваяСтрока.РегистраторПоказателей = ИсключаемыйРегистратор;
	...
	РаспределениеНачислений = ПолучитьРаспределениеНачислений(Результаты, РезультатыПоказателей, ИсключаемыйРегистратор);
	...
	Для Каждого Строка Из Результаты Цикл
		...
		Если ТекущийИдентификаторСтроки <> Строка.ИдентификаторСтроки 
			...
			НовыйПерерасчетСторно.СторнируемыйДокумент = Строка.СторнируемыйДокумент;
			...
			Если ТекущийВидРасчета <> Строка.ВидРасчета 
				...
				Или ТекущийСотрудник <> Строка.Сотрудник Тогда
...
Процедура СформироватьДанныеПерерасчетаДляИсправленияСлужебный(Организация, ПериодРегистрации, ИсправленныйДокумент, Начисления, НачисленияПерерасчет, ДокументСсылка, ОписаниеТаблицы, КонтролируемыеПоля, ОписаниеДокумента, Сотрудник, ДоначислитьЗарплатуПриНеобходимости)
	...
	УчитыватьСкидкуПоВзносам = Результаты[0].Колонки.Найти("СкидкаПоВзносам") <> Неопределено;
	...
	Пока ВыборкаДанныеИсправленногоДокумента.Следующий() Цикл
		...
		Если УчитыватьСкидкуПоВзносам Тогда 
			НоваяСтрокаПерерасчета.СкидкаПоВзносам = - ВыборкаДанныеИсправленногоДокумента.СкидкаПоВзносам;
		КонецЕсли;
		Если ЗначениеЗаполнено(НачислениеДокумента) Тогда
