...
Функция ЭтоШтрихкодАлкогольнойПродукции(ЗначениеШтрихкода, УчитыватьЛогистическуюУпаковку = Ложь) Экспорт

	ЭтоШтрихкодАлкогольнойПродукции = ЭтоШтрихкодАкцизнойМарки(ЗначениеШтрихкода)
		Или ЭтоШтрихкодСНомеромИСерией(ЗначениеШтрихкода);
		
	Если УчитыватьЛогистическуюУпаковку Тогда
		ЭтоШтрихкодАлкогольнойПродукции = ЭтоШтрихкодАлкогольнойПродукции
			Или ШтрихкодированиеЕГАИСКлиентСервер.ЭтоШтрихкодЛогистическойУпаковки(ЗначениеШтрихкода);
	КонецЕсли;
...
Функция ЭтоШтрихкодАкцизнойМарки(ЗначениеШтрихкода, ТипШтрихкода = Неопределено) Экспорт
	...
	Попытка
		ТипШтрихкодМарки.Проверить(ЗначениеШтрихкода);
	Исключение
		Возврат Ложь;
	КонецПопытки;
	
	Если СтрДлина(ЗначениеШтрихкода) = 150 Тогда
...
Процедура ДополнитьОбработанныеШтрихкодыПоДеревуУпаковок(ДанныеПоШтрихкодам)
	...
	Для Каждого КлючЗначение Из ОбработанныеШтрихкоды Цикл
		...
		ДополнитьСвойстваКоллекцииДанныеШтрихкода(ДанныеШтрихкода);
		СтрокаДерева = ДеревоУпаковок.Строки.Найти(КлючЗначение.Ключ, "ЗначениеШтрихкода");
...
Процедура ДополнитьДанныеШтрихкода(ДанныеСпецифики, ДанныеШтрихкода, СтрокаДерева, КодыАлкогольнойПродукции, ЭлементыДляЗаполненияАлкогольнойПродукции)
	...
	Если Не ЗначениеЗаполнено(ДанныеШтрихкода.АлкогольнаяПродукция)
		...
		Иначе
			ЗначениеШтрихкода = ДанныеШтрихкода.ЗначениеШтрихкода;
		КонецЕсли;
		
		ДанныеШтрихкода.КодАлкогольнойПродукции = АкцизныеМаркиВызовСервера.КодКлассификатораНоменклатурыЕГАИС(ЗначениеШтрихкода);
...
Функция РезультатОбработкиШтрихкодаМаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Если ЭтоШтрихкодАкцизнойМарки(ЗначениеШтрихкода) Тогда
	...
	ИначеЕсли ЭтоШтрихкодСНомеромИСерией(ЗначениеШтрихкода) Тогда
...
Функция ТипШтрихкода(ЗначениеШтрихкода, ТипУпаковки) Экспорт
	
	Если ШтрихкодированиеИСКлиентСервер.ЭтоУпаковка(ТипУпаковки) Тогда
		Возврат ТипШтрихкодаУпаковки(ЗначениеШтрихкода);
	Иначе
		Возврат ТипШтрихкодаМаркируемойПродукции(ЗначениеШтрихкода);
	КонецЕсли;
...
Функция ТипШтрихкодаМаркируемойПродукции(ЗначениеШтрихкода)
...
Функция ТипШтрихкодаУпаковки(ЗначениеШтрихкода)
...
Функция ДанныеНеизвестныхШтрихкодовАкцизныхМарок(Штрихкоды, ПараметрыСканирования)
	...
	Для Каждого ЗначениеШтрихкода Из Штрихкоды Цикл
		...
		НайденныеДанные = ДанныеНеизвестныхАкцизныхМарок[ЗначениеШтрихкода];
		Если НайденныеДанные <> Неопределено Тогда
			НайденныеДанные.Количество = НайденныеДанные.Количество + 1;
			Продолжить;
		КонецЕсли;
		
		ТипШтрихкода = ОпределитьТипШтрихкодаАкцизнойМарки(ЗначениеШтрихкода);
		...
		Если ТипШтрихкода = Перечисления.ТипыШтрихкодов.PDF417 Тогда
			АкцизныеМаркиВызовСервера.ЗаполнитьСопоставленнуюНоменклатуруПоАкцизнойМарке(ЗначениеШтрихкода, ДанныеШтрихкода);
		...
		Если ПараметрыСканирования.СоздаватьШтрихкодУпаковки
			...
			ДанныеНовойУпаковки.ЗначениеШтрихкода = ЗначениеШтрихкода;
		...
		ДанныеНеизвестныхАкцизныхМарок.Вставить(ЗначениеШтрихкода, ДанныеШтрихкода);
...
Функция ОпределитьТипШтрихкодаАкцизнойМарки(ЗначениеШтрихкода)

	ТипШтрихкодаАкцизнойМарки = Неопределено;
	
	Если АкцизныеМаркиВызовСервера.ЭтоШтрихкодАкцизнойМарки(ЗначениеШтрихкода, ТипШтрихкодаАкцизнойМарки) Тогда
...
Процедура ВыполнитьПроверкуНаОшибкиКонтрольАкцизныхМарок(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки)
	...
	Иначе
		...
		Для Каждого СтрокаМаркированныеТовары Из МаркированныеТовары Цикл
			...
			ТекстОшибки = "";
			ПроверитьМаркуПоСтатусу(СтрокаМаркированныеТовары.ЗначениеШтрихкода, СтрокаМаркированныеТовары.ШтрихкодУпаковки,
			...
			Если ПустаяСтрока(ТекстОшибки) И Не ЗначениеЗаполнено(СтрокаМаркированныеТовары.Статус) Тогда
				...
				АкцизныеМаркиЕГАИСПереопределяемый.ПроверитьУникальностьАкцизнойМарки(
					ПараметрыСканирования.ОперацияКонтроляАкцизныхМарок,
					СтрокаМаркированныеТовары.ЗначениеШтрихкода,
					ТекстОшибки);
...
Процедура ПроверитьМаркуПоСтатусу(ЗначениеШтрихкода, ШтрихкодУпаковки, СтатусМарки, ПараметрыСканирования, ТекстОшибки)
	
	ПредставлениеСтатуса = НСтр("ru = '<Нет статуса>';
								|en = '<Нет статуса>'");
	
	ЭтоМарка150Символов = ЭтоАкцизнаяМарка150Символов(ЗначениеШтрихкода);
...
Функция ЭтоАкцизнаяМарка150Символов(ЗначениеШтрихкода)
	
	Возврат ЭтоШтрихкодАкцизнойМарки(ЗначениеШтрихкода) И СтрДлина(ЗначениеШтрихкода) = 150;
