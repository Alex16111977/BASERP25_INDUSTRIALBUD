...
Функция РасшифроватьПереборомСертификатов(ЗашифрованныеДанные, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение(НСтр("ru = 'Недопустимое значение параметра ЗашифрованныеДанные - файл не является криптосообщением';
								|en = 'Invalid value of the ЗашифрованныеДанные parameter - the file is not a crypto message'"));
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Сервис криптографии.Выполнение запроса';
				|en = 'Digital signature in SaaS.Cryptography service.Query execution'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ЗашифроватьПроверкаВходящихПараметров(Данные, Получатели, ТипШифрования, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru = 'Недопустимое значение параметра ТипШифрования (неизвестный тип шифрования)';
			|en = 'Invalid value of the ТипШифрования parameter (unknown encryption type)'"), 
		"СервисКриптографии.Зашифровать");
	...
	Если ТипЗнч(Данные) = Тип("Строка") Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(
				...
				НСтр("ru = 'Недопустимое значение параметра Данные (указан адрес, который не является адресом временного хранилища)';
					|en = 'Invalid value of the Данные parameter (specified address is not a temporary storage address)'"), 
				"СервисКриптографии.Зашифровать");
	КонецЕсли;	
		...
		Если ТипЗнч(Элемент) = Тип("Строка") Тогда
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтрШаблон(НСтр("ru = 'Недопустимое значение параметра Данные[%1] (указан адрес, который не является адресом временного хранилища)';
								|en = 'Invalid value of the Data [%1] parameter (specified address is not a temporary storage address)'"), Индекс), 
				"СервисКриптографии.Зашифровать");
		КонецЕсли;
		Индекс = Индекс + 1;
	КонецЦикла;
	...
	Если ТипЗнч(ЗашифрованныеДанные) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru = 'Недопустимое значение параметра ЗашифрованныеДанные (указан адрес, который не является адресом временного хранилища)';
				|en = 'Invalid value of the ЗашифрованныеДанные parameter (specified address is not a temporary storage address)'"), 
			"СервисКриптографии.Расшифровать");
	КонецЕсли;
...
Процедура РасшифроватьБлокПроверкаВходящихПараметров(ЗашифрованныеДанные, Получатель, КлючеваяИнформация, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru = 'Отсутствует одно и/или более обязательных свойств параметра КлючеваяИнформация (ephemeral_key|session_key|iv_data)';
			|en = 'One and/or more required properties of the КлючеваяИнформация (ephemeral_key|session_key|iv_data) parameter are missing'"), 
		"СервисКриптографии.РасшифроватьБлок");
...
Процедура ХешированиеДанныхПроверкаВходящихПараметров(Данные, АлгоритмХеширования, ПараметрыХеширования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru = 'Недопустимое значение параметра АлгоритмХеширования (неизвестный алгоритм хеширования)';
			|en = 'Invalid value of the АлгоритмХеширования parameter (unknown hash algorithm)'"), 
		"СервисКриптографии.ХешированиеДанных");
...
Процедура ПодписатьПроверкаВходящихПараметров(Данные, Подписант, ТипПодписи, ПараметрыПодписания)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru = 'Недопустимое значение параметра ТипПодписи (неизвестный тип подписи)';
			|en = 'Invalid value of the ТипПодписи parameter (unknown signature type)'"), 
		"СервисКриптографии.Подписать");
	...
	Если ТипЗнч(Подпись) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru = 'Недопустимое значение параметра Подпись (указан адрес, который не является адресом временного хранилища)';
				|en = 'Invalid value of the Signature parameter (specified address is not a temporary storage address)'"), 
			"СервисКриптографии.ПроверитьПодпись");
	КонецЕсли;
	...
	Если ТипЗнч(Криптосообщение) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru = 'Недопустимое значение параметра Криптосообщение (указан адрес, который не является адресом временного хранилища)';
				|en = 'Invalid value of the Криптосообщение parameter (specified address is not a temporary storage address)'"), 
			"СервисКриптографии.ПолучитьСвойстваКриптосообщения");
	КонецЕсли;
	...
	Если ТипЗнч(СпособДоставкиПаролей) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru = 'Недопустимое значение параметра СпособДоставкиПаролей, допустимые значения ""phone"" или ""email""';
				|en = 'Invalid value of the СпособДоставкиПаролей parameter, available values are phone or email'"), 
			"СервисКриптографии.ПолучитьВременныйПароль");
	КонецЕсли;
...
Функция ИмяСобытияШифрование()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Шифрование';
				|en = 'Cryptography service.Encryption'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияШифрованиеБлока()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Шифрование блока';
				|en = 'Cryptography service.Block encryption'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияРасшифровка()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Расшифровка';
				|en = 'Cryptography service.Explanation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияРасшифровкаБлока()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Расшифровка блока';
				|en = 'Cryptography service.Block decryption'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияАутентификация()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Аутентификация';
				|en = 'Cryptography service.Authentication '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияСвойстваКриптосообщения()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Свойства криптосообщения';
				|en = 'Cryptography service.Crypto message properties'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияПроверкаСертификата()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Проверка сертификата';
				|en = 'Cryptography service.Certificate check'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияСвойстваСертификата()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Свойства сертификата';
				|en = 'Cryptography service.Certificate properties'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияПроверкаПодписи()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Проверка подписи';
				|en = 'Cryptography service.Signature check'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияХеширование()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Хеширование';
				|en = 'Cryptography service.Hashing'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ИмяСобытияПодписание()
	...
	Возврат НСтр("ru = 'Сервис криптографии.Подписание';
				|en = 'Cryptography service.Signing'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
