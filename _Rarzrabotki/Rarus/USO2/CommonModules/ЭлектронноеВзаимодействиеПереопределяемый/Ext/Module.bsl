...
Процедура ТекстСообщенияОНеобходимостиНастройкиСистемы(ВидОперации, ТекстСообщения) Экспорт
	...
	ИначеЕсли ВРег(ВидОперации) = "ДОПОЛНИТЕЛЬНЫЕОТЧЕТЫИОБРАБОТКИ" Тогда
		ТекстСообщения = НСтр("ru = 'Для возможности подключения через дополнительную обработку необходимо 
			...
			|en = 'To connect via additional data processor, enable usage of additional reports and data processors in the application settings
			|'");
	Иначе
		ТекстСообщения = НСтр("ru = 'Операция не может быть выполнена';
								|en = 'Operation cannot be executed'");
	КонецЕсли;
	//-- НЕ ГОСИС
	...
	Если ДокументыКорректировкиРеализацияУслугПрочихАктивов.Количество() > 0 Тогда
		ЭлектронноеВзаимодействиеУТ.УдалитьДокументыНеподходящиеДляФормированияЭД(
			...
			НСтр("ru = 'Документ %1 введен на основании документа ""Реализация услуг и прочих активов"". Электронный документ не сформирован.';
				|en = 'Document %1 is entered based on the ""Sales of goods and other assets"" document. Electronic document is not created. '"));
	КонецЕсли;
