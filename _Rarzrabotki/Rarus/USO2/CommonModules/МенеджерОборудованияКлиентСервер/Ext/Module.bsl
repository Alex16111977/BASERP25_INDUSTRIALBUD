...
Функция ПолучитьСерийныйНомерПоTID(Знач TID, Знач EPC = Неопределено) Экспорт
	...
	Если ПризнакG2 <> "E2" Тогда // Фиксированное значение "E2" признак того что тип соответствует EPC Class1Gen2. 
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru = 'Указанный TID не соответствует типу EPC Class1Gen2.';
													|en = 'Specified TID does not correspond to the EPC Class1Gen2 type.'");
	...
	Иначе
		...
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru = 'Генерация серийного номера для чипа производителя указанного в TID не поддерживается.';
													|en = 'Generating serial number for the chip of the manufacturer specified in TID is not supported.'");
		Возврат РезультатВыполнения;
...
Функция СформироватьТекстНефискальногоДокумента(ТипДокумента, ОбщиеПараметры, ШиринаСтроки = 32, ДополнительныйТекст = Неопределено, ФорматФФД = "1.1") Экспорт
	...
	Если ТипДокумента = 0 Тогда
		...
		ТипыЧеков.Вставить(1, НСтр("ru = 'ПРИХОД';
									|en = 'RECEIPT'"));          // Чек продажи     
		ТипыЧеков.Вставить(2, НСтр("ru = 'ВОЗВРАТ ПРИХОДА';
									|en = 'RECEIPT RETURN'")); // Чек возврата продажи 
		ТипыЧеков.Вставить(3, НСтр("ru = 'РАСХОД';
									|en = 'EXPENSE'"));          // Чек покупки 
		ТипыЧеков.Вставить(4, НСтр("ru = 'ВОЗВРАТ РАСХОДА';
	...
	Если ПечатьКлише Тогда 
		...
		Если ТипДокумента = 0  Тогда
			...
			Если НЕ ПустаяСтрока(ОбщиеПараметры.Получатель) Тогда
				Текст = Текст + ПостроитьПолеПереносом(" " + НСтр("ru = 'ПОКУПАТЕЛЬ:';
																	|en = 'CUSTOMER:'") + ОбщиеПараметры.Получатель, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ПолучательИНН) Тогда
				Текст = Текст + ПостроитьПоле(" " + НСтр("ru = 'ПОКУПАТЕЛЬ ИНН:';
														|en = 'CUSTOMER TIN'") + ОбщиеПараметры.ПолучательИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			...
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ПостроитьПолеПереносом(" " + НСтр("ru = 'ПОСТАВЩИК:';
																	|en = 'SUPPLIER:'") + ОбщиеПараметры.ДанныеПоставщика.Наименование, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПОСТАВЩИК ТЛФ:';
														|en = 'PHONE SUPPLIER:'"), ОбщиеПараметры.ДанныеПоставщика.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПОСТАВЩИК ИНН:';
														|en = 'SUPPLIER TIN:'"), ОбщиеПараметры.ДанныеПоставщика.ИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Операция) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПЛ.АГЕНТ ОПЕР.:';
														|en = 'PAYMENT AGENT OPERATION:'"), ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Операция, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля("  " + НСтр("ru = 'ПЛ.АГЕНТ.ТЛФ.:';
															|en = 'PAYMENT AGENT PHONE NUMBER:'"), ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ОПЕРАТОР ПЕРЕВОДА:';
														|en = 'TRANSACTION OPERATOR:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Наименование, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля("   " + НСтр("ru = 'ТЛФ.ОП.ПЕРЕВОДА:';
															|en = 'TRANSACTION OPERATOR PHONE NUMBER:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Адрес) Тогда
				Текст = Текст + ВыстроитьПоля("   " + НСтр("ru = 'АДР.ОП.ПЕРЕВОДА:';
															|en = 'TRANSACTION OPERATOR ADDRESS:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Адрес, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПеревода.ИНН) Тогда
				Текст = Текст + ВыстроитьПоля("   " + НСтр("ru = 'ИНН ОП.ПЕРЕВОДА:';
															|en = 'TRANSACTION OPERATOR TIN:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.ИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';
														|en = 'OPERATOR TO RECEIVE PAYMENTS PHONE NUMBER:'"), ОбщиеПараметры.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДополнительныйРеквизитПользователя.Наименование) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'РЕК.ПОЛЬЗ.:';
														|en = 'USER ADVERT:'"), ОбщиеПараметры.ДополнительныйРеквизитПользователя.Наименование, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДополнительныйРеквизитПользователя.Значение) Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'РЕК.ПОЛЬЗ.:';
														|en = 'USER ADVERT:'"), ОбщиеПараметры.ДополнительныйРеквизитПользователя.Значение, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если СуммаСкидок > 0 Тогда
					ТекстСкидки = НСтр("ru = 'СКИДКА';
										|en = 'DISCOUNT'") + "=" + Формат(СуммаСкидок, ФорматЧисла);
				ИначеЕсли СуммаСкидок < 0 Тогда
					ТекстСкидки = НСтр("ru = 'НАДБАВКА';
										|en = 'STANDARD BONUS'") + "=" + Формат(СуммаСкидок, ФорматЧисла);
				Иначе
					ТекстСкидки = "";
				КонецЕсли;
				...
				Если ФорматФФД = "1.1" Тогда
					...
					Если НЕ ПустаяСтрока(КТ) Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru = 'КТ:';
																|en = 'CR:'") + КТ, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
					
					Если НЕ ПустаяСтрока(ПозицияЧека.КодСтраныПроисхожденияТовара) Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru = 'КОД СТРАНЫ:';
																|en = 'COUNTRY CODE:'") + ПозицияЧека.КодСтраныПроисхожденияТовара, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
					Если НЕ ПустаяСтрока(ПозицияЧека.НомерТаможеннойДекларации) Тогда
						Текст = Текст + ПостроитьПолеПереносом(" " + НСтр("ru = 'ДЕКЛАРАЦИЯ:';
																			|en = 'DECLARATION:'") + ПозицияЧека.НомерТаможеннойДекларации, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
					Если НЕ ПустаяСтрока(ПозицияЧека.СуммаАкциза) И ПозицияЧека.СуммаАкциза > 0 Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru = 'АКЦИЗ:';
																|en = 'EXCISE:'") + ПозицияЧека.СуммаАкциза, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
					Если НЕ ПустаяСтрока(ПозицияЧека.ЕдиницаИзмерения) Тогда
						Текст = Текст + ПостроитьПоле(" " + НСтр("ru = 'ЕД.ИЗМЕРЕНИЯ:';
																|en = 'MEASUREMENT UNIT:'") + ПозицияЧека.ЕдиницаИзмерения, ШиринаСтроки) + Символы.ПС;
					КонецЕсли;
				КонецЕсли;
				...
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПОСТАВЩИК:';
															|en = 'SUPPLIER:'"), ПозицияЧека.ДанныеПоставщика.Наименование, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПОСТАВЩИК ТЛФ.:';
															|en = 'SUPPLIER PHONE NUMBER:'"), ПозицияЧека.ДанныеПоставщика.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля("  " + НСтр("ru = 'ПОСТАВЩИК ИНН:';
																|en = 'SUPPLIER TIN:'"), ПозицияЧека.ДанныеПоставщика.ИНН, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Операция) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПЛ.АГЕНТ ОПЕР.:';
															|en = 'PAYMENT AGENT OPERATION:'"), ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Операция, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = ' ПЛ.АГЕНТ ТЛФ.:';
															|en = ' PAYMENT AGENT PHONE NUMBER:'"), ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ОПЕРАТОР ПЕРЕВОДА:';
															|en = 'TRANSACTION OPERATOR:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Наименование, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля("   " + НСтр("ru = 'ТЛФ.ОП.ПЕРЕВОДА:';
																|en = 'TRANSACTION OPERATOR PHONE NUMBER:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.Адрес) Тогда
					Текст = Текст + ВыстроитьПоля("   " + НСтр("ru = 'АДР.ОП.ПЕРЕВОДА:';
																|en = 'TRANSACTION OPERATOR ADDRESS:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Адрес, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПеревода.ИНН) Тогда
					Текст = Текст + ВыстроитьПоля("   " + НСтр("ru = 'ИНН ОП.ПЕРЕВОДА:';
																|en = 'TRANSACTION OPERATOR TIN:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.ИНН, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля("  " + НСтр("ru = 'ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';
																|en = 'OPERATOR TO RECEIVE PAYMENTS PHONE NUMBER:'"), ПозицияЧека.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДополнительныйРеквизит) Тогда
					Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ДОП.РЕК.:';
															|en = 'ADD. ATTR.:'"), ПозицияЧека.ДополнительныйРеквизит, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Текст = Текст + РазделительВнут;
		Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ИТОГ';
											|en = 'TOTAL'"), "=" + Формат(СуммаЧека, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		Текст = Текст + РазделительВнут;
		
		Текст = Текст + НСтр("ru = 'ОПЛАТА';
							|en = 'PAYMENT'") + Символы.ПС;
		
		Если ФорматФФД = "1.0" Тогда // ФФД 1.0
			Если СуммаНаличными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'НАЛИЧНЫМИ';
														|en = 'CASH'"), "=" + Формат(СуммаНаличными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаЭлектронными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
														|en = 'ELECTRONIC'"), "=" + Формат(СуммаЭлектронными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПостоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
														|en = 'ELECTRONIC'"), "=" + Формат(СуммаПостоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПредоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
														|en = 'ELECTRONIC'"), "=" + Формат(СуммаПредоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
			Если СуммаПредоставлением > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
														|en = 'ELECTRONIC'"), "=" + Формат(СуммаПредоставлением, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
		Иначе // ФФД 1.0.5 & ФФД 1.1
			...
			КонецЕсли;
			Если СуммаПредоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПРЕДВАРИТЕЛЬНАЯ';
														|en = 'PRELIMINARY'"), "=" + Формат(СуммаПредоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
			Если СуммаПостоплатой > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ПОСЛЕДУЮЩАЯ';
														|en = 'SUBSEQUENT'"), "=" + Формат(СуммаПостоплатой, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПредоставлением > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'ИНАЯ ФОРМА';
														|en = 'OTHER FORM'"), "=" + Формат(СуммаПредоставлением, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;          
		КонецЕсли; 
		
		Если СуммаОплаты - СуммаЧека > 0 Тогда
			Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ПОЛУЧЕНО';
												|en = 'RECEIVED'"), "=" + Формат(СуммаОплаты, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'СДАЧА';
													|en = 'CHANGE'"), "=" + Формат(СуммаОплаты - СуммаЧека, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли; 
		...
		Если ОбщиеПараметры.Свойство("СистемаНалогообложения") Тогда
			...
			Текст = Текст + ВыстроитьПоля(НСтр("ru = 'СНО:';
												|en = 'TS:'"), СистемаНалогообложения, ШиринаСтроки) + Символы.ПС;
		КонецЕсли;          
		
		Если СуммаПозицийНДС20 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС20 > 0, СуммаНДС20, СуммаПозицийНДС20 / 120 * 20);
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА НДС 20%';
												|en = ' VAT AMOUNT 20%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС18 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС18 > 0, СуммаНДС18, СуммаПозицийНДС18 / 118 * 18);
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА НДС 18%';
												|en = ' VAT AMOUNT 18%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС10 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС10 > 0, СуммаНДС10, СуммаПозицийНДС10 / 110 * 10);
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА НДС 10%';
												|en = ' VAT AMOUNT 10%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС120 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС120 > 0, СуммаНДС120, СуммаПозицийНДС120 / 120 * 20);
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА НДС 20/120%';
												|en = ' VAT AMOUNT 20/120%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС118 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС118 > 0, СуммаНДС118, СуммаПозицийНДС118 / 118 * 18);
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА НДС 18/118%';
												|en = ' VAT AMOUNT 18/118%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС110 > 0 Тогда
			ЗначениеНДС = ?(СуммаНДС110 > 0, СуммаНДС110, СуммаПозицийНДС110 / 110 * 10);
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА НДС 10/110%';
												|en = ' VAT AMOUNT 10/110%'"), "=" + Формат(ЗначениеНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийНДС0 > 0 Тогда
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА C НДС 0%';
												|en = ' AMOUNT WITH VAT 0%'")  , "=" + Формат(СуммаПозицийНДС0, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		Если СуммаПозицийБезНДС > 0 Тогда
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА БЕЗ НДС';
												|en = ' AMOUNT VAT EXEMPT'"), "=" + Формат(СуммаПозицийБезНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		
		Кассир = ?(ОбщиеПараметры.Свойство("Кассир"), ОбщиеПараметры.Кассир, "");
		Текст = Текст + ПостроитьПоле(НСтр("ru = 'КАССИР';
											|en = 'CASHIER'") + ":" + Символы.НПП + Кассир, ШиринаСтроки) + Символы.ПС + Символы.ПС;                        
		Текст = Текст + ПостроитьПоле(НСтр("ru = 'ПОДПИСЬ';
											|en = 'SIGNATURE'") + ":", ШиринаСтроки, "_") + Символы.ПС;
...
Функция ПараметрыQRКодаЧекаККТ() Экспорт
	...
	ПараметрыШтрихкода.Вставить("СуммаРасчета", 0);  // Cумма расчета в рублях и копейках
...
Функция СформироватьQRКодЧекаККТ(ПараметрыQRКода) Экспорт
	...
	ДанныеQRКода = СтрЗаменить(ДанныеQRКода, "%I%", ПараметрыQRКода.НомерФискальногоДокумента);
...
Функция НаименованиеСистемыНалогообложения(СистемаНалогообложения, Кратко = Ложь) Экспорт;
	...
	Иначе
		...
		СистемыНалогообложения.Вставить(3, НСтр("ru = 'Единый налог на вмененный доход';
												|en = 'Unified tax on imputed income'"));
...
Функция НаименованиеПризнакПредметаРасчетаПоКоду(КодПризнакаПредметаРасчета) Экспорт;
	...
	ПризнакиПредметаРасчета.Вставить(1, НСтр("ru = 'Доход от долевого участия в других организациях';
											|en = 'Income from equity participation in other companies'"));
	ПризнакиПредметаРасчета.Вставить(2, НСтр("ru = 'Доход в виде курсовой разницы, образующейся вследствие отклонения курса продажи (покупки) иностранной валюты от официального курса';
											|en = 'Income in the form of exchange rate differences resulting from the variance of the sale rate (purchase) of foreign currency from the official rate'"));
	ПризнакиПредметаРасчета.Вставить(3, НСтр("ru = 'Доход в виде подлежащих уплате должником штрафов, пеней и (или) иных санкций за нарушение договорных обязательств';
											|en = 'Income in the form of fines payable by the debtor, penalties and (or) other sanctions for breaching the contractual liabilities'"));
	ПризнакиПредметаРасчета.Вставить(4, НСтр("ru = 'Доход от сдачи имущества (включая земельные участки) в аренду (субаренду)';
											|en = 'Income from property (including land plots) lease (sublease)'"));
	ПризнакиПредметаРасчета.Вставить(5, НСтр("ru = 'Доход от предоставления в пользование прав на результаты интеллектуальной деятельности';
											|en = 'Income from granting rights for intellectual property'"));
	ПризнакиПредметаРасчета.Вставить(6, НСтр("ru = 'Доход в виде процентов, полученных по договорам займа и другим долговым обязательствам';
											|en = 'Interest income from loan contracts and other debt liabilities'"));
	ПризнакиПредметаРасчета.Вставить(7, НСтр("ru = 'Доход в виде сумм восстановленных резервов';
											|en = 'Income in the form of recovered reserves'"));
	ПризнакиПредметаРасчета.Вставить(8, НСтр("ru = 'Доход в виде безвозмездно полученного имущества (работ, услуг) или имущественных прав';
											|en = 'Income in the form of donated property (works, services) or property rights'"));
	ПризнакиПредметаРасчета.Вставить(9, НСтр("ru = 'Доход в виде дохода, распределяемого в пользу налогоплательщика при его участии в простом товариществе';
											|en = 'Income in the form of revenue distributed in favor of the taxpayer when he participates in a simple partnership'"));
	ПризнакиПредметаРасчета.Вставить(10, НСтр("ru = 'Доход в виде дохода прошлых лет, выявленного в отчетном (налоговом) периоде';
												|en = 'Income in the form of income for previous years, identified in the reporting (fiscal) period'"));
	ПризнакиПредметаРасчета.Вставить(11, НСтр("ru = 'Доход в виде положительной курсовой разницы';
												|en = 'Income in the form of positive exchange rate differences'"));
	ПризнакиПредметаРасчета.Вставить(12, НСтр("ru = 'Доход в виде основных средств и нематериальных активов, безвозмездно полученных атомными станциями';
												|en = 'Income in the form of fixed assets and intangible assets received free of charge by nuclear power plants'"));
	ПризнакиПредметаРасчета.Вставить(13, НСтр("ru = 'доход в виде стоимости полученных материалов при ликвидации выводимых из эксплуатации основных средств';
												|en = 'income in the form of materials value, that are as a result of liquidation of fixed assets'"));
	ПризнакиПредметаРасчета.Вставить(14, НСтр("ru = 'Доход в виде использованных не по целевому назначению имущества, работ, услуг';
												|en = 'Income in the form of property, works, services used for non-earmarked purposes'"));
	ПризнакиПредметаРасчета.Вставить(15, НСтр("ru = 'Доход в виде использованных не по целевому назначению средств, предназначенных для формирования резервов по обеспечению безопасности производств';
												|en = 'Income in the form of funds used in non-earmarked purposes, originally intended to form reserves that ensure the safety of production sites'"));
	ПризнакиПредметаРасчета.Вставить(16, НСтр("ru = 'Доход в виде сумм, на которые уменьшен уставной (складочный) капитал (фонд) организации';
												|en = 'Income in the form of amounts for which the capital (joint-stock capital) stock (fund) of the organization is reduced'"));
	ПризнакиПредметаРасчета.Вставить(17, НСтр("ru = 'Доход в виде сумм возврата от некоммерческой организации ранее уплаченных взносов (вкладов)';
												|en = 'Income in the form of return amounts of previously paid contributions (deposits) from the non-profit organization'"));
	ПризнакиПредметаРасчета.Вставить(18, НСтр("ru = 'Доход в виде сумм кредиторской задолженности, списанной в связи с истечением срока исковой давности или по другим основаниям';
												|en = 'Income in the form of accounts payable written off due to limitation period expiration or for other reasons'"));
	ПризнакиПредметаРасчета.Вставить(19, НСтр("ru = 'Доход в виде доходов, полученных от операций с производными финансовыми инструментами';
												|en = 'Income in the form of incomes earned from transactions with derived financial instruments'"));
	ПризнакиПредметаРасчета.Вставить(20, НСтр("ru = 'Доход в виде стоимости излишков материально-производственных запасов и прочего имущества, которые выявлены в результате инвентаризации';
												|en = 'Income in the form of cost of inventory items surplus and other property detected after stocktaking'"));
	ПризнакиПредметаРасчета.Вставить(21, НСтр("ru = 'Доход в виде стоимости продукции СМИ и книжной продукции, подлежащей замене при возврате либо при списании';
												|en = 'Income in the form of cost of mass media and book products which are subject to replacement upon return or upon write-off'"));
	ПризнакиПредметаРасчета.Вставить(22, НСтр("ru = 'Доход в виде сумм корректировки прибыли налогоплательщика';
												|en = 'Income in the form of amounts of taxpayer profit adjustment'"));
	ПризнакиПредметаРасчета.Вставить(23, НСтр("ru = 'Доход в виде возвращенного денежного эквивалента недвижимого имущества и (или) ценных бумаг, переданных на пополнение целевого капитала некоммерческой организации';
												|en = 'Income in the form of returned cash equivalent of real estate and (or) securities passed for replenishment of non-profit organization endowment'"));
	ПризнакиПредметаРасчета.Вставить(24, НСтр("ru = 'Доход в виде разницы между суммой налоговых вычетов из сумм акциза и указанных сумм акциза';
												|en = 'Income in the form of the difference between the amount of tax deductions from excise amounts and the specified excise amounts'"));
	ПризнакиПредметаРасчета.Вставить(25, НСтр("ru = 'Доход в виде прибыли контролируемой иностранной компании';
												|en = 'Income in the form of the profit of a controlled foreign company'"));
	...
	ПризнакиПредметаРасчета.Вставить(27, НСтр("ru = 'Взносы на ОСС в связи с нетрудоспособностью';
												|en = 'Contributions for CSI due to incapacity '"));
