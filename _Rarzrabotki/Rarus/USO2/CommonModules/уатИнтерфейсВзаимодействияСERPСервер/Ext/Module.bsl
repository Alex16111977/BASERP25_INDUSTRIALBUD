...
Процедура уатЗаполнитьЦеныНоменклатуры(ДокОбъект, ИмяТЧ, Склад) Экспорт
	...
	|			И (СправочникКлючейАналитики.МестоХранения = &Склад);
	|
	|	ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
//
Функция ПолучитьБригадуСотрудника(ФизЛицо, Период) Экспорт
	Если Метаданные.Документы.Найти("ВыработкаСотрудников") = Неопределено Тогда
	...
	|	И (ВыработкаСотрудников.Работники.Работник = &ФизЛицо
	|	ИЛИ ВыработкаСотрудников.ВидыРабот.Работник = &ФизЛицо)
	|	И ВыработкаСотрудников.Проведен
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВыработкаСотрудников.Дата УБЫВ");
