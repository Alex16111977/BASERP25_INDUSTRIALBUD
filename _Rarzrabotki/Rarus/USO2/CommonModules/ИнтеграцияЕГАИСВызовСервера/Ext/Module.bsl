		...
		Для Каждого ЭлементОчереди Из Очередь Цикл
			...
			Исключение
				...
				Если Не (Не ИнтеграцияЕГАИС.СообщатьОбОшибкахПриЗагрузкеДанных()
					И ( СтрНайти(НРег(ТекстОшибки), НСтр("ru = 'не найден документ';
														|en = 'не найден документ'")) > 0
					Или СтрНайти(НРег(ТекстОшибки), НСтр("ru = 'не найден исходящий запрос';
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки) Экспорт
	...
	ИнтеграцияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
		ТекстОшибки,
		НСтр("ru = 'ЕГАИС';
			|en = 'ЕГАИС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
