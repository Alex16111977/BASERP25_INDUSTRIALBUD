...
Процедура ПолучениеИОтправкаЭлектронныхПисем() Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru = 'Начато регламентное получение и отправка электронной почты';
			|en = 'Scheduled email receiving and sending is started'", ОбщегоНазначения.КодОсновногоЯзыка()));
				...
				Если ЭтоПроблемаОтвергнутыхСерверомПочтовыхАдресов 
					...
					ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Следующие адресаты электронного письма %1 не приняты почтовым сервером:
							...
							|%2. Email is sent to other addressees.'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), 
		...
		НСтр("ru = 'Закончено регламентное получение и отправка электронной почты';
			|en = 'Scheduled receiving and sending of emails is completed'", ОбщегоНазначения.КодОсновногоЯзыка()));
			...
			Для Каждого Сообщение Из Сообщения Цикл
				...
				Исключение
					...
					ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					                      ...
					                      |%4';
					                      |en = 'Error %4
					                      |occurred while receiving email %1 from %2 sent from address %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
					                      Сообщение.Тема,
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не установить письмо основание у %Ссылка% по причине: %Причина%';
									|en = 'Cannot determine the basis email at %Ссылка% due to: %Причина%'");
	...
	Если ПустаяСтрока(ИмяБезРасширения) Тогда
		...
		ИмяБезРасширения =
			НСтр("ru = 'Вложение без имени';
				|en = 'Attachment without name'") + ?(КоличествоПустыхИменВоВложениях = 0, ""," " + Строка(КоличествоПустыхИменВоВложениях + 1));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Взаимодействия';
				|en = 'Interactions'", ОбщегоНазначения.КодОсновногоЯзыка());
