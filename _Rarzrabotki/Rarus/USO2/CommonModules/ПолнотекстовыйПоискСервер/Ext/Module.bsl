...
Процедура СлияниеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ru = 'Слияние индекса ППД';
						|en = 'Merging full-text search index.'"), Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление = НСтр("ru = 'Ошибка настройки полнотекстового поиска';
								|en = 'An error occurred while setting full-text search'");
	...
	Дело.Подсказка = 
		НСтр("ru = 'Рассинхронизация значения константы ИспользоватьПолнотекстовыйПоиск
		           ...
		           |en = 'The value of the UseFullTextSearch
		           | constant is out of sync with the set full-text search mode in infobase.
		           |Try to disable full-text search and enable it again.'");
	...
	Дело.Представление = НСтр("ru = 'Индекс полнотекстового поиска устарел';
								|en = 'Full-text search index is out of date'");
	Дело.Форма = "Обработка.ПолнотекстовыйПоискВДанных.Форма.УправлениеПолнотекстовымПоискомИИзвлечениемТекстов";
	...
	Исключение
		ЗаписьЖурнала(
			...
			НСтр("ru = 'Не удалось проверить состояние индекса полнотекстового поиска';
				|en = 'Cannot check status of the full-text search index'"),
			ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ВыполнитьПолнотекстовыйПоиск(ПараметрыПоиска) Экспорт 
	...
	Попытка
		...
		Иначе 
			ВызватьИсключение НСтр("ru = 'Параметр НаправлениеПоиска задан неверно.';
									|en = 'The НаправлениеПоиска parameter is incorrect.'");
		КонецЕсли;
	Исключение
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда 
		ОписаниеОшибки = НСтр("ru = 'Слишком много результатов, уточните запрос';
								|en = 'Too many results, refine your request'");
	...
	Попытка
		...
		ЗаписьЖурнала(
			...
			НСтр("ru = 'Успешное завершение процедуры ""%1"".';
				|en = 'Procedure ""%1"" is completed successfully.'"),, 
			ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(
			УровеньЖурналаРегистрации.Предупреждение, 
			НСтр("ru = 'Не удалось выполнить процедуру ""%1"":';
				|en = 'Cannot execute the ""%1"" procedure:'"),
			ИнформацияОбОшибке(), 
			ПредставлениеПроцедуры);
	КонецПопытки;
