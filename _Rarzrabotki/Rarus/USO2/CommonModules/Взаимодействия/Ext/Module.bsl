	...
	Для Каждого Элемент Из МассивЗаменить Цикл
		...
		Исключение
			...
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при замене предмета у %1: %2';
					|en = 'An error occurred when replacing subject from %1: %2'"), Элемент, ПредставлениеОшибки), , , , Отказ);
			Отказ = Истина;
			Возврат;
		КонецПопытки;
	КонецЦикла;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Переносится хранение реквизитов ""Рассмотрено"" и ""РассмотретьПосле"" в регистр сведений ""ПредметыПапкиВзаимодействий"". Без выполнения обработчика значения данных реквизитов будут отображаться некорректно.';
									|en = 'Storage of the ""Рассмотрено"" and ""РассмотретьПосле"" attributes is transferred to the ""ПредметыПапкиВзаимодействий"" information register. If handler is not run, attribute data values will be displayed incorrectly.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняется вспомогательный регистр сведений ""КонтактыВзаимодействий"". Без выполнения обработчика не будут корректно работать отборы по контактам взаимодействий.';
									|en = 'Auxiliary information register ""КонтактыВзаимодействий"" is populated. Filters by interaction contacts will not function correctly if the handler is not executed.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняется первоначальный расчет количества нерассмотренных писем по папкам. Без выполнения обработчика не будет корректно выводится информация о количестве нерассмотренных писем по папкам.';
									|en = 'Initial calculation of the number of not reviewed emails by folders is in progress. Information on the number of not reviewed emails by folders will not be displayed correctly without the handler execution. '");
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняется первоначальный расчет количества состояний предметов взаимодействий. Без выполнения обработчика не будет корректно выводится информация в панель навигации журнала взаимодействий по предметам.';
									|en = 'Initial calculation of the number of subject states is in progress. The information will not be displayed correctly in the navigation panel of the interaction log by subjects without the handler execution. '");
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняется первоначальный расчет количества состояний контактов взаимодействий. Без выполнения обработчика не будет корректно выводится информация в панель навигации журнала взаимодействий по контактам.';
									|en = 'Initial calculation of the number of interaction contact states is in progress. The information will not be displayed correctly in the navigation panel of the interaction log by contacts without the handler execution. '");
	...
	Обработчик.Комментарий = НСтр("ru = 'Выполняет дозаполнение реквизита ВзаимодействиеОснование для входящих писем, у которых письмо основание в ИБ есть, но оно не указано.';
									|en = 'Fills in the ВзаимодействиеОснование attribute for incoming emails which have basis email in the infobase but it is not specified.'");
	...
	Обработчик.Комментарий =
		НСтр("ru = 'Технологический перенос данных идентификаторов сообщения IMAP (реквизит ИдентификаторСообщенияОтправкаIMAP) для предотвращения потери данных
		           ...
		           |en = 'Technological transfer of IMAP message ID data (ИдентификаторСообщенияОтправкаIMAP attribute) to prevent data loss
		           |when transferring to the next application release.
		           |Fills in the ВзаимодействиеОснование attribute for outgoing emails that have a basis email in the infobase but it is not specified'");
	Обработчик.РежимВыполнения = "Отложенно";
...
Функция СписокДоступныхПоисков(ППДВключен, Параметры, ЭлементыФормы, ДляАдреснойКниги) Экспорт
	...
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоEmail", НСтр("ru = 'По email';
																			|en = 'By email'"), Адрес);
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоДомену", НСтр("ru = 'По доменному имени';
																			|en = 'By domain name'"), ДоменныйАдрес);
	
	Если Не ДляАдреснойКниги
		И (Параметры.Свойство("ТолькоEmail") 
		   И Не Параметры.ТолькоEmail) Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоТелефону", НСтр("ru = 'По телефону';
																				|en = 'By phone number'"), Адрес);
	КонецЕсли;
	...
	Если ППДВключен Тогда
		ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "ПоСтроке",
			НСтр("ru = 'По строке';
				|en = 'By line'"), ВариантыПоискаПоСтроке);
	КонецЕсли;
	
	ДобавитьВариантПоиска(ВсеСпискиПоиска, ЭлементыФормы, "НачинаетсяС", НСтр("ru = 'Начинается с';
...
Функция ОсуществитьПоискКонтактовПоСтроке(Форма,ДляАдреснойКниги = Ложь) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Запретить
		...
		Возврат НСтр("ru = 'Использование индекса полнотекстового поиска данных запрещено.';
					|en = 'Full-text search index use is prohibited.'");
	КонецЕсли;
	...
	Возврат ?(КоличествоНайденныхЭлементов < 101, "", НСтр("ru = 'Уточните параметры поиска. В списке отображены не все найденные контакты.';
															|en = 'Review search parameters. Not all found contacts are displayed in the list.'"));
...
Процедура УстановитьЗаголовокФормыЭлектронногоПисьма(Форма) Экспорт
	...
	Если НЕ ОбъектПисьмо.Ссылка.Пустая() Тогда
		...
		ЗаголовокФормы = ?(ПустаяСтрока(ОбъектПисьмо.Тема), НСтр("ru = 'Письмо без темы';
																|en = 'Email without subject'"), ОбъектПисьмо.Тема);
	...
	Пока ВыборкаПисьма.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось заблокировать документ: %Регистратор% по причине: %Причина%';
									|en = 'Cannot lock document: %Регистратор% due to: %Причина%'");
...
Функция СформироватьЭлементДанныхШапкиПисьма(ЭлементРодитель,Выборка,ТолькоПоПредставлениюОтправителя = Ложь) Экспорт
	...
	Тема = ?(ПустаяСтрока(Выборка.Тема), НСтр("ru = '<Без Темы>';
												|en = '<No Subject>'"), Выборка.Тема);
...
Процедура ДополнитьТелоПисьмаПодваломВложения(ДокументHTML, Вложения) Экспорт
	...
	СтрокаКоличествоВложений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вложений: %1.';
																							|en = 'Attachments: %1'"), Вложения.Количество());
...
Функция ОтправкаSMSПоДокументу(Документ) Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не выполнены настройки отправки SMS.';
													|en = 'Settings of SMS message sending are not executed.'"),,"Объект");
...
Процедура ОтправкаSMS() Экспорт
	...
	Если Не ОтправкаSMS.НастройкаОтправкиSMSВыполнена() Тогда
		...
		НСтр("ru = 'Не выполнены настройки отправки SMS.';
			|en = 'Settings of SMS message sending are not executed.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось отправить : %Ссылка% по причине: %Причина%';
									|en = 'Cannot send : %Ссылка% due to: %Причина%'");
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось обновить статусы доставки : %Ссылка% по причине: %Причина%';
									|en = 'Cannot update delivery statuses: %Ссылка% due to: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", ВыборкаДокументы.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(),
			                         УровеньЖурналаРегистрации.Предупреждение,
	...
	Попытка
		...
		Если НЕ НовыйРодитель.Пустая()Тогда
			...
			Если ЗначенияРеквизитовПапки <> Неопределено 
				...
				И ЗначенияРеквизитовПапки.Наименование = НСтр("ru = 'Удаленные';
																|en = 'Deleted'") Тогда
...
Процедура ВыполнитьУдалениеПапкиЭлектронныхПисем(Папка, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'При удалении папки произошла ошибка.
		                      ...
		                      |en = 'An error occurred when deleting the folder.
...
Функция КомментарийПоНаименованиюШаблона(НаименованиеШаблона)
	...
	Возврат НСтр("ru = 'Создано и отправлено по шаблону';
				|en = 'Created and sent from template'") + " - " + НаименованиеШаблона;
