...
Процедура ВставитьПоказательВСтруктуру(СтруктураПоказателей, ИмяПоказателя, ПризнакАвтозаполнения, ПризнакРасшифровки,
									   ПризнакЗапретаРедактирования = Ложь) Экспорт
...
Функция ПолучитьДокФизЛица(ФизЛицо, ДатаЗначения) Экспорт
	...
	Удостоверения = РегистрыСведений.ДокументыФизическихЛиц.СрезПоследних(
		ДатаЗначения, Новый Структура("ФизЛицо", ФизЛицо));
...
Процедура РегламентированныйОтчетОтправляетсяВКонтролирующиеОрганы(ДокСсылка, ОбъектОтправляется) Экспорт
	...
	РеглОтчетыНеОтправляютсяВКонтролирующиеОрганы.Добавить("РегламентированныйОтчетДоходыОтТуристскоРекреационнойДеятельности");
...
Функция КоличествоОбъектовНеЗаписанныхВРегистр(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Проверка полноты данных журнала регламентированной отчетности';
								|en = 'Check completeness of compliance reporting log data'");
...
Функция ДобавлениеОбъектовОтсутствующихВРегистре(ПараметрыЗапуска) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Заполнение данных журнала регламентированной отчетности';
								|en = 'Fill in data of compliance reporting log '");
...
Процедура ОбработатьСобытие1СОтчетности(ИмяСобытия, СсылкаНаИсточник = Неопределено, ДопПараметрСтруктура = Неопределено) Экспорт
	...
	Если ИмяСобытия = НСтр("ru = 'Регламентированный отчет. Создание';
							|en = 'Compliance report. Creation'") Тогда
	...
	ИначеЕсли ИмяСобытия = НСтр("ru = 'Регламентированный отчет. Выгрузка';
								|en = 'Compliance report. Export'") Тогда
	...
	ИначеЕсли ИмяСобытия = НСтр("ru = 'Регламентированный отчет. Проверка выгрузки';
								|en = 'Compliance report. Export check'") Тогда
...
Функция РеглОтчеты(Наим, НеЗапоминатьВКэше = Ложь, ВыводитьСообщение = Истина) Экспорт
	...
	Если НЕ ПризнакИспользованияВнешнегоОтчетаСУчетомКэша(НайденныйЭлемент, НеЗапоминатьВКэше) ИЛИ НайденныйЭлемент.ВнешнийОтчетХранилище = Неопределено Тогда // используется внутренний отчет
		...
		Иначе
			...
			Если ВыводитьСообщение Тогда
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найден отчет %1!';
	...
	Иначе // используется внешний отчет
		...
		Исключение
			...
			Если ВыводитьСообщение Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось запустить внешний отчет %1!';
...
Функция ПродолжитьПриНесоответствииВерсийФорм(Знач ВерсияФормы, Знач мВерсияФормы) Экспорт
	...
	Если ДатамВерсияФормы < ДатаВерсияФормы Тогда
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сохраненные данные не могут быть открыты текущей версией отчета!%1Версия отчета - от %2Версия данных - от %3';
																						|en = 'Saved data cannot be opened by the current report version.%1 Report version from %2 Data version from %3'"), Символы.ПС, Формат(ДатамВерсияФормы, "ДЛФ=DD; ДП='Пустая дата'") + Символы.ПС, Формат(ДатаВерсияФормы,  "ДЛФ=DD; ДП='Пустая дата'"));
	...
	Для Каждого Стр Из ТП Цикл
		...
		Если БылаЦифра Тогда
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибки преобразования номера из окончания: %1 === %2';
...
Процедура ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(Форма, ИмяМакетаАттрибутов, ИмяМакетаШаблона, ПараметрыВыгрузки) Экспорт
	Попытка
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Версия формата выгрузки была изменена на ';
