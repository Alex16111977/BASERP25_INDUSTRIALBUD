...
Функция МакетИзДвоичныхДанных(ДвоичныеДанныеМакета) Экспорт
	...
	Если НЕ Расширение = "docx" Тогда
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", НСтр("ru = 'Макет шаблона офисного документа имеет не верный формат.';
																						|en = 'Office document template has an incorrect format.'"));
		ВызватьИсключение(НСтр("ru = 'Ошибка анализа макета шаблона. Макет шаблона офисного документа имеет не верный формат.';
								|en = 'An error occurred while analyzing the template. Office document template has an incorrect format.'"));
	КонецЕсли;
...
Процедура ЗакрытьСоединение(ПечатнаяФорма) Экспорт
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Не удалось удалить временный каталог шаблона печатной формы по причине:';
								|en = 'Cannot delete a temporary directory of print form template due to:'") + Символы.ПС 
...
Процедура РазобратьКонтейнерДокументDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Не удалось открыть файла шаблона по причине:';
								|en = 'Cannot open the template file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Не удалось выполнить разбор файла шаблона по причине:';
								|en = 'Cannot parse the template file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Не удалось создать файл документа по причине:';
								|en = 'Cannot create document file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'Не удалось сформировать файл документа по причине:';
								|en = 'Cannot generate the document file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
