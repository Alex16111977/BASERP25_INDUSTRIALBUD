...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() И НЕ РазделениеПоОбластямДанныхВключено() Тогда
		...
		Обработчик.Комментарий     = НСтр("ru = 'Включает отправку сведений об использовании программы в фирму ""1С"". Отключить отправку сведений можно в разделе Администрирование / Интернет-поддержка и сервисы / Центр мониторинга';
											|en = 'Enables the option of sending information about using the application to 1С company. You can disable this option in Settings/Online user support and services/Monitoring center'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru = 'Сообщить о проблемах производительности';
									|en = 'Notify of performance issues'");
		Дело.Количество     = 0;
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Центр мониторинга - регистрация дампов';
						|en = 'Monitoring center - dump registration'",
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Центр мониторинга - собрать статистику конфигурации';
						|en = 'Monitoring center - obtain configuration statistics'",
					ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					,
					,
	...
	Если ОбработатьЗаписиДо > ГраницаАгрегирования Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Агрегировать замеры операций статистики';
											|en = 'Monitoring center.Aggregate measurements of statistics operations '", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
			ВызватьИсключение Ошибка;
		КонецПопытки;
	КонецЕсли;
...
Процедура УдалитьУстаревшиеДанныеОперацийСтатистики()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Удалить устаревшие данные операций статистики';
										|en = 'Monitoring center.Delete obsolete data of statistics operations '", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
				...
				Если РегистрыСведений.ОбластиСтатистики.СобиратьСтатистикуКонфигурации(ОбластьДанныхСтрока) Тогда
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Статистика конфигурации переопределяемая';
														|en = 'Monitoring center.Configuration statistics overridable '", ОбщегоНазначения.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,
						,,
						НСтр("ru = 'Не удалось установить разделение сеанса.Область данных';
							|en = 'Cannot set the session separation.Data area'", ОбщегоНазначения.КодОсновногоЯзыка()) + " = " + Формат(Выборка.ОбластьДанных, "ЧГ=0")
...
Процедура СформироватьПакетДляОтправки()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Формирование пакета для отправки';
										|en = 'Monitoring center.Generate a package for sending'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыОтправки(Параметры)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Установка параметров отправки';
										|en = 'Monitoring center.Set sending parameters'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыЦентраМониторинга(НовыеПараметры)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Установка параметров центра мониторинга';
										|en = 'Monitoring center.Set monitoring center parameters'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
	Возврат Ошибка;
КонецФункции
...
Процедура УдалитьПараметрыЦентраМониторинга()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга.Удалить параметры центра мониторинга';
										|en = 'Monitoring center.Delete monitoring center parameters '", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецПроцедуры
	...
	Если Файл.Существует() Тогда
		...
		Исключение
			...
			Сообщение = Сообщение + НСтр("ru = 'Скорее всего файл поврежден. Регистрация дампов не возможна. Удалите поврежденный файл или восстановите настройки.';
										|en = 'The file is likely to be corrupt. Cannot register dumps. Delete the corrupt file or reset settings.'");
			
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга';
											|en = 'Monitoring center'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
		...
		Если ВРЕГ(ПервыйДочерний.ИмяУзла) = "CONFIG" Тогда
			...
			Исключение
				Сообщение = НСтр("ru = 'Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';
								|en = 'An error occurred while writing the setting file of the technological log. Cannot register dumps.'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Центр мониторинга - регистрация дампов';
												|en = 'Monitoring center - dump registration'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
		...
		Иначе
			...
			Иначе
				...
				Иначе
					КаталогНастроек.Путь = "";
					КаталогНастроек.Существует = Ложь;
					КаталогНастроек.ОписаниеОшибки = НСтр("ru = 'Не найден файл конфигурации настроек в каталоге';
															|en = 'Setting configuration file is not found in the directory'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ПутьКаталога;
...
Функция ПолучитьКаталогИзФайлаКонфигурацииНастроек(ФайлКонфигурацииНастроек)
	...
	Иначе
		...
		КаталогНастроек.Существует = Ложь;
		КаталогНастроек.ОписаниеОшибки = НСтр("ru = 'Не найдена секция ConfLocation в файле';
												|en = 'The ConfLocation section is not found in the file'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ФайлКонфигурацииНастроек.ПолноеИмя;
	КонецЕсли;
...
Функция СвободноеМестоНаДиске(БукваДиска, СКлиентаНаСервере)
	...
	Если ЗначениеЗаполнено(ПотокОшибок) Тогда 
		...
		РезультатЗапроса.ОписаниеОшибки = НСтр("ru = 'Ошибка при выполнении команды typeperf';
												|en = 'An error occurred while running the typeperf command'");
	Иначе 
	...
	Если ОтправкаТестовогоПакетаВозможна(ПараметрыЦентраМониторинга, ДатаЗапуска) И ПараметрыВыполнения.ОтправкаТестовогоПакета
		...
		Исключение
			...
			НСтр("ru = 'Центр мониторинга - сформировать тестовый пакет для отправки';
				|en = 'Monitoring center - generate test package to send'",
			ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
