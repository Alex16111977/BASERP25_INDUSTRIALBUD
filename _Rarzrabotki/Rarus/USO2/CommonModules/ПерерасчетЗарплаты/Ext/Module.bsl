...
Процедура РегистрацияПерерасчетовПоПредварительнымДанным(Регистратор = Неопределено, Организация = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Регистрация перерасчетов по предварительным данным';
										|en = 'Recalculation registration by preliminary data'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПерерассчитыватьПособия(Организация) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если ЗначениеЗаполнено(ДатаВступленияВПроект) И ДатаВступленияВПроект < '20210101' Тогда
...
Процедура СоздатьВТСторнируемыеРегистраторы(МенеджерВременныхТаблиц, ИсключаемыеРегистраторы)
	...
	Запрос.Текст =
		...
		|	И НЕ (СторнируемыеРегистраторы.Регистратор ССЫЛКА Документ.НачислениеЗарплаты И СторнируемыеРегистраторы.Сторно)";
