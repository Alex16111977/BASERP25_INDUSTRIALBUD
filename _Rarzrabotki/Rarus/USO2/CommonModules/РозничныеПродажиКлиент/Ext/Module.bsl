...
Процедура ОбработатьСостояниеСмены(Форма, ОписаниеОповещенияЗавершение) Экспорт
	...
	Если Не Форма.СтруктураСостояниеКассовойСмены.СменаОткрыта
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Перед выполнением операции требуется открыть новую кассовую смену.';
				|en = 'Open a new register shift before executing the transaction.'"), Кнопки);
		Возврат;
	...
	Если Форма.СтруктураСостояниеКассовойСмены.Ошибка24Часа
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'С момента открытия смены прошло более 24 часов. Перед выполнением операции требуется открыть новую кассовую смену.';
				|en = 'More than 24 hours have passed since the shift was opened. It is required to open a new register shift before executing the operation.'"), Кнопки);
		Возврат;
	...
	ИначеЕсли Не Форма.СтруктураСостояниеКассовойСмены.СменаОткрыта
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'С момента открытия смены прошло более 24 часов. Перед выполнением операции требуется открыть новую кассовую смену.';
				|en = 'More than 24 hours have passed since the shift was opened. It is required to open a new register shift before executing the operation.'"));
...
Процедура СформироватьОтчетОРозничныхПродажах(Объект)
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		ТекстСообщения = НСтр("ru = 'Смена закрыта на фискальном регистраторе.
		                      ...
		                      |en = 'Shift is closed on fiscal register.
		                      |Additional details:
		                      |%ДополнительноеОписание%'");
		...
		Иначе		
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'При выемке денежных средств из кассы ККМ произошла ошибка.
				           |Дополнительное описание:
				           |%1';
				           |en = 'An error occurred during cash withdrawal from the cash register.
				           |Additional details:
				           |%1'"), ОписаниеОшибки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		...
		Иначе
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'При внесении денежных средств в кассу ККМ произошла ошибка.
				           |Дополнительное описание:
				           |%1';
				           |en = 'An error occurred while depositing cash into the cash register.
				           |Additional details:
				           |%1'"),
				ОписаниеОшибки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
