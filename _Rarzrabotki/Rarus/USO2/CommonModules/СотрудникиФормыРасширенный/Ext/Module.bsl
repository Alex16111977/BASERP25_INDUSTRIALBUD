...
Процедура ОбработкаПолученияДанныхВыбора(РезультатЗапроса, ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Отбор.Свойство("ТекущееПодразделение") 
		...
		НеобходимыДанныеДляПодбора = Истина;
		ТекстЗапросаУсловий = ?(ПустаяСтрока(ТекстЗапросаУсловий), "", ТекстЗапросаУсловий + "
			|	И ") + "(ЕСТЬNULL(ДанныеДляПодбора.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) В ИЕРАРХИИ (&ТекущееПодразделение))";
	...
	Если НеобходимыДанныеДляПодбора Тогда
		...
		Иначе
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|				ГДЕ
				|					ДанныеДляПодбораОтбор.Сотрудник = Сотрудники.Ссылка
				|					И ДанныеДляПодбораОтбор.ВидСобытия <> ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
				|					И (&ПоказыватьПодработки = ИСТИНА
			...
			Если СтрокиССтраховойНомерПФР.Количество() > 0 Тогда
				...
				СтруктураСообщения.ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Найдена запись о человеке, имеющем такой же СНИЛС (%1)';
						|en = 'Entry about a person with the same IIAN (%1) is found'"),
					СтраховойНомерПФР);
				СтруктураВозврат.СообщенияПроверки.Добавить(СтруктураСообщения);
			КонецЕсли;
		КонецЕсли; 
	...
	Если НЕ РезультатПроверки.ФизическоеЛицоУникально 
		...
		Если НЕ РезультатПроверки.ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц Тогда
			ТекстСообщения = НСтр("ru = 'Запись невозможна. Для разрешения конфликтов, обратитесь к администратору информационной системы.';
									|en = 'Cannot write. Contact the information system administrator to resolve the conflict.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		ИначеЕсли Форма.Параметры.Ключ.Пустая() Тогда
			ТекстСообщения = НСтр("ru = 'Запись данных нового человека невозможна.';
									|en = 'Cannot write the data of a new person.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		КонецЕсли; 
...
Процедура ОбновитьСостояниеСотрудника(Форма)
	...
	ОписаниеФильтра = СостоянияСотрудников.ОписаниеФильтраСоздатьВТСостоянияСотрудниковНаДаты("ВТСотрудники");
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	ОписаниеМеню.Пункты.Добавить(
		СотрудникиФормы.РазделМеню(НСтр("ru = 'НДФЛ';
										|en = 'PIT (personal income tax)'"), ПунктыМеню));
