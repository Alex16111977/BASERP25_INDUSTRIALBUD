	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияВалютаВзаиморасчетов Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru = 'Валюта взаиморасчетов';
																	|en = 'Mutual settlements currency'")),
			,
			,
			,
			Отказ);
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияТребуетсяЗалогЗаТару Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru = 'Требуется залог за тару';
																	|en = 'Deposit is required for the package'")),
			,
			,
			,
			Отказ);
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияСрокВозвратаМногооборотнойТары Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрЗаменить(ТекстСообщения, "%ПредставлениеПоля%", НСтр("ru = 'Срок возврата многооборотной тары';
																	|en = 'Reusable package return cycle length'")),
			,
			,
			,
			Отказ);
	...
	Если ВыборкаРеквизитыШапки.ЕстьНесовместимыеСпособыДоставки Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'В одном из выделенных распоряжений указан способ доставки ""Перевозчик (с нашего склада)"", несовместимый со способами доставки других выделенных распоряжений.';
				|en = 'The ""Carrier (from our warehouse)"" delivery method which is incompatible with delivery methods of other selected references is specified in one of the selected references.'"),
			,
			,
			,
			Отказ);
...
Функция ПерезаполнитьНакладнуюПоОрдерам(Объект) Экспорт
	...
	Если НЕ СостояниеОтгрузки.ЕстьОтгруженныеТовары Тогда
		Текст = НСтр("ru = 'По данному документу не начата отгрузка!';
					|en = 'Shipment has not started for this document.'");
...
Процедура ПроверитьКорректностьЗаполненияДокументаПродажи(Знач ДокументПродажи, Отказ) Экспорт
	...
	Если НевозможноПровестиБезСогласования Тогда
		ТекстОшибки = НСтр("ru = 'Недостаточно прав на отклонение от условий продаж.';
							|en = 'Insufficient rights to deviate from sales conditions.'");
		...
		Если (ИмяТаблицы = "Документ.ЗаказКлиента"
			ИЛИ ИмяТаблицы = "Документ.ЗаявкаНаВозвратТоваровОтКлиента") И ИспользоватьРасширенныеВозможностиЗаказаКлиента Тогда
				ТекстОшибки = ТекстОшибки + " " + НСтр("ru = 'Документ необходимо провести в статусе ""На согласовании"".';
														|en = 'Post the document in the ""On approval"" status.'");
		КонецЕсли;
		...
		Если СуммаОплатыВсего > 0 Тогда
			...
			Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
				ТекстОшибки = НСтр("ru = 'Не заполнены условия оплаты';
									|en = 'Payment conditions are not filled in'");
			Иначе
			...
			Если Не ЗначениеЗаполнено(ДанныеЭтапа.ДатаПлатежа) Тогда
				...
				Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
					ТекстОшибки = НСтр("ru = 'Не заполнена ""Дата платежа""';
										|en = '""Payment date"" is not filled in'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Дата платежа""';
										|en = 'The ""Payment date"" column is not filled in'");
				КонецЕсли;
			...
			Если ТекущееЗначениеДатыПлатежа < ПредыдущееЗначениеДатыПлатежа И 
				...
				Если ИспользоватьУпрощеннуюСхемуОплаты Тогда
					ТекстОшибки = НСтр("ru = 'Дата платежа по кредиту не должна быть меньше, чем дата платежа по предоплате';
										|en = 'Loan payment date must be not less than the prepayment date'");
				Иначе
					...
					|en = 'Payment date in the %ИндексТекущегоЭтапа% line
					|of the ""Payment steps"" list must be not less than those in the %ИндексПредыдущегоЭтапа% line'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки,"%ИндексТекущегоЭтапа%",    ИндексТекущегоЭтапа + 1);
					ТекстОшибки = СтрЗаменить(ТекстОшибки,"%ИндексПредыдущегоЭтапа%", ИндексПредыдущегоЭтапа + 1);
				КонецЕсли;
...
Процедура ПроверитьКорректностьВозвращаемыхТоваров(Знач ДокументПродажи, ИмяТаблицы, Отказ, ТаблицаПроверяемыеТовары = Неопределено) Экспорт
	...
	Если ТаблицаПроверяемыеТовары.Количество() > 0 Тогда
		...
		Если НЕ ВозвратПоЧеку Тогда
			...
			|		ПО КорректировкаРеализации.Ссылка.ДокументОснование = ДанныеКорректировки.СсылкаРеализации
			|			И КорректировкаРеализации.Ссылка.Дата = ДанныеКорректировки.Дата
			|
			|СГРУППИРОВАТЬ ПО
			...
			|		ПО СсылкиНаРеализации.Ссылка = КорректировкаРеализацииТовары.Ссылка
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроверяемыеТовары КАК ПроверяемыеТовары
			|		ПО КорректировкаРеализацииТовары.Номенклатура = ПроверяемыеТовары.Номенклатура
			|			И КорректировкаРеализацииТовары.Характеристика = ПроверяемыеТовары.Характеристика
			|			И ВЫБОР КОГДА КорректировкаРеализацииТовары.СтатусУказанияСерий = 14
			|				ТОГДА КорректировкаРеализацииТовары.Серия
			|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
			|			КОНЕЦ = ПроверяемыеТовары.Серия
			|ГДЕ
			...
			|		ПО СсылкиНаРеализации.Ссылка = РеализацияТоваровУслугТовары.Ссылка
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроверяемыеТовары КАК ПроверяемыеТовары
			|		ПО РеализацияТоваровУслугТовары.Номенклатура = ПроверяемыеТовары.Номенклатура
			|			И РеализацияТоваровУслугТовары.Характеристика = ПроверяемыеТовары.Характеристика
			|			И ВЫБОР КОГДА РеализацияТоваровУслугТовары.СтатусУказанияСерий = 14
			|				ТОГДА РеализацияТоваровУслугТовары.Серия
			|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
			|			КОНЕЦ = ПроверяемыеТовары.Серия
			|ГДЕ
			...
			|		ПО СсылкиНаРеализации.Ссылка = ОтчетОРозничныхПродажахТовары.Ссылка
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроверяемыеТовары КАК ПроверяемыеТовары
			|		ПО ОтчетОРозничныхПродажахТовары.Номенклатура = ПроверяемыеТовары.Номенклатура
			|			И ОтчетОРозничныхПродажахТовары.Характеристика = ПроверяемыеТовары.Характеристика
			|			И ОтчетОРозничныхПродажахТовары.Серия = ПроверяемыеТовары.Серия
			|ГДЕ
...
Процедура ПроверитьВозможностьВводаНаОснованииСоглашения(ИмяДокумента, ОперацияСоглашения) Экспорт
	...
	Если ОперацииДокумента.Найти(ОперацияСоглашения) = Неопределено Тогда
		ТекстОшибки = НСтр("ru = 'Нельзя выполнить ввод документа на основании соглашения с операцией ""%Операция%"".';
							|en = 'Cannot enter the document based on the agreement with the ""%Transaction%"" transaction.'");
...
Функция ЗаполнитьЦеныПоСебестоимости(ТабличнаяЧасть, ВыделенныеСтроки, ПараметрыЗаполнения, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если Не Параметры.Свойство("Дата") 
		...
		ВызватьИсключение НСтр("ru = 'В параметрах заполнения должны быть указаны ""Дата"", ""Организация"", ""Склад""';
								|en = 'Specify ""Date"", ""Company"" and ""Warehouse"" in the population parameters'");
		Возврат ЦеныЗаполнены;
...
Функция ДополнитьПредставлениеПредставлениемФискальнойОперацииВДокументеПродажи(Основание, СчетаФактурыВыданныеНаОсновании = Неопределено, СчетаФактурыПолученныеНаОсновании = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
		ВозможноПробитиеЧека = Истина;
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВыкупТоваровХранителем") Тогда
		ВозможноПробитиеЧека = Истина;
	//-- НЕ УТ
...
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен = Истина) Экспорт
	...
	Возврат НСтр("ru = 'Счет-фактура';
				|en = 'Tax invoice'") + ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(СтруктураШапки) + " " + ?(НЕ Проведен, НСтр("ru = '(не проведен)';
																																					|en = '(not posted)'"),"");
				...
				Если УпрощенныйРежимДиалога Тогда
					Если ДанныеЭтапа.ВариантОплаты = Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки Тогда
						ТекстОшибки = НСтр("ru = 'Не заполнена ""Дата платежа"" по кредиту';
											|en = 'Credit ""Payment date"" is not filled in'");
						ПолеОшибки  = "ДатаКредит";
					Иначе
						ТекстОшибки = НСтр("ru = 'Не заполнена ""Дата платежа"" по предоплате';
											|en = 'Prepayment ""Payment date"" is not filled in'");
						ПолеОшибки  = "ДатаПредоплата";
					КонецЕсли;
				Иначе
					ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Дата платежа""';
										|en = 'The ""Payment date"" column is not filled in'") + " " + АдресОшибки;
...
Процедура ОбновитьОграничениеЗадолженности(Договор, Дата, Текст, ВидимостьТекста, ЦветТекста, ВидимостьКартинки) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов)
		...
		ИначеЕсли ДанныеДоговора.ОграничиватьСуммуЗадолженности Тогда
			Текст = НСтр("ru = 'Остаток допустимого кредита:';
						|en = 'Allowable credit balance:'") + " " + Формат(ОстатокДопустимогоКредита, "ЧДЦ=2; ЧН=0,00") + " " + Строка(ДанныеДоговора.ВалютаВзаиморасчетов);
...
Процедура ВыполнитьКонтрольЗаказаПослеПроведения(КонтролируемыйЗаказ, Отказ) Экспорт
	...
	Если ВыборкаОтгрузка.Следующий() Тогда 
		Если ТипЗнч(КонтролируемыйЗаказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
			...
			|en = 'The ""%Заказ%"" document is not fully shipped.
			|Order can be closed only with lines fully shipped/canceled'");
		ИначеЕсли ТипЗнч(КонтролируемыйЗаказ) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
			...
			|en = 'Substitute goods for the ""%Заказ%"" document are not shipped fully.
			|The request can be transferred to the ""Completed"" status only with fully shipped/canceled lines'");
		КонецЕсли;
	...
	Если ВыборкаРасчеты.Следующий() Тогда 
		...
		ИначеЕсли ТипЗнч(КонтролируемыйЗаказ) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
			...
			|en = 'Settlements for the ""%Заказ%"" document are not completed. Payment is required: %СуммаКОплате% %Валюта%.
			|The request can be transferred to the ""Completed"" status only with fully paid/canceled lines'");
		КонецЕсли;
...
Функция ТекстЗапросаКонтрольЗаказаПослеПроведения() Экспорт
	...
	Возврат "
		...
		|				ИЛИ &Статус  = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.Выполнена))
		|			И (ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаказКлиента).ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию)
		|				И ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи)
		|				И ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаявкаНаВозвратТоваровОтКлиента).ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера))
		|	) КАК ТоварыКОтгрузкеОстатки
		|ГДЕ
		|	&КонтролироватьОтгрузку
...
Процедура СформироватьЗапросШапкаДокументаПродажи(ТекстЗапроса, ИмяТаблицы)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		//-- НЕ УТ
				И ИмяТаблицы <> "Документ.ЗаданиеТорговомуПредставителю","
		|	ВЫБОР
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
			...
			ТекстОшибки = НСтр("ru = 'Номенклатура ""%Номенклатура%"" с характеристикой ""%Характеристика%"" повторяется';
								|en = 'Products ""%Номенклатура%"" with characteristic ""%Характеристика%"" are repeated'");
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru = 'Номенклатура ""%Номенклатура%"" повторяется';
								|en = 'Products ""%Номенклатура%"" are repeated'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%", Выборка.Номенклатура);
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЕстьОшибкиЗаполненияСкладПоступления Тогда
			...
			ТекстОшибки = НСтр("ru = 'Поле ""Склад поступления"" не заполнено';
								|en = 'Receiving warehouse is not filled in'");
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не заполнена колонка ""Склад"" в строке %1 списка ""%2""';
				|en = 'The ""Warehouse"" column is not populated in line %1 of the ""%2"" list'"),
			Выборка.НомерСтроки,
...
Процедура СообщитьОбОшибкахКорректностьЗаполненияСкладаВТабличнойЧастиТовары(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Склад"" в строке %НомерСтроки% списка ""%ИмяТЧ%""';
							|en = 'The ""Warehouse"" column is not populated in line %НомерСтроки% of the ""%ИмяТЧ%"" list'");
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ЕстьОшибкиЗаполненияЦен Тогда
			...
			ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Цена"" в строке %НомерСтроки% списка ""%ТабЧасть%""';
								|en = 'The ""Price"" column is not populated in line %НомерСтроки% of the ""%ТабЧасть%"" list'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",  Выборка.НомерСтроки);
		...
		Если Выборка.ЕстьОшибкиЗаполненияСумм Тогда
			...
			ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Сумма"" в строке %НомерСтроки% списка ""%ТабЧасть%""';
								|en = 'The ""Amount"" column is not populated in line %НомерСтроки% of the ""%ТабЧасть%"" list'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",  Выборка.НомерСтроки);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ТабЧасть%", ПредставлениеТЧ);
			...
			Если Выборка.ЭтоНабор Тогда
				Если Выборка.ЭтоСоглашение Тогда
					ТекстОшибки = НСтр("ru = 'Цена набора должна быть не ниже фиксированной цены в типовом соглашении ""%ЦенаСоглашение%""';
										|en = 'Set price should not be less than the fixed price in the ""%ЦенаСоглашение%"" standard agreement'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Цена набора должна быть не ниже фиксированной цены в соглашении ""%ЦенаСоглашение%""';
										|en = 'Set price should not be less than the fixed price in the ""%ЦенаСоглашение%"" agreement'");
				КонецЕсли;
			Иначе
				Если Выборка.ЭтоСоглашение Тогда
					ТекстОшибки = НСтр("ru = 'Цена должна быть не ниже фиксированной цены в типовом соглашении ""%ЦенаСоглашение%""';
										|en = 'Price should not be less than the fixed price in the ""%ЦенаСоглашение%"" standard agreement'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Цена должна быть не ниже фиксированной цены в соглашении ""%ЦенаСоглашение%""';
										|en = 'Price should not be less than the fixed price in the ""%ЦенаСоглашение%"" agreement'");
				КонецЕсли;
			КонецЕсли;
				...
				Если ЗначениеЗаполнено (Выборка.ВидЦены) Тогда
					...
					Если Выборка.ЭтоСоглашение Тогда
						ТекстОшибки = НСтр("ru = 'Вид цены не должен отличаться от вида цены в типовом соглашении ""%ВидЦеныСоглашение%""';
											|en = 'Price type must not be different from the price type in the ""%ВидЦеныСоглашение%"" standard agreement'");
					Иначе
						ТекстОшибки = НСтр("ru = 'Вид цены не должен отличаться от вида цены в соглашении ""%ВидЦеныСоглашение%""';
											|en = 'Price type must not be different from the price type in the ""%ВидЦеныСоглашение%"" agreement'");
					КонецЕсли;
				...
				Иначе
					...
					Если Выборка.ЭтоСоглашение Тогда
						ТекстОшибки = НСтр("ru = 'Вид цены должен быть заполнен и не должен отличаться от вида цены в типовом соглашении ""%ВидЦеныСоглашение%""';
											|en = 'Price type is required and must not be different from the price type in the ""%ВидЦеныСоглашение%"" standard agreement'");
					Иначе
						ТекстОшибки = НСтр("ru = 'Вид цены должен быть заполнен и не должен отличаться от вида цены в соглашении ""%ВидЦеныСоглашение%""';
											|en = 'Price type is required and must not be different from the price type in the ""%ВидЦеныСоглашение%"" agreement'");
					КонецЕсли;
				...
				Если ЗначениеЗаполнено(Выборка.ВидЦены) Тогда
					...
					Если Выборка.ЭтоСоглашение Тогда
						ТекстОшибки = НСтр("ru = 'В типовом соглашении цены не определены. Вид цены ""%ВидЦены%"" по данному соглашению не может быть установлен без согласования';
											|en = 'Prices are not set in the standard agreement. The ""%ВидЦены%"" price type under this agreement cannot be set without approval'");
					Иначе
						ТекстОшибки = НСтр("ru = 'В соглашении цены не определены. Вид цены ""%ВидЦены%"" по данному соглашению не может быть установлен без согласования';
											|en = 'Prices are not set in the agreement. The ""%ВидЦены%"" price type under this agreement cannot be set without approval'");
					КонецЕсли;
				...
				Иначе
					...
					Если Выборка.ЭтоСоглашение Тогда
						ТекстОшибки = НСтр("ru = 'В типовом соглашении цены не определены. Цена по данному соглашению не может быть установлена без согласования';
											|en = 'Prices are not set in the standard agreement. A price under this agreement cannot be set without approval.'");
					Иначе
						ТекстОшибки = НСтр("ru = 'В соглашении цены не определены. Цена по данному соглашению не может быть установлена без согласования';
											|en = 'Prices are not set in the agreement. A price under this agreement cannot be set without approval.'");
					КонецЕсли;
...
Процедура СообщитьОбОшибкахСоответствиеГруппЦенНоменклатурыУсловиямПродаж(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		АдресОшибки = " " + НСтр("ru = 'в строке %НомерСтроки% списка ""Группы цен номенклатуры""';
								|en = 'in line %НомерСтроки% of the ""Product price groups"" list'");
		АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", НомерСтроки);
		...
		Если Выборка.ЕстьОшибкиЗаполненияЦеновыеГруппы Тогда
			...
			ТекстОшибки = НСтр("ru = 'Ценовая группа ""%ЦеноваяГруппа%"" отсутствует в типовом соглашении';
								|en = 'The ""%ЦеноваяГруппа%"" price group is missing in the standard agreement'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ЦеноваяГруппа%", Выборка.ЦеноваяГруппа);
		...
		Если Выборка.ЕстьОшибкиЗаполненияРучнойСкидки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Процент ручной скидки не должен отличаться от значения в типовом соглашении (%Значение%%)';
								|en = 'Manual discount percent should not differ from the value in standard agreement (%Значение%%)'");
		...
		Если Выборка.ЕстьОшибкиЗаполненияРучнойНаценки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Процент ручной наценки не должен отличаться от значения в типовом соглашении (%Значение%%)';
								|en = 'Manual markup percent should not differ from the value in standard agreement (%Значение%%)'");
		...
		Если Выборка.ЕстьОшибкиЗаполненияСрокПоставки Тогда
			...
			ТекстОшибки = НСтр("ru = 'Срок поставки не должен быть меньше, чем значение в типовом соглашении - %СрокПоставкиСоглашение% дн.';
								|en = 'Delivery period should not be less than the value specified in the standard agreement - %СрокПоставкиСоглашение% days'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%СрокПоставкиСоглашение%", Выборка.СрокПоставкиСоглашение);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСегментыНоменклатуры") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
				...
				|en = 'Products ""%Номенклатура%"" with characteristic ""%Характеристика%"" 
				|should be included in the ""%СегментНоменклатуры%"" product segment specified in the agreement'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",        Выборка.Номенклатура);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Характеристика%",      Выборка.Характеристика);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСегментыПартнеров") И Выборка.Следующий() Тогда
		Если Выборка.ЕстьОшибкиЗаполненияСегментПартнеров Тогда
			...
			ТекстОшибки = НСтр("ru = 'Партнер должен входить в сегмент партнеров ""%СегментПартнеров%"", определенный в типовом соглашении';
								|en = 'Partner should be included in partner segment ""%СегментПартнеров%"" determined in the standard agreement'");
...
Процедура СообщитьОбОшибкахКорректностьЭтаповГрафикаОплаты(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		АдресОшибки = " " + НСтр("ru = 'в строке %НомерСтроки% списка ""Этапы оплаты""';
								|en = 'in line %НомерСтроки% of the ""Payment steps"" list'");
		АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", НомерСтроки);

		Если Выборка.ЕстьОшибкиЗаполненияВариантаОплаты Тогда

			ТекстОшибки = НСтр("ru = 'Вариант оплаты отличается от значения в графике оплаты ""%ВариантОплатыШаблона%""';
								|en = 'Payment option is different from the value in the ""%ВариантОплатыШаблона%"" payment schedule '");
		...
		Если ДатаПлатежаШаблона < Выборка.ДатаПлатежа Тогда
			...
			ТекстОшибки = НСтр("ru = 'Дата платежа должна быть не больше, чем в графике оплаты ""%ДатаПлатежаШаблона%""';
								|en = 'Payment date must not be greater than the date in payment schedule ""%ДатаПлатежаШаблона%""'");
		...
		Если СуммаПроцентов < СуммаПроцентовШаблона Тогда
			...
			ТекстОшибки = НСтр("ru = 'Проценты до следующего платежа по графику (""%ПроцентПлатежа%%"") не должны быть меньше процентов по графику соглашения (""%ПроцентПлатежаШаблона%%"")';
								|en = 'Interest before the next payment on schedule (""%ПроцентПлатежа%%"") cannot be less than the interest under the agreement schedule (""%ПроцентПлатежаШаблона%%"")'");
		...
		Если СуммаПроцентовЗалога < СуммаПроцентовЗалогаШаблона Тогда
			...
			ТекстОшибки = НСтр("ru = 'Проценты залога за тару до следующего платежа по графику (""%ПроцентЗалогаЗаТару%%"") не должны быть меньше процентов по графику соглашения (""%ПроцентЗалогаЗаТаруШаблона%%"")';
								|en = 'Percents of the package deposit until the next payment on schedule (""%ПроцентЗалогаЗаТару%%"") cannot be less than the percent on the agreement schedule (""%ПроцентЗалогаЗаТаруШаблона%%"")'");
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ДатаПлатежа > ДатаПлатежаШаблона Тогда
			...
			ТекстОшибки = НСтр("ru = 'Дата платежа должна быть не больше, чем в графике оплаты, указанном в соглашении ""%ДатаПлатежаШаблона%""';
								|en = 'Payment date must not be greater than those in the payment schedule specified in the agreement ""%ДатаПлатежаШаблона%""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ДатаПлатежаШаблона%", Формат(ДатаПлатежаШаблона, "ДЛФ=DD")); 
			
			Если ВыводитьВОтчет Тогда
				Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Дата платежа';
																								|en = 'Payment date'"), ТекстОшибки, МассивОтклонений);
			Иначе
				СообщитьПользователюОбОшибке(
					ТекстОшибки,
					ДокументПродажи,
...
Процедура СообщитьОбОшибкахКорректностьКоличестваЭтаповГрафикаОплаты(Знач Выборка,
	...
	Если Выборка.ЕстьОшибкиЗаполненияЭтапыГрафикаОплаты Тогда
		...
		ТекстОшибки = НСтр("ru = 'Количество этапов оплаты в документе (%КоличествоЭтаповДокумента%) должно совпадать с количеством этапов в графике (%КоличествоЭтаповГрафика%)';
							|en = 'Number of payment steps in the document (%КоличествоЭтаповДокумента%) should be equal to the number of steps in the schedule (%КоличествоЭтаповГрафика%)'");
		...
		Если ВыводитьВОтчет Тогда
			Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Этапы оплаты';
																							|en = 'Payment steps'"), ТекстОшибки, МассивОтклонений);
		Иначе 
			СообщитьПользователюОбОшибке(
				ТекстОшибки,
				ДокументПродажи,
			...
			Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
				...
				Если ИспользоватьСкидкиНаценки Тогда
					ТекстОшибки = НСтр("ru = 'Цена с учетом скидки (наценки) на номенклатуру ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не должна быть ниже минимально допустимой цены ""%МинимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price with discount (markup) for the ""%Номенклатура%"" products with the ""%Характеристика%"" characteristic should not be less than minimum allowable price ""%МинимальноДопустимаяЦена% %ВалютаЦены%""'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Цена на номенклатуру ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не должна быть ниже минимально допустимой цены ""%МинимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price of the ""%Номенклатура%"" products with the ""%Характеристика%"" characteristic should not be lower than minimum allowable price ""%МинимальноДопустимаяЦена% %ВалютаЦены%""'");
				КонецЕсли;
			...
			Иначе
				...
				Если ИспользоватьСкидкиНаценки Тогда
					ТекстОшибки = НСтр("ru = 'Цена с учетом скидки (наценки) на номенклатуру ""%Номенклатура%"" не должна быть ниже минимально допустимой цены ""%МинимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price with discount (markup) for the ""%Номенклатура%"" products should not be less than minimum allowable price ""%МинимальноДопустимаяЦена% %ВалютаЦены%""'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Цена на номенклатуру ""%Номенклатура%"" не должна быть ниже минимально допустимой цены ""%МинимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price of the ""%Номенклатура%"" products should not be less than minimum allowable price of ""%МинимальноДопустимаяЦена% %ВалютаЦены%""'");
				КонецЕсли;
			...
			Если ЗначениеЗаполнено(Выборка.Характеристика) Тогда
				...
				Если ИспользоватьСкидкиНаценки Тогда
					ТекстОшибки = НСтр("ru = 'Цена с учетом скидки (наценки) на номенклатуру ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не должна быть выше максимально допустимой цены ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price with discount (markup) for the ""%Номенклатура%"" products with the ""%Характеристика%"" characteristic should not exceed maximum allowable price ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Цена на номенклатуру ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не должна быть выше максимально допустимой цены ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price of the ""%Номенклатура%"" products with the ""%Характеристика%"" characteristic should not exceed maximum allowable price ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""'");
				КонецЕсли;
			...
			Иначе
				...
				Если ИспользоватьСкидкиНаценки Тогда
					ТекстОшибки = НСтр("ru = 'Цена с учетом скидки (наценки) на номенклатуру ""%Номенклатура%"" не должна быть выше максимально допустимой цены ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price with discount (markup) for the ""%Номенклатура%"" products should not exceed maximum allowable price ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""'");
				Иначе
					ТекстОшибки = НСтр("ru = 'Цена на номенклатуру ""%Номенклатура%"" не должна быть выше максимально допустимой цены ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""';
										|en = 'Price of the ""%Номенклатура%"" products should not exceed maximum allowable price ""%МаксимальноДопустимаяЦена% %ВалютаЦены%""'");
				КонецЕсли;
...
Процедура СообщитьОбОшибкахНаличиеУслугВДокументеПродажи(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Нельзя указывать номенклатуру с типом ""Услуга"" и ""Работа"" (строка %НомерСтроки% списка ""%ПредставлениеТЧ%"")';
							|en = 'Cannot specify products with type ""Service"" and ""Work"" (line %НомерСтроки% of list ""%ПредставлениеТЧ%"")'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",    Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",     Выборка.НомерСтроки);
...
Процедура СообщитьОбОшибкахНаличиеТоваровГИСМВДокументеПродажи(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Нельзя указывать номенклатуру с типом ""Продукция, маркируемая для ГИСМ"" (строка %НомерСтроки% списка ""%ПредставлениеТЧ%"")';
							|en = 'Cannot specify products with the ""Products marked for SPMS"" type (the %LineNumber% line of the ""%TSPresentation%"" list)'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",    Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",     Выборка.НомерСтроки);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПредставлениеТЧ%", ПредставлениеТЧ);
...
Процедура СообщитьОбОшибкахНаличиеАлкогольнойПродукцииВДокументеПродажи(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Нельзя указывать номенклатуру с типом ""Алкогольная (спиртосодержащая) продукция"" (строка %НомерСтроки% списка ""%ПредставлениеТЧ%"")';
							|en = 'Cannot specify products with the ""Alcoholic (alcohol-containing) products"" type (the %LineNumber% line of the ""%TSPresentation%"" list)'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",    Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",     Выборка.НомерСтроки);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПредставлениеТЧ%", ПредставлениеТЧ);
...
Процедура СообщитьОбОшибкахНаличиеТоваровВЕТИСВДокументеПродажи(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Нельзя указывать номенклатуру с типом ""Подконтрольная продукция ВЕТИС"" (строка %НомерСтроки% списка ""%ПредставлениеТЧ%"")';
							|en = 'Cannot specify products with the ""Products under VETIS control"" type (the %LineNumber% line of the ""%TSPresentation%"" list)'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",    Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",     Выборка.НомерСтроки);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПредставлениеТЧ%", ПредставлениеТЧ);
		...
		Если Выборка.ЕстьОшибкиЗаполненияРучнойСкидки Тогда
			...
			Если Выборка.ЭтоНабор Тогда
				ТекстОшибки = НСтр("ru = 'Ручная скидка на набор превышает максимально допустимую скидку ""%МаксимальнаяСуммаРучнойСкидки% %""';
									|en = 'Manual discount for the set exceeds maximum allowed discount ""%МаксимальнаяСуммаРучнойСкидки% %""'");
			Иначе
				ТекстОшибки = НСтр("ru = 'Ручная скидка превышает максимально допустимую скидку ""%МаксимальнаяСуммаРучнойСкидки% %""';
									|en = 'Manual discount exceeds maximum allowed discount ""%МаксимальнаяСуммаРучнойСкидки% %""'");
			КонецЕсли;
		...
		ИначеЕсли Выборка.ЕстьОшибкиЗаполненияРучнойНаценки Тогда
			...
			Если Выборка.ЭтоНабор Тогда
				ТекстОшибки = НСтр("ru = 'Ручная наценка на набор превышает максимально допустимую наценку ""%МаксимальнаяСуммаРучнойНаценки% %""';
									|en = 'Manual markup for the set is greater than maximum allowed markup ""%МаксимальнаяСуммаРучнойНаценки% %""'");
			Иначе
				ТекстОшибки = НСтр("ru = 'Ручная наценка превышает максимально допустимую наценку ""%МаксимальнаяСуммаРучнойНаценки% %""';
									|en = 'Manual markup exceeds maximum allowed markup ""%МаксимальнаяСуммаРучнойНаценки% %""'");
			КонецЕсли;
...
Процедура СообщитьОбОшибкахСкидкиНеРассчитаны(Знач ДокументПродажи, Отказ)
	...
	ТекстОшибки = НСтр("ru = 'Автоматические скидки (наценки) не рассчитаны. Необходимо осуществить расчет автоматических скидок (наценок) в документе';
						|en = 'Automatic discounts (markups) are not calculated. It is required to calculate the automatic discounts (markups) in the document'");
...
Процедура СообщитьОбОшибкахКорректностьКоличестваЭтаповОплатыСоглашения(Знач Выборка,
	...
	Если Выборка.ЕстьОшибкиЗаполненияЭтапыОплатыСоглашения Тогда
		...
		ТекстОшибки = НСтр("ru = 'Количество этапов оплаты в индивидуальном соглашении (%КоличествоЭтаповСоглашения%) должно совпадать с количеством этапов в типовом соглашении (%КоличествоЭтаповСоглашенияОснования%)';
							|en = 'Number of payment steps in the individual agreement (%КоличествоЭтаповСоглашения%) should be the same as in the standard agreement (%КоличествоЭтаповСоглашенияОснования%)'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СдвигШаблона < Выборка.Сдвиг Тогда
			...
			ТекстОшибки = НСтр("ru = 'Срок должен быть не больше, чем в типовом соглашении ""%СдвигШаблона%""';
								|en = 'Period cannot be greater than in the ""%СдвигШаблона%"" standard agreement'");
		...
		Если СуммаПроцентов < СуммаПроцентовШаблона Тогда
			...
			ТекстОшибки = НСтр("ru = 'Проценты до следующего платежа по графику (""%ПроцентПлатежа%%"") не должны быть меньше процентов по соглашению (""%ПроцентПлатежаШаблона%%"")';
								|en = 'Interest before the next payment on schedule (""%ПроцентПлатежа%%"") cannot be less than the interest under the agreement (""%ПроцентПлатежаШаблона%%"")'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПроцентПлатежа%", СуммаПроцентов);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ПроцентПлатежаШаблона%", СуммаПроцентовШаблона);
		...
		Если СуммаПроцентовЗалога < СуммаПроцентовЗалогаШаблона Тогда
			...
			ТекстОшибки = НСтр("ru = 'Проценты залога за тару до следующего платежа по графику (""%ПроцентЗалогаЗаТару%%"") не должны быть меньше процентов по соглашению (""%ПроцентЗалогаЗаТаруШаблона%%"")';
								|en = 'Percents of the package deposit until the next payment on schedule (""%ПроцентЗалогаЗаТару%%"") cannot be less than the percents under the agreement (""%ПроцентЗалогаЗаТаруШаблона%%"")'");
...
Процедура СообщитьОбОшибкахКорректностиВозвращаемыхТоваров(Знач ТаблицаОшибок, ВозвратПоЧеку)
	...
	Для каждого СтрокаОшибки Из ТаблицаОшибок Цикл
		СообщениеОбОшибке = НСтр("ru = 'Возврат по номенклатуре %Номенклатура% \ %Характеристика%%Серия% превышает количество реализованных товаров по документу продажи %НомерРеализации% на %Количество% %ЕдиницаИзмерения%';
								|en = 'Return of products %Номенклатура% / %Характеристика%%Серия% exceeds the quantity of purchased goods against sales document %НомерРеализации% at %Количество% %ЕдиницаИзмерения%'");
		Если СтрокаОшибки.Количество < 0 Тогда
			...
			Если ВозвратПоЧеку Тогда 
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, НСтр("ru = 'документу продажи';
																		|en = 'sales document'"), НСтр("ru = 'чеку ККМ';
																										|en = 'cash receipt'"));
			КонецЕсли;
...
Процедура СообщитьОбОшибкахСоответствиеСуммыПредоплатыГрафикуОплат(Знач Выборка,
	...
	Если Выборка.Следующий() И Выборка.ЕстьОшибкиСоответствиеСуммыПредоплатыГрафикуОплат Тогда
		...
		ТекстОшибки = НСтр("ru = 'Сумма предоплаты не соответствует сумме, рассчитанной по графику оплаты соглашения %СуммаПредоплатыПоГрафику%';
							|en = 'Prepayment amount does not correspond to the amount calculated by payment schedule under agreement %СуммаПредоплатыПоГрафику%'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%СуммаПредоплатыПоГрафику%", Формат(Выборка.СуммаПредоплатыПоГрафику, "ЧДЦ=2; ЧН=0,00"));
		
		Если ВыводитьВОтчет Тогда
			Отчеты.ОтклоненияОтУсловийПродаж.ДобавитьОтклонениеШапкиВМассивОтклонений(НСтр("ru = 'Предоплата';
																							|en = 'Prepayment'"), ТекстОшибки, МассивОтклонений);
		Иначе
			СообщитьПользователюОбОшибке(
				ТекстОшибки,
				ДокументПродажи,
	...
	Если Выборка.Следующий() Тогда 
		Если Выборка.ЕстьНезавершенныеРасчеты  Тогда
			...
			Иначе
				...
				|Заявка может быть переведена в статус ""Выполнена"" только с полностью оплаченными/отмененными строками';
				|en = 'Settlements for the request are not completed. Payment is required: %СуммаКОплате% %Валюта%.
				|The request can be transferred to the ""Completed"" status only with fully paid/canceled lines'");
			КонецЕсли;
	...
	Если ДокументПродажи.ЭтоНовый() Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
				ТекстОшибки = НСтр("ru = 'Заказ не оплачен. Закрытие заказа возможно только с полностью оплаченными/отмененными строками';
									|en = 'Order is not paid. You can close orders only with fully paid/canceled lines'");
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				ТекстОшибки = НСтр("ru = 'Заказ не оплачен. Закрытие заказа возможно только с полностью оплаченными/отмененными строками';
									|en = 'Order is not paid. You can close orders only with fully paid/canceled lines'");
			//++ НЕ УТКА
			...
			Иначе
				ТекстОшибки = НСтр("ru = 'Расчеты по заявке не завершены. Заявка может быть переведена в статус ""Выполнена"" только с полностью оплаченными/отмененными строками';
									|en = 'Settlements for the request are not completed. The request can be transferred to the ""Completed"" status only with fully paid/canceled lines'");
			КонецЕсли;
...
Процедура СообщитьОбОшибкахКонтрольЗавершенияОтгрузки (Знач Выборка,
	...
	Если Выборка.Следующий() Тогда 
		Если Выборка.ЕстьНезавершеннаяОтгрузка Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
				...
				|Закрытие заказа возможно только с полностью отгруженными/отмененными строками';
				|en = 'The order is not shipped fully.
			...
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				...
				|Закрытие заказа возможно только с полностью отгруженными/отмененными строками';
				|en = 'Materials are not fully shipped.
				|The order can be closed only with fully shipped/canceled lines'");
			//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru = 'Продукция отгружена не полностью.
				|Закрытие заказа возможно только с полностью отгруженными/отмененными строками';
				|en = 'Products are not shipped fully.
				|You can close the order only with fully shipped/canceled lines'");
			//-- НЕ УТКА
			Иначе
				ТекстОшибки = НСтр("ru = 'Заменяющие товары отгружены не полностью.
				|Заявка может быть переведена в статус ""Выполнена"" только с полностью оплаченными/отмененными строками';
				|en = 'Substitute goods are not shipped fully.
				|The request can be transferred to the ""Completed"" status only with fully paid/canceled lines'");
			КонецЕсли;
	...
	Если ДокументПродажи.ЭтоНовый() Тогда
			Если ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
				...
				|Закрытие заказа возможно только с полностью отгруженными/отмененными строками';
				|en = 'Order is not shipped. Cannot close the order.
			...
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказПереработчику") Тогда
				...
				|Закрытие заказа возможно только с полностью отгруженными/отмененными строками';
				|en = 'Materials are not shipped. The order cannot be closed.
				|The order can be closed only with fully shipped/canceled lines'");
			//++ НЕ УТКА
			ИначеЕсли ТипЗнч(ДокументПродажи) = Тип("ДокументОбъект.ЗаказДавальца") Тогда
				ТекстОшибки = НСтр("ru = 'Продукция не отгружена. Закрытие заказа невозможно.
				|Закрытие заказа возможно только с полностью отгруженными/отмененными строками';
				|en = 'Products are not shipped. Cannot close the order.
				|You can close the order only with fully shipped/canceled lines'");
			//-- НЕ УТКА
			Иначе
				ТекстОшибки = НСтр("ru = 'Заменяющие товары не отгружены. Заявка не может быть переведена в статус ""Выполнена"".
				|Заявка может быть переведена в статус ""Выполнена"" только с полностью отгруженными/отмененными строками';
				|en = 'Substitute goods are not shipped. The request cannot be transferred to the ""Completed"" status.
				|The request can be transferred to the ""Completed"" status only with fully shipped/canceled lines'");
			КонецЕсли;
...
Процедура СообщитьОбОшибкахКонтрольОформленияОтчетов(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда 
		...
		Если Выборка.ЕстьНеоформленныеОтчеты  Тогда
			...
			|en = 'Not all reports to material providers are generated.
			|To close the order, it is required to generate reports for %СуммаКОплате%%Валюта%.
	...
	Если ДокументПродажи.ЭтоНовый() Тогда
		...
		|en = 'Reports to material providers are not generated. 
		|You can close the order only after their generation'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			,
			,
	...
	Если Не ПартнерКлиент Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
			ТекстОшибки = НСтр("ru = 'Контрагент не является клиентом. Ввод на основании доступен только для клиента.';
								|en = 'Counterparty is not a customer. Only the customer can enter on the basis.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Партнер не является клиентом. Ввод на основании доступен только для клиента.';
								|en = 'Partner is not a customer. Input on the basis is available for customer only.'");
		КонецЕсли;
...
Процедура ПроверитьЗапретОтгрузки(Партнер, Отказ) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Клиенту ""%Партнер%"" запрещена отгрузка.';
						|en = 'Shipment is prohibited for the ""%Партнер%"" customer.'");
