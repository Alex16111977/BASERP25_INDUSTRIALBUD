...
Процедура ПриЗачетеРанееУдержанныхСуммм(СтрокаУдержаний, КатегорияУдержания, РанееУдержано, РассчитыватьТолькоПоТекущемуДокументу, СтандартнаяОбработка) Экспорт
	Если КатегорияУдержания <> Перечисления.КатегорииУдержаний.ИсполнительныйЛист Тогда
		Возврат;
	КонецЕсли;	
	
	СтандартнаяОбработка = Ложь;
	Если СтрокаУдержаний.УдерживатьВЦеломЗаМесяц 
		Или Не РассчитыватьТолькоПоТекущемуДокументу Тогда
		
		СтрокаУдержаний.Результат = СтрокаУдержаний.Результат - РанееУдержано;
...
Функция ИсчисленныйПоДоходамНалог(МенеджерВременныхТаблиц, ОписаниеВТ, МесяцНачисления, ПогашениеЗаймов, ИсключаемыйРегистратор, НастройкиУдержаний, ОчередностьРасчета = Неопределено, ЭтоИсправлениеТекущимПериодом = Ложь) Экспорт 
	...
	Иначе 
		ОблагаемыеНалогомДоходы = РегистрыНакопления.СведенияОДоходахНДФЛ.СоздатьНаборЗаписей().Выгрузить(, "Период,ФизическоеЛицо,Начисление,СуммаДохода");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Уточнение значения реквизита «Прекратить по достижению предела» в исполнительных листах.';
									|en = 'Clarification of the ""Terminate upon reaching the limit"" attribute in enforcement orders.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Уточнение движений по регистру «Предельные суммы удержаний сотрудников».';
									|en = 'Clarify records of the ""Employee deduction amount limit"" register.'");
