				...
				Если ЗначениеЗаполнено(СобытиеОтпусков.НакопленоСПредыдущегоСобытия) Тогда
					...
					ОбластьМакета.Параметры.Текст  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						Формат(?(СобытиеОтпусков.Событие = НСтр("ru = 'Отпуск по уходу за ребенком';
																|en = 'Child care leave'"), СобытиеОтпусков.ДатаСобытия - 86400, СобытиеОтпусков.ДатаСобытия), ФорматДаты));
					ТабДокумент.Вывести(ОбластьМакета);
				...
				Если ЕстьКомпенсация И ЕстьРеальныйОтпуск Тогда
					ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Период отпуска: %1 (+ %2 дн. компенсации)';
							|en = 'Leave period: %1 (+ %2 days of compensation)'"), ПредставлениеПериода, Формат(ДнейКомпенсации, ФорматЧисла));
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Очистка данных вредных и северных отпусков.';
									|en = 'Clear data on hazard and northern leaves.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Пересчет вредных и северных отпусков.';
									|en = 'Recalculate hazard and northern leaves.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение реквизитов документа Отпуска сотрудников.';
									|en = 'Fill in attributes of the ""Employee leaves"" document.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка использования параметров наборов свойств документов ""График отпусков"".';
									|en = 'Set the use of parameters for property sets of the ""Leave schedule"" documents.'");
