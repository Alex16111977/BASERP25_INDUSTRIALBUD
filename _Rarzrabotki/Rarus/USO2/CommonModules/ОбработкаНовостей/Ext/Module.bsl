...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	ТекстСообщения = НСтр("ru = 'УстановитьСостояниеНовостейДляПользователей, начало работы.
		...
		|en = 'УстановитьСостояниеНовостейДляПользователей, started.
		|МассивПользователей:
		|	%1
		|СтруктураОтборов:
		|	%2
		|ЗначенияСостояний:
		|	%3
		|Functional option Work with news is allowed: %4'")
		+ Символы.ПС;
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru = 'БИП:Новости.Сервис и регламент';
			|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
		НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';
			|en = 'News. Service and regulation. УстановитьСостояниеНовостейДляПользователей'"), // ИдентификаторШага
	...
	Если (ТипЗнч(ЗначенияСостояний) = ТипСтруктура)
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ТекстСообщения = НСтр("ru = 'УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: при работе в модели сервиса запускать получение новостей вручную можно только при включенных разделителях.
					...
					|en = 'УстановитьСостояниеНовостейДляПользователей, execution error: while working in SaaS, you can initiate receiving news manually only when the separators are enabled.
					...
					|Configuratin (metadata); %Метаданные_Имя%, %Метаданные_Версия%
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: в качестве значений заполнения передано значение типа, отличного от Структура, либо не заполнены настройки заполнения (не указано ни одного ключа структуры).
			...
			|en = 'УстановитьСостояниеНовостейДляПользователей, execution error: type value was passed as population values  which differs from Structure or population settings are not filled in (no structure key is specified).
			|Possible key names match names of information register resources СостоянияНовостей'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';
	...
	Если ТипЗнч(ТекущийПользователь) <> ТипПользователь Тогда // Внешний пользователь, например.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Передан параметр Пользователь неправильного типа: %1.';
				|en = 'Incorrect type of the User parameter passed: %1.'"),
			ТипЗнч(Пользователь));
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Передан параметр ЛентыНовостей неправильного типа: %1.';
				|en = 'Incorrect type of the ЛентыНовостей parameter passed: %1.'"),
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Передан параметр МассивЛентНовостей неправильного типа: %1.';
				|en = 'Incorrect type of the МассивЛентНовостей parameter passed: %1.'"),
			...
			Если ПравоДоступа("Чтение", Метаданные.Справочники.Новости) Тогда
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru = 'БИП:Новости.Разное';
						|en = 'ISL:News.Miscellaneous'"), // ИмяСобытия
					НСтр("ru = 'Новости. Разное. Ошибка работы с пользователями';
						|en = 'News. Miscellaneous. User operation error'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПриИзмененииЗначенийОтборовОбщих() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'При изменении значений общих отборов';
			|en = 'When changing values of common filters'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Общие';
				|en = 'News. Service and regulation. ПриИзмененииЗначенийОтборов. Общие'"), // ИдентификаторШага
			?(КодРезультата > 0,
...
Процедура ПриИзмененииЗначенийОтборовДляОбластиДанных() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'При изменении значений отборов для области данных';
			|en = 'When changing values of filters for data area'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. ДляОбластиДанных';
				|en = 'News. Service and regulation. ПриИзмененииЗначенийОтборов. ДляОбластиДанных'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПриИзмененииЗначенийОтборовПользовательских(Пользователь = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'При изменении значений отборов для пользователя';
			|en = 'When changing values of filters for users '"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Пользовательские';
				|en = 'News. Service and regulation. ПриИзмененииЗначенийОтборов. Пользовательские'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = НСтр("ru = 'УстановитьНастройкиПоказаНовостей: в разделенном сеансе нельзя установить настройки показа новостей.';
									|en = 'УстановитьНастройкиПоказаНовостей: you cannot set up news display settings in the divided session.'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("ru = 'Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';
											|en = 'Started for each data area on migration to a new version. Configure user settings (separated data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Функция СоединитьсяССерверомНовостей(ИмяСервера, Протокол, Логин, Пароль, ХттпСоединение, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Соединение с сервером новостей';
			|en = 'Connection to the news server'"));
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Попытка %1 / %2: Ошибка определения порта из имени сервера %3 (%4). Лента новостей будет пропущена.';
									|en = 'Attempt %1 / %2: An error occurred while determining a port from the server name %3 (%4). The news feed will be skipped.'"),
								ПопыткаСоединения,
					...
					ОписаниеРезультата = ОписаниеРезультата
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Попытка %1 / %2: Протокол %3, Имя сервера %4, Порт %5';
								|en = 'Attempt %1 / %2: Protocol %3, Server name %4, Port %5'"),
							ПопыткаСоединения,
							НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
							Протокол,
							ИмяСервера_Сервер,
					...
					ОписаниеРезультата = ОписаниеРезультата
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Попытка %1 / %2: Ошибка подключения к серверу %3 (%4) для получения новостей по причине:
								|%5';
								|en = 'Attempt %1 / %2: An error occurred while connecting to server %3 (%4) to receive news due to: 
								|%5'"),
							ПопыткаСоединения,
							НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
							ИмяСервера,
		...
		Иначе
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Протокол получения новостей %1 отличается от http и https. Соединение с сервером новостей не устанавливалось.';
					|en = 'The %1 protocol for news receipt is different from http and https. Connection with the news server was not established.'"),
				Протокол);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Загрузка новостей';
				|en = 'ISL:News.News import'"), // ИмяСобытия
			НСтр("ru = 'Новости. Загрузка новостей. Соединение с сервером новостей';
				|en = 'News. Import news. Connection to news server'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область Шаблоны
	...
	ШаблонРезультатаРегламентныеОперации = Символы.ПС
		+ НСтр("ru = '### РегистрСведений.ВсеОбновленияНовостей
			...
			|%РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей%';
			|en = '### РегистрСведений.ВсеОбновленияНовостей
			|%РегистрСведений.ВсеОбновленияНовостей%
			|
			|### РегистрСведений.ДатыЗагрузкиПоследнихНовостей
			|%РегистрСведений.ДатыЗагрузкиПоследнихНовостей%
	...
	ШаблонРезультатаПрограммноеОкружение = Символы.ПС
		+ НСтр("ru = '### Константы
			...
			|%Константы%
			|
			|### Other programming environment parameters
			|%ДругиеПараметрыПрограммногоОкружения%
			|
			...
			|%ПараметрыСеанса%
			|
			|### Data on configuration (from metadata) and operating system
			|%ДанныеОКонфигурацииИзМетаданных%
			|
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ОбработкаНовостей.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%1] по причине:
						|%2';
						|en = 'ОбработкаНовостей.НайтиНовости: news search error, search line [%1] due to:
						|%2'"), 
					лкСтрокаПоиска,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Разное. Ошибка полнотекстового поиска';
						|en = 'News. Miscellaneous. Full-text search error'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ВсеОбновленияНовостей() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Регламентное обновление подсистемы новостей';
			|en = 'Scheduled update of news subsystem'"));
	...
	Если МонопольныйРежим() Тогда
		...
		ОписаниеРезультата = НСтр("ru = 'Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.';
									|en = 'Forcefully end scheduled job execution: infobase works in the exclusive mode.'");
	КонецЕсли;
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			ОписаниеРезультата = НСтр("ru = 'Разрешена работа с новостями. Выполнение шагов обновления.';
										|en = 'Work with news is allowed. Running update steps.'");
...
#Область ВсеОбновленияНовостей_ЗагрузкаНовостей
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = '1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)';
					|en = '1. Receive and process news from news feeds (dimension ЛентаНовостей is required) '"));
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
					...
					ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Разрешена работа через интернет
							...
							|en = 'Work via the Internet is allowed 
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления: %2'"),
...
#Область ВсеОбновленияНовостей_ОбновлениеПериодическихСвойств
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = '4. Обновление периодических свойств (важность, актуальность)';
					|en = '4. Periodic properties update (importance, relevance) '"));
...
#Область ВсеОбновленияНовостей_ОбновлениеПривязокКМетаданным
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = '5. Обновление привязок к метаданным (+ важность)';
					|en = '5. Metadata link update (+ importance)'"));
...
#КонецОбласти
		...
		Иначе
			ОписаниеРезультата = НСтр("ru = 'Запрещена работа с новостями.';
										|en = 'Operations with news are prohibited.'");
		КонецЕсли;
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru = 'БИП:Новости.Все обновления новостей';
			|en = 'ISL: News.All news updates'"), // ИмяСобытия
		НСтр("ru = 'Новости. Все обновления новостей';
			|en = 'News. All news updates'"), // ИдентификаторШага
		?(КодРезультата > 0,
			УровеньЖурналаРегистрации.Ошибка,
			УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
...
Процедура УдалитьНовости_Ежедневно(КонтекстВыполнения = Неопределено) Экспорт
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Удаление новостей (ежедневно) / УдалениеНовостей_ПолучитьСписокДанныхДляУдаления';
					|en = 'Remove news (daily) / УдалениеНовостей_ПолучитьСписокДанныхДляУдаления'"));
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Удаление новостей (ежедневно) / УдалениеНовостей_ПометитьНаУдалениеДанные';
					|en = 'Remove news (daily) / УдалениеНовостей_ПометитьНаУдалениеДанные'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно';
				|en = 'News. Service and regulation. УдалитьНовости_Ежедневно'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновлениеПериодическихСвойствНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Обновление периодических свойств новостей';
			|en = 'Update periodic news properties'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей';
				|en = 'News. Service and regulation. ОбновлениеПериодическихСвойствНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			...
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Обработка периодических реквизитов';
					|en = 'Process periodic attributes'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ОбновлениеПривязокКМетаданным(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным';
				|en = 'News. Service and regulation. ОбновлениеПривязокКМетаданным'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Получение и обработка новостей по лентам новостей';
			|en = 'Receive and process news by news feeds'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("ru = 'ПолучитьИОбработатьНовостиПоЛентамНовостей, при работе в модели сервиса запускать получение новостей вручную можно только в неразделенном сеансе.';
											|en = 'ПолучитьИОбработатьНовостиПоЛентамНовостей, when operating in SaaS you can start news receiving manually only in an undivided session.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
					...
					ОписаниеРезультата = ОписаниеРезультата
						...
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей
								...
								|The news feed list may be empty, or all news feeds may have the ЛокальнаяЛентаНовостей flag set (which means that this news does not have to be imported), or all news feeds may have been marked for deletion.'"),
							МассивЛентНовостей.Количество())
						+ Символы.ПС;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Все обновления новостей';
				|en = 'ISL: News.All news updates'"), // ИмяСобытия
			НСтр("ru = 'Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей';
				|en = 'News. All news updates. Receive and process news by news feeds'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполнения = Неопределено)
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("ru = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать получение классификаторов вручную можно только в неразделенном сеансе.';
											|en = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, you can run classifier receipt manually only in an undivided session while working in SaaS.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
			...
			Иначе
				ОписаниеРезультата = ОписаниеРезультата
					...
					+ НСтр("ru = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, отключен режим [Разрешена работа с новостями],
						...
						|en = 'ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, mode [Work with news is allowed] is disabled,
						|that is why all news center tools are disabled.'");
			КонецЕсли;
				...
				ОписаниеРезультата = ОписаниеРезультата
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Классификаторы были обновлены без ошибок.
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления: %2';
							|en = 'Classifiers were updated without errors.
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
				...
				ОписаниеРезультата = ОписаниеРезультата
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Были ошибки обновления классификаторов. Время следующего запуска изменено.
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления: %2';
							|en = 'Errors occurred while updating classifiers. Next launch time is changed.
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления:%2'"),
						ТекущаяДатаДляРегистров,
						Запись.ДатаСледующегоОбновления);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Все обновления новостей';
				|en = 'ISL: News.All news updates'"), // ИмяСобытия
			НСтр("ru = 'Новости. Все обновления новостей. Обновление классификаторов';
				|en = 'News. All news updates. Update classifiers'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьНеобработанныеКатегорииНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Пересчет необработанных категорий новостей';
			|en = 'Recalculation of unprocessed news categories'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("ru = 'ПересчитатьНеобработанныеКатегорииНовостей, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях.';
											|en = 'ПересчитатьНеобработанныеКатегорииНовостей, you can update standard lists in SaaS only with disabled separators.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
...
#Область ОбновлениеКатегорийИЗначений
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("ru = 'Пересчет необработанных категорий новостей';
						|en = 'Recalculation of unprocessed news categories'") + " / " + ИдентификаторШага1);
					...
					Если ЕстьДанныеДляОбработки = Истина Тогда
						...
						ОписаниеРезультатаВложенный.Добавить(
							НСтр("ru = 'Есть данные для обработки.';
								|en = 'There is data for processing.'"));
...
#Область ОчисткаМусора
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						КонтекстВыполненияВложенный,
						"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ИдентификаторШага1, // Идентификатор.
						НСтр("ru = 'ПересчитатьНеобработанныеКатегорииНовостей, удаление записей, не привязанных к новостям.';
							|en = 'ПересчитатьНеобработанныеКатегорииНовостей, delete records not linked to the news.'") + " / " + ИдентификаторШага1);
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей';
				|en = 'News. Service and regulation. ПересчитатьНеобработанныеКатегорииНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Пересчет отборов по новостям (редко меняющихся)';
			|en = 'Filter recalculation by news (rarely changed)'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_Общие(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Пересчет отборов по новостям (общих)';
			|en = 'Filter recalculation by (common) news'"));
...
#Область ЗаполнениеЗначенийАвтоматическихОтборов
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Заполнить значения автоматических отборов';
				|en = 'Filter recalculation by news / Fill in automatic filter values'"));
...
#Область ПересчетОтборов_ВыполнениеЗапроса
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Выполнение запроса';
				|en = 'Filter recalculation by news / Query execution'"));
...
#Область ПересчетОтборов_ЗаполнениеДанными
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Заполнение данными';
				|en = 'Filter recalculation by news / Fill in with data'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. Common'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Пересчет отборов по новостям (общие)';
					|en = 'Filter recalculation by news (common)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПересчитатьОтборыПоНовостям_ДляОбластиДанных(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Пересчет отборов по новостям (общих для области данных)';
			|en = 'Filter recalculation by news (common for data area)'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. ДляОбластиДанных';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. ДляОбластиДанных'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Пересчет отборов по новостям (общие по области данных)';
					|en = 'Filter recalculation by news (common by data area)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПересчитатьОтборыПоНовостям_Пользовательские(Пользователь = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Пересчет отборов по новостям (пользовательских)';
			|en = 'Filter recalculation by (user) news'"));
...
#Область ПолучениеСпискаПользователей
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Получение списка пользователей';
				|en = 'Filter recalculation by news / Receive user list'"));
...
#Область ОчисткаПредыдущегоРезультата
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("ru = 'Пересчет отборов по новостям / Очистка предыдущего результата';
				|en = 'Filter recalculation by news / Clear the previous result'"));
...
#Область ФормированиеСпискаНовостейИсключений_ВыполнениеЗапроса
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Пересчет отборов по новостям / Формирование списка новостей-исключений / Выполнение запроса';
					|en = 'Recalculate filters by news / Generate exception news list / Query execution'"));
...
#Область ФормированиеСпискаНовостейИсключений_РаботаСДаннымиЗапроса
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Пересчет отборов по новостям / Формирование списка новостей-исключений / Работа с данными запроса';
					|en = 'Recalculate filters by news / Generate exception news list / Process query data'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Пользовательские';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. Пользовательские'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			КомментарийДляЖурналаРегистрации(
				НСтр("ru = 'Пересчет отборов по новостям (пользовательские)';
					|en = 'Filter recalculation by news (user)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие';
						|en = 'News. Service and regulation. РассчитатьОтборПоКатегории. Общие'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных';
						|en = 'News. Service and regulation. РассчитатьОтборПоКатегории. ДляОбластиДанных'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПроверитьИзменениеВерсииПлатформы(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Проверка изменения версии платформы';
			|en = 'Check platform version change'"));
	...
	Если МонопольныйРежим() Тогда
		...
		ОписаниеРезультата = НСтр("ru = 'В монопольном режиме проверка версии платформы не осуществляется';
									|en = 'Platform version is not checked in the exclusive mode'");
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		ОписаниеРезультата = НСтр("ru = 'Сохраненная версия платформы: %СохраненнаяВерсияПлатформы%
			...
			|en = 'Saved platform version: %СохраненнаяВерсияПлатформы%
			|Current platform version: %ТекущаяВерсияПлатформы%
			|%ТребуетсяОбновлениеРедкоменяющихсяОтборов%
			|
			|Configuration version in this data processor is not checked, but forcefully recalculated in the configuration update module as well as after every news receipt.'");
							...
							ФоновыеЗадания.Выполнить(
								...
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';
										|en = 'ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся due to platform version change from %1 to %2'"),
									лкПоследняяВерсия,
									лкТекущаяВерсия)));
				...
				ОписаниеРезультата = ОписаниеРезультата
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Попытка зафиксировать транзакцию.
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления: %2';
							|en = 'Attempt to commit the transaction.
							|ТекущаяДатаДляРегистров: %1
							|ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
						ДатаСледующегоОбновления);
				...
				ОписаниеРезультата = ОписаниеРезультата
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось заблокировать запись регистра сведений ВсеОбновленияНовостей для вида обновлений %1.
							...
							|en = 'Cannot lock the record of the ВсеОбновленияНовостей information register for update kind %1.
							|Another process may be saving data at this moment. The following was supposed to be written:
							|ТекущаяДатаДляРегистров: %2
							|ДатаСледующегоОбновления: %3'"),
						ВидОбновления,
						ТекущаяДатаДляРегистров,
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Все обновления новостей';
				|en = 'ISL: News.All news updates'"), // ИмяСобытия
			НСтр("ru = 'Новости. Все обновления новостей. Проверка версии платформы';
				|en = 'News. All news updates. Check the platform version.'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ВыполнениеЗапроса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Выполнение запроса';
			|en = 'Optimize filters by news / Query execution'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Удаление отборов по несуществующим категориям / Для области данных';
			|en = 'Optimize filters by news / Delete filters for non-existent categories / For data area'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Пользовательские
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / УдалениеОтборовПоНесуществующимКатегориям / Пользовательские';
			|en = 'Optimize filters by news / УдалениеОтборовПоНесуществующимКатегориям / User'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Общие
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / УдалениеОтборовПоНесуществующимКатегориям / Общие';
			|en = 'Optimize filters by news / УдалениеОтборовПоНесуществующимКатегориям / Shared'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Удаление значений отборов / Общие для области данных';
			|en = 'Optimize filters by news / Delete filter values / Shared for data area'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_Пользовательские
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / УдалениеЗначенийОтборов / Пользовательские';
			|en = 'Optimize filters by news / УдалениеЗначенийОтборов / User'"));
...
#Область Оптимизация_ЛишниеКатегорииВНовостях
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Оптимизация отборов по новостям / Удаление лишних категорий в новостях';
			|en = 'Optimize filters by news / Delete extra categories in news'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. ОптимизацияОтборовПоНовостям';
				|en = 'News. Service and regulation. ОптимизацияОтборовПоНовостям'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура НайтиИОбработатьНеактивныеЛентыНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Поиск неактивных лент новостей';
			|en = 'Search for inactive news feeds'"));
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ОписаниеРезультата = НСтр("ru = 'Запрос выполнен.';
											|en = 'The query is executed.'") + Символы.ПС;
				...
				Пока Выборка.Следующий() Цикл
					...
					РезультатВыполнения_ЛентаНовостей = НСтр("ru = 'Лента новостей %ЛентаНовостей%, частота обновления ленты новостей: %ЧастотаОбновления%, дата последней успешной загрузки: %ДатаЗагрузкиПоследнихНовостей%, неудач: %КоличествоНеудачПриЗагрузкеНовостей%, последняя попытка загрузки: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%';
															|en = 'News feed %ЛентаНовостей%, news feed update frequency: %ЧастотаОбновления%, date of the last successful import: %ДатаЗагрузкиПоследнихНовостей%, number of failed attempts: %КоличествоНеудачПриЗагрузкеНовостей%, last importing attempt:%ДатаПоследнейПопыткиЗагрузкиНовостей% . %Действие%'");
						...
						Если Отказ <> Истина Тогда
							...
							Исключение
								...
								ТекстСообщения = НСтр("ru = 'Произошла ошибка при создании / обновлении новости о неактивности ленты новостей %ЛентаНовостей%
									|Подробно:
									|%ИнформацияОбОшибке%';
									|en = 'An error occurred when creating / updating the news about inactivity of the %ЛентаНовостей% news feed.
									|Details: 
									|%ИнформацияОбОшибке%'")
									+ Символы.ПС;
			...
			Иначе
				ОписаниеРезультата = НСтр("ru = 'Запрос пустой.';
											|en = 'Request is empty.'") + Символы.ПС;
			КонецЕсли;
			...
			ТекстСообщения = НСтр("ru = 'Окончание проверки лент новостей на актуальность.
				...
				|%РезультатВыполнения%';
				|en = 'News feed relevance check end.
				...
				|- this is not a mandatory news feed,
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей';
				|en = 'News. Service and regulation. НайтиИОбработатьНеактивныеЛентыНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			Иначе
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий:
						...
						|en = 'ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий:
	...
	Если ЕстьОшибка = Истина Тогда
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';
				|en = 'News. Service and regulation. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
Процедура УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Получение списка новостей для удаления';
			|en = 'Receive list of news for deletion'"));
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ДанныеДляУдаления.Свойство("НовостиДляУдаления"))
				...
				ТекстСообщения = НСтр("ru = 'Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.';
										|en = 'Some news are marked for deletion (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% of news). You should delete them manually in All functions / Standard / Deletion of marked objects.'");
			...
			Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления"))
				...
				ТекстСообщения = НСтр("ru = 'Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';
										|en = 'All obsolete news are successfully marked for deletion (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).'");
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Было удалено %1 необработанных простых категорий.';
						|en = '%1 unprocessed simple categories were deleted.'"),
					ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
			Иначе
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Было удалено %1 необработанных категорий интервалов версий.';
						|en = '%1 of unprocessed version interval categories were deleted.'"),
					ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
			Иначе
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления';
				|en = 'News. Service and regulation. УдалитьНовости_Ежедневно. After data collection for deletion'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполнения = Неопределено)
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'"), // ИмяСобытия
			НСтр("ru = 'Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения';
				|en = 'News. Service and regulation. Import standard values. All standard values'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Проверка правильности внедрения подсистемы. Начало.';
							|en = 'Check whether the subsystem is implemented correctly. Start.'")
					+ Символы.ПС;
				...
				Если НЕ ПустаяСтрока(Результат) Тогда
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Обновление ИБ';
							|en = 'ISL:News.IB update'"), // ИмяСобытия
						НСтр("ru = 'Новости. Обновление ИБ. Проверка внедрения';
							|en = 'News. Update IB. Check implementation'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Проверка правильности внедрения подсистемы. Конец.';
							|en = 'Check whether the subsystem is implemented correctly. End.'")
					+ Символы.ПС;

				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Установка основных констант. Начало.';
							|en = 'Set main constants. Start.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Очистка регистра ДатыЗагрузкиПоследнихНовостей. Начало.';
							|en = 'Clear the ДатыЗагрузкиПоследнихНовостей register. Start.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Очистка регистра ДатыЗагрузкиПоследнихНовостей. Конец.';
							|en = 'Clear the ДатыЗагрузкиПоследнихНовостей. register. End.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Установка даты загрузки новостей на -3 месяца. Начало.';
							|en = 'Set date of the news import to 3 months. Start.'")
					+ Символы.ПС;
				Запрос = Новый Запрос;
				Запрос.Текст = "
					|ВЫБРАТЬ
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Установка даты загрузки новостей на -3 месяца. Конец.';
							|en = 'Set date of the news import to 3 months. End.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("ru = 'Установка расписания регламентного задания. Начало.';
							|en = 'Set scheduled job schedule. Start.'")
					+ Символы.ПС;
				ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.Задание(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей);
				Если ТекущееРегламентноеЗадание <> Неопределено Тогда
					ТекущееРегламентноеЗадание.Наименование = НСтр("ru = 'Все обновления новостей';
					...
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("ru = 'Установка расписания регламентного задания. Конец.';
								|en = 'Set scheduled job schedule. End.'")
						+ Символы.ПС;
				Иначе
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ';
				|en = 'ISL:News.IB update'"), // ИмяСобытия
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск';
				|en = 'News. Infobase Update. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			...
			ТекстСообщения = НСтр("ru = 'Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.';
									|en = 'Work with news is not supported for an external user. Update handler is skipped.'");
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:Новости.Обновление ИБ';
					|en = 'ISL:News.IB update'"), // ИмяСобытия
				НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец';
					|en = 'News. Infobase Update. ОбластьДанных. ПервыйЗапуск. End'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Иначе
					ОписаниеРезультатаВложенный = НСтр("ru = 'Работа с новостями отключена. Обработчик пропущен.';
														|en = 'Work with news is disabled. Handler is skipped.'");
				КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ';
				|en = 'ISL:News.IB update'"), // ИмяСобытия
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск';
				|en = 'News. Infobase Update. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
				...
				Иначе
					...
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						...
						НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию';
							|en = 'News. Infobase Update. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						0, // Данные
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ';
				|en = 'ISL:News.IB update'"), // ИмяСобытия
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ';
				|en = 'News. Infobase Update. ОбщиеДанные. ПерейтиНаВерсию_ХХХ'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ОсновныеДействия_ВключениеФОНовости
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("ru = 'Основные действия / Установка ФО Разрешена работа с новостями';
					|en = 'Main actions / Set FR Work with news allowed'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ';
				|en = 'ISL:News.IB update'"), // ИмяСобытия
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию';
				|en = 'News. Infobase Update. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Обновление ИБ';
				|en = 'ISL:News.IB update'"), // ИмяСобытия
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ';
				|en = 'News. Infobase Update. ОбластьДанных. ПерейтиНаВерсию_ХХХ'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
		...
		Иначе
			...
			Если НайденныеНеправильныеСимволы.Количество() > 0 Тогда
				РезультатЛокальный = НСтр("ru = 'Неправильно настроена версия конфигурации (есть неправильные символы).
					...
					|ОбработкаНовостейПереопределяемый.ПолучитьЗначениеПредопределеннойКатегории,
					|for the ProductVersion condition line.
					|List of incorrect characters: %НайденныеНеправильныеСимволы%'");
