		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сведения по численности и оплате труда работников этой должности будут включены в указанную строку отчета по статистическому наблюдению по форме %1';
					|en = 'Information on the number of employees and labor compensation of this position will be included in the selected line of the report on the statistical monitoring according to form %1'"), 
				НастройкиСтатистики.ФормаОтчетностиМониторингаРаботниковСоциальнойСферы));
...
Процедура ЗначенияПоказателей_СтатистикаФорма57Т_2017Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ДанныеОтчета.Строки.Количество() > 0 Тогда
		...
		Если ДанныеОтчета.Строки.Количество() > 0 Тогда
			...
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П010204"] =
				СтрокаОрганизации[СоответствиеПользовательскихПолей.Получить(ЗарплатаКадрыКлиентСервер.ИдентификаторМакетаПечатнойФормы("Зарплата мужчин"))];
			Контейнер.ПолеТабличногоДокументаФормаОтчета["П010304"] =
				СтрокаОрганизации[СоответствиеПользовательскихПолей.Получить(ЗарплатаКадрыКлиентСервер.ИдентификаторМакетаПечатнойФормы("Зарплата женщин"))];
...
//  ПоказателиОтчета - структура
//
Процедура ОписаниеПоказателей_СтатистикаФорма1_2019Кв1(ПоказателиОтчета) Экспорт
...
//  Контейнер - структура - содержит все показатели отчета и их значения.
//
Процедура ЗначенияПоказателей_СтатистикаФорма1_2019Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	ПараметрыОтчетаДляЗаполненияКадры = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ПараметрыОтчета);
	...
	Если ДанныеОтчета.Строки.Количество() > 0 Тогда
		...
		ПараметрыОтчетаДляЗаполненияЗарплата = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ПараметрыОтчета);
	...
	ПараметрыОтчетаДляЗаполненияКадры = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ПараметрыОтчетаДляЗаполненияЗарплата);
...
Функция СотрудникиДляЗаполненияФормы1(Организация, СписокСотрудников, ПериодОтчета)
	...
	Для Каждого ИмяВидаВремени Из ИменаЭлементов Цикл
		ВидВремени = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(ИмяВидаВремени);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	|en = 'Adding records to the ""Social workers monitoring reporting line"" register.
	|Until the processing is complete, it is recommended not to edit the ""Staff list"" items.'");
