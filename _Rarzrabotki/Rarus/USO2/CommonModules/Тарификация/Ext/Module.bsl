...
Функция УслугаПоИдентификаторуИИдентификаторуПоставщика(Знач ИдентификаторУслуги, Знач ИдентификаторПоставщика, ВызыватьИсключение = Истина) Экспорт
	...
	Иначе
		ШаблонОшибки = НСтр("ru = 'Не найдена услуга по следующим полям:
                                        ...
                                        |en = 'Service for the following fields was not found:
                                        |- Supplier ID: %1
                                        |- Service ID: %2'");
...
Функция ЗарегистрированаЛицензияУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, КонтекстЛицензии = "") Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Проверка наличия лицензии уникальной услуги из приложения';
											|en = 'Check for the unique service license from the application'");
...
Функция ЗанятьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, ИдентификаторОперации, КонтекстЛицензии = "") Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Заем лицензии уникальной услуги из приложения';
											|en = 'Borrow unique service license from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Код возврата веб-сервера:';
									|en = 'Web service return code:'") + Строка(Ответ.КодСостояния);
		КонецЕсли;
...
Функция ОсвободитьЛицензиюУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, ИмяЛицензии, ИдентификаторОперации, КодОбластиДанных = Неопределено,
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Освобождение лицензии уникальной услуги из приложения';
											|en = 'Release of the unique service license from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
...
Функция ЗанятьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Заем лицезний лимитированной услуги из приложения';
											|en = 'Borrow limited service licenses from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			,
			КоличествоЛицензий,
...
Функция ОсвободитьЛицензииЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КоличествоЛицензий, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Освобождение лицезний лимитированной услуги из приложения';
											|en = 'Release of limited service licenses from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
...
Функция ПодтвердитьОперацию(ИдентификаторОперации) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Фиксация операции';
											|en = 'Operation record'");
		ЗаголовокКомментария = НСтр("ru = 'Идентификатор операции:';
									|en = 'Operation ID:'") + Строка(ИдентификаторОперации);
...
Функция КоличествоЛицензийУникальнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Получение количества свободных лицензий уникальной услуги из приложения';
											|en = 'Receive quantity of available unique service licenses from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			ИмяЛицензии,
			,
...
Функция КоличествоЛицензийЛимитированнойУслуги(ИдентификаторПоставщика, ИдентификаторУслуги, КодОбластиДанных = Неопределено) Экспорт
	...
	Попытка
		...
		ИмяСобытия = ИмяСобытияЖР() + НСтр("ru = 'Получение количества доступных лицензий лимитированной услуги из приложения';
											|en = 'Receive quantity of available limited service licenses from the application'");
		ЗаголовокКомментария = ЗаголовокКомментария(ИдентификаторПоставщика,
			ИдентификаторУслуги,
			,
			КоличествоЛицензий,
	...
	Иначе
		...
		Если Код = Неопределено Тогда 
			ВызватьИсключение НСтр("ru = 'При обновлении поставщика поле ""Код"" должно быть установлено';
									|en = 'The ""Code"" field must be set when updating the supplier'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТипУслугиПоИдентификатору(Знач ИдентификаторТипаУслуги) Экспорт
	...
	ВызватьИсключение НСтр("ru = 'Неизвестный код услуги:';
							|en = 'Unknown service code:'") + ИдентификаторТипаУслуги;
...
Функция РезультатПредопределенныхПроверокОперацийСЛицензиями() Экспорт
	...
	Если Не Результат Тогда
		ЗаголовокКомментария = НСтр("ru = 'Менеджер сервиса не поддерживает версию тарификации ""1.0.1.2""';
									|en = 'Service manager does not support tariffication version 1.0.1.2'");
...
Функция СтруктураОтветаHTTP(ОтветСтрокой)
	...
	Если ПустаяСтрока(ОтветСтрокой) Тогда
		ВызватьИсключение(НСтр("ru = 'Пустое тело ответа системы тарификации';
								|en = 'Empty response body of tariffication system'"));
	КонецЕсли;
...
Функция ЗначениеРазделителя(КодОбластиДанных)
	...
	Иначе
		Если Не ЗначениеЗаполнено(КодОбластиданных) Тогда
			ВызватьИсключение НСтр("ru = 'Для получения данных в неразделенном сеансе не указан код области данных';
									|en = 'Specify data area code to receive data in an undivided session'", КодЯзыка());
		Иначе
