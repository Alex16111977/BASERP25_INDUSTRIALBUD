...
Процедура ЗарегистрироватьДоходыИСуммыНДФЛПоВременнойТаблицеНачислений(Регистратор, Движения, Отказ, Организация, Дата, ПериодРегистрации, ПорядокВыплаты, ПланируемаяДатаВыплаты, ДанныеДляПроведения, Записывать = Ложь, РегистрироватьСуммыНалога = Неопределено, ДоходПолученНаТерриторииРФ = Ложь, ПеречислитьНалогПриВыплатеЗарплаты = Ложь, ИмяТаблицыФизлиц = "ВТФизическиеЛица") Экспорт 
	...
	Если РегистрироватьСуммыНалога Тогда
		...
		ДанныеДляПроведенияУчетЗарплаты.ВыплатитьКакАванс 		= ПеречислитьНалогПриВыплатеЗарплаты;
		УчетНачисленнойЗарплатыРасширенный.ЗарегистрироватьНДФЛИКорректировкиВыплаты(ДанныеДляПроведенияУчетЗарплаты, Отказ, ДанныеДляПроведения.НДФЛПоСотрудникам, Неопределено, Записывать);
...
Процедура ПередСкрытиемПерсональныхДанныхСубъектов(Субъекты, ТаблицаИсключений, Отказ) Экспорт
	...
	ПричинаОтменыСкрытия = НСтр("ru = 'субъект получал доходы, облагаемые НДФЛ';
								|en = 'the subject earned income subject to PIT'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение вторичных данных о заграничных подразделениях.';
									|en = 'Fill in secondary data on foreign departments.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Уточнение реквизита Начисление в сведениях о доходах по НДФЛ для документов «Приз, подарок», «Выплата бывшим сотрудникам» и «Начисление прочих доходов».';
									|en = 'Clarification of the Accrual attribute in PIT income information for the ""Prize, gift"", ""Payment to former employees"" and ""Other income accrual"" documents.'");
