...
Процедура ОбновитьНастройкиПолучениеВнешнихКомпонент(Параметры) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление настроек подсистемы ""Получение внешних компонент"". Начало обновления.';
			|en = 'Update settings of the ""External component receipt"" subsystem. Start updating.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Обновление настроек подсистемы ""Получение внешних компонент"". Успешно завершено.';
			|en = 'Updating settings of the ""External component receipt"" subsystem. Completed successfully.'"),
		Ложь);
...
Процедура УдалитьРегламентноеЗаданиеОбновления()
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Удалено регламентное задание обновления внешних компонент';
			|en = 'Scheduled job of external component update was deleted.'"),
		Ложь,
		Метаданные.РегламентныеЗадания.ОбновлениеВнешнихКомпонент);
...
Функция ИнформацияОбАктуальныхВерсияхВнешнихКомпонент(ОписаниеВнешнихКомпонент)
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ЧастиСтрок.Добавить(НСтр("ru = 'Не удалось получить актуальные версии внешних компонент.';
								|en = 'Cannot receive relevant versions of external components.'"));
		...
		ЧастиСтрок.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Техническая информация об ошибке:
				...
				|While receiving information about the current external component versions, the service returned an error.
				|URL: %1
				|Error code: %2
				|Details:
				|%3'"),
...
Функция ИнформацияОВерсияхВнешнихКомпонент(ОписаниеВнешнихКомпонент)
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ЧастиСтрок.Добавить(НСтр("ru = 'Не удалось версии внешних компонент.';
								|en = 'Cannot receive external component versions.'"));
		ЧастиСтрок.Добавить(Символы.ПС);
		ЧастиСтрок.Добавить(РезультатОперации.СообщениеОбОшибке);
		ЧастиСтрок.Добавить(Символы.ПС);
		ЧастиСтрок.Добавить(Символы.ПС);
		ЧастиСтрок.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%3';
				|en = 'Technical information about the error:
				|While receiving information about external component versions, the service returned an error.
				|URL: %1
				|Error code: %2
				|Details:
				|%3'"),
	...
	Для Каждого ОписательВерсии Из ДанныеВнешнихКомпонент Цикл
		Если Не ЗначениеЗаполнено(ОписательВерсии.Идентификатор) Тогда
			...
			СообщениеОбОшибке = НСтр("ru = 'Неверный формат ответа Сервиса внешних компонент.';
									|en = 'Incorrect format of external component service response.'");
		...
		Если Прав(НРег(СокрЛП(СтруктураURI.Хост)), 6) <> Прав(НРег(СокрЛП(ХостСервиса)), 6) Тогда
			...
			СообщениеОбОшибке = НСтр("ru = 'Неверный адрес файла обновления внешней компоненты.';
									|en = 'Incorrect address of external component update file.'");
			ЗаписатьИнформациюВЖурналРегистрации(СообщениеОбОшибке);
			ВызватьИсключение СообщениеОбОшибке;
...
Функция ПереопределитьСообщениеПользователю(КодОшибки)
	...
	Если КодОшибки = "НетДоступаКПрограмме" Тогда
		...
		ЧастиСтрок.Добавить(НСтр("ru = 'Доступ к обновлениям внешних компонент невозможен,
			...
			|en = 'Cannot access external component updates
			|as your application is not under official support'"));
		ЧастиСтрок.Добавить(Символы.ПС);
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Получение внешних компонент.';
				|en = 'Receive external components.'", ОбщегоНазначения.КодОсновногоЯзыка());
