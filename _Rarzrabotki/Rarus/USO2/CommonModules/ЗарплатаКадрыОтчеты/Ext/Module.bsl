...
Процедура ПередНачаломКомпоновкиРезультата(ОбъектОтчета) Экспорт
	...
	СхемаВXML = ОбщегоНазначения.ЗначениеВСтрокуXML(ОбъектОтчета.СхемаКомпоновкиДанных);
	ОбъектОтчета.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("СхемаВXML", СхемаВXML);
	
	// Сбор сведений о дополнительных полях
	...
	ВычисляемыеПоля = ВычисляемыеПоляОтчета(ОбъектОтчета);
	Для Каждого ВычисляемоеПоле Из ВычисляемыеПоля Цикл
		...
		Для Каждого КоллекцияПолей Из ДополнительныеПоля Цикл
			...
			ОписаниеПолей = СтрокиПутей.Получить(ВычисляемоеПоле.Значение);
			Если ОписаниеПолей <> Неопределено Тогда
				...
				ВычисляемоеПоле.Ключ.Выражение = ВычисляемоеПоле.Значение;
...
Процедура ПриЗавершенииКомпоновкиРезультата(ОбъектОтчета) Экспорт
	...
	Если ОбъектОтчета.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("СхемаВXML") Тогда
		
		ОбъектОтчета.СхемаКомпоновкиДанных = ОбщегоНазначения.ЗначениеИзСтрокиXML(
			ОбъектОтчета.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.СхемаВXML);
		
		ОбъектОтчета.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Удалить("СхемаВXML");
		
	КонецЕсли;
	
КонецПроцедуры

Функция ВычисляемыеПоляОтчета(ОбъектОтчета)
	
	ВычисляемыеПоля = Новый Соответствие;
	...
	Если ВыбранныеПоля.Количество() > 0 Тогда
		...
		Для Каждого ВыбранноеПоле Из ВыбранныеПоля Цикл
			...
			ВычисляемоеПоле = ОбъектОтчета.СхемаКомпоновкиДанных.ВычисляемыеПоля.Найти(ВыбранноеПоле.Ключ);
			Если ВычисляемоеПоле <> Неопределено Тогда
				ВычисляемыеПоля.Вставить(ВычисляемоеПоле, ВыбранноеПоле.Ключ);
			КонецЕсли;
...
Функция ТарифнаяСтавкаНаПечать(Знач ТарифнаяСтавка, Группировка = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТарифнаяСтавка) Тогда
		...
		Возврат Формат(Цел(ТарифнаяСтавка), ФорматнаяСтрока) + " " + НСтр("ru = 'руб';
																			|en = 'rub'") + ". " + Формат((ТарифнаяСтавка - Цел(ТарифнаяСтавка)), "ЧЦ=2; ЧДЦ=; ЧС=-2; ЧН=; ЧВН=") + " " + НСтр("ru = 'коп';
																																															|en = 'cents'") + ".";
			...
			Если ВсегоВзносов <> 0 Тогда
				...
				ТекстСекции = ТекстСекции
					+ СтрокаСНепереносимымиПробелами(НСтр("ru = 'Страховые взносы в ПФР';
															|en = 'Insurance contributions to PF'") + ": " + Формат(ВсегоВзносов, "ЧДЦ=2; ЧН="));
							...
							Если СвойстваРегистратора <> Неопределено Тогда
								...
								Если НастройкиВывода.ДетализироватьВыплатыПоВедомостям = Истина
									...
									СтрокаКоллекции.ВидРасчета = СтрокаКоллекции.ВидРасчета + " "
										...
										+ НСтр("ru = 'от';
												|en = 'dated'") + " " + Формат(СвойстваРегистратора.Дата, "ДФ=dd.MM.yy");	
...
Функция ПроверитьНастройкиРассылки(Рассылка) Экспорт
	...
	Результат.ИмяСобытия  = НСтр("ru = 'Рассылка отчетов. Проверка настроек';
								|en = 'Report bulk mail. Check settings'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
