...
Функция ПредопределенныйЭлемент(ПолноеИмяПредопределенного) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		Если ЗарплатаКадрыПовтИсп.СтандартныеПодсистемыВерсии302() Тогда
			ВызываемыйМодуль = ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначения");
		Иначе
			ВызываемыйМодуль = ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияКлиентСервер");
		КонецЕсли;
		
	#Иначе
		
		Если ЗарплатаКадрыРасширенныйКлиентПовтИсп.СтандартныеПодсистемыВерсии302() Тогда
			ВызываемыйМодуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбщегоНазначенияКлиент");
		Иначе
			ВызываемыйМодуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбщегоНазначенияКлиентСервер");
		КонецЕсли;
		
	#КонецЕсли
	
	Возврат ВызываемыйМодуль.ПредопределенныйЭлемент(ПолноеИмяПредопределенного);
