...
Функция ПостроитьДеревоОператоров(УсловиеДляЗапроса = Истина) Экспорт
	...
	Если УсловиеДляЗапроса Тогда
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Условие';
																		|en = 'Condition'"), НСтр("ru = 'ВЫБОР КОГДА <Условие> ТОГДА <РезультатИстина> ИНАЧЕ <РезультатЛожь> КОНЕЦ';
																								|en = 'SELECTION WHEN <Condition> THEN <РезультатИстина> OTHERWISE <РезультатЛожь> END'"), 3);
	Иначе
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Условие';
																		|en = 'Condition'"), НСтр("ru = '?(<Условие>, <РезультатИстина>, <РезультатЛожь>)';
																								|en = '?(<Условие>, <РезультатИстина>, <РезультатЛожь>)'"), 3);
	КонецЕсли;
