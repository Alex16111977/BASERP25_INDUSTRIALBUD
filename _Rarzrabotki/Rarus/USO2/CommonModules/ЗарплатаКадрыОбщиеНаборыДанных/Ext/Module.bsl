...
Процедура ДобавитьВНаборПапкуПоПутиКПолю(НаборДанных, ПутьКПолю)
	...
	ПутьКПапке = ПутьКПапкеПоля(ПутьКПолю);
	Если Не ПустаяСтрока(ПутьКПапке) Тогда
		...
		Если Папка = Неопределено Тогда
			...
			Папка.Заголовок = ЗаголовокПоПутиПоля(ПутьКПапке);
		КонецЕсли;
...
Функция ПутьКПапкеПоля(ПутьКПолю)
...
Процедура ЗаполнитьВычисляемыеПоляСКД(СКД, ДополнительныеПоля) Экспорт
	
	ПоляНаборов = Новый Соответствие;
	Для Каждого ПолеНабора Из СКД.НаборыДанных[0].Поля Цикл
		...
		ПоляНаборов.Вставить(ПолеНабора.Поле, Истина);
		
	КонецЦикла;
	
	Для Каждого ОписаниеТаблицы Из ДополнительныеПоля Цикл
		
		Если ОписаниеТаблицы.Ключ = "_" Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого ОписаниеПоля Из ОписаниеТаблицы.Значение Цикл
			
			Если ПоляНаборов.Получить(ОписаниеПоля.ИмяПоля) = Истина Тогда
				Продолжить;
			КонецЕсли;
			
			Если ОписаниеПоля.ПсевдонимыПолей.Количество() > 0 Тогда
				ПсевдонимыПолей = ОписаниеПоля.ПсевдонимыПолей;
			Иначе
				ПсевдонимыПолей = Новый Структура("ЗаголовокПоляСКД,ПутьПоляСКД", ОписаниеПоля.ЗаголовокПоляСКД, ОписаниеПоля.ПутьПоляСКД);
			КонецЕсли;
			
			Для Каждого ОписаниеПсевдонимаПоля Из ПсевдонимыПолей Цикл
				
				Если СКД.ВычисляемыеПоля.Найти(ОписаниеПсевдонимаПоля.Значение.ПутьПоляСКД) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				Поле = СКД.ВычисляемыеПоля.Добавить();
				Поле.Выражение = ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов;
				Поле.ПутьКДанным = ОписаниеПсевдонимаПоля.Значение.ПутьПоляСКД;
				
				Если ЗначениеЗаполнено(ОписаниеПсевдонимаПоля.Значение.ЗаголовокПоляСКД) Тогда
					Поле.Заголовок = ОписаниеПсевдонимаПоля.Значение.ЗаголовокПоляСКД;
				Иначе
					Поле.Заголовок = ЗаголовокПоПутиПоля(Поле.ПутьКДанным);
				КонецЕсли;
