...
Функция ПрочитанныеДанныеОтвета(Ответ)
	...
	Если ЗначениеЗаполнено(ТелоОтвета) Тогда
		...
		Исключение
			...
			Событие = ИмяСобытия() + "." + НСтр("ru = 'Ошибка чтения JSON';
												|en = 'Error reading JSON'", Локаль);
		...
		Для каждого Элемент Из РезультатОтвет Цикл
			...
			Исключение
				...
				Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
				Событие = ИмяСобытия() + "." + НСтр("ru = 'Ошибка преобразования JSON';
													|en = 'JSON converting error'", Локаль);
				ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка,,, Комментарий);
				ВызватьИсключение;
			КонецПопытки; 
		КонецЦикла; 
...
Функция ИмяСобытия()
	...
	Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
	Возврат НСтр("ru = 'Программный интерфейс сервиса';
				|en = 'Service application interface'", Локаль);
