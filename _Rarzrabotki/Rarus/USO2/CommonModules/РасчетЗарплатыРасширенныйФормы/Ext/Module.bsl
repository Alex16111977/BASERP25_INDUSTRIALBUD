...
Процедура РасчетЗарплатыНачисленияВДанныеФормы(ТаблицыНачислений, ТаблицаИсточник, Организация, МесяцНачисления = Неопределено, НачальныеПозицииВставки = Неопределено, РежимДоначисления = Ложь) Экспорт
	...
	ВидыРасчетаИнфо = ЗарплатаКадрыРасширенный.ИнформацияОВидахРасчета(ТаблицаИсточник.ВыгрузитьКолонку("Начисление"));
...
Процедура СтрокаКорректировкиВыплатыВДанныеМенеджераРасчета(МенеджерРасчета, СтрокаИсточник, ДанныеРезультатовРаспределения = Неопределено) Экспорт
	...
	ЗаполнитьЗначенияСвойств(СтрокаПриемник, СтрокаИсточник);
	СтрокаПриемник.РаспределениеПоСтатьям = ОтражениеЗарплатыВУчетеРасширенный.НоваяТаблицаРаспределениеРезультатовУдержаний();
	РезультатРаспределенияВМенеджерРасчета(СтрокаПриемник, СтрокаИсточник, СтрокаИсточник.ИдентификаторСтроки, ДанныеРезультатовРаспределения);
	...
	Иначе
		...
		Для каждого Элемент Из МесяцыПериодаПредоставленияВычета Цикл
			...
			Иначе
				...
				СтрокаДляЗаполнения.РаспределениеПоСтатьям = ОтражениеЗарплатыВУчетеРасширенный.НоваяТаблицаРаспределениеРезультатовУдержаний();
			КонецЕсли;
			Для Каждого СтрокаВычета Из СтрокиПримененныеВычеты Цикл
