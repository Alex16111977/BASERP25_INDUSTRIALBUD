...
Процедура ПроверитьЗаполнениеПлатежногоДокумента(ПлатежныйДокумент, МассивВедомостей, МассивОшибок) Экспорт
	...
	Если Не ДоступностьПлатежногоДокумента(ПлатежныйДокумент) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'По документу %1 приняты файлы подтверждения зачисления зарплаты. Изменение документа невозможно.';
				|en = 'Files which confirm salary crediting were received against document %1. You cannot change the document.'"), ПлатежныйДокумент);
	...
	Для каждого Ведомость Из МассивВедомостей Цикл
		...
		Иначе
			Если МассивПовторяющихсяВедомостей.Найти(Ведомость) = Неопределено Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 уже включена в %2';
																							|en = '%1 is already included in %2'"), Ведомость, ПлатежныйДокумент);
	...
	Если Результат.Количество() > 1 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В документе %1 должны присутствовать ведомости только из одной организации и зарплатного проекта.';
				|en = 'The %1 document should contain paysheets only from one company and payroll card program.'"), ПлатежныйДокумент);
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Неопределено));
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ведомость %1 не проведена.';
	...
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'По ведомости %1 уже получено подтверждение зачисления зарплаты.';
				|en = 'Salary crediting confirmation already received by paysheet %1.'"), Выборка.Ведомость);
		МассивОшибок.Добавить(Новый Структура("ТекстСообщения, Ведомость", ТекстОшибки, Выборка.Ведомость));
	КонецЦикла;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение серии-номера документа сотрудника в ""Заявка на открытие лицевых счетов"".';
									|en = 'Fill in the employee document series and number in ""Request for opening personal accounts"".'");
...
Процедура ПрисоединитьФайлОбменаСБанкамиКОбъекту(ВладелецФайла, ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла)
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Номер",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'По документу %1 получено подтверждение из банка. Изменение документа невозможно';
																		|en = 'Bank confirmation has already been received for document %1, therefore it cannot be modified.'"), ВладелецФайла),
			ВладелецФайла,
			,
				...
				ИначеЕсли Фасет.Вид = ВидФасетаXDTO.Длина И СтрДлина(ЗначениеПоля) <> Число(Фасет.Значение) Тогда
					...
					Если СвойствоОбъектаXDTO.Имя = "НомерСчета" Тогда
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
							...
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У сотрудника %1 неверный номер лицевого счета, он должен содержать 20 цифр - %2.';
																						|en = 'Incorrect account number of employee %1, it should contain 20 digits - %2.'"), ТекущиеДанные.Сотрудник, ЗначениеПоля),
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Обмен с банками по зарплатным проектам.Загрузка подтверждения банка из файла';
					|en = 'Bank exchange by payroll card programs. Import bank confirmation from the file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
