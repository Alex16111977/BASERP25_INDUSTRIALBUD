...
Процедура ПриЧтенииСозданииНаСервере(Форма) Экспорт
	...
	ВнеоборотныеАктивыКлиентСервер.УстановитьВидимостьЗначенияСпискаВыбора(
		...
		НСтр("ru = 'Принятие к учету по договору лизинга';
			|en = 'Recognition under lease agreement'"));
...
Процедура ДополнитьВспомогательныеРеквизиты(Объект, ВспомогательныеРеквизиты) Экспорт
	...
	ВспомогательныеРеквизиты.Вставить("РаздельныйУчетТоваровПоНалогообложениюНДС", 
		ПараметрыУчетнойПолитики <> Неопределено И ПараметрыУчетнойПолитики.РаздельныйУчетТоваровПоНалогообложениюНДС);
...
Процедура ПроверитьПринятиеКУчетуПоДоговоруЛизинга(Объект, Отказ)
	...
	ТекстСообщения = НСтр("ru = 'Дата принятия к учету должна быть позже даты поступления предметов лизинга %1';
							|en = 'Date of recognition must be later than the leasing item inpayment date %1'");
...
Процедура ЗаполнитьНаОснованииПоступленияПредметовЛизингаПриВводеНаОсновании(Объект, Основание)
	...
	Если НЕ ЗаполнитьНаОснованииПоступленияПредметовЛизинга(Объект) Тогда
		ТекстОшибки = НСтр("ru = 'Принятие к учету основных средств, учитываемых за балансом, не предусмотрено';
							|en = 'Recognition of fixed assets recorded off the balance sheet is not supported'");
