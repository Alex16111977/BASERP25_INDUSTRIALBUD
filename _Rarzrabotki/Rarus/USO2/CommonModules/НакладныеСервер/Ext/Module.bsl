		...
		Если Параметры.ОрдернаяСхемаПриОтгрузке Тогда
			...
			Элемент.Заголовок = ?(Параметры.ИспользоватьНакладныеПоНесколькимЗаказам,
				...
				НСтр("ru = 'Подобрать товары по заказу/ордерам';
					|en = 'Select goods by order/notes'"));
			
		Иначе
			
			Элемент.Заголовок = ?(Параметры.ИспользоватьНакладныеПоНесколькимЗаказам,
				НСтр("ru = 'Подобрать товары по заказам';
					|en = 'Select goods by orders'"),
				НСтр("ru = 'Подобрать товары по заказу';
					|en = 'Select goods against the order'"));
...
Процедура ЗаполнитьСостоянияНакладной(СписокВыбора, ТолькоАктивные = Ложь) Экспорт
	...
	Иначе
		СписокВыбора.Добавить("0", НСтр("ru = 'Все оформлено';
										|en = 'Everything is registered'"),          Ложь, БиблиотекаКартинок.ВыполненоУспешно);
		СписокВыбора.Добавить("1", НСтр("ru = 'Создать накладную';
										|en = 'Create invoice'"),      Ложь, БиблиотекаКартинок.СоздатьНакладную);
		СписокВыбора.Добавить("2", НСтр("ru = 'Дооформить накладную';
										|en = 'Finish creating invoice'"),   Ложь, БиблиотекаКартинок.ДооформитьНакладную);
		СписокВыбора.Добавить("4", НСтр("ru = 'Не используются заказы';
										|en = 'Orders are not used'"), Ложь, БиблиотекаКартинок.ПустаяКартинка);
	КонецЕсли;
...
Процедура ЗаполнитьСостоянияОрдера(СписокВыбора, ТолькоАктивные = Ложь) Экспорт
	...
	Иначе
		СписокВыбора.Добавить("0", НСтр("ru = 'Соответствуют накладным';
										|en = 'Correspond to invoices'"),    Ложь, БиблиотекаКартинок.ВыполненоУспешно);
		СписокВыбора.Добавить("1", НСтр("ru = 'Не оформлены';
										|en = 'Not registered'"),               Ложь, БиблиотекаКартинок.СоздатьНакладную);
		СписокВыбора.Добавить("3", НСтр("ru = 'Не соответствуют накладным';
										|en = 'Do not correspond to invoices'"), Ложь, БиблиотекаКартинок.НесоответствиеОрдерНакладная);
		СписокВыбора.Добавить("4", НСтр("ru = 'Не используются';
										|en = 'Not used'"),            Ложь, БиблиотекаКартинок.ПустаяКартинка);
	КонецЕсли;
...
Функция ПроверитьВозможностьВводаНаОсновании(Документ, Статус, ЕстьОшибкиПроведен, ЕстьОшибкиСтатус, МассивДопустимыхСтатусов) Экспорт
	...
	Если ЕстьОшибкиПроведен Тогда
		...
		ТекстОшибки = НСтр("ru = 'Документ не проведен. Ввод на основании непроведенного документа запрещен.';
							|en = 'The document is not posted. Input on basis of unposted document is prohibited.'");
		...
		Если МассивДопустимыхСтатусов.Количество() = 1 Тогда
			Если Статус <> Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Документ находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусе %СтрокаДопустимыхСтатусов%.';
									|en = 'Document is in the ""%Статус%"" status. The ""Input on basis"" method is allowed only in the %СтрокаДопустимыхСтатусов% status.'");
			Иначе
				ТекстОшибки = НСтр("ru = 'Ввод на основании разрешен только в статусе %СтрокаДопустимыхСтатусов%.';
									|en = 'Input on the basis is allowed only in the %СтрокаДопустимыхСтатусов% status.'");
			КонецЕсли;
		Иначе
			Если Статус <> Неопределено Тогда
				ТекстОшибки = НСтр("ru = 'Документ находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусах %СтрокаДопустимыхСтатусов%.';
									|en = 'Document is in the ""%Статус%"" status. The ""Input on basis"" method is allowed only in the %СтрокаДопустимыхСтатусов% statuses. '");
			Иначе
				ТекстОшибки = НСтр("ru = 'Ввод на основании разрешен только в статусах %СтрокаДопустимыхСтатусов%.';
									|en = 'Input on the basis is allowed only in the %СтрокаДопустимыхСтатусов% statuses.'");
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаСостоянияПередачИзПроизводства(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|   			//ПолеНоменклатура_И РаспоряженияНаПередачуИзПроизводства.Номенклатура   = Распоряжения.Номенклатура
	|   			//ПолеНоменклатура_И РаспоряженияНаПередачуИзПроизводства.Характеристика = Распоряжения.Характеристика
