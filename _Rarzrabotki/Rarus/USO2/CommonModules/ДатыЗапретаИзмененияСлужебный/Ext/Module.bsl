	...
	ЗаписьЖурналаРегистрации(
		?(УзелПроверкиЗапретаЗагрузки <> Неопределено,
			НСтр("ru = 'Даты запрета изменения.Найдены запреты загрузки';
				|en = 'Change closing date.Import prohibitions are found'",
			     ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Даты запрета изменения.Найдены запреты изменения';
				|en = 'Change closing date.Change prohibitions are found'",
			     ОбщегоНазначения.КодОсновногоЯзыка())),
...
Функция ТекстОшибкиДатыЗапретаЗагрузкиНеВнедрены() Экспорт
	...
	Возврат НСтр("ru = 'Даты запрета загрузки данных прошлых периодов из других программ
	                   ...
	                   |en = 'Closing dates of data import of previous periods from other applications
				...
				Иначе
					...
					ИначеЕсли Выборка.Ссылка <> СвойстваРаздела.Ссылка Тогда
						...
						Объект.Наименование = "(" + НСтр("ru = 'не используется';
														|en = 'not used'") + ") " + СвойстваРаздела.Представление;
					КонецЕсли;
				КонецЕсли;
	 ...
	 Или ИсточникиДанных.Количество() = 0 Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для таблицы ""%1""
			           ...
			           |en = 'For the ""%1""
			           | table in the FillInDataSourcesToCheckChangeProhibition procedure
			           |of the ChangeClosingDatesOverridable common module
			           | data sources required to check closing date are not specified.'"),
			Таблица);
...
Функция ТекущиеИсточникиДанныхДляПроверкиЗапретаИзменения(СвойстваРазделов)
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
		           |общего модуля ДатыЗапретаИзмененияПереопределяемый.';
		           |en = 'An error occurred in the ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для проверки запрета изменения в источнике данных неверно указана таблица
				           |""%1"".';
				           |en = 'The ""%1""
				           |table is specified incorrectly in the data source to check change prohibition.'"),
				Строка.Таблица);
		КонецЕсли;
			...
			Если СвойстваРаздела = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для проверки запрета изменения в источнике данных указан несуществующий
					           ...
					           |en = 'Non-existing section ""%1"" is specified in the data source for the ""%2"" table
					           |to check change prohibition.'"),
		...
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для проверки запрета изменения 
				           ...
				           |не задано поле даты.';
				           |en = 'The date field is not specified
				           |in data source for the ""%1"" table
				           |for checking the change prohibition. '"),
				Источник.Таблица);
		Иначе
			Возврат Новый ФиксированнаяСтруктура(Свойства);
		КонецЕсли;
		...
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для проверки запрета изменения 
				           ...
				           |en = 'To check the change prohibition
		...
		Если ВидПоля = "ПолеДаты" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для проверки запрета изменения 
				           ...
				           |the field of the specified tabular section ""%2"" is not set.'"),
	...
	Если Не ЭлементДанных.Отбор.Пользователь.Использование Тогда
		ВызватьИсключение
			НСтр("ru = 'Выгрузка или загрузка записей регистра сведений ""Даты запрета изменения""
			           ...
			           |en = 'You can export and import records of the ""Change closing dates"" information register
			           |only by the User dimension.'");
	КонецЕсли;
