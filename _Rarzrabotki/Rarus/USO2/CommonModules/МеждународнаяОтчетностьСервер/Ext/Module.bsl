...
Функция ЗначениеПростогоПоказателя(Параметры, Показатель = Неопределено)
	...
	Если ЭлементОтчета.ВидЭлемента = Монетарный
		...
		ШаблонТекста = НСтр("ru = 'Не определено значение показателя 
							...
							|as: an account of the chart of accounts is not specified.'");
...
Функция ЗначениеПроизводногоПоказателя(Параметры, Показатель = Неопределено)
	...
	Если ЗначениеЗаполнено(ОперандыФормулы) Тогда
		...
		Если Параметры.Свойство("ОшибкаРасчета") Тогда
			ШаблонТекста = НСтр("ru = 'Недостоверное значение показателя 
								...
								|%1(%2)
								|as: an error occurred while calculating compound indicators.'");
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонТекста, 
				ЭлементОтчета.НаименованиеДляПечати, 
				ЭлементОтчета.Код);
	...
	Если ПустаяСтрока(ЭлементОтчета.Формула) Тогда
		ШаблонТекста = НСтр("ru = 'Не определено значение показателя 
							...
							|as: a calculation formula is not specified.'");
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонТекста, 
			ЭлементОтчета.НаименованиеДляПечати, 
			ЭлементОтчета.Код);
...
Процедура ВывестиШапкуТаблицы(Параметры, ЭлементВывода, ДокументРезультат)
	...
	Если Параметры.ВыводитьКодСтроки Тогда
		...
		ОбъединитьПоВертикали(ДокументРезультат, ПерваяСтрокаШапки, КоличествоУровнейШапки, ПерваяКолонкаШапки, НСтр("ru = 'Код строки';
																													|en = 'Line code'"), Истина);
	КонецЕсли;
	Если Параметры.ВыводитьПримечание Тогда
		ПерваяКолонкаШапки = ПерваяКолонкаШапки + 1;
		ОбъединитьПоВертикали(ДокументРезультат, ПерваяСтрокаШапки, КоличествоУровнейШапки, ПерваяКолонкаШапки, НСтр("ru = 'Примечание';
																													|en = 'Note'"), Истина);
	КонецЕсли;
...
Процедура УстановитьВалюту(ДокументРезультат, Валюта, ПредставлениеВалюты) Экспорт
	...
	Если СтрНайти(Область.Текст, ": " + НСтр("ru = 'Да';
											|en = 'Yes'")) > 0 Тогда
...
Процедура ОбновитьДеревоНовыхЭлементов(Форма, Параметры) Экспорт
	...
	ЗаголовокПоказателяМежд = НСтр("ru = 'Монетарный показатель международного учета';
									|en = 'International accounting monetary indicator'");
	ЗаголовокПоказателяРегл = НСтр("ru = 'Монетарный показатель регламентированного учета';
									|en = 'Monetary indicator of compliance accounting'");
...
Процедура ОбновитьДеревоСохраненныхЭлементов(Форма, Параметры) Экспорт
	...
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Счет плана счетов';
																	|en = 'Account of chart of accounts'")+"]", НСтр("ru = 'Счет плана счетов';
																											|en = 'Account of chart of accounts'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Тип итога';
																	|en = 'Total type'")+"]", НСтр("ru = 'Тип итога';
																									|en = 'Total type'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Начальное сальдо';
																	|en = 'Start balance'")+"]", НСтр("ru = 'Начальное сальдо';
																										|en = 'Start balance'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Немонетарный показатель';
																	|en = 'Non-monetary indicator'")+"]", НСтр("ru = 'Немонетарный показатель';
																												|en = 'Non-monetary indicator'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Выводить заголовок элемента';
																	|en = 'Display item title'")+"]", НСтр("ru = 'Выводить заголовок элемента';
																													|en = 'Display item title'"));
...
Процедура ДобавитьИзмеренияВДеревоНовыхЭлементов(ДеревоЭлементов, ВидПоказателей, БыстрыйПоиск)
	...
	ЗаголовокМежд = НСтр("ru = 'Виды субконто международные';
						|en = 'International extra dimension types'");
	ЗаголовокРегл = НСтр("ru = 'Виды субконто хозрасчетные';
						|en = 'Self-financing extra dimension types'");
	Если ВидПоказателей = ВидыПоказателейОтчетности.Международные Тогда
...
Процедура ЗаполнитьВалютуОтчета(ЭлементРесурс, ВидПоказателей) Экспорт
	...
	Если ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.Международные Тогда
		...
		СписокВыбора.Добавить("Сумма",НСтр("ru = 'В функциональной валюте';
											|en = 'In functional currency'") + СтрШаблон(" (%1)", ВалютаМУ.Функциональная));
		СписокВыбора.Добавить("СуммаПредставления",НСтр("ru = 'В валюте представления';
														|en = 'In reporting currency'") + СтрШаблон(" (%1)", ВалютаМУ.Представления));
	...
	ИначеЕсли ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.Регламентированные Тогда
		...
		СписокВыбора.Добавить("Сумма", НСтр("ru = 'Бухгалтерский учет';
											|en = 'Bookkeeping'") + СтрШаблон(" (%1)", ВалютаРегл));
		Если ПолучитьФункциональнуюОпцию("ВестиУУНаПланеСчетовХозрасчетный") Тогда
			СписокВыбора.Добавить("СуммаУУ", НСтр("ru = 'Управленческий учет';
													|en = 'Management accounting'") + СтрШаблон(" (%1)", ВалютаУУ));
		...
		Если ПолучитьФункциональнуюОпцию("ВестиУчетНаПланеСчетовХозрасчетныйВВалютеФинОтчетности") Тогда
			СписокВыбора.Добавить("СуммаФО", НСтр("ru = 'В валюте финансовой отчетности';
													|en = 'In financial reporting currency'") + СтрШаблон(" (%1)", ВалютаФО));
			ДоступностьРесурса = Истина;
			ВидимостьРесурса = Истина;
		КонецЕсли;
	...
	ИначеЕсли ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.МеждународныеИРегламентированные Тогда
		...
		Если ВалютаРегл = ВалютаМУ.Функциональная Тогда
			СписокВыбора.Добавить("Сумма",НСтр("ru = 'В функц. и регл. валюте';
												|en = 'In functional and compl. currency'") + СтрШаблон(" (%1)", ВалютаМУ.Функциональная));
		ИначеЕсли ВалютаРегл = ВалютаМУ.Представления Тогда
			СписокВыбора.Добавить("Сумма",НСтр("ru = 'В представл. и регл. валюте';
												|en = 'In present. and compl. currency'") + СтрШаблон(" (%1)", ВалютаМУ.Представления));
		Иначе
			СписокВыбора.Добавить("Сумма",НСтр("ru = 'В функц. и регл. валюте';
												|en = 'In functional and compl. currency'") + СтрШаблон(" (%1, %2)", ВалютаМУ.Функциональная, ВалютаРегл));
		КонецЕсли;
