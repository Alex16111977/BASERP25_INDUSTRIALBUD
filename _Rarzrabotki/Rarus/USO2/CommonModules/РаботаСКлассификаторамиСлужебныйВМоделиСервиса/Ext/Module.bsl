		...
		РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработка классификатора %1 не требуется, т.к. настройка ОбновлятьАвтоматически имеет значение Ложь
					...
					|en = 'Processing the %1 classifier is not required, as the UpdateAutomatically setting is set to the False value
					|for more information, see the ClassifierOperationsOverridable.OnAddingClassifiers method implementation.'"),
				Идентификатор),
			Ложь);
		Возврат;
	КонецЕсли;
		...
		РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Файл классификатора %1 сохранен в кэше поставляемых данных.';
					|en = 'The %1 classifier file is saved to cash of supplied data.'"),
				Идентификатор),
			Ложь);
	КонецЕсли;
	...
	РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начало обработки Файл классификатора %1.';
				|en = 'Start processing The %1 classifier file.'"),
			Идентификатор),
		Ложь);
		...
		Иначе
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать поставляемые данные классификатора:
					|Идентификатор: %1
					|Версия: %2';
					|en = 'Cannot process supplied classifier data:
					|ID: %1
					|Version: %2'"),
				Идентификатор,
				Версия);
	...
	РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершена обработка файла классификатора %1.';
				|en = 'Processing the %1 classifier file is completed.'"),
			Идентификатор),
		Ложь);
	...
	РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Удален кэш поставляемых данных классификаторов: %1';
				|en = 'Cache of supplied classifier data is deleted: %1'"),
				...
				Если ТранзакцияАктивна() Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По завершении выполнения обработчика ПриОбработкеОбластиДанных классификатора %1 не была закрыта транзакция.';
							|en = 'Transaction was not closed after the ПриОбработкеОбластиДанных handler of the %1 classifier is finished.'"),
						НастройкиКлассификатора.Идентификатор);
					РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
						ТекстСообщения,
						Истина);
			...
			Исключение
				...
				СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось обработать разделенные данные классификатора:
						...
						|%4';
						|en = 'Cannot process separated classifier data:
						|ID: %1
						|Version: %2
						|Data area: %3,
						|Detailed error information:
						|An exception is thrown after the ПриОбработкеОбластиДанных handler is finished:
						|%4'"),
					НастройкиКлассификатора.Идентификатор,
					НастройкиКлассификатора.Версия,
					ОбластьДанных,
			...
			Если ТранзакцияАктивна() Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'По завершении выполнения обработчика ПриЗагрузкеКлассификатора классификатора %1 не была закрыта транзакция.';
						|en = 'Transaction was not closed after the ПриЗагрузкеКлассификатора handler of the %1 classifier is finished.'"),
					Идентификатор);
				РаботаСКлассификаторами.ЗаписатьИнформациюВЖурналРегистрации(
					ТекстСообщения,
					Истина);
		...
		Исключение
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать поставляемые данные классификатора:
					...
					|Data area: %3,
					|Detailed error information:
					|An exception is thrown after the ПриЗагрузкеКлассификатора handler is finished:
					|%4'"),
				Идентификатор,
				Версия,
				ОбластьДанных,
...
Функция ИнформацияОбАктуальныхВерсияхКлассификаторов(Идентификаторы) Экспорт
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Начало получения информации об актуальных версиях классификаторов: %1';
			|en = 'Start receiving information on relevant classifier versions: %1'"),
		СтрСоединить(Идентификаторы, ","));
	...
	Если ДанныеФайла = Неопределено Тогда
		...
		РезультатОперации.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при получении файла классификатора %1: 
				...
				|en = 'An error occurred while receiving the%1 classifier file:
				|Classifier file was not found in supplied data.'"),
			Идентификатор);
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить файл классификатора %1.
				...
				|Причина:
				|Файл классификатора не обнаружен в поставляемых данных.';
				|en = 'Cannot receive the %1 classifier file.
				|Service returned an error when importing the file: %2.
				|File ID: %3
				|Reason:
				|Classifier file was not found in supplied data.'"),
			Идентификатор,
