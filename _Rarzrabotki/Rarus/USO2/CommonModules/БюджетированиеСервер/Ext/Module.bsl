...
Функция ТекстПоляАналитикиСУсловиемПоКоличествуВидовАналитики(Параметры, НомерТекущейАналитики) Экспорт
	...
	Если ЕстьОшибкиВПараметрах Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Получение выражения значения аналитики бюджетирования';
...
Процедура УдалитьСохраненныеМакетыБюджетов(Отбор) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'УдалитьСохраненныеМакетыБюджетов: Неверные параметры вызова';
								|en = 'УдалитьСохраненныеМакетыБюджетов: Incorrect call parameters'");
...
Процедура ИнициализироватьКомпоновщикНастроекПравила(Правило, ПараметрыЗаполненияАналитик, НастройкаКомпоновки) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Выполнение инициализации компоновщика настроек правила';
				|en = 'Initializing rule setting composer'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
...
Процедура УстановитьНастройкиЗаполненияАналитикиАвтоматически(ПараметрыЗаполненияАналитик, ВыраженияЗаполненияАналитики = Неопределено) Экспорт
	...
	Если ЭтоАдресВременногоХранилища(ПараметрыЗаполненияАналитик.АдресСхемыКомпоновкиДанных) Тогда
		...
		ШаблонСообщения = НСтр("ru = 'В источнике есть несколько подходящих вариантов заполнения аналитики ""%1"". Требуется ручной выбор.';
								|en = 'There are several suitable options for populating the ""%1"" dimension in the source.  Select manually.'");
		ШаблонПоля = "НастройкиЗаполненияАналитики[%1].ПредставлениеВыраженияЗаполненияАналитики";
...
Процедура ЗначенияНефинансовогоПоказателяПоПравилу(Правило, Параметры, ТаблицаЗначений) Экспорт
	...
	Если СхемаКомпоновкиДанных = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'Не определена схема компоновки данных для нефинансового показателя ""%1""';
								|en = 'Data composition schema is not identified for the ""%1"" non-financial indicator'");
	...
	Если Не СхемаКомпоновкиДанных.НаборыДанных.Количество() Тогда
		ШаблонСообщения = НСтр("ru = 'Отсутствует набор данных в схеме заполнения для ""%1""';
								|en = 'No data set in the population scheme for ""%1""'");
	...
	Если ПоляНабора.Найти("ЗначениеПоказателя") = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле ""ЗначениеПоказателя""';
								|en = 'The ""ЗначениеПоказателя"" field is missing in the data composition schema of the ""%1"" non-financial indicator.'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
	...
	Если ПоляНабора.Найти("Период") = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле ""Период""';
								|en = 'The ""Period"" field is missing in the data composition schema of the ""%1"" non-financial indicator.'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
	...
	Если Правило.ЗаполнениеВалюты = "ОпределяетсяПоИсточникуДанных" И ПоляНабора.Найти("Валюта") = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле ""Валюта""';
								|en = 'The ""Currency"" field is missing in the data composition schema of the ""%1"" non-financial indicator.'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
...
Процедура ПоставитьДокументВОчередьПроведения(Документ) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Отложенное проведение экземпляров бюджета';
				|en = 'Deferred posting of budget entry documents'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ЗапуститьОтложенноеПроведениеФоновымЗаданием(Ссылка) Экспорт
	...
	НаименованиеЗадания = НСтр("ru = 'Выполняется отложенное проведение экземпляров бюджета';
								|en = 'Deferred posting of budget entry documents is in progress'");
		...
		Для каждого ЭлементОтбора Из ЭлементыОтбораОстатки Цикл
			Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
				...
				ГруппаИЛИ.Представление = НСтр("ru = 'Отбор по сценариям при получении факта применяется только для статей (включая влияющие на показатель статьи) и целевые показатели';
												|en = 'Filter by scenarios when receiving a fact is used only for items (including items that affect item indicator) and target indicators'");
