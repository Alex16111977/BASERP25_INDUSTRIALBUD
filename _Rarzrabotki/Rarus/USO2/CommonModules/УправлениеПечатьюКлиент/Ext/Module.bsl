...
#КонецЕсли
	...
	Если ПечатныеФормы.Отказ Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';
														|en = 'You have no rights to send print form on printer. Contact your administrator.'"));
		Возврат;
	КонецЕсли;
...
Функция ОбластьМакета(Знач СсылкаНаМакет, Знач ОписаниеОбласти) Экспорт
	...
	ИначеЕсли СсылкаНаМакет.Тип = "ODT" Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Тип области не указан или указан некорректно: %1.';
																							|en = 'Area type is not specified or specified incorrectly: %1.'"), ОписаниеОбласти.ИмяОбласти);
		КонецЕсли;
