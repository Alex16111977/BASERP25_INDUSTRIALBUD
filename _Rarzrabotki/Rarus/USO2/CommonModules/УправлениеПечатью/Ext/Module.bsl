...
Процедура ЗаполнитьПрофильРедактированиеПечатныхФорм(ОписанияПрофилей) Экспорт
	...
	ОписаниеПрофиля.Наименование = НСтр("ru = 'Редактирование, отправка по почте, сохранение в файл печатных форм (дополнительно)';
										|en = 'Edit, send by email, save print forms to file (additionally)'",
	...
	ОписаниеПрофиля.Описание = НСтр("ru = 'Дополнительно назначается пользователям, которым должна быть доступна возможность редактирования,
		...
		|before printing, send by email, and save to file of generated print forms.'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция МакетПечатнойФормы(ПутьКМакету) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Макет ""%1"" не существует. Операция прервана.';
			|en = 'Template ""%1"" does not exist. Operation aborted.'"), ПутьКМакету);
...
Процедура ОтключитьПользовательскийМакет(ПутьКМакету) Экспорт
	...
	Если ЧастиСтроки.Количество() <> 2 И ЧастиСтроки.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Макет ""%1"" не найден.';
																						|en = 'The ""%1"" template not found.'"), ПутьКМакету);
	КонецЕсли;
...
Функция ОбластьМакета(СсылкаНаМакет, ОписаниеОбласти) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Тип области не указан или указан некорректно: %1.';
																						|en = 'Area type is not specified or specified incorrectly: %1.'"), ОписаниеОбласти.ТипОбласти);
	КонецЕсли;
...
Процедура ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, Знач ПереходНаСледующуюСтроку = Ложь) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = СообщениеОбОшибке + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при попытке вывести область ""%1"" из макета.';
																													|en = 'An error occurred when attempting to get area ""%1"" from the template.'"),
	...
	Если ВнешняяОбработкаОбъект = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Внешняя обработка ""%1"" (тип ""%2"") не обслуживается.';
				|en = 'The ""%1"" external data processor (the ""%2"" type) is not supported.'"),
		...
		Если ПечатнаяФорма.ТабличныйДокумент = Неопределено Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В обработчике печати не был сформирован табличный документ для: %1';
					|en = 'Spreadsheet document for %1 was not generated in the print processor'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Определяет, какие поставляемые макеты печатных форм изменились по отношению к предыдущей версии конфигурации.';
									|en = 'Defines which supplied templates of print forms were changed in relation to the previous configuration version.'");
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Для Каждого ОписаниеПечатнойФормы Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не задано количество экземпляров для печатной формы ""%1"".';
																			|en = 'Number of copies is not specified for print form ""%1"".'"),
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				...
				|en = 'An error occurred when generating print form ""%1"". Contact your administrator.'"), ИмяМакета);
			ВызватьИсключение ТекстСообщенияОбОшибке;
		КонецЕсли;
...
Функция КомпонентаФормированияQRКода()
	...
	ТекстОшибки = НСтр("ru = 'Не удалось подключить внешнюю компоненту для генерации QR-кода. Подробности в журнале регистрации.';
						|en = 'Failed to connect the external component to generate a QR-code. See event log for details.'");
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Печать %1 не выполнена: выбранная печатная форма недоступна.';
																					|en = '%1 was not printed. The selected print form is not available.'"), Объект);
...
Процедура ПроверитьРазметкуТабличногоДокументаПоОбъектамПечати(ТабличныйДокумент, ОбъектыПечати, Знач МенеджерПечати, Знач Идентификатор)
	...
	ТекстОшибкиРазметки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		...
		|en = 'No ""%1"" spreadsheet document markup by print objects.
		|Use the УправлениеПечатью.ЗадатьОбластьПечатиДокумента() procedure
		|when generating a spreadsheet document'"), Идентификатор);
...
Функция ТекстТипОбластиУказанНекорректно()
	Возврат НСтр("ru = 'Тип области не указан или указан некорректно.';
				|en = 'Area type is not specified or specified incorrectly.'");
КонецФункции
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		Если ДокументСодержитНомер Тогда
			...
			Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] № [Номер] от [Дата]';
							|en = '[НазваниеПечатнойФормы] No. [Номер] from [Дата]'");
		Иначе
			СписокРеквизитов = "Дата";
			Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] от [Дата]';
							|en = '[НазваниеПечатнойФормы] dated [Дата]'");
		КонецЕсли;
