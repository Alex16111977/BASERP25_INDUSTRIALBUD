	...
	Если ЗначениеЗаполнено(КодСтроки) Тогда
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru = 'Дублируется значение реквизита ""КодСтроки"" строки %1 и строки %2 заказа.
				...
				|en = 'The ""КодСтроки"" attribute value of line %1 and line %2 of the order has a duplicate.
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Не удалось заблокировать объект при автоматической корректировке строк мерных товаров.';
					|en = 'Cannot lock the object on automatic adjustment of measuring goods lines.'", КодОсновногоЯзыка),
		...
		Если ЗаказИзменен Тогда
			Попытка
				...
				ТекстСообщения = НСтр("ru = 'Выполнена корректировка строк мерных товаров по заказу %Заказ%';
										|en = 'Adjustment of measuring goods lines against the %Заказ% order is executed'", КодОсновногоЯзыка);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Окончание корректировки строк мерных товаров.';
						|en = 'End of measuring goods lines adjustment.'", КодОсновногоЯзыка),
					УровеньИнформация,
					,
					Выборка.Заказ,
					ТекстСообщения);
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось выполнить корректировку строк
				                            ...
				                            |against the %Заказ% order due to: %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Фоновая корректировка строк заказов мерных товаров';
												|en = 'Background adjustment of lines of measuring goods orders'", КодОсновногоЯзыка),
		...
		Иначе
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Не удалось удалить %Заказ% из очереди на корректировку строк
				                            ...
				                            |en = 'Cannot delete %Заказ% from measuring goods line adjustment
				                            |queue due to: %Причина%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Заказ%", Выборка.Заказ);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
