...
Функция ТабличныйДокументИзПрисоединенногоФайла(ПрисоединенныйФайл) Экспорт
	...
	Если Не ВнедренаБСП() Тогда
		ВызватьИсключение НСтр("ru = 'Метод поддерживается только в конфигурациях, в которых внедрена БСП.';
								|en = 'Method is supported only in configurations with implemented SL.'");
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.СсылкаСуществует(ПрисоединенныйФайл) Тогда
		ВызватьИсключение НСтр("ru = 'Присоединенный файл не существует.';
								|en = 'Attached file does not exist.'");
	КонецЕсли;
	...
	Если ТабличныйДокумент = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Присоединенный файл не содержит табличный документ.';
								|en = 'The attached file does not contain a spreadsheet document.'");
	КонецЕсли;
	...
	Для Каждого Проверка Из Проверки Цикл
		...
		Если Модуль = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Не найден обработчик подсистемы проверки и корректировки данных.';
									|en = 'Data processor of data check and correction subsystem is not found.'");
		КонецЕсли;
...
Функция ПроверитьДанныеОбщая(Проверки, Исправлять, СохранятьРезультат, Адрес = Неопределено)
	...
	Комментарий = СтрШаблон(НСтр("ru = 'Количество проверок: %1.';
								|en = 'Number of checks: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), КоличествоПроверок);
	...
	Если Не ТребуетсяЗаполнитьНастройки(Проверки, Исправлять, Обработчики) Тогда
		...
		Для Каждого Проверка Из Проверки Цикл
			...
			Комментарий = НСтр("ru = 'Начало выполнения проверки.';
								|en = 'Check start.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			Комментарий = СтрШаблон(НСтр("ru = 'Результат проверки: %1.';
										|en = 'Check result: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), РезультатПроверки.ПредставлениеРезультата);
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , , Комментарий);
	...
	Если Результат.Выполнено Тогда
		Комментарий = НСтр("ru = 'Проверки выполнены.';
							|en = 'Checks executed.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Комментарий = НСтр("ru = 'Требуется заполнение настроек';
							|en = 'It is required to fill in settings'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ШаблонОшибкиВыполненияПроверки()
	...
	Возврат НСтр("ru = 'Не удалось выполнить проверку по причине: %1';
				|en = 'Cannot check due to: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЗапускЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запуск';
				|en = 'Check and correct data.Start '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЗавершениеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Завершение';
				|en = 'Check and correct data. Complete'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЗаписьИсторииЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запись истории';
				|en = 'Check and correct data.Write history '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеПроверкиЖурналаРегистрации(Проверка)
	...
	Возврат СтрШаблон(НСтр("ru = 'Проверка и корректировка данных.%1';
							|en = 'Check and correct data.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Проверка);
