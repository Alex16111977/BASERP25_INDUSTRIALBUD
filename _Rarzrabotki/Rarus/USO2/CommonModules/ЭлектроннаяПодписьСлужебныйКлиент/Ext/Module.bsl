...
Процедура НайтиУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверкиНаКлиенте, НСтр("ru = 'Не указан путь к программе.';
																			|en = 'Path to the application is not specified.'"));
	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент:
			           ...
			           |en = 'An error occurred when calling procedure ОткрытьСертификат of common module ЭлектроннаяПодписьКлиент:
			           |Incorrect value of parameter ДанныеСертификата ""%1"".'"),
...
Процедура СохранитьЗапросНаСертификат(Оповещение, ЗапросНаСертификат, ИмяФайлаБезРасширения = "") Экспорт
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru = 'Выберите файл для сохранения запроса на сертификат';
												|en = 'Select a file to save the certificate request to'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru = 'Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*';
												|en = 'Certificate files (*.p10)|*.p10|All files (*.*)|*.*'");
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Оповещение) Экспорт
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Запрос на сертификат сохранен в файл:';
											|en = 'Certificate request is saved to file:'"),,
			ПолученныеФайлы[0].Имя);
	КонецЕсли;
...
Процедура СохранитьПодпись(АдресПодписи) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*';
			|en = 'Digital signature files (*.%1)|*.%1|All files (*.*)|*.* '"),
...
Процедура ПолучитьСертификатПоОтпечаткуЗавершение(Сертификат, ПредставлениеОшибки, Контекст)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Сертификат не найден на компьютере.';
							|en = 'Certificate is not found on the computer.'");
	КонецЕсли;
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ru = 'Сертификат не найден в данных подписи.';
				|en = 'Certificate is not found in signature data.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("ru = '(на сервере)';
																		|en = '(on server)'") + Символы.ПС
	...
	ИначеЕсли Операция <> "" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в функции МенеджерКриптографии.
			           ...
			           |en = 'An error occurred in function МенеджерКриптографии.
			           |Incorrect value of parameter Operation ""%1"".'"), Операция);
	Иначе
	...
	Иначе
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Ни одна из программ не доступна или не установлена на компьютере.';
								|en = 'None of the applications are available or installed on computer.'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайла(ПараметрыСоздания) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';
											|en = 'Certificate X.509 (*.cer;*.crt)|*.cer;*.crt|All files(*.*)|*.*'");
	...
	Если ТипЗнч(ОшибкаНаКлиенте) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru = 'Для процедуры ПоказатьОшибкуОбращенияКПрограмме
			           ...
			           |en = 'Incorrect type of the ОшибкаНаКлиенте parameter is specified for the
			           |ПоказатьОшибкуОбращенияКПрограмме procedure.'");
	КонецЕсли;
	...
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru = 'Для процедуры ПоказатьОшибкуОбращенияКПрограмме
			           ...
			           |en = 'Incorrect type of the ОшибкаНаСервере parameter is specified for the
			           |ПоказатьОшибкуОбращенияКПрограмме procedure.'");
	КонецЕсли;
		...
		Если ЭлектроннаяПодписьКлиент.СоздаватьЭлектронныеПодписиНаСервере() Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru = 'Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';
								|en = 'The following trusted certificates were not found either on computer, or on server:'");
			Иначе
				Заголовок = НСтр("ru = 'Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';
								|en = 'None of the following trusted certificates was found either on the computer, or on the server:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru = 'Следующие разрешенные сертификаты не найдены на компьютере:';
								|en = 'The following trusted certificates were not found on computer: '");
			Иначе
				Заголовок = НСтр("ru = 'Ни один из следующих разрешенных сертификатов не найден на компьютере:';
								|en = 'None of the following trusted certificates was found on the computer:'");
			КонецЕсли;
		КонецЕсли;
			...
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					...
					+ Символы.ПС + НСтр("ru = 'Внешняя компонента XMLDSig может использоваться только для подписания.';
										|en = 'External component XMLDSig can be used only for signing.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи подписи возникла ошибка:';
				|en = 'An error occurred when writing the signature: '") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи зашифрованных данных возникла ошибка:';
				|en = 'An error occurred when writing encrypted data:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи расшифрованных данных возникла ошибка:';
				|en = 'An error occurred when writing decrypted data:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура НачатьПроверкуПодписиПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	ПараметрыПодключения.ТекстПояснения = НСтр("ru = 'Для подписания XML необходима установка компоненты XMLDSig';
												|en = 'To sign XML, install the XMLDSig component'");
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ЗавершитьОперациюСОшибкой(
				...
				НСтр("ru = 'Операция невозможна. Требуется установка компоненты для HTTP-запросов.';
					|en = 'This operation is not possible. Component for making HTTP requests is required.'"));
		...
		ЗавершитьОперациюСОшибкой(Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось определить тип криптопровайдера %1';
					|en = 'Cannot define a type of cryptographic service provider %1'"),
	...
	Иначе
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Не установлен режим работы компоненты XMLDSig.';
				|en = 'The XMLDSig component mode is not set.'"));
	КонецЕсли;
...
Процедура НачатьПодписаниеСообщенияSOAP(Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru = 'Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';
				|en = 'An error occurred when calling method GetSignOIDFromCert of component XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеВыполненияGetSignOIDFromCert(OIDАлгоритмаПодписи, Параметры, Контекст) Экспорт
	...
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода GetSignOIDFromCert произошла ошибка:';
										|en = 'An error occurred while executing GetSignOIDFromCert method:'"), Контекст);
		Возврат;
	КонецЕсли;
		...
		ЗавершитьОперациюСОшибкой(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';
					|en = 'Certificate signature algorithm of cryptography differs from algorithm %1.'"),
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				...
				НСтр("ru = 'Ошибка вызова метода C14N компоненты XMLDSig.';
					|en = 'An error occurred when calling method C14N of component XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияC14N_ПодписываемыйТег(КаноникализированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КаноникализированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода C14N произошла ошибка:';
										|en = 'An error occurred while executing C14N method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Ошибка вызова метода Hash компоненты XMLDSig.';
				|en = 'An error occurred when calling method Hash of component XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияHash_ПодписываемыйТег(DigestValue, Параметры, Контекст) Экспорт
	...
	Если DigestValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода Hash произошла ошибка:';
										|en = 'An error occurred while executing Hash method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Ошибка вызова метода Sign компоненты XMLDSig.';
				|en = 'An error occurred when calling method Sign of component XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияSign(SignatureValue, Параметры, Контекст) Экспорт
	...
	Если SignatureValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода Sign произошла ошибка:';
										|en = 'An error occurred while executing Sign method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Ошибка вызова метода VerifySign компоненты XMLDSig.';
				|en = 'An error occurred when calling method VerifySign of component XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода VerifySign произошла ошибка:';
										|en = 'An error occurred while executing VerifySign method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Подпись не верна (SignatureValue некорректно.';
				|en = 'Signature is invalid (SignatureValue incorrect.'"));
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Подпись не верна (SignatureValue корректно, отличается DigestValue).';
				|en = 'Signature is invalid (SignatureValue correct, DigestValue different).'"));
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Ошибка вызова метода GetLastError компоненты XMLDSig.';
				|en = 'An error occurred when calling method GetLastError of component XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
