	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Ошибка получения пользователя из справочника Пользователи по его имени (%ИмяПользователяИБ%) в ИБ (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) по причине:
				|%ИнформацияОбОшибке%';
				|en = 'An error occurred while receiving a user from the Users catalog by its name (%ИмяПользователяИБ%) in IB (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) due to:
				|%ИнформацияОбОшибке%'");
		...
		Если Добавить = Истина Тогда
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru = 'Ошибка поиска пользователя (%ПолныеПрава%) по уникальному идентификатору:
					...
					|Причина:
					|%ИнформацияОбОшибке%';
					|en = 'An error occurred when searching for a user (%ПолныеПрава%) by a UUID:
					|Short username: %КраткоеИмяПользователя%
					|Full username:%ПолноеИмяПользователя%
					|UUID: %УникальныйИдентификатор%
					|Reason:
					|%ИнформацияОбОшибке%'")
					+ Символы.ПС;
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПолныеПрава%", ?(ПолныеПрава = Истина, НСтр("ru = 'доступны полные права';
