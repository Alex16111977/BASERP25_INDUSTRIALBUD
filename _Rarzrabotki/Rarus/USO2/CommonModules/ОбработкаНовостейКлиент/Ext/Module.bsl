				...
				Исключение
					...
					ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Клиент';
							|en = 'ISL:News.Client'"), // ИмяСобытия
						НСтр("ru = 'Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки';
							|en = 'News. Client. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки'"), // ИдентификаторШага
					...
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'ДанныеСобытия.Anchor.nameProp = %1';
								|en = 'ДанныеСобытия.Anchor.nameProp = %1'"),
							ТолькоСсылка));
				Исключение
					ТолькоСсылка = "";
					...
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Ошибка получения ДанныеСобытия.Anchor.nameProp по причине:
								|%1';
								|en = 'An error occurred when receiving EventData.Anchor.nameProp due to:
								|%1'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
				КонецПопытки;
			КонецЕсли;
				...
				ИнформацияПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'СтрокаСоединенияИнформационнойБазы() = %1';
							|en = 'СтрокаСоединенияИнформационнойБазы() = %1'"),
						СтрокаСоединенияСИБ));
						...
						ИнформацияПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Веб-адрес базы в СтрокаСоединенияИнформационнойБазы() = %1';
									|en = 'Web address of infobase in СтрокаСоединенияИнформационнойБазы() = %1'"),
							...
							ИнформацияПереходаПоСсылке.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Веб-адрес базы в Ссылке = %1';
										|en = 'Web address of infobase in Link = %1'"),
									ВебАдресБазыВСсылке));
			...
			ИнформацияПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Только ссылка без адреса базы = %1';
						|en = 'Only link without infobase address = %1'"),
					ТолькоСсылка));
					...
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Попытка поиска элемента с идентификатором %1';
								|en = 'Attempt to search the item with %1 ID'"),
					...
					ОшибкиПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Ошибка получения элемента якоря с идентификатором %1 по причине:
								|%2';
								|en = 'An error occurred when receiving the anchor item with %1 ID due to:
								|%2'"),
							ИдентификаторЭлемента,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
				КонецПопытки;
						...
						ИнформацияПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Попытка перехода к элементу с идентификатором %1';
									|en = 'Attempt to go to the item with %1 ID'"),
								ИдентификаторЭлемента));
						...
						ОшибкиПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка перехода к якорю с идентификатором %1 по причине:
									|%2';
									|en = 'An error occurred when going to the anchor with %1 ID due to:
									|%2'"),
								ИдентификаторЭлемента,
								ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
					КонецПопытки;
				...
				Если ОшибкиПереходаПоСсылке.Количество() > 0 Тогда
					...
					ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						...
						НСтр("ru = 'Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке';
							|en = 'News. Client. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'"), // ИдентификаторШага
						"Ошибка", // УровеньЖурналаРегистрации строкой
						"", // ОбъектМетаданных строкой
