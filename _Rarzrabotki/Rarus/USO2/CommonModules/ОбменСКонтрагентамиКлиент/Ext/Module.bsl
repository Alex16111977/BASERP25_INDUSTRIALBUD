	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если РезультатПроверки = Неопределено Тогда
			...
			Если Форма.Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
				...
				|Записать и провести?';
				|en = 'write and post.
				|Write and post?'"), НСтр("ru = 'записать.
				|Записать?';
				|en = 'write.
				|Write?'"));
				
				ШаблонСообщения = НСтр("ru = 'Документ изменен. Для формирования электронного документа его необходимо %1';
										|en = 'The document was changed. To generate an electronic document, you should %1 it'");
...
Процедура ЗакрытьПринудительноЭДО(МассивВладельцевЭД) Экспорт
	...
	ПричинаЗакрытия = "";
	ПоказатьВводСтроки(Обработчик, ПричинаЗакрытия, НСтр("ru = 'Укажите причину закрытия документооборота';
														|en = 'Specify the reason for closing EDI'"),,Истина);
	...
	Если Форма.Модифицированность Тогда
		...
		Если НРег(Реквизит.ТекстСостоянияЭДО) = НРег("ЭДОНеНачат") Тогда
			ТекстВопроса = НСтр("ru = 'Для того чтобы начать ЭДО с контрагентом, нужно записать документ. Записать?';
								|en = 'To start EDI with a counterparty, write the document. Write?'");
		...
		ИначеЕсли НРег(Реквизит.ТекстСостоянияЭДО) = НРег("ПригласитьКонтрагента") Тогда
			ТекстВопроса = НСтр("ru = 'Для того чтобы пригласить контрагента к обмену, нужно записать документ. Записать?';
								|en = 'To invite a counterparty for exchange, write the document. Write?'");
...
Процедура ЗакрытьПринудительноРезультатВводаСтроки(ПричинаЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ТекстОповещения = НСтр("ru = 'Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)';
							|en = 'ED document states are changed to ""Closed forcefully"": (%1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", КоличествоОбработанныхЭД);
	ПоказатьОповещениеПользователя(НСтр("ru = 'Обработка документов';
										|en = 'Document processing'"), , ТекстОповещения);
...
Процедура ОбработкаНавигационнойСсылкиВФормеОбъектаИБ(СсылкаНаОбъект)
	...
	Если Не ОбменСКонтрагентамиСлужебныйВызовСервера.ЕстьПравоНастройкиОбмена() Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для настройки ЭДО с контрагентом.
									...
									|en = 'Insufficient rights to set up EDI with counterparty.
