		...
		Если ЗначениеЗаполнено(Запись.УзелОбмена) Тогда
			...
			Исключение
				ШаблонСообщения = НСтр("ru = 'Отмена миграции на сервисе завершилась с ошибкой, обратитесь к администратору сервиса. 
		                                |Описание ошибки:
		                                |%1';
		                                |en = 'Migration cancellation failed on service. Contact service administrator.
		                                |Error description:
		                                |%1'");
...
Процедура ЗагрузитьСообщение(Источник) Экспорт
	...
	Если Выборка.Следующий() И Выборка.ОчередьЗагрузки >= МаксимальнаяОчередьЗагрузки() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Очередь загрузки больше %1!';
										|en = 'Download queue is more than %1.'"), МаксимальнаяОчередьЗагрузки());
	КонецЕсли;
	...
	Исключение
		...
		РегистрацияОшибки(НСтр("ru = 'Загрузка';
								|en = 'Load'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если Не СостояниеВыгрузки.Выбран() Тогда
		...
		РегистрацияОшибки(НСтр("ru = 'Выгрузка';
								|en = 'Export'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПредставлениеОшибки);
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийВыгрузка);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыДоступа = Неопределено Тогда
		ПредставлениеОшибки = НСтр("ru = 'Не найдены параметры подключения.';
									|en = 'Connection parameters are not found.'");
		ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПредставлениеОшибки, ПредставлениеОшибки);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Шаблон = НСтр("ru = 'Не удалось получить состояние загрузки, возможно сервис временно недоступен: %1 %1';
						|en = 'Cannot get import status, the service might be temporarily unavailable: %1 %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Комментарий = СтрШаблон(Шаблон, Символы.ПС, ПредставлениеОшибки);
		РегистрацияПримечания(НСтр("ru = 'Выгрузка';
									|en = 'Export'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
	...
	Исключение
		...
		Иначе
			...
			РегистрацияОшибки(НСтр("ru = 'Выгрузка';
									|en = 'Export'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		...
		Если ЗавершитьМиграциюАвтоматически Тогда
			...
			Исключение
				Комментарий = НСтр("ru = 'Не удалось установить монопольный режим.';
									|en = 'Cannot set the exclusive mode.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				РегистрацияПримечания(НСтр("ru = 'Выгрузка';
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Если Не СостояниеЗагрузки.Выбран() Тогда
		Комментарий = НСтр("ru = 'Состояние загрузки не найдено. Загрузка прервана';
							|en = 'Import status not found. Import aborted'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		РегистрацияОшибки(НСтр("ru = 'Загрузка';
								|en = 'Load'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
		// Самоудаление.
		УдалитьОчередь();
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийЗагрузка);
		Возврат;
	...
	Исключение
		...
		Иначе 
			РегистрацияОшибки(НСтр("ru = 'Загрузка';
									|en = 'Load'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ЗагрузитьДанныеИзКаталога(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки) Экспорт
	...
	Если Манифест.Свойство("ОбъектыВыгруженныеПолностью", ОбъектыВыгруженныеПолностью) Тогда
		Шаблон = НСтр("ru = 'Не удалось выгрузить порциями объекты: %1.';
						|en = 'Cannot export objects in portions: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция АбонентыПользователя(ПараметрыДоступа) Экспорт
	...
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неверный формат ответа внешнего программного интерфейса: ""%1""';
										|en = 'Incorrect response format of the external application interface: ""%1""'"), "ext/account/list");
	КонецПопытки;
...
Функция СервисПоддерживаетМиграцию(ИмяСервера, АдресПрограммногоИнтерфейса, АдресРегистрации, АдресВосстановления, РегистрацияРазрешена) Экспорт
	...
	Исключение
		Шаблон = НСтр("ru = 'Сервис временно недоступен или не поддерживает миграцию,
                       |описание ошибки:
                       |%1';
                       |en = 'The service is temporarily unavailable or does not support migration,
                       |error description:
                       |%1'");
...
Функция Зарегистрироваться(ИмяСервера, Наименование, Логин, ЭлектроннаяПочта, Пароль, Телефон) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось выполнить запрос, код ответа: %1';
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			...
			РегистрацияОшибки(НСтр("ru = 'Загрузка сообщения';
									|en = 'Message import'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
...
Функция ВыгрузитьПользовательскиеДанные(УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ПараметрыВыгрузки)
	...
	ШаблонНачалоВыгрузки = НСтр("ru = 'Выгрузка объекта: %1';
								|en = 'Export object: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ШаблонОкончаниеВыгрузки = НСтр("ru = 'Выгружен объект: %1, Количество: %2, Длительность (сек): %3';
									|en = 'Exported object: %1, Quantity: %2, Duration (sec): %3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВсегоОбъектов = 0;
...
Функция ЗагрузитьПользовательскиеДанные(ПараметрыЗагрузки)
	...
	ШаблонНачалоЗагрузки = НСтр("ru = 'Загружается объект: %1';
								|en = 'Object is being imported: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ШаблонОкончаниеЗагрузки = НСтр("ru = 'Загружен объект: %1, Количество: %2, Длительность (сек): %3';
									|en = 'Imported object: %1, Quantity: %2, Duration (sec): %3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	ИначеЕсли ПараметрыВыгрузки.ЗапрещенныеТипыОбщихДанных[ТипЗначения] <> Неопределено Тогда 
		...
		Если ЗначениеЗаполнено(Значение) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'В объекте обнаружена недопустимая ссылка. Тип данных: %1';
											|en = 'Invalid link is found in the object. Data type: %1'"), Значение.Метаданные().ПолноеИмя());
		КонецЕсли;
...
Функция ТипПоляПоиска(ОбъектМетаданных, ИмяПоля)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
		...
		|en = 'Field %1 cannot be used as a natural key field of object %2:
		|object field was not found.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяПоля,
		ОбъектМетаданных.ПолноеИмя());
...
Процедура ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru = 'Выгрузка';
							|en = 'Export'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru = 'Загрузка';
							|en = 'Load'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура РегистрацияОшибки(Событие, Комментарий)
	...
	ИмяСобытия = СтрШаблон(НСтр("ru = 'Миграция приложений.%1';
								|en = 'Application migration.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Событие);
