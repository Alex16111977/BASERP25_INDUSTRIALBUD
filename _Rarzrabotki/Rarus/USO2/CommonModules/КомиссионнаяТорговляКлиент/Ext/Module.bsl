...
Процедура ОповеститьОбОкончанииРасчетаВознаграждения(СпособРасчетаВознаграждения) Экспорт
	...
	ИначеЕсли не ЗначениеЗаполнено(СпособРасчетаВознаграждения) Тогда
		...
		НСтр("ru = 'Способ расчета не заполнен.';
			|en = 'Calculation method is filled in.'"),
		,
		...
		Если ДополнительныеПараметры.Форма.Объект.ПроцентВознаграждения = 0 Тогда
			ПоказатьВводЗначения(
				...
				НСтр("ru = 'Введите процент вознаграждения';
					|en = 'Enter percentage of remuneration'"),
				ОписаниеТипов); 
	...
	Если Форма.Объект.Товары.Итог("СуммаВознаграждения") <> 0 Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Комиссионное вознаграждение будет очищено, продолжить?';
				|en = 'Commission will be cleared, continue?'"),
			Кнопки,,
			Истина);
