...
Процедура СинхронизироватьСБанком(Оповещение, НастройкаОбмена, ВыводитьОкноОжидания = Ложь) Экспорт
	...
	Если РеквизитыНастройкиОбмена.Недействительна Тогда
		ТекстСообщения = НСтр("ru = 'Настройка обмена недействительна.
									|Операция прервана.';
									|en = 'Exchange setting is invalid.
...
Процедура ПослеПолученияСпискаБизнесСистемСТокенаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова <> 0 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Внешняя компонента VpnKey-TLS при получении списка бизнес систем вернула код ошибки %1';
							|en = 'External component VpnKey-TLS returned an error code %1 when receiving a business system list'");
...
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//
Процедура ПолучитьДокументыПоТикетамСбербанк(Оповещение, НастройкаОбмена) Экспорт
	
	ИдентификаторОрганизации = Неопределено;
	МассивТикетов = ОбменСБанкамиСлужебныйВызовСервера.ТикетыЗапросовСтатусовСбербанк(
...
// Отправляет сообщения обмена в Сбербанк. Сессия уже должна быть установлена.
//
// Параметры:
//  Оповещение - ОписаниеОповещения - Оповещение, вызываемое после выполнения процедуры:
//      * Результат - Структура - результат выполнения процедуры:
//           * Успех - Булево - если Истина, то все электронные документы были отправлены;
//           * ОтправленныеСообщенияОбмена - Массив - содержит ДокументСсылка.СообщениеОбменСБанками, отправленные сообщения обмена.
//      * ДополнительныеПараметры - Структура - контекст выполнения процедуры;
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//  МассивСообщенийОбмена - Массив - содержит ДокументСсылка.СообщениеОбменСБанками, отправляемые в банк сообщения.
//
Процедура ОтправитьСообщенияОбменаВСбербанк(Оповещение, НастройкаОбмена, МассивСообщенийОбмена) Экспорт
	
	МассивОтправки = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(МассивСообщенийОбмена);
	ДополнительныеПараметры = Новый Структура;
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'К компьютеру подключен не подходящий банковский ключ';
									|en = 'Unsuitable bank key is connected to the computer'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВнешнийПодключаемыйМодуль <> Неопределено Тогда
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'К компьютеру не подключен банковский ключ для отправки данных по настройке обмена: %1';
									|en = 'The bank key is not connected to the computer for sending data on the exchange setting: %1'");
...
Процедура ПодписаниеЭДЧерезДополнительнуюОбработку(АутентификацияВыполнена, Параметры) Экспорт
	...
	Если АутентификацияВыполнена = Истина Тогда
		...
		Если НЕ ПрерватьПодписание Тогда
			...
			Параметры.ИтогКоличествоПодписанных = Параметры.ИтогКоличествоПодписанных + МассивПодписей.Количество();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПодключитьКомпоненту(Оповещение, Идентификатор, ТекстПояснения = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекстПояснения) Тогда
		ШаблонПояснения = НСтр("ru = 'Для продолжения необходимо установить внешнюю компоненту %1.';
								|en = 'To continue, install the %1 external component.'");
...
Процедура ИнициализироватьВК(Оповещение, ПодключаемыйМодуль) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'При работе с внешним модулем произошла ошибка.
								...
								|Select another external module or contact technical support.'");
...
Процедура АутентификацияНаКлючеПослеПодключенияВК(РезультатВызова, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВызова = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Не удалось подключить внешний модуль';
								|en = 'Cannot connect the external module'");
...
Процедура СохранитьЖурналВФайлПослеПодключенияРасширения(Подключено, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Операция = НСтр("ru = 'Журналирование обмена с банком';
						|en = 'Bank exchange logging'");
		ТекстОшибки = НСтр("ru = 'Не удалось подключить расширение для работы с файлами.
								|Журнал обмена не выгружен в файл.';
								|en = 'Cannot connect the file operation extension.
								|The exchange journal was not exported to the file.'");
...
Процедура ОбработатьОшибкуСохраненияЖурналаВФайле(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ВидОперации = НСтр("ru = 'Сохранение журнала обмена в файле.';
						|en = 'Save exchange log to a file.'");
	ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
...
Процедура ПроверитьВерсиюВКСбербанк(Оповещение, ИмяВнешнегоМодуля)
	...
	Если НЕ ИнформацияОВК.Существует Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ИнформацияОВК.ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
...
Процедура ПодобратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ВыборкаСертификатов.Количество() Тогда
		ТекстСообщения = НСтр("ru = 'Не найден подходящий сертификат подписи.
									...
									|en = 'Suitable signature certificate not found.
									|Possible reasons:
...
Процедура ОтправитьПлатежныеПорученияПослеУстановкиКаналаСбербанк(Успех, Параметры) Экспорт
	...
	ОтправитьСообщенияОбменаВСбербанк(Оповещение, Параметры.НастройкаОбмена, Параметры.МассивСообщенийОбмена);
...
Процедура ПолучитьОчереднойДокументИзСбербанка(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.МассивТикетов.Количество() = 0 Тогда // Все документы получены
		...
		ОпределитьСтатусыПодписейСбербанк(
			Оповещение, ДополнительныеПараметры.НастройкаОбмена, ДополнительныеПараметры.МассивСообщенийОбменаДляПроверки);
		Возврат;
	КонецЕсли;
	...
	ПолучитьСтатусЗапросаСбербанк(
		Оповещение, ДополнительныеПараметры.ИдентификаторОрганизации, Тикет, ДополнительныеПараметры.НастройкаОбмена);
...
Процедура ОбработатьРезультатПолученияДокументаСбербанк(Результат, Параметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		Параметры.Вставить("ЕстьОшибка", Истина);
		ОбменСБанкамиСлужебныйВызовСервера.УдалитьИдентификаторЗапроса(НастройкаОбмена, Параметры.Тикет);
		Параметры.МассивТикетов.Очистить();
		ПолучитьОчереднойДокументИзСбербанка(Параметры);
		Возврат;
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Параметры.МассивСообщенийОбменаДляПроверки, МассивНовыхСообщенийОбмена);
...
Процедура ПодписатьОчереднойЭДСбербанк(Параметры)
	...
	РеквизитыСертификата = Новый Структура("ДанныеСертификата");
	ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовСертификата(
		Параметры.СертификатСсылка, РеквизитыСертификата);

	ПодписатьДанныеСбербанк(ОписаниеОповещения, Параметры.ИмяВнешнегоМодуля, СтрокаПодписиBase64,
		Параметры.ИдентификаторСертификата, ПодписываемоеСообщение, РеквизитыСертификата.ДанныеСертификата);
...
Процедура ПродолжитьПодписаниеЭДСбербанк(Результат, Параметры) Экспорт
	...
	Если Результат.Успех Тогда
		Параметры.ИтогКоличествоПодписанных = Параметры.ИтогКоличествоПодписанных + Результат.Количество;
...
Процедура ПослеУстановкиКаналаСбербанк(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВызова = 0 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Компонента AddIn.Bicrypt при установке виртуального канала вернула код ошибки';
							|en = 'The AddIn.Bicrypt component returned an error code during installation of the virtual channel'")
							+ " " + РезультатВызова;
		ОбработатьОшибку(Операция, ТекстОшибки, ТекстСообщения);
		ОчиститьДанныеАвторизацииСбербанк();
	КонецЕсли;
...
Процедура ОтправитьДокументыВСбербанкБазоваяАутентификация(Оповещение, ДанныеДляОтправки, ДополнительныеПараметры = Неопределено)
	...
	СтруктураВозврата = Новый Структура("КоличествоОтправленных, КоличествоПолученных");
...
Процедура ОтправитьОчередноеСообщениеОбменаВСбербанк(Параметры)
	
	Если Параметры.МассивСообщенийОбмена.Количество() = 0 Тогда // Все сообщения обмена были отправлены
		
		Результат = Новый Структура;
		Результат.Вставить("Успех", Истина);
		Результат.Вставить("ОтправленныеСообщенияОбмена", Параметры.ОтправленныеСообщенияОбмена);
		ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеОтправкиСообщений, Результат);
		Возврат;
	КонецЕсли;
	...
	Если ПакетXML = Неопределено Тогда
		ПослеОтправкиСообщенияОбменаВСбербанк(Неопределено, Параметры)
	Иначе
		Оповещение = Новый ОписаниеОповещения("ПослеОтправкиСообщенияОбменаВСбербанк", ЭтотОбъект, Параметры);
		ВыполнитьОтправкуДанныхЧерезТокенСбербанк(Оповещение, ПакетXML, Параметры.НастройкаОбмена);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПослеОтправкиСообщенияОбменаВСбербанк(Тикет, Параметры) Экспорт
	...
	ОтправитьОчередноеСообщениеОбменаВСбербанк(Параметры);
...
Процедура ПослеОтправкиПлатежныхПорученийВСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ОтправленныеСообщенияОбмена.Количество() Тогда
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПродолжитьОтправкуДокументовПослеПолученияСтатусовЗапросовСбербанк", ЭтотОбъект, ДополнительныеПараметры);
	
	ПолучитьДокументыПоТикетамСбербанк(Оповещение, ДополнительныеПараметры.НастройкаОбмена);
...
Процедура ОтправитьСообщениеВБанкПослеПолученияМаркера(Маркер, Параметры) Экспорт
	...
	СформироватьИОтправитьПакетыВБанк(ОписаниеОповещения, МассивСообщенийОбменаКОтправке, СоотвНастроекОбменаИСтруктурСертификатов);
	...
	Если НастройкиОбмена.Подписывать Тогда
		Если НЕ ЗначениеЗаполнено(НастройкиОбмена.СертификатОрганизацииДляПодписи)
			...
			ТекстСообщения = НСтр("ru = 'Не найден подходящий сертификат для подписи документа Запрос-зонд';
									|en = 'Failed to find a signature certificate for document Probe query '");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			СтруктураРезультата = Новый Структура("Успех", Ложь);
			ВыполнитьОбработкуОповещения(Параметры.ОбработчикПослеТестаНастройки, СтруктураРезультата);
			Возврат;
...
Процедура ПолучитьМаркерБанкаПоЛогинуИПаролю(Обработчик, АдресСервера, ИдентификаторОрганизации, ДанныеАутентификации, ВерсияAPI, НастройкаОбмена = Неопределено, ПробнаяОперация = Ложь) Экспорт
	...
	Если Результат.НеверныеДанныеАутентификации Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Неверный логин или пароль.';
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ТекстЗаголовка = НСтр("ru = '1С:ДиректБанк';
							|en = '1C:DirectBank'");
	ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, Параметры.ИтогКоличествоНовыхЭД, Параметры.ИтогКоличествоПодписанных,
		Параметры.КоличествоПодготовленных, Параметры.КоличествоОтправленных);
...
Процедура ПродолжитьПодписаниеБанковскихЭД(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("ВыбранныйСертификат", ВыбранныйСертификат)
		...
		ИначеЕсли Параметры.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезВК") Тогда
			...
			ДополнительныеПараметры.Вставить("ИтогКоличествоПодписанных", 0);
...
//       КоличествоПодготовленных - Число.
...
Процедура ПослеПолученияОтпечатковОбработатьЭД(ОтпечаткиСертификатов, Параметры) Экспорт
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
	Иначе
		ПослеОбработкиДокументов(Результат, Параметры)
	КонецЕсли;
...
Процедура ПодписатьЭД(РезультатВыполнения, Параметры) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		...
		Параметры.ИтогКоличествоПодписанных = Параметры.ИтогКоличествоПодписанных
			+ ЭлектронноеВзаимодействиеСлужебныйКлиент.КоличествоПодписанныхЭД(РезультатВыполнения);
			
		Если РезультатВыполнения.Свойство("ИтогКоличествоПодписанных") Тогда // подписание через внешний модуль
			Параметры.ИтогКоличествоПодписанных = Параметры.ИтогКоличествоПодписанных + РезультатВыполнения.ИтогКоличествоПодписанных;
		КонецЕсли;
	...
	#Если НЕ ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если Не ВнешняяОбработкаПодключена Тогда
			ТекстСообщения = НСтр("ru = 'Не удалось подключить дополнительную обработку.';
									|en = 'Cannot enable additional data processor.'");
...
//  КоличествоСформированных - Число - количество сформированных электронных документов;
//  КоличествоПодписанных - Число -  количество подписанных электронных документов;
//  КоличествоПодготовленных - Число -  количество подготовленных к отправке электронных документов;
//  КоличествоОтправленных - Число - количество отправленных электронных документов.
//
Процедура ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, КоличествоСформированных, КоличествоПодписанных, КоличествоПодготовленных, КоличествоОтправленных = 0)
	
	Если КоличествоПодготовленных + КоличествоОтправленных > 0 Тогда
		ДопТекст = ?(КоличествоОтправленных > 0, НСтр("ru = 'отправлено';
														|en = 'sent'"), НСтр("ru = 'подготовлено к отправке';
																				|en = 'prepared for dispatch'"));
		...
		Если КоличествоПодписанных > 0 Тогда
			Если КоличествоСформированных > 0 Тогда
				...
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Текст, КоличествоСформированных, КоличествоПодписанных, ДопТекст, Количество);
			Иначе
				Текст = НСтр("ru = 'Подписано: (%1), %2 пакетов: (%3)';
							|en = 'Signed: (%1), %2 packages: (%3)'");
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КоличествоПодписанных, ДопТекст, Количество);
			КонецЕсли;
		Иначе
	...
	Иначе
		Если КоличествоПодписанных > 0 Тогда
			Если КоличествоСформированных > 0 Тогда
				...
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КоличествоСформированных, КоличествоПодписанных);
			Иначе
				Текст = НСтр("ru = 'Подписано: (%1)';
							|en = 'Signed: (%1)'");
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КоличествоПодписанных);
			КонецЕсли;
		Иначе
			Если КоличествоСформированных > 0 Тогда
				Текст = НСтр("ru = 'Сформировано: (%1)';
							|en = 'Generated: (%1)'");
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КоличествоСформированных);
			Иначе
...
Процедура ПослеОбработкиДокументов(РезультатЗадания, Параметры) Экспорт
	...
	Иначе // выполнено
		...
		Если РезультатЗадания.Свойство("Сообщения") И РезультатЗадания.Сообщения <> Неопределено Тогда
		...
		Если Результат <> Неопределено Тогда
			...
			ИтогКоличествоНовыхЭД = 0; ИтогКоличествоПодписанных = 0; КоличествоПодготовленных = 0;
			...
			Если НЕ Результат.Свойство("КоличествоПодписанных", ИтогКоличествоПодписанных) Тогда
				ИтогКоличествоПодписанных = 0;
			КонецЕсли;
			
			Если НЕ Результат.Свойство("КоличествоПодготовленных", КоличествоПодготовленных) Тогда
				КоличествоПодготовленных = 0;
			КонецЕсли;
			
			Если НЕ Результат.Свойство("КоличествоОтправленных", КоличествоОтправленных) Тогда
				КоличествоОтправленных = 0;
			КонецЕсли;
			
			Если НЕ Результат.Свойство("КоличествоНовыхЭД", ИтогКоличествоНовыхЭД) Тогда
				ИтогКоличествоНовыхЭД = 0;
			КонецЕсли;
			
			Если ИтогКоличествоНовыхЭД > 0 Тогда
				Оповестить("ОбновитьСостояниеОбменСБанками");
			КонецЕсли;
			...
			Параметры.Вставить("КоличествоОтправленных", КоличествоОтправленных);
			Параметры.Вставить("КоличествоПодготовленных", КоличествоПодготовленных);
			Параметры.Вставить("ИтогКоличествоНовыхЭД", ИтогКоличествоНовыхЭД);
			Параметры.Вставить("ИтогКоличествоПодписанных", ИтогКоличествоПодписанных);
...
Процедура ПодписатьЗапросыВыписокПослеВводаПароляЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("Пароль", Результат.ПарольСертификата);
	ДополнительныеПараметры.Вставить("ИтогКоличествоПодписанных", 0);
	МассивСообщенийОбмена = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ДополнительныеПараметры.МассивСообщенийОбменаКОтправке);
	ДополнительныеПараметры.Вставить("МассивСообщенийОбмена", МассивСообщенийОбмена);
...
Процедура ПодписатьРекурсивноЭДПоСертификатуЧерезВК(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.МассивСообщенийОбмена.Количество() = 0 Тогда
		СтруктураВозврата = Новый Структура("ИтогКоличествоПодписанных", ДополнительныеПараметры.ИтогКоличествоПодписанных);
...
Процедура ПродолжитьРекурсивноеПодписаниеЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.ИтогКоличествоПодписанных = ДополнительныеПараметры.ИтогКоличествоПодписанных + 1;
...
Процедура ОбработатьОшибкуИнициализацииВК(ИнформацияОбОшибке, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ВидОперации = НСтр("ru = 'Инициализация внешней компоненты.';
						|en = 'External component initialization.'");
...
Процедура ПослеПодписанияЗапросовВыписокЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ИтогКоличествоПодписанных = 0 Тогда
		СтруктураВозврата = Новый Структура("Успех", Ложь);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПодготовкиЗапросовВыписки, СтруктураВозврата);
	Иначе
		СтруктураВозврата = Новый Структура;
