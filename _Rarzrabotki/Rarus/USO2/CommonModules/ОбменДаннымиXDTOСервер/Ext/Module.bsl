...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПКО с именем %1';
																						|en = 'Cash receipt statement with name %1 is not found'"), Имя);
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Направление: %1.
			...
			|%2.';
			|en = 'Direction: %1.
			|Handler: BeforeConversion.
			|
			|An error occurred when executing handler.
			|%2.'"),
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ВызватьИсключение НСтр("ru = 'При формировании сообщения обмена данными произошли ошибки. Подробнее см. в журнале регистрации.';
								|en = 'Errors occurred while generating a data exchange message. For more information, see the event log.'");
	КонецЕсли;
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неизвестный тип свойства <%1>. Тип объекта: %2';
					|en = 'Unknown property type <%1>. Object type: %2'"),
				Свойство.Имя,
				Строка(ТипXDTO));
		КонецЕсли;
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|
				|%3';
				|en = 'An error occurred when generating XDTO object: Property type <%1>. Property name: <%2>.
				|
				|%3'"),
				ТипСвойства,
				Свойство.Имя,
	...
	Если ПравилоКонвертации.ЭтоКонстанта Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|Тип источника: %1
				|Тип приемника: %2';
				|en = 'XML scheme error. One property shall be set for the target.
				|Source type: %1
				|Target type: %2'"),
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ru = 'Блокировка на обмен данными не может быть установлена в активной транзакции.';
									|en = 'Exchange data lock cannot be set in an active transaction.'");
		КонецЕсли;
			...
			Если СообщениеОтНеобновленнойНастройки(ЧтениеXML) Тогда
				СтрокаСообщенияОбОшибке = НСтр("ru = 'Получение данных от источника, в котором не выполнено
					...
					+ Символы.ПС + НСтр("ru = '1) Выполнить повторную синхронизацию данных через некоторое время.';
										|en = '1) Repeat data synchronization later.'")
					...
					+ Символы.ПС + НСтр("ru = '(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)';
										|en = '(1 - for transport kinds Via internet, 2- for transport kind Another)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
				...
				Если ФорматПланаОбмена <> ФорматОбмена.БазовыйФормат Тогда
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Формат сообщения обмена ""%1"" не соответствует формату плана обмена ""%2"".';
							|en = 'Exchange message format ""%1"" does not correspond to exchange plan format ""%2"".'"),
				...
				Если Не КомпонентыОбмена.ОбменДаннымиСВнешнейСистемой Тогда
					...
					Если ОтправительИзСообщения = Неопределено
						...
						СтрокаСообщения = НСтр("ru = 'Не найден узел обмена для загрузки данных. План обмена: %1, Идентификатор: %2';
												|en = 'Exchange node for data import is not found. Exchange plan: %1, ID: %2'");
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.';
				|en = 'Attempting to change shared data (%1: %2) in undivided mode.'"),
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не определен Менеджер конвертации для версии формата обмена <%1>.';
				|en = 'Conversion Manager for exchange format version <%1> is not defined.'"),
			ВерсияФормата);
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'An error occurred when converting the predefined data.
				|More than one conversion rule is specified for source type <%1> and target type <%2>.'"),
				Строка(Тип),
				Строка(ТипXDTO));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'An error occurred when converting the predefined data.
			|The conversion rule is not specified for source type <%1> and target type <%2>.'"),
			Строка(Тип),
			Строка(ТипXDTO));
...
Процедура ПроверитьОбъектXDTOПоСхеме(ОбъектXDTO, ТипXDTO, Контекст, Отказ, ОписаниеОшибки)
	...
	|en = 'Cannot execute conversion into an object of the ""%1"" format: 
	|%2
	|
		...
		Если Не ОбъектXDTO.Установлено(Свойство) Тогда
			Если Свойство.НижняяГраница = 1
				...
				СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + " - " + НСтр("ru = 'не заполнено обязательное поле.';
																							|en = 'required field is not filled in.'");
			...
			Если ТипЗнч(ЗначениеСвойстваXDTO) = Тип("ЗначениеXDTO") Тогда
				...
				Исключение
					...
					СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + " - " + НСтр("ru = 'значение поля не соответствует заданным ограничениям.';
																								|en = 'field value does not match specified restrictions.'");
					СтекОшибок.Добавить(СообщениеОбОшибке);
				КонецПопытки;
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка чтения объекта XDTO, имя свойства: <%1>.';
																					|en = 'An error occurred when reading XDTO object, property name: <%1>.'"), ИмяСвойства)
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка конвертации свойства объекта XDTO, имя свойства: <%1>.';
																					|en = 'An error occurred when converting the XDTO object property, property name: <%1>.'"), ПКС.СвойствоКонфигурации)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеОбъекты Цикл
		...
		Исключение
			ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Событие: %1.
					...
					|%3.';
					|en = 'Event: %1.
					|Handler: ОтложенноеЗаполнениеОбъектов.
					|Object: %2.
					|
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое имя формата обмена <%1>';
																						|en = 'Non-canonical name of the exchange format <%1>'"), ФорматОбмена);
	КонецЕсли;
		...
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(УдалитьНепосредственно,
				НСтр("ru = 'Попытка удаления неразделенных данных (%1: %2) в разделенном режиме.';
					|en = 'Attempting to delete shared data (%1: %2) in split mode.'"),
				НСтр("ru = 'Попытка пометки на удаление неразделенных данных (%1: %2) в разделенном режиме.';
					|en = 'Attempting to mark for deletion of shared data (%1: %2) in split mode.'")),
	...
	Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
		...
		ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
			НСтр("ru = 'Изменен номер версии формата обмена.';
				|en = 'Exchange format version number is changed.'"), , Ложь, , , Истина);
	КонецЕсли;
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД с именем %1';
																						|en = 'DER with name %1 is not found'"), Имя);
...
Процедура ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена, Знач ИмяОбработчика)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Событие: %1.
				...
				|%3.';
				|en = 'Event: %1.
				|Handler: ПриКонвертацииДанныхXDTO.
				|Object: %2.
				|
				|Error running handler.
				|%3.'"),
...
Процедура ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КомпонентыОбмена, ИмяОбработчика, КонвертацияСвойств)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Событие: %1.
				...
				|%3.';
				|en = 'Event: %1.
				|Handler: ПередЗаписьюПолученныхДанных.
				|Object: %2.
				|
				|Error running handler.
				|%3.'"),
		...
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 №%2 от %3';
																										|en = '%1 No%2 from %3'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, %2';
																										|en = '%1, %2'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.';
																						|en = 'Non-canonical presentation of the exchange format version: <%1>.'"), Версия);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неправильный формат параметра СтрокаВерсии1: %1';
																						|en = 'Invalid format for parameter СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неправильный формат параметра СтрокаВерсии2: %1';
																						|en = 'Invalid format for parameter СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
