...
Процедура УстановитьУсловноеОформлениеСчетаУчетаПриСовместномИспользованииСтатей(Форма, 
	...
	Если ТипыСтатей.Найти(1)  <> Неопределено Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<определяется для статьи расходов>';
																	|en = '<determined for expense item>'"));
...
Функция ЗадаватьВопросПоИзменениюДеятельностиПриобретениеУслугПрочихАктивов(Объект)
	...
	Результат.Вставить("ТекстВопроса", НСтр("ru = 'Оформляется поступление прочих активов, учитываемых на счете 08.04 (08.05). Установить закупку под деятельность ""Ввод в эксплуатацию ОС/НМА""?';
											|en = 'Receipt of other assets recorded on account 08.04 (08.05) is registered. Set purchase for the ""FA/IA commissioning"" activity?'"));
	...
	ШаблонСообщения = НСтр("ru = 'В строке ""%1"" выбран счет учета %2';
							|en = 'The %2 GL account is selected in line ""%1""'");
...
Функция ЗадаватьВопросПоИзменениюДеятельностиВнутреннееПотреблениеТоваров(Объект)
	...
	Результат.Вставить("ТекстВопроса", НСтр("ru = 'Оформляется списание на прочие активы, учитываемых на счете 08.04 (08.05). Установить потребление под деятельность ""Ввод в эксплуатацию ОС/НМА""?';
											|en = 'Write-off to other assets recorded on account 08.04 (08.05) is registered. Set consumption for the ""FA/IA commissioning"" activity?'"));
