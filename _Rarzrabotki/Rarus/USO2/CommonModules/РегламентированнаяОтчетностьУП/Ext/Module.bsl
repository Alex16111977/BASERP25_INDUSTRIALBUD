...
Процедура ЗаполнитьОтчетАвансыИмуществоФормаОтчета2019Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПервоначальныеСведенияОС.ОсновноеСредство КАК ОсновноеСредство,
	|	ПервоначальныеСведенияОС.АдресМестонахождения КАК АдресМестонахождения
	...
	|	СУММА(РасчетНалогаНаИмущество.ОстаточнаяСтоимостьНа0107) КАК ОстаточнаяСтоимостьНа0107,
	|	СУММА(РасчетНалогаНаИмущество.ОстаточнаяСтоимостьНа0110) КАК ОстаточнаяСтоимостьНа0110,
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.КодПоОКОФ.Код, """") КАК КодПоОКОФ,
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.ШифрПоЕНАОФ, """") КАК ШифрПоЕНАОФ,
	|	ЕСТЬNULL(ПОДСТРОКА(ОсновныеСредстваМестонахождениеОС.АдресМестонахождения, 1, 1000), """") КАК АдресМестонахождения
	|ИЗ
	|	НеСнятыеСУчета КАК НеСнятыеСУчета
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОС КАК ПервоначальныеСведенияОС
	|		ПО НеСнятыеСУчета.ОсновноеСредство = ПервоначальныеСведенияОС.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ МестонахождениеОС КАК ОсновныеСредстваМестонахождениеОС
	|		ПО НеСнятыеСУчета.ОсновноеСредство = ОсновныеСредстваМестонахождениеОС.ОсновноеСредство
	|
	|СГРУППИРОВАТЬ ПО
	|	НеСнятыеСУчета.ОсновноеСредство,
	|	РасчетНалогаНаИмущество.КодПоОКТМО,
	...
	|	РасчетНалогаНаИмущество.ВидНалоговойБазы,
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.ИнвентарныйНомер, """"),
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.КодПоОКОФ.Код, """"),
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.ШифрПоЕНАОФ, """"),
	|	ЕСТЬNULL(ПОДСТРОКА(ОсновныеСредстваМестонахождениеОС.АдресМестонахождения, 1, 1000), """")";
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.УсловныйНомер) Тогда
			...
			СтраницаРаздел21.П000210002001 = Выборка.УсловныйНомер;
		ИначеЕсли ЗначениеЗаполнено(Выборка.АдресМестонахождения) Тогда
		...
		Если СтраницаРаздел21.П000210001001 = "3" Тогда
			...
			ПоляАдреса = ИнтерфейсыВзаимодействияБРО.АдресВФорматеФИАС(Выборка.АдресМестонахождения);
...
//  ПоказателиОтчета - Структура - структура, в которую добавляются показатели отчета.
//
Процедура ПолучитьСведенияОПоказателяхОтчетаИмуществоФормаОтчета2019Кв1(ПоказателиОтчета) Экспорт
...
//  Контейнер - Структура - структура, в которую помещаются заполненные показатели отчета.
//
Процедура ЗаполнитьОтчетИмуществоФормаОтчета2019Кв1(ПараметрыОтчета, Контейнер) Экспорт
