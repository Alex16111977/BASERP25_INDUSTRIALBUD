...
Процедура ИзолироватьУчетом(Набор) Экспорт
	
	ИсправлениеДокументовЗарплатаКадры.ИзолироватьУчетом(Набор, "УправлениеШтатнымРасписанием");
...
Процедура ДокументыОбработкаПроверкиЗаполнения(Объект, Отказ, ДатаВступленияВСилу, ИмяЭлемента, ПроверятьВсеПозиции) Экспорт
	...
	Если НЕ МожноИзменятьШтатноеРасписание Тогда
		...
		ТекстСообщения = НСтр("ru = 'Дата последнего изменения штатного расписания больше даты, с которой вступит в силу этот документ.';
								|en = 'Date of the last staff list change is greater than the date this document will become effective. '");
	...
	Если НЕ МожноИзменятьШтатноеРасписание Тогда
		...
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			ТекстСообщения = НСтр("ru = 'Дата последнего изменения штатного расписания больше даты, с которой вступит в силу этот документ. Проведение невозможно.';
									|en = 'Date of the last staff list change is greater than the date this document will become effective. Cannot post.'");
		...
		Иначе
			ТекстСообщения = НСтр("ru = 'Дата последнего изменения штатного расписания больше даты, с которой вступит в силу этот документ. Отмена проведения невозможна.';
									|en = 'Date the staff list was changed last is later than the effective date of this document. Cannot cancel posting. '");
		КонецЕсли;
...
Процедура ЗаписатьНастройкиШтатногоРасписания(Настройки) Экспорт
	...
	Исключение
		...
		ТекстИсключения = НСтр("ru = 'Не удалось применить изменение настроек использования штатного расписания.';
								|en = 'Cannot apply change of staff list use settings.'") + " " + ПодробноеПредставлениеОшибки(ОписаниеОшибки);
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При заполнении позиций в кадровых приказах не удалось записать %1, этот документ необходимо обработать в ручном режиме.';
						|en = 'Cannot write %1 while populating positions in personnel orders, this document should be processed manually.'"),
...
Процедура ПроверитьИсключенияПроверкиЗапретаИзменения(Объект, ПроверкаЗапретаИзменения, УзелПроверкиЗапретаЗагрузки, ВерсияОбъекта) Экспорт
	...
	Если ПроверкаЗапретаИзменения Тогда
		...
		Если ТипЗнч(Объект) = Тип("РегистрНакопленияНаборЗаписей.ЗанятыеПозицииШтатногоРасписания") Тогда
			...
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ИсправленныйДокумент", РегистраторНабора.Метаданные()) Тогда
				...
				Если ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
					...
					Запрос.Текст =
						...
						|	ЗанятыеПозицииШтатногоРасписания.ВидДвижения КАК ВидДвижения,
					...
					Если Не РезультатЗапроса.Пустой() Тогда
						...
						ТаблицаНабора.Свернуть("Период,ПозицияШтатногоРасписания,Сотрудник");
						...
						Для Каждого СтрокаТаблицы Из ТаблицаОбъекта Цикл
							...
							Если СтрокаТаблицы.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
								СтрокаТаблицы.КоличествоСтавок = - СтрокаТаблицы.КоличествоСтавок;
							КонецЕсли;
							
						КонецЦикла;
						ТаблицаОбъекта.Свернуть("Период,ПозицияШтатногоРасписания,Сотрудник", "КоличествоСтавок");
						
						Для Каждого СтрокаНабора Из ТаблицаНабора Цикл
							
							СтрокиТаблицы = ТаблицаОбъекта.НайтиСтроки(Новый Структура("Период,ПозицияШтатногоРасписания,Сотрудник", СтрокаНабора.Период, СтрокаНабора.ПозицияШтатногоРасписания, СтрокаНабора.Сотрудник));
							Если СтрокиТаблицы.Количество() > 0 Тогда
								
								Если СтрокиТаблицы[0].КоличествоСтавок <> 0 Тогда
	...
	Для Каждого ИмяСведений Из НеобходимыеСведения Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Не найдено сведений штатного расписания с именем';
									|en = 'No information of staff list with name'") + " - """ + ИмяСведений + """";
		КонецЕсли;
