...
Функция ПериодыОсновныхСотрудниковФизическихЛиц(ТаблицаФильтр) Экспорт
	...
	|	ОсновныеСотрудникиФизическихЛиц.Сотрудник КАК Сотрудник,
	|	ВЫБОР
	|		КОГДА ОсновныеСотрудникиФизическихЛиц.ДатаНачала < &ФильтрДатаНачала_
	...
	|				ИЛИ ОсновныеСотрудникиФизическихЛиц.ДатаНачала МЕЖДУ &ФильтрДатаНачала_ И &ФильтрДатаОкончания_)";
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Фамилия"", ""Имя"" и ""Отчество"" справочника ""Физические лица""';
									|en = 'Fills in the ""Last name"", ""Name"" and ""Patronymic"" attributes of the ""Individuals"" catalog'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Инициалы имени"" регистра сведений ""ФИО физических лиц""';
									|en = 'Fills in the ""Name initials"" attribute of the ""Individual full name"" information register'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Головной сотрудник"" и интервальный регистр регистра сведений ""Кадровая история сотрудников""';
									|en = 'Fills in the ""Main employee"" attribute and the interval register of the ""Employment record of employees"" information register'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет ограничения доступа к сотрудникам по организациям';
									|en = 'Fills in access limits to employees by companies'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные наименования физических лиц';
									|en = 'Fills in service names of individuals'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Переносит виды занятости в специализированный регистр сведений';
									|en = 'Transfers employment kinds to the special information register'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет основных сотрудников физических лиц';
									|en = 'Fills in individuals'' main employees'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет признак оформленности трудовых отношений';
									|en = 'Fills in whether labor relations are registered'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправляет представления Паспортов РФ';
									|en = 'Corrects presentations of RF passports'");
			...
			Исключение
				...
				ТекстИсключенияЗаписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось обновить права доступа к %1.
						...
						|en = 'Cannot update access rights to %1.
...
Процедура ЗаполнитьФамилиюИмяИОтчествоПоДаннымРегистра(Параметры) Экспорт
	...
	Иначе
		...
		Если Параметры.ФизическиеЛицаСОшибкой.Количество() > 0 Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось обработать %1 физических лиц';
											|en = 'Cannot process %1 individuals'"), Параметры.ФизическиеЛицаСОшибкой.Количество());
	...
	Для Каждого КадровыеДанныеСотрудника Из КадровыеДанныеСотрудников Цикл
		...
		Если СреднийЗаработок <> Неопределено И СреднийЗаработок > 0 Тогда
			...
			ДанныеСправки.СреднийЗаработокПрописью = ЧислоПрописью(СреднийЗаработок, "Л = ru_RU", НСтр("ru = 'рубль, рубля, рублей, м, копейка, копейки, копеек, ж, 2';
																										|en = 'ruble, ruble, rubles, m, copeck, copeck, copecks, f, 2'"));
