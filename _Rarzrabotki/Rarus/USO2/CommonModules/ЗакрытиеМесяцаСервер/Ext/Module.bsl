	...
	Иначе
		...
		Пока НачалоРасчета <= ПараметрыОбработчика.ПараметрыРасчета.КонецПериода Цикл
			...
			Для Каждого Организация Из ДанныеКРасчетуЗаМесяц.Организация Цикл
				...
				//Проверка на наличие развернутого сальдо в регистрах взаиморасчетов.
				...
				Запрос.Текст = "
					...
					|	ВЫБОР КОГДА РасчетыСКлиентамиОстатки.ДолгОстаток > РасчетыСКлиентамиОстатки.ПредоплатаОстаток
					|		ТОГДА РасчетыСКлиентамиОстатки.ДолгОстаток 
					|		ИНАЧЕ РасчетыСКлиентамиОстатки.ПредоплатаОстаток
					|	КОНЕЦ                                                           КАК СуммаРазвернутогоОстатка
					|	
					|ИЗ
					|	РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(&КонецПериодаГраница,) КАК РасчетыСКлиентамиОстатки
					|
					|ГДЕ
					|	РасчетыСКлиентамиОстатки.ДолгОстаток > 0 
					|	И РасчетыСКлиентамиОстатки.ПредоплатаОстаток > 0
					|	И РасчетыСКлиентамиОстатки.АналитикаУчетаПоПартнерам.Организация = &Организация
					|
					|ОБЪЕДИНИТЬ ВСЕ
					|
					|ВЫБРАТЬ
					...
					|	ВЫБОР КОГДА РасчетыСПоставщикамиОстатки.ДолгОстаток > РасчетыСПоставщикамиОстатки.ПредоплатаОстаток 
					|		ТОГДА РасчетыСПоставщикамиОстатки.ДолгОстаток 
					|		ИНАЧЕ РасчетыСПоставщикамиОстатки.ПредоплатаОстаток
					|	КОНЕЦ                                                              КАК СуммаРазвернутогоОстатка
					|	
					|ИЗ
					|	РегистрНакопления.РасчетыСПоставщикамиПоСрокам.Остатки(&КонецПериодаГраница,) КАК РасчетыСПоставщикамиОстатки
					|
					|ГДЕ
					|	РасчетыСПоставщикамиОстатки.ДолгОстаток > 0 
					|	И РасчетыСПоставщикамиОстатки.ПредоплатаОстаток > 0
					|	И РасчетыСПоставщикамиОстатки.АналитикаУчетаПоПартнерам.Организация = &Организация
					|
					|";
				
				ВыборкаРазвернутоеСальдо = Запрос.Выполнить().Выбрать();
				
				Если ВыборкаРазвернутоеСальдо.Количество() > 0 Тогда
					
					Шаблон = НСтр("ru = 'Обнаружено развернутое сальдо взаиморасчетов %1 на конец периода %2
								|по аналитике взаиморасчетов ""%3"" и объекту расчетов ""%4"".';
								|en = 'Detailed balance of mutual settlements is found %1 at the end of the period %2
								| on the mutual settlements dimension ""%3"" and the settlements object ""%4"".'");
					...
					Пока ВыборкаРазвернутоеСальдо.Следующий() Цикл
						...
						ТекстТипРасчетов = ?(ВыборкаРазвернутоеСальдо.ТипРасчетов = ПредопределенноеЗначение("Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом"),
							НСтр("ru = 'с клиентами';
								|en = 'with customers'"),НСтр("ru = 'с поставщиками';
															|en = 'with suppliers'"));
						
						ПолныйТекстПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, 
							ТекстТипРасчетов,
							ОкончаниеПериода,
							ВыборкаРазвернутоеСальдо.АналитикаУчетаПоПартнерам,
							ВыборкаРазвернутоеСальдо.ОбъектРасчетов);
				...
				#Область СоответствиеСуммРегистров
				//Проверка на соответствие сумм в первичных регистрах взаиморасчетов и в регистрах по срокам.
				...
				|				ТОГДА ВЫРАЗИТЬ(РасчетыСКлиентами.Регистратор КАК Документ.РеализацияТоваровУслуг).Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.ВПути)
				|				КОГДА РасчетыСКлиентами.Регистратор ССЫЛКА Документ.РеализацияУслугПрочихАктивов
				|				ТОГДА ВЫРАЗИТЬ(РасчетыСКлиентами.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.ВПути)
				|			ИНАЧЕ ЛОЖЬ
				|		КОНЕЦ
				...
				|	И НЕ (РасчетыСПоставщиками.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет)
				|			И РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Организация = ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация))
				|	И НЕ РасчетыСПоставщиками.Регистратор Ссылка Документ.КорректировкаРегистров";
				
				МассивДокументовДляПроверкиСумм = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ДокументРегистратор");
				
				Запрос.УстановитьПараметр("МассивДокументов", МассивДокументовДляПроверкиСумм);
				Запрос.Текст = РегистрыСведений.СуммыДокументовВВалютеРегл.ТекстЗапросаСуммВзаиморасчетовПоМассиву("СуммыРасчетовПоСрокам");
				Запрос.Выполнить();
				...
				|	СУММА(ВложенныйЗапрос.СуммаРасчетовПоСрокам) КАК СуммаРасчетовПоСрокам
				|ПОМЕСТИТЬ НекорректныеРегистраторы
				|ИЗ (ВЫБРАТЬ
				|		РасчетыПоСрокам.ТипРасчетов КАК ТипРасчетов,
				|		РасчетыПоСрокам.Регистратор КАК Регистратор,
				|		РасчетыПоСрокам.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
				|		РасчетыПоСрокам.Предоплата +  РасчетыПоСрокам.Долг    КАК СуммаРасчетовПоСрокам,
				|		0                                                   КАК СуммаРасчетов
				|	ИЗ СуммыРасчетовПоСрокам КАК РасчетыПоСрокам
				|	ГДЕ
				|		РасчетыПоСрокам.АналитикаУчетаПоПартнерам.Организация = &Организация
				|	
				|	ОБЪЕДИНИТЬ ВСЕ
				|	
				|	ВЫБРАТЬ
				...
				|		0 КАК СуммаРасчетовПоСрокам,
				|		ВЫБОР КОГДА РасчетыСКлиентами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) 
				...
				|		И НЕ (РасчетыСКлиентами.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
				|			И РасчетыСКлиентами.АналитикаУчетаПоПартнерам.Организация = ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация))
				|		И РасчетыСКлиентами.АналитикаУчетаПоПартнерам.Организация = &Организация
				|	
				|	ОБЪЕДИНИТЬ ВСЕ
				|	
				...
				|		РегистрНакопления.РасчетыСПоставщиками КАК РасчетыСПоставщиками
				|	ГДЕ
				|		РасчетыСПоставщиками.Регистратор В (&МассивДокументов)
				|		И НЕ (РасчетыСПоставщиками.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет)
				|				И РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Организация = ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация))
				|		И РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Организация = &Организация) КАК ВложенныйЗапрос
				|СГРУППИРОВАТЬ ПО
				|	ВложенныйЗапрос.ТипРасчетов,
				|	ВложенныйЗапрос.АналитикаУчетаПоПартнерам,
				|	ВложенныйЗапрос.Регистратор
				|ИМЕЮЩИЕ
				|	СУММА(ВложенныйЗапрос.СуммаРасчетов) <> СУММА(ВложенныйЗапрос.СуммаРасчетовПоСрокам)
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				...
				|ПОМЕСТИТЬ ПериодыРегистраторов
				|ИЗ
				|	(ВЫБРАТЬ
				...
				|	ИЗ 
				|		РегистрНакопления.РасчетыСКлиентами КАК РасчетыСКлиентами
				|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ НекорректныеРегистраторы КАК НекорректныеРегистраторы
				|				ПО НекорректныеРегистраторы.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
				|					И НекорректныеРегистраторы.Регистратор = РасчетыСКлиентами.Регистратор
				|	ГДЕ
				...
				|		РегистрНакопления.РасчетыСПоставщиками КАК РасчетыСПоставщиками
				|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ НекорректныеРегистраторы КАК НекорректныеРегистраторы
				|				ПО НекорректныеРегистраторы.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
				|					И НекорректныеРегистраторы.Регистратор = РасчетыСПоставщиками.Регистратор
				|	ГДЕ
				|		РасчетыСПоставщиками.Сумма <> 0) КАК ВложенныйЗапрос
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	МИНИМУМ(ПериодыРегистраторов.Период) КАК Период,
				|	ПериодыРегистраторов.ОбъектРасчетов КАК ОбъектРасчетов,
				|	ПериодыРегистраторов.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
				|	ПериодыРегистраторов.Валюта КАК Валюта
				|ПОМЕСТИТЬ МинимальныеПериоды
				|ИЗ
				|	ПериодыРегистраторов
				|СГРУППИРОВАТЬ ПО 
				|	ПериодыРегистраторов.ОбъектРасчетов,
				|	ПериодыРегистраторов.АналитикаУчетаПоПартнерам,
				|	ПериодыРегистраторов.Валюта
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	ПериодыРегистраторов.Регистратор КАК Регистратор,
				|	ПериодыРегистраторов.ОбъектРасчетов КАК ОбъектРасчетов
				|ИЗ
				|	ПериодыРегистраторов КАК ПериодыРегистраторов
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ МинимальныеПериоды КАК МинимальныеПериоды
				|			ПО МинимальныеПериоды.Период = ПериодыРегистраторов.Период
				|				И МинимальныеПериоды.ОбъектРасчетов = ПериодыРегистраторов.ОбъектРасчетов
				|				И МинимальныеПериоды.АналитикаУчетаПоПартнерам = ПериодыРегистраторов.АналитикаУчетаПоПартнерам
				|				И МинимальныеПериоды.Валюта = ПериодыРегистраторов.Валюта
				|";
				
				Результат = Запрос.Выполнить();
				ВыборкаНекорректныхДвижений = Результат.Выбрать();
				
				Если ВыборкаНекорректныхДвижений.Количество() > 0 Тогда
					
					Шаблон = НСтр("ru = 'Обнаружены ошибки в движениях по взаиморасчетам по объекту расчетов ""%1"" на конец периода %2.
								|Необходимо отменить проведение и провести %3.';
								|en = 'Errors in the movements on settlements on the settlements object are found ""%1"" at the end of the period %2.
								|Cancel posting and post %3'");
					...
					Пока ВыборкаНекорректныхДвижений.Следующий() Цикл
						
						ГруппаПроблем = НСтр("ru = 'При выполнении операции были диагностированы ошибки';
											|en = 'Errors were found when executing the operation'");
						
						ПолныйТекстПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, 
							ВыборкаНекорректныхДвижений.ОбъектРасчетов,
							ОкончаниеПериода,
							ВыборкаНекорректныхДвижений.Регистратор);
