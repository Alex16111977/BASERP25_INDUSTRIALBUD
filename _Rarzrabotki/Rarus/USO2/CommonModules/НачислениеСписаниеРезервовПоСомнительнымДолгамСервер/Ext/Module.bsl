	...
	Если РазмерыВременныхТаблиц.вт_ОрганизацииСРезервами = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет организаций, для которых в учетной политике используется формирование резервов в БУ или НУ';
				|en = 'No companies for which generation of reserves in AC or TA is used in accounting policy'"));
	...
	ИначеЕсли РазмерыВременныхТаблиц.вт_ОрганизацииСОстатками = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков по регистрам расчетов с клиентами, поставщиками, по регистрам финансовых инструментов и по резервов';
				|en = 'No balance in registers of payments to customers, suppliers, in registers of financial instruments and in reserves'"));
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
					...
					|%2'"),
				ПротоколРасчетаПартийИСебестоимости.ПредставлениеПериодаРасчета(НачалоПериода),
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр(
					...
					|%2';
					|en = 'Generation of reserves for doubtful debts for period %1 was completed with an error:
					|%2'"),
				ПротоколРасчетаПартийИСебестоимости.ПредставлениеПериодаРасчета(НачалоПериода),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОписаниеПроверок_ФормированиеРезервовПоСомнительнымДолгам(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не отражена операция формирования резервов по сомнительным долгам';
			|en = 'The transaction of generation of reserves for doubtful debts is not recorded'"),
		НСтр("ru = 'В каждом месяце должен быть создан документ ""Начисление и списание резервов по сомнительным долгам"".';
			|en = 'Create the ""Accrual and write-off of reserves for doubtful debts"" document for each month.'"));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Изменилась дебиторская задолженность резервов по сомнительным долгам';
			|en = 'Accounts receivable of reserves for doubtful debts was changed'"),
		НСтр("ru = 'Дебиторская задолженность по оперативным регистрам учета должна совпадать с документом ""Начисление и списание резервов по сомнительным долгам"".';
			|en = 'Accounts receivable for operational accounting registers must match the ""Accrual and write-off of reserves for doubtful debts"" document.'"));
...
Процедура ПроверкаНеобходимостиФормированияРезервовПоСомнительнымДолгам(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Не создан документ ""Начисление и списание резервов по сомнительным долгам"" по организации ""%1"" за период %2';
			|en = 'The ""Accrual and write-off of reserves for doubtful debts"" document is not created for the ""%1"" company for period %2'"),
		СписокПолей);
...
Процедура ПроверкаИзмененияДебиторскойЗадолженностиРезервовПоСомнительнымДолгам(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Документ", НСтр("ru = 'Документ ""Начисление и списание резервов по сомнительным долгам""';
											|en = 'The ""Accrual and write-off of reserves for doubtful debts"" document'"));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		|en = 'There are accounts receivable changes for the ""Accrual and write-off of reserves for doubtful debts"" document for the ""%1"" company for period %2
		|Go to the ""Accrual and write-off of reserves for doubtful debts"" document and correct the lines (correct reserve amount, valuation, remove the line, ...), marked by the ""Need check"" check box, manually.
...
Процедура ОбновитьДвиженияДокумента(Ссылка, СоставОбновления)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось записать движения %Обновление% по причине %Причина%';
								|en = 'Cannot record register records %Обновление% due to %Причина%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Частичное обновление движений документа';
				|en = 'Partial update of document records'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
