		...
		Если ИсточникиЧерезЗапятую = "" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В формах отчетов, обработок и в общих формах
					...
					|en = 'Explicitly specify the ""%2"" parameter in forms of reports,
					|data processors and in common forms
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать настройки объекта %1 из модуля менеджера:';
				|en = 'Cannot read settings of the %1 object from the manager module:'"), ПолноеИмя);
		ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Подключаемые команды';
				|en = 'Connected commands'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Для Каждого Настройка Из НастройкиПрограммногоИнтерфейса Цикл
		...
		Исключение
			ТекстОшибки = НСтр("ru = 'Не удалось зарегистрировать настройку программного интерфейса подключаемых объектов.
				...
				|en = 'Cannot register setting of connected object application interface. 
...
Процедура ПроверитьИмяВидаКоманд(ИмяВида)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';
							|en = 'The ""%1"" command kind name does not satisfy variable naming requirements.'");
	...
	Если Команда = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сведения о команде ""%1"" не найдены.';
				|en = 'Information on the ""%1"" command is not found.'"),
			ИмяКомандыВФорме);
	КонецЕсли;
