...
Процедура ЗаполнитьОтчетЗемельныйНалогФормаОтчета2011Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%4';
			|en = 'No data about registration of land lots with the tax authority - ""%1"" on %2 for %3
			|%4'"), 
...
Процедура ЗаполнитьОтчетЗемельныйНалогФормаОтчета2018(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Check data on land lot registration %1
		...
		Пока ВыборкаЗемельныхУчастковОКАТО.Следующий() Цикл
			...
			Если ВыборкаЗемельныхУчастковОКАТО.КодПоОКТМО = "" Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					|en = 'RNCMT code is required in the ""Land lot registration"" document by BCC %1.
					| Specify RNCMT code and calculate the land tax again.'"),
		...
		Если ВыборкаОКТМО.КодПоОКТМО = "" Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'В документе ""Регистрация транспортных средств"" не заполнен код ОКТМО. Укажите код ОКТМО и произведите расчет транспортного налога заново.';
					|en = 'RNCMT code is required in the ""Vehicle registration"" document. Specify RNCMT code and calculate the vehicle tax again.'"));
		КонецЕсли;
