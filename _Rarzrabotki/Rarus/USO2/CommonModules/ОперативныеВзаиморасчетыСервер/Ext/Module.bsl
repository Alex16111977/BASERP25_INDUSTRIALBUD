...
Процедура ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ГлобальныеПеременные.Порядок) Тогда
		Запрос.УстановитьПараметр("НачалоДняПорядка",           Дата(Лев(ГлобальныеПеременные.Порядок,8)));
	Иначе
		Запрос.УстановитьПараметр("НачалоДняПорядка",           Дата(1,1,1,1,1,2));
	КонецЕсли;
		...
		Если Индексы.ИндексОплаты <= ЗаписейОплаты - 1 И Индексы.ИндексОтгрузкиПоставки <= ЗаписейОтгрузкиПоставки - 1 Тогда 
			...
			Если СтрокаОплаты.ПорядокОперации < СтрокаОтгрузкиПоставки.ПорядокОперации 
				...
				Если СтрокаОтгрузкиПоставки.Сумма <> 0 Тогда
					...
					Если ДолгРаспределеноРегл = Неопределено Тогда
						ДолгРаспределеноРегл = 0;
					КонецЕсли;
					
					Если ДолгРаспределеноУпр = Неопределено Тогда
						ДолгРаспределеноУпр = 0;
					КонецЕсли;						 
					// {УСО 2.4}:{Управление строительной организацией}}}
			...
			Если ПланыОплат[счПриход].КОплате > 0 И ПланыОплат[счРасход].Оплачено > 0
				...
				Если ПланыОплат[счПриход].Регистратор <> ПланыОплат[счРасход].Регистратор Тогда
					...
					Если ПланыОплат[счПриход].ПорядокОперации <= ПланыОплат[счРасход].ПорядокОперации  Тогда
						ПроверитьДобавитьПриходПланаОплат(ТаблицаПланОплат, ПланыОплат[счПриход], ГлобальныеПеременные.ХозяйственныеОперации["Планирование"], СуммаСписанияКОплате, СуммаСписанияОплачено);
	...
	Если НЕ ОсновныеПараметры.НачальноеЗаполнение Тогда
		...
		Если ОсновныеПараметры.Свойство("ДополнительныеСвойстваПроведения") И ОсновныеПараметры.ДополнительныеСвойстваПроведения <> Неопределено Тогда
			...
			|		ДвиженияПосле.КорАналитикаУчетаПоПартнерам  КАК КорАналитикаУчетаПоПартнерам,
			|		
	...
	// {УСО 2.4}:{Управление строительной организацией}{{
	УСОERP_МодификацияКонфигурации.ОбработатьТаблицуПоставок(ТаблицаПланОтгрузокПоставок);
	// {УСО 2.4}:{Управление строительной организацией}}}
...
Процедура ЗаполнитьРегистрыПриВключенииНовойАрхитектуры(Параметры, УникальныйИдентификатор) Экспорт
	...
	Попытка
		...
		ТаблицаАналитик = Запрос.МенеджерВременныхТаблиц.Таблицы["ВтДвижения"].ПолучитьДанные().Выгрузить();
		...
		|ВЫБРАТЬ
		|	*
		|ИЗ ВтДвижения КАК Движения
		|ГДЕ Движения.АналитикаУчетаПоПартнерам = &АналитикаУчетаПоПартнерам
		|	И Движения.ОбъектРасчетов = &ОбъектРасчетов
		|	И Движения.Валюта = &Валюта";
		
		Для Каждого СтрокаАналитики Из ТаблицаАналитик Цикл
			НаборЗаписей = РегистрыНакопления.РасчетыСКлиентамиПоСрокам.СоздатьНаборЗаписей();
			НаборЗаписей.ОбменДанными.Загрузка = Истина;
			
			Запрос.УстановитьПараметр("АналитикаУчетаПоПартнерам", СтрокаАналитики.АналитикаУчетаПоПартнерам);
			Запрос.УстановитьПараметр("ОбъектРасчетов", СтрокаАналитики.ОбъектРасчетов);
			Запрос.УстановитьПараметр("Валюта", СтрокаАналитики.Валюта);
		...
		Запрос.Текст = "УНИЧТОЖИТЬ ПорядокДокументов; УНИЧТОЖИТЬ ВтРасчеты; УНИЧТОЖИТЬ ВтДвижения";
		Запрос.Выполнить();
...
Процедура СформироватьВТДанныхРегистровДоНачалаРасчетов(Запрос, ГлобальныеПеременные)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПланОплат КАК РасчетыПланОплатДо
	|			ПО РасчетыПланОплатДо.АналитикаУчетаПоПартнерам = РасчетыПланыОплатОстатки.АналитикаУчетаПоПартнерам
	|				И РасчетыПланОплатДо.ОбъектРасчетов            = РасчетыПланыОплатОстатки.ОбъектРасчетов
	|				И РасчетыПланОплатДо.Валюта                    = РасчетыПланыОплатОстатки.Валюта
	|				И РасчетыПланОплатДо.ДокументПлан              = РасчетыПланыОплатОстатки.ДокументПлан
	|				И РасчетыПланОплатДо.ПорядокОперации           < &Порядок
	|СГРУППИРОВАТЬ ПО
	|	РасчетыПланыОплатОстатки.ДокументПлан,
	|	РасчетыПланыОплатОстатки.ДатаПлановогоПогашения,
	|	РасчетыПланыОплатОстатки.ДатаВозникновения,
	|	РасчетыПланыОплатОстатки.ВариантОплаты,
	|	РасчетыПланыОплатОстатки.КОплатеОстаток,
	|	РасчетыПланыОплатОстатки.ОплаченоОстаток
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументПлан,
	|	ДатаПлановогоПогашения
	|;
...
Функция ПолучитьОплаты(Запрос, ГлобальныеПеременные)
	...
	|		ОстаткиОплат.ДокументРегистратор                            КАК РасчетныйДокумент,
	|		НЕОПРЕДЕЛЕНО                                                КАК КорОбъектРасчетов,
	|		ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПоПартнерам.ПустаяСсылка) КАК КорАналитикаУчетаПоПартнерам,
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	// {УСО 2.4}:{Управление строительной организацией}{{
	  Строки = РасчетыОтгрузки.НайтиСтроки(Новый Структура("ДатаВозникновения",Дата(1,1,1)));
	  Для Каждого НайденнаяСтрока ИЗ Строки Цикл
	      НайденнаяСтрока.ДатаВозникновения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГлобальныеПеременные.Регистратор, "Дата");
	  КонецЦикла; 	  
	// {УСО 2.4}:{Управление строительной организацией}}}
...
Процедура ДобавитьРасходПланаОплат(ТаблицаПланОплат, Приход, Расход, СуммаКОплате, СуммаОплачено)
...
Функция ВидТипДокумента(ТипДокумента, ВидДвижения=Неопределено) Экспорт
	...
	// {УСО 2.4}:{Управление строительной организацией}{{
	УСОERP_МодификацияКонфигурации.ВидТипДокумента(ТипДокумента,ВидДвижения,Вид,Тип);
	// {УСО 2.4}:{Управление строительной организацией}}}
...
Функция СписокТиповРегистраторовПланов() Экспорт
	...
	// {УСО 2.4}:{Управление строительной организацией}{{
	УСОERP_МодификацияКонфигурации.СписокТиповРегистраторовПланов(СписокТиповРегистраторовПланов);
	// {УСО 2.4}:{Управление строительной организацией}}}
