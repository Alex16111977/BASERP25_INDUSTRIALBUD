		...
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
			Шаблон =
				НСтр("ru = 'Поле ""%1"" заполнено некорректно.
				           |%4';
				           |en = 'The ""%1"" field is filled in incorrectly.
				           |%4'");
		КонецЕсли;
	...
	Если Условие <> Истина Тогда
		...
		Если ПустаяСтрока(Сообщение) Тогда
			ТекстИсключения = НСтр("ru = 'Недопустимая операция';
									|en = 'Invalid transaction'"); // Assertion failed
		Иначе
...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(
		...
		НСтр("ru = 'Недопустимое значение параметра ИмяПроцедурыИлиФункции';
			|en = 'Invalid value of the ИмяПроцедурыИлиФункции parameter'"), 
		Контекст);
	
	Проверить(
		ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ru = 'Недопустимое значение параметра ИмяПараметра';
			|en = 'Invalid value of the ИмяПараметра parameter'"), 
		Контекст);
	...
	Проверить(
		...
		НСтр("ru = 'Недопустимое значение параметра ОжидаемыеТипы';
			|en = 'Invalid value of the ОжидаемыеТипы parameter'"), 
		Контекст);
			...
			Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимое значение параметра %1 (Структура) в %2. 
					           ...
					           |en = 'Invalid value for parameter %1 (Structure) in %2.
					           |Expected a property %3 (type %4) in the structure.'"), 
					ИмяПараметра, 
					ИмяПроцедурыИлиФункции, 
			...
			Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимое значение свойства %1 в параметре %2 (Структура) в %3. 
					           ...
					           |en = 'Invalid value for property %1 in parameter %2 (Structure) in %3.
					           |Expected value: %4; value passed: %5 (type %6).'"), 
					ОжидаемоеИмяСвойства,
					ИмяПараметра,
					ИмяПроцедурыИлиФункции,
					ПредставлениеТипов(ОжидаемыеТипы), 
		...
		Если Заменять <> Истина И Приемник.Свойство(Элемент.Ключ) Тогда
			...
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пересечение ключей источника и приемника: ""%1"".';
																								|en = 'Intersection of source and receiver keys: ""%1"".'"), Элемент.Ключ);
			КонецЕсли
		КонецЕсли;
		...
		Если Не АдресОпределен Тогда 
			Если ЕстьПризнакНаличияАдресаВСтроке Тогда 
				ОписаниеОшибки = НСтр("ru = 'Адрес электронной почты содержит ошибки';
										|en = 'Email address contains errors'");
			Иначе
				ОписаниеОшибки = НСтр("ru = 'Строка не содержит адреса электронной почты';
										|en = 'The string does not contain an email address'");
			КонецЕсли;
		КонецЕсли;
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Иначе
		...
		КраткоеОписаниеОшибки = НСтр("ru = 'Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';
									|en = 'Direct connection to the infobase is available only on a client running Windows OS.'");
	#КонецЕсли
...
#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Запуск программ недоступен в веб-клиенте.';
							|en = 'Application launch is not available in web client.'");
#Иначе
...
#Если ВнешнееСоединение Тогда
		ВызватьИсключение 
			НСтр("ru = 'Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
			           ...
			           |en = 'Invalid value of the ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами. parameter.
			           |Elevation of system privileges is not available from external connection.'");
#КонецЕсли
...
#Если Сервер Тогда
		ВызватьИсключение 
			НСтр("ru = 'Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
			           ...
			           |en = 'Invalid value of the ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами. parameter.
			           |Elevation of system privileges is not available on the server.'");
#КонецЕсли
			...
			Если ДождатьсяЗавершения Тогда
				ВызватьИсключение 
					НСтр("ru = 'Недопустимо одновременно устанавливать параметры 
					           ...
					           | - ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами:
					           |Operating system does not allow you to track processes
					           |launched by administrator under username.'");
			КонецЕсли;
...
#Если ВебКлиент Тогда
	ВызватьИсключение НСтр("ru = 'Выполнение диагностики соединения недоступно в веб-клиенте.';
							|en = 'Cannot diagnose connection in web client.'");
#Иначе
...
Функция ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	...
	НедопустимыйСимволВИмениАдресата = НСтр("ru = 'Недопустимый символ в имени адресата.';
											|en = 'Recipient name contains invalid characters.'");
	НедопустимыйСимволВПочтовомАдресе = НСтр("ru = 'Недопустимый символ в почтовом адресе.';
											|en = 'Invalid character in postal address.'");
	СимволыОбрамления = "<>[]";
...
#Если Сервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Базовая функциональность';
										|en = 'Basic functionality'", КодОсновногоЯзыка()),
...
#Иначе 
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Подключение проводится на компьютере (клиенте) <%1>.';
			|en = 'Connecting on the <%1> computer (client).'"), ИмяКомпьютера());
#КонецЕсли
