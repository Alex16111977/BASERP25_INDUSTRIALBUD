...
Функция НаименованиеФоновогоРезервногоКопирования() Экспорт
	...
	Возврат НСтр("ru = 'Резервное копирование области данных';
				|en = 'Data area backup'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ОбластьДанных = Неопределено Тогда
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'При вызове процедуры УстановитьФлагАктивностиВОбласти из неразделенного сеанса параметр ОбластьДанных является обязательным.';
									|en = 'The ОбластьДанных parameter is required while calling the УстановитьФлагАктивностиВОбласти procedure from the undivided session.'");
		КонецЕсли;
	Иначе
		Если НЕ РаботаВМоделиСервиса.СеансЗапущенБезРазделителей()
			...
			ВызватьИсключение(НСтр("ru = 'Запрещено работать с данными области кроме текущей';
									|en = 'Working with data of the area apart from the current one is prohibited'"));
...
Процедура СозданиеКопий(Знач ПараметрыСоздания, Знач СостояниеСоздания) Экспорт
	...
	Если НЕ ТребуетсяСоздание Тогда
		ЗаписьЖурналаРегистрации(
			СобытиеЖурналаРегистрации() + "." 
				+ НСтр("ru = 'Пропуск создания';
						|en = 'Skip creation'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание резервной копии области данных.Не удалось удалить временный файл';
											|en = 'Backing up data area. Cannot delete temporary file.'", ОбщегоНазначения.КодОсновногоЯзыка()), 
		...
		ЗаписатьСобытиеВЖурнал(
			НСтр("ru = 'Создание';
				|en = 'Create'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ПараметрыСобытия);
...
Процедура ОшибкаСозданияКопии(Знач ПараметрыЗадания, Знач ИнформацияОбОшибке) Экспорт
	...
	Если ПараметрыЗадания.НомерПопытки < ПараметрыЗадания.КоличествоПовторовПриАварийномЗавершении Тогда
		...
		Событие = НСтр("ru = 'Ошибка итерации создания';
						|en = 'Creation iteration error'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		...
		Событие = НСтр("ru = 'Ошибка создания';
						|en = 'Creation error'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Процедура ЗапланироватьАрхивациюВОчереди(Знач ПараметрыВыгрузки) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ru = 'Не достаточно прав для выполнения операции';
								|en = 'Insufficient rights for the operation'"));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Резервное копирование приложений';
				|en = 'Application backup '", ОбщегоНазначения.КодОсновногоЯзыка());
