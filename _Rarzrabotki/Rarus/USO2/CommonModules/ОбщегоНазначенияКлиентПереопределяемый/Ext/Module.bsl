...
Процедура ПередНачаломРаботыСистемы(Параметры) Экспорт
	...
	// {УАТ}
	уатОбщегоНазначенияКлиент.УстановитьРежимОсновногоОкнаДляТерминалаПЛ();
	ItobЦентрСпутниковогоМониторингаКлиент.ПередНачаломРаботыСистемы(Параметры);
	// {/УАТ}
	
	// {УСО}
	УСОERP_ЗащищенныеФункцииКлиент.ПередНачаломРаботыСистемы(Параметры);
	// {/УСО}
...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	//{ Эрикос.СметнаяПодсистема
	ОпцияИспользоватьВНБ 		= СМ_СметаВызовСервера.ПроверитьИспользованиеВНБ();
	НастройкаИспользоватьВНБ 	= ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("НастройкиПользователяСмета","ИспользоватьВНБ",ОпцияИспользоватьВНБ);
	Если ОпцияИспользоватьВНБ И НастройкаИспользоватьВНБ Тогда
		СМ_СметаКлиент.СметнаяПодсистемаПриЗапуске();
	КонецЕсли;
	//} Эрикос.СметнаяПодсистема	
	
	// {УАТ}
	уатОбщегоНазначенияКлиент.ОткрытьТерминалПЛПриНачалеРаботыСистемы();
		...
		Если глКомпонентаОбменаСМобильнымиПриложениями <> Неопределено Тогда
			...
			Исключение
				ИмяСобытия = НСтр("ru = 'Ошибка при отключении компоненты обмена данными';
									|en = 'An error occurred when disabling the data exchange component'");
