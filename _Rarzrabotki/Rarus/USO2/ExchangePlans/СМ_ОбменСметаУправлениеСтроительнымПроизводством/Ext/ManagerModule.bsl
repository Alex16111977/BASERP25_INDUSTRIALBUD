...
//  IDНастройки - Строка - содержит идентификатор настройки.
//
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Управление строительным производством'");
...
// Прикладной разработчик на основе установленных значений по умолчанию на узле базы корреспондента должен 
// сформировать строку описания удобную для восприятия пользователем.
// 
// Параметры:
...
//
// Параметры:
//	ДополнительныеДанные - Структура - дополнительные данные, которые будут использованы в базе-корреспонденте 
//						   при настройке обмена. В качестве значений структуры применимы только значения, 
//						   поддерживающие XDTO-сериализацию.
//
