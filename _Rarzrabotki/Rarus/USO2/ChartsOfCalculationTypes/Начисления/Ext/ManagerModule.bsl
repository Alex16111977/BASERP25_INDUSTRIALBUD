...
Процедура СоздатьНачисленияПоНастройкам(ПараметрыПланаВидовРасчета = Неопределено, НачальноеЗаполнение = Ложь) Экспорт
	...
	Если НачальноеЗаполнение Тогда
		...
		Описание.Наименование			= НСтр("ru = 'Пособие по уходу за ребенком до трех лет';
												|en = 'Child care allowance for children under 3 years old'");
		Описание.КраткоеНаименование 	= НСтр("ru = 'Уход за реб. до 3';
												|en = 'Care for child under the age of 3'");
		СоздатьИзменитьНачисленияПоОписанию(КоллекторНачислений, Описание);
	КонецЕсли;
...
Функция ПоказателиНачислений(ОтборНачислений) Экспорт	
	...
	|РазовыйПоказательОрганизации, РазовыйПоказательПодразделения";
...
Функция ТребованияНачисленийТаблица(НачисленияОтбор) Экспорт
	...
	Запрос.УстановитьПараметр("ПоказательНадбавкаЗаВредность", ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ПроцентНадбавкиЗаВредность"));
	...
	Запрос.УстановитьПараметр("НормаДнейЧасов", ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДнейЧасов"));
	...
	Запрос.Текст = 
		...
		|	ПоказателиРасчета.ПериодРасчетаБазовыхНачислений КАК ПериодРасчетаБазовыхНачислений,
		...
		|		ВЫБОР
		|			КОГДА ПОДСТРОКА(ВидыРасчета.ФормулаРасчета, 1, 1) = ""0""
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		...
		|	ПоказателиРасчета.ПериодРасчетаБазовыхНачислений";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоДовольствия") Тогда 
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоДовольствия");
		Модуль.ПриОпределенииСвойствНачисленийДляРасчета(ТребованияНачислений);
	КонецЕсли;
...
Процедура ОбновитьЗаписимостиНачислений() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка создания начисления «Компенсация морального вреда»';
				|en = 'An error occurred when creating the ""Compensation for moral damages"" accrual'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка создания начисления ""Оплата вынужденного простоя""';
				|en = 'An error occurred when creating the ""Forced downtime payment"" accrual'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.ПланыВидовРасчета.Начисления, ,
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка создания начисления «Компенсация за неотработанные дни при увольнении»';
				|en = 'An error occurred when creating the ""Compensation for unearned days on termination of employment"" accrual'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.ПланыВидовРасчета.Начисления, ,
		...
		Попытка
			...
			Если ИспользоватьГосударственнуюСлужбу Тогда
				...
				Описание.Наименование								= НСтр("ru = 'Компенсация за неотработанные дни при увольнении государственного служащего';
																			|en = 'Compensation for unearned days on termination of employment of a public service employee'");
			...
			Иначе
				...
				Описание.Наименование								= НСтр("ru = 'Компенсация за неотработанные дни при увольнении муниципального служащего';
																			|en = 'Compensation for unearned days on termination of employment of a municipal employee'");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление списка показателей расчета простоя.';
									|en = 'Update the list of downtime calculation indicators.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление списка показателей доплаты до среднего заработка.';
									|en = 'Update indicators of supplement to average earnings.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление начислений категории ""Неявка по болезни"".';
									|en = 'Updating accruals of category ""Sickness absence"".'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение регистра ЗаполнитьРегистрЗависимостиНачислений';
										|en = 'Filling of the Register FillRegisterOfAccrualsDependence'");
...
Процедура ЗаполнитьКраткиеНаименованияВидовРасчета() Экспорт 
	...
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата по окладу';
										|en = 'Payment by base salary'"), 											НСтр("ru = 'Оклад';
																													|en = 'Base salary'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата по дневному тарифу';
										|en = 'Payment by daily tariff '"), 									НСтр("ru = 'Опл. по дням';
																													|en = 'Payment by days'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за совмещение должностей, исполнение обязанностей';
										|en = 'Extra pay for holding multiple positions, performing duties'"), 	НСтр("ru = 'Совмещение';
																													|en = 'Secondary employment'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата до среднего заработка';
										|en = 'Supplement to average earnings'"), 								НСтр("ru = 'Допл. до среднего';
																													|en = 'Supplement to average'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за дни болезни';
										|en = 'Sick pay supplement'"), 									НСтр("ru = 'Допл. за дни болезни';
																												|en = 'Sick pay supplement'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Больничный';
										|en = 'Sick leave '"), 												НСтр("ru = 'Больничный';
																												|en = 'Sick leave '"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Больничный за счет работодателя';
										|en = 'Sick leave at the expense of the employer'"), 							НСтр("ru = 'Больн. (работодат.)';
																													|en = 'Sick leave (employer)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск по беременности и родам';
										|en = 'Maternity leave'"), 							НСтр("ru = 'Отп. по берем.';
																												|en = 'Parental leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Больничный при травме на производстве';
										|en = 'Sick leave on the incidence of industrial injury '"), 						НСтр("ru = 'Больн. при травме';
																													|en = 'Sick leave on the incidence of injury'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Больничный при профзаболевании';
										|en = 'Sick leave due to occupational disease'"), 							НСтр("ru = 'Больн. при профзаб.';
																												|en = 'Sick leave occupat. disease'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отсутствие по болезни';
										|en = 'Sick leave'"), 										НСтр("ru = 'Неявка по болезни';
																													|en = 'Sickness absence'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Неоплачиваемые дни отпуска по беременности и родам';
										|en = 'Unpaid days of maternity leave'"), 		НСтр("ru = 'Отп. по берем. без опл.';
																												|en = 'Unpaid maternity leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Компенсация при увольнении (выходное пособие)';
										|en = 'Compensation on termination of employment (severance pay)'"), 				НСтр("ru = 'Выходн. пособие';
																													|en = 'Severance pay'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Командировка';
										|en = 'Business trip'"), 												НСтр("ru = 'Командировка';
																													|en = 'Business trip'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск без оплаты согласно ТК РФ';
										|en = 'Unpaid leave according to the Labor Code of the Russian Federation'"), 							НСтр("ru = 'Отп. без опл.';
																													|en = 'Unpaid leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск за свой счет';
										|en = 'Unpaid leave'"), 										НСтр("ru = 'Отп. за свой счет';
																													|en = 'Unpaid leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Дополнительный учебный отпуск (оплачиваемый)';
										|en = 'Excess study leave (paid)'"), 				НСтр("ru = 'Учебн. отп.';
																													|en = 'St. leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Дополнительный учебный отпуск без оплаты';
										|en = 'Excess study leave (unpaid)'"), 					НСтр("ru = 'Учебн. отп. без опл.';
																													|en = 'Study leave (unpaid)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Дополнительный отпуск пострадавшим в аварии на ЧАЭС';
										|en = 'Excess leaves for citizens exposed to radiation at the Chernobyl NPP'"), 		НСтр("ru = 'Доп. отп. ЧАЭС';
																													|en = 'Additional leave for harmed in disaster at the Chernobyl NPP'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отсутствие по невыясненной причине';
										|en = 'Absence due to unknown reason'"), 						НСтр("ru = 'Неявка';
																												|en = 'Non-attendance'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Прогул';
										|en = 'Absenteeism'"), 													НСтр("ru = 'Прогул';
																												|en = 'Absenteeism'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Простой по вине работника';
										|en = 'Downtime due to the employee fault'"), 									НСтр("ru = 'Простой без опл.';
																													|en = 'Downtime without paym.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата простоя по независящим от работодателя причинам';
										|en = 'Payment for downtime beyond the employer control'"), 	НСтр("ru = 'Простой незав. прич.';
																												|en = 'Downtime nobod. fault'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата простоя по среднему заработку';
										|en = 'Payment for downtime by average earnings'"), 						НСтр("ru = 'Простой по средн.';
																													|en = 'Downtime on average.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Материальная помощь';
										|en = 'Support payment'"), 										НСтр("ru = 'Мат. пом.';
																													|en = 'Supp. paym.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Материальная помощь при рождении ребенка';
										|en = 'Childbirth support payments '"), 					НСтр("ru = 'Мат. пом. при рожд. реб.';
																													|en = 'Childbirth supp. paym.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Материальная помощь, не облагаемая налогами';
										|en = 'Support payments not subject to taxes'"), 				НСтр("ru = 'Мат. пом. не обл. нал.';
																													|en = 'Supp. paym. not subj. to tax.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Материальная помощь при отпуске';
										|en = 'Leave allowance'"), 							НСтр("ru = 'Мат. пом. при отп.';
																													|en = 'Leave allow.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата за дни сдачи крови и ее компонентов';
										|en = 'Payment for blood donation days'"), 				НСтр("ru = 'Донорство';
																												|en = 'Donation'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Единовременное начисление';
										|en = 'One-time accrual'"), 									НСтр("ru = 'Единовр. начисл.';
																													|en = 'Non-recurr. accruals'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Пособие по уходу за ребенком до полутора лет';
										|en = 'Child care allowance for children under 1.5 years old'"), 				НСтр("ru = 'Уход за реб. до 1,5';
																													|en = 'Care for a child under the age of 1,5'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Пособие по уходу за ребенком до трех лет';
										|en = 'Child care allowance for children under 3 years old'"), 					НСтр("ru = 'Уход за реб. до 3';
																													|en = 'Care for child under the age of 3'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Сдельный заработок (для работающих по окладу)';
										|en = 'Piecework earnings (for the employees working with base salary)'"), 				НСтр("ru = 'Сдельно';
																													|en = 'By piece'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата дней ухода за детьми-инвалидами';
										|en = 'Payment for disabled child care days '"), 					НСтр("ru = 'Уход за детьми-инв.';
																												|en = 'Disabled child care'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск основной';
										|en = 'Basic leave'"), 											НСтр("ru = 'Отпуск';
																													|en = 'Leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск лицам, работающим в районах Крайнего Севера';
										|en = 'Leave for persons working in Far North Districts'"), 		НСтр("ru = 'Отп. северн.';
																												|en = 'North. leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка за вредные условия труда';
										|en = 'Hazard pay '"), 							НСтр("ru = 'Вредность';
																													|en = 'Harmful conditions'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка за выслугу лет';
										|en = 'Seniority pay'"), 									НСтр("ru = 'Выслуга лет';
																													|en = 'Seniority'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата в натуральной форме';
										|en = 'Payment in kind'"), 								НСтр("ru = 'Оплата в натур. форме';
																												|en = 'Paym. in kind'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Натуральный доход';
										|en = 'Income in kind'"), 											НСтр("ru = 'Натур. доход';
																													|en = 'Income in kind'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Районный коэффициент';
										|en = 'Regional factor'"), 										НСтр("ru = 'Районн. коэфф.';
																													|en = 'Regional. coeff.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Северная надбавка';
										|en = 'Northern allowance'"), 											НСтр("ru = 'Северн. надб.';
																													|en = 'North. allowance'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата по результатам оценки эффективности';
										|en = 'Extra pay based on efficiency evaluation results'"), 				НСтр("ru = 'Допл. за эффективн.';
																													|en = 'Extra pay for effect.'"));
	
	// Премии
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия ежемесячная процентом';
										|en = 'Monthly bonus as a percent'"), 								НСтр("ru = 'Премия мес.';
																													|en = 'Month. bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия ежемесячная суммой';
										|en = 'Monthly bonus as an amount'"), 									НСтр("ru = 'Премия мес.';
																													|en = 'Month. bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия за год (процентом)';
										|en = 'Year-end bonus (as percent)'"), 									НСтр("ru = 'Премия год.';
																													|en = 'Year-end bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия за год (суммой)';
										|en = 'Year-end bonus (as an amount)'"), 									НСтр("ru = 'Премия год.';
																												|en = 'Year-end bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия за квартал (процентом)';
										|en = 'Quarterly bonus (as a percent)'"), 								НСтр("ru = 'Премия кварт.';
																													|en = 'Quart. bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия за квартал (суммой)';
										|en = 'Quarterly bonus (as amount)'"), 								НСтр("ru = 'Премия кварт.';
																												|en = 'Quart. bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия разовая (процентом)';
										|en = 'One-off bonus (as a percent)'"), 								НСтр("ru = 'Премия разов.';
																												|en = 'One-off bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия разовая (суммой)';
										|en = 'One-off bonus (as an amount)'"), 									НСтр("ru = 'Премия разов.';
																													|en = 'One-off bonus'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за выручку от реализации';
										|en = 'Extra pay for revenue from sales'"), 							НСтр("ru = 'Допл. от реализ.';
																													|en = 'Extra pay from sold'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за выполнение плана';
										|en = 'Plan fulfillment supplement'"), 								НСтр("ru = 'Допл. за выполн. плана';
																													|en = 'Plan fulfillment supplement'"));
	
	// Государственные и муниципальные служащие.
	КраткиеНаименования.Вставить(НСтр("ru = 'Должностной оклад';
										|en = 'Base salary'"), 											НСтр("ru = 'Оклад';
																													|en = 'Base salary'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Единое денежное вознаграждение';
										|en = 'Standard monetary remuneration '"), 							НСтр("ru = 'Денежн. вознагр.';
																												|en = 'Monet. remunerat.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оклад за классный чин';
										|en = 'Base salary for civil service grade'"), 										НСтр("ru = 'Оклад за классн. чин';
																													|en = 'Base salary for civil service grade'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка к окладу за классный чин';
										|en = 'Standard bonus to base salary for civil service grade'"), 							НСтр("ru = 'Надб. за классн. чин';
																													|en = 'Standard bonus for civil service grade'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Ежемесячная квалификационная надбавка';
										|en = 'Monthly qualification pay'"), 						НСтр("ru = 'Квалиф. надб.';
																													|en = 'Qualif. pay'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка за выслугу лет на государственной службе';
										|en = 'Public service seniority pay'"), 			НСтр("ru = 'Выслуга лет';
																													|en = 'Seniority'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка за выслугу лет на муниципальной службе';
										|en = 'Municipal service seniority pay'"), 			НСтр("ru = 'Выслуга лет';
																													|en = 'Seniority'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка за особые условия службы';
										|en = 'Special working conditions pay'"), 							НСтр("ru = 'Особые усл.';
																													|en = 'Special cond.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Надбавка за секретность';
										|en = 'Secrecy pay'"), 									НСтр("ru = 'Секретность';
																													|en = 'Secrecy'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Ежемесячное денежное поощрение';
										|en = 'Monthly bonus'"), 							НСтр("ru = 'Денежн. поощр.';
																												|en = 'Monet. promot.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Премия за выполнение особо важных и сложных заданий';
										|en = 'Bonus for particularly important and difficult jobs'"), 		НСтр("ru = 'Особ. задания';
																													|en = 'Spec. jobs'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Единовременная выплата к отпуску госслужащего';
										|en = 'One-off payment for a public service employee leave'"), 				НСтр("ru = 'Выпл. к отп.';
																													|en = 'Payment for leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Единовременная выплата к отпуску муницип. служащего';
										|en = 'One-off payment for municipal employee leave'"), 		НСтр("ru = 'Выпл. к отп.';
																													|en = 'Payment for leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Материальная помощь к отпуску госслужащего';
										|en = 'Leave allowance for a public service employee'"), 				НСтр("ru = 'Матпом. при отп.';
																												|en = 'Supp. payment on send.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Командировка (денежное содержание)';
										|en = 'Business trip (monetary pay)'"), 						НСтр("ru = 'Командировка';
																												|en = 'Business trip'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Сохраняемое денежное содержание';
										|en = 'Retained monetary pay'"), 							НСтр("ru = 'Денежн. содерж.';
																													|en = 'Mon. pay'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Компенсация при увольнении (месячное денежное содержание)';
										|en = 'Compensation on termination of employment (monthly monetary pay)'"), 	НСтр("ru = 'Выходн. пособие';
																													|en = 'Severance pay'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Дополнительный учебный отпуск (сохр. денежное содержание)';
										|en = 'Excess study leave (retained monetary pay)'"), 	НСтр("ru = 'Учебн. отп.';
																													|en = 'St. leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск основной (сохр. ден. содержание)';
										|en = 'Basic leave (retain. monet. pay)'"), 					НСтр("ru = 'Отпуск';
																													|en = 'Leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск за выслугу лет на государственной службе';
										|en = 'Leave for long public service'"), 			НСтр("ru = 'Отп. (выслуга лет)';
																													|en = 'Leave (seniority)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск за выслугу лет на муниципальной службе';
										|en = 'Leave for long municipal service'"), 				НСтр("ru = 'Отп. (выслуга лет)';
																													|en = 'Leave (seniority)'"));
	
	// Учет времени сотрудников в часах.
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата по окладу (по часам)';
										|en = 'Payment by base salary (hours)'"), 								НСтр("ru = 'Оклад (по часам)';
																													|en = 'Base salary (hourly)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата по часовому тарифу';
										|en = 'Payment by hourly tariff '"), 									НСтр("ru = 'Часовой тариф';
																													|en = 'Hour tariff '"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Сдельный заработок (для работающих по часовому тарифу)';
										|en = 'Piecework earnings (for the employees working as per hour tariff)'"), 	НСтр("ru = 'Сдельно';
																												|en = 'By piece'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за работу в ночное время';
										|en = 'Night shift pay'"), 							НСтр("ru = 'Ночные часы';
																													|en = 'Night hours'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за работу в праздничные дни (ночное время)';
										|en = 'Holiday pay (night shift)'"), 		НСтр("ru = 'Ночные часы (праздн.)';
																												|en = 'Night hours (pub. holidays)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за работу в вечернее время';
										|en = 'Extra pay for evening work'"), 						НСтр("ru = 'Вечерние часы';
																												|en = 'Evening hours'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за работу в праздничные дни (вечернее время)';
										|en = 'Holiday pay (evening hours)'"), 		НСтр("ru = 'Вечерние часы (праздн.)';
																													|en = 'Evening hours (public holidays)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата времени кормления ребенка';
										|en = 'Breastfeeding time payment'"), 							НСтр("ru = 'Кормл. ребенка';
																													|en = 'Nursing'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата сверхурочных часов';
										|en = 'Overtime compensation'"), 									НСтр("ru = 'Сверхурочно';
																													|en = 'Overtime'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата работы в праздничные и выходные дни';
										|en = 'Payment for work at weekends and public holidays'"), 				НСтр("ru = 'Праздн. и выходн.';
																												|en = 'Public holidays and weekends'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Доплата за работу в праздничные дни (дневное время)';
										|en = 'Holiday pay (daytime)'"), 		НСтр("ru = 'Праздн. и выходн.';
																													|en = 'Public holidays and weekends'"));
	
	// Внутрисменные отклонения
	КраткиеНаименования.Вставить(НСтр("ru = 'Командировка (внутрисменная)';
										|en = 'Business trip (part-shift)'"), 								НСтр("ru = 'Командировка';
																													|en = 'Business trip'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата за дни сдачи крови и ее компонентов (внутрисменная)';
										|en = 'Payment for blood donation days (part-shift)'"), НСтр("ru = 'Донорство';
																												|en = 'Donation'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск без оплаты согласно ТК РФ (внутрисменный)';
										|en = 'Unpaid leave according to the Labor Code of the Russian Federation (part-shift)'"), 			НСтр("ru = 'Отп. без опл.';
																													|en = 'Unpaid leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отпуск за свой счет (внутрисменный)';
										|en = 'Unpaid leave (part-shift)'"), 						НСтр("ru = 'Отп. за свой счет';
																													|en = 'Unpaid leave'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Дополнительный учебный отпуск без оплаты (внутрисменный)';
										|en = 'Unpaid excess study leave (part-shift) '"), 	НСтр("ru = 'Учебн. отп. без опл';
																													|en = 'Study leave (unpaid)'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Отсутствие по невыясненной причине (внутрисменное)';
										|en = 'Absence due to unknown reason (part-shift)'"), 		НСтр("ru = 'Неявка';
																												|en = 'Non-attendance'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Внутрисменный прогул';
										|en = 'Part-shift absenteeism'"), 										НСтр("ru = 'Прогул';
																													|en = 'Absenteeism'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Внутрисменный простой по вине работника';
										|en = 'Part-shift downtime due to the employee fault'"), 					НСтр("ru = 'Простой без опл.';
																													|en = 'Downtime without paym.'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата часового простоя от оклада по часам';
										|en = 'Hourly downtime payment based on the hourly cost of base salary'"), 				НСтр("ru = 'Простой';
																												|en = 'Simple'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Оплата часового простоя по среднему заработку';
										|en = 'Payment for hourly downtime by average earnings'"), 				НСтр("ru = 'Простой';
																													|en = 'Simple'"));
	...
	КраткиеНаименования = Новый Соответствие;
	КраткиеНаименования.Вставить(НСтр("ru = 'Профсоюзные взносы';
										|en = 'Labor union contributions'"), 					НСтр("ru = 'Профвзносы';
																							|en = 'Union contributions'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Удержание по исполнительному документу';
										|en = 'Deduction based on enforcement document'"), НСтр("ru = 'Исп. лист';
																							|en = 'Enforc. order'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Вознаграждение платежного агента';
										|en = 'Remuneration of paying agent'"), 		НСтр("ru = 'Возн. плат. агента';
																								|en = 'Remuner. of paying agent'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Добровольные страховые взносы';
										|en = 'Voluntary insurance contributions'"), 			НСтр("ru = 'Добр. страх. взносы';
																								|en = 'Volunt. insur. contributions'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Добровольные взносы в НПФ';
										|en = 'Voluntary contributions to NPF '"), 				НСтр("ru = 'Добр. взносы НПФ';
																								|en = 'Volunt. contributions to NPF'"));
	КраткиеНаименования.Вставить(НСтр("ru = 'Удержание в счет возмещения ущерба';
										|en = 'Deduction against damage compensation '"), 	НСтр("ru = 'Возмещ. ущерба';
																							|en = 'Damage compensat.'"));
...
Процедура СоздатьНачислениеПоказателиДниОплаченныеДоНачалаЭксплуатации() Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка создания начисления «Дни, оплаченные до начала эксплуатации»';
				|en = 'An error occurred while creating the ""Days paid before operation start"" accrual'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.ПланыВидовРасчета.Начисления, , 
			НСтр("ru = 'Не удалось создать начисление «Дни, оплаченные до начала эксплуатации» из-за предположительно неверной (зацикленной) настройки плана видов расчета.';
				|en = 'Cannot create the ""Days paid before operation start"" accrual as setting of chart of calculation types may be incorrect (looped).'", 
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецПопытки;
...
Функция ИмяСобытияПодборНачислений()
	Возврат НСтр("ru = 'Подбор начислений';
				|en = 'Select accruals'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции
...
Процедура УточнитьФормулыНачисленийЗависящихОтСреднегоЗаработка(ПараметрыОбновления = Неопределено) Экспорт 
	...
	Запрос.УстановитьПараметр("УчитыватьМРОТ", ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.УчитыватьМРОТ"));
