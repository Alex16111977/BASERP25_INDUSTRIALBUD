...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Элементы.НалоговыйИБухгалтерскийУчет.Видимость Тогда
		Если ОблагаетсяНДФЛ = 1 И Не ЗначениеЗаполнено(Объект.КодДоходаНДФЛ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Код дохода НДФЛ не заполнен.';
		...
		Если ОблагаетсяНДФЛ = 1 И Не ЗначениеЗаполнено(Объект.КатегорияДохода) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Категория дохода НДФЛ не заполнена.';
		...
		Если ВключаетсяВРасходыПоСт255НКРФ = 1 И Не ЗначениеЗаполнено(Объект.ВидНачисленияДляНУ) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Вид расхода по ст. 255 НК РФ не заполнен.';
...
Функция ИзменитьСвойстваНачисленияОплачиваемогоОтпускаЧАЭС(СвойстваПоКатегориям, КатегорияНачисления, ВидОтпуска)
	...
	ВидОтпускаОплачиваемыйЧАЭС = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.ОтпускПострадавшимВАварииЧАЭСОплачиваемый");
	...
	Если ВидОтпуска = ВидОтпускаОплачиваемыйЧАЭС Тогда		
		ВидыДоходовНеОблагаетсяЦеликом = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыДоходовПоСтраховымВзносам.НеОблагаетсяЦеликом");
		...
		СвойстваНачисления.ОбозначениеВТабелеУчетаРабочегоВремени = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ДополнительныйОтпуск");
...
Процедура ОбработатьИзменениеВидаОтпускаНаСервере()
	...
	Если ИзменитьСвойстваНачисленияОплачиваемогоОтпускаЧАЭС(ЭтаФорма.СвойстваПоКатегориям,Объект.КатегорияНачисленияИлиНеоплаченногоВремени,Объект.ВидОтпуска) Тогда
		ЗаполнитьСведенияОКатегориях();
		ОбработатьИзменениеКатегорииНачисленияНаСервере();	
	КонецЕсли;
...
Функция ОписаниеСпособаРасчета(КатегорияНачисления)
	...
	ИначеЕсли КатегорияНачисления = ПредопределенноеЗначение("Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ДоначислениеДоУправленческогоУчета") Тогда
		Возврат НСтр("ru = 'Результат рассчитывается как разница между начисленным в регламентированном и управленческом учетах.';
					|en = 'Result is calculated as the difference between the accrued in compliance and management accounting.'");
...
Процедура УстановитьВидимостьПредупрежденияОСуществующихНачислениях()
	...
	Если НачисленияКатегории.Количество() > 0 Тогда
		...
		Текст = НСтр("ru = 'Начисления с таким назначением уже существуют в программе. Возможно, создание еще одного такого начисления будет избыточным.';
					|en = 'Accruals with this purpose already exist in the application. Another accrual may be excessive.'");
