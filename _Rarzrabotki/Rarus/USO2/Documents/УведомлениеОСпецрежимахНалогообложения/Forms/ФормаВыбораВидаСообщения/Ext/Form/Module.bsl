...
Процедура ЗаполнитьТаблицуКНД()
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не удалось сформировать таблицу кодов КНД';
																|en = 'Не удалось сформировать таблицу кодов КНД'"));
	КонецПопытки;
...
Процедура СоздатьПоКНД(Команда)
	...
	Если СтрокиОтчетов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Отчет с КНД ';
																|en = 'Отчет с КНД '") + СокрЛП(КНД) + НСтр("ru = ' не поддерживается';
	...
	Если Не ЗначениеЗаполнено(Организация) И СозданиеСообщения Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Укажите организацию';
				|en = 'Укажите организацию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),, "Организация");
		Возврат;
	КонецЕсли;
