...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//++ НЕ УТКА
	УправлениеПроизводством.ЗаполнитьФлагПередатьДавальцуПоНазначениюВКоллекции(ВыходныеИзделия);
	УправлениеПроизводством.ЗаполнитьФлагПередатьДавальцуПоНазначениюВКоллекции(ПобочныеИзделия);
	//-- НЕ УТКА
...
Функция ТаблицаРеквизитовВидовЗапасовПоУмолчаниюДляПроведения()
	...
	Запрос.Текст =
		...
		|	ПО КлючиАналитики.Ссылка = ТаблицаТоваров.АналитикаУчетаНоменклатуры
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК СпрНазначения
	...
	Для Каждого ТекИзделие Из ВыходныеИзделия Цикл
		Если (ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям
			...
			ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% списка ""%ПредставлениеТЧ%"" не заполнено назначение.';
									|en = 'Assignment is not filled in line %НомерСтроки% of the ""%ПредставлениеТЧ%"" list.'");
