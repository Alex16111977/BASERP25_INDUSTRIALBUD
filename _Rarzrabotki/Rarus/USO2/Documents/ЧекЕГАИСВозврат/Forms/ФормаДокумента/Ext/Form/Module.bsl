...
Процедура ОткрытьПодборСерий(Текст = "", СтандартнаяОбработка)
	...
	ИнтеграцияИСКлиент.ОткрытьПодборСерий(
		ЭтаФорма, ПараметрыУказанияСерий, Текст, Неопределено);
...
Процедура ОбработатьДанныеТСДНаСервере(ДанныеПартионнойПродукции, КэшированныеЗначения)
	...
	Для Каждого МаркируемаяПродукция Из ЗагрузкаДанныхТСД.МаркируемаяПродукция Цикл
		...
		РезультатОбработкиШтрихкода = Документы.ЧекЕГАИСВозврат.ОбработатьДанныеШтрихкодаМаркируемойПродукции(ЭтотОбъект, 
	...
	Если ЗагрузкаДанныхТСД.Упаковки.Количество() > 0 Тогда
		...
		РезультатОбработкиШтрихкода = Документы.ЧекЕГАИСВозврат.ОбработатьДанныеШтрихкодаУпаковки(
			ЭтотОбъект,
			ДанныеШтрихкода, 
			ВложенныеШтрихкоды, 
			ПараметрыСканирования);
...
Процедура Подключаемый_ПослеОбработкиШтрихкодаТСД()
	...
	Если ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента <> ЗагрузкаДанныхТСД.МаркируемаяПродукцияДопУказание.Количество() Тогда
