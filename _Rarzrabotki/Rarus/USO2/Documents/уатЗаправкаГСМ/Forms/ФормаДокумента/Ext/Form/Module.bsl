...
Процедура ЗаправкиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
			...
			ТекСтрока.ПластиковаяКарта = уатОбщегоНазначения.уатПластиковаяКартаТСВодителя(ТекСтрока.ТС, ТекСтрока.Водитель, АЗС, ТекСтрока.Дата);
	...
	Если Объект.ВидДвиженияГСМ = ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаПластиковаяКарта")
		...
		Если ЗначениеЗаполнено(ТекСтрока.ТС) и ЗначениеЗаполнено(ТекСтрока.ПутевойЛист)  Тогда
			...
			Иначе
				...
				ТекСтрока.ПластиковаяКарта  = уатОбщегоНазначения.уатПластиковаяКартаТСВодителя(ТС,ТекСтрока.Водитель, АЗС, ТекСтрока.Дата);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	ОбновитьСписокВыбораЭкипажа();
	...
	Иначе
		Если Объект.ВидДвиженияГСМ <> ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаТалоны") Тогда
			...
			Если Объект.ВидДвиженияГСМ = ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаПластиковаяКарта")
				ИЛИ Объект.ВидДвиженияГСМ = ПредопределенноеЗначение("Перечисление.уатВидыДвиженияГСМ.ЗаправкаПластиковаяКартаСклад") Тогда
				АЗС = ?(ЗначениеЗаполнено(Объект.АЗС), Объект.АЗС, Неопределено);
				ТекСтрокаЗаправки.ПластиковаяКарта = уатОбщегоНазначения.уатПластиковаяКартаТСВодителя(мТС,	ТекСтрокаЗаправки.Водитель, АЗС, ТекСтрокаЗаправки.Дата);
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
		...
		Для Каждого ТекСтрока Из ТаблицаЗаправок Цикл
			...
			Если ЗначениеЗаполнено(ТекСтрока.ТС) Тогда
				...
				Если НЕ ПустыеПЛ.Количество() Тогда
					...
					ТестСообщенияТС = СтрШаблон("Заправка ""%1"" от %2. Путевые листы не найдены!", ТекСтрока.ТС, ТекСтрока.Дата);
...
Процедура УстановитьДоступность()
	...
	Элементы.ТекТС.ТолькоПросмотр                     = мТолькоПросмотр;
	Элементы.Водитель.ТолькоПросмотр                  = мТолькоПросмотр;
	Элементы.Партнер.ТолькоПросмотр                   = мТолькоПросмотр;
	Элементы.Соглашение.ТолькоПросмотр                = мТолькоПросмотр;
	...
	Элементы.Всего.ТолькоПросмотр                     = мТолькоПросмотр;
	//Элементы.Валюта.ТолькоПросмотр                    = мТолькоПросмотр;
	Элементы.ВидЦен.ТолькоПросмотр                    = мТолькоПросмотр;
	...
	Элементы.ГруппаНДС.ТолькоПросмотр                 = мТолькоПросмотр;
	...
	Для Каждого ТекСтрока Из ТаблицаТС Цикл
		...
		Если ТекСтрока.Количество > ОбъемБака Тогда
			ТекстПревышение = ТекстПревышение + "
				|Превышение уровня топлива в баке ТС """ + Строка(ТекСтрока.ТС) + """";
			Продолжить;
		КонецЕсли;
...
Функция СтруктураОтборТС()
	...
	Если ЗначениеЗаполнено(Объект.ПодразделениеОрганизации) Тогда
		СтруктураОтбор.Вставить("уатПодразделение", Объект.ПодразделениеОрганизации);
	КонецЕсли;
...
Процедура КонтрольЗаполненияСтавкиНДС(Отказ, ВызовПередЗакрытиемФормы = Ложь)
	Если Объект.ВидДвиженияГСМ = Перечисления.уатВидыДвиженияГСМ.ЗаправкаСклад
		ИЛИ Объект.ВидДвиженияГСМ = Перечисления.уатВидыДвиженияГСМ.ЗаправкаПластиковаяКартаСклад
		ИЛИ Объект.ВидДвиженияГСМ = Перечисления.уатВидыДвиженияГСМ.ЗаправкаТалоны Тогда
		Возврат;
	КонецЕсли;
