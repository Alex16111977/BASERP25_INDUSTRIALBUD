...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ОписаниеТабличныхЧастей.Вставить("МатериалыИУслуги", Новый Структура("Ресурсы, Дата, ВключитьВМинимум", "Количество,КоличествоУпаковок", "ДатаРасхода", Истина));
	
	РезультатПроверки =
		УниверсальныеМеханизмыПартийИСебестоимости.ПериодыИзмененияТабличныеЧастиДокумента(
			ЭтотОбъект,
	...
	Если Не РезультатПроверки.ОбщийМинимальныйПериодДень = Неопределено Тогда
		ПериодПроверки = Мин(ПериодПроверки, РезультатПроверки.ОбщийМинимальныйПериодДень);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСклада(Отказ)
	...
	ШаблонСообщенияПодразделение = НСтр("ru = 'Не указано подразделение-получатель в строке %1.';
										|en = 'Receiving department is not specified in line %1.'");
...
Процедура ПроверитьДатыМаршрута(Отказ)
	...
	Если Начало > Окончание И Начало <> '000101010000' И Окончание <> '000101010000' Тогда
		ТекстСообщения = НСтр("ru = 'Начало выполнения маршрутного листа должно быть раньше даты окончания выполнения маршрутного листа.';
								|en = 'Start of route sheet completion should be earlier than end of route sheet completion.'");
	...
	ИначеЕсли ФактическоеОкончаниеРаботыКлючевогоРабочегоЦентра <> '000101010000'
		...
		ТекстСообщения = НСтр("ru = 'Дата завершения работы ключевого рабочего центра должна быть раньше даты завершения выполнения маршрутного листа.';
								|en = 'Date of key work center operation end must be earlier than the date of route sheet completion end.'");
		...
		Для ТекИндекс = 0 По Операции.Количество()-1 Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполнения)
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru = '""Время штучное""';
										|en = '""Time per piece""'"), СтрокаОперация.НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполненияЕдИзм)
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru = '""Время штучное ед. изм.""';
										|en = '""Time per piece UOM""'"), СтрокаОперация.НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполнения)
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru = '""Время ПЗ""';
										|en = '""Setup time""'"), СтрокаОперация.НомерСтроки);
			...
			Если НЕ ЗначениеЗаполнено(СтрокаОперация.ВремяВыполненияЕдИзм)
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонТекста, НСтр("ru = '""Время ПЗ ед. изм.""';
										|en = '""Setup time UOM""'"), СтрокаОперация.НомерСтроки);
