	...
	ИначеЕсли Поле = Элементы.ТоварыСумма
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Для редактирования суммы необходимо выбрать вид цены ""<произвольная>""';
				|en = 'To edit the amount, select the ""<arbitrary>"" price type'"),
			Объект.Ссылка,
			ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Товары", ТекущиеДанные.НомерСтроки, "ВидЦены"));
	...
	Если РезультатВопроса = КодВозвратаДиалога.Отмена Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не указана дата окончания периода.';
				|en = 'Period end date is not specified.'"),
			Объект.Ссылка,
			"КонецПериода");
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьЗаголовкиПоВариантуОформленияПродажи()
	...
	Если Объект.ВариантОформленияПродажи = Перечисления.ВариантыОформленияПродажи.АктНаПередачуПрав Тогда
		...
		ЗаголовокТекстом = НСтр("ru = 'Акт на передачу прав между организациями';
								|en = 'Assignment deed between companies'");
...
Процедура ЗаполнитьПоОстаткамКПередачеПослеПроверки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Для заполнения по остаткам должна быть указана дата окончания периода.
			...
			|en = 'To fill in the document by remaining goods, specify the period end date.
			|Fill in the document by remaining goods as of the current date or cancel the action?'");
		
		КнопкиВыбора = Новый СписокЗначений;
		КнопкиВыбора.Добавить(КодВозвратаДиалога.ОК, НСтр("ru = 'Заполнить на текущую дату';
															|en = 'Fill in on the current date'"));
...
Процедура ОбновитьСостояниеЭлементовУправленияИтоговПоРасчетам()
	...
	Иначе
		РасчетыПолучатель = НСтр("ru = 'Оплачено';
								|en = 'Paid'") + ": " + Формат(СуммаОплатыПолучатель, "ЧДЦ=2; ЧН=") + " " + Объект.ВалютаВзаиморасчетов + "  " + ПроцентОплатыПолучатель + "%";
		РасчетыОтправитель = НСтр("ru = 'Получено';
									|en = 'Received'") + ": " + Формат(СуммаОплатыОтправитель, "ЧДЦ=2; ЧН=") + " " + Объект.ВалютаВзаиморасчетов + "  " + ПроцентОплатыОтправитель + "%";
	КонецЕсли;
