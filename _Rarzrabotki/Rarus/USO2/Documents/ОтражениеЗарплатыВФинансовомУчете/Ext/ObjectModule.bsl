...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НачисленнаяЗарплатаИВзносы.Количество() = 0 И НачисленныйНДФЛ.Количество() = 0
		...
		ТекстСообщения = НСтр("ru = 'Списки документа не содержат ни одной строки';
								|en = 'Document lists do not contain any line'");
	...
	Для Каждого Строка Из НачисленнаяЗарплатаИВзносы Цикл
		...
		Если ЗначениеЗаполнено(Строка.СпособОтраженияЗарплатыВБухучете)
			...
			И Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.КомпенсацияЕжегодногоОтпускаОценочныеОбязательства Тогда
			
			ТекстСообщения = НСтр("ru = 'Для отражения компенсации отпуска за счет резервов требуется способ отражения на статьи расходов, а не на производство в строке %1 списка ""Начисления и взносы""';
									|en = 'To record the leave reimbursement at the expense of reserves, use the method of recording for expense items, not for production in the string %1 of the ""Accruals and contributions"" list.'");
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		Если Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательстваИРезервы
			Или Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательства
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодразделения") Тогда
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Подразделение"" в строке %1 списка ""НДФЛ""';
									|en = 'The ""Department"" column is not populated in line %1 of the ""PIT"" list'");
...
Процедура ПроверитьНаличиеБазыРаспределения(Отказ)
	...
	ТекстыСводно.Добавить(НСтр("ru = 'В подразделении %1 не регистрировалась выработка сотрудников (строка %2 списка ""Начисленная зарплата и взносы"")';
								|en = 'In department %1, employee output was not registered (line %2 of the ""Accrued salary and contributions"" list)'"));
	ТекстыСводно.Добавить(НСтр("ru = 'Не задана трудоемкость выполннных работ в подразделении %1 (строка %2 списка ""Начисленная зарплата и взносы"")';
								|en = 'Labor intensity is not specified in the %1 department (line %2 of the Accrued salary and contributions list)'"));
	...
	ТекстыДетально.Добавить(НСтр("ru = 'Выработка сотрудника (-ов) %1 в подразделении ""%2"" не регистрировалась (строка %3 списка ""Начисленная зарплата и взносы"")';
								|en = 'Output of employee(s) %1 in the ""%2"" department has not been registered (line %3 of the ""Accrued salary and contributions"" list)  '"));
	ТекстыДетально.Добавить(НСтр("ru = 'Не задана трудоемкость работ сотрудника (-ов) %1 в подразделении ""%2"" (строка %3 списка ""Начисленная зарплата и взносы"")';
								|en = 'Labor intensity is not specified for employee(s) %1 in the %2 department (line %3 of the Accrued salary and contributions list)'"));
