...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Основание) И ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.уатМаршрутныйЛист") Тогда 
		ЗаполнитьЗаданиеПоДаннымМЛ(Параметры.Основание, 0);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.ТранспортноеСредство) Тогда 
		...
		Если ЗначениеЗаполнено(МодельТС) Тогда
			Если ПолучитьЗначениеРеквизита(МодельТС, "НаличиеСпидометра") Тогда
				Элементы.ДекорацияСпидометр.Заголовок = "Одометр";
...
Процедура ПриСозданииНаСервере_пп()
	...
	Если Не Константы.уатИспользоватьDynafleet.Получить()
		...
		Если Константы.уатИспользоватьМониторинг.Получить() И уатРаботаСРасширениями.ПСМ_ПроверитьПодключение() Тогда
			Элементы.ФормаРассчитатьСпидометрПоGPS.Видимость = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииОрганизации(ВызовПриОткрытии = Ложь)
	...
	Если ПолучитьПраваИНастройкиПользователя(Объект.Организация, "ВестиУчетЗатрат") Тогда
...
Процедура ТранспортноеСредствоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураОтбор.Вставить("уатОрганизация", Объект.Организация);
	СтруктураОтбор.Вставить("уатВидМоделиТС", ПредопределенноеЗначение("Перечисление.уатВидыМоделейТС.Автотранспорт"));
	уатИнтерфейсВводаТС.РеквизитТСНачалоВыбора(Элемент, Объект.ТранспортноеСредство, ДанныеВыбора, СтандартнаяОбработка, СтруктураОтбор);
КонецПроцедуры
...
Процедура РасшифровкаРасходаГСМСервер(ТС, ГСМ, Отказ, СписокНормРасхода, флЭтоТЖ)
	...
	Иначе
		...
		ПараметрыРасчетаГСМ = ДокОбъект.ПолучитьПараметрыРасчетаРасходаГСМ();
...
Функция ПолучитьРасшифровкуРасходаТЖ(ТС, ТЖ)
	...
	тблНормыРасходаТЖ = уатГСМ.ПолучитьНормыРасходаТЖ(ТС, Объект.ДатаВозвращения, ТЖ);
...
Процедура ЗатратыЗаполнитьСервер()
	...
	Если Не СтрокаВидаЗатрат = Неопределено Тогда
		...
		ДокОбъект = РеквизитФормыВЗначение("Объект");
		уатГСМ.ПодготовитьТаблицуСписанияГСМ(ДокОбъект, мТаблицаПоСписаниюГСМ, Истина);
...
Процедура РасчетИтогов(Команда)
	...
	Если ТребуетсяПолучениеДанныхПоGPS(НаименованиеССМ) Тогда 
		ТекстОповещения = "Выполняется получение данных из" + " " + НаименованиеССМ;
...
Процедура ЗаполнитьЗаданиеПоЗаказуНаТСЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = "Табличная часть ""Задание"" заполнена.
...
Процедура ЗаполнитьЗаданиеПоМЛ_ПунктыПогрузкиЗаказы(Команда)
	...
	Если Объект.Задание.Количество() Тогда 
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаполнитьПоМЛПунктыПогрузкиЗаказыОтвет", ЭтотОбъект),
			ТекстВопроса,
			РежимДиалогаВопрос.ДаНетОтмена
		);
		
	Иначе 
		ЗаполнитьЗаданиеПоМЛ(0);
	КонецЕсли;
...
Процедура ЗаполнитьЗаданиеПоМЛ_ОдиночныеПункты(Команда)
	...
	Если Объект.Задание.Количество() Тогда 
		ТекстВопроса = НСтр("en='Do you want to clear the table before filling?';ru='Очистить таблицу перед заполнением?'");
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаполнитьПоМЛОдиночныеПунктыОтвет", ЭтотОбъект),
			ТекстВопроса,
			РежимДиалогаВопрос.ДаНетОтмена
		);
		
	Иначе 
		ЗаполнитьЗаданиеПоМЛ(1);
	КонецЕсли;
...
Процедура ЗаполнитьЗаданиеПоМЛ_ПарныеПункты(Команда)
	...
	Если Объект.Задание.Количество() Тогда 
		ТекстВопроса = НСтр("en='Do you want to clear the table before filling?';ru='Очистить таблицу перед заполнением?'");
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаполнитьПоМЛПарныеПунктыОтвет", ЭтотОбъект),
			ТекстВопроса,
			РежимДиалогаВопрос.ДаНетОтмена
		);
		
	Иначе 
		ЗаполнитьЗаданиеПоМЛ(2);
	КонецЕсли;
...
Процедура ВывестиТаблицуВыработкаТС()
	...
	Для Каждого ТекСтрокаЗадания Из Объект.Задание Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрокаЗадания.Прицеп2) Тогда
			Прицеп2Представление = ТекСтрокаЗадания.Прицеп2;
	...
	Если ЗначениеЗаполнено(Объект.ТранспортноеСредство) Тогда
		мСчетчикКолонок = мСчетчикКолонок + 1;
		ИмяКолонки = "К" + Строка(мСчетчикКолонок);
		ЗаголовокКолонки = Объект.ТранспортноеСредство;
...
Процедура ОбновитьСписокПластиковыхКарт()
	...
	СписокПластикКарт = уатОбщегоНазначения.уатСписокТопливныхКарт(ВладельцыКарт, ТекущиеДанные.Дата).ВыгрузитьЗначения();
	Элементы.ЗаправкиГСМПластиковаяКарта.СписокВыбора.ЗагрузитьЗначения(СписокПластикКарт);
КонецПроцедуры
	...
	Для Каждого СтрокаРасхода Из Объект.РасходГСМ Цикл 
		...
		Если ЗначениеЗаполнено(СтрокаРасхода.ТС) И ЗначениеЗаполнено(ТекОсновноеТопливо) И СтрокаРасхода.ГСМ = ТекОсновноеТопливо Тогда
			...
			Если Константы.уатИспользоватьDynafleet.Получить() И СтрокаРасхода.ТС.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.Dynafleet Тогда
				...
				Запрос.УстановитьПараметр("ДатаНач", Объект.ДатаВыезда);
			...
			ИначеЕсли Константы.уатИспользоватьOmnicomm.Получить() И СтрокаРасхода.ТС.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.Omnicomm Тогда
				...
				ТаблицаСобытий = уатЗащищенныеФункцииСервер_проф.ПолучитьДанныеПоТСOmnicomm(
				Объект.ДатаВыезда, 
			...
			Иначе
				...
				ТС, 
				ТекстОшибки,
				Новый Структура("ЭтотОбъект, ПараметрыРасчетаГСМ, Отказ, ИспользуемаяСистемаGPS", ЭтотОбъект, ПараметрыРасчетаГСМ, Отказ, СтрокаРасхода.ТС.уатИспользуемаяСистемаGPS)
				);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьНадписьПробег()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ТранспортноеСредство) ИЛИ ПолучитьЗначениеРеквизита(ПолучитьЗначениеРеквизита(Объект.ТранспортноеСредство, "уатМодель"), "НаличиеСпидометра") Тогда
		НадписьПробег = "Пробег по одометру: " + мПробегПоОдометру + " км, по данным мониторинга " + Объект.ПробегПоGPS + " км, разница " + мРазница + " км.";
	Иначе
		НадписьПробег = "Наработка по счетчику м/ч: " + мПробегПоОдометру + " ч, по данным мониторинга " + Объект.ПробегПоGPS + " ч, разница " + мРазница + " ч.";
	КонецЕсли;
	...
	Для Каждого СтрокаПрицепа Из Объект.Прицепы Цикл 
		...
		Если Не ЗначениеЗаполнено(СтрокаПрицепа.ТС) Или ПолучитьЗначениеРеквизита(ПолучитьЗначениеРеквизита(СтрокаПрицепа.ТС, "уатМодель"), "НаличиеСпидометра") Тогда
			СтрокаПрицепа.НадписьПробег = "Пробег по одометру: " + пПробегПоОдометру + " км, по данным мониторинга " + СтрокаПрицепа.СчетчикМЧGPS + " км, разница " + пРазница + " км.";
		Иначе
			СтрокаПрицепа.НадписьПробег = "Наработка по счетчику м/ч: " + пПробегПоОдометру + " ч, по данным мониторинга " + СтрокаПрицепа.СчетчикМЧGPS + " ч, разница " + пРазница + " ч.";
		КонецЕсли;
...
Функция ТребуетсяПолучениеДанныхПоGPS(НаименованиеССМ)
	...
	Если ЗначениеЗаполнено(Объект.Подразделение) Тогда 
		СтруктураОбъектовНастроек.Вставить("Подразделение", Объект.Подразделение);
	Иначе
		СтруктураОбъектовНастроек.Вставить("Подразделение", Объект.ТранспортноеСредство.уатПодразделение);
	КонецЕсли;
	...
	Если Константы.уатИспользоватьDynafleet.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.Dynafleet Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
		
	ИначеЕсли Константы.уатИспользоватьOmnicomm.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.Omnicomm Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
		
	ИначеЕсли Константы.уатИспользоватьАвтотрекер.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.Автотрекер Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
		
	ИначеЕсли Константы.уатИспользоватьСКАУТ.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.СКАУТ Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
		
	ИначеЕсли Константы.уатИспользоватьWialon.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.Wialon Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
		
	ИначеЕсли Константы.уатИспользоватьАвтоГРАФ.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS.АвтоГРАФ Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
		
	ИначеЕсли Константы.уатИспользоватьЦСМ.Получить() И Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS = Перечисления.уатСистемаGPS._1СЦСМ Тогда
		ТребуетсяПолучениеДанныхПоGPS = Истина;
		НаименованиеССМ = Строка(Объект.ТранспортноеСредство.уатИспользуемаяСистемаGPS);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьПоМЛПунктыПогрузкиЗаказыОтвет(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьПоМЛОдиночныеПунктыОтвет(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьПоМЛПарныеПунктыОтвет(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьЗаданиеПоМЛ(РежимЗаполнения)
	
	Оповещение = Новый ОписаниеОповещения(
		"ЗаполнитьЗаданиеПоМЛПослеВыбораДокумента", 
		ЭтотОбъект,
		Новый Структура("РежимЗаполнения", РежимЗаполнения)
	);
	
	ОткрытьФорму("Документ.уатМаршрутныйЛист.ФормаВыбора",, ЭтотОбъект,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Процедура ЗаполнитьЗаданиеПоДаннымМЛ(ДокументМЛ, РежимЗаполнения)
	
	МЛПоЗаказам = (ДокументМЛ.ВидОперации = Перечисления.уатВидыОперацийМаршрутныйЛист.ПоЗаказуГрузоотправителя);
	
	Если РежимЗаполнения = 0 И МЛПоЗаказам Тогда 
		ВремяРасстояниеПоЗаказам = Документы.уатМаршрутныйЛист.ПолучитьТаблицуВремениРасстоянияПоЗаказам(ДокументМЛ);
		ТаблицаПлановыхДат       = Документы.уатМаршрутныйЛист.ПолучитьПлановыеДатыПоЗаказам(ДокументМЛ);
	Иначе 
	...
	ТаблЗаполнения.Колонки.Добавить("ВремяПрибытия",        Новый ОписаниеТипов("Дата"));
	ТаблЗаполнения.Колонки.Добавить("ВремяУбытия",          Новый ОписаниеТипов("Дата"));
	...
	Для Каждого ТекСтрока Из ДокументМЛ.Заказы Цикл 
		...
		Если РежимЗаполнения = 0 И Не ТекСтрока.ТипТочкиМаршрута = Перечисления.уатТипыТочекМаршрута.Погрузка Тогда 
			Продолжить;
		КонецЕсли;
		
		Если Не ПредПункт = ТекСтрока.Пункт Или Не ПредЗаказ = ТекЗаказ Тогда 
		...
		Если РежимЗаполнения = 0 И МЛПоЗаказам Тогда 
			НайдСтрока = ТаблицаПлановыхДат.Найти(ТекСтрока.ЗаказГрузоотправителя, "Заказ");
			Если Не НайдСтрока = Неопределено Тогда 
				СтрокаЗаполнения.ВремяПрибытия = НайдСтрока.ПрибытиеПлан;
				СтрокаЗаполнения.ВремяУбытия   = НайдСтрока.УбытиеПлан;
			КонецЕсли;
		...
		Иначе 
			Если РежимЗаполнения = 0 И Не МЛПоЗаказам Тогда
				СтрокаЗаполнения.ВремяПрибытия = ТекСтрока.ПрибытиеПлан;
			Иначе 
				
				НайдСтроки = ТаблицаПлановыхДат.НайтиСтроки(Новый Структура("Пункт", ТекСтрока.Пункт));
				
				Если НайдСтроки.Количество() = 1 Тогда 
					СтрокаЗаполнения.ВремяПрибытия = НайдСтроки[0].ПрибытиеПлан;
					СтрокаЗаполнения.ВремяУбытия   = НайдСтроки[0].УбытиеПлан;
				...
				ИначеЕсли НайдСтроки.Количество() Тогда 
					...
					Если Не СтрокаИндСтроки = Неопределено Тогда 
						...
						СтрокаЗаполнения.ВремяПрибытия = НайдСтроки[ИндСтроки].ПрибытиеПлан;
						СтрокаЗаполнения.ВремяУбытия   = НайдСтроки[ИндСтроки].УбытиеПлан;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Если ТекСтрока.ТипТочкиМаршрута = Перечисления.уатТипыТочекМаршрута.Погрузка Тогда 
			СтрокаЗаполнения.КоличествоТонн = СтрокаЗаполнения.КоличествоТонн + (ТекСтрока.ВесБрутто/ 1000);
			СтрокаЗаполнения.Объем          = СтрокаЗаполнения.Объем + ТекСтрока.Объем;
		КонецЕсли;
	КонецЦикла;
	
	Если РежимЗаполнения = 2 Тогда
		Для Сч = 0 По ТаблЗаполнения.Количество() - 2 Цикл 
			...
			ТекСтрока.ВремяУбытия = СледСтрока.ВремяПрибытия;
		КонецЦикла;
		
		ТаблЗаполнения.Удалить(ТаблЗаполнения[ТаблЗаполнения.Количество() - 1]);
	КонецЕсли;
	...
	Для Каждого ТекСтрока Из ТаблЗаполнения Цикл 
		...
		НоваяСтрока.ВремяПрибытия       = ТекСтрока.ВремяПрибытия;
		НоваяСтрока.ВремяУбытия         = ТекСтрока.ВремяУбытия;
		НоваяСтрока.КоличествоЕздок     = 1;
...
КонецФункции // ПолучитьЗначениеТаблЗаполненияДляПодстановки()
...
Процедура УстановитьДоступностьРассчитанногоДокумента()
	...
	Элементы.ЗатратыТС.Доступность = флДоступность;
	Элементы.ФормаРассчитатьСпидометрПоGPS.Доступность = флДоступность;
