	...
	Если СтандартнаяОбработка Тогда
		...
		Для каждого СтрокаДокумента Из Сотрудники Цикл
			...
			Если НайденныеСтроки = Неопределено Или НайденныеСтроки.Количество() = 0 Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке №%1 указан сотрудник, отсутствующий в первичном документе.';
																							|en = 'The employee who was missing in the source document is specified in line No. %1.'"), СтрокаДокумента.НомерСтроки);
