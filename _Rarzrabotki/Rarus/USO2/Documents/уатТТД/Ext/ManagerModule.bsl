...
Функция ПечатьТранспортнаяНакладная(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ТекущийДокумент.Дата < '20171222' Тогда 
...
Функция ПечатьЭСМ7(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ТекущийДокумент.Стоимость.Количество() = 0 Тогда
			Сообщить(НСтр("ru='Не расчитана стоимость услуг!';en='Cost of services is not calculated.'"));
			Возврат ТабличныйДокумент;
		Иначе
			Если ТекущийДокумент.Стоимость[0].ПутЛист.Пустая() Тогда
				Сообщить(НСтр("ru='Не указан путевой лист на закладке ""Стоимость услуг""!';en='Trip sheet is not specified on tab ""Cost of services"".'"));
				Возврат ТабличныйДокумент;
			КонецЕсли;
		КонецЕсли;
		...
		Обл.Параметры.НомерСправки  = уатОбщегоНазначенияТиповые.уатПолучитьНомерНаПечать(ТекущийДокумент);
		ФорматДаты      			= "ДЛФ='Д'";
		Обл.Параметры.ДатаВыписки 	= Формат(ТекущийДокумент.Дата, ФорматДаты);
		Обл.Параметры.Организация = ТекущийДокумент.Организация;
		Обл.Параметры.ОрганизацияПредставление  	= уатОбщегоНазначенияТиповыеСервер.ОписаниеОрганизации(уатОбщегоНазначенияСервер.СведенияОЮрФизЛице(ТекущийДокумент.Организация, ТекущийДокумент.Дата), "ПолноеНаименование,ЮридическийАдрес,Телефоны");
		Обл.Параметры.МодельТСПредставление = уатОбщегоНазначения.уатПредставлениеМоделиТС(ОснПутевойЛист.ТранспортноеСредство);
		Обл.Параметры.МодельТС = ОснПутевойЛист.ТранспортноеСредство.уатМодель;
		Обл.Параметры.ТранспортноеСредствоГосНомер	= ОснПутевойЛист.ТранспортноеСредство.Код;
		...
		Обл.Параметры.ДатаСостДень 	= Формат(ОснПутевойЛист.ДатаВыезда,"ДФ=дд");
		Обл.Параметры.ДатаСостМесяц = Формат(ОснПутевойЛист.ДатаВыезда,"ДФ=ММ");
		Обл.Параметры.ДатаСостГод 	= Формат(ОснПутевойЛист.ДатаВыезда,"ДФ=гг");
		...
		мЗаказчикПредставление = уатОбщегоНазначенияТиповыеСервер.ОписаниеОрганизации(
		уатОбщегоНазначенияСервер.СведенияОЮрФизЛице(ТекущийДокумент.Контрагент, ТекущийДокумент.Дата), "ПолноеНаименование,ЮридическийАдрес,Телефоны");
		...
		Обл.Параметры.Всего = ?(ТекущийДокумент.СуммаВключаетНДС, СуммаИтого, СуммаИтого + СуммаНДСИтого);
		...
		Руководители = уатИнтерфейсВзаимодействияСERPСервер.уатПолучитьОтветственныеЛицаОрганизации(ТекущийДокумент.Организация, ТекущийДокумент.Дата);
		
		Обл.Параметры.ФИОРуководителя = Руководители.РуководительНаименование;
