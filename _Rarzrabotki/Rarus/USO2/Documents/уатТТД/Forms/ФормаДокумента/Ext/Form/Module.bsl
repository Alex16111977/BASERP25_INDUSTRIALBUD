...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредКурсДокумента = Объект.КурсВзаиморасчетов;
	...
	ИспользоватьСоглашения = уатИнтерфейсВзаимодействияСERPВызовСервера.уатЗначениеФункциональнойОпции("ИспользоватьСоглашенияСКлиентами");
...
Процедура ПриИзмененииПараметровНДСиКурса()
	Если Объект.УчитыватьНДС <> ПредУчитыватьНДС Тогда
		ЗаполнитьСтавкуНДСПоНалогообложениеНДС();		
	КонецЕсли;
	Если Объект.КурсВзаиморасчетов <> ПредКурсДокумента Тогда
		уатОбщегоНазначенияКлиент.ПересчитатьЦеныТабличнойЧастиПоВалюте(ЭтаФорма, Объект.ВалютаДокумента, "Стоимость");
	КонецЕсли;
...
Процедура ПриИзмененииДоговора(Знач Оповещение, ДоговорПередИзменением)
	...
	Если (ЗначениеЗаполнено(Объект.ДоговорКонтрагента)
		...
		ПредВалюта = Объект.ВалютаДокумента;
		...
		Для Каждого ТекСтрока Из Объект.Стоимость Цикл
			ТекСтрока.Цена = уатОбщегоНазначенияТиповые.ПересчитатьИзВалютыВВалюту(ТекСтрока.Цена,
				ПредВалюта, Объект.ВалютаДокумента, Объект.Дата, Объект.Дата);
	...
	Для Каждого ТекЕздка Из тблЕздки Цикл
		...
		Если ТекЕздка.НомерЕздки <> 0 Тогда
			...
			Для Каждого ТекСтрокаТЗПромежуточная Из ТЗПромежуточная Цикл
				//в пределах одной ездки количество ездок всегда равно 1 и не суммируется
				ТекСтрокаТЗПромежуточная.КоличествоЕздок = 1;
...
Процедура УстановитьВидимость()
	...
	Элементы.Соглашение.Видимость = ИспользоватьСоглашения;
