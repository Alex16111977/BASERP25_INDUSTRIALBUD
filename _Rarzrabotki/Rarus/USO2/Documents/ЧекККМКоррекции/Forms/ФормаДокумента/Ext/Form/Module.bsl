...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.ЧекККМКоррекции")
		...
		ТекстСообщения = НСтр("ru = 'Копирование документа ""Чек ККМ коррекции"" запрещено!';
								|en = 'Copying of the ""Correction cash receipt"" document is not allowed!'");
...
Функция ПараметрыОперацииФискализацииЧека()
	...
	ПараметрыОперацииФискализацииЧека.ДанныеКоррекции.ТипКоррекции      = ?(Объект.ТипКоррекции = ПредопределенноеЗначение("Перечисление.ТипыЧекаКоррекции.Самостоятельно"), 0, 1);
	...
	Если Результат.ВыполненаОперацияНаУстройстве Тогда
		...
		Если Не Результат.ИзмененныеДанныеЗаписаны Тогда
			...
			ПараметрыФормы.Вставить("ТекстСообщения",
				...
				| Correction cash receipt is issued but not recorded in the system.'"));
...
Функция ЗаписатьФискальнуюОперациюНаСервере(ТребуетсяПовторнаяПопыткаЗаписи, ПараметрыФискализации)
	...
	Попытка
		...
		УспешноЗаписанДокумент = Записать(Новый Структура("РежимЗаписи, РежимПроведения", РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Оперативный));
		Если Не УспешноЗаписанДокумент Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось записать документ: %1';
																							|en = 'Cannot save document:%1'"), Объект.Ссылка);
		КонецЕсли;
...
Процедура ТоварыПередНачаломИзменения(Элемент, Отказ)
	...
	Если ИзменяетсяНоменклатура(Элемент.ТекущийЭлемент.Имя)
		...
		ТекстСообщения = НСтр("ru = 'Корректировка алкогольной и табачной продукции запрещена. По данной номенклатуре можно изменять только ставку НДС.';
								|en = 'Adjustment of alcohol and tobacco products is prohibited. You can only change the VAT rate for this item.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если НЕ РазрешеноИзменятьНоменклатуру(ТекущаяСтрока.Номенклатура) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Корректировка алкогольной и табачной продукции запрещена.';
								|en = 'Adjustment of alcohol and tobacco products is prohibited.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе	
...
Функция РазрешеноИзменятьНоменклатуру(Номенклатура)
	...
	РеквизитыВидаНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "АлкогольнаяПродукция, ТабачнаяПродукция");
	
	Если РеквизитыВидаНоменклатуры.АлкогольнаяПродукция
		ИЛИ РеквизитыВидаНоменклатуры.ТабачнаяПродукция Тогда
			...
			Новый ФорматированнаяСтрока(
				СтрШаблон(
					НСтр("ru = 'Пробит чек коррекции №%1';
						|en = 'Adjustment receipt No.%1 is issued'"),
