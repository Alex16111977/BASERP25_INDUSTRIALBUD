...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ВыборкаПроверки.ЭтоРеализацияКлиентуРеглУчет Тогда
			...
			ТекстОшибки = НСтр("ru = 'У документа %Документ% статус ""%Статус%"" не установлен, т.к. для реализаций с операцией ""Реализация по регл. учету"" статусы не применимы';
								|en = 'Status ""%Статус%"" is not set for document %Документ% because statuses are not available for the sales using transaction ""Sale in compl. accounting""'");
	...
	ИначеЕсли ЗначениеНовогоСтатуса = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате Тогда
		...
		Если ВыборкаПроверки.ЭтоПередачаНаКомиссию Тогда
			...
			ТекстОшибки = НСтр("ru = 'У документа %Документ% статус ""%Статус%"" не установлен, т.к. это передача на комиссию';
								|en = 'Status ""%Статус%"" is not set for document %Документ% as it is a consignment goods transfer'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ВыборкаПроверки.Ссылка);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Статус%", ВыборкаПроверки.ПредставлениеНовогоСтатуса);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ВыборкаПроверки.Ссылка, , , Отказ);
	...
	ИначеЕсли ЗначениеНовогоСтатуса = Перечисления.СтатусыРеализацийТоваровУслуг.ВПути
		И Не ВыборкаПроверки.ЭтоРеализацияБезПереходаПраваСобственности Тогда
			...
			ТекстОшибки = НСтр("ru = 'У документа %Документ% статус ""%Статус%"" не установлен, т.к. этот статус применим
				...
				|en = 'Status ""%Статус%"" is not set for document %Документ% as this status is applied only for sales 
				|with transaction ""Sale (goods on the way)""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ВыборкаПроверки.Ссылка);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Статус%", ВыборкаПроверки.ПредставлениеНовогоСтатуса);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ВыборкаПроверки.Ссылка, , , Отказ);	
	...
	ИначеЕсли ЗначениеНовогоСтатуса = Перечисления.СтатусыРеализацийТоваровУслуг.Отгружено
		...
		И ДатаПереходаПраваСобственности < НачалоДня(ВыборкаПроверки.Дата) Тогда
			...
			ТекстОшибки =  НСтр("ru = 'У документа %Документ% не установлена дата перехода права собственности ""%ДатаПерехода%"", 
				...
				|en = 'Set date ""%ДатаПерехода%"" of ownership right transfer in document %Документ%. 
...
Функция ВыполнитьПроверкиРаспоряжения(Распоряжение, Склад) Экспорт
	...
	Если Статус = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате Тогда
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности Тогда
			...
			|You can enter a goods issue note based on the current reference only in status ""For shipment"", ""On the way"", or ""Sold"".'");
		...
		Иначе
			...
			|en = 'Reference ""%Распоряжение%"" is in status ""%Статус%"".
			|You can enter a goods issue note based on the current reference only in status ""For shipment"" or ""Sold"".'");
...
Процедура УстановитьПараметрЗапросаТипыНалогообложенияНДСОблагаемые(Запрос) Экспорт
	...
	Запрос.УстановитьПараметр("ТипыНалогообложенияНДСОблагаемые", УчетНДСУП.ТипыНалогообложенияНДСНеУчитываетсяВСтоимости());
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеОтгрузкиПоРеализациям") Тогда
		...
		Если КомандаСоздания <> Неопределено Тогда
			КомандаСоздания.Представление = НСтр("ru = 'Акт о расхождениях после реализации клиенту';
												|en = 'Discrepancy certificate after sales to a customer'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ЭтоПартнер
		...
		КомандаПечати.Представление = НСтр("ru = 'Счет на оплату (не выводить скидки)';
											|en = 'Proforma invoice (do not display discounts)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ОтображатьСкидки", Ложь);
	...
	Если НЕ ЭтоПартнер
		...
		КомандаПечати.Представление = НСтр("ru = 'Счет на оплату (выводить скидки)';
											|en = 'Proforma invoice (display discounts)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ОтображатьСкидки", Истина);
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.СертификатыНоменклатуры)
		...
		КомандаПечати.Представление = НСтр("ru = 'Сертификаты (по одному на сертификат)';
											|en = 'Certificates (one to a certificate)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Функция СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ЕстьНДС Тогда
			СтруктураДанныхПодвалНДС.Вставить("НДС", ?(ДанныеПечати.ЦенаВключаетНДС, НСтр("ru = 'В том числе НДС:';
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	Если Не ПодключаемоеОборудованиеУТСервер.ПоддерживаетсяФорматФФД1_0_5ИЛИФФД1_1(ВерсияФФД) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	|	ТабличнаяЧасть.Номенклатура.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	ТабличнаяЧасть.Номенклатура.ТабачнаяПродукция КАК ТабачнаяПродукция
	|ПОМЕСТИТЬ Товары
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК ТабличнаяЧасть
	|ГДЕ
	...
	ТаблицаТоваровСРаспределениемПоМОТП(Товары, ДанныеДляМОТП(ДокументСсылка, МенеджерВременныхТаблиц));
	...
	ДанныеДокументаДляЧека.Заказы = РезультатЗапроса[РезультатЗапроса.Количество() - 3].Выгрузить();
...
Функция ДанныеДляМОТП(ДокументСсылка, МенеджерВременныхТаблиц)
	...
	|	Товары.ТабачнаяПродукция КАК ТабачнаяПродукция,
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Пока КоличествоСтрокМассиваМаркированныхТоваров >= ТекущаяСтрока Цикл
			...
			Если НЕ (МаркированныйТовар.СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар
				И МаркированныйТовар.СтрокаДерева.ГрупповаяТоварнаяУпаковка = Ложь) Тогда
			...
			СтрокаТоварыРазобранные.СуммаСкидки 		= Окр(СтрокаТовары.СуммаСкидки / СтрокаТовары.КоличествоЕдиниц, 2);
			СтрокаТоварыРазобранные.СуммаСНДС 			= Окр(СтрокаТовары.СуммаСНДС / СтрокаТовары.КоличествоЕдиниц, 2);
			СтрокаТоварыРазобранные.Цена	 			= СтрокаТоварыРазобранные.СуммаСНДС - СтрокаТоварыРазобранные.СуммаСкидки;
			
			Если ЗначениеЗаполнено(СтрокаТоварыРазобранные.ЕдиницаИзмерения) И СтрокаТоварыРазобранные.ЕдиницаИзмерения <> СтрокаТоварыРазобранные.Упаковка Тогда
...
Функция ДанныеДляГТД(ДокументСсылка, ЦенаВключаетНДС)
	...
	|	ЕСТЬNULL(СтраныМира.КодАльфа3, НЕОПРЕДЕЛЕНО)                 КАК КодСтраныПроисхождения
	|ИЗ
	...
	|	ЕСТЬNULL(СтраныМира.КодАльфа3, НЕОПРЕДЕЛЕНО)
	|");
...
Процедура ТаблицаТоваровСРаспределениемПоМОТП(Товары, ДанныеДляМОТП)
	
	ТаблицаТоваровСРаспределениемПоМОТП = Товары.СкопироватьКолонки();
	ТаблицаТоваровСРаспределениемПоМОТП.Колонки.Добавить("Штрихкод");
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ТабачнаяПродукция")
			...
			МассивСтрокДляДобавленияВТовары = ДанныеДляМОТП.НайтиСтроки(Новый Структура("НомерСтроки", СтрокаТЧ.НомерСтроки));
		КонецЕсли;	
		
		Для каждого СтрокаДляДобавленияВТовары из МассивСтрокДляДобавленияВТовары Цикл
			СтрокаТоваровСРаспределениемПоМОТП = ТаблицаТоваровСРаспределениемПоМОТП.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТоваровСРаспределениемПоМОТП, СтрокаТЧ);
			
			СтрокаТоваровСРаспределениемПоМОТП.КоличествоУпаковок  = ?(СтрокаДляДобавленияВТовары.КоличествоУпаковок = 0, 1, СтрокаДляДобавленияВТовары.КоличествоУпаковок);
			СтрокаТоваровСРаспределениемПоМОТП.СуммаСНДС = СтрокаДляДобавленияВТовары.СуммаСНДС;
			СтрокаТоваровСРаспределениемПоМОТП.СуммаСкидки = СтрокаДляДобавленияВТовары.СуммаСкидки;
			
			Если ЭтоТабачнаяПродукция И ЗначениеЗаполнено(СтрокаДляДобавленияВТовары.Упаковка) Тогда
				СтрокаТоваровСРаспределениемПоМОТП.УпаковкаНаименование = СтрокаДляДобавленияВТовары.Упаковка;
			КонецЕсли;
			
			Если ЭтоТабачнаяПродукция И ЗначениеЗаполнено(СтрокаДляДобавленияВТовары.Штрихкод) Тогда
				СтрокаТоваровСРаспределениемПоМОТП.Штрихкод = СтрокаДляДобавленияВТовары.Штрихкод;
			КонецЕсли;
		КонецЦикла;
		
	КонецЦикла;
	
	Товары = ТаблицаТоваровСРаспределениемПоМОТП;
...
Процедура ТаблицаТоваровСРаспределениемПоГТД(Товары, ДанныеДляГТД)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		КолонкаКоличестваТаблицыРаспределениемПоГТД = ?(СтрокаТЧ.ТабачнаяПродукция, "КоличествоЕдиниц", "КоличествоУпаковок");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ДелоРодитель.Представление  = НСтр("ru = 'Документы реализации товаров и услуг';
										|en = 'Documents of selling goods and services'");
