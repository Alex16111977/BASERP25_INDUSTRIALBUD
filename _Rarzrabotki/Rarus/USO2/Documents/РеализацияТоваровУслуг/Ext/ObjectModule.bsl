...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если Отказ Тогда
		ТекстСообщения = НСтр("ru = 'Статус ""%Статус%"" не установлен - есть товары, по которым нужно указать серии.';
								|en = 'Status ""%Статус%"" is not set, there are goods by which it is required to specify series.'");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не РеализацияПоЗаказам И Не ПраваПользователяПовтИсп.СозданиеРеализацииТоваровУслугБезЗаказа() Тогда
		...
		ТекстОшибки = НСтр("ru = 'Нет прав на создание реализации товаров и услуг без заказа';
							|en = 'No rights to create sales of goods and services without order'");
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности 
		...
		ТекстОшибки = НСтр("ru = 'Дата перехода права собственности должна быть больше даты документа.';
							|en = 'Date of ownership transfer must be greater than the document date.'");
...
Процедура ЗаполнитьДокументНаОснованииПриобретенияТоваровУслуг(ПриобретениеТоваровУслуг)
	...
	Если Не Шапка.Следующий() Тогда
		...
		ТекстОшибки = НСтр("ru = 'Документ %Документ% не содержит товаров. Ввод на основании документа запрещен.';
							|en = 'Document %Документ% does not have goods. Input based on the document is prohibited.'");
...
Процедура ПроверитьКорректировки(Отказ)
	...
	Если ЕстьКорректировки Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'На основании документа %1 введена корректировка реализации. Отмена проведения невозможна.';
																						|en = 'Sale adjustment is entered based on the %1 document. Cannot cancel posting.'"),Ссылка);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
