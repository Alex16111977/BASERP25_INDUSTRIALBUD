...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Интеграция МОТП
	ПроверкаИПодборПродукцииМОТП.ПослеЗаписиНаСервере(ЭтотОбъект, ПараметрыИнтеграцииФормыПроверкиИПодбора());
	// Конец Интеграция МОТП
			...
			Если Не ЭтоГруппаСкладовИСкладыИспользуютсяВТЧДокументовПродажи(Объект.Склад) Тогда
				...
				Если РеализацияСверхЗаказа Тогда
					...
					ТекстВопроса = НСтр("ru = 'При изменении склада список ""Товары"" необходимо очистить, либо отвязать строки от заказа. Продолжить?';
										|en = 'It is required to clear the	Goods list or unlink lines from the order while changing the warehouse. Continue?'")
				Иначе
					СписокКнопок.Добавить("Продолжить", НСтр("ru = 'Продолжить';
															|en = 'Continue'"));
					ТекстВопроса = НСтр("ru = 'Список ""Товары"" будет очищен. Продолжить?';
										|en = 'The Goods list will be cleared. Continue?'")
				КонецЕсли;
		...
		Если ИспользоватьРеализациюПоНесколькимЗаказам Тогда
			...
			Если Объект.Товары.Количество() = 0 Тогда
				ТекстВопроса = НСтр("ru = 'Список Товары будет перезаполнен остатками неоформленных товаров по заказам. Продолжить?';
									|en = 'The Goods list will be refilled by remaining unregistered goods against orders. Continue?'");
			Иначе
				ТекстВопроса = НСтр("ru = 'Строки в списке Товары будут привязаны к строкам заказов. Продолжить?';
									|en = 'The Goods list lines will be linked with the order lines. Continue?'");
			КонецЕсли;
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ПодключитьОбработчикОжидания("ПрименитьКешПоСтрокеМОТП", 0.5, Истина);
...
Процедура ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакМаркируемаяПродукцияТабачная",
		Новый Структура("Номенклатура","МаркируемаяПродукцияГосИС"));
...
Процедура ЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "Товары", НСтр("ru = 'Товары';
																				|en = 'Goods'")) Тогда
	...
	Если Элементы.Товары.ВыделенныеСтроки <> Неопределено И РеализацияСверхЗаказаПоЗаказу Тогда
		...
		Если СтрокиТабличнойЧасти.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Ручная скидка (наценка) не может быть назначена в строках по заказу.';
										|en = 'Manual discount (markup) cannot be assigned in the order lines.'");
			ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// Проверка и подбор МОТП
	ПроверкаИПодборПродукцииМОТП.ПриСозданииНаСервере(ЭтотОбъект, ПараметрыИнтеграцииФормыПроверкиИПодбора());
	// Конец Проверка и подбор МОТП
	...
	// Проверка и подбор МОТП
	ПроверкаИПодборПродукцииМОТПВызовСервера.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект, ПараметрыИнтеграцииФормыПроверкиИПодбора());
	// Конец Проверка и подбор МОТП
...
Процедура ТоварыПослеУдаленияСервер(НеобходимоОбновитьСтатусыСерий, КэшированныеЗначения)
	...
	//Проверка и подбор МОТП
	ПроверкаИПодборПродукцииМОТПВызовСервера.ПрименитьКешШтрихкодовУпаковок(
		ЭтотОбъект,
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	// Проверка и подбор МОТП
	ПараметрыЗаполненияРеквизитов.Вставить("ЗаполнитьПризнакМаркируемаяПродукцияТабачная",
		Новый Структура("Номенклатура","МаркируемаяПродукцияГосИС"));
	// Проверка и подбор МОТП
	...
	ИначеЕсли Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным
	 ...
	 И ТипЗнч(Объект.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Расчеты = НСтр("ru = 'Оплачено по заказу';
						|en = 'Paid against order'") + ": " + Формат(СуммаОплаты, "ЧДЦ=2; ЧН=") + " " + ВалютаВзаиморасчетовДокумента + "  " + ПроцентОплаты + "%";
	ИначеЕсли Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным
	 И Объект.РеализацияПоЗаказам
	 И ТипЗнч(Объект.ЗаказКлиента) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		Расчеты = НСтр("ru = 'Оплачено по заявке на возврат';
						|en = 'Paid against return request'") + ": " + Формат(СуммаОплаты, "ЧДЦ=2; ЧН=") + " " + ВалютаВзаиморасчетовДокумента + "  " + ПроцентОплаты + "%";
	ИначеЕсли СписокЗаказов.Количество() > 1 Тогда
		Расчеты = НСтр("ru = 'Расчеты по заказам (';
						|en = 'Settlements on orders ('")+ Строка(СписокЗаказов.Количество()) + ")";
	Иначе
		Расчеты = НСтр("ru = 'Оплачено';
						|en = 'Paid'") + ": " + Формат(СуммаОплаты, "ЧДЦ=2; ЧН=") + " " + ВалютаВзаиморасчетовДокумента + "  " + ПроцентОплаты + "%";
	КонецЕсли;
...
Функция ВозможноНазначениеРучнойСкидкиНаценки()
	...
	Если ВозможноНазначениеРучнойСкидкиНаценки И РеализацияСверхЗаказа И Объект.РеализацияПоЗаказам И Объект.Товары.Итог("РасхождениеЗаказ") = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Ручная скидка (наценка) не может быть назначена на строки по заказу.';
									|en = 'Manual discount (markup) cannot be assigned for the order lines.'");
...
Функция ВозможнаОтменаРучныхСкидокНаценок()
	...
	Если ВозможнаОтменаРучныхСкидокНаценок И РеализацияСверхЗаказа И Объект.РеализацияПоЗаказам И Объект.Товары.Итог("РасхождениеЗаказ") = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Ручная скидка (наценка) не может быть отменена в строках по заказу.';
									|en = 'Manual discount (markup) cannot be canceled in the order lines.'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат Ложь;
	Иначе
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыНаПередачуПрав") Тогда
		Элементы.РеквизитыПечатиТОРГ12.Заголовок = 
			НСтр("ru = 'Реквизиты печати ТОРГ-12 / Акта на передачу прав';
				|en = 'TORG-12/Assignment deed print attributes'");
	КонецЕсли;
...
Функция ВыполнитьПроверкиПередИзменениемСтатуса()
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено")
		...
		ТекстСообщения = НСтр("ru = 'На основании документа введена корректировка реализации.
			...
			|en = 'Sale adjustment is entered based on the document.
			|Insufficient rights to create sale adjustment. '");
	...
	ИначеЕсли ЕстьКорректировки И ИспользоватьКорректировкиРеализаций Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'На основании документа введена корректировка реализации.
				...
				|en = 'Sale adjustment is entered based on the document.
				|Changing original document is forbidden.'"),
			СписокКнопок,
			,
			"ВвестиКорректировку");
...
Процедура РеализацияПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если Объект.Товары.Количество() = 0 Тогда
        ТекстВопроса = СтрЗаменить(НСтр("ru = 'Список Товары будет заполнен остатками неоформленных товаров по документу %ЗаказКлиента%. Продолжить?';
										|en = 'The Goods list will be filled in with non-generated goods under the %ЗаказКлиента% document. Continue?'"), "%ЗаказКлиента%", ЗаказКлиента);
    Иначе
...
Процедура СпособМестоДоставкиПеревозчикПриИзменении(Элемент)
	...
	Если КоличествоТранспортныхНакладных > 0
		...
		ТекстВопроса = НСтр("ru = 'По текущему распоряжению имеются оформленные ТТН. Для выбранного способа доставки транспортные накладные можно будет оформить из задания на перевозку. Информация о ТТН будет очищена без возможности возврата.';
							|en = 'There are registered waybills by the current reference. For the selected delivery method waybills can be registered from transportation job. Information about waybills will be cleared with no return.'");
	...
	Если КоличествоТранспортныхНакладных > 0
		...
		ТекстВопроса = НСтр("ru = 'По распоряжению имеются оформленные ТТН. Для выбранного способа доставки транспортные накладные можно будет оформить из текущего документа. Информация о ТТН будет очищена без возможности возврата.';
							|en = 'There are registered waybills by reference. Waybills can be registered from the current document for the selected delivery method. Information about waybill will be cleared with no return.'");
...
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыИнтеграцииФормыПроверкиИПодбора()
	
	ПараметрыИнтеграции = ПроверкаИПодборПродукцииМОТПКлиентСервер.ПараметрыИнтеграцииФормыПроверкиИПодбора();
...
&НаКлиенте
Процедура Подключаемый_ДекорацияСканироватьПроверитьТоварыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ПриЗакрытииФормыПроверкиИПодбораНаСервере(Результат, ДополнительныеПараметры)
	...
	Прочитать();
	ПроверкаИПодборПродукцииМОТП.УправлениеЭлементамиОткрытияФормыПроверкиИПодбора(
		ЭтотОбъект,
		ПараметрыИнтеграцииФормыПроверкиИПодбора());
...
&НаКлиенте
Процедура ПрименитьКешПоСтрокеМОТП()
	...
	НужноПересчитатьКеш = ПроверкаИПодборПродукцииМОТПКлиент.ПрименитьКешПоСтроке(
		ЭтотОбъект,
		ПараметрыИнтеграцииФормыПроверкиИПодбора(),
		ТекущаяСтрока,
		ТоварыКешТекущейСтроки);
	
	Если НужноПересчитатьКеш Тогда
		ПересчитатьКешМаркируемойПродукции();
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПересчитатьКешМаркируемойПродукции()
...
//{АУН}
#Область ПодключаемыеПроцедурыПодсистемыУправлениеНедвижимостью

&НаКлиенте
Процедура Подключаемый_ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	
	ОбработкаОповещения(ИмяСобытия, Параметр, Источник);
	
	Если ИмяСобытия = "АУН_РассчитатьИтоговыеПоказатели" Тогда
		ПродажиКлиентСервер.РассчитатьИтоговыеПоказателиРеализации(ЭтаФорма);
	КонецЕсли;
...
Процедура Подключаемый_ПриИзменении_ВыполнитьПереопределяемоеСобытие(Элемент)
	...
	МодификацияКонфигурацииКлиентПереопределяемый.ВыполнитьПереопределяемоеСобытиеПриИзменении(ЭтаФорма, Элемент);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_НачалоВыбора_ВыполнитьПереопределяемоеСобытие(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	МодификацияКонфигурацииКлиентПереопределяемый.ВыполнитьПереопределяемоеСобытиеНачалоВыбора(ЭтаФорма, Элемент, ДанныеВыбора, СтандартнаяОбработка);
		
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбработкаВыбора_ВыполнитьПереопределяемоеСобытие(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	МодификацияКонфигурацииКлиентПереопределяемый.ВыполнитьПереопределяемоеСобытиеОбработкаВыбора(ЭтаФорма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_АвтоПодбор_ВыполнитьПереопределяемоеСобытие(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка)
	
	МодификацияКонфигурацииКлиентПереопределяемый.ВыполнитьПереопределяемоеСобытиеАвтоПодбор(ЭтаФорма, Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОкончаниеВводаТекста_ВыполнитьПереопределяемоеСобытие(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	
	МодификацияКонфигурацииКлиентПереопределяемый.ВыполнитьПереопределяемоеСобытиеОкончаниеВводаТекста(ЭтаФорма, Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_Нажатие_ВыполнитьПереопределяемоеСобытие(Элемент, СтандартнаяОбработка)
	
	МодификацияКонфигурацииКлиентПереопределяемый.ВыполнитьПереопределяемоеСобытиеНажатие(ЭтаФорма, Элемент, СтандартнаяОбработка);
