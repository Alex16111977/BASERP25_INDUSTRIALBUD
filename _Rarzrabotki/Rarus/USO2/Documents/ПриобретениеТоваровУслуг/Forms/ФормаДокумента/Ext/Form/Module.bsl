...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.ЗаполнятьПоОрдеру
		...
		Если ЭтаФорма.ТолькоПросмотр Тогда    	
			ТекстСообщения = НСтр("ru = 'Нельзя перезаполнить документ ""%Документ%"", т.к. установлена дата запрета изменений!';
									|en = 'Can not refill the document ""%Документ%"", because the closing date of changes is set!'");
		...
		Если НеобходимВопросОбОчисткеСклада() Тогда
			...
			Если СкладГруппа Тогда
				ТекстВопроса = НСтр("ru = 'Строки по заказу, в которых склад не входит в группу складов ""%Склад%"" будут удалены из списка ""Товары"". Продолжить?';
									|en = 'The order lines containing a warehouse that does not belong to the ""%Склад%"" group will be deleted from the Goods list. Continue?'");
			Иначе
				ТекстВопроса = НСтр("ru = 'Строки по заказу, в которых склад отличается от ""%Склад%"" будут удалены из списка ""Товары"". Продолжить?';
									|en = 'The order lines containing a warehouse other than ""%Склад%"" will be deleted from the Goods list. Continue?'");
			КонецЕсли;
		...
		Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
			...
			Если Объект.Товары.Количество() = 0 Тогда
				ТекстВопроса = НСтр("ru = 'Список Товары будет перезаполнен остатками непоступивших товаров по заказам. Продолжить?';
									|en = 'The Goods list will be repopulated with residual unreceived goods against orders. Continue?'");
			Иначе
...
Процедура ПоступлениеПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если Объект.Товары.Количество() = 0 Тогда
        ТекстВопроса = СтрЗаменить(НСтр("ru = 'Список Товары будет заполнен остатками непоступивших товаров по документу %ЗаказПоставщику%. Продолжить?';
										|en = 'The Goods list will be populated with residual unreceived goods according to the %ЗаказПоставщику% document. Continue?'"), "%ЗаказПоставщику%", ЗаказПоставщику);
    Иначе
        ТекстВопроса = СтрЗаменить(НСтр("ru = 'Строки в списке Товары будут привязаны к строкам документа %ЗаказПоставщику%. Продолжить?';
										|en = 'The Goods list lines will be linked to lines of the %ЗаказПоставщику% document. Continue?'"), "%ЗаказПоставщику%", ЗаказПоставщику);
    КонецЕсли;
...
Процедура ЗаполнитьЦеныПоСоглашению(Команда)
	...
	Если ЗакупкиКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект, "Товары", НСтр("ru = 'Товары';
																					|en = 'Goods'")) Тогда
...
Процедура ЗаполнитьЦеныВыделенныхСтрокПоВидуЦенПоставщика(Команда)
	...
	Если ЗакупкиКлиент.НеобходимоЗаполнениеЦенПоВидуЦенПоставщика(Объект, "Товары", НСтр("ru = 'Товары';
																						|en = 'Goods'")) Тогда
...
Процедура ЗаполнитьЦеныПоПоступлениям(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Партнер) Тогда
		ВызватьИсключение НСтр("ru = 'Для заполнения цен по последним поступлениям необходимо указать поставщика';
								|en = 'To complete the prices in the latest receipts, specify the supplier'");
	КонецЕсли;
	...
	Иначе
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru = 'Ни в одной строке цена не заполнена';
				|en = 'Price is not filled in in any line'"),
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура ИзменитьКачествоНоменклатуры(Команда)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Выберите строку табличной части';
								|en = 'Select a tabular section row'");
	...
	Если НЕ ЕстьПравоДобавленияКорректировок() И (ЕстьКорректировки ИЛИ ЕстьСчетаФактуры) И ИспользоватьКорректировкиПриобретений Тогда
		...
		Если ЕстьКорректировки Тогда
			ТекстСообщения = НСтр("ru = 'На основании документа введена корректировка приобретения.
				...
				|en = 'Purchase adjustment is entered based on the document.
	...
	ИначеЕсли ЕстьКорректировки И ИспользоватьКорректировкиПриобретений Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'На основании документа введена корректировка приобретения.
				...
				|en = 'Purchase adjustment is entered based on the document.
				|Changing the original document is prohibited.'"),
			СписокКнопок,
			,
			"ВвестиКорректировку");
...
Процедура ПолучитьТоварыИзХранилища(АдресТоваровВХранилище)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействийБезПересчетаСумм, ЭтаФорма);
	СтруктураДействийБезПересчетаСумм.Удалить("ОчиститьСуммуВзаиморасчетов");
	СтруктураДействийБезПересчетаСумм.Удалить("ПересчитатьСуммуСверхЗаказа");
...
Процедура ВопросПользователюПередЗаполнениемЦенПоСоглашению(ОписаниеОповещения)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Перезаполнить цены по соглашению?';
			|en = 'Refill prices by the agreement?'"),
		ВариантыОтветов);
...
Процедура ЗаполнитьПодразделение(Команда)
	...
	Если Объект.Товары.Количество() = 0 Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'В список ""Товары"" не введено ни одной строки.';
										|en = 'No lines entered in the Goods list.'"));
		Возврат;
	Иначе
		...
		Если Не ТребуетсяЗаполнение Тогда
			ПоказатьПредупреждение( , НСтр("ru = 'В выделенных строках списка ""Товары"" заполнение подразделения-получателя не требуется.';
											|en = 'In the selected lines of the Goods list, it is not required to fill in a receiving department.'"));
			Возврат;
		КонецЕсли;
	...
	Если ВестиСведенияДляДекларацийАлкоВРознице
		...
		Если ЗначениеЗаполнено(Результат.Ссылка) Тогда
			...
			ТекстВопроса = НСтр("ru = 'Связать Товарно-транспортную накладную ЕГАИС %1 от %2 (Поставщик ЕГАИС - %3) с текущим поступлением? %4';
								|en = 'Link USAIS waybill %1 from %2 (USAIS supplier - %3) to the current receipt? %4'");
			...
			Если Результат.ТребуетсяСвязатьОрганизацию Тогда
				...
				Уточнение = Символы.ПС + НСтр("ru = 'Поставщик ЕГАИС будет связан с поставщиком %1.';
												|en = 'USAIS supplier will be associated with supplier %1.'");
		...
		Если ЕстьРасхожденияМеждуДокументомПоступленияИТТНЕГАИС(ДополнительныеПараметры.Ссылка) Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'В товарах выбранного поступления есть алкогольная продукция, которой нет в ТТН. Продолжить выбор?';
					|en = 'There are alcoholic products which are absent in the waybill in goods of the selected receipt. Continue selection?'"),
				РежимДиалогаВопрос.ДаНет);
	...
	Иначе
		...
		ПоказатьВопрос(
				...
				НСтр("ru = 'В документе найдены несопоставленные элементы классификаторов ЕГАИС.
				     ...
				     |en = 'Unmapped USAIS classifier items were found in the document.
				     |Map the classifiers? '"),
				Кнопки);
	...
	Иначе
		...
		Иначе
			Шаблон = НСтр("ru = 'Связать с ТТН ЕГАИС (входящей) (%1)';
							|en = 'Link to USAIS waybill (incoming) (%1)'");
		КонецЕсли;
...
Функция ПараметрыИнтеграцииФормыПроверкиИПодбора()
	...
	ПараметрыИнтеграции.ИмяПоследующегоЭлементаФормы = "Товары";
...
&НаКлиенте
Процедура Подключаемый_ВозобновитьПроверкуПродукцииМОТП(Команда)
	
	ПроверкаИПодборПродукцииМОТПКлиент.ВозобновитьПроверку(ЭтотОбъект, ПараметрыОткрытияФормыПроверкиИПодбора());
...
Процедура ПриЗакрытииФормыПроверкиИПодбораНаСервере(Результат, ДополнительныеПараметры)
	...
	Прочитать();
	ПроверкаИПодборПродукцииМОТП.УправлениеЭлементамиОткрытияФормыПроверкиИПодбора(ЭтотОбъект, ПараметрыИнтеграцииФормыПроверкиИПодбора());
