...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СвойстваСписка.ТекстЗапроса = ДокументыЗакупки.ТекстЗапроса + "
	|ОБЪЕДИНИТЬ ВСЕ " + "
	|ВЫБРАТЬ
	...
	Если Не Объект.ПрочиеРасходы.Количество()
		...
		Если ИспользоватьНесколькоВалют Тогда
			Текст = НСтр("ru = 'Сумма израсходованных денежных средств равна 0.
				...
				|Fill in data on the ""Purchases"", the ""Expenses"", the ""Payment to suppliers"" or ""Currency conversion"" tab'");
		Иначе
			Текст = НСтр("ru = 'Сумма израсходованных денежных средств равна 0.
				|Необходимо заполнить данные на закладке ""Закупки"", ""Расходы "" или ""Оплата поставщикам""';
				|en = 'Spent cash amount is 0.
				|Fill in data on the ""Purchases"", the ""Expenses"" or the ""Payment to suppliers"" tab'");
		КонецЕсли;
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если ЗадаватьВопрос Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Очистить суммы взаиморасчетов?';
										|en = 'Clear mutual settlements amounts?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
...
Процедура ВалютаПриИзмененииКлиент()
	...
	Если ТекущаяВалюта <> Объект.Валюта
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пересчитать суммы в документе в валюту %1 ?';
																					|en = 'Recalculate amounts in the document into currency %1?'"), Объект.Валюта);
...
Процедура ВалютаПриИзмененииКлиентЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Суммы в документе пересчитаны в валюту %1';
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Принять к вычету по СФ';
																|en = 'Accept for deduction by invoice'"));
...
Функция ВыборкаПолучено()
	...
	Если ЗначениеЗаполнено(ВыборкаДаты.Дата) Тогда
		ДатаОстатков = ВыборкаДаты.Дата;
	Иначе
		ДатаОстатков = ТекущаяДатаСеанса();
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДатаДокумента", Новый Граница(ДатаОстатков, ВидГраницы.Исключая));
	Запрос.Текст = "
	|ВЫБРАТЬ
		...
		Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
			Текст = СтрШаблон(
				НСтр("ru = 'У подотчетного лица отсутствуют выданные авансы, за которые он не отчитался перед организацией %1 в подразделении %2 в валюте %3';
					|en = 'The advance holder has no issued advances for which he or she has not reported to the %1 company in the %2 department in %3 currency'"),
		...
		Иначе
			Текст = СтрШаблон(
				НСтр("ru = 'У подотчетного лица отсутствуют выданные авансы, за которые он не отчитался перед организацией %1 в валюте %2 без указания подразделения';
					|en = 'The advance holder has no issued advances for which he or she has not reported to the %1 company in %2 currency without specifying a department'"),
...
Функция ПредставлениеСчетФактуры(СтрокаТЧ)
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.НомерСФ) Или ЗначениеЗаполнено(СтрокаТЧ.ДатаСФ) Тогда
		Результат = "№" + СтрокаТЧ.НомерСФ + " " + НСтр("ru = 'от';
														|en = 'dated'") + " " + Формат(СтрокаТЧ.ДатаСФ, "ДЛФ=""Д""");
	Иначе
		Результат = НСтр("ru = '<Указать реквизиты>';
						|en = '<Specify attributes>'");
	КонецЕсли;
