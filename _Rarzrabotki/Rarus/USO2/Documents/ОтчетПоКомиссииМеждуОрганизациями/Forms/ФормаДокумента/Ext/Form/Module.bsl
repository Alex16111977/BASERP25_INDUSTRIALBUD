	...
	Если ЗначениеЗаполнено(Объект.ВидЦены)
	 И Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидЦеныПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Перезаполнить цены в табличной части документа?';
																									|en = 'Refill prices in the document tabular section?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьСостояниеЭлементовУправленияИтоговПоРасчетам()
	...
	Если Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов Тогда
		...
		РасчетыОтправитель = НСтр("ru = 'Расчеты по договору';
									|en = 'Settlements under contract'");
	Иначе
		РасчетыПолучатель = НСтр("ru = 'Оплачено';
								|en = 'Paid'") + ": " + Формат(СуммаОплатыПолучатель, "ЧДЦ=2; ЧН=") + " " + Объект.Валюта + "  " + ПроцентОплатыПолучатель + "%";
		РасчетыОтправитель = НСтр("ru = 'Получено';
									|en = 'Received'") + ": " + Формат(СуммаОплатыОтправитель, "ЧДЦ=2; ЧН=") + " " + Объект.Валюта + "  " + ПроцентОплатыОтправитель + "%";
	КонецЕсли;
...
Процедура УправлениеОтображениемСоздатьНаОсновании(ПодменюСоздатьНаОсновании, УдержатьВознаграждение)
	...
	Для каждого Элемент Из ПодменюСоздатьНаОсновании.ПодчиненныеЭлементы Цикл
		Если Найти(Элемент.Имя,НСтр("ru = 'ЗаявкаНаРасходованиеДсНаОснованииОтчетаПоКомиссии_Вознаграждение';
									|en = 'ЗаявкаНаРасходованиеДсНаОснованииОтчетаПоКомиссии_Вознаграждение'")) > 0
			ИЛИ Найти(Элемент.Имя,НСтр("ru = 'ПоступлениеБезналичныхДсНаОснованииОтчетаПоКомиссии_Вознаграждение';
										|en = 'ПоступлениеБезналичныхДсНаОснованииОтчетаПоКомиссии_Вознаграждение'")) > 0 
				ИЛИ Найти(Элемент.Имя,НСтр("ru = 'СписаниеБезналичныхДсНаОснованииОтчетаПоКомиссии_Вознаграждение';
											|en = 'СписаниеБезналичныхДсНаОснованииОтчетаПоКомиссии_Вознаграждение'")) > 0 Тогда
