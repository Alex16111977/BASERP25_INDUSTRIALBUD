...
Функция ЗаполнитьНомераГТДПоОстаткамТоваровВПутиНаСервере()
	...
	|	ТоварыОрганизацийОстатки.НомерГТД.СтранаПроисхождения КАК СтранаПроисхождения,
...
Процедура РассчитатьИтоговыеПоказатели(Форма)
	...
	Иначе
		...
		Если УчетНДСУПКлиентСервер.ЗакупкаОблагаетсяНДС(Форма.Объект.НалогообложениеНДС) Тогда
