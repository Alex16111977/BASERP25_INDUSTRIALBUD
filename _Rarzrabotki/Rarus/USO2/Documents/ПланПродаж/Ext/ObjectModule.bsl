	...
	Если ПроверятьЗаполнениеПрогнозныхКурсовВалют И ВалютыПроверкиКурсов.Количество() <> 0 Тогда
		...
		Если НеНайденныеКурсы.Количество() <> 0 Тогда
			...
			ТекстСообщения = 
				...
				|en = 'Sales plan is recorded in budgeting.
	...
	Если Не КроссТаблица
		...
		Для Каждого СтрокаОшибки Из ТаблицаОшибок Цикл
			...
			ТекстСообщения = НСтр("ru = 'Для строк плана с номенклатурой %Номенклатура%%Характеристика%%Назначение%%Склад%%Партнер%%Соглашение% не запланировано количество ни в одном периоде планирования.';
									|en = 'Quantity is not planned in any planning period for lines of plan with the %Номенклатура%%Характеристика%%Назначение%%Склад%%Партнер%%Соглашение% products.'");
...
Процедура ЗаполнитьРеквизитыПланаПоСценариюВидуПлана()
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		РеквизитыВидаПлана = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидПлана, 
			"ОтражаетсяВБюджетировании, ОтражаетсяВБюджетированииОплаты, ОтражаетсяВБюджетированииОплатыКредит, 
			|СтатьяБюджетов, СтатьяБюджетовОплат, СтатьяБюджетовОплатКредит");
