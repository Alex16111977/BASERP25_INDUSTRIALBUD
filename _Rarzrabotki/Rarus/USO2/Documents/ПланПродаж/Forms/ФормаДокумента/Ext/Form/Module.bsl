		...
		Если Объект.СуммаДокумента = 0 И Объект.ПланОплаты.Количество() > 0 Тогда 
			...
			Если КоличествоЗапланированныхСтрок = 0 И Объект.ПланОплаты.Количество() > 0 Тогда
				...
				НСтр("ru = 'В плане все строки отменены. Таблица плана оплаты будет очищена';
					|en = 'All plan lines are canceled. The payment plan table will be cleared'"),
			...
			Иначе
				...
				НСтр("ru = 'Сумма запланированных строк нулевая. Таблица плана оплаты будет очищена';
					|en = 'Sum of the planned lines equals to 0. The payment plan table will be cleared'"),
			...
			Если Объект.ПланОплаты.Количество() = 0 Тогда
				...
				Иначе
					ТекстВопроса = НСтр("ru = 'Отсутствуют этапы плана оплаты. 
						...
						|en = 'Payment plan steps are missing. 
...
Процедура ЗаполнитьЭтапыГрафикаОплаты(Команда)
	...
	Если Объект.СуммаДокумента = 0 Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Этапы оплаты очищены';
				|en = 'Payment steps are cleared'"),
			,
			НСтр("ru = 'Сумма запланированных строк нулевая. Таблица этапов оплаты очищена';
				|en = 'Sum of planned lines equals to 0. Payment step table is cleared'"),
	...
	Если Объект.СуммаДокумента = СуммаОплатыПоЭтапам Тогда
		...
		ТекстВопроса = НСтр("ru = 'Сумма запланированных строк совпадает с суммой этапов оплаты';
							|en = 'Sum of planned lines matches the payment step sum'") + Символы.ПС +
	...
	ИначеЕсли Объект.ПланОплаты.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Таблица этапов оплаты заполнена';
							|en = 'Payment steps table is filled in'")+ Символы.ПС +
							НСтр("ru = 'Перезаполнить этапы оплаты %ИсточникЗаполнения%?';
								|en = 'Refill steps of payment %ИсточникЗаполнения%?'");
	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			...
			ТекстВопроса = НСтр("ru = 'В строках списка ""Товары"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';
								|en = 'Dates outside the planning limits are specified in lines of the Goods list. Lines will be deleted. Delete the lines?'");
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Номенклатура)';
								|en = 'Unique ID (Products)'"),"ИдентификаторНоменклатура");
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Характеристика)';
								|en = 'Unique ID (Characteristic)'"),"ИдентификаторХарактеристика");
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Упаковка)';
								|en = 'Unique ID (Packaging)'"),"ИдентификаторУпаковка");
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Номенклатура';
								|en = 'Products and services'"),"ЗагружаемаяНоменклатура");
	КолонкиПоиска.Добавить(НСтр("ru = 'Характеристика';
								|en = 'Characteristic'"),"ЗагружаемаяХарактеристика");
	
	Если ЗаполнятьПартнераВТЧ Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Поставщик';
									|en = 'Supplier'"),"ЗагружаемыйПартнер");
	КонецЕсли; 
	Если ЗаполнятьСоглашениеВТЧ Тогда
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'План продаж +15%';
																	|en = 'Sales plan +15%'"), "[ПланыПродаж] * 1.15");
	
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'План продаж с учетом корректировки по факту продаж';
																	|en = 'Sales plan considering adjustment according to sales fact'"), 
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Факт продаж с учетом наличия товаров на остатках';
																	|en = 'Sales fact considering goods availability in stock'"), 
...
Функция ДополнительныеНастройки(СтруктураНастроек, ДополнительныеПоля = Неопределено)
	...
	Если НЕ ТипЗнч(СтруктураНастроек) = Тип("Структура") Тогда
		...
		СтруктураНастроек.Вставить("ФормулаПредставление",НСтр("ru = 'Увеличение планов продаж прошлых периодов на 15%';
																|en = 'Increase of sales plans of previous periods by 15%'"));
...
Функция ЗаполнитьДокументНаСервере(Знач ПараметрыЗадания, КэшированныеЗначения)
	...
	НаименованиеЗадания = НСтр("ru = 'Заполнение документа плана:';
								|en = 'Fill in plan:'") + " " + Строка(Объект.Ссылка);
	...
	Если НЕ ЗначениеЗаполнено(Объект.Сценарий) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Сценарий"" не заполнено';
				|en = 'The Script field is not filled in'"),
			Объект.Ссылка,
			"Объект.Сценарий");
	...
	Если ЗаполнятьФорматМагазина И НЕ ЗначениеЗаполнено(Объект.ФорматМагазина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Формат магазина"" не заполнено';
				|en = 'The ""Store format"" field is not filled in'"),
			Объект.Ссылка,
