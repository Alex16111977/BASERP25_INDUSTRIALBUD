	...
	Иначе
		...
		Если ТаблицаОрганизаций.Количество() = 0 Тогда
			ТекстОшибки = НСтр("ru = 'Невозможно сформировать инвентаризационную опись, т.к. в настройках программы не введены сведения об организации.';
								|en = 'Cannot generate an stocktaking sheet as the company information is not entered in the application settings.'");
			Отказ = Истина;
...
Процедура Назад(Команда)
	...
	ТекстВопроса = НСтр("ru = 'При возврате на предыдущий шаг сформированные инвентаризационные описи будут удалены. Продолжить?';
						|en = 'Generated stocktaking sheets will be removed if you return to the previous step. Continue?'");
...
Процедура Далее(Команда)
	...
	Если Не ЗначениеЗаполнено(ПериодИнвентаризации.ДатаНачала) Или Не ЗначениеЗаполнено(ПериодИнвентаризации.ДатаОкончания) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните период инвентаризации.';
																|en = 'Fill in stocktaking period.'"), , "ПериодИнвентаризации");
		ЕстьОшибки = Истина;
	КонецЕсли;
	
	Если ПериодИнвентаризации.ДатаНачала > ПериодИнвентаризации.ДатаОкончания Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните корректно период инвентаризации.';
																|en = 'Fill in stocktaking period correctly.'"), , "ПериодИнвентаризации");
		ЕстьОшибки = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Склад) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните Склад.';
																|en = 'Fill in Warehouse.'"), , "Склад");
		ЕстьОшибки = Истина;
	КонецЕсли;
		...
		Если ТаблицаОрганизаций.НайтиСтроки(Новый Структура("Отметка", Истина)).Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Выберите одну или несколько организаций, по которым необходимо создать инвентаризационные описи.';
					|en = 'Select one or several companies for which stocktaking sheets should be created.'"),
				,
				"ТаблицаОрганизаций",
				"Объект");
			ЕстьОшибки = Истина;
