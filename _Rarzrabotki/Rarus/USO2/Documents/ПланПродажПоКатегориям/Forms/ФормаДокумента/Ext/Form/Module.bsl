	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			...
			ТекстВопроса = НСтр("ru = 'В строках списка ""Товарные категории"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';
								|en = 'Dates outside the planning limits are specified in lines of the ""Product categories"" list. Lines will be deleted. Delete the lines?'");
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'УИ_ТоварнаяКатегория';
								|en = 'УИ_ТоварнаяКатегория'"),"ИдентификаторТоварнаяКатегория");
	КолонкиПоиска.Добавить(НСтр("ru = 'УИ_РейтингПродаж';
								|en = 'УИ_РейтингПродаж'"),"ИдентификаторРейтингПродаж");
	КолонкиПоиска.Добавить(НСтр("ru = 'УИ_Склад';
								|en = 'UI_Warehouse'"),"ИдентификаторСклад");
	
	КолонкиПоиска.Добавить(НСтр("ru = 'Номенклатура';
								|en = 'Products and services'"),"ЗагружаемаяТоварнаяКатегория");
	КолонкиПоиска.Добавить(НСтр("ru = 'УИ_Назначение';
								|en = 'УИ_Назначение'"),"ИдентификаторНазначение");
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	Если Сценарий.ИспользоватьРасчетПоСкоростиПродаж Тогда
		...
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'План продаж +15%';
																		|en = 'Sales plan +15%'"), "[ПланыПродажПоКатегориямПоСкоростиПродаж] * 1.15");
	...
	Иначе
		...
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'План продаж +15%';
																		|en = 'Sales plan +15%'"), "[ПланыПродажПоКатегориям] * 1.15");
