...
// 
Процедура ЗаполнитьДокумент(ДанныеДебиторскойЗадолженности) Экспорт
	
	ОбновитьЛимитРасходовНУ(Ложь);
...
Процедура ЗаполнитьДебиторскуюЗадолженность(ДанныеДебиторскойЗадолженности, РассчитыватьРезервы = Истина) Экспорт
	...
	ДебиторскаяЗадолженность.Сортировать("Контрагент,Договор,РасчетныйДокумент,ДатаВозникновенияЗадолженности");
	
	ТаблицаОценкиБУ = Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ТаблицаОценкиБУ(Организация, Дата);
	ПараметрыОценки = Новый Структура("ТаблицаОценки", ТаблицаОценкиБУ);
...
Процедура РассчитатьРезервы(АналитикаУчета = Неопределено, БУ = Истина, НУ = Истина, УУ = Истина) Экспорт
	...
	ФормироватьНУ = ПараметрыУчетнойПолитики.УчетнаяПолитикаСуществует И
	...
	Для Каждого СтрокаДЗ Из СтрокиОбработкиДЗ Цикл
		...
		БазаРасчета = (СтрокаДЗ.СуммаЗадолженностиРегл - СтрокаДЗ.СуммаОбеспечения - СтрокаДЗ.ВстречнаяЗадолженность);
...
// 	РассчитыватьРезервы - Булево - если Истина, то после заполнения рассчитываются резервы
//
Процедура ОбновитьЛимитРасходовНУ(РассчитыватьРезервы = Истина) Экспорт
	
	ЛимитРасходовНУ = НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ПолучитьЛимитРасходовНУ(Дата, Организация);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ПометкаУдаления Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ТекстСообщения = СтрШаблон(ТекстСообщения, Организация, Формат(Дата, "ДФ='ММММ'"), Выборка.Ссылка);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
	...
	НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ОтразитьРезервыПоСомнительнымДолгам(ДополнительныеСвойства, Движения, Отказ);
	НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ОтразитьРасчетРезервовПоСомнительнымДолгам(ДополнительныеСвойства, Движения, Отказ);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ОбработкаУдаленияПроведения(ЭтотОбъект, Отказ);
...
Функция ИнициализироватьСтруктуруПоиска(АналитикаУчета, ЭтоДебиторскаяЗадолженность = Истина)
	...
	Возврат ?(ЭтоДебиторскаяЗадолженность И АналитикаУчета.ВидЗадолженности = Перечисления.ВидыЗадолженности.ЗаймыВыданные,
		Новый Структура("Контрагент,Договор,ТипСуммы"),
		Новый Структура("Контрагент,Договор,РасчетныйДокумент"));
...
Процедура РаспределитьОбеспечение(Знач СтруктураОтбора = Неопределено)
	...
	Если СтруктураОтбора = Неопределено Тогда
		...
		СтруктураОтбора = Новый Структура("Контрагент,Договор,РасчетныйДокумент");
	КонецЕсли;
	
	ТаблицаОбеспечения = ДебиторскаяЗадолженность.Выгрузить(, "Контрагент,Договор,РасчетныйДокумент,СуммаОбеспеченияПоАналитике");
	ТаблицаОбеспечения.Свернуть("Контрагент,Договор,РасчетныйДокумент,СуммаОбеспеченияПоАналитике");
...
Процедура ЗаполнитьИдентификаторыСтрокПогашеннойЗадолженности(МенеджерВременныхТаблиц)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	вт_АналитикаУчета.КлючАналитики,
	|	вт_ДанныеДокумента.ОбъектРасчетов,
	|	вт_ДанныеДокумента.РасчетныйДокумент,
	|	вт_ДанныеДокумента.ТипСуммы
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("ГраницаКонцаМесяца", Новый Граница(Новый МоментВремени(Дата, Ссылка), ВидГраницы.Исключая));
