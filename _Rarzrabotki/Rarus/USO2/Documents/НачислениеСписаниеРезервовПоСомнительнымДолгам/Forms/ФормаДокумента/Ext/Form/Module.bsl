...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			...
			|period-end operation of month-end closing is executed. '"),,,, Отказ);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура КомандаОткрытьУчетнуюПолитику(Команда)
	
	ОткрытьФорму("Справочник.УчетныеПолитикиОрганизаций.ФормаОбъекта", Новый Структура("Ключ", УчетнаяПолитикаТекущая));
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	ИначеЕсли Не ЗначениеЗаполнено(УчетнаяПолитикаТекущая) Тогда
	...
	Иначе
		УчетнаяПолитикаЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			Нстр("ru = 'Порядок оценки задолженности определяется учетной политикой ""%1"" организации';
				|en = 'The debt evaluation procedure is defined by the accounting policy of the ""%1"" company'"), УчетнаяПолитикаТекущая);
	КонецЕсли;
	
	Элементы.ГруппаКредиторскаяЗадолженность.Видимость = УчитыватьВстречнуюЗадолженность;
	Элементы.КомандаОткрытьУчетнуюПолитику.Заголовок = УчетнаяПолитикаЗаголовок;
	Элементы.КомандаОткрытьУчетнуюПолитику.Доступность = ЗначениеЗаполнено(УчетнаяПолитикаТекущая);
	
	Элементы.ОтражатьВБухгалтерскомУчете.ТолькоПросмотр = Не ФормироватьБУ;
...
Функция ЗаполнитьДебиторскуюЗадолженностьНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Заполнение дебиторской задолженности';
															|en = 'Filling accounts receivable'");
...
Процедура ПослеЗаполненияДебиторскойЗадолженности(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ТекстСообщения = НСтр("ru = 'Произошла ошибка заполнения дебиторской задолженности:';
								|en = 'An error occurred while filling accounts receivable:'") + Символы.ПС + Результат.КраткоеПредставлениеОшибки;
...
Процедура ОбновитьЛимитРасходовНУ()
	...
	ТекущийОбъект = РеквизитФормыВЗначение("Объект");
	ТекущийОбъект.ОбновитьЛимитРасходовНУ();
...
Процедура ЗаполнитьДополнительныеРеквизитыДебиторскойЗадолженности()
	...
	Для Каждого ТекДанные Из Объект.ДебиторскаяЗадолженность Цикл
		ТекДанные.СуммаОбеспеченияПредставление = ?(ТекДанные.СуммаЗадолженности = 0 Или ТекДанные.СуммаОбеспечения = 0, "",
			Формат(ТекДанные.СуммаОбеспечения, "ЧДЦ=2; ЧН=;") + " из " + Формат(ТекДанные.СуммаОбеспеченияПоАналитике, "ЧДЦ=2; ЧН=;"));
...
Процедура УстановитьУчетнуюПолитикуОрганизаций()
	...
	УчетнаяПолитикаТекущая = Справочники.УчетныеПолитикиОрганизаций.ПустаяСсылка();
	ФормироватьБУ = Ложь;
	ФормироватьНУ = Ложь;
	...
	Если ПараметрыУчетнойПолитики.УчетнаяПолитикаСуществует Тогда
		УчетнаяПолитикаТекущая = ПараметрыУчетнойПолитики.Ссылка;
		ФормироватьБУ = ПараметрыУчетнойПолитики.ФормироватьРезервыПоСомнительнымДолгамБУ;
		ФормироватьНУ = ПараметрыУчетнойПолитики.ФормироватьРезервыПоСомнительнымДолгамНУ;
		
		Если Объект.Ссылка.Пустая() Тогда
			Объект.ОтражатьВБухгалтерскомУчете = ФормироватьБУ И
				НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ТребуетсяФормированиеРезервовВУказанномПериоде(
					ПараметрыУчетнойПолитики.ПериодичностьРезервовПоСомнительнымДолгамБУ, Объект.Дата);
			Объект.ОтражатьВНалоговомУчете = ФормироватьНУ И
				НачислениеСписаниеРезервовПоСомнительнымДолгамСервер.ТребуетсяФормированиеРезервовВУказанномПериоде(
...
Функция ЗаполнитьТекущиеДанныеДЗ()
	...
	Результат = Новый Структура(
		"Контрагент,Договор,РасчетныйДокумент,ТипСуммы," +
