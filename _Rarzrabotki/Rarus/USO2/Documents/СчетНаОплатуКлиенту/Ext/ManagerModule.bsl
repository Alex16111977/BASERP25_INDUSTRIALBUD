...
Функция ДобавитьКомандуСоздатьНаОснованииСчетаНаОплатуПредоплатаПоДоговору(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетНаОплатуКлиенту) Тогда 
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Счет на оплату (предоплата)';
														|en = 'Proforma invoice (prepayment)'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "ИспользоватьСчетаНаОплатуКлиентам";
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Запись счетов на оплату';
				|en = 'Save proforma invoices'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция СформироватьНазначениеПлатежа(Знач Номер, Знач ДокументОснование) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионераОСписании") Тогда
		...
		ТекстНазначениеПлатежа = НСтр("ru = 'Оплата по отчету комиссионера о списании №%Номер%';
										|en = 'Payment by write-off report from consignee No.%Номер%'");
...
Процедура ПроверитьКорректностьХозяйственнойОперацииДокументаОснования(ЕстьОшибкиХозяйственнаяОперация, ХозяйственнаяОперация) Экспорт
	...
	Если ЕстьОшибкиХозяйственнаяОперация Тогда
		...
		ТекстОшибки = НСтр("ru = 'Ввод на основании документа с хозяйственной операцией %ХозяйственнаяОперация% невозможен.';
							|en = 'Input based on a document with business transaction %ХозяйственнаяОперация% is unavailable.'");
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ВЫБОР КОГДА Документы.БанковскийСчет.РучноеИзменениеРеквизитовБанка ТОГДА Документы.БанковскийСчет.АдресБанка
	|		ИНАЧЕ Документы.БанковскийСчет.Банк.Адрес КОНЕЦ КАК АдресБанка,
	|	ВЫБОР КОГДА Документы.БанковскийСчет.РучноеИзменениеРеквизитовБанкаДляРасчетов ТОГДА Документы.БанковскийСчет.АдресБанкаДляРасчетов
	|		ИНАЧЕ Документы.БанковскийСчет.БанкДляРасчетов.Адрес КОНЕЦ КАК АдресБанкаДляРасчетов,
