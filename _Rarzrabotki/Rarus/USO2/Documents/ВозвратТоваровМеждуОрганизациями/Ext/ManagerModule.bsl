...
Функция ПараметрыРегистрацииСчетовФактурВыданных(Документ) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями") Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВозвратТоваровМеждуОрганизациямиВидыЗапасов.ДокументПередачи КАК ДокументПередачи
		|ИЗ
		|	Документ.ВозвратТоваровМеждуОрганизациями.ВидыЗапасов КАК ВозвратТоваровМеждуОрганизациямиВидыЗапасов
		|ГДЕ
		|	ВозвратТоваровМеждуОрганизациямиВидыЗапасов.Ссылка = &Ссылка";
		
		Запрос.УстановитьПараметр("Ссылка", Документ);
		
		ДокументыПередачи = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ДокументПередачи");
	Иначе
		ЗаполнитьЗначенияСвойств(РеквизитыДокумента, Документ);
		ДокументыПередачи = Документ.ВидыЗапасов.Выгрузить().ВыгрузитьКолонку("ДокументПередачи");
	КонецЕсли;
	...
	ВозвратТолькоПоПередачамТоваров = Истина;
	Для Каждого ДокументПередачи Из ДокументыПередачи Цикл
		Если Не ЗначениеЗаполнено(ДокументПередачи) Или ТипЗнч(ДокументПередачи) <> Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
			ВозвратТолькоПоПередачамТоваров = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если
		ПараметрыРегистрации.ВозвратТоваровПоставщику
		И ВозвратТолькоПоПередачамТоваров
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка, ДополнительныеСвойств)
	...
	ИнформацияПоОрганизации = НСтр("ru = 'Возврат из ""%Организация%""';
									|en = 'Return from ""%Организация%""'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ТекстЗапросаВтЗапасыПолучателя(Запрос, ТекстыЗапроса)
	...
	|	(ВЫБОР КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов ТОГДА ТаблицаВидыЗапасов.ВидЗапасовОтгрузки
	|		ИНАЧЕ Неопределено КОНЕЦ)                                                               КАК ВидЗапасовОтгрузкиУчетный,
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	Неопределено                                                            КАК КорРазделУчета,
	|	ВидыЗапасов.ВидЗапасовОтгрузкиУчетный									КАК КорВидЗапасов,
	|	ВЫБОР КОГДА &ИспользуетсяНазначение
	|		ТОГДА ВидыЗапасов.АналитикаОтгрузки
	|		ИНАЧЕ ВидыЗапасов.АналитикаОтгрузкиБезНазначения
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ВозвратПоставщику") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ВозвратПоставщику", НСтр("ru = 'Возврат поставщику';
																												|en = 'Return to supplier'"), СформироватьПечатнуюФормуВозвратПоставщику(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;
...
Функция СформироватьПечатнуюФормуВозвратПоставщику(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ДанныеПечати.УчитыватьНДС И Не ДанныеПечати.ЭтоПередачаНаКомиссию Тогда
			...
			ТабличныйДокумент.Присоединить(ОбластьТовар);
			ОбластьДанных.Параметры.НДС = ?(ДанныеПечати.ЦенаВключаетНДС, НСтр("ru = 'В том числе НДС:';
