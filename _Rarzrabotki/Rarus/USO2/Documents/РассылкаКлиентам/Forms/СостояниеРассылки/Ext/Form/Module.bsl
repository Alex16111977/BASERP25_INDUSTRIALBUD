...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.ЖурналыДокументов.Взаимодействия) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'У вас нет прав на просмотр отчета по созданным документам рассылки.';
																|en = 'You are not authorized to view the report on the created bulk email documents.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(ДанныеРассылки.Основание) = Тип("ДокументСсылка.НазначениеОпросов") И ЗначениеЗаполнено(ДанныеРассылки.Основание) Тогда
		...
		Если ДанныеОснования.ТипРеспондентов = Справочники.Пользователи.ПустаяСсылка() Тогда
			ВызватьИсключение НСтр("ru = 'В ""Назначении опросов"" указан некорректный тип респондентов. Для пользователей не может использоваться рассылка клиентам.';
									|en = 'There is wrong type of respondents indicated in the ""Purpose of survey"". For users, distribution to customers cannot be used.'");
		КонецЕсли
	Иначе
...
Функция ТекстЛегенды(ДанныеРассылки)
	...
	Если ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.Черновик Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""Черновик"". В отчет выведены возможные адресаты рассылки и их контактная информация.
		                           ...
		                           |en = 'Bulk email is in the ""Draft"" status. Possible bulk email addressees and their contact information are displayed in the report.
		                           |If contact information is not specified or specified incorrectly, bulk email to these addressees
		                           |will not be performed. You can change the required contact information by double-clicking a bulk email recipient. '");
	...
	ИначеЕсли ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.КОтправке Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""К отправке"" и запланирована %1
		                          ...
		                          |en = 'Mailing has the ""For dispatch"" status and is planned %1 ""
		                          |""The report contains possible recipients and their contact information.
		                          |If contact information is not specified or specified incorrectly, mailing to these addressees
		                          |will not be performed. You still can change the contact information of the required kind by double-clicking a mailing recipient.'");
		...
		Иначе
			...
			Если ДанныеРассылки.ДатаАктуальности < ТекущаяДатаСеанса() Тогда
				ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""К отправке"" и была запланирована %1
				                  ...
				                  |В отчет выведены возможные адресаты рассылки и их контактная информация.';
				                  |en = 'Bulk email is in the ""For dispatch"" status, and %1 was planned
				                  |Bulk email relevance date expired and will not be executed.
				                  |Possible bulk email addressees and their contact information are displayed in the report.'");
			КонецЕсли;
	...
	ИначеЕсли ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.Обрабатывается Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""Обрабатывается"". В данный момент создаются сообщения адресатам рассылки.
		                           ...
		                           |en = 'Bulk email is in the ""In progress"" status. Messages to bulk email addressees are being created.
		                           |Possible bulk email addressees and their contact information are displayed in the report. If contact information is not specified
		                           |or specified incorrectly, messages to these addressees will not be created.'");
	...
	ИначеЕсли ДанныеРассылки.Статус = Перечисления.СтатусыРассылокКлиентам.Выполнена Тогда
		...
		ТекстЛегенды = НСтр("ru = 'Рассылка находится в статусе ""Выполнена"". Все возможные сообщения созданы.
		                           ...
		                           |en = 'Bulk email is in the ""Completed"" status. All possible messages are created.
		                           |Messages were not created if contact information was not specified or was specified incorrectly.
