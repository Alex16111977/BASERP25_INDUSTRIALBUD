...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьАктыВыполненныхРаботПоНесколькимЗаказам Тогда
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "УслугиПодобратьУслугиИзЗаказа", "Заголовок", НСтр("ru = 'Подобрать услуги из заказов';
																																	|en = 'Select services from orders'"));
...
Процедура АктВыполненныхРаботПоЗаказамПриИзмененииПослеВыбора(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если Объект.Услуги.Количество() = 0 Тогда
        ТекстВопроса = СтрЗаменить(НСтр("ru = 'Список Услуги будет перезаполнен остатками неоформленных услуг по документу %ЗаказКлиента%. Продолжить?';
										|en = 'List Services will be refilled with non-generated services by document %ЗаказКлиента%. Continue?'"), "%ЗаказКлиента%", ЗаказКлиента);
    Иначе
        ТекстВопроса = СтрЗаменить(НСтр("ru = 'Строки в списке Услуги будут привязаны к строкам документа %ЗаказКлиента%. Строки, которые невозможно привязать к строкам заказа, будут удалены.  Продолжить?';
										|en = 'Lines in list Services will be linked to lines of document %ЗаказКлиента%. Lines that cannot be linked will be deleted. Continue?'"), "%ЗаказКлиента%", ЗаказКлиента);
    КонецЕсли;
...
Процедура ЗаполнитьЦеныПоСоглашению(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект, "Услуги", НСтр("ru = 'Услуги';
																					|en = 'Services'")) Тогда
...
Процедура ЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "Услуги", НСтр("ru = 'Услуги';
																				|en = 'Services'")) Тогда
...
Процедура ОткрытьПодбор(Команда)
	...
	ПараметрЗаголовок = НСтр("ru = 'Подбор услуг в %Документ%';
							|en = 'Select services in %Документ%'");
...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "Услуги", НСтр("ru = 'Услуги';
																							|en = 'Services'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьРучныеСкидки(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "Услуги", НСтр("ru = 'Услуги';
																						|en = 'Services'")) Тогда
		Возврат;
	КонецЕсли;
