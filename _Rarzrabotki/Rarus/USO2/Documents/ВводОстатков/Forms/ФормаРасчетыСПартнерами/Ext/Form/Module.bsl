...
Процедура ЗаполнитьПоОстаткам(Команда)
	...
	Если Объект.РасчетыСПартнерами.Количество() Тогда
		...
		ТекстВопроса = НСтр("ru = 'При заполнении текущие данные табличной части будут очищены. Продолжить?';
							|en = 'Current tabular section data will be cleared on population. Continue?'");
		ПоказатьВопрос(ОповещениеВопросЗаполнитьПоОстаткам, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,,  НСтр("ru = 'Ввод остатков регл. учета по данным оперативного';
																											|en = 'Enter compl. accounting balance according to real-time accounting data '"));
	Иначе
		...
		Иначе
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыМеждуОрганизациями") Тогда
				СписокВыбора.Добавить(Справочники.ДоговорыМеждуОрганизациями.ПустаяСсылка(), НСтр("ru = 'Договоры между организациями';
																									|en = 'Contracts between companies'"));
			КонецЕсли;
		КонецЕсли;
		...
		Если Объект.ВводОстатков22 Тогда
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") Тогда
				СписокВыбора.Добавить(Документы.ВыкупПринятыхНаХранениеТоваров.ПустаяСсылка(), НСтр("ru = 'Выкуп товаров с хранения';
																									|en = 'Redemption of goods from storage'"));
			КонецЕсли;
