...
Процедура ИнициализацияФормыПриСозданииНаСервере()
	...
	Элементы.ОСГруппаСтоимостьРегл.Заголовок = СтрШаблон(НСтр("ru = 'Регламентированный учет (%1)';
																|en = 'Compliance accounting (%1)'"), Строка(ВалютаРегл));
	...
	Если РезультатПроверки.ПроблемныеОС.Количество() <> 0 Тогда
		...
		Если ТекстВопроса = Неопределено Тогда
			ТекстВопроса = НСтр("ru = 'Из табличной части будут удалены основные средства,
                                 ...
                                 |Продолжить?';
                                 |en = 'Fixed assets 
                                 |with other lines of business will be removed from the tabular section.
                                 |Continue?'");
		КонецЕсли; 
