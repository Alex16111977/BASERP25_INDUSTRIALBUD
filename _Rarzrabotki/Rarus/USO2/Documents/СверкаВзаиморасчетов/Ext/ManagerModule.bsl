...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиВзаимныхРасчетовСДаннымиКонтрагента") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			...
			НСтр("ru = 'Акт сверки взаимных расчетов (с задолженностью по данными контаргента)';
				|en = 'Mutual settlements reconciliation statement (with debt amount according to the counterparty data)'"),
			МакетНаПечать);
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		Если ТаблицаЗадолженности.Количество() > 0 Тогда
			...
			Для Каждого СтрокаЗадолженностиПоВалютам Из ТаблицаЗадолженностиПоВалютам Цикл
				...
				ОбластьДолг.Параметры.Валюта = СтрокаЗадолженностиПоВалютам.Валюта;
				...
				Если СтрокаЗадолженностиПоВалютам.СуммаДт > 0 И СтрокаЗадолженностиПоВалютам.СуммаКт = 0 Тогда
					...
					ОбластьДолг.Параметры.Сумма = Формат(СтрокаЗадолженностиПоВалютам.СуммаДт, "ЧДЦ=2; ЧГ=3,0");
					ОбластьДолг.Параметры.Дебитор = ПредставлениеПартнера;
				Иначе
					// Наш долг
					СуммаДолга = СтрокаЗадолженностиПоВалютам.СуммаКт;
					ОбластьДолг.Параметры.Сумма = Формат(СтрокаЗадолженностиПоВалютам.СуммаКт, "ЧДЦ=2; ЧГ=3,0");
				...
				КонецЕсли;
				ОбластьДолг.Параметры.СуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаДолга,СтрокаЗадолженностиПоВалютам.Валюта);
				ТабличныйДокумент.Вывести(ОбластьДолг);
				Если СтрокаЗадолженностиПоВалютам.Валюта <> ВалютаРегл Тогда
					СуммаДолгаРегл = РаботаСКурсамиВалют.ПересчитатьВВалюту(СуммаДолга, СтрокаЗадолженностиПоВалютам.Валюта, ВалютаРегл, ДанныеДокумента.КонецПериода);
...
Функция ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	|	ТЧДетальныеЗаписиРасчеты.ТипРасчетов КАК ТипРасчетов,
	...
	|
	|	СУММА(ДетальныеЗаписи.СуммаДебетКонтагент) КАК СуммаДебетКонтагент,
...
Процедура СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
	...
	ЕстьСчетФактуры = СчетаФактуры.Количество() > 0;
	//-- Локализация
				...
				Для Каждого СФ Из СФДокумента Цикл
					ОписаниеСФ = СтрШаблон(
									НСтр("ru = 'Счет-фактура №%1 от %2';
										...
										ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СФ.Номер),
										Формат(СФ.Дата, "ДЛФ=D")
									);
					ОписаниеСчетовФактур = ОписаниеСчетовФактур + ?(ПустаяСтрока(ОписаниеСчетовФактур),"",", ") + ОписаниеСФ;
				КонецЦикла;
	...
	ДетальныеЗаписи.Свернуть("ТипРасчетов, Партнер, ОбъектРасчетов, РасчетныйДокумент, ОписаниеДокумента, ВалютаДокумента", "СуммаДокумента, СуммаДолг, СуммаАванс, СуммаАвансРегл");
...
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикОтбор, УникальныйИдентификаторФормы = Неопределено, НастройкиОтбора = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		Поле = СхемаКомпоновкиДанных.НаборыДанных.ОтстаткиИОбороты.Поля.Найти("Партнер");
	...
	ПартнерыИКонтрагенты.ЗаголовокПоляСКДВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
		СхемаКомпоновкиДанных.НаборыДанных.ОтстаткиИОбороты, "Партнер", НСтр("ru = 'Контрагент';
																			|en = 'Counterparty'"));
	...
	КонецЕсли;
	ПолеТипРасчетов = СхемаКомпоновкиДанных.НаборыДанных.ОтстаткиИОбороты.Поля.Найти("ТипРасчетов");
...
Функция ЗапрещенныеТипыРасчетов() Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыНакопления.РасчетыПоФинансовымИнструментам) Тогда
		...
		КонецЕсли;
		//++ НЕ УТ
		Если НЕ ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыЛизинга) Тогда
			Результат.Добавить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем);
		КонецЕсли;
		//-- НЕ УТ
...
Функция ТекстыЗапросовДоступныхТаблиц()
	...
	КонецЕсли;
	//++ НЕ УТ
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыЛизинга) Тогда
		Результат.Вставить("СправочникДоговорыЛизинга", ТекстЗапросаДоговоровЛизинга());
	КонецЕсли;
	//-- НЕ УТ
...
Функция ТекстЗапросаДоговоровЛизинга()
	...
	|ВЫБРАТЬ
	|	&НачалоПериода КАК Период,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем) КАК ТипРасчетов,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаПоПартнерам,
	|	Договоры.Организация КАК Организация,
	|	Договоры.Партнер КАК Партнер,
	|	Договоры.Контрагент КАК Контрагент,
	...
	|				ИЛИ Договоры.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.ЗаймВыданный)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСДебитором)
	|		КОГДА Договоры.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.Лизинг)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем)
	|	КОНЕЦ КАК ТипРасчетов,
	|	ИСТИНА КАК ДоговорыБезОборотов}";
	...
	Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда 
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1, в валюте: %2';
				|en = '%1, in currency: %2'"), СтрокаЗадолженности.Партнер, ВыборкаДетальныхЗаписей.Валюта);
	ИначеЕсли ДанныеДокумента.РасшифровкаПоДоговорам Тогда 
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1, в валюте: %2';
				|en = '%1, in currency: %2'"), СтрокаЗадолженности.Договор, ВыборкаДетальныхЗаписей.Валюта);
	Иначе
		ОбластьМакетаГруппировка.Параметры.ПартнерВалюта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'в валюте: %1';
				|en = 'in the currency: %1'"), ВыборкаДетальныхЗаписей.Валюта);
	КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДетальныеЗаписи.ОрганизацияРеглУчет) Тогда
				...
				ПредставлениеВладельцаДокумента = НСтр("ru = 'от';
														|en = 'from'") + " " + ПредставлениеОрганизации;
