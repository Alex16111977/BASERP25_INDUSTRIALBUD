...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ДоступныДоговорыЛизинга = ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыЛизинга);
	//-- НЕ УТ
	...
	Если ДоступныРасчетыПоФинансовымИнструментам Тогда
		...
		КонецЕсли;
		//++ НЕ УТ
		Если ДоступныДоговорыЛизинга Тогда
			Элементы.РасчетыСЛизингодателем.Видимость = Истина;
		КонецЕсли;
		//-- НЕ УТ
	КонецЕсли;
...
Процедура РасчетыСПартнерами(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаНастройкаФильтров Тогда
		...
		Для Каждого ЭлементОтбора Из КомпоновщикОтбор.Настройки.Отбор.Элементы Цикл
			...
			Если СтрНайти(ПоляОтбораОтчета, Строка(ЭлементОтбора.ЛевоеЗначение)) <> 0
				...
				ПараметрыФормы.Отбор.Вставить(ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ПравоеЗначение);
			КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаТаблицаРасчетов Тогда
		...
		Если Элементы.ТаблицаРасчетов.ТекущиеДанные = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбрана ни одна строка';
																	|en = 'No line selected'"),,"ТаблицаРасчетов");
			Возврат;
		КонецЕсли;
...
Процедура ПересчитатьКоличествоСтрокТаблицыРасчетов()
	...
	Для Каждого СтрокаТаблицыРасчетов Из ТаблицаРасчетов Цикл
		...
		ИначеЕсли СтрокаТаблицыРасчетов.ИзменятьДокумент = "Создавать" Тогда
			КоличествоСозданных = КоличествоСозданных + 1;
...
Процедура СоздатьВФоновомЗаданииНаСервере()
	...
	НаименованиеЗадания = НСтр("ru = 'Создание документов ""Сверка взаиморасчетов""';
								|en = 'Create documents ""Reconciliation of settlements""'");
...
Процедура СоздатьДокументыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	КоличествоСозданных = 0;
	Для Каждого СтрокаПротокола Из РезультатСоздания.ОтработанныеДокументы Цикл
	...
	ТекстРезультата = НСтр("ru = 'Создано %КоличествоСозданных% из %КоличествоВсего% документов';
							|en = 'Created %КоличествоСозданных% of %КоличествоВсего% documents'");
...
Процедура УстановитьПометкуНаУдаление(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Изменить пометку удаления для выделенных элементов?';
						|en = 'Change deletion flag for selected items?'");
...
Процедура УстановитьПометкуНаУдалениеНаСервере()
	...
	НаименованиеЗадания = НСтр("ru = 'Изменение пометки на удаление документов ""Сверка взаиморасчетов""';
								|en = 'Change the documents deletion mark ""Reconciliation of settlements""'");
	...
	Если Команда.Имя = "Далее"
		...
		Если НЕ (РасчетыСКлиентами ИЛИ РасчетыСПоставщиками ИЛИ РасчетыСКредитором ИЛИ РасчетыСДебитором ИЛИ РасчетыСЛизингодателем) Тогда
			ТекстСообщения = НСтр("ru = 'Не выбран тип расчетов для сверки.';
									|en = 'No calculation type selected for reconciliation.'");
...
Функция ПараметрыЗаполнены()
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаНастройкаФильтров Тогда
		...
		Если ПериодСверки.ДатаОкончания = '00010101' Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указано окончание периода сверки';
																	|en = 'Reconciliation period end is not specified'"),,"ПериодСверки.ДатаОкончания");
			Отказ = Истина;
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаТаблицаРасчетов Тогда
		...
		Если БудетСоздано = 0 И БудетПерезаполнено = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не отмечена ни одна строка для создания или перезаполнения';
																	|en = 'No lines are marked to create or refill.'"),,"ТаблицаРасчетов");
			Отказ = Истина;
		КонецЕсли;
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаНастроек Тогда
		...
		Если ДатаДокументов = '00010101' Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Указана пустая дата документов';
																	|en = 'Empty document date is specified'"),,"ДатаДокументов");
			Отказ = Истина;
		КонецЕсли;
...
Процедура УстановитьОтборКомпоновщика(ИмяОтбора, ЗначениеОтбора)
	...
	ИспользуетсяОтбор = ЭлементОтбора <> Неопределено И ЭлементОтбора.Использование;
	
	Если ЗначениеЗаполнено(ЗначениеОтбора) ИЛИ НЕ ИспользуетсяОтбор Тогда
...
Процедура ОбновитьГиперссылкуОтбора(КнопкаРасширенногоОтбора, Отбор)
	...
	ВидимыеОтборы = СтрРазделить("ДоговорыБезОборотов,ТипРасчетов,Организация,Контрагент,ОсновнойМенеджер", ",");
...
Процедура УстановитьОтборНаФорме(ИмяОтбора)
	...
	Элементы[ИмяОтбора].Доступность = Истина;
	Элементы[ИмяОтбора].ПодсказкаВвода = НСтр("ru = 'Укажите значение отбора';
	...
	Если ЭлементОтбора <> Неопределено Тогда
		...
		Иначе
			...
			Если ЭлементОтбора.Использование ИЛИ ЭлементВИспользуемойГруппеОтбора Тогда
				Элементы[ИмяОтбора].Доступность = Ложь;
				Элементы[ИмяОтбора].ПодсказкаВвода = НСтр("ru = 'Установлен расширенный отбор';
															|en = 'Advanced filter is set'");
			КонецЕсли;
