				...
				Пока ВыборкаЗаписиСтажа.СледующийПоЗначениюПоля("НомерСтрокиСтаж") Цикл
					...
					Иначе//Обе даты заполнены
						...
						Если Запись.ДатаОкончанияПериода > '20011231'  Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дата окончания периода должна быть не позднее 31 декабря 2001 года.';
																										|en = 'period end date cannot be later than December 31, 2001. '"), НомерСтроки, Запись.СотрудникНаименование);
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
					...
					Если УказаныТерриториальныеУсловия Тогда
						Если Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.РКС
							...
							Если НЕ УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий РКС, МКС, МКСР и РКСМ должен быть указан районный коэффициент.';
																											|en = 'regional factor should be specified for site conditions of regions of the Far North and equated localities.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						Иначе
							Если УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий, отличных от РКС, МКС, МКСР и РКСМ, районный коэффициент указываться не должен.';
																											|en = 'regional factor should not be specified for site conditions which differ from the regions of the Far North and equated localities. '"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
