...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение
		...
		ТекстВопроса = НСтр("ru = 'При проведении будет перезаполнено учетное количество во всех строках табличной части. Продолжить?';
							|en = 'Accounting quantity will be refilled in all rows of the tabular section while posting. Continue?'");
...
Процедура Заполнить(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект), НСтр("ru = 'Перед заполнением список товаров будет очищен. Продолжить?';
																						|en = 'The Goods list will be cleared before population. Continue?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьФактПоУчету(Команда)
	...
	Если КоличествоФактЗаполнено() Тогда
		...
		ТекстВопроса = НСтр("ru = 'В табличной части уже есть строки с заполненным фактическим количеством.
			...
			|en = 'There are rows with populated actual quantity in the tabular section.
			|This information will be lost while filling in. Continue?'");
...
Функция СкладПомещениеЗаполнены()
	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.Помещение)
			...
			ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Помещение"".
				...
				|To perform the action, specify a wareroom in which goods are recounted'");
			ОчиститьСообщения();
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Склад"".
			...
			|en = 'Warehouse is not populated. 
			|To perform the action, specify a warehouse in which goods are recalculated'");
		ОчиститьСообщения();
		...
		Если ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач 
			...
			Если ТаблицаНоменклатуры.Количество() > 0 Тогда
				МассивСтрок.Добавить(НСтр("ru = 'Не по всем расхождениям оформлены складские акты.';
											|en = 'Warehouse acts are registered not for all discrepancies.'")+ " ");
		...
		Если ПравоДоступа("Изменение", Метаданные.Документы.ИнвентаризационнаяОпись) Тогда 
			...
			Если КоличествоОрганизаций > 0 Тогда
				МассивСтрок.Добавить(НСтр("ru = 'Этот документ учитывается в';
											|en = 'This document is recorded in'")+ " ");
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'инвентаризационных описях';
																		|en = 'stocktaking sheets'"),,,,"СписокОписей"));
				Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда 		
					МассивСтрок.Добавить(" " + НСтр("ru = 'по';
													|en = 'to'")+ " " + КоличествоОрганизаций + " ");
					МассивСтрок.Добавить(ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
						КоличествоОрганизаций, 
						НСтр("ru = 'организации';
							|en = 'companies'"), НСтр("ru = 'организациям';
														|en = 'companies'"), НСтр("ru = 'организациям';
																					|en = 'companies'"),"м"));
				КонецЕсли;
			...
			Если КоличествоОрганизацийБезОписей > 0 Тогда
				...
				МассивСтрок.Добавить(ТекстРазделителя + НСтр("ru = 'Необходимо';
															|en = 'Required'") + " ");
				МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'сформировать';
																		|en = 'generate'"),,,,"СформироватьОписи"));
				МассивСтрок.Добавить(Символы.НПП + НСтр("ru = 'описи по';
														|en = 'sheets by'") + Символы.НПП + КоличествоОрганизацийБезОписей + " " + 
