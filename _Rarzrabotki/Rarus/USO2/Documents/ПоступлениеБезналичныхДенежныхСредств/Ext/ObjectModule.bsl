...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	УСОERP_МодификацияКонфигурации.ВосстановитьДанныеЗаполнения(ДанныеЗаполнения,ДокОснование);
	// {УСО 2.4}:{Управление строительной организацией}}}
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Тогда
		Если Не ЗначениеЗаполнено(ПодотчетноеЛицо) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Сотрудник"" не заполнено.';
																	|en = 'Employee is not filled in.'"), , "ПодотчетноеЛицо", "Объект", Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		...
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств Тогда
			...
			ДенежныеСредстваСервер.СформироватьВозмещениеРасходовПоСоциальномуСтрахованию(, ЭтотОбъект, Отказ);
		...
		Если Выборка.УдержатьВознаграждение Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить поступление безналичных денежных средств на основании документа %1, поскольку в нем удержано вознаграждение';
					|en = 'It is not required to enter inpayment based on document %1 as remuneration is deducted in it'"),
				ОтчетКомитенту);
...
Процедура ПроверитьЗаполнениеПоВозвратуНеперечисленнойЗарплаты(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНеперечисленнойЗарплатыПоЗарплатномуПроекту
		И ЗначениеЗаполнено(ПодтверждениеЗачисленияЗарплаты) Тогда
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНеперечисленныхДС
		И ЗначениеЗаполнено(ДокументВыдачи) Тогда
