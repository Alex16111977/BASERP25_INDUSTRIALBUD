...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	Иначе
		ТекстИслючения = НСтр("ru = 'Не поддерживается получение параметров серий для акта с типом основания %ТипАкта%.';
								|en = 'Receipt of series parameters for certificate with basis type %ТипАкта% is not supported.'");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтКлиента Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'после возврата от клиента';
										|en = 'after return from a customer'");
	//++ НЕ УТ
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриемкаТоваровНаХранение Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'после приемки товаров на хранение';
										|en = 'after goods receiving for storage'");
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтХранителя Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'после поступления товаров от хранителя';
										|en = 'after goods receipt from the custodian'");
	//-- НЕ УТ
	Иначе
	...
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Акт о расхождениях %1 %2 от %3';
																				|en = 'Discrepancy certificate %1 %2 from %3'"),
...
#Область СозданиеПоРезультатамПроверкиИПодбораМОТП

// Создает акт по результатам работы в форме проверки и подбора МОТП
// 
// Параметры:
// 	ПараметрыОкончанияСканирования - Структура - (См. ПроверкаИПодборМОТП.ЗафиксироватьРезультатПроверкиИПодбора)
Процедура СоздатьПоРезультатамПроверкиИПодбораМОТП(ПараметрыОкончанияСканирования, Проводить) Экспорт
	...
	ТекстЗапросаОпределениеСклада = ИнтеграцияМОТПУТ.ТекстЗапросаОпределениеСклада(ПараметрыОкончанияСканирования.ПроверяемыйДокумент);
