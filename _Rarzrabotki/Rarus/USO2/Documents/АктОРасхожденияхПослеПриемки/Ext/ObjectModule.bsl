	...
	Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
		...
		Если ПроверкиПоЗаказуТребуются Тогда 
			Если Не ЗначениеЗаполнено(ТекущаяСтрока.ЗаказПоставщику) И ЗначениеЗаполнено(ТекущаяСтрока.ДокументОснование)
				...
				ТекстОшибки = НСтр("ru = 'Не заполнено поле ""Заказ поставщика"" в строке %НомерСтроки% списка ""Товары""';
									|en = 'Purchase order is not populated in line %НомерСтроки% of the Goods list'");
		...
		Если Статус <> Перечисления.СтатусыАктаОРасхождениях.НеСогласовано 
			...
			ТекстОшибки = НСтр("ru = 'Не заполнено поле ""Как отработать расхождение"" в строке %НомерСтроки% списка ""Товары""';
								|en = 'The ""How to process discrepancy"" field is not populated in line %НомерСтроки% of the Goods list'");
			ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекущаяСтрока.НомерСтроки);
...
Процедура ЗаполнитьНаОсновании(Основание, НеПроверятьВозможностьВвода = Ложь)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") 
		...
		ТекстОшибки = НСтр("ru = 'Создание ""Акта о расхождениях после поступления"" при закупке через подотчетное лицо не предусмотрено.';
							|en = 'Cannot create ""Discrepancy certificate after receipt"" on purchase via advance holder.'");
