...
Процедура СформироватьЗависимыеОтТипаОснованияЗаголовки()
	...
	Если Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриобретениеТоваровУслуг Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после поступления (Создание)';
						|en = 'Discrepancy certificate after receipt (Creation)'");
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтКлиента Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после возврата товаров от клиента (Создание)';
						|en = 'Discrepancy certificate after goods return from customer (Creation) '");
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриемкаТоваровНаХранение Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после приемки товаров на хранение (Создание)';
						|en = 'Discrepancy certificate after goods receiving for storage (Creation)'");
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтХранителя Тогда
		Заголовок = НСтр("ru = 'Акт о расхождениях после поступления товаров от хранителя (Создание)';
						|en = 'Act of discrepancies after goods receipt from the custodian (Creation)'");
		Элементы.Договор.Подсказка = НСтр("ru = 'Договор, в рамках которого оформлено поступление';
											|en = 'Contract under which the receiving is registered'");
		Элементы.Соглашение.Подсказка = НСтр("ru = 'Соглашение, по которому был передан товар.';
											|en = 'Agreement under which the goods have been transfered.'");
	//-- НЕ УТ
	КонецЕсли;
...
Процедура ЗаполнитьФактПоПриемке(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьФактПоПриемкеЗавершение", ЭтотОбъект),
	                                        НСтр("ru = 'Заполнить товары по факту по фактической приемке на ордерных складах?';
												|en = 'Fill in the actual goods by actual receiving in advanced warehouse?'"),
	                                        СписокКнопок);
	...
	Если НоменклатураКлиент.ДляУказанияСерийНуженСерверныйВызов(ЭтаФорма,ПараметрыУказанияСерий,Текст, ТекущиеДанные) Тогда
		...
		Если ТекущиеДанные.ЗаполненоПоОснованию
			...
			ПоказатьПредупреждение(,НСтр("ru = 'Серия в строке из документа-основания не редактируется, добавьте новую строку.';
										|en = 'Series cannot be edited in a line of the base document, add a new line.'"));
			Возврат;
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Списать за счет сторонней компании';
																|en = 'Write off at the expense of a third party'"));
...
Процедура УправлениеРеквизитамиФормыВЗависимостиОтТипаОснования()
	...
	ИначеЕсли Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратТоваровОтХранителя Тогда
		...
		Элементы.Партнер.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбора);
		Элементы.Партнер.Заголовок       = НСтр("ru = 'Партнер';
												|en = 'Partner'");
		
		Элементы.ТоварыКомментарийПоставщикаПодвал.Заголовок = НСтр("ru = 'Комментарии партнера по расхождениям в строке';
																	|en = 'Partner comments to the line discrepancies'");
		Элементы.ТоварыЕстьКомментарийПоставщика.Заголовок   = НСтр("ru = 'партнера';
																	|en = 'partner'");
		...
		//Товары
		Элементы.ТоварыЗаполнитьПоОснованиям.Заголовок  = НСтр("ru = 'По поступлениям от хранителя';
																|en = 'By receipts from the custodian'");
		Элементы.ТоварыНоменклатураПоставщика.Видимость = Ложь;
		Элементы.ТоварыВидЦеныПоставщика.Видимость      = Ложь;
		Элементы.ТоварыДокументОснование.Заголовок      = НСтр("ru = 'Поступление от хранителя';
		...
		//Итоги
		Элементы.ПоОснованиям.Заголовок = НСтр("ru = 'По поступлениям';
												|en = 'By receipts'");
