...
Процедура ОтменитьВыделенныеСтроки(Команда)
	...
	Если Объект.Товары.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Таблица товаров не заполнена.';
													|en = 'Goods table is not populated.'"));
		Возврат;
...
Процедура ОтменитьНепоставленныеСтрокиФрагмент(Знач ПричинаОтмены, Знач ПроверятьОстатки)
    ...
    Если КоличествоСтрокОтменено = 0 Тогда
        ...
        НСтр("ru = 'По всем строкам заказа оформлено поступление. Строки не могут быть отменены.';
			|en = 'Receipt is registered for every line of the order. The lines cannot be canceled.'"),
        БиблиотекаКартинок.Информация32);
    ...
    Иначе
        ...
        Если ПроверятьОстатки Тогда
            ...
            НСтр("ru = 'Все непоставленные строки заказа отменены.';
				|en = 'All undelivered order lines are canceled.'"),
            БиблиотекаКартинок.Информация32);
        ...
        Иначе
            ...
            НСтр("ru = 'Все строки заказа отменены.';
				|en = 'All order lines are canceled.'"),
            БиблиотекаКартинок.Информация32);
...
Процедура ЗаполнитьДатуПоступленияДляВыделенныхСтрок(Команда)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'В документе не выбраны строки для заполнения. Дата поступления не будет заполнена.';
									|en = 'No lines to fill in are selected in the document. The receipt date will not be filled in.'");
...
Процедура ЗаполнитьДатуПоступления(МассивВыделенныхСтрок = Неопределено)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'В документе нет строк для заполнения. Дата поступления не будет заполнена.';
									|en = 'No lines to fill in in the document. The date received will not be populated.'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
	КонецЕсли;
	...
	Если СписокЗаказов.Количество() > 0 Тогда
		...
		Если СписокЗаказов.Количество() = 1 Тогда
			ТекстВопроса = НСтр("ru = 'В информационной базе уже существует заказ с таким же номером по данным поставщика. Продолжить?';
								|en = 'An order with such number already exists in the infobase according to data of the supplier. Continue?'");
		...
		Иначе
			ТекстВопроса = НСтр("ru = 'В информационной базе уже существуют заказы с таким же номером по данным поставщика. Продолжить?';
								|en = 'Orders with such number already exist in the infobase according to data of the supplier. Continue?'");
...
Процедура ЗаполнитьУстановитьРеквизитыДоставкиСервер(ИмяЭлементаФормы);
	...
	УСОERP_МатериальноТехническоеОбеспечение.ЗаказПоставщикуФормаДокументаРеквизитыДоставки(Элементы, Объект);
	//>{УСО 2.0}:{Управление строительной организацией}{Мокшанов А.О.}
...
#Область ПодключаемыеДействияДляРеквизитовПодсистемыОМТО 
//<{УСО 2.0}:{Управление строительным предприятием}{Зайко Е.Н.}

&НаКлиенте
Процедура Подключаемый_ДоставкаПриИзменении(Элемент)
	УСОERP_МатериальноТехническоеОбеспечениеКлиент.Подключаемый_ДоставкаПриИзменении(ЭтаФорма, Объект.УСОERP_Доставка);
КонецПроцедуры
