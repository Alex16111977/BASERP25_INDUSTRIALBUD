		...
		Если Объект.ПричинаСовмещения = Перечисления.ПричиныСовмещения.ИсполнениеОбязанностей Тогда
			...
			Иначе
				ЗаголовокПункта = "% " + НСтр("ru = 'от ФОТ';
												|en = 'from salary budget'") + " " + ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФамилияИнициалы(ФамилияИООтсутствующегоСотрудника);
			КонецЕсли;
...
Процедура УстановитьЗаголовокГруппыФОТСовмещающегоСотрудника()
	...
	Если ЭлементСпособРасчетаДоплатыПоФОТСовмещаемого <> Неопределено Тогда
		...
		Иначе
			ЗаголовокПункта = "% " + НСтр("ru = 'от ФОТ';
											|en = 'from salary budget'") + " " + ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФамилияИнициалы(ФамилияИОСовмещающегоСотрудника);
		КонецЕсли;
