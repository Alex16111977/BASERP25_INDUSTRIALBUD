		...
		Иначе
			...
			Если Не Объект.ПометкаУдаления И ПроверитьЗаполнение() Тогда
				Если Модифицированность ИЛИ Объект.Ссылка.Пустая() Тогда
					...
					ТекстВопроса = НСтр("ru = 'Расшифровка возможна только в проведенном документе. Провести документ?';
										|en = 'Explanation is possible only in the posted document. Post the document?'");
			...
			Иначе
				ТекстПредупреждения = НСтр("ru = 'Расшифровка возможна только в проведенном документе';
											|en = 'Explanation is possible only in a posted document'");
...
Процедура ВидБюджетаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Не ВидБюджетаОбработкаВыбораНаСервере(ВыбранноеЗначение, ИспользоватьДляВводаПлана) Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Вид бюджета не доступен для выбора в экземпляре бюджета:
										 |%1';
										 |en = 'Cannot select a budget kind in the budget entry document:
										 |%1'");
...
Процедура ВыгрузитьВExcel(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Перед выгрузкой в эксель документ следует записать';
										|en = 'Write the document before export it to Excel'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьДанныеПередЗаписью(ТекущийОбъект)
	...
	// {УСО 2}:{Управление строительной организацией}{Мокшанов А.О.} 29.01.2018 {{
...
Функция СоздатьПрисоединенныйФайлXLS97(ИДТаблицы, КолонкиПоИзмерениям)
	...
	Исключение
		СообщениеОбОшибке =
			НСтр("ru = 'Не удалось выгрузить бюджет Excel. 
				...
				|Подробности:';
				|en = 'Cannot export the budget to Excel.
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Бюджетирование.Ввод экземпляров бюджета';
				|en = 'Budgeting.Budget entry document input'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Предупреждение, , ,
			НСтр("ru = 'Не удалось установить автофильтр при выгрузке бюджета в Excel';
				|en = 'Cannot set an auto filter while exporting the budget to Excel'"));
	КонецПопытки;
...
Функция ЗагрузитьИзExcelНаКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Исключение
		СообщениеОбОшибке = 
			...
			|Подробности: %1';
			|en = 'Cannot import data from Excel.
			|Make sure Microsoft Excel is installed on the computer.
			|Details: %1'");
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ru = 'Не удалось прочитать данные из файла. Подробности: %1';
								|en = 'Cannot read data from the file. Details: %1'");
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
...
Процедура ЗагрузитьБюджетНаСервере(АдресВоВременномХранилище)
	...
	Для Каждого СтрокаРезультат Из Результат.СписокРедактированияБюджета Цикл
		...
		ОтразитьИзмененияВДанных(НоваяСтрока.ПолучитьИдентификатор(),, ЕстьИзмененияВСвязанныхЯчейках);
	КонецЦикла;
	ЗапуститьОтражениеИзмененияБюджета(ЕстьИзмененияВСвязанныхЯчейках);
...
&НаСервере
Процедура ОтразитьИзмененияВДанных(ИндексСтроки, ИмяКолонки = "", ЕстьИзмененияВСвязанныхЯчейках = Ложь)
...
Процедура ДобавитьПримечание(Команда)
	...
	ПоказатьВводСтроки(Оповещение, Примечание, НСтр("ru = 'Примечание';
													|en = 'Note'"), 500, Истина);
...
Процедура ДобавитьСтрокуВБюджетЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВставки = НСтр("ru = 'Дубли строк';
								|en = 'Line duplicates'") Тогда
...
Функция ВставитьОбластьНаСервере(СоответствиеИзмерений, ДополнительныеПараметры)
	...
	Для Каждого СтрокаСтруктурыЯчеек Из НайденныеСтроки Цикл
		...
		Если СтрокиПроверки.Количество() Тогда
			Возврат НСтр("ru = 'Дубли строк';
						|en = 'Line duplicates'"); // дубли строк
		КонецЕсли;
	КонецЦикла;
