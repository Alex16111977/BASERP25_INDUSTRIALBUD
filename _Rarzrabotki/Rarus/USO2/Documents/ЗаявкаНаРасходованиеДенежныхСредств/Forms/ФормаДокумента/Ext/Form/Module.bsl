...
Процедура ЗаполнитьПодтверждающиеДокументы(Команда)
	...
	ОбщегоНазначенияУТКлиент.ПроверитьВозможностьЗаполненияТабличнойЧасти(
		Оповещение, ЭтаФорма, Объект.ПодтверждающиеДокументы, Новый Структура("ТипПлатежаФЗ275", НСтр("ru = 'Тип платежа 275-ФЗ';
																										|en = 'Payment type 275-FZ'")), Ложь);
	//-- НЕ УТ
	Возврат;
...
Процедура ЗаполнитьДанныеОбОплатеЗаявки()
	...
	Если ФактическаяОплата Тогда
		...
		КонецЕсли;
	Иначе
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Не оплачена';
																|en = 'Not paid'"), , Оформление.ЦветПредупреждение));
	КонецЕсли;
