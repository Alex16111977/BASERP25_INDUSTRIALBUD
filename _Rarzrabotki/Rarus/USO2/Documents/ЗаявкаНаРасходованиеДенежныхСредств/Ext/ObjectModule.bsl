...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Статус = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.Согласована
		...
		ТекстОшибки = НСтр("ru = 'У вас нет права согласования заявок на расходование денежных средств';
							|en = 'You are not authorized to approve expenditure requests'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			ЭтотОбъект,,,
			Отказ);
	...
	ИначеЕсли Статус = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.КОплате
		...
		ТекстОшибки = НСтр("ru = 'У вас нет права утверждения к оплате заявок на расходование денежных средств';
							|en = 'You are not authorized to confirm expenditure requests'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстОшибки,
			ЭтотОбъект,,,
			Отказ);
		...
		Если ЗаполнениеПоДоговору Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить Заявку на расходование ДС на основании договора %1';
					|en = 'Entering expenditure request on basis of contract %1 is not required'"),
				ДокументОснование);
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить Заявку на расходование ДС на основании документа %1';
					|en = 'Entering expenditure request on basis of document %1 is not required'"),
				ДокументОснование);
		КонецЕсли;
	...
	Если РезультатЗапроса.Пустой() Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не требуется вводить заявку на основании документа %1';
				|en = 'Entering request on basis of document %1 is not required'"),
			ДокументОснование);
		ВызватьИсключение Текст;
	Иначе
		...
		ВызватьИсключение
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Заявка на расходование ДС не требуется для документа %1';
					|en = 'Expenditure request is not required for document %1'"), ДокументОснование);
	КонецЕсли;
...
Процедура ЗаполнитьПоВедомостямНаВыплатуЗарплаты(Знач ДанныеКоманды, ДанныеЗаполнения, МассивВедомостей)
	...
	Если РасшифровкаПлатежа.Количество() = 0 Тогда
		Текст = НСтр("ru = 'На основании выбранных ведомостей заявки уже созданы или оплата не требуется.';
					|en = 'Requests are already created based on selected lists and payment is not required.'");
		ВызватьИсключение Текст;
	КонецЕсли;
	...
	Если Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
		...
		Пока Выборка.Следующий() Цикл
			ТекстОшибки = НСтр("ru = 'По документу %Ведомость% уже оформлена %Заявка%';
								|en = '%Заявка% is already registered by document %Ведомость%'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Валюта взаиморасчетов в строке %1 списка ""Расшифровка платежа"" не соответствует валюте документа %2';
				|en = 'Currency of mutual settlements in line %1 of ""Amount allocation"" list does not match the currency of document %2'"),
			Выборка.НомерСтроки,
			Валюта);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
...
Процедура ПроверитьЗаполнениеДоговоровГОЗ(Отказ, РасшифровкаБезРазбиения)
	...
	Если СписокДоговоров.Количество() > 1 Тогда
		...
		Текст = НСтр("ru = 'Не допускается заявка на оплату за счет средств ГОЗ более чем по одному договору с поставщиком / исполнителем.';
					|en = 'You cannot make a payment request out of SDO funds for more than one contract with supplier / assignee.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
			КлючДанных,
			,
