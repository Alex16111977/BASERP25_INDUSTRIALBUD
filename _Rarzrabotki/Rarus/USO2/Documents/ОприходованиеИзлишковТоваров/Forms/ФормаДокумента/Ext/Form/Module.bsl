...
Процедура Заполнить(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект), НСтр("ru = 'Табличная часть будет очищена и заполнена всеми товарами, по которым нужно оформить оприходование. Продолжить?';
																						|en = 'Tabular section will be cleared and filled in with all the goods for which recording as received is required. Continue? '"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
	...
	Пока ПредварительныеДанные.Следующий() Цикл
		Если ПредварительныеДанные.Количество = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не было произведено ни одного расчета себестоимости товаров по организации ""%Организация%"".';
									|en = 'Goods cost of company ""%Организация%"" was not calculated at all.'");
