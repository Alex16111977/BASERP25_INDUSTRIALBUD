...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Коэффициенты = РаботаСКурсамивалютУТ.ПолучитьКоэффициентыПересчетаВалюты(Запрос.Параметры.ВалютаСчетаФактуры,
		Запрос.Параметры.ВалютаСчетаФактуры, Запрос.Параметры.Период);
...
// {УСО 2.4}:{Управление строительной организацией}{{
#Область ТекстУСОERP_РаспределениеИнвестиционногоНДС
	ТекстУСОERP_РаспределениеИнвестиционногоНДС = "
	|
	|ВЫБРАТЬ
	...
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = ДокументУСОERP_РаспределениеИнвестиционногоНДС.Валюта
	|			ТОГДА ТаблицаТовары.СуммаБезНДС + ТаблицаТовары.СуммаНДС
	|		КОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл,0) = 0
	|			ТОГДА ВЫРАЗИТЬ((ТаблицаТовары.СуммаБезНДС + ТаблицаТовары.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2))
	...
	|	ДокументыОснования.ДокументОснование КАК ДокументОснование
	|ИЗ
	|	Документ.УСОERP_РаспределениеИнвестиционногоНДС.СдачаИнвестору КАК ТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УСОERP_РаспределениеИнвестиционногоНДС КАК ДокументУСОERP_РаспределениеИнвестиционногоНДС
	|		ПО ТаблицаТовары.Ссылка = ДокументУСОERP_РаспределениеИнвестиционногоНДС.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументыПоиска КАК ДокументыОснования
	...
	|ГДЕ
	|	ТаблицаТовары.СчетФактура = &Ссылка
	|";
#КонецОбласти  
// {УСО 2.4}:{Управление строительной организацией}}}
	...
	;
	// {УСО 2.4}:{Управление строительной организацией}{{
	ТекстЗапроса = ТекстЗапроса + " ОБЪЕДИНИТЬ ВСЕ " + ТекстУСОERP_РаспределениеИнвестиционногоНДС;
	// {УСО 2.4}:{Управление строительной организацией}}}
...
#Область ТекстПолученныеСчетаФактурыКорректировочный
	...
	|	ДанныеОснований.НомерИсходногоДокумента,
	|	ДанныеОснований.ДатаИсходногоДокумента,
	|	&НомерНаПечать,
	|	&Период,
	|	ДанныеОснований.НомерИсправленияИсходногоДокумента,
	|	ДанныеОснований.ДатаИсправленияИсходногоДокумента,
	|	&НомерИсправления,
	|	ВЫБОР
	|		КОГДА &Исправление
	|			ТОГДА &Период
	...
	|	
	|ИЗ
	|	ВтТаблицаКорректировки КАК ТаблицаСуммДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументыОснования КАК ДанныеОснований
	|			ПО ТаблицаСуммДокументов.ДокументОснование = ДанныеОснований.ДокументОснование
	|ГДЕ
	|	НЕ ТаблицаСуммДокументов.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя)
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Маркировка                = ЭлектронноеВзаимодействиеМОТП.ЧастичноеСодержимое(Маркировка);
	
	СтруктураДанныхДляПечати  = Новый Структура(
		"РезультатПоШапке, РезультатПоТабличнойЧасти, РезультатПоИсходнымДанным, Маркировка",
		РезультатПоШапке,
...
Процедура СформироватьВременнуюТаблицуТоваровДляПечати(МенеджерВременныхТаблиц, МассивОснований, ПараметрыПечати)
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		Если ИмяОбъекта = "АктВыполненныхРабот"
				...
				Или ИмяОбъекта = "УСОERP_РаспределениеИнвестиционногоНДС"
				// {УСО 2.0}:{Управление строительной организацией}{Юркевич М.И.} }}
		...
		Если ИмяОбъекта = "КорректировкаРеализации" Или ИмяОбъекта = "ВозвратТоваровОтКлиента" Или ИмяОбъекта = "ВозвратТоваровМеждуОрганизациями" Тогда
			ТекстЗамены =
...
Функция ТекстЗапросаДанныхМаркировки()
...
Функция ТекстЗапросаТаблицаОснований()
	...
	|ГДЕ
	|	ТаблицаОснований.Ссылка В(&МассивОбъектов)
	|	И ТаблицаОснований.ИсходныйДокумент = ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка)
