	...
	Если РеквизитыДляПроведения.ДокументРассчитан Тогда
		...
		Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
			...
			УчетНДФЛРасширенный.ЗарегистрироватьДоходыИСуммыНДФЛПоВременнойТаблицеНачислений(
				...
				РеквизитыДляПроведения.ПериодРегистрации, РеквизитыДляПроведения.ПорядокВыплаты, РеквизитыДляПроведения.ПланируемаяДатаВыплаты, ДанныеДляПроведения, Истина, Истина, РеквизитыДляПроведения.ДоходПолученНаТерриторииРФ);
			...
			УчетНачисленнойЗарплатыРасширенный.ЗарегистрироватьУдержанияИЗаймы(ДанныеДляПроведенияУчетЗарплаты, Отказ,
				ДанныеДляПроведения.УдержанияПоСотрудникам, ДанныеДляПроведения.УдержанияЗаймов, ДанныеДляПроведения.НалогНаМатериальнуюВыгоду);
	...
	Если Пользователи.РолиДоступны("ПолныеПрава,ДобавлениеИзменениеРабочегоВремениНачисленнойЗарплаты,ЧтениеРабочегоВремениНачисленнойЗарплаты", , Ложь) Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьРасчетЗарплатыРасширенная") Тогда
			...
			КомандаПечати.Представление  = НСтр("ru = 'Расчет пособия к листку нетрудоспособности';
												|en = 'Sick pay calculation for the sick leave record'");
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ДоплатаЗаДниНетрудоспособности") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Доплата за дни нетрудоспособности';
				|en = 'Extra pay for days of incapacity for work '"),
...
Процедура ВывестиДанныеРасчетаПособияВТабличныйДокумент(Макет, ДокументРезультат, ДанныеДляПечатиРасчетаПособия, ОбъектыПечати)
	...
	Пока ВыборкаДляШапки.Следующий() Цикл
		...
		Если ВыборкаДляШапки.РасчетПоПравилам2010Года Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = '%1: Печать расчета пособия по правилам 2010 года не поддерживается.';
											|en = '%1: Printing of allowance calculation by 2010 year rules is not supported.'"), ВыборкаДляШапки.Ссылка);
		...
		СекцияШапка.Параметры.ПодписьЛетСтажа = ПерсонифицированныйУчет.ФормаМножественногоЧисла(НСтр("ru = 'год';
																										|en = 'year'"), НСтр("ru = 'года';
																															|en = 'year'"), НСтр("ru = 'лет';
																																				|en = 'years'"), СекцияШапка.Параметры.СтажРасширенныйЛет);
		СекцияШапка.Параметры.ПодписьМесяцевСтажа = ПерсонифицированныйУчет.ФормаМножественногоЧисла(НСтр("ru = 'месяц';
																											|en = 'month'"), НСтр("ru = 'месяца';
																																|en = 'month'"), НСтр("ru = 'месяцев';
																																						|en = 'months'"), СекцияШапка.Параметры.СтажРасширенныйМесяцев);
		...
		Если ВыборкаДляШапки.ФинансированиеФедеральнымБюджетом = Перечисления.ОснованияФинансированияПособияФедеральнымБюджетом.ЗачетНестраховыхПериодов Тогда
			...
			Если НестраховыхПериодовЛет > 0 Или НестраховыхПериодовМесяцев > 0 Тогда
				...
				СекцияШапка.Параметры.ПодписьЛетНестраховогоСтажа 		= ПерсонифицированныйУчет.ФормаМножественногоЧисла(НСтр("ru = 'год';
																																	|en = 'year'"), НСтр("ru = 'года';
																																						|en = 'year'"), НСтр("ru = 'лет';
																																											|en = 'years'"), СекцияШапка.Параметры.ЛетНестраховогоСтажа);
				СекцияШапка.Параметры.ПодписьМесяцевНестраховогоСтажа 	= ПерсонифицированныйУчет.ФормаМножественногоЧисла(НСтр("ru = 'месяц';
																																	|en = 'month'"), НСтр("ru = 'месяца';
																																						|en = 'month'"), НСтр("ru = 'месяцев';
																																												|en = 'months'"), СекцияШапка.Параметры.МесяцевНестраховогоСтажа);
			КонецЕсли;
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(ДнейПо50процентов) Тогда
				ОписаниеОплаченыхДнейПособия = СтрШаблон(НСтр("ru = '%1%% за %2%3';
																|en = '%1%% for %2%3'"), ПроцентОплаты, ?(ДнейВПолномРазмере = 0, " -", ДнейВПолномРазмере), ФормаМножественногоЧислаДней(ДнейВПолномРазмере));
			ИначеЕсли Не ЗначениеЗаполнено(ДнейВПолномРазмере) Тогда
				ОписаниеОплаченыхДнейПособия = СтрШаблон(НСтр("ru = '50%% за %1%2';
																|en = '50%% for %1%2'"), ДнейПо50процентов, ФормаМножественногоЧислаДней(ДнейПо50процентов));
			Иначе
				ОписаниеОплаченыхДнейПособия = СтрШаблон(НСтр("ru = '%1%% за %2%3 и 50%% за %4%5';
																|en = '%1%% for %2%3 and 50%% for %4%5'"), ПроцентОплаты, ДнейВПолномРазмере, ФормаМножественногоЧислаДней(ДнейВПолномРазмере), ДнейПо50процентов, ФормаМножественногоЧислаДней(ДнейПо50процентов));
			КонецЕсли;
			
			ОписаниеНазначениеПособия = СтрШаблон(НСтр("ru = 'Назначено пособие %1 в размере %2.';
														|en = 'Allowance %1 in the amount of %2 is assigned.'"), ПричинаНетрудоспособностиСтрокой(ВыборкаДляШапки.ПричинаНетрудоспособности), ОписаниеОплаченыхДнейПособия);
			
			Если ВыборкаДляШапки.Снижено Тогда
				ОписаниеНазначениеПособия = ОписаниеНазначениеПособия + Символы.ПС + СтрШаблон(НСтр("ru = 'Пособие снижено с %1';
																									|en = 'Allowance amount is reduced from %1'"), Формат(ВыборкаДляШапки.ДатаНачалаПоловиннойОплаты, "ДЛФ=DD"));
			КонецЕсли;
		...
		Если ТребуетсяВыводРайонногоКоэффициента(ВыборкаДляШапки) Тогда
			СекцияРасчетПособияШапка.Параметры.УчетРК = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'с учетом РК: %1';
																													|en = 'considering RC: %1'"), ВыборкаДляШапки.РайонныйКоэффициентРФнаНачалоСобытия);
		КонецЕсли;
	...
	Если Количество = 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Добавлен родственник сотрудника %1. Нажмите на сообщение, чтобы заполнить степень родства и дату рождения.';
				|en = 'Relative of employee %1 was added. Click the message to specify relationship and birth date.'"),
			ФИОРодственника);
