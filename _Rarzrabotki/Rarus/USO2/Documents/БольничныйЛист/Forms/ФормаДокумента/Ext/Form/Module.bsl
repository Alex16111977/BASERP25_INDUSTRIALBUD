...
Процедура ПолучитьИзФСС(Команда)
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбрана организация.';
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	Если ЗарплатаКадрыРасширенныйКлиентПовтИсп.СтандартныеПодсистемыВерсии302() Тогда
		
		ВызываемыйМодуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ФайловаяСистемаКлиент");
		ПараметрыЗагрузки = ВызываемыйМодуль.ПараметрыЗагрузкиФайла();
		...
		ВызываемыйМодуль.ЗагрузитьФайл(Обработчик, ПараметрыЗагрузки);
		
	Иначе
		
		ВызываемыйМодуль = ОбщегоНазначенияКлиент.ОбщийМодуль("СтандартныеПодсистемыКлиент");
		ВызываемыйМодуль.ПоказатьПомещениеФайла(Обработчик, УникальныйИдентификатор, "", ПараметрыДиалога);
	...
	Если ЗначениеЗаполнено(Объект.НомерЛисткаНетрудоспособности) И ЗначениеЗаполнено(Объект.ДатаВыдачиЛисткаНетрудоспособности) Тогда
		ЗаголовокГиперссылкиЭЛН = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Заполнены данные ЛН №%1 от %2';
				|en = 'Data on sick leave record data No. %1 from %2 is populated'"),
			Объект.НомерЛисткаНетрудоспособности,
	...
	Если Элементы.СтраницаПилотныйПроектФСС.Видимость Тогда
		...
		ГиперссылкаВводДополнительныхДанных = ?(
			...
			НСтр("ru = 'Заполнить данные для реестра сведений, передаваемого в ФСС';
				|en = 'Populate data for the information registry transferred to SSF'"),
			ЗаголовокГиперссылкиЭЛН);
	КонецЕсли;
КонецПроцедуры
