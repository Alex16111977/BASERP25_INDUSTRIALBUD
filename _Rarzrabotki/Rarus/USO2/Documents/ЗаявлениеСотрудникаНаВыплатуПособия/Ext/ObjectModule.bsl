	...
	Если СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.НаКартуМИР Тогда
		Если ЗначениеЗаполнено(НомерКартыМИР)
			...
			Текст = НСтр("ru = 'Номер карты МИР заполнен некорректно (ожидается 16 или 19 чисел без пробелов).';
						|en = 'MIR card number is specified incorrectly (it is supposed to have 16 or 19 digits without spaces).'");
...
Процедура ПроверитьРеквизитыПолучателя(Отказ)
	...
	Если ЗначениеЗаполнено(СтрЗаменить(СНИЛСПолучателя, "-",""))
		...
		Текст = НСтр("ru = 'СНИЛС получателя пособия указан не верно. %1';
					|en = 'Beneficiary''s IIAN is incorrect. %1'");
...
Процедура ПроверитьРеквизитыИзвещенияИзФСС(Отказ)
	...
	Если ЗаполненРеквизитНомер И Не ЗаполненРеквизитДата Тогда
		Текст = НСтр("ru = 'В группе ""Извещение о представлении недостающих сведений в ФСС"" не заполнена Дата, но заполнен Номер.';
					|en = 'In the ""Notification of providing missing information to SSF"" group, Date is not specified, but Number is filled in.'");
	...
	ИначеЕсли ЗаполненРеквизитДата И Не ЗаполненРеквизитНомер Тогда
		Текст = НСтр("ru = 'В группе ""Извещение о представлении недостающих сведений в ФСС"" не заполнен Номер, но заполнена Дата.';
					|en = 'In the ""Notification of providing missing information to SSF"" group, Number is not specified, but Date is filled in.'");
