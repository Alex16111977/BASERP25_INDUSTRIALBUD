		...
		Если Отказ Тогда
			...
			ТекстВопроса = СтрШаблон(
				НСтр("ru = 'В строках списка ""%1"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';
					|en = 'The %1 list lines contain dates with planning limits - lines will be removed. Remove the lines?'"),
...
Процедура ПересчитатьПотребностиПланаПроизводства(Команда)
	...
	Если НЕ ЗаполнятьСпецификацию Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Для использования расчета потребностей в материалах, видах РЦ и трудовых ресурсах необходимо включить соответствующую настройку в ""Сценарии"".';
									|en = 'To use calculation of demands for materials, work center kinds and human resources, enable the corresponding setting in ""Scripts"".  '"));
		Возврат;
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'УИ_Спецификация';
								|en = 'UI_BOM'"),"ИдентификаторСпецификация");
	...
	Если ЗаполнятьСпецификацию Тогда
		КолонкиПоиска.Добавить(НСтр("ru = 'Спецификация';
									|en = 'Bill of resources'"),"ЗагружаемаяСпецификация");
	КонецЕсли; 
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'План производства +15%';
																	|en = 'Production plan +15%'"), "[ПланыПроизводства] * 1.15");
		...
		Если ИнтерактивныйРежим Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не найдены вхождения выбранного полуфабриката';
					|en = 'Occurrences of the selected semi-finished product are not found'"),
				Объект.Ссылка);
		КонецЕсли;
...
Процедура ПоказатьИнформационныйТекст(Полуфабрикат)
	...
	МассивСтрок.Добавить(НСтр("ru = 'Установлен отбор по вхождениям полуфабриката';
								|en = 'Filter is set by semi-finished product occurrences'"));
