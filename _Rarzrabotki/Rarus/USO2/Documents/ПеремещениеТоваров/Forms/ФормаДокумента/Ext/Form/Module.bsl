	...
	Если ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры Тогда
		...
		Если МожноЗаполнитьСерииПоОтгрузке
			...
			ТекстВопроса = НСтр("ru = 'Заполнить серии по данным расходных ордеров?';
								|en = 'Fill in series according to goods issue note data?'");
...
Процедура ПеремещениеПоЗаказуПриИзменении(Элемент)
	...
	Если Объект.ПеремещениеПоЗаказам И ЭтоРаспоряжениеНакладная Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Перемещение со склада-отправителя уже начато. Установка признака невозможна.';
									|en = 'Transfer from the shipping warehouse already started. Cannot set the flag.'");
...
Процедура ПодобратьТовары(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Склад"" не заполнено';
																|en = 'Warehouse is not filled in'"), Объект.Ссылка, "Объект.СкладОтправитель");
		Возврат;
	...
	Иначе
		...
		Если Объект.Товары.Количество() = 0 Тогда
			ТекстВопроса = СтрЗаменить(НСтр("ru = 'Список Товары будет заполнен остатками неоформленных товаров по документу %ЗаказНаПеремещение%. Продолжить?';
											|en = 'The Goods list will be populated with remaining unregistered goods under the %ЗаказНаПеремещение% document. Continue?'"), "%ЗаказНаПеремещение%", Результат);
		Иначе
			ТекстВопроса = СтрЗаменить(НСтр("ru = 'Строки в списке Товары будут привязаны к строкам документа %ЗаказНаПеремещение%. Строки, которые невозможно привязать к строкам заказа, будут удалены.  Продолжить?';
											|en = 'Lines in the Goods list will be linked to lines of the %ЗаказНаПеремещение% document. Lines which cannot be linked to order lines will be deleted. Continue?'"), "%ЗаказНаПеремещение%", Результат);
		КонецЕсли;
...
Процедура ОповеститьОбОтвязкеСтрокОтЗаказов()
	...
	Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
		ТекстОповещения = НСтр("ru = 'Строки в списке ""Товары"" отвязаны от строк заказов.';
								|en = 'Lines in Goods list are unlinked from order lines.'");
	Иначе
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Обработка списка ""Товары""';
										|en = 'The Goods list processing'"),, ТекстОповещения);
...
Процедура ОповеститьОПривязкеСтрокКЗаказам(ЗаказСсылка = Неопределено)
	...
	Если ЗаказСсылка <> Неопределено Тогда
		...
		ШаблонТекста = НСтр("ru = 'Строки в списке ""Товары"" привязаны к строкам документа %Заказ%.
			...
			|en = 'Lines in Goods list are linked to lines of document %Заказ%.
	...
	Иначе
		...
		ТекстОповещения = НСтр("ru = 'Строки в списке ""Товары"" привязаны к строкам неоформленных заказов.
			...
			|en = 'Lines in Goods list are linked to ungenerated order lines.
			|Lines which cannot be linked to order lines are deleted.'");
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		Если ЗаказСсылка <> Неопределено Тогда
			...
			ШаблонТекста = НСтр("ru = 'Список ""Товары"" заполнен остатками неоформленных товаров по документу %Заказ%.';
								|en = 'The Goods list is filled in with remaining unregistered goods under the %Заказ% document.'");
			ТекстОповещения = СтрЗаменить(ШаблонТекста, "%Заказ%", ЗаказСсылка);
			
		Иначе
			
			ТекстОповещения = НСтр("ru = 'Список ""Товары"" заполнен остатками неоформленных товаров по заказам.';
									|en = 'The Goods list is filled in with remaining unregistered goods against orders.'");
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
				ТекстОповещения = НСтр("ru = 'Не заполнен склад-отправитель.';
										|en = 'Shipping warehouse is not filled in.'");
			...
			ИначеЕсли ИспользоватьНесколькоОрганизаций И ИспользоватьПодразделения Тогда
				Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        ...
					                        | from warehouse ""%1"" to warehouse ""%2"" in department ""%3"" of company ""%4""'");
				Иначе
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        ...
					                        | from warehouse ""%1"" to warehouse ""%2"" without specifying a department in company ""%4""'");
				КонецЕсли;
			...
			ИначеЕсли ИспользоватьПодразделения Тогда
				Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        ...
					                        |en = 'No unregistered goods against orders of goods transfer
					                        | from warehouse ""%1"" to warehouse ""%2"" in department ""%3""'");
				Иначе
					ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на перемещение
					                        |со склада ""%1"" на склад ""%2"" без указания подразделения';
					                        |en = 'No unregistered goods against orders of transfer
					                        | from warehouse ""%1"" to warehouse ""%2"" without specifying a department'");
				КонецЕсли;
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
