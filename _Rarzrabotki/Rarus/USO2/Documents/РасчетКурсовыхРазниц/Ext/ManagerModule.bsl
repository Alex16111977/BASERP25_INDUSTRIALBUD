...
Процедура ПереоценитьРезервыПоСомнительнымДолгам(Организация, Период, АналитикаРасчета = Неопределено) Экспорт
	...
	ПараметрыПереоценки = Новый Структура("АналитикаРасчета", АналитикаРасчета);
	ПровестиДокументПереоценки(Организация, Период, Операция, ПараметрыПереоценки);
...
Функция ДокументыПереоценки(Организация, НачалоПериода, КонецПериода, ЭтоРасчетыСКлиентами) Экспорт
	...
	Если НачалоПериода <= Дата('19800101') ИЛИ НЕ ЗначениеЗаполнено(НачалоПериода) Тогда
		ВызватьИсключение НСтр("ru = 'Задана некорректная дата первого периода расчета курсовых разниц.';
								|en = 'Incorrect date of the first period of exchange rate difference calculation is specified.'");
	КонецЕсли;
...
Функция ТекстЗапросаВтТаблицаРезервыПоСомнительнымДолгам(ТекстыЗапроса)
	...
	|	&ОбъектРасчетовНаправлениеДеятельности КАК НаправлениеДеятельности,
	...
	ТекстПоляПодразделения = ДенежныеСредстваПовтИсп.ТекстЗапросаРеквизитаОбъектаРасчетов(
								"РезервыПоДолгам.ОбъектРасчетов", "Подразделение", "ОпределяемыеТипы.ОбъектРасчетов");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОбъектРасчетовПодразделение", ТекстПоляПодразделения);
	
	ТекстПоляНаправления = ДенежныеСредстваПовтИсп.ТекстЗапросаРеквизитаОбъектаРасчетов(
								"РезервыПоДолгам.ОбъектРасчетов", "НаправлениеДеятельности", "ОпределяемыеТипы.ОбъектРасчетов");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОбъектРасчетовНаправлениеДеятельности", ТекстПоляНаправления);
...
Процедура ДобавитьВременныеТаблицы(ТекстыЗапроса, ГруппаТаблиц = "") Экспорт
	...
	Если Не ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ТаблицаРезервыПоСомнительнымДолгам", ТекстыЗапроса) Тогда
		ТекстЗапросаВтТаблицаРезервыПоСомнительнымДолгам(ТекстыЗапроса);
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ВыполнитьРасчетКурсовыхРазниц(ДокументСсылка, ИменаРегистров, АналитикаРасчета = Неопределено, КоличествоОбработанныхЭлементов = 0)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось выполнить расчет курсовых разниц по причине: %Причина%';
								|en = 'Cannot calculate exchange rate differences due to: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Расчет курсовых разниц';
				|en = 'Exchange rate difference calculation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура РассчитатьКурсовыеРазницы(Параметры, КоличествоОбработанныхЭлементов = 0)
	...
	ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(НСтр("ru = 'Расчет курсовых разниц';
															|en = 'Exchange rate difference calculation'"), Параметры.ИменаРегистров);
...
Функция ТекстЗапросаКурсовыеРазницыРезервыПоСомнительнымДолгам()
	...
	|		ИЛИ &КонецПериода = КОНЕЦПЕРИОДА(&КонецПериода, МЕСЯЦ)
	|		ИЛИ &ПереоценкаПоДням)
	|	И ВЫРАЗИТЬ(ОстаткиНаДень.СуммаВВалюте * КурсВалюты.Курс / КурсВалюты.Кратность * КурсВалютыУпр.Кратность / КурсВалютыУпр.Курс КАК ЧИСЛО(31,2)) <> ОстаткиНаДень.СуммаУпр
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТребуетсяПереоценка(Организация, Период, РегистрыДляРасчета = Неопределено, АналитикаРасчета = Неопределено, ВернутьОрганизацииКПереоценке = Ложь) Экспорт
	...
	Если РегистрыДляРасчета.Свойство("РезервыПоСомнительнымДолгам") Тогда
		...
		|	Остатки.Валюта КАК Валюта,
		|	Остатки.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
		|	Остатки.ТипСуммы КАК Организация,
		|	Остатки.ОбъектРасчетов КАК Объект,
		|	Остатки.РасчетныйДокумент КАК РасчетныйДокумент
		|ИЗ
		|	РегистрНакопления.РезервыПоСомнительнымДолгам.Остатки(
