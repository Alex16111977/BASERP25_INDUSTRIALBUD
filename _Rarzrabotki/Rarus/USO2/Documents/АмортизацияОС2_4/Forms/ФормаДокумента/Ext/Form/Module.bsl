...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АмортизацияОСЛокализация.ПриСозданииНаСервере(ЭтаФорма);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ЗавершениеРаботы Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Отменить расчет и закрыть форму';
															|en = 'Cancel calculation and close the form'"));
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Расчет амортизации не завершен. Результаты расчета не будут записаны.';
									|en = 'Depreciation calculation is not completed. Calculation results will not be written.'");
	КонецЕсли; 
...
Процедура Подключаемый_ПриВыполненииКоманды(Команда)
	...
	ВнеоборотныеАктивыКлиентЛокализация.ПриВыполненииКоманды(Команда, ЭтаФорма);
...
Процедура ВыполнитьДействиеПослеРасчета()
	...
	Если ДействиеПослеРасчета = "ЗакрытьФорму" Тогда
		Закрыть();
	Иначе
		ВнеоборотныеАктивыКлиентЛокализация.ВыполнитьДействиеПослеРасчета(ЭтаФорма, ДействиеПослеРасчета);
	КонецЕсли; 
...
Процедура Подключаемый_ПриВыполненииКомандыЗавершение(ИмяКоманды, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("Выполнить_ЗаписатьИВыполнитьДействие", ПараметрыДействия) Тогда
