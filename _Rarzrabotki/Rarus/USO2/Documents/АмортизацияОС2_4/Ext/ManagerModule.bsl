...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Представление = СтрШаблон(НСтр("ru = 'Амортизация ОС %1 за %2';
									|en = 'The %1 FA depreciation for %2'"), Данные.Номер, Формат(Данные.Дата, "ДФ='ММММ гггг'"));
			...
			Если ТаблицаРазукомплектации <> Неопределено Тогда
				...
				Если ДанныеСтроки.ЭтоРазукомплектация Тогда
					...
					Для каждого ПолученныйОС Из СписокСтрок Цикл
						...
						Если (ДанныеСтроки.Стоимость + ДанныеСтроки.СтоимостьИзменение + СтоимостьНовыхОС) <> 0 Тогда
							ПолученныйОС.Амортизация = (ДанныеСтроки.Амортизация + ДанныеСтроки.СуммаУУ + НачисленнаяАмортизацияЗаМесяц) 
															* ПолученныйОС.Стоимость 
															/ (ДанныеСтроки.Стоимость + ДанныеСтроки.СтоимостьИзменение + СтоимостьНовыхОС);
						...
						Если (ДанныеСтроки.СтоимостьРегл + ДанныеСтроки.СтоимостьРеглИзменение + СтоимостьНовыхОСРегл) <> 0 Тогда
							ПолученныйОС.АмортизацияРегл = (ДанныеСтроки.АмортизацияРегл + ДанныеСтроки.СуммаБУ + НачисленнаяАмортизацияЗаМесяцРегл) 
															* ПолученныйОС.СтоимостьРегл 
															/ (ДанныеСтроки.СтоимостьРегл + ДанныеСтроки.СтоимостьРеглИзменение + СтоимостьНовыхОСРегл);
...
Функция РассчитатьСуммуАмортизацииУУ(ДанныеСтроки, ОстаточнаяСтоимость, ЛиквидационнаяСтоимость, НачисленнаяАмортизацияЗаМесяц)
	...
	Если СуммаУУ > ОстаточнаяСтоимостьПослеНачисленияАмортизации Тогда
...
Процедура ЗарегистрироватьОшибкиРасчетаАмортизации(ПараметрыРасчета, МенеджерВременныхТаблиц) Экспорт
	...
	ШаблонОбъемНаработки      = НСтр("ru = 'Для основного средства ""%1"" (инв. №%2) не указан предполагаемый объем наработки (продукции, работ).';
									|en = 'Estimated running time (of products, works) is not specified for the ""%1"" fixed asset (inv. No. %2).'");
	ШаблонСрокИспользованияУУ = НСтр("ru = 'Для основного средства ""%1"" (инв. №%2) не указан срок использования в управленческом учете.';
									|en = 'Useful life in management accounting is not specified for the ""%1"" fixed asset (inv. No. %2).'");
...
Процедура ТекстЗапросаПараметрыРасчетаАмортизации(СписокЗапросов, ПараметрыРасчета)
	...
	Иначе
		...
		|	NULL КАК ЛиквидационнаяСтоимостьРегл,
