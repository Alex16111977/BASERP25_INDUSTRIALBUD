...
Функция ПодготовленныеДанныеПартионнойПродукцииТСД()
	...
	МаркируемаяПродукцияВТЧ = (Объект.ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.АктПостановкиНаБалансВРегистр1"));
	
	ПараметрыЗаполнения = ИнтеграцияЕГАИСКлиентСервер.ПараметрыЗаполненияТабличнойЧасти();
	ПараметрыЗаполнения.ПересчитатьКоличествоЕдиниц        = Истина;
	ПараметрыЗаполнения.МаркируемаяПродукцияВТЧ            = МаркируемаяПродукцияВТЧ;
	ПараметрыЗаполнения.ЗаполнитьАлкогольнуюПродукцию      = Истина;
...
Процедура ОбработатьДанныеТСДНаСервере(ДанныеПартионнойПродукции, КэшированныеЗначения)
	...
	Для Каждого МаркируемаяПродукция Из ЗагрузкаДанныхТСД.МаркируемаяПродукция Цикл
		...
		Если ВложенныеШтрихкоды <> Неопределено Тогда
			...
			ПараметрыПоиска.Вставить("ЗначениеШтрихкода", МаркируемаяПродукция.ДанныеШтрихкода.Штрихкод);
		...
		РезультатОбработкиШтрихкода = Документы.АктСписанияЕГАИС.ОбработатьДанныеШтрихкодаМаркируемойПродукции(
			ЭтотОбъект,
	...
	Если ЗагрузкаДанныхТСД.Упаковки.Количество() > 0 Тогда
		...
		РезультатОбработкиШтрихкода = Документы.АктСписанияЕГАИС.ОбработатьДанныеШтрихкодаУпаковки(
			ЭтотОбъект,
			ДанныеШтрихкода,
			ВложенныеШтрихкоды,
			ПараметрыСканирования);
