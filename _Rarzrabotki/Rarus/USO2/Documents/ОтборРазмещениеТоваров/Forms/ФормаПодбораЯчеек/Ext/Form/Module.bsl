...
Процедура ЗаполнитьСписокЯчеекПоОтбору(Команда)
	Если Ячейки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСписокЯчеекПоОтборуЗавершение", ЭтотОбъект), НСтр("ru = 'Перед заполнением список ячееек будет очищен. Продолжить?';
																											|en = 'Bin list will be cleared before population. Continue?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
