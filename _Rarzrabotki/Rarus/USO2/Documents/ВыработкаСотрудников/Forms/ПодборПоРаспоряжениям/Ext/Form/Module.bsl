...
Процедура ТаблицаКоличествоПриИзменении(Элемент)
	...
	ТекущиеДанные.Выбран = ТекущиеДанные.Количество > 0;
КонецПроцедуры
...
Процедура ЗаполнитьТаблицу()
	...
	|	ДД.Количество - ЕСТЬNULL(Подобранные.Количество, 0) КАК Количество,
	|	ДД.Расценка КАК Расценка,
	|	(ДД.Количество - ЕСТЬNULL(Подобранные.Количество, 0)) * ДД.Расценка КАК Сумма,
	...
	|ГДЕ
	|	ДД.Количество - ЕСТЬNULL(Подобранные.Количество, 0) > 0";
	
	ПодобранныеВидыРабот =  ПолучитьИзВременногоХранилища(АдресВХранилище);
	ПодобранныеВидыРабот.Свернуть("Распоряжение,КодСтрокиРаспоряжения","Количество");
