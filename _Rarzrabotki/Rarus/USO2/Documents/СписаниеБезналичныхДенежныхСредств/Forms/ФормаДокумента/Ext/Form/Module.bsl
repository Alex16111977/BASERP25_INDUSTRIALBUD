		...
		Если ЗначениеЗаполнено(ЗеркальныйПлатежСсылка) И Не СоответствуетЗеркальномуПлатежу Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ПривестиВСоответствиеЗеркальныйПлатежЗавершение", ЭтотОбъект, Ложь),
				СтрШаблон(НСтр("ru = 'Привести в соответствие %1?';
								|en = 'Adjust to %1?'"), ЗеркальныйПлатежСсылка),
...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	УСОERP_МодификацияКонфигурацииКлиент.ПД_ПриИзмененииХозОперации(ЭтотОбъект);
	// {УСО 2.4}:{Управление строительной организацией}}}
...
Процедура СуммаДокументаПриИзменении(Элемент)
	...
	Если ИспользоватьНачислениеЗарплаты
		...
		ТекстВопроса = НСтр("ru = 'Сумма документа будет приведена к сумме по Ведомостям на закладке ""Расшифровка платежа"". Продолжить?';
							|en = 'The document amount will be reduced to the Paysheet amount on the ""Amount allocation"" tab. Continue?'");
		...
		ОткрытьФорму(
			...
			Новый Структура("СписокДокументов, Заголовок",
				...
				НСтр("ru = 'Заявки на расход ДС (%КоличествоДокументов%)';
					|en = 'Expenditure requests (%КоличествоДокументов%)'")
			),
	...
	ИначеЕсли ЗначениеЗаполнено(КонтрагентЗаявки) И Объект.Контрагент <> КонтрагентЗаявки Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получатель платежа ""%1"" в документе оплаты не соответствует получателю ""%2"" в заявке!';
				|en = 'Payment recipient ""%1"" in the payment document does not match the recipient ""%2"" in the request.'"),
			Объект.Контрагент,
			КонтрагентЗаявки);
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Списание безналичных денежных средств, зеркальное изменение связанного документа.';
					|en = 'Non-cash debiting, inverted change of related document.'",
