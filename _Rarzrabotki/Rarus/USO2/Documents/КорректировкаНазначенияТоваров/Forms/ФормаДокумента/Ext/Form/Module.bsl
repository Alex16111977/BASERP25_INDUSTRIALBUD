...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.ЗаполнениеКорректировкиНазначения.Форма.ФормаОбъекта" Тогда
		...
		Если Объект.Товары.Количество() > 0 Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнениеКорректировкиНазначенияЗавершение", ЭтотОбъект, ВыбранноеЗначение), НСтр("ru = 'Таблица товаров будет очищена. Продолжить?';
																																		|en = 'Goods table will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
		Иначе
...
Процедура Заполнить(Команда)
	...
	ИначеЕсли (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.СнятьРезерв")
		...
		ТекстПредупреждения = НСтр("ru = 'По данному назначению нет потребностей. Заполнение возможно только через операцию ""Произвольная корректировка""';
									|en = 'No demands for this assignment. Filling is available via the ""Arbitrary adjustment"" operation only'");
...
Процедура УстановитьВидимостьДоступность()
	...
	ЗаголовокЗаполнитьПоВидуОперации.Вставить(ВидыОпераций.Резервировать,                  НСтр("ru = 'Подобрать из свободных остатков';
																								|en = 'Select from available stock'"));
