...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		ДанныеВременноОсвобожденныхПозиций = ДанныеВременноОсвобожденныхПозиций(МенеджерВременныхТаблиц, РеквизитыДляПроведения);
		Если ДанныеВременноОсвобожденныхПозиций.Количество() > 0 Тогда
			КадровыйУчетРасширенный.ЗанятьСтавку(
				Движения, ДанныеВременноОсвобожденныхПозиций);
		КонецЕсли;
		...
		Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
			...
			Для Каждого СтрокаПериодовРегистрации Из РеквизитыДляПроведения.ПериодыРегистрации Цикл
				...
				УчетНачисленнойЗарплатыРасширенный.ЗарегистрироватьУдержанияИЗаймы(ДанныеДляПроведенияУчетЗарплаты, Отказ,
					УдержанияПоСотрудникам, УдержанияЗаймов, НалогНаМатериальнуюВыгоду);
...
Функция ДанныеВременноОсвобожденныхПозиций(МенеджерВременныхТаблиц, РеквизитыДляПроведения)
	...
	Запрос.Текст =
		...
		|	Сотрудники.Период КАК ДатаОкончания,
		|	Сотрудники.Период КАК ДатаЗанятияСтавки,
		...
		|			И ЗанятыеПозицииШтатногоРасписания.Период > Сотрудники.Период
		|			И ЗанятыеПозицииШтатногоРасписания.ДатаОтменыУсловногоДвижения < Сотрудники.Период
		|ГДЕ
