...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонтролироватьСтатусыКодовМаркировкиВРозницеМОТП          = ИнтеграцияМОТПУТ.КонтролироватьСтатусыКодовМаркировкиВРозницеМОТП();
		...
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияУТКлиент.ЕстьНеобработанноеСобытие() Тогда
			...
			Иначе
				...
				ШтрихкодированиеИСКлиент.ОбработатьВводШтрихкода(
					ЭтотОбъект,
					ШтрихкодированиеИСКлиентСервер.ОбработатьДанныеШтрихкода(
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Копирование Тогда
		...
		АкцизныеМаркиЗаполнитьСлужебныеРеквизитыСервер();
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ДанныеУдаляемыхСтрокМаркировка = Новый Массив;
	ВыделенныеСтроки = Элементы.Товары.ВыделенныеСтроки;
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		СтрокаТЧ = Объект.Товары.НайтиПоИдентификатору(ВыделеннаяСтрока);
		ДанныеОтбора = ИнтеграцияИСУТКлиентСервер.ПоляДляПоискаМаркированнойПродукции();
		ДанныеОтбора.Вставить("Количество", 0);
		ДанныеОтбора.Вставить("НомерСтроки", СтрокаТЧ.НомерСтроки);
		ЗаполнитьЗначенияСвойств(ДанныеОтбора, СтрокаТЧ);
		ДанныеУдаляемыхСтрокМаркировка.Добавить(ДанныеОтбора);
	КонецЦикла;
	
	ОчиститьСообщения();
	
	УдалитьАкцизныеМарки(ДанныеУдаляемыхСтрокМаркировка, Отказ);
...
Процедура ТоварыНоменклатураПриИзмененииСервер(ИдентификаторТекущейСтроки)
	...
	Если КонтролироватьАссортимент Тогда
		...
		СтруктураПроверкиАссортимента.ТекстСообщения = НСтр("ru = 'Товар %1 не включен в ассортимент магазина или запрещен к продаже.';
															|en = 'Goods %1 are not included in the store assortment or prohibited for sale.'");
	...
	ИнтеграцияИСУТ.АкцизныеМаркиЗаполнитьСлужебныеРеквизиты(ЭтотОбъект);
...
&НаСервере
Функция ВидыПродукцииВТоварахСервер()
	Возврат ИнтеграцияИСУТ.ВидыПродукцииВТоварах(Объект.Товары);
КонецФункции

&НаКлиенте
Процедура АкцизныеМарки(Команда)
	
	ДобавленныеВидыПродукции = ВидыПродукцииВТоварахСервер();
	...
	Если ДобавленныеВидыПродукции.Количество() = 1 Тогда
		...
		ИначеЕсли ДобавленныеВидыПродукции.НайтиПоЗначению(ТабачнаяПродукция)  <> Неопределено Тогда
			ОткрытьФормуСканированияТабачнойПродукции();
		КонецЕсли;
		
	ИначеЕсли ДобавленныеВидыПродукции.Количество() > 1 Тогда
		
		ПараметрыФормы = Новый Структура("СписокВыбора", ДобавленныеВидыПродукции);
		ОповещениеОВыборе = Новый ОписаниеОповещения("ОбработатьВыборФормыСканирования", ЭтаФорма, ДобавленныеВидыПродукции);
		ОткрытьФорму("Перечисление.ВидыПродукцииИС.Форма.ФормаВыбораЗаданных",
			ПараметрыФормы,
			ЭтаФорма,,,,
			ОповещениеОВыборе,
			РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработатьВыборФормыСканирования(Выбор, ДополнительныеПараметры) Экспорт
	...
	Если Выбор <> Неопределено Тогда
		...
		Если ВыбранныйВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Алкогольная") Тогда
			ОткрытьФормуСканированияАлкогольнойПродукции();
		ИначеЕсли ВыбранныйВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табачная") Тогда
			ОткрытьФормуСканированияТабачнойПродукции();
		КонецЕсли;
	...
	Если (ИспользоватьАвтоматическиеСкидкиВПродажах
		...
		Если Не Результат.РасчетВыполненУспешно Тогда
			...
			ДополнительныеПараметрыПопыткаЗаписи.ТекстСообщения             = НСтр("ru = 'При выполнении операции резервирования не удалось записать документ.';
																					|en = 'Cannot write the document during the reservation.'");
...
Функция ДанныеДляПробитияЧека()
	...
	ДанныеПоТоварам = Документы.ЧекККМ.ДанныеПоТоварам(Объект, МенеджерВременныхТаблиц);
	
	ДанныеДляЕГАИС = Документы.ЧекККМ.ДанныеДляЕГАИС(Объект, МенеджерВременныхТаблиц);
	
	ДанныеДляМОТП = Документы.ЧекККМ.ДанныеДляМОТП(Объект, МенеджерВременныхТаблиц);
	...
	Для каждого СтрокаТЧ из ДанныеПоТоварам Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "АлкогольнаяПродукция")
			...
			ЭтоАлкогольнаяПродукция = Истина;
			СтрокиДанныхПоЕГАИС = ДанныеДляЕГАИС.НайтиСтроки(Новый Структура("ИдентификаторСтроки, НомерСтроки", СтрокаТЧ.ИдентификаторСтроки, СтрокаТЧ.НомерСтроки));
			Если СтрокиДанныхПоЕГАИС.Количество()>0 Тогда
				ПараметрыАлкогольнойПродукции.Добавить(СтрокиДанныхПоЕГАИС[0].МаркируемаяПродукция); // Признак наличия штрихкода PDF417
				ПараметрыАлкогольнойПродукции.Добавить(СтрокиДанныхПоЕГАИС[0].КодАкцизнойМарки);                // Штрихкод марки в формате PDF417
				ПараметрыАлкогольнойПродукции.Добавить(СтрокиДанныхПоЕГАИС[0].Объем);                           // Объем номенклатуры в литрах
				ПараметрыАлкогольнойПродукции.Добавить(СтрокиДанныхПоЕГАИС[0].Крепость);                        // Процент содержания алкоголя
				ПараметрыАлкогольнойПродукции.Добавить(СтрокиДанныхПоЕГАИС[0].КодВидаАлкогольнойПродукции);     // Код вида алкогольной продукции
			КонецЕсли;	
		КонецЕсли;
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ТабачнаяПродукция")
			И СтрокаТЧ.ТабачнаяПродукция Тогда
			ЭтоТабачнаяПродукция = Истина;
			// заменяем строки для добавления в чек детализированными данными по табаку
			МассивСтрокДляДобавленияВЧек = ДанныеДляМОТП.НайтиСтроки(Новый Структура("ИдентификаторСтроки, НомерСтроки", СтрокаТЧ.ИдентификаторСтроки, СтрокаТЧ.НомерСтроки));
		КонецЕсли;	
		...
		Для каждого СтрокаДляДобавленияВЧек из МассивСтрокДляДобавленияВЧек Цикл
			...
			СтрокаПозицииЧека.Количество             = СтрокаДляДобавленияВЧек.КоличествоУпаковок;
...
Процедура ПробитьЧек()
	...
	Если Объект.ПометкаУдаления Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Документ помечен на удаление';
		...
		Если ЗначениеЗаполнено(Объект.КартаЛояльности)
			...
			ТекстоваяСтрокаДляПечатиНаККТ = ТекстоваяСтрокаДляПечатиНаККТ + СтрШаблон(
				НСтр("ru = 'Списано бонусных баллов: %1 %2';
					|en = 'Written off bonus points: %1 %2'"),
	...
	Иначе
		...
		Если Результат.ВыполненаОперацияНаУстройстве И Не Результат.ИзмененныеДанныеЗаписаны Тогда
			...
			ПараметрыФормы.Вставить("ТекстСообщения",
				...
				|Cash receipt was issued but it was not recorded in the application.'"));
...
Процедура ПослеПередачиЧекаЕГАИС(Изменения, ПараметрыВыполнения) Экспорт
	...
	Если Ошибки.Количество() > 0 Тогда
		Ошибки.Вставить(0, НСтр("ru = 'В процессе передачи данных в ЕГАИС возникли ошибки:';
								|en = 'The following errors occurred while transferring data to USAIS:'"));
	КонецЕсли;
	...
	Для Каждого СтрокаИсточник Из ТаблицаТовары Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Для Каждого АкцизнаяМарка Из АкцизныеМарки Цикл
				Отбор = Новый Структура("АкцизнаяМарка", АкцизнаяМарка.АкцизнаяМарка);
				НайденнаяАкцизнаяМарка = Объект.АкцизныеМарки.НайтиСтроки(Отбор);
				
				Если НайденнаяАкцизнаяМарка.Количество() = 0 Тогда
					НоваяСтрокаАкцизныхМарок = Объект.АкцизныеМарки.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаАкцизныхМарок, АкцизнаяМарка);
				КонецЕсли;
			КонецЦикла;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПомещение", Новый Структура("Склад", Объект.Склад));
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПродавца", Новый Структура("Продавец", ТекущийПродавец));
	СтруктураДействийСДобавленнымиСтроками.Вставить("ЗаполнитьПризнакМаркируемаяПродукция", Новый Структура("Номенклатура", "МаркируемаяПродукция"));
	...
	Если ИспользоватьАкцизныеМарки И ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияАкцизныхМарокПослеОбработкиШтрихкодов(СтруктураДействий) Тогда
		...
		ДополнительныеПараметры = ШтрихкодированиеИСКлиентСервер.ИнициализироватьПараметрыСканирования(ЭтотОбъект);
...
Процедура ВывестиИнформациюНаДисплейПокупателя()
	...
	Если ВывестиКОплатеНаДисплейПокупателя Тогда
		...
		ДПТекст1 = РозничныеПродажиКлиент.ПодготовитьСтрокуКВыводуНаДисплейПокупателя(
			НСтр("ru = 'К оплате:';
				|en = 'For payment:'"), ИнформацияОбОплате.СуммаКОплате);
		ДПТекст2 = "";
...
Функция Подключаемый_ОбработатьВводШтрихкода(ДанныеШтрихкода, КэшированныеЗначения)
	...
	РезультатОбработкиШтрихкода = ШтрихкодированиеИС.ОбработатьВводШтрихкода(ЭтотОбъект, ДанныеШтрихкода, КэшированныеЗначения);
...
Процедура Подключаемый_ПослеОбработкиШтрихкодов()
	...
	Если ИспользоватьАкцизныеМарки И ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияАкцизныхМарокПослеОбработкиШтрихкодов(ДанныеДляОбработки) Тогда
		...
		ИнтеграцияИСУТКлиент.ОткрытьФормуСчитыванияАкцизнойМарки(Неопределено, ДополнительныеПараметры);
		
	ИначеЕсли ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияСерийПослеОбработкиШтрихкодов(ДанныеДляОбработки) Тогда
...
&НаСервере
Процедура АкцизныеМаркиЗаполнитьСлужебныеРеквизитыСервер()
	ИнтеграцияИСУТ.АкцизныеМаркиЗаполнитьСлужебныеРеквизиты(ЭтотОбъект);
КонецПроцедуры
...
Процедура ПриЗакрытииФормыПроверкиИПодбора(Результат, ДополнительныеПараметры) Экспорт
	...
	ПриЗакрытииФормыПроверкиИПодбораНаСервере(Результат, ДополнительныеПараметры);
	
КонецПроцедуры

&НаСервере
Процедура ПриЗакрытииФормыПроверкиИПодбораНаСервере(Результат, ДополнительныеПараметры)
	
	Если ЭтоАдресВременногоХранилища(Результат) Тогда
		ДанныеПроверкиИПодбора = ПолучитьИзВременногоХранилища(Результат);
		
		ИнтеграцияИСУТ.УдалитьСтрокиАкцизныхМарокПоВидуПродукции(Объект.АкцизныеМарки, Перечисления.ВидыПродукцииИС.Табачная);
		
		ИнтеграцияИСУТ.УдалитьСтрокиТоваровПоВидуПродукции(Объект.Товары, Перечисления.ВидыПродукцииИС.Табачная);
		...
		ОбработатьСтрокиТЧ(ДобавленныеСтроки, Новый Массив);
	
		ЗаполнитьСлужебныеРеквизитыПоНоменклатуре();
	
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(
			Объект, НоменклатураСервер.ПараметрыУказанияСерий(Объект, Документы.ЧекККМ));
...
Процедура СканированиеОтгружаемойПродукцииПослеЗакрытияНаСервере(АдресВременногоХранилища)
	...
	ИнтеграцияИСУТ.УдалитьСтрокиАкцизныхМарокПоВидуПродукции(Объект.АкцизныеМарки, Перечисления.ВидыПродукцииИС.Алкогольная);
...
Процедура ОткрытьФормуСканированияАлкогольнойПродукции()
	...
	ПараметрыОткрытия.Вставить("АдресДанныхПроверяемойАлкогольнойПродукции",             АдресДанныхПроверкиАлкогольнойПродукции());
	...
	ПараметрыОткрытия.Вставить("НачальныйСтатусПроверки",                                ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияПродукцииИС.ВНаличии"));
...
&НаКлиенте
Функция ПараметрыОткрытияФормыПроверкиИПодбора()
	
	ПараметрыОткрытия = ПроверкаИПодборПродукцииМОТПКлиентСервер.ПараметрыОткрытияФормыПроверкиИПодбора();
	ПараметрыОткрытия.ИмяРеквизитаДокументОснование = "";
	ПараметрыОткрытия.ИмяРеквизитаФормыОбъект       = "Объект";
	ПараметрыОткрытия.АдресПроверяемыхДанных        = АдресДанныхПроверкиТабачнойпродукции();
	ПараметрыОткрытия.ПроверятьМодифицированность   = Ложь;
	ПараметрыОткрытия.ОписаниеОповещенияПриЗакрытии = Новый ОписаниеОповещения("ПриЗакрытииФормыПроверкиИПодбора", ЭтотОбъект);
...
&НаКлиенте
Процедура ОткрытьФормуСканированияТабачнойПродукции()
	
	СтандартнаяОбработка = Ложь;
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(
		Истина,
		"Документ.ЧекККМ.Форма.ФормаДокументаРМК.ОткрытьФормуСканированияТабачнойПродукции");
	
		ПроверкаИПодборПродукцииМОТПКлиент.ДекорацияСканироватьПроверитьТоварыОбработкаНавигационнойСсылки(
			ЭтотОбъект,
			ПроверкаИПодборПродукцииМОТПКлиентСервер.НавигационнаяСсылкаОткрытьФормуПроверкиИПодбора(),
			СтандартнаяОбработка,
			ПараметрыОткрытияФормыПроверкиИПодбора());
	
КонецПроцедуры

&НаСервере
Функция АдресДанныхПроверкиАлкогольнойПродукции()
	
	ДанныеХранилища = Новый Структура("ДеревоУпаковок, ТаблицаАлкогольнойПродукцииКОпределениюСправок2",
		Неопределено,
		Новый ТаблицаЗначений);
		
	ДанныеХранилища.ДеревоУпаковок = АкцизныеМаркиЕГАИС.ШтрихкодыУпаковок(
		Объект,
		Объект.ОрганизацияЕГАИС,
		"НоменклатураЕГАИС",
		Истина).ДеревоУпаковок;
...
&НаСервере
Функция АдресДанныхПроверкиТабачнойПродукции()
	
	ПараметрыСканирования = ШтрихкодированиеИСКлиентСервер.ИнициализироватьПараметрыСканирования(ЭтотОбъект, Перечисления.ВидыПродукцииИС.Табачная);
	
	СписокШтрихкодов = Объект.АкцизныеМарки.Выгрузить().ВыгрузитьКолонку("АкцизнаяМарка");
	ШтрихкодыТабачнойПродукции = ИнтеграцияИСУТ.ШтрихкодыСодержащиеВидыПродукции(
		СписокШтрихкодов,
	...
	ТаблицаТабачнойПродукции = ПроверкаИПодборПродукцииМОТПУТ.ТаблицаТабачнойПродукцииДокумента(ЭтотОбъект);
...
Процедура ЗарезервироватьНаКлиенте(РезультатОтложенДо, ДополнительныеПараметры) Экспорт
	...
	Если Не Результат Тогда
		...
		ДополнительныеПараметрыПовторЗаписи.ТекстСообщения             = НСтр("ru = 'После выполнения операции отмены оплаты не удалось записать документ.';
																				|en = 'Cannot write the document after payment cancellation transaction has been executed.'");
...
Функция УдалитьЧек(Ссылка)
	...
	Если ЧекОбъект.Статус = Перечисления.СтатусыЧековККМ.Пробит Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Пробитый чек не может быть удален';
																|en = 'Issued receipt cannot be deleted'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОплатитьНаличнымиЗавершениеРасчета(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыполнитьДействиеПослеЗаписи(
		...
		НСтр("ru = 'Перед открытием формы оплаты не удалось записать документ.';
			|en = 'Cannot write the document before opening a payment form.'"));
...
Процедура ДобавитьОплатуКартой(ИзмененныеДанныеЗаписаны, ПараметрыЗавершения) Экспорт
	...
	ВыполнитьДействиеПослеЗаписи(
		...
		НСтр("ru = 'Перед оплатой платежной картой не удалось записать документ.';
			|en = 'Cannot write the document before paying by payment card.'"));
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	ЗаполнитьПризнакМаркируемаяПродукция            = Новый Структура("Номенклатура", "МаркируемаяПродукция");
	ЗаполнитьПризнакАлкогольнаяПродукция            = Новый Структура("Номенклатура", "АлкогольнаяПродукция");
	ЗаполнитьПризнакМаркируемаяПродукцияТабачная    = Новый Структура("Номенклатура", "ТабачнаяПродукция");
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакМаркируемаяПродукция",         ЗаполнитьПризнакМаркируемаяПродукция);
	СтруктураДействий.Вставить("ЗаполнитьПризнакАлкогольнаяПродукция",         ЗаполнитьПризнакАлкогольнаяПродукция);
	СтруктураДействий.Вставить("ЗаполнитьПризнакМаркируемаяПродукцияТабачная", ЗаполнитьПризнакМаркируемаяПродукцияТабачная);
...
Процедура ОбновитьЗаголовокФормы()
	...
	Заголовок = НСтр("ru = 'Продажа (Кассир: %Кассир%, Продавец: %Продавец%)';
					|en = 'Sale (Cashier: %Кассир%, Seller: %Продавец%)'");
...
Процедура ПередЗакрытиемЧека(Отказ, ОписаниеОповещения = Неопределено)
	...
	Если Не ТолькоПросмотр И (Модифицированность ИЛИ ЗначениеЗаполнено(Объект.Ссылка)) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные чека ККМ были изменены.';
							|en = 'Cash receipt data was changed.'");
...
Процедура НастроитьРМК()
	...
	ИспользоватьАкцизныеМарки = ИспользуетсяРегистрацияРозничныхПродажВЕГАИС Или ПолучитьФункциональнуюОпцию("ВестиУчетТабачнойПродукцииМОТП");
...
&НаСервере
Процедура УдалитьАкцизныеМарки(Данные, Отказ)
	
	ИнтеграцияИСУТ.УдалитьАкцизныеМаркиЧека(ЭтотОбъект, Данные, Отказ);
