...
Функция ДанныеДляЕГАИС(Объект, МенеджерВременныхТаблиц) Экспорт
	...
	|	Товары.Штрихкод,
	|
	|	ЕСТЬNULL(АкцизныеМарки.КодАкцизнойМарки, Неопределено) КАК КодАкцизнойМарки,
	|
	...
	|
	|	ВЫБОР КОГДА Товары.НоменклатураЕГАИС = ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка) ТОГДА
	|		Товары.Номенклатура.ВидАлкогольнойПродукции.Код
	|	ИНАЧЕ
	|		Товары.НоменклатураЕГАИС.ВидПродукции.Код
	...
	|
	|	ВЫБОР КОГДА Товары.НоменклатураЕГАИС = ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка) ТОГДА
	|		Товары.Номенклатура.ПроизводительИмпортерДляДекларацийАлко.ИНН
	|	ИНАЧЕ
	|		ВЫБОР КОГДА Товары.НоменклатураЕГАИС.Импортер <> ЗНАЧЕНИЕ(Справочник.КлассификаторОрганизацийЕГАИС.ПустаяСсылка) ТОГДА
	|			Товары.НоменклатураЕГАИС.Импортер.ИНН
	|		ИНАЧЕ
	|			Товары.НоменклатураЕГАИС.Производитель.ИНН
	|		КОНЕЦ
	|	КОНЕЦ КАК ИНН,
	|
	|	ВЫБОР КОГДА Товары.НоменклатураЕГАИС = ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка) ТОГДА
	|		Товары.Номенклатура.ПроизводительИмпортерДляДекларацийАлко.КПП
	|	ИНАЧЕ
	|		ВЫБОР КОГДА Товары.НоменклатураЕГАИС.Импортер <> ЗНАЧЕНИЕ(Справочник.КлассификаторОрганизацийЕГАИС.ПустаяСсылка) ТОГДА
	|			Товары.НоменклатураЕГАИС.Импортер.КПП
	|		ИНАЧЕ
	|			Товары.НоменклатураЕГАИС.Производитель.КПП
	|		КОНЕЦ
	|	КОНЕЦ КАК КПП,
	|	
	|	ВЫБОР КОГДА Товары.НоменклатураЕГАИС = ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка) ТОГДА
	...
	|ИЗ
	|	Товары КАК Товары
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаАкцизныеМарки КАК АкцизныеМарки 
	|	ПО Товары.ИдентификаторСтроки = АкцизныеМарки.ИдентификаторСтроки 
	|	И Товары.НомерСтроки = АкцизныеМарки.НомерСтроки
	|ГДЕ Товары.АлкогольнаяПродукция
	|";
...
//  Массив - Данные о продажах (см. ИнтеграцияЕГАИСКлиентСервер.СтруктураСтрокиЧекаЕГАИС()).
//
Функция ДанныеДляМОТП(Объект, МенеджерВременныхТаблиц) Экспорт
	...
	|ИЗ ТоварыПоСериямБезСуммовыхРазниц КАК ТоварыПоСериямБезСуммовыхРазниц
	|ГДЕ &КоличествоСерий > 0
	|СГРУППИРОВАТЬ ПО
	...
	|ИЗ ТоварыПоСериям КАК ТоварыПоСериям
	...
	ПараметрыСканирования = ШтрихкодированиеИСКлиентСервер.ИнициализироватьПараметрыСканирования(Объект.Ссылка,,Перечисления.ВидыПродукцииИС.Табачная);
...
Функция ДанныеПоТоварам(Объект, МенеджерВременныхТаблиц) Экспорт
	...
	|	ВЫБОР КОГДА Товары.НоменклатураЕГАИС = ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка) ТОГДА
	|		Товары.Номенклатура.ТабачнаяПродукция
	|	ИНАЧЕ
	|		ЛОЖЬ
...
Функция ОбработатьДанныеШтрихкодаУпаковкиМаркируемойПродукции(Форма, ДанныеШтрихкода, ВложенныеШтрихкоды, ПараметрыСканирования) Экспорт
	...
	ЗаполнитьТоварыИАкцизныеМаркиМаркируемойпродукции(Объект, ВложенныеШтрихкоды.ДеревоУпаковок, ПараметрыЗаполнения);
...
Процедура ЗаполнитьТоварыИАкцизныеМаркиМаркируемойПродукции(ДокументОбъект, ДеревоУпаковок, ПараметрыЗаполнения, УпаковкаВерхнегоУровня = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если СтрокаДерева.Видпродукции <> Неопределено
			...
			И СтрокаДерева.Серия <> Неопределено Тогда
...
Функция ОбработатьДанныеШтрихкодаЕдиницыТабачнойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыПоиска, ДанныеШтрихкода,,
		?(ЗначениеЗаполнено(ДанныеШтрихкода.Серия), "", "Серия"));
	...
	Если НайденныеСтрокиТовары.Количество() > 0 Тогда
		...
		СерииВТЧТовары = НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
			НайденныеСтрокиТовары[0].СтатусУказанияСерий, ПараметрыЗаполнения.ПараметрыУказанияСерий);
			
		Если Не СерииВТЧТовары Тогда
			СтрокаТЧСерии = Объект.Серии.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТЧСерии, ДанныеШтрихкода);
			СтрокаТЧСерии.Количество = ДанныеШтрихкода.КоличествоПачек;
		КонецЕсли;
	...
	Иначе
		...
		ПараметрыПоиска.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		НайденныеСтрокиТовары = Объект.Товары.НайтиСтроки(ПараметрыПоиска);
		Если НайденныеСтрокиТовары.Количество() > 0 Тогда
			...
			Если СерииВТЧТовары Тогда
				Если НайденныеСтрокиТовары[0].СтатусУказанияСерий <> 0 Тогда
					НайденныеСтрокиТовары[0].Серия = ДанныеШтрихкода.Серия;
				КонецЕсли;
			Иначе
				СтрокаТЧСерии = Объект.Серии.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТЧСерии, ДанныеШтрихкода);
				СтрокаТЧСерии.Количество = ДанныеШтрихкода.КоличествоПачек;
				НайденныеСтрокиТовары[0].Серия = Неопределено;
			КонецЕсли;
		...
		Иначе
			...
			НоваяСтрокаТовары.Характеристика      = ДанныеШтрихкода.Характеристика;
			...
			НоменклатураКлиентСервер.ОбновитьКешированныеЗначенияДляУчетаСерий(НоваяСтрокаТовары,КэшированныеЗначения,
				ПараметрыЗаполнения.ПараметрыУказанияСерий);
			
			НоменклатураСервер.ЗаполнитьСтатусыУказанияСерийПриОкончанииРедактированияСтрокиТЧ(Объект,
				ПараметрыЗаполнения.ПараметрыУказанияСерий, НоваяСтрокаТовары.ПолучитьИдентификатор(), КэшированныеЗначения);
				
			Если ЗначениеЗаполнено(ДанныеШтрихкода.Серия)
				И НЕ НоменклатураКлиентСервер.ВЭтомСтатусеСерииНеУказываются(НоваяСтрокаТовары.СтатусУказанияСерий, ПараметрыЗаполнения.ПараметрыУказанияСерий) Тогда
				
				Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
					НоваяСтрокаТовары.СтатусУказанияСерий, ПараметрыЗаполнения.ПараметрыУказанияСерий) Тогда
					НоваяСтрокаТовары.Серия = ДанныеШтрихкода.Серия;
				Иначе
					СтрокаТЧСерии = Объект.Серии.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТЧСерии, ДанныеШтрихкода);
					СтрокаТЧСерии.Количество = ДанныеШтрихкода.КоличествоПачек;
