...
Процедура ЗаписатьРеквизитыТСиОборудования(Отказ)
	Для Каждого СтрТС Из ТС Цикл
		...
		Попытка 
			ТСОбъект.Записать();
		Исключение
			уатОбщегоНазначенияТиповые.СообщитьОбОшибке("Не удалось записать ТС / оборудование """ + Строка(СтрТС.ТС) + """", Отказ);
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры
...
Процедура ВосстановитьТСиОборудованиеПередЗаписью(Отказ)
	Для Каждого ТекТС Из таблТССсылка Цикл
		...
		Попытка 
			ТСОбъект.Записать();
		Исключение
			уатОбщегоНазначенияТиповые.СообщитьОбОшибке("Не удалось записать ТС / оборудование """ + Строка(ТекТС.ТС) + """", Отказ);
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры
...
Функция ТСВведеныВЭксплуатацию(Отказ)
	...
	Для Каждого ТекСтрока Из ТС Цикл
		Если ТекСтрока.ТС.уатДатаВводаВЭксплуатацию = '00010101' ИЛИ ТекСтрока.ДатаВыбытия < ТекСтрока.ТС.уатДатаВводаВЭксплуатацию Тогда
			уатОбщегоНазначенияТиповые.СообщитьОбОшибке("ТС / оборудование """ + Строка(ТекСтрока.ТС) + """ не введено в эксплуатацию!", Отказ);
			Рез = Ложь;
		ИначеЕсли ТекСтрока.ТС.уатДатаВыбытия <> '00010101' И ТекСтрока.ДатаВыбытия > ТекСтрока.ТС.уатДатаВыбытия Тогда
			уатОбщегоНазначенияТиповые.СообщитьОбОшибке("ТС / оборудование """ + Строка(ТекСтрока.ТС) + """ выбыло из эксплуатации (дата выбытия: " + Формат(ТекСтрока.ТС.уатДатаВыбытия, "ДФ = дд.ММ.гггг") + ")!", Отказ);
			Рез = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ТССоответствуютМестонахождению(Отказ)
	...
	Для Каждого ТекСтрока Из ТС Цикл
		...
		Если МестоТС.Организация <> Организация Тогда
			уатОбщегоНазначенияТиповые.СообщитьОбОшибке("Местонахождение ТС / оборудования """ + Строка(ТекСтрока.ТС)
