...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СведенияОДоходах.Количество() = 0
		...
		СообщениеОбОшибке = НСтр("ru = 'Не заполнены табличные части, должна быть заполнена хотя бы одна табличная часть.';
								|en = 'Tabular sections are not filled in, at least one tabular section required.'");
	...
	Для Каждого СтрокаНалога Из НДФЛИсчисленныйПоСтавке13 Цикл
		Если СтрокаНалога.Сумма = 0 И СтрокаНалога.ЗачтеноАвансовыхПлатежей = 0 Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В строке №%1 не указано ни суммы налога ни суммы зачтенных авансовых платежей.';
																							|en = 'The string No.%1 does not contain either a tax amount or the amount of accounted advance payments.'"), СтрокаНалога.НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке,,,,Отказ);
		КонецЕсли;
	КонецЦикла;
