	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеПриемкиПоВозвратам")
		...
		Если КомандаСоздания <> Неопределено Тогда
			КомандаСоздания.Представление = НСтр("ru = 'Акт о расхождениях после возврата от клиента';
												|en = 'Discrepancy certificate after return from a customer'");
		КонецЕсли;
...
//
Функция ДанныеДляЕГАИС(Объект, МенеджерВременныхТаблиц) Экспорт
	
	Возврат Документы.ЧекККМВозврат.ДанныеДляЕГАИС(Объект, МенеджерВременныхТаблиц);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Заявление на возврат товаров';
										|en = 'Application for return of goods'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			ИтоговаяСтрока = НСтр("ru = 'Всего наименований %КоличествоНаименований%';
									|en = 'Total items %КоличествоНаименований%'");
...
Функция ДанныеДляМОТП(ДокументСсылка, МенеджерВременныхТаблиц)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	КОЛИЧЕСТВО(*) КАК КоличествоСерий
	               |ИЗ
	...
	МассивУпаковок = РезультатЗапроса[РезультатЗапроса.Количество() - 2].Выгрузить().ВыгрузитьКолонку("ШтрихкодУпаковки");
	КоличествоСерий = РезультатЗапроса[РезультатЗапроса.Количество() - 1].Выгрузить()[0][0]; 
	...
	|		КОГДА НЕ Серии.Серия ЕСТЬ NULL
	|				И Серии.Количество <> Товары.Количество
	|			ТОГДА ВЫРАЗИТЬ(Товары.НомерСтроки * &КоличествоСерий + Серии.НомерСтроки КАК ЧИСЛО(10, 0))
	|		ИНАЧЕ ВЫРАЗИТЬ(Товары.НомерСтроки * ВЫБОР
	|					КОГДА &КоличествоСерий = 0
	|						ТОГДА 1
	|					ИНАЧЕ &КоличествоСерий
	|				КОНЕЦ КАК ЧИСЛО(10, 0))
	|	КОНЕЦ КАК ИндексСерии,
	|	ВЫБОР
	|		КОГДА НЕ Серии.Серия ЕСТЬ NULL
	|				И Серии.Количество <> Товары.Количество
	|			ТОГДА ВЫРАЗИТЬ(Товары.СуммаСкидки * Серии.Количество / Товары.Количество КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ Товары.СуммаСкидки
	|	КОНЕЦ КАК СерииСуммаСкидки,
	|	ВЫБОР
	|		КОГДА НЕ Серии.Серия ЕСТЬ NULL
	|				И Серии.Количество <> Товары.Количество
	|			ТОГДА ВЫРАЗИТЬ(Товары.СуммаСНДС * Серии.Количество / Товары.Количество КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ Товары.СуммаСНДС
	|	КОНЕЦ КАК СерииСуммаСНДС
	...
	|	СУММА(ТоварыПоСериямБезСуммовыхРазниц.СерииСуммаСкидки) КАК СуммаСкидки,
	|	СУММА(ТоварыПоСериямБезСуммовыхРазниц.СерииСуммаСНДС) КАК СуммаСНДС
	...
	|	Товары.Упаковка КАК Упаковка,
	|	Товары.Серия КАК Серия,
	|	Товары.ИндексСерии КАК ИндексСерии,
	|	Товары.СерииКоличество КАК Количество,
	|	Товары.СерииКоличествоУпаковок КАК КоличествоУпаковок,
	|	Товары.Штрихкод КАК Штрихкод,	
	|	Товары.Цена КАК Цена,
	|	ВЫБОР
	|		КОГДА СуммовыеРазницыСерий.СуммаСкидки ЕСТЬ NULL
	|			ТОГДА Товары.СерииСуммаСкидки
	|		ИНАЧЕ ВЫРАЗИТЬ(Товары.СерииСуммаСкидки + Товары.СуммаСкидки - СуммовыеРазницыСерий.СуммаСкидки КАК ЧИСЛО(15, 2))
	|	КОНЕЦ КАК СуммаСкидки,
	|	Товары.СтавкаНДС КАК СтавкаНДС,
	|	ВЫБОР
	|		КОГДА СуммовыеРазницыСерий.СуммаСНДС ЕСТЬ NULL
	|			ТОГДА Товары.СерииСуммаСНДС
	|		ИНАЧЕ ВЫРАЗИТЬ(Товары.СерииСуммаСНДС + Товары.СуммаСНДС - СуммовыеРазницыСерий.СуммаСНДС КАК ЧИСЛО(15, 2))
	|	КОНЕЦ КАК СуммаСНДС,
	|	Товары.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	Товары.ТабачнаяПродукция КАК ТабачнаяПродукция
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыПоСериям.НомерСтроки КАК НомерСтроки,
	|	ТоварыПоСериям.ИндексСерии КАК ИндексСерии,
	|	ТоварыПоСериям.Номенклатура КАК Номенклатура,
	|	ТоварыПоСериям.Упаковка КАК Упаковка,
	|	ТоварыПоСериям.СуммаСкидки КАК СуммаСкидки,
	|	ТоварыПоСериям.СуммаСНДС КАК СуммаСНДС,
	|	ТоварыПоСериям.Количество,
	|	ТоварыПоСериям.КоличествоУпаковок КАК КоличествоУпаковок,
	...
	|				И ТоварыПоСериям.Количество <> 1
	|			ТОГДА ВЫРАЗИТЬ(ТоварыПоСериям.ИндексСерии * &КоличествоМаркированныхТоваров + МаркированныеТовары.НомерСтроки КАК ЧИСЛО(10, 0))
	|		ИНАЧЕ ВЫРАЗИТЬ(ТоварыПоСериям.ИндексСерии * ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА НЕ МаркированныеТовары.Штрихкод ЕСТЬ NULL
	|				И ТоварыПоСериям.КоличествоУпаковок <> 1
	|			ТОГДА ВЫРАЗИТЬ(ТоварыПоСериям.СуммаСкидки / ТоварыПоСериям.Количество КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ТоварыПоСериям.СуммаСкидки
	|	КОНЕЦ КАК МаркТоварыСуммаСкидки,
	|	ВЫБОР
	|		КОГДА НЕ МаркированныеТовары.Штрихкод ЕСТЬ NULL
	|				И ТоварыПоСериям.КоличествоУпаковок <> 1
	|			ТОГДА ВЫРАЗИТЬ(ТоварыПоСериям.СуммаСНДС / ТоварыПоСериям.Количество КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ТоварыПоСериям.СуммаСНДС
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.УстановитьПараметр("КоличествоСерий", КоличествоСерий);
...
Процедура УстановитьПараметрЗапросаОбъектРасчета(Запрос)
	...
	Если Запрос.Параметры.ВернутьДенежныеСредства Тогда
		Если Запрос.Параметры.РасчетыПоДоговорам Тогда
