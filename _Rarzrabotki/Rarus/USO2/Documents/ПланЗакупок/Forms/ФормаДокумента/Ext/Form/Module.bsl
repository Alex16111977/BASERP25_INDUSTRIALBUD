...
Процедура ЗаполнитьСкладПоЗначению(Команда)
	...
	Если СкладыКлиент.ПроверитьВозможностьЗаполненияСкладовВТабличнойЧасти(Объект, ТоварыПоПериодам, НСтр("ru = 'Товары';
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Поставщик)';
								|en = 'Unique ID (Supplier)'"),"ИдентификаторПартнер");
	...
	Иначе
		...
		КолонкиПоиска.Добавить(НСтр("ru = 'Дата поступления';
									|en = 'Inpayment date'"),"ДатаПоступления");
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'План закупок +15%';
																	|en = 'Purchase plan +15%'"), "[ПланыЗакупок] * 1.15");
