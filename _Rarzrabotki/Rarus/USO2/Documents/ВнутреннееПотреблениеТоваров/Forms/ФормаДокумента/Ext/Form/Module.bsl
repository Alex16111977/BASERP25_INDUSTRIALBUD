	...
	Если Склад <> Объект.Склад Тогда
		...
		Если Объект.ПотреблениеПоЗаказам И Объект.Товары.Количество() > 0 Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("СкладПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Список ""Товары"" будет очищен. Продолжить?';
																									|en = 'The Goods list will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			...
			НСтр("ru = 'Заказы на внутреннее потребление (списание) (%КоличествоДокументов%)';
				|en = 'Orders for internal consumption (write-off) (%КоличествоДокументов%)'")), ЭтаФорма,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		...
		Иначе
			...
			ИначеЕсли ИспользоватьНесколькоОрганизаций И ИспользоватьПодразделения И ЗначениеЗаполнено(Объект.Подразделение) Тогда
				ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на внутреннее потребление
				                        ...
				                        |en = 'No unregistered goods of orders for internal consumption
				                        |from warehouse ""%1"" in department ""%2"" in company ""%3""'");
			...
			ИначеЕсли ИспользоватьПодразделения И ЗначениеЗаполнено(Объект.Подразделение) Тогда
				ШаблонТекста = НСтр("ru = 'Нет неоформленных товаров по заказам на внутреннее потребление
				                        ...
				                        |en = 'No unregistered goods of orders for internal consumption
				                        |from warehouse ""%1"" in department ""%2""'");
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
...
Функция ПривязатьСтрокиКЗаказамНаВнутреннееПотребление(ОтобратьПоЗаказу = Ложь)
	...
	//++ НЕ УТКА
	|			КОГДА НЕ Ремонты.СтатьяРасходов ЕСТЬ NULL ТОГДА
	|				Ремонты.СтатьяРасходов
	//-- НЕ УТКА
	|			ИНАЧЕ
	|				НЕОПРЕДЕЛЕНО
	...
	//++ НЕ УТКА
	|			КОГДА НЕ Ремонты.Ссылка.ОбъектЭксплуатации ЕСТЬ NULL ТОГДА
	...
	|				
	//++ НЕ УТКА
	|			КОГДА МатериалыИРаботы.ВариантОбеспечения В(
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно),
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно)) ТОГДА
	|				
	|				ТаблицаЗаказы.ЗаказНаВнутреннееПотребление.Назначение
	...
	//++ НЕ УТКА
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.МатериалыИРаботы КАК МатериалыИРаботы
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаРемонт.Ремонты КАК Ремонты
