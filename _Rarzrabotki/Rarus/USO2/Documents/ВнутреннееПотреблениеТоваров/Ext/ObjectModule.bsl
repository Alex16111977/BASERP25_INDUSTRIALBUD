...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство И НЕ ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru = 'Запись документа запрещена, т.к. операция ""Передача в производство"" больше не используется.
								...
								|en = 'It is prohibited to write the document as operation ""Transfer to production"" is not used.
								|Material transfer is registered in form ""Production - Receipt and return of materials"".'");
...
Процедура ЗаполнитьНаОснованииПоступления(ДанныеЗаполнения)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Ввод на основании поступления на разные склады не поддерживается.';
							|en = 'Input based on receipt to different warehouses is not supported.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
