...
Процедура СоглашениеПриИзменении(Элемент)
	...
	Если Объект.СпособКомпенсации = ПредопределенноеЗначение("Перечисление.СпособыКомпенсацииВозвратовТоваров.ЗаменитьТовары") 
		...
		ТекстВопроса = НСтр("ru = 'При изменении соглашения будут пересчитаны цены возвращаемых товаров.
			...
			|en = 'Returned goods prices will be recalculated on changing the agreement.
			|Populate replacing goods with the returned ones?'");
...
Процедура СпособКомпенсацииПриИзменении(Элемент)
	...
	Если Не Объект.СпособКомпенсации=ПредопределенноеЗначение("Перечисление.СпособыКомпенсацииВозвратовТоваров.ЗаменитьТовары")	
		...
		ТекстВопроса = НСтр("ru = 'Табличная часть заменяющих товары будет очищена, продолжить?';
							|en = 'Tabular section of substitute goods will be cleared, continue?'");
...
Процедура ЗаполнятьЦеныЗаменяющихТоваровПоСоглашениюПриИзменении(Элемент)
	...
	ТекстВопроса = НСтр("ru = 'Табличная часть заменяющих товары будет перезаполнена, продолжить?';
						|en = 'Tabular section of substitute goods will be refilled, continue?'");
...
Процедура ВозвращаемыеТоварыЗаполнитьЦеныПоСоглашению()
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект, "ВозвращаемыеТовары", НСтр("ru = 'Возвращаемые товары';
																								|en = 'Returned goods'")) Тогда
...
Процедура ВозвращаемыеТоварыЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "ВозвращаемыеТовары", НСтр("ru = 'Возвращаемые товары';
																							|en = 'Returned goods'")) Тогда
...
Процедура ЗаменяющиеТоварыЗаполнитьЦеныПоСоглашению()
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоСоглашению(Объект,"ЗаменяющиеТовары", НСтр("ru = 'Заменяющие товары';
																							|en = 'Substitute goods'")) Тогда
...
Процедура ЗаменяющиеТоварыЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "ЗаменяющиеТовары", НСтр("ru = 'Заменяющие товары';
																						|en = 'Substitute goods'")) Тогда
...
Процедура ЗаменяющиеТоварыОтменитьВыделенныеСтроки(Команда)
	...
	Если Не ПродажиКлиент.НеобходимоЗаполнениеПричиныОтменыВыделенныхСтрок(Объект.ЗаменяющиеТовары, НСтр("ru = 'Заменяющие товары';
																										|en = 'Substitute goods'"), Элементы.ЗаменяющиеТовары.ВыделенныеСтроки) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВозвращаемыеТоварыОтменитьВыделенныеСтроки(Команда)
	...
	Если Объект.ВозвращаемыеТовары.Количество() = 0 Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Таблица возвращаемых товаров не заполнена.';
													|en = 'Returned goods table is not filled in. '"));
		Возврат;
...
Процедура ЗаполнитьЗаменяющиеТоварыПоВозвращаемым(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Табличная часть заменяющих товаров будет перезаполнена. 
		...
		|en = 'Tabular section of substitute goods will be refilled.
		|Fill in the prices by sales conditions or returned goods?'");
...
Процедура НазначитьРучнуюСкидку(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможноНазначениеРучнойСкидкиНаценки(Объект, "ЗаменяющиеТовары", НСтр("ru = 'Заменяющие товары';
																										|en = 'Substitute goods'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтменитьРучныеСкидки(Команда)
	...
	Если Не СкидкиНаценкиКлиент.ВозможнаОтменаРучныхСкидокНаценок(Объект, "ЗаменяющиеТовары", НСтр("ru = 'Заменяющие товары';
																									|en = 'Substitute goods'")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаменяющиеТоварыОткрытьПодбор(Команда)
	...
	Иначе
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", НСтр("ru = 'заявку на возврат товаров от клиента';
																				|en = 'request for goods return from customer'"));
	КонецЕсли;
...
Процедура ЗаполнитьРеализацииИЦены(Команда)
	...
	Если Объект["ВозвращаемыеТовары"].Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'В документе не заполнен список Возвращаемые товары.  Документы продажи и цены не могут быть заполнены.';
									|en = 'The Returned goods list is not populated in the document. Sales documents and prices cannot be filled in.'");
...
Процедура ОбработкаВыбораПодборПереданныхТоваров(ВыбранноеЗначение)
	...
	ЦеныРассчитаны = ПродажиСервер.ЗаполнитьЦены(Объект.ВозвращаемыеТовары,
												МассивСтрок,
	...
	Иначе
		...
		Иначе
			...
			Если СтруктураРасчетов.НашДолг > СтруктураРасчетов.ДолгКлиента Тогда
				Расчеты = НСтр("ru = 'Наш долг по заявке';
								|en = 'Our debt against the request'") + ": "+ Формат(ИтогРасчетов,"ЧДЦ=2") + ВалютаДокумента;
			ИначеЕсли СтруктураРасчетов.ДолгКлиента > СтруктураРасчетов.НашДолг Тогда
				Расчеты = НСтр("ru = 'Долг клиента по заявке';
								|en = 'Customer debt against request'") + ": "+ Формат(ИтогРасчетов*-1,"ЧДЦ=2") + ВалютаДокумента;
			Иначе
