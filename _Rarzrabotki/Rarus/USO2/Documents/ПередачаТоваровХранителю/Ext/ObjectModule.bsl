...
//
// Параметры:
//	ПереданныеТовары - Булево, Истина - Признак формирования временных таблиц данных документа для товаров,
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для ТекИндекс = 0 По Товары.Количество() - 1 Цикл
		...
		Если ПередачаПоЗаказам
			...
			И Не ЗначениеЗаполнено(Товары[ТекИндекс].ЗаказКлиента) Тогда
	...
	Если Не ПередачаПоЗаказам
		...
		ТекстОшибки = НСтр("ru = 'Нет прав на создание передачи товаров хранителю без заказа';
							|en = 'No rights to create goods transfer to the custodian without order'");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.ПередачаТоваровХранителю.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
...
Процедура ЗаполнитьДокументНаОснованииАктаОРасхожденияхПослеОтгрузки(Знач ДанныеЗаполнения)
	...
	|	ПередачаТоваров.ДатаРаспоряжения  КАК ДатаРаспоряжения,
