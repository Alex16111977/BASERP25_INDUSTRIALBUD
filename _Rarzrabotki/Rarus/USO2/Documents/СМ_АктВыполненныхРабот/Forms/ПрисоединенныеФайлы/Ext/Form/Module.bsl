...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СтруктураФайла") Тогда
		КомментарийКФайлу = Параметры.СтруктураФайла.КомментарийКФайлу;
		ЭлементОтбора = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбора.Использование = Истина;
		ЭлементОтбора.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
		ЭлементОтбора.ПравоеЗначение = Параметры.СтруктураФайла.СсылкаНаФайл;
	КонецЕсли;
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Параметры.РежимВыбора Тогда	
		Если Не ЗначениеЗаполнено(ДатаАкта)
			ИЛИ Не ЗначениеЗаполнено(НомерАкта) Тогда
			Состояние(НСтр("ru='Необходимо указать номер и дату акта'"),100);
			Возврат;
		КонецЕсли;
		Если Элементы.Список.РежимВыбора Тогда
			КомментарийКФайлу = "акт №"+строка(НомерАкта)+" от "+ Формат(ДатаАкта,"ДФ=dd.MM.yyyy");
			Структура = Новый Структура("Файл,Комментарий",ВыбраннаяСтрока,КомментарийКФайлу);
			Закрыть(Структура);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Иначе
		РаботаСФайламиКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ОткрытьКаталогФайла(Команда)
	...
	//КонецЕсли;
	//
	//РаботаСфайламиСлужебныйКлиент.ОткрытьКаталогСФайлом(ДанныеФайла);
...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	//КонецЕсли;
	//
	//РаботаСфайламиСлужебныйКлиент.ОбновитьПрисоединенныйФайл(ТекущиеДанные.Ссылка, ДанныеФайла, УникальныйИдентификатор);
...
Процедура Подписать(Команда)
	...
	РаботаСфайламиСлужебныйКлиент.ПодписатьФайл(ПрисоединенныйФайл, ДанныеФайла, УникальныйИдентификатор,
...
Процедура СохранитьВместеСЭП(Команда)
	...
	РаботаСФайламиКлиент.СохранитьВместеСЭП(ТекущиеДанные.Ссылка, ДанныеФайла, УникальныйИдентификатор);
...
Процедура ДобавитьЭПИзФайла(Команда)
	...
	РаботаСфайламиСлужебныйКлиент.ДобавитьПодписьИзФайла(ПрисоединенныйФайл, ДанныеФайла, УникальныйИдентификатор,
		Новый ОписаниеОповещения("ДобавлениеПодписейЗавершить", ЭтотОбъект));
...
Процедура ЗакончитьРедактирование(Команда)
	...
	//РаботаСфайламиСлужебныйКлиент.ПоместитьРедактируемыйФайлНаДискеВХранилище(ОписаниеОповещения, ДанныеФайла, УникальныйИдентификатор);
...
Процедура ПоместитьФайлВХранилищеИОсвободить(Знач ПрисоединенныйФайл,
	...
	РаботаСфайламиСлужебный.ПоместитьФайлВХранилищеИОсвободить(
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Элементы.Список.РежимВыбора Тогда
			КомментарийКФайлу = "акт №"+строка(НомерАкта)+" от "+ Формат(ДатаАкта,"ДФ=dd.MM.yyyy");
			Структура = Новый Структура("Файл,Комментарий",Значение,КомментарийКФайлу);
			Закрыть(Структура);
			Возврат;
		КонецЕсли;
	КонецЕсли;
