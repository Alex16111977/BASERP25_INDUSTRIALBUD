	...
	Если Объект.ОС.Количество() <> 0
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'При изменеии направления деятельности табличная часть будет очищена. Продолжить?';
				|en = 'When changing the line of business, the tabular section will be cleared. Continue?'"),
...
Процедура ОбновитьЗаголовкиКолонокСумм(Элементы, Валюта, ВалютаВзаиморасчетов)
	...
	Если ЗначениеЗаполнено(Валюта) Тогда
		Элементы.ОССумма.Заголовок = СтрШаблон(НСтр("ru = 'Сумма (%1)';
													|en = 'Amount (%1)'"), Валюта);
	Иначе
	...
	Если ЗначениеЗаполнено(ВалютаВзаиморасчетов) Тогда
		Элементы.ОССуммаВзаиморасчетов.Заголовок = СтрШаблон(НСтр("ru = 'Сумма взаиморасчетов (%1)';
