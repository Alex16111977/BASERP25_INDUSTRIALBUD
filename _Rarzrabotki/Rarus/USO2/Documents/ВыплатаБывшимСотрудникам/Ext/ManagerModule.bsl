...
Процедура СоздатьВТДанныеДокумента(МенеджерВременныхТаблиц, ДокументСсылка, СписокФизическихЛиц = Неопределено) Экспорт
	...
	РасчетыСКонтрагентами = ОтражениеЗарплатыВБухучетеРасширенный.СтатьяРасчетыСКонтрагентами();
	...
	Запрос.Текст =
		...
		|	Начисления.Ссылка.ОтношениеКЕНВД КАК ОтношениеКЕНВД,
		...
		|	Начисления.Ссылка.СтатьяФинансирования КАК СтатьяФинансирования,
		|	ВЫБОР
		|		КОГДА &РаботаВХозрасчетнойОрганизации
		|			ТОГДА &РасчетыСКонтрагентами
		|		ИНАЧЕ Начисления.Ссылка.СтатьяРасходов
		|	КОНЕЦ КАК СтатьяРасходов,
		|	Начисления.Ссылка.СпособОтраженияЗарплатыВБухучете КАК СпособОтраженияЗарплатыВБухучете,
...
Процедура СоздатьВТНачисленияУдержанияДокумента(МенеджерВременныхТаблиц, ДокументОбъект, Организация, ПериодРегистрации) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплата") Тогда
		...
		Запрос.Текст = 
			...
			|	ДанныеДокумента.СкидкаПоВзносам,
		...
		Запрос.Текст = 
			...
			|	ВЫБОР
			|		КОГДА Начисления.Сумма = 0
			|			ТОГДА Удержания.Удержано
			|		ИНАЧЕ ВЫРАЗИТЬ(Удержания.Удержано * БухучетНачислений.Сумма / Начисления.Сумма КАК ЧИСЛО(15, 2))
			|	КОНЕЦ КАК Сумма,
			|	Удержания.Получатель,
			|	Удержания.ПлатежныйАгент,
