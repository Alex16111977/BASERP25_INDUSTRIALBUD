...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// {УАТ}
	уатДобавитьКомандуЗаполненияПоПЛ();
	// {/УАТ}
...
&НаСервере 
Процедура уатДобавитьКомандуЗаполненияПоПЛ() 
 	КомандаЗаполнитьПоПЛ = Команды.Добавить("уатЗаполнитьПоПЛ"); 
 	КомандаЗаполнитьПоПЛ.Действие = "ЗаполнитьПоПЛ"; 
 	КомандаЗаполнитьПоПЛ.Заголовок = "Заполнить по ПЛ"; 
 КомандаЗаполнитьПоПЛ.Подсказка = "Заполнить данными рабочего времени по путевым листам"; 
 	Кнопка 	= 	Элементы.Добавить("уатЗаполнитьПоПЛ", 	Тип("КнопкаФормы"), Элементы.ФормаКоманднаяПанель); 
 	Кнопка.Заголовок = "Заполнить по ПЛ"; 
 	Кнопка.ИмяКоманды = "уатЗаполнитьПоПЛ"; 
КонецПроцедуры 
 
&НаКлиенте 
Процедура ЗаполнитьПоПЛ(Команда) 
 	ОбработкаЗаполненияПоПЛСервер(); 
КонецПроцедуры 
 
&НаСервере 
Процедура ОбработкаЗаполненияПоПЛСервер(СписокСотрудников = Неопределено)
	уатИнтеграцияСервер.ЗаполнитьТабельПоДаннымПЛ(ЭтотОбъект);
КонецПроцедуры 
