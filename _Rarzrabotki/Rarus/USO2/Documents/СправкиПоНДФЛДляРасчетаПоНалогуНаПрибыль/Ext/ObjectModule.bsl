...
Процедура ПроверитьДанныеДокумента(Отказ) Экспорт
	...
	Пока ВыборкаСотрудниковДляПроверки.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		Если ВыборкаСотрудниковДляПроверки.ЕстьПовторяющиесяДанныеСправок Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Данные по ставке %1 были введены в документе ранее';
																							|en = 'Данные по ставке %1 были введены в документе ранее'"), ВыборкаСотрудниковДляПроверки.СтавкаПредставление);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, ПутьКДанным, ,Отказ);   
		КонецЕсли;
		...
		НачалоСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Получатель дохода %1, справка № %2:';
																						|en = 'Получатель дохода %1, справка № %2:'"), ВыборкаСотрудниковДляПроверки.СотрудникНаименование, ВыборкаСотрудниковДляПроверки.НомерСправки) + " ";
