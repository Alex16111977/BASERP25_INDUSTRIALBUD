...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого СтрокаТовары Из Товары Цикл
		...
		Если СтрокаТовары.ЕстьУточнения Тогда
			...
			Если ИтогоКоличество <> 0 И Не ЗначениеЗаполнено(ТоварыУточнения.Получить(0).Номенклатура) Тогда
				...
				ШаблонСообщенияСроки = НСтр("ru = 'Нет данных по уточнямым товарам в строке %1 списка ""%2""';
											|en = 'Нет данных по уточнямым товарам в строке %1 списка ""%2""'");
		...
		Если ЗначениеЗаполнено(СтрокаТовары.Номенклатура) И СтрокаТовары.Количество = 0 Тогда
			...
			ШаблонСообщенияСроки = НСтр("ru = 'Не заполнено поле ""%1"" в строке %2 списка ""%3""';
										|en = 'Не заполнено поле ""%1"" в строке %2 списка ""%3""'");
			ТекстСообщения = СтрШаблон(ШаблонСообщенияСроки, НСтр("ru = 'Количество';
																	|en = 'Количество'"),	СтрокаТовары.НомерСтроки, ПредставлениеТЧ);
