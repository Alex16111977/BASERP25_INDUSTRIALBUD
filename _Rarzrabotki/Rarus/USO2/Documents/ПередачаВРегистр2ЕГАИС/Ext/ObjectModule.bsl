...
Процедура ЗаполнитьНаОснованииТТНВходящейЕГАИС(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИнтеграцияЕГАИСПереопределяемый.ПолеКоэффициентПересчетаНеупакованнойПродукции(КоэффициентПересчета);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеКоэффициентПересчетаНеупакованнойПродукции", КоэффициентПересчета);
...
Процедура ЗаполнитьПоВыбраннымСтрокамОстатков(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	КоэффициентПересчета = "1";
	ИнтеграцияЕГАИСПереопределяемый.ПолеКоэффициентПересчетаНеупакованнойПродукции(КоэффициентПересчета, "СопоставленыеПозиции");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеКоэффициентПересчетаНеупакованнойПродукции", КоэффициентПересчета);
