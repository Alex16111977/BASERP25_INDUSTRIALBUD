...
Функция ТоварыЗаполнитьЦеныПоВидуЦенСервер()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'В таблице ""Товары и работы"" не выбрано ни одной строки. Цены не могут быть заполнены.';
								|en = 'No row is selected in the ""Goods and works"" table. Prices cannot be filled in.'");
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоРаспоряжениямЗавершение", ЭтотОбъект), 
			НСтр("ru = 'Список ""Товары и работы"" будет перезаполнен. Продолжить?';
				|en = 'The ""Goods and works"" list will be refilled. Continue?'"), 
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<для работ без назначения>';
																|en = '<for works without assignment>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
...
Процедура ПроверитьСпецификации(Отказ)
	...
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru = 'Изделие ""%1"" не производится по указанной спецификации в строке %2 списка Товары. Выберите другую спецификацию.';
											|en = 'Product ""%1"" is not manufactured by the specified bill of materials in line %2 of the Goods list. Select another bill of materials.'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru = 'Спецификация ""%1"" в строке %2 списка Товары не является действующей. Выберите действующую спецификацию.';
											|en = 'The ""%1"" bill of materials in line %2 of the Goods list is not valid. Select a valid BOM.'"));
	...
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru = 'В спецификации ""%1"" в строке %2 списка Товары не заполнены статьи калькуляции.';
											|en = 'Costing items are not populated in the ""%1"" bill of materials in line %2 of the Goods list.'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru = 'Спецификация ""%1"" в строке %2 списка Товары содержит уточняемую при производстве номенклатуру';
											|en = 'The ""%1"" bill of materials in line %2 of the Goods list contains products specified during production'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru = 'Спецификация ""%1"" в строке %2 списка Товары содержит уточняемую при производстве характеристику';
											|en = 'The ""%1"" bill of materials in line %2 of the Goods list contains a characteristic specified during production'"));
