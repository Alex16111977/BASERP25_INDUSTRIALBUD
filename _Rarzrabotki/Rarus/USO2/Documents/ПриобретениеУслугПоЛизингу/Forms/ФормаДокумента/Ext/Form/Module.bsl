...
Процедура НачисленияДокументОбеспечительногоПлатежаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСТР("ru = 'Не заполнено поле ""Организация""';
																|en = 'Company is not filled in'"), , , , Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСТР("ru = 'Не заполнено поле ""Контрагент""';
																|en = 'Counterparty is not filled in'"), , , , Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Договор) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСТР("ru = 'Не заполнено поле ""Договор""';
																|en = 'Contract is not filled in'"), , , , Отказ);
	КонецЕсли;
	...
	Если ПроверитьЗаполнениеПолейОтбора() Тогда
		...
		Если Не Объект.Начисления.Количество() Тогда
			Текст = СтрШаблон(НСтр("ru = 'По договору %1 отсутствует график начисления за %2';
									|en = 'Accrual schedule dated %2 is missing for contract %1'"),
				Объект.Договор,
...
Процедура ОбновитьЗаголовкиКолонокСумм(Элементы, Валюта, ВалютаВзаиморасчетов)
	...
	Если ЗначениеЗаполнено(ВалютаВзаиморасчетов) Тогда
		Элементы.НачисленияСуммаВзаиморасчетов.Заголовок = СтрШаблон(НСтр("ru = 'Сумма взаиморасчетов (%1)';
																			|en = 'Mutual settlements amount (%1)'"), ВалютаВзаиморасчетов);
	Иначе
		Элементы.НачисленияСуммаВзаиморасчетов.Заголовок = НСтр("ru = 'Сумма взаиморасчетов';
																|en = 'Mutual settlements amount'");
	КонецЕсли;
КонецПроцедуры
