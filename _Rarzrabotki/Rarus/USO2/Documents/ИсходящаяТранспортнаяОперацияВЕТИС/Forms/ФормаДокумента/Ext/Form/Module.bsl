...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТоварыБлагополучиеМестности.СписокВыбора.Добавить(НСтр("ru = 'Местность благополучна по заразным болезням животных';
...
Процедура ОбновитьДанные(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИмяОперации = "ТоварыНоменклатура" Тогда
		...
		Если ПродукцияДоИзменения <> ИзменяемаяСтрока.Продукция Тогда
			ИзменяемаяСтрока.ТипЖивыеЖивотные = ИнтеграцияВЕТИСВызовСервера.ПродукцияПринадлежитТипуЖивыеЖивотные(ИзменяемаяСтрока.Продукция);
