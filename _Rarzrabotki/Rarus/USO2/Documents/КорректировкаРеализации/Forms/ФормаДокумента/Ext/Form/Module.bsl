		...
		ИначеЕсли ПараметрыЗаписи.РежимЗаписи <> РежимЗаписиДокумента.ОтменаПроведения Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru = 'Табличная часть ""Товары"" была изменена. Перезаполнить расхождения?';
					|en = 'The Goods tabular section was changed. Fill out the discrepancies?'"),
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.ВидЦены) И НЕ Объект.Согласован Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Для редактирования суммы с НДС выберите вид цены ""<произвольная>""';
					|en = 'To edit amount VAT included, select price type ""<arbitrary>""'"),
				Объект.Ссылка,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Объект.Товары", ТекущаяСтрока.НомерСтроки, "ВидЦены"),);
		КонецЕсли;
...
Процедура ТоварыПослеУдаленияСервер()
	...
	ПроверкаИПодборПродукцииМОТПВызовСервера.ПрименитьКешШтрихкодовУпаковок(
		ЭтотОбъект,
		ПараметрыИнтеграцииФормыПроверкиИПодбора(ЗначениеЗаполнено(Объект.Склад)),
		Истина);
КонецПроцедуры
	...
	Если Не ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Документ основание"" не заполнено';
				|en = 'The ""Base document"" field is not filled in'"),
			,
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// Проверка и подбор МОТП
	ПараметрыИнтеграции = ПараметрыИнтеграцииФормыПроверкиИПодбора(ЗначениеЗаполнено(Объект.Склад));
	ПроверкаИПодборПродукцииМОТП.ПриСозданииНаСервере(ЭтотОбъект, ПараметрыИнтеграции);
	Если НЕ ЗначениеЗаполнено(Объект.Склад) И Элементы.Найти(ПараметрыИнтеграции.ИмяТабличнойЧастиТовары+"СтатусПроверкиГосИС") <> Неопределено Тогда 
		Элементы[ПараметрыИнтеграции.ИмяТабличнойЧастиТовары+"СтатусПроверкиГосИС"].Видимость = Ложь;
	КонецЕсли;
	// Конец Проверка и подбор МОТП
	ОбновитьСлужебныеРеквизитыТабличныхЧастей();
	// Проверка и подбор МОТП
	ПроверкаИПодборПродукцииМОТПВызовСервера.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект, ПараметрыИнтеграцииФормыПроверкиИПодбора(ЗначениеЗаполнено(Объект.Склад)));
	// Конец Проверка и подбор МОТП
...
Процедура УстановитьПредставлениеДокументаОснования()
	...
	ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		ПредставлениеДокументаОснования = НСтр("ru = 'Реализация товаров и услуг <не выбран>';
												|en = 'Sales of goods and services <not selected>'");
...
Процедура УстановитьВариантыОтражения(ИмяТЧ)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Нет строк, в которых доступно изменение варианта отражения.';
									|en = 'No lines in which display option can be changed.'"));
...
Процедура ОбновитьТекстСчетаФактурыВыданные()
	...
	ТекстСчетаФактурыВыданные = ПродажиСервер.ДополнитьПредставлениеПредставлениемФискальнойОперацииВДокументеПродажи(Объект.Ссылка, СчетаФактурыВыданныеНаОсновании); // +++ frov
...
Процедура ПроверитьВозможностьСозданияТранспортнойНакладной()
	...
	Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов")
		...
		СообщениеПользователю = НСтр("ru = 'Невозможно оформить транспортную накладную, т.к. в качестве документа-основания выступает документ ""%ДокументОснование%"".';
									|en = 'Cannot register the consignment note because document ""%ДокументОснование%"" is the base document.'");
...
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыИнтеграцииФормыПроверкиИПодбора(ЗаполненСклад)
	...
	ПараметрыИнтеграции.ИспользоватьБезТабачнойПродукции = ЗаполненСклад;
