...
Процедура ГСМвТСТСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	уатИнтерфейсВводаТС.РеквизитТСНачалоВыбора(Элемент, Элементы.ГСМвТС.ТекущиеДанные.ТС, ДанныеВыбора, СтандартнаяОбработка, СтруктураОтбора);
...
&НаКлиенте
Процедура ГСМвТС_ПерезаполнитьУчетныеКоличестваИСуммыФрагмент(НомерСтроки = Неопределено)
    ...
    Если Модифицированность Или Параметры.Ключ.Пустая() Тогда
        ...
        |Продолжить?";
        ПоказатьВопрос(Новый ОписаниеОповещения("ГСМвТС_ПерезаполнитьУчетныеКоличестваИСуммыФрагментЗавершение", ЭтотОбъект, НомерСтроки), ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да, );
        Возврат;
    КонецЕсли;
    
    ГСМвТС_ПерезаполнитьУчетныеКоличестваИСуммыФрагментФрагмент(флЗаписать, НомерСтроки);
КонецПроцедуры

&НаКлиенте
Процедура ГСМвТС_ПерезаполнитьУчетныеКоличестваИСуммыФрагментЗавершение(РезультатВопроса, НомерСтроки) Экспорт
...
&НаКлиенте
Процедура ГСМвТС_ПерезаполнитьУчетныеКоличестваИСуммыФрагментФрагмент(Знач флЗаписать, НомерСтроки)
    
    ПерезаполнитьУчетныеКоличестваИСуммыСервер(мВалютаРегламентированногоУчета, флЗаписать, НомерСтроки);
