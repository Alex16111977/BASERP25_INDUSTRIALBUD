	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если Не ИспользоватьОбособленноеОбеспечениеЗаказов Тогда
			ТекстИсключения = НСтр("ru = 'Ввод разрешений на замену возможен при использовании обособленного обеспечения заказов.';
									|en = 'You can enter the substitution permissions when using pegged order fulfillment.'");
		...
		Если Не ЕстьТоварыПодЗаказ(ДанныеЗаполнения) Тогда
			ТекстИсключения = НСтр("ru = 'Ввод разрешений на замену возможен при наличии товаров с обеспечением ""Обособленно"".';
									|en = 'You can enter the substitution permissions if there are goods with supply ""On pegging basis"".'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		ПроверитьДублиНабора("Материалы", НСтр("ru = 'Обнаружено дублирование строк в наборе заменяемых материалов.';
												|en = 'Line duplication in the substituted material set is detected.'"), Отказ);
	КонецЕсли;
	
	ПроверитьДублиНабора("Аналоги", НСтр("ru = 'Обнаружено дублирование строк в наборе аналогов.';
										|en = 'Line duplication in the analog set is detected.'"), Отказ);
		...
		Если Не ЗначениеЗаполнено(Строка.КлючСвязиСпецификация) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Ошибка связи со строкой спецификации в наборе заменяемых материалов.';
					|en = 'An error occurred while connecting to BOM line in the replaced material set.'"),
				ЭтотОбъект,
		...
		Иначе
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Обнаружено дублирование строк в наборе заменяемых материалов.';
					|en = 'Line duplication in the substituted material set is detected.'"),
				ЭтотОбъект,
				"Материалы[" + (Строка.НомерСтроки - 1) + "].ПрименениеМатериала",
				"Объект",
				Отказ);
