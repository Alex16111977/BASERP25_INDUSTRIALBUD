...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	ИдСтроки = Элементы.НаборЗаписей.ТекущаяСтрока;
	Если ИдСтроки <> Неопределено Тогда
		Проводка = НаборЗаписей.НайтиПоИдентификатору(ИдСтроки);
...
&НаСервере
Процедура ЗарегистрироватьКОтражениюВМеждународномУчете()
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	ДанныеРегистра.Организация КАК Организация,
	|	ДанныеРегистра.Период КАК Период,
	|	ДанныеРегистра.Период КАК ДатаОтражения,
	...
	ДокументыКОтражению = Запрос.Выполнить().Выгрузить();
...
&НаКлиенте
Процедура ПереОтразитьДокументВМеждународномУчете(Команда)
    
    ЗарегистрироватьКОтражениюВМеждународномУчете();
    ОтразитьВМеждународномУчете(Команда);
...
Процедура УстановитьДоступностьПриОткрытииФормы()
    ...
    Элементы.ФормаПереОтразитьДокументВМеждународномУчете.Видимость   = ОбщегоНазначения.РежимОтладки();
    Элементы.ФормаПереОтразитьДокументВМеждународномУчете.Доступность = ОбщегоНазначения.РежимОтладки();
...
Процедура ПолучитьСостояниеОтраженияДокумента()
	...
	Иначе
		...
		Иначе
			Состояние = НСтр("ru='Документ не отражается в международном учете или отсутствует учетная политика для организации.';uk='Документ не відображається в міжнародному обліку або відсутня облікова політика для організації.'");
