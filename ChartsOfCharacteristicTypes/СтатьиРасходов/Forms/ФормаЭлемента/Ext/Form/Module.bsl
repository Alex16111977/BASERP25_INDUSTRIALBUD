...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТКА
	Элементы.ГруппаСтраницаРеглУчет.Заголовок = НСтр("ru='Регламентированный учет и МФУ';uk='Регламентований облік і МФО'");
	//-- НЕ УТКА
...
Процедура ЗаполнитьСписокВидовАналитикРасходов()
	...
	ИначеЕсли Объект.ТипРасходов = ПредопределенноеЗначение("Перечисление.ТипыРасходов.Производственные") Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользуетсяУправлениеПроизводством21и22") Тогда
			СписокВыбора.Добавить("ДокументСсылка.ЗаказНаПроизводство2_2", 	НСтр("ru='Заказ на производство (2.5)';uk='Замовлення на виробництво (2.5)'"));
		Иначе
		//-- Устарело_Производство21	
			СписокВыбора.Добавить("ДокументСсылка.ЗаказНаПроизводство2_2", 	НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'"));
		//++ Устарело_Производство21	
		КонецЕсли;
		//-- Устарело_Производство21
		СтатьиРасходовЛокализация.ДополнитьСписокТиповАналитикамиПроизводством21(СписокВыбора);
		
		СписокВыбора.Добавить("ДокументСсылка.ЗаказНаРемонт",           НСтр("ru='Заказ на ремонт';uk='Замовлення на ремонт'"));
		//-- НЕ УТКА
