...
// Настройки – структура, возвращаемая функцией ОбменДаннымиСервер.НастройкиПланаОбменаПоУмолчанию().
//
// Параметры:
//  Настройки - Структура - настройки плана обмена
//
Процедура ПриПолученииНастроек(Настройки) Экспорт
	
	// Параметры безопасности обмена
	Настройки.ПредупреждатьОНесоответствииВерсийПравилОбмена = Истина;
	
	// Подключаем обработчики мастера настройки
	Настройки.Алгоритмы.ПриПолученииОписанияВариантаНастройки = Истина;
	Настройки.Алгоритмы.ПриПолученииДанныхОтправителя = Истина;
	
КонецПроцедуры

// Формирует описание варианта настройки обмена для помощника синхронизации данных.
//
// Параметры:
//  ОписаниеВарианта - Структура - описание варианта настройки обмена
//  ИдентификаторНастройки - Строка - идентификатор настройки
//  ПараметрыКонтекста - Структура - параметры контекста вызова
//
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	ОписаниеВарианта.ИмяКонфигурацииКорреспондента = "Казначейство";
	ОписаниеВарианта.НаименованиеКонфигурацииКорреспондента = НСтр("ru='Казначейство';uk='Казначейство'");
	ОписаниеВарианта.ЗаголовокКомандыДляСозданияНовогоОбменаДанными = ЗаголовокКомандыДляСозданияНовогоОбменаДанными();
	ОписаниеВарианта.ЗаголовокПомощникаСозданияОбмена = НСтр("ru='Настройка синхронизации с Казначейство';uk='Налаштування синхронізації з Казначейство'");
	ОписаниеВарианта.ЗаголовокУзлаПланаОбмена = НСтр("ru='Синхронизация с Казначейство';uk='Синхронізація з Казначейство'");
	
	// Общие данные узлов и формы мастера
	ОписаниеВарианта.ИмяФормыСозданияНачальногоОбраза = ИмяФормыСозданияНачальногоОбраза();
	ОписаниеВарианта.Отборы = НастройкаОтборовНаУзле(ПараметрыКонтекста.ВерсияКорреспондента, "", ИдентификаторНастройки);
	ОписаниеВарианта.ЗначенияПоУмолчанию = ЗначенияПоУмолчаниюНаУзле(ПараметрыКонтекста.ВерсияКорреспондента, "", ИдентификаторНастройки);
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки = "") Экспорт
	...
	ПоясняющийТекст = НСтр("ru='Позволяет синхронизировать данные между конфигурациями ""BAS ERP"", и ""BAS Казначейство"".
                            |Синхронизация данных выполняется в двустороннем режиме на уровне документов и справочной информации.'
                            |;uk='Дозволяє синхронізувати дані між конфігураціями ""BAS ERP"", ред. 2.5, і ""BAS Казначейство"".
...
Функция ПодробнаяИнформацияПоОбмену(ИдентификаторНастройки = "") Экспорт
	Возврат "ПланОбмена.КазначействоBASERP.Форма.ПодробнаяИнформация";
КонецФункции
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Казначейство';uk='Казначейство'");
...
Процедура ПриПодключенииККорреспонденту(ВерсияКорреспондента) Экспорт
	...
	Если ВерсияКорреспондента = "0.0.0.0" Тогда
		ВерсияКорреспондента = "1.0.1.45";
	КонецЕсли;                  
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияКорреспондента, "1.0.1.45") < 0 Тогда
...
Функция ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента(ВерсияКорреспондента) Экспорт
    ...
    |из приложения BAS ERP ред. 2.5, а также заполнить счета учета по умолчанию в регистрах сведений Счета учета номенклатуры и Счета расчетов с контрагентами.
    ...
    |з програми BAS ERP ред. 2.5, а також заповнити рахунки обліку за замовчуванням в регістрах дані Рахунки обліку номенклатури та Рахунки розрахунків з контрагентами.
