...
Процедура ОткрытьВыполнениеОпераций(Этап, Форма) Экспорт
	...
	Для каждого Ссылка Из МассивСсылок Цикл
		...
		Если ВыбранноеПодразделение <> Подразделения[Ссылка] Тогда
			ПоказатьПредупреждение(, НСтр("ru='Команда не может быть выполнена для выбранных объектов!';uk='Команда не може бути виконана для вибраних об''єктів!'"));
			Возврат;
		КонецЕсли;
	...
	Если Не ПараметрыПодразделения.ИспользоватьПооперационноеУправление Тогда
		ПоказатьПредупреждение(, НСтр("ru='Команда не может быть выполнена, в подразделении не используется пооперационное управление!';uk='Команда не може бути виконана, в підрозділі не використовується поопераційне управління!'"));
		Возврат;
	КонецЕсли;
...
Процедура ОповеститьПользователяОПередачиЭтаповКВыполнению(Результат, Источник = Неопределено) Экспорт
	...
	Если Результат.Количество > 0 Тогда
		...
		ТекстДокументов = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
				НСтр("ru='документ';uk='документ'"), НСтр("ru='документа';uk='документа'"), НСтр("ru='документов';uk='документів'"), Результат.Количество);
		
		ТекстСообщения = НСтр("ru='Передано к выполнению %Количество% %ТекстДокументов%';uk='Передано до виконання %Количество% %ТекстДокументов%'");
		...
		ТекстЗаголовка = НСтр("ru='Этапы производства переданы к выполнению';uk='Етапи виробництва передані до виконання'");
		
	Иначе
		
		ТекстСообщения = НСтр("ru='Ни один документ не передан к выполнению';uk='Жоден документ не переданий до виконання'");
		ТекстЗаголовка = НСтр("ru='Этапы производства не переданы к выполнению';uk='Етапи виробництва не передані до виконання'");
...
Процедура ОповеститьПользователяОФормированииЭтаповПроизводства(РезультатФормирования, Источник = Неопределено) Экспорт
	...
	ТекстЗаголовка = НСтр("ru='Результат формирования этапов производства';uk='Результат формування етапів виробництва'");
	...
	Если РезультатФормирования.СформированоДокументов > 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Сформировано %КоличествоОбработанных% %ТекстДокументов%';uk='Сформовано%КоличествоОбработанных% %ТекстДокументов%'");
...
Процедура ОповеститьПользователяОПланированииГрафикаПроизводства(Результат, Источник = Неопределено) Экспорт
	...
	Если Результат.ЕстьОшибки Тогда
		...
		Если Результат.КоличествоОбработанных > 0 Тогда
			ТекстЗаголовка = НСтр("ru='Запланировано %КоличествоОбработанных% из %КоличествоВсего% документов';uk='Заплановано %КоличествоОбработанных% з %КоличествоВсего% документів'");
		...
		Иначе
			ТекстЗаголовка = НСтр("ru='График производства не запланирован';uk='Графік виробництва не запланований'");
		КонецЕсли;
		
		ТекстСообщения = СтрШаблон(
			НСтр("ru='При планировании заказа %1 возникли ошибки. Подробнее см. в рабочем месте ""Планирование графика заказа"" (нажмите для перехода).';uk='При плануванні замовлення %1 виникли помилки. Детальніше див. в робочому місці ""Планування графіка замовлення"" (натисніть для переходу).'"),
	...
	Иначе
		...
		Если Результат.КоличествоОбработанных > 0 Тогда
			...
			ТекстСообщения = НСтр("ru='Запланировано %КоличествоОбработанных% из %КоличествоВсего% документов';uk='Заплановано %КоличествоОбработанных% з %КоличествоВсего% документів'");
			...
			ТекстЗаголовка = НСтр("ru='График производства запланирован';uk='Графік виробництва запланований'");
			
		Иначе
			
			ТекстСообщения = НСтр("ru='Не запланирован ни один документ';uk='Не заплановано жодний документ'");
			ТекстЗаголовка = НСтр("ru='График производства не запланирован';uk='Графік виробництва не запланований'");
...
Процедура ОповеститьПользователяОПометкеНаУдалениеЭтаповПроизводства(Результат) Экспорт
	...
	Если Результат.КоличествоОбработано > 0 ИЛИ Результат.ЕстьОшибки Тогда
		...
		Если Результат.ЕстьОшибки Тогда
			ТекстЗаголовка = НСтр("ru='Ошибка установки пометки на удаления этапов!';uk='Помилка встановлення позначки на вилучення етапів!'");
			ТекстШаблон = НСтр("ru='При пометке на удаление цепочки этапов %1 возникла ошибка! Помечено на удаление %2 %3.';uk='При позначці на видалення ланцюжка етапів %1 виникла помилка! Позначено на вилучення %2 %3.'");
			ТекстСообщения = СтрШаблон(ТекстШаблон,Результат.ОшибкаСсылка,Формат(Результат.КоличествоОбработано,"ЧН=; ЧГ="),ТекстДокументов);
		Иначе
			ТекстЗаголовка = НСтр("ru='Этапы производства помечены на удаление.';uk='Етапи виробництва позначені на вилучення.'");
			ТекстШаблон = НСтр("ru='Помечено на удаление %1 %2';uk='Помічено на вилучення %1 %2'");
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Ни один документ не помечен на удаление';uk='Жоден документ не позначений на вилучення'");
		ТекстЗаголовка = НСтр("ru='Не удалось пометить на удаление этапы производства';uk='Не вдалося помітити на вилучення етапи виробництва'");
...
Процедура ОбеспечениеМатериаламиИРаботамиЗаполнитьОбеспечение(Объект, ВыделенныеСтроки, Форма, ПутьМатериалыИУслуги) Экспорт
	...
	ПараметрыПроверки = ОбеспечениеКлиентСервер.ИнициализироватьПараметрыПроверкиЗаполнения(
		"ОбеспечениеМатериаламиИРаботами", НСтр("ru='Обеспечение';uk='Забезпечення'"));
...
Функция ЭкономияЗаполнитьСписокВыбораДействия(ТекущиеДанные, Форма)
	...
	СписокВыбораЭлемента.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратИзПроизводства"),
									НСтр("ru='Передать на склад';uk='Передати на склад'"));
...
Процедура ПроверитьУдалитьНеполныеЦепочкиЭтаповИзСписка(Список) Экспорт
	...
	Если СоответствиеСтрок.Количество() <> МассивЭтапов.Количество() Тогда
		...
		ТекстСообщения = НСтр("ru='Частичное удаление этапов при многоэтапном производственном процессе запрещено, воспользуйтесь рабочим местом ""Структура заказа на производство""';uk='Часткове вилучення етапів при багатоетапному виробничому процесі заборонено, скористайтеся робочим місцем ""Структура замовлення на виробництво""'");
...
//  ДокументСсылка.ЭтапПроизводства2, Неопределено - ссылка на найденный этап, Неопределено если значение не найдено.
//
...
//
// Параметры:
//	КоллекцияСтрок - Массив, ДанныеФормыКоллекция, ТаблицаЗначений - коллекция строк.
