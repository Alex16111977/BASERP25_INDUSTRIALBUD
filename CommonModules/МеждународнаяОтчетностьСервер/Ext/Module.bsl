...
Процедура ДобавитьТекстовыйБлок(Параметры, ЭлементОтчета)
	...
	Если СтрНайти(ТекстЗаголовка, ПеременныеФинОтчетов.Организация.Имя) > 0 Тогда
		...
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка,
				ПеременныеФинОтчетов.Организация,
				ИмяОрганизации);
	КонецЕсли;
...
Процедура ЗагрузитьЗначенияПоказателяВСтроку(ВыборкаПоказателя, ГруппировкиПоказателя, ОписаниеТаблицы, ПериодОтчета)
	...
	Для Каждого Колонка Из ДетальныеКолонки Цикл
		...
		Для Каждого Ячейка Из ВыборкаПоКолонкамПоказателя Цикл
			...
			Если ДанныеЗаПериод Тогда
				Период = ОписаниеПериода(Ячейка.ДатаНачала, Ячейка.ДатаОкончания);
			Иначе
				Период = ОписаниеПериода(ПериодОтчета.НачалоПериода, ПериодОтчета.КонецПериода);
			КонецЕсли;
			ПериодыЗначений.Вставить(Колонка.ИмяКолонки, Период);
...
// 	ОписаниеТаблицы - Структура - см. ОписаниеТаблицы
...
Процедура СформироватьДеревоГоризонтальныхИтогов(ДеревоКолонок, ВыборкаЗначений, ОписаниеТаблицы, КолонкаОтчета, Отбор = Неопределено, Знач КолонкиГруппировки = "")
	...
	Для Каждого Колонка Из ДеревоКолонок.Строки Цикл
		...
		ИначеЕсли Колонка.ВидЭлемента = ВидыЭлементов.Измерение Тогда
			...
			ДанныеГруппировки.Сортировать(Колонка.ИмяКолонки);
...
// Параметры:
// 	ИмяКолонки - Строка -
// 	НоваяКолонкаОтчета - СтрокаДереваЗначений - см. ОписаниеДереваКолонокОтчета
		...
		ИначеЕсли Строка.ВидЭлемента = ВидыЭлементов.Измерение Тогда
			...
			Для Каждого ДанныеСтроки Из Данные Цикл
				...
				Если ЗначениеЗаполнено(Строка.ВидСубконто) 
							И  Не Строка.ВидСубконто.ТипЗначения.СодержитТип(ТипЗнч(ЗначениеИзмерения))  Тогда
					Продолжить;
				КонецЕсли;
...
Процедура ПоказателиПроизвольнойТаблицы(ЯчейкиТаблицы, ЭлементыТаблицы, Параметры, ЭтоКолонки = Ложь)
	...
	Для Каждого ЭлементЯчейки Из ЯчейкиТаблицы.Строки Цикл
		Если ЭтоПоказатель(ЭлементЯчейки.ВидЭлемента, Параметры.ВидыЭлементов, ЭлементЯчейки.ЭтоСвязанный) Тогда
			ЭлементОтчета = ЯчейкиТаблицы.ЭлементОтчета; // СправочникСсылка.ЭлементыФинансовыхОтчетов - 
...
Процедура ДобавитьЭлементТаблицыИзмерение(ЭлементыТаблицы, Элемент, ДопСуффикс, ЭтоКолонки, ВидыЭлементов, ВидТаблицы)
	...
	ИначеЕсли ЭтоСубконто Тогда
		...
		Иначе 
			...
			Иначе
				ИмяКолонки = "Аналитика" + ?(ЭтоКолонки, "Колонок", "Строк") + НомерСубконто;
			КонецЕсли;
...
// 
// Параметры:
// 	ЭлементыТаблицы - Структура - см. ОписаниеТаблицы
...
// * ИнтервалыОтчета - см. Справочники.ЭлементыФинансовыхОтчетов.ИнтервалыОтчета
...
|з причини: не вказано рахунок плану рахунків.'");
	...
	ТочностьПоказателя = Неопределено;
	...
	КонецЕсли;
	Если ТочностьПоказателя  <> Неопределено Тогда
...
Функция РасшифровкаЯчейки(ОписаниеТаблицы, ДанныеСтрокиТаблицы, ИмяКолонки)
	...
	ИначеЕсли ТипЗнч(ИдСтрокиОтчета) = Тип("СправочникСсылка.ЭлементыФинансовыхОтчетов")
			И ИдСтрокиОтчета.ВидЭлемента = ВидыЭлементов.Измерение Тогда
			...
			Если СтрокаДляРасшифровки.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение Тогда
				...
				Если ЗначениеЗаполнено(ИмяКолонкиГруппировки) Тогда
					...
					Если ОписаниеПоказателя <> Неопределено Тогда
						Если ОписаниеПоказателя.Субконто.Свойство(ИмяКолонкиГруппировки) 
									И СписокВидовСубконто.НайтиПоЗначению(ОписаниеПоказателя.Субконто[ИмяКолонкиГруппировки].Вид) = Неопределено Тогда
							СписокВидовСубконто.Добавить(ОписаниеПоказателя.Субконто[ИмяКолонкиГруппировки].Вид);
						КонецЕсли;
						Для каждого ОтборСтроки из Расшифровка.Отбор Цикл
							Если ОписаниеПоказателя.Субконто.Свойство(ОтборСтроки.Ключ) 
										И СписокВидовСубконто.НайтиПоЗначению(ОписаниеПоказателя.Субконто[ОтборСтроки.Ключ].Вид) = Неопределено Тогда
								СписокВидовСубконто.Добавить(ОписаниеПоказателя.Субконто[ОтборСтроки.Ключ].Вид);
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
...
// * СчетПоказательИзмерение - ПланСчетовСсылка.Международный, ПланСчетовСсылка.Хозрасчетный, СправочникСсылка.ЭлементыФинансовыхОтчетов -
// * ВидОтчета - СправочникСсылка.ВидыФинансовыхОтчетов -
...
// * Сортировка - Строка -
// * Точность - Число -
// * ВидСубконто - ПланВидовХарактеристикСсылка.ВидыСубконтоМеждународные, ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные -
// * КодСтрокиОтчета - Строка -
// * Примечание - Строка -
// * ЭтоСвязанный - Булево - 
...
// 	Субконто - Неопределено - Описание
// Возвращаемое значение:
// 	ТаблицаЗначений - Описание:
// * Показатель - СправочникСсылка.ЭлементыФинансовыхОтчетов -
// * КодСтроки - Строка -
...
Функция ПустоеЗначениеПоказателя(ИнтервалыОтчета, Измерения = Неопределено, Субконто = Неопределено)
	...
	Если Субконто <> Неопределено Тогда
		Для Каждого СубконтоПоказателя Из Субконто Цикл
			ТекущееСубконто = СубконтоПоказателя.Значение;
...
Процедура НайтиГруппировкуПоПолю(Группировки, ИмяПоля, НайденнаяГруппировка)
	...
	Для каждого Группировка Из Группировки Цикл
		...
		ИначеЕсли ТипЗнч(Группировка) = Тип("ГруппировкаКомпоновкиДанных") Тогда
...
Процедура ПриКомпоновкеРезультата(СхемаКомпоновкиДанных, КомпоновщикНастроек, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	Если ПараметрВидСубконто <> Неопределено И  ПараметрВидСубконто.Использование Тогда
		...
		Если ТипЗнч(ПараметрВидСубконто.Значение) = Тип("ПланВидовХарактеристикСсылка.ВидыСубконтоМеждународные") Тогда
	...
	ВалютаМУ = МеждународнаяОтчетностьВызовСервера.УчетнаяВалюта();
	УстановитьОбщиеПараметрыСтандартныхОтчетов(КомпоновщикНастроек, ВалютаМУ);
	...
	ГруппаОтбора.Представление = НСтр("ru='Отбор по выбранным полям';uk='Відбір за вибраними полями'");
	...
	УстановитьВалюту(ДокументРезультат, ВалютаМУ.Функциональная, Параметр.Заголовок);
	...
	УстановитьВалюту(ДокументРезультат, ВалютаМУ.Представления, Параметр.Заголовок);
...
Процедура УдалитьЗаголовкиПолей(Документ)
	...
	ОбъединитьЗаголовкиПолей(Документ, НСтр("ru='Показатели';uk='Показники'"));
...
Процедура УстановитьОбщиеПараметрыСтандартныхОтчетов(КомпоновщикНастроек, ВалютаМУ)
	
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВалютаФункциональная", ВалютаМУ.Функциональная);
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВалютаПредставления", ВалютаМУ.Представления);
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СтрокаОбщиеИтоги", НСтр("ru='Общие итоги';uk='Загальні підсумки'"));
...
Процедура ОбновитьДеревоНовыхЭлементов(Форма, Параметры) Экспорт
	...
	//++ Локализация
	ПустойСчетРегл = Новый(РеглУчетКлиентСервер.ТипПланСчетов());
	//-- Локализация
	...
	ФинансоваяОтчетностьСервер.РезультатЗапросаВДерево(МассивРезультатов[КоличествоРезультатов-1], СтрокаНемонетарных);
...
Процедура ОбновитьДеревоСохраненныхЭлементов(Форма, Параметры) Экспорт
	...
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru='Счет плана счетов';uk='Рахунок плану рахунків'")+"]", НСтр("ru='Счет плана счетов';uk='Рахунок плану рахунків'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru='Тип итога';uk='Тип підсумку'")+"]", НСтр("ru='Тип итога';uk='Тип підсумку'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru='Начальное сальдо';uk='Початкове сальдо'")+"]", НСтр("ru='Начальное сальдо';uk='Початкове сальдо'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru='Немонетарный показатель';uk='Немонетарний показник'")+"]", НСтр("ru='Немонетарный показатель';uk='Немонетарний показник'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru='Выводить заголовок элемента';uk='Виводити заголовок елемента'")+"]", НСтр("ru='Выводить заголовок элемента';uk='Виводити заголовок елемента'"));
...
Функция ТекстЗапросаНовыеЭлементыОтчета(БыстрыйПоиск)
	...
	Фрагмент = СтрШаблон(Фрагмент, ТекстОтбора);
	МассивТекстовЗапроса.Добавить(Фрагмент);
	
	//++ Локализация
	ИмяПланСчетовРегл = РеглУчетКлиентСервер.ИмяПланаСчетов();
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет") И ЗначениеЗаполнено(ИмяПланСчетовРегл) Тогда
		Фрагмент = 
		"////////////////////////////////////////////////////////////////////////////////
		...
		ТаблицаПланСчетов = "ПланСчетов." + ИмяПланСчетовРегл;
		ТекстОтбора = 
			?(ЗначениеЗаполнено(БыстрыйПоискДляЗапроса), 
				СтрШаблон("(ЭлементыФинОтчетов.Наименование ПОДОБНО ""%1"" ИЛИ ЭлементыФинОтчетов.Код ПОДОБНО ""%1"")", БыстрыйПоискДляЗапроса),
				"ИСТИНА");
		Фрагмент = СтрШаблон(Фрагмент, ТаблицаПланСчетов, ТекстОтбора);
		МассивТекстовЗапроса.Добавить(Фрагмент);
...
Процедура ДобавитьМонетарныеПоказатели(ДеревоЭлементов, РезультатЗапроса, Счет, НаименованиеДляПечати, ДобавитьСтроку = Ложь)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		СтрокаПоказателей.НестандартнаяКартинка = ?(ТипЗнч(Счет) = МеждународныйУчетКлиентСервер.ТипПланСчетов(),9,39);
...
// 	 *Наименование - Строка - 
// 	 *НаименованиеДляПечати - Строка - 
// 	ВидПоказателей - ПеречислениеСсылка.ВидыПоказателейОтчетности - 
// 	БыстрыйПоиск - Строка -
//
Процедура ДобавитьИзмеренияВДеревоНовыхЭлементов(ДеревоЭлементов, ВидПоказателей, БыстрыйПоиск)
	...
	// Добавление периода
	СтрокаПериод = ДобавитьСтрокуИзмерения(СтрокаИзмерения, НСтр("ru='Период';uk='Період'"));
	...
	ДобавитьСтрокуИзмерения(СтрокаИзмерения, НСтр("ru='Организация';uk='Організація'"));
	
	// Добавление подразделения
	ДобавитьСтрокуИзмерения(СтрокаИзмерения, НСтр("ru='Подразделение';uk='Підрозділ'"));
	
	// Добавление направление деятельности
	Если Справочники.НаправленияДеятельности.ИспользуетсяУчетПоНаправлениям() Тогда
		ДобавитьСтрокуИзмерения(СтрокаИзмерения, НСтр("ru='Направление деятельности';uk='Напрям діяльності'"));
	КонецЕсли;
...
Процедура ЗаполнитьВалютуОтчета(ЭлементРесурс, ВидПоказателей) Экспорт
	...
	Если ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.Международные Тогда
		...
		СписокВыбора.Добавить("Сумма",НСтр("ru='В функциональной валюте';uk='У функціональній валюті'") + СтрШаблон(" (%1)", ВалютаМУ.Функциональная));
		СписокВыбора.Добавить("СуммаПредставления",НСтр("ru='В валюте представления';uk='У валюті подання'") + СтрШаблон(" (%1)", ВалютаМУ.Представления));
	...
	ИначеЕсли ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.Регламентированные Тогда
		...
		СписокВыбора.Добавить("Сумма", НСтр("ru='Бухгалтерский учет';uk='Бухгалтерський облік'") + СтрШаблон(" (%1)", ВалютаРегл));
		Если НастройкиРеглУчета.ДополнительноВедетсяУправленческийУчет Тогда
			СписокВыбора.Добавить("СуммаУУ", НСтр("ru='Управленческий учет';uk='Управлінський облік'") + СтрШаблон(" (%1)", ВалютаУУ));
		...
		Если НастройкиРеглУчета.ДополнительноВедетсяУчетВВалютеФинОтчетности Тогда
			СписокВыбора.Добавить("СуммаФО", 
				НСтр("ru='В валюте финансовой отчетности';uk='У валюті фінансової звітності'") + СтрШаблон(" (%1)", НастройкиРеглУчета.ВалютаФинОтчетности));
			ДоступностьРесурса = Истина;
			ВидимостьРесурса = Истина;
		КонецЕсли;
		ЭлементРесурс.Видимость = ВидимостьРесурса;
	...
	ИначеЕсли ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.МеждународныеИРегламентированные Тогда
		...
		Если ВалютаРегл = ВалютаМУ.Функциональная Тогда
			СписокВыбора.Добавить("Сумма",НСтр("ru='В функц. и регл. валюте';uk='У функц. і регл. валюті'") + СтрШаблон(" (%1)", ВалютаМУ.Функциональная));
		ИначеЕсли ВалютаРегл = ВалютаМУ.Представления Тогда
			СписокВыбора.Добавить("Сумма",НСтр("ru='В представл. и регл. валюте';uk='У представл. і регл. валюті'") + СтрШаблон(" (%1)", ВалютаМУ.Представления));
		Иначе
			СписокВыбора.Добавить("Сумма",НСтр("ru='В функц. и регл. валюте';uk='У функц. і регл. валюті'") + СтрШаблон(" (%1, %2)", ВалютаМУ.Функциональная, ВалютаРегл));
		КонецЕсли;
	...
	Для каждого Вершина  Из Граф.Строки  Цикл
		...
		Если РаскраскаВершин.Получить(Вершина.ЭлементОтчета) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Вершина.СвязанныйЭлемент) Тогда
				ПодчиненныйГраф = ОбновитьДеревоОтчета(Вершина.СвязанныйЭлемент.Владелец, Вершина.СвязанныйЭлемент);
			...
			Если Вершина.ВидЭлемента =  Перечисления.ВидыЭлементовФинансовогоОтчета.ИтогПоГруппе  И Не ЗначениеЗаполнено(Вершина.СвязанныйЭлемент)  Тогда
				...
				ПодчиненныйГраф = ОбновитьДеревоОтчета(Вершина.ЭлементОтчета.Владелец, Вершина.ЭлементОтчета);
				ПроверкаЗацикливанияВидовОтчетов(ПодчиненныйГраф, ТекстНайденногоЦикла, РаскраскаВершин);
			КонецЕсли;
		...
		ИначеЕсли РаскраскаВершин[Вершина.ЭлементОтчета] = 1 Тогда
			...
			ТекстНайденногоЦикла = СтрШаблон(НСтр("ru='Обнаружено зацикливание элементов отчетов: %1';uk='Виявлено зациклення елементів звітів: %1'"), Лев(ТекстНайденногоЦикла, СтрДлина(ТекстНайденногоЦикла)-2));
			Возврат;
	...
	Для каждого СтрокаЭлемента Из СтрокиЭлементов Цикл
		...
		Если СтрокаЭлемента.ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Субконто Тогда
			...
			Если СтрокаЭлемента.ЕстьНастройки И Не ОписаниеТаблицы.ЭтоПроизвольная  Тогда
				НастройкиОтбора = СтрокаЭлемента.ДополнительныйОтбор.Получить();
				...
				Для каждого ПоказательОтчета Из ОписаниеТаблицы.Показатели Цикл
					Если ЭтоКолонки И ПоказательОтчета.Значение.Колонка = СтрокаЭлемента.ЭлементОтчета
						...
						Для Каждого СубконтоПоказателя Из ПоказательОтчета.Значение.Субконто Цикл
							Если СтрНайти(СубконтоПоказателя.Ключ, "Аналитика" + ?(ЭтоКолонки, "Колонок", "Строк") ) <> 0 Тогда
								НомерСубконтоПоказателя = НомерСубконтоПоказателя + 1;
							КонецЕсли;
						КонецЦикла;
						Для каждого ЗначениеСубконто Из Субконто Цикл
							ИмяСубконто = "Аналитика" + ?(ЭтоКолонки, "Колонок", "Строк") + НомерСубконтоПоказателя; 
							Если ЗначениеСубконто.Значение.ЕстьНастройки Тогда
								НастройкиОтбора = ЗначениеСубконто.Значение.Отбор.Получить();
								ЗаменитьИмяСубконтоВОтборе(НастройкиОтбора.Отбор, ИмяСубконто);
								ОтборАналитики = Новый ХранилищеЗначения(НастройкиОтбора);
							Иначе
