...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	//-- НЕ УТ
 	...
 	|					(ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ЭтапПроизводства2_2)
 	...
 	|					(ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ОтчетПереработчика)
    ...
    РасшифровкаКодовОшибок.Вставить(31, НСтр("ru='Не корректно заполнено налоговое назначение для документа ""Сборка (разборка) товаров"".';uk='Не коректно заповнене податкове призначення для документа ""Складання (розбирання) товарів"".'",ОбщегоНазначения.КодОсновногоЯзыка()));
 	...
 	//++ НЕ УТКА
 	РасшифровкаКодовОшибок.Вставить(35, НСтр("ru='Не заполнен реквизит ""Документ движения"" в движениях продукции по фиксированной стоимости.';uk='Не заповнений реквізит ""Документ руху"" в рухах продукції за фіксованою вартістю.'"));
 	//-- НЕ УТКА
 ...
 //++ НЕ УТ
 	...
 	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru='Требуется удалить движения по регистру ""Выпуск продукции"" для поступлений от переработчиков';uk='Потрібно вилучити рухи по регістру ""Випуск продукції"" для надходжень від переробників'",ОбщегоНазначения.КодОсновногоЯзыка()));
 ...
 #Область БалансоваяСтоимостьПоПродукцииСОформлениемОтчетаДавальца
 	...
 	// Исправление движений по регистру Выпуск продукции
 	РасшифровкаКодовОшибок = Новый Соответствие;
 	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru='Продукция давальца, по которой необходимо отчитаться, не должна иметь плановую стоимость.';uk='Продукція давальця, по якій необхідно відзвітувати, не повинна мати планову вартість.'",ОбщегоНазначения.КодОсновногоЯзыка()));
 	...
 	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
 		...
 		Запрос,
 		РасшифровкаКодовОшибок,
 		НСтр("ru='некорректные движения по регистру ""Себестоимость товаров""';uk='некоректні рухи по регістру ""Собівартість товарів""'",ОбщегоНазначения.КодОсновногоЯзыка()));
 ...
 //++ НЕ УТКА
 	...
 	// Исправление движений по регистру Выпуск продукции
 	РасшифровкаКодовОшибок = Новый Соответствие;
 	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru='Движения этапа производства по регистру ""Себестоимость товаров"" не соответствуют регистру ""Товары организаций"". Требуется перепровести документ по регистру ""Себестоимость товаров""';uk='Рухи етапу виробництва у регістру ""Собівартість товарів"" не відповідають регістру ""Товари організацій"". Потрібно перепровести документ по регістру ""Собівартість товарів""'",ОбщегоНазначения.КодОсновногоЯзыка()));
