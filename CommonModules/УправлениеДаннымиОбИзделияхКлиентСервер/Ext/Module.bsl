...
Функция ОтборПоСвойствамКлючСвязиПредопределенный(ВидНастройки) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Для получения ключа отбора по свойствам передан неверный вид настройки.';uk='Для отримання ключа відбору за властивостями переданий невірний вид настройки.'");
...
//++ НЕ УТКА
	Если Строка.Вероятность > 0 Тогда
		...
		Если Строка.Альтернативный Тогда
			Шаблон = НСтр("ru='по требованию (вероятность %1)';uk='на вимогу (ймовірність %1)'");
		Иначе
			Шаблон = НСтр("ru='по умолчанию (вероятность %1)';uk='по умовчанню (ймовірність %1)'");
		КонецЕсли;
		СтрокаВероятность = СтрШаблон(Шаблон, "" + Строка.Вероятность + "%");
		
	ИначеЕсли Строка.Альтернативный Тогда
		
		СтрокаВероятность = НСтр("ru='по требованию';uk='на вимогу'");
