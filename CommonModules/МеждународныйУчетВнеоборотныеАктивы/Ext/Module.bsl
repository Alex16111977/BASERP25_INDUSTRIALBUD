...
// 			*Состояние (необязательно) - Массив, ПеречислениеСсылка.СостоянияНМА, ПеречислениеСсылка.СостоянияОС - Список состояний, которым может принадлежать актив
...
Функция ОшибкиЗаполненияШаблоновПроводокАмортизации(Дата, Организация, СтатьиРасходов) Экспорт
	...
	ШаблонОшибки = НСтр("ru='Не настроен счет по статье расходов ""%1"" в шаблоне проводок для операции ""Амортизация необоротных активов""';uk='Не налаштований рахунок за статтею витрат ""%1"" в шаблоні проводок для операції ""Амортизація необоротних активів""'");
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
...
Процедура Использование_НачислениеАмортизацииОСиНМАМеждународныйУчет(ПараметрыОбработчика) Экспорт
	...
	МеждународныйУчетПроведениеСервер.ПроверитьИспользованиеМеждународногоУчета(ПараметрыОбработчика);
	
	Если ЗакрытиеМесяцаСервер.РасчетЭтапаНеТребуется(ПараметрыОбработчика.ДанныеЭтапа) Тогда
		Возврат;
	КонецЕсли;
	
	МеждународныйУчетПроведениеСервер.ПроверитьНаличиеУчетнойПолитикиМеждународногоУчета(ПараметрыОбработчика);
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода) Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			...
			НСтр("ru='Операция не используется при учете необоротных активов версии 2.5';uk='Операція не використовується при обліку необоротних активів версії 2.5'",ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если РазмерыВременныхТаблиц.ТребуетсяАмортизацияОС = 0 И РазмерыВременныхТаблиц.ТребуетсяАмортизацияНМА = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru='Нет ОС и НМА, по которым требуется начисление амортизации.';uk='Немає ОЗ і НМА, по яких потрібно нарахування амортизації.'",ОбщегоНазначения.КодОсновногоЯзыка()));
		
	Иначе
		
		ЗакрытиеМесяцаСервер.ЗафиксироватьРезультатыОбработчикаИспользованияПоОрганизациям(
			ПараметрыОбработчика,
			НСтр("ru='По организации ""%1"" за период %2 не выполнено начисление амортизации ОС.';uk='По організації ""%1"" за період %2 не виконано нарахування амортизації ОЗ.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			Запрос,
			"НетАмортизацииОС",
			Ложь,
			Истина);
		...
		ЗакрытиеМесяцаСервер.ЗафиксироватьРезультатыОбработчикаИспользованияПоОрганизациям(
			ПараметрыОбработчика,
			НСтр("ru='По организации ""%1"" за период %2 не выполнено начисление амортизации НМА.';uk='По організації ""%1"" за період %2 не виконано нарахування амортизації НМА.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			Запрос,
			"НетАмортизацииНМА",
			Ложь,
			Истина);
...
Процедура Выполнить_НачислениеАмортизацииОСиНМАМеждународныйУчет(ПараметрыОбработчика) Экспорт
	...
	МеждународныйУчетПроведениеСервер.СформироватьДокументРегламентнаяОперацияМУ(
		ПараметрыОбработчика,
...
|%4'
|;uk='Формування документа ""%1"" по організації ""%2"" за період %3 завершилося з помилкою: %4'",ОбщегоНазначения.КодОсновногоЯзыка()),
