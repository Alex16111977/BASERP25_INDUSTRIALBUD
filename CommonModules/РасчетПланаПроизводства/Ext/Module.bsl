...
//++ НЕ УТКА
		...
		|	Движения.*,
		|	&Статус КАК Статус,
		|	ISNULL(ИзмененныеДвижения.Количество * Движения.КоличествоНаЕдиницуПродукции, Движения.Занятость) КАК Количество
		|ИЗ
...
Процедура ДобавитьДвиженияВПланыЗанятостиВидовРабочихЦентров(Движения, ЗначенияЗаполнения, Данные, Таблица, Периоды)
	...
	#КонецОбласти
		...
		Если Строка.КоличествоИзделие > 0
			И НЕ Данные.ЕстьНекратныйВыпуск
...
Функция ОбщиеНастройки(ПроверитьРежимЭксплуатации = Ложь) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось получить параметры расчета плана производства, значения будут приведены к настройкам по умолчанию';uk='Не вдалося отримати параметри розрахунку плану виробництва, значення будуть приведені до настройок по умовчанню'");
