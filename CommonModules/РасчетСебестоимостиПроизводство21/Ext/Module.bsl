...
Функция ТекстТрудозатратыВыпускиПоЗаказам()
	Возврат
		...
		|	ЕСТЬNULL(АналитикаУчетаПартий.КлючАналитики, ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаПартий.ПустаяСсылка)) КАК КорАналитикаУчетаПартий,
		|	НЕОПРЕДЕЛЕНО КАК СтатьяКалькуляцииБезЗаказа,
		|	НЕОПРЕДЕЛЕНО КАК ДокументВыпуска,
		|	0 КАК КодСтроки,
		|	НЕОПРЕДЕЛЕНО КАК ДокументИсточник,
		...
		|		ПО АналитикаУчетаПартий.ГруппаФинансовогоУчета = ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка)
		|		И АналитикаУчетаПартий.Поставщик = ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
		|		И АналитикаУчетаПартий.Контрагент = НЕОПРЕДЕЛЕНО
		|		И АналитикаУчетаПартий.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
        |		И АналитикаУчетаПартий.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
		|		И ВЫБОР
		|			КОГДА Аналитика.Номенклатура.ТипНоменклатуры В
		|		  		(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
		|				ТОГДА АналитикаУчетаПартий.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары)
		...
		|				ТОГДА АналитикаУчетаПартий.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги)
        ...
        |				ТОГДА АналитикаУчетаПартий.ВидЦенности = Статьи.ВидЦенности
		|			ИНАЧЕ АналитикаУчетаПартий.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги)
		|		КОНЕЦ
		|		И АналитикаУчетаПартий.КодСтроки = 0
		|		И МаршрутныйЛист.Организация В (&ОрганизацииСФИФОСкользящая)
		|ГДЕ
		|	ДД.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|	И МаршрутныйЛист.Организация В (&МассивОрганизаций)
		|	И ДД.Активность
		...
		|	ДД.Номенклатура,
		|	ДД.Характеристика,
		|	АналитикаУчетаПартий.КлючАналитики
		|";
КонецФункции
...
Функция ТекстДанныеПоПродукции() // вт ФактЭтапов_ПоПродукции, ПланЭтаповПоЗаказам_ПоПродукции, Данные_ПоПродукции
	...
	ТекстЗапроса =
		...
		|	ДД.Подразделение,
		|	ДД.Распоряжение,
        |	ДД.Распоряжение.НалоговоеНазначение,
		|	ДД.КодСтрокиПродукция,
		|	ДД.Этап,
		|	ДД.Этап.Владелец
		|;
...
Процедура ДополнитьЗапросОстаткамиБазРаспределения(ТекстыЗапросов) Экспорт
	...
	ТекстЗапроса =
        ...
        |		КОГДА СУММА(ДвижениеМатериалов.Количество) = 0
        |			ТОГДА 0
        |		ИНАЧЕ СУММА(ДвижениеМатериалов.НДСРегл)
        |	КОНЕЦ КАК ЧИСЛО(23,10)),
        |	ВЫРАЗИТЬ(ВЫБОР
        |		КОГДА СУММА(ДвижениеМатериалов.Количество) = 0
        |			ТОГДА 0
        ...
        |		КОГДА СУММА(ДвижениеМатериалов.Количество) = 0
        |			ТОГДА 0
        |		ИНАЧЕ СУММА(ДвижениеМатериалов.СтоимостьРеглБезНДС)
        |	КОНЕЦ КАК ЧИСЛО(23,10)),
        |	ВЫРАЗИТЬ(ВЫБОР
        |		КОГДА СУММА(ДвижениеМатериалов.Количество) = 0
        |			ТОГДА 0
        |		ИНАЧЕ СУММА(ДвижениеМатериалов.СтоимостьУпрБезНДС)
        |	КОНЕЦ КАК ЧИСЛО(23,10))
		|ИЗ
		|	(ВЫБРАТЬ
        ...
        |				ТОГДА ДД.Количество
        |			ИНАЧЕ -ДД.Количество
        |		КОНЕЦ * (ЕСТЬNULL(Цены.НДСРегл, 0)) КАК НДСРегл,
        |		ВЫБОР
        |			КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
        |				ТОГДА ДД.Количество
        |			ИНАЧЕ -ДД.Количество
        ...
        |				ТОГДА ДД.Количество
        |			ИНАЧЕ -ДД.Количество
        |		КОНЕЦ * (ЕСТЬNULL(Цены.СтоимостьРеглБезНДС, 0) + ЕСТЬNULL(Цены.ДопРасходыРеглБезНДС, 0) + ЕСТЬNULL(Цены.СтоимостьЗабалансоваяРегл, 0)) КАК СтоимостьРеглБезНДС,
        |		ВЫБОР
        |			КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
        |				ТОГДА ДД.Количество
        |			ИНАЧЕ -ДД.Количество
        |		КОНЕЦ * (ЕСТЬNULL(Цены.СтоимостьУпрБезНДС, 0) + ЕСТЬNULL(Цены.ДопРасходыУпрБезНДС, 0) + ЕСТЬNULL(Цены.СтоимостьЗабалансовая, 0)) КАК СтоимостьУпрБезНДС
		|	ИЗ
