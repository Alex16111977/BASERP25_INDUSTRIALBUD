...
Процедура ОбработчикКоманды(ЭтаФорма, Команда, Результат) Экспорт
	...
	Если ПолноеИмяОтчета = "Отчет.СверкаОборотовМУиБУ" Тогда
		Если Команда.Имя = "НастройкаСоответствияСчетов" Тогда
			НастройкаСоответствияСчетов(Неопределено, Неопределено, ЭтаФорма);
		ИначеЕсли Команда.Имя = "НастройкаСоответствияСчетовИОборотов" Тогда
			ОткрытьНастройкуСоответствияСчетовИОборотов(Неопределено, Неопределено, ЭтаФорма);
		КонецЕсли;
...
//
Процедура ОткрытьНастройкуСоответствияСчетовИОборотов(УчетнаяПолитика, СчетРеглУчета, ФормаВладелец) Экспорт
...
Процедура ОткрытьОтчетАнализСчетаМеждународный(Отчет, Счет)
	...
	ОтчетРасшифровки = Новый Структура("ИмяОтчета, ИмяВарианта", "БухгалтерскийОтчетМеждународный", "АнализСчетаМеждународный");	
	МеждународныйУчетКлиент.ОткрытьРасшифровку(ОтчетРасшифровки, НастройкаАнализаСчета, КлючНастроек, КлючНастроек);
...
// 	ФормаВладелец - ФормаКлиентскогоПриложения - Владелец формы настройки. 
//
Процедура НастройкаСоответствияСчетов(УчетнаяПолитика, СчетРеглУчета, ЭтаФорма)
