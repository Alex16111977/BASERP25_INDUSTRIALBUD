...
Функция ТекстЗапросаСпецификацийНоменклатуры(ПараметрыЗапроса, ПараметрыВыбораСпецификаций) Экспорт
	...
	|	ДоступныеСпецификации.КонецДействия,
...
//
// Параметры:
//  Объект - ДокументОбъект.ЭтапПроизводства2_2, СправочникОбъект.ЭтапПроизводства, Структура - объект, содержащий реквизиты этапа.
...
Процедура ЗаполнитьЕдиницуИзмеренияИДлительностьЭтапаББВ(Объект, ВидыРабочихЦентров, ФормулыРассчитаны) Экспорт
	...
	ИнтервалПланирования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Подразделение, "ИнтервалПланирования");
...
Функция ПрочитатьДеревоСпецификаций(Форма) Экспорт
	...
	Если Форма.ДинамическоеСчитывание Тогда
		...
		Если СтруктураДерева = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Ошибка чтения кешированных значений!';uk='Помилка читання кеш значень!'");
		КонецЕсли;
...
Процедура ЗаполнитьСтрокиДерева(Параметры, Дерево, СтрокаДерева, ИспользованныеСпецификации = Неопределено, КлючСвязи = Неопределено) Экспорт
	...
	ВыводитьЗаголовкиГруппировок = Параметры.ВыводитьЗаголовкиГруппировок И (Параметры.ВыводитьВыходныеИзделия
																			ИЛИ Параметры.ВыводитьМатериалы
																			ИЛИ Параметры.ВыводитьТрудозатраты);
	...
	Если МатериалыИУслуги.Количество() > 0 Тогда
		...
		Для каждого Строка Из МатериалыИУслуги Цикл
			...
			Если Строка.ПроизводитсяВПроцессе И Строка.СпособПолученияМатериала = Перечисления.СпособыПолученияМатериаловВСпецификации.ПроизводитсяНаЭтапе Тогда
				Продолжить;
			КонецЕсли;
...
Процедура НастроитьУсловноеОформлениеДереваСпецификаций(УсловноеОформление) Экспорт
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<указывается в свойстве основного изделия>';uk='<вказується у властивості основного виробу>'"));
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийТекст);
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<подбирается по алгоритму>';uk='<підбирається за алгоритмом>'"));
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийТекст);
...
Функция ПостроитьДеревоОператоров() Экспорт
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Степень';uk='Ступінь'"),           "Pow(,)",  2);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Квадратный корень';uk='Квадратний корінь'"), "Sqrt()",  1);
	
	ГруппаОператоров = РаботаСФормулами.ДобавитьГруппуОператоров(Дерево, НСтр("ru='Тригонометрические функции';uk='Тригонометричні функції'"));
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Sin';uk='Sin'"),  "Sin()", 1);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Cos';uk='Cos'"),  "Cos()", 1);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Tan';uk='Tan'"),  "Tan()", 1);
...
|по причине: %2'
|;uk='не вдалося розрахувати формулу: ""%1"" 
|з причини: %2'"),
...
Функция ПроверитьОчиститьАлгоритмРасчетаКоличества(Объект, ОписаниеИсточников, ПараметрыПроверки, Отказ = Ложь) Экспорт
	...
	Шаблон = НСтр("ru='Алгоритм расчета количества содержит недоступные операнды (список ""%1"", строка %2).';uk='Алгоритм розрахунку кількості містить недоступні операнди (список ""%1"", рядок %2).'");
...
Функция ПолучитьСвойстваДляОтбораПоСвойствам(ВидНоменклатуры, ИсточникиСвойств = "", ТолькоДействующие = Истина) Экспорт
	...
	Если СписокИсточников.Найти("НаправлениеДеятельности") <> Неопределено
		...
		Запрос.УстановитьПараметр("СписокНаборов", Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_НаправленияДеятельности);
...
Процедура УстановитьУсловноеОформлениеАвтовыбораНоменклатуры(Форма, ИмяОбъекта, ИмяТаблицы) Экспорт
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы[ИмяТаблицы+"Номенклатура"].Имя);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы[ИмяТаблицы+"НоменклатураСтрокой"].Имя);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы[ИмяТаблицы+"Характеристика"].Имя);
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы[ИмяТаблицы+"ХарактеристикаСтрокой"].Имя);
...
Процедура УстановитьУсловноеОформлениеОтбораПоСвойствамИРасчетаПоФормулам(Форма, ИмяОбъекта, ИмяТаблицы, ИмяКолонки) Экспорт
	...
	ПолеФормы = Элементы[ИмяТаблицы + ИмяКолонки]; // ПолеФормы
		...
		Если ЕстьАвтовыбор Тогда
			...
			Если ТекущаяСтрока.СпособАвтовыбораНоменклатуры = Перечисления.СпособыАвтовыбораНоменклатуры.ЗадаетсяВСвойствеПродукции Тогда
				...
				ТекущаяСтрока.НоменклатураСтрокой = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("ru='<указывается в свойстве ""%1"">';uk='<вказується у властивості ""%1"">'"),
														ЗаголовокСвойства);
			...
			ИначеЕсли ТекущаяСтрока.СпособАвтовыбораНоменклатуры = Перечисления.СпособыАвтовыбораНоменклатуры.УточняетсяПриПроизводстве Тогда
				...
				ТекущаяСтрока.НоменклатураСтрокой = НСтр("ru='<уточняется при производстве>';uk='<уточнюється при виробництві>'");
			Иначе
			...
			Если ТекущаяСтрока.СпособАвтовыбораХарактеристики = Перечисления.СпособыАвтовыбораХарактеристики.ПодбираетсяПоСвойствамПродукции Тогда
				...
				ТекущаяСтрока.ХарактеристикаСтрокой = НСтр("ru='<определяется по свойствам основного изделия>';uk='<визначається за властивостями основного виробу>'");
			ИначеЕсли ТекущаяСтрока.СпособАвтовыбораХарактеристики = Перечисления.СпособыАвтовыбораХарактеристики.ПодбираетсяПоАлгоритму Тогда
				ТекущаяСтрока.ХарактеристикаСтрокой = НСтр("ru='<определяется по алгоритму>';uk='<визначається за алгоритмом>'");
			ИначеЕсли ТекущаяСтрока.СпособАвтовыбораХарактеристики = Перечисления.СпособыАвтовыбораХарактеристики.УточняетсяПриПроизводстве Тогда
				...
				ТекущаяСтрока.ХарактеристикаСтрокой = НСтр("ru='<уточняется при производстве>';uk='<уточнюється при виробництві>'");
			Иначе
...
Процедура УстановитьПараметрыНазначенияПоПодразделениюДиспетчеру() Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru='Не удалось записать параметры назначения спецификаций по причине: %Причина%';uk='Не вдалося записати параметри призначення специфікацій з причини: %Причина%'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПредставлениеПараметраНазначения(ВидПараметра, Реквизит = Неопределено, ПолноеПредставление = Истина) Экспорт
	...
	ПредставлениеПодразделениеДиспетчер  = НСтр("ru='Подразделение-диспетчер';uk='Підрозділ-диспетчер'");
	ПредставлениеНаправлениеДеятельности = НСтр("ru='Направление деятельности';uk='Напрям діяльності'");
...
//++ НЕ УТКА
	...
	// Номенклатура основного изделия
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ОтборПоНоменклатуре,
		"Ссылка.ОсновноеИзделиеНоменклатура",
		ВидСравненияКомпоновкиДанных.Равно,
		Номенклатура,
		,
		ЗначениеЗаполнено(Номенклатура) И (НЕ ЗначениеЗаполнено(Назначение) ИЛИ Назначение = ПредопределенноеЗначение("Перечисление.ИспользованиеНоменклатурыВНСИПроизводства.Изделие")));

	ОтборПоВидуОсновногоИзделия = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоНоменклатуре.Элементы,
		НСтр("ru='Отбор по виду основного изделия';uk='Відбір за видом основного виробу'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);

	// Вид номенклатуры основного изделия
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ОтборПоВидуОсновногоИзделия,
		"Ссылка.ОсновноеИзделиеНоменклатура",
		ВидСравненияКомпоновкиДанных.Равно,
		ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка"),
		,
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ОтборПоВидуОсновногоИзделия,
		"Ссылка.ОсновноеИзделиеВидНоменклатуры",
		ВидСравненияКомпоновкиДанных.Равно,
		ВидНоменклатуры,
		,
		ЗначениеЗаполнено(Номенклатура) И (НЕ ЗначениеЗаполнено(Назначение) ИЛИ Назначение = ПредопределенноеЗначение("Перечисление.ИспользованиеНоменклатурыВНСИПроизводства.Изделие")));
...
Процедура ОбработатьВыборНоменклатурыИзСвойства(НаборДанных, ПараметрыАвтовыбора)
	...
	|	ВЫРАЗИТЬ(ЗначенияСвойств.Значение КАК Справочник.Номенклатура),
	|
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ЗначенияСвойств.Значение КАК Справочник.Номенклатура).ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)),
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ЗначенияСвойств.Значение КАК Справочник.Номенклатура).ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук), ЛОЖЬ),
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ЗначенияСвойств.Значение КАК Справочник.Номенклатура).ИспользованиеХарактеристик, НЕОПРЕДЕЛЕНО) В (ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры), ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеСДругимВидомНоменклатуры), ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ИндивидуальныеДляНоменклатуры))
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	Свойства.Свойство.Заголовок                КАК СвойствоЗаголовок,
	|
	...
	|	ВЫБОР КОГДА ЗначенияСвойств.ХарактеристикиИспользуются ТОГДА
	|		ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораХарактеристики.УточняетсяПриПроизводстве)
	|	ИНАЧЕ
	...
	|			И ЗначенияСвойств.Номенклатура.ТипНоменклатуры В (&ДопустимыеТипыНоменклатуры)
	|
	|УПОРЯДОЧИТЬ ПО
		...
		Иначе
			...
			Если ПараметрыАвтовыбора.ПолучитьСведенияАвтовыбора Тогда
				СтрокаНабораДанных.СведенияАвтовыбора =
					СтрШаблон(НСтр("ru='Номенклатуру не удалось получить из свойства <%1>.';uk='Номенклатуру не вдалося отримати з властивості <%1>.'"),
						Выборка.СвойствоЗаголовок);
			КонецЕсли; 
	...
	Пока ИндексСтроки < Таблица.Количество() Цикл
		...
		Пока ИндексСтроки < Таблица.Количество() И ТекстыЗапроса.Количество() < 100 Цикл
			...
			Если КоличествоСвойств = 0 Тогда
				...
				ТекстЗапросаПакета = СтрШаблон(ШаблонСопоставления, ИмяПараметраХарактеристикаМатериала);
			...
			Иначе
				...
				ТекстЗапросаПакета = СтрШаблон(ШаблонПакета,
						ИмяПараметраСписокСвойствПакета,
						ИмяПараметраВладелецХарактеристики,
						СтрСоединить(ТекстыОтборовПоСвойствам, Символы.ПС + "ИЛИ "),
						ИмяПараметраКоличествоСвойствПакета);
		...
		Если ПараметрыАвтовыбора.ПолучитьПредставления Тогда
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
				"//Характеристика_Представление",
				"ПРЕДСТАВЛЕНИЕ(Характеристики.Ссылка) КАК ХарактеристикаПредставление");
		КонецЕсли;
		...
		Для Х = 0 По РезультатыЗапроса.Количество() - 1 Цикл
			...
			Если Выборка.Количество() <> 1 Тогда
				Если ПараметрыАвтовыбора.ПолучитьСведенияАвтовыбора Тогда
					СтрокаНабораДанных.СведенияАвтовыбора =
						СтрШаблон(НСтр("ru='Характеристику не удалось подобрать по свойствам: %1';uk='Характеристику не вдалося підібрати за властивостями: %1'"),
...
Процедура ОбработатьВыборХарактеристикиПоАлгоритму(НаборДанных, ПараметрыАвтовыбора)
	...
	ШаблонОшибкиОписания   = НСтр("ru='Не найден алгоритм автовыбора характеристики: %1';uk='Не знайдено алгоритм автовибору характеристики: %1'");
	ШаблонОшибкиВыполнения = НСтр("ru='Не удалось подобрать характеристику по алгоритму: %1';uk='Не вдалося підібрати характеристику за алгоритмом: %1'");
...
Процедура ОбработатьУточнениеНоменклатурыХарактеристикиПриПроизводстве(НаборДанных, ПараметрыАвтовыбора)
	...
	СтрокиНабораДанных = НаборДанных.НайтиСтроки(Новый Структура("СпособАвтовыбораНоменклатуры",
																Перечисления.СпособыАвтовыбораНоменклатуры.УточняетсяПриПроизводстве));
	Для каждого СтрокаНабораДанных Из СтрокиНабораДанных Цикл
		...
		СтрокаНабораДанных.СведенияАвтовыбора = УточнениеМатериалаПриПроизводстве(ЗначенияСвойствИзделия);
		
	КонецЦикла;
		
	СтрокиНабораДанных = НаборДанных.НайтиСтроки(Новый Структура("СпособАвтовыбораХарактеристики,ХарактеристикиИспользуются",
																Перечисления.СпособыАвтовыбораХарактеристики.УточняетсяПриПроизводстве,
																Истина));
	Для каждого СтрокаНабораДанных Из СтрокиНабораДанных Цикл
		...
		СтрокаНабораДанных.СведенияАвтовыбора = СтрокаНабораДанных.СведенияАвтовыбора
													+ ?(ПустаяСтрока(СтрокаНабораДанных.СведенияАвтовыбора),""," ")
													+ УточнениеМатериалаПриПроизводстве(ЗначенияСвойствИзделия);
...
Функция УточнениеМатериалаПриПроизводстве(ЗначенияСвойствИзделия)
	...
	СведенияАвтовыбора = СтрШаблон(НСтр("ru='Требуется уточнить с учетом свойств: %1';uk='Потрібно уточнити з урахуванням властивостей: %1'"),
...
Функция ЗначенияСвойствИзделия(ДанныеПоНоменклатуре) Экспорт
	...
	Результат.Индексы.Добавить("СвойствоЗаголовок");
...
Функция НастройкиСоответствияСвойств(Спецификация) Экспорт
	...
	|	СоответствиеСвойств.СвойствоМатериала.Заголовок КАК СвойствоЗаголовок,
...
Функция ТрансляторФормул(Строка, Параметры, Описание) Экспорт
	...
	Пока СледующийОперанд(Операнд, ФормулаДляРазбора) Цикл
		...
		Если Не ЗаполнитьПутьКДаннымПоОперанду(
			...
			ПутьКДанным.ИмяСвойства = НСтр("ru='Количество';uk='Кількість'");
		КонецЕсли;
...
// 	ПутьКДанным - СтрокаТаблицыЗначений - см. см. УправлениеДаннымиОбИзделиях.ПараметрыДляРасчетаПоФормулам
...
Процедура ДобавитьСоответствиеДляАвтоРасчета(ДанныеДляРасчета, ПутьКДанным, Источник)
	...
	Если (ПутьКДанным.Ключ = "ВыходныеИзделия" 
		...
		И ПутьКДанным.ИмяСвойства = НСтр("ru='Количество';uk='Кількість'") Тогда
...
// 
// Параметры:
// 	ПутьКДанным - СтрокаТаблицыЗначений - см. ПараметрыДляРасчетаПоФормулам
	...
	Для каждого КлючИЗначение Из ОписаниеИсточников Цикл
		...
		Если ОписаниеИсточника.Коллекция Тогда
			...
			Для каждого Строка Из Таблица Цикл
				...
				ПредставлениеСтроки = СтрШаблон(НСтр("ru='Строка[%1].';uk='Рядок [%1].'"), Формат(Строка.НомерСтроки, "ЧН=; ЧГ="));
...
Функция СоздатьТаблицуИсточниковЗначенийПараметровДляРасчетаПоФормулам() Экспорт
	...
	ИсточникиЗначенийПараметров.Колонки.Добавить("СвойствоЗаголовок", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(75)));
...
Процедура ДобавитьЗаписьВТаблицуИсточниковЗначенийПараметров(ИсточникиЗначенийПараметров, Объект, ПутьКДанным) Экспорт
	...
	НоваяСтрока.СвойствоЗаголовок = ПутьКДанным.ИмяСвойства;
...
Процедура ЗаполнитьЗначенияПараметровДляРасчетаПоФормулам(ИсточникиЗначенийПараметров) Экспорт
	...
	КлючевыеПоля = "Объект,СвойствоЗаголовок";
	...
	"ВЫБРАТЬ
	|	Т.Объект            КАК Объект,
	|	Т.СвойствоЗаголовок КАК СвойствоЗаголовок
	...
	|	ИсточникиЗначенийПараметров.СвойствоЗаголовок КАК СвойствоЗаголовок
	|ПОМЕСТИТЬ ИсточникиЗначений
	|ИЗ
	|	ИсточникиЗначенийПараметров КАК ИсточникиЗначенийПараметров
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
	|		ПО ИсточникиЗначенийПараметров.СвойствоЗаголовок = ДополнительныеРеквизитыИСведения.Заголовок
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Объект,
	|	Свойство
	...
	|	ИсточникиЗначений.СвойствоЗаголовок КАК СвойствоЗаголовок,
	...
	|	ИсточникиЗначений.СвойствоЗаголовок,
...
Процедура УстановитьУсловноеОформлениеТаблицыОтбораПоСвойствам(УсловноеОформление, ИмяТаблицы = "ОтборПоСвойствам") Экспорт
	...
	Для каждого КлючИЗначение Из СтруктураУсловий Цикл
		...
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1Условие", ИмяТаблицы));
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1Условие", ИмяТаблицы));
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1Значение", ИмяТаблицы));
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1ГруппаДиапазон", ИмяТаблицы));
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1Значение1", ИмяТаблицы));
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СтрШаблон("%1Значение2", ИмяТаблицы));
...
Функция ТекстЗапросаДоступныеСвойстваИсточников(ТолькоДействующие = Истина)
	...
	Если ТолькоДействующие Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТолькоДействующие", "И НЕ Т.ПометкаУдаления");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТолькоДействующие", "");
	КонецЕсли;
...
Процедура ЗаполнитьДеревоСпецификаций(Параметры, Дерево)
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Если Не ЗначениеЗаполнено(Параметры.Спецификация) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДанныеСпецификации) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Не найдена спецификация для изделия.';uk='Не знайдена специфікація для виробу.'"),
					,
					"ДеревоСпецификаций");
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Спецификация) Тогда
		...
		НоваяСтрокаДерева.ВидСтроки = Перечисления.ВидыСтрокДереваСпецификаций.Номенклатура;
		Если ОсновноеИзделие.ВариантНазначения = Перечисления.ВариантыНазначенияСпецификации.ВидНоменклатуры Тогда
		...
		Иначе
			...
			КонецЕсли;
			НоваяСтрокаДерева.ЕдиницаИзмерения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновноеИзделие.Номенклатура, "ЕдиницаИзмерения");
		КонецЕсли;
