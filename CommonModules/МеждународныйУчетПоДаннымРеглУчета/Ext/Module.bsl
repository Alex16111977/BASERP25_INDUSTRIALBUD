...
Процедура Отразить(ПараметрыОтражения, ТаблицаПроводок) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВестиУУНаПланеСчетовХозрасчетный.Значение
	|				И ДатаНачалаУУНаПланеСчетовХозрасчетный.Значение < ХозрасчетныйДвиженияССубконто.Период
	|			ТОГДА ХозрасчетныйДвиженияССубконто.СуммаУУ
	|		ИНАЧЕ ХозрасчетныйДвиженияССубконто.Сумма
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА ВестиУУНаПланеСчетовХозрасчетный.Значение
	|				И ДатаНачалаУУНаПланеСчетовХозрасчетный.Значение < ХозрасчетныйДвиженияССубконто.Период
	|			ТОГДА &ВалютаФункциональная
	|		ИНАЧЕ &ВалютаРеглУчета
	|	КОНЕЦ КАК ВалютаРеглУчета,
	...
	|	Константа.ВестиУУНаПланеСчетовХозрасчетный КАК ВестиУУНаПланеСчетовХозрасчетный,
	|	Константа.ДатаНачалаУУНаПланеСчетовХозрасчетный КАК ДатаНачалаУУНаПланеСчетовХозрасчетный
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор,
	|	Период,
	...
	|	ПроводкиРеглУчета.ВалютаРеглУчета КАК ВалютаХраненияСуммыФункциональной,
	|	ПроводкиРеглУчета.ВалютаРеглУчета КАК ВалютаХраненияСуммыПредставления,
	...
	Пока ВыборкаДокументыОжидаютОтраженияВРеглУчете.Следующий() Цикл
		...
		МеждународныйУчетПроведениеСервер.ДобавитьОшибкуПроводки(Проводка, НСтр("ru='Для отражения в международном учете документ предварительно необходимо отразить в регл.';uk='Для відображення в міжнародному обліку документ попередньо необхідно відобразити в регл.'"));
	...
	ПроводкиРеглУчета = РезультатыЗапросов[5].Выгрузить(); // ТаблицаЗначений - 
...
// 	                      * ДатаОтражения - Дата - дата, на которую документ формирует проводки.
//
Процедура ДополнитьТаблицыРегистрацииКОтражению(Объект, ДополнительныеСвойства, ТаблицаРегистрации, ВыборочнаяРегистрация, ТаблицаВыборочнойРегистрации) Экспорт
	//++ Локализация
	Если Не ПолучитьФункциональнуюОпцию("ФормироватьПроводкиМеждународногоУчетаПоДаннымРегламентированного") Тогда
		Возврат;
	КонецЕсли;
	...
	ПереданаТаблицаТаблицаОтражениеДокументовВРеглУчете = 
		ДополнительныеСвойства.Свойство("ТаблицаОтражениеДокументовВРеглУчете", ТаблицаОтражениеДокументовВРеглУчете);
	...
	Если ДополнительныеСвойства.Свойство("ВыборочнаяРегистрацияКОтражениюВРеглУчете")
		 И ДополнительныеСвойства.ВыборочнаяРегистрацияКОтражениюВРеглУчете Тогда
...
// 	              * Счет - ПланСчетовСсылка.Хозрасчетный - Счет регл. учета, для которого требуется настройка соответствия.
// 	              * УчетнаяПолитика -  Учетная политика, для которой требуется настройка.
...
Функция ПроверитьПравилаОтражения(МенеджерВременныхТаблиц, Организация) Экспорт
	...
	ТаблицаОшибок.Колонки.Добавить("Комментарий",     Новый ОписаниеТипов("Строка"));
	ТаблицаОшибок.Индексы.Добавить("Счет,             УчетнаяПолитика");
	...
	|		ПО ПроводкиРеглУчета.Регистратор = ДокументыКОтражению.Регистратор
	|
	|СГРУППИРОВАТЬ ПО
	|	ДокументыКОтражению.УчетнаяПолитика,
	|	ПроводкиРеглУчета.Организация,
	...
	|	ОборотыДокументовКОтражениюВУчете.НаправлениеДеятельностиДт,
	...
	|	ОборотыДокументовКОтражениюВУчете.НаправлениеДеятельностиКт,
	...
	ТаблицаОборотов = РезультатЗапроса[6].Выгрузить(); // ТаблицаЗначений - 
	...
	ТаблицаСчетовБезСоответствий = ПрименитьСоответствияСчетов(ТаблицаОборотов, ВыборкаСоответствияСчетов);
	
	Отбор = Новый Структура("Счет, УчетнаяПолитика");
	Для каждого Строка Из ТаблицаСчетовБезСоответствий Цикл
...
//
Процедура ЗаполнитьТаблицуДокументовПоСчету(ТаблицаДокументов, Счет, УчетнаяПолитика, Организация) Экспорт
...
Процедура ПередЗаписьюПравилОтражения(ПравилаОтраженияВМеждународномУчете, Отказ) Экспорт
	...
	ВыборкаПоИгнорируемымСчетам = Запрос.Выполнить().Выбрать();
	
	Пока ВыборкаПоИгнорируемымСчетам.Следующий() Цикл
		ТекстСообщения = 
			СтрШаблон(НСтр("ru='Счет регл. учета, указанный в соответствии ""%1"", игнорируется в учетной политике ""%2"".';uk='Рахунок регл. обліку, зазначений у відповідності ""%1"", ігнорується в обліковій політиці ""%2"".'"),
				ВыборкаПоИгнорируемымСчетам.СоответствиеСчетов, 
				ВыборкаПоИгнорируемымСчетам.УчетнаяПолитика);
...
// Дополняет структуру запросами по регистрам налогового учета.
//
// Параметры:
// 	ТекстыЗапросов - Структура - Структура текстов запросов.
// 	                                 * Ключ - Строка - Имя регистра
// 	                                 * Значение - Строка - Текст запроса данных регистра
//
Процедура ДополнитьТекстыЗапросовПоРегистрам(ТекстыЗапросов) Экспорт
...
#Область СоответствиеСчетовМеждународногоИРегламентированногоУчета
	...
	ОписаниеВарианта.Описание = НСтр("ru='Показывает использование счетов учета в настройках соответствия счетов и оборотов
                                      ...
                                      |;uk='Показує використання рахунків обліку в настройках відповідності рахунків і оборотів 
                                      |міжнародного і регламентованого обліку'");
...
Процедура ТранслироватьПроводкиПоСоответствиямОборотов(ПроводкиРеглУчета, ТаблицаПроводок, СтруктураПроводки)
	...
	|	И НЕ ПроводкиРеглУчета.УчетнаяПолитика ЕСТЬ NULL
	|
	|УПОРЯДОЧИТЬ ПО
...
|по причине'
|;uk='Не вдалося отримати дані для відображення обороту по відповідності ""%ОборотМежд%"" 
|через'") + ":" + Символы.ПС + Символы.Таб + "%Причина%";
				...
				ТекстСообщения = ТекстСообщения + Символы.ПС
					+ НСтр("ru='Возможно в соответствии оборота установлены некорректные настройки дополнительного отбора или заполнения субконто.';uk='Можливо у відповідності обороту встановлені некоректні настройки додаткового відбору або заповнення субконто.'");
...
Процедура ТранслироватьПроводкиПоСоответствиямСчетов(ПроводкиРеглУчета, ТаблицаПроводок, СтруктураПроводки)
	...
	ПрименениеСоответствий.Колонки.Добавить("СчетДтКоличественный");
	...
	ПрименениеСоответствий.Колонки.Добавить("СчетКтКоличественный");
...
|по причине'
|;uk='Не вдалося отримати дані для відображення рахунку по відповідності ""%СоответствиеСчета%""
|через'") + ":" + Символы.ПС + Символы.Таб + "%Причина%";
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СоответствиеСчета%", СоответствиеСчетов.Ссылка);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ТекстСообщения = ТекстСообщения + Символы.ПС
					+ НСтр("ru='Возможно в соответствии счета установлены некорректные настройки дополнительного отбора или заполнения субконто.';uk='Можливо у відповідності рахунку встановлені некоректні настройки додаткового відбору або заповнення субконто.'");
				ВызватьИсключение ТекстСообщения;
			КонецПопытки;
	...
	Если ПроводкиРеглУчета.Количество() Тогда
		...
		ШаблонОшибки = НСтр("ru='Не указан соответствующий счет для %Счет%.';uk='Не вказаний відповідний рахунок для %Счет%.'");
...
#Область ПроверкаПравилОтражения

Процедура ПрименитьСоответствияОборотов(ТаблицаОборотов, ВыборкаСоответствияОборотов)
	
	СхемаКомпоновкиДанных = СхемаКомпоновкиДанныхРеглУчета(ТаблицаОборотов);
	...
	Пока ВыборкаСоответствияОборотов.Следующий() Цикл
		...
		УстановитьОтборПроводокПоСоответствиюОборотов(СхемаКомпоновкиДанных, Настройки, ВыборкаСоответствияОборотов);
...
Функция ПрименитьСоответствияСчетов(ТаблицаОборотов, ВыборкаСоответствияСчетов)
	...
	|	ТаблицаОборотов.НаправлениеДеятельностиДт,
	...
	|	ТаблицаОборотов.НаправлениеДеятельностиКт,
	...
	|	ТаблицаОборотов.НаправлениеДеятельностиДт КАК НаправлениеДеятельности,
	...
	|	ТаблицаСчетов.НаправлениеДеятельности,
	...
	СхемаКомпоновкиДанных = СхемаКомпоновкиДанныхРеглУчета(ТаблицаСчетов);
