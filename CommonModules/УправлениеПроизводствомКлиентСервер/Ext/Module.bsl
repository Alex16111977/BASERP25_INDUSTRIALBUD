...
Функция ПараметрыФормыИсполнениеЗаказа() Экспорт
	...
	ПараметрыФормы.ЗаголовокФормы            = НСтр("ru='Заполнение обеспечения и отгрузки в этапах';uk='Заповнення забезпечення та відвантаження в етапах'");
	ПараметрыФормы.ЗаголовокКомандыЗаполнить = НСтр("ru='Сформировать';uk='Сформувати'");
...
Процедура ПроверитьРазмещениеЭтапаВГрафике(ГрафикПроизводства, Ссылка, ПутьКДанным, Отказ) Экспорт
	...
	Если ГрафикПроизводства.НачалоЭтапа = '00010101'
		...
		ТекстСообщения = НСтр("ru='График производства не заполнен.';uk='Графік виробництва не заповнений.'");
	...
	ИначеЕсли ГрафикПроизводства.НачалоЭтапа > ГрафикПроизводства.ОкончаниеЭтапа Тогда
		...
		ТекстСообщения = НСтр("ru='График производства заполнен не правильно.';uk='Графік виробництва заповнений неправильно.'");
...
//
Функция ПредставлениеЕдиницыИзмеренияОперации(ЕдиницаИзмерения, Количество = 0, Сокращать = Ложь) Экспорт
...
Функция НастройкиШкалыДиаграммыГантаВРежимеВсеДанные(Начало, Окончание, КоличествоИнтервалов) Экспорт
	...
	ИначеЕсли РазницаКварталы <= КоличествоИнтервалов Тогда
		...
		Формат = СтрШаблон(ФорматШаблон, НСтр("ru='кв.';uk='кв.'"));
	...
	Иначе
		...
		Формат = СтрШаблон(ФорматШаблон, НСтр("ru='г.';uk='р.'"));
...
Функция ОписаниеНастроекПланирования(ПолноеПерепланирование, ОтменитьРучныеИзмененияГрафика) Экспорт
	...
	Если ПолноеПерепланирование И ОтменитьРучныеИзмененияГрафика Тогда
		...
		Результат = НСтр("ru='Планировать график всех этапов производства. Ручные изменения графика будут отменены.';uk='Планувати графік всіх етапів виробництва. Ручні зміни графіка будуть скасовані.'");
		
	ИначеЕсли ПолноеПерепланирование И НЕ ОтменитьРучныеИзмененияГрафика Тогда
		
		Результат = НСтр("ru='Планировать график всех этапов производства, исключая размещенные вручную этапы.';uk='Планувати графік всіх етапів виробництва, виключаючи розміщені вручну етапи.'");
		
	ИначеЕсли НЕ ПолноеПерепланирование И ОтменитьРучныеИзмененияГрафика Тогда
		
		Результат = НСтр("ru='Планировать график этапов, требующих пересчета. Ручные изменения графика будут отменены.';uk='Планувати графік етапів, що потребують перерахунку. Ручні зміни графіка будуть скасовані.'");
		
	Иначе
		
		Результат = НСтр("ru='Планировать график этапов, требующих пересчета, исключая размещенные вручную этапы.';uk='Планувати графік етапів, що потребують перерахунку, виключаючи розміщені вручну етапи.'");
