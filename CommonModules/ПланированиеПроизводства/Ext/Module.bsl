	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Свойства.ЭтоНовый
			...
			Или Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
				И (ТипДокумента = Тип("ДокументОбъект.ЗаказПереработчику")
				Или ТипДокумента = Тип("ДокументОбъект.МаршрутныйЛистПроизводства")) Тогда
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияЗаказыМатериаловСУчетомКорректировокИзменение") Тогда
		...
		ШаблонСообщения = НСтр("ru='Корректировка заказа материала ""%1"" превышает количество по заказу на %2 %3';uk='Коригування замовлення матеріалу ""%1"" перевищує кількість за замовленням на %2 %3'");
		//++ НЕ УТКА
		Если ТипЗнч(Документ) = Тип("ДокументОбъект.ЗаказНаПроизводство") Тогда
			ШаблонСообщения = НСтр("ru='По материалу ""%1"" введены корректировки, которые превышают количество по заказу на %2 %3';uk='По матеріалу ""%1"" введені коригування, які перевищують кількість за замовленням на %2 %3'");
		КонецЕсли;
		//-- НЕ УТКА
...
|;uk='Номенклатура %1 
|По рядку %2 оформлено більше, ніж вказано в розпорядженні на оформлення, на %3 %4'");
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияРаспоряженияНаВыпускПродукцииИзменение") Тогда
		...
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиРаспоряженияНаВыпускПродукции Цикл
			...
			Если ТипЗнч(Документ) = Тип("ДокументОбъект.МаршрутныйЛистПроизводства") Тогда
				...
				Если СтрокаОшибки.КонтрольНаДату Тогда
					ТекстСообщения = СтрШаблон(
						НСтр("ru='На позицию %1 уже оформлен выпуск и распоряжение должно быть оформлено не позже месяца в котором оформлен выпуск';uk='На позицію %1 вже оформлений випуск і розпорядження має бути оформлено не пізніше місяця в якому оформлений випуск'"),
			...
			Иначе
				...
				Если СтрокаОшибки.КонтрольНаДату Тогда
					ТекстСообщения = СтрШаблон(
						НСтр("ru='Выпуск позиции %1 должен быть оформлен не раньше месяца завершения выполнения маршрутного листа %2';uk='Випуск позиції %1 повинен бути оформлений не раніше місяця завершення виконання маршрутного листа %2'"),
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТоварыНаСкладахИзменение") Тогда
		...
		Если ТипЗнч(Документ) = Тип("ДокументОбъект.ВыпускПродукции") Тогда
			ШаблонСообщения = НСтр("ru='На изделие %1 уже списаны материалы (%2 %3)';uk='На виріб %1 вже списані матеріали (%2 %3)'");
		ИначеЕсли ТипЗнч(Документ) = Тип("ДокументОбъект.СписаниеЗатратНаВыпуск") Тогда
			ШаблонСообщения = НСтр("ru='На изделие %1 еще не оформлен выпуск продукции (%2 %3)';uk='На виріб %1 ще не оформлений випуск продукції (%2 %3)'");
		КонецЕсли;
...
|;uk='Номенклатура %1 
|По рядку %2 оформлено виконання етапів графіка виробництва в більшій кількості одиниць або партії виробу, ніж заплановано замовленням'");
		...
		Иначе
			...
			ШаблонСообщения = НСтр("ru='Превышено выполнение этапа графика производства на %1 %2';uk='Перевищено виконання етапу графіка виробництва на %1 %2'");
			...
			Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиЭтапыПроизводства Цикл
				...
				ЕдиницаИзмерения = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
					НСтр("ru='единицу или партию изделия';uk='одиницю або партію виробу'"), НСтр("ru='единицы или партии изделия';uk='одиниці або партії виробу'"),
					НСтр("ru='единиц или партий изделий';uk='одиниць або партій виробів'"), Количество);
...
Процедура ПроверитьЗаполнениеПродукция(Объект, Отказ) Экспорт
	...
	АдресОшибки = " " + НСтр("ru='в строке %НомерСтроки% списка ""Продукция""';uk='в рядку%НомерСтроки% списку ""Продукція""'");
	...
	Если Не ТекущиеДанные.ЭтапВосстановленияБрака Тогда
		...
		Если СтрокиВыходныеИзделия.Количество() > 0 Тогда
			...
			Иначе
				...
				Отказ = Истина;
				ОписаниеОшибки = НСтр("ru='Нельзя удалять этап. Передан недопустимый тип данных.';uk='Неможна вилучати етап. Передано неприпустимий тип даних.'");
				Возврат;
