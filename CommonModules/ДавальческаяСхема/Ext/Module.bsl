...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Свойства.ЭтоНовый
			...
			Или ТипДокумента = Тип("ДокументОбъект.ОтчетДавальцу") Тогда
			...
			Если ТипЗнч(Документ) = Тип("ДокументОбъект.ОтчетДавальцу") Тогда
				...
				Если СтрокаОшибки.КонтрольНаДату
					...
					ТекстСообщения = СтрШаблон(
						НСтр("ru='Продукция %1 в количестве %2 %3 еще не произведена. Отчет оформляется после производства продукции.';uk='Продукція %1 в кількості %2 %3 ще не вироблена. Звіт оформляється після виробництва продукції.'"),
						ПредставлениеНоменклатуры, -СтрокаОшибки.КОформлению, СтрокаОшибки.ЕдиницаИзмерения);
				ИначеЕсли Не СтрокаОшибки.КонтрольНаДату
					И ЗначениеЗаполнено(СтрокаОшибки.КОформлению) Тогда
					ТекстСообщения = СтрШаблон(
						НСтр("ru='Продукция %1 в количестве %2 %3 еще не произведена.';uk='Продукція %1 в кількості %2 %3 ще не вироблена.'"),
						ПредставлениеНоменклатуры, -СтрокаОшибки.КОформлению, СтрокаОшибки.ЕдиницаИзмерения);
				КонецЕсли;
			Иначе
				...
				Если СтрокаОшибки.КонтрольНаДату Тогда
					ТекстСообщения = СтрШаблон(
						НСтр("ru='На продукцию %1 уже оформлен отчет давальцу (по отчетам давальцу должно быть выпущено еще %2 %3). Производство должно быть отражено не позже месяца оформления отчета.';uk='На продукцію %1 вже оформлений звіт давальцю (за звітами давальцю має бути випущено ще %2 %3). Виробництво повинно бути відображено не пізніше місяця оформлення звіту.'"),
						ПредставлениеНоменклатуры, -СтрокаОшибки.КОформлению, СтрокаОшибки.ЕдиницаИзмерения);
				Иначе
					ТекстСообщения = СтрШаблон(
						НСтр("ru='На продукцию %1 уже оформлен отчет давальцу (по отчетам давальцу должно быть выпущено еще %2 %3).';uk='На продукцію %1 вже оформлений звіт давальцю (за звітами давальцю має бути випущено ще %2 %3).'"),
						ПредставлениеНоменклатуры, -СтрокаОшибки.КОформлению, СтрокаОшибки.ЕдиницаИзмерения);
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьПодразделениеДоговораНазначения(Объект, ИмяТЧ, Отказ) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		Шаблон = НСтр("ru='В строке %1 списка ""%2"" в договоре назначения ""%3"" не указано подразделение.';uk='У рядку %1 списку ""%2"" в договорі призначення ""%3"" не вказано підрозділ.'");
