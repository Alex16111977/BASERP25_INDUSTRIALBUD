...
Функция Разузловать(Изделия, ПараметрыРазузлования, ПараметрыВыборки) Экспорт
	...
	Если НЕ ОписаниеОшибки.ЕстьОшибка Тогда
		...
		Если ПараметрыРазузлования.ГруппироватьДанные Тогда
			СвернутьПотребности(Потребности, ПараметрыРазузлования);
		КонецЕсли;
		//-- НЕ УТКА
		...
		Для каждого Данные Из СтрокиДляРазузлования Цикл
			...
			Если ОчередьПодбораПФ <> Неопределено
				...
				Если ПараметрыРазузлования.ЗаменятьПФНаАналоги Тогда
					...
					СтрокаПодбор.КлючСвязиСпецификация = Строка.КлючСвязи;
...
Процедура ПодобратьАналогиПолуфабрикатов(СтрокиПотребности, ТаблицаМатериалов, РезервыПФ)
	...
	ТаблицаПодбора.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10)));
	...
	ТаблицаПодбора.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.СтруктураПредприятия"));
	ТаблицаПодбора.Колонки.Добавить("НаправлениеДеятельности", Новый ОписаниеТипов("СправочникСсылка.НаправленияДеятельности"));
	ТаблицаПодбора.Колонки.Добавить("ЗаказНаПроизводство", Новый ОписаниеТипов("ДокументСсылка.ЗаказНаПроизводство2_2"));
	ТаблицаПодбора.Колонки.Добавить("Спецификация", Новый ОписаниеТипов("СправочникСсылка.РесурсныеСпецификации"));
	ТаблицаПодбора.Колонки.Добавить("КлючСвязиСпецификация", Новый ОписаниеТипов("УникальныйИдентификатор"));
	ТаблицаПодбора.Колонки.Добавить("Изделие", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаПодбора.Колонки.Добавить("ХарактеристикаИзделия", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаПодбора.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	
	Для каждого Строка Из ТаблицаМатериалов Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаПодбора.Добавить(), Строка);
	КонецЦикла;
	
	ТаблицаМатериалов.Индексы.Добавить("Идентификатор");
	
	Аналоги = АналогиМатериалов.ПолучитьТаблицуЗаменыДляФормированияЭтапов(ТаблицаПодбора, РезервыПФ);
	...
	Для Индекс = 0 По Аналоги.Количество()-1 Цикл
		...
		ЗаполнитьЗначенияСвойств(
			...
			,
			"Номенклатура,Характеристика,Склад,Количество,Назначение");
				...
				Если ВхПотребность.ЭтоРазборка Тогда
					...
					Для каждого Строка Из ДанныеСпецификации.ВходящиеИзделия Цикл
						...
						Если ЗначениеЗаполнено(ВхПотребность.ЭтапИсточник) Тогда
							...
							СтрокаЗаполнить.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно;
			...
			//++ НЕ УТКА
				Если Обеспечение.ЗаменаНаАналог Тогда
					...
					Для каждого Аналог Из Потребность.Аналоги.НайтиСтроки(СтПоискаАналоги) Цикл
						...
						НоваяСтрока.ВариантОбеспечения = Обеспечение.ВариантОбеспечения;
...
Функция НовыйОчередьПодбораПолуфабрикатов(ЗаменятьПФНаАналоги)
	...
	Если ЗаменятьПФНаАналоги Тогда
		Колонки = "НаправлениеДеятельности,ЗаказНаПроизводство,Спецификация,КлючСвязиСпецификация,Изделие,ХарактеристикаИзделия";
		Для каждого Колонка Из СтрРазделить(Колонки, ",") Цикл
			Если Результат.Колонки.Найти(Колонка) = Неопределено Тогда
...
Процедура ЗаполнитьСрокиОбеспеченияПролеживанияПотребностей(Потребности)
	...
	Для каждого Потребность Из Потребности Цикл
		...
		Для каждого Этап Из Потребность.ДанныеСпецификации.Этапы Цикл
			...
			Этап.ЖелаемаяДатаОбеспечения = ГраницаКНачалу + ЗаписьГрафика.ДлительностьДоЗапуска;
...
Функция НормативныйГрафикПоТаблицеПотребностей(Потребности)
	...
	РегистрыСведений.НормативныйГрафикЭтаповПроизводства.РассчитатьДлительностьДоЗапускаВыпуска(Этапы, Зависимости);
...
Процедура СвернутьПотребности(Потребности, ПараметрыРазузлования)
	...
	Для Сч = 1 По 2 Цикл
		...
		Потребности.Сортировать(?(ТолкающаяСхема,
			"Начало Возр, Идентификатор, Количество",
			"Окончание Убыв, Идентификатор, Количество"));
		...
		Для ИндексПотребность = 0 По Потребности.Количество()-1 Цикл
			...
			Если СводнаяПотребность.Обеспечение.Количество() > 1 Тогда
				СводнаяПотребность.Обеспечение.Свернуть("СтрокаИндекс,ДатаОтгрузки,ВариантОбеспечения,Разрешение,ЗаменаНаАналог", "Количество");
			КонецЕсли;
			...
			Для каждого Строка Из СвернутыеПотребности Цикл
				ПартияВыпускаНов = Справочники.РесурсныеСпецификации.РассчитатьПартиюВыпускаПоНоменклатуре(Строка, Строка.ДанныеСпецификации);
				...
				Для каждого Связь Из ?(Потребность.ЭтоРазборка, Потребность.Выходы, Потребность.Входы) Цикл
					...
					Если Количество > 0 Тогда
						...
						Потомок.ПартияВыпуска = Справочники.РесурсныеСпецификации.РассчитатьПартиюВыпускаПоНоменклатуре(
							Потомок, Потомок.ДанныеСпецификации);
			...
			Для каждого Потребность Из МассивПотребностей Цикл
				...
				Если Не Потребность.ЭтоРазборка Тогда // Сборка и ремонт
					Для каждого Связь Из Потребность.Выходы Цикл
						...
						НоваяСвязь = СводнаяПотребность.Выходы.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСвязь, Связь);
...
Функция ПодобратьПотребностиДляСвертки(Потребности, Потребность, СтруктураПодбора)
	...
	Для каждого СледующаяПотребность Из НайденныеСтроки Цикл
		...
		// Ограничение по дате обеспечения
		Если СледующаяПотребность.ДатаОбеспеченияПФ < Потребность.ДатаОбеспеченияПФ
			И Потребность.ДатаОбеспеченияПФ > СледующаяПотребность.Окончание Тогда
			Продолжить;
		КонецЕсли;
