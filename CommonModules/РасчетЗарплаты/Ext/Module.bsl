...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "РасчетЗарплаты.ИсправитьОпечатку3073";
...
Процедура ИсправитьОпечатку3073() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	Начисления.Ссылка КАК Ссылка,
	               |	Начисления.КраткоеНаименование КАК КраткоеНаименование
	               |ИЗ
	               |	ПланВидовРасчета.Начисления КАК Начисления
				   |ГДЕ
				   |	Начисления.КраткоеНаименование = ""Простій з вини работод.""";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ВР = Выборка.Ссылка.ПолучитьОбъект();
		ВР.КраткоеНаименование = "Простій з вини роботод.";
		ВР.Записать();
	КонецЦикла;	
