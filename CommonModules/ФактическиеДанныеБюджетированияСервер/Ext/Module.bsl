...
Процедура ОтметитьДокументКОтражениюВБюджетированииПриЗаписиНабораЗаписей(Источник, Отказ, РежимЗаписи) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		СобытиеЖР = НСтр("ru='Бюджетирование.Отражение факта';uk='Бюджетування.Відображення факту'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура СформироватьДвиженияПоРегиструФактаБюджетирования(ДокументыКОбработке)
	...
	Для Каждого ОписаниеПравила Из ТаблицаПравил Цикл
		...
		Если ИменаВидовДокументов = Неопределено Тогда
			...
			СобытиеЖР = НСтр("ru='Не выполнено правило бюджетирования';uk='Не виконано правило бюджетування'",КодОсновногоЯзыка);
			ТекстСообщения = НСтр("ru='Не выполнено правило бюджетирования, т.к. не определена СКД получения данных';uk='Не виконано правило бюджетування, тому що не визначена СКД отримання даних'",КодОсновногоЯзыка);
...
Процедура ВывестиАктуальностьОтраженияФактическихДанных(ДокументРезультат, ДопСвойства, ПроверятьАктивныеЗадания = Истина) Экспорт
	...
	Если НужнаНадпись Тогда
		...
		Если ЕстьАктивныеЗадания Тогда
			ЧастиТекста.Добавить(НСтр("ru='Отражения в бюджетировании ожидает %1 документов.';uk='Відображення в бюджетуванні очікує %1 документів.'"));
			ЧастиТекста.Добавить(НСтр("ru='Запущено задание по отражению документов в бюджетировании с %2 по %3.';uk='Запущено завдання відображення документів у бюджетуванні з %2 по %3.'"));
			ЧастиТекста.Добавить(НСтр("ru='После отражения Вам будет предложено переформировать отчет.';uk='Після відображення Вам буде запропоновано переформувати звіт.'"));
		Иначе                                                                                          
			ЧастиТекста.Добавить(НСтр("ru='Данные учета могут быть неактуальны.';uk='Дані обліку можуть бути неактуальними.'"));
			ЧастиТекста.Добавить(НСтр("ru='Отражения в бюджетировании с %2 по %3 ожидает %1 документов.';uk='Відображення у бюджетуванні з %2 по %3 очікує %1 документів.'"));
			ЧастиТекста.Добавить(НСтр("ru='Рекомендуется выполнить отражение документов в бюджетировании обработкой ""Настройка правил получения фактических данных бюджетирования""';uk='Рекомендується виконати відображення документів у бюджетуванні обробкою ""Настройка правил отримання фактичних даних бюджетування""'"));
			ЧастиТекста.Добавить(НСтр("ru='и сформировать отчет повторно.';uk='і сформувати звіт повторно.'"));
		КонецЕсли;
...
Процедура ОтразитьДокументыФоновымЗаданием(НачалоПериода = Неопределено, КонецПериода = Неопределено, ОжидатьВыполнения = Истина) Экспорт
	...
	Если ОжидатьВыполнения Тогда
		...
		Исключение
			СобытиеЖР = НСтр("ru='Выполнение отражения документов в бюджетировании';uk='Виконання відображення документів в бюджетуванні'",ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(СобытиеЖР,
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
