	...
	ИначеЕсли ТипЗнч(МассивСсылок[0]) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		...
		Для Каждого Строка Из Массив Цикл
			...
			Если ЕстьОшибкиСтатус Тогда
				ТекстОшибкиСтатус = НСтр("ru='Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен в статусах ""Начат"", и ""Завершен"".';uk='Документ %Документ% знаходиться в статусі ""%Статус%"". Введення на підставі дозволений в статусах ""Розпочато"", і ""Завершено"".'");
	...
	ИначеЕсли ТипЗнч(МассивСсылок[0]) = Тип("ДокументСсылка.МаршрутныйЛистПроизводства") Тогда
		...
		Для Каждого Строка Из Массив Цикл
			...
			Если ЕстьОшибкиСтатус Тогда
				...
				ТекстОшибкиСтатус = НСтр("ru='Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен в статусах ""К выполнению"", ""Выполняется"" и ""Выполнен"".';uk='Документ %Документ% знаходиться в статусі ""%Статус%"". Введення на підставі дозволений в статусах ""До виконання"", ""Виконується"" і ""Виконано"".'");
	...
	ИначеЕсли ТипЗнч(МассивСсылок[0]) = Тип("ДокументСсылка.ЗаказНаРемонт") Тогда
		...
		Для Каждого Строка Из Массив Цикл
			...
			Если ЕстьОшибкиСтатус Тогда
				...
				ТекстОшибкиСтатус = НСтр("ru='Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен в статусах ""Выполняется"" и ""Закрыт"".';uk='Документ %Документ% знаходиться в статусі ""%Статус%"". Введення на підставі дозволений в статусах ""Виконується"" і ""Закрито"".'");
	...
	ИначеЕсли ТипЗнч(МассивСсылок[0]) = Тип("ДокументСсылка.СменноеЗадание") Тогда
		...
		Для Каждого Строка Из Массив Цикл
			...
			Если ЕстьОшибкиСтатус Тогда
				...
				ТекстОшибкиСтатус = НСтр("ru='Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен в статусе ""Закрыто"".';uk='Документ %Документ% знаходиться в статусі ""%Статус%"". Введення на підставі дозволений в статусі ""Закрито"".'");
