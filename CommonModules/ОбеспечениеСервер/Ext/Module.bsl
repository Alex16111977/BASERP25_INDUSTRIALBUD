...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ТипДокумента = Тип("ДокументСсылка.ЗаказНаПроизводство2_2") Тогда
			...
			ДвиженияПоОбеспечениюЗаказов = Ложь;
		КонецЕсли;
		//-- НЕ УТКА
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Если Не Документ.ДополнительныеСвойства.Свойство("КонтролироватьОбеспечение")
			...
			Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
					...
					Или (ДополнительныеДокументы.Свойство("КонтрольныеРегистрыИзменений")
						И ДополнительныеДокументы.КонтрольныеРегистрыИзменений.Свойство("СвободныеОстатки")
						И ДополнительныеДокументы.КонтрольныеРегистрыИзменений.СвободныеОстатки.Найти(ТипДокумента)<>Неопределено)
					Или ТипДокумента = Тип("ДокументОбъект.ЗаказНаРемонт")
					Или ТипДокумента = Тип("ДокументОбъект.ЗаказДавальца")
					//-- НЕ УТКА
					)
		...
		//++ НЕ УТКА
		// Для инициализации свойств набора записей регистра "Обеспечение заказов",
		// который пишется при записи регистра Движение товаров.
		Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
			И ТипДокумента = Тип("ДокументОбъект.ЗаказНаПроизводство2_2")
			И Не Документ.ДополнительныеСвойства.Свойство("РассчитыватьИзменения") Тогда
			Параметры.КонтрольныеРегистрыИзменений.Добавить(Метаданные.РегистрыНакопления.ДвижениеТоваров);
		КонецЕсли;
		//-- НЕ УТКА
