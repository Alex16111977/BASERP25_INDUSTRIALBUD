...
Функция СформироватьРасписаниеВыполненияМаршрутныхЛистов(
	...
	Если ГрафикРаботыСУчетомРасписания.Количество() = 0 Тогда
		...
		Для каждого СтрокаРасписание Из Расписание Цикл
			...
			ТекстПроблемы = НСтр("ru='Не удалось определить период, т.к. для рабочего центра не указан график работы.';uk='Не вдалося визначити період, тому що для робочого центру не зазначений графік роботи.'");
...
//  Исполнитель - СправочникСсылка.ФизЛица - Исполнитель для которого нужно расчитать загрузку.
