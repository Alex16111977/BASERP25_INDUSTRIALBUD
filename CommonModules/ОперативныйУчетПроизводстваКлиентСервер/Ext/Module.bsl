...
//
Функция РассчитатьВремяОперацииОтКоличества(Количество, КоличествоШтучное, ВремяШтучное, ВремяПЗ, ПараллельнаяЗагрузка) Экспорт
	
	Результат = 0;
	
	Если ПараллельнаяЗагрузка Тогда
		Результат = ВремяШтучное + ВремяПЗ;
	ИначеЕсли КоличествоШтучное <> 0 Тогда
		Результат = ВремяШтучное * Количество / КоличествоШтучное + ВремяПЗ;
	КонецЕсли;
...
//
Функция РассчитатьКоличествоОперацииОтВремени(Время, ВремяШтучное, ВремяПЗ, КоличествоШтучное) Экспорт
	
	Если ВремяШтучное <> 0 Тогда
		Возврат (Время - ВремяПЗ) * КоличествоШтучное / ВремяШтучное;
	Иначе
		Возврат 0;
	КонецЕсли;
