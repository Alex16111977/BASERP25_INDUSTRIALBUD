...
Функция СформироватьДанныеЗаполненияОтчетаДавальцу(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	МИНИМУМ(ЗаказДавальца.НалоговоеНазначение)  КАК НалоговоеНазначение,
		...
		|
		|	ВЫБОР КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказДавальца.НалоговоеНазначение) > 1 ТОГДА
		|		ИСТИНА
		|	ИНАЧЕ
		|		ЛОЖЬ
		|	КОНЕЦ КАК ЕстьОтличияНалоговоеНазначение,
		|
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПодразделение Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Подразделение';uk='Підрозділ'"), Отказ);
	КонецЕсли;
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияНалоговоеНазначение Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Налоговое назначение';uk='Податкове призначення'"), Отказ);
	КонецЕсли;
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер Или
		...
		ВыборкаРеквизитыШапки.ЕстьОтличияНалоговоеНазначение Или
	...
	Иначе
		...
		РеквизитыШапки.Вставить("НалоговоеНазначение",			ВыборкаРеквизитыШапки.НалоговоеНазначение);
