...
#Область ТекстДебиторскаяЗадолженность
	...
	|	ВЫБОР
	|		КОГДА Операция.Договор.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.Депозит)
	...
	|					И Операция.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговораКредитовИДепозитов.ДепозитВБанкеСКапитализацией)))
	|			ТОГДА 	
	|				ВЫБОР 
	|					КОГДА Расчеты.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.ОсновнойДолг)
	|						ТОГДА Операция.Договор.СтатьяДДСПоступленияВыдачи
	|					ИНАЧЕ Операция.Договор.СтатьяДДСПроцентов 
	|				КОНЕЦ
	|		ИНАЧЕ
	|			Операция.Договор
	|	КОНЕЦ КАК СубконтоКт2,
	|	Операция.Договор.БанковскийСчетКонтрагента КАК СубконтоКт3,
	|	
...
#Область ТекстКредиторскаяЗадолженность
	...
	|	Операция.Договор.БанковскийСчетКонтрагента КАК СубконтоДт3,
	|	
