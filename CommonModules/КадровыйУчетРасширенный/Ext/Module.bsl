...
Функция СреднесписочнаяЧисленностьРаботающих(Организация, НачалоПериода, КонецПериода, ИсключатьПериодыПоПравилам1ПВ = Истина) Экспорт 
	...
	СтатистикаПерсонала.СреднесписочнаяЧисленностьРаботающих(Организация, НачалоПериода, КонецПериода, ДанныеЧисленности, ИсключатьПериодыПоПравилам1ПВ);
...
Процедура СоздатьВТДанныеДокументовУвольнение(МенеджерВременныхТаблиц, МассивОбъектов) Экспорт
	...
	Запрос.Текст =
		...
		|   ЕстьNULL(УвольнениеНачисления.Результат,0) КАК НачисленоВыходноеПособие
		|ПОМЕСТИТЬ ВТДанныеДокументов
		|ИЗ
		|	Документ.Увольнение КАК Увольнение
		...
		|		   И Увольнение.ДнейЧасовВыходногоПособия > 0
		|		   И УвольнениеНачисления.Сотрудник  = Увольнение.Сотрудник
		|		   И УвольнениеНачисления.Начисление = Увольнение.ВыходноеПособие
		|ГДЕ
		|	Увольнение.Проведен
		...
		|   ЕстьNULL(УвольнениеНачисления.Результат,0)
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УвольнениеСписком.Начисления КАК УвольнениеНачисления
		|		ПО УвольнениеСпискомСотрудники.Ссылка = УвольнениеНачисления.Ссылка
		|		   И УвольнениеСпискомСотрудники.ДнейЧасовВыходногоПособия > 0
		|		   И УвольнениеНачисления.ИдентификаторСтрокиСотрудника = УвольнениеСпискомСотрудники.ИдентификаторСтрокиСотрудника
		|		   И УвольнениеНачисления.Сотрудник  = УвольнениеСпискомСотрудники.Сотрудник
		|		   И УвольнениеНачисления.Начисление = УвольнениеСпискомСотрудники.ВыходноеПособие
		|ГДЕ
