...
Функция ПрочиеРасходы_АмортизацияРеглСтоимости() Экспорт
	...
	|	
	|	// {А_Доработка} Было: 0 КАК СуммаСНДС — управленческая сумма не заполнялась.
	|	// Т.к. ИспользоватьАмортизациюБухгалтерскогоУчетаВУправленческомУчете = Истина,
	|	// заполняем управленческую сумму = регламентной (СуммаБУ).
	|	ВЫБОР КОГДА &ПогашениеУпрЗатрат ТОГДА втНачисленнаяАмортизация.СуммаБУ ИНАЧЕ 0 КОНЕЦ КАК СуммаСНДС,
	|	ВЫБОР КОГДА &ПогашениеУпрЗатрат ТОГДА втНачисленнаяАмортизация.СуммаБУ ИНАЧЕ 0 КОНЕЦ КАК СуммаБезНДС,
	|	ВЫБОР КОГДА &ПогашениеУпрЗатрат ТОГДА втНачисленнаяАмортизация.СуммаБУ ИНАЧЕ 0 КОНЕЦ КАК СуммаБезНДСУпр,
...
Функция ПодготовитьТаблицуАмортизацияОС(ТаблицаОС, ТаблицаРеквизиты, ТаблицаПараметровРасчетаАмортизации = Неопределено, КоличествоОбработанныхДанных = 0, Отказ = Ложь) Экспорт
	...
	|	СписокОС.ОсновноеСредство КАК АналитикаРасходов, // {А_Доработка} было: ПорядокУчетаОСБУ.АналитикаРасходов (пустое) -> ОбъектЭксплуатации
	...
	|	СписокОСАмортизируемыеВМесяцПринятияКУчету.ОсновноеСредство КАК АналитикаРасходов, // {А_Доработка} было: ПорядокУчетаОСБУ...АналитикаРасходов (пустое) -> ОбъектЭксплуатации
