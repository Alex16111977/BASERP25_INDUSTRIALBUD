...
#Область ВыплатаЗарПлатыПоВедомостям
	...
	|	ПО ВыплатыСотрудникам.ГруппаУчетаНачислений = ГруппыУчетаНачисленийИУдержаний.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаСоотвествияФизлицКонтрагентов КАК ТаблицаСоотвествияФизлицКонтрагентов
	|	ПО ВыплатыСотрудникам.ФизическоеЛицо = ТаблицаСоотвествияФизлицКонтрагентов.ФизическоеЛицо
    |
	|ГДЕ
...
#Область ВыплатыСотрудникам
	...
	|;
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВыплатыСотрудникам.ФизическоеЛицо КАК ФизическоеЛицо,
	...
	|	 И ВыплатыСотрудникам.ФизическоеЛицо.КодПоДРФО = Контрагенты.КодПоЕДРПОУ
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваДоходыРасходы(Запрос, ТекстыЗапроса, Регистры)
	...
	// ▼▼▼ Локализация-1: РКО с нашей ВедомостьНаВыплатуЗарплатыВКассу
	// Детальная разбивка из ТЧ.А_РасшифровкаВыплатыЗарплатаПоФизлицам:
	//   Подразделение = Рш.Подразделение.А_Подразделение (реальный СтрПредпр)
	//   НаправлениеДеятельностиДС = Рш.НаправлениеДеятельности (из БДДС Казны)
	//   СтатьяДвиженияДенежныхСредств = Рш.СтатьяДвиженияДенежныхСредств (детальная)
	// Гард: ТИПЗНАЧЕНИЯ Ведомость = ВКассу + ХозОп зарплатная.
	// Типовые ветки ниже получают взаимоисключающий гард — НЕ ВКассу.
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ДанныеДокумента.Дата                                                                             КАК Период,
	...
	|	И &ИспользоватьНачислениеЗарплаты
	|	И (&ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику)
	|		ИЛИ Рш.ФизическоеЛицо = ДанныеДокумента.ПодотчетноеЛицо)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// ▼▼▼ Локализация-2 (типовая ветка 1): ВыплатаЗарплатыРаботнику без нашей ВКассу
	|ВЫБРАТЬ
	|	ДанныеДокумента.Дата                                                                             КАК Период,
	|	&ХозяйственнаяОперация                                                                           КАК ХозяйственнаяОперация,
	|	ДанныеДокумента.Организация                                                                      КАК Организация,
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// ▼▼▼ Локализация-2 (типовая ветка 2): ВыплатаЗарплатыЧерезКассу / Раздатчиком без нашей ВКассу
	|ВЫБРАТЬ
