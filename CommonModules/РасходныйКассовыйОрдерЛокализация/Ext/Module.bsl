...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаздатчиком))
	|	И &ИспользоватьНачислениеЗарплаты
	|	// А_кастом 2026-05-28 (Подход C): для РКО с нашей кастомной ВКассу — типовой UNION блок ОТ
	|	// из ТЧ ВыплатаЗаработнойПлаты ОТКЛЮЧАЕТСЯ, потому что наш ManagerModule РКО уже формирует
	|	// детальные движения per СтрПредпр через Ведомость.А_РасшифровкаВыплатыЗарплатаПоФизлицам.
	|	// Без этого гарда платформа писала бы лишнюю агрегированную строку Подр=∅ Σ=СуммаДокумента.
	|	И ТИПЗНАЧЕНИЯ(Строки.Ссылка.Ведомость) <> ТИП(Документ.ВедомостьНаВыплатуЗарплатыВКассу)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДанныеВедомостей.ВедомостьДепонирована,
	|	ТаблицаДанныеВедомостей.ФизическоеЛицо
