...
Процедура ЗарегистрироватьКОтражениюВМеждународномУчете(ПараметрыРасчета)
	...
	Запрос.Текст = "
		...
		|
		|//ВТДокументыКОтражениюЛокализация
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор
		|;
	...
	Если Не ПустаяСтрока(ТекстЗапросаВТДокументыКОтражениюЛокализация) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ВТДокументыКОтражениюЛокализация", ТекстЗапросаВТДокументыКОтражениюЛокализация);
	КонецЕсли;

	РезультатЗапроса = ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина, "ДокументыКОтражению", Истина);
...
Процедура ВосстановитьДвиженияДокументовПоРегиструНакопления(ПараметрыРасчета,
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Если ЭтоПараметрыРасчетаПУ22
			...
			ДопПараметры.ДополнительныеСвойства.Вставить("ПериодПерезаписиДвижений", ПараметрыРасчета.РасчетныйПериод.НачалоПериода);
...
Функция ТекстЗапросаЗавершенныеПартии() Экспорт
	...
	Возврат "ВЫБРАТЬ
		...
		|	ВЫБРАТЬ
		|		ПартииПроизводства.Партия	КАК Партия,
		|		Изделия.ДатаПроизводства	КАК Дата
		|	ИЗ
		|		ПартииПроизводства КАК ПартииПроизводства
		|	
		...
		|	ВЫБРАТЬ
		|		ПартииПроизводства.Партия		КАК Партия,
		|		ДАТАВРЕМЯ(3999,12,31,23,59,59)	КАК Дата
		|	ИЗ
		|		ПартииПроизводства КАК ПартииПроизводства
		|	
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК РеквизитыДокумента
...
|Перевірка по умовчанню відключена, тому що ще перебуває в режимі тестування.'");
