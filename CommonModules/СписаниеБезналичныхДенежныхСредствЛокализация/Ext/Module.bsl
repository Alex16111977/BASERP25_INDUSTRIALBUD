...
#Область ТекстОплатаАвансаПоставщику // (Дт 371 :: Кт 31)
	...
	|	Операция.Подразделение КАК ПодразделениеКт,//***ERP
...
#Область ТекстСнятиеНаличныхВБанке
	...
	|	Операция.Подразделение КАК ПодразделениеДт,//***ERP
...
#Область ТекстВыплатаЗарплатыНаЛицевыеСчета // (Дт 661 :: Кт 31)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
    |	ЕСТЬNULL(ВыплатыСотрудникам.КВыплате, ЕСТЬNULL(Ведомости.Сумма, Операция.СуммаДокумента)) КАК Сумма,
	|	ЕСТЬNULL(ВыплатыСотрудникам.КВыплате / КурсВалютыУпрУчета.Курс , ЕСТЬNULL(Строки.Сумма, Операция.СуммаДокумента) / КурсВалютыУпрУчета.Курс) КАК СуммаУУ,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
