...
Процедура ПолучитьДанныеДляПрочихРасходовНЗП(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.РежимОтладки Тогда
		...
		//++ НЕ УТКА
		ПолучателиПостатейныхРасходов22Материальные                 = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ПолучателиПостатейныхРасходов22Материальные");
		//-- НЕ УТКА
		ВТСтоимостьПартийТоваров                                    = ОбщегоНазначенияУТ.ПоказатьВременнуюТаблицу(ПараметрыРасчета.МенеджерВременныхТаблиц, "ВТСтоимостьПартийТоваров");
...
Функция ТекстПартииДляОтнесенияРасходов()
	...
	ТекстЗапроса =
        ...
        |		ИЛИ ЕСТЬNULL(ДД.СтоимостьРеглБезНДСОстаток,0) <> 0
        |		ИЛИ ЕСТЬNULL(ДД.СтоимостьУпрБезНДСОстаток,0) <> 0
        |		ИЛИ ЕСТЬNULL(ДД.НДСРеглОстаток,0) <> 0
...
Функция ТекстОстаткиБазРаспределения()
	...
	ТекстЗапроса =
        ...
        |	ВЫРАЗИТЬ(СУММА(ДД.Количество * (ЕСТЬNULL(Цены.НДСРегл, 0))) КАК ЧИСЛО(23, 10)) КАК ОстатокНДСРегл,
        ...
        |	,ВЫРАЗИТЬ(СУММА(ДД.Количество * (ЕСТЬNULL(Цены.СтоимостьРеглБезНДС, 0) + ЕСТЬNULL(Цены.ДопРасходыРеглБезНДС, 0) + ЕСТЬNULL(Цены.СтоимостьЗабалансоваяРегл, 0))) КАК ЧИСЛО(23, 10)) КАК ОстатокРеглБезНДС,
        |	ВЫРАЗИТЬ(СУММА(ДД.Количество * (ЕСТЬNULL(Цены.СтоимостьУпрБезНДС, 0) + ЕСТЬNULL(Цены.ДопРасходыУпрБезНДС, 0) + ЕСТЬNULL(Цены.СтоимостьЗабалансовая, 0))) КАК ЧИСЛО(23, 10)) КАК ОстатокУпрБезНДС
		...
		|	СУММА(Остатки.ПлановоеКоличествоПродукции),
        |	СУММА(Остатки.ПлановоеКоличествоПродукции),
        |	СУММА(Остатки.ПлановоеКоличествоПродукции) КАК ОстатокНДСРегл,
        |	СУММА(Остатки.ПлановоеКоличествоПродукции)
        |	,СУММА(Остатки.ПлановоеКоличествоПродукции),
        |	СУММА(Остатки.ПлановоеКоличествоПродукции)
		|ИЗ
...
Функция ТекстКоэффициентыПриведения()
	...
	ТекстЗапроса =
        ...
        |		ОстаткиБазРаспределения.ОстатокНДСРегл,
        |		ОстаткиБазРаспределения.ОстатокУпр
        |		,ОстаткиБазРаспределения.ОстатокРеглБезНДС,
        |		ОстаткиБазРаспределения.ОстатокУпрБезНДС
		|	ИЗ
