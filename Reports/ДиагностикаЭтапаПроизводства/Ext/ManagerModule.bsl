...
Функция ДобавитьКомандуДиагностикаГрафикаЭтапаПроизводства(КомандыОтчетов) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.ДиагностикаЭтапаПроизводства)
		...
		КомандаОтчет.Представление      = НСтр("ru='Диагностика графика';uk='Діагностика графіка'");
...
Процедура ВывестиСмежныеЭтапы(ТабличныйДокумент, ДанныеЭтапа, МакетОформления)
	...
	Если ДанныеЭтапа.РазмещениеКНачалу Тогда
		ОбластьЗаголовок.Область("R1C1").Текст = НСтр("ru='Предшественники';uk='Попередники'");
	Иначе
		ОбластьЗаголовок.Область("R1C1").Текст = НСтр("ru='Последователи';uk='Послідовники'");
	КонецЕсли;
...
Процедура ВывестиЗаголовокИсполнитель(ТабличныйДокумент, Исполнитель, Исполнители, МакетОформления, РасшифровкиАльтернатив)
	...
	Если ВыводитьИспользование Тогда
		...
		Если НЕ Исполнитель.Альтернативный Тогда
			ОбластьИспользование.Области.Исполнитель_Использование.Текст = НСтр("ru='показать альтернативы';uk='показати альтернативи'");
		...
		Иначе
			ОбластьИспользование.Области.ИсполнительАльтернативный_Использование.Текст = НСтр("ru='использовать';uk='використовувати'");
