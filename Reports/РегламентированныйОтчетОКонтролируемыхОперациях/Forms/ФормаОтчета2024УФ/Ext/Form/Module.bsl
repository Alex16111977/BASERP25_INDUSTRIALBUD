...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.Вставить("ВерсияОтчета", "07");
	...
	СтруктураРеквизитовФормы.Вставить("СписокКодовИсточника", КонтролируемыеОперацииПовтИсп.ПолучитьСписокВыбораКодыИсточника());	
...
Процедура КнопкаПередатьДанныеВ1СЗвитЗавершение(Результат, ДопПараметры) Экспорт
	...
	Для  каждого Контрагент Из ДанныеПоКонтрагентам Цикл 
		...
		ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB1", "TAB1_A41", Контрагент.КодСтраны);
	...
	Для  каждого Контрагенты Из ДанныеПоКонтрагентам Цикл
		...
		// учитываем, что порции должны быть по 20000 строк
		...
		Для каждого СтрокиПриложений Из СтрокиПриложенийМассив Цикл
			...
			Если НомерСтроки = 1 Тогда
				...
				ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "PNUM", НомерПриложения);
				...
				// коды операций
				//ДА, НЕТ Обязательно для заполнения для 010 
				ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N13", 1); 
				
				Если СтрокиПриложений.КодКонтролируемойОперации1 = "015"
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации2 = "015" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации3 = "015" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации4 = "015" Тогда
					                                        
					ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N3", 1);

				КонецЕсли;
				Если СтрокиПриложений.КодКонтролируемойОперации1 = "020"
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации2 = "020" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации3 = "020" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации4 = "020" Тогда
					                                        
					ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N4", 1);

				КонецЕсли;
				Если СтрокиПриложений.КодКонтролируемойОперации1 = "030"
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации2 = "030" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации3 = "030" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации4 = "030" Тогда
					                                        
					ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N5", 1);

				КонецЕсли;
				Если СтрокиПриложений.КодКонтролируемойОперации1 = "040"
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации2 = "040" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации3 = "040" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации4 = "040" Тогда
					                                        
					ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N6", 1);

				КонецЕсли;
				Если СтрокиПриложений.КодКонтролируемойОперации1 = "050"
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации2 = "050" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации3 = "050" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации4 = "050" Тогда
					                                        
					ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N12", 1);

				КонецЕсли;
				Если СтрокиПриложений.КодКонтролируемойОперации1 = "010"
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации2 = "010" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации3 = "010" 
					ИЛИ СтрокиПриложений.КодКонтролируемойОперации4 = "010" Тогда
					
					//ДА, НЕТ
					Если    ЗначениеЗаполнено(СтрокиПриложений.КодСвязанностиЛиц1)
						ИЛИ ЗначениеЗаполнено(СтрокиПриложений.КодСвязанностиЛиц2) 
						ИЛИ ЗначениеЗаполнено(СтрокиПриложений.КодСвязанностиЛиц3) 
						ИЛИ ЗначениеЗаполнено(СтрокиПриложений.КодСвязанностиЛиц4) 
						ИЛИ ЗначениеЗаполнено(СтрокиПриложений.КодСвязанностиЛиц5) Тогда
						ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N14", 1);
						// надо очистить N13
						ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N13", 0, Истина);
					КонецЕсли;
				КонецЕсли;
				...
				ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "", "N11", СтрокиПриложений.КодСвязанностиЛиц5);
			...
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A111", СтрокиПриложений.МестоПоставкиПоИнкотермс);
			...
			Если СтрокиПриложений.КодПредмета = "208" Тогда 
				//фінансування нерезидентом поточної діяльності його постійного представництва в Україні 
				СумаВал = ?(СтрокиПриложений.Количество = 0, 0, СтрокиПриложений.ЦенаВал * СтрокиПриложений.Количество);
				ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A16", 0);
				ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A17", СумаВал);
			Иначе	
			...
			КонецЕсли;
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A171", СтрокиПриложений.КоличествоСогласноКонтракта);
			...
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A191", СтрокиПриложений.КурсВалюты);
			...
			Если ЗначениеЗаполнено(СтрокиПриложений.КодИсточника) Тогда
				НаименованиеИсточника = СтруктураРеквизитовФормы.СписокКодовИсточника.НайтиПоЗначению(СтрокиПриложений.КодИсточника).Представление;
				НаименованиеИсточника = СокрЛП(СтрЗаменить(НаименованиеИсточника, СтрокиПриложений.КодИсточника, ""));
			КонецЕсли;
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A26", СтрокиПриложений.КодИсточника);
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение,  "TAB1", "TAB1_A27", НаименованиеИсточника);
			...
			Если НомерСтроки = 20000 Тогда
				...
				ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение, "", "PORNUM",   НомерПорции);
		...
		Если ПриложениеЗаписано = Ложь Тогда
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение, "", "PORNUM", НомерПорции, Истина);
			ДобавитьПоказательВОтчетЗвит1С(ОтчетПриложение, "", "A1_20", R01G20);
			глКомпонентаЗвит1С.SaveReportEx(ОтчетПриложение, 0);
			Попытка
				Результат = глКомпонентаЗвит1С.SetParentDoc(ИдОтчетПриложение, ИдГоловнойОтчет);
...
Процедура ЗаполнитьОтчетИнформация(ОтчетИнформация, Контрагенты, НомерПриложения)
	...
	// получим форму регистра, в ней будут развернутые данные по связанности
	...
	ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "PNUM",     НомерПриложения);
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "501"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "501" 
		ИЛИ Контрагенты.КодСвязанностиЛиц1 = "511" 
		ИЛИ Контрагенты.КодСвязанностиЛиц2 = "511" 
		ИЛИ Контрагенты.КодСвязанностиЛиц3 = "511" 
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "511" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "511" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A1_1", ПреобразоватьВ01(ДанныеСвязанности.Код501_2019КорпПраваПлательщик));
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A1_2", ПреобразоватьВ01(СтруктураДанных.Код501_2019КорпПраваКонрагент));
		
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A1_3", СтруктураДанных.Код501_2019ПроцентКорпПрава);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A1_4", СтруктураДанных.Код501_2019КорпПраваНепосредственно);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A1_5", СтруктураДанных.Код501_2019КорпПраваОпосредовано);
		
		Для каждого СтрокаТаблицы Из СтруктураДанных.Код501_2019Контрагенты Цикл
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "502"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "502" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "502" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_1", ЗначениеРеквизитаОбъекта(СтруктураДанных.Код502_2019Контрагент, "НаименованиеПолное"));
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_2", ЗначениеРеквизитаОбъекта(СтруктураДанных.Код502_2019Страна, "НаименованиеПолное"));
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_3", СтруктураДанных.Код502_2019Код);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_4", СтруктураДанных.Код502_2019ПроцентКорпПраваОрганизации);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_5", СтруктураДанных.Код502_2019КорпПраваНепосредственноОрганизации);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_6", СтруктураДанных.Код502_2019КорпПраваОпосредованоОрганизации);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_7", СтруктураДанных.Код502_2019ПроцентКорпПраваКонтрагента);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_8", СтруктураДанных.Код502_2019КорпПраваНепосредственноКонтрагента);
		ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, , "A2_9", СтруктураДанных.Код502_2019КорпПраваОпосредованоКонтрагента);
		
		Для каждого СтрокаТаблицы Из СтруктураДанных.Код502_2019Контрагенты Цикл
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "503"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "503" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "503" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код503_2019Контрагенты Цикл
			ДобавитьСтрокуВОтчетЗвит1С(ОтчетИнформация, "TAB3");
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB3", "TAB3_A1", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Контрагент, "Наименование"));
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "504"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "504" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "504" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код504_2019Контрагенты Цикл
			ДобавитьСтрокуВОтчетЗвит1С(ОтчетИнформация, "TAB4");
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB4", "TAB4_A1", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Контрагент, "Наименование"));
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "505"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "505" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "505" Тогда
		                                        
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код505_2019ФизЛица Цикл
			...
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB5", "TAB5_A2", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.СтранаРегистрации, "НаименованиеПолное"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB5", "TAB5_A3", СтрокаТаблицы.Код);
		КонецЦикла;
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "506"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "506" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "506" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код506_2019Контрагенты Цикл
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "507"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "507" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "507" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код507_2019Контрагенты Цикл
	...
	Если   Контрагенты.КодСвязанностиЛиц1 = "508"
		...
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "508" Тогда
		                                        
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код508_2019ФизЛица Цикл
			ДобавитьСтрокуВОтчетЗвит1С(ОтчетИнформация, "TAB9");
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB9", "TAB9_A1", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.ФизЛицо,"Наименование"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB9", "TAB9_A2", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.СтранаРегистрации, "НаименованиеПолное"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB9", "TAB9_A3", СтрокаТаблицы.Код);
		КонецЦикла;
		
	КонецЕсли;
	
	Если   Контрагенты.КодСвязанностиЛиц1 = "509"
		ИЛИ Контрагенты.КодСвязанностиЛиц2 = "509" 
		ИЛИ Контрагенты.КодСвязанностиЛиц3 = "509" 
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "509" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "509" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код509_2019Контрагенты Цикл
			ДобавитьСтрокуВОтчетЗвит1С(ОтчетИнформация, "TAB10");
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB10", "TAB10_A1", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Контрагент, "НаименованиеПолное"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB10", "TAB10_A2", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.СтранаРегистрации, "НаименованиеПолное"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB10", "TAB10_A3", СтрокаТаблицы.Код);
		КонецЦикла;
		
	КонецЕсли;
	
	Если   Контрагенты.КодСвязанностиЛиц1 = "510"
		ИЛИ Контрагенты.КодСвязанностиЛиц2 = "510" 
		ИЛИ Контрагенты.КодСвязанностиЛиц3 = "510" 
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "510" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "510" 
		ИЛИ Контрагенты.КодСвязанностиЛиц1 = "516" 
		ИЛИ Контрагенты.КодСвязанностиЛиц2 = "516" 
		ИЛИ Контрагенты.КодСвязанностиЛиц3 = "516" 
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "516" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "516" Тогда

		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код510_2019Контрагенты Цикл
			...
			//ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB8", "TAB8_A", СтрокаТаблицы.Код);
		КонецЦикла;
	КонецЕсли;
	
	Если   Контрагенты.КодСвязанностиЛиц1 = "523"
		ИЛИ Контрагенты.КодСвязанностиЛиц2 = "523" 
		ИЛИ Контрагенты.КодСвязанностиЛиц3 = "523" 
		ИЛИ Контрагенты.КодСвязанностиЛиц4 = "523" 
		ИЛИ Контрагенты.КодСвязанностиЛиц5 = "523" Тогда
		
		СтруктураДанных = ДанныеСвязанности;
		
		Для каждого СтрокаТаблицы Из ДанныеСвязанности.Код523_2021ФизЛица Цикл
			ДобавитьСтрокуВОтчетЗвит1С(ОтчетИнформация, "TAB11");
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB11", "TAB11_A1", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.ФизЛицо, "Наименование"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB11", "TAB11_A2", СтрокаТаблицы.Признак);
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB11", "TAB11_A3", ЗначениеРеквизитаОбъекта(СтрокаТаблицы.СтранаРегистрации, "НаименованиеПолное"));
			ДобавитьПоказательВОтчетЗвит1С(ОтчетИнформация, "TAB11", "TAB11_A4", СтрокаТаблицы.Код);
		КонецЦикла;
...
Функция ПроверитьКорректностьДанных(СтруктураРеквизитовФормы)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	Сведения.Код501_2019 КАК Код501_2019
				   |ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли НЕ ТипЗнч(Выборка.Код501_2019.Получить()) = Тип("Структура") Тогда
			Если СообщениеОбОшибкеВыдавалось2 = Ложь Тогда
				СообщениеОбОшибкеВыдавалось2 = Истина;
				Сообщить(НСтр("ru='Список контрагентов, по которым нет данных по классификации 2018 года Связанность лиц контролируемых операций';uk=""Список контрагентів, щодо яких немає даних по класифікації 2018 року Пов'язаність осіб контрольованих операцій """));
			КонецЕсли;
			Сообщить("   " + Выборка.Контрагент);
		КонецЕсли;
...
Функция ПолучитьДанныеОперацийКонтрагентаНаСервере(СтруктураРеквизитовФормы)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	СвязанностьЛицКонтролируемыхОпераций.Страна.Код КАК КодСтраны,
...
Функция ПолучитьСтрокиПриложений(СтруктураРеквизитовФормы, Контрагент)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	КонтролируемыеОперации.МестоПоставкиПоИнкотермс,
	               ...
	               |	КонтролируемыеОперации.КоличествоСогласноКонтракта,
				   ...
				   |	СвязанностьЛицКонтролируемыхОпераций.КодСвязанностиЛиц5 КАК КодСвязанностиЛиц5,
