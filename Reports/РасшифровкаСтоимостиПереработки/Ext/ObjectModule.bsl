...
Функция СтоимостьПереработкиДавальческогоСырья()
	...
	Пока Выборка.Следующий() Цикл
		...
		ТЧМатериалы.Колонки.Добавить("ЗаказДавальца",Новый ОписаниеТипов("ДокументСсылка.ЗаказДавальца"));
		ТЧМатериалы.ЗаполнитьЗначения(Выборка.ЗаказДавальца, "ЗаказДавальца");
		
		ТЧМатериалы.Колонки.Добавить("Продукция",Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		ТЧМатериалы.ЗаполнитьЗначения(Выборка.Номенклатура, "Продукция");
		
		ТЧМатериалы.Колонки.Добавить("ХарактеристикаПродукции",Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
		ТЧМатериалы.ЗаполнитьЗначения(Выборка.Характеристика, "ХарактеристикаПродукции");
		
		ТЧМатериалы.Колонки.Добавить("ВидЦены",Новый ОписаниеТипов("СправочникСсылка.ВидыЦен"));
		ТЧМатериалы.ЗаполнитьЗначения(Выборка.ВидЦены, "ВидЦены");
		
		ТЧМатериалы.Колонки.Добавить("Валюта",Новый ОписаниеТипов("СправочникСсылка.Валюты"));
		ТЧМатериалы.ЗаполнитьЗначения(Выборка.Валюта, "Валюта");
		
		ТЧМатериалы.ЗаполнитьЗначения(Выборка.Спецификация, "Спецификация");
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТЧМатериалы, ТаблицаЗатрат);
	
	КонецЦикла;
	
	ТаблицаЗатрат.Свернуть("ЗаказДавальца, Спецификация, Продукция, ХарактеристикаПродукции, Номенклатура, Характеристика, ВидЦены, Валюта", "Количество");
	...
	|	ВТТаблицаМатериалов.Валюта КАК ВалютаДокумента
	|ИЗ
