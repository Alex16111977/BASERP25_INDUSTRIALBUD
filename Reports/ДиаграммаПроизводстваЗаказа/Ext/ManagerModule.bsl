...
Функция РежимыОтображения() Экспорт
	...
	Результат.Добавить(
		...
		НСтр("ru='По подразделениям';uk='По підрозділах'"));
		
	Результат.Добавить(
		РежимПоВидамРабочихЦентров(),
		НСтр("ru='По видам рабочих центров';uk='За видами робочих центрів'"));
		
	Результат.Добавить(
		РежимПоПартиямЗапуска(),
		НСтр("ru='По партиям запуска';uk='По партіях запуску'"));
...
Функция ДобавитьКомандуГрафикПроизводстваЗаказа(КомандыОтчетов) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.ДиаграммаПроизводстваЗаказа)
		...
		КомандаОтчет.Представление               = НСтр("ru='График производства';uk='Графік виробництва'");
