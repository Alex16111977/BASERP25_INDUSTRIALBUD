...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Иначе
		Отказ = Истина;
		ВызватьИсключение НСтр("ru='Отчет предназначен только для открытия в документе ""Начальные остатки НЗП по подразделениям"".';uk='Звіт призначений тільки для відкриття в документі ""Початкові залишки НЗВ по підрозділах"".'") ;
	КонецЕсли;
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если Область <> Неопределено Тогда
		...
		ЗаголовокОтчета = НСтр("ru='Ошибки распределения остатков НЗП в подразделении ""%1""';uk='Помилки розподілу залишків НЗВ в підрозділі ""%1""'");
		ЗаголовокОтчета =
