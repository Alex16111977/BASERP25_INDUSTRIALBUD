...
Процедура ПередЗагрузкойВариантаНаСервере(ЭтаФорма, НовыеНастройкиКД) Экспорт
	...
	// Установка значений по умолчанию (валюта/период) — применяется к компоновщику.
	// Сам ВАРИАНТ (НовыеНастройкиКД) НЕ трогаем: иначе любой сохранённый вариант
	// затирается текущим состоянием компоновщика и откатывается к предопределённому
	// при загрузке. Обязательные настройки повторно применяются в ПриКомпоновкеРезультата.
