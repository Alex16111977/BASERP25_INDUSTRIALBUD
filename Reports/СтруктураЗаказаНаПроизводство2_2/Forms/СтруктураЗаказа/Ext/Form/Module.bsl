...
Процедура ДиаграммаГанта(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не выбрана строка структуры заказа';uk='Не вибраний рядок структури замовлення'"),
			,
			"СтруктураЗаказов");
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли ТекущиеДанные.ТипСтроки = ТипСтрокиПолуфабрикат()
		...
		Иначе
			...
			Если ПартииПроизводства.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ru='Формирование отчета для выбранной строки недоступно';uk='Формування звіту для вибраного рядка недоступно'");
			Иначе
				ТекстСообщения = НСтр("ru='Производство запланировано несколькими партиями, для формирования отчета необходимо выбрать этап производства';uk='Виробництво заплановано кількома партіями, для формування звіту необхідно вибрати етап виробництва'");
			КонецЕсли;
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Формирование отчета для выбранной строки недоступно';uk='Формування звіту для вибраного рядка недоступно'"));
...
Процедура ДиагностикаЭтапа(Команда)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для формирования отчета необходимо выбрать этап производства';uk='Для формування звіту необхідно вибрати етап виробництва'"));
...
Процедура ИзменитьЦепочкуЭтапов(Команда)
	...
	ИначеЕсли ТекущиеДанные.ТипСтроки = ТипСтрокиПродукция()
		...
		Иначе
			...
			ПартииПроизводства = ПартииПроизводстваИзделия(ТекущиеДанные.Источник);
			Если ПартииПроизводства.Количество() = 0 Тогда
				ТекстСообщения = НСтр("ru='Для текущей строки невозможно выполнить выбранное действие';uk='Для поточного рядка неможливо виконати вибрану дію'");
			Иначе
				ТекстСообщения = НСтр("ru='Производство запланировано несколькими партиями, для изменения необходимо выбрать этап производства';uk='Виробництво заплановано кількома партіями, для зміни необхідно вибрати етап виробництва'");
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для текущей строки невозможно выполнить выбранное действие';uk='Для поточного рядка неможливо виконати вибрану дію'"));
...
Процедура ПометитьНаУдалениеЦепочкуЭтапов(Команда)
	...
	ИначеЕсли ТекущиеДанные.ТипСтроки = ТипСтрокиЗаказ() Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ru='Пометить на удаление все этапы заказа %1?';uk='Помітити на вилучення всі етапи замовлення %1?'"), Представление);
	...
	ИначеЕсли ТекущиеДанные.ТипСтроки = ТипСтрокиПродукция()
		...
		Если ЗначениеЗаполнено(ПартияПроизводства) Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения,
				НСтр("ru='Пометить на удаление цепочки этапов?';uk='Помітити на вилучення ланцюжки етапів?'"),
...
Процедура ПометитьНаУдалениеЦепочкуЭтаповЗавершение(РезультатВопроса, ПараметрыПроцедуры) Экспорт
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru='Пометка на удаление этапов производства';uk='Помітка на вилучення етапів виробництва'"),
...
Процедура ИзменитьПартию(Команда)
	...
	Если ПартияПроизводства <> Неопределено Тогда
		...
		Если ЗначениеРеквизитаОбъекта(ПартияПроизводства, "ТипПроцесса") 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru='Для заказов с типом производственного процесса ""Без спецификаций"" изменение партии недоступно';uk='Для замовлень з типом виробничого процесу ""Без специфікацій"" зміна партії недоступна'"));
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьДанныеНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru='Формирование отчета ""Структура заказа на производство""';uk='Формування звіту ""Структура замовлення на виробництво""'");
...
Процедура ОпределитьПараметрыИСформироватьЭтапыЗавершение(Результат, Распоряжения) Экспорт
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru='Формирование этапов производства';uk='Формування етапів виробництва'"),
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Партия производства';uk='Партія виробництва'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Создать этапы';uk='Створити етапи'"));
	...
	Если ЗначениеЗаполнено(Заказы) Тогда
		...
		Если Заказы.Количество() = 1 Тогда
			...
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Структура заказа на производство № %1 от %2';uk='Структура замовлення на виробництво № %1 від %2'"),
				НомерЗаказа,
				ДатаЗаказа);
			
		Иначе
			
			Заголовок = НСтр("ru='Структура заказов на производство';uk='Структура замовлень на виробництво'");
			
		КонецЕсли;
		
	Иначе
		
		Заголовок = НСтр("ru='Структура заказа на производство (не выбран заказ)';uk='Структура замовлення на виробництво (не вибрано замовлення)'");
...
Процедура ОбновитьИнформационнуюПанель()
	...
	Если НЕ СкрытьКомандуТребуетсяСформироватьЭтапы
		И Заказы.Количество() <> 0 Тогда
