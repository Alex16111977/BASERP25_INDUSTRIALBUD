...
Процедура ДобавитьНоменклатуру(Данные, Документ, Родитель)
	...
	Для каждого Изделие Из Изделия Цикл
		...
		ЗачестьОбеспечениеВыполняемоеРодителем(Изделие, Обеспечение, Родитель);
		Если Изделие.КоличествоПлан = 0
				ИЛИ Изделие.КоличествоПлан = Изделие.КоличествоОтменено Тогда
	...
	Если ЗаказБезСпецификации Тогда
		...
		Если Строки.ВГраница() <> -1 Тогда
			...
			НоваяСтрока.ПредставлениеСтроки = НСтр("ru='<Прочее>';uk='<Інше>'");
...
Функция ОбеспечениеПотребности(Данные, Потребность)
	...
	Для Индекс = 0 По Связи.ВГраница() Цикл
		...
		СтруктураПоиска.Заказ      = Связь[ПолеПоиска + "Заказ"];
...
Процедура ЗачестьОбеспечениеВыполняемоеРодителем(Потребность, Обеспечение, Родитель)
	
	Если Родитель.Уровень() < 4 Тогда
		Возврат;
	КонецЕсли;
	
	ПартияПроизводстваРодителя = Родитель.Родитель.Родитель.ПартияПроизводства;
	...
	Для каждого Строка Из Обеспечение Цикл
		...
		Если Потребность.КоличествоПлан = 0
			ИЛИ Потребность.КоличествоПлан = Потребность.КоличествоОтменено Тогда
			Прервать;
		КонецЕсли;
...
Функция ТекстЗапросаДанныеСтруктурыЗаказов()
	...
	ТекстЗапроса =
		...
		|		И НЕ ВТОбеспечение.Отменено) КАК Обеспечение
		|
		|СГРУППИРОВАТЬ ПО
		|	Обеспечение.Этап,
