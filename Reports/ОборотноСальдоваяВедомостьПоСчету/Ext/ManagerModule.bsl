...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	ТекстЗаголовкаОтчета = НСтр("ru='Оборотно-сальдовая ведомость по счету %1 %2';uk='Оборотно-сальдова відомість по рахунку %1 %2'");
...
Функция ТекстЗапросаДляСчетовБезСубконто()
	...
	|    ХозрасчетныйОстаткиИОбороты.НалоговоеНазначение КАК НалоговоеНазначение,
	...
	|    ПРЕДСТАВЛЕНИЕССЫЛКИ(ХозрасчетныйОстаткиИОбороты.НалоговоеНазначение) КАК НалоговоеНазначениеПредставление,
	...
	|ИЗ
	|    РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Месяц {(&Периодичность)}, , Счет В (&СчетаБезСубконто) {(Счет).*}, ,{(Организация).*, (Субконто1).*, (Субконто2).*, (Субконто3).*, (Подразделение).*, (НаправлениеДеятельности).*, (НалоговоеНазначение).*, (Валюта).*}) КАК ХозрасчетныйОстаткиИОбороты
	|{ГДЕ
...
Функция ШаблонЗапроса(Счетчик)
	...
	Если Счетчик = 1 Тогда
		Возврат "ВЫБРАТЬ
			...
			|    &ВидыСубконто, 
			|    {(Организация).*, (Подразделение).*, (НаправлениеДеятельности).*, (НалоговоеНазначение).*, (Валюта).* &УсловиеПоСубконто}) КАК ХозрасчетныйОстаткиИОбороты
			|{ГДЕ
			|	ХозрасчетныйОстаткиИОбороты.СуммаНачальныйОстатокДт КАК БУНачальныйОстатокДт,
			|	ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокДт КАК БУКонечныйОстатокДт,
			|	ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокКт КАК БУКонечныйОстатокКт,
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
	...
	Правило.ШаблонПредставления = НСтр("ru='Карточка счета [Счет]';uk='Картка рахунку [Счет]'");
