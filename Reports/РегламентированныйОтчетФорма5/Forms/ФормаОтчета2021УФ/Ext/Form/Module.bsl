...
Процедура ФормироватьСтруктуруСвойствСтраниц() Экспорт
	...
	СтрокаУровня1.ИмяПредставления                     = "";
	СтрокаУровня1.ПредставлениеДанных                  = Истина;
	СтрокаУровня1.РазделОбязателенДляВыгрузки          = Ложь;
	СтрокаУровня1.РазделЯвляетсяАвтозаполняемым        = Ложь;
...
Процедура ОбработатьТаблицуРезультатов(ТаблицаРезультатов, ФлагОС)
	...
	Если ТаблицаРезультатов.Количество() <> 0 Тогда
		Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
			...
			Если ФлагОС = 0 Тогда
				Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчетаКонецПериода) 
					И СтрокаТаблицы.СчетУчета <> СтрокаТаблицы.СчетУчетаКонецПериода Тогда  
					// изменение параметров
					СчетОбъекта =  СтрокаТаблицы.СчетУчетаКонецПериода;
				КонецЕсли;	
			КонецЕсли;	
			//износ на конец года
...
Процедура ДозаполнитьНаКлиенте(ПараметрыЗаполненияЗвит1С)
	...
	Если СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Истина Тогда
		...
		Попытка
			...
			Если (Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2021 и Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 9) Тогда
				ИмяСхемы = "S0100213";
...
//функция возвращает таблицу с данными для ОС для заполнения примечаний к финансовой отчетности (формы № 5)
	...
	|	СчетаБухгалтерскогоУчетаКон.СчетУчета                                         КАК СчетУчетаКонецПериода
	|
	|ИЗ
	...
	|		                    ,
	|		                    Организация = &Организация
	|		                    И ОсновноеСредство В (&СписокОС)) КАК СчетаБухгалтерскогоУчетаКон
	|		ПО ПервоначальныеСведения.ОсновноеСредство = СчетаБухгалтерскогоУчетаКон.ОсновноеСредство";
...
Функция СформироватьЗапросДляОСERP25(НачалоПериода,КонецПериода,Организация) Экспорт
	...
	|		                    ,
	|		                    ОсновноеСредство В (&СписокОС)) КАК СчетаБухгалтерскогоУчетаКон
	|		ПО ПервоначальныеСведения.ОсновноеСредство = СчетаБухгалтерскогоУчетаКон.ОсновноеСредство";
