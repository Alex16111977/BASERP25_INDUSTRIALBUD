...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат НСтр("ru='Ведомость по ОС';uk='Відомість по ОЗ'") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода);
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Для ИндексСтроки = 2 По КоличествоСтрок - 1 Цикл
		...
		Для ИндексКолонки = КоличествоКолонок - 7 По КоличествоКолонок - 1 Цикл
