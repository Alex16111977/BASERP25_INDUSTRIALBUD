...
// При изменении элемента подключаемый.
// Параметры:
//	Элемент - ПолеФормы - изменяемое поле.
&НаКлиенте
...
Процедура УстановитьВидимостьЗаголовки()
	...
	Если СтруктураПараметров.ЕдиницаИзмерения = 3 Тогда
		ШаблонЗаголовка = НСтр("ru='%1 (тысячи %2)';uk=' %1 (тисячі %2)'");
	ИначеЕсли СтруктураПараметров.ЕдиницаИзмерения = 6 Тогда
		ШаблонЗаголовка = НСтр("ru='%1 (миллионы %2)';uk=' %1 (мільйони %2)'");
	Иначе
		
		СтруктураПараметров.ЕдиницаИзмерения = 0;
		ШаблонЗаголовка = НСтр("ru='%1 (%2)';uk='%1 (%2)'");
		
	КонецЕсли;
	
	Элементы.ПолноеДеревоСебестоимостиСумма.Заголовок			= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Сумма';uk='Сума'"), Валюта);
	Элементы.ПолноеДеревоСебестоимостиМатериальные.Заголовок	= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Материальные';uk='Матеріальні'"), Валюта);
	Элементы.ПолноеДеревоСебестоимостиТрудозатраты.Заголовок	= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Трудозатраты';uk='Трудовитрати'"), Валюта);;
	Элементы.ПолноеДеревоСебестоимостиДопРасходы.Заголовок		= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Доп. расходы';uk='Дод. витрати'"), Валюта);;
	Элементы.ПолноеДеревоСебестоимостиСуммаЗабалансовая.Заголовок		= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Забалансовая сумма';uk='Позабалансова сума'"), Валюта);;
	Элементы.ПолноеДеревоСебестоимостиПостатейныеПостоянные.Заголовок	= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Постатейные (постоянные)';uk='Постатейні (постійні)'"), Валюта);;
	Элементы.ПолноеДеревоСебестоимостиПостатейныеПеременные.Заголовок	= СтрШаблон(ШаблонЗаголовка, НСтр("ru='Постатейные (переменные)';uk='Постатейні (змінні)'"), Валюта);;
...
Функция ФоновоеЗаданиеЗапустить()
	...
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Построение отчета: %1';uk='Побудова звіту: %1'"),
		ИмяОтчета);
