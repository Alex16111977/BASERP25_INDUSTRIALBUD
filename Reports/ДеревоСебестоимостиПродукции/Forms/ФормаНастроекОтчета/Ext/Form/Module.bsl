...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПустыеНастройки = СтруктураСебестоимости.ИнициализироватьНастройкиОтчета(
		Отчет, КлючТекущегоВарианта, Неопределено);
	...
	КлючСохраненияПоложенияОкна = ПустыеНастройки.ПолноеИмя;
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ВариантНаименование = ПустыеНастройки.Наименование;
		КонецЕсли;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Настройки отчета ""%1""';uk='Настройки звіту ""%1""'"), ВариантНаименование);
	КонецЕсли;	
	
	Если ПустыеНастройки.СхемаМодифицирована Тогда
		Отчет.КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(ПустыеНастройки.АдресСхемы));
	КонецЕсли;
...
// Устанавливает период в компоновщике настроек при изменении сгенерированного поля период.
// Параметры:
//	Элемент - ПолеФормы - поле формы из которого берется значение.
&НаКлиенте
Процедура Подключаемый_Период_ПриИзменении(Элемент)
	ОтчетыКлиент.УстановитьПериод(ЭтотОбъект, Элемент.Имя);
КонецПроцедуры

// Переносит значение в компоновщик настроек при изменении сгенерированного поля.
// Параметры:
//	Элемент - ПолеФормы - поле формы из которого берется значение.
&НаКлиенте
...
Процедура Отборы_Сгруппировать(Команда)
	...
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(
		Отчет.КомпоновщикНастроек, "Отбор", ИдентификаторЭлементаСтруктурыНастроек); // ОтборКомпоновкиДанных
	
	// Обработка элементов настроек.
	ЭлементНастройкиИсточник = СвойствоЭлементаСтруктуры;
	СтрокаРодителя = Отборы.НайтиПоИдентификатору(ПараметрыГруппировки.Родитель);
	Если ТипЗнч(СтрокаРодителя.Идентификатор) = Тип("ИдентификаторКомпоновкиДанных") Тогда 
		ЭлементНастройкиИсточник = СвойствоЭлементаСтруктуры.ПолучитьОбъектПоИдентификатору(СтрокаРодителя.Идентификатор);
	КонецЕсли;
	...
	// Обработка строк.
	СтрокаПриемник = СтрокаРодителя.ПолучитьЭлементы().Вставить(ПараметрыГруппировки.Индекс);
	...
	НаследникиСтрок.Вставить(СтрокаРодителя, СтрокаПриемник.ПолучитьИдентификатор());
...
Процедура Отборы_Разгруппировать(Команда)
	...
	НаследникиСтрок = Новый Соответствие;
	НаследникиСтрок.Вставить(РодительСтрокиИсточника, РодительСтрокиПриемника.ПолучитьИдентификатор());
	
	ИдентификаторыСтрок = Новый Массив;
	Для Каждого Элемент Из РодительСтрокиИсточника.ПолучитьЭлементы() Цикл
		ИдентификаторыСтрок.Добавить(Элемент.ПолучитьИдентификатор());
	КонецЦикла;
	
	ИзменитьГруппировкуОтборов(СвойствоЭлементаСтруктуры, ИдентификаторыСтрок, НаследникиЭлементовНастроек, НаследникиСтрок);
...
Процедура ОтборыПередУдалением(Элемент, Отказ)
	...
	ИдентификаторыСтрок = Отборы.ВыделенныеСтроки;
	
	Индекс = ИдентификаторыСтрок.ВГраница();
	Пока Индекс >= 0 Цикл 
		Строка = Отборы.НайтиПоИдентификатору(ИдентификаторыСтрок[Индекс]);
		Индекс = Индекс - 1;
		...
		Строки = ПолучитьРодителя("Отборы", Строка).ПолучитьЭлементы();
...
// Инициализация параметров для группировки выделенных строк в отборе.
// Возвращаемое значение:
//	Структура - содержит описание группировки.
//		* Строки - Массив из Число - идентификаторы выделенных строк в массиве.
//		* Родитель - Число - идентификатор родителя выделенных строк.
//		* Индекс - Число - индекс первого группировочного элемента.
&НаКлиенте
Функция ПараметрыГруппировкиОтборов()
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл 
		...
		Строки.Добавить(ИдентификаторСтроки);
	...
	Родитель = Родители[0].ПолучитьИдентификатор();
	СтрокаРодителя = Родители[0]; // ДанныеФормыЭлементДерева
	Индекс = СтрокаРодителя.ПолучитьЭлементы().Индекс(Отборы.НайтиПоИдентификатору(Строки[0]));
...
Процедура ИзменитьГруппировкуОтборов(ОтборыУзлаНастроек, Строки, НаследникиЭлементовНастроек, НаследникиСтрок)
	Для Каждого ИдентификаторСтроки Из Строки Цикл 
		...
		СтрокаИсточник = Отборы.НайтиПоИдентификатору(ИдентификаторСтроки);
		...
		КонецЕсли;
		РодительЭлементаНастройкиПриемника = НаследникиЭлементовНастроек.Получить(РодительЭлементаНастройкиИсточника); // ОтборКомпоновкиДанных
		
		РодительСтрокиИсточника = СтрокаИсточник.ПолучитьРодителя();
		РодительСтрокиПриемника = Отборы.НайтиПоИдентификатору(НаследникиСтрок.Получить(РодительСтрокиИсточника)); // ДанныеФормыЭлементДерева
		...
		НаследникиЭлементовНастроек.Вставить(ЭлементНастройкиИсточник, ЭлементНастройкиПриемник);
		НаследникиСтрок.Вставить(СтрокаИсточник, СтрокаПриемник.ПолучитьИдентификатор());
...
Процедура УдалитьБазовыеЭлементыГруппировкиОтборов(ОтборыУзлаНастроек, ПараметрыГруппировки)
	...
	СтрокаРодителя = Отборы.НайтиПоИдентификатору(ПараметрыГруппировки.Родитель);
	Строки = СтрокаРодителя.ПолучитьЭлементы();
	
	ЭлементыНастроек = ОтборыУзлаНастроек.Элементы;
	СтрокаРодителя = Отборы.НайтиПоИдентификатору(ПараметрыГруппировки.Родитель);
	Если ТипЗнч(СтрокаРодителя.Идентификатор) = Тип("ИдентификаторКомпоновкиДанных") Тогда 
		ЭлементыНастроек = ОтборыУзлаНастроек.ПолучитьОбъектПоИдентификатору(СтрокаРодителя.Идентификатор).Элементы;
	КонецЕсли;
	
	Индекс = ПараметрыГруппировки.Строки.ВГраница();
	Пока Индекс >= 0 Цикл 
		Строка = Отборы.НайтиПоИдентификатору(ПараметрыГруппировки.Строки[Индекс]);
...
Процедура СдвинутьОтборы(ВНачало = Истина)
	...
	Для Каждого Строка Из ПараметрыСдвига.Строки Цикл 
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка); // ЭлементОтбораКомпоновкиДанных
...
Процедура РежимОтображенияПослеВыбора(РежимОтображения, Контекст) Экспорт
	...
	Строка = Отборы.НайтиПоИдентификатору(Контекст.ИдентификаторСтроки);
	Если Строка = Неопределено Тогда
		Возврат;
	КонецЕсли;
