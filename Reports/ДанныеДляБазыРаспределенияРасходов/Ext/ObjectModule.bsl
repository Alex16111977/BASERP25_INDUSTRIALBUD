...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Для базы распределения документа расшифровка не предусмотрена.';uk='Для бази розподілу документа розшифровка не передбачена.'");
	КонецЕсли;
...
Процедура УдалитьГруппировкуИзВсехНастроекОтчета(КомпоновщикНастроек, ИмяЭлемента) Экспорт
	...
	Для Каждого КлючИЗначение Из СоответствиеЭлементов Цикл
		...
		Иначе
            Группировка.Выбор.Элементы.Удалить(КлючИЗначение.Значение);
		КонецЕсли;
