...
Перем R721GA; 
&НаСервере
Перем R722GA, R722GB, R723GA, R723GB; 
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если    Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Декларация"
		...
		ИЛИ Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Приложение5" Тогда
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	////2024
	//// перенесем в строку 15.1 Декларации
	//ЗаполнитьПоказательНаСервере("R151GA", R0220G4, Форма, "Декларация");
	//ЗаполнитьПоказательНаСервере("R151GB", R0220G8, Форма, "Декларация");
	...
	////////////////////////////////////////////////
	//Расчет для Приложения 4 (Справки к декларации)
	...
	R90GB4 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R11GB4", Форма, "Декларация"))
											...
											//2024
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R232GB4", Форма, "Декларация"))
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R233GB4", Форма, "Декларация"))
											...
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R71GB4", Форма, "Декларация"))
											//2024
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R722GB4", Форма, "Декларация"))
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R723GB4", Форма, "Декларация"))
	...
	R170GB = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R101GB", Форма, "Декларация"))
											...
											//2024
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R151GB", Форма, "Декларация"))
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R152GB", Форма, "Декларация"))
	...
	R170GB4 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R101GB4", Форма, "Декларация"))
											...
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R151GB4", Форма, "Декларация"))
											//2024
											+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R152GB4", Форма, "Декларация"))
	...
	////////////////////////////////////////////////
	// Расчет Приложения 2  (продолжение)
	//2024
...
Процедура ЗаполнитьАвто(Команда)
	...
	Если     НЕ ЗначениеЗаполнено(УточняемыйГод) ИЛИ НЕ ЗначениеЗаполнено(УточняемыйМес)  Тогда
	...
	Иначе
		...
		ИначеЕсли УточняемыйОтчет.ИсточникОтчета = "РегламентированныйОтчетУточняющийРасчетДекларацияНДС" Тогда
			...
			Если ПроверитьУточняемыйОтчет(СтруктураРеквизитовФормы.Организация, УточняемыйОтчет.Ссылка, УточняемыйГод, УточняемыйМес) Тогда	
...
&НаСервереБезКонтекста
Функция ПроверитьУточняемыйОтчет(Организация, УточняемыйОтчет, УточняемыйГод, УточняемыйМес)

	ДанныеОтчета = УточняемыйОтчет.ДанныеОтчета.Получить();
	ПоказателиОтчета = ""; СлужебныеДанные = ""; HZYP = ""; HZMP = "";
		...
		Для каждого строка Из НайденныеСтроки Цикл
			...
			Если Строка.НДСОборот <> 0 Тогда
				...
				Если Строка.СтавкаНДС = НДС20 Тогда
					...
					СтрокаТаб.НДС = Строка.НДСОборот;
					R722GA = R722GA + Строка.БазаНДСОборот;
					R722GB = R722GB + Строка.НДСОборот;
				
				ИначеЕсли Строка.СтавкаНДС = НДС14 Тогда
					
					СтрокаТаб.НДС14 = Строка.НДСОборот;
					R723GA = R723GA + Строка.БазаНДСОборот;
					R723GB = R723GB + Строка.НДСОборот;
...
//2024
&НаСервере
Процедура ПодготовитьДанныеДляТаблицы2(Таблица,ИмяКолонки,Статья)
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	СтруктураДанныхДекларация.R722GA = R722GA;
	СтруктураДанныхДекларация.R722GB = R722GB;
	СтруктураДанныхДекларация.R723GA = R723GA;
	СтруктураДанныхДекларация.R723GB = R723GB;
	...
	ПодготовитьДанныеДляТаблицы2(ТаблицаРезультатов, ИмяКолонкиНК, НужнаяСтатья);
	...
	//2024
	// перенесем в строку 15.1 Декларации
	СтруктураДанныхДекларация.R151GA = СтруктураДанныхПриложение1.R0220G4;
	СтруктураДанныхДекларация.R151GB = СтруктураДанныхПриложение1.R0220G8;
...
Процедура УстановитьФлаги()
	...
	ТекЗначениеФлага =  ЗначениеЗаполнено(СтруктураДанныхПриложение1.R0111G6)
					...
					//2024
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R0220G4)	
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R013G9)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R013G10)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R013G11)	
					
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R0223G4)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R0224G4);		
	СтруктураДанныхДекларация.HD1 = ?(ТекЗначениеФлага, "X","");
	
	ТекЗначениеФлага = ЗначениеЗаполнено(СтруктураДанныхДекларация.R210GB)
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхДекларация.R201GB)	
					
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхДекларация.R190GB)		
	...
	Если СтруктураРеквизитовФормы.ВыгружатьСправку Тогда
	...
	И (    ЗначениеЗаполнено(СтруктураДанныхПриложение5.R02G10)
	   ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение5.R02G9)
	   ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение5.R02G11));
