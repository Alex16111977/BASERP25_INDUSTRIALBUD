...
Процедура СформироватьОтчетСостояниеВыполненияДокументов(ВходящиеДокументы, ТаблицаОтчета) Экспорт
	...
	Если СтруктураПараметров.ВыводитьТаблицуУслугДавальцуКОформлению <> 0
		...
		ТекстыЗапроса.Добавить(ТекстЗапросаВТСуммаПоОтчетуДавальцу(), "ВТСуммаПоОтчетуДавальцу");
...
Функция ТекстЗапросаВТСуммаПоОтчетуДавальцу()
	
	ТекстЗапроса =
	"
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетДавальцуПродукция.ЗаказДавальца КАК ЗаказДавальца,
	|	ОтчетДавальцуПродукция.Номенклатура КАК Номенклатура,
	|	ОтчетДавальцуПродукция.Характеристика КАК Характеристика,
	|	СУММА(ОтчетДавальцуПродукция.СуммаВзаиморасчетов) КАК Сумма
	|ПОМЕСТИТЬ ВТСуммаПоОтчетуДавальцу
	|ИЗ
	|	Документ.ОтчетДавальцу.Продукция КАК ОтчетДавальцуПродукция
	|ГДЕ
	|	ОтчетДавальцуПродукция.Ссылка.Проведен
	|	И ОтчетДавальцуПродукция.ЗаказДавальца В(&МассивДокументов)
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетДавальцуПродукция.ЗаказДавальца,
...
Функция ТекстЗапросаТаблицаУслугиДавальцуКОформлению()
	...
	|	ЕСТЬNULL(ВТСуммаПоОтчетуДавальцу.Сумма, 0) КАК Сумма,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСуммаПоОтчетуДавальцу КАК ВТСуммаПоОтчетуДавальцу
	|		ПО УслугиДавальцуКОформлениюОстатки.ЗаказДавальца = ВТСуммаПоОтчетуДавальцу.ЗаказДавальца
	|			И УслугиДавальцуКОформлениюОстатки.Номенклатура = ВТСуммаПоОтчетуДавальцу.Номенклатура
	|			И УслугиДавальцуКОформлениюОстатки.Характеристика = ВТСуммаПоОтчетуДавальцу.Характеристика
	|ГДЕ
...
Функция ВывестиТаблицуПродукцияКОформлениюВОтчетеПереработчику(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	СтруктураЗаголовка = Новый Структура("Заголовок", 
							СтрЗаменить(Заголовок, "%Кол-во%", ПродукцияКОформлению.Строки.Количество()));
	Область.Параметры.Заполнить(СтруктураЗаголовка);
	ТаблицаОтчета.Вывести(Область);
...
Функция ВывестиТаблицуУслугиДавальцуКОформлению(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	СтруктураЗаголовка = Новый Структура("Заголовок", 
							СтрЗаменить(Заголовок, "%Кол-во%", УслугиДавальцуКОформлению.Строки.Количество()));
	Область.Параметры.Заполнить(СтруктураЗаголовка);
	ТаблицаОтчета.Вывести(Область);
...
Функция ВывестиТаблицуСырьеУПереработчика(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	ИмяОбласти = "ЗаголовокТаблицы";
	Область = Макет.ПолучитьОбласть(ИмяОбласти);
	СтруктураЗаголовка = Новый Структура("Заголовок", НСтр("ru='Сырье и материалы у переработчика';uk='Сировина і матеріали у переробника'"));
	Область.Параметры.Заполнить(СтруктураЗаголовка);
	ТаблицаОтчета.Вывести(Область);
