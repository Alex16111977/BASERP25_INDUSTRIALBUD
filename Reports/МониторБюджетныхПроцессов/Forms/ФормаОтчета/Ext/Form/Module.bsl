	...
	Если Не СтандартнаяОбработка Тогда
		...
		ИначеЕсли СписокРезультат.Количество() Тогда
			...
			СписокРезультат.ПоказатьВыборЭлемента(
				ОписаниеОповещения,
				НСтр("ru='Сведения о шаге процесса';uk='Відомості про крок процесу'"));
		Иначе
...
Процедура СформироватьВФоне()
	...
	Элементы.ТекущиеНевыполненныеЗадачи.Заголовок = НСтр("ru='Получение данных...';uk='Отримання даних...'");
...
Процедура ПолучитьСведенияОЗадачахВФонеЗавершение(РезультатФоновогоЗадания,ДополнительныеПараметры) Экспорт
	...
	Если РезультатФоновогоЗадания = Неопределено Тогда
		Элементы.ТекущиеНевыполненныеЗадачи.Заголовок = НСтр("ru='Ошибка получения данных';uk='Помилка отримання даних'");
		Возврат;
	ИначеЕсли РезультатФоновогоЗадания.Статус = "Ошибка" Тогда
		Элементы.ТекущиеНевыполненныеЗадачи.Заголовок = НСтр("ru='Ошибка получения данных';uk='Помилка отримання даних'");
		ВызватьИсключение РезультатФоновогоЗадания.КраткоеПредставлениеОшибки;
...
Функция ЗаполнитьДиаграммуВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Монитор бюджетных процессов, заполнение диаграммы';uk='Монітор бюджетних процесів, заповнення діаграми'");
...
Функция ПолучитьСведенияОЗадачахВФоне()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.ОжидатьЗавершение = 0; // запускать сразу
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Монитор бюджетных процессов, получение сведений о задачах';uk='Монітор бюджетних процесів, отримання відомостей про задачі'");
