	...
	Для i = 0 по МассивСтатей.Количество()- 1 Цикл
		...
		Для каждого строка Из НужныеСтроки Цикл
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(Строка.ИННКонтрагента) Тогда	
				Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Имеются входящие налоговые накладные от контрагента  %1 с незаполненным ИНН!';uk= 'Є вхідні податкові накладні від контрагента  %1  з незаповненим ІПН!'"), Строка.ДоговорКонтрагента.Владелец));
			КонецЕсли;	
