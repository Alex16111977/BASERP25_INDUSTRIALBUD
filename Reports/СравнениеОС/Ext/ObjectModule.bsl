...
Функция РеквизитыСравнения()
	...
	ОписаниеРаздела.Реквизиты.Добавить(ОписаниеРеквизита("ПервоначальнаяСтоимость", НСтр("ru='Первоначальная стоимость';uk='Первісна вартість'"),, "ЧДЦ=2"));
	ОписаниеРаздела.Реквизиты.Добавить(ОписаниеРеквизита("ЛиквидационнаяСтоимость", НСтр("ru='Ликвидационная стоимость';uk='Ліквідаційна вартість'"), Ложь, "ЧДЦ=2"));
	...
	ОписаниеРаздела.Реквизиты.Добавить(ОписаниеРеквизита("ОбъемНаработки", НСтр("ru='Предполагаемый объем наработки (продукции или выполненных работ)';uk='Передбачуваний обсяг напрацювання (продукції або виконаних робіт)'"),, "ЧДЦ=2"));
	ОписанияРазделов.Добавить(ОписаниеРаздела);
...
Функция ТекстЗапросаДанныеБУ()
	...
	|ИЗ
	|	РегистрСведений.ПервоначальныеСведенияОС.СрезПоследних(, ОсновноеСредство В (&Список)) КАК ПервоначальныеСведения
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Актив,
	|	Организация
	...
	|	СчетаУчета.ПоказательНаработки КАК ПоказательНаработки,
	|	ЕСТЬNULL(СчетУчетаСтоимости.СчетУчета, СчетаУчета.СчетУчета) КАК СчетУчета,
	|	ЕСТЬNULL(СчетУчетаСтоимости.СчетУчета, СчетаУчета.СчетНачисленияАмортизации) КАК СчетНачисленияАмортизации
	...
	|		И СчетУчетаСтоимости.АналитикаУчета = СчетаУчета.ГруппаФинансовогоУчета
	...
	|		И СчетУчетаАмортизации.АналитикаУчета = СчетаУчета.ГруппаФинансовогоУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
