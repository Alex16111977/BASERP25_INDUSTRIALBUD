...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПараметрВыводОтчета.Значение = 0 
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, 
			"Документ", 
			НСтр("ru='Не заполнено поле ""Лимиты, применяемые для документа"".';uk='Не заповнене поле ""Ліміти, що застосовуються для документа"".'"),
			""); 
	...
	ИначеЕсли ПараметрВыводОтчета.Значение = 1
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, 
			"Валюта", 
			НСтр("ru='Не заполнено поле ""Валюта"".';uk='Не заповнене поле ""Валюта"".'"),
			""); 
...
Функция ПолучитьОтборРасшифровкиДляПолученияДанных(КомпоновщикНастроек)
	...
	Если Настройки.Структура.Количество() > 0 Тогда
		...
		Если НЕ ПоляГруппировки = Неопределено
			...
			Иначе
				ТекстСообщения = НСтр("ru='Расшифровка по регистратору может использоваться только в рамках правила лимитов.';uk='Розшифровка за реєстратором може використовуватися тільки в рамках правила лімітів.'");
...
Функция ПреобразоватьПланФактЛимитовДляВыводаВОтчет(ПланФактЛимитов, ОтборРасшифровки)
	...
	Если ПланФактЛимитов.Количество() > 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СвернутьМассив(ИспользуемыеПравилаЛимитов);
