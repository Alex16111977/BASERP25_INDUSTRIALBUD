...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ПараметрКоманды") Тогда
		...
		Если Не ЗначениеЗаполнено(Параметры.ПараметрКоманды) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Распределение материала не выполнено или не требуется.';uk='Розподіл матеріалу не виконаний або не потрібний.'"),,,,Отказ);
		КонецЕсли;
		...
		Если ТипЗнч(Параметры.ПараметрКоманды) = Тип("ДокументСсылка.РаспределениеПроизводственныхЗатрат") 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Отчет предназначен для использования только для документов производства версии 2.5.';uk='Звіт призначений для використання тільки для документів виробництва версії 2.5.'"),,,,Отказ);
		КонецЕсли;
	...
	Иначе
		...
		Отказ = Истина;
		ВызватьИсключение НСтр("ru='Отчет предназначен только для открытия в документе ""Распределение материалов и работ"".';uk='Звіт призначений тільки для відкриття в документі ""Розподіл матеріалів і робіт"".'") ;
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если Область <> Неопределено Тогда
		Если ЭтоРаспределениеОтходов Тогда
			ЗаголовокОтчета = НСтр("ru='Распределение возвратного отхода ""%1"" в подразделении ""%2""';uk='Розподіл зворотного відходу ""%1"" в підрозділі ""%2""'");
		...
		Иначе
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Номенклатура, "ТипНоменклатуры") = Перечисления.ТипыНоменклатуры.Работа Тогда
				ЗаголовокОтчета = НСтр("ru='Распределение работы ""%1"" в подразделении ""%2"" за %3';uk='Розподіл роботи ""%1"" в підрозділі ""%2"" за %3'");
				ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ЗаголовокОтчета,
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Реквизиты.Номенклатура, Реквизиты.Характеристика),
					Реквизиты.Подразделение,
			...
			Иначе
				ЗаголовокОтчета = НСтр("ru='Распределение материала ""%1"" в кладовой ""%2"" за %3';uk='Розподіл матеріалу ""%1"" в коморі ""%2"" за %3'");
				ЗаголовокОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ЗаголовокОтчета,
					НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Реквизиты.Номенклатура, Реквизиты.Характеристика),
					Реквизиты.Склад,
	...
	ПредставлениеПравила    = НСтр("ru='Прочее правило распределения %1';uk='Інше правило розподілу %1'");
	ПредставлениеПоказателя = НСтр("ru='Значение показателя';uk='Значення показника'");
	...
	Если Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.СуммаМатериальныхЗатрат Тогда 
		...
		ПредставлениеПоказателя = НСтр("ru='Стоимость';uk='Вартість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально стоимости материальных затрат %1';uk='Пропорційно вартості матеріальних витрат %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.КоличествоУказанныхМатериалов Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Материалы';uk='Матеріали'");
		ПредставлениеПоказателя = НСтр("ru='Количество';uk='Кількість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально количеству указанных материалов %1';uk='Пропорційно кількості зазначених матеріалів %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.ОбъемУказанныхМатериалов Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Материалы';uk='Матеріали'");
		ПредставлениеПоказателя = НСтр("ru='Объем';uk='Об''єм'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально объему указанных материалов %1';uk='Пропорційно об''єму зазначених матеріалів %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.ВесУказанныхМатериалов Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Материалы';uk='Матеріали'");
		ПредставлениеПоказателя = НСтр("ru='Вес';uk='Вага'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально весу указанных материалов %1';uk='Пропорційно вазі зазначених матеріалів %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.СуммаРасходовНаОплатуТруда Тогда
		
		ПредставлениеПозиции    = "";
		ПредставлениеПоказателя = НСтр("ru='Стоимость';uk='Вартість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально сумме расходов на оплату труда %1';uk='Пропорційно сумі витрат на оплату праці %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.НормативыОплатыТруда Тогда
		
		ПредставлениеПозиции    = "";
		ПредставлениеПоказателя = НСтр("ru='Стоимость';uk='Вартість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально нормативной сумме расходов на оплату труда %1';uk='Пропорційно нормативній сумі витрат на оплату праці %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.КоличествоРаботУказанныхВидов Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Виды работ';uk='Види робіт'");
		ПредставлениеПоказателя = НСтр("ru='Количество';uk='Кількість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально количеству указанных видов работ %1';uk='Пропорційно кількості зазначених видів робіт %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.ПлановаяСтоимостьПродукции Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Продукция';uk='Продукція'");
		ПредставлениеПоказателя = НСтр("ru='Стоимость';uk='Вартість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально плановой стоимости продукции %1';uk='Пропорційно плановій вартості продукції %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.КоличествоПродукции Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Продукция';uk='Продукція'");
		ПредставлениеПоказателя = НСтр("ru='Количество';uk='Кількість'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально количеству продукции %1';uk='Пропорційно кількості продукції %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.ОбъемПродукции Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Продукция';uk='Продукція'");
		ПредставлениеПоказателя = НСтр("ru='Объем';uk='Об''єм'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально объему продукции %1';uk='Пропорційно об''єму продукції %1'");
		
	ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.ВесПродукции Тогда
		
		ПредставлениеПозиции    = НСтр("ru='Продукция';uk='Продукція'");
		ПредставлениеПоказателя = НСтр("ru='Вес';uk='Вага'");
		ПредставлениеПравила    = НСтр("ru='Пропорционально весу продукции %1';uk='Пропорційно вазі продукції %1'");
		
	КонецЕсли;
	
	Если ОтборПоГруппамПродукции Тогда
		ПредставлениеПравила = СтрШаблон(ПредставлениеПравила, НСтр("ru='на выбранные группы продукции';uk='на вибрані групи продукції'"));
	Иначе
	...
	Пока Область <> Неопределено Цикл
		...
		Если ПредставлениеПозиции = НСтр("ru='Продукция';uk='Продукція'") Тогда
			ПредставлениеПозицииВСтроке = НСтр("ru='<другая продукция>';uk='<інша продукція>'");
		Иначе
			ПредставлениеПозицииВСтроке = СтрШаблон(НСтр("ru='<другие %1>';uk='<інші %1>'"), НРег(ПредставлениеПозиции));
		КонецЕсли;
