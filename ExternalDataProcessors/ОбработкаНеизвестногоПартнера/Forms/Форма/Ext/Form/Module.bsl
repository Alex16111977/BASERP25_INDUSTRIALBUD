...
// Версия: 2.5 (Исправлена проблема с отложенными проведениями)
// Дата: 2025-11-24
...
//
// ИСПРАВЛЕНИЯ v2.5:
// - КРИТИЧЕСКОЕ: Удаление записей с ОтложенноеПроведение=Истина перед перепроведением
//   (эти записи НЕ пересчитываются при проведении - они копируются из регистра!)
// - Новая функция УдалитьОтложенныеДвиженияНеизвестногоПартнера()
//
// ИСПРАВЛЕНИЯ v2.4:
...
Функция ОбработатьРегистраторДляРегистра(СтрокаТаблицы)
	...
	// 4. КРИТИЧЕСКИ ВАЖНО v2.5: Удаляем записи с ОтложенноеПроведение=Истина
	// Эти записи НЕ пересчитываются при проведении - они копируются из регистра!
	// См. ManagerModule.bsl строки 5051-5056:
	//   ИСТИНА  -- ОтложенноеПроведение
	//   ИЗ РегистрНакопления.ДвиженияДенежныеСредстваКонтрагент
	//   ГДЕ Регистратор = &Ссылка И ОтложенноеПроведение
	УдалитьОтложенныеДвиженияНеизвестногоПартнера(Ссылка);

	// 5. ЗАПИСЫВАЕМ И ПЕРЕПРОВОДИМ ДОКУМЕНТ
...
// ============================================================================
// КРИТИЧЕСКАЯ ФУНКЦИЯ v2.5: Удаление отложенных движений с Неизвестным партнером
// ============================================================================
// ПРОБЛЕМА:
// Записи с ОтложенноеПроведение=Истина НЕ пересчитываются при проведении!
// Они просто КОПИРУЮТСЯ из существующего регистра обратно (см. ManagerModule.bsl:5051-5056)
//
// Поэтому если в регистре ДвиженияДенежныеСредстваКонтрагент есть запись
// с ОтложенноеПроведение=Истина и Партнер="Неизвестный партнер",
// то после перепроведения она останется без изменений!
//
// РЕШЕНИЕ:
// Удаляем такие записи ДО перепроведения документа
// ============================================================================
&НаСервере
Процедура УдалитьОтложенныеДвиженияНеизвестногоПартнера(ДокументСсылка)

	Попытка
		// Получаем движения документа по регистру с отложенным проведением
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ
			...
			|	И Движения.ОтложенноеПроведение = ИСТИНА
			|	И Движения.Партнер = &НеизвестныйПартнер";
		...
		Если НЕ Результат.Пустой() Тогда
			...
			// Получаем набор записей и удаляем только записи с ОтложенноеПроведение
			НаборЗаписей = РегистрыНакопления.ДвиженияДенежныеСредстваКонтрагент.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Регистратор.Установить(ДокументСсылка);
			НаборЗаписей.Прочитать();

			// Создаем новый набор без отложенных движений с неизвестным партнером
			СтрокиДляУдаления = Новый Массив;

			Для Каждого Запись Из НаборЗаписей Цикл
				Если Запись.ОтложенноеПроведение И Запись.Партнер = НеизвестныйПартнер Тогда
					СтрокиДляУдаления.Добавить(Запись);
				КонецЕсли;
			КонецЦикла;

			// Удаляем найденные строки
			Для Каждого СтрокаДляУдаления Из СтрокиДляУдаления Цикл
				НаборЗаписей.Удалить(СтрокаДляУдаления);
			КонецЦикла;

			// Записываем набор без удаленных строк
	...
	Исключение
		// Ошибка при удалении - продолжаем
		Сообщить("Предупреждение: не удалось удалить отложенные движения: " + ОписаниеОшибки());
	КонецПопытки;
