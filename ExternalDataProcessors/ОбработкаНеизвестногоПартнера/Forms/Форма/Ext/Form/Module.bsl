...
Процедура НайтиДокументыНаСервере()
	...
	МассивДляПоиска.Добавить(НеизвестныйПартнер);
	МассивСсылок = НайтиПоСсылкам(МассивДляПоиска);

	КоличествоНайденных = 0;

	// НайтиПоСсылкам возвращает ТаблицаЗначений с колонками: Ссылка, Данные, Метаданные
	Для Каждого СтрокаТаблицы Из МассивСсылок Цикл

		// Работаем только с документами
		Если НЕ Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СтрокаТаблицы.Ссылка)) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрНайти(СтрокаТаблицы.Метаданные, "ТабличнаяЧасть") > 0
			ИЛИ СтрНайти(СтрокаТаблицы.Метаданные, "TabularSection") > 0 Тогда
