...
// Версия: 2.3 (Исправлена проблема с кэшем АналитикаУчетаПоПартнерам)
// Дата: 2025
...
//
// ИСПРАВЛЕНИЯ v2.3:
// - Добавлена очистка регистра АналитикаУчетаПоПартнерам перед перепроведением
//   (система кэширует ключ аналитики с "Неизвестный партнер", и при перепроведении
//   находит старый ключ вместо создания нового)
// - Новая процедура ОчиститьАналитикуНеизвестногоПартнера()
//
// ИСПРАВЛЕНИЯ v2.2:
...
Функция ОбработатьРегистраторДляРегистра(СтрокаТаблицы)
	...
	// 3. КРИТИЧНО: Удаляем записи из регистра АналитикаУчетаПоПартнерам с "Неизвестный партнер"
	// Иначе при перепроведении система найдет СТАРЫЙ ключ аналитики!
	Если Изменен И ЗначениеЗаполнено(КонтрагентДокумента) Тогда
		ОчиститьАналитикуНеизвестногоПартнера(КонтрагентДокумента);
	КонецЕсли;
	...
	Если Изменен Тогда
		...
		Если ДокументОбъект.Проведен Тогда
			// ПЕРЕПРОВОДИМ - это обновит регистры с НОВЫМ ключом аналитики!
...
// Удаляет записи из регистра АналитикаУчетаПоПартнерам с "Неизвестный партнер" для указанного контрагента
&НаСервере
Процедура ОчиститьАналитикуНеизвестногоПартнера(Контрагент)
	...
	Попытка
		...
		МетаданныеРегистра = Метаданные.РегистрыСведений.Найти("АналитикаУчетаПоПартнерам");
		Если МетаданныеРегистра = Неопределено Тогда
			Возврат;
		КонецЕсли;

		// Ищем записи с "Неизвестный партнер" для данного контрагента
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ
			...
			|	И Аналитика.Партнер = &НеизвестныйПартнер";
		...
		Пока Выборка.Следующий() Цикл
			...
			НаборЗаписей.Отбор.КлючАналитики.Установить(Выборка.КлючАналитики);
			НаборЗаписей.Записать(Истина); // Пустой набор = удаление
		КонецЦикла;

	Исключение
		// Ошибка при очистке аналитики - продолжаем без неё
		Сообщить("Ошибка при очистке АналитикаУчетаПоПартнерам: " + ОписаниеОшибки());
	КонецПопытки;
