...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Представление.Заголовок = ?(ИспользоватьУчастки,
		НСтр("ru='Смена, Участок';uk='Зміна, Ділянка'"),
		НСтр("ru='Смена';uk='Зміна'"));
...
Процедура Вверх(Команда)
	...
	Иначе
		...
		Если Индекс <> Неопределено Тогда
			ДобавитьСтрокуВВидимыеСмены(ЭтотОбъект, ВсеСмены[Индекс], Истина);
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Данные графика не заполнены';uk='Дані графіка не заповнені'"));
		КонецЕсли;
...
Процедура ОткрытьЗадание(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Задание не создано';uk='Завдання не створено'"));
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<задание не создано>';uk='<завдання не створено>'"));
...
Процедура ИнициализироватьСмены()
	...
	Если ДоступнаПрокрутка Тогда
		...
		Если НачалоДня(ДатаНачалаСписка) = ТекущаяДата Тогда
			Элементы.ПерейтиВНачалоСписка.Заголовок = СтрШаблон(НСтр("ru='%1 сегодня';uk=' %1 сьогодні'"), Формат(ДатаНачалаСписка, "ДЛФ=DD"));
		Иначе
...
Функция СоздатьИПолучитьВыбранныеЗадания()
	...
	Если ВыбранныеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не отмечены смены для выбора';uk='Не помічені зміни для вибору'"),, "ВидимыеСмены");
		Возврат Неопределено;
	КонецЕсли;
...
Процедура НастроитьПодсказкуВыбраноСмен(Форма)
	...
	Если Форма.КоличествоВыбранныхСмен <> 0 Тогда
		Форма.Элементы.ПодсказкаВыбраноЗаданий.Заголовок = СтрШаблон(
			НСтр("ru='Выбрано заданий: %1';uk='Вибрано завдань: %1'"),
