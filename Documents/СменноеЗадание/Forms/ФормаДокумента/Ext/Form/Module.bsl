...
|;uk='Зміна операцій можлива тільки після запису документа. 
...
Процедура ОперацииДокумент_УстановитьСтатусПропущена(Команда)
	...
	Для каждого Ссылка Из ВыделенныеСтроки Цикл
		...
		Если НЕ Элементы.ОперацииДокумент.ДанныеСтроки(Ссылка).МожноПропустить Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выделенные операции не могут быть пропущены.';uk='Виділені операції не можуть бути пропущені.'"));
			Возврат;
		КонецЕсли;
	...
	Для каждого Ссылка Из ВыделенныеСтроки Цикл
		...
		Если НЕ ДанныеСтроки.ВидРабочегоЦентра.Пустая() Тогда
			...
			ИначеЕсли ВидРабочегоЦентра <> ДанныеСтроки.ВидРабочегоЦентра Тогда
				...
				ПоказатьПредупреждение(, НСтр("ru='Выбраны операции с различными видами рабочих центров';uk='Вибрані операції з різними видами робочих центрів'"));
				Возврат;
...
|Записать и продолжить?'
|;uk='Для зміни статусу потрібно записати документ. 
|Записати і продовжити?'");
...
Функция УстановитьСтатусВФоне(Статус)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru='Установка статуса сменному заданию';uk='Встановлення статусу змінному завданням'");
...
Функция ПоказатьВопросУдалениеРучныхОпераций()
	...
	ПоказатьВопрос(
		...
		НСтр("ru='При изменении статуса будут удалены операции, введенные вручную. Продолжить?';uk='При зміні статусу будуть вилучені операції, введені вручну. Продовжити?'"),
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", СтрШаблон("ЧДЦ=1; ЧФ='Ч %1'", НСтр("ru='сек';uk='сек'")));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", СтрШаблон("ЧДЦ=1; ЧФ='Ч %1'", НСтр("ru='мин';uk='хв'")));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", СтрШаблон("ЧДЦ=1; ЧФ='Ч %1'", НСтр("ru='ч';uk='год'")));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", СтрШаблон("ЧДЦ=1; ЧФ='Ч %1'", НСтр("ru='сут';uk='доб'")));
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("Статус")
		...
		Иначе
		    ...
		    МассивСтрок.Добавить(НСтр("ru='В статусе';uk='У статусі'"));
			МассивСтрок.Добавить(" ");
			
			// Создана
			Содержимое = ?(Форма.ВСтатусеСоздана = 0, НСтр("ru='Создана';uk='Створена'"),
				СтрШаблон(НСтр("ru='Создана (%1)';uk='Створено (%1)'"), Форма.ВСтатусеСоздана));
			...
			// Выполняется
			Содержимое = ?(Форма.ВСтатусеВыполняется = 0, НСтр("ru='Выполняется';uk='Виконується'"),
				СтрШаблон(НСтр("ru='Выполняется (%1)';uk='Виконується (%1)'"), Форма.ВСтатусеВыполняется));
			...
			// Выполнена
			Содержимое = ?(Форма.ВСтатусеВыполнена = 0, НСтр("ru='Выполнена';uk='Виконано'"),
				СтрШаблон(НСтр("ru='Выполнена (%1)';uk='Виконано (%1)'"), Форма.ВСтатусеВыполнена));
			...
			// Не выполнена
			Содержимое = ?(Форма.ВСтатусеНеВыполнена = 0, НСтр("ru='Не выполнена';uk='Не виконана'"),
				СтрШаблон(НСтр("ru='Не выполнена (%1)';uk='Не виконана (%1)'"), Форма.ВСтатусеНеВыполнена));
			...
			// Пропущена
			Содержимое = ?(Форма.ВСтатусеПропущена = 0, НСтр("ru='Пропущена';uk='Пропущена'"),
				СтрШаблон(НСтр("ru='Пропущена (%1)';uk='Пропущена (%1)'"), Форма.ВСтатусеПропущена));
	...
	Если СтруктураРеквизитов.Свойство("Статус") 
		...
		Если НЕ Форма.СтатусОперацийОтбор.Пустая()
			...
			МассивСтрок.Добавить(
				СтрШаблон(НСтр("ru='Установлен отбор по операциям в статусе %1';uk='Встановлено відбір за операціями в статусі %1'"),
...
Функция ПроверитьПолучитьОперацииДляИзменения()
	...
	Если Элементы.ОперацииРегистр.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не выбраны операции для выполнения действия';uk='Не вибрані операції для виконання дії'"));
	...
	Иначе
		...
		Для каждого Строка ИЗ Элементы.ОперацииРегистр.ВыделенныеСтроки Цикл
			...
			Если ТипЗнч(Строка) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
				...
				ПоказатьПредупреждение(, НСтр("ru='Действие не может быть выполнено для строки группировки списка';uk='Дію не може бути виконано для рядка групування списку'"));
