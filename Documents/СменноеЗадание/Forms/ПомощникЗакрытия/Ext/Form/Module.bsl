	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.Создана > 0 Тогда
			Элементы.ВСтатусеСоздана.Заголовок = СтрШаблон(
				НСтр("ru='Операций в статусе ""Создана"": %1';uk='Операцій в статусі ""Створено"": %1'"),
				Выборка.Создана);
		...
		Если Выборка.Выполняется > 0 Тогда
			Элементы.ВСтатусеВыполняется.Заголовок = СтрШаблон(
				НСтр("ru='Операций в статусе ""Выполняется"": %1';uk='Операцій в статусі ""Виконується"": %1'"),
				Выборка.Выполняется);
		Иначе
...
Процедура ОК(Команда)
	...
	Исключение
	    ...
	    ПоказатьПредупреждение(,
			НСтр("ru='Операция не может быть выполнена по причине:';uk='Операція не може бути виконана з причини:'")
			+ Символы.ПС
