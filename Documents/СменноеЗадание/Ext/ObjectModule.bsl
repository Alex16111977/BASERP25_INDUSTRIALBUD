...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Для выполнения действия документ должен иметь статус ""Формируется"".';uk='Для виконання дії документ повинен мати статус ""Формується"".'"),
			ЭтотОбъект,,,
			Отказ);
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='В системе уже имеется сменное задание за указанную дату.';uk='В системі вже є змінне завдання за вказану дату.'"),
				ЭтотОбъект,,,
				Отказ);
	...
	Если ДанныеДоИзменения.Подразделение <> Подразделение Тогда
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Имеются операции, назначенные сменному заданию. Изменять подразделение запрещено.';uk='Є операції, призначені змінному завданню. Змінювати підрозділ заборонено.'"),
				ЭтотОбъект,,,
				Отказ);
	...
	Если Статус = Перечисления.СтатусыСменныхЗаданий.Формируется И НЕ ЭтоНовый() Тогда
		...
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Имеются производственные операции в статусе отличном от ""Создана"".';uk='Є виробничі операції в статусі відмінному від ""Створено"".'"),
				ЭтотОбъект,,,
				Отказ);
	...
	Если Статус = Перечисления.СтатусыСменныхЗаданий.Закрыто И НЕ ЭтоНовый() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ОшибкаСтатус Тогда
				ТекстСообщения = НСтр("ru='Имеются производственные операции в статусе отличном от ""Выполнена"", ""Не выполнена"" и ""Пропущена"".';uk='Є виробничі операції в статусі відмінному від ""Виконано"", ""Не виконано"" і ""Пропущено"".'");
			Иначе
				ТекстСообщения = НСтр("ru='Имеются непроведенные производственные операции.';uk='Є непроведені виробничі операції.'");
			КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.РежимЗаписи = РежимЗаписиДокумента.Проведение
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не удалось рассчитать операции к созданию сменных заданий.';uk='Не вдалося розрахувати операції до створення змінних завдань.'"),
				ЭтотОбъект,,,
				Отказ);
