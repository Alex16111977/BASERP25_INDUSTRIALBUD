...
Процедура СоздатьДоначислениеВНУ(Команда)
	...
	ОткрытьФорму("Документ.АмортизацияНМА.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
...
Функция СоздатьДоначисленияВНУНаСервере()
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ
		|	ТаблицаПериодов.Организация КАК Организация,
		|	НАЧАЛОПЕРИОДА(АмортизацияНМА.Дата, МЕСЯЦ) КАК ПериодДокумента
		|ИЗ
		|	ТаблицаПериодов КАК ТаблицаПериодов
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АмортизацияНМА КАК АмортизацияНМА
		|		ПО ТаблицаПериодов.Организация = АмортизацияНМА.Организация
		|			И (АмортизацияНМА.ДоначислениеВНУ = Истина)
		|			И (АмортизацияНМА.Дата > ТаблицаПериодов.ПериодНачало)
		|			И (АмортизацияНМА.Дата <= ТаблицаПериодов.ПериодКонец)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Организация,
		|	ПериодДокумента"; 
	...
	Для каждого СтрокаПериодов Из ТаблицаПериодов Цикл
		...
		Пока ТекущийМесяц < СтрокаПериодов.ПериодКонец Цикл
			...
			Если ВыборкаСуществующиеДокументы.НайтиСледующий(СтруктураПоиска) = Ложь Тогда
				...
				НовыйДокумент = Документы.АмортизацияНМА.СоздатьДокумент();
