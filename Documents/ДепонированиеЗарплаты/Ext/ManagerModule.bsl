...
Функция ТекстОтраженияВРеглУчетеУКР() Экспорт
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		ТаблицаДанныеВедомостей КАК Строки
	|	ПО 
	|		ИСТИНА
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	...
	ТекстДепонирование = СтрШаблон(ТекстДепонирование, 
		НСтр("ru='Депонирование заработной платы';uk='Депонування заробітної плати'",ЯзыкСодержания));
...
Функция ТекстЗапросаВТОтраженияВРеглУчетеУКР() Экспорт
	...
	|
	|ВЫБРАТЬ
	|	Строки.Ссылка.Ведомость КАК Ведомость,
	|	Строки.ФизическоеЛицо
	|ПОМЕСТИТЬ ВТДанныеДепонирования
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	...
	|				Депоненты.Ведомость
	|			ИЗ
	|				ВТДанныеДепонирования КАК Депоненты)
	...
	|СГРУППИРОВАТЬ ПО
	|	Зарплата.Ссылка,
	|	Зарплата.ГруппаУчетаНачислений,
