...
Процедура ОбработкаНаправлениеВыпускаПриИзмененииНаСервере()
	...
	ПараметрыНастройки = Документы.ВыпускПродукции.ПараметрыНастройкиСчетовУчета(Объект.НаправлениеВыпуска);
	НастройкаСчетовУчета.ПриИзмененииПараметровНастройки(ЭтотОбъект, ПараметрыНастройки);
	//-- НЕ УТ
...
Функция ТоварыЗаполнитьЦеныПоВидуЦенСервер()
	...
	Если Не ЗначениеЗаполнено(Объект.ВидЦены) Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.ВидЦены) Тогда
				ТекстСообщения = НСтр("ru='Не настроены параметры прайс-листа. Не найден вид цен, используемый при выпуске продукции по фиксированной стоимости. Заполнение цен невозможно.';uk='Не налаштовані параметри прайс-листа. Не знайдено вид цін, який використовується при випуску продукції за фіксованою вартістю. Заповнення цін неможливо.'");
...
Процедура ЗаполнитьПоРаспоряжениям(Команда)
	...
	ПараметрыПроверки.ЗаголовокТабличнойЧасти   = НСтр("ru='Товары и работы';uk='Товари і роботи'");
	ПараметрыПроверки.ПроверятьРаспроведенность = Ложь;
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("Организация",   НСтр("ru='Организация';uk='Організація'"));
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"));
	Если Объект.НаправлениеВыпуска = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад") Тогда
...
Процедура ОбновитьИнформациюПоРаспоряжениям()
	...
	ПараметрыОбновления.НадписьВсегоЗаказов               = НСтр("ru='Всего заказов';uk='Всього замовлень'");
...
Процедура ПроверитьСпецификации(Отказ)
	...
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru='Изделие ""%1"" не производится по указанной спецификации в строке %2 списка Товары. Выберите другую спецификацию.';uk='Виріб ""%1"" не виробляється за зазначеною специфікацією в рядку %2 списку Товари. Виберіть іншу специфікацію.'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru='Спецификация ""%1"" в строке %2 списка Товары не является действующей. Выберите действующую спецификацию.';uk='Специфікація ""%1"" в рядку %2 списку Товари не є чинною. Виберіть чинну специфікацію.'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru='Спецификация ""%1"" в строке %2 содержит производимые в процессе полуфабрикаты. Выберите спецификацию без полуфабрикатов.';uk='Специфікація ""%1"" в рядку %2 містить напівфабрикати, що виробляються в процесі . Виберіть специфікацію без напівфабрикатів.'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru='В спецификации ""%1"" в строке %2 списка Товары не заполнены статьи калькуляции.';uk='У специфікації ""%1"" в рядку %2 списку Товари не заповнені статті калькуляції.'"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru='Спецификация ""%1"" в строке %2 списка Товары содержит уточняемую при производстве номенклатуру';uk='Специфікація ""%1"" в рядку %2 списку Товари містить номенклатуру, що уточнюється при виробництві '"));
	ШаблоныСообщенийОбОшибке.Добавить(НСтр("ru='Спецификация ""%1"" в строке %2 списка Товары содержит уточняемую при производстве характеристику';uk='Специфікація ""%1"" в рядку %2 списку Товари містить характеристику, що уточнюється при виробництві '"));
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ВыпускПродукции.ПараметрыЗаполненияНалоговогоНазначения(Объект);
