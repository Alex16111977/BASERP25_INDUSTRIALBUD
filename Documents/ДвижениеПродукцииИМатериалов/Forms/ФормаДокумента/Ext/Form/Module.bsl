...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.ЗначенияЗаполнения.Свойство("ЗаполнятьПоОрдеру") Тогда
		...
		УстановитьПризнакЦеховаяКладовая(ЭтотОбъект, "Отправитель");
		УстановитьПризнакЦеховаяКладовая(ЭтотОбъект, "Получатель");
...
|Змінювати госп.операцію не допускається.'");
	...
	ИначеЕсли ПоРаспоряжениямДоИзменения Тогда
		...
		Если ОтвязатьДокументОтРаспоряжений
			...
			Если ЗначениеЗаполнено(Объект.Распоряжение) Тогда
				ТекстВопроса = НСтр("ru='Строки документа перестанут быть связанными со строками распоряжения. Продолжить?';uk='Рядки документа перестануть бути пов''язаними з рядками розпорядження. Продовжити?'");
			Иначе
				ТекстВопроса = НСтр("ru='Строки документа перестанут быть связанными со строками распоряжений. Продолжить?';uk='Рядки документа перестануть бути пов''язаними з рядками розпоряджень. Продовжити?'");
			КонецЕсли;
...
Процедура ПоРаспоряжениямПриИзменении(Элемент)
	...
	Если Объект.ПоРаспоряжениям И НачатаОтгрузка Тогда
		...
		ТекстПредупреждения = НСтр("ru='По данному документу уже начата отгрузка. Установка признака невозможна.';uk='За даним документом вже розпочато відвантаження. Встановлення ознаки неможливо.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Объект.ПоРаспоряжениям = Ложь;
		Возврат;
	...
	Если Объект.ПоРаспоряжениям Тогда
		...
		Если Объект.Товары.Количество() = 0 Тогда
			ТекстВопроса = НСтр("ru='Список Продукция и материалы будет перезаполнен остатками неоформленных товаров по распоряжениям. Продолжить?';uk='Список Продукція та матеріали буде перезаповнений залишками неоформлених товарів за розпорядженнями. Продовжити?'");
		Иначе
			ТекстВопроса = НСтр("ru='Строки в списке Продукция и материалы будут привязаны к строкам распоряжений. Продолжить?';uk='Рядки в списку Продукція і матеріали будуть прив''язані до рядків розпоряджень. Продовжити?'");
		КонецЕсли;
	...
	Иначе
		...
		Если ЕстьСтрокиПривязанныеКЗаказу() Тогда
			...
			ТекстВопроса = НСтр("ru='Строки в списке Продукция и материалы перестанут быть связанными со строками распоряжений. Продолжить?';uk='Рядки в списку Продукція і матеріали перестануть бути пов''язаними з рядками розпоряджень. Продовжити?'");
...
Процедура ОтправительПриИзменении(Элемент)
	...
	Если Объект.Отправитель <> ОтправительДоИзменения 
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Очистить и продолжить';uk='Очистити і продовжити'"));
...
Процедура НадписьРаспоряженияНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Распоряжения (%КоличествоДокументов%)';uk='Розпорядження (%КоличествоДокументов%)'"));
...
Процедура ВидЦеныПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦены) И ЕстьСтрокиДляИзменения Тогда
		...
		ТекстВопроса = НСтр("ru='Перезаполнить цены товаров?';uk='Перезаповнити ціни товарів?'");
...
Процедура ПодобратьТовары(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Отправитель) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Склад"" не заполнено';uk='Поле ""Склад"" не заповнено'"), Объект.Ссылка, "Объект.Отправитель");
		Возврат;
	КонецЕсли;
...
Процедура ОтвязатьОтЗаказа(Команда)
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru='Выберите строки, которые необходимо отвязать от распоряжений.';uk='Виберіть рядки, які необхідно відв''язати від розпоряджень.'");
...
Процедура ОкруглитьДоЦелойУпаковки(Команда)
	...
	Если ВсегоСтрок = 0 Тогда
		ПоказатьОповещениеПользователя(
			НСтр("ru='Округление не требуется';uk='Округлення не потрібно'"),, 
			НСтр("ru='В выбранных строках уже целое количество единиц.';uk='У вибраних рядках вже ціле кількість одиниць.'"),
...
|Продолжить?'
|;uk='У вибраних рядках кількість буде округлено до цілих одиниць. 
|Продовжити?'");
...
Процедура ХозяйственнаяОперацияПриИзмененииНаСервере()
	...
	СтруктураДействий = Новый Структура;
	УстановитьПризнакЦеховаяКладовая(ЭтотОбъект, "Отправитель", СтруктураДействий);
	УстановитьПризнакЦеховаяКладовая(ЭтотОбъект, "Получатель", СтруктураДействий);
...
Процедура ПривязатьСтрокиКРаспоряжениям()
	...
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		...
		Если ТекСтрока.Распоряжение = Неопределено
			И ТекСтрока.КодСтроки = 0 Тогда
...
Процедура ОбновитьИнформациюПоЗаказам()
	...
	ПараметрыОбновления.НадписьВсегоЗаказов               = НСтр("ru='Всего распоряжений';uk='Усього розпоряджень'");
...
Процедура ОбновитьИнформациюОбОтклоненииОтРаспоряжений(Товары, НадписьРасхождение, КартинкаРасхождениеЗаказ, КартинкаРасхождениеЗаказы)
	...
	Если КоличествоРасхождений > 0 Тогда
		...
		НадписьРасхождение = СтрЗаменить(НСтр("ru='Строк сверх распоряжения: %КоличествоРасхождений%';uk='Рядків понад розпорядження:%КоличествоРасхождений%'"), "%КоличествоРасхождений%", КоличествоРасхождений);
	Иначе
...
Процедура ОповеститьОбОтвязкеСтрокОтЗаказов()
	...
	ТекстОповещения = НСтр("ru='Строки в списке ""Продукция и материалы"" отвязаны от строк распоряжений.';uk='Рядки в списку ""Продукція і матеріали"" відв''язані від рядків розпоряджень.'");
	ПоказатьОповещениеПользователя(НСтр("ru='Обработка списка ""Продукция и материалы""';uk='Обробка списку ""Продукція та матеріали""'"),, ТекстОповещения);
...
Процедура ОповеститьОбОкончанииОтвязкиСтрок(КоличествоОтработанныхСтрок, СтрокиОтвязаны = Истина)
	...
	Если СтрокиОтвязаны Тогда
		...
		ТекстОповещения = НСтр("ru='В документе от распоряжений отвязано строк (%%Количество%%).';uk='У документі від розпоряджень відв''язано рядків (%%Количество%%).'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийОшибкуТекст);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Распоряжение не заполнено';uk='Розпорядження не заповнено'"));
	#КонецОбласти
...
Процедура УстановитьЗаголовкиПриИзмененииРеквизитов(СтруктураИзмененныхРеквизитов)
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ХозяйственнаяОперация") ИЛИ ВсеРеквизиты Тогда
		...
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства
			...
			Элементы.ТоварыПодобратьТовары.Заголовок = НСтр("ru='Подобрать продукцию';uk='Підібрати продукцію'");
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства
			...
			Элементы.ТоварыПодобратьТовары.Заголовок = НСтр("ru='Подобрать материалы';uk='Підібрати матеріали'");
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ДвижениеПродукцииИМатериалов.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
Процедура НастроитьКомандуПодобратьПоЗаказамОрдерам()
	...
	Если ИспользоватьРаспоряжения Тогда
		...
		Если ОрдернаяСхема Тогда
			...
			Элементы.ТоварыПодобратьТоварыПоЗаказамОрдерам.Заголовок = НСтр("ru='Подобрать товары по распоряжениям/ордерам';uk='Підібрати товари за розпорядженнями/ордерами'");
			
		Иначе
			
			Элементы.ТоварыПодобратьТоварыПоЗаказамОрдерам.Заголовок = НСтр("ru='Подобрать товары по распоряжениям';uk='Підібрати товари за розпорядженнями'");
			
		КонецЕсли;
		
	Иначе
		
		Элементы.ТоварыПодобратьТоварыПоЗаказамОрдерам.Заголовок = НСтр("ru='Подобрать товары по ордерам';uk='Підібрати товари по ордерах'");
...
Процедура ОкруглитьДоЦелойУпаковкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ТекстОповещения = НСтр("ru='В выбранных строках выполнено округление до целых единиц.';uk='У вибраних рядках виконано округлення до цілих одиниць.'");
	ПоказатьОповещениеПользователя(НСтр("ru='Выполнено округление';uk='Виконано округлення'"),, ТекстОповещения, БиблиотекаКартинок.Успешно32);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПризнакЦеховаяКладовая(Форма, ИмяПоляСклад, СтруктураДействий = Неопределено)
	
	ИмяПоляЦеховаяКладовая = ИмяПоляСклад + "ЦеховаяКладовая";
	
	ЗначениеСклад = Форма["Объект"][ИмяПоляСклад];
	ЗначениеЦеховаяКладоваяДоИзменения = Форма[ИмяПоляЦеховаяКладовая];
	
	ЗначениеЦеховаяКладовая = 
		?(ТипЗнч(ЗначениеСклад) <> Тип("СправочникСсылка.Склады") Или Не ЗначениеЗаполнено(ЗначениеСклад),
			Ложь, ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ЗначениеСклад, "ЦеховаяКладовая"));
	
	Форма[ИмяПоляЦеховаяКладовая] = ЗначениеЦеховаяКладовая;
...
Функция ТоварыЗаполнитьЦеныПоВидуЦенСервер()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='В таблице ""Товары и работы"" не выбрано ни одной строки. Цены не могут быть заполнены.';uk='У таблиці ""Товари і роботи"" не вибрано жодного рядка. Ціни не можуть бути заповнені.'");
	...
	Если Не ЗначениеЗаполнено(Объект.ВидЦены) Тогда
		...
		ТекстСообщения = НСтр("ru='Не заполнено поле ""Вид цены"".';uk='Не заповнене поле ""Вид ціни"".'");
