...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru='Документы производства (к оформлению)';uk='Документи виробництва (до оформлення)'");
...
Процедура СостояниеВыполнения(Команда)
	...
	Иначе
		...
		Если СписокДокументов.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выполнение команды возможно только для заказов материалов в производство.';uk='Виконання команди можливо тільки для замовлень матеріалів у виробництво.'"));
			Возврат;
		КонецЕсли;
...
Функция ОписаниеОперацийИТиповДокументов()
	...
	СвойстваПоУмолчанию.ЗаголовокРабочегоМеста       = НСтр("ru='Внутренние документы (движения продукции и материалов)';uk='Внутрішні документи (рухи продукції і матеріалів)'");
	...
	Если ДоступныеОперацииДокумента.НайтиПоЗначению(ТекущаяОперация) <> Неопределено Тогда
		...
		Строка.ХозяйственнаяОперация = ТекущаяОперация;
		ЗаполнитьЗначенияСвойств(Строка, СвойстваПоУмолчанию);
		Строка.ЗаголовокФормыПереоформления = НСтр("ru='Переоформление передачи в кладовую по выбранным распоряжениям';uk='Переоформлення передачі в комору за вибраними розпорядженням'");
	...
	Если ДоступныеОперацииДокумента.НайтиПоЗначению(ТекущаяОперация) <> Неопределено Тогда
		...
		Строка.ХозяйственнаяОперация = ТекущаяОперация;
		ЗаполнитьЗначенияСвойств(Строка, СвойстваПоУмолчанию);
		Строка.ЗаголовокФормыПереоформления = НСтр("ru='Переоформление передачи в производство по выбранным распоряжениям';uk='Переоформлення передачі у виробництво за вибраними розпорядженням'");
	...
	Если ДоступныеОперацииДокумента.НайтиПоЗначению(ТекущаяОперация) <> Неопределено Тогда
		...
		Строка.ХозяйственнаяОперация = ТекущаяОперация;
		ЗаполнитьЗначенияСвойств(Строка, СвойстваПоУмолчанию);
		Строка.ЗаголовокФормыПереоформления = НСтр("ru='Переоформление возврата из производства по выбранным распоряжениям';uk='Переоформлення повернення з виробництва за вибраними розпорядженням'");
	...
	Если ДоступныеОперацииДокумента.НайтиПоЗначению(ТекущаяОперация) <> Неопределено Тогда
		...
		Строка.ХозяйственнаяОперация = ТекущаяОперация;
		ЗаполнитьЗначенияСвойств(Строка, СвойстваПоУмолчанию);
		Строка.ЗаголовокФормыПереоформления = НСтр("ru='Переоформление передачи из производства по выбранным распоряжениям';uk='Переоформлення передачі з виробництва за вибраними розпорядженням'");
...
Процедура СформироватьТекстыЗапросов()
	...
	Для каждого КЗ Из Состояния Цикл
		...
		ИначеЕсли КЗ.Ключ = "СостоянияПередачИзПроизводства" Тогда
			...
			|	//ПолеНоменклатура_СостоянияПередачИзПроизводства.Номенклатура   КАК Номенклатура,
			|	//ПолеНоменклатура_СостоянияПередачИзПроизводства.Характеристика КАК Характеристика,
