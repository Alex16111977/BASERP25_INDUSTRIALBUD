...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.ПоступлениеПоЗаказам
		...
		Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
			ТекстСообщения = НСтр("ru='Уточните склад поступления и заполните материалы с помощью команды ""Заполнить по заказам""';uk='Уточніть склад надходження і заповніть матеріали за допомогою команди ""Заповнити за замовленнями""'");
		Иначе
			ТекстСообщения = НСтр("ru='Уточните склад поступления и заполните материалы с помощью команды ""Заполнить по заказу""';uk='Уточніть склад надходження і заповніть матеріали за допомогою команди ""Заповнити за замовленнями""'");
		КонецЕсли;
...
Процедура СкладПриИзменении(Элемент)
	...
	Если Склад <> Объект.Склад Тогда
		...
		Если НеобходимВопросОбОчисткеСклада() Тогда
			...
			ТекстВопроса = НСтр("ru='Строки по заказу, в которых склад не входит в группу складов ""%Склад%"" будут удалены из списка ""Сырье и материалы"". Продолжить?';uk='Рядки на замовлення, в яких склад не входить в групу складів ""%Склад%"" будуть вилучені зі списку ""Сировина і матеріали"". Продовжити?'");
...
Процедура ЗаполнитьСкладВВыделенныхСтроках(Команда)
	...
	ЕстьВозможностьУказатьСклад = СкладыКлиент.ПроверитьВозможностьЗаполненияСкладовВТабличнойЧасти(
		Объект,
		Объект.Товары, НСтр("ru='Сырье и материалы';uk='Сировина й матеріали'"),
		ВыделенныеСтроки);
