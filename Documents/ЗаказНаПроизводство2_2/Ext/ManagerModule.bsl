...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'");
...
Процедура ПереместитьДокументВОчереди(ТекущийЗаказ, Направление) Экспорт
	...
	Исключение
		ШаблонОшибки = НСтр("ru='Не удалось заблокировать %Документ%. %ОписаниеОшибки%';uk='Не вдалося заблокувати %Документ%. %ОписаниеОшибки%'");
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Исключение
			...
			ШаблонОшибки = НСтр("ru='Не удалось записать %Документ%. %ОписаниеОшибки%';uk='Не вдалося записати %Документ%. %ОписаниеОшибки%'");
...
//
// Параметры:
//  ДанныеЗамены - Массив из Структура - содержит структуру параметров замены:
...
Функция ЗаменитьСпецификации(ДанныеЗамены, ДоступнаЧастичнаяЗамена = Ложь) Экспорт
	...
	Заказы = Новый Массив;//Массив из ДокументСсылка.ЗаказНаПроизводство2_2 -
	Замены = Новый Соответствие;
	
	Для каждого Данные Из ДанныеЗамены Цикл
		
		Замена = СтруктураЗаменыСпецификации();
		ЗаполнитьЗначенияСвойств(Замена, Данные);
		
		Если Замены[Данные.Заказ] <> Неопределено Тогда
			ЗначениеКлюча = Замены[Данные.Заказ];// Массив из см. СтруктураЗаменыСпецификации - 
			ЗначениеКлюча.Добавить(Замена);
		Иначе
	...
	Для каждого Заказ Из Заказы Цикл
		...
		Попытка
			...
			Если ЗаказОбъект = Неопределено ИЛИ ЗаказЗапланирован(Заказ) Тогда
				ВызватьИсключение НСтр("ru='Заказ на производство запланирован (сформированы этапы).';uk='Замовлення на виробництво заплановано (сформовані етапи).'");
			КонецЕсли;
			...
			Для каждого Строка Из ЗаказОбъект.Продукция Цикл
				...
				Для Индекс = -КоличествоЗамен + 1 По 0 Цикл
					...
					Замена = ЗаменыЗаказа[-Индекс];// см. СтруктураЗаменыСпецификации
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось обработать документ: %1 по причине: %2';uk='Не вдалося обробити документ: %1 з причини: %2'"),
								Заказ,
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если Не ДополнительныеПараметры = Неопределено
		...
		ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к. заказ отработан не полностью';uk='У документа %Документ% статус ""%Статус%"" не встановлено, тому що замовлення не відпрацьовано в повному обсязі'");
	...
	Если ВыборкаПроверки.ЕстьЗатратыПослеВыпуска Тогда
		...
		ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к. есть затраты, для которых нет выпуска продукции позже их отнесения.';uk='У документа %Документ% статус ""%Статус%"" не встановлено, тому що є витрати, для яких немає випуску продукції пізніше їх віднесення.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ВыборкаПроверки.Ссылка);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Статус%", ВыборкаПроверки.ПредставлениеНовогоСтатуса);
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ВыборкаПроверки.Ссылка);
		Отказ = Истина;
...
Функция ПредставлениеЗаказа(Ссылка, Префикс = Неопределено) Экспорт
	...
	ШаблонПрефикс = ?(Префикс <> Неопределено, Префикс, НСтр("ru='Заказ';uk='Замовлення'"));
	Шаблон = ШаблонПрефикс + ?(ПустаяСтрока(ШаблонПрефикс),""," ") + НСтр("ru='№ %1 от %2';uk='№ %1 від %2'");
...
Функция ПолноеПредставлениеЗаказа(Ссылка, Префикс = Неопределено) Экспорт
	...
	ШаблонПрефикс = ?(Префикс <> Неопределено, Префикс, НСтр("ru='Заказ';uk='Замовлення'"));
	Шаблон = ШаблонПрефикс + ?(ПустаяСтрока(ШаблонПрефикс),""," ") + НСтр("ru='№ %1 от %2, дата потребности %3';uk='№ %1 від %2, дата потреби %3'");
...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'",КодОсновногоЯзыка);
...
//
// Возвращаемое значение:
//  Структура - данные полей партии производства
//
Функция ПоляПартии(Объект) Экспорт
    ...
    ПоляПартии.НалоговоеНазначение = Объект.НалоговоеНазначение;
...
Функция ТекстыЗапросаНоменклатураКПроизводству(СоздатьВременнуюТаблицу, ИмяВременнойТаблицы = "ВтНоменклатураКПроизводству", ЕстьОтборПоПериоду = Ложь, ТекстОтбораПоИзмерениям = "")
	...
	|
	|	ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ТребуетсяОборот
	|		- ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ЗапланированоОборот
	|		- ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ПереданоКПроизводствуОборот
	|		- ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ПроизводитсяОборот
	|		- ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ПроизведеноОборот
	|		- ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ТребуетсяОборот
	|			* (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100) КАК ТребуетсяПроизводствоСУчетомДопустимогоОтклонения
	|
	...
	|		&КонецПериода,
	|		,
	|		&ОтборПоИзмерениям) КАК ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|
	|		ИМЕЮЩИЕ
	|			СУММА(ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ТребуетсяРазборкаОборот)
	|				- ВЫБОР
	|					КОГДА СУММА(ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ЗапланированаРазборкаОборот)
	|							- СУММА(ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ГотовКРазборкеОборот) > 0
	|						ТОГДА СУММА(ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ЗапланированаРазборкаОборот)
	|								- СУММА(ОбеспечениеПроизводственныхПроцессовВРазрезеСкладов.ГотовКРазборкеОборот)
	|					ИНАЧЕ 0
	|				КОНЕЦ > 0)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			КОНЕЦ > 0
	|
	|//ИНДЕКСИРОВАТЬ ПО Распоряжение,Номенклатура,Характеристика,Назначение,Этап,СписатьНаРасходы
	|";
	
	Если СоздатьВременнуюТаблицу Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ПОМЕСТИТЬ",     "ПОМЕСТИТЬ");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ИНДЕКСИРОВАТЬ", "ИНДЕКСИРОВАТЬ");
...
Функция ТекстЗапросаВТНезавершенныеЗаказы() Экспорт
	...
	|				,
	|				,
	|				(Распоряжение,Номенклатура,Характеристика,Назначение) В(
	|					ВЫБРАТЬ
...
Функция ТекстЗапросаВтСпособыПолученияМатериалов2_2() Экспорт
	...
	Возврат
		...
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Распоряжение, Номенклатура, Характеристика";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаказНаПроизводство") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"ЗаказНаПроизводство",
			НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'"),
			СформироватьПечатнуюФормуЗаказНаПроизводство(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			"Документ.ЗаказНаПроизводство2_2.ПФ_MXL_ЗаказНаПроизводство",
			, 
			Истина);
...
Функция СформироватьПечатнуюФормуЗаказНаПроизводство(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаказНаПроизводство2_2.ПФ_MXL_ЗаказНаПроизводство", КодЯзыкаПечать);
	...
	Пока ВыборкаШапка.Следующий() Цикл
		...
		Область = Макет.ПолучитьОбласть("Заголовок"); // см. АналогиМатериалов.СлужебнаяСтруктураТабличныйДокумент
		
		Область.Параметры.ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
			ВыборкаШапка,
			СтрШаблон(НСтр("ru='Заказ на производство (%1)';uk='Замовлення на виробництво (%1)'",КодЯзыкаПечать), НРег(МультиязычностьУкр.ПолучитьЛокализованноеПредставление(ВыборкаШапка.ТипПроизводственногоПроцесса, КодЯзыкаПечать))),КодЯзыкаПечать);
		...
		Если ВыборкаПродукция.НайтиСледующий(Новый Структура("Ссылка", ВыборкаШапка.Ссылка)) Тогда
			...
			Если ВыводитьКоды Тогда
				...
				ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаТаблицы_СКодом"); // см. АналогиМатериалов.СлужебнаяСтруктураТабличныйДокумент
				
			Иначе
				
				ОбластьШапка = Макет.ПолучитьОбласть("ШапкаТаблицы_БезКода");
				ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаТаблицы_БезКода"); // см. АналогиМатериалов.СлужебнаяСтруктураТабличныйДокумент
		
			КонецЕсли;
			
			ОбластьШапка.Параметры.ИмяКолонкиНоменклатура = ПроизводствоКлиентСервер.ЗаголовокТабличнойЧастиПоТипуПроцесса(
				ВыборкаШапка.ТипПроизводственногоПроцесса, КодЯзыкаПечать);
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством") Тогда
		ТекстГиперссылки = НСтр("ru='Заказы на производство 2.5';uk='Замовлення на виробництво 2.5'");
	Иначе
		ТекстГиперссылки = НСтр("ru='Заказы на производство';uk='Замовлення на виробництво'");
	КонецЕсли;
...
//Возвращает структуру замены спецификации
//
// Возвращаемое значение:
//  Структура - содержит:
...
//
Функция СтруктураЗаменыСпецификации()

	Возврат Новый Структура("НомерСтроки, Спецификация, НоваяСпецификация");
