...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
    ...
    ПараметрыЗаполнения = Документы.ЗаказНаПроизводство2_2.ПараметрыЗаполненияНалоговогоНазначения(ЭтотОбъект);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыОкругления = Документы.ЗаказНаПроизводство2_2.ПараметрыТЧДляОкругления();
...
Процедура ЗаполнитьПоЗаказуДавальца(ЗаказДавальца)
	...
	// Заполнение шапки
    РеквизитыШапки = "Приоритет, ТипПроизводственногоПроцесса, Организация, НалоговоеНазначение, НаправлениеДеятельности, Партнер, Договор";
...
Процедура ВыполнитьКонтрольЗапланированныхЭтапов(Отказ, УдалениеПроведения = Ложь)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если УдалениеПроведения Тогда
			ТекстШаблона = НСтр("ru='Не удалось сделать непроведенным заказ на производство № %1 от %2';uk='Не вдалося зробити непроведеним замовлення на виробництво № %1 від %2'");
		Иначе
			ТекстШаблона = НСтр("ru='Не удалось провести заказ на производство № %1 от %2';uk='не вдалося провести замовлення на виробництво № %1 від %2'");
		КонецЕсли;
		
		ТекстШаблона = ТекстШаблона + НСтр("ru=', т.к. по заказу запланированы этапы.';uk=', тому що за замовленням заплановані етапи.'");
...
Процедура ПроверитьЗаполнениеТаблицыПродукция(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Если Не ЗначениеЗаполнено(Продукция) Тогда
		...
		Шаблон = НСтр("ru='Не заполнена табличная часть ""%1""';uk='Не заповнена таблична частина ""%1""'");
	...
	ШаблонАдреса = НСтр("ru='в строке %1 списка ""%2""';uk='в рядку %1 списку ""%2""'");
...
|При включеній опції ""Заборонити оформлення операцій з імпортними товарами без номерів ВМД"" вибір такої номенклатури заборонений.'");
	...
	Для каждого Строка Из Продукция Цикл
		...
		Если Строка.Номенклатура.Пустая() Тогда
			...
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""Номенклатура""';uk='Не заповнена колонка ""Номенклатура""'");
		...
		Если ХарактеристикиИспользуются И РеквизитыНоменклатуры[Строка.Номенклатура].ХарактеристикиИспользуются
			...
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""Характеристика""';uk='Не заповнена колонка ""Характеристика""'");
		...
		Если ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.Разборка
			...
			Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья
				...
				ТекстСообщения = НСтр("ru='Не заполнена колонка ""Назначение""';uk='Не заповнена колонка ""Призначення""'");
		...
		Если РеквизитыНоменклатуры[Строка.Номенклатура].ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Работа Тогда
			...
			Если Строка.СписатьНаРасходы Тогда
				...
				Если Строка.Подразделение.Пустая() Тогда
					...
					ТекстСообщения = НСтр("ru='Не заполнена колонка ""Подразделение""';uk='Не заповнена колонка ""Підрозділ""'");
			...
			Иначе	
				...
				Если Строка.Склад.Пустая() Тогда
					...
					ТекстСообщения = НСтр("ru='Не заполнена колонка ""Склад""';uk='Не заповнена колонка ""Склад""'");
		...
		Иначе
			...
			Если ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.Сборка
				...
				ТекстСообщения = НСтр("ru='Не допускается указание работы';uk='Не допускається зазначення роботи'");
				Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Продукция", Строка.НомерСтроки, "Номенклатура");
		...
		Если Строка.Отменено И Не ЗначениеЗаполнено(Строка.ПричинаОтмены) Тогда
			...
			ТекстСообщения = НСтр("ru='Необходимо указать причину отмены';uk='Необхідно вказати причину скасування'");
		...
		Если Строка.Количество = 0 И Не Строка.Отменено Тогда
			...
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""Количество""';uk='Не заповнена колонка ""Кількість""'");
		...
		Если ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций
			...
			ТекстСообщения = НСтр("ru='Не заполнена колонка ""Доля стоимости""';uk='Не заповнена колонка ""Частка вартості""'");
