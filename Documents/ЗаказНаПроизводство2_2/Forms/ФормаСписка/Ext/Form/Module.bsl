...
Процедура УстановитьСтатусКПроизводству(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке заказов будет установлен статус ""К производству"". Продолжить?';uk='У виділених в списку замовлень буде встановлено статус ""До виробництва"". Продовжити?'");
...
Процедура УстановитьСтатусКПроизводствуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияУТКлиент.ОповеститьПользователяОбУстановкеСтатуса(Элементы.Список, КоличествоОбработанных, ВыделенныеСтроки.Количество(), НСтр("ru='К производству';uk='До виробництва'"));
