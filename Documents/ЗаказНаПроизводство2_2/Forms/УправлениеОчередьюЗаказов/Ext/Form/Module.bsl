...
Процедура ПередатьЭтапыКВыполнению(Команда)
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru='Передача этапов к выполнению';uk='Передача етапів до виконання'"),
...
Процедура ПометитьНаУдалениеЭтапыПроизводства(Команда)
	...
	Если ВыбранныеЗаказы.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ru='Пометить на удаление все этапы выбранных заказов?';uk='Помітити на вилучення всі етапи вибраних замовлень?'");
	Иначе
...
Процедура ОпределитьПараметрыИЗапланироватьЗавершение(Результат, Распоряжения) Экспорт
	...
	ЗапуститьФоновоеЗадание(
		...
		НСтр("ru='Планирование графика производства';uk='Планування графіка виробництва'"),
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ИнформационныйТекст")
		...
		Если ЗначениеЗаполнено(Форма.ТипПроизводственногоПроцесса) Тогда
			...
			ТекстСтроки = СтрШаблон(НСтр("ru='Тип производственного процесса: ""%1""';uk='Тип виробничого процесу: ""%1""'"), Форма.ТипПроизводственногоПроцесса);;
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстСтроки));
			МассивСтрок.Добавить(" (");
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='отключить';uk='відключити'"),,,, "Отключить_ТипПроизводственногоПроцесса"));
			МассивСтрок.Добавить(")");
	...
	Если СтруктураРеквизитов.Свойство("ДопПанель")
		...
		Элементы.ДекорацияОтобратьЗаказы.Заголовок = 
				?(Форма.ДопПанельСвернута, НСтр("ru='Отобрать';uk='Відібрати'"), 
					НСтр("ru='Отобрать заказы, для которых требуется';uk='Відібрати замовлення, для яких потрібно'"));
		
		Элементы.ДекорацияОтобратьЗаказыПродолжение.Заголовок = 
				?(НЕ Форма.ДопПанельСвернута, НСтр("ru='Или по которым';uk='Або за якими'"), "");
...
Процедура ОбновитьГиперссылки(Форма)
	...
	Элементы.ФормирующиесяЗаказы.Заголовок = СтрШаблон(НСтр("ru='Формирующиеся заказы (%1)';uk='Замовлення, що формуються (%1)'"), КоличествоЗаказовСтрокой);
	Элементы.ФормирующиесяЗаказыСвернутая.Заголовок = СтрШаблон(НСтр("ru='(%1)';uk='(%1)'"), КоличествоЗаказовСтрокой);
