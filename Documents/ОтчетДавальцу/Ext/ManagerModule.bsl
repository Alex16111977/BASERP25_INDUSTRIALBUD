...
Функция КлючевыеПоляШапкиРаспоряжения() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтчетДавальцуПоНесколькимЗаказам") Тогда
		Возврат "Партнер, Контрагент, Договор, Организация, Сделка, Валюта,
            |НаправлениеДеятельности, Подразделение, НалоговоеНазначение, ЦенаВключаетНДС, ПорядокРасчетов,
...
Процедура РассчитатьСостояние(Знач ОтчетДавальцу, Знач Договор, СостояниеРасчетов) Экспорт
	...
	Если  ЗначениеЗаполнено(ОтчетДавальцу) И ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСКлиентами) Тогда
		...
		Запрос = Новый Запрос("
			...
			|	ИСТИНА
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	РегистрНакопления.РасчетыСКлиентами.Остатки(КОНЕЦПЕРИОДА(&ТекущаяДата, ДЕНЬ), ОбъектРасчетов.Объект = &РасчетныйДокумент) КАК РасчетыСКлиентамиОстаткиНаДатуАктуальности
			|ПО
			|	ИСТИНА
			|ГДЕ
...
Функция ПараметрыВзаиморасчеты() Экспорт
	...
	СтруктураПараметров.НакладнаяПоЗаказам              = ИСТИНА;
...
Функция ТекстЗапросаВтУслуги(Запрос, ТекстыЗапроса) Экспорт
	...
	|		И ЕСТЬNULL(ТаблицаУслуги.ЗаказДавальца.Назначение, &НазначениеБезЗаказа) = Аналитики.Назначение
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
    ...
    |	&НалоговоеНазначение							КАК НалоговоеНазначениеСписания,
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|
	|	ВЫБОР
	|		КОГДА ТаблицаУслуги.ЗаказДавальца = ЗНАЧЕНИЕ(Документ.ЗаказДавальца.ПустаяСсылка)
	|			ТОГДА &Ссылка
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОМатериалах") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"ОтчетОМатериалах",
			НСтр("ru='Отчет о материалах';uk='Звіт про матеріали'"),
			СформироватьПечатнуюФормуОтчетОМатериалах(
                МассивОбъектов, 
                ОбъектыПечати,
                ,
                ПараметрыВывода
...
Функция ПолучитьДанныеДляПечатнойФормыАктОбОказанииУслуг(ПараметрыПечати, МассивОбъектов) Экспорт	
    ...
    |		ОтчетДавальцу.СуммаСНДС						КАК СуммаСНДС,
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов) Экспорт
    ...
    |	Документы.БанковскийСчетОрганизации.АдресБанкаДляРасчетовМеждународный КАК АдресБанкаДляРасчетов,
	|	
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		//-- Устарело_Производство21	
			...
			Иначе
                ...
                |			И ДвиженияОтчетаДавальца.НалоговоеНазначение = СебестоимостьТоваров.НалоговоеНазначение
		...
		ПараметрыОбласти.Вставить(
            ...
            ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
            	ДанныеПечати,
                НСтр("ru='Отчет об израсходованных материалах';uk= 'Звіт про витрачені матеріали'", КодЯзыкаПечать),
                КодЯзыкаПечать
            )
        );
		ПараметрыОбласти.Вставить(
...
Функция ПолучитьРезультатЗапросаПоОстаткамУслугДавальцуКОформлению(ДанныеОтбора, Дата, МассивЗаказов = Неопределено)
    ...
    |	И ЗаказыДавальца.СтавкаНДС = &СтавкаНДС
	...
	|	ВЫБРАТЬ
	|		РеквизитыДокумента.Договор КАК Договор,
	|		ТаблицаПродукция.ЗаказДавальца КАК ЗаказДавальца,
	|		ТаблицаПродукция.Номенклатура КАК Номенклатура,
	|		ТаблицаПродукция.Характеристика КАК Характеристика,
	|		0 КАК Количество,
	|		-ТаблицаПродукция.Количество КАК Заказано,
	...
	|	ГДЕ
	|		РеквизитыДокумента.Проведен
	|		И (РеквизитыДокумента.Договор, ТаблицаПродукция.ЗаказДавальца) В(
	|			ВЫБРАТЬ
	|				ТаблицаДляОтбора.Договор КАК Договор,
	|				ТаблицаДляОтбора.Ссылка КАК Ссылка
	|			ИЗ
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("8ed404b5-6a70-4887-ab73-1c06283e939a");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет реквизиты Курс и Кратность документа.
                                   ...
                                   |Заполняет служебный реквизит ДоговорНеОбязателен.
                                   |Заполняет реквизит ОбъектРасчетов.'
                                   |;uk='Заповнює реквізити Курс та Кратність документа.
                                   ...
                                   |Заповнює службовий реквізит ДоговорНеОбязателен.
	...
	Изменяемые.Добавить(Метаданные.Документы.ОтчетДавальцу.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ОтчетДавальцу.ПолноеИмя());
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.ОтчетДавальцу.СгенерироватьОбъектыРасчетов";
	Обработчик.Версия = "2.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("91050b9e-bcd6-418b-8e19-d9b55b5add38");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетДавальцу.ЗарегистрироватьДанныеКГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru='Генерация элементов справочника Объекты расчетов на основании данных документа.';uk='Генерація елементів довідника Об''єкти розрахунків на підставі даних документа.'");
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ОтчетДавальцу";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ОтчетДавальцу";
	...
	|ГДЕ Документ.УдалитьПорядокОплаты = Значение(Перечисление.УдалитьПорядокОплатыПоСоглашениям.ПустаяСсылка)
	...
	|ГДЕ
	|	Операция.УдалитьДатаПлатежа = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	...
	|		ГДЕ
	|			ОтчетДавальцу.Ссылка = ОтчетДавальцуПродукция.Ссылка
	|			И (ОтчетДавальцуПродукция.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ОтчетДавальцуПродукция.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|			ИЛИ ОтчетДавальцуПродукция.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)))
	|	ИЛИ (ОтчетДавальцу.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ОтчетДавальцу.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ОтчетДавальцу.РасшифровкаПлатежа КАК ТЧРасшифровкаПлатежа
	|		ГДЕ
	|			ТЧРасшифровкаПлатежа.Ссылка = ОтчетДавальцу.Ссылка
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.ОтчетДавальцу.ЭтапыГрафикаОплаты КАК ТЧЭтапыГрафикаОплаты
	|		ГДЕ
	|			ТЧЭтапыГрафикаОплаты.Ссылка = ОтчетДавальцу.Ссылка
	|			И ТЧЭтапыГрафикаОплаты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка))
	|
	//Установить значение служебного реквизита "ДоговорНеОбязателен" в истину
	|	ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	...
	|						ПО ОтчетДавальцуПродукция.ЗаказДавальца = Заказы.Ссылка
	|				ГДЕ
	|					ОтчетДавальцуПродукция.Ссылка = ОтчетДавальцу.Ссылка
	|					И ОтчетДавальцу.Проведен
	|					И НЕ ОтчетДавальцу.ДоговорНеОбязателен
	|				ИМЕЮЩИЕ	
	|					КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Заказы.Договор) > 1)
	|	ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	...
	|					И ОтчетДавальцу.Проведен
	|					И НЕ ОтчетДавальцу.ДоговорНеОбязателен
	|					И (Заказы.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|					   ИЛИ Заказы.Договор <> ОтчетДавальцу.Договор))
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ВЫБРАТЬ
	|	ОбновляемыеДанные.Ссылка КАК Ссылка,
	|	ИСТИНА КАК ДоговорНеОбязателен
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу КАК ОтчетДавальцу
	|		ПО ОбновляемыеДанные.Ссылка = ОтчетДавальцу.Ссылка
	|ГДЕ
	|	ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	...
	ТаблицаЗаполненияРеквизитаДоговорНеОбязателен = МассивРезультатов[1].Выгрузить();
	ТаблицаЗаполненияРеквизитаДоговорНеОбязателен.Индексы.Добавить("Ссылка");
	
	ОтборДоговорНеОбязателен = Новый Структура("Ссылка");
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ДокументОбъект.УдалитьДатаПлатежа = Дата(1, 1, 1, 0, 0, 0) Тогда
					ДокументОбъект.УдалитьДатаПлатежа = ДокументОбъект.Дата;
					ОбъектИзменен = Истина;          
				КонецЕсли;
				...
				ЗаполнитьЗначенияСвойств(ОтборДоговорНеОбязателен, Документ);
				МассивСтрок = ТаблицаЗаполненияРеквизитаДоговорНеОбязателен.НайтиСтроки(ОтборДоговорНеОбязателен);
				Если МассивСтрок.Количество() > 0 Тогда
					ЗаполнитьЗначенияСвойств(ДокументОбъект, МассивСтрок[0], "ДоговорНеОбязателен");
					ОбъектИзменен = Истина;
				КонецЕсли;
