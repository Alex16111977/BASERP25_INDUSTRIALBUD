...
//***ERP 
Процедура ОбновитьСписаниеБезналичныхДенежныхСредствПослеОбмена() Экспорт
	
	Если А_ВведенВЕРП Тогда
	...
	Если А_Обработан Тогда
		...
		Возврат;
	Иначе
	  А_ПокупкаПродажаВалюты = Документы.ПокупкаПродажаВалюты.ПустаяСсылка();	
	КонецЕсли;  
	
	НаправлениеДеятельности_ = ?(ЗначениеЗаполнено(А_НаправлениеДеятельностиКазна),А_НаправлениеДеятельностиКазна,А_НаправлениеДеятельности);
	Подразделение_ = ?(ЗначениеЗаполнено(А_ПодразделениеКазна),А_ПодразделениеКазна,А_Подразделение);
	
	ПодразделениеШапка = А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны(НаправлениеДеятельности_,Подразделение_); 
	СтатьяДвиженияДенежныхСредствШапка = ?(ЗначениеЗаполнено(А_СтатьяДвиженияДенежныхСредствКазна),А_СтатьяДвиженияДенежныхСредствКазна,А_СтатьяДвиженияДенежныхСредств);
	
	Подразделение = ПодразделениеШапка;  

	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочаяВыдачаДенежныхСредств 
		Тогда
		
		
		Если А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ДругиеСчетаВБанкеВНациональнойВалюте Тогда  //313
			
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеДенежныхСредствНаДругойСчет;	
			БанковскийСчетПолучатель = А_Субконто1; 
			СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.СпециальныеСчетаВНациональнойВалюте Тогда  //315
			
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеДенежныхСредствНаДругойСчет;	
			БанковскийСчетПолучатель = А_Субконто1; 
			СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
			
			Если НЕ ЗначениеЗаполнено(А_Субконто1) Тогда
				
				ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеДенежныхСредствНаДругойСчет;  
				СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
				ЗапросПДВ = Новый Запрос;  
				ЗапросПДВ.УстановитьПараметр("А_ОрганизацияБухгалтерия",БанковскийСчет.А_ОрганизацияБухгалтерия);
				ЗапросПДВ.УстановитьПараметр("А_СчетБухгалтерский",Планысчетов.Хозрасчетный.СпециальныеСчетаВНациональнойВалюте);
				ЗапросПДВ.Текст =  "ВЫБРАТЬ
				|	БанковскиеСчетаОрганизаций.Ссылка КАК БанковскийСчет
				|ИЗ
				|	Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
				|ГДЕ
				|	БанковскиеСчетаОрганизаций.А_ОрганизацияБухгалтерия = &А_ОрганизацияБухгалтерия
				...
				Выборка = ЗапросПДВ.Выполнить().Выбрать();
				Если Выборка.Следующий() Тогда
					
					БанковскийСчетПолучатель = Выборка.БанковскийСчет;	
				Иначе
					ТекстОшибки = НСтр("ru='Поле ""Банковский счет получатель"" для рахунку 315 (ПДВ) не заполнено';uk='Поле ""Банківський рахунок одержувач"" для рахунку 315 (ПДВ) не заповнено'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,, "БанковскийСчетПолучатель",, Ложь);
		...
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.КраткосрочныеКредитыБанковВНациональнойВалюте //601
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.КраткосрочныеКредитыБанковВИностраннойВалюте //602
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыСДругимиКредиторамиВНациональнойВалюте //6851
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоДолгосрочнымФинансовымИнвестициямУчастиеВКапиталеВалюта //6855
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыСДругимиКредиторамиВИностраннойВалюте //6852 
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыСДругимиДебиторамиВНациональнойВалюте //3771 
			
			ИЛИ 
			(ЗначениеЗаполнено(А_Субконто1) И ТипЗнч(А_Субконто1)=Тип("СправочникСсылка.Контрагенты")
			И ЗначениеЗаполнено(А_Субконто2) И ТипЗнч(А_Субконто2)=Тип("СправочникСсылка.ДоговорыКонтрагентов")) 
			
			Тогда    
			Контрагент = А_Субконто1;
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику;
			СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
			ПартнерПолученныйПоКонтрагенту = ДенежныеСредстваСервер.ПолучитьПартнераПоКонтрагенту(Контрагент);
			Если ЗначениеЗаполнено(А_Субконто2) И ТипЗнч(А_Субконто2) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				Договор = А_Субконто2;
				ГруппаФинансовогоУчета =Договор.ГруппаФинансовогоУчета;
				Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
					СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
					СтрокаРасшифровкаПлатежа.Партнер = ПартнерПолученныйПоКонтрагенту;
					СтрокаРасшифровкаПлатежа.СуммаВзаиморасчетов = СуммаДокумента;
					СтрокаРасшифровкаПлатежа.Сумма = СуммаДокумента;
					СтрокаРасшифровкаПлатежа.ВалютаВзаиморасчетов = Валюта;

					ПараметрыОР = ОбъектыРасчетовСервер.ПолучитьПараметрыОбъектаРасчетов();
					ПараметрыОР.Организация = Организация;
					ПараметрыОР.Партнер     = ПартнерПолученныйПоКонтрагенту;
					ПараметрыОР.Контрагент  = Контрагент;
					ПараметрыОР.Вставить("ПометкаУдаления",Ложь) ;
					ПараметрыОР.Объект = А_Субконто2;
					ПараметрыОР.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком;
					ПараметрыОР.Договор = А_Субконто2;
					ПараметрыОР.Вставить("ТипОбъектаРасчетов", Перечисления.ТипыОбъектовРасчетов.Договор);

					СтрокаРасшифровкаПлатежа.ОбъектРасчетов =
		...
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.АдминистративныеРасходы //92
			Тогда
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.Подразделение = ПодразделениеШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяРасходов = А_Субконто2;    
				СтрокаРасшифровкаПлатежа.ПериодВозникновенияРасхода = НачалоМесяца(Дата); 	
				
			КонецЦикла; 
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ПроцентыЗаКредит //951
			Тогда
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.Подразделение = ПодразделениеШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
				Если ЗначениеЗаполнено(А_Субконто1)
					И ТипЗнч(А_Субконто1)=Тип("СправочникСсылка.А_СтатьиНеоперационныхРасходов") Тогда
					СтрокаРасшифровкаПлатежа.СтатьяРасходов = А_Субконто1.А_СтатьяРасходов;
				Иначе
					СтрокаРасшифровкаПлатежа.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000053"); //Послуги банків
				КонецЕсли;
				СтрокаРасшифровкаПлатежа.ПериодВозникновенияРасхода = НачалоМесяца(Дата); 	
				
			КонецЦикла; 
			
		ИначеЕсли Лев(СокрЛП(А_СчетУчетаБухгалтерия.Код),1) ="9"//9
			Тогда   
			СтатьРасходовСтрока = Неопределено;
			Если ЗначениеЗаполнено(А_Субконто1) И ТипЗнч(А_Субконто1)=Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
				СтатьРасходовСтрока = А_Субконто1;
			ИначеЕсли ЗначениеЗаполнено(А_Субконто2) И ТипЗнч(А_Субконто2)=Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
				СтатьРасходовСтрока = А_Субконто2;
			КонецЕсли;
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.Подразделение = ПодразделениеШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяРасходов = СтатьРасходовСтрока;    
				СтрокаРасшифровкаПлатежа.ПериодВозникновенияРасхода = НачалоМесяца(Дата); 	
		...
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.КассаВНациональнойВалюте //301
			Тогда 
			
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СнятиеНаличныхДенежныхСредств;	
			КассаПолучатель = Справочники.Кассы.НайтиПоНаименованию("Основная");
			СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
		...
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоНДФЛ //6411   
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоДругимНалогам //6415 
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоОбязательнымПлатежам //642
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоПенсионномуОбеспечению //651
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоНалогуНаПрибыль //6413 

			Тогда 
			 Субконто1Шапка = А_Субконто1;
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет;	
			Если А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоНалогуНаПрибыль Тогда
				ТипНалога = Перечисления.ТипыНалогов.НалогНаПрибыль;    
				Субконто1Шапка = Справочники.СтатьиНалоговыхДеклараций.НП15_РИ_Резерв_Банк_Связан
			ИначеЕсли ЗначениеЗаполнено(А_Субконто1) Тогда
				ТипНалога = А_Субконто1.А_ТипНалога;
			КонецЕсли;   
			...
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.Подразделение = ПодразделениеШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
				СтрокаРасшифровкаПлатежа.СчетУчета = А_СчетУчетаБухгалтерия;
				СтрокаРасшифровкаПлатежа.Субконто1 = Субконто1Шапка;
			КонецЦикла; 
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоЗаработнойПлате //661   
			И ЗначениеЗаполнено(А_Субконто1)
			И ТипЗнч(А_Субконто1) = Тип("СправочникСсылка.ФизическиеЛица") 
			И НЕ ЭтоНовый()
			Тогда
			...
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.Подразделение = ПодразделениеШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
				СтрокаРасшифровкаПлатежа.Ведомость=	А_Привилегированный.А_СоздатьВедомостьНаВыплатуЗарплатыВБанкПоСписаниеБезналичныхДенежныхСредств(Ссылка);
			КонецЦикла; 
			
			
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоИсполнительнымДокументам //6853   
			Тогда
			...
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				...
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
				СтрокаРасшифровкаПлатежа.СчетУчета = А_СчетУчетаБухгалтерия;
				СтрокаРасшифровкаПлатежа.Субконто1 = А_Субконто1;
				СтрокаРасшифровкаПлатежа.СтатьяРасходов =  ПланыВидовХарактеристик.СтатьиАктивовПассивов.ПрочиеПассивы;
			КонецЦикла; 
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
		...
		Если А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоНДФЛ //6411   
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоДругимНалогам //6415 
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоОбязательнымПлатежам //642
			ИЛИ А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоПенсионномуОбеспечению //651
			
			Тогда 
			   Если ЗначениеЗаполнено(А_Субконто1) Тогда
			...
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				...
				СтрокаРасшифровкаПлатежа.Субконто1 = А_Субконто1;
			КонецЦикла; 
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыПоНДС  Тогда 
...
Если (А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте //3331
 ...
 Тогда
	...
	ОплатаПоЗаявкам = Истина;

	РасшифровкаПлатежа.Очистить();
	РасшифровкаПлатежаСтрока = РасшифровкаПлатежа.Добавить();
	РасшифровкаПлатежаСтрока.Сумма = СуммаДокумента;
	РасшифровкаПлатежаСтрока.СтатьяДвиженияДенежныхСредств =  СтатьяДвиженияДенежныхСредствШапка;
	
		
		// ================================================================================
	// ДОРАБОТКА: Заполнение А_ПокупкаПродажаВалюты с фильтром по А_ОрганизацияБухгалтерия
	// и приоритетным поиском по обратной ссылке
	// ================================================================================

	ЗапросПокупкаПродажаВалюты = Новый Запрос;
	ЗапросПокупкаПродажаВалюты.УстановитьПараметр("Дата1", НачалоДня(Дата));
	ЗапросПокупкаПродажаВалюты.УстановитьПараметр("Дата2", КонецДня(Дата));
	ЗапросПокупкаПродажаВалюты.УстановитьПараметр("Валюта", БанковскийСчет.ВалютаДенежныхСредств);
	ЗапросПокупкаПродажаВалюты.УстановитьПараметр("СуммаДокумента", СуммаДокумента);
	ЗапросПокупкаПродажаВалюты.УстановитьПараметр("ТекущийДокумент", Ссылка);

	// КЛЮЧЕВОЕ: Фильтр по А_ОрганизацияБухгалтерия (реальная организация из бухгалтерии)
	ЗапросПокупкаПродажаВалюты.УстановитьПараметр("А_ОрганизацияБухгалтерия", А_ОрганизацияБухгалтерия);

	ЭтоВалютаУАХ = (БанковскийСчет.ВалютаДенежныхСредств = Константы.ВалютаРегламентированногоУчета.Получить());

	Если ЭтоВалютаУАХ Тогда
		// Списание гривны = перечисление на покупку валюты
		ЗапросПокупкаПродажаВалюты.УстановитьПараметр("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ПокупкаВалюты);

		ЗапросПокупкаПродажаВалюты.Текст =
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ПВ.А_ОрганизацияБухгалтерия = &А_ОрганизацияБухгалтерия
		|	И НЕ ПВ.ПометкаУдаления
		|	И ПВ.А_ПлатежкаПеречисленияНаПокупкуПродажиВалюты = &ТекущийДокумент
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	Документ.ПокупкаПродажаВалюты КАК ПВ
		|ГДЕ
		|	ПВ.Дата МЕЖДУ &Дата1 И &Дата2
		|	И ПВ.А_ОрганизацияБухгалтерия = &А_ОрганизацияБухгалтерия
		|	И НЕ ПВ.ПометкаУдаления
		|	И ПВ.ХозяйственнаяОперация = &ХозяйственнаяОперация
		|	И ПВ.СуммаГривневая + ПВ.СуммаКомиссионные + ПВ.СуммаПенсионный >= &СуммаДокумента
		|	И ПВ.А_ПлатежкаПеречисленияНаПокупкуПродажиВалюты = ЗНАЧЕНИЕ(Документ.СписаниеБезналичныхДенежныхСредств.ПустаяСсылка)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";

	Иначе
		// Списание валюты = продажа валюты
		ЗапросПокупкаПродажаВалюты.УстановитьПараметр("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ПродажаВалюты);
		...
		|	И НЕ ПВ.ПометкаУдаления
		|	И ПВ.ХозяйственнаяОперация = &ХозяйственнаяОперация
		|	И ПВ.СуммаВалютная = &СуммаДокумента
		|	И ПВ.Валюта = &Валюта
		|	И ПВ.А_ПлатежкаПеречисленияНаПокупкуПродажиВалюты = ЗНАЧЕНИЕ(Документ.СписаниеБезналичныхДенежныхСредств.ПустаяСсылка)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";
	КонецЕсли;
     //Документы.ПокупкаПродажаВалюты.НайтиПоНомеру("ІБ00-0127",Дата)
	// Выполнение с проверкой на дубли
	РезультатЗапроса = ЗапросПокупкаПродажаВалюты.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТаблицаРезультатов = РезультатЗапроса.Выгрузить();

		// Проверка на дубли с одинаковым приоритетом
		ПервыйПриоритет = ТаблицаРезультатов[0].Приоритет;
		КоличествоСПервымПриоритетом = 0;
		Для Каждого СтрокаРез Из ТаблицаРезультатов Цикл
			Если СтрокаРез.Приоритет = ПервыйПриоритет Тогда
				КоличествоСПервымПриоритетом = КоличествоСПервымПриоритетом + 1;
			КонецЕсли;
		КонецЦикла;

		//Если КоличествоСПервымПриоритетом > 1 И ПервыйПриоритет > 1 Тогда
		//	// Найдено несколько документов без обратной ссылки - не заполняем автоматически
		//	ЗаписьЖурналаРегистрации(
		//		"СписаниеБезналичныхДС.ЗаполнениеПВ",
		//		УровеньЖурналаРегистрации.Предупреждение,
		//		Метаданные.Документы.СписаниеБезналичныхДенежныхСредств,
		//		Ссылка,
		//		"Найдено " + КоличествоСПервымПриоритетом + " документов ПокупкаПродажаВалюты на дату " + Дата + " с суммой " + СуммаДокумента + ". Автозаполнение отменено. Организация бухгалтерии: " + А_ОрганизацияБухгалтерия
		//	);
		//Иначе
			А_ПокупкаПродажаВалюты = ТаблицаРезультатов[0].Ссылка;
		//КонецЕсли;
	КонецЕсли;  	
		
	
	Если ЗначениеЗаполнено(А_ПокупкаПродажаВалюты) Тогда
		Если А_ПокупкаПродажаВалюты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПродажаВалюты Тогда
			ВалютаКонвертации = Константы.ВалютаРегламентированногоУчета.Получить();
			СуммаКонвертации  = А_ПокупкаПродажаВалюты.СуммаГривневая;  
			КурсКонвертации   = ?(СуммаДокумента=0,1,СуммаКонвертации/СуммаДокумента); 
			БанковскийСчетПолучатель = ?(ЗначениеЗаполнено(А_ПокупкаПродажаВалюты.А_СчетГривневыйБанк),А_ПокупкаПродажаВалюты.А_СчетГривневыйБанк,А_ПокупкаПродажаВалюты.СчетГривневый);
		Иначе
			ВалютаКонвертации = А_ПокупкаПродажаВалюты.Валюта;  
			СуммаКонвертации  = А_ПокупкаПродажаВалюты.СуммаВалютная;  
			КурсКонвертации   = ?(СуммаКонвертации=0,1,СуммаДокумента/СуммаКонвертации );    
			БанковскийСчетПолучатель = ?(ЗначениеЗаполнено(А_ПокупкаПродажаВалюты.А_СчетВалютныйБанк),А_ПокупкаПродажаВалюты.А_СчетВалютныйБанк,А_ПокупкаПродажаВалюты.СчетВалютный);
		КонецЕсли;  
...
ТЗ = А_Привилегированный.А_ОчиститьТЗ_ПоУсловию(РасшифровкаПлатежа.Выгрузить(), "Сумма", "=", 0);
РасшифровкаПлатежа.Загрузить(ТЗ);
	
	Для каждого СтрокатЗ Из РасшифровкаПлатежа Цикл
		
		СтрокатЗ.Подразделение = ПодразделениеШапка;	
		СтрокатЗ.ЗаявкаНаРасходованиеДенежныхСредств = А_ЗаявкаНаРасходованиеДенежныхСредствКазна; 
		СтрокатЗ.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
...
Процедура ОбновитьСписаниеБезналичныхДенежныхСредств_КазнаПослеОбмена() Экспорт
	
	Если А_ВведенВЕРП Тогда
		
		А_ОбработанКазна = Истина;
		Возврат;
	КонецЕсли;

	Если А_ОбработанКазна Тогда
	...
	Для каждого СтрокатЗ Из РасшифровкаПлатежа Цикл
		...
		СтрокатЗ.ЗаявкаНаРасходованиеДенежныхСредств = А_ЗаявкаНаРасходованиеДенежныхСредствКазна; 
		СтрокатЗ.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
		Если ЗначениеЗаполнено(А_ФинАгент) И ЗначениеЗаполнено(СтрокатЗ.А_Договор)
			и (НЕ СтрокатЗ.А_Договор.А_ЭтоДоговорФинАгента или  А_ФинАгент <>СтрокатЗ.А_Договор.А_ФинАгент) Тогда
			спрДоговор = СтрокатЗ.А_Договор.ПолучитьОбъект();
			спрДоговор.ОбменДанными.Загрузка = Истина;
			спрДоговор.А_ЭтоДоговорФинАгента = Истина;
			спрДоговор.А_ФинАгент = А_ФинАгент;
			спрДоговор.Записать();
		КонецЕсли;
		
		Если ЗначениеЗаполнено(А_ФинАгент) И ЗначениеЗаполнено(СтрокатЗ.А_ЗаказПоставщику)
			 Тогда
			Док_ЗаказПоставщику = СтрокатЗ.А_ЗаказПоставщику.ПолучитьОбъект();
			Док_ЗаказПоставщику.ОбменДанными.Загрузка = Истина;
			Док_ЗаказПоставщику.А_ПроцентФинАгента = А_ПроцентФинАгента;
			Док_ЗаказПоставщику.А_ФинАгент = А_ФинАгент;
			Док_ЗаказПоставщику.А_ДатаВозвратаФинАгента = А_ДатаВозвратаФинАгента;
			Док_ЗаказПоставщику.Записать();
		КонецЕсли;

	КонецЦикла;
	
	А_Распределение.Очистить();
		Если А_РасшифровкаКазна.Количество()>0 Тогда 
			Для каждого СтрокаТЗКазна Из А_РасшифровкаКазна Цикл
				СтрокаРаспределение = А_Распределение.Добавить();
				СтрокаРаспределение.Подразделение = А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны(СтрокаТЗКазна.Направление,СтрокаТЗКазна.Подразделение);
				СтрокаРаспределение.СтатьяДвиженияДенежныхСредств = СтрокаТЗКазна.СтатьяДвиженияДенежныхСредств;
				СтрокаРаспределение.Сумма = СтрокаТЗКазна.Сумма;
				СтрокаРаспределение.НеОтражатьВДДС = СтрокаТЗКазна.НеОтражатьВДДС;
			КонецЦикла; 
	...
	Если ЗначениеЗаполнено(А_ЗаявкаНаРасходованиеДенежныхСредствКазна) Тогда
		ОплатаПоЗаявкам = Истина;
		
				
		ДокЗаявкаНаРасходованиеДенежныхСредств =  А_ЗаявкаНаРасходованиеДенежныхСредствКазна.ПолучитьОбъект();
		ДокЗаявкаНаРасходованиеДенежныхСредств.ГруппаФинансовогоУчета = ГруппаФинансовогоУчета; 
		Если НЕ ЗначениеЗаполнено(ДокЗаявкаНаРасходованиеДенежныхСредств.Валюта) Тогда
		
			ДокЗаявкаНаРасходованиеДенежныхСредств.Валюта = Константы.ВалютаРегламентированногоУчета.Получить();
		КонецЕсли;
		
		ДокЗаявкаНаРасходованиеДенежныхСредств.РасшифровкаПлатежа.Очистить();
		Для каждого СтрокаТЗ Из РасшифровкаПлатежа Цикл
			СтрокаРасшифровкаПлатежа = ДокЗаявкаНаРасходованиеДенежныхСредств.РасшифровкаПлатежа.Добавить();
			СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = ДокЗаявкаНаРасходованиеДенежныхСредств.А_СтатьяДвиженияДенежныхСредств;
			СтрокаРасшифровкаПлатежа.Партнер = СтрокаТЗ.Партнер;
			СтрокаРасшифровкаПлатежа.СуммаВзаиморасчетов = СтрокаТЗ.СуммаВзаиморасчетов;
			СтрокаРасшифровкаПлатежа.Сумма =  СтрокаТЗ.Сумма;
			СтрокаРасшифровкаПлатежа.ВалютаВзаиморасчетов = СтрокаТЗ.ВалютаВзаиморасчетов;
			СтрокаРасшифровкаПлатежа.ОбъектРасчетов =  СтрокаТЗ.ОбъектРасчетов; 
			
			Если НЕ ЗначениеЗаполнено(СтрокаРасшифровкаПлатежа.ВалютаВзаиморасчетов) Тогда
				СтрокаРасшифровкаПлатежа.Валюта = Константы.ВалютаРегламентированногоУчета.Получить();
			КонецЕсли;
			
		КонецЦикла;   
		
		Если ЗначениеЗаполнено(А_ФинАгент) Тогда
			ДокЗаявкаНаРасходованиеДенежныхСредств.А_ПроцентФинАгента = А_ПроцентФинАгента;
			ДокЗаявкаНаРасходованиеДенежныхСредств.А_ФинАгент = А_ФинАгент;
			ДокЗаявкаНаРасходованиеДенежныхСредств.А_ДатаВозвратаФинАгента = А_ДатаВозвратаФинАгента;
			ДокЗаявкаНаРасходованиеДенежныхСредств.Записать();
		КонецЕсли;
		
		

		//Если Проведен Тогда   
		ДокЗаявкаНаРасходованиеДенежныхСредств.УстановитьПометкуУдаления(Ложь);
		// ДокЗаявкаНаРасходованиеДенежныхСредств.Записать(РежимЗаписиДокумента.Проведение);
		//Иначе	 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***
	ОбновитьСписаниеБезналичныхДенежныхСредствПослеОбмена();
	ОбновитьСписаниеБезналичныхДенежныхСредств_КазнаПослеОбмена();
	//***
...
Процедура ПровестиРегистрА_ДенежныеСредстваФинАгенты(Отказ) Экспорт

	Если ЗначениеЗаполнено(А_ФинАгент) И НЕ Отказ Тогда
		...
		Для Каждого СтрокаРП Из РасшифровкаПлатежа Цикл
			...
			Если ЗначениеЗаполнено(СтрокаРП.ОбъектРасчетов) Тогда
			
			СтрокаРП_Договор =  СтрокаРП.ОбъектРасчетов.Договор;	
		...
		Иначе
			...
			Движение.ФинАгент               = А_ФинАгент;
			Движение.ДокументПередачи       = Ссылка;
			Движение.НаправлениеДеятельности = Подразделение.А_НаправлениеДеятельности;
			Движение.Подразделение           = Подразделение;
			Движение.Контрагент             = Контрагент;
			Движение.ДоговорКонтрагента     = СтрокаРП_Договор;
			Движение.ОбъектРасчетов         = СтрокаРП.ОбъектРасчетов;
			Движение.СуммаКомиссии          = Окр(СтрокаРП.Сумма*А_ПроцентФинАгента/100,2);
			Движение.Сумма                  = СтрокаРП.Сумма-Движение.СуммаКомиссии;
