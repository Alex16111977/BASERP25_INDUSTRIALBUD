...
Процедура ОбновитьСписаниеБезналичныхДенежныхСредствПослеОбмена11() Экспорт
...
Если (А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте //3331
 ...
 Тогда
	...
	РасшифровкаПлатежаСтрока.СтатьяДвиженияДенежныхСредств =  СтатьяДвиженияДенежныхСредствШапка;
		...
		ЗапросПокупкаПродажаВалюты.УстановитьПараметр("СуммаГривневая",СуммаДокумента);
        ЗапросПокупкаПродажаВалюты.УстановитьПараметр("СуммаВалютная",СуммаДокумента);
		
		Если БанковскийСчет.ВалютаДенежныхСредств = Константы.ВалютаРегламентированногоУчета.Получить() Тогда
			ЗапросПокупкаПродажаВалюты.УстановитьПараметр("ХозяйственнаяОперация",Перечисления.ХозяйственныеОперации.ПокупкаВалюты);
			
			ЗапросПокупкаПродажаВалюты.Текст =  "ВЫБРАТЬ
			                                    |	ПокупкаПродажаВалюты.Ссылка КАК Ссылка
			                                    |ИЗ
			                                    |	Документ.ПокупкаПродажаВалюты КАК ПокупкаПродажаВалюты
			                                    |ГДЕ
			                                    |	ПокупкаПродажаВалюты.Дата МЕЖДУ &Дата1 И &Дата2
			                                    |	И НЕ ПокупкаПродажаВалюты.ПометкаУдаления
			                                    |	И ПокупкаПродажаВалюты.СуммаГривневая + ПокупкаПродажаВалюты.СуммаКомиссионные >= &СуммаГривневая
			                                    |	И ПокупкаПродажаВалюты.ХозяйственнаяОперация = &ХозяйственнаяОперация
			                                    |
			                                    |ОБЪЕДИНИТЬ ВСЕ
			                                    |
			                                    |ВЫБРАТЬ
			                                    |	ПокупкаПродажаВалюты.Ссылка
			                                    |ИЗ
			                                    |	Документ.ПокупкаПродажаВалюты КАК ПокупкаПродажаВалюты
			                                    |ГДЕ
			                                    |	ПокупкаПродажаВалюты.Дата МЕЖДУ &Дата1 И &Дата2
		...
		Иначе	
			...
			ЗапросПокупкаПродажаВалюты.Текст =  "ВЫБРАТЬ
			                                    ...
			                                    |	ПокупкаПродажаВалюты.Дата МЕЖДУ &Дата1 И &Дата2
			                                    |	И НЕ ПокупкаПродажаВалюты.ПометкаУдаления
			                                    |	И ПокупкаПродажаВалюты.СуммаВалютная = &СуммаВалютная
			                                    |	И ПокупкаПродажаВалюты.Валюта = &Валюта
			                                    |	И ПокупкаПродажаВалюты.ХозяйственнаяОперация = &ХозяйственнаяОперация
			                                    |
			                                    |ОБЪЕДИНИТЬ ВСЕ
			                                    |
		...
		Выборка = ЗапросПокупкаПродажаВалюты.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			А_ПокупкаПродажаВалюты = Выборка.Ссылка;	
		КонецЕсли;  	
