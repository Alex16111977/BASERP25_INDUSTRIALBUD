...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр("ru='Заказ материалов в производство';uk='Замовлення матеріалів у виробництво'",КодОсновногоЯзыка);
...
Процедура ДополнитьСтруктуруПолейТаблицыДляЗаменыНаАналоги(СтруктураПолей) Экспорт
	...
	СтруктураПолей.ДополнительныеПоля.Вставить(
		"Ссылка", Новый ОписаниеТипов("ДокументСсылка.ЗаказМатериаловВПроизводство")
	);
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2")
		...
		КомандаВПроизводство.Представление = КомандаВПроизводство.Представление + " " + НСтр("ru='(производство 2.1)';uk='(виробництво 2.1)'");
		...
		КомандаВКладовую.Представление = КомандаВКладовую.Представление + " " + НСтр("ru='(производство 2.5)';uk='(виробництво 2.5)'");
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.Подразделение) Тогда
		ИнформацияПоПодразделению = НСтр("ru='Передача в ""%Подразделение%""';uk='Передача в ""%Подразделение%""'",ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ЗначениеЗаполнено(Реквизиты.Склад) Тогда
		ИнформацияПоСкладу = НСтр("ru='Передача с ""%Склад%""';uk='Передача з ""%Склад%""'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОтразитьРезерв(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИЛИ ДанныеИсточника.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КОбеспечению)
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|										ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.Закрыт))
	|	И ДанныеИсточника.ВариантОбеспечения В(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
	|											ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	|";
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И ИсточникДанных.ВариантОбеспечения В(
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))";
...
// 	ПараметрыУказанияСерий - см. номенк НоменклатураСервер.ПараметрыУказанияСерий, Неопределено
// Возвращаемое значение:
// 	Строка - обработано строк
//
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Заказы материалов';uk='Замовлення матеріалів'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ЗаказМатериаловВПроизводство";
	НоваяСтрока.Порядок    = "После";
	НоваяСтрока = Исключения.Добавить();
...
#Область ОбработатьДанныеДляГенерацииНазначений
	...
	Обработчик.Комментарий = НСтр("ru='Создает новые назначения связанные с документом ""Заказ материалов в производство""';uk='Створює нові призначення, пов''язані з документом ""Замовлення матеріалів у виробництво""'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
