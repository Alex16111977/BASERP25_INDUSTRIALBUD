...
Процедура ЗаполнитьОкончаниеПоступления(МассивВыделенныхСтрок = Неопределено)
	...
	ПоказатьВводДаты(
		...
		НСтр("ru='Введите дату окончания поступления';uk='Введіть дату закінчення надходження'"),
		ЧастиДаты.Дата);
...
Процедура СостояниеОбеспечения(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"Документ.ЗаказМатериаловВПроизводство.Форма.ФормаДокумента.СостояниеОбеспечения");
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ЗаказМатериаловВПроизводство.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
Процедура ОтметитьНаличиеАналогов()
	...
	ИмяТаблицы = "Товары";
	АналогиМатериалов.ОтметитьНаличиеАналогов(
		Объект[ИмяТаблицы], ПараметрыВыбораАналогов(ИмяТаблицы, Ложь, ОтборСтрокЗаменаНаАналоги(Ложь)));
...
Процедура ОткрытьПодборАналогов()
	...
	АналогиМатериаловКлиент.ОткрытьПодборАналогов(
		ПараметрыВыбораАналогов(ИмяТаблицы,, ОтборСтрокЗаменаНаАналоги(Ложь), Истина), ЭтаФорма);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	Если АдресВХранилище = Неопределено Тогда
		...
		Результат = АналогиМатериалов.ВыполнитьЗаменуНаАналогиАвтоматически(
			Объект[ИмяТЧ], ПараметрыВыбораАналогов,, Истина);
		
	Иначе
		
		Результат = АналогиМатериалов.ВыполнитьЗаменуНаАналоги(
			Объект[ИмяТЧ], АдресВХранилище,, Истина);
	...
	Для каждого ИндексСтроки Из ИндексыНовыхСтрок Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЧ.ВариантОбеспечения) Тогда
			...
			Если СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечение") Тогда
				СтруктураДействий.Удалить("ПроверитьЗаполнитьОбеспечение");	
			КонецЕсли;
			
			Если СтруктураДействий.Свойство("ЗаполнитьВариантОбеспеченияПоДатеОтгрузки") Тогда
				СтруктураДействий.Удалить("ЗаполнитьВариантОбеспеченияПоДатеОтгрузки");	
			КонецЕсли;
		...
		Иначе	
			...
			СтруктураДействий.Вставить(
				"ПроверитьЗаполнитьОбеспечение", Новый Структура("ЗаполнитьОбособленно", Ложь));
			
			СтруктураДействий.Вставить(
				"ЗаполнитьВариантОбеспеченияПоДатеОтгрузки", 
				ОбеспечениеКлиентСервер.СтруктураЗаполненияВариантаОбеспечения(Объект, СтрокаТЧ.НачалоОтгрузки));
	...
	ОбеспечениеСервер.РассчитатьДатуОтгрузкиВСтрокахТабличнойЧасти(
		ИдентификаторыНовыхСтрок, Объект, Объект[ИмяТЧ], "НачалоОтгрузки");
...
Функция ПолучитьДанныеТоваров(ИмяТаблицы, ВыделенныеСтроки, ОтборСтрок, Остатки)
	...
	СтруктураДанных.Вставить("Склад", Объект.Склад);
	
	СтруктураПолей = АналогиМатериалов.СтруктураПолейТаблицыДляЗаменыНаАналоги("Документ.ЗаказМатериаловВПроизводство");
	...
	Если Остатки Тогда
		...
		КорректировкиОстатковАдресВХранилище = АналогиМатериалов.ПоместитьВоВременноеХранилищеКорректировкиОстатков(
			МассивСсылок, Объект[ИмяТаблицы], ИмяТаблицы, Объект.Склад, УникальныйИдентификатор); 
...
&НаКлиентеНаСервереБезКонтекста
Функция ОтборСтрокЗаменаНаАналоги(Автозамена = Истина)
	
	ОтборСтрок = Новый Структура;
	ОтборСтрок.Вставить("Отменено", Новый Массив);
	ОтборСтрок.Отменено.Добавить(Ложь);
	
	Если Автозамена Тогда
		ОтборСтрок.Вставить("ВариантОбеспечения", Новый Массив);
