...
//***ERP
Процедура ОбновитьАктВыполненныхРаботНаПослеОбмена(Док) Экспорт
	...
	Если ЗначениеЗаполнено(Док.ЗаказКлиента) Тогда
		Док.АктПоЗаказам = Истина;
	КонецЕсли;
	
	


     ПодразделениеШапка = А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны( Док.Договор.НаправлениеДеятельности, Док.Договор.Подразделение); 
	...
	Для каждого СтрокаТовары Из Док.Услуги Цикл
		
		КодСтроки = КодСтроки + 1;

		// === СтавкаНДС - КОНВЕРТАЦИЯ из Перечисления в Справочник ===
		Попытка
			Если ТипЗнч(СтрокаТовары.УдалитьСтавкаНДС) = Тип("ПеречислениеСсылка.СтавкиНДС") Тогда
				// Конвертируем перечисление в справочник через типовую функцию
				СтрокаТовары.СтавкаНДС = НДСКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(СтрокаТовары.УдалитьСтавкаНДС);
			Иначе
	...
	КонецЦикла;
		...
		Если ЗначениеЗаполнено(Док.ЗаказКлиента) Тогда
		Для каждого СтрокаТабличнойЧасти Из Док.Услуги Цикл
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьАктВыполненныхРаботНаПослеОбмена(ЭтотОбъект); 
	//***ERP
