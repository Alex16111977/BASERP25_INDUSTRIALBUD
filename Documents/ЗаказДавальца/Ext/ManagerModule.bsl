...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	СинонимДляЗаписиВИнформационнуюБазу = НСтр("ru='Заказ давальца';uk='Замовлення давальця'",КодОсновногоЯзыка);
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ИспользуетсяПроизводство22 И ПравоДоступа("Добавление", МенеджерОбъекта) Тогда
		...
		Если ИспользуетсяПроизводство21 Тогда
			КомандаСоздатьНаОсновании.Представление = НСтр("ru='Заказ на производство 2.5';uk='Замовлення на виробництво 2.5'");
		Иначе
...
//
// Параметры:
//	РеквизитыЗаказа - Объект, Структура - содержит реквизиты шапки документа
...
// Возвращаемое значение:
// 	Строка - количество обработанных строк
//
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И ДанныеИсточника.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно)
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстПланыОплат =
		...
		|			И Таблица.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыКлиентом.АвансДоОбеспечения)
		|			ТОГДА ЛОЖЬ
		...
		|				ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.КОтгрузке),
		|				ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.Закрыт))
		|			И Таблица.ВариантОплаты В (ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыКлиентом.ПредоплатаДоОтгрузки),
	...
	ТекстПланыОтгрузок =
		...
		|	Таблица.СуммаСНДС							КАК Отгружается,
		|	Таблица.Ссылка.Дата							КАК ДатаРегистратора,
		|	Таблица.Ссылка.Номер						КАК НомерРегистратора,
		|	Таблица.Ссылка.ПорядокРасчетов				КАК ПорядокРасчетов,
		|	Таблица.Ссылка.Валюта						КАК ВалютаВзаиморасчетов,
...
Функция ПолучитьДанныеДляПечатнойФормыЗаказаНаУслуги(МассивОбъектов, ПараметрыПечати, ПараметрыВывода) Экспорт
	...
	|	ВЫБОР КОГДА Документы.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС) ТОГДА
	|		ЛОЖЬ
	|	ИНАЧЕ
	|		ИСТИНА
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Документы.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НеНДС)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК УчитыватьНДС,
 	|	Документы.Контрагент КАК Контрагент,
...
Функция СоздатьЗаказНаПроизводствоНаОснованииЗаказаДавальцаПроверкаОснований(ОбъектыОснований, УправлениеПроизводством2_2)
	...
	Если РезультатПроверки.ОбъектыОснований.Количество() = 0 Тогда
		...
		Если ОбъектыОснований.Количество() = 1 Тогда
			РезультатПроверки.ТекстОшибки = НСтр("ru='Команда не может быть выполнена для данного документа';uk='Команда не може бути виконана для даного документа'");
		Иначе
			РезультатПроверки.ТекстОшибки = НСтр("ru='Команда не может быть выполнена для выбранных документов';uk='Команда не може бути виконана для обраних документів'");
		КонецЕсли;
...
Функция ЗаполнитьПоФактическойПотребности2_2(Параметры, ТабЧастьМатериалы)
	...
	Источник = Этапы;
	КолонкаКоличествоЭтап = Источник.Колонки.КоличествоЭтап; // КолонкаТаблицыЗначений
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1d07c39b-ade3-4103-9a83-ad24fce94bf6");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет реквизиты  Спецификация, Цена собственных материалов.
                                   |Заполняет служебный реквизит ""Договор является не обязательным"" для заказов давальцев, созданных до перехода на версию 2.5.4.
                                   ...
                                   |;uk='Заповнює реквізити Специфікація, Ціна власних матеріалів.
                                   |Заповнює службовий реквізит ""Договір є не обов''язковим"" для замовлень давальців, створених до переходу на версію 2.5.4.
                                   ...
                                   |Заповнює реквізит ""Тип виробничого процесу"" за замовчуванням ""виготовлення, складання, виконання робіт"".
	...
	Изменяемые.Добавить(Метаданные.Документы.ЗаказДавальца.ПолноеИмя());
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.ЗаказДавальца.СгенерироватьОбъектыРасчетов";
	Обработчик.Версия = "2.5.4.400";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1d8e000c-7b41-4542-8879-476cf878c3b6");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаказДавальца.ЗарегистрироватьДанныеКГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ЗаказДавальца";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ЗаказДавальца";
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	(Документ.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.ПустаяСсылка)
	...
	|	Документ.ЗаказДавальца КАК ЗаказДавальца
	|ГДЕ
	|	НЕ ЗаказДавальца.УдалитьСтавкаНДС = &ПустаяСтавкаНДС
	|	И ЗаказДавальца.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|	ИЛИ ЗаказДавальца.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ЗаказДавальца.Продукция КАК ЗаказДавальцаПродукция
	|		ГДЕ
	|			ЗаказДавальцаПродукция.ИдентификаторСтроки = """"
	|			И ЗаказДавальцаПродукция.Ссылка = ЗаказДавальца.Ссылка)";
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ЗапросМатериалы.УстановитьПараметр("Ссылка", Документ.Ссылка);
				...
				Если Документ.ТребуетсяУказаниеДоговорНеОбязателен Тогда
				...
				ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(ДокументОбъект.Продукция, ОбъектИзменен);
				#КонецОбласти
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Заказы давальцев';uk='Замовлення давальців'");
