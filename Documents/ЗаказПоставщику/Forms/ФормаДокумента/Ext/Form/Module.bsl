...
Процедура ОткрытьПодборАналогов()
	...
	АналогиМатериаловКлиент.ОткрытьПодборАналогов(ПараметрыВыбораАналогов(ИмяТаблицы), ЭтаФорма);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	ИмяТЧ = "Товары";
	ДополнительныеПоля = "ВидЦеныПоставщика, Подразделение, Склад" 
						 + ?(ПолучитьФункциональнуюОпцию("РазрешитьОбособлениеТоваровСверхПотребности"), ", Назначение", "");
	...
	Для каждого ИндексСтроки Из ИндексыНовыхСтрок Цикл
		...
		СтруктураДействий.Вставить(
			"ПроверитьСопоставленнуюНоменклатуруПоставщика",
			ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПроверкиСопоставленнойНоменклатурыПоставщикаВСтрокеТЧ(
				Объект, НеВыполнятьПроверкуСопоставленнойНоменклатурыПоставщика));
		...
		ИначеЕсли СтрокаТЧ.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда
...
&НаСервере
Функция ПараметрыВыбораАналогов(ИмяТаблицы)
	...
	ДанныеТоваров = ПолучитьДанныеТоваров(ИмяТаблицы);
...
&НаСервере
Функция ПолучитьДанныеТоваров(ИмяТаблицы)
	
	СтруктураДанных = Новый Структура;
	СтруктураДанных.Вставить("Ссылка", Объект.Ссылка);
	
	СтруктураПолей = АналогиМатериалов.СтруктураПолейТаблицыДляЗаменыНаАналоги("Документ.ЗаказПоставщику");
	
	ТоварыАдресВХранилище = ПроизводствоСервер.ПоместитьВоВременноеХранилищеДанныеСтрок(
		ЭтаФорма, "Объект", ИмяТаблицы, Истина, СтруктураПолей, ОтборСтрокЗаменаНаАналоги());			
...
//++ А_Визирование
#Область А_Визирование

// Подменю «Виза» в командной панели: кнопки строятся динамически из доступных
// состояний движка (паттерн Казны СчетНаОплатуПоставщика.ФормаДокумента).

&НаСервере
Процедура А_Виза_УстановитьДоступностьФормы()

	ГруппаКнопок = Элементы.А_Кнопки_Виза;

	Пока ГруппаКнопок.ПодчиненныеЭлементы.Количество() > 0 Цикл
		Элементы.Удалить(ГруппаКнопок.ПодчиненныеЭлементы[0]);
	КонецЦикла;

	Элементы.А_Кнопки_Виза_Заголовок.Заголовок =
		?(ЗначениеЗаполнено(Объект.А_Виза_ТекущаяРоль), Строка(Объект.А_Виза_ТекущаяРоль), НСтр("ru='Виза'"));

	Элементы.А_Виза_Причина.Видимость = НЕ ПустаяСтрока(Объект.А_Виза_Причина);

	// Действия — только по сохранённому заказу (виза ничего не блокирует, форма не гасится).
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Возврат;
	КонецЕсли;

	Состояния = Документы.ЗаказПоставщику.А_Виза_ДоступныеСостояния(Объект.Ссылка).Состояния;

	СоответствиеИмен = Новый Соответствие;
	Для Каждого ЗначениеПеречисления Из Метаданные.Перечисления.А_Визы_Состояния.ЗначенияПеречисления Цикл
		СоответствиеИмен.Вставить(Перечисления.А_Визы_Состояния[ЗначениеПеречисления.Имя], ЗначениеПеречисления.Имя);
	КонецЦикла;
	СоответствиеИмен.Вставить(Перечисления.А_Визы_Состояния.ПустаяСсылка(), "Предварительная");

	СоответствиеКартинок = Новый Соответствие;
	СоответствиеКартинок.Вставить(Перечисления.А_Визы_Состояния.Утвержден,               БиблиотекаКартинок.ВыполненоУспешно);
	СоответствиеКартинок.Вставить(Перечисления.А_Визы_Состояния.УтвержденСКомментариями, БиблиотекаКартинок.ВыполненоУспешно);
	СоответствиеКартинок.Вставить(Перечисления.А_Визы_Состояния.Отклонено,               БиблиотекаКартинок.ОтменитьОперацию);
	СоответствиеКартинок.Вставить(Перечисления.А_Визы_Состояния.Черновик,                БиблиотекаКартинок.РедактироватьФайл);
	СоответствиеКартинок.Вставить(Перечисления.А_Визы_Состояния.Анулировано,             БиблиотекаКартинок.ВыполненоСОшибками);
	СоответствиеКартинок.Вставить(Перечисления.А_Визы_Состояния.ПустаяСсылка(),          БиблиотекаКартинок.ВыполнитьРегламентноеЗаданиеВручную);

	Для Каждого ЭлементСписка Из Состояния Цикл

		ИмяСостояния = СоответствиеИмен.Получить(ЭлементСписка.Значение);
		Если ИмяСостояния = Неопределено Тогда
			Продолжить;
		КонецЕсли;

		ИмяКоманды = А_Виза_КомандаНайтиСоздать("А_Виза_" + ИмяСостояния);
		...
		Картинка = СоответствиеКартинок.Получить(ЭлементСписка.Значение);
...
&НаСервере
Функция А_Виза_КомандаНайтиСоздать(ИмяКоманды, Действие = "А_Виза_Нажатие")
...
&НаКлиенте
Процедура А_Виза_Нажатие(Команда)

	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) ИЛИ Объект.Ссылка.Пустая() Тогда
		Сообщить(НСтр("ru='Сначала запишите заказ, затем визируйте.'"));
		Возврат;
	КонецЕсли;

	Если Модифицированность Тогда
		Сообщить(НСтр("ru='Есть несохранённые изменения. Сначала запишите заказ.'"));
		Возврат;
	КонецЕсли;

	ИмяСостояния = СтрЗаменить(Команда.Имя, "А_Виза_", "");

	ЗапроситьПричину = (ИмяСостояния = "Отклонено"
		ИЛИ ИмяСостояния = "Анулировано"
		ИЛИ ИмяСостояния = "УтвержденСКомментариями");

	Контекст = Новый Структура("ИмяСостояния, Причина", ИмяСостояния, "");
	Если ЗапроситьПричину Тогда
		Подсказка = ?(ИмяСостояния = "УтвержденСКомментариями",
			НСтр("ru='Укажите комментарий'"), НСтр("ru='Укажите причину'"));
		ПоказатьВводСтроки(Новый ОписаниеОповещения("А_Виза_ПослеПричины", ЭтотОбъект, Контекст),
			, Подсказка, 0, Ложь);
	Иначе
		А_Виза_Применить(Контекст);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура А_Виза_ПослеПричины(Строка, Контекст) Экспорт
...
&НаКлиенте
Процедура А_Виза_Применить(Контекст)
	ОчиститьСообщения();
	Обработано = А_Виза_ДействиеСервер(Контекст.ИмяСостояния, Контекст.Причина);
	Если Обработано > 0 Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Визирование'"), , НСтр("ru='Состояние визы изменено.'"));
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция А_Виза_ДействиеСервер(ИмяСостояния, Причина)
	...
	Если НЕ ПустаяСтрока(Причина) Тогда
		Допы.Вставить("Виза_Причина", Причина);
	КонецЕсли;

	Обработано = А_ВизированиеВызовСервера.УстановитьВизуДокументов(Ссылки, ИмяСостояния, Допы);

	// Перечитываем объект формы, чтобы показать новое состояние/ТЧ, и перестраиваем кнопки.
