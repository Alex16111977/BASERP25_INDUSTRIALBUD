...
Процедура ОбновитьЗаказПоставщикуПослеОбмена(Док) Экспорт
	...
	Если ЗначениеЗаполнено(Док.Контрагент) Тогда
		Док.Партнер = ДенежныеСредстваСервер.ПолучитьПартнераПоКонтрагенту(Док.Контрагент);
		Если НЕ ЗначениеЗаполнено(Док.Партнер) Тогда
			Док.Партнер = Справочники.Партнеры.НеизвестныйПартнер;
		КонецЕсли;
	КонецЕсли;   
	
	Если СокрЛП(Док.Валюта.Код)="980" Тогда
	...
	Док.ГруппаФинансовогоУчета = Док.Договор.ГруппаФинансовогоУчета;
...
//++ А_Визирование
// Устанавливает состояние визы. Тонкая обёртка над движком — вся логика в А_ВизированиеСервер.
// Документ НЕ пишется: запись выполняет вызывающий (см. контракт в А_ВизированиеСервер.УстановитьСостояние).
//
// Параметры:
//  Состояние    - ПеречислениеСсылка.А_Визы_Состояния (пустая ссылка = предварительная виза)
//  ДопПараметры - Структура
//
// Возвращаемое значение:
//  Структура - Успех (Булево), ТекстОшибки (Строка)
//
Функция А_Виза_УстановитьСостояние(Состояние, ДопПараметры = Неопределено) Экспорт

	Возврат А_ВизированиеСервер.УстановитьСостояние(ЭтотОбъект, Состояние, ДопПараметры);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		//***ERP
	ОбновитьЗаказПоставщикуПослеОбмена(ЭтотОбъект); 
	//***ERP
	...
	//++ А_Визирование
	// Всё, что ниже, выполняется только для документов, записываемых НЕ обменом.
	// Денормализованная строка "кто сейчас должен подписать" — для колонки в журнале визирования.
	Если ЗначениеЗаполнено(А_Виза_Состояние) Тогда
		А_Виза_ТекущиеВизирующие = А_ВизированиеСервер.ПредставлениеТекущихВизирующих(ЭтотОбъект);
	КонецЕсли;
	// ЗАПРЕТ ОБХОДА ВИЗИРОВАНИЯ включается отдельным этапом — см.
	// _Rarzrabotki\Обработки\ПЛАН_А_ВизированиеЗаказовПоставщику.md, этап 5.
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//++ А_Визирование
	// Сброс визы копии. Обязателен ДО ИнициализироватьДокумент(): та ставит Черновик только при
	// пустом состоянии, а копия унаследовала бы чужую визу (например, Утвержден) вместе с историей.
	А_Виза_Состояние               = Перечисления.А_Визы_Состояния.ПустаяСсылка();
	А_Виза_ТекущаяРоль             = Справочники.А_РолиВизирования.ПустаяСсылка();
	А_Виза_ТекущиеВизирующие       = "";
	А_Виза_Дата                    = Дата(1, 1, 1);
	А_Виза_Причина                 = "";
	А_ПриоритетнаяСхемаВизирования = Справочники.А_СхемыВизирования.ПустаяСсылка();
	А_Визы.Очистить();
	А_ТекущиеВизирующие.Очистить();
	А_ПредварительныеВизы.Очистить();
	//-- А_Визирование
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	//++ А_Визирование
	// Ввод заказа в контур визирования.
	// Врезка стоит именно здесь, а не в ОбработкаЗаполнения: ИнициализироватьДокумент вызывается
	// из ОБОИХ интерактивных путей — из ОбработкаЗаполнения и из ПриКопировании. Если поставить
	// только в ОбработкаЗаполнения, копия заказа приедет с пустой визой и ложно выпадет из контура.
	// При загрузке обменом (КД 2.0) эта процедура не вызывается — движок обмена создаёт документ
	// через СоздатьДокумент() и не вызывает Заполнить(). Поэтому обменный заказ остаётся
	// с пустым А_Виза_Состояние, что и служит признаком "вне контура визирования".
