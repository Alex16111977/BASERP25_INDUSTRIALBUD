...
//++ А_Визирование
// Состояния визы, доступные текущему пользователю для документа.
// Тонкая обёртка над движком — вся логика в А_ВизированиеСервер.
//
Функция А_Виза_ДоступныеСостояния(ДокСсылка, ДопПараметры = Неопределено) Экспорт

	Возврат А_ВизированиеСервер.ДоступныеСостояния(ДокСсылка, ДопПараметры);
