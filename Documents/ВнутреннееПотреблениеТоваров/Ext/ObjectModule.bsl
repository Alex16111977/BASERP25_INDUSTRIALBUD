...
//***ERP
Процедура ОбновитьВнутреннееПотреблениеТоваровНаПослеОбмена(Док) Экспорт
	...
	Если ЗначениеЗаполнено(А_ПроизводственныеУслугиАкт) И ЗначениеЗаполнено(А_ПроизводственныеУслугиАкт.Подразделение) Тогда
		Подразделение = А_ПроизводственныеУслугиАкт.Подразделение;      
	ИначеЕсли ЗначениеЗаполнено(А_Подразделение) Тогда
		Подразделение = А_Подразделение;
	ИначеЕсли ЗначениеЗаполнено(А_ПодразделениеОрганизаций) И ЗначениеЗаполнено(А_ПодразделениеОрганизаций.А_Подразделение) Тогда
		Подразделение = А_ПодразделениеОрганизаций.А_Подразделение;
	КонецЕсли;  
	...
	Для каждого СтрокаТабличнойЧасти Из Док.Товары Цикл
		//Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.КатегорияЭксплуатации)
		//	И ЗначениеЗаполнено(СтрокаТабличнойЧасти.КатегорияЭксплуатации.СпособОтраженияРасходов)
		//	И СтрокаТабличнойЧасти.КатегорияЭксплуатации.СпособОтраженияРасходов.Способы.Количество()>0
		//	И ЗначениеЗаполнено(СтрокаТабличнойЧасти.КатегорияЭксплуатации.СпособОтраженияРасходов.Способы[0].Субконто3)
		//	И ТипЗнч(СтрокаТабличнойЧасти.КатегорияЭксплуатации.СпособОтраженияРасходов.Способы[0].Субконто3) = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
		//	СтрокаТабличнойЧасти.СтатьяРасходов = СтрокаТабличнойЧасти.КатегорияЭксплуатации.СпособОтраженияРасходов.Способы[0].Субконто3;
		//Иначе
		//	СтрокаТабличнойЧасти.СтатьяРасходов =ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000028"); //МШП
		//КонецЕсли;
		СтрокаТабличнойЧасти.СтатьяРасходов =ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000028"); //МШП
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьВнутреннееПотреблениеТоваровНаПослеОбмена(ЭтотОбъект); 
	//***ERP
