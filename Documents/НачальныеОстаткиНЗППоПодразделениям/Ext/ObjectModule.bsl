...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ШаблонСообщения = НСтр("ru='В строке %НомерСтроки% списка ""Затраты"" не заполнено поле ""%ИмяПоля%"".';uk='У рядку %НомерСтроки% списку ""Витрати"" не заповнено поле ""%ИмяПоля%"".'");
	...
	Для Каждого ТекСтрока Из Затраты Цикл
		...
		Если ТекСтрока.ТипЗатрат = Перечисления.ТипыЗатрат.Прочее
			...
			Если Не ЗначениеЗаполнено(ТекСтрока.СтатьяРасходов) Тогда
				...
				СообщениеОбОшибке = СтрЗаменить(ТекстСообщения, "%ИмяПоля%", НСтр("ru='Статья расходов';uk='Стаття витрат'"));
