...
Функция РаспределитьНачальныеОстаткиПоПартиямПроизводства(Ссылка, АдресРезультата = Неопределено) Экспорт
	...
	Для Каждого ТекРасход Из ПрочиеРасходы Цикл
		...
		Для Индекс = 0 По МаксимальныйИндекс Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекРасход,, "Сумма,СуммаРегл");
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	Запрос.УстановитьПараметр("МатериальныеЗатраты",	НСтр("ru='Материальные';uk='Матеріальні'"));
	Запрос.УстановитьПараметр("ВозвратныеОтходы",		НСтр("ru='Возвратные отходы';uk='Зворотні відходи'"));
	Запрос.УстановитьПараметр("Трудозатраты",			НСтр("ru='Трудозатраты';uk='Трудовитрати'"));
	Запрос.УстановитьПараметр("Амортизация",			НСтр("ru='Амортизация';uk='Амортизація'"));
	Запрос.УстановитьПараметр("Прочие",					НСтр("ru='Прочие';uk='Інші'"));
...
Функция ТекстЗапросаТаблицаМатериалыЭтаповПодготовительная()
	...
	|	СУММА(ТаблицаМатериалы.КоличествоУпаковок)	КАК КоличествоУпаковок
	...
	|		ТоварыОрганизаций.Количество										КАК КоличествоУпаковок
	|	ИЗ
	...
	|		-ТаблицаМатериалы.Количество			        КАК КоличествоУпаковок
	|	ИЗ
	|		ЭтапыБазыРаспределения КАК ЭтапыБазыРаспределения
	|	
	...
	|		И НЕ ТаблицаМатериалы.ВидЗапасов.ТипЗапасов В (
...
Функция ТекстЗапросаТаблицаВтМатериалыЭтапов()
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ)) КАК КурсыВалютыЦены
	|	ПО КурсыВалютыЦены.Валюта = ЦеныНоменклатуры.Валюта
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ), Валюта = &ВалютаУпрУчета) КАК КурсыВалюты
	|	ПО ИСТИНА
	|	
	|";
...
Функция ТекстЗапросаТаблицаВтТрудозатратыЭтапов()
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ), Валюта = &ВалютаВидовРабот) КАК КурсыВалютыЦены
	|	ПО ИСТИНА
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ), Валюта = &ВалютаУпрУчета) КАК КурсыВалюты
	|	ПО ИСТИНА
