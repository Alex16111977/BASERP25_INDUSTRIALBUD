...
Процедура ЗаполнитьТипЗатрат(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых необходимо заполнить тип затрат.';uk='Необхідно вибрати рядки, для яких необхідно заповнити тип витрат.'");
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	Если Не Объект.Затраты.Количество() = 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Таблица затрат будет перезаполнена. Продолжить?';uk='Таблиця витрат буде перезаповнена. Продовжити?'");
...
Процедура УстановитьЗаголовки()
	...
	Если ВалютаУправленческогоУчета = ВалютаРегламентированногоУчета Тогда
		Элементы.ЗатратыСумма.Заголовок = НСтр("ru='Сумма';uk='Сума'");
	Иначе
...
Процедура ПоказатьФормуЗагрузкиИзФайла()
	...
	ПараметрыЗагрузки.Заголовок = НСтр("ru='Загрузка остатков затрат НЗП из файла';uk='Завантаження залишків витрат НЗВ з файлу'");
