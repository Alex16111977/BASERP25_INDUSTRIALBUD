...
Функция ДанныеЗаполненияНакладной(МассивСсылок, СвойстваЗаказов = Неопределено) Экспорт
	...
	Иначе
        ...
        |Статус, НаправлениеДеятельности, НалоговоеНазначение");
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения) Экспорт
	...
	ТекстЗапросаРегистраЗаказы = ?(ПараметрыЗаполнения.Свойство("СписокМатериалов"), 
		ТекстЗапросаВтСписокМатериаловЗаполнениеПоЗаказамОрдерам() + ОбщегоНазначенияУТ.РазделительЗапросовВПакете(), "");
...
#Область  ТекстСписаниеТоваровНаРасходы
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Списание материалов в производство';uk='Списання матеріалів у виробництво'",ЯзыкСодержания));
...
#КонецОбласти
	...
	//++ НЕ УТКА
#Область  ТекстПередачаМатериаловДавальцаВПроизводство
...
#Область  ТекстПередачаПолуфабрикатовДавальцаВПроизводствоБаланс
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Передача полуфабрикатов в производство (Дт 23 :: Кт 23)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		НСтр("ru='Передача полуфабрикатов давальца в производство';uk='Передача напівфабрикатів давальця у виробництво'",ЯзыкСодержания));
	
	ТекстыОтражения.Добавить(ТекстЗапроса);

#КонецОбласти
	
#Область ТекстПередачаПолуфабрикатовДавальцаВПроизводствоЗабаланс
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Передача полуфабрикатов давальца в производство (Дт 023 :: Кт 023)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область  ТекстСписаниеТоваровВПроизводстве
	
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Списание материалов в производство (Дт 23 :: Кт 23, 68ОК)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область  ТекстВключениеИсключениеНДСВСтоимостьТоваров
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.Подразделение) Тогда
		ИнформацияПоПодразделению = НСтр("ru='Передача в ""%Подразделение%""';uk='Передача в ""%Подразделение%""'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|	КОНЕЦ КАК РазделУчета,
	|	Строки.КорВидЗапасов КАК ВидЗапасов,
	|	НЕОПРЕДЕЛЕНО КАК НалоговоеНазначение,
	|	ВЫБОР КОГДА &ПартионныйУчетВерсии21 
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользуетсяУправлениеПроизводством21и22") Тогда
		ПредставлениеДела = НСтр("ru='Передачи материалов в производство 2.1';uk='Передачі матеріалів у виробництво 2.1'");
	Иначе
...
Функция СозданиеНаОснованииПроверкаРеквизитовШапки()
    ...
    |		ДокМатериалыИРаботы.Ссылка.НалоговоеНазначение           КАК НалоговоеНазначение,
	|		ИСТИНА                                                   КАК Проведен,
    ...
    |		ДокМатериалыИРаботы.Ссылка.НалоговоеНазначение 
	//-- НЕ УТКА
	|) КАК Набор
	|;
	|
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("9264756f-c709-4584-bcf8-ddc4d7e53432");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "Документ.ПередачаМатериаловВПроизводство,"
	...
	Обработчик.Комментарий = НСтр("ru='В документе ""Передача материалов в производство"" переносит назначение из вида запасов в аналитику учета номенклатуры, перезаполняет реквизит ""СтатусУказанияСерийОтправитель"", если он заполнен некорректно. До завершения обработчика работа с документами не возможна.';uk='У документі ""Передача матеріалів у виробництво"" переносить призначення з виду запасів в аналітику обліку номенклатури, перезаповнює реквізит ""СтатусУказанияСерийОтправитель"", якщо він заповнений некоректно. До завершення обробника робота з документами неможлива.'");
...
#Область Документы_ПередачаМатериаловВПроизводство_СгенерироватьКлючиАналитикиНоменклатуры
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("4a68b9c2-4780-4461-8a67-c576095dd245");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	Обработчик.БлокируемыеОбъекты = "Справочник.КлючиАналитикиУчетаНоменклатуры,"
		+ "РегистрСведений.АналитикаУчетаНоменклатуры";
	Обработчик.Комментарий = НСтр("ru='Генерирует ключи аналитики учета номенклатуры по данным документа ""Передача материалов в производство"". До завершения обработчика работа с документами не возможна.';uk='Генерує ключі аналітики обліку номенклатури за даними документа ""Передача матеріалів у виробництво"". До завершення обробника робота з документами неможлива.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ПередачаМатериаловВПроизводство";
