...
Процедура ПрочиеРасходыСуммаПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьСуммуБезНДС");
	Если ТекущиеДанные.ВариантРаспределенияРасходовРегл = ПредопределенноеЗначение("Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты") Тогда
...
Процедура ПрочиеРасходыЗаполнитьПравилоОтнесенияНаВыпускУпр(Команда)
	...
	ЗаполнитьПравилоОтнесенияНаВыпуск("ПрочиеРасходы", "Упр");
...
Процедура ПрочиеРасходыЗаполнитьПравилоОтнесенияНаВыпускРегл(Команда)
	...
	ЗаполнитьПравилоОтнесенияНаВыпуск("ПрочиеРасходы", "Регл");
...
&НаКлиенте
Процедура ЗаполнитьПравилоОтнесенияНаВыпуск(ИмяТЧ, ВидУчета)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых необходимо заполнить правило отнесения на выпуск.';uk='Необхідно вибрати рядки, для яких необхідно заповнити правило віднесення на випуск.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура МатериалыИРаботыЗагрузитьИзФайла(Команда)
	...
	ЗагрузитьИзФайла("МатериалыИРаботы", НСтр("ru='Материалы и работы';uk='Матеріали та роботи'"));
...
Процедура ТрудозатратыЗагрузитьИзФайла(Команда)
	...
	ЗагрузитьИзФайла("Трудозатраты", НСтр("ru='Трудозатраты';uk='Трудовитрати'"));
...
Процедура ПрочиеРасходыЗагрузитьИзФайла(Команда)
	...
	ЗагрузитьИзФайла("ПрочиеРасходы", НСтр("ru='Прочие расходы';uk='Інші витрати'"));
...
Процедура ЗагрузитьИзФайла(ИмяТЧ, СинонимТЧ)
	...
	Если Не Объект[ИмяТЧ].Количество() = 0 Тогда
		...
		ТекстВопроса = НСтр("ru='Таблица %1 будет перезаполнена. Продолжить?';uk='Таблицю %1 буде перезаповнено. Продовжити?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не указывается>';uk='<не вказується>'"));
...
Процедура УстановитьЗаголовки()
	...
	Если ВалютаРасценокВидовРабот = ВалютаРегламентированногоУчета Тогда
		ЗаголовокСумма = НСтр("ru='Нормативная расценка';uk='Нормативна розцінка'");
	Иначе
		ТекстЗаголовка = НСтр("ru='Нормативная расценка (%1)';uk='Нормативна розцінка (%1)'");
	...
	Если ВалютаРасценокВидовРабот = ВалютаРегламентированногоУчета Тогда
		ЗаголовокСумма = НСтр("ru='Нормативная сумма';uk='Нормативна сума'");
	Иначе
		ТекстЗаголовка = НСтр("ru='Нормативная сумма (%1)';uk='Нормативна сума (%1)'");
		ЗаголовокСумма = СтрШаблон(ТекстЗаголовка, ВалютаРасценокВидовРабот);
	КонецЕсли;
	...
	Если ВалютаУправленческогоУчета = ВалютаРегламентированногоУчета Тогда
		ЗаголовокСумма = НСтр("ru='Расценка';uk='Розцінка'");
	Иначе
		ТекстЗаголовка = НСтр("ru='Расценка (%1)';uk='Розцінка (%1)'");
...
Процедура РазбитьСтрокуТЧ(ИмяТЧ)
	...
	РаботаСТабличнымиЧастямиКлиент.РазбитьСтроку(ЭтаФорма, Элементы[ИмяТЧ], Оповещение, ПараметрыРазбиенияСтроки);
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.НачальныеОстаткиНЗППоПартиямПроизводства.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
Процедура ПоказатьФормуЗагрузкиИзФайла(ДополнительныеПараметры)
	...
	ЗаголовокФормыЗагрузки	= НСтр("ru='Загрузка таблицы ""%1"" из файла';uk='Завантаження таблиці ""%1"" з файлу'");
