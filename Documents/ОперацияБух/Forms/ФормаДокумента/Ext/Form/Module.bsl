...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВестиУУНаПланеСчетовХозрасчетный Тогда
		Элементы.ГруппаХозрасчетный.Заголовок = НСтр("ru='Бухгалтерский, управленческий и налоговый учет';uk='Бухгалтерський, управлінський і податковий облік'");
	КонецЕсли;
...
Процедура ПредставлениеДокументовОснованияНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Объект.ЗаполнениеДвижений.Количество() = 0 Тогда
		...
		СписокТиповДокументовОснования.ПоказатьВыборЭлемента(Обработчик, НСтр("ru='Выберите тип:';uk='Виберіть тип:'"));
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Редактирование списка связанных документов';uk='Редагування списку пов''язаних документів'"));
...
Процедура ОбработатьИзменениеСчета(ДтКт)
	...
	Если СвойстваСчета.УчетПоНалоговымНазначениямНДС Тогда
		ТекущиеДанные["НалоговоеНазначение" + ДтКт] = ОписаниеТиповНалоговоеНазначение.ПривестиЗначение(ТекущиеДанные["НалоговоеНазначение" + ДтКт]);
	Иначе
...
Процедура ПодготовитьФормуНаСервере()
	...
	ТипыСвязанныеСОрганизацией = НастройкаСчетовУчета.ТипыСвязанныеСОрганизацией();
	// Назначаем общий обработчик
	...
	Если Не СтатусОтраженияДокумента.Пустая() Тогда 
		...
		ИначеЕсли СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияДокументовВРеглУчете.КОтражениюВУчетеВручную Тогда
			Если ЕстьПроводки Тогда
				НачалоСостояния = ?(Объект.ЗаполнениеДвижений.Количество() > 1, НСтр("ru='Исходные документы были изменены.';uk='Вихідні документи були змінені.'"),
...
Процедура ОтобразитьВводДокументовОснования(Форма)
	...
	Форма.ЗаголовокДокументовОснования = ?(КоличествоДокументов > 1,	НСтр("ru='Связанные документы:';uk='Пов''язані документи:'"), НСтр("ru='Связанный документ:';uk='Пов''язаний документ:'"));
		
	Если КоличествоДокументов = 0 Тогда
		ПредставлениеДокументовОснования = НСтр("ru='Выбрать документ';uk='Вибрати документ'");
	...
	Иначе
		ПредставлениеДокументовОснования = СтрШаблон(НСтр("ru='Открыть список (%1)';uk='Відкрити список (%1)'"), КоличествоДокументов);
	КонецЕсли;
...
Процедура НастроитьОтображениеТаблицыПроводок()
	...
	Элементы.ХозрасчетныйСуммы.Заголовок   = СтрШаблон(НСтр("ru='Сумма БУ (%1)';uk='Сума БО (%1)'"), ВалютаРУ);
	Элементы.ХозрасчетныйСуммаНУДт.Заголовок   = СтрШаблон(НСтр("ru='Сумма НУ Дт (%1)';uk='Сума ПО Дт (%1)'"), ВалютаРУ);
	Элементы.ХозрасчетныйСуммаНУКт.Заголовок   = СтрШаблон(НСтр("ru='Сумма НУ Кт (%1)';uk='Сума ПО Кт (%1)'"), ВалютаРУ);
	
	Элементы.ХозрасчетныйСуммаУУ.Заголовок = СтрШаблон(НСтр("ru='Сумма УУ (%1)';uk='Сума УО (%1)'"), ВалютаУУ);
	Элементы.ХозрасчетныйСуммаФО.Заголовок = СтрШаблон(НСтр("ru='Сумма ФО (%1)';uk='Сума ФЗ (%1)'"), ВалютаФО);
	
	Элементы.ХозрасчетныйСуммаПРДт.Видимость = Ложь;
	Элементы.ХозрасчетныйСуммаВРДт.Видимость = Ложь;
	Элементы.ХозрасчетныйСуммаПРКт.Видимость = Ложь;
	Элементы.ХозрасчетныйСуммаВРКт.Видимость = Ложь;
