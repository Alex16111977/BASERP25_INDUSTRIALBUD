...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
    ...
    |	-ВЫРАЗИТЬ(ВидыЗапасов.Сумма * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СуммаСНДСРегл,
    |	-ВЫРАЗИТЬ(ВидыЗапасов.Сумма * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СуммаБезНДСРегл,
    |	0 КАК НДСРегл,
    ...
    |	-ВЫРАЗИТЬ(&Цена * ТаблицаРасходы.Количество * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СуммаСНДСРегл,
    |	-ВЫРАЗИТЬ(&Цена * ТаблицаРасходы.Количество * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СуммаБезНДСРегл,
    |	0 КАК НДСРегл,
    |	0									    КАК ПостояннаяРазница,
	|	0										КАК ВременнаяРазница
	|ИЗ
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ТОГДА ВЫБОР
	|			КОГДА ЕСТЬNULL(АналитикаПоПолучателю.Назначение.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) <>
	|					ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
	|				ТОГДА АналитикаПоПолучателю.Назначение.НалоговоеНазначение
    ...
    |	ВЫРАЗИТЬ(&Цена * &КоличествоПоПравилу * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))	КАК СтоимостьРеглБезНДС,
	|	0																					КАК ПостояннаяРазница,
    |	ВЫБОР
    |		КОГДА &УправленческийУчетОрганизаций
    ...
    |	ВЫРАЗИТЬ(&Цена * ПартииПроизводства.Количество * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))	КАК СтоимостьРеглБезНДС,
	|	0																					КАК ПостояннаяРазница,
    |	ВЫБОР
    |		КОГДА &УправленческийУчетОрганизаций
    ...
    |	ВЫРАЗИТЬ(&Цена * ТаблицаПрочиеРасходы.Количество * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СтоимостьРегл,
    |	ВЫРАЗИТЬ(&Цена * ТаблицаПрочиеРасходы.Количество * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) КАК СтоимостьРеглБезНДС,
    |	0																				КАК ПостояннаяРазница,
    |	ВЫБОР
    |		КОГДА &УправленческийУчетОрганизаций
	...
	|	КОНЕЦ																						КАК АналитикаФинансовогоУчета,
	|	ВЫБОР КОГДА &ПартионныйУчетВерсии22 
	|		ТОГДА СпрПартииПроизводства.НалоговоеНазначение
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО 
	|	КОНЕЦ КАК НалоговоеНазначение,
	|	ВЫБОР КОГДА &ПартионныйУчетВерсии21 
	|		ТОГДА СпрПартииПроизводства.НалоговоеНазначение
    ...
    |	-ВЫРАЗИТЬ(ПартииПроизводства.Сумма * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))	КАК СтоимостьРеглБезНДС,
	|	0																									КАК ПостояннаяРазница,
    |	ВЫБОР
    |		КОГДА &УправленческийУчетОрганизаций
...
Функция СформироватьПечатнуюФормуНакладной(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
        ...
        ОбластьМакета.Параметры.ТипПолучателя = НСтр("ru='Склад';uk='Склад'",КодЯзыкаПечать);
