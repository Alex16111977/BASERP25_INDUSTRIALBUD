...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкаСчетовУчета.ПриСозданииНаСервере(ЭтотОбъект, 
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтборПоМатериалам.Количество() = 0 И (Объект.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.КоличествоУказанныхМатериалов
		...
		ТекстСообщения = НСтр("ru='Не указаны материалы для базы распределения';uk='Не вказані матеріали для бази розподілу'");
	...
	ИначеЕсли ОтборПоВидамРабот.Количество() = 0 И (Объект.БазаРаспределения = Перечисления.ТипыБазыРаспределенияРасходов.КоличествоРаботУказанныхВидов) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указаны виды работ для базы распределения';uk='Не вказані види робіт для бази розподілу'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,КлючДанных,,,Отказ);
...
Процедура НалоговоеНазначениеПриИзменении(Элемент)
	НалоговоеНазначениеПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура НалоговоеНазначениеПриИзмененииНаСервере()
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	НастройкаСчетовУчета.ПриЧтенииНаСервере(ЭтотОбъект, 
		Документы.РаспределениеВозвратныхОтходов.ПараметрыНастройкиСчетовУчета());
...
Процедура УстановитьОформлениеХарактеристики()
	...
	Иначе
		...
		ПолеХарактеристика.ПодсказкаВвода = НСтр("ru='<характеристики не используются>';uk='<характеристики не використовуються>'");
...
Процедура УстановитьОформлениеНазначения()
	...
	Если ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара")
		...
		Объект.Назначение = Неопределено;
		ПолеНазначение.ПодсказкаВвода = НСтр("ru='<только для товаров и работ>';uk='<тільки для товарів і робіт>'");
...
Процедура УстановитьЗаголовокКоличества()
	...
	Если ЗначениеЗаполнено(Объект.Номенклатура.ЕдиницаИзмерения.Наименование) Тогда
		ТекстЗаголовка = НСтр("ru='Количество%1';uk='Кількість %1'");
...
Процедура УстановитьЗаголовокПолучателя()
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
		Элементы.Получатель.Заголовок = НСтр("ru='Подразделение-получатель';uk='Підрозділ-одержувач'");
	Иначе
		Элементы.Получатель.Заголовок = НСтр("ru='Склад';uk='Склад'");
	КонецЕсли;
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.РаспределениеВозвратныхОтходов.ПараметрыЗаполненияНалоговогоНазначения(Объект);
