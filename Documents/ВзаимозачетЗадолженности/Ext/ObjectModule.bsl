...
//***ERP
Процедура ОбновитьВзаимозачетЗадолженностиПослеОбмена() Экспорт
	...
	ПартнерДебиторПолученныйПоКонтрагенту = ДенежныеСредстваСервер.ПолучитьПартнераПоКонтрагенту(КонтрагентДебитор);
    ПартнерКредиторПолученныйПоКонтрагенту = ДенежныеСредстваСервер.ПолучитьПартнераПоКонтрагенту(КонтрагентКредитор);

	ДебиторскаяЗадолженность.Очистить();
	КредиторскаяЗадолженность.Очистить();
	Для каждого СтрокаТЗ Из А_СуммыДолга Цикл
          
		Если СтрокаТЗ.ТипЗадолженности = Перечисления.ТипыЗадолженности.Дебиторская Тогда
			СтрокаТабличнойЧасти = ДебиторскаяЗадолженность.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти,СтрокаТЗ);

			Если СтрокаТабличнойЧасти.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом Тогда
				ТипДебитора =  Перечисления.ТипыУчастниковВзаимозачета.Клиент;
			ИначеЕсли СтрокаТабличнойЧасти.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком Тогда
				ТипДебитора =  Перечисления.ТипыУчастниковВзаимозачета.Поставщик;
			КонецЕсли;
			СтрокаТабличнойЧасти.Партнер = ПартнерДебиторПолученныйПоКонтрагенту;

			// Заповнення ОбъектРасчетов
			СтрокаТабличнойЧасти.ОбъектРасчетов = ПолучитьОбъектРасчетовДляСтроки(
				СтрокаТЗ,
				Организация,
				ПартнерДебиторПолученныйПоКонтрагенту,
				КонтрагентДебитор);

		ИначеЕсли СтрокаТЗ.ТипЗадолженности = Перечисления.ТипыЗадолженности.Кредиторская Тогда
			СтрокаТабличнойЧасти = КредиторскаяЗадолженность.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти,СтрокаТЗ);
			...
			КонецЕсли;
			СтрокаТабличнойЧасти.Партнер = ПартнерКредиторПолученныйПоКонтрагенту;

			// Заповнення ОбъектРасчетов
			СтрокаТабличнойЧасти.ОбъектРасчетов = ПолучитьОбъектРасчетовДляСтроки(
				СтрокаТЗ,
				Организация,
				ПартнерКредиторПолученныйПоКонтрагенту,
				КонтрагентКредитор);
...
// Функція визначає та повертає ОбъектРасчетов для рядка табличної частини
// на основі договору, замовлення клієнта або замовлення постачальнику
//
Функция ПолучитьОбъектРасчетовДляСтроки(СтрокаТЗ, ОрганизацияДок, Партнер, Контрагент)

	Договор = СтрокаТЗ.А_Договор;
	ТипРасчетов = СтрокаТЗ.ТипРасчетов;

	// Визначаємо порядок розрахунків з договору
	...
	// Визначаємо джерело (об'єкт) для ОбъектаРасчетов
	...
	Если ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным Тогда
		...
		Если ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом
			И ЗначениеЗаполнено(СтрокаТЗ.А_ЗаказКлиента) Тогда
			Источник = СтрокаТЗ.А_ЗаказКлиента;
		...
		ИначеЕсли ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком
			И ЗначениеЗаполнено(СтрокаТЗ.А_ЗаказПоставщику) Тогда
			Источник = СтрокаТЗ.А_ЗаказПоставщику;
			ТипОР = Перечисления.ТипыОбъектовРасчетов.Заказ;
	...
	// Якщо джерело не визначено - повертаємо порожнє посилання
	Если НЕ ЗначениеЗаполнено(Источник) Тогда
		Возврат Справочники.ОбъектыРасчетов.ПустаяСсылка();
	КонецЕсли;

	// Формуємо параметри для отримання/створення ОбъектаРасчетов
	ПараметрыОР = ОбъектыРасчетовСервер.ПолучитьПараметрыОбъектаРасчетов();
	ПараметрыОР.Организация = ОрганизацияДок;
	ПараметрыОР.Партнер     = Партнер;
	...
	Возврат ОбъектыРасчетовСервер.ПроверитьСоздатьОбъектРасчетов(ПараметрыОР);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***
	ОбновитьВзаимозачетЗадолженностиПослеОбмена();
	//***
