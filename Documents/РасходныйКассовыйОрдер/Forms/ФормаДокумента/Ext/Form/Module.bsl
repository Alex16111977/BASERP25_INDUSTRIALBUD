...
Процедура ПодотчетноеЛицоПриИзмененииСервер()
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику")
		...
		ЗаполнитьСуммыНевыплаченнойЗарплатыНаСервере(СтруктураПараметров);
		ОбновитьОтображениеСтрокаФизическиеЛица();
...
Процедура РасшифровкаПлатежаОбъектРасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// А_ Доработка: дублируем логику из ВзаиморасчетыКлиент.ОбъектРасчетовНачалоВыбора
	СтандартнаяОбработка = Ложь;
	...
	ПараметрыМеханизма  = ОбщегоНазначенияУТКлиентСервер.ПолучитьДанныеМеханизмаИзКэшаФормы(ЭтаФорма, "Взаиморасчеты");
	СтруктураПараметров = ПараметрыМеханизма.МассивПараметров[0];

	Ссылка = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ЭтаФорма, СтруктураПараметров.Ссылка);
	...
	Контрагент  = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ЭтаФорма, СтруктураПараметров.Контрагент, ИдентификаторСтроки);
	Организация = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ЭтаФорма, СтруктураПараметров.Организация, ИдентификаторСтроки);
	Валюта      = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ЭтаФорма, СтруктураПараметров.ВалютаДокумента);
	...
	Если ЗначениеЗаполнено(СтруктураПараметров.ЭлементыФормы.РасшифровкаПлатежа) Тогда
		ЭлементРасшифровкаПлатежа = Элементы[СтруктураПараметров.ЭлементыФормы.РасшифровкаПлатежа];
		Сумма = ЭлементРасшифровкаПлатежа.ТекущиеДанные.Сумма;
	Иначе
		Сумма = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ЭтаФорма, СтруктураПараметров.СуммаДокумента);
	КонецЕсли;
...
Процедура РасшифровкаПлатежаДоговорЛизингаПриИзменении(Элемент)
	...
	//++ НЕ УТ
	РасшифровкаПлатежаДоговорЛизингаПриИзмененииНаСервере();
...
//++ НЕ УТ
&НаСервере
Процедура РасшифровкаПлатежаДоговорЛизингаПриИзмененииНаСервере()
...
Процедура ВедомостьПриИзмененииНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтатьяДвиженияДенежныхСредств) Тогда
	 ...
	 Объект.СтатьяДвиженияДенежныхСредств = Объект.А_СтатьяДвиженияДенежныхСредствКазна;	
...
Процедура ВедомостьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаздатчиком")
		     И ЗначениеЗаполнено(Объект.ПодотчетноеЛицо) Тогда
			ОтборВедомостей.Вставить("Раздатчик", Объект.ПодотчетноеЛицо);
		КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(Объект.Касса) Тогда
		ОтборВедомостей.Вставить("Касса", Объект.Касса);
	КонецЕсли;
...
&НаСервере 
Процедура ЗаполнитьСуммыНевыплаченнойЗарплатыНаСервере(СтруктураПараметров = Неопределено)
	...
	|	ДанныеВедомости.ГруппаУчетаНачислений,
...
&НаСервере
Процедура А_ЗаполнитьРасшифровкаПлатежаИзРаспределениеНаСервере()
	...
	Для Каждого СтрокаТЗ Из Объект.А_Распределение Цикл
		...
		СтрокаРасшифровкаПлатежа = Объект.РасшифровкаПлатежа.Добавить();

		// === Основные реквизиты ===
		СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств     = СтрокаТЗ.СтатьяДвиженияДенежныхСредств;
		СтрокаРасшифровкаПлатежа.Подразделение      = СтрокаТЗ.Подразделение;
		СтрокаРасшифровкаПлатежа.Сумма         = СтрокаТЗ.Сумма;
		СтрокаРасшифровкаПлатежа.СуммаВзаиморасчетов         = СтрокаТЗ.Сумма;
        СтрокаРасшифровкаПлатежа.ВалютаВзаиморасчетов         = Объект.Валюта;
		СтрокаРасшифровкаПлатежа.СтатьяРасходов = СтрокаТЗ.СтатьяДвиженияДенежныхСредств.А_СтатьяРасходов;
...
&НаКлиенте
Процедура А_ЗаполнитьРасшифровкаПлатежаИзРаспределение(Команда)
	А_ЗаполнитьРасшифровкаПлатежаИзРаспределениеНаСервере();
КонецПроцедуры
