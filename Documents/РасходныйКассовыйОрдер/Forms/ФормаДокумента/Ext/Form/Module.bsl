...
Процедура РасшифровкаПлатежаОбъектРасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// чтобы добавить параметр А_СтатьяДДСОтбор для фильтрации формы выбора объектов расчетов
	СтандартнаяОбработка = Ложь;
	...
	// А_ Доработка: передаем СтатьюДДС только для партнеров-подразделений (у которых заполнен А_Подразделение)
	СтрокаРасшифровки = Элементы.РасшифровкаПлатежа.ТекущиеДанные;
	Если СтрокаРасшифровки <> Неопределено
		И ЗначениеЗаполнено(СтрокаРасшифровки.СтатьяДвиженияДенежныхСредств)
		И ЗначениеЗаполнено(СтрокаРасшифровки.Партнер)
		И А_ПартнерЭтоПодразделение(СтрокаРасшифровки.Партнер) Тогда
		НастройкиВыбора.Вставить("А_СтатьяДДСОтбор", СтрокаРасшифровки.СтатьяДвиженияДенежныхСредств);
	КонецЕсли;
...
// А_ Доработка: проверка что партнер создан по подразделению (имеет заполненный реквизит А_Подразделение)
&НаСервереБезКонтекста
Функция А_ПартнерЭтоПодразделение(ПартнерСсылка)
	Если НЕ ЗначениеЗаполнено(ПартнерСсылка) Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПартнерСсылка, "А_Подразделение"));
КонецФункции
