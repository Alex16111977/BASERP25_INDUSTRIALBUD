...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "
	|// ▼ Блок 1 ОТ — Ветка C-1: РКО с нашей ВКассу — детальная разбивка по СтрПредпр (2026-05-28, Подход C v2)
	|// Подразделение вычисляется через инлайн-аналог А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны:
	|//   если СтрПредпр из Рш.Подразделение.А_Подразделение НЕ ""Главный офис"" (код 00-000043) → берём как есть.
	|//   если СтрПредпр = ""Главный офис"" → ищем СтрПредпр-направление с А_НаправлениеДеятельности = Рш.НаправлениеДеятельности.
	|// НаправлениеДеятельности берётся прямо из Рш.НаправлениеДеятельности (а не из СтрПредпр.А_НаправлениеДеятельности).
	|// Без этой логики Пономаренко (Главный офис) попадает в ПАП как ""Главный офис"" вместо реального ""Производство"".
	|ВЫБРАТЬ
	|	&Период КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	&Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА Рш.Подразделение.А_Подразделение.Код <> ""00-000043""
	|			ТОГДА Рш.Подразделение.А_Подразделение
	|		КОГДА СтрПредпрНапр.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА СтрПредпрНапр.Ссылка
	|		ИНАЧЕ Рш.Подразделение.А_Подразделение
	|	КОНЕЦ КАК Подразделение,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СтрПредпрНапр
	|			ПО СтрПредпрНапр.А_ЭтоПодразделениеНаправление
	|				И СтрПредпрНапр.А_НаправлениеДеятельности = Рш.НаправлениеДеятельности
	|				И Рш.Подразделение.А_Подразделение.Код = ""00-000043""
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
	|	И &ХозяйственнаяОперация В (
