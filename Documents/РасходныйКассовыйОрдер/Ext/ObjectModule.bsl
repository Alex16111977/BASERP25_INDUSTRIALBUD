...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ИсходныеДанныеЗаполнения) = Тип("СправочникСсылка.ДоговорыЛизинга") Тогда
		ЗаполнитьПоДоговоруЛизинга(ИсходныеДанныеЗаполнения, ДанныеЗаполнения);
	//-- НЕ УТ
...
//***ERP 
Процедура ОбновитьРасходныйКассовыйОрдерПослеОбмена() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(А_ОрганизацияБухгалтерия) Тогда
		А_Обработан  = Истина;
		Возврат;
	КонецЕсли;
	...
	Касса = А_ОрганизацияБухгалтерия.А_КассаБух;
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику Тогда
		Подразделение = А_ОрганизацияБухгалтерия.А_ПодразделениеЗарплатаБух;
		СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("УТ-001211");//Транзит Исх
	КонецЕсли;
	А_Обработан  = Истина;
...
Процедура ОбновитьРасходныйКассовыйОрдерПослеОбмена_КазнаПослеОбмена() Экспорт
	...
	Если ЗначениеЗаполнено(Ведомость) 
		И НЕ ЗначениеЗаполнено(А_ОрганизацияБухгалтерия) Тогда
		А_Необновлять = Истина;
	КонецЕсли;   
	...
	Если НЕ ЗначениеЗаполнено(СтатьяДвиженияДенежныхСредствШапка) Тогда
		СтатьяДвиженияДенежныхСредствШапка = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("УТ-001211");   //Транзит Исх
	КонецЕсли; 
	...
	Иначе
		...
		СтрокаПлатежа.СтатьяРасходов = А_СтатьяДвиженияДенежныхСредствКазна.А_СтатьяРасходов; //Прочие доходи
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		//***ERP      
		ОбновитьРасходныйКассовыйОрдерПослеОбмена();
	ОбновитьРасходныйКассовыйОрдерПослеОбмена_КазнаПослеОбмена(); 
	//***ERP
