...
Процедура ЗаполнитьОтражениеАмортизационныхРасходов(ПараметрНематериальныйАктив)
	...
	Запрос.Текст = 
		...
		|	ПорядокУчетаНМАБУСрезПоследних.СтатьяРасходовБУ КАК СтатьяРасходов,
		|	ПорядокУчетаНМАБУСрезПоследних.АналитикаРасходовБУ КАК АналитикаРасходов,
		|	1 КАК Коэффициент
		|ИЗ
		...
		|				&Период,
		|				СпособОтраженияРасходовБУ = НЕОПРЕДЕЛЕНО
		|					И НематериальныйАктив = &НематериальныйАктив
		|					И Организация = &Организация) КАК ПорядокУчетаНМАБУСрезПоследних
		|		ПО МестоУчетаНМАСрезПоследних.НематериальныйАктив = ПорядокУчетаНМАБУСрезПоследних.НематериальныйАктив
		|			И МестоУчетаНМАСрезПоследних.Организация = ПорядокУчетаНМАБУСрезПоследних.Организация";
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.ИзменениеПараметровНМА.ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(
		НСтр("ru='Изменение параметров НМА';uk='Зміна параметрів НМА'"));
...
Процедура РассчитатьАмортизацию(Отказ)
	...
	ТаблицаНачисленнаяАмортизация = УчетНМА.НачисленнаяАмортизация(
		НМА.Выгрузить(, "НомерСтроки, НематериальныйАктив"), ТаблицаРеквизиты,, Отказ);
...
Процедура ЗаполнитьПоНематериальномуАктиву(Основание)
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru='Нематериальный актив ""%1"" не принят к учету.';uk='Нематеріальний актив ""%1"" не прийнятий до обліку.'"), Строка(Основание));
	...
	Для каждого Строка Из ОтражениеАмортизационныхРасходов Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.ОрганизацияПолучательРасходов) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не заполнено поле ""Получатель"" в строке ""%1"" табличной части.';uk='Не заповнено поле ""Одержувач"" у рядку ""%1"" табличної частини.'"), Строка.НомерСтроки);
		...
		Если Не ЗначениеЗаполнено(Строка.СчетПередачиРасходов) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не заполнено поле ""Счет передачи"" в строке ""%1"" табличной части.';uk='Не заповнено поле ""Рахунок передачі"" у рядку ""%1"" табличної частини.'"), Строка.НомерСтроки);
