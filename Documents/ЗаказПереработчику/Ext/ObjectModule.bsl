...
Процедура ПриЗаписи(Отказ)
	...
	ШаблонНазначения = Документы.ЗаказПереработчику.ШаблонНазначения(ЭтотОбъект);
    Справочники.Назначения.ПриЗаписиДокумента(Назначение, ШаблонНазначения, ЭтотОбъект, Партнер, НалоговоеНазначение);
...
#Область ТЧ_Продукция
	...
	Для ТекИндекс = 0 По Продукция.Количество() - 1 Цикл
		...
		Если СтатусДокументаВышеНеСогласованного
			...
			ТекстОшибки = НСтр("ru='Не заполнена колонка ""Дата поступления"" в строке %НомерСтроки% списка ""Выпускаемая продукция""';uk='Не заповнена колонка ""Дата надходження"" в рядку %НомерСтроки% списку ""Продукція, що випускається""'");
		...
		Если Не ПоступлениеОднойДатой
			...
			ТекстОшибки = НСтр("ru='Дата поступления должна быть не меньше даты документа %Дата% в строке %НомерСтроки% списка ""Выпускаемая продукция""';uk='Дата надходження повинна бути не менше дати документа %Дата% в рядку %НомерСтроки% списку ""Продукція, що випускається""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Дата%", Формат(Дата,"ДЛФ=D"));
			ПутьКТЧ     = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Продукция", ТекСтрока.НомерСтроки, "ДатаПоступления");
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПричиныОтменыЗаказовПоставщикам")
			...
			ТекстОшибки = НСтр("ru='Необходимо указать причину отмены в строке %НомерСтроки% списка ""Выпускаемая продукция""';uk='Необхідно вказати причину скасування в рядку %НомерСтроки% списку ""Продукція, що випускається""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
		...
		Если Не ТекСвойство = Неопределено
			...
			ТекстОшибки =
				НСтр("ru='В строке %НомерСтроки% списка ""Выпускаемая продукция"" указано назначение давальца, что недопустимо.';uk='У рядку %НомерСтроки% списку ""Продукція, що випускається"" зазначено призначення давальця, що неприпустимо.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
...
#Область ТЧ_ВозвратныеОтходы
	...
	Для ТекИндекс = 0 По ВозвратныеОтходы.Количество() - 1 Цикл
		...
		Если СтатусДокументаВышеНеСогласованного
			...
			ТекстОшибки = НСтр("ru='Не заполнена колонка ""Дата поступления"" в строке %НомерСтроки% списка ""Возвратные отходы""';uk='Не заповнена колонка ""Дата надходження"" в рядку %НомерСтроки% списку ""Зворотні відходи""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
		...
		Если Не ПоступлениеОднойДатой
			...
			ТекстОшибки = НСтр("ru='Дата поступления должна быть не меньше даты документа %Дата% в строке %НомерСтроки% списка ""Возвратные отходы""';uk='Дата надходження повинна бути не менше дати документа %Дата% в рядку %НомерСтроки% списку ""Зворотні відходи""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Дата%", Формат(Дата,"ДЛФ=D"));
			ПутьКТЧ     = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ВозвратныеОтходы", ТекСтрока.НомерСтроки, "ДатаПоступления");
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПричиныОтменыЗаказовПоставщикам")
			...
			ТекстОшибки = НСтр("ru='Необходимо указать причину отмены в строке %НомерСтроки% списка ""Возвратные отходы""';uk='Необхідно вказати причину скасування в рядку %НомерСтроки% списку ""Зворотні відходи""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
		...
		Если ТекСтрока.Цена = 0 Тогда
			...
			ТекстОшибки = НСтр("ru='Необходимо указать цену, по которой будет оприходованы возвратные отходы в строке %НомерСтроки% списка ""Возвратные отходы""';uk='Необхідно вказати ціну, по якій буде оприбутковано зворотні відходи у рядку %НомерСтроки% списку ""Зворотні відходи""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
		...
		Если ТекСтрока.Сумма = 0 Тогда
			...
			ТекстОшибки = НСтр("ru='Необходимо указать сумму, по которой будут оприходованы возвратные отходы в строке %НомерСтроки% списка ""Возвратные отходы""';uk='Необхідно вказати суму, по якій будуть оприбутковані зворотні відходи у рядку %НомерСтроки% списку ""Зворотні відходи""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
...
#Область ТЧ_Материалы
	...
	Для ТекИндекс = 0 По Материалы.Количество()-1 Цикл
		...
		Если Не ТекСтрока.Отменено И Не ТекСтрока.ВариантОбеспечения = ВариантНеТребуется
			...
			ТекстОшибки = НСтр("ru='Не заполнена колонка ""Склад"" в строке %НомерСтроки% списка ""Сырье и материалы""';uk='Не заповнена колонка ""Склад"" в рядку %НомерСтроки% списку ""Сировина і матеріали""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
		...
		Если Не ЗначениеЗаполнено(ТекСтрока.ДатаОтгрузки) И ДатаОтгрузкиВСтрокеОбязательна Тогда
			...
			Если Не НеОтгружатьЧастями Тогда
				...
				ТекстОшибки = НСтр("ru='Не заполнена колонка ""Дата отгрузки"" в строке %НомерСтроки% списка ""Сырье и материалы""';uk='Не заповнена колонка ""Дата відвантаження"" в рядку %НомерСтроки% списку ""Сировина і матеріали""'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
		...
		Если Не НеОтгружатьЧастями 
			...
			ТекстОшибки = НСтр("ru='Дата отгрузки должна быть не меньше даты документа ""%Дата%"" в строке %НомерСтроки% списка ""Сырье и материалы""';uk='Дата відвантаження повинна бути не менше дати документа ""%Дата%"" в рядку %НомерСтроки% списку ""Сировина і матеріали""'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", ТекСтрока.НомерСтроки);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Дата%", Формат(Дата, "ДЛФ=D"));
			ПутьКТЧ     = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Материалы", ТекСтрока.НомерСтроки, "ДатаОтгрузки");
...
Процедура ЗаполнитьДокументНаОснованииПартнера(Знач Основание)
	...
	Иначе
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
			...
			ВызватьИсключение НСтр("ru='Контрагент должен являться одновременно и ""поставщиком"" и ""клиентом"".';uk='Контрагент повинен бути одночасно і ""постачальником"" і ""клієнтом"".'");
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
    ...
    ПараметрыЗаполнения = Документы.ЗаказПереработчику.ПараметрыЗаполненияНалоговогоНазначения(ЭтотОбъект);
...
Процедура ОбновитьЭтапыПроизводстваСвязаныеСЗаказом(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТекстСообщения = НСтр("ru='Не удалось записать изменения в этапе %1';uk='Не вдалося записати зміни в етапі %1'");
...
Процедура ПроверитьВозможностьСвязиСЭтапами(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПроизводствоНаСтороне Тогда
			ТекстСообщения = НСтр("ru='%1 включен в другой заказ переработчику. Не допускается включать один этап в разные заказы.';uk=' %1 включений в інший замовлення переробнику. Не допускається включати один етап в різні замовлення.'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Ссылка); 
		Иначе
			ТекстСообщения = НСтр("ru='%1 относится к собственному производству. Не допускается включать в заказ этапы собственного производства.';uk=' %1 відноситься до власного виробництва. Не допускається вносити в замовлення етапи власного виробництва.'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Ссылка); 
		КонецЕсли;
