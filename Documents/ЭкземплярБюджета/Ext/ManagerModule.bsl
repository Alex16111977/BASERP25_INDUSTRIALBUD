...
//	*Расшифровка - Неопределено, Соответствие - опционально
...
Функция СформироватьПечатнуюФормуАналитическойЗаписки(ПараметрыОтчета, СтруктураКолонокТаблицыДанных, Примечания)
	...
	Для Каждого СтатьяБюджетов Из СтатьиБюджетов Цикл
		...
		Представление = НСтр("ru='Статья бюджета ""%1""';uk='Стаття бюджету ""%1""'");
		...
		Если ВыборкаСтатейБюджетов.УчитыватьПоКоличеству Тогда
			Представление = НСтр("ru='Количество';uk='Кількість'");
		...
		Представление = НСтр("ru='Примечание';uk='Примітка'");
...
//
// Параметры:
//  ПараметрыФормированияСпискаВвода - Структура - Параметры формирования заголовка. См. БюджетнаяОтчетностьРасчетКэшаСервер.ПараметрыЭлементаСпискаВвода.
//
// Возвращаемое значение:
...
Функция ПараметрыЭлементаВводаЗначенийНаЯзыкеПользователя(ПараметрыФормированияСпискаВвода) Экспорт
	...
	Если ПараметрыФормированияСпискаВвода.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
		Результат.Заголовок = НСтр("ru='Статья/показатель бюджетов';uk='Стаття/показник бюджетів'");
		Результат.ПредставлениеПрочихЗначений = НСтр("ru='<прочие статьи/показатели бюджетов>';uk='<інші статті/показники бюджетів>'");
	...
	ИначеЕсли ПараметрыФормированияСпискаВвода.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение
		...
		Если Периодичность = Перечисления.Периодичность.Год Тогда
			Формат = НСтр("ru='ДФ=гггг';uk='ДФ=гггг'");
		ИначеЕсли Периодичность = Перечисления.Периодичность.Полугодие Тогда
			Формат = НСтр("ru='ДФ=''""Полугодие с"" ММММ гггг''';uk='Л=uk; ДФ=''""Півріччя з"" ММММ гггг'''");
		ИначеЕсли Периодичность = Перечисления.Периодичность.Квартал Тогда
			НСтр("ru='ДФ=''к ""квартал""''';uk='ДФ=''до ""квартал""'''")
		ИначеЕсли Периодичность = Перечисления.Периодичность.Месяц Тогда
			Формат = НСтр("ru='ДФ=''MMMM yyyy''';uk='ДФ=''MMMM yyyy'''");
		ИначеЕсли Периодичность = Перечисления.Периодичность.Декада Тогда
			Формат = НСтр("ru='ДФ=''""Декада с"" dd.MM.yy''';uk='ДФ=''""Декада з"" dd.MM.yy'''");
		ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда
			Формат = НСтр("ru='ДФ=''""Неделя с"" dd.MM.yy''';uk='ДФ=''""Тиждень з"" dd.MM.yy'''");
	...
	Для Каждого КолонкаБюджета Из СтруктураКолонок.Строки Цикл
		...
		Если Не КолонкаБюджета.Строки.Количество() Тогда
			...
			Если Не НеВводимаяКолонка Тогда
				...
				ОписаниеКолонки.Вставить("Заголовок", НСтр("ru='Ячейка ввода';uk='Комірка введення'"));
				...
				ЗаголовокКолонки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ЗаголовокКолонки,
					НСтр("ru='картинка комментария';uk='картинка коментаря'"));
				ОписаниеКолонки = Новый Структура();
...
Функция ПолучитьСКДПечатиДокументаБезГруппировок(ИДТаблицы, КолонкиПоИзмерениям, ГлубинаДерева,
	...
	Представление = НСтр("ru='Статья бюджетов';uk='Стаття бюджетів'");
	ФинОтчеты.НовоеПолеНабора(Набор, "СтатьяБюджетов",,Представление);
	Представление = НСтр("ru='Показатель бюджетов';uk='Показник бюджетів'");
	ФинОтчеты.НовоеПолеНабора(Набор, "ПоказательБюджетов",,Представление);
	Представление = НСтр("ru='Нефинансовый показатель';uk='Нефінансовий показник'");
	ФинОтчеты.НовоеПолеНабора(Набор, "НефинансовыйПоказатель",,Представление);
	Представление = НСтр("ru='Редактируемое значение';uk='Редаговане значення'");
	...
	Для Каждого Периодичность Из СтруктураОписанияКолонок.Периодичности Цикл
		...
		ЗаголовокПоля = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Периодичность);
		ФинОтчеты.НовоеПолеНабора(Набор, "Период" + ИмяПериода,, ЗаголовокПоля);
	КонецЦикла;
...
Функция СоздатьПоляВыбораПоДеревуКолонок(СКД, ПараметрыСозданияПолей, ТекущаяГлубина = 1)
	...
	ЭлементыДобавления  = ПараметрыСозданияПолей.ЭлементыДобавления; // Массив
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсиюМонопольно

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.5.4.310";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура = "Документы.ЭкземплярБюджета.ОбработатьДанныеДляПереходаНаНовуюВерсиюМонопольно";
	...
	Обработчик.Комментарий = НСтр("ru='Устанавливает флаг используется на дополнительном свойстве документа экземпляр бюджета.';uk='Встановлює прапор використовується на додатковій властивісті документа екземпляр бюджету.'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.ЭкземплярБюджетаПрисоединенныеФайлы.ОбработатьДанныеДляПереходаНаНовуюВерсию";
...
// Монопольный обработчик обновления КА 2.5.4
// Устанавливаем использование Доп. свойств документа "Экземпляр бюджета".
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюМонопольно() Экспорт
	...
	УправлениеСвойствами.УстановитьПараметрыНабораСвойств(
		Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ЭкземплярБюджета.ИмяПредопределенныхДанных,
...
// Замена пустых значений аналитики на единое значение пустой аналитики Неопределено
