...
Процедура БюджетПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Не Область.СодержитЗначение Тогда
		...
		Область.Текст = "";
		ТекстПредупреждения = НСтр("ru='Редактирование ячейки не предусмотрено';uk='Редагування комірки не передбачено'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьСписком(Команда)
	...
	Если Элементы.РедактироватьСписком.Заголовок = НСтр("ru='Редактировать в списке';uk='Редагувати в списку'") Тогда
...
Процедура ВыгрузитьВExcel(Команда)
	...
	#Если ВебКлиент Тогда
		ТекстПредупреждения = НСтр("ru='Функциональность не поддерживается в веб-клиенте';uk='Функціональність не підтримується у вебклієнті'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	#КонецЕсли
	#Если МобильныйКлиент Тогда
		ТекстПредупреждения = НСтр("ru='Функциональность не поддерживается в мобильном клиенте';uk='Функціональність не підтримується в мобільному клієнті'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	#КонецЕсли
	
	Если Объект.Ссылка.Пустая() Тогда
		ТекстПредупреждения = НСтр("ru='Перед выгрузкой в эксель документ следует записать';uk='Перед вивантаженням в ексель документ слід записати'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоДаннымБюджетирования(Команда)
	...
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("МодельБюджетирования", НСтр("ru='Модель бюджетирования';uk='Модель бюджетування'"));
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("НачалоПериода", НСтр("ru='Начало периода';uk='Початок періоду'"));
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("ОкончаниеПериода", НСтр("ru='Окончание периода';uk='Закінчення періоду'"));
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("Сценарий", НСтр("ru='Сценарий';uk='Сценарій'"));
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("ВидБюджета", НСтр("ru='Вид бюджета';uk='Вид бюджету'"));
	...
	Если ПолучитьФункциональнуюОпциюФормы("ФормироватьБюджетыПоПодразделениям") Тогда
		ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"));
	КонецЕсли;
...
Процедура СкопироватьПоПериодам(Команда)
	...
	Если Не КопированиеВыполнено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Текущая область недоступна для копирования!';uk='Поточна область недоступна для копіювання!'"));
	КонецЕсли;
...
Функция ВозможностьЧтенияПрисоединенногоФайла(ПрисоединенныйФайл)
	...
	Если ЗначениеЗаполнено(РеквизитыФайла.ТипСлужебногоФайла) Тогда
		...
		Результат.ТекстОшибки    = НСтр("ru='Файл является служебным, выбор запрещен.';uk='Файл є службовим, вибір заборонений.'");
	ИначеЕсли НРЕГ(РеквизитыФайла.Расширение) <> "xls"
		...
		Результат.ЧтениеВозможно = Ложь;
		Результат.ТекстОшибки    = НСтр("ru='Файл не является файлом формата Excel или Open Office (ods), выбор запрещен.';uk='Файл не є файлом формату Excel або Open Office (ods), вибір заборонений.'");
	КонецЕсли;
...
Процедура ОтразитьИзмененияБюджета(Режим = "", СтрокиОснования = Неопределено, Расшифровка = Неопределено)
	...
	Иначе
		НаименованиеЗадания = НСтр("ru='Подготовка бюджета';uk='Підготовка бюджету'");
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			СобытиеЖР = НСтр("ru='Выполнение операции';uk='Опрацювання операції'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция МожноОбновитьБюджет(ПроверкаСПредупреждениями = Истина)
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Если ФормироватьБюджетыПоОрганизациям Тогда
			НезаполненныеПоля.Добавить(НСтр("ru='Организация';uk='Організація'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.Подразделение) Тогда
		...
		Если ФормироватьБюджетыПоПодразделениям Тогда
			НезаполненныеПоля.Добавить(НСтр("ru='Подразделение';uk='Підрозділ'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если НезаполненныеПоля.Количество() > 0 Тогда
		Если ПроверкаСПредупреждениями Тогда
			...
			ШаблонСообщенияСостояния = ?(НезаполненныеПоля.Количество() = 1,
				НСтр("ru='Заполните поле %1';uk='Заповніть поле %1'"), НСтр("ru='Заполните поля: %1';uk='Заповніть поля: %1'"));
...
Процедура СкопироватьПоПериодамПоРасшифровке(Расшифровка, Значение, ИсключаяПериоды = Неопределено)
	...
	Для Каждого Период Из МассивПериодов Цикл
		...
		ИмяКолонки = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность);
...
Функция СоздатьПрисоединенныйФайлXLS97(ИДТаблицы, КолонкиПоИзмерениям)
	...
	ПредставлениеТекущейДатыДляИмениФайла = Формат(ТекущаяДатаСеанса(), НСтр("ru='ДФ=''dd.MM.yyyy ЧЧ.мм.сс''';uk='ДФ=''dd.MM.yyyy ЧЧ.мм.сс'''"));
...
Функция ЗагрузитьИзExcelНаКлиенте(ПрисоединенныйФайл)
	...
	Если Не МожноВыгрузитьТекущуюТаблицу(Бюджет.ТекущаяОбласть.Верх, ИДТаблицы, КолонкиПоИзмерениям) Тогда
		ТекстПредупреждения = НСтр("ru='Невозможно загрузить в текущую таблицу, т.к. ее структура не однородна!';uk='Неможливо завантажити в поточну таблицю, тому що її структура не однорідна!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат Ложь;
	КонецЕсли;
		...
		Для Каждого СтрокаДанных Из СтрокиДанных Цикл
			...
			Для Каждого Расшифровка Из СтруктураАналогРасшифровки Цикл
				...
				Если НайденныеСтроки.Количество() > 1 Тогда
					ТекстОшибки = НСтр("ru='Ошибка в структуре вида бюджета';uk='Помилка в структурі виду бюджету'");
...
Процедура ДобавитьПримечание(Команда)
	...
	ТекстПодсказки = НСтр("ru='Примечание';uk='Примітка'");
...
Процедура ДобавитьСтроку(Команда)
	...
	Если ПараметрыРедактирования <> Неопределено Тогда
		ДобавитьСтрокуВРежимеТаблицы(ПараметрыРедактирования);
	Иначе
		ТекстПредупреждения = НСтр("ru='Операция не доступна, аналитика не динамическая.';uk='Операція не доступна, аналітики не динамічна.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура УдалитьСтроку(Команда)
	...
	УдалитьСтрокуНаСервере(МассивСтрок, МассивРасшифровокПоСтрокам, ПараметрыВыводаВидаБюджета);
...
&НаСервере
Процедура УдалитьСтрокуНаСервере(МассивСтрок, МассивРасшифровокПоСтрокам, АдресПараметровВидаБюджета)
...
Процедура ДобавитьСтрокуВБюджетЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВставки = "ДублиСтрок" Тогда
		ТекстПредупреждения = НСтр("ru='Строка с такими значениями уже есть в бюджете';uk='Рядок з такими значеннями вже є в бюджеті'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
		Модифицированность = Истина;
	КонецЕсли;
...
Функция ВставитьОбластьНаСервере(СоответствиеИзмерений, ДополнительныеПараметры)
	...
	Если Не ЗначениеЗаполнено(КонечнаяСтрока) Тогда
		ТекстПредупреждения = НСтр("ru='Ошибка в структуре вида бюджета';uk='Помилка в структурі виду бюджету'");
	...
	Для Каждого СтрокаСтруктурыЯчеек Из НайденныеСтроки Цикл
		...
		Для Каждого СтрокаПериодов Из ТаблицаПериодов Цикл
			...
			Если ДобавленныеПериоды.Получить(ПериодСтрокой) = Неопределено Тогда
				БюджетнаяОтчетностьВыводСервер.ДобавитьСтрокуДанныхЯчеек(
					ДанныеЯчеек,
...
Процедура УстановитьДоступностьКнопокПриСменеСтраницы()
	...
	Если Элементы.ПанельБюджет.ТекущаяСтраница = Элементы.Таблица Тогда
		...
		Элементы.РедактироватьСписком.Заголовок = НСтр("ru='Редактировать в списке';uk='Редагувати в списку'");
	...
	Иначе
		Элементы.РедактироватьСписком.Заголовок = НСтр("ru='Редактировать в таблице';uk='Редагувати в таблиці'");
