...
Функция МассивЗначенийЗагружаемогоФайла(АдресФайлаДанных, РасширениеФайла, КоличествоКолонокПоСтруктуреБюджета)
	...
	Исключение
		ТекстОшибки = НСтр("ru='Не удалось прочитать выбранный файл по причине %1';uk='Не вдалося розпізнати обраний файл через %1'");
	...
	Исключение
		...
		СобытиеЖР = НСтр("ru='Бюджетирование.Удаление временного файла загрузки данных';uk='Бюджетування.. Вилучення тимчасового файлу завантаження даних'",КодЯзыка);
		ТекстОшибки = НСтр("ru='Не удалось удалить временный файл загрузки данных по причине %1';uk='Не вдалося вилучити тимчасовий файл завантаження даних з прични %1'",КодЯзыка);
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
