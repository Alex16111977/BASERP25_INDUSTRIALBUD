...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.РеализацияУслугПрочихАктивов.ПараметрыНастройкиСчетовУчетаОперации(Объект.ХозяйственнаяОперация);
	...
	Если Объект.РеализацияНаОсновании Тогда
		...
		Если ИспользуетсяУчетВНА_2_4 Тогда
			Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав")
				...
				СписокТипов.Добавить("Документ.ПодготовкаКПередачеОС2_4.ФормаВыбора", НСтр("ru='Подготовка к передаче ОС';uk='Підготовка до передачі ОЗ'"));
			Иначе
				СписокТипов.Добавить("Документ.ПодготовкаКПередачеНМА2_4.ФормаВыбора", НСтр("ru='Подготовка к передаче НМА';uk='Підготовка до передачі НМА'"));
				СписокТипов.Добавить("Документ.ПодготовкаКПередачеОС2_4.ФормаВыбора", НСтр("ru='Подготовка к передаче ОС';uk='Підготовка до передачі ОЗ'"));
			КонецЕсли;
		КонецЕсли; 
		...
		Иначе
			...
			СписокТипов.ПоказатьВыборЭлемента(
				...
				НСтр("ru='Документ основание';uk='Документ підстава'"));
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.РеализацияУслугПрочихАктивов.ПараметрыНастройкиСчетовУчетаОперации(Объект.ХозяйственнаяОперация);
	НастройкаСчетовУчета.ПриИзмененииПараметровНастройки(ЭтаФорма, ПараметрыНастройки);
	//-- НЕ УТ
...
Процедура УстановитьВидимостьСатьиАктиваПассива()
	...
	Элементы.РасходыВнеоборотныйАктив.Заголовок = НСтр("ru='Внеоборотный актив';uk='Необоротний актив'");
	Если ОтложенныйПереходПраваСобственности Тогда
		Элементы.РасходыВнеоборотныйАктив.Заголовок = НСтр("ru='Основное средство';uk='Основний засіб'");
	КонецЕсли;
...
Процедура УстановитьЗаголовокСтраницыРасходы()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияВнеоборотныхАктивов Тогда
		Элементы.СтраницаРасходы.Заголовок = НСтр("ru='Необоротные активы';uk='Необоротні активи'");
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав Тогда
		Элементы.СтраницаРасходы.Заголовок = НСтр("ru='Основные средства';uk='Основні засоби'");
	Иначе
		Элементы.СтраницаРасходы.Заголовок = НСтр("ru='Себестоимость (расходы)';uk='Собівартість (витрати)'");
	КонецЕсли; 
