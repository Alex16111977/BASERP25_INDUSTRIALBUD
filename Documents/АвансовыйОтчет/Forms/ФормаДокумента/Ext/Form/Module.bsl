...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли КнопокСоздания > 1 Тогда
		...
		ЭлементСоздания = Элементы.СписокДокументовЗакупкиЗакупкиСоздатьПТиУ;
		ЭлементСоздания.Заголовок = НСтр("ru='Приобретение товаров и услуг';uk='Придбання товарів і послуг'");
		Элементы.Переместить(ЭлементСоздания, Элементы.СписокДокументовЗакупкиГруппаСоздать, Элементы.СписокДокументовЗакупкиЗакупкиСоздатьПДД);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	//++ НЕ УТ
	Если ИмяСобытия = "Запись_ПоступлениеДенежныхДокументов" Тогда
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	|			ДанныеДокументаПереопределяемый.Ссылка.НомерВходящегоДокумента
	|		ИНАЧЕ
	...
	|			ДанныеДокументаПереопределяемый.Ссылка.ДатаВходящегоДокумента
	|		ИНАЧЕ
	...
	|			ДанныеДокументаПереопределяемый.Ссылка.НаименованиеВходящегоДокумента
	|		ИНАЧЕ
	|			ДанныеДокументаПереопределяемый.НаименованиеВходящегоДокумента
	|	КОНЕЦ КАК НаименованиеВходящегоДокумента,
	|	ДанныеДокументаПереопределяемый.Ссылка.Валюта                           КАК Валюта,
	|	ДанныеДокументаПереопределяемый.Ссылка.СтатьяДвиженияДенежныхСредств    КАК СтатьяДвиженияДенежныхСредств,
	|	ДанныеДокументаПереопределяемый.Ссылка                                  КАК ДокументЗакупки,
	|	ДанныеДокументаПереопределяемый.Ссылка.Контрагент                       КАК Контрагент,
	...
	|ГДЕ
	|	ДанныеДокументаПереопределяемый.Ссылка В (&СписокДокументовЗакупки)
	|	И ДанныеДокументаПереопределяемый.Ссылка.Проведен
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА ДанныеДокументаПереопределяемый.НомерВходящегоДокумента = """" ТОГДА
	|			ДанныеДокументаПереопределяемый.Ссылка.НомерВходящегоДокумента
	...
	|			ДанныеДокументаПереопределяемый.НаименованиеВходящегоДокумента
	|	КОНЕЦ,
	|	ДанныеДокументаПереопределяемый.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
