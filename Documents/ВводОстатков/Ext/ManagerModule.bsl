...
Функция ТекстЗапросаТаблицаДенежныеСредстваБезналичные(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаБанковскиеСчета.Подразделение   КАК Подразделение,
...
Функция ОстаткиПоТоварам(ДатаОстатков, ХозяйственнаяОперация, МассивОрганизаций, ДопОтбор, ПараметрыЗаполненияОстатков)
	...
	Если ЗаполнятьПоСебестоимости Тогда
        ...
        |	ВЫБОР
        |		КОГДА ЕСТЬNULL(НастройкиСистемыНалогообложенияСрезПоследних.ПлательщикНДС, ЛОЖЬ)
        ...
        |	КОНЕЦ КАК НалоговоеНазначение
		|ПОМЕСТИТЬ НалоговыеНазначенияОрганизаций
		|ИЗ
		...
		|	&ХозяйственнаяОперация,
        |	ВЫБОР
        |		КОГДА СебестоимостьТоваровОстатки.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА СебестоимостьТоваровОстатки.НалоговоеНазначение
        |		КОГДА ЕСТЬNULL(СебестоимостьТоваровОстатки.ВидЗапасов.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА НалоговыеНазначенияОрганизаций.НалоговоеНазначение
        |		ИНАЧЕ СебестоимостьТоваровОстатки.ВидЗапасов.НалоговоеНазначение
        |	КОНЕЦ КАК НалоговоеНазначение,
		...
		|					КОНЕЦ
		|			КОНЕЦ)
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НалоговыеНазначенияОрганизаций КАК НалоговыеНазначенияОрганизаций
        |		ПО СебестоимостьТоваровОстатки.Организация = НалоговыеНазначенияОрганизаций.Организация
        ...
        |	КОНЕЦ КАК НалоговоеНазначение,
		|	ДанныеАналитики.Подразделение.* КАК Подразделение,
		|	ДанныеАналитики.СкладскаяТерритория.* КАК Склад,
		|	ДанныеАналитики.Партнер.* КАК Партнер}
		|
		|УПОРЯДОЧИТЬ ПО
		|	Организация,
	...
	Если Не ДопОтбор = Неопределено Тогда
		...
		Для Каждого УсловиеОтбора Из ДопОтбор Цикл
			...
			Если Запрос.ДоступныеПоля.Найти(УсловиеОтбора.Ключ) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(ДокументОбъект.УдалитьРазделУчета) Тогда
					...
					//++ НЕ УТ
					Если Документы.ВводОстатков.ТипыОперацийНеОтражаемыеВРеглУчете().Найти(ДокументОбъект.УдалитьТипОперации) = Неопределено Тогда
						ДокументОбъект.УдалитьРазделУчета = Перечисления.УдалитьРазделыВводаНачальныхОстатков.ОперативныйИРегламентированныйУчет;
					КонецЕсли;
					//-- НЕ УТ
