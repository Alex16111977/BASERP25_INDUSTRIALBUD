...
Процедура ЗаполнитьМатериалыИУслугиПоНормативам(Команда)
	...
	Если Объект.МатериалыИУслуги.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьМатериалыИУслугиПоСпецификацииЗавершение", ЭтотОбъект), НСтр("ru='Список ""Материалы и работы"" будет перезаполнен. Продолжить?';uk='Список ""Матеріали і роботи"" буде перезаповнений. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ПодобратьИзОбособленныхОстатков(Команда)
	...
	ПараметрыПроверки.ПроверяемыеРеквизиты.Вставить("Назначение",  НСтр("ru='Назначение';uk='Призначення'"));
...
Процедура ЗаполнитьПродукцию(УчитыватьЛимиты)
	...
	Если Объект.ВыходныеИзделия.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПродукциюПоСпецификацииЗавершение", ЭтотОбъект, ДополнительныеПараметры),
			НСтр("ru='Список ""Продукция"" будет перезаполнен. Продолжить?';uk='Список ""Продукція"" буде перезаповнений. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Процедура ЗаполнитьВозвратныеОтходы(УчитыватьЛимиты)
	...
	Если Объект.ВозвратныеОтходы.Количество() > 0 Тогда
		...
		ДополнительныеПараметры = Новый Структура("УчитыватьЛимиты", УчитыватьЛимиты);
			...
			НСтр("ru='Список ""Возвратные отходы"" будет перезаполнен. Продолжить?';uk='Список ""Зворотні відходи"" буде перезаповнений. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
...
Функция РеквизитыЗаполнены(МассивПроверок)
	...
	Если МассивПроверок.Найти("Номенклатура") <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(Объект.Номенклатура) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указана основная продукция, на которую списываются затраты.';uk='Не вказана основна продукція, на яку списуються витрати.'"),
			Объект.Ссылка,
			"Объект.Номенклатура");
