	...
	Иначе
		...
		Для Каждого Элемент Из МассивОбъектов Цикл
			...
			ИначеЕсли РегистрацияОшибок Тогда
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Списание затрат на выпуск';uk='Списання витрат на випуск'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("a38a5d01-3590-48cc-aab8-6aafe79d0a0c");
	...
	Обработчик.Комментарий = НСтр("ru='В документе ""Списание затрат на выпуск"" переносит назначение из вида запасов в аналитику учета номенклатуры. До завершения обработчика работа с документами не возможна.';uk='У документі ""Списання витрат за випуск"" переносить призначення з виду запасів в аналітику обліку номенклатури. До завершення обробника робота з документами неможлива.'");
...
#Область СгенерироватьКлючиАналитикиНоменклатуры
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("110c83fe-5538-40d0-8732-650ce2ff94b7");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	Обработчик.БлокируемыеОбъекты = "Справочник.КлючиАналитикиУчетаНоменклатуры,"
		+ "РегистрСведений.АналитикаУчетаНоменклатуры";
	Обработчик.Комментарий = НСтр("ru='Генерирует ключи аналитики учета номенклатуры по данным документа ""Списание затрат на выпуск"". До завершения обработчика работа с документами не возможна.';uk='Генерує ключі аналітики обліку номенклатури за даними документа ""Списання витрат на випуск"". До завершення обробника робота з документами неможлива.'");
	...
	НоваяСтрока.ИмяОбъекта = "Документы.СписаниеЗатратНаВыпуск";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.СписаниеЗатратНаВыпуск.МатериалыИУслуги КАК Материалы
	|ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.СписаниеЗатратНаВыпуск";
	МетаданныеДокумента = Метаданные.Документы.СписаниеЗатратНаВыпуск;
...
Процедура ЗарегистрироватьДанныеКОбновлениюКлючейАналитики(Параметры) Экспорт
	...
	|	ПО КлючиКГенерации.Ссылка = Материалы.АналитикаУчетаНоменклатуры
	|		И КлючиКГенерации.Назначение = Материалы.Назначение
	|ГДЕ
	|	Материалы.АналитикаУчетаНоменклатуры.Назначение <> Материалы.Назначение
	|	И Материалы.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|	И Материалы.Ссылка.Проведен
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоРегистратору", " ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.СписаниеЗатратНаВыпуск)");
...
Процедура СгенерироватьКлючиАналитикиНоменклатуры(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		|ИЗ
		|	Документ.СписаниеЗатратНаВыпуск.МатериалыИУслуги КАК Товары
		|
