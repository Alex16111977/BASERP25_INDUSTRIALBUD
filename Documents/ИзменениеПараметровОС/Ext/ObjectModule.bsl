...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ СпособНачисленияАмортизацииБУФлаг 
	 ...
	 Если СпособНачисленияАмортизацииНУФлаг И (СпособНачисленияАмортизацииНУ = Перечисления.СпособыНачисленияАмортизацииОС.ПропорциональноОбъемуПродукции) Тогда
		Если СпособНачисленияАмортизацииБУФлаг И (СпособНачисленияАмортизацииБУ <> Перечисления.СпособыНачисленияАмортизацииОС.ПропорциональноОбъемуПродукции) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСообщения, ЭтотОбъект, "СпособНачисленияАмортизацииБУ",, Отказ);
		ИначеЕсли НЕ СпособНачисленияАмортизацииБУФлаг Тогда 
	
			Запрос = Новый Запрос;
			Запрос.Текст = 
				"ВЫБРАТЬ
				|	ВЫРАЗИТЬ(ИзменениеПараметровОСОС.ОсновноеСредство КАК Справочник.ОбъектыЭксплуатации) КАК ОсновноеСредство
				|ПОМЕСТИТЬ ДанныеДокумента
				|ИЗ
				|	Документ.ИзменениеПараметровОС.ОС КАК ИзменениеПараметровОСОС
				|ГДЕ
				|	ИзменениеПараметровОСОС.Ссылка = &Ссылка
				|
				|ИНДЕКСИРОВАТЬ ПО
				|	ОсновноеСредство
				|;
				...
				|						ИЗ
				|							ДанныеДокумента КАК ДанныеДокумента)
				|					И Регистратор <> &Ссылка) КАК ПервоначальныеСведенияОС
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.ИзменениеПараметровОС.ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(
		НСтр("ru='Изменение параметров ОС';uk='Зміна параметрів ОЗ'"));
...
Процедура ЗаполнитьПоКорректировкеНалоговогоНазначения(Основание)
	...
	Запрос = Новый Запрос(
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МестонахождениеОС.Местонахождение КАК Подразделение,
		|	СпособыОтраженияРасходов.СтатьяРасходовБУ КАК СтатьяРасходов,
		|	СпособыОтраженияРасходов.АналитикаРасходовБУ КАК АналитикаРасходов,
		|	1 КАК Коэффициент
		|ИЗ
		|	Документ.КорректировкаНалоговогоНазначенияОС.ОС КАК ТабличнаяЧасть
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокУчетаОСБУ.СрезПоследних(
		|				&Дата,
		|				(СпособОтраженияРасходовБУ В (ЗНАЧЕНИЕ(Документ.ИзменениеПараметровОС.ПустаяСсылка), ЗНАЧЕНИЕ(Документ.ИзменениеПараметровОС2_4.ПустаяСсылка), НЕОПРЕДЕЛЕНО))
		|					И ОсновноеСредство В
		|						(ВЫБРАТЬ
		|							Т.ОсновноеСредство
		|						ИЗ
...
|;uk='Введення зміни параметрів ОЗ на підставі групи ОЗ неможливо! 
|Виберіть ОЗ. Для розкриття групи використовуйте клавіші Ctrl і стрілку вниз'");
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru='Основное средство ""%1"" не принято к учету.';uk='Основний засіб ""%1"" не прийнято до обліку.'"), Строка(Основание));
		ВызватьИсключение ТекстСообщения;
	КонецЕсли; 
...
|;uk='Рахунок "" %1"" вже використовується в якості рахунку обліку цільових коштів у вартості одного або декількох основних засобів.
...
|;uk='Рахунок "" %1"" вже використовується в якості рахунку обліку цільових коштів в накопиченої амортизації одного або декількох основних засобів. 
|Потрібно вибрати інший рахунок обліку накопиченої амортизації ОС.'");
...
Процедура СформироватьЗаданияКЗакрытиюМесяцаПриЗаписи()
	...
	ВнеоборотныеАктивы.СформироватьЗаписиРегистровЗаданий(Ссылка, ДополнительныеСвойства.ДанныеТаблиц);
