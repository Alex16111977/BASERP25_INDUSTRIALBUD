...
Процедура ОбъектыКалькуляцииПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Не ЗначениеЗаполнено(Объект.ОбъектКалькуляции) Тогда
		...
		ТекстСообщения = НСтр("ru='Перед добавлением требуется выбрать объект калькуляции.';uk='Перед додаванням потрібно вибрати об''єкт калькуляції.'");
...
Процедура ОбъектыКалькуляцииПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не НоваяСтрока Или НоваяСтрока И Не ОтменаРедактирования Тогда
		...
		Если Объект.ОбъектыКалькуляции.НайтиСтроки(ОтборДублей).Количество() > 1 Тогда
			...
			ТекстОшибки = НСтр("ru='В списке уже существует строка с таким же набором значений.';uk='У списку вже існує рядок з таким самим набором значень.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
			Отказ = Истина;
...
Функция ДобавитьОбъектыКалькуляции(ВыбранноеЗначение)
	...
	Если КоличествоДублей = ВыбранноеЗначение[ИсточникВыходныхИзделий].Количество() Тогда
		ТекстОшибки = НСтр("ru='Текущий объект уже подобран в документ.';uk='Поточний об''єкт вже підібрано в документ.'");
	КонецЕсли;
...
Функция ПараметрыОбъектаКалькуляции(ОбъектКалькуляции)
	...
	Если ОбъектКалькуляции = ПредопределенноеЗначение("Перечисление.ОбъектыКалькуляции.ЗаказНаПроизводство") Тогда
		...
		Параметры.Вставить("ЗаголовокСтраницы", НСтр("ru='Заказы на производство';uk='Замовлення на виробництво'"));
		Параметры.Вставить("ЗаголовокКолонки", НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'"));
	...
	ИначеЕсли ОбъектКалькуляции = ПредопределенноеЗначение("Перечисление.ОбъектыКалькуляции.РесурснаяСпецификация") Тогда
		...
		Параметры.Вставить("ЗаголовокСтраницы", НСтр("ru='Ресурсные спецификации';uk='Ресурсні специфікації'"));
		Параметры.Вставить("ЗаголовокКолонки", НСтр("ru='Ресурсная спецификация';uk='Ресурсна специфікація'"));
	...
	Иначе
		...
		Параметры.Вставить("ЗаголовокСтраницы", НСтр("ru='Объекты калькуляции';uk='Об''єкти калькуляції'"));
		Параметры.Вставить("ЗаголовокКолонки", НСтр("ru='Объект калькуляции';uk='Об''єкт калькуляції'"));
