...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбъектКалькуляции = Перечисления.ОбъектыКалькуляции.ЗаказНаПроизводство Тогда
		...
		ТекстОшибки = НСтр("ru='Перед расчетом требуется запланировать %1';uk='Перед розрахунком потрібно запланувати %1'");
...
Процедура ПроверитьВозможностьВводаНаОсновании(Основания)
	...
	Если Выборка.НомерОшибки = 0 Тогда
		ТекстИсключения = НСтр("ru='Калькуляция разборки, утилизации не поддерживается.';uk='Калькуляція розбирання, утилізації не підтримується.'");
	Иначе
		ТекстИсключения = НСтр("ru='На основании спецификации для вида номенклатуры ввод калькуляции недоступен.';uk='На підставі специфікації для виду номенклатури введення калькуляції недоступний.'");
	КонецЕсли;
