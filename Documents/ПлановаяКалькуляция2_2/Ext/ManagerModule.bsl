...
Процедура ЗаполнитьДокументУстановкаЦенНоменклатуры(ДокументУстановки, ДанныеЗаполнения) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(ПлановыйВидЦены) Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не установлен плановый вид цены.';uk='Не встановлений плановий вид ціни.'"),
			ДанныеЗаполнения);
		ВызватьИсключение Текст;
...
Процедура ЗаполнитьГруппыПроверокДляРегистрации(ТаблицаГруппПроверок) Экспорт
	...
	ОписаниеГруппыПроверок.Наименование = НСтр("ru='Калькуляции продукции';uk='Калькуляції продукції'");
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Наличие движений с нулевой стоимостью.';uk='Наявність рухів з нульовою вартістю.'",ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Для всей номенклатуры и видов работ должны быть назначены плановые цены.';uk='Для всієї номенклатури і видів робіт повинні бути призначені планові ціни.'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаДвиженийСНулевойСтоимостью(ПараметрыПроверки) Экспорт
	...
	РегистрыСведений.МножителиПартийИПолуфабрикатов.ПолучитьЦеныНоменклатуры(Калькуляции, МенеджерВременныхТаблиц, Истина);
	...
	Пока ВыборкаПоОрганизациям.Следующий() Цикл
		...
		АудитСостоянияСистемы.ДобавитьСтрокуВТаблицуВыявленныхПроблем(
			...
			'00010101', 
			НСтр("ru='Присутствуют движения по материалам или видам работы с нулевой стоимостью.';uk='Присутні рухи по матеріалах або видах роботи з нульовою вартістю.'"));
		...
		Пока ВыборкаДетальная.Следующий() Цикл
			...
			Если ВыборкаДетальная.ТипЗатраты = "Материальные" Тогда
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не заданы плановые цены для %1 позиций номенклатуры.';uk='Не задані планові ціни для %1 позицій номенклатури.'"),
					ВыборкаДетальная.КоличествоСтрок);
			Иначе
				Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не заданы плановые расценки для %1 видов работ.';uk='Не задані планові розцінки для %1 видів робіт.'"),
					ВыборкаДетальная.КоличествоСтрок);
				КонецЕсли;
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.ПлановаяСебестоимостьПродукции) Тогда
		...
		КомандаОтчет.Представление		= НСтр("ru='Плановая себестоимость продукции';uk='Планова собівартість продукції'");
	...
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.ОшибкиРасчетаКалькуляцииПродукции) Тогда
		...
		КомандаОтчет.Представление               = НСтр("ru='Протокол расчета';uk='Протокол розрахунку'");
		КомандаОтчет.ВидимостьВФормах            = "ФормаДокумента, ФормаСписка";
		КомандаОтчет.Порядок                     = 1;
		КомандаОтчет.Важность = "Важное";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Сводная калькуляция';uk='Зведена калькуляція'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПлановаяКалькуляцияПодробно";
	КомандаПечати.Представление = НСтр("ru='Подробная калькуляция';uk='Детальна калькуляція'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПротоколОшибок";
	КомандаПечати.Порядок = 60;
	КомандаПечати.Представление = НСтр("ru='Протокол ошибок';uk='Протокол помилок'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Функция ОписаниеПараметраЗаполненияЦенВКоллецкии() Экспорт
...
Процедура ЗаполнитьЦеныПоКалькуляцииВКоллекции(Коллекция, НаборСтрок, ПараметрыЗаполнения) Экспорт
	...
	Если ТребуетсяПересчет Тогда
		...
		ПараметрыКурсаКалькуляции = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаУпр, 
			ПараметрыЗаполнения.ДатаДокумента);
		ПараметрыКурсаДокумента = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ПараметрыЗаполнения.ВалютаДокумента, 
			ПараметрыЗаполнения.ДатаДокумента); 
	...
	Для Каждого ДанныеСтроки Из НаборСтрок Цикл
		...
		Пока Выборка.НайтиСледующий(ОтборСтрок) Цикл
			...
			Если ТребуетсяПересчет Тогда
				ДанныеСтроки.Цена = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Выборка.Цена, 
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПлановаяКалькуляцияСводно") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПлановаяКалькуляцияСводно", НСтр("ru='Калькуляция продукции (сводно)';uk='Калькуляція продукції (зведено)'"), СформироватьПечатнуюФормуКалькуляции(МассивОбъектов, ОбъектыПечати, Истина, ПараметрыВывода)
		,,,, Истина);
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПлановаяКалькуляцияПодробно") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПлановаяКалькуляцияПодробно", НСтр("ru='Калькуляция продукции (подробно)';uk='Калькуляція продукції (детально)'"), СформироватьПечатнуюФормуКалькуляции(МассивОбъектов, ОбъектыПечати, Ложь, ПараметрыВывода)
		,,,, Истина);
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПротоколОшибок") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПротоколОшибок", НСтр("ru='Протокол ошибок';uk='Протокол помилок'"), СформироватьПечатнуюФормуПротоколаОшибок(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
		,,,, Истина);
	КонецЕсли;
...
Функция СформироватьПечатнуюФормуКалькуляции(МассивОбъектов, ОбъектыПечати, Сводно, ПараметрыВывода)
	
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;

	УстановитьПривилегированныйРежим(Истина);
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПлановаяКалькуляция2_2.ПФ_MXL_ПлановаяКалькуляция", КодЯзыкаПечать);
	...
	Пока ДанныеШапки.Следующий() Цикл
		...
		ПараметрыОбласти.ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
			ДанныеШапки, НСтр("ru='Калькуляция продукции';uk='Калькуляція продукції'",КодЯзыкаПечать), КодЯзыкаПечать);
		...
		Для Каждого ДетальнаяЗапись Из ДетальныеЗаписиНабора Цикл
			...
			Если Не ТипЗатраты = ДетальнаяЗапись.ТипЗатраты Тогда
				...
				Если ДетальнаяЗапись.ТипЗатраты = "Материальные" Тогда
					...
					ПредставлениеТипаЗатрат = НСтр("ru='Материальные';uk='Матеріальні'",КодЯзыкаПечать);
				...
				ИначеЕсли ДетальнаяЗапись.ТипЗатраты = "ВозвратныеОтходы" Тогда
					...
					ПредставлениеТипаЗатрат = НСтр("ru='Возвратные отходы/Побочный выпуск';uk='Зворотні відходи/Побічний випуск'",КодЯзыкаПечать);
					ВозвратныеОтходы = ИтогоПоТипуЗатрат;
					
				ИначеЕсли ДетальнаяЗапись.ТипЗатраты = "ОплатаТруда" Тогда
					
					ПредставлениеТипаЗатрат = НСтр("ru='Оплата труда';uk='Оплата праці'",КодЯзыкаПечать);
					ОплатаТруда = ИтогоПоТипуЗатрат;
					
				Иначе
					
					ПредставлениеТипаЗатрат = НСтр("ru='Постатейные';uk='Постатейні'",КодЯзыкаПечать);
		...
		ПараметрыОбласти.Вставить("Валюта", ВалютаРеглУчета);
...
Функция СформироватьПечатнуюФормуПротоколаОшибок(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;

	УстановитьПривилегированныйРежим(Истина);
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПлановаяКалькуляция2_2.ПФ_MXL_ПротоколОшибок", КодЯзыкаПечать);
	...
	РегистрыСведений.МножителиПартийИПолуфабрикатов.ПолучитьЦеныНоменклатуры(МассивОбъектов, МенеджерВременныхТаблиц, Истина);
	...
	Для Каждого СтрокаРегистратора Из Затраты.Строки Цикл
		...
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Протокол ошибок %1';uk='Протокол помилок %1'",КодЯзыкаПечать),
			ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(СтрокаРегистратора.Регистратор, НСтр("ru='калькуляции продукции';uk='калькуляції продукції'",КодЯзыкаПечать), КодЯзыкаПечать));
