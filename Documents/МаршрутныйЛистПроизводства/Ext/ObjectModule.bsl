...
Процедура ПроверитьСоответствиеСтатусаИМетодикиУправления(Отказ)
	...
	Если Статус = Перечисления.СтатусыМаршрутныхЛистовПроизводства.Выполняется
		...
		Если УправлениеМаршрутнымиЛистами = Перечисления.УправлениеМаршрутнымиЛистами.ПооперационноеПланирование Тогда
			ПараметрМетодика = НСтр("ru='Пооперационное планирование';uk='Поопераційне планування'");
		Иначе
			ПараметрМетодика = НСтр("ru='Регистрация операций';uk='Реєстрація операцій'");
		КонецЕсли;
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Статус ""Выполняется"" не может быть установлен для документа с методикой управления ""%1"".';uk='Статус ""Виконується"" не може бути встановлений для документа з методикою управління ""%1"".'"),
			ПараметрМетодика);
