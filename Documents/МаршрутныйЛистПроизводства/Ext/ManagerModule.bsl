...
Функция ПроверитьПорядокОпераций(Операции, КлючДанных = Неопределено, ПутьКДанным = "", Отказ = Ложь) Экспорт
	...
	СтруктураСообщений.Вставить(
		"НетПервойОперации", НСтр("ru='Отсутствует первая операция.';uk='Відсутня перша операція.'"));
	
	СтруктураСообщений.Вставить(
		"НесколькоПервыхОпераций", НСтр("ru='Не может быть несколько первых операций.';uk='Не може бути кілька перших операцій.'"));
	
	СтруктураСообщений.Вставить(
		"НетПоследнейОперации", НСтр("ru='Отсутствует последняя операция.';uk='Відсутня остання операція.'"));
	
	СтруктураСообщений.Вставить(
		"НетСледующейОперации", НСтр("ru='Операция ""%1"" ссылается на несуществующую операцию.';uk='Операція ""%1"" посилається на неіснуючу операцію.'"));
	
	СтруктураСообщений.Вставить(
		"НомерСледующейБольше", НСтр("ru='Номер операции ""%1"" должен быть меньше номера следующей.';uk='Номер операції  ""%1"" повинен бути менше номера наступної.'"));
...
// Возвращает массив с именами объектов метаданных, которые читаются при заполнении документа на основании
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ЧитаемыеОбъекты = "Справочник.ПолитикиУчетаСерий,"
	...
	Обработчик.Комментарий = НСтр("ru='Переносит назначение в аналитику учета номенклатуры.
                                   ...
                                   |Перезаполняет статусы указания серий для материалов с политикой ""Управление по FEFO остатками серий"".
                                   ...
                                   |Перезаповнює статуси зазначення серій для матеріалів з політикою ""Управління по FEFO залишками серій"".
...
#Область СгенерироватьКлючиАналитикиНоменклатуры
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("cd02a465-0286-4ec0-a240-ff8194608f7c");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	Обработчик.БлокируемыеОбъекты = "Справочник.КлючиАналитикиУчетаНоменклатуры,"
		+ "РегистрСведений.АналитикаУчетаНоменклатуры";
	Обработчик.Комментарий = НСтр("ru='Генерирует ключи аналитики учета номенклатуры по данным документа ""Маршрутный лист производства"". До завершения обработчика работа с документами не возможна.';uk='Генерує ключі аналітики обліку номенклатури за даними документа ""Маршрутний лист виробництва"". До завершення обробника робота з документами неможлива.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	НоваяСтрока.ИмяОбъекта = "Документы.МаршрутныйЛистПроизводства";
...
// переносит назначение в аналитику учета номенклатуры.
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ГДЕ
	|		Материалы.АналитикаУчетаНоменклатуры.Назначение <> Материалы.Назначение
	|		И Материалы.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|		И Материалы.Ссылка.Проведен
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ИЗ
	|		Документ.МаршрутныйЛистПроизводства.ВыходныеИзделия КАК Изделия
	|	ГДЕ
	|		Изделия.АналитикаУчетаНоменклатуры.Назначение <> Изделия.Назначение
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				МестаУчета = РегистрыСведений.АналитикаУчетаНоменклатуры.МестаУчета(Неопределено, ДокументОбъект.Подразделение, ДокументОбъект.Подразделение, Неопределено);
				
				ПараметрыЗаполненияКлючейМатериалов = РегистрыСведений.АналитикаУчетаНоменклатуры.ПараметрыЗаполненияКлючейАналитики();
				РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(
					ДокументОбъект.МатериалыИУслуги, 
					МестаУчета, 
					Неопределено, 
					ПараметрыЗаполненияКлючейМатериалов
				);
				
				ПараметрыЗаполненияКлючейПродукции = РегистрыСведений.АналитикаУчетаНоменклатуры.ПараметрыЗаполненияКлючейАналитики();
				РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(
					ДокументОбъект.ВыходныеИзделия, 
					МестаУчета, 
					Неопределено, 
					ПараметрыЗаполненияКлючейПродукции
				);
				
				ОбъектИзменен = ОбъектИзменен ИЛИ ПараметрыЗаполненияКлючейМатериалов.ИзмененаАналитика ИЛИ ПараметрыЗаполненияКлючейПродукции.ИзмененаАналитика;
...
Процедура ЗарегистрироватьДанныеКОбновлениюКлючейАналитики(Параметры) Экспорт
	...
	|	ПО КлючиКГенерации.Ссылка = Данные.АналитикаУчетаНоменклатуры
	|		И КлючиКГенерации.Назначение = Данные.Назначение
	|ГДЕ
	|	КлючиКГенерации.Ссылка ЕСТЬ NULL
	|";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборПоРегистратору", " ТИПЗНАЧЕНИЯ(ДанныеРегистра.Регистратор) = ТИП(Документ.МаршрутныйЛистПроизводства)");
...
Процедура СгенерироватьКлючиАналитикиНоменклатуры(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.МаршрутныйЛистПроизводства";
	МетаданныеДокумента = Метаданные.Документы.МаршрутныйЛистПроизводства;
	...
	Пока Выборка.Следующий() Цикл
		...
		|		ПО Ключи.Ссылка = Материалы.АналитикаУчетаНоменклатуры
		|	ГДЕ
		|		Ключи.Назначение <> Материалы.Назначение
		|		И Материалы.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		И Материалы.Ссылка.Проведен
		...
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Ключи
		|		ПО Ключи.Ссылка = Изделия.АналитикаУчетаНоменклатуры
		|	ГДЕ
		|		Изделия.АналитикаУчетаНоменклатуры.Назначение <> Изделия.Назначение
		|		И Изделия.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		И Изделия.Ссылка.Проведен
		|		И Изделия.Ссылка = &Ссылка
		|	) КАК Поля
		|
