...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкаСчетовУчета.ПриСозданииНаСервере(ЭтаФорма, ПараметрыНастройки);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// НастройкаСчетовУчета
	ПараметрыНастройки = Документы.ПроизводствоБезЗаказа.ПараметрыНастройкиСчетовУчета();
	НастройкаСчетовУчета.ПриЧтенииНаСервере(ЭтаФорма, ПараметрыНастройки);
	// Конец НастройкаСчетовУчета
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// НастройкаСчетовУчета
	НастройкаСчетовУчета.ПередЗаписьюНаСервере(ЭтаФорма, ТекущийОбъект);
	// Конец НастройкаСчетовУчета
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// НастройкаСчетовУчета
	НастройкаСчетовУчета.ПослеЗаписиНаСервере(ЭтаФорма);
	// Конец НастройкаСчетовУчета
...
Процедура ВидЦеныПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦены) 
		...
		ТекстВопроса = НСтр("ru='Перезаполнить цены изделий?';uk='Перезаповнити ціни виробів?'");
...
Процедура ТаблицаГруппаЗатратНачалоВыбора(ИмяТЧ)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых будет заполнена группа затрат.';uk='Необхідно вибрати рядки, для яких буде заповнена група витрат.'");
...
Процедура ТаблицаНаправлениеВыпускаПриИзменении(ИмяТЧ)
	...
	ДоходыИРасходыКлиентСервер.СтатьяПриИзменении(ЭтаФорма, Элементы[ИмяТЧ + "Получатель"]);
...
Процедура УстановитьУсловноеОформление()
	...
	// Видимость команд заполнения плановой стоимости.
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ВыходныеИзделияЗаполнитьЦеныПоВидуЦен.Имя);
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ВыходныеИзделияЗаполнитьЦеныПоКалькуляции.Имя);
...
Процедура УстановитьУсловноеОформлениеНаправлениеВыпуска(ИмяТЧ)
	...
	ПолеНаправлениеВыпуска = Элементы[ИмяТЧ + "НаправлениеВыпуска"]; // ПолеФормы
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект." + ИмяТЧ + ".ОформитьОтчетДавальцу");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
...
Процедура УстановитьУсловноеОформлениеПодразделениеРасходов(ИмяТЧ, ИмяПоля)
	...
	ПолеФормы = Элементы[ИмяТЧ + ИмяПоля]; // ПолеФормы
...
Процедура УстановитьЗаголовкиГруппПродукции(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ВыходныеИзделия")
		...
		НадписьПродукция = НСтр("ru='Продукция по рассчитываемой стоимости%1';uk='Продукція за розраховуваною вартістю %1'");
		НадписьПродукция =
	...
	Если СтруктураРеквизитов.Свойство("ПобочныеИзделия")
			ИЛИ Инициализация Тогда
		НадписьПобочныеИзделия = НСтр("ru='Побочный и промежуточный выпуск по фиксированной стоимости%1';uk='Побічний і проміжний випуск за фіксованою вартістю %1'");
...
Процедура НастроитьЭлементыГруппировкиЗатрат(Инициализация)
	...
	ЗаголовокГруппыЗатрат = НСтр("ru='Группа затрат%1';uk='Група витрат %1'");
	...
	ПодсказкаГруппыПобочныеИзделия	= НСтр("ru='%1, к которой относится возвратный отход';uk=' %1, до якої відноситься зворотний відхід'");
	ПодсказкаГруппыМатериалыИРаботы	= НСтр("ru='%1, к которой относится материал/работа';uk=' %1, до якої відноситься матеріал/робота'");
	ПодсказкаГруппыТрудозатраты		= НСтр("ru='%1, к которой относится трудозатрата';uk=' %1, до якої відноситься трудовитрата'");
...
Процедура ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТЧ,
	...
	Если ИмяТЧ = "ВыходныеИзделия"
		Или ИмяТЧ = "ПобочныеИзделия" Тогда
		НастройкаСчетовУчета.ОбновитьПредставлениеНастройки(ЭтаФорма, "Объект." + ИмяТЧ, МассивИдентификаторовСтрок);
	КонецЕсли;
...
Процедура ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(
	...
	Если ИмяТЧ = "ВыходныеИзделия"
		...
		Если ТекущаяСтрока = Неопределено
			Или Не ТекущаяСтрока.НаправлениеВыпуска =
				ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад") Тогда
...
Функция ПредставлениеГруппыЗатрат(Группа, ГруппировкаЗатрат)
	...
	Иначе
		ПредставлениеГруппы = НСтр("ru='<группа с пустыми значения>';uk='<група з порожніми значення>'");
	КонецЕсли;
...
|;uk='У таблиці ""Вихідні вироби"" не вибрано жодного рядка. 
|Ціни не можуть бути заповнені.'");
			Иначе
				ТекстСообщения = НСтр("ru='В таблице ""Побочные изделия"" не выбрано ни одной строки. 
|Цены не могут быть заполнены.'
|;uk='У таблиці ""Побічні вироби"" не вибрано жодного рядка. 
|Ціни не можуть бути заповнені.'");
			КонецЕсли;
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, ИмяТЧ, "Объект", Отказ);
...
Процедура ЗаполнитьСписокВыбораНаправлениеВыпуска(ИмяТЧ, ТекущиеДанные)
	...
	Если ИспользоватьСписаниеНаРасходы Тогда
		СписокВыбораЭлемента.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию"),
									НСтр("ru='Списать на расходы';uk='Списати на витрати'"));
	КонецЕсли;
...
Процедура ЗаполнитьСклад(ИмяТЧ, ИмяПоля)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых необходимо заполнить склад.';uk='Необхідно вибрати рядки, для яких необхідно заповнити склад.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПодразделениеЗатрат(ИмяТЧ)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых необходимо заполнить подразделение затрат.';uk='Необхідно вибрати рядки, для яких необхідно заповнити підрозділ витрат.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ПроизводствоБезЗаказа.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
//  Параметры - Структура            - параметры заполнения см. ПараметрыЗаполненияНаправлениеДеятельности()
...
Процедура ОкруглитьДоЦелойУпаковки(ИмяТЧ)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых необходимо округлить количество.';uk='Необхідно вибрати рядки, для яких необхідно округлити кількість.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстПредупреждения);
		Возврат;
	КонецЕсли;
