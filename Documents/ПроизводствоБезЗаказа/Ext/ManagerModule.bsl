...
// 				см. ОбщегоНазначенияУТ.ПараметрыПроверкиЗаполненияКоличества
// 
...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр("ru='Производство без заказа';uk='Виробництво без замовлення'",КодОсновногоЯзыка);
...
Процедура ИнициализироватьКлючиАналитикиНоменклатуры(Запрос)
	...
	|				   ИЛИ (СпрСклады.ЦеховаяКладовая И СпрСклады.Подразделение <> &Подразделение)
	|               )
	|       )
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|			ИЛИ СпрСклады.Ссылка ЕСТЬ НЕ NULL
	|				И (НЕ СпрСклады.ЦеховаяКладовая 
	|				   ИЛИ (СпрСклады.ЦеховаяКладовая И СпрСклады.Подразделение <> ТаблицаТовары.Подразделение)
	|               )
	|       )
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
...
|;uk='У документі %ПолноеИмяДокумента% не реалізована адаптація тексту запиту 
|формування рухів за регістром %ИмяРегистра%.'");
...
Функция ТекстЗапросаВтТовары(Запрос, ТекстыЗапроса)
	...
	|	НЕОПРЕДЕЛЕНО										КАК КорАналитикаФинансовогоУчета,
    |	ВЫБОР
    |		КОГДА ЕСТЬNULL(ТаблицаТовары.Назначение.НалоговоеНазначение,ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) <>
    |				ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
    |			ТОГДА ТаблицаТовары.Назначение.НалоговоеНазначение
    |		ИНАЧЕ &НалоговоеНазначение
    |	КОНЕЦ 												КАК НалоговоеНазначение,
	|	НЕОПРЕДЕЛЕНО										КАК НомерГТД,
	|	ТаблицаТовары.Спецификация							КАК Спецификация,
	...
	|	НЕОПРЕДЕЛЕНО										КАК КорАналитикаФинансовогоУчета,
    |	ВЫБОР
    |		КОГДА ЕСТЬNULL(КлючиАналитикиПолучателя.Назначение.НалоговоеНазначение,ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) <>
    |				ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
    |			ТОГДА КлючиАналитикиПолучателя.Назначение.НалоговоеНазначение
    |		ИНАЧЕ &НалоговоеНазначение
    |	КОНЕЦ 												КАК НалоговоеНазначение,
	|	НЕОПРЕДЕЛЕНО										КАК НомерГТД,
	|	НЕОПРЕДЕЛЕНО										КАК Спецификация,
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
    ...
    |	втТовары.НалоговоеНазначение								КАК НалоговоеНазначение,
	|	втТовары.ВидЗапасов.НалоговоеНазначение						КАК НалоговоеНазначениеВидаЗапасов,
...
Функция ТекстЗапросаВтТаблицаЗаказовДавальцаПоНазначениям(Запрос, ТекстыЗапроса, ПрефиксТЧ = "")
	...
	ТекстЗапроса = ДавальческаяСхема.ТекстЗапросаВтТаблицаЗаказовДавальцаПоНазначениям(
						...
						ПрефиксТЧ + "ВыходныеИзделия",
						ПрефиксТЧ + "ПобочныеИзделия",
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
    ...
    |	втВидыЗапасов.НалоговоеНазначение						КАК НалоговоеНазначение,
...
Процедура ОформитьПоступлениеТоваровПоОдноходовкеТовары(Запрос, ТекстыЗапроса, Регистры)
	...
	СкладыСервер.ОформитьПоступлениеТоваровПоОдноходовке(Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаДокумента, ТекущийОбъектМетаданных, "ВыходныеИзделияСерии");
...
Процедура ОформитьПоступлениеТоваровПоОдноходовкеВозвратныеОтходы(Запрос, ТекстыЗапроса, Регистры)
	...
	СкладыСервер.ОформитьПоступлениеТоваровПоОдноходовке(Запрос, ТекстыЗапроса, Регистры, ТекстЗапросаДокумента, ТекущийОбъектМетаданных, "ПобочныеИзделияСерии");
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ТОГДА втВидыЗапасов.НалоговоеНазначение
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО 
	|	КОНЕЦ КАК НалоговоеНазначение,
    ...
    |	ВтВидыЗапасов.СуммаРегл					        КАК СтоимостьРеглБезНДС,
    |	0							                    КАК НДСРегл,
	...
	|		ИНАЧЕ ВтВидыЗапасов.АналитикаУчетаНоменклатурыПоПолучателюБезНазначения
	|	КОНЕЦ													КАК КорАналитикаУчетаНоменклатуры,
    |	втВидыЗапасов.НалоговоеНазначение						КАК КорНалоговоеНазначение,
	|	ВЫБОР
	|		КОГДА втВидыЗапасов.НаправлениеВыпуска =
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	...
	|	КОНЕЦ																				КАК КорАналитикаУчетаНоменклатуры,
	|	ВЫБОР 
	|		КОГДА втВидыЗапасов.НалоговоеНазначение <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность) 
	|			  И ВтВидыЗапасов.КорВидЗапасов.НалоговоеНазначение = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально) 
	|		ТОГДА ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально) 
	|		ИНАЧЕ втВидыЗапасов.НалоговоеНазначение 
	...
	|	КОНЕЦ																				КАК КорАналитикаУчетаНоменклатуры,
	|	втВидыЗапасов.НалоговоеНазначениеВидаЗапасов										КАК КорНалоговоеНазначение,
	|	НЕОПРЕДЕЛЕНО																		КАК КорПартия,
	|	ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22
	|				И НЕ &ФИФОСкользящаяОценка
...
Функция ТекстЗапросаТаблицаСвободныеОстатки(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИЛИ СпрСклады.ДатаНачалаОрдернойСхемыПриПоступлении > &Дата)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция ТекстЗапросаТаблицаДвиженияНоменклатураНоменклатура(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА ВЫБОР
	|				КОГДА ВЫРАЗИТЬ(ВидыЗапасов.Склад КАК Справочник.Склады).Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|					ТОГДА &Подразделение
	|				ИНАЧЕ ВЫРАЗИТЬ(ВидыЗапасов.Склад КАК Справочник.Склады).Подразделение
	|           КОНЕЦ
	|		ИНАЧЕ 
	|			ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(ВидыЗапасов.Склад) = ТИП(Справочник.СтруктураПредприятия)
	|					ТОГДА ВидыЗапасов.Склад
	|				ИНАЧЕ &Подразделение
	|           КОНЕЦ
	|	КОНЕЦ                                КАК Подразделение,
	...
	|			ИЛИ ВидыЗапасов.Работа
	|			ТОГДА ВидыЗапасов.СуммаЗабалансоваяУпр
	|		ИНАЧЕ 0
	|	КОНЕЦ								 КАК СтоимостьЗабалансовая,
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.ЦеховаяКладовая
	|			ИЛИ ВидыЗапасов.Работа
	|			ТОГДА ВидыЗапасов.СуммаЗабалансоваяРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ								 КАК СтоимостьЗабалансоваяРегл,
	|
	|	ВЫБОР
...
Процедура СформироватьПартииПроизводства(Объект) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		...
		СписокРеквизитов =
            "Ссылка, Дата, Организация, ГруппировкаЗатрат, НаправлениеДеятельности, НалоговоеНазначение";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			СформироватьПечатнуюФормуНакладной(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,,Истина);  // ЭтоМногоязычнаяПечатнаяФорма
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПередачаМатериаловВПроизводство") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьПередачаМатериаловВПроизводство(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,,Истина);// ЭтоМногоязычнаяПечатнаяФорма
			...
			Пока ВыборкаПоПолучателям.Следующий() Цикл
				...
				ПараметрыОбласти.Вставить(
					...
					ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
						ДанныеПечати, 
						  НСтр("ru='Выпуск продукции';uk='Випуск продукції'",КодЯзыкаПечать), КодЯзыкаПечать));
				ПараметрыОбласти.Вставить(
				...
				Пока СтрокаТовары.Следующий() Цикл
					...
					Если НомерСтроки = 1 Тогда
						...
						ОбластьЗаголовокТаблицы.Параметры.НомерСтраницы =
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Страница %1';uk='Сторінка %1'"),
																					НомерСтраницы);
				...
				ОбластьПодвала.Параметры.ИтоговаяСтрока =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Всего наименований %1';uk='Всього найменувань %1'"),
...
Функция ПечатьПередачаМатериаловВПроизводство(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = 
		УправлениеПечатью.МакетПечатнойФормы("Документ.ДвижениеПродукцииИМатериалов.ПФ_MXL_ПередачаМатериаловВПроизводство", КодЯзыкаПечать);
