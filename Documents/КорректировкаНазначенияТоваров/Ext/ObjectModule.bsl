...
//++ НЕ УТКА
	...
	Если ЭтоДавальческоеНазначение Тогда
		...
		ШаблонСообщенияРавные = НСтр("ru='Новое и исходное назначения в строке %НомерСтроки% списка ""Товары"" не должны совпадать.';uk='Нове і вихідне призначення в рядку %НомерСтроки% списку ""Товари"" не повинні збігатися.'");
		...
		Если Таблица.Количество() > 0 Тогда
			...
			ШаблонСообщенияСнятиеРезерва  = НСтр("ru='В колонке ""Новое назначение"" в строке %1 должно быть указано назначение заказа давальца ""%2"" версии производства %3.';uk='У колонці ""Нове призначення"" в рядку %1 має бути вказано призначення замовлення давальця ""%2"" версії виробництва %3.'");
			ШаблонСообщенияРезервирование = НСтр("ru='В колонке ""Исходное назначение"" в строке %1 должно быть указано назначение заказа давальца ""%2"" версии производства %3.';uk='У колонці ""Початкове призначення"" в рядку %1 має бути вказано призначення замовлення давальця ""%2"" версії виробництва %3.'");
			
			Пока Выборка.Следующий() Цикл
				
				ПартнерДляСообщения = ?(ЭтоСнятиеРезерва, Выборка.ИсходныйПартнер, Выборка.НовыйПартнер);
				ВерсияПроизводства = ?(ЗначениеЗаполнено(Назначение.Партнер), НСтр("ru='2.5';uk='2.5'"), НСтр("ru='2.1';uk='2.1'"));
