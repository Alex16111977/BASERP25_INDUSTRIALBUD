...
Функция РезультатЗаполненияВДлительнойОперации(СписокТаблиц)
	...
	Если Результат.ЗаданиеВыполнено Тогда
		...
		Если СписокТаблиц.НайтиПоЗначению("УдержаннаяЗарплата") <> Неопределено Тогда
			НастройкаСчетовУчета.ОбновитьПредставлениеНастройки(ЭтотОбъект, "Объект.УдержаннаяЗарплата");
		КонецЕсли;
...
Процедура ЗаполнитьСтатьюАналитикуРасходовСтроки(ИдентификаторСтроки)
	...
	НастройкаСчетовУчета.ОбновитьПредставлениеНастройки(ЭтаФорма, "Объект.УдержаннаяЗарплата", ИдентификаторСтроки);
