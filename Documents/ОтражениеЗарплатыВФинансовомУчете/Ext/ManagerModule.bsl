...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	// Вспомогательная таблица для заполнения поля А_Сотрудник при включенной настройке "Проводки по работникам"
	ТекстЗапросаВТФизЛица =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДД.Ссылка КАК Ссылка,
	|	ДД.ПодразделениеПредприятия КАК Подразделение,
	|	ДД.ВидОперации КАК ВидОперации,
	|	ДД.ФизическоеЛицо КАК ФизическоеЛицо
	|ПОМЕСТИТЬ ВТФизЛицаДляАктивовПассивов
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ДД
	|ГДЕ
	|	ДД.Ссылка = &Ссылка
	|	И ДД.Ссылка.ПроводкиПоРаботникам
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	Подразделение,
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаВТФизЛица, "ВТФизЛицаДляАктивовПассивов");
	...
	|	ЕСТЬNULL(ФизЛица.ФизическоеЛицо, Строки.ФизическоеЛицо) КАК А_Сотрудник
	|ИЗ
	|	ВТНачисленнаяЗарплатаИВзносы КАК Строки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТФизЛицаДляАктивовПассивов КАК ФизЛица
	|		ПО Строки.Ссылка = ФизЛица.Ссылка
	|			И Строки.Подразделение = ФизЛица.Подразделение
	|			И Строки.ВидОперации = ФизЛица.ВидОперации
	|			И Строки.ФизическоеЛицо = ФизЛица.ФизическоеЛицо
	|ГДЕ
	...
	|СГРУППИРОВАТЬ ПО
	|	Строки.Подразделение,
	|	ЕСТЬNULL(ФизЛица.ФизическоеЛицо, Строки.ФизическоеЛицо)
	|
	|ИМЕЮЩИЕ
	...
	|	Строки.ТипНалога,
	|	СУММА(ВЫРАЗИТЬ(Строки.Сумма * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2))) КАК Сумма,
	|	ЕСТЬNULL(ФизЛица2.ФизическоеЛицо, Строки.ФизическоеЛицо) КАК А_Сотрудник
	|ИЗ
	|	ВТНачисленнаяЗарплатаИВзносы КАК Строки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТФизЛицаДляАктивовПассивов КАК ФизЛица2
	|		ПО Строки.Ссылка = ФизЛица2.Ссылка
	|			И Строки.Подразделение = ФизЛица2.Подразделение
	|			И Строки.ВидОперации = ФизЛица2.ВидОперации
	|			И Строки.ФизическоеЛицо = ФизЛица2.ФизическоеЛицо
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	|	И Строки.ТипНалога <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.ПустаяСсылка)
	|	И Строки.ВидОперации НЕ В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательстваИРезервы),
	...
	|	Строки.ТипНалога,
	|	Строки.Подразделение,
	|	ЕСТЬNULL(ФизЛица2.ФизическоеЛицо, Строки.ФизическоеЛицо)
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВЫРАЗИТЬ(Строки.Сумма * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2))) <> 0
	|
	...
	|	НЕОПРЕДЕЛЕНО КАК Аналитика,
	|	СУММА(Строки.Сумма) КАК Сумма,
	|	Строки.Сотрудник КАК А_Сотрудник
	|ИЗ
	...
	|	СУММА(ВЫРАЗИТЬ(Строки.Сумма * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2))) КАК Сумма,
	|	ЕСТЬNULL(ФизЛица3.ФизическоеЛицо, Строки.ФизическоеЛицо) КАК А_Сотрудник
	|ИЗ
	|	ВТНачисленнаяЗарплатаИВзносы КАК Строки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТФизЛицаДляАктивовПассивов КАК ФизЛица3
	|		ПО Строки.Ссылка = ФизЛица3.Ссылка
	|			И Строки.Подразделение = ФизЛица3.Подразделение
	|			И Строки.ВидОперации = ФизЛица3.ВидОперации
	|			И Строки.ФизическоеЛицо = ФизЛица3.ФизическоеЛицо
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	|	И Строки.СтатьяРасходов Ссылка ПланВидовХарактеристик.СтатьиАктивовПассивов
	|	И Строки.СтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка)
	...
	|	Строки.Подразделение,
	|	Строки.НаправлениеДеятельности,
	|	ЕСТЬNULL(ФизЛица3.ФизическоеЛицо, Строки.ФизическоеЛицо)
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВЫРАЗИТЬ(Строки.Сумма * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2))) <> 0
	|
	...
	|	СУММА(ВыработкаСотрудников.Сумма) КАК Сумма,
	|	ВыработкаСотрудников.Сотрудник КАК А_Сотрудник
	|ИЗ
	|	ВтТаблицаВыработкаСотрудников КАК ВыработкаСотрудников
	|ГДЕ
	|	ВыработкаСотрудников.СтатьяРасходов Ссылка ПланВидовХарактеристик.СтатьиАктивовПассивов
	...
	|	Удержанния.ФизическоеЛицо КАК А_Сотрудник
	|ИЗ
	...
	|	НачисленныйНДФЛ.ФизическоеЛицо КАК А_Сотрудник
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленныйНДФЛ КАК НачисленныйНДФЛ
	|ГДЕ
	|	НачисленныйНДФЛ.Ссылка = &Ссылка
	...
	|	НДФЛ.ФизическоеЛицо КАК А_Сотрудник
	|ИЗ
	...
	|	Проценты.ФизическоеЛицо КАК А_Сотрудник
	|ИЗ
	...
	ТекстЗапроса = ТекстЗапроса + "ОБЪЕДИНИТЬ ВСЕ"
		+ Регистр_ТекстЗапросаТаблицаПрочиеАктивыПассивы();
...
//
Функция Регистр_ТекстЗапросаТаблицаПрочиеАктивыПассивы(ЕстьВтПартииПрочихРасходов = Истина) Экспорт
...
ТекстЗапроса = "
	...
	|	СУММА(Строки.Сумма) КАК Сумма,
	|	ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка) КАК А_Сотрудник
	|ИЗ
...
#Область ОтладкаА_Сотрудник

// ОТЛАДКА: Процедура для аналізу заповнення поля А_Сотрудник
// Викликати з консолі коду:
// Документы.ОтражениеЗарплатыВФинансовомУчете.ОтладкаА_Сотрудник(Документы.ОтражениеЗарплатыВФинансовомУчете.НайтиПоНомеру("000Ц-000002", '20250831235959'))
//
Процедура ОтладкаА_Сотрудник(СсылкаНаДокумент) Экспорт

	Если Не ЗначениеЗаполнено(СсылкаНаДокумент) Тогда
		Сообщить("ПОМИЛКА: Документ не знайдено!");
		Возврат;
	КонецЕсли;

	Сообщить("========== ВІДЛАДКА А_Сотрудник ==========");
	Сообщить("Документ: " + СсылкаНаДокумент);
	Сообщить("Сводно: " + СсылкаНаДокумент.Сводно);
	Сообщить("ПроводкиПоРаботникам: " + СсылкаНаДокумент.ПроводкиПоРаботникам);
	Сообщить("");

	// 1. Перевірка табличної частини НачисленнаяЗарплатаИВзносыПоФизлицам
	Сообщить("=== 1. ТЧ НачисленнаяЗарплатаИВзносыПоФизлицам ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Т.ПодразделениеПредприятия.Наименование КАК Подразделение,
	|	Т.ВидОперации КАК ВидОперации,
	|	Т.ФизическоеЛицо.Наименование КАК ФизЛицо,
	|	Т.Сумма КАК Сумма
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Т
	|ГДЕ
	|	Т.Ссылка = &Ссылка";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		Сообщить("  " + Стр.Подразделение + " | " + Стр.ВидОперации + " | " + Стр.ФизЛицо + " | " + Стр.Сумма);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());
	Сообщить("");

	// 2. Перевірка ВТФизЛицаДляАктивовПассивов
	Сообщить("=== 2. Тимчасова таблиця ВТФизЛицаДляАктивовПассивов ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДД.Ссылка КАК Ссылка,
	|	ДД.ПодразделениеПредприятия КАК Подразделение,
	|	ДД.ПодразделениеПредприятия.Наименование КАК ПодразделениеНазва,
	|	ДД.ВидОперации КАК ВидОперации,
	|	ДД.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ДД.ФизическоеЛицо.Наименование КАК ФизЛицоНазва
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ДД
	|ГДЕ
	|	ДД.Ссылка = &Ссылка
	...
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		Сообщить("  Підр: " + Стр.ПодразделениеНазва + " | ВидОп: " + Стр.ВидОперации + " | ФізОсоба: " + Стр.ФизЛицоНазва);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());
	Сообщить("");

	// 3. Перевірка сирих даних ТЧ (без групування)
	Сообщить("=== 3. Сирі дані ТЧ НачисленнаяЗарплатаИВзносыПоФизлицам ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Т.ПодразделениеПредприятия.Наименование КАК Подразделение,
	|	Т.ПодразделениеПредприятия КАК ПодразделениеСсылка,
	|	Т.ВидОперации КАК ВидОперации,
	|	Т.ФизическоеЛицо.Наименование КАК ФизЛицо,
	|	Т.ФизическоеЛицо КАК ФизЛицоСсылка,
	|	Т.Сумма КАК Сумма
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Т
	|ГДЕ
	...
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		ФизЛицоПустое = Не ЗначениеЗаполнено(Стр.ФизЛицоСсылка);
		Сообщить("  Підр: " + Стр.Подразделение + " | ВидОп: " + Стр.ВидОперации + " | ФізОсоба: " + ?(ФизЛицоПустое, "<<<ПУСТО>>>", Стр.ФизЛицо) + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());
	Сообщить("Сводно = " + СсылкаНаДокумент.Сводно + ", ПроводкиПоРаботникам = " + СсылкаНаДокумент.ПроводкиПоРаботникам);
	Сообщить("");

	// 4. Перевірка симуляції групування (як в основному запиті)
	Сообщить("=== 4. Симуляція групування (Підр + ВидОп + ФізОсоба) ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Т.ПодразделениеПредприятия.Наименование КАК Подразделение,
	|	Т.ПодразделениеПредприятия КАК ПодразделениеСсылка,
	...
	// 5. Перевірка JOIN
	Сообщить("=== 5. Результат JOIN (А_Сотрудник) ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Строки.ПодразделениеПредприятия КАК ПодрСтроки,
	|	Строки.ВидОперации КАК ВидОпСтроки,
	|	Строки.ФизическоеЛицо КАК ФизЛицоСтроки,
	|	ФизЛица.ПодразделениеПредприятия КАК ПодрФизлица,
	|	ФизЛица.ВидОперации КАК ВидОпФизлица,
	|	ФизЛица.ФизическоеЛицо КАК ФизЛицоФизлица,
	|	ЕСТЬNULL(ФизЛица.ФизическоеЛицо, Строки.ФизическоеЛицо) КАК А_Сотрудник,
	|	Строки.Сумма КАК Сумма
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Строки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ФизЛица
	|		ПО Строки.Ссылка = ФизЛица.Ссылка
	|			И Строки.ПодразделениеПредприятия = ФизЛица.ПодразделениеПредприятия
	|			И Строки.ВидОперации = ФизЛица.ВидОперации
	|			И Строки.ФизическоеЛицо = ФизЛица.ФизическоеЛицо
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	|	И Строки.Ссылка.ПроводкиПоРаботникам";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		А_СотрудникПустой = Не ЗначениеЗаполнено(Стр.А_Сотрудник);
		Сообщить("  А_Сотрудник: " + ?(А_СотрудникПустой, "<<<ПУСТО>>>", Стр.А_Сотрудник) + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());
	Сообщить("");

	// 6. Перевірка поточних рухів регістру
	Сообщить("=== 6. Поточні рухи регістру ПрочиеАктивыПассивы (детально) ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Р.ВидДвижения КАК ВидДвижения,
	|	Р.Подразделение.Наименование КАК Подразделение,
	|	Р.Статья КАК Статья,
	|	Р.Аналитика КАК Аналитика,
	|	Р.Сумма КАК Сумма,
	|	Р.А_Сотрудник.Наименование КАК А_Сотрудник,
	|	Р.А_Сотрудник КАК А_СотрудникСсылка
	|ИЗ
	|	РегистрНакопления.ПрочиеАктивыПассивы КАК Р
	|ГДЕ
	|	Р.Регистратор = &Ссылка";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		А_СотрудникПустой = Не ЗначениеЗаполнено(Стр.А_СотрудникСсылка);
		Сообщить("  " + Стр.ВидДвижения + " | Стаття: " + Стр.Статья + " | Аналітика: " + Стр.Аналитика + " | Сума: " + Стр.Сумма + " | А_Сотрудник: " + ?(А_СотрудникПустой, "<<<ПУСТО>>>", Стр.А_Сотрудник));
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());

	// 7. Перевірка ВТНачисленнаяЗарплатаИВзносы (проміжна таблиця)
	Сообщить("");
	Сообщить("=== 7. ВТНачисленнаяЗарплатаИВзносы (джерело для регістру) ===");
	Сообщить("Перевіряємо що потрапляє в проміжну таблицю ВТНачисленнаяЗарплатаИВзносы");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Т.ПодразделениеПредприятия.Наименование КАК Подразделение,
	|	Т.ВидОперации КАК ВидОперации,
	|	Т.СпособОтраженияЗарплатыВБухучете КАК СпособОтражения,
	|	Т.ФизическоеЛицо.Наименование КАК ФизЛицо,
	|	Т.ФизическоеЛицо КАК ФизЛицоСсылка,
	|	Т.Сумма КАК Сумма,
	|	Т.ВзносыВсего КАК Взносы,
	|	Т.ГруппаУчетаНачислений КАК ГруппаУчета
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Т
	|ГДЕ
	|	Т.Ссылка = &Ссылка";
	...
	Для Каждого Стр Из Результат Цикл
		ФизЛицоПустое = Не ЗначениеЗаполнено(Стр.ФизЛицоСсылка);
		Сообщить("  Підр: " + Стр.Подразделение + " | ВидОп: " + Стр.ВидОперации + " | Спосіб: " + Стр.СпособОтражения + " | ФізОс: " + ?(ФизЛицоПустое, "<<<ПУСТО>>>", Стр.ФизЛицо) + " | Сума: " + Стр.Сумма + " | Внески: " + Стр.Взносы + " | Група: " + Стр.ГруппаУчета);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());

	// 8. Перевірка табличної частини НачисленнаяЗарплатаИВзносы (зведена)
	Сообщить("");
	Сообщить("=== 8. ТЧ НачисленнаяЗарплатаИВзносы (зведена) ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Т.ПодразделениеПредприятия.Наименование КАК Подразделение,
	|	Т.ВидОперации КАК ВидОперации,
	...
	|	Т.ВзносыВсего КАК Взносы
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК Т
	|ГДЕ
	|	Т.Ссылка = &Ссылка";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		Сообщить("  Підр: " + Стр.Подразделение + " | ВидОп: " + Стр.ВидОперации + " | Спосіб: " + Стр.СпособОтражения + " | Сума: " + Стр.Сумма + " | Внески: " + Стр.Взносы);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());

	Сообщить("");
	Сообщить("========== КІНЕЦЬ ВІДЛАДКИ ==========");

КонецПроцедуры

// ОТЛАДКА: Процедура для повної симуляції проведення з виводом тимчасових таблиць
// Викликати з консолі коду:
// Документы.ОтражениеЗарплатыВФинансовомУчете.ОтладкаПроведения(Документы.ОтражениеЗарплатыВФинансовомУчете.НайтиПоНомеру("000Ц-000002", '20250831235959'))
//
Процедура ОтладкаПроведения(СсылкаНаДокумент) Экспорт
	...
	Сообщить("========== ВІДЛАДКА ПРОВЕДЕННЯ ==========");
	Сообщить("Документ: " + СсылкаНаДокумент);
	Сообщить("Сводно: " + СсылкаНаДокумент.Сводно);
	Сообщить("ПроводкиПоРаботникам: " + СсылкаНаДокумент.ПроводкиПоРаботникам);
	Сообщить("");

	// Виконуємо запит, що симулює формування ВТТаблицаДокумента
	Сообщить("=== 1. ВТТаблицаДокумента (джерело даних) ===");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗарплатаДетально.Ссылка КАК Ссылка,
	|	ЗарплатаДетально.ПодразделениеПредприятия КАК Подразделение,
	|	ЗарплатаДетально.ПодразделениеПредприятия.Наименование КАК ПодразделениеНазва,
	|	ЗарплатаДетально.ВидОперации КАК ВидОперации,
	|	ЗарплатаДетально.СпособОтраженияЗарплатыВБухучете КАК СпособОтражения,
	|	СУММА(ЗарплатаДетально.Сумма) КАК Сумма,
	|	ВЫБОР КОГДА ЗарплатаДетально.Ссылка.ПроводкиПоРаботникам ТОГДА
	|		ЗарплатаДетально.ФизическоеЛицо
	|	ИНАЧЕ
	|		ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)
	|	КОНЕЦ КАК ФизическоеЛицо,
	|	ВЫБОР КОГДА ЗарплатаДетально.Ссылка.ПроводкиПоРаботникам ТОГДА
	|		ЗарплатаДетально.ФизическоеЛицо.Наименование
	|	ИНАЧЕ
	|		""<<<ПУСТО (ПроводкиПоРаботникам=Ложь)>>>""
	|	КОНЕЦ КАК ФизЛицоНазва,
	|	СУММА(ЗарплатаДетально.ВзносыВсего) КАК ВзносыВсего
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ЗарплатаДетально
	|ГДЕ
	|	ЗарплатаДетально.Ссылка = &Ссылка
	|	И НЕ ЗарплатаДетально.Ссылка.Сводно
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗарплатаДетально.Ссылка,
	|	ЗарплатаДетально.ПодразделениеПредприятия,
	...
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		ФизЛицоПустое = Не ЗначениеЗаполнено(Стр.ФизическоеЛицо);
		Сообщить("  Підр: " + Стр.ПодразделениеНазва + " | ВидОп: " + Стр.ВидОперации + " | ФізОс: " + ?(ФизЛицоПустое, "<<<ПУСТО>>>", Стр.ФизЛицоНазва) + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("Рядків: " + Результат.Количество());
	Сообщить("");

	// Виконуємо запит для ВТФизЛицаДляАктивовПассивов
	Сообщить("=== 2. ВТФизЛицаДляАктивовПассивов (для JOIN) ===");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДД.Ссылка КАК Ссылка,
	...
	|	И ДД.Ссылка.ПроводкиПоРаботникам";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	ВТФизЛица = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из ВТФизЛица Цикл
		Сообщить("  Підр: " + Стр.ПодразделениеНазва + " (" + XMLСтрока(Стр.Подразделение.УникальныйИдентификатор()) + ") | ВидОп: " + Стр.ВидОперации + " | ФізОс: " + Стр.ФизЛицоНазва);
	КонецЦикла;
	Сообщить("Рядків: " + ВТФизЛица.Количество());
	Сообщить("");

	// Симулюємо JOIN як в основному запиті
	Сообщить("=== 3. Симуляція JOIN для А_Сотрудник (ВТНачисленнаяЗарплатаИВзносы + ВТФизЛицаДляАктивовПассивов) ===");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Строки.ПодразделениеПредприятия КАК ПодразделениеСтроки,
	|	Строки.ПодразделениеПредприятия.Наименование КАК ПодрСтрокиНазва,
	|	Строки.ВидОперации КАК ВидОперацииСтроки,
	|	Строки.ФизическоеЛицо КАК ФизЛицоСтроки,
	|	Строки.ФизическоеЛицо.Наименование КАК ФизЛицоСтрокиНазва,
	|	ФизЛица.ПодразделениеПредприятия КАК ПодразделениеФизЛица,
	|	ФизЛица.ПодразделениеПредприятия.Наименование КАК ПодрФизЛицаНазва,
	|	ФизЛица.ВидОперации КАК ВидОперацииФизЛица,
	|	ФизЛица.ФизическоеЛицо КАК ФизЛицоФизЛица,
	|	ФизЛица.ФизическоеЛицо.Наименование КАК ФизЛицоФизЛицаНазва,
	|	ЕСТЬNULL(ФизЛица.ФизическоеЛицо, Строки.ФизическоеЛицо) КАК А_Сотрудник,
	|	ЕСТЬNULL(ФизЛица.ФизическоеЛицо.Наименование, Строки.ФизическоеЛицо.Наименование) КАК А_СотрудникНазва,
	|	ВЫБОР КОГДА ФизЛица.ФизическоеЛицо ЕСТЬ NULL ТОГДА ""JOIN НЕ ЗНАЙДЕНО"" ИНАЧЕ ""JOIN OK"" КОНЕЦ КАК СтатусJOIN,
	|	Строки.Сумма КАК Сумма
	|ПОМЕСТИТЬ ВтРезультатJOIN
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Строки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ФизЛица
	|		ПО Строки.Ссылка = ФизЛица.Ссылка
	...
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	|	И НЕ Строки.Ссылка.Сводно
	|;
	|ВЫБРАТЬ * ИЗ ВтРезультатJOIN";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Результат = Запрос.ВыполнитьПакет()[1].Выгрузить();
	Для Каждого Стр Из Результат Цикл
		А_СотрудникПустой = Не ЗначениеЗаполнено(Стр.А_Сотрудник);
		Сообщить("  [" + Стр.СтатусJOIN + "] Строки.Підр: " + Стр.ПодрСтрокиНазва + " | Строки.ВидОп: " + Стр.ВидОперацииСтроки + " | Строки.ФізОс: " + Стр.ФизЛицоСтрокиНазва);
		Сообщить("                ФізЛіца.Підр: " + Стр.ПодрФизЛицаНазва + " | ФізЛіца.ВидОп: " + Стр.ВидОперацииФизЛица + " | ФізЛіца.ФізОс: " + Стр.ФизЛицоФизЛицаНазва);
		Сообщить("                >>> А_Сотрудник: " + ?(А_СотрудникПустой, "<<<ПУСТО>>>", Стр.А_СотрудникНазва) + " | Сума: " + Стр.Сумма);
		Сообщить("");
	КонецЦикла;
	Сообщить("Рядків: " + Результат.Количество());
	Сообщить("");

	// Перевірка порівняння підрозділів
	Сообщить("=== 4. Детальне порівняння ключів для JOIN ===");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СтрокиСводно.ПодразделениеПредприятия КАК ПодрСводно,
	|	СтрокиСводно.ПодразделениеПредприятия.Наименование КАК ПодрСводноНазва,
	|	СтрокиСводно.ВидОперации КАК ВидОпСводно,
	|	СтрокиДетально.ПодразделениеПредприятия КАК ПодрДетально,
	|	СтрокиДетально.ПодразделениеПредприятия.Наименование КАК ПодрДетальноНазва,
	|	СтрокиДетально.ВидОперации КАК ВидОпДетально,
	|	СтрокиДетально.ФизическоеЛицо КАК ФизЛицо,
	|	СтрокиДетально.ФизическоеЛицо.Наименование КАК ФизЛицоНазва,
	|	ВЫБОР
	|		КОГДА СтрокиСводно.ПодразделениеПредприятия = СтрокиДетально.ПодразделениеПредприятия ТОГДА ""Підр: ОК""
	|		ИНАЧЕ ""Підр: РІЗНІ!""
	|	КОНЕЦ КАК ПорівнянняПідр,
	|	ВЫБОР
	|		КОГДА СтрокиСводно.ВидОперации = СтрокиДетально.ВидОперации ТОГДА ""ВидОп: ОК""
	|		ИНАЧЕ ""ВидОп: РІЗНІ!""
	|	КОНЕЦ КАК ПорівнянняВидОп
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК СтрокиСводно
	|		ПОЛНОЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК СтрокиДетально
	|		ПО СтрокиСводно.Ссылка = СтрокиДетально.Ссылка
	|			И СтрокиСводно.ПодразделениеПредприятия = СтрокиДетально.ПодразделениеПредприятия
	|			И СтрокиСводно.ВидОперации = СтрокиДетально.ВидОперации
	|ГДЕ
	|	СтрокиСводно.Ссылка = &Ссылка
	|	ИЛИ СтрокиДетально.Ссылка = &Ссылка";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		Сообщить("  Сводно: " + Стр.ПодрСводноНазва + " (" + Стр.ВидОпСводно + ")");
		Сообщить("  Детально: " + Стр.ПодрДетальноНазва + " (" + Стр.ВидОпДетально + ") | ФізОс: " + Стр.ФизЛицоНазва);
		Сообщить("  Результат: " + Стр.ПорівнянняПідр + " | " + Стр.ПорівнянняВидОп);
		Сообщить("");
	КонецЦикла;
	Сообщить("");

	// Перевірка UUID підрозділів
	Сообщить("=== 5. UUID підрозділів в обох ТЧ ===");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	""Сводно"" КАК Джерело,
	|	Т.ПодразделениеПредприятия КАК Подразделение,
	|	Т.ПодразделениеПредприятия.Наименование КАК Назва,
	|	Т.ВидОперации КАК ВидОперации
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК Т
	|ГДЕ
	...
	|	""Детально"",
	|	Т.ПодразделениеПредприятия,
	|	Т.ПодразделениеПредприятия.Наименование,
	|	Т.ВидОперации
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Т
	|ГДЕ
	...
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		Сообщить("  [" + Стр.Джерело + "] Підр: " + Стр.Назва + " (UUID: " + XMLСтрока(Стр.Подразделение.УникальныйИдентификатор()) + ") | ВидОп: " + Стр.ВидОперации);
	КонецЦикла;
	Сообщить("");

	Сообщить("========== КІНЕЦЬ ВІДЛАДКИ ПРОВЕДЕННЯ ==========");

КонецПроцедуры

// ОТЛАДКА: Виконує пакет запитів проведення і виводить вміст всіх тимчасових таблиць
// Викликати з консолі коду:
// Документы.ОтражениеЗарплатыВФинансовомУчете.ОтладкаПакетаЗапросов(Документы.ОтражениеЗарплатыВФинансовомУчете.НайтиПоНомеру("000Ц-000002", '20250831235959'))
//
Процедура ОтладкаПакетаЗапросов(СсылкаНаДокумент) Экспорт
	...
	Сообщить("========== ВІДЛАДКА ПАКЕТУ ЗАПИТІВ ==========");
	Сообщить("Документ: " + СсылкаНаДокумент);
	Сообщить("");

	// Формуємо запит як при проведенні
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;

	// Встановлюємо параметри як при проведенні
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Запрос.УстановитьПараметр("Организация", СсылкаНаДокумент.Организация);
	Запрос.УстановитьПараметр("Период", КонецМесяца(СсылкаНаДокумент.ПериодРегистрации));
	Запрос.УстановитьПараметр("ДатаНачала", НачалоМесяца(СсылкаНаДокумент.ПериодРегистрации));
	Запрос.УстановитьПараметр("ДатаОкончания", КонецМесяца(СсылкаНаДокумент.ПериодРегистрации));
	Запрос.УстановитьПараметр("Сводно", СсылкаНаДокумент.Сводно);
	Запрос.УстановитьПараметр("ПроводкиПоРаботникам", СсылкаНаДокумент.ПроводкиПоРаботникам);
	Запрос.УстановитьПараметр("КоэффициентПересчетаВВалютуУпр", 1);
	Запрос.УстановитьПараметр("ИспользоватьУчетПрочихАктивовПассивов", Истина);

	// 1. Створюємо ВТФизЛицаДляАктивовПассивов
	Сообщить("=== 1. Створення ВТФизЛицаДляАктивовПассивов ===");
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДД.Ссылка КАК Ссылка,
	|	ДД.ПодразделениеПредприятия КАК Подразделение,
	...
	// Перевіряємо вміст ВТФизЛицаДляАктивовПассивов
	Запрос.Текст = "ВЫБРАТЬ * ИЗ ВТФизЛицаДляАктивовПассивов";
	ТаблицаФизЛиц = Запрос.Выполнить().Выгрузить();
	Сообщить("Рядків у ВТФизЛицаДляАктивовПассивов: " + ТаблицаФизЛиц.Количество());
	Для Каждого Стр Из ТаблицаФизЛиц Цикл
		Сообщить("  Підр: " + Стр.Подразделение + " | ВидОп: " + Стр.ВидОперации + " | ФізОс: " + Стр.ФизическоеЛицо);
	КонецЦикла;
	Сообщить("");

	// 2. Створюємо ВТНачисленнаяЗарплатаИВзносы (спрощено - тільки для перевірки JOIN)
	// ВАЖЛИВО: Для СВОДНОГО документа дані беруться зі СВОДНОЇ ТЧ, де ФизическоеЛицо = ПУСТО!
	Сообщить("=== 2. Симуляція ВТНачисленнаяЗарплатаИВзносы ===");

	Если СсылкаНаДокумент.Сводно Тогда
		// Для СВОДНОГО документа - дані зі СВОДНОЇ ТЧ (як у реальному проведенні)
		Сообщить("Документ СВОДНИЙ - дані беруться зі сводної ТЧ НачисленнаяЗарплатаИВзносы");
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Строки.Ссылка КАК Ссылка,
		|	Строки.ПодразделениеПредприятия КАК Подразделение,
		|	Строки.ВидОперации КАК ВидОперации,
		|	ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка) КАК ФизическоеЛицо,
		|	Строки.Сумма КАК Сумма
		|ПОМЕСТИТЬ ВТНачисленнаяЗарплатаИВзносыТест
		|ИЗ
		|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК Строки
		|ГДЕ
		|	Строки.Ссылка = &Ссылка";
	Иначе
		// Для НЕ сводного документа - дані з детальної ТЧ
		Сообщить("Документ НЕ сводний - дані беруться з детальної ТЧ НачисленнаяЗарплатаИВзносыПоФизлицам");
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Строки.Ссылка КАК Ссылка,
		|	Строки.ПодразделениеПредприятия КАК Подразделение,
		|	Строки.ВидОперации КАК ВидОперации,
		|	ВЫБОР КОГДА Строки.Ссылка.ПроводкиПоРаботникам ТОГДА
		|		Строки.ФизическоеЛицо
		|	ИНАЧЕ
		|		ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)
		|	КОНЕЦ КАК ФизическоеЛицо,
	...
	// Перевіряємо вміст ВТНачисленнаяЗарплатаИВзносы
	Запрос.Текст = "ВЫБРАТЬ * ИЗ ВТНачисленнаяЗарплатаИВзносыТест";
	ТаблицаНачислений = Запрос.Выполнить().Выгрузить();
	Сообщить("Рядків у ВТНачисленнаяЗарплатаИВзносыТест: " + ТаблицаНачислений.Количество());
	Для Каждого Стр Из ТаблицаНачислений Цикл
		ФизЛицоПустое = Не ЗначениеЗаполнено(Стр.ФизическоеЛицо);
		Сообщить("  Підр: " + Стр.Подразделение + " | ВидОп: " + Стр.ВидОперации + " | ФізОс: " + ?(ФизЛицоПустое, "<<<ПУСТО>>>", Строка(Стр.ФизическоеЛицо)) + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("");

	// 3. Виконуємо JOIN як в основному запиті
	Сообщить("=== 3. Результат JOIN для А_Сотрудник ===");
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Строки.Подразделение КАК ПодразделениеСтроки,
	|	Строки.ВидОперации КАК ВидОперацииСтроки,
	|	Строки.ФизическоеЛицо КАК ФизЛицоСтроки,
	|	ФизЛица.Подразделение КАК ПодразделениеФизЛица,
	|	ФизЛица.ВидОперации КАК ВидОперацииФизЛица,
	|	ФизЛица.ФизическоеЛицо КАК ФизЛицоФизЛица,
	|	ЕСТЬNULL(ФизЛица.ФизическоеЛицо, Строки.ФизическоеЛицо) КАК А_Сотрудник,
	|	ВЫБОР КОГДА ФизЛица.ФизическоеЛицо ЕСТЬ NULL ТОГДА ""JOIN НЕ ЗНАЙДЕНО"" ИНАЧЕ ""JOIN OK"" КОНЕЦ КАК СтатусJOIN,
	|	Строки.Сумма КАК Сумма
	|ИЗ
	|	ВТНачисленнаяЗарплатаИВзносыТест КАК Строки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТФизЛицаДляАктивовПассивов КАК ФизЛица
	|		ПО Строки.Ссылка = ФизЛица.Ссылка
	|			И Строки.Подразделение = ФизЛица.Подразделение
	|			И Строки.ВидОперации = ФизЛица.ВидОперации";
	РезультатJOIN = Запрос.Выполнить().Выгрузить();
	Сообщить("Рядків після JOIN: " + РезультатJOIN.Количество());
	Для Каждого Стр Из РезультатJOIN Цикл
		А_СотрудникПустой = Не ЗначениеЗаполнено(Стр.А_Сотрудник);
		Сообщить("  [" + Стр.СтатусJOIN + "]");
		Сообщить("    Строки: Підр=" + Стр.ПодразделениеСтроки + " | ВидОп=" + Стр.ВидОперацииСтроки + " | ФізОс=" + Стр.ФизЛицоСтроки);
		Сообщить("    ФізЛіца: Підр=" + Стр.ПодразделениеФизЛица + " | ВидОп=" + Стр.ВидОперацииФизЛица + " | ФізОс=" + Стр.ФизЛицоФизЛица);
		Сообщить("    >>> А_Сотрудник: " + ?(А_СотрудникПустой, "<<<ПУСТО>>>", Строка(Стр.А_Сотрудник)));
		Сообщить("");
	КонецЦикла;

	// 4. Порівняння UUID підрозділів
	Сообщить("=== 4. Порівняння UUID ключів ===");
	Запрос.Текст =
	"ВЫБРАТЬ
	|	""Строки"" КАК Источник,
	|	Строки.Подразделение КАК Подразделение,
	...
	|ВЫБРАТЬ
	|	""ФизЛица"",
	|	ФизЛица.Подразделение,
	|	ФизЛица.ВидОперации
	|ИЗ
	|	ВТФизЛицаДляАктивовПассивов КАК ФизЛица";
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		UUIDПідр = ?(ЗначениеЗаполнено(Стр.Подразделение), XMLСтрока(Стр.Подразделение.УникальныйИдентификатор()), "ПУСТО");
		Сообщить("  [" + Стр.Источник + "] Підр UUID: " + UUIDПідр + " | ВидОп: " + Стр.ВидОперации);
	КонецЦикла;
	Сообщить("");

	Сообщить("========== КІНЕЦЬ ВІДЛАДКИ ПАКЕТУ ==========");

КонецПроцедуры

// ОТЛАДКА: Порівнює сводну та детальну ТЧ для виявлення невідповідності ключів JOIN
// Викликати з консолі коду:
// Документы.ОтражениеЗарплатыВФинансовомУчете.ОтладкаСводногоДокумента(Документы.ОтражениеЗарплатыВФинансовомУчете.НайтиПоНомеру("000Ц-000002", '20250831235959'))
//
Процедура ОтладкаСводногоДокумента(СсылкаНаДокумент) Экспорт
	...
	Сообщить("========== ВІДЛАДКА СВОДНОГО ДОКУМЕНТА ==========");
	Сообщить("Документ: " + СсылкаНаДокумент);
	Сообщить("Сводно: " + СсылкаНаДокумент.Сводно);
	Сообщить("ПроводкиПоРаботникам: " + СсылкаНаДокумент.ПроводкиПоРаботникам);
	Сообщить("");

	Если Не СсылкаНаДокумент.Сводно Тогда
		Сообщить("Документ НЕ сводний - використовуйте ОтладкаПакетаЗапросов замість цієї процедури.");
		Возврат;
	КонецЕсли;
	...
	// 1. Вміст СВОДНОЇ ТЧ НачисленнаяЗарплатаИВзносы
	Сообщить("=== 1. СВОДНА ТЧ: НачисленнаяЗарплатаИВзносы ===");
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТЧ.ПодразделениеПредприятия КАК Подразделение,
	|	ТЧ.ПодразделениеПредприятия.Наименование КАК ПодразделениеНазва,
	|	ТЧ.ВидОперации КАК ВидОперации,
	|	ТЧ.Сумма КАК Сумма
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК ТЧ
	|ГДЕ
	|	ТЧ.Ссылка = &Ссылка";
	РезультатСводна = Запрос.Выполнить().Выгрузить();
	Сообщить("Рядків у сводній ТЧ: " + РезультатСводна.Количество());
	Для Каждого Стр Из РезультатСводна Цикл
		Сообщить("  Підр: " + Стр.ПодразделениеНазва + " (UUID: " + XMLСтрока(Стр.Подразделение.УникальныйИдентификатор()) + ") | ВидОп: " + Стр.ВидОперации + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("");

	// 2. Вміст ДЕТАЛЬНОЇ ТЧ НачисленнаяЗарплатаИВзносыПоФизлицам
	Сообщить("=== 2. ДЕТАЛЬНА ТЧ: НачисленнаяЗарплатаИВзносыПоФизлицам ===");
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЧ.ПодразделениеПредприятия КАК Подразделение,
	|	ТЧ.ПодразделениеПредприятия.Наименование КАК ПодразделениеНазва,
	...
	|	ТЧ.ФизическоеЛицо.Наименование КАК ФизическоеЛицоНазва,
	|	ТЧ.Сумма КАК Сумма
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ТЧ
	|ГДЕ
	|	ТЧ.Ссылка = &Ссылка";
	РезультатДетальна = Запрос.Выполнить().Выгрузить();
	Сообщить("Рядків у детальній ТЧ: " + РезультатДетальна.Количество());
	Для Каждого Стр Из РезультатДетальна Цикл
		Сообщить("  Підр: " + Стр.ПодразделениеНазва + " (UUID: " + XMLСтрока(Стр.Подразделение.УникальныйИдентификатор()) + ") | ВидОп: " + Стр.ВидОперации + " | ФізОс: " + Стр.ФизическоеЛицоНазва + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("");

	// 3. Порівняння ключів JOIN (Подразделение, ВидОперации)
	Сообщить("=== 3. ПОРІВНЯННЯ КЛЮЧІВ JOIN (Подразделение + ВидОперации) ===");
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Сводна.ПодразделениеПредприятия КАК ПодрСводна,
	|	Сводна.ПодразделениеПредприятия.Наименование КАК ПодрСводнаНазва,
	|	Сводна.ВидОперации КАК ВидОпСводна,
	|	Детальна.ПодразделениеПредприятия КАК ПодрДетальна,
	|	Детальна.ПодразделениеПредприятия.Наименование КАК ПодрДетальнаНазва,
	|	Детальна.ВидОперации КАК ВидОпДетальна,
	|	Детальна.ФизическоеЛицо КАК ФизЛицо,
	|	Детальна.ФизическоеЛицо.Наименование КАК ФизЛицоНазва,
	|	ВЫБОР
	|		КОГДА Сводна.ПодразделениеПредприятия = Детальна.ПодразделениеПредприятия
	|			И Сводна.ВидОперации = Детальна.ВидОперации
	|			ТОГДА ""JOIN OK""
	|		КОГДА Детальна.ПодразделениеПредприятия ЕСТЬ NULL
	|			ТОГДА ""НЕМАЄ В ДЕТАЛЬНІЙ""
	|		КОГДА Сводна.ПодразделениеПредприятия ЕСТЬ NULL
	|			ТОГДА ""НЕМАЄ В СВОДНІЙ""
	|		ИНАЧЕ ""JOIN FAIL - РІЗНІ КЛЮЧІ""
	|	КОНЕЦ КАК СтатусJOIN
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносы КАК Сводна
	|		ПОЛНОЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Детальна
	|		ПО Сводна.Ссылка = Детальна.Ссылка
	|			И Сводна.ПодразделениеПредприятия = Детальна.ПодразделениеПредприятия
	|			И Сводна.ВидОперации = Детальна.ВидОперации
	|ГДЕ
	|	Сводна.Ссылка = &Ссылка
	|	ИЛИ Детальна.Ссылка = &Ссылка";

	РезультатПорівняння = Запрос.Выполнить().Выгрузить();
	КількістьПомилок = 0;
	Для Каждого Стр Из РезультатПорівняння Цикл
		Если Стр.СтатусJOIN <> "JOIN OK" Тогда
			КількістьПомилок = КількістьПомилок + 1;
		КонецЕсли;
		Сообщить("  [" + Стр.СтатусJOIN + "]");
		Сообщить("    Сводна: Підр=" + Стр.ПодрСводнаНазва + " | ВидОп=" + Стр.ВидОпСводна);
		Сообщить("    Детальна: Підр=" + Стр.ПодрДетальнаНазва + " | ВидОп=" + Стр.ВидОпДетальна + " | ФізОс=" + Стр.ФизЛицоНазва);
		Сообщить("");
	КонецЦикла;

	Если КількістьПомилок > 0 Тогда
		Сообщить("!!! УВАГА: Знайдено " + КількістьПомилок + " невідповідностей ключів JOIN !!!");
		Сообщить("Це причина чому А_Сотрудник пустий - JOIN не знаходить співпадіння між сводною та детальною ТЧ.");
		Сообщить("");
		Сообщить("РІШЕННЯ: Перевірте що ПодразделениеПредприятия та ВидОперации однакові в обох ТЧ.");
	Иначе
		Сообщить("Всі ключі JOIN співпадають. Проблема може бути в іншому місці коду.");
	КонецЕсли;

	Сообщить("");
	Сообщить("========== КІНЕЦЬ ВІДЛАДКИ СВОДНОГО ДОКУМЕНТА ==========");
