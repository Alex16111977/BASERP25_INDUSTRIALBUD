...
Функция ТекстЗапросаТаблицаА_НачисленнаяЗарплатаИВзносыПоФизлицам(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
	// Начисления зарплаты по физлицам (основная зарплата) - Статья = ОплатаТруда
	"ВЫБРАТЬ
	|	&Период КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	Строки.Организация КАК Организация,
	...
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	|	И НЕ Строки.ВидОперации В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательстваИРезервы), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.КомпенсацияЕжегодногоОтпускаОценочныеОбязательства), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательства), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ЕжегодныйОтпускРезервы))
	|
	|СГРУППИРОВАТЬ ПО
	|	Строки.Организация,
	|	Строки.ФизическоеЛицо,
	...
	|	Строки.ПодразделениеПредприятия,
	|	Строки.СпособОтраженияЗарплатыВБухучете,
	|	Строки.СпособОтраженияЗарплатыВБухучете.СтатьяРасходовВзносов,
	|	Строки.Налог,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.НачисленныйЕСВ),
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.Налоги),
	...
	|	Начисления.СпособОтраженияЗарплатыВБухучете.СтатьяРасходов,
	...
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленныйНДФЛ КАК НДФЛ
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Начисления
	|		ПО НДФЛ.Ссылка = Начисления.Ссылка
	|			И НДФЛ.ФизическоеЛицо = Начисления.ФизическоеЛицо
	|			И НДФЛ.ПодразделениеПредприятия = Начисления.ПодразделениеПредприятия
	|ГДЕ
	|	НДФЛ.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
