...
Функция ТекстЗапросаТаблицаА_НачисленнаяЗарплатаИВзносыПоФизлицам(Запрос, ТекстыЗапроса, Регистры)

	ИмяРегистра = "А_НачисленнаяЗарплатаИВзносыПоФизлицам";
	...
	ТекстЗапроса =
	// ═══════════════════════════════════════════════════════════════════
	// БЛОК 1: Начисления зарплаты (Оплата труда)
	// ИЗМЕНЕНИЕ: Исключаем строки с удержаниями (оставляем только начисления)
	// ═══════════════════════════════════════════════════════════════════
	"ВЫБРАТЬ
	|	&Период КАК Период,
	...
	|	Строки.СпособОтраженияЗарплатыВБухучете КАК СпособОтраженияЗарплатыВБухучете,
	|	Строки.СпособОтраженияЗарплатыВБухучете.СтатьяРасходов КАК СтатьяРасходов,
	...
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	|	И НЕ Строки.СпособОтраженияЗарплатыВБухучете.А_ЭтоУдержание
	|
	|СГРУППИРОВАТЬ ПО
	|	Строки.ФизическоеЛицо,
	|	Строки.ПодразделениеПредприятия,
	|	Строки.СпособОтраженияЗарплатыВБухучете,
	|	Строки.СпособОтраженияЗарплатыВБухучете.СтатьяРасходов,
	|	Строки.Ссылка.Организация
	|
	|ИМЕЮЩИЕ
	|	СУММА(Строки.Сумма) <> 0
	|
	...
	|	Строки.ПодразделениеПредприятия,
	|	Строки.СпособОтраженияЗарплатыВБухучете,
	|	ЕСТЬNULL(Строки.СпособОтраженияЗарплатыВБухучете.СтатьяРасходовВзносов, Строки.СпособОтраженияЗарплатыВБухучете.СтатьяРасходов),
	|	Строки.Налог,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.НачисленныйЕСВ),
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.Налоги),
	|	СУММА(Строки.ВзносыВсего)
	|ИЗ
	|	Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК Строки
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
	...
	|	НДФЛ.ФизическоеЛицо,
	|	НДФЛ.ПодразделениеПредприятия,
	|	НДФЛ.СпособОтраженияЗарплатыВБухучете,
	|	ЕСТЬNULL(НДФЛ.СпособОтраженияЗарплатыВБухучете.СтатьяРасходовВзносов, НДФЛ.СпособОтраженияЗарплатыВБухучете.СтатьяРасходов),
	|	ВЫБОР
	|		КОГДА НДФЛ.ТипНалога = ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.ВоенныйСбор)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.Налоги.ВоенныйСбор)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Налоги.ПустаяСсылка)
	|	КОНЕЦ,
	|	НДФЛ.ТипНалога,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.Налоги),
	|	СУММА(НДФЛ.Сумма)
...
// А_фикс (2026-06-04): типовой запрос не выбирал Подразделение → движение РН.ДенежныеСредстваУПодотчетныхЛиц
// формировалось с пустым подразделением (остаток не закрывался по подразделению). Добавлена колонка
// УдержаннаяЗарплата.ПодразделениеПредприятия КАК Подразделение. См. docs/superpowers/specs/2026-06-04-podzvitne-vidoperacii-register-design.md
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		ТекстЗапросаТаблицаА_НачисленнаяЗарплатаИВзносыПоФизлицам(Запрос, ТекстыЗапроса, Регистры);
...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	МеханизмыДокумента.Добавить("А_УчетНачисленнойЗарплаты");
