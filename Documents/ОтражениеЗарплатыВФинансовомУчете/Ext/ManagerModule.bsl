...
Процедура ОтладкаА_Сотрудник(СсылкаНаДокумент) Экспорт
	...
	// 3. Перевірка ВТТаблицаДокумента (джерело для ВТНачисленнаяЗарплатаИВзносы)
	Сообщить("=== 3. ВТТаблицаДокумента (для НЕ Сводно) ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ЗарплатаДетально.ПодразделениеПредприятия.Наименование КАК Подразделение,
	|	ЗарплатаДетально.ВидОперации КАК ВидОперации,
	|	ВЫБОР КОГДА &ПроводкиПоРаботникам ТОГДА
	...
	|ГДЕ
	|	НЕ &Сводно
	|	И ЗарплатаДетально.Ссылка = &Ссылка
	|СГРУППИРОВАТЬ ПО
	|	ЗарплатаДетально.ПодразделениеПредприятия,
	|	ЗарплатаДетально.ВидОперации,
	|	ВЫБОР КОГДА &ПроводкиПоРаботникам ТОГДА
	...
	Результат = Запрос.Выполнить().Выгрузить();
	Для Каждого Стр Из Результат Цикл
		Сообщить("  Підр: " + Стр.Подразделение + " | ВидОп: " + Стр.ВидОперации + " | ФізОсоба: " + Стр.ФизическоеЛицо + " | Сума: " + Стр.Сумма);
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());
	Сообщить("");

	// 4. Перевірка ВТНачисленнаяЗарплатаИВзносы (фінальна таблиця для JOIN)
	Сообщить("=== 4. Симуляція ВТНачисленнаяЗарплатаИВзносы (з групуванням) ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	|			ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)
	|		КОНЕЦ КАК ФизическоеЛицо,
	|		ЗарплатаДетально.Сумма КАК Сумма
	|	ИЗ
	|		Документ.ОтражениеЗарплатыВФинансовомУчете.НачисленнаяЗарплатаИВзносыПоФизлицам КАК ЗарплатаДетально
	|	ГДЕ
	|		НЕ &Сводно
	|		И ЗарплатаДетально.Ссылка = &Ссылка) КАК Т
	|СГРУППИРОВАТЬ ПО
	|	Т.Подразделение,
	|	Т.ВидОперации,
	|	Т.ФизическоеЛицо
	|;
	|ВЫБРАТЬ * ИЗ ВТТаблицаДокумента";
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДокумент);
	Запрос.УстановитьПараметр("Сводно", СсылкаНаДокумент.Сводно);
	Запрос.УстановитьПараметр("ПроводкиПоРаботникам", СсылкаНаДокумент.ПроводкиПоРаботникам);
	Результат = Запрос.Выполнить().Выгрузить();
	...
	Сообщить("=== 6. Поточні рухи регістру ПрочиеАктивыПассивы ===");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	Для Каждого Стр Из Результат Цикл
		...
		Сообщить("  Підр: " + Стр.Подразделение + " | Сума: " + Стр.Сумма + " | А_Сотрудник: " + ?(А_СотрудникПустой, "<<<ПУСТО>>>", Стр.А_Сотрудник));
	КонецЦикла;
	Сообщить("Всього рядків: " + Результат.Количество());
