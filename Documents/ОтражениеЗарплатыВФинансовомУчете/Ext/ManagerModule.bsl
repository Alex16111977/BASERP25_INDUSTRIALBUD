...
Функция ТекстЗапросаТаблицаА_НачисленнаяЗарплатаИВзносыПоФизлицам(Запрос, ТекстыЗапроса, Регистры)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Взносы ЕСВ по физлицам (из начислений) - Статья = Налоги
	|ВЫБРАТЬ
	|	&Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
	|	Строки.Организация,
	...
	|	Строки.ПодразделениеПредприятия,
	|	Строки.СпособОтраженияЗарплатыВБухучете,
	|	Строки.СпособОтраженияЗарплатыВБухучете.СтатьяРасходовВзносы,
	|	Строки.Налог,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.НачисленныйЕСВ),
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.Налоги),
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// НДФЛ по физлицам - Статья = Налоги
	|ВЫБРАТЬ
	|	&Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
	|	&Организация,
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Удержания по физлицам - Статья = ОплатаТруда
	|ВЫБРАТЬ
	|	&Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	&Организация,
