...
Процедура ПоступлениеПоЗаказамПриИзменении(Элемент)
		...
		Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
			...
			Если Объект.Товары.Количество() = 0 И Объект.ВозвратныеОтходы.Количество() = 0 Тогда
				Если ПродукцияИОтходыВОднойТЧ Тогда
					ТекстВопроса = НСтр("ru='Продукция будет перезаполнена по заказам. Продолжить?';uk='Продукція буде перезаповнена за замовленням. Продовжити?'");
				Иначе
			...
			Иначе
				Если ПродукцияИОтходыВОднойТЧ Тогда
					ТекстВопроса = НСтр("ru='Продукция будет привязана к заказам. Продолжить?';uk='Продукція буде прив''язана до замовлень. Продовжити?'");
				Иначе
	...
	Иначе
		...
		Если Объект.Товары.Количество() > 0 ИЛИ Объект.ВозвратныеОтходы.Количество() > 0 Тогда
			...
			Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
				Если ПродукцияИОтходыВОднойТЧ Тогда
					ТекстВопроса = НСтр("ru='Продукция перестанет быть связанной с заказами. Продолжить?';uk='Продукція перестане бути пов''язаною із замовленнями. Продовжити?'");
				Иначе
			...
			Иначе
				Если ПродукцияИОтходыВОднойТЧ Тогда
					ТекстВопроса = НСтр("ru='Продукция перестанет быть связанной с заказом. Продолжить?';uk='Продукція перестане бути пов''язаною із замовленням. Продовжити?'");
				Иначе
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ПоступлениеОтПереработчика.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
&НаКлиенте
Процедура НалоговоеНазначениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
