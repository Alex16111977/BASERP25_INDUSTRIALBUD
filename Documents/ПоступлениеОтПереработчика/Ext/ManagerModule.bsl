	...
	Иначе
		...
		Пока ВыборкаОтходы.Следующий() Цикл
			...
			Если НоваяВалютаЗаполнена 
				...
				СуммаПоЗаказу = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
...
Функция ДанныеЗаполненияНакладной(МассивСсылок, СвойстваЗаказов = Неопределено) Экспорт
	...
	РеквизитыШапки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(МассивСсылок[0],
        ...
        |НалоговоеНазначение, Подразделение, ВариантПриемкиТоваров, УчетСырьяПоНазначениям");
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Поступление продукции';uk='Надходження продукції'");
...
#Область СгенерироватьКлючиАналитикиНоменклатуры
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("619ceabf-85b5-431e-9eb4-5373aaf84f15");	
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	НоваяСтрока.ИмяОбъекта = "Документы.ПоступлениеОтПереработчика";
