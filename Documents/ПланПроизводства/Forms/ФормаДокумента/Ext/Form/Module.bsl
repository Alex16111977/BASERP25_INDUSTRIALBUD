...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_КорректировкаПлановыхПотребностей" Тогда
		...
		Если Параметр.Свойство("План")
			...
			ТекстВопроса = НСтр("ru='Потребность в продукции изменилась, перезаполнить табличную часть?';uk='Потреба в продукції змінилася, перезаповнити табличну частину?'");
...
Процедура ПланПроизводстваНеРассчитанЕстьОшибкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ПовторитьРасчет" Тогда
		...
		ДобавитьЗаданиеКРасчетуПланаПроизводства(Объект.Сценарий ,Объект.Ссылка);
...
Процедура ЗагрузитьИзВнешнегоФайла(Команда)
	...
	ПараметрыЗагрузки.Подразделение = Объект.Подразделение;
...
Процедура ПереключитьПериодыВКолонки(Команда)
	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		ПараметрыПроверки.Вставить("ПредставлениеДатыПериода", ?(ЭтоРазборка, НСтр("ru='Дата запуска';uk='Дата запуску'"), НСтр("ru='Дата выпуска';uk='Дата випуску'")));
		...
		Если Отказ Тогда
			...
			ТекстВопроса = СтрШаблон(
				НСтр("ru='В строках списка ""%1"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';uk='У рядках списку ""%1"" вказані дати за межами планування - рядки будуть вилучені. Вилучити рядки?'"),
...
Процедура ЗапланироватьПолуфабрикаты(Команда)
	...
	Если ЕстьПолуфабрикаты Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru='Все полуфабрикаты будут перепланированы. Продолжить?';uk='Всі напівфабрикати будуть переплановані. Продовжити?'"), Кнопки);
		Возврат;
...
//++ НЕ УТКА
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru='Для выполнения действия необходимо выбрать строку';uk='Для виконання дії необхідно вибрати рядок'");
...
Процедура ПодобратьАналоги(Команда)
	...
	Если Объект.КроссТаблица Тогда
		...
		ПоказатьВопрос(
			ОписаниеОповещение, 
			НСтр("ru='Подбор аналогов возможно в режиме ""Периоды в строках"". Переключить режим?';uk='Підбір аналогів можливий в режимі ""Періоди в рядках"". Переключити режим?'"), 
			РежимДиалогаВопрос.ДаНет, 
			45, 
			КодВозвратаДиалога.Нет, 
			НСтр("ru='Подбор аналогов';uk='Підбір аналогів'"), 
			КодВозвратаДиалога.Нет
		);
	...
	Иначе
		...
		Если ВыбранныеНазначения.Количество() > 1 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Необходимо выбрать строки с одинаковым назначением.';uk='Необхідно вибрати рядки з однаковим призначенням.'"), 45);
		Иначе	
...
Процедура ЗаполнитьСпецификацию(Команда)
	...
	Если Элементы[ОписаниеТаблицы.ИмяТЧ].ВыделенныеСтроки.Количество() = 0 Тогда
	...
	ЗаполнитьСпецификациюВСтрокахПродукции(Элементы[ОписаниеТаблицы.ИмяТЧ].ВыделенныеСтроки);
...
Процедура ДобавитьЗаданиеКРасчетуПланаПроизводства(Сценарий, Ссылка)
	
	РегистрыСведений.ОчередьРасчетаПланаПроизводства.ДобавитьЗадание(Сценарий, Ссылка, Истина);
...
Процедура ПродукцияНоменклатураПриИзмененииНаСервере(ИдентификаторСтроки, КэшированныеЗначения)
	...
	ПараметрыЗаполнения.ДанныеОбИзделии.НачалоПроизводства =
		?(ЗаполнятьСпецификациюПоПериодам И НЕ Объект.КроссТаблица, ТекущаяСтрока[?(ЭтоРазборка, "ДатаЗапуска", "ДатаВыпуска")], Объект.НачалоПериода);
...
Процедура ПродукцияДатаВыпускаПриИзмененииНаСервере(ИдентификаторСтроки, КэшированныеЗначения)
	...
	ПараметрыЗаполнения.ДанныеОбИзделии.НачалоПроизводства =
		?(ЗаполнятьСпецификациюПоПериодам И НЕ Объект.КроссТаблица, ТекущиеДанные[?(ЭтоРазборка, "ДатаЗапуска", "ДатаВыпуска")], Объект.НачалоПериода);
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСпецификацию", ПараметрыЗаполнения);
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План производства +15%';uk='План виробництва +15%'"),
		"[ПланыПроизводства] * 1.15");
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров,
		НСтр("ru='Максимальный план производства с учетом корректировки продаж +10%';uk='Максимальний план виробництва з урахуванням коригування продажів +10%'"),
		"(Макс([ПланыПроизводства], "
		+ ?(ИспользоватьПланыПродаж,"[ПланыПродаж])","[ФактыПродаж] / [ДоляДнейНаличияТовараНаОстатках])")
		+ ?(ИспользоватьПланыОстатков," - [ПланыОстатков]) * 1.1",
...
Процедура ОткрытьПодборАналогов()
	...
	АналогиМатериаловКлиент.ОткрытьПодборАналогов(ПараметрыВыбораАналогов(ИмяОбъекта, ИмяТаблицы), ЭтаФорма);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище)
	...
	Результат = АналогиМатериалов.ВыполнитьЗаменуНаАналоги(
		?(Объект.КроссТаблица, ПродукцияПоДатам, Объект.Продукция), АдресВХранилище, "Назначение, ДатаВыпуска"
	);
...
&НаСервере
Функция ПараметрыВыбораАналогов(ИмяОбъекта, ИмяТаблицы)
	...
	ДанныеТоваров = ПолучитьДанныеТоваров(ИмяОбъекта, ИмяТаблицы);
...
&НаСервере
Функция ПолучитьДанныеТоваров(ИмяОбъекта, ИмяТаблицы)
	
	СтруктураДанных = Новый Структура;
	СтруктураДанных.Вставить("Ссылка", Объект.Ссылка);
	
	СтруктураПолей = АналогиМатериалов.СтруктураПолейТаблицыДляЗаменыНаАналоги("Документ.ПланПроизводства");
	
	ТоварыАдресВХранилище = ПроизводствоСервер.ПоместитьВоВременноеХранилищеДанныеСтрок(
		ЭтаФорма, ИмяОбъекта, ИмяТаблицы, Истина, СтруктураПолей, ОтборСтрокЗаменаНаАналоги());			
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Спецификация не подобрана';uk='Специфікацію не підібрано'"));
...
Процедура УстановитьВидимость()
	...
	Элементы.ПродукцияЗагрузитьИзВнешнегоФайлаПоФормуле.Видимость = Объект.ЗаполнятьПоФормуле И Не Объект.ЗаполнятьПоДефициту;
...
Процедура КонтрольРассчетаПланаПроизводства()
	...
	Если ЗаблокироватьСписокПолуфабрикатов Тогда
		...
		ПолуфабрикатыСостояниеРасчета = НСтр("ru='рассчитываются';uk='розраховуються'");
...
Процедура УстановитьОтборПоВхождениямПолуфабриката()
	...
	Если ПродукцияВхождения.Количество() > 0 Тогда
		...
		Шаблон = НСтр("ru='Полуфабрикат %1 входит в продукцию %2, спецификация %3';uk='Напівфабрикат %1 входить в продукцію %2, специфікація %3'");
...
Процедура ПоказатьИнформационныйТекст()
	...
	Если ПолуфабрикатыОтбор <> Неопределено Тогда
		...
		Шаблон = НСтр("ru='Установлен отбор по вхождениям полуфабриката %1, спецификация %2';uk='Встановлено відбір за входженням напівфабрикату %1, специфікація %2'");
		...
		КомандаОтключить = Новый ФорматированнаяСтрока(
			НСтр("ru='отключить';uk='відключити'"),
			,
			,
			, 
...
Процедура ЗаполнитьСпецификациюВСтрокахПродукции(Знач Идентификаторы = Неопределено, РежимПроверитьЗаполнить = Ложь, ТолькоНезаполненные = Ложь)
	...
	Для каждого Строка Из МассивСтрок Цикл
		...
		ДанныеОбИзделии.НачалоПроизводства =
			?(ЗаполнятьСпецификациюПоПериодам И НЕ Объект.КроссТаблица, Строка[?(ЭтоРазборка, "ДатаЗапуска", "ДатаВыпуска")], Объект.НачалоПериода);
...
Процедура НачатьИзменениеСпискаПолуфабрикатов(ВыделенныеСтроки)
	...
	Если Не ЗначениеЗаполнено(ВыделенныеСтроки) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не выбрана строка (-и) для выполнения действий';uk='Не вибрани рядок(-и) для виконання дій'"));
		Возврат;
	...
	Если НЕ Объект.Проведен ИЛИ Модифицированность Тогда
		...
		Кнопки = Новый СписокЗначений();
		ТекстВопроса = НСтр("ru='Перед началом ввода изменений необходимо провести документ. Провести?';uk='Перед початком введення змін необхідно провести документ. Провести?'");
		Кнопки.Добавить(КодВозвратаДиалога.Да,НСтр("ru='Провести';uk='Провести'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена,НСтр("ru='Отмена';uk='Відмінити'"));
