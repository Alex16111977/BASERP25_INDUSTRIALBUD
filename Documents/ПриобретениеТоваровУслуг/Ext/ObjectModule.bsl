...
//***ERP
Процедура ОбновитьПоступлениеНаПослеОбмена() Экспорт
	
	
	 Если ЗначениеЗаполнено(А_РеализацияТоваровУслуг)
	 ИЛИ ЗначениеЗаполнено(А_ОказаниеУслугМеждуПодразделениями)
	 ИЛИ ЗначениеЗаполнено(А_ПриходныйКассовыйОрдерКомиссия)
	 ИЛИ ЗначениеЗаполнено(А_ПриходДенегОтФинАгента)
	 ИЛИ ЗначениеЗаполнено(А_АвансовыйОтчет)
 ...
 Если ЗначениеЗаполнено(ЗаказПоставщику) Тогда
	 ...
	 Если А_Услуги.Количество()>0 Тогда
		 А_СтатьяДвиженияДенежныхСредств = ЗаказПоставщику.А_СтатьяДвиженияДенежныхСредств;
	 Иначе
		 А_СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка();
	 КонецЕсли;
 Иначе
	 А_СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка(); 
 КонецЕсли;
 
 //Если ЗначениеЗаполнено(Договор.Подразделение) И ЗначениеЗаполнено(Договор.А_ПодразделениеБанк) Тогда
	 
	 Подразделение = Договор.Подразделение;
	 
 //ИначеЕсли ЗначениеЗаполнено(А_НаправлениеДеятельности) И ЗначениеЗаполнено(А_Подразделение) Тогда         
 //    
 ...
 //КонецЕсли; 
   ...
   ПодразделениеШапка = Подразделение;
	...
	Для каждого СтрокаИсточник Из А_Товары Цикл
		...
		СтрокаТовары.КоличествоУпаковок = СтрокаИсточник.Количество ;
		...
		Попытка
			Если ТипЗнч(СтрокаИсточник.СтавкаНДС) = Тип("ПеречислениеСсылка.СтавкиНДС") Тогда
				...
				Если СтрокаИсточник.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС И НЕ Контрагент.ПлательщикНДС Тогда
        ...
        СтрокаТовары.ЗаказПоставщику = ЗаказПоставщику;
		
		СтрокаТовары.НалоговоеНазначение = Справочники.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая;

	КонецЦикла;
	
	// Переносим данные из А_Оборудование в Товары
	Для каждого СтрокаИсточник Из А_Оборудование Цикл
	...
	Для каждого СтрокаУслуга Из А_Услуги Цикл
		...
		//СтрокаТовары.СтатьяРасходов = СтрокаУслуга.СтатьяРасходов;
		СтрокаТовары.Подразделение = ПодразделениеШапка;
		...
		Если ЗначениеЗаполнено(СтрокаУслуга.СтатьяРасходов) 
			И ЗначениеЗаполнено(СтрокаУслуга.СтатьяРасходов.А_СтатьяРасходовДляРасходовБудущегоПериода) Тогда
		
		СтрокаТовары.СтатьяРасходов = СтрокаУслуга.СтатьяРасходов.А_СтатьяРасходовДляРасходовБудущегоПериода;	
	Иначе
		 СтрокаТовары.СтатьяРасходов = СтрокаУслуга.СтатьяРасходов;
	    КонецЕсли;    
		
		Если  ЗначениеЗаполнено(А_СтатьяДвиженияДенежныхСредств)
			И ЗначениеЗаполнено(А_СтатьяДвиженияДенежныхСредств.А_СтатьяРасходов) Тогда
		
			
		  СтрокаТовары.СтатьяРасходов = А_СтатьяДвиженияДенежныхСредств.А_СтатьяРасходов;
		...
		Попытка
			Если ТипЗнч(СтрокаУслуга.СтавкаНДС) = Тип("ПеречислениеСсылка.СтавкиНДС") Тогда
				// Конвертируем перечисление в справочник через типовую функцию     
				Если СтрокаУслуга.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС И НЕ Контрагент.ПлательщикНДС Тогда
					 СтрокаТовары.СтавкаНДС = НДСКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(Перечисления.СтавкиНДС.НеНДС);
				  Иначе
	...
	//Если ЗначениеЗаполнено(ЗаказПоставщику) Тогда
	//	   
	//	Док_ЗаказПоставщику=ЗаказПоставщику.ПолучитьОбъект();
	//	Док_ЗаказПоставщику.Товары.Загрузить(Товары.Выгрузить());
	//	Док_ЗаказПоставщику.А_Необновлять = Истина;
	//	Док_ЗаказПоставщику.Склад = Склад;
	//	Док_ЗаказПоставщику.ЦенаВключаетНДС = ЦенаВключаетНДС;
	//	Док_ЗаказПоставщику.УстановитьПометкуУдаления(Ложь);
	//	Док_ЗаказПоставщику.ОбменДанными.Загрузка = Истина;
	//	Док_ЗаказПоставщику.Записать(РежимЗаписиДокумента.Запись);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьПоступлениеНаПослеОбмена(); 
	//***ERP
