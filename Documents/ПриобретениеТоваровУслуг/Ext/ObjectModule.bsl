...
//***ERP
Процедура ОбновитьПоступлениеНаПослеОбмена(Док) Экспорт
 ...
 Если ЗначениеЗаполнено(Док.ЗаказПоставщику) Тогда
	 Док.ПоступлениеПоЗаказам = Истина;   
 КонецЕсли;
 
 Если ЗначениеЗаполнено(Док.Договор.Подразделение) И ЗначениеЗаполнено(Док.Договор.А_ПодразделениеБанк) Тогда
	 
	 Док.Подразделение = Док.Договор.Подразделение;
	 
 ИначеЕсли ЗначениеЗаполнено(Док.А_НаправлениеДеятельности) И ЗначениеЗаполнено(Док.А_Подразделение) Тогда         
	 
	Док.Подразделение =  А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны(Док.А_НаправлениеДеятельности,Док.А_Подразделение); 
	
 КонецЕсли; 
 
   ПодразделениеШапка = Док.Подразделение;
	...
	Если ЗначениеЗаполнено(Док.ЗаказПоставщику) Тогда
		...
		//	НайденнаяСтрока = Док.ЗаказПоставщику.Товары.Найти(СтрокаТабличнойЧасти.Номенклатура,"Номенклатура");
		...
		Док_ЗаказПоставщику.Товары.Загрузить(Док.Товары.Выгрузить());
		Док_ЗаказПоставщику.А_Необновлять = Истина;
		Док_ЗаказПоставщику.Склад = Док.Склад;
		Док_ЗаказПоставщику.ЦенаВключаетНДС = Док.ЦенаВключаетНДС;
		Док_ЗаказПоставщику.УстановитьПометкуУдаления(Ложь);
		Док_ЗаказПоставщику.Записать(РежимЗаписиДокумента.Проведение);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьПоступлениеНаПослеОбмена(ЭтотОбъект); 
	//***ERP
