...
#Область ЕРП_СоздатьОказаниеУслугМеждуПодразделениям

&НаСервере
Процедура А_ЗаполнитьРаспределениеПоА_ОказаниеУслугМеждуПодразделениямиНаСервере()
	Объект.А_Распределение.Очистить();
	Для каждого СтрокаТЗ Из Объект.А_ОказаниеУслугМеждуПодразделениями.Услуги Цикл
		СтрокаТабличнойЧасти = Объект.А_Распределение.Добавить();
		СтрокаТабличнойЧасти.Подразделение                 = СтрокаТЗ.ПодразделениеПокупатель;
		СтрокаТабличнойЧасти.Сумма                         = СтрокаТЗ.Сумма;
		СтрокаТабличнойЧасти.СтатьяДвиженияДенежныхСредств = СтрокаТЗ.СтатьяДвиженияДенежныхСредствПродавец;
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура А_ЗаполнитьРаспределениеПоА_ОказаниеУслугМеждуПодразделениями(Команда)
	А_ЗаполнитьРаспределениеПоА_ОказаниеУслугМеждуПодразделениямиНаСервере();
КонецПроцедуры

&НаСервере
Процедура А_ЗаполнитьТоварыИзРаспределениеНаСервере()

	Объект.Товары.Очистить();
	КодСтроки = 0;

	// Определяем СтавкуНДС по умолчанию: из первой строки А_Услуги (если есть),
	// иначе "Без НДС".
	СтавкаНДСПоУмолч = Неопределено;
	Если Объект.А_Услуги.Количество() > 0 И ЗначениеЗаполнено(Объект.А_Услуги[0].СтавкаНДС) Тогда
		СтавкаНДСПоУмолч = Объект.А_Услуги[0].СтавкаНДС;
	КонецЕсли;
	Если СтавкаНДСПоУмолч = Неопределено Тогда
		СтавкаНДСПоУмолч = Справочники.СтавкиНДС.НайтиПоНаименованию("Без НДС");
	КонецЕсли;

	Для Каждого СтрокаТЗ Из Объект.А_Распределение Цикл

		Если НЕ ЗначениеЗаполнено(СтрокаТЗ.СтатьяДвиженияДенежныхСредств) ИЛИ СтрокаТЗ.Сумма = 0 Тогда
			Продолжить;
		КонецЕсли;

		КодСтроки = КодСтроки + 1;
		СтатьяДДС = СтрокаТЗ.СтатьяДвиженияДенежныхСредств;

		СтрокаТовары = Объект.Товары.Добавить();

		// === Основные реквизиты ===
		СтрокаТовары.Номенклатура       = СтатьяДДС.А_Номенклатура;
		СтрокаТовары.СтатьяРасходов     = СтатьяДДС.А_СтатьяРасходов;
		СтрокаТовары.Подразделение      = СтрокаТЗ.Подразделение;
		СтрокаТовары.Количество         = 1;
		СтрокаТовары.КоличествоУпаковок = 1;

		// === СтавкаНДС (Справочник.СтавкиНДС) ===
		Если ТипЗнч(СтавкаНДСПоУмолч) = Тип("СправочникСсылка.СтавкиНДС") Тогда
			СтрокаТовары.СтавкаНДС = СтавкаНДСПоУмолч;
		ИначеЕсли ТипЗнч(СтавкаНДСПоУмолч) = Тип("ПеречислениеСсылка.СтавкиНДС") Тогда
			
			Если СтавкаНДСПоУмолч = Перечисления.СтавкиНДС.БезНДС И НЕ Контрагент.ПлательщикНДС Тогда
				СтрокаТовары.СтавкаНДС = НДСКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(Перечисления.СтавкиНДС.НеНДС);
			Иначе
				СтрокаТовары.СтавкаНДС = НДСКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(СтавкаНДСПоУмолч); 
			КонецЕсли; 
			
		Иначе
			СтрокаТовары.СтавкаНДС = Справочники.СтавкиНДС.НайтиПоНаименованию("Без НДС");
		КонецЕсли;

		// === Обратный расчёт НДС от заданной Суммы с НДС ===
		// Входящая Сумма из А_Распределение трактуется как СуммаСНДС (итоговая с НДС).
		// УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(Сумма, Ставка, СуммаВключаетНДС=Истина):
		//   СуммаНДС = Сумма − 100·Сумма / (100 + ЗначениеСтавкиНДС)
		СуммаСНДСВходящая     = СтрокаТЗ.Сумма;
		СтрокаТовары.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(
			СуммаСНДСВходящая, СтрокаТовары.СтавкаНДС, Истина);

		Если Объект.ЦенаВключаетНДС Тогда
			// Сумма уже содержит НДС внутри
			СтрокаТовары.Сумма    = СуммаСНДСВходящая;
			СтрокаТовары.Цена     = СуммаСНДСВходящая;
			СтрокаТовары.СуммаСНДС = СуммаСНДСВходящая;
		Иначе
			// Цена без НДС, НДС идёт сверху
			СуммаБезНДС            = СуммаСНДСВходящая - СтрокаТовары.СуммаНДС;
			СтрокаТовары.Сумма     = СуммаБезНДС;
			СтрокаТовары.Цена      = СуммаБезНДС;
			СтрокаТовары.СуммаСНДС = СуммаСНДСВходящая;
		КонецЕсли;

		// === КРИТИЧНО для регистров взаиморасчётов ===
		СтрокаТовары.СуммаВзаиморасчетов    = СтрокаТовары.СуммаСНДС;
		СтрокаТовары.СуммаНДСВзаиморасчетов = СтрокаТовары.СуммаНДС;
...
&НаКлиенте
Процедура А_ЗаполнитьТоварыИзРаспределение(Команда)
	А_ЗаполнитьТоварыИзРаспределениеНаСервере();
КонецПроцедуры
