...
|;uk='Для створення звіту переробника на підставі надходження за замовленнями 
|необхідно використовувати робоче місце ""Документи до оформлення"" переробки на стороні'");
	...
	ПараметрыВзаиморасчеты = Документы.ОтчетПереработчика.ПараметрыВзаиморасчеты(ПоЗаказам);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
    ...
    ПараметрыЗаполнения = Документы.ОтчетПереработчика.ПараметрыЗаполненияНалоговогоНазначения(ЭтотОбъект);
...
Функция ПроверитьТипСтоимостиПриПроизводствеПоЭтапам(Отказ, МассивНепроверяемыхРеквизитов)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого ТекСтрока Из СписокСтрок Цикл
			ТекстОшибки = НСтр("ru='Выпуск продукции по рассчитываемой стоимости недоступен, т.к. этап, к которому относится продукция, не является выпускающим.';uk='Випуск продукції за розраховуваною вартістю недоступний, тому що етап, до якого належить продукція, не є випусковим.'");
