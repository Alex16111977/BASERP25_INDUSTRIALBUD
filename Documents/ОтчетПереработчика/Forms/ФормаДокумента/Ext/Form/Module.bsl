...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ЗаказПереработчику) Тогда
		...
		ПредставлениеУслугиУказываютсяВЗаказеОтчете = НСтр("ru='Указываются в отчете';uk='Зазначаються у звіті'");
	...
	// Взаиморасчеты
	ПараметрыВзаиморасчеты = Документы.ОтчетПереработчика.ПараметрыВзаиморасчеты(Объект.ПоЗаказам);
...
Процедура ПродукцияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		...
		ЗаполнитьНомерГруппыЗатратВСтрокеПродукцииПриОкончанииРедактирования(ТекущиеДанные, КэшСтроки, НоваяСтрока);
		ЗаполнитьУслугиПоПродукции();
...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ОтчетПереработчика.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
Процедура ЗаполнитьМатериалыОтходы()
	...
	Если Объект.ПоЗаказам Тогда
		ТекстВопроса = НСтр("ru='Заполнить возвратные отходы, сырье и материалы по заказу?';uk='Заповнити зворотні відходи, сировину і матеріали за замовленням?'");
	Иначе
...
Процедура НастроитьФормуПоСпособуГруппировкиЗатрат()
	...
	ИначеЕсли Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки
		...
		ЗаголовокКомандыЗаполнитьГруппуЗатрат = НСтр("ru='Документ поступления';uk='Документ надходження'");
...
Процедура УстановитьУсловноеОформлениеСписаниеНаРасходы(ИмяТаблицы)
	...
	ПолеСписатьНаРасходы = Элементы[ИмяТаблицы + "СписатьНаРасходы"]; // ПолеФормы
	ПолеСтатьяРасходов = Элементы[ИмяТаблицы + "СтатьяРасходов"]; // ПолеФормы
	ПолеАналитикаРасходов = Элементы[ИмяТаблицы + "АналитикаРасходов"]; // ПолеФормы
	...
	ПолеНазначение = Элементы[ИмяТаблицы + "Назначение"]; // ПолеФормы
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
