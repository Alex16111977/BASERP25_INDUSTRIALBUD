...
Процедура ВеличиныБазРаспределенияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не НоваяСтрока Или НоваяСтрока И Не ОтменаРедактирования Тогда
		...
		Если Дубли.Количество() > 1 Тогда
			...
			ТекстОшибки = НСтр("ru='В списке уже существует строка с правилом распределения %1';uk='У списку вже існує рядок з правилом розподілу %1'");
...
Процедура ВеличиныБазРаспределенияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не НоваяСтрока И Не КэшСтроки = Неопределено 
		...
		Если ПараметрыУдаления.ЕстьСтрокиКУдалению Тогда
			...
			ТекстОповещения = НСтр("ru='Расходы с правилом распределения %1 будут удалены. Продолжить?';uk='Витрати з правилом розподілу %1 будуть вилучені. Продовжити?'");
...
Процедура ВеличиныБазРаспределенияПередУдалением(Элемент, Отказ)
	...
	Если ПараметрыУдаления.ЕстьСтрокиКУдалению Тогда
		...
		ТекстОповещения = НСтр("ru='Нормы расходов с правилами распределения удаляемых строк будут удалены. Продолжить?';uk='Норми витрат з правилами розподілу рядків, що вилучаються, будуть вилучені. Продовжити?'");
...
#Область ПредставлениеПравила
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Не поддерживается';uk='Не підтримується'"));
...
Процедура УстановитьЗаголовки()
	...
	ТекстЗаголовкаСуммы = НСтр("ru='Сумма (%1)';uk='Сума (%1)'");
	ТекстЗаголовкаНорматива = НСтр("ru='Норматив (%1)';uk='Норматив (%1)'");
...
Процедура ЗаполнитьСписокВыбораПравил()
	...
	СписокВыбора.Добавить("ДругиеПравила", 
		Новый ФорматированнаяСтрока(НСтр("ru='Другие правила...';uk='Інші правила ...'"),,,, "заглушка"));
...
Функция ПростыеПравилаРаспределения()
	...
	СписокПравил.Добавить("СуммаРасходовНаОплатуТруда", НСтр("ru='Сумма расходов на оплату труда';uk='Сума витрат на оплату праці'"));
	СписокПравил.Добавить("СуммаМатериальныхЗатрат", НСтр("ru='Сумма материальных затрат';uk='Сума матеріальних витрат'"));
	СписокПравил.Добавить("СуммаМатериальныхИТрудозатрат", НСтр("ru='Сумма материальных и трудозатрат';uk='Сума матеріальних і трудовитрат'"));
	СписокПравил.Добавить("ПлановаяСтоимостьПродукции", НСтр("ru='Плановая стоимость продукции';uk='Планова вартість продукції'"));
	СписокПравил.Добавить("НормативыОплатыТруда", НСтр("ru='Нормативные расходы на оплату труда';uk='Нормативні витрати на оплату праці'"));
	
	СписокПравил.Добавить("КоличествоРаботУказанныхВидов", НСтр("ru='Количество работ';uk='Кількість робіт'"));
	СписокПравил.Добавить("КоличествоУказанныхМатериалов", НСтр("ru='Количество материалов';uk='Кількість матеріалів'"));
	СписокПравил.Добавить("ВесУказанныхМатериалов", НСтр("ru='Вес материалов';uk='Вага матеріалів'"));
	СписокПравил.Добавить("ОбъемУказанныхМатериалов", НСтр("ru='Объем материалов';uk='Об''єм матеріалів'"));
	
	СписокПравил.Добавить("КоличествоПродукции", НСтр("ru='Количество продукции';uk='Кількість продукції'"));
	СписокПравил.Добавить("ВесПродукции", НСтр("ru='Вес продукции';uk='Вага продукції'"));
	СписокПравил.Добавить("ОбъемПродукции", НСтр("ru='Объем продукции';uk='Об''єм продукції'"));
...
Функция ПравилоРаспределенияСтатьиРасходов(СтатьяРасходов)
	...
	Запрос.Текст = 
		...
		|		ПО СтатьиРасходов.ПравилоРаспределенияРасходовУпр = ПравилаРаспределенияРасходов.Ссылка
		|ГДЕ
...
Функция НаименованиеПравила(Правило)
	...
	ШаблонНаименования = НСтр("ru='%1 с отбором по группам продукции: %2';uk=' %1 з відбором за групами продукції: %2'");
...
Процедура ЗадатьВопросПользователю(ОбъектЗаполнения, УточнитьКоличествоМесяцев = Ложь)
	...
	Если ОбъектЗаполнения = "ПостоянныеРасходы" И Объект.НормыПостоянныхРасходов.Количество() > 0 Тогда
		ТекстОповещения = НСтр("ru='Табличная часть ""Постоянные расходы"" будет очищена. Продолжить?';uk='Таблична частина ""Постійні витрати"" буде очищена. Продовжити?'");
	ИначеЕсли ОбъектЗаполнения = "ПеременныеРасходы" И Объект.НормыПеременныхРасходов.Количество() > 0 Тогда
		ТекстОповещения = НСтр("ru='Табличная часть ""Переменные расходы"" будет очищена. Продолжить?';uk='Таблична частина ""Змінні витрати"" буде очищена. Продовжити?'");
	ИначеЕсли Объект.НормыПостоянныхРасходов.Количество() > 0 Или Объект.НормыПеременныхРасходов.Количество() > 0 Тогда
			ТекстОповещения = НСтр("ru='Табличные части ""Переменные расходы"" и ""Постоянные расходы"" будут очищены. Продолжить?';uk='Табличні частини ""Змінні витрати"" і ""Постійні витрати"" будуть очищені. Продовжити?'");
	КонецЕсли;
...
Процедура ОпределитьПараметрыЗаполненияЗвершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(РезультатЗакрытия) Тогда
		...
		ТекстОшибки = НСтр("ru='Не указано количество месяцев за которое необходимо получить расходы. Заполнение невозможно.';uk='Не вказано кількість місяців за які необхідно отримати витрати. Заповнення неможливо.'");
