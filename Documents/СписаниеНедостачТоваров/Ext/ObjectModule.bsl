...
//***ERP
Процедура ОбновитьСписаниеНедостачТоваровПослеОбмена() Экспорт
	...
	СтатьяРасходов = А_СтатьяРасходовБух;
	...
	Если ЗначениеЗаполнено(А_СтатьяРасходовБух.А_ЗаменитьНаСтатьяРасходовПриСписании) Тогда
		СтатьяРасходов = А_СтатьяРасходовБух.А_ЗаменитьНаСтатьяРасходовПриСписании;
	КонецЕсли; 
	
	Если А_ЭтоТребованиеНакладнаяизБух Тогда
		Номер = "ВН-"+СокрЛП(А_НомерТребованияНакладнаяИзБух);
		
		//Если ЗначениеЗаполнено(Склад.Подразделение) Тогда
		//	Подразделение = Склад.Подразделение;
		//КонецЕсли;
		
		СтатьяРасходов =ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000029"); //Будівельні матеріали 
		
		Если ЗначениеЗаполнено(Склад.А_ПодразделениеПринудительноДокументСписание_ЭтоТребование) Тогда
			Подразделение = Склад.А_ПодразделениеПринудительноДокументСписание_ЭтоТребование;
		КонецЕсли;
		
	Иначе    
		
		Если ЗначениеЗаполнено(Склад.А_ПодразделениеПринудительноДокументСписание) Тогда
			Подразделение = Склад.А_ПодразделениеПринудительноДокументСписание;
		КонецЕсли;
		
		//Если ЗначениеЗаполнено(Склад.А_СтатьяЗатратПоУмолчаниюВДокументеСписания) Тогда
		//
		//СтатьяРасходов = Склад.А_СтатьяЗатратПоУмолчаниюВДокументеСписания;	
		//
		//КонецЕсли;
	КонецЕсли;  
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		//***ERP
	ОбновитьСписаниеНедостачТоваровПослеОбмена(); 
	//***ERP
