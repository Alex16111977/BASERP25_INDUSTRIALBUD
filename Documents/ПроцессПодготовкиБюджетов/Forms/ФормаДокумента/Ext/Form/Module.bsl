...
Процедура Запустить(Команда)
	...
	Если НЕ ИнформацияОДублях = Неопределено Тогда
		...
		ТекстыПредупреждения.Добавить(
			НСтр("ru='Для текущей модели, группы этапов в периоде действия бюджетного процесса уже есть дублирующиеся объекты:';uk='Для поточної моделі, групи етапів в періоді дії бюджетного процесу вже є дублюючі об''єкти:'"));
		Если ИнформацияОДублях.ЕстьДублиПроцессов Тогда
			ТекстыПредупреждения.Добавить(
				НСтр("ru='не завершенные документы ""Процесс подготовки бюджетов"".';uk='не завершені документи ""Процес підготовки бюджетів"".'"));
		КонецЕсли;
		Если ИнформацияОДублях.ЕстьДублиЗадач Тогда
			Если ТекстыПредупреждения.Количество() > 1 Тогда
				...
				ТекстыПредупреждения.Добавить(НСтр("ru='и';uk='і'"));
			КонецЕсли;
			ТекстыПредупреждения.Добавить(НСтр("ru='не выполненные задачи.';uk='невиконані задачі.'"));
		КонецЕсли;
		ТекстыПредупреждения.Добавить(НСтр("ru='Запустить дублирующий процесс?';uk='Запустити дублюючий процес?'"));
