...
Процедура ЗаменитьНаАналоги(АдресВХранилище)
	...
	Результат = АналогиМатериалов.ВыполнитьЗаменуНаАналоги(
		?(Объект.КроссТаблица, ТоварыПоПериодам, Объект.Товары), АдресВХранилище, "Назначение, ДатаПоступления");
...
Функция ПолучитьДанныеТоваров(ИмяОбъекта, ИмяТаблицы)
	...
	СтруктураПолей = АналогиМатериалов.СтруктураПолейТаблицыДляЗаменыНаАналоги("Документ.ПланЗакупок");
