...
//***ЕРП
&НаСервере
Процедура А_ЗаполнитьНачисленияИзЗПНаСервере()

	ДокументОбъект = РеквизитФормыВЗначение("Объект");

	Попытка
		ДокументОбъект.ЗаполнитьНачисленияИзБазЗП();
	Исключение
...
&НаКлиенте
Процедура А_ЗаполнитьНачисленияИзЗП(Команда)
	А_ЗаполнитьНачисленияИзЗПНаСервере();
КонецПроцедуры
