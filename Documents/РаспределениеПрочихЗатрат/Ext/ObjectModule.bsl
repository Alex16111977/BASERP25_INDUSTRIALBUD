...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение
		...
		Если НаправлениеРаспределения = Перечисления.НаправлениеРаспределенияПоПодразделениям.Вышестоящее Тогда
			ТекстОшибки = НСтр("ru='У подразделения отсутствует вышестоящее подразделение.';uk='У підрозділу відсутній підрозділ вищого рівня.'");
		Иначе
			ТекстОшибки = НСтр("ru='У подразделения отсутствуют нижестоящие подразделения.';uk='У підрозділу відсутні підпорядковані підрозділи'");
		КонецЕсли;
...
|Операція не виконана.'");
	...
	Если Не РаспределятьПоПартиям И Не РаспределятьПоПодразделениям
		...
		И НазначениеНастройкиРаспределения = Перечисления.НазначениеПравилРаспределенияРасходов.РаспределениеСтатейРасходовПоЭтапамПроизводства Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не выбрано ни одно направление распределения статьи расходов.';uk='Не вибраний жодний напрям розподілу статті витрат.'"),
				,
				"НаправлениеРаспределения",
				,
				Отказ);
	...
	Если РаспределятьНаСтатьи И Списание.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не введено ни одной строки в список ""Другие статьи""';uk='Не введено жодного рядка в список ""Інші статті""'"),
			,
			"Списание",
			,
			Отказ);
	...
	Если НазначениеНастройкиРаспределения = Перечисления.НазначениеПравилРаспределенияРасходов.РаспределениеСтатейРасходовПоЭтапамПроизводства
		...
		И ОтборПоПодразделениям.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не заданы подразделения в отборе.';uk='Не задані підрозділи у відборі.'"),
				,
				"НаправлениеРаспределения",
				,
				Отказ);
	...
	Если ПартииУказаныВручную 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указаны партии.';uk='Не вказані партії.'"),,,,	Отказ);
	...
	Если БазаРаспределенияПоПартиям = Перечисления.ТипыБазыРаспределенияРасходов.СуммаМатериальныхИТрудозатрат 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='База распределение ""Сумма материальных и трудозатрат"" доступна только для партионного учета 2.5.';uk='База розподілу ""Сума матеріальних і трудовитрат"" доступна тільки для партійного обліку 2.5.'"),
