...
//***ERP
Процедура ОбновитьРеализацияТоваровУслугПослеОбмена(Док,РежимЗаписи) Экспорт
	
	Если ЗначениеЗаполнено(А_ОказаниеУслугМеждуПодразделениями) Тогда
		А_ВведенВЕРП=Истина;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Док.ЗаказКлиента) Тогда
		Док.РеализацияПоЗаказам = Истина;
	КонецЕсли;
	
	Если А_ЭтоРеализацияУслугПоПереработкеБух Тогда
		Номер = "РУП"+Прав(СокрЛП(А_НомерРеализацияУслугПоПереработкеБух),7);
	КонецЕсли;

	Док.ГруппаФинансовогоУчета=Док.Договор.ГруппаФинансовогоУчета;
	Док.Подразделение = Док.Договор.Подразделение;
	...
	Док.СпособДоставки =  Перечисления.СпособыДоставки.Самовывоз;
	 ...
	 Если ЗначениеЗаполнено(Док.Договор.Подразделение) Тогда
	 ...
	 КонецЕсли;  
		...
		ПараметрыОР.Организация = Док.Организация;
		ПараметрыОР.Партнер     = Док.Партнер;
		ПараметрыОР.Контрагент  = Док.Контрагент;
		...
		Если ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным Тогда 
			...
			Если  ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом 
				И ЗначениеЗаполнено(Док.ЗаказКлиента) Тогда
				Источник = Док.ЗаказКлиента;   
		...
		ОбъектРасчетовШапка = ОбъектыРасчетовСервер.ПроверитьСоздатьОбъектРасчетов(ПараметрыОР);
    ...
    ЗаказКлиентаИзШапки = Док.ЗаказКлиента;
	...
	Для каждого СтрокаИсточник Из Док.А_Товары Цикл
		...
		СтрокаТовары.ЗаказКлиента = ЗаказКлиентаИзШапки; 
		 СтрокаТовары.ОбъектРасчетов = ОбъектРасчетовШапка;
		// === ИдентификаторСтроки - генерируем новый GUID ===
		СтрокаТовары.ИдентификаторСтроки = Строка(Новый УникальныйИдентификатор);
	...
	Для каждого СтрокаУслуга Из Док.А_Услуги Цикл
		...
		Если ЗначениеЗаполнено(Док.ЗаказКлиента) Тогда
		Для каждого СтрокаТабличнойЧасти Из Док.Товары Цикл
			НайденнаяСтрока = Док.ЗаказКлиента.Товары.Найти(СтрокаТабличнойЧасти.Номенклатура,"Номенклатура");
			Если НайденнаяСтрока = Неопределено Тогда
			//	СтрокаТабличнойЧасти.КодСтроки =1;
			Иначе
				СтрокаТабличнойЧасти.КодСтроки =НайденнаяСтрока.НомерСтроки;	
			Конецесли;
		КонецЦикла; 
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		//***ERP
	ОбновитьРеализацияТоваровУслугПослеОбмена(ЭтотОбъект,РежимЗаписи); 
	//***ERP
