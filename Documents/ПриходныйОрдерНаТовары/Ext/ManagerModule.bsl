...
//++ НЕ УТ
	...
	ИначеЕсли ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		...
		|	
		|	ВЫБРАТЬ
		|		ТаблицаСерииВыходныеИзделия.Серия,
		|		ТаблицаСерииВыходныеИзделия.Количество,
		|		ТаблицаСерииВыходныеИзделия.Номенклатура,
		|		ТаблицаСерииВыходныеИзделия.Характеристика,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ТаблицаСерииВыходныеИзделия.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
		|				ТОГДА ТаблицаСерииВыходныеИзделия.Назначение
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		КОНЕЦ
		|	ИЗ
		|		Документ.ЭтапПроизводства2_2.ВыходныеИзделияСерии КАК ТаблицаСерииВыходныеИзделия
		|	ГДЕ
		|		ТаблицаСерииВыходныеИзделия.Ссылка = &Распоряжение
		|		И ТаблицаСерииВыходныеИзделия.Получатель = &Склад
		|		И ТаблицаСерииВыходныеИзделия.Ссылка.Проведен
		|		И ТаблицаСерииВыходныеИзделия.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		И ПобочныеИзделия.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
		|		И НЕ ПобочныеИзделия.Отменено
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ТаблицаСерииПобочныеИзделия.Серия,
		|		ТаблицаСерииПобочныеИзделия.Количество,
		|		ТаблицаСерииПобочныеИзделия.Номенклатура,
		|		ТаблицаСерииПобочныеИзделия.Характеристика,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ТаблицаСерииПобочныеИзделия.Назначение.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
		|				ТОГДА ТаблицаСерииПобочныеИзделия.Назначение
		|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		КОНЕЦ
		|	ИЗ
		|		Документ.ЭтапПроизводства2_2.ПобочныеИзделияСерии КАК ТаблицаСерииПобочныеИзделия
		|	ГДЕ
		|		ТаблицаСерииПобочныеИзделия.Ссылка = &Распоряжение
		|		И ТаблицаСерииПобочныеИзделия.Получатель = &Склад
		|		И ТаблицаСерииПобочныеИзделия.Ссылка.Проведен
		|		И ТаблицаСерииПобочныеИзделия.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Серия,
		|	ВложенныйЗапрос.Номенклатура,
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию22(Параметры) Экспорт
	...
	|			ТОГДА ИСТИНА
	//++ НЕ УТКА
	|		КОГДА ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ВыпускПродукции
	|				И ВЫРАЗИТЬ(ПриходныйОрдерНаТовары.Распоряжение КАК Документ.ВыпускПродукции).Распоряжение = ЗНАЧЕНИЕ(Документ.МаршрутныйЛистПроизводства.ПустаяСсылка)
	|				И &ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным)
	|			ТОГДА ЛОЖЬ
	//-- НЕ УТКА
	//++ НЕ УТ
	...
	//++ НЕ УТ
	|					ИЛИ ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ЗаказПереработчику
	//++ НЕ УТКА			
	|					ИЛИ ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ЗаказДавальца
	|					ИЛИ ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ЗаказНаПроизводство
	//-- НЕ УТКА
	//-- НЕ УТ
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию22(Параметры) Экспорт
	...
	//++ НЕ УТ
	|		КОГДА ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ВозвратМатериаловИзПроизводства
	|			ТОГДА ПриходныйОрдерНаТовары.Распоряжение.Подразделение
	|		КОГДА ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ВыпускПродукции
	|			ТОГДА ПриходныйОрдерНаТовары.Распоряжение.Подразделение
	//-- НЕ УТ
	//++ НЕ УТКА
	|		КОГДА ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ЗаказНаПроизводство
	|			ТОГДА ВЫРАЗИТЬ(ПриходныйОрдерНаТовары.Распоряжение КАК Документ.ЗаказНаПроизводство).Подразделение
	//-- НЕ УТКА
	|		КОГДА ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ПеремещениеТоваров
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию24(Параметры) Экспорт
	...
	//++ НЕ УТ
	РезультатВТЗаказыПереработчику = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
	...
	|		ПО ОбъектыДляОбработки.Ссылка = ОрдерНаТовары.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеОтПереработчика.Товары КАК ДокументРаспоряжение
	|		ПО ОрдерНаТовары.Распоряжение = ДокументРаспоряжение.Ссылка
	|ГДЕ
	|	НЕ ОрдерНаТовары.ЗакрыватьГрафикПоступления
	|	И ДокументРаспоряжение.Ссылка.ПоступлениеПоЗаказам
	...
	|		ПО ОбъектыДляОбработки.Ссылка = ОрдерНаТовары.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеСырьяОтДавальца.Товары КАК ДокументРаспоряжение
	|		ПО ОрдерНаТовары.Распоряжение = ДокументРаспоряжение.Ссылка
	|ГДЕ
	|	НЕ ОрдерНаТовары.ЗакрыватьГрафикПоступления
	|	И ДокументРаспоряжение.Ссылка.ПоступлениеПоЗаказам
	...
	//++ НЕ УТ	
	|	ЛОЖЬ КАК ТребуетсяИзменениеСкладскойОперации,
