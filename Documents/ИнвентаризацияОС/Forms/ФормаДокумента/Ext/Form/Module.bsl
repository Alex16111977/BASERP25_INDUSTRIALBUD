...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ИнвентаризацияНезавершенногоСтроительства") 
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.ОК, НСтр("ru='Очистить и продолжить';uk='Очистити і продовжити'"));
...
|;uk='Таблична частина документа буде очищена,
|тому що на обрану дату недоступний облік об''єктів будівництва'"),
			СписокКнопок);
	Иначе
...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	Если (Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ИнвентаризацияНезавершенногоСтроительства")
		...
		ПоказатьВопрос(
			...
			НСтр("ru='При изменении операции табличная часть документа будет очищена';uk='При зміні операції табличну частину документа буде очищено'"),
			СписокКнопок);
	Иначе
...
Процедура ИнвентаризационнаяКомиссияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Строки.Количество() > 0 Тогда
		ШаблонСообщения = НСтр("ru='Физическое лицо <%1> уже подобрано';uk='Фізичну особу <%1> вже підібрано'");
...
Процедура ИнвентаризационнаяКомиссияФизЛицоОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущаяСтрокаТЧ.ФизЛицо <> ВыбранноеЗначение Тогда
		...
		Если СтрокиТабличнойЧасти.Количество() > 0 Тогда
			СтандартнаяОбработка = Ложь;
			ШаблонСообщения = НСтр("ru='Физическое лицо <%1> уже включено в состав комиссии';uk='Фізичну особа <%1> вже включено до складу комісії'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ВыбранноеЗначение);
			ПоказатьПредупреждение(Неопределено, ТекстСообщения, 60);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоОстаткам(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ХозяйственнаяОперация) Тогда
		ТекстСообщения = НСтр("ru='Необходимо заполнить операцию';uk='Необхідно заповнити операцію'");
...
Процедура ЗаполнитьТабличнуюЧасть(ОписаниеОповещения, ТолькоВыбранныеСтроки)
	...
	Если ТолькоВыбранныеСтроки Тогда
		Если Элементы.ОС.ВыделенныеСтроки.Количество() <> 0 Тогда
			ТекстВопроса = НСтр("ru='При заполнении существующие данные в выбранных строках будут перезаполнены.
|Продолжить?'
|;uk='При заповненні існуючі дані в обраних рядках будуть перезаповнені.
|Продовжити?'");
		КонецЕсли; 
	ИначеЕсли Объект.ОС.Количество() <> 0 Тогда
		ТекстВопроса = НСтр("ru='При заполнении существующие данные будут перезаполнены.
|Продолжить?'
|;uk='При заповненні існуючі дані будуть перезаповнені. 
|Продовжити?'");
	КонецЕсли; 
...
|;uk='Для створення документа необхідно попередньо провести документ.
|Провести і продовжити?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Перемещение невозможно, организации не взаимосвязаны';uk='Переміщення неможливо, організації не взаємопов''язані'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветНедоступногоТекста);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='ОС списан, повторное принятие к учету в ту же организацию не поддерживается';uk='ОЗ списано, повторне прийняття до обліку в ту ж організацію не підтримується'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ХозяйственнаяОперация")
		...
		Элементы.ОСОбъектУчета.Заголовок = 
			?(ЭтоИнвентаризацияСтроительства, НСтр("ru='Объект строительства';uk='Об''єкт будівництва'"), НСтр("ru='Основное средство';uk='Основний засіб'"));
			
		Элементы.СтраницаОсновныеСредства.Заголовок = 
			?(ЭтоИнвентаризацияСтроительства, НСтр("ru='Объекты строительства';uk='Об''єкти будівництва'"), НСтр("ru='Основные средства';uk='Основні засоби'"));
		...
		Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ИнвентаризацияВложенийВОС")
			...
			Элементы.ИтогиСуммаПоДаннымУчета.Заголовок = НСтр("ru='Затраты по учету';uk='Витрати за обліком'");
			Элементы.ИтогиСуммаФактическая.Заголовок = НСтр("ru='Затраты по факту';uk='Витрати за фактом'");
			Элементы.ОССтоимостьПоДаннымУчета.Подсказка = НСтр("ru='Наличие и сумма затрат по данным учета';uk='Наявність і сума витрат за даними обліку'");
			Элементы.ОССтоимостьФактическая.Подсказка = НСтр("ru='Наличие и сумма затрат по факту';uk='Наявність і сума витрат за фактом'");
		Иначе
			Элементы.ИтогиСуммаПоДаннымУчета.Заголовок = НСтр("ru='Стоимость по учету';uk='Вартість за обліком'");
			Элементы.ИтогиСуммаФактическая.Заголовок = НСтр("ru='Стоимость по факту';uk='Вартість за фактом'");
			Элементы.ОССтоимостьПоДаннымУчета.Подсказка = НСтр("ru='Наличие и стоимость по данным учета';uk='Наявність і вартість за даними обліку'");
			Элементы.ОССтоимостьФактическая.Подсказка = НСтр("ru='Наличие и стоимость по факту';uk='Наявність і вартість за фактом'");
		КонецЕсли; 
