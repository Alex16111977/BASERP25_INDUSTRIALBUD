...
#Область А_РасшифровкаВыплатыЗарплатаПоФизлицам
...
Процедура А_РасшифровкаЗакрытаНаСервере(АдрХр, ФЛ)
	...
	// 1. UID Состава этого ФЛ — общий ключ для всех трёх ТЧ.
	UIDСостава    = Неопределено;
	СтрокаСостава = Неопределено;
	НайденныеСостав = Объект.Состав.НайтиСтроки(Новый Структура("ФизическоеЛицо", ФЛ));
	Если НайденныеСостав.Количество() > 0 Тогда
		СтрокаСостава = НайденныеСостав[0];
		UIDСостава    = СтрокаСостава.ИдентификаторСтроки;
	КонецЕсли;

	// 2. Перестроить А_Расшифровку для ФЛ (нормализуя ИдентификаторСтроки к UID Состава).
	...
	Для Каждого СтрН Из НоваяТЗ Цикл
		...
		НоваяСтр.ФизическоеЛицо = ФЛ;
		Если ЗначениеЗаполнено(UIDСостава) Тогда
			НоваяСтр.ИдентификаторСтроки = UIDСостава;
		ИначеЕсли НЕ ЗначениеЗаполнено(НоваяСтр.ИдентификаторСтроки) Тогда
			НоваяСтр.ИдентификаторСтроки = Новый УникальныйИдентификатор();
		КонецЕсли;
	КонецЦикла;

	// Без строки Состава синхронизировать Зарплату/агрегат невозможно — выходим.
	Если НЕ ЗначениеЗаполнено(UIDСостава) Тогда
		Возврат;
	КонецЕсли;

	// 3. Перестроить ТЧ Зарплата для ФЛ как ОДНУ свёрнутую строку.
	УдаляемыеЗ = Объект.Зарплата.НайтиСтроки(
		Новый Структура("ИдентификаторСтроки", UIDСостава));
	Для Каждого СтрУдЗ Из УдаляемыеЗ Цикл
		Объект.Зарплата.Удалить(СтрУдЗ);
	КонецЦикла;

	Если НоваяТЗ.Количество() > 0 Тогда
		НовЗ = Объект.Зарплата.Добавить();
		ЗаполнитьЗначенияСвойств(НовЗ, НоваяТЗ[0]); // зеркальные реквизиты из первой строки
		НовЗ.ФизическоеЛицо      = ФЛ;
		НовЗ.КВыплате            = НоваяТЗ.Итог("КВыплате"); // свёртка
		НовЗ.ИдентификаторСтроки = UIDСостава;
	КонецЕсли;

	// 4. Пересчитать агрегат Состава (КВыплате = Σ Зарплата + представления Финансирование/НДФЛ).
	ПриПолученииДанныхСтрокиСостава(СтрокаСостава);

	// 5. Пересчитать итог документа.
	Объект.СуммаПоДокументу = Объект.Зарплата.Итог("КВыплате");
