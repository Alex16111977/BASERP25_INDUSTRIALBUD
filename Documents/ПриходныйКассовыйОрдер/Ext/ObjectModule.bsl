...
Процедура ОбновитьПриходныйКассовыйОрдерПослеОбмена() Экспорт
	...
	Если А_ОбработанКазна
		ИЛИ А_Необновлять Тогда
	...
	СтатьяДвиженияДенежныхСредствШапка = А_СтатьяДвиженияДенежныхСредствКазна; 
	Если НЕ ЗначениеЗаполнено(СтатьяДвиженияДенежныхСредствШапка) Тогда
		  Если ЗначениеЗаполнено(А_ФинАгент) Тогда
		   СтатьяДвиженияДенежныхСредствШапка = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("УТ-001082");   //Поступление от Фин агента
	   Иначе
		   СтатьяДвиженияДенежныхСредствШапка = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("УТ-001210");   //Транзит Вх
	   КонецЕсли;
	 КонецЕсли;  
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Если ЗначениеЗаполнено(Контрагент) Тогда
			...
			Если НЕ ЗначениеЗаполнено(Партнер)
				ИЛИ ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Контрагент").Контрагент <> Контрагент Тогда
				Партнер = ПартнерПолученныйПоКонтрагенту;
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента
		ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика Тогда
		Если А_РасшифровкаФинАгент.Количество()>0 Тогда 
			СуммаДокумента = А_РасшифровкаФинАгент.Итог("Сумма")+А_РасшифровкаФинАгент.Итог("СуммаКомиссииФА");	
			А_СуммаКомиссии = А_РасшифровкаФинАгент.Итог("СуммаКомиссииФА");	
			...
			Для каждого СтрокаТЗ Из А_РасшифровкаФинАгент Цикл    
				...
				СуммаФинАгентСтрока =  СтрокаТЗ.Сумма+ СтрокаТЗ.СуммаКомиссииФА;  ном=0;
				Для каждого СтрокаПлатежки Из СтрокаТЗ.ДокументПередачи.РасшифровкаПлатежа Цикл
					ном=ном+1;
					Если ном=1 Тогда
						
						Партнер = СтрокаПлатежки.Партнер;
					...
					СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = А_СтатьяДвиженияДенежныхСредствКазна;
					СтрокаРасшифровкаПлатежа.Партнер = СтрокаПлатежки.Партнер;
					СтрокаРасшифровкаПлатежа.СуммаВзаиморасчетов = СуммаКСписанию;
				...
				Если СуммаФинАгентСтрока>0 И РасшифровкаПлатежа.Количество()>0 Тогда
					...
					СтрокаРасшифровкаПлатежа.СуммаВзаиморасчетов = СтрокаРасшифровкаПлатежа.Сумма;
		...
		Иначе
			...
			СтрокаПлатежа.СуммаВзаиморасчетов	= СуммаДокумента;
			СтрокаПлатежа.Сумма					= СуммаДокумента;
			СтрокаПлатежа.ВалютаВзаиморасчетов	= Валюта;  
			СтрокаПлатежа.СтатьяДвиженияДенежныхСредств	= А_СтатьяДвиженияДенежныхСредствКазна;
			СтрокаПлатежа.СтатьяДоходов = ПланыВидовХарактеристик.СтатьиДоходов.НайтиПоКоду("000001007"); //Прочие доходи
			СтрокаПлатежа.Подразделение = ПодразделениеШапка ;
		КонецЕсли;                             
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьПриходныйКассовыйОрдерПослеОбмена(); 
	//***ERP
