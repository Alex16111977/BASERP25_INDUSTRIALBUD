...
Процедура ЗагрузкаДанныхИзФайла(Команда)
	...
	Если Объект.Резервы.Количество() Тогда
		...
		ТекстВопроса = НСтр("ru='Очистить таблицу резервов перед заполнением по данным из файла?';uk='Очистити таблицю резервів перед заповненням за даними з файлу?'");
...
Процедура Заполнить(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстСообщения = НСтр("ru='Перед заполнением резервов необходимо заполнить организацию документа';uk='Перед заповненням резервів необхідно заповнити організацію документа'");
	...
	Если Не ЗначениеЗаполнено(Объект.ВидРезервов) Тогда
		ТекстСообщения = НСтр("ru='Перед заполнением резервов необходимо заполнить вид резервов документа';uk='Перед заповненням резервів необхідно заповнити вид резервів документа'");
	...
	Если Объект.Резервы.Количество() Тогда
		ПараметрыОповещения = Новый Структура("ИмяСобытия", Команда.Имя);
		ОчисткаТЧОписаниеОповещения = Новый ОписаниеОповещения("ВопросОчисткиТабличнойЧастиЗавершение", ЭтотОбъект, ПараметрыОповещения);
		ТекстВопроса = НСтр("ru='Очистить таблицу резервов перед заполнением по данным регистра?';uk='Очистити таблицю резервів перед заповненням за даними регістра?'");
		ПоказатьВопрос(ОчисткаТЧОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена);
	Иначе
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<без изменений>';uk='<без змін>'"));
...
Процедура ЗагрузитьДанныеИзФайлаНачало()
	...
	ПараметрыЗагрузки.Заголовок = НСтр("ru='Загрузка резервов из файла';uk='Завантаження резервів з файлу'");
...
Процедура ЗагрузитьДанныеИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Если ЗагруженныеДанные.Количество() <> Выборка.Количество() Тогда
		Сообщение = НСтр("ru='Не для всех загружаемых строк были найдены соответствия в справочнике ""Объекты учета резервов"". Не все строки были загружены.';uk='Не для всіх завантажуваних рядків були знайдені відповідності в довіднику ""Об''єкти обліку резервів"". Не всі рядки були завантажені.'");
			...
			Если МассивСтрокСТекущимОбъектомРезервов.Количество() > 1 Тогда
				Для каждого СтрокаТекущегоОбъекта Из МассивСтрокСТекущимОбъектомРезервов Цикл
					Если СтрокаТекущегоОбъекта.ПолучитьИдентификатор() <> ИдентификаторСтроки Тогда
						ТекстОшибки = НСтр("ru='Строка с введенным объектом учета резервов уже есть в документе.';uk='Рядок з введенням об''єктом обліку резервів вже є в документі.'");
...
Процедура ПроверитьСоответствиеРеквизитовРезервам()
	...
	Если Объект.Резервы.Количество() Тогда
		...
		Если РеквизитыРезерва.Организация <> Объект.Организация
			...
			ОчисткаТЧОписаниеОповещения = Новый ОписаниеОповещения("ВопросОчисткиТабличнойЧастиЗавершение", ЭтотОбъект, ПараметрыОповещения);
			ТекстВопроса = НСтр("ru='Таблица резервов будет очищена. Продолжить?';uk='Таблиця резервів буде очищена. Продовжити?'");
			ПоказатьВопрос(ОчисткаТЧОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена);
