...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.ПередачаПоЗаказам
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Уточните склад передачи и заполните сырье и материалы с помощью команды ""%1""';uk='Уточніть склад передачі і заповніть сировину і матеріали за допомогою команди ""%1""'"),
...
|;uk='За даним документом вже розпочато відвантаження. Міняти ознаку 
|""Передача за замовленням"" після початку відвантаження не допускається.'"));
...
Процедура ВидЦеныПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦены) И Объект.Товары.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидЦеныПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru='Перезаполнить цены сырья и материалов?';uk='Перезаповнити ціни сировини і матеріалів?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
