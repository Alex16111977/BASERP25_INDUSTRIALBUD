...
Процедура АктуализироватьНалоговоеНазначение(Заполнить = Истина)
	...
	ПараметрыЗаполнения = Документы.ЗаказНаПроизводство.ПараметрыЗаполненияНалоговогоНазначения(Объект);
...
Процедура ПродукцияПроверитьСообщитьОшибкиСпецификации(СостояниеСпецификации, НомерСтроки, Отказ = Ложь)
	...
	Если СостояниеСпецификации = 4 ИЛИ СостояниеСпецификации = 2 Тогда
		ШаблонСообщения = НСтр("ru='Не заданы уточнения номенклатуры в строке ""%1"" списка ""Продукция"".';uk='Не задані уточнення номенклатури в рядку ""%1"" списку ""Продукція"".'");
