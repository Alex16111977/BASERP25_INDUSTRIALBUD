...
Процедура НадписьЗаголовокЗаказыДавальцевНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму(
		...
		СписокЗаказов,
		НСтр("ru='Заказы давальца (%КоличествоДокументов%)';uk='Замовлення давальця (%КоличествоДокументов%)'")
		),
...
Процедура ПерезаполнитьПоОтгрузке(Команда)
	...
	ПерезаполнитьПоОрдерам();
	ПоказатьОповещениеПользователя(НСтр("ru='Перезаполнение по отгрузке';uk='Перезаповнення за відвантаженням'"),,
...
Процедура ПерезаполнитьПоОрдерам()
	...
	КолонкаКоличество = ТаблицаНакладная.Колонки.КоличествоВОрдере; // КолонкаТаблицыЗначений
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ТекстСообщения = НСтр("ru='Не хватает остатков по ""%1"". Количество в строке приведено к остаточному.';uk='Недостатньо залишків з ""%1"". Кількість в рядку приведено до залишкової.'");
