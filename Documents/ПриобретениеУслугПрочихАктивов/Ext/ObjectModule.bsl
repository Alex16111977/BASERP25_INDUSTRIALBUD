...
//***ERP
Процедура ОбновитьПриобретениеУслугПрочихАктивовПослеОбмена(Док) Экспорт
	...
	// Очищаем ТЧ Товары
	Док.Расходы.Очистить();
	
	Если А_СуммаДопРасходы=0 Тогда
		Возврат;
	КонецЕсли;   
	...
	ДокументПартии = Неопределено;
	Если А_Товары.Количество()>0 Тогда
		ДокументПартии = А_Товары[0].ДокументПартии;
	ИначеЕсли А_Оборудование.Количество()>0 Тогда
		ДокументПартии = А_Оборудование[0].ДокументПартии;	
	КонецЕсли;
	
	СтрокаТабличнойЧасти = Расходы.Добавить();
	СтрокаТабличнойЧасти.Содержание = А_Содержание;
	СтрокаТабличнойЧасти.Количество = 1;
	СтрокаТабличнойЧасти.Цена  = А_СуммаДопРасходы;
	СтрокаТабличнойЧасти.Сумма = А_СуммаДопРасходы;
	СтрокаТабличнойЧасти.СтавкаНДС = НДСКлиентСерверПовтИсп.СтавкаНДСПоЗначениюПеречисления(Док.А_СтавкаНДС);
	СтрокаТабличнойЧасти.СуммаНДС  = А_СуммаНДС;  
	СтрокаТабличнойЧасти.Подразделение = ДокументПартии.Подразделение;
	
	Если ЦенаВключаетНДС Тогда
		СтрокаТабличнойЧасти.СуммаСНДС  = А_СуммаДопРасходы;
	Иначе
		СтрокаТабличнойЧасти.СуммаСНДС  = А_СуммаДопРасходы + А_СуммаНДС;
	КонецЕсли;
	
	Если А_СпособРаспределения = Перечисления.А_СпособыРаспределенияДопРасходов.ПоКоличеству Тогда
		СтрокаТабличнойЧасти.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("0Ц-000006"); //Додаткові витрати (розподіл за кількістю)
	ИначеЕсли А_СпособРаспределения = Перечисления.А_СпособыРаспределенияДопРасходов.ПоСумме Тогда	
		СтрокаТабличнойЧасти.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("0Ц-000005"); //Додаткові витрати (розподіл за сумою)
	КонецЕсли;
	СтрокаТабличнойЧасти.АналитикаРасходов = ДокументПартии; 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		//***ERP
	ОбновитьПриобретениеУслугПрочихАктивовПослеОбмена(ЭтотОбъект); 
	//***ERP
