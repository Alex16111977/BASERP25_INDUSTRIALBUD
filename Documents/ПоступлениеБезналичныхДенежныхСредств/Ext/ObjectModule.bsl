...
Процедура ОбновитьПоступлениеБезналичныхДенежныхСредствПослеОбмена() Экспорт
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств Тогда
		...
		Если А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ДругиеСчетаВБанкеВНациональнойВалюте Тогда  //313
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствСДругогоСчета;	
			БанковскийСчетОтправитель = А_Субконто1;  
			СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
			
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.РасчетыСГосударственнымиЦелевымиФондами Тогда   //378
			 ПодразделениеШапка= А_ОрганизацияБухгалтерия.А_ПодразделениеЗарплатаБух;
			 Подразделение = А_ОрганизацияБухгалтерия.А_ПодразделениеЗарплатаБух;

			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.СтатьяДоходов = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ЦелевоеФинансирование; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;    
				СтрокаРасшифровкаПлатежа.СчетУчета = Планысчетов.Хозрасчетный.РасчетыСГосударственнымиЦелевымиФондами; 
			    СтрокаРасшифровкаПлатежа.Субконто1 = Справочники.СтатьиНалоговыхДеклараций.ФССУтрТрудосп_ВремНетрудосп; 	
			КонецЦикла;     
			
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ПроцентыПолученные Тогда
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.СтатьяДоходов = А_Субконто1; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка;
			КонецЦикла;  
		ИначеЕсли А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.КраткосрочныеКредитыБанковВИностраннойВалюте //602
			...
			ИначеЕсли Лев(СокрЛП(А_СчетУчетаБухгалтерия.Код),1) ="7"//
			Тогда   
			СтатьяДоходовСтрока = Неопределено;
			Если ЗначениеЗаполнено(А_Субконто1) И ТипЗнч(А_Субконто1)=Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов") Тогда
				СтатьяДоходовСтрока = А_Субконто1;
			ИначеЕсли ЗначениеЗаполнено(А_Субконто2) И ТипЗнч(А_Субконто2)=Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов") Тогда
				СтатьяДоходовСтрока = А_Субконто2;
			КонецЕсли;
			Для каждого СтрокаРасшифровкаПлатежа Из РасшифровкаПлатежа Цикл
				СтрокаРасшифровкаПлатежа.Подразделение = ПодразделениеШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств = СтатьяДвиженияДенежныхСредствШапка; 
				СтрокаРасшифровкаПлатежа.СтатьяДоходов = СтатьяДоходовСтрока;    
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КонвертацияВалюты
		ИЛИ 
		(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств 
		И А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте ) //3331
		Тогда
		
		Если (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств 
			И А_СчетУчетаБухгалтерия = Планысчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте ) //3331
			Тогда
	...
	Если ЭтоВалютаУАХ Тогда
		// Поступление гривны = выручка от продажи валюты ИЛИ остаток от покупки валюты
		ЗапросПокупкаПродажаВалюты.УстановитьПараметр("ХозОперацияПродажа", Перечисления.ХозяйственныеОперации.ПродажаВалюты);
		ЗапросПокупкаПродажаВалюты.УстановитьПараметр("ХозОперацияПокупка", Перечисления.ХозяйственныеОперации.ПокупкаВалюты);
		...
		|	И (ПВ.А_ПлатежкаЗачисленияНаПокупкуПродажиВалюты = &ТекущийДокумент
		|			ИЛИ ПВ.А_ПлатежкаЗачисленияВозвратаНаПокупкуПродажиВалюты = &ТекущийДокумент)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И НЕ ПВ.ПометкаУдаления
		|	И ПВ.ХозяйственнаяОперация = &ХозОперацияПродажа
		|	И ПВ.СуммаГривневая = &СуммаДокумента
		|	И ПВ.А_ПлатежкаЗачисленияНаПокупкуПродажиВалюты = ЗНАЧЕНИЕ(Документ.ПоступлениеБезналичныхДенежныхСредств.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И НЕ ПВ.ПометкаУдаления
		|	И ПВ.ХозяйственнаяОперация = &ХозОперацияПродажа
		|	И ПВ.СуммаДокумента = &СуммаДокумента
		|	И ПВ.А_ПлатежкаЗачисленияНаПокупкуПродажиВалюты = ЗНАЧЕНИЕ(Документ.ПоступлениеБезналичныхДенежныхСредств.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	И ПВ.А_ОрганизацияБухгалтерия = &А_ОрганизацияБухгалтерия
		|	И НЕ ПВ.ПометкаУдаления
		|	И ПВ.ХозяйственнаяОперация = &ХозОперацияПокупка
		|	И ПВ.А_ПлатежкаЗачисленияВозвратаНаПокупкуПродажиВалюты = ЗНАЧЕНИЕ(Документ.ПоступлениеБезналичныхДенежныхСредств.ПустаяСсылка)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет";

	Иначе
		// Поступление валюты = купленная валюта
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		//		"ПоступлениеБезналичныхДС.ЗаполнениеПВ",
	...
	// Обновление банковских счетов в ПокупкаПродажаВалюты
	Если ЗначениеЗаполнено(А_ПокупкаПродажаВалюты) Тогда
		Если БанковскийСчет.ВалютаДенежныхСредств <> Константы.ВалютаРегламентированногоУчета.Получить()
			И БанковскийСчет <> А_ПокупкаПродажаВалюты.А_СчетВалютныйБанк Тогда

			ДокПВ = А_ПокупкаПродажаВалюты.ПолучитьОбъект();
			ДокПВ.ОбменДанными.Загрузка = Истина;
			ДокПВ.А_СчетВалютныйБанк = БанковскийСчет;
			ДокПВ.Записать();

		ИначеЕсли БанковскийСчет <> А_ПокупкаПродажаВалюты.А_СчетГривневыйБанк Тогда

			ДокПВ = А_ПокупкаПродажаВалюты.ПолучитьОбъект();
			ДокПВ.ОбменДанными.Загрузка = Истина;
			ДокПВ.А_СчетГривневыйБанк = БанковскийСчет;
			ДокПВ.Записать();
	...
	Если ЗначениеЗаполнено(А_ПокупкаПродажаВалюты) Тогда
		Если А_ПокупкаПродажаВалюты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПокупкаВалюты Тогда 
			...
			СуммаКомиссии =  А_ПокупкаПродажаВалюты.СуммаКомиссионные;  
            СуммаПенсионный =  А_ПокупкаПродажаВалюты.СуммаПенсионный;  
            СтатьяРасходовКомиссия =  А_ПокупкаПродажаВалюты.СтатьяРасходов; 	
			Иначе
			ВалютаКонвертации = Константы.ВалютаРегламентированногоУчета.Получить();
			СуммаКонвертации= СуммаДокумента;  
			КурсКонвертации  = 1; 
			СуммаКомиссии =  0;  
            СуммаПенсионный =  0;  
            СтатьяРасходовКомиссия =  А_ПокупкаПродажаВалюты.СтатьяРасходов; 
			БанковскийСчетОтправитель =  ?(ЗначениеЗаполнено(А_ПокупкаПродажаВалюты.А_СчетВалютныйБанк),А_ПокупкаПродажаВалюты.А_СчетВалютныйБанк,А_ПокупкаПродажаВалюты.СчетВалютный);
			КонецЕсли;
...
Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств Тогда
	...
	Для каждого СтрокатЗ Из РасшифровкаПлатежа Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокатЗ.СтатьяДоходов)
			И ЗначениеЗаполнено( СтатьяДвиженияДенежныхСредствШапка.А_СтатьяДоходов) Тогда
		
		СтрокатЗ.СтатьяДоходов = СтатьяДвиженияДенежныхСредствШапка.А_СтатьяДоходов;	
...
Процедура ОбновитьПоступлениеБезналичныхДенежныхСредств_КазнаПослеОбмена() Экспорт
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьПоступлениеБезналичныхДенежныхСредствПослеОбмена(); 
	ОбновитьПоступлениеБезналичныхДенежныхСредств_КазнаПослеОбмена();
	//***ERP
...
Процедура СформироватьСписокЗависимыхЗаказов()
	...
	|ГДЕ
	|	ЗаказДавальца.Ссылка В(ВЫБРАТЬ Ссылка ИЗ ТаблицаСсылок)
	|	
	|СГРУППИРОВАТЬ ПО
	|	ЗаказДавальца.Ссылка
	//-- НЕ УТКА
