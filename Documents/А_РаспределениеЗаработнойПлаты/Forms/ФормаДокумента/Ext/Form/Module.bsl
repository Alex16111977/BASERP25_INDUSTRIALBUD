...
Процедура РаспределитьНаСервере()
	...
	Запрос.Текст=    "ВЫБРАТЬ
	                 |	А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник КАК Сотрудник,
	                 |	ВЫБОР
	                 |		КОГДА &ОпределятьНаправлениеПоТабелю
	                 |				И НЕ А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.РаспределятьПоКарточке
	                 |			ТОГДА ЕСТЬNULL(ДанныеТабелированияОбороты.Направление, А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеНаправление)
	                 |		ИНАЧЕ А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеНаправление
	                 |	КОНЕЦ КАК Направление,
	                 |	ВЫБОР
	                 |		КОГДА А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.РаспределятьПоКарточке
	                 |			ТОГДА А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеПодразделение
	                 |		ИНАЧЕ ЕСТЬNULL(ДанныеТабелированияОбороты.Подразделение, ВЫБОР
	                 |					КОГДА А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеПодразделение = ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)
	                 |						ТОГДА А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.Родитель.ОсновноеПодразделение
	                 |					ИНАЧЕ А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеПодразделение
	                 |				КОНЕЦ)
	                 |	КОНЕЦ КАК Подразделение,
	                 |	ЕСТЬNULL(СУММА(ДанныеТабелированияОбороты.ЧасыФактОборот), 0) КАК ОтработаноЧасов
	                 |ИЗ
	                 |	Документ.А_РаспределениеЗаработнойПлаты.Сотрудники КАК А_РаспределениеЗаработнойПлатыСотрудники
	                 ...
	                 |		ПО А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник = ДанныеТабелированияОбороты.Сотрудник
	                 |			И (ДанныеТабелированияОбороты.Состояние В (&СпСостояний))
	                 |			И (НЕ ДанныеТабелированияОбороты.Регистратор.НеРаспределять)
	                 |ГДЕ
	                 |	А_РаспределениеЗаработнойПлатыСотрудники.Ссылка = &Ссылка
	                 |
	                 |СГРУППИРОВАТЬ ПО
	                 |	ДанныеТабелированияОбороты.Сотрудник,
	                 |	ДанныеТабелированияОбороты.Подразделение,
	                 |	А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник,
	                 |	ЕСТЬNULL(ДанныеТабелированияОбороты.Подразделение, А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеПодразделение),
	                 |	ВЫБОР
	                 |		КОГДА А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.РаспределятьПоКарточке
	                 |			ТОГДА А_РаспределениеЗаработнойПлатыСотрудники.Сотрудник.ОсновноеПодразделение
	                 |		ИНАЧЕ ЕСТЬNULL(ДанныеТабелированияОбороты.Подразделение, ВЫБОР
