...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Производственная операция';uk='Виробнича операція'");
	...
	КомандаПечати.Представление = НСтр("ru='Этикетка операции';uk='Етикетка операції'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	...
	Для каждого Операция Из МассивСсылок Цикл
		...
		Исключение
			ТекстИсключения = СтрШаблон(
				НСтр("ru='Не удалось изменить документ %1. Возможно, документ в настоящий момент редактируется.';uk='Не вдалося змінити документ %1. Можливо, документ зараз редагується.'"),
				Операция);
...
Процедура Разделить(Ссылка, ЗначенияЗаполнения, БлокироватьДанные = Истина) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Производственные операции';uk='Виробничі операції'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВыполнитьЧастично(Ссылка, Выполнено, СтатусНовой = Неопределено) Экспорт
	...
	Попытка
		...
		ОбъектСтарый.УстановитьСтатус("Выполнена", Новый Структура);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Производственные операции';uk='Виробничі операції'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Документы.ПроизводственнаяОперация2_2,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	Иначе
		...
		ШаблонИсключения = НСтр("ru='В документе %1 не реализована адаптация текста запроса формирования движений по регистру %2.';uk='У документі %1 не реалізована адаптація тексту запиту формування рухів за регістром %2.'");
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ДокументСсылка,
		"Дата, Подразделение, Организация, Этап.ПартияПроизводства, Статус, ОперацияКакРаспоряжениеВыработки");
	
	Запрос.УстановитьПараметр("Период", Реквизиты.Дата);
	Запрос.УстановитьПараметр("Подразделение", Реквизиты.Подразделение);
	Запрос.УстановитьПараметр("Организация", Реквизиты.Организация);
	Запрос.УстановитьПараметр("ПартияПроизводства", Реквизиты.ЭтапПартияПроизводства);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПроизводственнаяОперация") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Производственная операция';uk='Виробнича операція'"),  
			СформироватьПечатнуюФормуОперация(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,,Истина); // ЭтоМногоязычнаяПечатнаяФорма
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПроизводственнаяОперацияЭтикетка") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Этикетка операции';uk='Етикетка операції'"),
			СформироватьПечатнуюФормуЭтикеткаОперации(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,,Истина);  // ЭтоМногоязычнаяПечатнаяФорма
...
Функция СформироватьПечатнуюФормуОперация(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПроизводственнаяОперация2_2.ПФ_MXL_ПроизводственнаяОперация",КодЯзыкаПечать);
	...
	Пока ВыборкаШапка.Следующий() Цикл
		...
		ВыборкаШапка, НСтр("ru='Производственная операция';uk='Виробнича операція'",КодЯзыкаПечать),КодЯзыкаПечать);
		...
		// Материалы
		ПечатнаяФормаОперацияВывестиМатериалы(Макет, ТабличныйДокумент, ВыборкаШапка, ДанныеДляПечати.ВыборкаМатериалы, КодЯзыкаПечать);
		
		// Изделия   
		ПечатнаяФормаОперацияВывестиИзделия(Макет, ТабличныйДокумент, ВыборкаШапка, ДанныеДляПечати.ВыборкаИзделия, КодЯзыкаПечать);
...
Процедура ПечатнаяФормаОперацияВывестиМатериалы(Макет, ТабличныйДокумент, ВыборкаШапка, ВыборкаМатериалы, КодЯзыка = "ru")
	...
	Область.Параметры.ИмяТаблицы = НСтр("ru='Материалы';uk='Матеріали'",КодЯзыка);
	ТабличныйДокумент.Вывести(Область);
	...
	ОбластьШапка.Параметры.ИмяКолонкиНоменклатура = НСтр("ru='Материал';uk='Матеріал'",КодЯзыка);
...
Процедура ПечатнаяФормаОперацияВывестиИзделия(Макет, ТабличныйДокумент, ВыборкаШапка, ВыборкаИзделия, КодЯзыка = "ru")
	...
	Область = Макет.ПолучитьОбласть("ЗаголовокТовары"); 
	Область.Параметры.ИмяТаблицы = НСтр("ru='Выходные изделия';uk='Вихідні вироби'",КодЯзыка);
	ТабличныйДокумент.Вывести(Область);
	...
	ОбластьШапка.Параметры.ИмяКолонкиНоменклатура = НСтр("ru='Изделие';uk='Виріб'",КодЯзыка);
...
Функция СформироватьПечатнуюФормуЭтикеткаОперации(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПроизводственнаяОперация2_2.ПФ_MXL_ПроизводственнаяОперацияЭтикетка", КодЯзыкаПечать);
	ПервыйДокумент = Истина;
	...
	Пока Выборка.Следующий() Цикл
		...
		Область.Параметры.ТекстЗаголовка = СтрШаблон(НСтр("ru='%1 от %2';uk=' %1 від %2'",КодЯзыкаПечать),
			Выборка.Номер,
