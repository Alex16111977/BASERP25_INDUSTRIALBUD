...
Процедура КоличествоФактПриИзменении(Элемент)
	...
	Если Объект.НаОснованииМаршрутнойКарты Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Заполнить материалы, изделия и трудозатраты по маршрутной карте?';uk='Заповнити матеріали, вироби і трудовитрати за маршрутною картою?'"),
...
Процедура ЗаполнитьПоМаршрутнойКарте(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Заполнение выполнено';uk='Заповнення виконано'"),
		ПолучитьНавигационнуюСсылку(МаршрутнаяКарта),
		НСтр("ru='Документ заполнен по маршрутной карте';uk='Документ заповнений за маршрутною картою'"),
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ИмяТЧ)
	...
	Если СтруктураДействий.ТекущаяСтрока <> Неопределено Тогда
		ТаблицаФрормы = Элементы[ИмяТЧ];//ТаблицаФормы -
		ТаблицаФрормы.ТекущаяСтрока = СтруктураДействий.ТекущаяСтрока;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормы()
	...
	Если Не Объект.Этап.Пустая()
		...
		Элементы.ТекстИнформация.Заголовок = НСтр("ru='Этап завершен, операция не может быть изменена.';uk='Етап завершено, операція не може бути змінена.'");
	...
	Если Не ИзменениеЗапрещено
		...
		ИзменениеЗапрещено = Истина;
		Элементы.ТекстИнформация.Заголовок = НСтр("ru='Сменное задание закрыто, операция не может быть изменена.';uk='Змінне завдання закрито, операція не може бути змінена.'");
		
	КонецЕсли;
	
	ЭтаФорма.ТолькоПросмотр                     = ИзменениеЗапрещено;
	Элементы.ВыполнениеНачато.ТолькоПросмотр    = ИзменениеЗапрещено;
	Элементы.ВыполнениеЗавершено.ТолькоПросмотр = ИзменениеЗапрещено;
	
	Элементы.ГруппаИнформация.Видимость = ИзменениеЗапрещено;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Форма.ЭтоМобильныйКлиент Тогда
		...
		Элементы.ВыполнениеНачато.Видимость    = Ложь;
		Элементы.ВыполнениеЗавершено.Видимость = Ложь;
		...
		Элементы.ВремяВыполнения.Ширина = 0;
		Элементы.ВремяВыполнения.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
		Элементы.ВремяВыполненияЕдИзм.Ширина = 0;
		Элементы.ВремяВыполненияЕдИзм.АвтоМаксимальнаяШирина = Ложь;
		Элементы.ВремяВыполненияЕдИзм.МаксимальнаяШирина = 7;
		Элементы.ВремяВыполненияЕдИзм.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
		
		Элементы.НачалоВыполнения.ТолькоПросмотр    = Истина;
		Элементы.ОкончаниеВыполнения.ТолькоПросмотр = Истина;
		
		Элементы.ВидРабочегоЦентра.Заголовок = НСтр("ru='Вид РЦ';uk='Вид РЦ'");
		Элементы.Наименование.Заголовок      = НСтр("ru='Изготовить';uk='Виготовити'");
		Элементы.ВремяВыполнения.Заголовок   = НСтр("ru='Время';uk='Час'");
		
		Элементы.СтраницаМатериалыИРаботы.Заголовок = НСтр("ru='Расход';uk='Видаток'");
		Элементы.СтраницаВыходныеИзделия.Заголовок  = НСтр("ru='Выпуск';uk='Випуск'");
		
		Элементы.МатериалыИРаботы_СкопироватьСтроки.Видимость                = Ложь;
		Элементы.МатериалыИРаботы_ВставитьСтроки.Видимость                   = Ложь;
		Элементы.МатериалыИРаботыГруппаПоиск.Видимость                       = Ложь;
		Элементы.МатериалыИРаботыГруппаЗаполнить.Видимость                   = Ложь;
		Элементы.КонтекстноеМенюМатериалыИРаботы_СкопироватьСтроки.Видимость = Ложь;
		Элементы.КонтекстноеМенюМатериалыИРаботы_ВставитьСтроки.Видимость    = Ложь;
		Элементы.ВыходныеИзделия_СкопироватьСтроки.Видимость                 = Ложь;
		Элементы.ВыходныеИзделия_ВставитьСтроки.Видимость                    = Ложь;
		Элементы.ВыходныеИзделия_УказатьСерии.Видимость                      = Ложь;
		Элементы.ВыходныеИзделия_ПолучитьВес.Видимость                       = Ложь;
		Элементы.ВыходныеИзделия_ПодобратьТовары.Видимость                   = Ложь;
		Элементы.КонтекстноеМенюВыходныеИзделия_СкопироватьСтроки.Видимость  = Ложь;
		Элементы.КонтекстноеМенюВыходныеИзделия_ВставитьСтроки.Видимость     = Ложь;
		Элементы.Трудозатраты_СкопироватьСтроки.Видимость                    = Ложь;
		Элементы.Трудозатраты_ВставитьСтроки.Видимость                       = Ложь;
		Элементы.КонтекстноеМенюТрудозатраты_СкопироватьСтроки.Видимость     = Ложь;
		Элементы.КонтекстноеМенюТрудозатраты_ВставитьСтроки.Видимость        = Ложь;
		
		Элементы.Переместить(Элементы.Этап, Элементы.СтраницаОсновное, Элементы.Подразделение);
		Элементы.Переместить(Элементы.СменноеЗадание ,Элементы.СтраницаОсновное, Элементы.Подразделение);
		
		Элементы.Этап.ПоложениеЗаголовка           = ПоложениеЗаголовкаЭлементаФормы.Нет;
		Элементы.СменноеЗадание.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
...
Процедура ВывестиНормативыДополнительныхРеквизитов()
	...
	Для каждого Норматив Из Нормативы Цикл
		...
		ЭлементФормыГруппа.Заголовок = СтрШаблон(НСтр("ru='%1 (группа)';uk='%1 (група)'"), ОписаниеРеквизита.Наименование);
		...
		Если Норматив.ЗначениеМакс <> 0 Тогда
			ОписаниеНорматива = СтрШаблон(НСтр("ru='норматив: %1 - %2';uk='норматив: %1 - %2'"), 
				Формат(Норматив.ЗначениеМин, Формат),
				Формат(Норматив.ЗначениеМакс, Формат));
		Иначе
			ОписаниеНорматива = СтрШаблон(НСтр("ru='норматив: %1';uk='норматив: %1'"),
				Формат(Норматив.ЗначениеМин, Формат));
		КонецЕсли;
