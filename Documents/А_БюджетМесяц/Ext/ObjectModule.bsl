...
Процедура ОбновитьА_БюджетМесяцПослеОбмена() Экспорт
	...
	КонецЕсли;  
		  	...
		  	ПодразделениеШапка = А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны(Направление,А_ПодразделениеКазна); 
        Подразделение = ПодразделениеШапка; 
		Для каждого СтрокаТЗ Из ПоказателиБДДС Цикл
		
		ПодразделениеСтрока = А_Привилегированный.А_ПолучитьПодразделениеПоНаправлениюИПодразделениюКазны(Направление,СтрокаТЗ.А_ПодразделениеСтрокаКазна); 
        СтрокаТЗ.Подразделение = ПодразделениеСтрока; 
...
//***ERP
  ...
  Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	  ...
	  //***ERP
	  ОбновитьА_БюджетМесяцПослеОбмена();
	  //***ERP
