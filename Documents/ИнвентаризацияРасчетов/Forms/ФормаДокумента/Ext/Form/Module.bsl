...
Процедура ИнвентаризационнаяКомиссияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		...
		Если СтрокиФЛ.Количество() > 1 Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("ru='Физическое лицо %физлицо% уже включено в состав комиссии!';uk='Фізична особа %физлицо% вже включена до складу комісії!'");
...
Процедура ИнвентаризационнаяКомиссияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ВыбранныйЭлемент Из ВыбранноеЗначение Цикл
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Физическое лицо %1 уже добавлено в список комиссии';uk='Фізичну особу %1 вже додано до списку комісії'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Не редактируется';uk='Не редагується'"));
...
Процедура ПроверитьВозможностьЗаполнения(ПараметрыЗаполнения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Организация) Тогда
		ТекстПредупреждения = НСтр("ru='Организация, по которой необходимо формировать документ, не заполнена!';uk='Організація, по якій необхідно формувати документ, не заповнена!'");			
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Дата) Тогда
		ТекстПредупреждения = НСтр("ru='Не указана дата, на которую необходимо формировать документ!';uk='Не вказана дата, на яку необхідно формувати документ!'");
	КонецЕсли;
...
Процедура ПроверитьЗаполненностьДанных(ПараметрыЗаполнения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Объект.Задолженность.Количество() > 0 И ПараметрыЗаполнения.ОчищатьЗадолженность Тогда
		ТекстПредупрежденияЗаполненныхДанных = НСтр("ru='Перед заполнением все строки задолженности контрагентов будут удалены.';uk='Перед заповненням всі рядки заборгованості контрагентів будуть вилучені.'")
			+ Символы.ПС + НСтр("ru='Продолжить?';uk='Продовжити?'");
	КонецЕсли;
...
Функция ЗаполнитьДолгиНаСервере()
	...
	ПараметрыФоновогоЗадания.НаименованиеФоновогоЗадания = НСтр("ru='Заполнить данными расчетов с дебиторами / кредиторами';uk='Заповнити даними розрахунків з дебіторами / кредиторами'");
...
Процедура ЗаполнитьДолгиЗавершение(РезультатВыполнения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		...
		Иначе
			...
			ТекстОповещения = НСтр("ru='Заполнение задолженности не выполнено';uk='Заповнення заборгованості не виконано'");
			ТекстПояснения = НСтр("ru='Заполнение задолженности завершено с ошибкой, по причине: %1';uk='Заповнення заборгованості завершено з помилкою, по причині: %1'");
...
Функция ПредупреждениеНеАктуальныхДанных(Организация, Дата)
	...
	Если ВзаиморасчетыНеАктуальны(Организация, Дата) Тогда
		Предупреждение = НСтр("ru='Распределение взаиморасчетов не выполнено';uk='Розподіл взаєморозрахунків не виконано'");
	...
	Если ОтражениеВРеглУчетеНеАктуально(Организация, Дата) Тогда
		Если ЗначениеЗаполнено(ТекстПредупрежденияНеАктуальныхДанных) Тогда
			Предупреждение = НСтр("ru='не все документы отражены в регл. учете';uk='не всі документи відображені в регл. обліку'");
			ТекстПредупрежденияНеАктуальныхДанных = ТекстПредупрежденияНеАктуальныхДанных + ", " + Предупреждение;
		Иначе
			Предупреждение = НСтр("ru='Не все документы отражены в регл. учете';uk='Не всі документи відображені в регл. обліку'");
			ТекстПредупрежденияНеАктуальныхДанных = ТекстПредупрежденияНеАктуальныхДанных + Предупреждение;
	...
	Если ЗначениеЗаполнено(ТекстПредупрежденияНеАктуальныхДанных) Тогда
		Вопрос = НСтр("ru='Продолжить заполнение?';uk='Продовжити заповнення?'");
...
Процедура ПересчитатьСуммыВСтроке(ИмяЭлемента)
	...
	Если СтрокаТаблицы.ИстекСрокДавности > СуммаВсего Тогда
		...
		ТекстСообщения = НСтр("ru='Сумма с истекшим сроком давности не может быть больше общей суммы задолженности';uk='Сума зі строком давності, що минув, не може бути більше загальної суми заборгованості'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , ИмяПоля);
...
Процедура ОборотноСальдоваяВедомость(Счет, Субконто = Неопределено)
	...
	Если Не ЗначениеЗаполнено(Счет) Тогда
		ТекстПредупреждения = НСтр("ru='Для данной строки не указан счет, оборотно-сальдовая ведомость сформирована не будет!';uk='Для цього рядка не вказано рахунок, оборотно-сальдова відомість сформована не буде!'");
		ПоказатьПредупреждение( , ТекстПредупреждения, , Заголовок);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьПериодИнвентаризации(ИзмененныйРеквизит)
	...
	Если ЗначениеЗаполнено(Объект.ДатаНачалаИнвентаризации) И ЗначениеЗаполнено(Объект.ДатаОкончанияИнвентаризации)
		...
		ТекстСообщения = НСтр("ru='Дата начала инвентаризации не может быть больше даты окончания инвентаризации!';uk='Дата початку інвентаризації не може бути більше дати закінчення інвентаризації!'");
