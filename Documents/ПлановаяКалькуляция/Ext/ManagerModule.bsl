...
Процедура ЗаполнитьПредставленияОбъектовКалькуляции(ДействиеКалькуляции, ОбъектКалькуляции, НесколькоОбъектов = Неопределено, ПредставлениеОбщее = "", ЗаголовокПереключателя = "") Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		ПараметрыПредметаИсчисления = НСтр("ru='наименование';uk='наймену-
|вання'") + "," + НСтр("ru='наименования';uk='наймену-
|вання'") + "," + НСтр("ru='наименований';uk='найменувань'");
...
Процедура РассчитатьСтроку(Основание, МассивВлияющихСтатей, ТаблицаСравнения, ДополнительныйИтогПоСтатьямКалькуляции, СтруктураОтбора)
	...
	ИмяПоляИдентификатор = Метаданные.Справочники.СтатьиКалькуляции.Реквизиты.Идентификатор.Имя;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Формула плановой калькуляции не рассчитана';uk='Формула планової калькуляції не розрахована'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПечатнаяФормаПлановойКалькуляции(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			Статус = МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(ДанныеПечати.Статус, КодЯзыкаПечать);
			ОбластьМакета.Параметры.Статус = Статус;
			ТабличныйДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
		...
		ОбъектКалькуляции = МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(ДанныеПечати.ОбъектКалькуляции, КодЯзыкаПечать);
		ОбластьМакета.Параметры.ОбъектКалькуляции = ОбъектКалькуляции;
		ТабличныйДокумент.Вывести(ОбластьМакета);
