...
//
// Параметры:
//  Документ - ДокументСсылка.АктВыполненныхВнутреннихРабот - ссылка на документ, по которому необходимо получить данные
...
Функция ТекстЗапросаВтТовары(Запрос, ТекстыЗапроса)
	...
	|	Работы.Назначение								КАК Назначение,
    |	ВЫБОР
    |		КОГДА ЕСТЬNULL(Работы.Назначение.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
    |			ТОГДА Работы.Назначение.НалоговоеНазначение
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ТОГДА ВтТовары.НалоговоеНазначение
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО 
	|	КОНЕЦ КАК НалоговоеНазначение,
    ...
    |	ВтТовары.НалоговоеНазначение													КАК КорНалоговоеНазначение,
...
Функция СписокОпераций() Экспорт
	...
	СписокОпераций.Добавить(ВыполнениеРабот,				НСтр("ru='Прием-передача между подразделениями';uk='Приймання-передача між підрозділами'"));
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбособленныеПодразделенияВыделенныеНаБаланс") Тогда
		СписокОпераций.Добавить(ВыполнениеРаботМеждуФилиалами, НСтр("ru='Прием-передача между филиалами';uk='Приймання-передача між філіями'"));
	КонецЕсли;
