...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не Отказ И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ВидИмущества = Перечисления.ВидыИмущества.Арендованное Тогда
			ВнеоборотныеАктивыСлужебный.ПроверитьЧтоОСПолученыВАренду(ЭтотОбъект, Отказ);
		Иначе
...
Процедура ЗаполнитьНаОснованииОбъектовЭксплуатации(Основание, ЗначенияОтбора, СтандартнаяОбработка)
	...
	Если СписокОС.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru='Основные средства должны быть приняты к учету';uk='Основні засоби повинні бути прийняті до обліку'");
...
Процедура ПроверитьТабличнуюЧасть(МассивНепроверяемыхРеквизитов, Отказ)
	...
	ШаблонСообщенияСтоимостьРегл = НСтр("ru='Для основного средства ""%1"" необходимо указать стоимость в регламентированном учете';uk='Для основного засобу ""%1"" необхідно вказати вартість в регламентованому обліку'");
