...
Функция ПолучитьБлокируемыеРеквизитыОбъекта() Экспорт
    ...
    Результат.Добавить("НалоговоеНазначение");
...
//
// Параметры:
//   КомандыОтчетов - см. ВариантыОтчетовПереопределяемый.ПередДобавлениемКомандОтчетов
	...
	Для каждого Строка Из Этапы Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Этап производства.Перенос желаемой даты обеспечения';uk='Етап виробництва. Перенос бажаної дати забезпечення'",ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВременныеТаблицыТоваровЗаказа(МенеджерВременныхТаблиц, Заказ, Склад = Неопределено) Экспорт
	...
	|	Обеспечение.Характеристика КАК Характеристика,
	|	Обеспечение.Склад КАК Склад,
	|	Обеспечение.Ссылка.Подразделение КАК Подразделение,
	|	Обеспечение.Назначение КАК Назначение,
	|	СУММА(Обеспечение.Количество) КАК Заказано,
	|	МАКСИМУМ(Обеспечение.Упаковка) КАК Упаковка1,
	|	МИНИМУМ(Обеспечение.Упаковка) КАК Упаковка2,
	|	МАКСИМУМ(Обеспечение.НомерСтроки) КАК НомерСтроки
	|ПОМЕСТИТЬ ВТНоменклатураЗаказаОбособленная
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК Обеспечение
	|ГДЕ
	|	Обеспечение.Ссылка = &Заказ
	|	И Обеспечение.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно)
	|	И НЕ Обеспечение.Производится
	|	И НЕ Обеспечение.Отменено
	...
	|	Обеспечение.Характеристика,
	|	Обеспечение.Склад,
	|	Обеспечение.Ссылка.Подразделение,
	...
	|ГДЕ
	|	Обеспечение.Ссылка = &Заказ
	|	И Обеспечение.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)
	|	И НЕ Обеспечение.Производится
	|	И НЕ Обеспечение.Отменено
	|	И НЕ Обеспечение.Ссылка.ПроизводствоНаСтороне
	|	И &Склад
	...
	Если ЗначениеЗаполнено(Склад) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Склад", "Обеспечение.Склад В(&Склад)");
...
Процедура ДополнитьСтруктуруПолейТаблицыДляЗаменыНаАналоги(СтруктураПолей) Экспорт
	...
	СтруктураПолей.ПодменяемыеПоля.Вставить("Комментарий", "ПрименениеМатериала");
	
КонецПроцедуры

// Возвращает отбор (соответствие) определяющий строки, которые изменили остаток
Функция ОтборСтрокКорректировкиОстатков() Экспорт
	...
	ОбщегоНазначенияУТ.ДобавитьЭлементОтбораВКоллекцию(
		ОтборСтрок, 
		"ВариантОбеспечения", 
		"(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.СоСклада), ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить))"
	);
	...
	Если Параметры.РассчитатьГрафик Тогда
		...
		Если РезультатПланирования.ЕстьОшибки Тогда
			...
			РезультатФормирования.ОшибкаТекст = СтрШаблон(
				НСтр("ru='При планировании заказа %1 возникли ошибки. Подробнее см. в рабочем месте ""Планирование графика заказа"".';uk='При плануванні замовлення %1 виникли помилки. Детальніше див. в робочому місці ""Планування графіка замовлення"".'"),
...
Функция ПараметрыФормированияЭтапов() Экспорт
	...
	// Параметры резервирования материалов (в том числе ПФ, производимых в процессе)
	Результат.Вставить("РезервМатериаловСоСклада", Ложь);
	Результат.Вставить("РезервМатериаловИзЗаказов", Ложь);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЦепочкаЭтапов.ВГраница() = -1 Тогда
				ВызватьИсключение НСтр("ru='Ошибка чтения этапов по партии производства';uk='Помилка читання етапів по партії виробництва'");
			КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось пометить на удаление цепочку этапов: %1 по причине: %2';uk='Не вдалося помітити на вилучення ланцюжок етапів: %1 з причини: %2'"),
								ПартияПроизводства,
								ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура РедактированиеЦепочкиЭтаповПроизводства(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Для каждого Действие Из Действия Цикл
			...
			Иначе
				ВызватьИсключение НСтр("ru='Найдены ошибки заполнения этапов производства';uk='Знайдено помилки заповнення етапів виробництва'");
			КонецЕсли;
	...
	Попытка
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				ТаблицыДляДвижений = ПроведениеДокументов.ДанныеДокументаДляПроведения(ТекущийОбъект.Ссылка,
					"ДвижениеТоваров,ОбеспечениеЗаказов");
				...
				Для каждого КлючИЗначение Из Регистры Цикл
					...
					ТаблицаДвижений = ТаблицыДляДвижений["Таблица" + ИмяРегистра];
				...
				ТекущийОбъект.Движения.Записать();
...
Процедура ОтменитьРучноеРазмещениеЭтаповВГрафике(Этапы, КонтролироватьПараметрыРазмещения) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Этап производства.Отмена ручного размещения в графике';uk='Етап виробництва. Відміна ручного розміщення в графіку'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗаполнитьГрафикЭтаповПоДаннымРасписания(Расписание) Экспорт
	...
	Пока ВыборкаЭтап.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Этап производства.Заполнение графика по данным расписания';uk='Етап виробництва. Заповнення графіка за даними розкладу'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеНовогоСтатуса = Перечисления.СтатусыЭтаповПроизводства2_2.КВыполнению
		...
		ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к. этап уже был передан к выполнению';uk='У документа %Документ% статус ""%Статус%"" не встановлено, тому що етап вже був переданий до виконання'");
	...
	ИначеЕсли ВыборкаПроверки.ПроизводствоНаСтороне Тогда
		Если ВыборкаПроверки.ЗаказПереработчику.Пустая() Тогда
			Если УправлениеПроизводствомКлиентСервер.СравнениеСтатусовЭтапа(
				...
				ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к. не оформлен заказ переработчику';uk='У документа %Документ% статус ""%Статус%"" не встановлено, тому що не оформлено замовлення переробнику'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ВыборкаПроверки.Ссылка);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Статус%", ВыборкаПроверки.ПредставлениеНовогоСтатуса);
		...
		Иначе
			Если УправлениеПроизводствомКлиентСервер.СравнениеСтатусовЭтапа(
				...
				ТекстОшибки = НСтр("ru='У документа %Документ% статус ""%Статус%"" не установлен, т.к. на основании него оформлен заказ переработчику';uk='У документа %Документ% статус ""%Статус%"" не встановлено, тому що на підставі нього оформлено замовлення переробнику'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ВыборкаПроверки.Ссылка);
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Статус%", ВыборкаПроверки.ПредставлениеНовогоСтатуса);
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				Если ДокументОбъект = Неопределено ИЛИ ДокументОбъект.Статус <> СтатусСформирован Тогда
					ВызватьИсключение НСтр("ru='Обрабатываемый объект не соответствует условиям отбора.';uk='Оброблювальний об''єкт не відповідає умовам відбору.'");
				КонецЕсли;
			...
			Исключение
				...
				ТекстСообщения = НСтр("ru='Не удалось передать к выполнению этап: %Ссылка% по причине: %Причина%';uk='Не вдалося передати до виконання етапу: %Ссылка% з причини: %Причина%'");
...
//	Отказ - Булево - состояние выполнения действия: Ложь если выполнено успешно, Истина - в противном случае.
// 
// Возвращаемое значение:
//  Структура - где Ключ - выполненное действие, а Значение - массив ссылок на документы.
//
Функция ИзменитьРазмерПартии(ДанныеПартии, РазмерПартии, ПослеЭтапа, ПересчитатьПоКоэффициенту, Отказ) Экспорт
	...
	Попытка
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Документ.ЭтапПроизводства2_2");
		ЭлементБлокировки.УстановитьЗначение("ПартияПроизводства", ДанныеПартии.ПартияПроизводства);
		Блокировка.Заблокировать();
		
		Ссылки = ЭтапыПартииДляИзменения(ДанныеПартии.ПартияПроизводства, ПослеЭтапа, Отказ);
		Если Отказ Тогда
			ОтменитьТранзакцию();
			Возврат Результат;
		КонецЕсли;
		...
		Изменения = ИзменитьРазмерПартииВЭтапах(
			ДанныеПартии, РазмерПартии, Ссылки, ДанныеЭтапов, ПересчитатьПоКоэффициенту, Отказ);
		...
		Запрос = Новый Запрос(
			...
			|	ОбеспечениеЗаказовОстатки.Назначение     КАК ИсходноеНазначение,
			|	ОбеспечениеЗаказовОстатки.НаличиеПодЗаказОстаток - ОбеспечениеЗаказовОстатки.ПотребностьОстаток КАК Количество,
			|	ОбеспечениеЗаказовОстатки.НаличиеПодЗаказОстаток - ОбеспечениеЗаказовОстатки.ПотребностьОстаток КАК КоличествоУпаковок
			|ИЗ
			...
			|					Документ.ЭтапПроизводства2_2 КАК Т
			|				ГДЕ
			|					Т.Ссылка В (&Ссылки))) КАК ОбеспечениеЗаказовОстатки
			|ГДЕ
			|	ОбеспечениеЗаказовОстатки.НаличиеПодЗаказОстаток > ОбеспечениеЗаказовОстатки.ПотребностьОстаток");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Этап производства.Сокращение партии';uk='Етап виробництва. Скорочення партії'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//  Структура - где Ключ - выполненное действие, а Значение - массив ссылок на документы.
//
Функция ОтменитьПартию(ДанныеПартии, ПослеЭтапа, ВыходныеИзделия, Серии, ПараметрыВыпуска, Отказ) Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			...
			|	ОбеспечениеЗаказовОстатки.НаличиеПодЗаказОстаток КАК КоличествоУпаковок
			|ИЗ
			|	РегистрНакопления.ОбеспечениеЗаказов.Остатки(
			|			,
			|			Назначение В
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Этап производства.Отмена партии';uk='Етап виробництва. Відміна партії'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//  Структура - где Ключ - выполненное действие, а Значение - массив ссылок на документы.
//
Функция РазделитьПартию(ДанныеПартии, НовыеПартии, Отказ) Экспорт
	...
	Попытка
		...
		Ссылки = ЭтапыПартииДляИзменения(ДанныеПартии.ПартияПроизводства, Неопределено, Отказ);
		Если Отказ Тогда
			ОтменитьТранзакцию();
			Возврат Результат;
		КонецЕсли;
		...
		Если ДанныеПартии.Спецификация.Пустая() Тогда
			...
			Изменения = ИзменитьРазмерПартииВЭтапах(
				ДанныеПартии, НовыеПартии[0], Ссылки, ДанныеПересчета, Истина, Отказ);
		...
		Иначе
			...
			// Изменение текущей партии (партия № 1)
			Изменения = ИзменитьРазмерПартииВЭтапах(
				ДанныеПартии, НовыеПартии[0], Ссылки, ДанныеДополнения, Ложь, Отказ);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Этап производства.Деление партии';uk='Етап виробництва. Поділ партії'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//  Структура - где Ключ - выполненное действие, а Значение - массив ссылок на документы.
//
Функция РазделитьПартиюПослеЭтапа(ДанныеПартии, Этап, ВыходныеИзделия, Серии, ПараметрыВыпуска, Отказ) Экспорт
	...
	Попытка
		...
		Ссылки = ЭтапыПартииДляИзменения(ДанныеПартии.ПартияПроизводства, Этап, Отказ);
		Если Отказ Тогда
			ОтменитьТранзакцию();
			Возврат Результат;
		КонецЕсли;
		...
		Для каждого Партия Из ВыходныеИзделия Цикл
			...
			Если Выборка.Следующий() Тогда
				...
				Если НайденныеСтроки.ВГраница() = -1 Тогда
					...
					НоваяСтрока.ДатаОтгрузки = ДокументОбъект.ЖелаемаяДатаОбеспечения;
...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр("ru='Этап производства';uk='Етап виробництва'",КодОсновногоЯзыка);
...
#Область ЭкономияМатериалов
	...
	ПараметрыНастройки.АналитикаАктивовПассивов	= "Объект.ЭкономияМатериалов.АналитикаАктивовПассивов";
...
Функция МожноРазделитьДокумент(Ссылка, ТекстСообщения) Экспорт
	...
	Если (Реквизиты.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка
		...
		Результат = Ложь;
		ТекстСообщения = НСтр("ru='Не может быть несколько последних этапов.';uk='Не може бути кілька останніх етапів.'");
		
	ИначеЕсли Реквизиты.ПроизводствоНаСтороне
		И НЕ Реквизиты.ЗаказПереработчику.Пустая() Тогда
		
		Результат = Ложь;
		ТекстСообщения = НСтр("ru='Для этапа создан заказ переработчику, действие не может быть выполнено.';uk='Для етапу створено замовлення переробнику, дія не може бути виконана.'");
...
Процедура РазделитьДокумент(Ссылка, КоличествоУпаковок, Упаковка, Отказ) Экспорт
		...
		Иначе
			...
			Если Коэффициент2 = 1 Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru='Количество в новом документе должно отличаться от количества в текущем.';uk='Кількість в новому документі має відрізнятися від кількості в поточному.'"),,,, Отказ);
				ОтменитьТранзакцию();
				Возврат;
			ИначеЕсли Коэффициент2 > 1 Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru='Количество в новом документе не может быть больше количества в текущем.';uk='Кількість в новому документі не може бути більше кількості в поточному.'"),,,, Отказ);
				ОтменитьТранзакцию();
				Возврат;
			КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Этап производства.Деление документа';uk='Етап виробництва. Поділ документа'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Ссылка.Метаданные(),
			Ссылка,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
#Область ПроизводствоНаСтороне
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Проведение документа со статусом ""Начат"" или ""Завершен"" доступно только при наличии заказа переработчику.';uk='Проведення документа зі статусом ""Розпочато"" або ""Завершено"" доступно тільки при наявності замовлення переробнику.'");
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Проведение документа со статусом ""Формируется"" или ""Сформирован"" доступно только при отсутствии заказа переработчику.';uk='Проведення документа зі статусом ""Формується"" або ""Сформовано"" доступно тільки при відсутності замовлення переробнику.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
#КонецОбласти
...
#Область ДатаПроизводства
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата производства не может быть меньше даты начала этапа.';uk='Дата виробництва не може бути менше дати початку етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаПроизводства", "ДатаПроизводстваБольшеЧемФактическоеОкончаниеЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата производства не может быть больше даты выполнения этапа.';uk='Дата виробництва не може бути більше дати виконання етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
#КонецОбласти
...
#Область ДатаРасхода
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата расхода не может быть меньше даты начала этапа.';uk='Дата витрачання не може бути менше дати початку етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаРасхода", "ДатаРасходаБольшеЧемФактическоеОкончаниеЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата расхода не может быть больше даты выполнения этапа.';uk='Дата витрачання не може бути більше дати виконання етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
#КонецОбласти
...
#Область ДатаВыполненияРабот
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата выполнения работ не может быть меньше даты начала этапа.';uk='Дата виконання робіт не може бути менше дати початку етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаВыполнения", "ДатаВыполненияРаботБольшеЧемФактическоеОкончаниеЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата выполнения работ не может быть больше даты выполнения этапа.';uk='Дата виконання робіт не може бути більше дати виконання етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
#КонецОбласти
...
#Область ФактическоеНачалоЭтапа
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Поле ""Дата начала выполнения этапа"" не заполнена.';uk='Поле ""Дата початку виконання етапу"" не заповнена.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
#КонецОбласти
...
#Область ФактическоеОкончаниеЭтапа
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Поле ""Дата завершения выполнения этапа"" не заполнена.';uk='Поле ""Дата завершення виконання етапу"" не заповнена.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата начала выполнения этапа не может быть больше даты завершения этапа.';uk='Дата початку виконання етапу не може бути більше дати завершення етапу.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
...
Процедура ПроверитьВыходныеИзделия(Объект, МенеджерВременныхТаблиц, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ)
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не заполнена колонка ""Получатель"" в строке %1 списка ""%2"".';uk='Не заповнена колонка ""Одержувач"" в рядку %1 списку ""%2"".'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
...
|;uk='У рядку %1 списку ""%2"" вибрана номенклатура, для якої ведеться облік по ВМД. 
|При включеній опції ""Заборонити оформлення операцій з імпортними товарами без номерів ВМД"" вибір такої номенклатури заборонений.'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Работы можно выпустить в подразделение или списать на расходы, передача работ на склад не предусмотрена (строка %1 списка ""%2"").';uk='Роботи можна випустити в підрозділ або списати на витрати, передача робіт на склад не передбачена (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("Назначение", "НазначениеНеЗаполнено");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не заполнена колонка ""Назначение"" в строке %1 списка ""%2"".';uk='Не заповнена колонка ""Призначення"" в рядку %1 списку ""%2"".'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("Назначение", "НазначениеСовпадаетСЭтапом");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Назначение изделия не может совпадать с назначением этапа производства (строке %1 списка ""%2"").';uk='Призначення виробу не може збігатися з призначенням етапу виробництва (рядку %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("Номенклатура", "ДоляСтоимостиНеЗаполнена");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не удалось рассчитать долю стоимости для номенклатуры в строке %1 списка ""%2"".';uk='Не вдалося розрахувати частку вартості для номенклатури в рядку %1 списку ""%2"".'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Обнаружено нулевое количество при пересчете в единицу хранения в строке %1 списка ""%2"".';uk='Виявлено нульову кількість при перерахунку в одиницю зберігання в рядку %1 списку ""%2"".'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	...
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаПроизводства", "ДатаПроизводстваМеньшеЧемФактическоеНачалоЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата производства не может быть меньше даты начала этапа (строка %1 списка ""%2"").';uk='Дата виробництва не може бути менше дати початку етапу (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаПроизводства", "ДатаПроизводстваБольшеЧемФактическоеОкончаниеЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата производства не может быть больше даты выполнения этапа (строка %1 списка ""%2"").';uk='Дата виробництва не може бути більше дати виконання етапу (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("Номенклатура", "СписаниеНаРасходТоваровПриПроизводствеНаСтороне");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не допускается списание товара (тары) на расходы при переработке на стороне (строка %1 списка ""%2"").';uk='Не допускається списання товару (тари) на витрати при переробці на стороні (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не заполнено подразделение договора назначения (строка %1 списка ""%2"").';uk='Не заповнений підрозділ договору призначення (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
...
Процедура ПроверитьОбеспечениеМатериаламиИРаботами(Объект, МенеджерВременныхТаблиц, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ)
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не допускается выбор работ при переработке на стороне (строка %1 списка ""%2"").';uk='Не допускається вибір робіт при переробці на стороні (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ВариантОбеспечения", "ВариантОбеспеченияПФ");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Для производимых полуфабрикатов необходимо установить действие ""Обеспечивать обособленно"" или ""Отгрузить обособленно"" (строка %1 списка ""%2"").';uk='Для напівфабрикатів, що виробляються, необхідно встановити дію ""Забезпечувати відокремлено"" або ""Відвантажити відокремлено"" (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ВариантОбеспечения", "НедоступнаОтгрузка");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Отгрузка материалов доступна только при статусах ""Начат"", ""Завершен"" (строка %1 списка ""%2"").';uk='Відвантаження матеріалів доступно тільки при статусах ""Розпочато"", ""Завершено"" (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ВариантОбеспечения", "ВариантОбеспеченияЭтапЗавершен");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='В завершенном этапе для всех материалов необходимо установить действие ""Отгрузить"" или ""Отгрузить обособленно"" (строка %1 списка ""%2"").';uk='У завершеному етапі для всіх матеріалів необхідно встановити дію ""Відвантажити"" або ""Відвантажити відокремлено"" (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("СтатьяКалькуляции", "СтатьяКалькуляцииНеЗаполнена");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='В статусе ""Начат"" и ""Завершен"" указание статьи калькуляции является обязательным (строка %1 списка ""%2"").';uk='У статусі ""Розпочато"" і ""Завершено"" зазначення статті калькуляції є обов''язковим (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	...
	Если ТипЗнч(Объект) <> Тип("ОбработкаОбъект.РедактированиеЭтаповПроизводства") Тогда // редактирование этапов производства на стороне
		...
		ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Отгрузка материалов недоступна т.к. заказ переработчику не проведен (строка %1 списка ""%2"").';uk='Відвантаження матеріалів недоступно томущо  замовлення переробнику не проведено (рядок %1 списку ""%2"").'");
		СписокРеквизитов.Добавить(ПараметрыПроверки);
		
		ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ВариантОбеспечения", "НедоступнаОтгрузкаНетЗаказа");
		ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Отгрузка материалов недоступна т.к. не оформлен заказ переработчику (строка %1 списка ""%2"").';uk='Відвантаження матеріалів недоступно тому не оформлено замовлення переробнику (рядок %1 списку ""%2"").'");
		СписокРеквизитов.Добавить(ПараметрыПроверки);
...
Процедура ПроверитьТрудозатраты(Объект, МенеджерВременныхТаблиц, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ)
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата выполнения работы не может быть меньше даты начала этапа (строка %1 списка ""%2"").';uk='Дата виконання роботи не може бути менше дати початку етапу (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаВыполнения", "ДатаВыполненияБольшеЧемФактическоеОкончаниеЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата выполнения работы не может быть больше даты выполнения этапа (строка %1 списка ""%2"").';uk='Дата виконання роботи не може бути більше дати виконання етапу (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
...
Процедура ПроверитьРасходМатериаловИРабот(Объект, МенеджерВременныхТаблиц, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ)
	...
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаРасхода", "ДатаРасходаМеньшеЧемФактическоеНачалоЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата расхода не может быть меньше даты начала этапа (строка %1 списка ""%2"").';uk='Дата витрачання не може бути менше дати початку етапу (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
	
	ПараметрыПроверки = ПараметрыПроверкиРеквизитов("ДатаРасхода", "ДатаРасходаБольшеЧемФактическоеОкончаниеЭтапа");
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Дата расхода не может быть больше даты выполнения этапа (строка %1 списка ""%2"").';uk='Дата витрачання не може бути більше дати виконання етапу (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
...
Процедура ПроверитьЭкономияМатериалов(Объект, МенеджерВременныхТаблиц, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ)
	...
	ПараметрыПроверки.ШаблонСообщения = НСтр("ru='Не допускается обособление тары (строка %1 списка ""%2"").';uk='Не допускається відокремлення тари (рядок %1 списку ""%2"").'");
	СписокРеквизитов.Добавить(ПараметрыПроверки);
...
Процедура ПроверитьРавенствоПоМатериалам(Объект, МенеджерВременныхТаблиц, Отказ)
	...
	Если Не Результат[3].Пустой() Тогда
		...
		ШаблонСообщенияОбеспечениеБольше = НСтр("ru='Не указан расход материала ""%1"" в количестве %2 %3';uk='Не вказано витрачання матеріалу  %1"" в кількості %2 %3'");
		ШаблонСообщенияОбеспечениеБольшеРабота = НСтр("ru='Не указан расход работ ""%1"" в количестве %2 %3';uk='Не вказано витрачання робіт  %1"" в кількості %2 %3'");
	...
	Если Не Результат[4].Пустой() Тогда
		...
		ШаблонСообщенияРасходБольше = НСтр("ru='Расход материала ""%1"" превышает запланированное к обеспечению на %2 %3';uk='Витрачання матеріалу  ""%1"" перевищує заплановане до забезпечення на %2 %3'");
		ШаблонСообщенияРасходБольшеРабота = НСтр("ru='Расход работы ""%1"" превышает запланированное к обеспечению на %2 %3';uk='Витрачання роботи  %1"" перевищує заплановане до забезпечення на %2 %3'");
	...
	Если Не Результат[5].Пустой() Тогда
		...
		ШаблонСообщенияЭкономия = НСтр("ru='Количество неизрасходованного материала ""%1"" превышает запланированное к обеспечению на %2 %3';uk='Кількість невитраченого матеріалу ""%1"" перевищує заплановану до забезпечення на %2 %3'");
...
|;uk='У періоді %1 за етапом є споживання матералов (робіт), при цьому випуск виробів не запланований. 
|У випускаючому етапі потрібно вказати вихідний виріб, на який будуть віднесені витрати, або змінити планову дату випуску.'");
...
Процедура ЗаполнитьПараметрыИнициализацииПоСсылке(Запрос)
	...
	ТекстЗапроса = 
		...
		|	Реквизиты.Организация                        КАК Организация,
        |	ВЫБОР
        |		КОГДА ЕСТЬNULL(Реквизиты.НазначениеПродукция.НалоговоеНазначение,ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.ПустаяСсылка)
        |			ТОГДА Реквизиты.НазначениеПродукция.НалоговоеНазначение
        |		ИНАЧЕ Реквизиты.НалоговоеНазначение
        ...
        |	Реквизиты.ПартияПроизводства.НалоговоеНазначение КАК НалоговоеНазначениеНЗП,
	...
	ПараметрыПартийИСебестоимости.Вставить("Период", ?(ЗначениеЗаполнено(Запрос.Параметры.ПериодПерезаписиДвижений), Запрос.Параметры.ПериодПерезаписиДвижений, Реквизиты.Период));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗаполнитьПараметрыИнициализации(Запрос, ПараметрыПартийИСебестоимости);
...
Функция ТекстЗапросаВтВидыЗапасовМатериалы()
	...
	|	СправочникВидыЗапасов.НалоговоеНазначение 					КАК НалоговоеНазначениеВидаЗапаса,
	...
	|	ПО ВидыЗапасов.ВидЗапасов = СправочникВидыЗапасов.Ссылка
	|
	|
	|ГДЕ
	|	ВидыЗапасов.Ссылка = &Ссылка
...
Функция ТекстЗапросаВтВидыЗапасовИзделия()
	...
	|	0														КАК СуммаЗабалансовая,
	|	ВидыЗапасов.ИдентификаторСтроки							КАК ИдентификаторСтроки,
	|	НЕОПРЕДЕЛЕНО 										    КАК НалоговоеНазначениеВидаЗапаса,
	|	ВЫБОР
	...
	|	И КорАналитика.МестоХранения = &Подразделение
	...
	|	И КорАналитикаБезНазначения.МестоХранения = &Подразделение
...
Функция ТекстЗапросаВтВидыЗапасовРаботыОбеспечение()
	...
	|	И ОбеспечениеРаботами.ВариантОбеспечения В (
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			ТОГДА 
	|				ВЫБОР 
	|					КОГДА &НалоговоеНазначениеНЗП <> ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_НеоблагаемаяНеХозДеятельность) 
	|						  И ТабличнаяЧасть.НалоговоеНазначениеВидаЗапаса = ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально) 
	|					ТОГДА ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Пропорционально) 
	|					ИНАЧЕ &НалоговоеНазначениеНЗП 
	|				КОНЕЦ
	...
	|					ТабличнаяЧасть.КорАналитикаУчетаНоменклатуры)
	|		И НЕ (НЕ &УчитыватьСебестоимостьТоваровПоНазначениям
    ...
    |	ВЫРАЗИТЬ(ТабличнаяЧасть.Сумма * ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуРегл, 0) КАК ЧИСЛО(31,2)) КАК СтоимостьРеглБезНДС,
    |	0 КАК НДСРегл,
    |	0 КАК НДСРеглРеквизит,
    |	ВЫБОР
	...
	|			ТОГДА ВЫБОР
    |					КОГДА ТабличнаяЧасть.СтатьяРасходов.ВидЦенности В (
    ...
    |					КОГДА ТабличнаяЧасть.СтатьяРасходов.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НМА)
...
Функция ТекстЗапросаТаблицаДвиженияНоменклатураНоменклатура(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|   ВЫБОР 
	|		КОГДА (&ПериодПерезаписиДвижений = НЕОПРЕДЕЛЕНО
	|					ИЛИ &ПериодПерезаписиДвижений <> ДатаВремя(1,1,1) И ТабличнаяЧасть.ДатаОперации >= &ПериодПерезаписиДвижений
	|				) 
	|				И ТабличнаяЧасть.ХозяйственнаяОперация В (
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции),
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииФиксированнаяСтоимость)
	...
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииФиксированнаяСтоимость)
	|				)
	|   		ТОГДА ВЫРАЗИТЬ(ТабличнаяЧасть.Сумма * ЕСТЬNULL(ВтКоэффициентыПересчетаВалют.КоэффициентПересчетаВВалютуРегл, 0) КАК ЧИСЛО(31, 2)) 
	|		ИНАЧЕ 0 
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюОтчетовКомитенту(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	ВидыЗапасов.Ссылка = &Ссылка
	|	И ВидыЗапасов.ВидЗапасов.ТипЗапасов =
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|		 И Кладовые.Подразделение = ТабличнаяЧасть.Ссылка.Подразделение
	|ГДЕ
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТабличнаяЧасть.Ссылка.Подразделение           КАК Получатель,
	|	ТабличнаяЧасть.Склад                          КАК Склад,
	|	ТабличнаяЧасть.Номенклатура                   КАК Номенклатура,
	|	ТабличнаяЧасть.Характеристика                 КАК Характеристика,
	|	ВЫБОР
...
Процедура ЗаполнитьОбеспечениеИОтразитьДвиженияПоЭтапам(Параметры, Этапы)
	...
	Если Параметры.РезервМатериаловСоСклада Тогда
		ВариантыОбеспечения.Добавить(Перечисления.ВариантыОбеспечения.СоСклада);
	КонецЕсли;
	Если Параметры.РезервМатериаловИзЗаказов Тогда
		ВариантыОбеспечения.Добавить(Перечисления.ВариантыОбеспечения.ИзЗаказов);
	КонецЕсли;
	ВариантыОбеспечения.Добавить(Перечисления.ВариантыОбеспечения.Требуется);
	...
	Для Индекс = 0 По МаксИндекс Цикл
		...
		ПартияОбработки.Добавить(ОчередьОбработки[Индекс]);
		
		Если Индекс = МаксИндекс ИЛИ ПартияОбработки.Количество() = 1000 Тогда // Обработка по 1000 документов
			
			// Заполнение обеспечения
			ЗаполнитьОбеспечениеВПакетномРежиме(
				ПартияОбработки,
				ВариантыОбеспечения,
				Параметры.ЗаменятьМатериалыНаАналоги);
			...
			УстановитьПривилегированныйРежим(Ложь);
			ЭтапыОтказ = СформироватьДвиженияВПакетномРежиме(ДанныеРегистров, ПорядокРегистров, ПартияОбработки);
			
			// Понижение статуса этапов и повторное формирование движений
			Если ЭтапыОтказ.Количество() <> 0 Тогда
				УстановитьЭтапамСтатусФормируется(ЭтапыОтказ);
				Запрос.УстановитьПараметр("Ссылки", ЭтапыОтказ);
				УстановитьПривилегированныйРежим(Истина);
				ДанныеРегистров = Запрос.ВыполнитьПакет();  
				УстановитьПривилегированныйРежим(Ложь);
				СформироватьДвиженияВПакетномРежиме(ДанныеРегистров, ПорядокРегистров, ЭтапыОтказ);
			КонецЕсли;
...
Процедура ЗаполнитьОбеспечениеВПакетномРежиме(Этапы, ВариантыОбеспечения, ЗаменятьНаАналоги)
	...
	Запрос = Новый Запрос(
		...
		|		ПО Этапы.Ссылка = Материалы.Ссылка
		...
		|	И Материалы.ВариантОбеспечения <> ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ИзЗаказов)
		...
		|	Материалы.ВариантОбеспечения           КАК ВариантОбеспечения,
		|	Материалы.ДатаОтгрузки                 КАК ДатаОтгрузки,
		|	ДАТАВРЕМЯ(1, 1, 1)                     КАК ДатаОтгрузкиРабот,
		...
		|	Материалы.Ссылка.Подразделение,
		|	Материалы.ДатаОтгрузки,
		|	Материалы.Склад,
		|	Материалы.Назначение,
		|	Материалы.ВариантОбеспечения,
		|	Материалы.Номенклатура.ТипНоменклатуры");
	Запрос.УстановитьПараметр("Этапы", Этапы);
	...
	Очередь = ПакетРезультатов[0].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам); // см. АналогиМатериалов.СлужебнаяСтруктураДеревоЗначений
	Резервы = ПакетРезультатов[1].Выгрузить();
	
	// Таблицы для получения остатков
	МатериалыСОбособлением = Новый ТаблицаЗначений; // см. АналогиМатериалов.СлужебнаяСтруктураТаблицыЗначений
	МатериалыБезОбособления = Новый ТаблицаЗначений; // см. АналогиМатериалов.СлужебнаяСтруктураТаблицыЗначений
	Для каждого Колонка Из СтрРазделить(ОбеспечениеСервер.ПутиКДаннымПоУмолчанию(), ",") Цикл
		МатериалыСОбособлением.Колонки.Добавить(СокрЛП(Колонка));
		МатериалыБезОбособления.Колонки.Добавить(СокрЛП(Колонка));
	КонецЦикла;
	
	Если ЗаменятьНаАналоги Тогда
		Колонки = СтрРазделить("ЗаказНаПроизводство,НаправлениеДеятельности,Спецификация,КлючСвязиСпецификация,Изделие,ХарактеристикаИзделия", ",");
		Для каждого Колонка Из Колонки Цикл
			Если МатериалыБезОбособления.Колонки.Найти(Колонка) = Неопределено Тогда
				МатериалыБезОбособления.Колонки.Добавить(Колонка);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Идентификатор = 0;
	Для Индекс = 0 По Очередь.Строки.Количество()-1 Цикл
		Строка = Очередь.Строки[Индекс];
		Для ИндексМ = 0 По Строка.Строки.Количество()-1 Цикл
			
			СтрокаМ = Строка.Строки[ИндексМ];
			Идентификатор = Идентификатор + 1;
			
			НоваяСтрока = ?(СтрокаМ.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно,
				МатериалыСОбособлением.Добавить(),
				МатериалыБезОбособления.Добавить());
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаМ);
			НоваяСтрока.Идентификатор = Идентификатор;
			
		КонецЦикла;
	КонецЦикла;
	МатериалыБезОбособления.Индексы.Добавить("Идентификатор");
	
	// Получение остатков
	Обеспечение = Неопределено;
	Если МатериалыСОбособлением.Количество() > 0 Тогда
		Обеспечение = ТаблицаЗаполнениеОбеспечения(МатериалыСОбособлением, ВариантыОбеспечения, Резервы, Истина);
	КонецЕсли;
	Если МатериалыБезОбособления.Количество() > 0 Тогда
		Если Обеспечение = Неопределено Тогда
			Обеспечение = ТаблицаЗаполнениеОбеспечения(МатериалыБезОбособления, ВариантыОбеспечения, Резервы, Ложь);
		Иначе
			Для каждого Строка Из ТаблицаЗаполнениеОбеспечения(МатериалыБезОбособления, ВариантыОбеспечения, Резервы, Ложь) Цикл
				НоваяСтрока = Обеспечение.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	Обеспечение.Индексы.Добавить("Идентификатор");
	
	// Получение аналогов
	Аналоги = Неопределено;
	Если ЗаменятьНаАналоги Тогда
		
		МатериалыЗаменить = Новый ТаблицаЗначений;
		МатериалыЗаменить.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10)));
		МатериалыЗаменить.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		МатериалыЗаменить.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
		МатериалыЗаменить.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.СтруктураПредприятия"));
		МатериалыЗаменить.Колонки.Добавить("НаправлениеДеятельности", Новый ОписаниеТипов("СправочникСсылка.НаправленияДеятельности"));
		МатериалыЗаменить.Колонки.Добавить("ЗаказНаПроизводство", Новый ОписаниеТипов("ДокументСсылка.ЗаказНаПроизводство2_2"));
		МатериалыЗаменить.Колонки.Добавить("Спецификация", Новый ОписаниеТипов("СправочникСсылка.РесурсныеСпецификации"));
		МатериалыЗаменить.Колонки.Добавить("КлючСвязиСпецификация", Новый ОписаниеТипов("УникальныйИдентификатор"));
		МатериалыЗаменить.Колонки.Добавить("Изделие", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		МатериалыЗаменить.Колонки.Добавить("ХарактеристикаИзделия", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
		МатериалыЗаменить.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
		
		Для каждого Строка Из Обеспечение Цикл
			Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада
				ИЛИ Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
				НоваяСтрока = Резервы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, МатериалыБезОбособления.Найти(Строка.Идентификатор, "Идентификатор"));
		...
		Если МатериалыЗаменить.Количество() > 0 Тогда
	...
	Для Индекс = 0 По Очередь.Строки.Количество()-1 Цикл
		...
		ДокументОбъект = Строка.Ссылка.ПолучитьОбъект(); // ДокументОбъект - 
		ДокументОбъект.Заблокировать();
		
		ПараметрыЗаполненияНазначений = ОбеспечениеПроизводстваКлиентСервер.
				ПараметрыДействияПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(ДокументОбъект);
		ПараметрыОбновленияДатыОтгрузки.ОтгружатьОднойДатой    = ДокументОбъект.НеОтгружатьЧастями;
		ПараметрыОбновленияДатыОтгрузки.ЖелаемаяДатаОтгрузки   = ДокументОбъект.ЖелаемаяДатаОбеспечения;
		ПараметрыОбновленияДатыОтгрузки.ДатаОтгрузкиВДокументе = ДокументОбъект.ДатаОтгрузки;
		...
		Для ИндексМ = 0 По Строка.Строки.Количество()-1 Цикл
			...
			ИсходнаяСтрока = ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Найти(СтрокаМ.КодСтроки, "КодСтроки");
			Если ИсходнаяСтрока = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаЗаменитьНаАналоги = Неопределено;
			СтруктураПоиска.Идентификатор = Идентификатор;
			
			// Зачет остатков
			СтрокиОбеспечение = Обеспечение.НайтиСтроки(СтруктураПоиска);
			ОбеспечениеСервер.СдвинутьДатыИСвернутьДублиСтрок(СтрокиОбеспечение, "СтрокиТовары", ПараметрыОбновленияДатыОтгрузки);
			...
			Если СтрокиОбеспечение.Количество() = 1 Тогда
				...
				Если ЗаменятьНаАналоги И ИсходнаяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется Тогда
					СтрокаЗаменитьНаАналоги = ИсходнаяСтрока;
				КонецЕсли;
			...
			Иначе
				...
				Для каждого ДанныеОбеспечения Из СтрокиОбеспечение Цикл
					...
					НоваяСтрока = ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Вставить(
					...
					Если ЗаменятьНаАналоги И НоваяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется Тогда
						СтрокаЗаменитьНаАналоги = НоваяСтрока;
					КонецЕсли;
			...
			// Зачет аналогов
			Если СтрокаЗаменитьНаАналоги <> Неопределено И Аналоги <> Неопределено Тогда
				...
				Если СтрокиАналоги.ВГраница() <> -1 Тогда
					...
					Для ИндексАналог = 0 По СтрокиАналоги.ВГраница() Цикл
						...
						Если Разрешение = Неопределено ИЛИ Разрешение <> СтрокаАналог.Разрешение Тогда
							СтрокаЗаменитьНаАналоги.Количество = СтрокаЗаменитьНаАналоги.Количество - СтрокаАналог.КоличествоЗамены;
						...
						НоваяСтрока.ДатаОтгрузки = ДокументОбъект.ЖелаемаяДатаОбеспечения;
						НоваяСтрока.СтатьяКалькуляции = СтрокаЗаменитьНаАналоги.СтатьяКалькуляции;
						
					КонецЦикла;
					
					Если СтрокаЗаменитьНаАналоги.Количество = 0 Тогда
						ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Удалить(СтрокаЗаменитьНаАналоги);
					Иначе
						ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаЗаменитьНаАналоги, СтруктураДействий, КэшированныеЗначения);
					КонецЕсли;
...
Функция СформироватьДвиженияВПакетномРежиме(ДанныеРегистров, ПорядокРегистров, ПартияОбработки)
	
	ЭтапыОтказ = Новый Массив;
	
	ДанныеДляДвижений = Новый СписокЗначений();
	Для Индекс = 0 По ПорядокРегистров.ВГраница() Цикл
		Выборка = ДанныеРегистров[Индекс].Выбрать();
		Если Выборка.Следующий() Тогда
			ДанныеДляДвижений.Добавить(Выборка, ПорядокРегистров[Индекс]);
		КонецЕсли;
	КонецЦикла;
	...
	Для Индекс = 0 По ПартияОбработки.ВГраница() Цикл
		...
		Если НЕ ФормироватьДвиженияЭтапаПакетно(ДокументСсылка, ДанныеДляДвижений) Тогда
			Продолжить;
		КонецЕсли;
		...
		Попытка
			...
			Для каждого Элемент Из ДанныеДляДвижений Цикл
			
				Выборка = Элемент.Значение;
				Если Выборка = Неопределено ИЛИ Выборка.Ссылка <> ДокументСсылка Тогда
					Продолжить;
				КонецЕсли;
				
				ДвиженияПоРегистру = ДокументОбъект.Движения[Элемент.Представление];
			...
			Если Отказ Тогда
				...
				Иначе
					ТекстИсключения = СтрШаблон(
						НСтр("ru='Не удалось провести этап № %1 от %2';uk='не вдалося провести етап № %1 від %2'"),
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Этап производства.Пакетное формирование движений';uk='Етап виробництва. Пакетне формування рухів'",ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
// Возвращает признак пактеного формирования движений этапа
//
// Параметры:
//  Этап - ДокументСсылка - 
//  ДанныеДляДвижений - СписокЗначений из ЭлементСпискаЗначений - совержит: 
//                                         * Значение - ВыборкаИзРезультатаЗапроса - содержит:
//                                                       ** Ссылка - ДокументСсылка - 
//
// Возвращаемое значение:
//  Булево -
...
Функция ФормироватьДвиженияЭтапаПакетно(Этап, ДанныеДляДвижений)
	...
	Для каждого Элемент Из ДанныеДляДвижений Цикл
		...
		Если Элемент.Значение <> Неопределено
			И Элемент.Значение.Ссылка = Этап Тогда
...
Процедура УстановитьЭтапамСтатусФормируется(Этапы)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Этап производства.Пакетное формирование движений';uk='Етап виробництва. Пакетне формування рухів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ТаблицаЗаполнениеОбеспечения(ТаблицаМатериалов, ВариантыОбеспечения, Резервы, ОбособленнаяСхема)
...
Процедура ЗаполнитьРеестрДокументовВПакетномРежиме(Этапы)
	...
	ДанныеРегистровРезультат = ДанныеРегистров[ИндексРезультата]; // РезультатЗапроса -  
	ВыборкаРазделитель = ДанныеРегистровРезультат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаРазделитель.Следующий() Цикл
		...
		Пока ВыборкаСпособЗаписи.Следующий() Цикл
			...
			Если ВыборкаСпособЗаписи.ЗаписьПоРазделителю Тогда
				...
				ТаблицыДляДвижений.ТаблицаРеестрДокументов = ТаблицаРеестрДокументов;
			
				РегистрыСведений.РеестрДокументов.ЗаписатьДанныеРазделителя(ТаблицыДляДвижений, РазделительЗаписи, Ложь);
			...
			Иначе
				...
				Пока Выборка.Следующий() Цикл
					...
					РегистрыСведений.РеестрДокументов.ЗаписатьДанные(ТаблицаРеестрДокументов, Ссылка, Ложь);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Задание на производство';uk='Завдання на виробництво'");
	...
	КомандаПечати.Представление = НСтр("ru='Лист сопровождения партии производства';uk='Лист супроводу партії виробництва'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаданиеНаПроизводство") Тогда
		...
		ТабличныйДокумент = СформироватьПечатнуюФормуЗаданиеНаПроизводство(
			МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода);

		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"ЗаданиеНаПроизводство",
			НСтр("ru='Задание на производство';uk='Завдання на виробництво'"),
			ТабличныйДокумент,
			,
			,
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЛистСопровожденияПартииПроизводства") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru='Лист сопровождения партии производства';uk='Лист супроводу партії виробництва'"),
			ТабличныйДокумент,
			,
			,
			,
...
Функция СформироватьПечатнуюФормуЗаданиеНаПроизводство(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЭтапПроизводства2_2.ПФ_MXL_ЗаданиеНаПроизводство", КодЯзыкаПечать);
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		ЗаданиеНаПроизводствоВывестиСмежныеЭтапы(
			ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаДокумент, ДанныеДляПечати, Истина, КодЯзыкаПечать);
		...
		ЗаданиеНаПроизводствоВывестиСмежныеЭтапы(
			ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаДокумент, ДанныеДляПечати, Ложь, КодЯзыкаПечать);
...
//  Предшественники -
// 
Процедура ЗаданиеНаПроизводствоВывестиСмежныеЭтапы(ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаДокумент, ДанныеДляПечати, Предшественники, КодЯзыкаПечать)
	...
	Если Предшественники Тогда
		ОбластьЗаголовок.Параметры.СмежныеЭтапыТекстЗаголовка = НСтр("ru='Принять работу';uk='Прийняти роботу'",КодЯзыкаПечать);
	Иначе
		ОбластьЗаголовок.Параметры.СмежныеЭтапыТекстЗаголовка = НСтр("ru='Сдать работу';uk='Здати роботу'",КодЯзыкаПечать);
	КонецЕсли;
...
//  ТабличныйДокумент - 
//  Макет - 
//  ВыборкаДокумент - ВыборкаИзРезультатаЗапроса - содердит:
//                     * Ссылка - ДокументСсылка - 
//  ДанныеДляПечати - 
//
...
Функция ЗаданиеНаПроизводствоПечататьСмежныеЭтапы(ПараметрыПечати)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Отсутствует параметр печати ""ПечататьСмежныеЭтапы""';uk='Відсутній параметр друку ""ПечататьСмежныеЭтапы""'");
	КонецЕсли;
...
Функция ЗаданиеНаПроизводствоПечататьМатериалы(ПараметрыПечати)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Отсутствует параметр печати ""ПечататьМатериалы""';uk='Відсутні параметр друку ""ПечататьМатериалы""'");
	КонецЕсли;
...
Функция ЗаданиеНаПроизводствоПечататьВыходныеИзделия(ПараметрыПечати)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Отсутствует параметр печати ""ПечататьВыходныеИзделия""';uk='Відсутня параметр друку ""ПечататьВыходныеИзделия""'");
	КонецЕсли;
...
Функция ЗаданиеНаПроизводствоПечататьМаршрутнуюКарту(ПараметрыПечати)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Отсутствует параметр печати ""ПечататьМаршрутнуюКарту""';uk='Відсутня параметр друку ""ПечататьМаршрутнуюКарту""'");
	КонецЕсли;
...
Функция ЗаданиеНаПроизводствоДобавлятьПустыеСтрокиВМатериалы(ПараметрыПечати)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Отсутствует параметр печати ""ДобавлятьПустыеСтрокиВМатериалы""';uk='Відсутня параметр друку ""ДобавлятьПустыеСтрокиВМатериалы""'");
	КонецЕсли;
...
Функция ЗаданиеНаПроизводствоДобавлятьПустыеСтрокиВВыходныеИзделия(ПараметрыПечати)
	...
	Иначе
		ВызватьИсключение НСтр("ru='Отсутствует параметр печати ""ДобавлятьПустыеСтрокиВВыходныеИзделия""';uk='Відсутня параметр друку ""ДобавлятьПустыеСтрокиВВыходныеИзделия""'");
	КонецЕсли;
...
Функция СформироватьПечатнуюФормуЛистСопровожденияПартииПроизводства(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЭтапПроизводства2_2.ПФ_MXL_ЛистСопровожденияПартииПроизводства", КодЯзыкаПечать);
	ПервыйДокумент = Истина;
	...
	Пока ВыборкаПартии.Следующий() Цикл
		...
		ЛистСопровожденияПартииПроизводстваВывестиСмежныеЭтапы(
			ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаПартии, ДанныеДляПечати, Истина, КодЯзыкаПечать);
		...
		ЛистСопровожденияПартииПроизводстваВывестиСмежныеЭтапы(
			ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаПартии, ДанныеДляПечати, Ложь, КодЯзыкаПечать);
...
Процедура ЛистСопровожденияПартииПроизводстваВывестиСмежныеЭтапы(ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаПартии, ДанныеДляПечати, Предшественники, КодЯзыкаПечать)
...
Процедура ЛистСопровожденияПартииПроизводстваВывестиЭтапы(ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаПартии, ДанныеДляПечати)
	...
	Если ИспользуетсяГрафик Тогда
		...
		ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаТаблицыЭтапыСГрафиком"); // см. АналогиМатериалов.СлужебнаяСтруктураТабличныйДокумент
	Иначе
		ОбластьШапка = Макет.ПолучитьОбласть("ШапкаТаблицыЭтапыБезГрафика");
		ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаТаблицыЭтапыБезГрафика"); // см. АналогиМатериалов.СлужебнаяСтруктураТабличныйДокумент
	КонецЕсли;
...
Процедура ЛистСопровожденияПартииПроизводстваВывестиМатериалы(ПараметрыПечати, ТабличныйДокумент, Макет, ВыборкаПартии, ДанныеДляПечати)
	...
	Если ВыводитьКоды Тогда
		...
		ОбластьСтрока = Макет.ПолучитьОбласть(ИмяОбластиСтрока); // см. АналогиМатериалов.СлужебнаяСтруктураТабличныйДокумент
...
Функция СформироватьЭтапыВнутриЗамераВремени(Параметры)
	...
	ПараметрыРазузлования.РезервПФСоСклада = Параметры.РезервМатериаловСоСклада;
	ПараметрыРазузлования.РезервПФИзЗаказов = Параметры.РезервМатериаловИзЗаказов;
	ПараметрыРазузлования.РезервПФИмяПоляНазначение = "НазначениеМатериалы";
	ПараметрыРазузлования.ЗаменятьПФНаАналоги = Параметры.ЗаменятьМатериалыНаАналоги;
	...
	Пока Очередь.Следующий() Цикл
		...
		КонецЦикла;
		РегистрыСведений.НормативныйГрафикЭтаповПроизводства.РассчитатьГрафик(Очередь.Распоряжение);
		
		// Заполнение обеспечения и формирование движений
		ЗаполнитьОбеспечениеИОтразитьДвиженияПоЭтапам(Параметры, Этапы);
		ЗаполнитьРеестрДокументовВПакетномРежиме(Этапы);
...
Процедура ПроверитьДанныеДляФормированияЭтапов(МенеджерВременныхТаблиц, РезультатФормирования)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка Тогда
			Подстановка = НСтр("ru='производства (разборки)';uk='виробництва (розбирання)'");
		ИначеЕсли Выборка.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Ремонт Тогда
			Подстановка = НСтр("ru='производства (ремонта)';uk='виробництва (ремонту)'");
		Иначе
			Подстановка = НСтр("ru='производства';uk='виробництва'");
		КонецЕсли;
		
		ТекстШаблон = НСтр("ru='Для %1 изделий необходимо создать этапы вручную (см. документ № %2)';uk='Для %1 виробів необхідно створити етапи вручну (див. документ № %2)'");
...
Процедура ОбособитьВыпускПодРазборку(СсылкиПотребность, СсылкиОбеспечение)
	...
	Для каждого Ссылка Из СсылкиПотребность Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Этап производства.Формирование этапов';uk='Етап виробництва. Формування етапів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Функция РассчитатьЭкономиюМатериалов(МенеджерВременныхТаблиц, ДатаОперации = '00010101') Экспорт
	...
	|	ТаблицаОбеспечение.ВариантОбеспечения В (
	|								ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно),
	|								ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно)) КАК Обособить,
	...
	КолонкиГруппировок = "Ссылка, Номенклатура, Характеристика, Получатель, Упаковка, Обособить, НужноОкруглять";
...
Функция ПоляПартии(РеквизитыШапки, ОсновноеИзделие) Экспорт
    ...
    ПоляПартии.НалоговоеНазначение = ДанныеЗаполнения.НалоговоеНазначение;
...
Функция ЭтапыПартииДляИзменения(ПартияПроизводства, ПослеЭтапа, Отказ)
	...
	Если ЗначениеЗаполнено(ПослеЭтапа) Тогда
		...
		Если НЕ Выборка.Проведен Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выбранный этап не проведен, действие не может быть выполнено.';uk='Вибраний етап не проведено, дія не може бути виконана.'"),,,, Отказ);
				
		ИначеЕсли Выборка.ЭтоПоследнийЭтап Тогда
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выбран последний этап партии, действие не может быть выполнено.';uk='Вибрано останній етап партії, дія не може бути виконана.'"),,,, Отказ);
		...
		Иначе
			...
			Если Таблица.Количество() = 0 Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Отсутствуют этапы партии для выполнения действия.';uk='Відсутні етапи партії для виконання дії.'"),,,, Отказ);
				
			ИначеЕсли Таблица.Найти(Истина, "ВыполнениеНачато") <> Неопределено Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Выполнение партии после выбранного этапа начато, действие не может быть выполнено.';uk='Виконання партії після вибраного етапу розпочато, дія не може бути виконана.'"),,,, Отказ);
			
			ИначеЕсли Таблица.Найти(Истина, "СозданЗаказПереработчику") <> Неопределено Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru='Для этапа %1 создан заказ переработчику, действие не может быть выполнено.';uk='Для етапу %1 створено замовлення переробнику, дія не може бути виконана.'"),
...
|;uk='У вибраній точці виробничого процесу є паралельні маршрути виконання, 
|дія не може бути виконана.'"),,,, Отказ);
					КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли Таблица.Найти(Истина, "ВыполнениеНачато") <> Неопределено Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выполнение партии начато, действие не может быть выполнено.';uk='Виконання партії розпочато, дія не може бути виконана.'"),,,, Отказ);
...
// Параметры:
//  Ссылки -
//  ПолучитьДанныеДополнения -
//  ПолучитьДанныеПересчета -
//
// Возвращаемое значение:
//  Структура - содержит:
//   * ДанныеДополнения -
...
Функция ДанныеЭтаповДляИзмененияПартий(Ссылки, ПолучитьДанныеДополнения, ПолучитьДанныеПересчета)
	...
	Если ПолучитьДанныеДополнения Тогда
		...
		Запрос = Новый Запрос(
			...
			|	ЭтапПроизводства2_2.ЖелаемаяДатаОбеспечения КАК ЖелаемаяДатаОбеспечения
			|ИЗ
			|	Документ.ЭтапПроизводства2_2 КАК ЭтапПроизводства2_2
			|ГДЕ
			|	ЭтапПроизводства2_2.Ссылка В(&Ссылки)
			...
			|	ОбеспечениеМатериаламиИРаботами.Ссылка.Подразделение КАК ПодразделениеЭтапа,
			...
			|	ВыходныеИзделия.Ссылка.Подразделение КАК ПодразделениеЭтапа,
			...
			|	ПобочныеИзделия.Ссылка.Подразделение КАК ПодразделениеЭтапа,
			...
			|	Трудозатраты.Ссылка.Подразделение КАК ПодразделениеЭтапа,
		...
		Таблица.Индексы.Добавить("Ссылка, Номенклатура, Характеристика, Получатель, Назначение, Произведено, ДатаПроизводства");
		ОбщегоНазначенияУТ.ДобавитьВТаблицуПорядковыйНомерСтрок(Таблица);
...
Процедура РаспределитьДанныеЭтаповНаДанныеСпецификации(ДанныеЭтапов, ДанныеСпецификации, ВыходныеИзделияСерии, ПобочныеИзделияСерии)
	...
	// Этапы
	ДанныеСпецификации.Этапы.Колонки.Добавить("ЖелаемаяДатаОбеспечения", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты));
	Для каждого Строка Из ДанныеСпецификации.Этапы Цикл
		НайденнаяСтрока = ДанныеЭтапов.Реквизиты.Найти(Строка.Этап, "Этап");
		Если НайденнаяСтрока <> Неопределено Тогда
			Строка.ЖелаемаяДатаОбеспечения = НайденнаяСтрока.ЖелаемаяДатаОбеспечения;
		КонецЕсли;
	КонецЦикла;
	...
	СписокКолонок = "Серия,ВариантОбеспечения,ДатаОтгрузки,Склад,Производится,Спецификация,Назначение,СтатьяКалькуляции";
	...
	// Выходные изделия
	СтруктураПоискаСерии = Новый Структура("Ссылка, Номенклатура, Характеристика, Получатель, Назначение, Произведено, ДатаПроизводства");
...
Функция ИзменитьРазмерПартииВЭтапах(ДанныеПартии, РазмерПартии, Ссылки, ДанныеЭтапов, ПересчитатьПоКоэффициенту, Отказ)
	...
	Иначе
		...
		Если Потребности.ВГраница() <> -1 Тогда
			...
			ТаблицаСоответствий = Запрос.Выполнить().Выгрузить(); // см. АналогиМатериалов.СлужебнаяСтруктураТаблицыЗначений
		...
		СтруктураПоискаДата = Новый Структура("ДатаОтгрузки", '00010101');
		...
		Для каждого Ссылка Из Ссылки Цикл
			...
			Иначе
				...
				Для каждого Строка Из ОбеспечениеМатериаламиИРаботами.НайтиСтроки(СтруктураПоискаДата) Цикл
					Строка.ДатаОтгрузки = ДокументОбъект.ЖелаемаяДатаОбеспечения;
				КонецЦикла;
				СтруктураДействий.Вставить(
					"ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства",
...
Функция СкопироватьПартиюИПересчитатьПоКоэффициенту(ДанныеПартии, ДанныеЭтапов, РазмерПартии, ОсновноеИзделиеНовойПартии)
	...
	СтруктураПоискаСерии = Новый Структура("Ссылка,Номенклатура,Характеристика,Назначение,Получатель,Произведено,ДатаПроизводства");
	НомерЭтапа = 1;
...
Функция ПересчитатьДанныеЭтапаПоКоэффициенту(ДанныеЭтапов, Ссылка, Коэффициент)
	...
	Если СтрокиССериями.ВГраница() <> -1 Тогда
		...
		Аналитики = "Ссылка,Номенклатура,Характеристика,Назначение,Получатель,Произведено,ДатаПроизводства";
...
Функция ПередатьРезервы(СсылкиОтправители, СсылкиПолучатели)
	...
	Запрос = Новый Запрос(
		...
		|	Потребности.Назначение                                      КАК НовоеНазначение,
		|	Остатки.НаличиеПодЗаказОстаток - Остатки.ПотребностьОстаток КАК Остаток,
		|	ВЫБОР
		|		КОГДА Потребности.ПотребностьОстаток > Потребности.НаличиеПодЗаказОстаток
		|			ТОГДА Потребности.ПотребностьОстаток - Потребности.НаличиеПодЗаказОстаток
		|		ИНАЧЕ 0
		...
		|					Документ.ЭтапПроизводства2_2 КАК Т
		|				ГДЕ
		|					Т.Ссылка В (&Отправители))) КАК Остатки
		...
		|						Документ.ЭтапПроизводства2_2 КАК Т
		|					ГДЕ
		|						Т.Ссылка В (&Получатели))) КАК Потребности
		...
		|ГДЕ
		|	Остатки.НаличиеПодЗаказОстаток > Остатки.ПотребностьОстаток
		|
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,
		|	Характеристика,
	...
	Запрос.Текст = 
		...
		|			ТОГДА Обеспечение.Ссылка.Подразделение
	...
	СтруктураПоиска3 = Новый Структура("Номенклатура, Характеристика, Назначение, Получатель, Произведено, ДатаПроизводства");
		...
		Пока Выборка.Следующий() Цикл
			...
			Для каждого Строка Из ДокументОбъект[Выборка.ИмяТЧ].НайтиСтроки(СтруктураПоиска1) Цикл
				...
				Для каждого Потребность Из ПотребностьПолучатели.НайтиСтроки(СтруктураПоиска2) Цикл
					...
					ОстатокСерии = КоличествоЗачет;
					Для каждого СтрокаСерии Из ДокументОбъект[Выборка.ИмяТЧ + "Серии"].НайтиСтроки(СтруктураПоиска3) Цикл
						...
						Иначе
							НоваяСтрока = ДокументОбъект[Выборка.ИмяТЧ + "Серии"].Добавить();
...
Функция ОформитьПромежуточныйВыпуск(ДанныеПартии, Этап, ВыходныеИзделия, Серии, ПараметрыВыпуска)
	...
	Если ПараметрыВыпуска.СоздатьЭтап Тогда
		...
		РеквизитыШапкиНовый.Вставить("Подразделение", ДокументОбъект.Подразделение);
		РеквизитыШапкиНовый.Вставить("НаименованиеЭтапа", НСтр("ru='Промежуточный выпуск';uk='Проміжний випуск'"));
	...
	Иначе
		...
		СтруктураПоиска2 = Новый Структура("Номенклатура, Характеристика, Назначение, Получатель, Произведено, ДатаПроизводства");
	...
	Для Индекс = 0 По ДанныеСпецификации.Этапы.Количество()-1 Цикл
		...
		Пока Истина Цикл
			...
			Попытка
				...
				Если ДокументОбъект.Статус <> Перечисления.СтатусыЭтаповПроизводства2_2.Формируется Тогда
					Если ПовторитьПопытку Тогда
						Если НЕ ДокументОбъект.ПроверитьЗаполнение() Тогда
							ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Формируется;
							ПовторитьПопытку = Ложь;
						КонецЕсли;
					Иначе
						ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Формируется;
					КонецЕсли;
			...
			Если НЕ ПовторитьПопытку Тогда
				Прервать;
			КонецЕсли;
...
Функция ТабличныеЧастиДляОформленияОтчетаПереработчика() Экспорт
	...
	ПараметрыПрименения = ПроизводствоСервер.ПараметрыПримененияШтучногоТовараВМернойЕдинице();
	ПараметрыПрименения.ШтучныйТоварВМерныхЕдиницах = Ложь;
	ПроизводствоСервер.ПрименитьНастройкиШтучногоТовараВМернойЕдинице(Результат.ВыходныеИзделия, "ПобочныеИзделияЭтапа", ПараметрыПрименения);	
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Этапы производства';uk='Етапи виробництва'");
...
Функция ИмяПоляЖелаемаяДатаОтгрузки() Экспорт
	...
	Возврат "ЖелаемаяДатаОбеспечения";
...
Функция ТекстЗапросаТоварыДокумента(Отбор, ХозяйственнаяОперация) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Ошибка в процедуре ЭтапПроизводства2_2.ТекстЗапросаТоварыДокумента. Обратитесь к администратору.';uk='Помилка в процедурі ЭтапПроизводства2_2.ТекстЗапросаТоварыДокумента. Зверніться до адміністратора.'");
	КонецЕсли;
...
Процедура ПодготовитьДанныеСпецификацииКСозданиюЭтапов(ДанныеСпецификации, ДанныеПартии, ТочкаДеления)
	...
	Если ДанныеСпецификации.Этапы.Найти('00010101', "ЖелаемаяДатаОбеспечения") <> Неопределено Тогда
		...
		ДанныеСпецификации.Этапы.ЗаполнитьЗначения('00010101', "ЖелаемаяДатаОбеспечения");
		...
		Для Индекс = 0 По ДанныеСпецификации.Этапы.Количество()-1 Цикл
			...
			Если ДанныеСпецификации.Этапы.Найти(Этап.НомерЭтапа, "НомерСледующегоЭтапа") = Неопределено Тогда
				Этап.ЖелаемаяДатаОбеспечения = НачальнаяДата;
			КонецЕсли;
			Если ЗначениеЗаполнено(Этап.ЖелаемаяДатаОбеспечения )
				...
				Для каждого СледующийЭтап Из ДанныеСпецификации.Этапы.НайтиСтроки(СтруктураПоиска) Цикл
					Если ЗначениеЗаполнено(СледующийЭтап.ЖелаемаяДатаОбеспечения) Тогда
						СледующийЭтап.ЖелаемаяДатаОбеспечения = Мин(
							СледующийЭтап.ЖелаемаяДатаОбеспечения,
							Этап.ЖелаемаяДатаОбеспечения + Этап.ДлительностьЭтапаВСекундах);
					Иначе
						СледующийЭтап.ЖелаемаяДатаОбеспечения =
							Этап.ЖелаемаяДатаОбеспечения + Этап.ДлительностьЭтапаВСекундах;
					КонецЕсли;
