...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность И НЕ ВыполняетсяЗакрытие И НЕ ЗавершениеРаботы Тогда
		...
		ТекстВопроса       = НСтр("ru='Форма будет закрыта, все введенные данные будут потеряны. Продолжить?';uk='Форма буде закрита, всі введені дані будуть втрачені. Продовжити?'");
...
Процедура ЗаполнитьПараметрыРучногоРазмещения(Параметры)
	...
	Если ОкончаниеПредыдущего > '00010101' Тогда 
		...
		ПараметрыПрижатия.ШаблонЛеваяГраница = НСтр("ru='прижать к предыдущему (%1)';uk='притиснути до попереднього (%1)'");
	...
	ИначеЕсли РеквизитыРаспоряжения.НачатьНеРанее > ТекущаяДатаСеанса() Тогда
		...
		ПараметрыПрижатия.ШаблонЛеваяГраница = НСтр("ru='прижать к началу (%1)';uk='притиснути до початку (%1)'");
	...
	Если НачалоСледующего > '00010101' Тогда
		...
		ПараметрыПрижатия.ШаблонПраваяГраница = НСтр("ru='прижать к следующему (%1)';uk='притиснути до наступного (%1)'");
	...
	ИначеЕсли РеквизитыРаспоряжения.ДатаПотребности > ТекущаяДатаСеанса()
		...
		ПараметрыПрижатия.ШаблонПраваяГраница = НСтр("ru='прижать к дате потребности (%1)';uk='притиснути до дати потреби (%1)'");
...
#Область ТекстОбщаяДоступность
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Общая доступность';uk='Загальна доступність'"));
...
#Область ТекстСвободно
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Свободно';uk='Вільно'"));
...
#Область ТекстЗанять
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Занять';uk='Зайняти'"));
...
Процедура ПриИзмененииПараметровРазмещенияНаСервере(ИмяРеквизита)
	...
	Если НЕ РассчитатьГраницыЭтапа(ИмяРеквизита) Тогда
		...
		ТекстСообщения = НСтр("ru='Не удалось рассчитать границы этапа.';uk='Не вдалося розрахувати межу етапу.'");
...
Функция ОбластиТаблицыОтклоненияОтНорматива()
	...
	Область.Текст = НСтр("ru='Вид рабочего центра';uk='Вид робочого центру'");
	
	// Время работы
	Область = ОбластьЯчеекЗаголовок(Таблица, "R2C3:R2C5", 45);
	Область.Текст = НСтр("ru='Время работы, ч';uk='Час роботи, год'");
	
	// Норматив
	Область = ОбластьЯчеекЗаголовок(Таблица, "R3C3:R3C3", 15);
	Область.Текст = НСтр("ru='Норматив';uk='Норматив'");
	
	// Занято
	Область = ОбластьЯчеекЗаголовок(Таблица, "R3C4:R3C4", 15);
	Область.Текст = НСтр("ru='Занято';uk='Зайнято'");
	
	// Отклонение
	Область = ОбластьЯчеекЗаголовок(Таблица, "R3C5:R3C5", 15);
	Область.Текст = НСтр("ru='Отклонение';uk='Відхилення'");
...
Процедура ОбработкаПроверкиЗанятостиВидовРабочихЦентров(Отказ)
	...
	ШаблонСообщения = НСтр("ru='Не заполнена колонка ""%1"" в строке %2 списка ""Занятость видов рабочих центров"".';uk='Не заповнена колонка ""%1"" в рядку %2 списку ""Зайнятість видів робочих центрів"".'");
	...
	Для Каждого Строка Из ЗанятостьВидовРабочихЦентров Цикл
		...
		Если НЕ ЗначениеЗаполнено(Строка.ВидРабочегоЦентра) Тогда
			...
			ТекстСообщения = СтрШаблон(ШаблонСообщения, 
				НСтр("ru='Вид рабочего центра';uk='Вид робочого центру'"), НомерСтроки);
...
Процедура ОбработкаПроверкиПериодаВыполненияИИнтерваловЗанятости(Отказ)
	...
	Если КодОшибки = КодОшибкиЗанятостьВыходитЗаГраницыЭтапа() Тогда
		...
		ТекстСообщения = НСтр("ru='Занятость видов рабочих центров выходит за границы этапа';uk='Зайнятість видів робочих центрів виходить за межі етапу'");
