	...
	Если ДанныеСтроки <> Неопределено Тогда
		...
		Если ДанныеСтроки.ТипПроизводственногоПроцесса 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru='Для этапов заказа с типом производственного процесса ""Без спецификаций"" изменение партии недоступно';uk='Для етапів замовлення з типом виробничого процесу ""Без специфікацій"" зміна партії недоступна'"));
			Возврат;
		КонецЕсли;
...
Процедура КомандаПередатьКВыполнению(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке этапов будет установлен статус ""К выполнению"". Продолжить?';uk='У виділених в списку етапів буде встановлено статус ""До виконання"". Продовжити?'");
...
Процедура КомандаОтметитьНачалоВыполнения(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке этапов будет установлен статус ""Начат"". Продолжить?';uk='У виділених в списку етапів буде встановлено статус ""Розпочато"". Продовжити?'");
...
Процедура ОтметитьНачалоВыполненияЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияУТКлиент.ОповеститьПользователяОбУстановкеСтатуса(Элементы.Список, КоличествоОбработанных, ВыделенныеСтроки.Количество(), НСтр("ru='Начат';uk='Розпочато'"));
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не рассчитана>';uk='<не розраховано>'"));
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация ИЛИ СтруктураРеквизитов.Свойство("ПоложениеДопПанели") Тогда
		...
		Элементы.ДекорацияОтобратьЭтапы.Заголовок = 
				?(Форма.ДопПанельСвернута, НСтр("ru='Отобрать';uk='Відібрати'"), НСтр("ru='Отобрать этапы, которые';uk='Відібрати етапи, які'"));
		
		Элементы.ДекорацияОтобратьЭтапыПродолжение.Заголовок = 
				?(НЕ Форма.ДопПанельСвернута, НСтр("ru='И которые';uk='І які'"), "");
	...
	Если СтруктураРеквизитов.Свойство("УстановитьОтборПоЦепочкеЭтапов") Тогда
		...
		Если НЕ ПоказатьВсеЭтапы Тогда
			...
			МассивСтрок.Добавить(НСтр("ru='Установлен отбор по';uk='Встановлено відбір за'"));
			МассивСтрок.Добавить(" ");
			
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='этапу';uk='етапу'"),,,, 
					ЗначениеГиперссылкиПерейтиКТекущемуЭтапуЦепочки()));
					
			МассивСтрок.Добавить(" ");
			МассивСтрок.Добавить(НСтр("ru='и всем его';uk='і всім його'"));
			МассивСтрок.Добавить(" ");
			
			Если ПоказатьЭтапИСмежныеЭтапы Тогда
				МассивСтрок.Добавить(НСтр("ru='смежным этапам';uk='суміжним етапам'"));
			ИначеЕсли ПоказатьПоследователей Тогда
				МассивСтрок.Добавить(НСтр("ru='последователям';uk='послідовникам'"));
			ИначеЕсли ПоказатьПредшественников Тогда
				МассивСтрок.Добавить(НСтр("ru='предшественникам';uk='попередникам'"));
			КонецЕсли;
			
			МассивСтрок.Добавить(" (");
			
			МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='отключить';uk='відключити'"),,,, 
...
Процедура УстановитьОтборыПоДаннымЭтапа()
	...
	ОтборПоВыходнымИзделиям = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		...
		НСтр("ru='Отбор по выходным изделиям';uk='Відбір за вихідними виробами'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	
	ОтборПоНоменклатуре = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоВыходнымИзделиям.Элементы,
		НСтр("ru='Отбор по выходным изделиям (рассчитываемая стоимость)';uk='Відбір за вихідними виробами (розраховується вартість)'"),
	...
	ОтборПоНоменклатуре = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоВыходнымИзделиям.Элементы,
		НСтр("ru='Отбор по выходным изделиям (фиксированная стоимость)';uk='Відбір за вихідними виробами (фіксована вартість)'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	...
	ОтборПоМатериалам = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		Список.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы,
		НСтр("ru='Отбор по материалам';uk='Відбір за матеріалами'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	
	ОтборПоНоменклатуре = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоМатериалам.Элементы,
		НСтр("ru='Отбор по материалам (обеспечение)';uk='Відбір за матеріалами (забезпечення)'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	...
	ОтборПоНоменклатуре = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоМатериалам.Элементы,
		НСтр("ru='Отбор по материалам (расход)';uk='Відбір за матеріалами (витрачання)'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	...
	ОтборПоНоменклатуре = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоМатериалам.Элементы,
		НСтр("ru='Отбор по материалам (экономия)';uk='Відбір за матеріалами (економія)'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	...
	ОтборПоВидам = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		Список.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы,
		НСтр("ru='Отбор по рабочим центрам';uk='Відбір за робочими центрами'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	
	ОтборПоВРЦ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоВидам.Элементы,
		НСтр("ru='Отбор по ВРЦ (основные)';uk='Відбір за ВРЦ (основні)'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	...
	ОтборПоВРЦ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		ОтборПоВидам.Элементы,
		НСтр("ru='Отбор по ВРЦ (альтернативные)';uk='Відбір за ВРЦ (альтернативні)'"),
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
...
Функция ЗапуститьЗаданиеПланироватьГрафикПроизводства(ПараметрыЗадания)
	...
	НаименованиеЗадания = НСтр("ru='Планирование графика этапов производства';uk='Планування графіка етапів виробництва'");
...
Функция ПолучитьПредставлениеСтатуса(СписокСтатусов)
	...
	Если СписокСтатусов.Количество() = КоличествоСтатусов - 1
		...
		Результат = НСтр("ru='Незавершенные';uk='Незавершені'");
