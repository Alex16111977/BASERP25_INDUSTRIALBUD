...
Процедура УстановитьВидимостьИЗаголовкиКолонок() Экспорт
	...
	// АДАПТОВАНО: Роль ТабелированиеАдминистратор не існує в BASERP25
	МожноРедактироватьВсеДаные=РольДоступна("ПолныеПрава");
	...
	Если ЗначениеЗаполнено(Объект.НачДата) И ЗначениеЗаполнено(Объект.КонДата) Тогда
		...
		Пока ТекДата<=Объект.КонДата Цикл
			...
			// АДАПТОВАНО: Константа КоличествоЧасовДляРедактированияТабеля не існує в BASERP25
			// Використовуємо фіксоване значення 24 години (1 день)
			Если ТекДата+60*60*24<НачалоДня(ТекущаяДата()) Тогда
...
Процедура ОтработанноеВремяПередНачаломИзменения(Элемент, Отказ)
	...
	//	// АДАПТОВАНО: Документ.А_ТабельУчетаРабочегоВремени
	//	ОткрытьФорму("Документ.А_ТабельУчетаРабочегоВремени.Форма.ФормаВводаОборудования",
	//	,
	//	ЭтаФорма,
