...
Процедура ЗаполнитьДокумент(Команда)
	...
	Если Объект.ВидыРабот.Количество() > 0 И Объект.Работники.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Списки работ и исполнителей будут перезаполнены. Продолжить?';uk='Списки робіт і виконавців будуть перезаповнені. Продовжити?'");
	ИначеЕсли  Объект.ВидыРабот.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Список выполненных работ будет перезаполнен. Продолжить?';uk='Список виконаних робіт буде перезаповнений. Продовжити?'");
	ИначеЕсли  Объект.Работники.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Список работников будет перезаполнен. Продолжить?';uk='Список працівників буде перезаповнений. Продовжити?'");
	КонецЕсли;
...
Процедура ЗаполнитьВидыРабот(Команда)
	...
	Если Объект.ВидыРабот.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Список работ будет перезаполнен. Продолжить?';uk='Список робіт буде перезаповнений. Продовжити?'");
	КонецЕсли;
...
Процедура ЗаполнитьСотрудниковНаКлиенте(ОтветНаВопрос, Параметры) Экспорт
	...
	Если Объект.Работники.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Документы выработки для заполнения состава бригады не найдены. Заполните состав бригады вручную.';uk='Документи виробітку для заповнення складу бригади не знайдені. Заповніть склад бригади вручну.'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<соответствует виду работ>';uk='<відповідає виду робіт>'"));
...
Процедура НастроитьФормуПоИспользуемымКадровымДанным(Форма)
	...
	Если Форма.Объект.ИспользоватьТарифныеСтавки И Форма.Объект.ИспользоватьОтработанноеВремя Тогда
		Форма.Элементы.СотрудникиЗаполнитьКадровыеДанные.Заголовок = НСтр("ru='Ставки и отработанное время';uk='Ставки і відпрацьований час'");
		Форма.Элементы.СотрудникиЗаполнитьКадровыеДанные.Видимость = Истина;
	ИначеЕсли Форма.Объект.ИспользоватьТарифныеСтавки Тогда
		Форма.Элементы.СотрудникиЗаполнитьКадровыеДанные.Заголовок = НСтр("ru='Тарифные ставки';uk='Тарифні ставки'");
		Форма.Элементы.СотрудникиЗаполнитьКадровыеДанные.Видимость = Истина;
	ИначеЕсли Форма.Объект.ИспользоватьОтработанноеВремя Тогда
		Форма.Элементы.СотрудникиЗаполнитьКадровыеДанные.Заголовок = НСтр("ru='Отработанное время';uk='Відпрацьований час'");
		Форма.Элементы.СотрудникиЗаполнитьКадровыеДанные.Видимость = Истина;
	Иначе
...
Функция ПоместитьВидыРаботВХранилище()
	...
	Возврат ПоместитьВоВременноеХранилище(Объект.ВидыРабот.Выгрузить(,"Распоряжение,КодСтрокиРаспоряжения,КоличествоКОформлению"));
...
Процедура УстановитьСоставОпераций()
	...
	Если Объект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.ПрочиеРаботы Тогда
		Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.ПрочиеРаботы, НСтр("ru='Прочим работам';uk='Іншим роботам'"));
	ИначеЕсли  Объект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Ремонт Тогда
		Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.Ремонт, НСтр("ru='Ремонтам';uk='Ремонтам'"));
	...
	Иначе
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
			Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.Бригадный, НСтр("ru='Бригаде';uk='Бригаді'"));
			Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.Персональный, НСтр("ru='Работникам';uk='Працівникам'"));
		КонецЕсли;
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством") Тогда
			Если ПолучитьФункциональнуюОпцию("КомплекснаяАвтоматизация") Тогда
				Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.БригадныйБезЗаказа21, НСтр("ru='Производству';uk='Виробництву'"));
			ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
				Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.БригадныйПоЗаказу21, НСтр("ru='Заказу на производство (2.1)';uk='Замовленням на виробництво (2.1)'"));
				Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.БригадныйБезЗаказа21, НСтр("ru='Производству без заказа (2.1)';uk='Виробництву без замовлення (2.1)'"));
			Иначе
				Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.БригадныйПоЗаказу21, НСтр("ru='Заказу на производство';uk='Замовлення на виробництво'"));
				Поле.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.БригадныйБезЗаказа21, НСтр("ru='Производству без заказа';uk='Виробництву без замовлення'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
