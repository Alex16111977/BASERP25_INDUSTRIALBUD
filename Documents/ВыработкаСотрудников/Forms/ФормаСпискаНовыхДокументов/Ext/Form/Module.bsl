...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		Заголовок,
		НСтр("ru='Оформление выработки сотрудников';uk='Оформлення виробітку співробітників'"),
		СозданныеДокументы.Количество(),
		СклонениеСлова);
	...
	Иначе
		Для Каждого ТекСтрока Из ВыделенныеСтроки Цикл
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Оформление выработки сотрудников.Проведение документов.';uk='Оформлення виробітку співробітників. Проведення документів.'",КодОсновногоЯзыка),
...
Процедура УдалитьДокументы()
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
			ЗаписьЖурналаРегистрации(НСтр("ru='Оформление выработки сотрудников.Удаление документов.';uk='Оформлення виробітку співробітників. Вилучення документів.'",КодОсновногоЯзыка),
									УровеньЖурналаРегистрации.Ошибка,
									,
									Выборка.Ссылка,
									ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура УстановитьПометкуУдаленияЗавершениеСервер(ДополнительныеПараметры)
	...
	Для Каждого ТекСтрока Из ВыделенныеСтроки Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			КодОсновногоЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
			ЗаписьЖурналаРегистрации(НСтр("ru='Оформление выработки сотрудников.Установка пометки на удаление.';uk='Оформлення виробітку співробітників. Встановлення позначки на вилучення.'",КодОсновногоЯзыка),
									УровеньЖурналаРегистрации.Ошибка,
									,
									ТекСтрока,
									ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
