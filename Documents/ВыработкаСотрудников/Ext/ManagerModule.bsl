...
Процедура ОформитьПриЗакрытииМесяца(ПараметрыОбработчика) Экспорт
	...
	Для Каждого Строка Из Результат.СписокОшибок Цикл 
		...
		ЗакрытиеМесяцаСервер.ЗарегистрироватьПроблемуВыполненияРасчета(
			...
			НСтр("ru='Рекомендуется оформить вручную из списка к оформлению. При автоматическом оформлении диагностированы ошибки';uk='Рекомендується оформити вручну зі списку до оформлення. При автоматичному оформленні діагностовано помилки'",ОбщегоНазначения.КодОсновногоЯзыка()),
			,
			Строка.ТекстОшибки);
...
Функция ТрудозатратыКОформлению(СтруктураОтбора, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МКТрудозатраты.Ссылка КАК Ссылка,
	|	МАКСИМУМ(МКТрудозатраты.СтатьяКалькуляции) КАК СтатьяКалькуляции,
	|	МКТрудозатраты.ВидРабот КАК ВидРабот
	|ПОМЕСТИТЬ ВтСтатьиКалькуляцииОпераций
	|ИЗ
	|	Справочник.МаршрутныеКарты.Трудозатраты КАК МКТрудозатраты
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Остатки КАК ВтТрудозатратыОпераций
	|		ПО ВтТрудозатратыОпераций.Распоряжение.Операция.Владелец = МКТрудозатраты.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	МКТрудозатраты.Ссылка,
	|	МКТрудозатраты.ВидРабот
	|
	|ИМЕЮЩИЕ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ МКТрудозатраты.СтатьяКалькуляции) = 1
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	ВидРабот
	...
	|			ЗНАЧЕНИЕ(Перечисление.ВидыБригадныхНарядов.Персональный)
	|	КОНЕЦ КАК ВидНаряда,
	|	ТрудозатратыОперации.Ссылка.ОкончаниеВыполнения КАК ДатаВыполнения,
	|	ТрудозатратыКОформлению.Периодичность КАК Периодичность,
	|	ТрудозатратыКОформлению.КодСтрокиРаспоряжения,
	|	ТрудозатратыКОформлению.ВидРабот,
	|	ТрудозатратыКОформлению.Количество,
	...
	|	ТрудозатратыКОформлению.Исполнитель,
	|	ТрудозатратыКОформлению.Подразделение,
	|	ТрудозатратыОперации.Ссылка.Этап.ПартияПроизводства.ГруппаПродукции КАК ГруппаПродукции,
	...
	|	Остатки КАК ТрудозатратыКОформлению
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводственнаяОперация2_2.Трудозатраты КАК ТрудозатратыОперации
	|		ПО ТрудозатратыОперации.Ссылка = ТрудозатратыКОформлению.Распоряжение
	|		И  ТрудозатратыОперации.КодСтроки = ТрудозатратыКОформлению.КодСтрокиРаспоряжения
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтатьиКалькуляцииОпераций КАК СтатьиКалькуляцииОпераций
	|		ПО (ТрудозатратыОперации.Ссылка.Операция.Владелец = СтатьиКалькуляцииОпераций.Ссылка
	|			И ТрудозатратыОперации.ВидРабот = СтатьиКалькуляцииОпераций.ВидРабот)
	//-- НЕ УТКА
	|
	|;
	|
	...
	|	ДД.КодСтрокиРаспоряжения КАК КодСтрокиРаспоряжения,
	|	ДД.ВидРабот КАК ВидРабот,
	|	ДД.Количество КАК КоличествоКОформлению,
	|	ДД.Количество КАК КоличествоКОплате,
...
Процедура УстановитьПараметрЗапросаТаблицаВидыРаботСотрудников(Запрос)
	...
	|	СУММА(ВидыРабот.КоличествоКОплате) КАК Количество,
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ВыработкаСотрудников") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьВыработкиСотрудников(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,,Истина);
	КонецЕсли;
	
КонецПроцедуры

Функция ПечатьВыработкиСотрудников(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
...
//-- НЕ УТКА
	...
	Запрос.УстановитьПараметр("СинонимЭтапа", НСтр("ru='Этап';uk='Етап'"));
	//-- НЕ УТКА
	Запрос.УстановитьПараметр("СинонимПроизводстваБезЗаказа", НСтр("ru='Производство без заказа';uk='Виробництво без замовлення'"));
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ВыработкаСотрудников.ПФ_MXL_ВыработкаСотрудников", КодЯзыкаПечать);
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Выработка сотрудников';uk='Виробіток співробітників'");
...
Процедура ДобавитьОшибкуАвтоматическогоСоздания(Документ, СписокОшибок, ТипОшибки, ОписаниеОшибки = "")
	...
	Если ТипОшибки = "Заполнение" Тогда
		Если Документ.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Персональный Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка заполнения персонального наряда за период с %1 по %2 по организации ""%3"" и подразделению ""%4"".';uk='Помилка заповнення персонального наряду за період з %1 по %2 по організації ""%3"" і підрозділу ""%4"".'",ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка заполнения бригадного наряда за период с %1 по %2 по организации ""%3"", бригаде ""%4"" и подразделению ""%5"".';uk='Помилка заповнення бригадного наряду за період з %1 по %2 по організації ""%3"", бригаді ""%4"" і підрозділу ""%5"".'",ОбщегоНазначения.КодОсновногоЯзыка()),
				Формат(Документ.НачалоПериода,"ДЛФ=dd.MM.yyyy"),
				Формат(Документ.КонецПериода,"ДЛФ=dd.MM.yyyy"),
				Документ.Организация,
				Документ.Бригада,
	...
	ИначеЕсли ТипОшибки = "Проведение" Тогда
		Если Документ.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Персональный Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка проведения персонального наряда за период с %1 по %2 по организации ""%3"" и подразделению ""%4"": %5.';uk='Помилка проведення персонального наряду за період з %1 по %2 по організації ""%3"" і підрозділу ""%4"": %5.'",ОбщегоНазначения.КодОсновногоЯзыка()),
				Формат(Документ.НачалоПериода,"ДЛФ=dd.MM.yyyy"),
				Формат(Документ.КонецПериода,"ДЛФ=dd.MM.yyyy"),
				Документ.Организация,
				Документ.Подразделение,
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка проведения бригадного наряда за период с %1 по %2 по организации ""%3"", бригаде ""%4"" и подразделению ""%5"": %6.';uk='Помилка проведення бригадного наряду за період з %1 по %2 по організації ""%3"", бригаді ""%4"" і підрозділу ""%5"": %6.'",ОбщегоНазначения.КодОсновногоЯзыка()),
				Формат(Документ.НачалоПериода,"ДЛФ=dd.MM.yyyy"),
				Формат(Документ.КонецПериода,"ДЛФ=dd.MM.yyyy"),
				Документ.Организация,
				Документ.Бригада,
	...
	ИначеЕсли ТипОшибки = "УдалениеДокумента" Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось удалить документ %1 по причине: %2.';uk='Не вдалося вилучити документ %1 з причини: %2.'",ОбщегоНазначения.КодОсновногоЯзыка()),
			Документ.Ссылка,
			ОписаниеОшибки);
	КонецЕсли;
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("ea18fc05-b658-44d3-b24a-f0785cbb1dd5");
	...
	Обработчик.Комментарий = НСтр("ru='Приводит документы ""Выработка сотрудников"" к новому формату: заполняет вид наряда и другие реквизиты документа.
                                   |Заполнение нового реквизита ""КоличествоЗаявлено"".'
                                   |;uk='Наводить документи ""Вироблення співробітників"" до нового формату: заповнює вид наряду та інші реквізити документа.
                                   |Заповнення нового реквізиту ""КоличествоЗаявлено"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		Документ.ВыработкаСотрудников.ВидыРабот КАК ВидыРабот
	|	ГДЕ
	|		ВидыРабот.КоличествоКОформлению = 0";
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Для Каждого Строка Из ДокументОбъект.ВидыРабот Цикл
					Если (Строка.КоличествоКОплате <> 0) Тогда
						Строка.КоличествоКОформлению = Строка.КоличествоКОплате;
					КонецЕсли;
				КонецЦикла;
...
Процедура ЗаполнитьСотрудниковВТаблицеВидовРабот(ДокументОбъект)
	...
	Для Каждого ВидРабот Из ДокументОбъект.ВидыРабот Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			НоваяСтрока.КоличествоКОплате = Строка.УдалитьКоличество;
