...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидНаряда = Перечисления.ВидыБригадныхНарядов.Бригадный Или Автораспределение Тогда
		Если ВидыРабот.Итог("Сумма") <> Работники.Итог("Сумма") Тогда
			...
			ТекстСообщения = НСтр("ru='Сумма работ не распределена по участникам бригады. Требуется распределить сумму на закладке ""Работники""';uk='Сума робіт не розподілена по учасниках бригади. Потрібно розподілити суму на закладці ""Працівники""'");
	...
	Если Не ЗначениеЗаполнено(Валюта) Тогда
		ТекстСообщения = НСтр("ru='Не задана валюта расценок на виды работ. Валюта задается в разделе ""НСИ и администрирование \ Производство"".';uk='Не задана валюта розцінок на види робіт. Валюта задається в розділі ""НДІ і адміністрування \ Виробництво"".'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидНаряда = Перечисления.ВидыБригадныхНарядов.Персональный Тогда
		...
		ИнтеграцияБЗК.ЗаполнитьКраткийСоставФизЛиц(ФизическиеЛица, Дата, Исполнители);
	Иначе
		Исполнители = Бригада;
	КонецЕсли;
