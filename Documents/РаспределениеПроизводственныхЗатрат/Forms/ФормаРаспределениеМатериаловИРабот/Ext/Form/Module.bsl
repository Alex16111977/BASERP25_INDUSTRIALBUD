...
Процедура МатериалыНаличиеПриИзменении(Элемент)
	...
	Если ТекущиеДанные.Наличие > ТекущиеДанные.ВходящийОстаток Тогда
		...
		ТекстСообщения = НСтр("ru='Фактическое наличие материала в кладовой не может превышать входящий остаток.';uk='Фактична наявність матеріалу в коморі не може перевищувати вхідний залишок.'");
...
|;uk='У зазначеному періоді використовується партіонний облік версії 2.1.
|Для розподілу матеріалів і робіт рекомендується скористатися аналогічним робочим місцем для версії 2.1. Робоче місце є в групі ""Див. також"" розділу Виробництво.'");
...
|Для распределения на производство версии 2.5 существующий документ распределения следует пометить на удаление.'
|;uk='Розподіл виконано документом для виробництва версії 2.1. Для роботи з документом потрібно перейти в робоче місце розподілу 2.1. 
|Для розподілу на виробництво версії 2.5 існуючий документ розподілу слід позначити на вилучення.'");
...
Процедура СоздатьДокументНаОсновании(Тип)
	...
	Если ЕстьОшибки Тогда
		Если ЕстьОшибкиНетДанных Тогда
			ТекстОшибки = НСтр("ru='Отсутствуют строки, требующие распределения.';uk='Відсутні рядки, що потребують розподілу.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		КонецЕсли;
		...
		Если ЕстьОшибкиОрганизация
			...
			ТекстОшибки = НСтр("ru='В выбранных строках различаются значения:';uk='У вибраних рядках розрізняються значення:'");
			
			Если ЕстьОшибкиОрганизация Тогда
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.Таб +  НСтр("ru='- Организаций';uk='- Організацій'");
			КонецЕсли;
			
			Если ЕстьОшибкиПодразделение Тогда
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.Таб + НСтр("ru='- Подразделений';uk='- Підрозділів'");
			КонецЕсли;
			
			Если ЕстьОшибкиКладовая Тогда
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.Таб + НСтр("ru='- Кладовых';uk='- Комор'");
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		КонецЕсли;
		
		Если ЕстьОшибкиТипНоменклатуры Тогда
			ТекстОшибки = НСтр("ru='Документ не предназначен для отражения движений по выбранной номенклатуре.';uk='Документ не призначений для відображення рухів по вибраній номенклатурі.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		КонецЕсли;
		
		Если ЕстьОшибкиВариантОформления Тогда
			ТекстОшибки = НСтр("ru='В выбранных строках вариант оформления продажи номенклатуры не соответствует типу оформляемого документа.';uk='У вибраних рядках варіант оформлення продажу номенклатури не відповідає типу оформлюваного документа.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		КонецЕсли;
		
		ТекстОшибки = НСтр("ru='Оформление документа невозможно!';uk='Оформлення документа неможливо!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		Возврат;
	Иначе
...
Функция ПараметрыЖурнала()
	...
	ПараметрыЖурнала.Вставить("СинонимЖурнала",НСтр("ru='Документы производства';uk='Документи виробництва'"));
