...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Элементы.ДекорацияПредупреждениеПоНормативам.Заголовок = НСтр("ru='Не учитывается нормативное количество, рассчитываемое по формуле/автовыбором.';uk='Не враховується нормативна кількість, яка розраховується за формулою / автовибором.'");
	
	Если ЗначениеЗаполнено(ЕдиницаИзмерения) Тогда
//++ НЕ УТКА
		Элементы.ЭтапыКоличество.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Количество (%1)';uk='Кількість (%1)'"),
					ЕдиницаИзмерения);
//-- НЕ УТКА
...
Процедура УстановитьУсловноеОформлениеПодразделениеПолучатель()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<совпадает с текущим>';uk='<збігається з поточним>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
