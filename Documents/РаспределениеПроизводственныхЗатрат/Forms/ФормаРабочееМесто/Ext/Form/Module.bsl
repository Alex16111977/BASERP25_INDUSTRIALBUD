...
Процедура ЗатратыФактОстатокПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Документ) И ТекущиеДанные.НовоеПроизводство Тогда
		Текст = НСтр("ru='Распределение выполнено документом для производства версии 2.5. Для работы с документом требуется перейти в рабочее место распределения 2.5.
|Для распределения на производство версии 2.1 существующий документ распределения следует пометить на удаление.'
|;uk='Розподіл виконано документом для виробництва версії 2.5. Для роботи з документом потрібно перейти в робоче місце розподілу 2.5. 
|Для розподілу на виробництво версії 2.1 існуючий документ розподілу слід позначити на вилучення.'");
