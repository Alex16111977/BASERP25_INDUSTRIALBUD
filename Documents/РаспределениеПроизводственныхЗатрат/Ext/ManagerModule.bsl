...
//
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийСтарая(ПараметрыУказанияСерий) Экспорт
...
Функция ТекстЗапросаМатериаловИРаботВКладовых(ВоВременнуюТаблицу = Ложь, ВыбиратьПервые = 0) Экспорт
	...
	|			И ПоступленияРабот.Подразделение = Аналитика.МестоХранения
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	ШаблонПредставления = НСтр("ru='Распределение материала (работы) ""%1"" за %2 (№%3)';uk='Розподіл матеріалу (роботи) ""%1"" за %2 (№%3)'");
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДокВыпуска.НалоговоеНазначение КАК НалоговоеНазначение,
	|	Выпуски.КодСтроки,
	|	НЕОПРЕДЕЛЕНО,
    ...
    |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыпускПродукции КАК ДокВыпуска
    |			ПО ДокВыпуска.Ссылка = Выпуски.ДокументВыпуска
	|ГДЕ
...
Функция ТекстЗапросаВидыЗапасовИРаботы(Запрос, ТекстыЗапроса)
	...
	|			ТОГДА &НалоговоеНазначениеРасходов 
	|			ИНАЧЕ СпрПартииПроизводства.НалоговоеНазначение 
	...
	|	СпрПартииПроизводства.НалоговоеНазначение        КАК КорНалоговоеНазначение
	|ИЗ
	...
	|	НЕОПРЕДЕЛЕНО                                     КАК АналитикаКомитента,
	|	НЕОПРЕДЕЛЕНО                                     КАК АналитикаУчетаПоПартнерам,
	|	&НалоговоеНазначениеРасходов                     КАК КорНалоговоеНазначение
	|ИЗ
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		КОГДА ВидыЗапасов.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ВидыЗапасов.ПартияПроизводства
	|	КОНЕЦ                                            КАК КорПартия,
    ...
    |	ВидыЗапасов.КорНалоговоеНазначение               КАК КорНалоговоеНазначение,
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	...
	|		ТОГДА ВидыЗапасов.КорНалоговоеНазначение                   
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО 
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктИнвентаризацииМатериаловИПолуфабрикатов") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
                                    ...
                                    ТабличныйДокумент,
									,						
									?(ПараметрыПечати.Свойство("НовоеПроизводство"),"Документ.РаспределениеПроизводственныхЗатрат.ПФ_MXL_АктИнвентаризацииМатериаловИПолуфабрикатов2_2","Документ.РаспределениеПроизводственныхЗатрат.ПФ_MXL_АктИнвентаризацииМатериаловИПолуфабрикатов"),
									,
...
Функция СформироватьАктИнвентаризацииМатериаловИПолуфабрикатов(ПараметрыПечати, ОбъектыПечати, ПараметрыВывода)
	...
	Если ПараметрыПечати.свойство("НовоеПроизводство") Тогда
        ...
        Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РаспределениеПроизводственныхЗатрат.ПФ_MXL_АктИнвентаризацииМатериаловИПолуфабрикатов2_2", КодЯзыкаПечать);
	...
	ТаблицаЗатраты = ПараметрыПечати.Затраты.Выгрузить(); // ТаблицаЗначений
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru='Распределение материалов и работ';uk='Розподіл матеріалів і робіт'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru='Очищается вид запасов для работ в ТЧ Виды запасов.';uk='Очищається вид запасів для робіт у ТЧ Види запасів.'");
...
// Обработчик обновления УП 2.5.4:
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСобытие = НСтр("ru='%1. Документы распределения материалов и работ';uk='%1. Документи розподілу матеріалів та робіт'");
			ТекстСобытие = СтрШаблон(ТекстСобытие, ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации());
			
			ЗаписьЖурналаРегистрации(ТекстСобытие,
