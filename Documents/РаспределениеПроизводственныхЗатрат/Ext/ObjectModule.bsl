		...
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			...
			Если ПрочиеРасходы.Количество()
				...
				Запрос.Текст = 
					...
					|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК Реквизиты
					|		ПО ВидыЗапасов.ВидЗапасов = Реквизиты.Ссылка
					|ГДЕ
					|	Реквизиты.ТипЗапасов В(
					|		ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца),
					|		ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца))";
				
				Запрос.УстановитьПараметр("ВидЗапасов", ВидыЗапасов.Выгрузить());
				
				Результат = Запрос.Выполнить();
				Если Не Результат.Пустой() Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Давальческие материалы/продукцию запрещено списывать на расходы.';uk='Давальницькі матеріали/продукцію заборонено списувати на витрати.'"),,,, Отказ);
				КонецЕсли;
...
Функция ВременныеТаблицыДанных() Экспорт
    ...
    |	&НалоговоеНазначениеРасходов                  КАК ЦелевоеНалоговоеНазначение,
	|	0                                             КАК СуммаСНДС,
	|	0                                             КАК СуммаНДС,
    ...
    Запрос.УстановитьПараметр("НалоговоеНазначениеРасходов",    НалоговоеНазначениеРасходов);
