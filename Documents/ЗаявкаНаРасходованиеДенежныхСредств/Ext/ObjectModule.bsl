...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		Если ОтчетПоКомиссии.УдержатьВознаграждение Тогда
			...
			НСтр("ru='Не требуется вводить Заявку на расходование ДС: Вознаграждение на основании документа %1, поскольку в нем удержано вознаграждение';uk='Не потрібно вводити Заявку на витрачання ГК: Винагорода на підставі документа %1, оскільки в ньому утримано винагороду'"),
			ОтчетПоКомиссии);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЖелательнаяДатаПлатежа < НачалоДня(Дата) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст, ЭтотОбъект, "ЖелательнаяДатаПлатежа",, Ложь);
	КонецЕсли;
	//***ERP
...
//***ERP
Процедура ОбновитьЗаявкаНаРасходованиеДенежныхСредствНаПослеОбмена(Док) Экспорт
	
	Если Док.А_ОбработанКазна Тогда
	...
	ПриоритетОплаты = Справочники.ПриоритетыОплаты.НайтиПоНаименованию("Середній");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//***ERP
	ОбновитьЗаявкаНаРасходованиеДенежныхСредствНаПослеОбмена(ЭтотОбъект); 
	//***ERP
