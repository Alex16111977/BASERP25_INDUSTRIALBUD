...
Функция РазделыТестирования() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") Тогда
		РазделыТестирования.Добавить("ВНА", " " + НСтр("ru='Формирование движений по ВНА';uk='Формування рухів за необоротними активами'"), Истина);
	КонецЕсли;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет") Тогда
		РазделыТестирования.Добавить("РеглУчет", " " + НСтр("ru='Отражение в регл. учете';uk='Відображення в регл. обліку'"), Истина);
	КонецЕсли;
	//-- НЕ УТ
	//-- Локализация
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьМеждународныйФинансовыйУчет") Тогда
		РазделыТестирования.Добавить("МеждународныйУчет", " " + НСтр("ru='Отражение в международном учете';uk='Відображення в міжнародному обліку'"), Истина);
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура УстановитьЗаданияКРасчетуНаДату(Период, ВыбранныеРазделы = Неопределено)
	...
	Для Каждого РазделРасчета Из ВыбранныеРазделы Цикл
		...
		Если РазделРасчета = "НДС" Тогда
			...
			// Задания к корректировке налогового назначения капитальных инвестиций
			ВыборкаДокументовКорректировкиНалоговогоНазначенияКапитальныхИнвестиций = ДокументыКорректировкиНалоговогоНазначенияКапитальныхИнвестиций(Период).Выбрать();
			Пока ВыборкаДокументовКорректировкиНалоговогоНазначенияКапитальныхИнвестиций.Следующий() Цикл
				РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьЗаписьРегистра(
					Период,
					, 
					ВыборкаДокументовКорректировкиНалоговогоНазначенияКапитальныхИнвестиций.Организация, 
...
//++ НЕ УТ
Функция ДокументыКорректировкиНалоговогоНазначенияКапитальныхИнвестиций(Период)
	...
	|ГДЕ
	|	КорректировкаНалоговогоНазначенияКапитальныхИнвестиций.Дата МЕЖДУ &НачалоПериода И &КонецПериода
