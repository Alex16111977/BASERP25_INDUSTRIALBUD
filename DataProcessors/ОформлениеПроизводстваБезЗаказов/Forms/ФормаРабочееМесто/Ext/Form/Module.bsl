...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст",  НСтр("ru='<основная спецификация>';uk='<основна специфікація>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<без серии>';uk='<без серії>'"));
...
Процедура УстановитьОформлениеИтогиРассчитаны()
	...
	Если Не ИтогиРассчитаны Тогда
		ТекстЗаголовка = НСтр("ru='Не рассчитаны итоги за: %1. Отбор по периоду не используется.';uk='Не розраховані підсумки за: %1. Відбір за періодом не використовується.'");
...
Процедура ОбработатьСформированныеДокументы(Результат)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru='Не сформировано ни одного документа.';uk='Не сформовано жодного документа.'"));
...
Процедура ОбновитьОтборы()
	...
	СтруктураОтборов.Вставить("ДатаОкончания",				Новый Структура("Тип, Значение", "Параметр", КонецМесяца(ПериодРегистрации)));
	СтруктураОтборов.Вставить("ПериодРегистрации",			Новый Структура("Тип, Значение", "Параметр", ПараметрПериод(ПериодРегистрации, ИтогиРассчитаны)));
	СтруктураОтборов.Вставить("ТекстНастройкиНеЗаданы",		Новый Структура("Тип, Значение", "Параметр", НСтр("ru='<Настройки не заданы>';uk='<Настройки не задані>'")));
	СтруктураОтборов.Вставить("Организация",				Новый Структура("Тип, Значение", "Отбор", Организация));
	СтруктураОтборов.Вставить("Подразделение",				Новый Структура("Тип, Значение", "Отбор", Подразделение));
	СтруктураОтборов.Вставить("Номенклатура",				Новый Структура("Тип, Значение", "Отбор", Номенклатура));
	СтруктураОтборов.Вставить("Назначение",					Новый Структура("Тип, Значение", "Отбор", Назначение));
...
Процедура КОформлениюОбщиеОформитьСогласноНастройке(ИмяТЧ)
	...
	Для Каждого ЭлКоллекции Из ВыделенныеСтроки Цикл
		...
		СтруктураИзделия = Новый Структура("Организация, ГруппировкаЗатрат,
											...
											|Характеристика, Серия, Назначение, НаправлениеДеятельности, МодельФормированияСтоимости,
	...
	ТекстВопроса = НСтр("ru='При проверке настроек модели формирования стоимости возникли ошибки:';uk='При перевірці настройок моделі формування вартості виникли помилки:'");
	ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru='Не указана модель формирования стоимости (%1 из %2)';uk='Не вказано модель формування вартості   %1 з %2)'");
	...
	Если НеУказанаМодель = ВыделенныеСтроки.Количество() Тогда
		ПоказатьПредупреждение(,НСтр("ru='Ни для одной строки не заданы настройки. Не будет сформировано ни одного документа.';uk='Для жодного рядка не задані настройки. Не буде сформовано жодного документа.'"));
		Возврат;
	КонецЕсли;
	
	Если Не НеУказанаМодель = 0 Тогда
		
		ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru='Документы по таким строкам сформированы не будут. Продолжить?';uk='Документи по таких рядках сформовані не будуть. Продовжити?'");
...
Процедура КОформлениюОбщиеОбновить(Элемент)
	...
	// СтандартныеПодсистемы.ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"Обработка.ОформлениеПроизводстваБезЗаказов.Форма.ФормаРабочееМесто.Команда." + Элемент.Имя + "Обновить");
	// Конец СтандартныеПодсистемы.ЗамерПроизводительности
