...
Процедура ТаблицаПотребностейПоЗаказамИОстатков(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.ТипОтбора <> "ОтборПоЗначениям" Тогда
		...
		Если ЗначениеЗаполнено(ТаблицаПолуфабрикатов) Тогда
			...
			Если ОптимизироватьЗапасыРаспределительногоЦентра Тогда
				
				НовыйМассив = ТаблицаПолуфабрикатов.ВыгрузитьКолонку("Количество");
				ТаблицаПолуфабрикатов.Колонки.Добавить("КоличествоОстаткиНаРЦ");
				ТаблицаПолуфабрикатов.ЗагрузитьКолонку(НовыйМассив, "КоличествоОстаткиНаРЦ");
				УдалитьСтрокиИзТаблицы(Таблица, ТаблицаПолуфабрикатов, "Требуется", "КоличествоОстаткиНаРЦ",
					"Заказ,Номенклатура,Характеристика,Склад,Назначение", Ложь);
				
			КонецЕсли;
			
			УдалитьСтрокиИзТаблицы(Таблица, ТаблицаПолуфабрикатов, "Требуется", "Количество",
				"Заказ,Номенклатура,Характеристика,Склад,Назначение", Истина);
...
//++ НЕ УТКА
Процедура УдалитьСтрокиИзТаблицы(Таблица, ТаблицаПолуфабрикатов, ИмяПоляКоличество, ИмяПоляКоличествоИсточник, КлючПоиска, УдаленныйСклад = Неопределено)
	...
	Для Счетчик = 1 По ВсегоСтрок Цикл
		...
		Если УдаленныйСклад = Истина И СтрокаТаблицы.Склад <> СтрокаТаблицы.СкладОтгрузкиЗаказа
				Или УдаленныйСклад = Ложь И СтрокаТаблицы.Склад = СтрокаТаблицы.СкладОтгрузкиЗаказа Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаТаблицы);
		Если УдаленныйСклад <> Неопределено Тогда
			Отбор.Склад = СтрокаТаблицы.СкладОтгрузкиЗаказа;
		КонецЕсли;
		НайденныеСтроки = ТаблицаПолуфабрикатов.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			КоличествоРаспределить = Мин(НайденныеСтроки[0][ИмяПоляКоличествоИсточник], СтрокаТаблицы[ИмяПоляКоличество]);
			НайденныеСтроки[0][ИмяПоляКоличествоИсточник] = НайденныеСтроки[0][ИмяПоляКоличествоИсточник] - КоличествоРаспределить;
...
Функция ВременнаяТаблицаСпособовОбеспечения(ТоварыПоддерживаемогоЗапаса)
	...
	ТекстЗапроса =
		...
		//++ НЕ УТКА
		+ СкорректироватьВременныеТаблицыОстатковТоваровИРаботНаВеличинуПолуфабрикатовПроизводимыхВПроцессе()
		//-- НЕ УТКА
		...
		//++ НЕ УТКА
		ЗаменитьИменаТаблицОстатковТоваровИРабот(ТекстЗапроса);
		//-- НЕ УТКА
...
Функция ВременнаяТаблицаЗаказыКПоступлениюИОтгрузке(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =
		...
		|	МАКСИМУМ(
		//++ НЕ УТ
		|		ЕСТЬNULL(ЗаказыМатериалов.Склад,
		//-- НЕ УТ
		...
		|		ПО ЗаказыМатериалов.Ссылка = ДвижениеТоваров.Распоряжение
		//-- НЕ УТ
		|		
...
Функция ТаблицаПолуфабрикатов(МенеджерВременныхТаблиц)
	...
	Возврат РегистрыНакопления.ОбеспечениеПроизводственныхПроцессов.ПолуфабрикатыВПроизводстве(
		МенеджерВременныхТаблиц, "ТоварыИСкладыПопавшиеВОбработку");
	
КонецФункции

Функция СкорректироватьВременныеТаблицыОстатковТоваровИРаботНаВеличинуПолуфабрикатовПроизводимыхВПроцессе()
	
	Тексты = Новый Массив();
	
	ИменаВременныхТаблиц = Новый Структура();
	
	ИменаВременныхТаблиц.Вставить("ОтборПоПотребности", "ВтОстаткиОбособленныхТоваров");
	ИменаВременныхТаблиц.Вставить("Результат",          "ВтПолуфабрикатыТовары");
	Текст = РегистрыНакопления.ОбеспечениеПроизводственныхПроцессов.ТекстЗапросаПолуфабрикатыВПроизводстве(ИменаВременныхТаблиц, "Товары");
	Тексты.Добавить(Текст);
	
	ИменаВременныхТаблиц.Вставить("ОтборПоПотребности", "ВтОстаткиРабот");
	ИменаВременныхТаблиц.Вставить("Результат",          "ВтПолуфабрикатыРаботы");
	Текст = РегистрыНакопления.ОбеспечениеПроизводственныхПроцессов.ТекстЗапросаПолуфабрикатыВПроизводстве(ИменаВременныхТаблиц, "Работы");
	Тексты.Добавить(Текст);
	...
	Текст =
		...
		|	Таблица.Склад                                          КАК Склад,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК Подразделение
		|ПОМЕСТИТЬ ТоварыБезПолуфабрикатовВПроцессе
		|ИЗ
		|	ВтОстаткиОбособленныхТоваров КАК Таблица
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПолуфабрикатыТовары КАК Полуфабрикаты
		|		ПО Полуфабрикаты.Номенклатура = Таблица.Номенклатура
		...
		|		 И Полуфабрикаты.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
		|СГРУППИРОВАТЬ ПО
		...
		|	Таблица.Склад
		|ИМЕЮЩИЕ
		|	МАКСИМУМ(Таблица.Количество) - СУММА(ЕСТЬNULL(Полуфабрикаты.Количество, 0)) > 0";
	Тексты.Добавить(Текст);
	...
	Текст =
		...
		|	ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад
		|ПОМЕСТИТЬ РаботыБезПолуфабрикатовВПроцессе
		|ИЗ
		|	ВтОстаткиРабот КАК Таблица
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтПолуфабрикатыРаботы КАК Полуфабрикаты
		|		ПО Полуфабрикаты.Номенклатура = Таблица.Номенклатура
		|		 И Полуфабрикаты.Характеристика = Таблица.Характеристика
		|		 И Полуфабрикаты.Назначение = Таблица.Назначение
		|		 И Полуфабрикаты.Подразделение = Таблица.Подразделение
		|		 И Полуфабрикаты.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|СГРУППИРОВАТЬ ПО
		|	Таблица.Номенклатура,
		|	Таблица.Характеристика,
		|	Таблица.Назначение,
...
Процедура ЗаменитьИменаТаблицОстатковТоваровИРабот(ТекстЗапроса)
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВтОстаткиОбособленныхТоваров",     "ВтОстаткиОбособленныхТоваровИПолуфабрикатыВПроцессе");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВтОстаткиРабот",                   "ВтОстаткиРаботИПолуфабрикатыВПроцессе");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТоварыБезПолуфабрикатовВПроцессе", "ВтОстаткиОбособленныхТоваров");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РаботыБезПолуфабрикатовВПроцессе", "ВтОстаткиРабот");
...
//++ НЕ УТКА
//++ Устарело_Производство21
Функция ВременнаяТаблицаКорректировкаЗаказаНаПроизводствоРаботФиктивная()
	
	Возврат
		"ВЫБРАТЬ
		|	NULL КАК Ссылка,
		|	NULL КАК Распоряжение
		|ПОМЕСТИТЬ КорректировкаЗаказаНаПроизводствоРабот";
...
//++ НЕ УТКА
//++ Устарело_Производство21
Функция ВременнаяТаблицаКорректировкаЗаказаНаПроизводствоРабот()
...
//++ Устарело_Производство21
		...
		|			
		|			ЛЕВОЕ СОЕДИНЕНИЕ КорректировкаЗаказаНаПроизводствоРабот КАК КорректировкиЗаказов
