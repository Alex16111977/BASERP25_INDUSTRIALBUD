...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводство") Тогда
		Элементы.ПартнерСклад.Заголовок = НСтр("ru='Партнер/Склад/Подразделение';uk='Партнер/Склад/Підрозділ'");
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ЗаказПереработчикуКИсполнению(Команда)
	...
	Если ЗначениеЗаполнено(МассивСсылок) Тогда
		...
		ОбновитьНадписьПредупреждениеОРазмещенииЗаказов();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно установить статус. Не выбраны заказы переработчику';uk='Неможливо встановити статус. Не вибрані замовлення переробнику'"));
	КонецЕсли;
	//-- НЕ УТ
	Возврат;
...
Процедура ЗаказНаПроизводствоКПроизводству(Команда)
	...
	Если ЗначениеЗаполнено(МассивСсылок) Тогда
		...
		ОбновитьНадписьПредупреждениеОРазмещенииЗаказов();
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Невозможно установить статус. Не выбраны заказы на производство';uk='Неможливо встановити статус. Не вибрані замовлення на виробництво'"));
	КонецЕсли;
	//-- НЕ УТКА
	Возврат;
