...
Процедура ЗаполнитьДоступностьПоГрафикамРаботы(СписокВидовРЦ)
	...
	Для каждого ВидРабочегоЦентра Из СписокВидовРЦ Цикл
		...
		Если СписокСтрок.Количество() = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Для вида рабочих центров ""%1"" не введены рабочие центры.';uk='Для виду робочих центрів ""%1"" не введені робочі центри.'"),
				ВидРабочегоЦентра);
		...
		Для каждого ДанныеРЦ Из СписокСтрок Цикл
			...
			Иначе
				ТекстСообщения = СтрШаблон(НСтр("ru='Для рабочего центра ""%1"" не указан график работы.';uk='Для робочого центру ""%1"" не вказаний графік роботи.'"),
