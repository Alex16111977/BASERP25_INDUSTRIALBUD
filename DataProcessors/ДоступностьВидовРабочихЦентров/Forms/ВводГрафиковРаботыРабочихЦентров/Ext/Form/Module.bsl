...
Процедура НастроитьФормуПоПараметрамВидаРЦ()
	...
	Если НЕ ИтогиВыводятсяВПодвалеТаблицы Тогда
		...
		Элементы.ДанныеРЦ.ВысотаПодвала = 1;
		Элементы.ДанныеРЦРабочийЦентр.ТекстПодвала = НСтр("ru='Итого:';uk='Всього:'");
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		Если ВыделеннаяСтрока <> Неопределено Тогда
			...
			Если Графики = Неопределено Тогда
				...
				ТекстОшибки = СтрШаблон(НСтр("ru='В строке %1 не указан график работы.';uk='У рядку %1 не вказано графік роботи.'"), НомерСтроки);
			...
			Если Не ЗначениеЗаполнено(ВыделеннаяСтрока.РабочийЦентр) Тогда
				...
				ТекстОшибки = СтрШаблон(НСтр("ru='В строке %1 не указан рабочий центр.';uk='У рядку %1 не вказано робочий центр.'"), НомерСтроки);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					,
