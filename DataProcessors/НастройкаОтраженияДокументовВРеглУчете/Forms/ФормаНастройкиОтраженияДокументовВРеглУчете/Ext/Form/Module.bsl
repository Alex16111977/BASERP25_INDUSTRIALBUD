...
Процедура ОтразитьДокументыВРегламентированномУчете(Команда)
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='При отражении документов в регл. учете возникли ошибки. Подробнее см. в журнале регистрации.';uk='При відображенні документів в регл. обліку виникли помилки. Детальніше див. у журналі реєстрації.'"));
	КонецЕсли;
...
Процедура ОповеститьОбОтраженииДокументовВРеглУчете(ДанныеПоОтражениюВУчете)
	...
	ТекстЗаголовок = СтрШаблон(НСтр("ru='Обработано %1';uk='Опрацьовано %1'"), ЧислоДокументовПрописью(ДанныеПоОтражениюВУчете.Отражено + ДанныеПоОтражениюВУчете.НеУказаныСчетаУчета ));
	
	Если ДанныеПоОтражениюВУчете.Отражено > 0 Тогда
		МассивСообщений.Добавить(СтрШаблон(НСтр("ru='Отражено в регл. учете %1';uk='Відображено у регл. обліку %1'"), ЧислоДокументовПрописью(ДанныеПоОтражениюВУчете.Отражено)));
	КонецЕсли;
	Если ДанныеПоОтражениюВУчете.НеУказаныСчетаУчета > 0 Тогда
		МассивСообщений.Добавить(СтрШаблон(НСтр("ru='Не отражены из-за ненастроенных счетов учета %1';uk='Не відображені через ще не настроєні рахунки обліку %1'"), ЧислоДокументовПрописью(ДанныеПоОтражениюВУчете.НеУказаныСчетаУчета)));
	КонецЕсли;
...
Функция ПредставлениеРасписанияРегламентногоЗадания(Расписание)
	...
	Возврат ?(Расписание = Новый РасписаниеРегламентногоЗадания, НСтр("ru='Расписание не задано';uk='Розклад не заданий'"), Строка(Расписание));
...
Процедура УстановитьСвойстваЭлементовОтраженияПоРасписанию(РазделениеВключено)
	...
	ЧастиЗаголовка.Добавить(НСтр("ru='Автоматическое отражение по расписанию';uk='Автоматичне відображення за розкладом'"));
...
Процедура ОбновитьДанныеПоАвтоматическомуОтражениюВУчете(РезультатВыполненияФоновогоЗадания)
	...
	Если РезультатВыполненияФоновогоЗадания.Статус = "Выполнено" Тогда
		...
		Если ДанныеАвтоматическогоОтраженияВУчете.СвойстваПоследнегоФоновогоЗадания = Неопределено
			...
			СостояниеАвтоматическогоОтраженияВУчете = НСтр("ru='(Не выполнялось)';uk='(Не виконувалося)'");
		Иначе
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Ошибка" Тогда
		ПредставлениеРасписания = НСтр("ru='Получение данных по регламентному заданию завершилось с ошибкой. Подробнее см. в журнале регистрации.';uk='Отримання даних по регламентному завданню завершилося з помилкою. Детальніше див. у журналі реєстрації.'");
	Иначе
		ПредставлениеРасписания = НСтр("ru='Выполняется получение данных по регламентному заданию...';uk='Виконується отримання даних по регламентному завданню ...'");
	КонецЕсли;
...
|за результатами %1.'") + Символы.ПС; 
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Ошибка" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='При получении данных по оффлайн расчетам возникли ошибки. Подробнее см. в журнале регистрации.';uk='При отриманні даних по оффлайн розрахункам виникли помилки. Детальніше див. у журналі реєстрації.'"));
		Элементы.ГруппаПредупреждение.Видимость = Ложь;
	Иначе
		СостояниеОффлайновыхОпераций = Новый ФорматированнаяСтрока(НСтр("ru='Выполняется получение данных по необходимым оффлайн расчетам...';uk='Виконується отримання даних про необхідні оффлайн розрахунки ...'"));
...
Процедура ОбновитьДанныеПоОтражениюИПроверкеДокументовВРеглУчете(РезультатВыполненияФоновогоЗадания, ВидОперации)
	...
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("ОтраженныеВручную",
		НСтр("ru='Документы, скорректированные вручную (%Количество%), по %Дата%';uk='Документи, що скориговані вручну (%Количество%), з %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("НеОтраженныеВручную",
		НСтр("ru='Документы, требующие ручной корректировки и подтверждения (%Количество%), с %Дата%';uk='Документи, що потребують ручного коригування та підтвердження (%Количество%), з %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("Отраженные",
		НСтр("ru='Отраженные документы (%Количество%), по %Дата%';uk='Відображені документи (%Количество%), по %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("НеОтраженные",
		НСтр("ru='Документы, ожидающие автоматического отражения (%Количество%), с %Дата%';uk='Документи, які очікують автоматичного відображення (%Количество%), з %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("ОтраженныеОперациейБух",
		НСтр("ru='Ручные операции регл. учета (%Количество%), по %Дата%';uk='Ручні операції регл. обліку (%Количество%), по %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("НеОтраженныеОперациейБух",
		НСтр("ru='Ручные операции регл. учета, требующие корректировки и подтверждения (%Количество%), с %Дата%';uk='Ручні операції регл. обліку, що потребують коригування та підтвердження (%Количество%), з %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("НеУказаныСчетаУчета",
		НСтр("ru='Документы, не отраженные в учете из-за ненастроенных счетов учета (%Количество%), с %Дата%';uk='Документи, які не відображені в обліку через не настроєні рахунки обліку (%Количество%), з %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("Проверенные",
		НСтр("ru='Проверенные документы (%Количество%), по %Дата%';uk='Перевірені документи (%Количество%), по %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("НеПроверенные",
		НСтр("ru='Документы, требующие проверки (%Количество%), с %Дата%';uk='Документи, що потребують перевірки (%Количество%), з %Дата%'"));
	СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("КПовторнойПроверке",
		НСтр("ru='Документы, требующие повторной проверки (%Количество%), с %Дата%';uk='Документи, що потребують повторної перевірки (%Количество%), з %Дата%'"));
	#КонецОбласти
	...
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("ОтраженныеВручную",
		НСтр("ru='Нет скорректированных вручную документов.';uk='Немає скоригованих вручну документів.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("НеОтраженныеВручную",
		НСтр("ru='Нет документов, требующих ручной корректировки и подтверждения.';uk='Немає документів, що потребують ручного коригування і підтвердження.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("Отраженные",
		НСтр("ru='Нет отраженных документов.';uk='Немає відображених документів.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("НеОтраженные",
		НСтр("ru='Нет документов, ожидающих автоматического отражения.';uk='Немає документів, які очікують автоматичного відображення.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("ОтраженныеОперациейБух",
		НСтр("ru='Нет ручных операций регл. учета.';uk='Немає ручних операцій регл. обліку.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("НеОтраженныеОперациейБух",
		НСтр("ru='Нет ручных операций регл. учета, требующих корректировки и подтверждения.';uk='Немає ручних операцій регл. обліку, які потребують коригування і підтвердження.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("НеУказаныСчетаУчета",
		НСтр("ru='Нет документов, не отраженных из-за ненастроенных счетов учета.';uk='Немає документів, які не відображені через настроєні рахунки обліку.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("Проверенные",
		НСтр("ru='Нет проверенных документов.';uk='Немає перевірених документів.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("НеПроверенные",
		НСтр("ru='Нет документов, требующих проверки.';uk='Немає документів, що потребують перевірки.'"));
	СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("КПовторнойПроверке",
		НСтр("ru='Нет документов, требующих повторной проверки.';uk='Немає документів, що потребують повторної перевірки.'"));
	#КонецОбласти
	...
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("ОтраженныеВручную",
		НСтр("ru='Получение данных о скорректированных вручную документах завершилось с ошибкой.';uk='Отримання даних про скориговані вручну документи завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("НеОтраженныеВручную",
		НСтр("ru='Получение данных о документах, требующих ручной корректировки и подтверждения, завершилось с ошибкой.';uk='Отримання даних про документи, які потребують ручного коригування і підтвердження, завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("Отраженные",
		НСтр("ru='Получение данных о неотраженных документах завершилось с ошибкой.';uk='Отримання даних про невідображенї документи завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("НеОтраженные",
		НСтр("ru='Получение данных о документах, ожидающих автоматического отражения, завершилось с ошибкой.';uk='Отримання даних про документи, які очікують автоматичного відображення, завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("ОтраженныеОперациейБух",
		НСтр("ru='Получение данных о ручных операциях регл. учета завершилось с ошибкой.';uk='Отримання даних про ручні операції регл. обліку завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("НеОтраженныеОперациейБух",
		НСтр("ru='Получение данных о ручных операциях регл. учета, требующих корректировки и подтверждения, завершилось с ошибкой.';uk='Отримання даних про ручні операції регл. обліку, які потребують коригування та підтвердження, завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("НеУказаныСчетаУчета",
		НСтр("ru='Получение данных о документах, не отраженных в учете, завершилось с ошибкой.';uk='Отримання даних про документи, які не відображені в обліку, завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("Проверенные",
		НСтр("ru='Получение данных о непроверенных документах завершилось с ошибкой.';uk='Отримання даних про неперевірені документи завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("НеПроверенные",
		НСтр("ru='Получение данных о документах, требующих проверки, завершилось с ошибкой.';uk='Отримання даних про документи, які потребують перевірки, завершилося з помилкою.'"));
	СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("КПовторнойПроверке",
		НСтр("ru='Получение данных о документах, требующих повторной проверки, завершилось с ошибкой.';uk='Отримання даних про документи, які потребують повторної перевірки, завершилося з помилкою.'"));
	#КонецОбласти
	...
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("ОтраженныеВручную",
		НСтр("ru='Получение скорректированных вручную документов...';uk='Отримання скоригованих вручну документів ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("НеОтраженныеВручную",
		НСтр("ru='Получение документов, требующих ручной корректировки и подтверждения...';uk='Отримання документів, що потребують ручного коригування і підтвердження ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("Отраженные",
		НСтр("ru='Получение отраженных документов...';uk='Отримання відображених документів ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("НеОтраженные",
		НСтр("ru='Получение документов, ожидающих автоматического отражения...';uk='Отримання документів, які очікують автоматичного відображення ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("ОтраженныеОперациейБух",
		НСтр("ru='Получение ручных операций регл. учета...';uk='Отримання ручних операцій регл. обліку ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("НеОтраженныеОперациейБух",
		НСтр("ru='Получение ручных операций регл. учета, требующих корректировки и подтверждения...';uk='Отримання ручних операцій регл. обліку, які потребують коригування та підтвердження ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("НеУказаныСчетаУчета",
		НСтр("ru='Получение документов, не отраженных в учете...';uk='Отримання документів, які не відображені в обліку ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("Проверенные",
		НСтр("ru='Получение проверенных документов...';uk='Отримання перевірених документів ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("НеПроверенные",
		НСтр("ru='Получение документов, требующих проверки...';uk='Отримання документів, що потребують перевірки ...'"));
	СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("КПовторнойПроверке",
		НСтр("ru='Получение документов, требующих повторной проверки...';uk='Отримання документів, що потребують повторної перевірки ...'"));
	#КонецОбласти
	...
	Если ВидОперации = "НеОтраженные" И ДанныеФО.ОтражениеПроверенных Тогда
		СоответствиеСообщенияНаличияДокументовВидуОперации.Вставить("НеОтраженные",
			НСтр("ru='Проверенные документы, ожидающие автоматического отражения (%Количество%), с %Дата%';uk='Перевірені документи, які чекають автоматичного відображення (%Количество%), з %Дата%'"));
		СоответствиеСообщенияОбОтсутствииДокументовВидуОперации.Вставить("НеОтраженные",
			НСтр("ru='Нет проверенных документов, ожидающих автоматического отражения.';uk='Немає перевірених документів, які очікують автоматичного відображення.'"));
		СоответствиеСообщенияОбОшибкеПриВыполненииВидуОперации.Вставить("НеОтраженные",
			НСтр("ru='Получение данных о проверенных документах, ожидающих автоматического отражения, завершилось с ошибкой.';uk='Отримання даних про перевірені документи, які очікують автоматичного відображення, завершилося з помилкою.'"));
		СоответствиеСообщенияОбОбновленииДанныхВидуОперации.Вставить("НеОтраженные",
			НСтр("ru='Получение проверенных документов, ожидающих автоматического отражения...';uk='Отримання перевірених документів, які очікують автоматичного відображення ...'"));
	КонецЕсли;
	#КонецОбласти
...
Процедура УстановитьСостояниеСчетовУчета(МассивОрганизаций)
	...
	Если КоличествоНеобходимыхНастроек > 0 Тогда
		СостояниеСчетовУчета = НСтр("ru='Настроить счета учета (%1)';uk='Настроїти рахунки обліку (%1)'");
	...
	Иначе
		СостояниеСчетовУчета = НСтр("ru='Счета учета не требуют настройки';uk='Рахунки обліку не вимагають настройки'");
		Элементы.ОткрытьФормуНастройкиСчетов.Заголовок = СостояниеСчетовУчета;
...
Функция ЗапуститьВыполнениеВФоне(ВыполняемыйМетод, ПараметрыОбработки)
	...
	СоответствиеНаименованийЗаданийВыполняемымМетодам.Вставить("РеглУчетПроведениеСервер.ОтразитьВсеВФоне",
		НСтр("ru='Отражение документов в регламентированном учете';uk='Відображення документів в регламентованому обліку'"));
	СоответствиеНаименованийЗаданийВыполняемымМетодам.Вставить("Обработки.НастройкаОтраженияДокументовВРеглУчете.СостояниеОффлайновыхРасчетов",
		НСтр("ru='Рабочее место отражения документов в регл. учете: Обновление состояния оффлайновых расчетов';uk='Робоче місце відображення документів в регл. обліку: Оновлення стану офлайнових розрахунків'"));
	СоответствиеНаименованийЗаданийВыполняемымМетодам.Вставить("Обработки.НастройкаОтраженияДокументовВРеглУчете.СостояниеОтраженияДокументов",
		НСтр("ru='Рабочее место отражения документов в регл. учете: Обновление состояния отражения документов';uk='Робоче місце відображення документів в регл. обліку: Оновлення стану відображення документів'"));
	СоответствиеНаименованийЗаданийВыполняемымМетодам.Вставить("Обработки.НастройкаОтраженияДокументовВРеглУчете.СостояниеНастроекСчетовУчета",
		НСтр("ru='Рабочее место отражения документов в регл. учете: Обновление состояния настройки счетов учета';uk='Робоче місце відображення документів в регл. обліку: Оновлення стану настройки рахунків обліку'"));
	СоответствиеНаименованийЗаданийВыполняемымМетодам.Вставить("Обработки.НастройкаОтраженияДокументовВРеглУчете.СостояниеРегламентногоЗадания",
		НСтр("ru='Рабочее место отражения документов в регл. учете: Обновление состояния регламентного задания';uk='Робоче місце відображення документів в регл. обліку: Оновлення стану регламентного завдання'"));
