...
Процедура ДобавитьТекстЗапросаДенежныеСредстваУПодотчетныхЛиц(Запрос, ТаблицаДвижений)
	...
	ИначеЕсли ТипЗнч(ТаблицаДвижений) = Тип("ТаблицаЗначений") Тогда
		...
		|	Т.Организация КАК Организация,
		|	Т.Подразделение КАК Подразделение,
		|	Т.ПодотчетноеЛицо КАК ПодотчетноеЛицо,
		|	Т.СуммаУпр КАК СуммаУпр
		|ПОМЕСТИТЬ ПервичныеДвижения
		|ИЗ
		|	&ИмяТаблицы КАК Т
	...
	|		КОГДА Сотрудники.ОсновноеПодразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И Сотрудники.ОсновноеПодразделение ЕСТЬ НЕ NULL
	|		ТОГДА Сотрудники.ОсновноеПодразделение
	...
	|	ПО Т.ПодотчетноеЛицо = Сотрудники.ФизическоеЛицо
	|		И Сотрудники.ГоловнаяОрганизация = Т.Организация
	|ГДЕ
	|	Т.СуммаУпр <> 0
	|	//&Отбор
	|
