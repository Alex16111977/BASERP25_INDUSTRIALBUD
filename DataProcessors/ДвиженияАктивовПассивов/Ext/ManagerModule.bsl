...
Функция ЗапросДвижений(ПараметрыДвижений, ПараметрыОтладки = Неопределено) Экспорт
	...
	Если ЗапросДвижений.Параметры.ВсеТаблицы.Количество() > 0 Тогда
		...
		КонецЕсли;
		//-- НЕ УТ
		//++ Локализация — РКО ВКассу: per СтрПредпр движения ПАП (24 строки вместо 2 типовых)
		ДобавитьТекстЗапросаПАП_РКО_ВКассу(ЗапросДвижений);
		//-- Локализация
...
Функция БалансовыеРегистры(ВключаяОнлайнРасчеты = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") ИЛИ ПолучитьФункциональнуюОпцию("БазоваяВерсия") Тогда
		...
		Исключения.Вставить("ПрочиеРасходыНезавершенногоПроизводства", Истина);
		// {А_Доработка} Не исключать АмортизацияОС — нужно для управленческого баланса (читаем из ПрочиеРасходы)
...
Процедура ДобавитьТекстЗапросаДенежныеСредства(Запрос, ТаблицаДвижений, ТипДС)
	...
	|			ИЛИ Т.Подразделение ЕСТЬ NULL
	|		ТОГДА МестаХраненияДС.Подразделение
...
Процедура ДобавитьТекстЗапросаДенежныеСредстваВПути(Запрос, ТаблицаДвижений)
	...
	|		ТОГДА ДокСписание.Подразделение
	...
	|		ТОГДА ДокПоступление.Подразделение
	...
	|		ТОГДА ПолучательБанк.Подразделение
	...
	|		ТОГДА ПолучательКасса.Подразделение
	...
	|		ТОГДА ОтправительБанк.Подразделение
	...
	|		ТОГДА ОтправительКасса.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ) КАК Подразделение,
	|	ВЫБОР
	|		КОГДА ПолучательБанк.НаправлениеДеятельности ЕСТЬ НЕ NULL
	|		ТОГДА ПолучательБанк.НаправлениеДеятельности
	|		КОГДА ПолучательКасса.НаправлениеДеятельности ЕСТЬ НЕ NULL
	|		ТОГДА ПолучательКасса.НаправлениеДеятельности
	|		КОГДА ОтправительБанк.НаправлениеДеятельности ЕСТЬ НЕ NULL
	|		ТОГДА ОтправительБанк.НаправлениеДеятельности
	|		КОГДА ОтправительКасса.НаправлениеДеятельности ЕСТЬ НЕ NULL
	|		ТОГДА ОтправительКасса.НаправлениеДеятельности
	...
	|	ПолучательБанк.НаправлениеДеятельности,
	|	ПолучательКасса.НаправлениеДеятельности,
	|	ОтправительБанк.НаправлениеДеятельности,
	|	ОтправительКасса.НаправлениеДеятельности";
...
Процедура ДобавитьТекстЗапросаДенежныеСредстваУПодотчетныхЛиц(Запрос, ТаблицаДвижений)
	...
	|	ВЫБОР
	|		КОГДА Т.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И Т.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА Т.Подразделение
	|		КОГДА ДокПриобретение.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И ДокПриобретение.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА ДокПриобретение.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ КАК Подразделение,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельности,
	...
	|ИЗ
	|	ПервичныеДвижения КАК Т
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг КАК ДокПриобретение
	|	ПО Т.Регистратор = ДокПриобретение.Ссылка
	|ГДЕ
	|	Т.СуммаУпр <> 0
	|	//&Отбор
	|
...
Процедура ДобавитьТекстЗапросаРасчетыСПартнерами(Запрос, ТаблицаДвижений, ТипРасчетов)
	...
	|	ВЫБОР КОГДА Т.ОбъектРасчетов.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) ТОГДА Т.ОбъектРасчетов.Договор.Подразделение ИНАЧЕ Т.ОбъектРасчетов.Подразделение КОНЕЦ КАК Подразделение,
	...
	|	Т.ВидДвижения,
	|	ВЫБОР КОГДА Т.ОбъектРасчетов.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) ТОГДА Т.ОбъектРасчетов.Договор.Подразделение ИНАЧЕ Т.ОбъектРасчетов.Подразделение КОНЕЦ,
...
Процедура ДобавитьТекстЗапросаРасчетыСПартнерамиПоДокументам(Запрос, ТаблицаДвижений, ТипРасчетов)
	...
	|	Т.Период КАК Период,
	|	АналитикаПартнеров.Организация КАК Организация,
	|	ВЫБОР КОГДА Т.ЗаказКлиента.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) ТОГДА АналитикаПартнеров.Договор.Подразделение ИНАЧЕ Т.ЗаказКлиента.Подразделение КОНЕЦ КАК Подразделение,
	|	АналитикаПартнеров.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР
	...
	|	АналитикаПартнеров.Организация,
	|	Т.ВидДвижения,
	|	ВЫБОР КОГДА Т.ЗаказКлиента.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) ТОГДА АналитикаПартнеров.Договор.Подразделение ИНАЧЕ Т.ЗаказКлиента.Подразделение КОНЕЦ,
	|	АналитикаПартнеров.НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА АналитикаПартнеров.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|			ТОГДА &СтатьяДолгаСобственныхОрганизаций
...
Процедура ДобавитьТекстЗапросаПрочиеДоходы(Запрос, ТаблицаДвижений)
	...
	|			И Т.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА Т.Подразделение
	|		КОГДА ДоговорКонтрагента.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И ДоговорКонтрагента.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА ДоговорКонтрагента.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ КАК Подразделение,
	|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
...
Процедура ДобавитьТекстЗапросаПрочиеРасходы(Запрос, ТаблицаДвижений)
	...
	|						И ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) <> ТИП(Справочник.ОбъектыЭксплуатации)
	|						И ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) <> ТИП(Справочник.ОбъектыСтроительства)
	|						И ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) <> ТИП(Справочник.НематериальныеАктивы)
	...
	|		// {А_Доработка} v2.2: заполнить аналитику по ОбъектыЭксплуатации для управленческого баланса
	|		КОГДА СтатьиРасходов.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|				И ТИПЗНАЧЕНИЯ(Т.АналитикаРасходов) = ТИП(Справочник.ОбъектыЭксплуатации)
	|			ТОГДА Т.АналитикаРасходов
	|		ИНАЧЕ
	...
	|			И Т.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА Т.Подразделение
	|		КОГДА ДоговорКонтрагента2.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И ДоговорКонтрагента2.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА ДоговорКонтрагента2.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ КАК Подразделение,
	|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПрибылиИУбытки) КАК Статья,
	|	ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|	СУММА(ВЫБОР КОГДА (Т.Сумма - Т.СуммаУпр) > 0
	|		ТОГДА Т.Сумма - Т.СуммаУпр
	|		ИНАЧЕ -(Т.Сумма - Т.СуммаУпр)
	|	КОНЕЦ) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА (Т.Сумма - Т.СуммаУпр) > 0
	|		ТОГДА Т.Сумма - Т.СуммаУпр
	|		ИНАЧЕ -(Т.Сумма - Т.СуммаУпр)
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ (
	|		ВЫБРАТЬ
	|			ДК2.Регистратор КАК Регистратор,
	|			ДК2.Период КАК Период,
	|			ДК2.Организация КАК Организация,
	|			МАКСИМУМ(ДК2.Договор) КАК Договор
	|		ИЗ
	|			РегистрНакопления.ДвиженияКонтрагентДоходыРасходы КАК ДК2
	|		СГРУППИРОВАТЬ ПО
	|			ДК2.Регистратор,
	|			ДК2.Период,
	|			ДК2.Организация
	|	) КАК ДвиженияКонтрагент2
	|	ПО Т.Регистратор = ДвиженияКонтрагент2.Регистратор
	|		И Т.Период = ДвиженияКонтрагент2.Период
	|		И Т.Организация = ДвиженияКонтрагент2.Организация
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорКонтрагента2
	|	ПО ДвиженияКонтрагент2.Договор = ДоговорКонтрагента2.Ссылка
	|ГДЕ
...
Процедура ДобавитьТекстЗапросаСебестоимостьТоваров(Запрос, ТаблицаДвижений)
	...
	|			ТОГДА ВЫБОР
	|				КОГДА Аналитика.СкладскаяТерритория.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|					ИЛИ Аналитика.СкладскаяТерритория.Подразделение ЕСТЬ NULL
	|				ТОГДА Аналитика.МестоХранения.Подразделение
...
Процедура ДобавитьТекстЗапросаТоварыКОформлениюОтчетовКомитенту(Запрос, ТаблицаДвижений)
	...
	|	ВЫБОР
	|		КОГДА Договоры.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И Договоры.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА Договоры.Подразделение
	|		КОГДА ДоговорыИнтеркампани.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И ДоговорыИнтеркампани.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА ДоговорыИнтеркампани.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ КАК Подразделение,
...
// АмортизацияОС
// {А_Доработка} Переписана процедура: читаем из ПрочиеРасходы вместо пустого регистра АмортизацияОС.
// В v2.1 регистр накопления АмортизацияОС пуст. Данные амортизации находятся в ПрочиеРасходы.
// Используем СуммаРегл (бухгалтерская амортизация) т.к. Сумма(УУ) = 0, а ИспользоватьАмортизациюБухУУ = Истина.
// ВидДвижения: РАСХОД (амортизация уменьшает стоимость ОС в управленческом балансе).
Процедура ДобавитьТекстЗапросаАмортизацияОС(Запрос, ТаблицаДвижений)

	ИмяТаблицы = "АмортизацияОС";
	ПервичныеДвижения = "РегистрНакопления.ПрочиеРасходы";
	...
	|	Т.АналитикаРасходов КАК Аналитика,
	|	СУММА(Т.СуммаРегл) КАК Сумма,
	|	СУММА(-Т.СуммаРегл) КАК СуммаБаланса,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковДвижений.ДвиженияБаланса) КАК ВидИсточника,
	|	ЗНАЧЕНИЕ(Перечисление.ИсточникиУправленческогоБаланса.АмортизацияОС) КАК Источник,
	|	ЛОЖЬ КАК РасчетСебестоимости
	|ПОМЕСТИТЬ АмортизацияОС
	|ИЗ
	|	ПервичныеДвижения КАК Т
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.АмортизацияОС)
...
Функция ДобавитьТекстЗапросаПрочиеАктивыПассивы(Запрос, ТаблицаДвижений, ТолькоДС = Ложь)
	...
	|			И Т.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА Т.Подразделение
	|		КОГДА ДокРеализация.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			И ДокРеализация.Подразделение ЕСТЬ НЕ NULL
	|		ТОГДА ДокРеализация.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	КОНЕЦ КАК Подразделение,
	|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	Т.Статья КАК Статья,
	...
	|ИЗ
	|	ПервичныеДвижения КАК Т
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК ДокРеализация
	|	ПО Т.Регистратор = ДокРеализация.Ссылка
	|ГДЕ
	|	Т.Сумма <> 0
	|	//&Отбор
	|
...
Процедура ДобавитьТекстЗапросаДвиженийДенежныхСредств(Запрос)
	...
	Запрос.УстановитьПараметр("ДвижениеКосвенныхДС", НСтр("ru='Движение косвенных ДС';uk='Рух непрямих ГК'"));
	Запрос.УстановитьПараметр("ВыделениеКосвенныхДС", НСтр("ru='Выделение косвенных ДС';uk='Виділення непрямих ГК'"));
	Запрос.УстановитьПараметр("ДвижениеПрямыхДС", НСтр("ru='Движения прямых ДС';uk='Рухи прямих ГК'"));
	Запрос.УстановитьПараметр("ДвиженияПрямыхДСпоПусто", НСтр("ru='Движения прямых ДС по пусто';uk='Рухи прямих ГК по порожньо'"));
...
Процедура ДобавитьТекстЗапросаДвиженийАктивовПассивов(Запрос)
	...
	|ИЗ
	|	ДвиженияНаправлений КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПАП_РКО_ВКассу_Регистраторы КАК ВКасРег
	|		ПО ВКасРег.Регистратор = Т.Регистратор
	|ГДЕ
	|	&Отбор
	|	И ВКасРег.Регистратор ЕСТЬ NULL";
	//++ НЕ УТ
	Если ПолучитьФункциональнуюОпцию("ФормироватьФинансовыйРезультат") Тогда
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		// ▼▼▼ Локализация — наша подмена per СтрПредпр (24 строки)
		|ВЫБРАТЬ
		|	Т.Регистратор КАК Регистратор,
		|	Т.ВидДвижения КАК ВидДвижения,
		|	Т.Период КАК Период,
...
//++ Локализация — РКО с нашей ВедомостьНаВыплатуЗарплатыВКассу:
// per СтрПредпр движения ПрочиеАктивыПассивы из ТЧ.А_РасшифровкаВыплатыЗарплатаПоФизлицам.
// Источник истины подразделения — Рш.Подразделение.А_Подразделение (см. v2-архитектуру в А_ОтражениеЗПпоКазне).
// Формирует две вспомогательные ВТ для подключения через ДобавитьТекстЗапросаДвиженийАктивовПассивов:
//   ПАП_РКО_ВКассу_Регистраторы — список РКО под жёстким гардом (3 условия одновременно).
//   ПАП_РКО_ВКассу_Замена       — 12 пар (Приход ОТ, Расход ДенежныеСредстваНаличные) per СтрПредпр.
// Финальная сборка исключает наши РКО из типовых движений и подмешивает наши 24 строки UNION ALL.
Процедура ДобавитьТекстЗапросаПАП_РКО_ВКассу(Запрос)

	// FIX 2026-05-28: источник регистраторов — ДвиженияНаправлений (не КонтрольБаланса).
	// КонтрольБаланса имеет ИМЕЮЩИЕ СУММА(СуммаБаланса) <> 0 — после §3.12 (12 ДенСрНал per СтрПредпр)
	// баланс per (Регистратор, НаправлениеДеятельности) сходится в 0 → ВТ пуста → наша Замена не активна.
	// ДвиженияНаправлений содержит все регистраторы пайплайна без агрегатных фильтров.
	ТекстЗапросаПАП_РКО_ВКассу =
	"// Регистраторы — РКО с нашей ВКассу, удовлетворяющие жёсткому гарду
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РКО.Ссылка КАК Регистратор
	|ПОМЕСТИТЬ ПАП_РКО_ВКассу_Регистраторы
	|ИЗ
	|	ДвиженияНаправлений КАК ДН
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РасходныйКассовыйОрдер КАК РКО
	|		ПО РКО.Ссылка = ДН.Регистратор
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(РКО.Ведомость) = ТИП(Документ.ВедомостьНаВыплатуЗарплатыВКассу)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// 12 Приход ПАП(ОплатаТруда) + 12 Расход ПАП(ДенежныеСредстваНаличные) per (Подр СтрПредпр, НапрДеят)
	|ВЫБРАТЬ
	|	РКО.Ссылка КАК Регистратор,
	...
	|ПОМЕСТИТЬ ПАП_РКО_ВКассу_Замена
	|ИЗ
	|	Документ.РасходныйКассовыйОрдер КАК РКО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПАП_РКО_ВКассу_Регистраторы КАК Регистраторы
	|			ПО Регистраторы.Регистратор = РКО.Ссылка
	...
	|ГДЕ
	|	(РКО.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику)
	...
	ДобавитьЗначениеВПараметр(Запрос, "ТекстыЗапросов", ТекстЗапросаПАП_РКО_ВКассу);
	ДобавитьЗначениеВПараметр(Запрос, "ВсеТаблицы", "ПАП_РКО_ВКассу_Регистраторы");
	ДобавитьЗначениеВПараметр(Запрос, "ВсеТаблицы", "ПАП_РКО_ВКассу_Замена");
