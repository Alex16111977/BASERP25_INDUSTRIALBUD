...
Процедура ДобавитьИнструменты(Инструменты) Экспорт
	...
	ДобавитьИнструментFindEnumRef(Инструменты);
...
Функция ВыполнитьИнструмент(ИмяИнструмента, Параметры) Экспорт
	...
	ИначеЕсли ИмяИнструмента = "find_enum_ref" Тогда
		Возврат НайтиСсылкуЗначенияПеречисления(Параметры);
...
Процедура ДобавитьИнструментFindEnumRef(Инструменты)
	...
	МассивПараметров.Добавить(mcp_Метаданные.ПараметрИнструмента(
		"enum_type",
		"string",
		"Ім'я перечислення (ТОЧНЕ, з метаданих). Приклад: ХозяйственныеОперации",
		,
		Истина
	));
	...
	МассивПараметров.Добавить(mcp_Метаданные.ПараметрИнструмента(
		"synonym",
		"string",
		"Синонім значення перечислення (як відображається в інтерфейсі). Приклад: Ввод остатков задолженности подотчетников",
		,
		Истина
	));
	...
	mcp_Метаданные.ДобавитьИнструмент(
		Инструменты,
		"find_enum_ref",
		"Знайти UUID значення перечислення за синонімом (назвою в інтерфейсі). Результат UUID можна передавати в update_document як значення реквізиту-перечислення.",
		СхемаПараметров
	);
	КонецПроцедуры
...
Функция НайтиСсылкуЗначенияПеречисления(Параметры)
	
	Попытка
		ИмяПеречисления = Параметры.enum_type;
		СинонимЗначения  = Параметры.synonym;
		
		// Перевіряємо існування перечислення в метаданих
		Если Метаданные.Перечисления.Найти(ИмяПеречисления) = Неопределено Тогда
			ВызватьИсключение "Перечисление не найдено в метаданных: " + ИмяПеречисления;
		КонецЕсли;
		
		// КРИТИЧНО: ПРЕДСТАВЛЕНИЕ() нельзя использовать в WHERE (неограниченная строка)!
		// Решение: выбрать ВСЕ значения, ПРЕДСТАВЛЕНИЕ в SELECT, итерация в BSL
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
		...
		// КРИТИЧНО: Ссылка перечисления НЕ имеет УникальныйИдентификатор()!
		// XMLСтрока(Ссылка) возвращает программное имя (prog_name) — это и есть идентификатор.
		// Для установки значения в update_document используй xml_value через ОбщегоНазначения.
		Пока Выборка.Следующий() Цикл
			Если Выборка.Синоним = СинонимЗначения Тогда
				ПрогИмя  = XMLСтрока(Выборка.Ссылка);  // напр. "АвансовыйОтчет"
				ЗначXML  = ОбщегоНазначения.ЗначениеВСтрокуXML(Выборка.Ссылка);
				
				РезультатПоиска = Новый Структура;
				РезультатПоиска.Вставить("found",      Истина);
				РезультатПоиска.Вставить("uuid",       ПрогИмя);  // программное имя как ID
				РезультатПоиска.Вставить("prog_name",  ПрогИмя);
				РезультатПоиска.Вставить("xml_value",  ЗначXML);  // для update_document
				РезультатПоиска.Вставить("enum_type",  ИмяПеречисления);
				РезультатПоиска.Вставить("synonym",    Выборка.Синоним);
				РезультатПоиска.Вставить("order",      Выборка.Порядок);
				
				Возврат ПреобразоватьРезультатВМассив(РезультатПоиска);
			КонецЕсли;
		КонецЦикла;
		
		// Не нашли — возвращаем found: false
		РезультатПоиска = Новый Структура;
		РезультатПоиска.Вставить("found",     Ложь);
		РезультатПоиска.Вставить("enum_type", ИмяПеречисления);
		РезультатПоиска.Вставить("synonym",   СинонимЗначения);
		РезультатПоиска.Вставить("error",     "Значення перечислення не знайдено за синонімом");
...
Функция НайтиСсылкуЭлементаСправочника(Параметры)
	...
	Попытка
		...
		// ✅ ДлинаКода = 0 — у справочника нет поля Код (Организации, Пользователи и др.)
		ИмеетПолеКод = МетаСправочник.ДлинаКода > 0;
		
		ИмеетПолеЭтоГруппа = МетаСправочник.Иерархический 
			И МетаСправочник.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов;
		
		// Если нет поля Код — переключаем на поиск по наименованию
		Если ПоискПоКоду И НЕ ИмеетПолеКод Тогда
			Если ПоискПоНаименованию Тогда
				ПоискПоКоду = Ложь;
			Иначе
				ВызватьИсключение "Справочник " + ТипСправочника + " не имеет поля Код. Используйте параметр 'name'.";
			КонецЕсли;
		КонецЕсли;
		
		// SELECT: Код добавляем только если поле есть
		ТекстВыбора = 
		"ВЫБРАТЬ
		|    Ссылка,
		|    Наименование,";
		
		Если ИмеетПолеКод Тогда
