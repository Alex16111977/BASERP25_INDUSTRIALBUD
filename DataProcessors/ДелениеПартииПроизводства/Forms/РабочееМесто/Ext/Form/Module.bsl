...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = СтрШаблон(НСтр("ru='Изменение партии %1';uk='Зміна партії %1'"),
	...
	ЦветНеВыбраннаяСсылка = ЦветаСтиля.ЦветГиперссылки;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru='Действие не может быть выполнено для заказа с типом производственного процесса ""Без спецификаций""';uk='Дію не може бути виконано для замовлення з типом виробничого процесу ""Без специфікацій""'"),,,,
			Отказ);
		Возврат;
	КонецЕсли;
	...
	ИмяСписка = НСтр("ru='Партии';uk='Партії'");
	...
	Для Индекс = 0 По Объект.Партии.Количество()-1 Цикл
		...
		Если Строка.Номенклатура.Пустая()
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Номенклатура';uk='Номенклатура'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.Номенклатура",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Номенклатура';uk='Номенклатура'")));
		...
		Если Строка.Характеристика.Пустая()
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Характеристика';uk='Характеристика'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.Характеристика",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Характеристика';uk='Характеристика'")));
		...
		Если СтатусыСерияНеУказана.Найти(Строка.СтатусУказанияСерий) <> Неопределено
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Корректность", НСтр("ru='Серия';uk='Серія'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.Серия",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", НСтр("ru='Серия';uk='Серія'")));
		...
		Если Строка.КоличествоУпаковок = 0
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Количество';uk='Кількість'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.КоличествоУпаковок",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Количество';uk='Кількість'")));
		...
		Если Строка.Количество = 0
			...
			СообщенияТаблица.Добавить(
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Объект.Партии", Строка.НомерСтроки, "КоличествоУпаковок"),
				 СтрШаблон(
				 	НСтр("ru='Обнаружено нулевое количество при пересчете в единицу хранения в строке %1 списка ""%2""';uk='Виявлена нульова кількість при перерахунку на одиницю зберігання в рядку %1 списку ""%2""'"),
					Строка.НомерСтроки,
					ИмяСписка));
			...
			Если Индекс = ТекущаяПартия Тогда
				...
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.КоличествоУпаковок",
					НСтр("ru='Обнаружено нулевое количество при пересчете в единицу хранения';uk='Виявлена нульова кількість при перерахунку на одиницю зберігання'"));
		...
		Если Строка.ДоляСтоимости = 0
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Доля стоимости';uk='Частка вартості'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.ДоляСтоимости",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Доля стоимости';uk='Частка вартості'")));
		...
		Если НЕ ЗначениеЗаполнено(Строка.Получатель)
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Получатель';uk='Одержувач'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.Получатель",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Получатель';uk='Одержувач'")));
		...
		Если НЕ ЗначениеЗаполнено(Строка.СтатьяРасходов)
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Статья расходов';uk='Стаття витрат'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.СтатьяРасходов",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Статья расходов';uk='Стаття витрат'")));
		...
		Если Строка.КоличествоУпаковокПартии = 0
			...
			СообщенияТаблица.Добавить(
				...
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
					"Колонка", "Заполнение", НСтр("ru='Размер партии';uk='Розмір партії'"), Строка.НомерСтроки, ИмяСписка));
			
			Если Индекс = ТекущаяПартия Тогда
				
				СообщенияШапка.Добавить(
					"Элементы.Партии.ТекущиеДанные.КоличествоУпаковокПартии",
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Размер партии';uk='Розмір партії'")));
		...
		Если НЕ Объект.Этап.Пустая()
			...
			Если Строка.ПродолжитьСпецификация.Пустая() Тогда
				...
				СообщенияТаблица.Добавить(
					...
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Колонка", "Заполнение", НСтр("ru='Спецификация';uk='Специфікація'"), Строка.НомерСтроки, ИмяСписка));
				
				Если Индекс = ТекущаяПартия Тогда
					
					СообщенияШапка.Добавить(
						"Элементы.Партии.ТекущиеДанные.ПродолжитьСпецификация",
						ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Спецификация';uk='Специфікація'")));
			...
			Если Строка.ПродолжитьЭтап.Пустая() Тогда
				...
				СообщенияТаблица.Добавить(
					...
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Колонка", "Заполнение", НСтр("ru='Этап';uk='Етап'"), Строка.НомерСтроки, ИмяСписка));
				
				Если Индекс = ТекущаяПартия Тогда
					
					СообщенияШапка.Добавить(
						"Элементы.Партии.ТекущиеДанные.ПродолжитьЭтап",
						ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru='Этап';uk='Етап'")));
...
Процедура ПриИзмененииЭтапа(СписокРеквизитов, КэшированныеЗначения)
	...
	Если ДанныеЭтапа <> Неопределено Тогда
		...
		Если Объект.Подразделение <> ДанныеДоИзменения.Подразделение Тогда
			ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(
				НоменклатураСервер.ПараметрыУказанияСерий(Объект, Обработки.ДелениеПартииПроизводства));
			СписокРеквизитов = СписокРеквизитов + ", ПараметрыУказанияСерий";
		КонецЕсли;
...
Процедура ПартииНоменклатураПриИзмененииНаСервере(РассчитатьПроцентЗатрат, ОбновитьСтатусыСерий, КэшированныеЗначения)
	...
	ДоходыИРасходыКлиентСервер.ВставитьДействиеОчиститьСтатьюИАналитикуРасходов(СтруктураДействий, ЭтотОбъект, "Объект.Партии");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненноеПолеТаблицы);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не используется при продолжении обработки>';uk='<не використовується при продовженні обробки>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Продолжить обработку';uk='Продовжити обробку'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='По этапам текущей партии';uk='По етапах поточної партії'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненноеПолеТаблицы);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не используется при выпуске на склад>';uk='<не використовується при випуску на склад>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненноеПолеТаблицы);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не используется при выпуске в подразделение>';uk='<не використовується при випуску в підрозділ>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НезаполненноеПолеТаблицы);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не используется при формировании по текущей партии>';uk='<не використовується при формуванні по поточній партії>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ИзделиеНоменклатура")
		...
		Иначе
			...
			Если Объект.КоличествоУпаковок <> 0 Тогда
				МассивСтрок.Добавить(" ");
                МассивСтрок.Добавить(НСтр("ru='Количество';uk='Кількість'"));
                МассивСтрок.Добавить(": ");
	...
	Если СтруктураРеквизитов.Свойство("КоличествоУпаковок")
		...
		Элементы.ДействиеПродолжить.СписокВыбора.Добавить(Действия.Продолжить,
			СтрШаблон(НСтр("ru='Оформить промежуточный выпуск и продолжить обработку %1 %2 одной новой партией';uk='Оформити проміжний випуск і продовжити обробку %1 %2 однією новою партією'"),
	...
	Если СтруктураРеквизитов.Свойство("КоличествоПартийРазделить")
		...
		Иначе
			...
			Элементы.ДекорацияКоличествоПартий.Заголовок = СтрШаблон(НСтр("ru='%1, распределить затраты';uk=' %1, розподілити витрати'"),
				ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
					Форма.КоличествоПартийРазделить,
					НСтр("ru='партию';uk='партію'"),
					НСтр("ru='партии';uk='партії'"),
	...
	Если СтруктураРеквизитов.Свойство("Действие")
		...
		Если Объект.Действие = Действия.Отменить Тогда
			Элементы.ГруппаПромежуточныйВыпуск.Заголовок = НСтр("ru='Выпуск';uk='Випуск'");
		Иначе
			Элементы.ГруппаПромежуточныйВыпуск.Заголовок = ?(Форма.КоличествоПартий > 1,
				СтрШаблон(НСтр("ru='Промежуточный выпуск %1';uk='Проміжний випуск %1'"), Форма.ТекущаяПартия+1),
				НСтр("ru='Промежуточный выпуск';uk='Проміжний випуск'"));
		КонецЕсли;
	...
	Если СтруктураРеквизитов.Свойство("Действие")
		...
		МассивСтрок = Новый Массив;                 
		МассивСтрок.Добавить(БиблиотекаКартинок.ИнформацияБСП);
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(?(Объект.Действие = Действия.Отменить,
			НСтр("ru='Будет создан дополнительный этап для оформления выпуска';uk='Буде створено додатковий етап для оформлення випуску'"),
			НСтр("ru='Будет создан дополнительный этап для оформления промежуточного выпуска';uk='Буде створено додатковий етап для оформлення проміжного випуску'")));
	...
	Если СтруктураРеквизитов.Свойство("КоличествоПартий")
		...
		Элементы.КоличествоУпаковокПартии.Заголовок = ?(Форма.КоличествоПартий > 1,
			СтрШаблон(НСтр("ru='Размер партии %1';uk='Розмір партії %1'"), Форма.ТекущаяПартия+1),
			НСтр("ru='Размер партии';uk='Розмір партії'"));
	...
	Если СтруктураРеквизитов.Свойство("Этап")
		...
		Элементы.ДействиеОтменить.СписокВыбора[0].Представление = ?(Объект.Этап.Пустая(),
			НСтр("ru='Отменить производство';uk='Скасувати виробництво'"),
			НСтр("ru='Отменить дальнейшее производство, оформить выпуск';uk='Скасувати подальше виробництво, оформити випуск'"));
	...
	Если Элементы.Партии.ТекущаяСтрока <> Неопределено
		...
		Если СтруктураРеквизитов.Свойство("ТекущаяПартия")
			...
			Элементы.Характеристика.ПодсказкаВвода = ?(ТекущиеДанные.ХарактеристикиИспользуются, "",
				НСтр("ru='<характеристики не используются>';uk='<характеристики не використовуються>'"));
		...
		Если СтруктураРеквизитов.Свойство("ТекущаяПартия")
			...
			Если ТекущиеДанные.СписатьНаРасходы Тогда
				...
				Элементы.Назначение.ПодсказкаВвода = НСтр("ru='<не используется при списании на расходы>';uk='<не використовується при списанні на витрати>'");
				
			ИначеЕсли ТекущиеДанные.ПродолжитьОбработку Тогда
				
				Элементы.Назначение.Доступность = Ложь;
				Элементы.Назначение.ПодсказкаВвода = НСтр("ru='<не используется при продолжении обработки>';uk='<не використовується при продовженні обробки>'");
			...
			ИначеЕсли ТекущиеДанные.ТипНоменклатуры.Пустая()
				...
				Элементы.Назначение.Доступность = Ложь;
				Элементы.Назначение.ПодсказкаВвода = НСтр("ru='<для товаров и работ>';uk='<для товарів і робіт>'");
		...
		Если СтруктураРеквизитов.Свойство("ТекущаяПартия")
			...
			Если ТекущиеДанные.ТипНоменклатуры.Пустая()
				...
				Элементы.Серия.ПодсказкаВвода = НСтр("ru='<для товаров>';uk='<для товарів>'");
			...
			Иначе
				...
				Элементы.Серия.Доступность = Ложь;
				Элементы.Серия.ПодсказкаВвода = НСтр("ru='<серия указывается отдельно>';uk='<серія зазначається окремо>'");
				Элементы.УказатьСерии.Доступность = Истина;
		...
		Если СтруктураРеквизитов.Свойство("ТекущаяПартия")
			...
			Элементы.Получатель.ПодсказкаВвода = ?(ТекущиеДанные.СписатьНаРасходы, Объект.Подразделение, "");
		...
		Если СтруктураРеквизитов.Свойство("ТекущаяПартия")
			...
			Элементы.АналитикаРасходов.ПодсказкаВвода = ?(ЗначениеЗаполнено(ТекущиеДанные.СтатьяРасходов),
				"",
				НСтр("ru='<выберите статью расходов>';uk='<виберіть статтю витрат>'"));
	...
	Если Объект.Партии.Количество() < КоличествоПартий Тогда
		...
		Для Индекс = Объект.Партии.Количество() По КоличествоПартий-1 Цикл
			...
			Если Индекс <> 0 Тогда
				...
				НовыйЭлемент.Заголовок = СтрШаблон(НСтр("ru='Партия %1';uk='Партія %1'"), Индекс + 1);
...
Функция СписокВыбораНаправлениеВыпускаФорма(Форма)
	...
	Если Форма.Объект.Действие <> Действия().Отменить
		...
		Результат.Добавить(НаправлениеПродолжитьОбработку(), НСтр("ru='Продолжить обработку';uk='Продовжити обробку'"));
...
Функция ВыполнитьДействиеВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Деление (сокращение, отмена) партии производства';uk='Поділ (скорочення, скасування) партії виробництва'");
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
	    ...
	    Если ТипЗнч(РезультатСтруктура) = Тип("Структура") Тогда
			...
			Если НЕ РезультатСтруктура.Отказ Тогда
				...
				ПоказатьРезультатВыполнения(РезультатСтруктура.Ссылки)
...
Процедура ПоказатьРезультатВыполнения(Ссылки)
	...
	Если Ссылки.Свойство("ЭтапСоздан") Тогда
		Для каждого Ссылка Из Ссылки.ЭтапСоздан Цикл
			...
			НоваяСтрока.Описание = НСтр("ru='Создан';uk='Створено'");
		КонецЦикла;
	КонецЕсли;
	...
	Если Ссылки.Свойство("ЭтапИзменен") Тогда
		Для каждого Ссылка Из Ссылки.ЭтапИзменен Цикл
			НоваяСтрока = Этапы.Добавить();
			НоваяСтрока.Ссылка = Ссылка;
			НоваяСтрока.Описание = НСтр("ru='Изменен';uk='Змінений'");
		КонецЦикла;
	КонецЕсли;
	...
	Если Ссылки.Свойство("ЭтапУдален") Тогда
		Для каждого Ссылка Из Ссылки.ЭтапУдален Цикл
			НоваяСтрока = Этапы.Добавить();
			НоваяСтрока.Ссылка = Ссылка;
			НоваяСтрока.Описание = НСтр("ru='Удален';uk='Вилучений'");
		КонецЦикла;
	КонецЕсли;
	
	Этапы.Свернуть("Ссылка, Описание");
	...
	Запрос.УстановитьПараметр("ОписаниеНеИзменялся", НСтр("ru='Не изменялся';uk='Не змінювався'"));
	...
	Пока ВыборкаИтоги.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка,
				"Ссылка, Статус, Описание, НомерКартинки, КоличествоУпаковок, Упаковка, НоменклатураПартии");
