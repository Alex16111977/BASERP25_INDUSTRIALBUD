...
Процедура ВыполнитьДействиеВФоне(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		Если Параметры.Действие = 0 Тогда // Сократить (увеличить)
			...
			Ссылки = Документы.ЭтапПроизводства2_2.ИзменитьРазмерПартии(
				Параметры,
				РазмерПартии,
				Параметры.Этап,
		...
		ИначеЕсли Параметры.Действие = 1 Тогда // Отменить
			...
			Ссылки = Документы.ЭтапПроизводства2_2.ОтменитьПартию(
				Параметры,
				Параметры.Этап,
				Параметры.Партии,
				Параметры.Серии,
		...
		ИначеЕсли Параметры.Действие = 2
			...
			Ссылки = Документы.ЭтапПроизводства2_2.РазделитьПартию(Параметры, Партии, Отказ);
				
		Иначе // Разделить (после этапа) И Продолжить
				
			Ссылки = Документы.ЭтапПроизводства2_2.РазделитьПартиюПослеЭтапа(
				Параметры,
				Параметры.Этап,
				Параметры.Партии,
				Параметры.Серии,
	...
	Исключение
		...
		Сообщения = Новый Массив(ПолучитьСообщенияПользователю());
		Сообщения.Добавить(Сообщение);
		
	КонецПопытки;
	
	Результат = Новый Структура("Отказ, Ссылки, Сообщения", Отказ, Ссылки, Сообщения);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий.ПараметрыЗапроса.Вставить("Подразделение", Объект.Подразделение);
...
//  
// Возвращаемое значение:
//  Массив из см. НастройкаСчетовУчета.ПараметрыНастройки
//
Функция ПараметрыНастройкиСчетовУчета() Экспорт
	...
	ПараметрыНастройки.АналитикаАктивовПассивов	= "Объект.Партии.АналитикаАктивовПассивов";
