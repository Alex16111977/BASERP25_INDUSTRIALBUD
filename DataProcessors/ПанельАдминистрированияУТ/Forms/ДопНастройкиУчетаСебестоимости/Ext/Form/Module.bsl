...
Функция ПартионныйУчетНельзяВыключать()
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.КодПричины = 5 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru='используется управление производством версии 2.5';uk='використовується управління виробництвом версії 2.5'");
		ИначеЕсли Выборка.КодПричины = 6 Тогда
			ТекстПредупреждения = ТекстПредупреждения + "
				|    - " + НСтр("ru='используется учет необоротных активов версии 2.5';uk='використовується облік необоротних активів версії 2.5'");
		//-- НЕ УТ
		Иначе
	...
	Если НаборКонстант.ВестиУправленческийУчетОрганизаций Тогда
		...
		Если Константы.ИспользоватьВнеоборотныеАктивы2_4.Получить() Тогда
			КомментарийОпции = КомментарийОпции + Символы.ПС
				+ НСтр("ru='- Учет необоротных активов версии 2.5 (раздел ""НСИ и администрирование"" - ""Необоротные активы"")';uk='- Облік необоротних активів версії 2.5 (розділ ""НДІ і адміністрування"" - ""Необоротні активи"")'");
			КонецЕсли;
