...
Функция СформироватьЗаказыНаРемонт(УдаляемыеЗаказы) Экспорт
	...
	Для Каждого ЗаказНаРемонт Из ЗаказыНаРемонт Цикл
		...
		ЗаполнитьЗначенияСвойств(ОбъектДокумента, ЗаказНаРемонт, "ОбъектЭксплуатации, ДатаНачала, Подразделение, РемонтноеМероприятие, ОбщийВидРемонта");
	...
	Пока Не ПрерватьРемонтныйЦикл Цикл
		...
		Если Правило.Разрешен Тогда
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(НовыйРемонт, СледующийРемонт, "ВидРемонта, ОбщийВидРемонта, ДатаНачала, Длительность");
...
Процедура ПолучитьЗаказНаРемонт(СтрокаРемонта)
	...
	Если СоздатьНовыйЗаказ Тогда
		...
		ЗаполнитьЗначенияСвойств(ЗаказНаРемонт, СтрокаРемонта, "ОбъектЭксплуатации, ДатаНачала, Подразделение, Длительность, ОбщийВидРемонта");
...
Функция СледующийРемонтПоПравилу(РемонтируемыйОбъект, Правило, ДатаПредыдущего, Выборки)
	...
	СтруктураРемонта = Новый Структура("ВидРемонта, ОбщийВидРемонта, ДатаНачала, Длительность, Отказ");
