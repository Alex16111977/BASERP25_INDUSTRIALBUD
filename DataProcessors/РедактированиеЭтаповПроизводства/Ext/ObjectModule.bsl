...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ДанныеЭтапа Из Этапы Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='Для этапа %1 необходимо указать выходное изделие.';uk='Для етапу %1 необхідно вказати вихідний виріб.'"), ДанныеЭтапа.Распоряжение);
...
|%1'
|;uk='Реквізит ""Спосіб розподілу витрат на вихідні вироби"" 
|повинен мати однакове значення для всіх випускних етапів партії: 
|%1'");
...
Функция ДанныеЭтаповПроизводства() Экспорт
	...
	// ТЧ
	ЗаполнитьДанныеДляПроверкиТЧ(СтруктураДанныхЭтапов.ВыходныеИзделия, ЭтотОбъект.ВыходныеИзделия);
	ЗаполнитьДанныеДляПроверкиТЧ(СтруктураДанныхЭтапов.ПобочныеИзделия, ЭтотОбъект.ПобочныеИзделия);
	ЗаполнитьДанныеДляПроверкиТЧ(СтруктураДанныхЭтапов.ОбеспечениеМатериаламиИРаботами, ЭтотОбъект.ОбеспечениеМатериаламиИРаботами);
