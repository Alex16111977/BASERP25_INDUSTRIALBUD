...
Функция УдалитьВыбранныеМаршрутныеЛистыНаСервере()
	...
	Для каждого ИдентификаторСтроки Из Элементы.СписокМаршрутныхЛистов.ВыделенныеСтроки Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось пометить на удаление документ: %Ссылка% по причине: %Причина%';uk='Не вдалося помітити на вилучення документ: %Ссылка% з причини: %Причина%'");
			
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", ДанныеСтроки.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Маршрутный лист производства';uk='Маршрутний лист виробництва'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОтметитьНачалоВыполненияНаСервере(ПараметрыДокумента, МассивСсылок)
	...
	Для каждого Ссылка Из МассивСсылок Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отметить начало выполнения документа: %Ссылка% по причине: %Причина%';uk='Не вдалося відзначити початок виконання документа: %Ссылка% з приини: %Причина%'");
...
Функция ОтметитьГотовностьКРаботеКлючевогоРЦНаСервере(ПараметрыДокумента, МассивСсылок)
	...
	Для каждого Ссылка Из МассивСсылок Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отметить готовность к работе ключевого РЦ в документе: %Ссылка% по причине: %Причина%';uk='Не вдалося відзначити готовність до роботи ключового РЦ в документі: %Ссылка% з причини: %Причина%'");
...
Функция ОтметитьЗавершениеРаботыКлючевогоРЦНаСервере(Знач ПараметрыДокумента, Знач МассивСсылок)
	...
	Для каждого Ссылка Из МассивСсылок Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отметить завершение работы ключевого РЦ в документе: %Ссылка% по причине: %Причина%';uk='Не вдалося відзначити завершення роботи ключового РЦ в документі: %Ссылка% з причини: %Причина%'");
...
Функция ОтметитьВыполненнымНаСервере(Знач ПараметрыДокумента, Знач ДанныеДокументов)
	...
	Для каждого ДанныеСтроки Из ДанныеДокументов Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отметить выполненным документ: %Ссылка% по причине: %Причина%';uk='Не вдалося відзначити виконаним документ: %Ссылка% з причини:%Причина%'");
