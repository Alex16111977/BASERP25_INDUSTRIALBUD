...
Функция ВыполнитьСравнениеЗаказаСоСпецификациями(ДанныеЗаказа, Этапы, МатериалыИУслуги, ВыходныеИзделия, ВозвратныеОтходы, ТаблицаВидыРЦ) Экспорт
	...
	Для каждого ВыборкаЭтапов Из СтрокиЭтапы Цикл
		...
		ДобавитьДанныеПоИзделиям(СтрокиВыходныеИзделия, ДанныеСравненияЭтапа.ТаблицаИзделий, ПолеЗначения);
...
Функция ВывестиОтчет(
	...
	Если СтруктураСравненияДанных <> Неопределено
		...
		Если (ПоказатьТолькоОтличия И СтруктураСравненияДанных.ТаблицаРеквизитов.Найти(Истина, "ЕстьОтличия") <> Неопределено
			...
			ОбластьШапкаНазвание.Параметры.Заголовок = НСтр("ru='Реквизиты';uk='Реквізити'");
	...
	Для каждого ДанныеЭтапа Из ДанныеСравнения.СписокЭтапов Цикл
		...
		Если НЕ ШапкаЭтаповВыведена Тогда
			...
			ОбластьШапкаНазвание.Параметры.Заголовок = НСтр("ru='Этапы';uk='Етапи'");
			РезультатСравнения.Вывести(ОбластьШапкаНазвание);
			Для каждого ЭлКоллекции Из СписокСпецификаций Цикл
				ОбластьШапкаИсточник.Параметры.ИмяИсточника         = ЭлКоллекции.Представление;
				ОбластьШапкаИсточник.Параметры.РасшифровкаИсточника = ЭлКоллекции.Значение;
		...
		ВывестиГруппуСвойств(НСтр("ru='Разбираемые изделия';uk='Розбирані вироби'"),
...
//++ НЕ УТКА
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
		...
		ДобавитьРеквизит(НСтр("ru='Тип';uk='Тип'"), Реквизиты.ТипПроизводственногоПроцесса, ТаблицаРеквизитов, ПолеЗначения);
		
		Если Реквизиты.ВыпускПроизвольнымиПорциями Тогда
			ДобавитьРеквизит(НСтр("ru='Выпуск';uk='Випуск'"), НСтр("ru='произвольными порциями';uk='довільними порціями'"), ТаблицаРеквизитов, ПолеЗначения);
		Иначе
			ДобавитьРеквизит(НСтр("ru='Выпуск';uk='Випуск'"), НСтр("ru='кратными партиями';uk='кратними партіями'"), ТаблицаРеквизитов, ПолеЗначения);
		КонецЕсли;
		
		ДобавитьРеквизит(НСтр("ru='Оптимальная партия';uk='Оптимальна партія'"), Реквизиты.ОптимальнаяПартияВыпуска, ТаблицаРеквизитов, ПолеЗначения);
		ДобавитьРеквизит(НСтр("ru='Минимальная партия';uk='Мінімальна партія'"), Реквизиты.МинимальнаяПартияВыпуска, ТаблицаРеквизитов, ПолеЗначения);
		
		ДобавитьРеквизит(НСтр("ru='Ограничен срок пролеживания';uk='Обмежений термін пролежування'"), Реквизиты.ОграниченСрокПролеживанияВыходныхИзделий, ТаблицаРеквизитов, ПолеЗначения);
		
		Если Реквизиты.ОграниченСрокПролеживанияВыходныхИзделий Тогда
			ДобавитьРеквизит(НСтр("ru='Максимальный срок пролеживания';uk='Максимальний строк пролежування'"), Реквизиты.МаксимальныйСрокПролеживанияВыходныхИзделий, ТаблицаРеквизитов, ПолеЗначения);
		Иначе
			ДобавитьРеквизит(НСтр("ru='Максимальный срок пролеживания';uk='Максимальний строк пролежування'"), 0, ТаблицаРеквизитов, ПолеЗначения);
		КонецЕсли;
...
Процедура ПолучитьДанныеСпецификаций(ДанныеСравнения, СписокСпецификаций)
	...
	|	КОНЕЦ КАК ЕдИзм,
	|	ДанныеТЧ.ОсновноеИзделиеКоличествоУпаковок КАК КоличествоУпаковок,
