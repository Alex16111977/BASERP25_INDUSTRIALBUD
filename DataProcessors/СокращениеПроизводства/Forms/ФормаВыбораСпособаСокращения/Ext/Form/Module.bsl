...
Процедура УстановитьНастройкиЭлементовФормы()
	...
	ИначеЕсли КоличествоЗапланировано = 0 Тогда
		...
		Элементы.ГруппаПроизвестиНачатое.Подсказка = НСтр("ru='Отменить неначатое невозможно, т.к. производство не запланировано.';uk='Скасувати непочате неможливо, оскільки виробництво не заплановано.'");
		
	ИначеЕсли КоличествоЗапланировано = КоличествоВыполнено Тогда
		
		Элементы.ГруппаПроизвестиНачатое.Подсказка = НСтр("ru='Отменить неначатое невозможно, т.к. завершено производство всего запланированного количества.';uk='Скасувати непочате неможливо, тому що завершено виробництво всієї запланованої кількості.'");
		
	ИначеЕсли ЭтоПолуфабрикат И КоличествоТребуется = КоличествоЗапланировано Тогда
		
		Элементы.ГруппаПроизвестиНачатое.Подсказка = НСтр("ru='Отменить неначатое невозможно, т.к. для следующих этапов требуется производство всего запланированного количества.';uk='Скасувати непочате неможливо, тому що для наступних етапів потрібно виробництво всієї запланованої кількості.'");
		
	ИначеЕсли КоличествоВыполненоИВыполняетсяСУчетомПолуфабрикатов = КоличествоЗапланировано Тогда
		
		Элементы.ГруппаПроизвестиНачатое.Подсказка = НСтр("ru='Отменить неначатое невозможно, т.к. начато производство всего запланированного количества.';uk='Скасувати непочате неможливо, тому що розпочато виробництво всієї запланованої кількості.'");
	...
	ИначеЕсли КоличествоЗапланировано = 0 Тогда
		...
		Элементы.ГруппаОстановитьВсе.Подсказка = НСтр("ru='Остановить невозможно, т.к. производство не запланировано.';uk='Зупинити неможливо, оскільки виробництво не заплановано.'");
		
	ИначеЕсли КоличествоЗапланировано = КоличествоВыполнено Тогда
		
		Элементы.ГруппаОстановитьВсе.Подсказка = НСтр("ru='Остановить невозможно, т.к. завершено производство всего запланированного количества.';uk='Зупинити неможливо, тому що завершено виробництво всієї запланованої кількості.'");
		
	ИначеЕсли ЭтоПолуфабрикат И КоличествоТребуется = КоличествоЗапланировано Тогда
		
		Элементы.ГруппаОстановитьВсе.Подсказка = НСтр("ru='Остановить невозможно, т.к. для следующих этапов требуется производство всего запланированного количества.';uk='Зупинити неможливо, оскільки для наступних етапів потрібно виробництво всієї запланованої кількості.'");
		
	ИначеЕсли КоличествоВыполненоИВыполняется = КоличествоЗапланировано Тогда
		
		Элементы.ГруппаОстановитьВсе.Подсказка = НСтр("ru='Остановить невозможно, т.к. начато производство всего запланированного количества.';uk='Зупинити неможливо, тому що розпочато виробництво всієї запланованої кількості.'");
...
Процедура УстановитьПодсказкуДляПоляУказатьНовоеКоличество()
	...
	Если НовоеКоличество > КоличествоЗапланировано Тогда
		...
		Элементы.ГруппаУказатьНовоеКоличество.Подсказка = НСтр("ru='Увеличить количество производимого изделия.';uk='Збільшити кількість виробленого виробу.'");
	...
	ИначеЕсли НовоеКоличество < КоличествоЗапланировано И КоличествоОсталось < КоличествоВыполняетсяСУчетомПолуфабрикатов Тогда
		...
		Элементы.ГруппаУказатьНовоеКоличество.Подсказка = НСтр("ru='Уменьшить, доделав начатые полуфабрикаты.';uk='Зменшити, доробивши розпочаті напівфабрикати.'");
		
	Иначе
	
		Элементы.ГруппаУказатьНовоеКоличество.Подсказка = НСтр("ru='Уменьшить количество производимого изделия.';uk='Зменшити кількість виробиу, що виробляється.'");
