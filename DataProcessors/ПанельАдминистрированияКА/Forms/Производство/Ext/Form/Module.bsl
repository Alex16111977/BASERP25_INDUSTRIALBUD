...
//++ НЕ УТКА
		...
		Элементы.ИспользоватьПараметрыНазначенияСпецификаций.Доступность = ИспользуетсяПроизводство22() И НЕ ИспользуетсяПроизводство21();
...
Процедура ОбновитьНастройкиИспользованияОперацииКакРаспоряжениеВыработки()
	...
	ИспользуетсяОперацияКакРаспоряжениеВыработки = 
		УправлениеПроизводствомПовтИсп.ДатаНачалаВеденияУчетаВыработкиПоОперациям() <> Дата("00010101");
