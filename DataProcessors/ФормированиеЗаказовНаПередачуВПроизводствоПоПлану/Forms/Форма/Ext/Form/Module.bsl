	...
	Если Строка <> Неопределено И (Поле.Имя = "КорзинаЗаказовПодразделениеИсполнитель"
		...
		Если Поле.Имя = "КорзинаЗаказовПодразделениеИсполнитель" Тогда
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Назначить для выделенного товара подразделение-исполнитель: %1?';uk='Призначити для виділеного товару підрозділ-виконавець: %1?'"), СтрокаКорзины.ПодразделениеИсполнитель);
	...
	Если Объект.ФормированиеЗаказов.Количество() = 0 Тогда
		Если ОтборПодразделения Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По сценарию %1 у подразделения исполнителя %2 за установленный период потребность в материалах отсутствует.';uk='За сценарієм %1 у підрозділу виконавця %2 за встановлений період потреба в матеріалах відсутня.'"),
				Объект.Сценарий,
				Объект.ПодразделениеИсполнитель);
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='По сценарию %1 за установленный период потребность в материалах отсутствует.';uk='За сценарієм %1 за встановлений період потреба в матеріалах відсутня.'"),
				Объект.Сценарий);
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
	КонецЕсли;
	...
	Для Каждого СтрокаДокумента Из ТаблицаДокументов Цикл
		...
		Если ДокументОбъект.Товары.Количество() > 0 Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Формирование заказов материалов в производство по планам';uk='Формування замовлень матеріалів у виробництво за планами'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ВводДоступен() Тогда
		Если ПроверитьЕстьДокументыДляОбновления() Тогда
			ПоказатьВопрос(
				...
				НСтр("ru='Заказы материалов в производство были изменены. Хотите обновить данные? При обновлении строки будут очищены.';uk='Замовлення матеріалів у виробництво були змінені. Хочете оновити дані? При оновленні рядки будуть очищені.'"), 
				РежимДиалогаВопрос.ДаНет,
				,
				,
				НСтр("ru='Формирование заказов на передачу в производство по плану';uk='Формування замовлень на передачу у виробництво за планом'"));
		КонецЕсли;
	Иначе
		ОбновитьДанныеКогдаФормаСтанетАктивной();
	КонецЕсли;
