		...
		Если ТребуетсяИзменитьОперацию(Операция, ДанныеОперации) Тогда
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Ошибка изменения производственной операции';uk='Помилка зміни виробничої операції'"),,,,
					Отказ);
...
Процедура УстановитьЗаголовокФормыСпискаОпераций()
	...
	Если РежимРаботы = РежимРаботыПринятьВРаботу() Тогда
		...
		ТекстЗаголовок = НСтр("ru='Принять операции в работу';uk='Прийняти операції в роботу'");
		
	ИначеЕсли РежимРаботы = РежимРаботыОтметитьВыполнение() Тогда
		
		ТекстЗаголовок = НСтр("ru='Отметить выполнение операций';uk='Відзначити виконання операцій'");
...
Процедура ЗаполнитьРеквизитыФормыСпискаПоДаннымОперации()
	...
	ЗаполнитьЗначенияСвойств(ЭтаФорма, ДанныеОперации, "Организация,Этап,МаршрутнаяКарта,Операция,ЕдиницаИзмерения");
...
Функция ФорматированнаяСтрокаДобавитьОперацию()
	...
	Если РежимРаботы = РежимРаботыПринятьВРаботу() Тогда
		МассивСтрок.Добавить(НСтр("ru='принять в работу';uk='прийняти в роботу'"));
	ИначеЕсли РежимРаботы = РежимРаботыОтметитьВыполнение() Тогда
		МассивСтрок.Добавить(НСтр("ru='отметить выполнение';uk='відзначити виконання'"));
	КонецЕсли;
	
	МассивСтрок.Добавить(" ");
	
	МассивСтрок.Добавить(НСтр("ru='еще';uk='ще'"));
...
Процедура ИсполнительПолучениеДанныхВыбора(ДанныеВыбора, Текст, Организация, Подразделение)
	...
	ПараметрыОтбора = Новый Структура("Организация, Подразделение", Организация, Подразделение);
