...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если (ИмяСобытия = "Запись_ПроизводственнаяОперация2_2"
		ИЛИ ИмяСобытия = "Запись_ПроизводственнаяОперация")
...
Процедура ОтборыСтрокаНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ВыполнениеОпераций2_2.Форма.МобильноеПриложениеУстановкаОтборов",
		...
		,
		,
		Новый ОписаниеОповещения("УстановитьОтборы",ЭтотОбъект));
КонецПроцедуры
...
Процедура ПропуститьОперацию(Команда)
	...
	Если КлючиОпераций.Количество() = 1
		...
		Если НЕ Элементы.ВыполнениеОпераций.ТекущиеДанные.МожноПропустить Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru='Операция не может быть пропущена';uk='Операція не може бути пропущена'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СканироватьШтрихКод(Команда)
	...
	Если СредстваМультимедиа.ПоддерживаетсяСканированиеШтрихКодов() Тогда
		...
		СредстваМультимедиа.ПоказатьСканированиеШтрихКодов(НСтр("ru='Наведите камеру на штрихкод';uk='Наведіть камеру на штрихкод'"),
...
Процедура ОбработкаСканирования(Штрихкод, Результат, Сообщение, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение = НСтр("ru='Ошибка обработки штрих кода';uk='Помилка обробки штрихкоду'");
...
&НаСервере
Процедура УстановитьОтборы(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура УстановитьОтборПоРабочемуЦентру()
	...
	Если ЗначениеЗаполнено(РабочийЦентр) Тогда
		...
		Если ПодразделениеРЦ <> Подразделение
			...
			ТекстСообщения = НСтр("ru='Выбранный рабочий центр не находится в текущем подразделении или участке';uk='Вибраний робочий центр не знаходиться в поточному підрозділі або ділянці'");
...
Процедура ЗаполнитьОтбор(Штрихкод)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ФизическиеЛица") И НЕ ИсполнительВНастройках Тогда
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ru='Штрих-код не найден';uk='Штрихкод не знайдений'");
		Сообщение.Сообщить();
...
Процедура НастроитьЭлементыОтбора()
	...
	Если ОтборыСтрока = "" Тогда
		ОтборыСтрока = ?(МожноВыполнять,НСтр("ru='Установить отбор*';uk='Встановити відбір*'"),НСтр("ru='Установить отбор';uk='Установити відбір'"));
...
Процедура РасчитатьЗагрузку()
	...
	Если ЗначениеЗаполнено(РабочийЦентр) > 0 Тогда
		...
		Загрузка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка РЦ %1 ч';uk='Завантаження РЦ %1 год'"), РассчитаннаяЗагрузка.РабочийЦентр);
	...
	Если ЗначениеЗаполнено(Исполнитель) > 0 Тогда
		...
		Если ЗначениеЗаполнено(РабочийЦентр) Тогда
			...
			Загрузка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1, исполнителя %2 ч';uk=' %1, виконавця %2 ч'"),Загрузка, РассчитаннаяЗагрузка.Исполнитель);
			
		Иначе
		
			Загрузка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Загрузка исполнителя %1 ч';uk='Завантаження виконавця %1 год'"), РассчитаннаяЗагрузка.Исполнитель);
...
Процедура ДополнительнаяЗагрузка(ВремяВыполнения)
	...
	Если ПозицияЗнакаПодстроки > 0 Тогда
		...
		Если НеИспольнительИРЦ Тогда
			Загрузка = Лев(Загрузка,ПозицияЗнакаПодстроки - 2) + Символы.НПП + НСтр("ru='ч';uk='год'");
		Иначе
	...
	Если ВремяВыполнения > 0 Тогда
		...
		Если НеИспольнительИРЦ Тогда
			...
			ПозицияЗнакаПодстроки = СтрНайти(Загрузка,НСтр("ru='ч';uk='год'"));
			
			Загрузка = Лев(Загрузка,ПозицияЗнакаПодстроки - 2) + " (+" + Окр(ВремяВыполнения,2) + НСтр("ru=') ч';uk=') год'");
