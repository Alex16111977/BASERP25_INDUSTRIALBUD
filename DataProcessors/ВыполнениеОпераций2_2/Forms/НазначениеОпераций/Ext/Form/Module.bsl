...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение,"ИспользоватьСменныеЗадания") Тогда
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Результат = Новый Структура("Участок,СменноеЗадание,ВидРабочегоЦентра,РабочийЦентр,Исполнитель,Количество");
	
	Если МножественныйРежим Тогда
		ЗаполнитьЗначенияСвойств(Результат, ЭтаФорма, "Участок,СменноеЗадание,ВидРабочегоЦентра,РабочийЦентр,Исполнитель");
	Иначе
		ЗаполнитьЗначенияСвойств(Результат, ЭтаФорма, "Участок,СменноеЗадание,ВидРабочегоЦентра,РабочийЦентр,Исполнитель,Количество");
	КонецЕсли;
	...
	Если Не МножественныйРежим Тогда
		...
		Если ДанныеОперации <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(
				ЭтаФорма,
				ДанныеОперации,
				"Организация,Этап,МаршрутнаяКарта,Операция,ВидРабочегоЦентра,ЕдиницаИзмерения,ВремяШтучное,ВремяПЗ,ВремяЕдИзм,КоличествоШтучное,ПараллельнаяЗагрузка");
		КонецЕсли;
...
Процедура НастроитьЭлементыФормы() 
	...
	Элементы.СменноеЗадание.Видимость = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение,"ИспользоватьСменныеЗадания");
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.РабочийЦентр.ПодсказкаВвода = НСтр("ru='<Рабочий центр>';uk='<Робочий центр>'");
		Элементы.Исполнитель.ПодсказкаВвода = НСтр("ru='<Исполнитель>';uk='<Виконавець>'");
...
Процедура УстановитьЗаголовкиЭлементовФормы()
	...
	Если МножественныйРежим Тогда
		Если РежимРаботы = РежимРаботыНазначить() Тогда
			ТекстЗаголовок = НСтр("ru='Назначить выбранные операции';uk='Призначити вибрані операції'");
		ИначеЕсли РежимРаботы = РежимРаботыПринятьВРаботу() Тогда
			ТекстЗаголовок = НСтр("ru='Принять в работу выбранные операции';uk='Прийняти в роботу вибрані операції'");
		ИначеЕсли РежимРаботы = РежимРаботыОтметитьВыполнение() Тогда
			ТекстЗаголовок = НСтр("ru='Отметить выполнение выбранных операций';uk='Відзначити виконання вибраних операцій'");
		КонецЕсли;
	Иначе
		Если РежимРаботы = РежимРаботыНазначить() Тогда
			ТекстЗаголовок = НСтр("ru='Назначить операцию';uk='Призначити операцію'");
		ИначеЕсли РежимРаботы = РежимРаботыПринятьВРаботу() Тогда
			ТекстЗаголовок = НСтр("ru='Принять в работу операцию';uk='Прийняти в роботу операцію'");
		ИначеЕсли РежимРаботы = РежимРаботыОтметитьВыполнение() Тогда
			ТекстЗаголовок = НСтр("ru='Отметить выполнение операции';uk='Відзначити виконання операції'");
		КонецЕсли;
	...
	Если РежимРаботы = РежимРаботыНазначить() Тогда
		Элементы.КоличествоДоступно.Заголовок = НСтр("ru='Ожидает назначения';uk='Очікує призначення'");
	Иначе
		Элементы.КоличествоДоступно.Заголовок = НСтр("ru='Можно выполнять';uk='Можна виконувати'");
	КонецЕсли;
