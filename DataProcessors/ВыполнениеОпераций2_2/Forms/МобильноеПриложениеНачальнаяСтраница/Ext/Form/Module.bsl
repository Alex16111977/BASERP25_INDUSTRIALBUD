...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Сообщение.Текст = НСтр("ru='Невозможно открыть рабочее место. Отсутствуют настройки открытия формы';uk='Неможливо відкрити робоче місце. Відсутні настройки відкриття форми'");
		Сообщение.Сообщить();
...
Процедура СменитьПодразделение(Команда)
	...
	ПоказатьВводЗначения(ВыборПодразделения,ПодразделениеОтбор,НСтр("ru='Выберите подразделение';uk='Виберіть підрозділ'"));
КонецПроцедуры
...
Процедура ЗаполнитьДанныеВнутриЗамераВремени()
	...
	Если РежимРаботы = Перечисления.РежимыРаботыРабМестоБригадира.НазначениеПринятиеВРаботуИВыполнение
		...
		СтрокаРаздела.Описание     = НСтр("ru='Назначить операцию';uk='Призначити операцію'");
		СтрокаРаздела.ВажныеДанные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Можно начинать: %1 %2Всего к назначению: %3';uk='Можна починати: %1 %2Всего до призначення: %3'"),
										МожноНачинать,
										Символы.ПС,
										ВсегоНазначить);
	...
	Если РежимРаботы = Перечисления.РежимыРаботыРабМестоБригадира.НазначениеПринятиеВРаботуИВыполнение 
		...
		СтрокаРаздела.Описание     = НСтр("ru='Принять в работу';uk='Прийняти в роботу'");
		СтрокаРаздела.ВажныеДанные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Не начато: %1';uk='Не розпочато: %1'"),
										Созданные);
	...
	СтрокаРаздела.Описание     = НСтр("ru='Отметить выполнение';uk='Позначити виконання'");
	СтрокаРаздела.ВажныеДанные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Назначено: %1';uk='Призначено: %1'"),
									ВсегоОпераций);
	
	СтрокаРаздела = Разделы.Добавить();
	СтрокаРаздела.Картинка   = БиблиотекаКартинок.ТиповоеСоглашениеСКлиентом;
	СтрокаРаздела.ИмяРаздела = "Журнал";
	СтрокаРаздела.Описание   = НСтр("ru='Журнал операций';uk='Журнал операцій'");
...
Процедура ОтобразитьДиаграмму()
	...
	Серия1.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполнение %1%';uk='Виконання %1%'"),
