...
//++ НЕ УТ
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		...
		Если Заказы.Количество() = 1 Тогда
			...
			РеквизитыЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Заказы[0].Значение, "Организация, СуммаДокумента, Договор, Валюта, УслугиПоПереработке");
			...
			Если РеквизитыЗаказа.УслугиПоПереработке = Перечисления.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете Тогда
				СтруктураРасчетов = ВзаиморасчетыСервер.СостояниеВзаиморасчетовОбъектаРасчетов(Заказы[0].Значение,
...
Процедура ЗаполнитьПодсказкиИДекорации()
	...
	ИначеЕсли ТипЗаказа = "ЗаказМатериаловВПроизводство" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлена передача и не осуществлялась отгрузка';uk='не оформлена передача і не здійснювалося відвантаження'");
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлена передача и не осуществлялась отгрузка';uk='не оформлена передача і не здійснювалося відвантаження'");
		ТекстНеОформленаПриемка = НСтр("ru='не оформлено поступление и не осуществлялась приемка';uk='не оформлене надходження і не здійснювалося приймання'");
	//-- НЕ УТ
	...
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		ТекстНеОформленаОтгрузка = НСтр("ru='не оформлена передача и не осуществлялась отгрузка';uk='не оформлена передача і не здійснювалося відвантаження'");
		ТекстНеОформленаПриемка = НСтр("ru='не оформлен возврат и не осуществлялась приемка';uk='не оформлено повернення і не здійснювалося приймання'");
	//-- НЕ УТКА
	КонецЕсли;
...
//++ НЕ УТ
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		ТекстСтрокиОтгрузка = НСтр("ru='Строки сырья и материалов';uk='Рядки сировини і матеріалів'");
		ТекстСтрокиПриемка  = НСтр("ru='Строки выходных изделий';uk='Рядки вихідних виробів'");
//-- НЕ УТ
//++ НЕ УТКА
	ИначеЕсли ТипЗаказа = "ЗаказДавальца" Тогда
		ТекстСтрокиОтгрузка = НСтр("ru='Строки выпускаемой продукции';uk='Рядки, продукції, що випускається'");
		ТекстСтрокиПриемка  = НСтр("ru='Строки сырья и материалов для производства';uk='Рядки сировини і матеріалів для виробництва'");
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		ТекстСтрокиОтгрузка = НСтр("ru='Строки обеспечиваемых материалов';uk='Рядки забезпечуваних матеріалів'");
		ТекстСтрокиПриемка  = НСтр("ru='Строки неизрасходованных материалов';uk='Рядки невитрачених матеріалів'");
//-- НЕ УТКА
	КонецЕсли;
...
//++ НЕ УТ
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		Элементы.СкорректироватьМерныеТовары.Заголовок          = НСтр("ru='Скорректировать строки мерных сырья и материалов.';uk='Скоригувати рядки мірних сировини і матеріалів.'");
		Элементы.СкорректироватьМерныеТоварыПоПриемке.Заголовок = НСтр("ru='Скорректировать строки мерных выходных изделий.';uk='Скоригувати рядки мірних вихідних виробів.'");
//-- НЕ УТ
//++ НЕ УТКА
	ИначеЕсли ТипЗаказа = "ЗаказДавальца" Тогда
		Элементы.СкорректироватьМерныеТовары.Заголовок          = НСтр("ru='Скорректировать строки мерной выпускаемой продукции.';uk='Скоригувати рядки мірної продукції, що випускається.'");
		Элементы.СкорректироватьМерныеТоварыПоПриемке.Заголовок = НСтр("ru='Скорректировать строки мерных сырья и материалов для производства.';uk='Скоригувати рядки мірних сировини і матеріалів для виробництва.'");
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		Элементы.СкорректироватьМерныеТовары.Заголовок          = НСтр("ru='Скорректировать строки мерных обеспечиваемых материалов.';uk='Скоригувати рядки мірних забезпечуваних матеріалів.'");
		Элементы.СкорректироватьМерныеТоварыПоПриемке.Заголовок = НСтр("ru='Скорректировать строки мерных неизрасходованных материалов.';uk='Скоригувати рядки мірних невитрачених матеріалів.'");
//-- НЕ УТКА
	КонецЕсли;
...
//++ НЕ УТКА
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		Если ЗаказыКоличество = 1 Тогда
			ТекстРП = НСтр("ru='этапа';uk= 'етапу'");
			ТекстИП = НСтр("ru='этап';uk='етап'");
			ТекстДП = НСтр("ru='этапу';uk='етапу'");
		Иначе
			ТекстРП = НСтр("ru='этапов';uk='етапів'");
			ТекстИП = НСтр("ru='этапы';uk='етапи'");
			ТекстДП = НСтр("ru='этапам';uk='етапам'");
		КонецЕсли;
		ТекстИПЕЧ = НСтр("ru='этап';uk='етап'");
		ТекстИПМЧ = НСтр("ru='этапы';uk='етапи'");
		СтатусЗакрытПредставление = НСтр("ru='""Завершен""';uk='""Завершено""'");
//-- НЕ УТКА
	Иначе
		Если ЗаказыКоличество = 1 Тогда
...
Функция СформироватьТекстОбработано()
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		ТекстДля = НСтр("ru='Для заказов переработчикам (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень переробникам (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЗаказМатериаловВПроизводство" Тогда
		ТекстДля = НСтр("ru='Для заказов материалов в производство (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень матеріалів у виробництво (%КоличествоОбработанных%) %ТекстДействия%'");
	//-- НЕ УТ
	//++ НЕ УТКА
	ИначеЕсли ТипЗаказа = "ЗаказДавальца" Тогда
		ТекстДля = НСтр("ru='Для заказов давальцев (%КоличествоОбработанных%) %ТекстДействия%';uk='Для замовлень давальців (%КоличествоОбработанных%) %ТекстДействия%'");
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		ТекстДля = НСтр("ru='Для этапов производства (%КоличествоОбработанных%) %ТекстДействия%';uk='Для етапів виробництва (%КоличествоОбработанных%) %ТекстДействия%'");
	//-- НЕ УТКА
	КонецЕсли;
