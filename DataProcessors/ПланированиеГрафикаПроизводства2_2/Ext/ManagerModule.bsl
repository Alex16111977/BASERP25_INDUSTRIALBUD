...
Процедура ПланироватьПредварительныйГрафикРаспоряжения(Настройки, АдресХранилища) Экспорт
	...
	Если ПланированиеВФоновомРежиме Тогда
		...
		НаименованиеЗадания = НСтр("ru='Планирование графика производства';uk='Планування графіка виробництва'");
...
Процедура ОчиститьПредварительныйГрафик(Распоряжение, АдресХранилища) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru='График производства.Очистка';uk='Графік виробництва. Очистка'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПланироватьОчередьЗаказовНаПроизводство() Экспорт
	...
	Пока Очередь.Следующий() Цикл
		...
		Если РезультатыПланирования.ЕстьОшибки Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru='Не удалось рассчитать график производства по заказу: %1, по причине %2';uk='Не вдалося розрахувати графік виробництва за замовленням: %1, з причини %2'"),
...
// * ДатыОбеспечения - см. Документы.ЭтапПроизводства2_2.ДатыОбеспеченияЭтапов
...
Процедура ЗафиксироватьОшибкуРазмещенияЭтапа(ПараметрыПланирования, Этап, ПараметрыРазмещения)
	...
	Если КодОшибкиРазмещения = КодОшибкиДоступностьВРЦ() Тогда
		...
		Причина = НСтр("ru='недостаточно доступного времени видов рабочих центров';uk='недостатньо доступного часу видів робочих центрів'");
	...
	ИначеЕсли КодОшибкиРазмещения = КодОшибкиГрафикРаботыПодразделения() Тогда
		...
		Причина = НСтр("ru='не определен график работы подразделения или данные графика не заполнены';uk='не визначений графік роботи підрозділу або дані графіка не заповнені'");
		Ключ = Этап.Подразделение;
		Расшифровка.Вставить("Подразделение", Этап.Подразделение);
		
	ИначеЕсли КодОшибкиРазмещения = КодОшибкиГрафикРаботыПредприятия() Тогда
		
		Причина = НСтр("ru='не определен график работы предприятия или данные графика не заполнены';uk='не визначений графік роботи підприємства чи дані графіка не заповнені'");
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		...
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При планировании %1 не размещен этап ""%2"" по причине: %3.';uk='При плануванні %1 не розміщене етап ""%2"" з причини: %3.'"),
				ОбластьПланирования,
				ПредставлениеЭтапа,
				Причина);
		...
		Если Строка.ГрафикОтсутствует Тогда
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				НомерРаспоряжения = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(
					...
					Истина);
				Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При планировании %1 возникла ошибка: отсутствует график предшествующего этапа ""%2"" (заказ № %3).';uk='При плануванні %1 виникла помилка: відсутній графік попереднього етапу ""%2"" (замовлення № %3).'"),
					ОбластьПланирования,
					ПредставлениеЭтапа,
					НомерРаспоряжения);
...
Процедура ЗафиксироватьОшибкуЕстьЦиклыВЦепочкеЭтапов(ПараметрыПланирования, ЭтапОшибка)
	...
	Если ЗначениеЗаполнено(ЭтапОшибка) Тогда
		...
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При планировании %1 возникла ошибка: обнаружено зацикливание этапов производства на документе ""%2"".';uk='При плануванні %1 виникла помилка: виявлено зациклення етапів виробництва на документі ""%2"".'"),
			ОбластьПланирования,
			ПредставлениеЭтапа);
	...
	Иначе
		...
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При планировании %1 возникла ошибка: обнаружено зацикливание этапов производства.';uk='При плануванні %1 виникла помилка: виявлено зациклення етапів виробництва.'"),
			ОбластьПланирования);
...
Процедура ДобавитьОповещениеОбИзмененииНаправленияПланирования(ПараметрыПланирования)
	...
	Оповещение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='При планировании заказа № %1 использован способ размещения выпуска ""К началу"".';uk='При плануванні замовлення № %1 використаний спосіб розміщення випуску ""До початку"".'"),
		НомерРаспоряжения);
...
Процедура ДобавитьОповещениеОтсутствуютЭтапыДляПланирования(ПараметрыПланирования)
	...
	Оповещение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Отсутствуют этапы заказа № %1 для планирования.';uk='Відсутні етапи замовлення № %1 для планування.'"),
		НомерРаспоряжения);
...
Функция ПредставлениеОбластиПланирования(СтатусГрафика)
	...
	Если СтатусГрафика = РегистрыСведений.ГрафикЭтаповПроизводства2_2.СтатусМодельГрафика() Тогда
		Результат = НСтр("ru='модели';uk='моделі'");
	Иначе
		Результат = НСтр("ru='графика';uk='графіка'");
	КонецЕсли;
...
Функция ЗафиксироватьНомерЗаданияКРасчетуГрафикаПроизводства()
	...
	Исключение
		...
		ОтменитьТранзакцию();
 		...
 		ИмяСобытия = НСтр("ru='График производства.Планирование';uk='Графік виробництва. Планування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Уровень = УровеньЖурналаРегистрации.Ошибка;
		Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(ИмяСобытия, Уровень,,, Комментарий);
...
Процедура ЗаписатьВЖурналРегистрацииОшибкуЗаписиГрафика(Комментарий)
	...
	ИмяСобытия = НСтр("ru='График производства.Запись';uk='Графік виробництва. Запіс'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Уровень = УровеньЖурналаРегистрации.Ошибка;
