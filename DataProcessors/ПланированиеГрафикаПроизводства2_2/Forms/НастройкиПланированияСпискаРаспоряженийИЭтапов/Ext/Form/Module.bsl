...
Процедура ЗаполнитьИнформационныеНадписиПланированиеРаспоряжений(Распоряжения)
	...
	Если РассчитатьГрафик > 0 Тогда
		...
		Элементы.НадписьАктуальностьГрафика.Заголовок = СтрШаблон(
			НСтр("ru='График не актуален. Имеются этапы, требующие перепланирования (всего: %1).';uk='Графік не актуальний. Є етапи, що потебують перепланування (усього: %1).'"),
			РассчитатьГрафик);
		
	Иначе
		
		Элементы.НадписьАктуальностьГрафика.Заголовок = НСтр("ru='График актуален.';uk='Графік актуальний.'");
