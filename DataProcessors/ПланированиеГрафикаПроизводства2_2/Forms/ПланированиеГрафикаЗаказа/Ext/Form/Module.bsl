...
Процедура ДиаграммаСмежныхЭтапов(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждение(,НСтр("ru='Необходимо выбрать этап.';uk='Необхідно вибрати етап.'"));
...
Функция ПланироватьГрафикВФоновомРежиме(НастройкиРасчета)
	...
	НаименованиеЗадания = НСтр("ru='Планирование графика производства заказа';uk='Планування графіка виробництва замовлення'");
...
Процедура ПрочитатьДанныеРаспоряжения()
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Планирование графика производства заказа № %1 от %2';uk='Планування графіка виробництва замовлення № %1 від %2'"),
		НомерРаспоряжения,
...
Процедура ПрочитатьСрокиВыпускаПродукцииВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Чтение сроков выпуска продукции';uk='Читання строків випуску продукції'");
...
Процедура ЗаполнитьИнформационныеНадписи()
	...
	Если РазмещеноВручную > 0 Тогда
		...
		Элементы.НадписьРазмещенныеВручную.Заголовок = СтрШаблон(
			НСтр("ru='Имеются этапы, запланированные вручную (всего: %1).';uk='Є етапи, заплановані вручну (всього: %1).'"),
			РазмещеноВручную);
	...
	Если РассчитатьГрафик > 0 Тогда
		...
		Элементы.НадписьАктуальностьГрафика.Заголовок = СтрШаблон(
			НСтр("ru='График заказа не актуален. Имеются этапы, требующие перепланирования (всего: %1).';uk='Графік замовлення не актуальний. Є етапи, що потребують перепланування (усього: %1).'"),
			РассчитатьГрафик);
		
	Иначе
		
		Элементы.НадписьАктуальностьГрафика.Заголовок = НСтр("ru='График заказа актуален.';uk='Графік замовлення актуальний.'");
...
Процедура СообщитьОбОшибкахПриПланировании()
	...
	Если Ошибки.Количество() > 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='При планировании графика возникли ошибки.';uk='При плануванні графіка виникли помилки.'"),
			,
			"Ошибки");
...
Функция СохранитьНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Сохранение графика производства заказа';uk='Збереження графіка виробництва замовлення'");
...
Функция УдалитьРезультатыПланированияНаСервере()
	...
	НаименованиеЗадания = НСтр("ru='Очистка предварительного графика производства';uk='Очищення попереднього графіка виробництва'");
...
Процедура ЗаполнитьДиаграммуЗагрузкаОборудованияВФоне()
	...
	Если МодельЗапланирована Тогда
		...
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Чтение загрузки оборудования по модели';uk='Читання завантаження устаткування за моделлю'");
...
Процедура ЗаполнитьДиаграммуОбеспечениеМатериаламиВФоне()
	...
	Если МодельЗапланирована Тогда
		...
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Чтение потребности в материалах по модели';uk='Читання потреби в матеріалах за моделлю'");
	...
	Если ЗначениеЗаполнено(Данные) Тогда
		...
		Для Индекс = 0 По Данные.Количество()-1 Цикл
			...
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1, %2 дн.';uk=' %1, %2 дн.'"), ТекстТочки, Значение);
