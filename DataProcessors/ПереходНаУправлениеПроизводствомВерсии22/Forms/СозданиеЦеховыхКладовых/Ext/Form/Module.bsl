...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЕстьОрдернаяСхема Тогда
			ТекстОшибки = НСтр("ru='Склад ""%1"" не может использоваться как цеховая кладовая для подразделения ""%2"", т.к. на складе используется ордерная схема.';uk='Склад ""%1"" не може використовуватися як цехова комора для підрозділу ""%2"", тому що на складі використовується ордерна схема.'");
		...
		Если Выборка.ОднаКладоваяРазныхПодразделений Тогда
			ТекстОшибки = НСтр("ru='Цеховая кладовая ""%1"", указанная для подразделения ""%2"", не может использоваться для других подразделений. У каждого подразделения должна быть своя цеховая кладовая.';uk='Цехова комора ""%1"", зазначена для підрозділу ""%2"", не може використовуватися для інших підрозділів. У кожного підрозділу повинна бути своя цехова комора.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Выборка.СкладНаименование, Выборка.ПодразделениеНаименование);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Выборка.Склад,,, Отказ); 
		КонецЕсли; 
...
|;uk='Відновлено раніше введені значення. 
|Для використання стандартних значень по умовчанню виконайте команду ""Оновити"".'"));
	КонецЕсли; 
...
Процедура ВыполнитьДействия(Команда)
	...
	Если ЕстьОшибки Тогда
		...
		ПоказатьОповещениеПользователя(
			,, 
			НСтр("ru='Создание цеховых кладовых завершено с ошибками (см. журнал регистрации)';uk='Створення цехових комор завершено з помилками (див. журнал реєстрації)'"),
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(
			,, 
			НСтр("ru='Создание цеховых кладовых завершено';uk='Створення цехових комор завершено'"), 
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<создание кладовой не требуется>';uk='<створення комори не потрібно>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<будет создана новая кладовая>';uk='<Буде створена нова комора>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<только для производственного подразделения>';uk='<тільки для виробничого підрозділу>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветНедоступногоТекста);
...
Процедура СоздатьЦеховыеКладовые()
	...
	Для каждого СтрокаПодразделение Из ТаблицаПодразделений Цикл
		...
		ИначеЕсли СтрокаПодразделение.СоздатьКладовую Тогда
			...
			СпрСклад.Наименование = СтрШаблон(НСтр("ru='Цеховая кладовая (%1)';uk='Цехова комора (%1)'"), Строка(СтрокаПодразделение.Подразделение));
			СпрСклад.ЦеховаяКладовая = Истина;
			СпрСклад.Подразделение = СтрокаПодразделение.Подразделение;

			Если НЕ СпрСклад.ПроверитьЗаполнение() Тогда
				ТекстОшибки = НСтр("ru='При проверке заполнения новой цеховой кладовой для подразделения ""%1"" возникли ошибки. Обратитесь к администратору.';uk='При перевірці заповнення нової цехової комори для підрозділу ""%1"" виникли помилки. Зверніться до адміністратора.'");
...
Функция ПолучитьСпособОбеспечения(РеквизитыСпособа)
	...
	НаименованиеСпособа = СтрШаблон(НСтр("ru='Передача в кладовую подразделения ""%1"" со склада ""%2""';uk='Передача в комору підрозділу ""%1"" зі складу ""%2""'"),
	...
	Если НЕ СпособОбеспечения.ПроверитьЗаполнение() Тогда
		ТекстОшибки = НСтр("ru='При проверке заполнения нового способа обеспечения возникли ошибки. Обратитесь к администратору.';uk='При перевірці заповнення нового способу забезпечення виникли помилки. Зверніться до адміністратора.'");
		ДобавитьОшибку(ТекстОшибки);
...
Процедура ДобавитьОшибку(ТекстОшибки, Ссылка = Неопределено)
	...
	ИмяСобытия = НСтр("ru='Переход на управление производством 2_2.Создание кладовых';uk='Перехід на управління виробництвом 2_2.Созданіе комор'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
