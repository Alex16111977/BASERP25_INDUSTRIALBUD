...
Процедура УстановитьСтатусЗакрыт(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке заказов будет установлен статус ""Закрыт"". Продолжить?';uk='У виділених в списку замовлень буде встановлено статус ""Закрито"". Продовжити?'");
...
Процедура ПоказатьИнформационнуюПанель(Результат)
	...
	Если Результат.Свойство("ОтсутствуютКладовые") И Результат.ОтсутствуютКладовые Тогда
		...
		СоздатьЦеховыеКладовые = Новый ФорматированнаяСтрока(
					НСтр("ru='создать цеховые кладовые';uk='створити цехові комори'"),,,, 
					ГиперссылкаСоздатьЦеховыеКладовые());
		
		МассивСтрок = Новый Массив;
		
		МассивСтрок.Добавить(НСтр("ru='Перед конвертацией необходимо';uk='Перед конвертацією необхідно'"));
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(СоздатьЦеховыеКладовые);
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(НСтр("ru='для всех производственных подразделений, используемых в выбранных заказах на производство';uk='для всіх виробничих підрозділів, які використовуються в вибраних замовленнях на виробництво'"));
...
Функция ПроверитьВозможностьКонвертации()
	...
	Если НЕ Результат Тогда
		ПоказатьПредупреждение(, НСтр("ru='Нет данных для конвертации!';uk='Немає даних для конвертації!'"));
	КонецЕсли;
...
Функция ЗапуститьЗаданиеЗаполнитьДеревоЗаказов()
	...
	НаименованиеЗадания = НСтр("ru='Заполнение дерева незавершенных заказов на производство';uk='Заповнення дерева незавершених замовлень на виробництво'");
...
Функция ЗапуститьЗаданиеВыполнитьКонвертациюЗаказов()
	...
	НаименованиеЗадания = НСтр("ru='Конвертация незавершенных заказов на производство';uk='Конвертація незавершених замовлень на виробництво'");
...
Процедура ОповеститьПользователяОРезультатеКонвертации(СтруктураОповещения)
	...
	Если СтруктураОповещения.Свойство("ОбработаноЭтапов", ОбработаноЭтапов) И ОбработаноЭтапов > 0 Тогда
		...
		ТекстСконвертировано = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
				НСтр("ru='Сконвертирован';uk='Сконвертовано'"), НСтр("ru='Сконвертированно';uk='сконвертовано'"), НСтр("ru='Сконвертированно';uk='сконвертовано'"), ОбработаноЭтапов);
		ТекстЭтапы = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
				НСтр("ru='этап';uk='етап'"), НСтр("ru='этапа';uk='етапи'"), НСтр("ru='этапов';uk='етапів'"), ОбработаноЭтапов);
		
		ТекстСообщения = НСтр("ru='%Сконвертированно% %КоличествоОбработанных% %этапов%';uk='%Сконвертированно% %КоличествоОбработанных% %этапов%'");
		...
		ТекстЗаголовка = НСтр("ru='Конвертация заказов завершена';uk='Конвертація замовлень завершена'");
		
	Иначе
		
		ТекстСообщения = НСтр("ru='Не сконвертирован ни один этап';uk='Не сконвертований жоден етап'");
		ТекстЗаголовка = НСтр("ru='Конвертация заказов завершена';uk='Конвертація замовлень завершена'");
