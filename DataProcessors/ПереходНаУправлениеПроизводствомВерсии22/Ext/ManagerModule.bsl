...
Функция ПредставлениеЗаказа(ТекущиеДанные)
	...
	Если ТекущиеДанные.Выполнен Тогда
		...
		ТекстСостояние = НСтр("ru='Выполнен, можно закрыть';uk='Виконано, можна закрити'");
		
	ИначеЕсли ТекущиеДанные.ПоЗаказуДавальца Тогда
		
		ТекстСостояние = НСтр("ru='Конвертация заказов на производство из давальческого сырья недоступна';uk='Конвертація замовлень на виробництво з давальницької сировини недоступна'");
		
	ИначеЕсли ТекущиеДанные.КоличествоДоступно > 0 Тогда
		
		ТекстСостояние = СтрШаблон(
							НСтр("ru='%1 %2 %3 (всего %4)';uk=' %1 %2 %3 (всього %4)'"),
							ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
								НСтр("ru='Доступна';uk='Доступна'"),
								НСтр("ru='Доступно';uk='Доступно'"),
								НСтр("ru='Доступны';uk='Доступні'"),
								ТекущиеДанные.КоличествоВсего),
							ТекущиеДанные.КоличествоДоступно,
							ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
								НСтр("ru='строка';uk='рядок'"),
								НСтр("ru='строки';uk='рядка'"),
								НСтр("ru='строк';uk='рядків'"),
								ТекущиеДанные.КоличествоВсего),
							ТекущиеДанные.КоличествоВсего);
	...
	Иначе
		...
		ТекстСостояние = СтрШаблон(
							НСтр("ru='Доступно 0 строк (всего %1)';uk='Доступно 0 рядків (всього %1)'"), 
							ТекущиеДанные.КоличествоВсего);
		
	КонецЕсли;
	
	Возврат СтрШаблон(НСтр("ru='Заказ № %1 от %2. %3.';uk='Замовлення № %1 від %2.  %3.'"), 
...
Функция ПредставлениеПродукции(ТекущиеДанные)
	...
	Возврат СтрШаблон(
				НСтр("ru='%1,  %2 %3  к  %4';uk=' %1, %2 %3 до %4'"),
...
Функция КомментарийКСтрокеПродукции(ИтогиПоПродукции)
	...
	Если ИтогиПоПродукции.ЭтаповДоступно > 0 Тогда
		...
		Результат = СтрШаблон(
						НСтр("ru='будет %1 %2 %3 (всего %4)';uk='буде %1 %2 %3 (всього %4)'"),
						ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
							НСтр("ru='сконвертирован';uk='сконвертовано'"),
							НСтр("ru='сконвертированно';uk='сконвертовано'"),
							НСтр("ru='сконвертированно';uk='сконвертовано'"),
							ИтогиПоПродукции.ЭтаповДоступно),
						ИтогиПоПродукции.ЭтаповДоступно,
						ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
							НСтр("ru='этап';uk='етап'"),
							НСтр("ru='этапа';uk='етапи'"),
							НСтр("ru='этапов';uk='етапів'"),
							ИтогиПоПродукции.ЭтаповДоступно),
						ИтогиПоПродукции.ЭтаповВсего);
		
	ИначеЕсли ИтогиПоПродукции.ПоЗаказуДавальца Тогда
		
		Результат = НСтр("ru='конвертация строки заказа на производство по давальческой схеме недоступна';uk='конвертація рядка замовлення на виробництво за давальницькою схемою недоступна'");
		
	ИначеЕсли ИтогиПоПродукции.СтрокаВыполнена Тогда
		
		Результат = НСтр("ru='завершено производство';uk='завершено виробництво'");
		
	ИначеЕсли НЕ ИтогиПоПродукции.ГрафикРассчитан Тогда
		
		Результат = НСтр("ru='требуется рассчитать график производства и завершить выполнение этапов';uk='потрібно розрахувати графік виробництва і завершити виконання етапів'");
		
	Иначе
		
		Результат = НСтр("ru='необходимо завершить начатые этапы и закрыть заказ';uk='необхідно завершити розпочаті етапи і закрити замовлення'");
...
Процедура ВыполнитьКонвертациюЗаказов(ПараметрыЗадания, АдресХранилища) Экспорт
	...
	Пока ЕстьЗаписиВВыборке Цикл
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось заблокировать %1. %2';uk='Не вдалося заблокувати %1. %2'"), 
...
Функция КомментарийЗаказа22(Выборка)
	...
	ТекстШаблона = НСтр("ru='Сформирован автоматически обработкой ""Конвертация незавершенных заказов на производство"" по заказу на производство %1 от %2.';uk='Сформовано автоматично обробкою ""Конвертація незавершених замовлень на виробництво"" за замовленням на виробництво %1 від %2.'");
...
Процедура ЗаписатьЗаказ21(Заказ21Объект)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Конвертация незавершенных заказов на производство.Сокращение заказа';uk='Конвертація незавершених замовлень на виробництво. Скороченя замовлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаписатьЗаказ22(Заказ22Объект)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Конвертация незавершенных заказов на производство.Формирование заказа';uk='Конвертація незавершених замовлень на виробництво. Формування замовлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДобавитьВРезультатДанныеСпецификацииЦепочкиЭтапов(Результат, Заказ21Объект, КоллекцияЭтапов, СтруктураИзделия)
	...
	ДанныеСпецификации.Вставить("НекратныйВыпуск", Ложь);
...
Функция КомментарийКЭтапуПроизводства(СтрокаЭтап)
	...
	Если ЗначениеЗаполнено(СтрокаЭтап.ДокументыКУточнению) Тогда
		...
		ТекстКомментария = НСтр("ru='Перед началом выполнения этапа проведите следующие документы:';uk='Перед початком виконання етапу проведіть наступні документи:'");
		...
		ТекстКомментария = ТекстКомментария + Символы.ПС + Символы.ПС;
		ТекстКомментария = ТекстКомментария + НСтр("ru='и установите действие ""Обеспечивать обособленно"" для материалов обеспечиваемых обособленно';uk='і встановіть дію ""Забезпечувати відокремлено"" для матеріалів забезпечуваних відокремлено'");
...
|;uk='Дія не виконана, тому що документ 
|був вилучений або змінений іншим користувачем.'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Конвертация незавершенных заказов на производство.Корректировка назначений в маршрутных листах';uk='Конвертація незавершених замовлень на виробництво. Коригування призначень в маршрутних листах'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПереместитьПолуфабрикатыВЦеховыеКладовые(Полуфабрикаты, СтрокаЭтап, Заказ21Объект, ПерваяПопытка = Истина)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Конвертация незавершенных заказов на производство.Перемещение полуфабрикатов в цеховые кладовые';uk='Конвертація незавершених замовлень на виробництво. Переміщення напівфабрикатів в цехові комори'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для каждого СтрокаЭтап Из КоллекцияЭтапов Цикл
		...
		Попытка
			...
			Если Этап22Объект = Неопределено Тогда
				ТекстИсключения = НСтр("ru='Не удалось получить объект типа <ДокументОбъект.ЭтапПроизводства2_2>.';uk='Не вдалося отримати об''єкт типу <ДокументОбъект.ЭтапПроизводства2_2>.'");
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Конвертация незавершенных заказов на производство.Корректировка заказа материалов';uk='Конвертація незавершених замовлень на виробництво. Коригування замовлення матеріалів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
