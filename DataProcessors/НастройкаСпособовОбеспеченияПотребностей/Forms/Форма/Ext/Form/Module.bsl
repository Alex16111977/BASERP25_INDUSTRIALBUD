...
Процедура ВыполнитьКомандуПослеВопроса(Результат, ПараметрыВыполненияКоманды) Экспорт
	...
	ИначеЕсли ПараметрыВыполненияКоманды.Имя = "ЗаполнитьСклад" Тогда
		...
		Если ВыбранныеПодразделения.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Необходимо выбрать производственные подразделения';uk='Необхідно вибрати виробничі підрозділи'"));
			Возврат;
		КонецЕсли; 
	...
	ИначеЕсли ПараметрыВыполненияКоманды.Имя = "ОчиститьСклад" Тогда
		...
		ТекстВопроса = НСтр("ru='Для выбранных подразделений будет очищен склад. Продолжить?';uk='Для обраних підрозділів буде очищений склад. Продовжити?'");
