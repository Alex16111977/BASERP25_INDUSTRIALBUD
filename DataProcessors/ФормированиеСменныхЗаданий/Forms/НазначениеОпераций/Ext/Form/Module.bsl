...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.КлючиОпераций) <> Тип("Массив")
		...
		ВызватьИсключение НСтр("ru='Не переданы операции для назначения';uk='Не передані операції для призначення'");
	...
	Если ГрупповоеНазначение Тогда
		...
		Заголовок = СтрШаблон(НСтр("ru='Назначение операций (%1 шт)';uk='Призначення операцій (%1 шт)'"), КлючиОпераций.Количество());
	...
	Иначе
		...
		Заголовок = НСтр("ru='Назначение операции';uk='Призначення операції'");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ НазначитьИсполнителя
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указан объект назначения';uk='Не вказаний об''єкт призначення'"),,,,
			Отказ);
...
Процедура НастроитьЭлементыФормы() 
	...
	Элементы.Исполнитель.ПодсказкаВвода       = ?(ГрупповоеНазначение, НСтр("ru='Исполнитель';uk='Виконавець'"), "");
	Элементы.ВидРабочегоЦентра.ПодсказкаВвода = ?(ГрупповоеНазначение, НСтр("ru='Вид рабочего центра';uk='Вид робочого центру'"), "");
	Элементы.РабочийЦентр.ПодсказкаВвода      = ?(ГрупповоеНазначение, НСтр("ru='Рабочий центр';uk='Робочий центр'"), "");
