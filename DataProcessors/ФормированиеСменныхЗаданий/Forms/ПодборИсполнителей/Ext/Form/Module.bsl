...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СменыГруппа.Заголовок = ?(ПараметрыПодразделения.ИспользоватьУчастки,
		НСтр("ru='Смена, Участок, Исполнитель';uk='Зміна, Ділянка, Виконавець'"),
		НСтр("ru='Смена, Исполнитель';uk='Зміна, Виконавець'"));
...
Процедура ОбработкаВыбора()
	...
	Если Элементы.СпособВыбора.ТекущаяСтраница = Элементы.ПрошлыеСмены Тогда
		...
		Если Ссылки.ВГраница() = -1 И НЕ ВыбраноДействие Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не выделены элементы для выбора';uk='Не виділені елементи для вибору'"));
		КонецЕсли;
	...
	Иначе
		...
		Если Ссылки.ВГраница() = -1 Тогда
			...
			Если ВыбранаГруппа Тогда
				ТекстПредупреждения = НСтр("ru='Команда не может быть выполнена для строки группировки списка';uk='Команда не може бути виконана для рядка групування списку'");
			Иначе
				ТекстПредупреждения = НСтр("ru='Не выделены элементы для выбора';uk='Не виділені елементи для вибору'");
			КонецЕсли;
	...
	Если Ссылки.Количество() > 1 И НЕ МножественныйВыбор Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Может быть выбран лишь один исполнитель';uk='Може бути вибраний лише один виконавець'"));
...
Процедура ПрочитатьСмены(ТочкаОтсчета, Количество)
	...
	Если ВыборкаИтоги.Количество() > Количество Тогда
		...
		СтрокаДействие.Действие = НСтр("ru='<показать еще>';uk='<показати ще>'");
...
Процедура УстановитьОтборПоПодразделению()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты")
		...
		ИнтеграцияБЗК.ЗаполнитьФизическиеЛицаПодразделения(МассивФизЛиц, ПодразделениеОтбор);
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Работники, 
			"Ссылка", 
			МассивФизЛиц,
