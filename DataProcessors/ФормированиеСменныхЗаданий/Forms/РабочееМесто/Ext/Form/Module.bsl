...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		МассивСтрок.Добавить(НСтр("ru='Установлен отбор по этапу';uk='Встановлено відбір за етапом'"));
		...
		МассивСтрок.Добавить(" (");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='отключить';uk='відключити'"),,,, "ОтключитьОтборПоЭтапу"));
		МассивСтрок.Добавить(") ");
...
Процедура НастроитьСоставСмен(Команда)
	...
	Если Подразделение.Пустая() Тогда
		...
		ОчиститьСообщения();
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru='Подразделение';uk='Підрозділ'"));
...
Процедура НазначитьЧасть(Команда)
	...
	Иначе
		...
		ИначеЕсли КлючиОпераций.Количество() > 1 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Действие может быть выполнено только для одной строки';uk='Дія може бути виконана тільки для одного рядка'"));
			Возврат;
		КонецЕсли;
...
Процедура ОткрытьЗадание(Команда)
	...
	Если Элементы.Смены.ТекущиеДанные = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не выбрана смена';uk='Не обрана зміна'"));
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("РежимНазначения") 
		...
		Если Форма.РежимНазначения = РежимНазначенияИсполнителям() Тогда
			ОбъектНазначения = НСтр("ru='Исполнитель';uk='Виконавець'");
		ИначеЕсли Форма.РежимНазначения = РежимНазначенияРабочимЦентрам() Тогда
			ОбъектНазначения = НСтр("ru='Рабочий центр';uk='Робочий центр'");
		ИначеЕсли Форма.РежимНазначения = РежимНазначенияВидамРЦ() Тогда
			ОбъектНазначения = НСтр("ru='Вид РЦ';uk='Вид РЦ'");
		КонецЕсли;
		
		Если Форма.ЕстьУчастки Тогда
			Элементы.СменыГруппаПредставление.Заголовок = СтрШаблон(НСтр("ru='Смена, Участок, %1';uk='Зміна, Ділянка, %1'"), ОбъектНазначения);
		Иначе
			Элементы.СменыГруппаПредставление.Заголовок = СтрШаблон(НСтр("ru='Смена, %1';uk='Зміна, %1'"), ОбъектНазначения);
		КонецЕсли;
...
Процедура ЗаполнитьСмены()
	...
	Пока ВыборкаСЗ.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ОбъектНазначения.Пустая() Тогда
				...
				СтрокаОбъект.ПредставлениеОбъекта = ?(РежимНазначения = РежимНазначенияИсполнителям(),
					НСтр("ru='<исполнитель не назначен>';uk='<виконавець не призначений>'"),
					?(РежимНазначения = РежимНазначенияРабочимЦентрам(),
						НСтр("ru='<рабочий центр не назначен>';uk='<робочий центр не призначений>'"),
						НСтр("ru='<вид РЦ не назначен>';uk='<тип РЦ не вказаний>'")));
			Иначе
		...
		Если РежимНазначения = РежимНазначенияИсполнителям() Тогда
			...
			СтрокаКоманда.Действие = НСтр("ru='<добавить исполнителей>';uk='<додати виконавців>'");
...
Процедура УдалитьТекущегоИсполнителя()
	...
	Если СтрокаОбъект = Неопределено Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Не выбран исполнитель';uk='Не вибраний виконавець'"));
		Возврат;
		
	ИначеЕсли СтрокаОбъект.Загрузка > 0 Тогда
		
		ПоказатьПредупреждение(, НСтр("ru='Исполнителю назначены операции, удаление невозможно';uk='Виконавцю призначені операції, вилучення неможливо'"));
		Возврат;
...
Процедура ПоказатьПредупреждениеНеВыбранаСмена()
	...
	ПоказатьПредупреждение(, НСтр("ru='Не выбрана смена для назначения';uk='Не вибрана зміна для призначення'"));
