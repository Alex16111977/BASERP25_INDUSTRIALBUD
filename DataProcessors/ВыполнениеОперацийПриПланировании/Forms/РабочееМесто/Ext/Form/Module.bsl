...
Процедура Операции_УстановитьСтатусВыполняется(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке операций будет установлен статус ""Выполняется"". Продолжить?';uk='У виділених в списку операцій буде встановлено статус ""Виконується"". Продовжити?'");
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("Операции", ВыделенныеСтроки);
	ДопПараметры.Вставить("Статус", "Выполняется");
	ДопПараметры.Вставить("СтатусПредставление", НСтр("ru='Выполняется';uk='Виконується'"));
...
Процедура Операции_УстановитьСтатусВыполнена(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке операций будет установлен статус ""Выполнена"". Продолжить?';uk='У виділених в списку операцій буде встановлено статус ""Виконано"". Продовжити?'");
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("Операции", ВыделенныеСтроки);
	ДопПараметры.Вставить("Статус", "Выполнена");
	ДопПараметры.Вставить("СтатусПредставление", НСтр("ru='Выполнена';uk='Виконано'"));
...
Процедура Операции_УстановитьСтатусНеВыполнена(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке операций будет установлен статус ""Не выполнена"". Продолжить?';uk='У виділених в списку операцій буде встановлено статус «Не виконано"". Продовжити?'");
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("Операции", ВыделенныеСтроки);
	ДопПараметры.Вставить("Статус", "НеВыполнена");
	ДопПараметры.Вставить("СтатусПредставление", НСтр("ru='Не выполнена';uk='Не виконана'"));
...
Процедура Операции_УстановитьСтатусПропущена(Команда)
	...
	ТекстВопроса = НСтр("ru='У выделенных в списке операций будет установлен статус ""Пропущена"". Продолжить?';uk='У виділених в списку операцій буде встановлено статус ""Пропущено"". Продовжити?'");
	
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("Операции", ВыделенныеСтроки);
	ДопПараметры.Вставить("Статус", "Пропущена");
	ДопПараметры.Вставить("СтатусПредставление", НСтр("ru='Пропущена';uk='Пропущена'"));
...
Процедура УстановитьОтборПоИсполнителю()
	...
	ОтборПоНоменклатуре = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		...
		НСтр("ru='Отбор по исполнителю';uk='Відбір за виконавцем'"),
...
Процедура СформироватьНадписьУстановленОтборПоЭтапам(СписокЭтапов)
	...
	Если СписокЭтапов.Количество() > 1 Тогда
		...
		МассивСтрок.Добавить(НСтр("ru='Установлен отбор по списку этапов';uk='Встановлено відбір за списком етапів'"));
...
Процедура НастроитьЗависимыеЭлементыФормы(СписокРеквизитов = "")
	...
	Если Инициализация ИЛИ СтруктураРеквизитов.Свойство("Состояние") Тогда
		...
		Если ТолькоАктивныеСостояния(Состояние) Тогда
			СостояниеСтрока = НСтр("ru='Незавершенные';uk='Незавершені'");
		Иначе
