	...
	Если ВариантОформления = Перечисления.ВариантыОформленияДокументовПеремещения.ЗаказНаПеремещение Тогда
		...
		Для Каждого Элемент Из РезультатРазбиения Цикл
			...
			Если СтруктураДоступа.ЗаказМатериаловДоступен И Кладовые.Получить(Элемент.Ключ.СкладПолучатель) = Истина Тогда
				...
				//++ НЕ УТ
				ДанныеЗаполненияДокумента = Новый Структура("Склад,ЦеховаяКладовая,НаправлениеДеятельности,Товары");
				...
				ДокументОбъект = Обработки.ОбеспечениеПотребностей.ЗаполнитьИПровестиДокумент(
					...
					Перечисления.СтатусыЗаказовМатериаловВПроизводство.КОбеспечению,
					СообщенияОбОшибках);
				//-- НЕ УТ
	...
	ИначеЕсли ВариантОформления = Перечисления.ВариантыОформленияДокументовПеремещения.ПеремещениеТоваров Тогда
		...
		Для Каждого Элемент Из РезультатРазбиения Цикл
			...
			Если СтруктураДоступа.ПередачаМатериаловДоступна И Кладовые.Получить(Элемент.Ключ.СкладПолучатель) = Истина Тогда
				...
				//++ НЕ УТ
				РеквизитыШапки = Новый Структура("Отправитель,Получатель,НаправлениеДеятельности,Дата");
				ДанныеЗаполненияДокумента = Новый Структура("РеквизитыШапки, Товары", РеквизитыШапки);
				ДанныеЗаполненияДокумента.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ПередачаМатериаловВКладовую);
				
				ДанныеЗаполненияДокумента.РеквизитыШапки.Отправитель             = Элемент.Ключ.СкладОтправитель;
				ДанныеЗаполненияДокумента.РеквизитыШапки.Получатель              = Элемент.Ключ.СкладПолучатель;
				ДанныеЗаполненияДокумента.РеквизитыШапки.НаправлениеДеятельности = Элемент.Ключ.НаправлениеДеятельности;
				ДанныеЗаполненияДокумента.РеквизитыШапки.Дата                    = ТекущаяДатаСеанса();
				ДанныеЗаполненияДокумента.Товары = Элемент.Таблица;
