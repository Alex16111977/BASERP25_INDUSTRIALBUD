...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ОбъектНастройки", ОбъектНастройки) Тогда
		ТекстСообщения = НСтр("ru='Непосредственное открытие этой формы не предусмотрено. Для открытия формы можно воспользоваться командой ""Настроить правила"" в форме настроек правил получения факта.';uk='Безпосереднє відкриття цієї форми не передбачено. Для відкриття форми можна скористатися командою ""Настроїти правила"" в формі настройок правил отримання факту.'");
	...
	ИначеЕсли ТипЗнч(Параметры.ОбъектНастройки) = РеглУчетКлиентСервер.ТипПланСчетов() Тогда
		Элементы.ОбъектНастройки.Заголовок = НСтр("ru='Счет РСБУ';uk='Рахунок РСБО'");
		ПерваяЧастьКлюча = "3";
	//++ НЕ УТКА	
