...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СнятиеРезерва И РеквизитыНазначения.ТипНазначения = Перечисления.ТипыНазначений.Давальческое21 Тогда
		ТекстИсключения = НСтр("ru='Запрещена операция снятия резерва для заказа давальца производства версии 2.1.';uk='Заборонена операція зняття резерву для замовлення давальця виробництва версії 2.1.'");
		ВызватьИсключение ТекстИсключения;
		Возврат;
	КонецЕсли;
	//-- НЕ УТКА
