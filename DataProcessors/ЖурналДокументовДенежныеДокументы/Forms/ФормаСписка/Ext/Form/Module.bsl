...
Процедура СоздатьПоступлениеДенежныхДокументов(ХозОперация, Список)
	...
	Иначе
		...
		Если СообщитьОбОшибкахФормированияДанныхЗаполненияДД(НаПроверку, Проверка) Тогда
			...
			ТекстСообщения = НСтр("ru='Поступление денежных документов будет создано с указанием значений %1 из строки, на которой установлен курсор';uk='Надходження грошових документів буде створено із зазначенням значень %1 з рядка, на якому встановлений курсор'");
			ТекстРеквизитов = "";
...
Процедура СоздатьВыбытиеДенежныхДокументов(ХозОперация, Список)
	...
	Иначе
		...
		Если СообщитьОбОшибкахФормированияДанныхЗаполненияДД(НаПроверку, Проверка) Тогда
			ТекстСообщения = НСтр("ru='Документ не может быть создан.';uk='Документ не може бути створений.'"); // В результате проверки обнаружены ошибки
