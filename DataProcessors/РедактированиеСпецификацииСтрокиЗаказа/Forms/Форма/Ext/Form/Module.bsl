...
|;uk='Безпосереднє відкриття форми ""Редагування специфікації рядку замовлення"" не передбачено. 
|Для відкриття форми можна скористатися командою ""Змінити позицію замовлення"" в формі документа ""Замовлення на виробництво"".'");
	...
	Иначе
		...
		Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
			...
			ИначеЕсли ВыделенныеЭтапы.Найти(СтрокаТаблицы.Этап) = Неопределено Тогда
				ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки одного этапа';uk='Необхідно вибрати рядки одного етапу'");
				Прервать;
			КонецЕсли;
...
&НаСервере
Функция ПараметрыВыбораАналогов(ИмяТаблицы, ВыделенныеСтроки = Истина, Остатки = Ложь)
	...
	ДанныеТоваров = ПолучитьДанныеТоваров(ИмяТаблицы, ВыделенныеСтроки, ОтборСтрокЗаменаНаАналоги(), Остатки);
...
Функция ПолучитьДанныеТоваров(ИмяТаблицы, ВыделенныеСтроки, ОтборСтрок, Остатки)
	...
	СтруктураПолей = АналогиМатериалов.СтруктураПолейТаблицыДляЗаменыНаАналоги("Документ.ЗаказНаПроизводство");
...
Процедура ОбновитьСостояниеСпецификацииЗаказа(Форма)
	...
	Если (ЕстьУточненияМатериалов ИЛИ ЕстьУточненияПобочныхИзделий) И ЕстьОшибкиЗаполнения Тогда
		Форма.ТекстИнформационнойНадписи = НСтр("ru='Требуется уточнить номенклатуру, обнаружены ошибки заполнения.';uk='Потрібно уточнити номенклатуру, виявлені помилки заповнення.'");
	...
	ИначеЕсли ЕстьУточненияПобочныхИзделий Тогда
		Форма.ТекстИнформационнойНадписи = НСтр("ru='Требуется уточнить побочные изделия.';uk='Потрібно уточнити побічні вироби.'");
...
Процедура СоздатьСпецификациюНаОснованииСпецификацииЗаказаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru='Ошибка создания спецификации.';uk='Помилка створення специфікації.'"));
