...
Функция ДокументыКОбработке(Первые)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 1", "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ " + Формат(Первые, "ЧГ="));
...
Процедура ЗаполнитьСтоимость(Параметры = "", АдресХранилища = "") Экспорт
	...
	Если ИспользоватьУчетСебестоимости Тогда
		ОбновитьЗаданияКРасчетуСтоимости();
	Иначе
...
Процедура ОбновитьЗаданияКРасчетуСтоимости()

	ВременныеТаблицы = Новый МенеджерВременныхТаблиц;
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗаданияКРасчетуСтоимостиВНА.Организация КАК Организация,
	|	ЗаданияКРасчетуСтоимостиВНА.ОбъектУчета КАК ОбъектУчета
	|ИЗ
	|	РегистрСведений.ЗаданияКРасчетуСтоимостиВНА КАК ЗаданияКРасчетуСтоимостиВНА";
	...
	РегистрыСведений.ЗаданияКРасчетуСтоимостиВНА.СформироватьЗаданияНаСледующийПериод(
		Константы.ДатаНачалаУчетаСебестоимости.Получить(), 
		ВременныеТаблицы);
