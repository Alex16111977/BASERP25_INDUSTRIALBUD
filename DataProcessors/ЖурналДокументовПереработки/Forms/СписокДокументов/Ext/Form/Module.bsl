...
Функция ОписаниеОперацийИТиповДокументов()
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru='Документы приема в переработку (все)';uk='Документи приймання в переробку (всі)'");
	Строка.Порядок						= 11;
	...
	Строка.ЗаголовокРабочегоМеста		= НСтр("ru='Документы передачи в переработку (все)';uk='Документи передачі в переробку (всі)'");
	Строка.Порядок						= 21;
...
Процедура НастроитьФормуПоВыбраннымОперациямИДокументам(ТЗХозОперацииИТипыДокументов)
	...
	ДанныеРабочегоМеста = ОбщегоНазначенияУТ.ДанныеРабочегоМеста(ТЗХозОперацииИТипыДокументов,
		КлючНазначенияФормыПоУмолчанию(), НСтр("ru='Документы приема и передачи в переработку (все)';uk='Документи приймання і передачі в переробку (всі)'"));
...
Процедура ОбновитьГиперссылкуКОформлению()
	...
	КОформлениюПрием = ОбщегоНазначенияУТ.СформироватьГиперссылкуКОформлению(
		...
		ПараметрыФормирования, ?(ЕстьПрием И ЕстьПередача, НСтр("ru='К оформлению (прием в переработку):';uk='До оформлення (прийом в переробку):'"),""));
	...
	КОформлениюПередача = ОбщегоНазначенияУТ.СформироватьГиперссылкуКОформлению(
		...
		ПараметрыФормирования, ?(ЕстьПрием И ЕстьПередача, НСтр("ru='К оформлению (передача в переработку):';uk='До оформлення (передача в переробку):'"),""));
