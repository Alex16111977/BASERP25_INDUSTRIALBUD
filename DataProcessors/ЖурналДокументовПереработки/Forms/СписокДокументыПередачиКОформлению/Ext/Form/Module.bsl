...
Процедура ОформитьОтчеты(Команда)
	...
	Иначе
		...
		Для каждого Данные Из Сводно Цикл
			ТекстОшибки = НСтр("ru='Одновременное оформление отчета по переработчику %1 в рамках организации %2 вместе с другими распоряжениями невозможно. Выберите данную строку отдельно.';uk='Одночасне оформлення звіту по переробнику %1 в рамках організації %2 разом з іншими розпорядженнями неможливо. Виберіть цей рядок окремо.'");
		...
		Иначе
			...
			ТекстОшибки = НСтр("ru='Не выбрано ни одного документа, для которого можно было бы оформить отчет переработчика.';uk='Не вибрано жодного документа, для якого можна було б оформити звіт переробника.'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Представление = НСтр("ru='Передача имеет высокий приоритет';uk='Передача має високий пріоритет'");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Поступление имеет высокий приоритет';uk='Надходження має високий пріоритет'");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Отчет имеет высокий приоритет';uk='Звіт має високий пріоритет'");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Передача имеет низкий приоритет';uk='Передача має низький пріоритет'");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Поступление имеет низкий приоритет';uk='Надходження має низький пріоритет'");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Отчет имеет низкий приоритет';uk='Звіт має низький пріоритет'");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Отчет без распоряжения';uk='Звіт без розпорядження'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<без распоряжения>';uk='<Без розпорядження>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	
	////
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru='Отчет поле не используется';uk='Звіт поле не використовується'");
...
Процедура НастроитьФормуПоПравамИФункциональнымОпциям()
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьОрдернуюСхемуПриОтгрузке")
		...
		Элементы.СписокРаспоряженияНаПередачуОформитьПоЗаказам.Заголовок = НСтр("ru='Оформить';uk='Оформити'");
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьОрдернуюСхемуПриПоступлении")
		...
		Элементы.СписокРаспоряженияНаПоступлениеОформитьПоЗаказам.Заголовок = НСтр("ru='Оформить';uk='Оформити'");
	...
	Иначе
		...
		СписокВыбора.Добавить("1", НСтр("ru='Создать отчет';uk='Створити звіт'"),    Ложь, БиблиотекаКартинок.СоздатьНакладную);
		СписокВыбора.Добавить("2", НСтр("ru='Дооформить отчет';uk='Дооформити звіт'"), Ложь, БиблиотекаКартинок.ДооформитьНакладную);
	...
	Если ЗначениеЗаполнено(Распоряжения) Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтрШаблон(НСтр("ru='Документы передачи в переработку (%1) (к оформлению)';uk='Документи передачі в переробку (%1) (до оформлення)'"),
...
Процедура ОткрытьОтчетСостояниеВыполненияДокументов(ИмяТаблицы, ТаблицаФормы)
	...
	Иначе
		...
		Если СписокДокументов.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru='Выполнение команды возможно только для заказов переработчику.';uk='Виконання команди можливо тільки для замовлень переробнику.'"));
			Возврат;
		КонецЕсли;
...
Функция ПараметрыЖурнала()
	...
	ПараметрыЖурнала.Вставить("СинонимЖурнала",				НСтр("ru='Документы передачи в переработку';uk='Документи передачі в переробку'"));
...
Процедура ЗаполнитьМетаданныеДокументов()
	...
	Строка.ЗаголовокФормыПереоформления	= НСтр("ru='Переоформление передачи сырья переработчику по выбранным распоряжениям';uk='Переоформлення передачі сировини переробнику за вибраними розпорядженням'");
	...
	Строка.ЗаголовокФормыПереоформления	= НСтр("ru='Переоформление поступления от переработчика по выбранным распоряжениям';uk='Переоформлення надходження від переробника за вибраними розпорядженням'");
