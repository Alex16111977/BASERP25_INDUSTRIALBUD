			...
			Для Каждого Множитель Из СтрокиМножителей Цикл
				...
				Для Каждого СтрокаМатериала Из ДанныеСпецификации.МатериалыИУслуги Цикл
					...
					Если Не (СтрокаМатериала.Запланировать 
						Или СтрокаМатериала.СпособПолученияМатериала = Перечисления.СпособыПолученияМатериаловВСпецификации.ПроизвестиПоСпецификации) Тогда
						Продолжить;
					КонецЕсли;
...
Процедура ЗаписатьМножителиДокумента(Калькуляция, ТаблицыДляДвижений)
	...
	Если Не ТаблицыДляДвижений.Свойство("ТаблицаМножителиПартийИПолуфабрикатов", Множители) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьВЖурналОшибкуРассчета(Калькуляция, Причина)
	...
	ТекстСообщения = НСтр("ru='Не удалось выполнить проведение документа %Ссылка%  по причине: %Причина%';uk='Не вдалося виконати проведення документа %Ссылка% через: %Причина%'");
...
Функция СобытиеЖурналаРегистрацииОшибкаРасчета() Экспорт
	...
	Возврат НСтр("ru='Фоновое проведение калькуляции продукции.';uk='Фонове проведення калькуляції продукції.'",ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьСпецификацииНоменклатуры(СпособыПолученияМатериалов)
	...
	ПараметрыЗапроса = УправлениеДаннымиОбИзделиях.ПараметрыТекстаЗапросаСпецификацийНоменклатуры();
	...
	ТекстЗапроса = ТекстЗапроса + УправлениеДаннымиОбИзделиях.ТекстЗапросаСпецификацийНоменклатуры(
																	ПараметрыЗапроса,
...
Процедура ЗаполнитьСтоимости(ПараметрыРасчета)
	...
	Попытка
		...
		ТаблицыДляДвижений = Новый Структура("ТаблицаМножителиПартийИПолуфабрикатов", МассивРезультатов[3].Выгрузить());
...
Процедура ЗаполнитьСтоимостьМатериалов(Калькуляция, МенеджерВТ)
	...
	Материалы = ПолучитьЦеныНоменклатуры(Калькуляция, МенеджерВТ);
	...
	Запрос.Текст =
		...
		|					ТОГДА 1
		|				ИНАЧЕ ВЫБОР
		|						КОГДА ПлановыеМатериальныеЗатраты.Номенклатура.СтавкаНДС.Ставка > 0
		|							ТОГДА (ПлановыеМатериальныеЗатраты.Номенклатура.СтавкаНДС.Ставка + 100) / 100
		|						ИНАЧЕ ПлановыеМатериальныеЗатраты.Номенклатура.СтавкаНДС.Ставка
		|					КОНЕЦ
		|			КОНЕЦ
		|	КОНЕЦ КАК Стоимость,
		...
		|			ТОГДА 1
		|		ИНАЧЕ (ПлановыеМатериальныеЗатраты.Номенклатура.СтавкаНДС.Ставка + 100) / 100
...
Функция ПолучитьЦеныНоменклатуры(Калькуляции, МенеджерВТ, ПоместитьВВТ = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрНакопления.ПлановыеМатериальныеЗатраты КАК ПлановыеМатериальныеЗатраты
		|ГДЕ
		|	ПлановыеМатериальныеЗатраты.Калькуляция В (&СписокКалькуляций)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПлановыеМатериальныеЗатраты.Калькуляция,
		|	ПлановыеМатериальныеЗатраты.Номенклатура,
	...
	Иначе
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОМЕСТИТЬ ЦеныНоменклатуры", "");
...
Процедура ЗаполнитьСтоимостьПостатейных(Калькуляция, МенеджерВТ)
	...
	ТекстЗапроса =
		...
		|		РегистрСведений.МножителиПартийИПолуфабрикатов КАК МножителиПартийИПолуфабрикатов
		|	ГДЕ
		|		МножителиПартийИПолуфабрикатов.Калькуляция = &Калькуляция) КАК Т
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Запрос.Текст = 
		...
		|					ЕСТЬNULL(БазыРаспределенияПредварительная.Затрата.ГруппаАналитическогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка))))
		...
		|		ПО (ВЫБОР	
		|				КОГДА НормыПостоянныхРасходов.СтатьяРасходов.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
		|					ТОГДА НормыПостоянныхРасходов.СтатьяРасходов.ПравилоРаспределенияРасходовРегл = ПравилаРаспределенияРасходов.Ссылка
		|				ИНАЧЕ
		|					НормыПостоянныхРасходов.СтатьяРасходов.ПравилоРаспределенияРасходовУпр = ПравилаРаспределенияРасходов.Ссылка
		|			КОНЕЦ)
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПравилаРаспределенияРасходов КАК ПравилаРаспределенияРасходов
		|		ПО (ВЫБОР	
		|				КОГДА НормыПеременныхРасходов.СтатьяРасходов.ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
		|					ТОГДА НормыПеременныхРасходов.СтатьяРасходов.ПравилоРаспределенияРасходовРегл = ПравилаРаспределенияРасходов.Ссылка
		|				ИНАЧЕ
		|					НормыПеременныхРасходов.СтатьяРасходов.ПравилоРаспределенияРасходовУпр = ПравилаРаспределенияРасходов.Ссылка
		|			КОНЕЦ)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПозицииОтборовПоЗатратам КАК ПозицииОтборовПоЗатратам
		|		ПО (ПравилаРаспределенияРасходов.Ссылка = ПозицииОтборовПоЗатратам.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПозицииОтборовПоГруппамПродукции КАК ПозицииОтборовПоГруппамПродукции
		...
		|	ВЫБОР 
		|		КОГДА КурсыВалютУУ.Кратность = 0
		|			ТОГДА 0
		|		ИНАЧЕ
		|			ПлановыеПрочиеЗатратыПредварительная.СтоимостьБезНДС * КурсыВалютУУ.Курс / КурсыВалютУУ.Кратность
		|	КОНЕЦ КАК СтоимостьРегл,
...
Процедура РассчитатьЦеныМатериалов(Калькуляция, МенеджерВТ)
	...
	РесурсыКРасчету = Новый Массив;
	РесурсыКРасчету.Добавить("Стоимость");
	РесурсыКРасчету.Добавить("СтоимостьБезНДС");
	РесурсыКРасчету.Добавить("СтоимостьРегл");
	РесурсыКРасчету.Добавить("Трудозатраты");
	РесурсыКРасчету.Добавить("ТрудозатратыРегл");
	РесурсыКРасчету.Добавить("ПостатейныеПостоянные");
	РесурсыКРасчету.Добавить("ПостатейныеПостоянныеБезНДС");
	РесурсыКРасчету.Добавить("ПостатейныеПостоянныеРегл");
	РесурсыКРасчету.Добавить("ПостатейныеПеременные");
	РесурсыКРасчету.Добавить("ПостатейныеПеременныеБезНДС");
	РесурсыКРасчету.Добавить("ПостатейныеПеременныеРегл");
	РесурсыКРасчету.Добавить("СтоимостьЗабалансовая");
	РесурсыКРасчету.Добавить("СтоимостьЗабалансоваяРегл");
	...
	Для Каждого Ресурс Из РесурсыКРасчету Цикл
		ЦеныМатериалов.Колонки.Добавить("Цена" + Ресурс, ТипДенежногоПоля);
	КонецЦикла;
	...
	Пока ВыборкаПолуфабрикатов.Следующий() Цикл
		...
		Для Каждого Ресурс Из РесурсыКРасчету Цикл
			ПоказателиРесурсов.Вставить(Ресурс, 0);
		КонецЦикла;
		...
		Для Каждого Затрата Из ЗатратыПолуфабриката Цикл
			...
			Если ЗначениеЗаполнено(Затрата.Материал) Тогда
				...
				Для Каждого Ресурс Из РесурсыКРасчету Цикл
					СредннееРесурсов.Вставить(Ресурс, 0);
				КонецЦикла;
				
				СтрокиСЦенами = ЦеныМатериалов.НайтиСтроки(ОтборЦен);
				Для Каждого СтрокаЦены Из СтрокиСЦенами Цикл
					Для Каждого Ресурс Из РесурсыКРасчету Цикл
						СредннееРесурсов[Ресурс] = СредннееРесурсов[Ресурс] + СтрокаЦены["Цена" + Ресурс];
					КонецЦикла;
				КонецЦикла;
				
				Для Каждого Ресурс Из РесурсыКРасчету Цикл
					ПоказателиРесурсов[Ресурс] = ПоказателиРесурсов[Ресурс] + СредннееРесурсов[Ресурс] * Затрата.КоличествоМатериала;
				КонецЦикла;
				
			Иначе
				Для Каждого Ресурс Из РесурсыКРасчету Цикл
					ПоказателиРесурсов[Ресурс] = ПоказателиРесурсов[Ресурс] + Затрата[Ресурс];
				КонецЦикла;
			КонецЕсли;
		...
		Если Не ВыборкаПолуфабрикатов.Количество = 0 Тогда
			Для Каждого Ресурс Из РесурсыКРасчету Цикл
				ЦенаМатериала["Цена" + Ресурс] = ПоказателиРесурсов[Ресурс] 
...
Процедура ПроверитьСтоимость(Калькуляции)
	...
	Если НЕ ЗначениеЗаполнено(Проверка) Тогда
		ВызватьИсключение НСтр("ru='Не найдена служебная контекстная проверка для операции ""Калькуляция продукциии""';uk='Не знайдена службова контекстна перевірка для операції ""Калькуляція продукциии""'");
	КонецЕсли;
