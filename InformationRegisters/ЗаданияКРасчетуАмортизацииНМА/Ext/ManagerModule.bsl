...
Процедура СоздатьЗаписиРегистраПоДаннымВыборки(Выборка, НомерЗадания = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать задание к расчету амортизации НМА за %1 в организации %2 по причине: %3';uk='Не вдалося сформувати завдання до розрахунку амортизації НМА за %1 в організації %2 по причині: %3'"),
			Выборка.Месяц,
			Выборка.Организация,
			ТекстОшибки);
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "РегистрыСведений.ЗаданияКРасчетуАмортизацииНМА.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "РегистрыСведений.ЗаданияКРасчетуАмортизацииНМА.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("6eb2ede9-8f6b-4f74-bc3e-73f6333bdf38");
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.УдалитьПервоначальныеСведенияНМАБухгалтерскийУчет,"
		+ "РегистрСведений.УдалитьСостоянияНМАОрганизаций," 
		+ "Документ.АмортизацияНМА"; 
	Обработчик.ИзменяемыеОбъекты = "РегистрСведений.ЗаданияКРасчетуАмортизацииНМА";
	Обработчик.БлокируемыеОбъекты = "РегистрСведений.ЗаданияКРасчетуАмортизацииНМА";
	Обработчик.Комментарий = НСтр("ru='Заполняет регистр ""Задания к расчету амортизации НМА"":
                                   |- добавляет задания к расчету амортизации НМА.
                                   |Пока обработчик не завершен состояние выполнения операций закрытия месяца может быть некорректным.'
                                   |;uk='Заповнює регістр ""Завдання для розрахунку амортизації НМА"":
                                   |- додає завдання для розрахунку амортизації НМА.
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	АмортизацияНМА.Ссылка, АмортизацияНМА.Организация";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ТребуетсяАмортизацияНМА21(Выборка.Организация, Выборка.Дата) Тогда	
			Продолжить;
		КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось сформировать задание к расчету амортизации НМА по причине: %Причина%';uk='Не вдалося сформувати завдання до розрахунку амортизації НМА по причині: %Причина%'");
...
// Определяет необходимость расчета амортизации НМА в указанном периоде по данным еще не перенесенных из 2.1
//
// Параметры:
...
//
Функция ТребуетсяАмортизацияНМА21(Организация, Период) Экспорт

	ТекстЗапроса = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	1 КАК Поле1
	|ИЗ
	|	РегистрСведений.УдалитьПервоначальныеСведенияНМАБухгалтерскийУчет.СрезПоследних(
	|			&Период,
	|			Организация В (&Организация)
	|				ИЛИ &ПоВсемОрганизациям) КАК СведенияНМА
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УдалитьСостоянияНМАОрганизаций.СрезПоследних КАК СостоянияНМА
	|		ПО СведенияНМА.НематериальныйАктив = СостоянияНМА.НематериальныйАктив
	|			И (СведенияНМА.НачислятьАмортизацию)
	|			И (СостоянияНМА.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияНМА.ПринятКУчету))
	|ГДЕ
