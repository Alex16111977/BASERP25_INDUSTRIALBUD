...
Функция ТекстЗапросаДляРасчетаСостоянийОбеспеченияЗаказа()
	...
	ТекстЗапроса = "
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗаказДавальцаТовары.Ссылка КАК Ссылка,
		|	ЗаказДавальцаТовары.Номенклатура КАК Номенклатура,
		|	ЗаказДавальцаТовары.Характеристика КАК Характеристика,
		|	ЗаказДавальцаТовары.Склад КАК Склад,
		|	ЗаказДавальцаТовары.ВариантОбеспечения КАК ВариантОбеспечения,
		|	СУММА(ЗаказДавальцаТовары.Количество) КАК Количество
		|ПОМЕСТИТЬ ВТСводТоваровПоЗаказамДавальца
		|ИЗ
		...
		|	ЗаказДавальцаТовары.Номенклатура,
		|	ЗаказДавальцаТовары.Характеристика,
		|	ЗаказДавальцаТовары.Склад,
		|	ЗаказДавальцаТовары.ВариантОбеспечения
		|;
		|
		//-- НЕ УТКА
		|////////////////////////////////////////////////////////////////////////////////
		...
		|					И ЕСТЬNULL(ОбеспечениеЗаказовОстатки.НаличиеПодЗаказОстаток, 0) < ЗаказДавальцаТовары.Количество - ЗаказДавальцаТовары.Количество * (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ)
		|ИЗ
		|	ВТСводТоваровПоЗаказамДавальца КАК ЗаказДавальцаТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ОбеспечениеЗаказов.Остатки(, Назначение.Заказ В (&МассивЗаказов)) КАК ОбеспечениеЗаказовОстатки
