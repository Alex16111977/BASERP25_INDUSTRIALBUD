...
Процедура СоздатьЗаписиРегистраПоДаннымВыборки(Выборка, НомерЗадания = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(
			НСтр("ru='Не удалось сформировать задание к расчету стоимости ОС и НМА за %1 в организации %2 по причине: %3';uk='Не вдалося сформувати завдання до розрахунку вартості ОЗ і НМА за %1 в організації %2 по причині: %3'"),
			Выборка.Месяц,
			Выборка.Организация,
			ТекстОшибки);
			
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Закрытие месяца';uk='Закриття місяця'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Процедура ЗафиксироватьРасчет(Организация, НачалоРасчета, НомерЗадания, ЗаданияКРасчету) Экспорт
	...
	Попытка
		...
		СформироватьЗаданияНаСледующийПериод(КонецМесяца(НачалоРасчета) + 1, ЗаданияКРасчету.ВременныеТаблицы);
...
//
// Параметры:
//  НачалоСледующегоМесяца	 - Дата				 - Начало месяца, начиная с которого нужно создать задания.
//  ВременныеТаблицы		 - МенеджерВременныхТаблиц	 - Должен содержать таблицу "КэшГраниц" с полями "Организация", "ОбъектУчета".
//
Процедура СформироватьЗаданияНаСледующийПериод(НачалоСледующегоМесяца, ВременныеТаблицы) Экспорт
	
	// Запрос аналогичен запросу в ВнеоборотныеАктивы.ТекстЗапросаРасчетСтоимости()
	ТекстЗапроса =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	(ДанныеКРасчету.Организация, ДанныеКРасчету.АналитикаКапитализацииРасходов) В
	|			(ВЫБРАТЬ
	|				КэшГраниц.Организация,
	...
	|ГДЕ
	|	(ДанныеКРасчету.Организация, ДанныеКРасчету.НематериальныйАктив) В
	|			(ВЫБРАТЬ
	|				КэшГраниц.Организация,
	|				КэшГраниц.ОбъектУчета
	|			ИЗ
	...
	Запрос.УстановитьПараметр(
		...
		ВнеоборотныеАктивы.ТипыДокументовПоКоторымРассчитываетсяСтоимость());
	
	Запрос.УстановитьПараметр(
		"ХозяйственныеОперацииПоКоторымРассчитываетсяСтоимость", 
		ВнеоборотныеАктивы.ХозяйственныеОперацииПоКоторымРассчитываетсяСтоимость());
