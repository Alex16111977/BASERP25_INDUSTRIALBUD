...
Функция СписокСчетовУчетаПоРазделуУчета(РазделУчета) Экспорт
	...
	Если РазделУчета = "РасчетыСПоставщиками" Тогда
		...
		СчетаУчетаРегл.Добавить("РасчетыСКредиторамиОсновнойДолг", НСтр("ru='Кредиты (счет 602)';uk='Кредити (рахунок 602)'"), ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыКредитовИДепозитов"));
   	КонецЕсли;
