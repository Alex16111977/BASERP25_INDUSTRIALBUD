...
Процедура ЗаписатьСостоянияПоРаспоряжениям(Выборка)
	...
	Пока ЕстьЗаписиВВыборке Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отразить состояние этапа по распоряжению: %Ссылка% по причине: %Причина%';uk='Не вдалося відобразити стан етапу за розпорядженням: %Ссылка% з причини: %Причина%'");
...
Процедура ЗаписатьСостоянияПоЭтапам(Выборка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отразить состояние этапа: %Ссылка% по причине: %Причина%';uk='Не вдалося відобразити стан етапу: %Ссылка% з причини: %Причина%'");
...
Процедура ЗаписатьСостоянияПоЭтапамВыборочно(Выборка, Показатели, ОбработанныеРаспоряжения = Неопределено)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось отразить состояние обеспечения этапа: %Ссылка% по причине: %Причина%';uk='Не вдалося відобразити стан забезпечення етапу: %Ссылка% з причини: %Причина%'");
...
Функция ТекстЗапросаВТСостоянияОпераций()
	...
	ТекстЗапроса =
		...
		|		И НЕ ПроизводственнаяОперация.НаОснованииМаршрутнойКарты
		|	
		|	СГРУППИРОВАТЬ ПО
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru='Межцеховое управление.Ошибка отражения состояния обеспечения этапов';uk='Міжцехове управління. Помилка відображення стану забезпечення етапів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
