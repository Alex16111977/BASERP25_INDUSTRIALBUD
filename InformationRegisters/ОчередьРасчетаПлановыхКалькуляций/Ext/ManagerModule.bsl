...
Процедура ЗаписатьОчередь(Очередь, Калькуляция) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось записать очередь расчета плана: %Ссылка% по причине: %Причина%';uk='Не вдалося записати чергу розрахунку плану: %Ссылка% через: %Причина%'");
...
Процедура ОчиститьОчередь(Очередь, Калькуляция) Экспорт
	...
	Для каждого Строка Из Очередь Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось очистить очередь расчета плана: %Ссылка% по причине: %Причина%';uk='Не вдалося очистити чергу розрахунку плану: %Ссылка% через: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", Калькуляция);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru='Запись очереди расчета калькуляции продукции';uk='Запис черги розрахунку калькуляції продукції'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
