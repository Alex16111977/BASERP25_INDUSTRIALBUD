...
Процедура ЗаписатьРезультатыПланирования(Распоряжение, СтатусГрафика, ЗагрузкаВРЦ) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru='Запись результатов планирования загрузки видов рабочих центров';uk='Запис результатів планування завантаження видів робочих центрів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ОчиститьПредварительныйГрафик(Распоряжение) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru='Очистка предварительного графика';uk='Очищення попереднього графіка'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
  		ВызватьИсключение;
