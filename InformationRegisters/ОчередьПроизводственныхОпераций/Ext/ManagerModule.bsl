...
//
Процедура РассчитатьОчередьОтложенно(Задание, ПропуститьЗадание = Ложь, ДополнительныеДанные = Неопределено) Экспорт
...
Функция ВыполнитьКонтрольПередРасчетомОчереди(ДанныеЭтапа, УдалениеПроведения = Ложь, Отказ = Ложь)
	...
	Если ДанныеЭтапа.ТребуетсяРассчитать И НЕ УдалениеПроведения Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если Выборка.ОшибкаВОперациях Тогда
				ТекстСообщения = НСтр("ru='Запрещено изменять маршрутную карту и коэффициент после создания производственных операций.';uk='Заборонено змінювати маршрутну карту і коефіцієнт після створення виробничих операцій.'");
			Иначе
				ТекстСообщения = НСтр("ru='Запрещено изменять маршрутную карту и коэффициент после создания сменных заданий.';uk='Заборонено змінювати маршрутну карту і коефіцієнт після створення змінних завдань.'");
			КонецЕсли;
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			...
			Если Выборка.ОшибкаВОперациях Тогда
				ТекстСообщения = НСтр("ru='По этапу созданы операции.';uk='По етапу створені операції.'");
			Иначе
				ТекстСообщения = НСтр("ru='По этапу созданы сменные задания.';uk='По етапу створені змінні завдання.'");
			КонецЕсли;
	...
	Если ЕстьОшибка Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru='Ошибка расчета очереди производственных операций. %1';uk='Помилка розрахунку черги виробничих операцій.  %1'"), ТекстСообщения);
...
Процедура ЗаблокироватьДанныеДляРасчетаОчереди(Этап, Отказ)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось заблокировать очередь производственных операций: %Причина%';uk='Не вдалося заблокувати чергу виробничих операцій: %Причина%'");
...
Процедура СообщитьПользователюОбОшибкеРасчетаОчереди(Отказ)
	...
	ТекстСообщения = НСтр("ru='Не удалось рассчитать очередь производственных операций.';uk='Не вдалося розрахувати чергу виробничих операцій.'");
