...
Процедура РассчитатьОперации(Этапы) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось рассчитать операции к созданию сменных заданий';uk='Не вдалося розрахувати операції до створення змінних завдань'");
...
Процедура НазначитьОперации(КлючиОпераций, Параметры) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось назначить операции';uk='Не вдалося призначити операції'");
...
Процедура УдалитьОперацииПоКлючам(КлючиОпераций) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось удалить операции';uk='Не вдалося вилучити операції'");
...
Процедура РегистрВДокументы(СменноеЗадание, СтатусДокументов) Экспорт
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтатусДокументов = Перечисления.СтатусыПроизводственныхОпераций.Выполнена
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(НСтр("ru='При создании производственной операции ""%1"" возникли ошибки';uk='При створенні виробничої операції ""%1"" виникли помилки'"),
						ОперацияОбъект.Наименование));
				
				ВызватьИсключение НСтр("ru='Ошибка создания производственной операции';uk='Помилка створення виробничої операції'");
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось создать производственные операции';uk='Не вдалося створити виробничі операції'");
...
Процедура ДокументыВРегистр(СменноеЗадание) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось загрузить данные из производственных операций';uk='Не вдалося завантажити дані з виробничих операцій'");
...
Процедура ИсключитьОперацииСНарушениемПорядкаНазначения(СменноеЗадание) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось исключить операции';uk='Не вдалося виключити операції'");
...
Процедура СкорректироватьОперацииСНарушениемПорядкаНазначения(СменноеЗадание) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось скорректировать операции';uk='Не вдалося скоригувати операції'");
...
Процедура ДобавитьВЗадание(Операции, СменноеЗадание) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Не удалось добавить операции';uk='Не вдалося додати операції'");
...
Функция ТекстЗапросаВремяОперации(ИмяТаблицыРегистр) Экспорт
	...
	Возврат	СтрЗаменить(
		...
		|			ТОГДА Регистр.ВремяШтучное
		|		ИНАЧЕ Регистр.ВремяШтучное * Регистр.Количество / Регистр.Операция.Количество
	...
	Пока ВыборкаЭтап.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Количество < 0 Тогда
				...
				ВызватьИсключение НСтр("ru='Попытка изменить несуществующую операцию';uk='Спроба змінити неіснуючу операцію'");
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Операции к созданию сменных заданий';uk='Операції до створення змінних завдань'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
