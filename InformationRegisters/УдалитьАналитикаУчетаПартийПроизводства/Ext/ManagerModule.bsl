	...
	Если НаборЗаписей <> Неопределено Тогда
		...
		Исключение
			...
			Если НаборЗаписей.Количество() = 0 Тогда // запись не создана из-за ошибки заполнения полей.
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Создание нового ключа аналитики учета партий производства';uk='Створення нового ключа аналітики обліку партій виробництва'",КодОсновногоЯзыка),
...
Функция ПолучитьНаборЗаписей(ПараметрыАналитики)
	...
	Иначе
		...
		Для Каждого КлючЗначение Из СтруктураАналитики Цикл
			...
			Измерение = НаборЗаписей.Отбор[КлючЗначение.Ключ]; // ЭлементОтбора
			Измерение.Установить(КлючЗначение.Значение);
...
Функция ПолучитьПолноеНаименованиеКлючаАналитики(МенеджерЗаписи)
	...
	ПустоеНаименование = НСтр("ru='<Запись с пустыми ключами>';uk='<Запис з порожніми ключами>'");
