...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// А_ Доработка: отбор по СтатьеДДС (оказание услуг между подразделениями)
	Если Параметры.Свойство("А_СтатьяДДСОтбор")
		И ЗначениеЗаполнено(Параметры.А_СтатьяДДСОтбор) Тогда

		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			СписокОбъектовРасчетов,
			"СтатьяДвиженияДенежныхСредств",
			Параметры.А_СтатьяДДСОтбор,
			ВидСравненияКомпоновкиДанных.Равно,
			,
			Истина);

		А_ОтборСтатьяДДС = Параметры.А_СтатьяДДСОтбор;
		Элементы.А_ОтборСтатьяДДСПоле.Видимость = Истина;
		Элементы.А_ОтборПодразделениеУслугиПоле.Видимость = Истина;
...
// А_ Доработка: обработчик фильтра по подразделению (оказание услуг между подразделениями)
&НаКлиенте
Процедура А_ОтборПодразделениеУслугиПриИзменении(Элемент)

	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		СписокОбъектовРасчетов,
		"ПодразделениеУслуги",
		А_ОтборПодразделениеУслуги,
		ВидСравненияКомпоновкиДанных.Равно,
		,
		ЗначениеЗаполнено(А_ОтборПодразделениеУслуги));

КонецПроцедуры

// А_ Доработка: обработчик фильтра по статье ДДС (оказание услуг между подразделениями)
&НаКлиенте
Процедура А_ОтборСтатьяДДСПриИзменении(Элемент)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		...
		ВидСравненияКомпоновкиДанных.Равно,
		,
		ЗначениеЗаполнено(А_ОтборСтатьяДДС));
