	...
	Пока ДатаОкончания < КонецПериода Цикл
		...
		Для Каждого Период Из ИерархияПериодов Цикл
			Если ЗначениеЗаполнено(Период) Тогда
				...
				НовыйИнтервал[КолонкаПериод] = ДатаКонцаПериода(НовыйИнтервал.ДатаНачала, Период, КонецПериода);
			КонецЕсли;
		КонецЦикла;
...
Функция СхемаМонетарногоПоказателя(Показатель, Измерения = Неопределено, Субконто = Неопределено, Ресурс = "Сумма")
	...
	Если Субконто <> Неопределено Тогда
		Для каждого СубконтоСчета Из Субконто Цикл
			...
			СхемаКД.НаборыДанных.ЗначенияПоказателя.Запрос = СтрЗаменить(СхемаКД.НаборыДанных.ЗначенияПоказателя.Запрос, "//" + Поле, Поле + " КАК " + СубконтоСчета.Ключ);
...
// 
// Параметры:
// 	ИерархияПериодов - Массив, Неопределено - Описание
// Возвращаемое значение:
// 	ТаблицаЗначений - Описание:
