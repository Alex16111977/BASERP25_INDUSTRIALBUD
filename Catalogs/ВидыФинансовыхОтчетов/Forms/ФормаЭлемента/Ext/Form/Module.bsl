...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() И Не ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		Объект.ЗаголовокКодаСтроки = НСтр("ru='Код';uk='Код'");
		Объект.ЗаголовокПримечания = НСтр("ru='Примечание';uk='Примітка'");
...
Процедура ВыбратьВидПоказателей(Команда)
	...
	ВидыИзмерений 		= ВидыИспользуемыхИзмерений();
	ПараметрыФормы.Вставить("ЕстьИзмерения", ВидыИзмерений.МеждународныйУчет ИЛИ ВидыИзмерений.РеглУчет);
	ПараметрыФормы.Вставить("ЕстьИзмеренияМУ", ВидыИзмерений.МеждународныйУчет);
	ПараметрыФормы.Вставить("ЕстьИзмеренияБУ", ВидыИзмерений.РеглУчет);
...
&НаСервере
Функция ВидыИспользуемыхИзмерений()

	ВидыПоказателей = Новый Структура("МеждународныйУчет,РеглУчет",Ложь,Ложь);
	ДеревоОтчета = РеквизитФормыВЗначение("ЭлементыОтчета");
	
	Измерение = ВидЭлементаСервер("Измерение");
	НайденныеСтроки = ДеревоОтчета.Строки.НайтиСтроки(Новый Структура("ВидЭлемента", Измерение), Истина);
	Для Каждого СтрокаОтчета Из НайденныеСтроки Цикл
		Если СтрокаОтчета.ЭтоСубконто Тогда
			Если ЗначениеЗаполнено(СтрокаОтчета.СчетПоказательИзмерение) Тогда
				Субконто = СтрокаОтчета.СчетПоказательИзмерение;
			Иначе
				Субконто = ФинансоваяОтчетностьСервер.ЗначениеДополнительногоРеквизита(Неопределено, "ВидСубконто", СтрокаОтчета.ЭлементОтчета.РеквизитыВидаЭлемента);
			КонецЕсли;
			ПроверитьТип(ВидыПоказателей, Неопределено, Субконто);
		КонецЕсли;
...
&НаСервере
Процедура ПроверитьТип(ВидыПоказателей, СчетПланаСчетов, Субконто = Неопределено)
	...
	Если ЗначениеЗаполнено(Субконто) Тогда
		Если ТипЗнч(Субконто) = МеждународныйУчетКлиентСервер.ТипВидСубконто() Тогда
			ВидыПоказателей.МеждународныйУчет = Истина;
		//++ Локализация
		ИначеЕсли ТипЗнч(Субконто) = РеглУчетКлиентСервер.ТипВидСубконто() Тогда
			ВидыПоказателей.РеглУчет = Истина;
		//-- Локализация
		КонецЕсли;
	КонецЕсли;
