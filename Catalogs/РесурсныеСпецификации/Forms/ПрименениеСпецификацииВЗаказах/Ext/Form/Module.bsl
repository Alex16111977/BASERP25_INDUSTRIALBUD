...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ НастройкиПодсистемыПроизводство.ИспользуетсяПроизводство Тогда 
		ВызватьИсключение НСтр("ru='Для открытия формы необходимо включить подсистему управления производством.';uk='Для відкриття форми необхідно включити підсистему управління виробництвом.'");
	КонецЕсли;
...
|;uk='Заміна специфікації допускається тільки для продукції і напівфабрикатів, 
|виробництво яких не запущено (заплановано).'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСписокЗаказов()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПроизводитсяВПроцессе И Выборка.УправлениеПроизводством22 Тогда
			...
			ПредставлениеЗаказа = СтрШаблон(НСтр("ru='№%1 от %2 (%3, строка %4)';uk='№%1 від %2 (%3, рядок %4)'"), 
											НомерЗаказа, 
		...
		Иначе
			...
			ПредставлениеЗаказа = СтрШаблон(НСтр("ru='№%1 от %2 (строка %3)';uk='№%1 від %2 (рядок %3)'"), 
											НомерЗаказа, 
											Формат(Выборка.Дата, "ДЛФ=D"),	
											Формат(Выборка.НомерСтроки, "ЧГ="));
