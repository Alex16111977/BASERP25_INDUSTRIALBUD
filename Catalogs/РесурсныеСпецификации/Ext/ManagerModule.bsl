...
Процедура ПроверитьВложенныеСпецификации(Объект, Отказ) Экспорт
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстОшибки = НСтр("ru='Спецификация ""%1"" должна быть действующей (см. список ""%2"", строка %3)';uk='Специфікація ""%1"" має бути діючою (див. список ""%2"", рядок %3)'");
		...
		Если ЕстьЦиклыВГрафе Тогда
			ТекстОшибки = НСтр("ru='Использование в спецификации ""%1"" спецификации ""%2"" приводит к зацикливанию.';uk='Використання в специфікації  ""%1"" специфікації ""%2"" призводить до зациклення.'");
...
//++ НЕ УТКА
	...
	|	ХарактеристикаДополнительныеРеквизиты.Свойство.Заголовок,
...
Функция ОписаниеИсточниковДанныхОперандов(Объект = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ФормироватьФинансовыйРезультат") Тогда
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"НаправлениеДеятельности", НСтр("ru='Направление деятельности';uk='Напрям діяльності'"));
...
Функция ПолучитьПараметрыАвтовыбора(Таблицы, МенеджерВременныхТаблиц, ПараметрыВыборки)
	...
	ЗначенияСвойствИзделий.Индексы.Добавить("СвойствоЗаголовок");
