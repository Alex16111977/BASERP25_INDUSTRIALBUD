...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НомерОперации >= НомерСледующейОперации 
		...
		ТекстСообщения = НСтр("ru='Номер следующей операции должен быть больше номера операции.';uk='Номер наступної операції повинен бути більше номера операції.'");
	...
	Если ТипЗнч(РабочийЦентр) = Тип("СправочникСсылка.РабочиеЦентры") И ЗначениеЗаполнено(РабочийЦентр) Тогда
		...
		Если РеквизитыРабочийЦентр.ВидРабочегоЦентраПараллельнаяЗагрузка 
			...
			ТекстСообщения = НСтр("ru='Загрузка операции превышает максимальную загрузку рабочего центра.';uk='Завантаження операції перевищує максимальне завантаження робочого центру.'");
