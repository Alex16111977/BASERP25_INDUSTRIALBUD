...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого Строка Из ДопРеквизиты Цикл
		...
		Если Строка.ЗначениеМин > Строка.ЗначениеМакс И Строка.ЗначениеМакс <> 0 Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru='Минимальное значение реквизита ""%1"" превышает максимальное значение';uk='Мінімальне значення реквізиту ""%1"" перевищує максимальне значення'"),
				Строка.Заголовок);
...
Процедура НастроитьВидимостьДоступностьПриЧтенииСоздании()
	...
	Элементы.ГруппаКоличество.Видимость         = ВидимостьПараметров;
	Элементы.ГруппаВыполнение.Видимость         = ВидимостьПараметров;
	Элементы.ГруппаПередаточнаяПартия.Видимость = ВидимостьПараметров
...
Процедура ЗаполнитьЕдиницуИзмеренияПередаточнойПартии(Форма)
	...
	Если Форма.Объект.ПередаточнаяПартия = 0 Тогда
		Форма.ЕдиницаИзмеренияПередаточнойПартии = НСтр("ru='передается вся партия этапа';uk='передається вся партія етапу'");
	Иначе
...
Процедура УстановитьУсловноеОформление()
	...
	Если СвойстваБезНормативов.Количество() > 0 Тогда
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не используются>';uk='<не використовуються>'"));
