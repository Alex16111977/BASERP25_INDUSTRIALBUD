...
Процедура КомандаПроверитьЗаполнение(Команда)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Маршрутная карта не может быть действующей.';uk='Маршрутна карта не може бути діючою.'");
	КонецЕсли;
...
Процедура Скопировать(Команда)
	...
	Если Модифицированность Тогда
		ПоказатьПредупреждение(, НСтр("ru='Объект не записан!';uk='Об''єкт не записано!'"));
		Возврат;
	КонецЕсли;
...
Процедура ВозвратныеОтходы_НастроитьАвтовыборНоменклатуры(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Не выбрана строка таблицы ""Побочный и промежуточный выпуск"".';uk='Не вибраний рядок таблиці ""Побічний і проміжний випуск"".'");
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийТекст);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<первая операция>';uk='<перша операція>'"));
...
Процедура ЗаполнитьПоМаршрутнойКарте(Источник)
	...
	Попытка
		...
		Если НЕ Справочники.МаршрутныеКарты.ЗаполнитьПоМаршрутнойКарте(Объект, Источник, СсылкаПриемника) Тогда
			ВызватьИсключение НСтр("ru='Не удалось заполнить по маршрутной карте.';uk='Не вдалося заповнити за маршрутною картою.'");
		КонецЕсли;
	...
	Исключение
		...
		ОтменитьТранзакцию();
 		...
 		ЗаписьЖурналаРегистрации(
			НСтр("ru='Заполнение маршрутной карты';uk='Заповнення маршрутної карти'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура НастроитьРасшифровкуКоэффициентЗнаменатель(Форма)
	...
	ПредставлениеЕдиницы = ОбщегоНазначенияУТКлиентСервер.ФормаМножественногоЧисла(
		НСтр("ru='единицу';uk='одиницю'"),
		НСтр("ru='единицы';uk='одиниці'"),
		НСтр("ru='единиц';uk='одиниць'"),
		Форма.Объект.КоэффициентЗнаменатель);
	
	Форма.Элементы.РасшифровкаКоэффициентЗнаменатель.Заголовок = СтрШаблон(
		НСтр("ru='%1 основного изделия спецификации';uk=' %1 основного виробу специфікації'"),
