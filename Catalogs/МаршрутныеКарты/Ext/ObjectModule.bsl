...
Процедура ПроверитьВложенныеМаршруты(Отказ) Экспорт
	...
	ТекстОшибки = НСтр("ru='Превышена максимальная глубина вложенности маршрутных карт %1';uk='Перевищена максимальна глибина вкладеності маршрутних карт %1'");
...
Процедура ПроверитьРеквизитыВложенногоМаршрута(Выборка, Отказ)
	...
	Если Выборка.Статус <> Перечисления.СтатусыМаршрутныхКарт.Действует Тогда
		...
		ТекстОшибки = НСтр("ru='Статус вложенного маршрута ""%1"" маршрутной карты ""%2"" должен быть ""Действует"".';uk='Статус вкладеного маршруту ""%1"" маршрутної карти ""%2"" повинен бути ""Діє"".'");
	...
	Если Выборка.Подразделение <> ЭтотОбъект.Подразделение Тогда
		...
		ТекстОшибки = НСтр("ru='Подразделение вложенного маршрута ""%1"" должно совпадать с подразделением маршрутной картой ""%2"".';uk='Підрозділ вкладеного маршруту ""%1"" має співпадати з підрозділом маршрутною картою ""%2"".'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, Выборка.Представление, Выборка.ВладелецПредставление);
...
Процедура ПроверитьЗаполнениеНоменклатурыИКоличества(Отказ)
	...
	ШаблонСообщенияНоменклатура = НСтр("ru='Не заполнена колонка ""Номенклатура"" в строке %1 списка ""%2""';uk='Не заповнена колонка ""Номенклатура"" в рядку %1 списку ""%2""'");
	ШаблонСообщенияКоличество   = НСтр("ru='Не заполнена колонка ""Количество"" в строке %1 списка ""%2""';uk='Не заповнена колонка ""Кількість"" в рядку %1 списку ""%2""'");
...
#Область Автовыбор
	...
	МассивПроверок = Новый Массив;
	Если ИспользоватьВыходныеИзделия Тогда
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "ВыходныеИзделия", НСтр("ru='Выходные изделия';uk='Вихідні вироби'"), "Номенклатура"));
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "ВозвратныеОтходы", НСтр("ru='Промежуточный и побочный выпуск';uk='Проміжний і побічний випуск'"), "Номенклатура"));
	КонецЕсли;
	Если ИспользоватьМатериалы Тогда
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "МатериалыИУслуги", НСтр("ru='Материалы и работы';uk='Матеріали та роботи'"), "Номенклатура"));
	КонецЕсли;
...
#Область ОтборПоСвойствам
	...
	Если ИспользоватьТрудозатраты Тогда
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "Трудозатраты", НСтр("ru='Трудозатраты';uk='Трудовитрати'"), "ВидРабот"));
	КонецЕсли;
...
Процедура ПроверитьОперацииВыполняютсяВТекущемПодразделении(Отказ)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ШаблонТекстаСообщения = НСтр("ru='Рабочий центр операции ""%1"" не принадлежит подразделению ""%2""';uk='Робочий центр операції ""%1"" не належить підрозділу ""%2""'");
