...
Функция ПоследовательностьОперацийПравильная(МаршрутнаяКарта, Отказ = Ложь) Экспорт
	...
	СтруктураСообщений.Вставить("НесколькоПоследнихОпераций",    НСтр("ru='Не может быть несколько последних операций.';uk='Не може бути кілька останніх операцій.'"));
...
Функция СкопироватьМаршрутнуюКартуСОперациями(Источник) Экспорт
	...
	Попытка
		...
		Если НЕ ЗаполнитьПоМаршрутнойКарте(Объект, Источник, Ссылка) Тогда
			ВызватьИсключение НСтр("ru='Не удалось скопировать маршрутную карту.';uk='Не вдалося скопіювати маршрутну карту.'");
		КонецЕсли;
	...
	Исключение
		...
		ОтменитьТранзакцию();
 		...
 		ЗаписьЖурналаРегистрации(
			НСтр("ru='Копирование маршрутной карты';uk='Копіювання маршрутної карти'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОписаниеИсточниковДанныхОперандов(Объект = Неопределено) Экспорт
	...
	// [ОсновноеИзделие]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
		"ОсновноеИзделие", НСтр("ru='Основное изделие';uk='Основний виріб'"));
	
	НовыйРеквизит = УправлениеДаннымиОбИзделиях.СоздатьОписанияРеквизитаИсточникаДанныхОперандов(
		"Количество", НСтр("ru='Количество';uk='Кількість'"));
	Описание.Реквизиты.Добавить(НовыйРеквизит);
	...
	Если Объект = Неопределено
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"ВыходныеИзделия", НСтр("ru='Выходные изделия';uk='Вихідні вироби'"));
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"ВыходноеИзделие", НСтр("ru='Выходное изделие (текущая строка)';uk='Вихідний виріб (поточний рядок)'"));
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"ВозвратныеОтходы", НСтр("ru='Побочный выход';uk='Побічний вихід'"));
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"ВозвратныйОтход", НСтр("ru='Побочный выход (текущая строка)';uk='Побічний вихід (поточний рядок)'"));
	...
	Если Объект = Неопределено
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"Материал", НСтр("ru='Материал (текущая строка)';uk='Матеріал (поточний рядок)'"));
	...
	// [Подразделение]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
		"Подразделение", НСтр("ru='Подразделение';uk='Підрозділ'"));
	...
	Если Настройки.ИспользуетсяПроизводство22 И Не Настройки.ИспользуетсяПроизводство21 Тогда
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			"Распоряжение", НСтр("ru='Заказ на производство';uk='Замовлення на виробництво'"));
	...
	Если ПолучитьФункциональнуюОпцию("ФормироватьФинансовыйРезультат") Тогда
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов("НаправлениеДеятельности", НСтр("ru='Направление деятельности';uk='Напрям діяльності'"));
		
		Описание.СодержитСвойства = Истина;
		Описание.НаборСвойств     = Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_НаправленияДеятельности;
...
Функция ТекстЗапросаОтборПоСвойствам()
	...
	|		ПО МаршрутныеКартыОтборПоСвойствам.Свойство = СвойстваНаправленияДеятельности.Свойство
	|			И (СвойстваНаправленияДеятельности.Ссылка = &НаправлениеДеятельности)
	|
	|ГДЕ
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("83ad3876-d006-49fc-9725-d6c674aa79b3");
	...
	Обработчик.Комментарий = НСтр("ru='Заполняет возможные пустые ключи связи в таблице трудозатрат';uk='Заповнює можливі порожні ключі зв''язку в таблиці трудовитрат'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Справочники.МаршрутныеКарты.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Справочники.МаршрутныеКарты.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Справочники.МаршрутныеКарты.ПолноеИмя());
