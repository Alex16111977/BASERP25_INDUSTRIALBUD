...
//++ НЕ УТКА
	...
	Если ТекущиеДанные = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru='Не выбрана строка таблицы ""Виды рабочих центров"".';uk='Не вибраний рядок таблиці ""Види робочих центрів"".'");
...
Процедура РассчитатьКоэффициентМаршрутнойКарты(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru='Коэффициент рассчитан';uk='Коефіцієнт розрахований'"),
		,
		,
...
//++ НЕ УТКА
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<определяется ВРЦ с синхронной загрузкой>';uk='<визначається ВРЦ з синхронним завантаженням>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
...
//++ НЕ УТКА
	...
	Если СтруктураРеквизитов.Свойство("МаршрутнаяКарта")
		...
		Элементы.КоэффициентМаршрутнойКарты.АвтоОтметкаНезаполненного = ЗначениеЗаполнено(Объект.МаршрутнаяКарта);
...
Процедура ЗаполнитьПояснениеОсновныхНастроек(Форма)
	...
	Если (НЕ ПланироватьПоПроизводственнымРесурсам И ПланироватьПоМатериальнымРесурсам)
		...
		МассивСтрок.Добавить(
			НСтр("ru='Виды рабочих центров используются для построения графика с учетом доступности (в управлении производством версии 2.1).';uk='Види робочих центрів використовуються для побудови графіка з урахуванням доступності (в управлінні виробництвом версії 2.1).'"));
		
		МассивСтрок.Добавить(
			НСтр("ru='Длительность этапа используется для построения графика производства (в управлении производством версии 2.5).';uk='Тривалість етапу використовується для побудови графіка виробництва (в управлінні виробництвом версії 2.5).'"));
	...
	Иначе
		...
		Если (ПланироватьПоПроизводственнымРесурсам ИЛИ Форма.ИспользуетсяПроизводство21) И ИспользоватьВРЦ Тогда
			...
			МассивСтрок.Добавить(
				НСтр("ru='Виды рабочих центров используются для построения графика с учетом доступности.';uk='Види робочих центрів використовуються для побудови графіка з урахуванням доступності.'"));
			
		ИначеЕсли ИспользоватьПланыПроизводства И ИспользоватьВРЦ Тогда
			
			МассивСтрок.Добавить(
				НСтр("ru='Виды рабочих центров используются для расчета потребностей по планам производства.';uk='Види робочих центрів використовуються для розрахунку потреб за планами виробництва.'"));
				
		ИначеЕсли ИспользоватьВРЦ Тогда
			
			МассивСтрок.Добавить(НСтр("ru='Виды рабочих центров задаются справочно.';uk='Види робочих центрів задаються довідково.'"));
		...
		Если (
			...
			МассивСтрок.Добавить(
				НСтр("ru='Длительность этапа используется для построения графика производства.';uk='Тривалість етапу використовується для побудови графіка виробництва.'"));
			
		ИначеЕсли ПланироватьПоПроизводственнымРесурсам И ИспользоватьВРЦ Тогда
			
			МассивСтрок.Добавить(
				НСтр("ru='Длительность этапа используется для определения сроков изготовления продукции без построения графика.';uk='Тривалість етапу використовується для визначення строків виготовлення продукції без побудови графіка.'"));
	...
	Если (ПланироватьПоПроизводственнымРесурсам ИЛИ ПланироватьПоМатериальнымРесурсам) И ПроизводствоНаСтороне Тогда
		...
		МассивСтрок.Добавить(
			НСтр("ru='Планирование производства будет происходить по графику работы предприятия.';uk='Планування виробництва буде відбуватися за графіком роботи підприємства.'"));
	...
	Если РассчитыватьДлительностьАвтоматически(Форма) Тогда
		...
		МассивСтрок.Добавить(
				НСтр("ru='Длительность рассчитывается автоматически по размеру буферов и времени работы видов рабочих центров.';uk='Тривалість розраховується автоматично за розміром буферів і часом роботи видів робочих центрів.'"));
