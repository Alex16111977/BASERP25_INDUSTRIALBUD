	...
	Если ПланироватьРаботуВидовРабочихЦентров Тогда
		...
		Для каждого Строка Из ВидыРабочихЦентров Цикл
			Если Не ПустаяСтрока(Строка.АлгоритмРасчетаКоличества) Тогда
				...
				ВидыРабочихЦентровСФормуламиОбновлены = Истина;
			КонецЕсли;
		КонецЦикла;
		...
		Если Настройки.ИспользуетсяПланированиеПоПроизводственнымРесурсам
			ИЛИ (Настройки.ИспользуетсяПроизводство21
				И НЕ Настройки.ИспользуетсяПланированиеПоМатериальнымРесурсам) Тогда
	...
	Если ПланироватьРаботуВидовРабочихЦентров Тогда
		...
		Если ВидыРабочихЦентров.Количество() > 1
			...
			РеквизитыДляПроверки.Добавить(
				...
				НСтр("ru='Порядок загрузки видов рабочих центров';uk='Порядок завантаження видів робочих центрів'"));
...
Процедура ПроверитьЗаполнениеДлительностиЭтапа(КлючДанных, СообщитьНазваниеЭтапа, Отказ)
	...
	Если ДлительностьЭтапаВСекундах < ДлительностьВРЦ Тогда
		...
		Если СообщитьНазваниеЭтапа И КлючДанных <> Неопределено Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Длительность этапа должна быть больше или равна времени работы видов рабочих центров (этап: %1).';uk='Тривалість етапу повинна бути більше або дорівнювати часу роботи видів робочих центрів (етап: %1).'"),
				КлючДанных.Наименование);
			
		Иначе
				
			ТекстСообщения = НСтр("ru='Длительность этапа должна быть больше или равна времени работы видов рабочих центров"".';uk='Тривалість етапу повинна бути більше або дорівнювати часу роботи видів робочих центрів"".'");
...
Процедура ПроверитьПараметрыОдновременнойРаботы(РеквизитыВидовРЦ, ЕстьСинхроннаяЗагрузка, КлючДанных, СообщитьНазваниеЭтапа, Отказ)
	...
	Если НЕ ВремяРаботыОдинаковое Тогда
		...
		Если СообщитьНазваниеЭтапа И КлючДанных <> Неопределено Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При использовании порядка загрузки ""Одновременно"" время работы видов рабочих центров должно быть одинаковым (этап: %1).';uk='При використанні порядку завантаження ""Одночасно"" час роботи видів робочих центрів має бути однаковим (етап: %1).'"),
				КлючДанных.Наименование);
		Иначе
