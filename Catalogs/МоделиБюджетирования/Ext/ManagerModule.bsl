...
//  	*НастройкиРасчетаПроцессов	- Соответствие - где:
...
Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Создает экземпляры регламентного задания для каждой модели бюджетирования при условии выполнения следующих условий: модель не помечена на удаление, статус = действует, установлен переключатель на ""Формировать задачи бюджетного процесса автоматически по расписанию"", включена функциональная опция ""Бюджетный процесс"". После создания экземпляров регламентного задания удаляет экземпляры регламентного задания без параметров (бывший предопределенный экземпляр)';uk='Створює екземпляри регламентного завдання для кожної моделі бюджетування за умови виконання наступних умов: модель не позначена для вилучення, статус = діє, встановлено перемикач ""Формувати задачі бюджетного процесу автоматично за розкладом"", включена функціональна опція ""Бюджетний процес"". Після створення екземплярів регламентного завдання видаляє екземпляри регламентного завдання без параметрів (колишній напередвизначений екземпляр)'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ВыборкаПоОбъектам.Следующий() Цикл
		...
		Попытка
			...
			Модель = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
				ВыборкаПоОбъектам.Ссылка,
