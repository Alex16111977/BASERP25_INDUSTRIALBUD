...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПланируетсяГрафикПроизводства И НЕ Объект.ИспользоватьПооперационноеПланирование Тогда
		...
		ПодсказкаДоп = НСтр("ru='Для использования пооперационного планирования необходимо включить использование графика производства (раздел ""НСИ и администрирование"" - ""Производство"").';uk='Для використання поопераційного планування необхідно включити використання графіка виробництва (розділ ""НДІ і адміністрування"" - ""Виробництво"").'");
...
//++ НЕ УТКА
	Если Объект.СпособНастройкиГрафикаРаботы = Перечисления.СпособыНастройкиГрафикаРаботыПодразделений.Смены
		...
		ТекстСообщения = НСтр("ru='Не введено ни одной строки в список ""Смены""';uk='Не введено жодного рядка в список ""Зміни""'");
	...
	Для каждого Строка Из Объект.ГрафикиРаботы Цикл
		...
		Если Строка.ГрафикРаботы.Пустая() Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru='Не заполнена колонка ""Смена"" в строке %1 списка ""Смены"".';uk='Не заповнена колонка ""Зміна"" в рядку %1 списку ""Зміни"".'"),
		...
		Иначе
			...
			Иначе
				...
				Если НЕ СоставСмен[Строка.ГрафикРаботы] Тогда
					...
					ТекстСообщения = СтрШаблон(НСтр("ru='Смена ""%1"" указана в списке ""Смены"" более одного раза.';uk='Зміна ""%1"" вказана в списку ""Зміни"" більш ніж один раз.'"),
						Строка.ГрафикРаботы);
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ГрафикиРаботы", Строка.НомерСтроки, "ГрафикРаботы");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, Поле, "Объект", Отказ);
