...
Процедура ЗаполнитьИспользуемыеМетодикиУправления() Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Не удалось записать используемые методики управления производством';uk='Не вдалося записати використовувані методики управління виробництвом'");
		
		СобытиеЖурналаРегистрации = НСтр("ru='Межцеховое управление.Ошибка заполнения методик управления производством';uk='Міжцехове управління. Помилка заповнення методик управління виробництвом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
