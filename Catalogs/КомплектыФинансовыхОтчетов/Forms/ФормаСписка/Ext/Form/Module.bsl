...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		Элементы.КнопкаСравнитьЭкземпляры.Заголовок = НСтр("ru='Сравнить экземпляры (Тонкий клиент)';uk='Порівняти примірники (Тонкий клієнт)'");
...
&НаКлиенте
Процедура СформироватьОтчеты(ТекущиеДанные)
	...
	ГенераторОтчета = Новый ОписаниеОповещения("СформироватьОтчетыЗавершение", ЭтаФорма, ТекущиеДанные.Ссылка);
...
Процедура СоздатьВФоновомЗаданииНаСервере(НаборОтчетов)
	...
	НаименованиеЗадания = НСтр("ru='Создание комплектов отчетов';uk='Створення комплектів звітів'");
...
Процедура СоздатьКомплектЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатСоздания.НаборОтчетов) = Тип("СправочникСсылка.ВидыФинансовыхОтчетов")
