...
Процедура ПередЗаписью(Отказ)
	...
	Если ДополнительныеСвойства.ОбновитьПодчиненияУзлов Тогда
		...
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	ПодчиненияУзловОбъектовЭксплуатацииСрезПоследних.УзелОбъекта,
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.ОбновитьПодчиненияУзлов Тогда
		...
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	УзлыОбъектовЭксплуатации.Ссылка КАК УзелОбъекта,
			...
			|		ПО Подчинение.УзелОбъекта = ПодчинениеПоДаннымРегистра.УзелОбъекта
			...
			|ГДЕ
			|	ПодчинениеПоДаннымРегистра.УзелОбъекта ЕСТЬ NULL ");
		Запрос.МенеджерВременныхТаблиц = ДополнительныеСвойства.МенеджерВременныхТаблиц;
		Запрос.УстановитьПараметр("ОбъектЭксплуатации", ЭтотОбъект.Ссылка);
		...
		Если Не Результат.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				НаборЗаписей.Отбор.УзелОбъекта.Установить(Выборка.УзелОбъекта);
