...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗначениеНеЗаполняется = НСтр("ru='Не заполняется';uk='Не заповнюється'");
		...
		Для Каждого СчетУчета Из СчетаУчета Цикл
			...
			Для Каждого Соответствие Из СоответствияСубконто Цикл
				Если Соответствие.ЗаполнятьИзИсточника И НЕ ПустаяСтрока(Соответствие.Выражение) И НЕ ПустаяСтрока(Соответствие.ПредставлениеВыражения)
					ИЛИ НЕ Соответствие.ЗаполнятьИзИсточника Тогда
...
Процедура ПроверитьИспользованиеВПравилахОтраженияВМеждународномУчете()
	...
	Если Выборка.Следующий() Тогда
		СостояниеИспользованияВУчетнойПолитике = СтрШаблон(" "+НСтр("ru='Используется';uk='Використовується'")+" (%1)", Выборка.УчетнаяПолитика);
...
Процедура УстановитьЗаголовкиКомандУточненияСчетов()
	...
	Если НЕ ДоступноИзменениеНастроекМФУ Тогда
		Для каждого СторонаСчета Из СтороныСчета Цикл
			...
			Команда.Заголовок =  СтрШаблон(НСтр("ru='Посмотреть уточнение счета %1';uk='Подивитися уточнення рахунку %1'"),ДтКт);
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовФормы(ИмяЭлементаФормы = Неопределено)
	...
	Для каждого СторонаСчета Из СтороныСчета Цикл
		...
		Если ОбновитьВсе ИЛИ ИмяЭлементаФормы = "Счет" + ?(ДтКт = "Дт", "Дебета", "Кредита") + "ПоУмолчанию" Тогда
...
Процедура УстановитьДоступностьИсточникаСуммыВВалюте(ДтКт)
	...
	СчетПоУмолчанию = "Счет" + ?(ДтКт = "Дт", "Дебета", "Кредита") + "ПоУмолчанию";
...
Процедура ОбновитьРеквизитыЗависящиеОтОперации()
	...
	ЗаполнитьСпискиВыбораПоИсточникуДанных("ИсточникКоличества", ИсточникДанных);
...
Процедура ЗаполнитьСпискиВыбораПоИсточникуДанных(ИмяЭлемента, ИсточникДанных, ПустоеЗначение = Неопределено)
	...
	ИначеЕсли ИмяЭлемента = "ИсточникКоличества" Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	Оформление = ЭлементУсловногоОформления.Оформление.Элементы.Найти("Текст");
	Оформление.Использование = Истина;
	Оформление.Значение = НСтр("ru='Из оперативного учета';uk='З оперативного обліку'");
...
Процедура ОформитьНастройкуЗаполненияСубконто(Форма, Знач ЗаполнениеПоСоответствию)
	...
	Если ЗаполнениеПоСоответствию Тогда
		Элементы.ИзменитьРежимНастройкиЗаполненияСубконто.Заголовок = НСтр("ru='Отключить расширенную настройку заполнения субконто';uk='Відключити розширену настройку заповнення субконто'");
		Элементы.ГруппаЗаполнениеСубконто.Заголовок = НСтр("ru='Заполнение субконто (Задано пользователем)';uk='Заповнення субконто (Задано користувачем)'");
	Иначе
		Элементы.ИзменитьРежимНастройкиЗаполненияСубконто.Заголовок = НСтр("ru='Изменить режим настройки заполнения субконто';uk='Змінити режим настройки заповнення субконто'");
		Элементы.ГруппаЗаполнениеСубконто.Заголовок = НСтр("ru='Заполнение субконто (Автоматически)';uk='Заповнення субконто (Автоматично)'");
	КонецЕсли;
...
Процедура ОбновитьЗаголовкиГруппИзмерений()
	...
	УстановитьЗаголовокГруппыИзмерений(НСтр("ru='Источники измерений Дт';uk='Джерела вимірювань Дт'"), "Дт");
	УстановитьЗаголовокГруппыИзмерений(НСтр("ru='Источники измерений Кт';uk='Джерела вимірювань Кт'"), "Кт");
...
Процедура УстановитьЗаголовокГруппыИзмерений(Заголовок, ДтКт)
	...
	Если Объект["ИсточникПодразделения"+ДтКт] <> ИсточникПодразделенияПоУмолчанию
		...
		Заголовок = Заголовок + " (" + НСтр("ru='Изменены';uk='Змінені'") + ")";
	КонецЕсли;
...
Процедура ОбновитьСписокСубконтоМеждународногоУчета(ПоложениеСубконто = Неопределено)
	...
	Шаблон = "%1 %2 " + НСтр("ru='и уточненные счета';uk='і уточнення рахунку'");
	ЗаполнитьСубконтоСчета(ВыборкаСубконтоДт, НСтр("ru='Дт';uk='Дт'"), Шаблон);
	ЗаполнитьСубконтоСчета(ВыборкаСубконтоКт, НСтр("ru='Кт';uk='Кт'"), Шаблон);
...
Процедура ЗаполнитьСубконтоСчета(ВыборкаСубконто, СторонаСчета, Шаблон)
	...
	Если ВыборкаСубконто.Количество() Тогда
		...
		Счет = ?(СторонаСчета = НСтр("ru='Дт';uk='Дт'"), Объект.СчетДебетаПоУмолчанию, Объект.СчетКредитаПоУмолчанию);
