...
//++ НЕ УТКА
	...
	Элемент.Наименование = НСтр("ru='Разбираемые изделия';uk='Розбирані вироби'",КодОсновногоЯзыка);
	Элемент.ТипЗатрат = Перечисления.ТипыЗатрат.Материальные;
	Элемент.РеквизитДопУпорядочивания = 0;
	Элемент.Идентификатор = НСтр("ru='РазбираемыеИзделия';uk='Розбирані вироби'",КодОсновногоЯзыка);
	#КонецОбласти

	#Область РемонтируемыеИзделия
	Элемент = Элементы.Добавить(); // СправочникОбъект.СтатьиКалькуляции -
	Элемент.ИмяПредопределенныхДанных = "РемонтируемыеИзделия";
	Элемент.Наименование = НСтр("ru='Ремонтируемые изделия';uk='Ремонтовані вироби'",КодОсновногоЯзыка);
	Элемент.ТипЗатрат = Перечисления.ТипыЗатрат.Материальные;
	Элемент.РеквизитДопУпорядочивания = 0;
	Элемент.Идентификатор = НСтр("ru='РемонтируемыеИзделия';uk='Ремонтовані вироби'",КодОсновногоЯзыка);
	#КонецОбласти
//-- НЕ УТКА
КонецПроцедуры
